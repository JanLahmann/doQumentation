---
title: "Contoh Primitives"
sidebar_label: "Contoh Primitives"
description: "Contoh praktis penggunaan primitives di Qiskit Runtime."
...
Rilis beta model eksekusi baru kini tersedia. Model eksekusi terarah memberikan fleksibilitas lebih saat mengkustomisasi alur kerja mitigasi error kamu. Lihat panduan [Model eksekusi terarah](/guides/directed-execution-model) untuk informasi lebih lanjut.
</Admonition>
{/*
...
Contoh-contoh di bagian ini mengilustrasikan beberapa cara umum menggunakan primitives. Sebelum menjalankan contoh-contoh ini, ikuti instruksi di [Instal dan atur.](install-qiskit)
...
## Contoh Estimator {#estimator-examples}
...
### Jalankan beberapa eksperimen dalam satu job {#run-multiple-experiments-in-a-single-job}
...
### Gunakan Session dan opsi lanjutan {#use-sessions-and-advanced-options}

Jelajahi Session dan opsi lanjutan untuk mengoptimalkan performa Circuit pada QPU.

<Admonition type="caution">
Blok kode berikut akan mengembalikan error bagi pengguna Open Plan karena menggunakan Session. Beban kerja Open Plan hanya dapat berjalan dalam [mode job](/guides/execution-modes#job-mode) atau [mode batch](/guides/execution-modes#batch-mode).
</Admonition>
...
## Contoh Sampler {#sampler-examples}
...
# Ambil hasil untuk PUB pertama (dan satu-satunya)
pub_result = result[0]
...
# Tentukan tiga set parameter untuk Circuit
rng = np.random.default_rng(1234)
parameter_values = [
...
# Ambil hasil untuk PUB pertama (dan satu-satunya)
pub_result = result[0]
# Ambil hitungan dari register klasik "meas".
print(
...
<Admonition type="caution">
Blok kode berikut akan mengembalikan error bagi pengguna paket Open Plan, karena menggunakan Session. Beban kerja di Open Plan hanya dapat berjalan dalam [mode job](/guides/execution-modes#job-mode) atau [mode batch](/guides/execution-modes#batch-mode).
</Admonition>
...
<Admonition type="tip" title="Rekomendasi">
    - [Tentukan opsi runtime lanjutan.](runtime-options-overview)
    ...
    - Pelajari cara men-transpile secara lokal di bagian [Transpile](./transpile/).
