...
## Dukungan Qiskit Runtime {#dukungan-qiskit-runtime}

Meskipun timeline drawer yang ada di Qiskit berguna untuk static circuit, ia mungkin tidak secara akurat mencerminkan timing [dynamic circuits](/guides/classical-feedforward-and-control-flow) karena adanya operasi implisit seperti broadcasting dan penentuan cabang. Sebagai bagian dari dukungan dynamic circuit, Qiskit Runtime mengembalikan informasi timing Circuit yang akurat di dalam hasil job bila diminta.

<Admonition type="note" title="Catatan">
- Ini adalah fungsi eksperimental. Fungsi ini masih dalam status preview release dan oleh karena itu dapat berubah sewaktu-waktu.
...
### Aktifkan pengambilan data timing {#aktifkan-pengambilan-data-timing}

Untuk mengaktifkan pengambilan data timing, atur flag eksperimental `scheduler_timing` ke `True` saat menjalankan job primitive.
...
### Akses data timing Circuit {#akses-data-timing-circuit}

Bila diminta, data timing Circuit untuk setiap PUB dikembalikan dalam metadata hasil job, di bawah `["compilation"]["scheduler_timing"]["timing"]`. Field ini berisi informasi timing mentah. Untuk menampilkan informasi timing, gunakan alat visualisasi bawaan, seperti yang dijelaskan di bagian [Visualisasikan timing](#visualize-timings).
...
#### Pahami data timing mentah {#pahami-data-timing-mentah}

Meskipun memvisualisasikan data timing Circuit menggunakan metode `draw_circuit_schedule_timing` adalah kasus penggunaan yang paling umum, mungkin berguna untuk memahami struktur data timing mentah yang dikembalikan. Ini bisa membantu kamu, misalnya, untuk mengekstrak informasi secara programatik.

Data timing yang dikembalikan dalam `["compilation"]["scheduler_timing"]["timing"]` adalah daftar string. Setiap string mewakili satu instruksi pada suatu channel dan dipisahkan dengan koma menjadi tipe data berikut:

- `Branch` - Menentukan apakah instruksi berada dalam alur kontrol (then / else) atau cabang utama.
- `Instruction` - Gate dan Qubit yang akan dioperasikan.
- `Channel` - Channel yang ditetapkan untuk instruksi tersebut. Bisa salah satu dari berikut:
      - `Qubit x` - Drive channel untuk Qubit _x_.
      - `AWGRx_y` (arbitrary waveform generator readout) - Digunakan oleh readout channel untuk berkomunikasi saat mengukur Qubit. Argumen _x_ dan _y_ masing-masing sesuai dengan ID instrumen readout dan nomor Qubit.
- `T0` - Waktu mulai instruksi dalam jadwal lengkap
- `Duration` - Durasi instruksi, dalam satuan _dt_ detik, di mana 1 dt = 1 siklus penjadwalan. Kamu bisa menemukan nilai `dt` dari sebuah Backend menggunakan [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt).
...
![Arahkan kursor ke output untuk melihat informasi seperti waktu mulai, selesai, dan durasi.](/docs/images/guides/visualize-circuit-timing/image_1.svg 'Contoh gambar yang dihasilkan')
#### Pahami gambar yang dihasilkan {#pahami-gambar-yang-dihasilkan}

Gambar output data timing Circuit yang dihasilkan oleh `draw_circuit_schedule_timing` menyampaikan informasi berikut:

- Sumbu X adalah waktu dalam satuan _dt_ detik, di mana 1 dt = 1 siklus penjadwalan. Kamu bisa menemukan nilai `dt` dari sebuah Backend menggunakan [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt).
- Sumbu Y adalah channel (bayangkan channel sebagai instrumen yang memancarkan pulsa).
    - `Receive channel` - Satu-satunya channel yang bukan instrumen itu sendiri. Ini adalah instruksi yang dimainkan pada semua channel yang merupakan bagian dari prosedur komunikasi dengan hub pada saat itu.
    - `Qubit x` - Drive channel untuk Qubit x.
    - `AWGRx_y` (arbitrary waveform generator readout) - Digunakan oleh readout channel untuk berkomunikasi saat mengukur Qubit. Argumen _x_ dan _y_ masing-masing sesuai dengan ID instrumen readout dan nomor Qubit.
    - `Hub` - Mengontrol broadcasting.

Selain itu, setiap instruksi memiliki format *X_Y*, di mana *X* adalah nama instruksi dan *Y* adalah tipe pulse. Tipe `play` menerapkan pulsa kontrol, dan `capture` merekam kondisi Qubit. Kamu juga bisa mengarahkan kursor ke setiap instruksi untuk mendapatkan detail lebih lanjut. Misalnya, gambar sebelumnya menunjukkan pulsa kontrol untuk Gate X yang diterapkan pada Qubit 10 pada 1161 dt.
...
Pertama, siapkan lingkungan, definisikan circuit dan konversikan ke ISA circuit, serta definisikan dan jalankan job.
...
<Admonition type="tip" title="Rekomendasi">
  - [Classical feedforward dan control flow](/guides/classical-feedforward-and-control-flow) (dynamic circuits)
