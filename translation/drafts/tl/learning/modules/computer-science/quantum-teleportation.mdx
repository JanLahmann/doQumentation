...
```python
# Idinagdag ng doQumentation — mga kinakailangang package para sa notebook na ito
!pip install -q numpy qiskit qiskit-aer qiskit-ibm-runtime
```

```python
# I-uncomment at baguhin ang linya na ito kung kinakailangan upang i-install ang mga dependency
...
# Mag-import ng ilang pangkalahatang package
from qiskit import ClassicalRegister, QuantumCircuit, QuantumRegister
import numpy as np

# Tukuyin ang mga register
...
# I-entangle ang mga qubit ni Alice at Bob tulad ng ginawa natin sa itaas. Mag-apply ng Hadamard gate at pagkatapos ay isang CNOT gate.
# Tandaan na ang pangalawang argumento sa CNOT gate ang target.
qc.h(Alice)
qc.cx(Alice, Bob)

# Ang paglalagay ng barrier ay walang epekto sa lohika. Pinapayagan lang nito tayong pilitin ang mga gate na iposisyon sa "mga layer".
qc.barrier()

# Ngayon gagamit tayo ng mga random variable upang lumikha ng secret state. Huwag mag-alala tungkol sa "u" gate at sa mga detalye.
np.random.seed(42)  # fixing seed for repeatability
theta = np.random.uniform(0.0, 1.0) * np.pi  # from 0 to pi
varphi = np.random.uniform(0.0, 2.0) * np.pi  # from 0 to 2*pi

# Italaga ang secret state sa qubit sa kabilang panig ni Alice (qubit 0), na may label na Q
qc.u(theta, varphi, 0.0, secret)
qc.barrier()

# I-entangle ang Q at ang mga qubit ni Alice tulad ng tinalakay sa itaas.
qc.cx(secret, Alice)
qc.h(secret)
qc.barrier()

# Ngayon susukat si Alice sa kanyang mga qubit, at itatago ang mga resulta sa "classical registers" na cr[]
qc.measure(Alice, cr[1])
qc.measure(secret, cr[0])

# Ngayon maglalagay tayo ng conditional na lohika. Kung sinusukat ni Alice ang Q sa "1" kailangan natin ng Z gate, at kung sinusukat ni Alice ang A sa "1" kailangan natin ng X gate (tingnan ang talahanayan).
...
```python
# Idagdag ang inverse ng U at sukatin ang qubit ni Bob.
qc.barrier()
...
```python
# I-load ang Qiskit Runtime service
from qiskit_ibm_runtime import QiskitRuntimeService

# I-load ang Qiskit Runtime service

# Syntax para sa unang pag-save ng iyong token. Tanggalin ang mga linyang ito pagkatapos i-save ang iyong mga kredensyal.
# QiskitRuntimeService.save_account(channel='ibm_quantum_platform', instance = '<YOUR_IBM_INSTANCE_CRN>', token='<YOUR-API_KEY>', overwrite=True, set_as_default=True)
# service = QiskitRuntimeService(channel='ibm_quantum_platform')

# I-load ang mga naka-save na kredensyal
service = QiskitRuntimeService()

# Gamitin ang pinaka-hindi abalang backend, o i-uncomment ang pag-load ng isang partikular na backend tulad ng "ibm_brisbane".
...
```python
# I-load ang Runtime primitive at session
...
```python
# I-load ang backend sampler
from qiskit.primitives import BackendSamplerV2

# I-load ang Aer simulator at bumuo ng noise model batay sa kasalukuyang piniling backend.
from qiskit_aer import AerSimulator
from qiskit_aer.noise import NoiseModel

noise_model = NoiseModel.from_backend(backend)

# Tukuyin ang isang simulator gamit ang Aer, at gamitin ito sa Sampler.
backend_sim = AerSimulator(noise_model=noise_model)
sampler_sim = BackendSamplerV2(backend=backend_sim)

# Bilang kahalili, mag-load ng fake backend na may generic na katangian at tukuyin ang isang simulator.
...
```python
# Tumagal ito ng 5 s para patakbuhin sa isang Heron r2 processor noong 10-28-24
...
# Magsisimula kami kay Alice sa gitna ng circuit, pagkatapos ay ilililipat ang impormasyon palabas sa magkabilang direksyon.
Alice = 5
Bob = 0
qc.h(ebitsa[Alice])
qc.cx(ebitsa[Alice], ebitsb[Bob])

# Simula kina Bob at Alice sa gitna, i-swap natin ang kanilang impormasyon sa mga katabing qubit, hanggang ang impormasyon ay nasa malalayong qubit.
...
# Lumikha ng random na estado para kay Alice (qubit zero)
np.random.seed(42)  # fixing seed for repeatability
# theta = np.random.uniform(0.0, 1.0) * np.pi    #from 0 to pi
theta = 0.3
varphi = np.random.uniform(0.0, 2.0) * np.pi  # from 0 to 2*pi
...
# I-entangle ang dalawang qubit ni Alice
qc.cx(secret, ebitsa[Alice])
qc.h(secret)

qc.barrier()

# Gumawa ng mga pagsukat ng mga qubit ni Alice at istore ang mga resulta sa classical register.
qc.measure(ebitsa[Alice], cr[1])
qc.measure(secret, cr[0])

# Magpadala ng mga instruksyon sa mga qubit ni Bob batay sa resulta ng mga pagsukat ni Alice.
with qc.if_test((cr[1], 1)):
    qc.x(ebitsb[Bob])
with qc.if_test((cr[0], 1)):
    qc.z(ebitsb[Bob])
...
# I-invert ang paghahanda na ginawa natin para sa qubit ni Carl upang masuri kung tama ang ating ginawa.
