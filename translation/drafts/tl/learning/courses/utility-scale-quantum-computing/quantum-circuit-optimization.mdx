...
---
{/* doqumentation-source-hash: b66549cb */}
...
```python
# patakbuhin ang mga circuit sa fake backend (noisy simulator)
...
```python
# Orihinal na GHZ circuit (simpleng synthesis)
ghz_circ.draw("mpl")
```
...
```python
# Isang matalinong na-synthesize na GHZ circuit
...
```python
# i-transpile ang dalawa gamit ang parehong optimization level 2
...
```python
# Bumuo ng isang toy example circuit
from math import pi
import itertools
from qiskit.circuit import QuantumCircuit
...
# Hindi na kailangang basahin ang buong larawang ito, ngunit inilalabas nito ang lahat ng hakbang sa tawag ng transpile()
# para sa optimization level 1
