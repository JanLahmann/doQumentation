...
# การ import ทั่วไปและฟังก์ชันช่วยเหลือ
...
def visualize_counts(res_counts, num_qubits, num_shots):
    """แสดงผลลัพธ์จาก Qiskit Sampler primitive"""
...
# เมทริกซ์ kernel ว่าง
num_samples = np.shape(X_train)[0]
kernel_matrix = np.full((num_samples, num_samples), np.nan)

# เตรียม feature map สำหรับคำนวณ overlap
...
# กำหนด tunable parameter ให้เป็นค่าที่ optimal และตั้งค่า data params สำหรับตัวอย่างสองตัวแรก
x1 = 14
x2 = 19
unitary1 = fm.assign_parameters(list(X_train[x1]) + [np.pi / 2])
unitary2 = fm.assign_parameters(list(X_train[x2]) + [np.pi / 2])

# สร้าง overlap circuit
...
## ประเมินปัญหาโดยใช้ statevector-based primitives จาก Qiskit
...
# ประเมินปัญหาโดยใช้ QPU ผ่าน Qiskit IBM Runtime
...
```python
# คำนวณ fidelity หรือความน่าจะเป็นในการวัดค่า 0
