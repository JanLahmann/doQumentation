...
##  Context
Acest tutorial demonstrează cum să atenuezi erorile folosind addon-ul Shaded lightcone (SLC). Acest addon reprezintă o evoluție a [tehnicii de anulare probabilistică a erorilor (PEC)](https://quantum.cloud.ibm.com/docs/guides/error-mitigation-and-suppression-techniques#probabilistic-error-cancellation-pec), prin care un utilizator învață zgomotul straturilor unice dintr-un Circuit și apoi anulează zgomotul aplicând Gate-uri cu un singur Qubit și tehnici de post-procesare. Față de alte metode, PEC oferă limite mai robuste asupra devierii rezultatului atenuat, dar tinde să aibă un overhead mai ridicat în ceea ce privește timpul pe QPU. În timpul PEC, pentru a compensa atenuarea valorii așteptate cauzată de zgomot, rezultatul mediu este rescalat cu un factor $\gamma = \exp(\sum_{l,\sigma} 2\lambda_{l,\sigma})$, unde $\lambda_{l,\sigma}$ este rata de zgomot învățată a erorii Pauli $\sigma$ la stratul $l$ din Circuit. Această rescalare mărește varianța cu un factor $\gamma^2$ și, astfel, înmulțește și numărul de execuții ale Circuit-ului necesare pe QPU cu $\gamma^2$, ceea ce numim costul de eșantionare sau overhead-ul de eșantionare. Deoarece $\gamma$ crește exponențial, PEC este adesea limitat la circuit-uri superficiale sau cu puțini Qubiți. Află mai multe despre PEC în [Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors.](https://arxiv.org/abs/2201.09866)

Dacă putem identifica erori care nu trebuie atenuate, putem reduce exponențial acest cost de eșantionare. Un prim pas în această direcție este implementarea atenuării erorilor cu conștientizare locală, care folosește un „lightcone" convențional, calculabil rapid, pentru a reduce overhead-ul PEC prin limitarea sensibilității unui observable la erori din Circuit, extinzând fezabilitatea PEC la scări mai mari pentru unele probleme. Erorile din afara acestui lightcone nu pot afecta rezultatul măsurat și, prin urmare, pot fi excluse din procedura de anulare a erorilor. Această excludere reduce overhead-ul de eșantionare, în unele cazuri substanțial, fără a introduce deviație suplimentară. În particular, pentru măsurarea unui observable local $O$ al unui Circuit cu adâncime fixă, overhead-ul de eșantionare necesar ajunge în cele din urmă la un platou atunci când se scalează numărul de Qubiți din Circuit (vezi Fig. 2b în [Locality and Error Mitigation of Quantum Circuits.](https://arxiv.org/abs/2303.06496))

Conurile de lumină umbrite (SLC) merg mai departe, folosind simulări clasice pentru a limita mai strict sensibilitatea la erori în tot Circuit-ul. Aceasta schimbă o parte din timpul QPU cu timp CPU și reduce overhead-ul de eșantionare necesar pentru renormalizarea devierii. În loc de o limită dură, fiecărei erori potențiale din Circuit îi este atribuită o „umbră" gradată care reprezintă o limită superioară a susceptibilității observable-ului față de acea eroare. Această caracterizare rafinată permite aplicații mai eficiente și țintite ale PEC cu varianță redusă, oferindu-i totodată utilizatorului posibilitatea de a ajusta controlabil devierea în estimarea observable-ului. Vezi [Lightcone shading for classically accelerated quantum error mitigation](https://arxiv.org/abs/2409.04401) pentru mai multe detalii.

Fluxul nostru de lucru pentru addon-ul SLC valorifică noul framework Samplomatic și Executor, permițând utilizatorilor să aibă un control mai modular al setărilor de execuție pentru suprimarea și atenuarea erorilor, păstrând totodată ușurința de utilizare pentru utilizatorii avansați. Pentru o înțelegere mai profundă a beneficiilor acestui framework și a caracteristicilor sale generale, consultă tutorialul [Hello samplomatic](https://github.com/qiskit-community/qdc-challenges-2025/blob/main/day3_tutorials/Track_A/hello_samplomatic/Samplomatic%20-%20Hello%20World.ipynb).

### Fluxul de lucru pentru lightcone shading, noise learning și injecția anti-zgomot
Pentru modelarea zgomotului QPU-ului, am ales să folosim un model de zgomot sparse Pauli-Lindblad cu rate de eroare Pauli pentru 1 și 2 Qubiți, generate local pe fiecare Qubit și pe fiecare muchie a dispozitivului. Cu această alegere, fluxul de lucru de atenuare a erorilor SLC prezentat în acest tutorial este următorul:

a. CPU — Limitează impactul per-eroare al erorilor Pauli pentru 1 și 2 Qubiți

  1. Propagare înainte (limitează efectul asupra observable-ului). Propagă fiecare eroare până la sfârșitul Circuit-ului și calculează comutatorul său cu observable-ul.  
      - Trunchiază termenii operatorilor în timpul evoluției pentru a menține calculul fezabil.  
      - Strânge și mai mult aceste limite printr-o propagare înapoi lejeră a observable-ului bazată pe limitele vitezei cuantice.
  2. Propagare înapoi (limitează efectul asupra stării inițiale). Propagă fiecare eroare până la începutul Circuit-ului și calculează comutatorul său cu starea inițială.
...
c. CPU — Prioritizează atenuarea
  ...
  1. Actualizează limitele îmbinate cu ratele de zgomot învățate. Combină limitele înainte și înapoi care au fost calculate anterior și actualizează-le cu ratele de zgomot învățate.  
  2. Clasifică componentele de zgomot de atenuat folosind limitele calculate și ratele învățate. Prioritizează fiecare posibilă eroare de zgomot pe baza impactului estimat asupra devierii și a costului asociat pentru corectare.

d. QPU — Inserează anti-zgomotul și rulează. Execută Circuit-ul de interes cu anti-zgomot (zgomot invers) specificat prin adnotări `Box`.

e. CPU — Estimează observable-ul. Calculează valoarea așteptată, aplicând post-selecție bazată pe măsurători pentru a reduce impactul zgomotului non-Markovian.

### Prezentare generală a noise learning
Noise learning este un pas comun în mai multe metode de atenuare a erorilor, realizat de [NoiseLearner](https://quantum.cloud.ibm.com/docs/en/guides/noise-learning), și poate fi observat în tutorialul nostru de [atenuare a erorilor PEA](https://quantum.cloud.ibm.com/docs/tutorials/probabilistic-error-amplification), precum și în [tutorialul Propagated noise absorption (PNA)](https://github.com/qiskit-community/qdc-challenges-2025/blob/main/day3_tutorials/Track_A/pna/propagated_noise_absorption.ipynb). În `NoiseLearnerV3`, un utilizator poate identifica în mod specific straturile de zgomot care urmează să fie învățate ca obiecte [`CircuitInstruction`](https://quantum.cloud.ibm.com/docs/api/qiskit/qiskit.circuit.CircuitInstruction), ceea ce le permite utilizatorilor să calculeze limitele SLC de zgomot dorite pentru fiecare strat în modul descris mai sus. Modelul Pauli-Lindblad învățat furnizează coeficienți care vor fi folosiți în prioritizarea PEC-SLC. Modul în care Gate-urile sunt grupate în straturi poate fi determinat folosind funcțiile de conveniență `generate_boxing_pass_manager` și `unique_2q_instructions`, apoi introduse în funcția utilitară SLC `generate_noise_model_paulis`, după cum este descris în Pasul 2 de mai jos.

| **Partea 1** | **Partea 2** | **Partea 3** |
|-----------|-----------|-----------|
| Pauli-twirl straturi de Gate-uri cu doi Qubiți | Repetă perechi identice de straturi și învață zgomotul | Derivă o fidelitate (eroare pentru fiecare canal de zgomot) |
...
După executarea pe hardware cuantic folosind framework-ul Samplomatic și Executor, convertim măsurătorile noastre de tip bitstring în valoarea dorită a observable-ului. În cazul Circuit-ului nostru Ising în oglindă, vom obține în mod ideal un observable măsurat de 1, deoarece toți Qubiții ar trebui să revină în mod ideal la punctul lor de start $\ket{0}$. La calcularea valorii observable-ului cu funcția noastră `expectation_values`, vom aplica câteva tehnici de post-procesare care reduc impactul zgomotului. Acestea includ eliminarea shot-urilor afectate de zgomot non-Markovian, atenuarea erorilor de citire, precum și contabilizarea detaliilor implementării noastre PEC. Detaliile sunt discutate în Pasul 4 de mai jos.
...
- Samplomatic v0.16 sau mai nou(`pip install samplomatic`)
- Suport Qiskit Visualization (`pip install "qiskit[visualization]"`)
## Pasul 0. Configurare
...
Pentru ușurința demonstrației, selectăm un lanț Ising mirror 1D. Lanțul Ising 1D oferă o structură de Circuit densă, care este convenabilă pentru prezentarea implementărilor PEC. Un circuit mirror face simplu de știut rezultatul așteptat (adică, ar trebui să măsurăm un observable de 1).

În plus, vrem să rulăm un circuit mirror, deci pentru fiecare Gate din a doua jumătate a circuitului, trebuie să existe un Gate invers în prima jumătate. Deoarece observabilul măsurat **$<X_6 Z_{13}>$** conține măsurători în baze non-Z, iar executorul ține cont de baza dorită la sfârșitul circuitului, furnizăm o funcție `prepare_basis` care inserează Gate-urile corespunzătoare la începutul circuitului mirror. Acest detaliu este specific demonstrației noastre cu circuit mirror. Funcția `get_measurement_bases` ne permite să identificăm ușor ce Gate-uri sunt necesare și unde să le adăugăm, precum și să urmărim subtilitățile de index ale Qubit-urilor care apar din convențiile de adnotare `box`, discutate în secțiunea „Prepare canonical bases measurements".
...
## Pasul 2. Optimizare
Vom optimiza detaliile asociate cu Circuit-ul care urmează să fie rulat, cu observable-ul care urmează să fie măsurat și cu parametrii de învățare a zgomotului. Ca punct de plecare, ne asigurăm că instanțiem un Backend cu gate-uri fracționale activate ca opțiune. Aceste gate-uri fracționale vor permite o sensibilitate mai mare în unele dintre filtrările noastre de post-selecție.
...
Mai întâi, vom transpila Circuit-ul nostru în instrucțiuni ISA, [după cum este necesar pentru execuția pe QPU-urile noastre](https://www.ibm.com/quantum/blog/isa-circuits). Pentru datele colectate în acest experiment, selectăm manual Qubit-urile bazându-ne pe evaluarea lanțului de cea mai înaltă calitate.
...
### Împachetează Circuit-ul în cutii (`box`)
Pentru ușurința implementării, vom utiliza pasul de transpilare `generate_boxing_pass_manager`, care plasează instrucțiunile Circuit-ului în cutii (box-uri) adnotate. Aceste box-uri indică clar unde, în cazul PEC, trebuie injectat antizgomotul în Circuit. Pentru detalii despre setări, consultați [documentația Samplomatic.](https://qiskit.github.io/samplomatic/)

Reține că fluxul de lucru SLC folosește `inject_noise_strategy="individual_modification"` mai târziu în proces, deoarece aceasta ne permite să identificăm în mod unic fiecare `BoxOp` din Circuit.

Funcția `find_unique_box_instructions` iterează prin Circuit-ul împachetat furnizat și identifică acele instrucțiuni care au straturi 2Q unice sau măsurători, în scopul învățării zgomotului și al injectării zgomotului.
...
### Pregătește măsurătorile bazelor canonice
Datorită modului în care qubiții sunt etichetați atunci când se identifică straturi 2Q unice, trebuie acordată o atenție deosebită urmăririi ordinii qubiților. Mai jos, introducem noțiunea de `canonical_qubits` ca mijloc de actualizare corespunzătoare a ordinii qubiților atunci când o furnizăm executorului, ca urmare a modului în care ordinea qubiților este capturată la boxarea circuitelor și găsirea instrucțiunilor unice. Consulta documentația [Qubit ordering convention](https://qiskit.github.io/samplomatic/guides/samplex_io.html#qubit-ordering-convention) pentru detalii.
...
### Fluxul de lucru pentru umbrirea lightcone, învățarea zgomotului și injecția anti-zgomot

> **Notă**: Pentru implementarea SLC-PEC în acest tutorial, rulăm calculele limitelor SLC **înainte** de finalizarea învățării zgomotului, astfel încât circuitul care urmează să fie atenuat este rulat cât mai aproape în timp de modelul de zgomot învățat. În principiu, acest flux de lucru poate fi îmbunătățit suplimentar pentru a fi executat simultan. Mai precis, o sarcină de învățare a zgomotului este rulată în timp ce, în paralel, se estimează limitele de zgomot. Pentru un Circuit cuantic arbitrar, calculul limitelor de zgomot poate scala cu o dependență slab exponențială. Ca atare, ar putea fi prudent să folosești execuția paralelizată atunci când încerci să maximizezi eficiența fluxului de lucru. În acest scop, demonstrăm pe scurt acest lucru incluzând resurse bazate pe cluster (128 de fire) și arătând cum poți obține un set mai rafinat de limite pentru un Circuit furnizat atunci când ești constrâns la limite egale de timp de calcul, comparativ cu laptop-ul nostru (8 fire). Mai mult, deși nu este implementat în acest flux de lucru, poți paraleliza execuțiile QPU pentru învățarea zgomotului și calculele limitelor de zgomot pentru a obține cel mai eficient flux de lucru.

#### Prezice operatorii Pauli ai modelului de zgomot care urmează să fie învățați

Funcția `generate_noise_model_paulis` parcurge fiecare strat boxat al circuitului furnizat și generează toți termenii Pauli relevanți de greutate unu și greutate doi, ținând cont de conectivitatea qubiților circuitului și selectând termenii relevanți pentru nodurile și muchiile active. Acești termeni sunt apoi folosiți pentru a calcula limitele de zgomot directe și inverse.
...
##### a. Calculează și strânge limitele directe

Funcția `compute_forward_bounds` evaluează relațiile de comutare dintre Gate-urile din fiecare strat și termenii Pauli generați mai sus, în ceea ce privește modul în care erorile de propagare directă afectează observable-ul dorit $A$. Pentru Gate-urile care comută cu termenii Pauli, nu se întreprinde nimic. Pentru Gate-urile Clifford, acestea sunt împinse spre începutul circuitului. Pentru Gate-urile non-Clifford, aproximăm influența lor asupra observable-lor țintă pentru a fi prioritizate ulterior la anularea zgomotului (după ce toate limitele au fost combinate). Această limită este obținută prin aplicarea mai întâi a normei L2 (adică rădăcina pătrată a sumei pătratelor coeficienților termenilor Pauli relevanți). Când sunt implicați prea mulți termeni de Qubit, revenim la o limită mai laxă care folosește inegalitatea triunghiului.
#### Resurse la nivel de laptop
...
Poți interpreta comportamentul limitelor umbrite examinând modul în care măsurătorile și termenii Pauli interacționează cu erorile locale. Aceste tipare sunt caracteristice acestei probleme de evoluție temporală a Hamiltonianului Ising cu impulsuri și apar și în lucrarea [Lightcone Shading for Classically Accelerated Quantum Error Mitigation](https://arxiv.org/abs/2409.04401), cu câteva caracteristici revelatoare:

- Putem distinge clar cele două conuri care provin din cei doi operatori Pauli non-identitate din observable.
- Putem vedea că măsurătoarea X pe Qubit-ul 6 comută cu eroarea X din stratul cel mai din dreapta.
- Putem vedea că operatorul Pauli Z de pe Qubit-ul 13 comută cu eroarea Z din stratul cel mai din dreapta.
- Când atingem timeout-ul specificat mai sus, straturile rămase din stânga sunt umplute în întregime cu limite triviale de două.
...
#### b. Calculează și strânge limitele directe
Strângem în continuare limitele folosind funcția `tighten_with_speed_limit`, care urmărește modul în care observable-ul se propagă înapoi prin Circuit și utilizează această propagare pentru a stabili limite superioare asupra efectului fiecărui operator de zgomot, alegând minimul dintre limita directă tocmai calculată și limita obținută prin propagarea inversă.
...
#### Vizualizează SLC-ul pentru inspecția manuală

Putem strânge și mai mult limitele ținând cont de restricția impusă de lightcone. În principiu, aceasta ne oferă o tranziție mai lină de la limitele calculate la limitele triviale stabilite după atingerea timeout-ului. Aici, efectul nu este la fel de vizibil, deoarece lightcone-urile au ajuns deja la marginea Circuit-ului.
...
#### c. Calculează limitele inverse

Această parte a predicției zgomotului evaluează modul în care o eroare dintr-un anumit strat poate afecta starea de intrare $\rho$. Funcția `compute_backward_bounds` inversează mai întâi Circuit-ul, elimină Gate-urile de măsurare și procedează apoi cu o analiză similară celei efectuate pentru calculul limitelor directe.
...
Din calcularea limitelor înapoi, putem vedea cum structura stării inițiale guvernează comportamentul timpuriu al propagării erorilor:

- Putem vedea clar cum erorile Z comută inițial cu starea inițială |0⟩.
- Doar pe qubit-ul 6, unde inițializăm starea proprie +1 a bazei X, o eroare Z nu reușește să comute, în timp ce o eroare X comută.
...
#### Previzualizează limitele combinate fără rate de zgomot învățate

Funcția `merged_bounds` determină punctul din circuit în care trecerea de la limitele înapoi la limitele înainte minimizează prejudecata totală estimată asupra observable-ului dorit. Această prejudecată este calculată ca suma contribuțiilor limitelor înapoi pentru toate locațiile de zgomot înainte de acel punct, plus contribuțiile limitelor înainte pentru toate locațiile de zgomot după acesta. În prezent, acest lucru se face uniform pentru toți qubiții.

> **Notă importantă**: Punctul de trecere de la limitele înainte la limitele înapoi depinde de ratele de zgomot învățate.
...
După combinarea limitelor înapoi și a limitelor înainte îmbunătățite, comportamentul SLC-urilor combinate devine clar:

- Funcția de mai sus ne spune că se alege o partiție la care are loc trecerea de la limitele înapoi la limitele înainte îmbunătățite.
- Putem vedea mai jos că SLC-urile conțin acum limite parțiale înapoi și limite parțiale înainte îmbunătățite.
...
#### Resurse la nivel de cluster
Aici demonstrăm cum utilizarea a 128 de fire de execuție pe un cluster ne permite să propagăm printr-o porțiune mai substanțială din acest circuit mai mare, când suntem limitați la același timp de calcul ca pe laptop-ul nostru.
...
1. Învată zgomotul folosind `NoiseLeanerV3`.
2. Execută un circuit de atenuare a erorilor cu noul framework Samplomatic și Estimator. 

Cu erorile delimitate din circuitul nostru cuantic, trebuie să învățăm ratele de zgomot asociate pentru a prioritiza bugetul de erori, a determina costul de eșantionare și a executa pe un QPU. În plus, cu aceste informații despre ratele de zgomot, putem evidenția și cum, prin utilizarea resurselor de calcul puternice din cluster-ul nostru, reducem costul de eșantionare menținând în același timp același bias rezidual.
### a. Învață ratele de zgomot

NoiseLearner permite caracterizarea proceselor de zgomot care afectează Gate-urile din unul sau mai multe circuite de interes, bazându-se pe modelul de zgomot Pauli-Lindblad descris în lucrarea [Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors](https://arxiv.org/abs/2201.09866). Metoda `run()` lansează un job de învățare a zgomotului pentru straturile unice de 2-qubiți furnizate, pe baza opțiunilor specificate în configurația noise learner-ului. În aceste opțiuni, poți ajusta strategia de twirl Pauli, care ajută la asigurarea că hardware-ul este bine descris de modelul de zgomot Pauli-Lindblad.

Detaliile modelului tău de zgomot riscă să se modifice în timp. Prin urmare, setăm un parametru pentru a asigura că modelul de zgomot învățat este recalculat pentru experimente mai vechi de patru ore. Aceasta este o regulă aproximativă și ar trebui luată în considerare cu atenție atunci când o aplici propriei tale lucrări.
...
### b.i. Actualizează limitele combinate cu ratele de zgomot învățate efectiv

Acum că modelul specific de zgomot a fost învățat, putem aplica ratele de zgomot învățate la limitele de zgomot prezise și obține o determinare finală a limitelor care au cel mai mare impact asupra minimizării biasului.
...
#### b.ii. Calculează `local_scales` pentru execuția pe hardware

`compute_local_scales` examinează fiecare eroare de zgomot posibilă din circuit și estimează cât de mult ar putea să biaseze acea eroare măsurătoarea finală, precum și cât de costisitor ar fi să o corectezi. Apoi clasifică erorile în funcție de cât de oportun este să le atenuezi și selectează subsetul care reduce biasul cât mai mult posibil, rămânând în limitele bugetului de cost de eșantionare permis (sau atingând o precizie dorită). Rezultatul este un set de factori de scalare care indică ce erori vor fi atenuate activ și care vor rămâne neatenuate (`local_scales`), împreună cu costul total de eșantionare estimat (`sampling_costs`) și biasul rezidual rămas (`residual_bias_bound`).

Capacitatea de a controla biasul rezidual dorit este o caracteristică critică a implementării SLC a PEC. Față de [implementarea originală](https://arxiv.org/abs/2201.09866), în care costul de eșantionare viza întotdeauna bias zero, putem regla costul de eșantionare necesar cu un compromis în biasul rezidual așteptat. Aceasta ajută utilizatorul să rămână în limitele unui buget de eșantionare fix, ceea ce poate fi deosebit de util atunci când se prototipează inițial un flux de lucru.
...
```
#### Avantajele clusterelor pentru reducerea overhead-ului de eșantionare pentru un timp clasic de calcul dat
...
Pe măsură ce calculăm valoarea de așteptare finală de interes folosind `expectation_values`, vom implementa câteva tehnici de post-procesare utile pentru a ne asigura că obținem cele mai bune rezultate posibile. Mai întâi, aplicăm [mitigarea erorilor de citire prin twirling, TREX](https://quantum.cloud.ibm.com/docs/guides/error-mitigation-and-suppression-techniques#twirled-readout-error-extinction-trex), care ține cont de erorile apărute în timpul procesului de citire. Apoi, corectăm erorile cauzate de zgomotul non-Markovian pe backend-urile noastre Heron printr-o metodă de post-selecție. Această metodă măsoară Qubit-urile active și spectator, aplică o rotație lentă fiecărui Qubit, după care măsoară din nou. În cazurile în care cele două măsurători nu confirmă un Qubit răsturnat conform așteptărilor, acele cadre sunt eliminate prin aplicarea unui `mask` din funcția `PostSelector`. În cadrul calculului măștii, se poate seta o strategie specifică pentru filtrarea bazată pe noduri de Qubit unic sau pe muchii spectator vecine, ceea ce poate influența atât numărul de cadre filtrate, cât și calitatea rezultatelor.
...
Examinând rezultatele experimentale, putem compara direct comportamentul diferitelor abordări: PEC, PEC combinat cu SLC și rezultatele de referință nemitigate. Câteva detalii specifice de evidențiat:

- Rezultatele nemitigate rămân în afara intervalului de eroare dorit și nu sunt afectate de costul de eșantionare.
- Având în vedere costul ridicat de eșantionare calculat mai sus (~10k), PEC singur nu converge în limita randomizărilor utilizate.
- PEC + SLC, în schimb, converge mult mai rapid.
- Limitele de eroare scad și ele semnificativ mai repede pentru PEC + SLC față de PEC simplu.
