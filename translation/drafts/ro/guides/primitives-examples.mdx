...
## Exemple cu Estimator {#estimator-examples}
...
<span id="sampler-examples"></span>
## Exemple cu Sampler {#sampler-examples}

Generează distribuții quasi-probabilistice complete, atenuate față de erori, eșantionate din ieșirile circuitelor cuantice. Valorifică capacitățile Sampler pentru algoritmi de căutare și clasificare precum Grover și QVSM.

### Rulează un singur experiment {#run-a-single-experiment}

Folosește Sampler pentru a returna rezultatul măsurătorii sub formă de șiruri de biți sau numărători pentru un singur Circuit.
...
# Obține rezultatele pentru primul (și singurul) PUB
pub_result = result[0]

print(f" > Primele zece rezultate: {pub_result.data.meas.get_bitstrings()[:10]}")
```
...
Folosește Sampler pentru a returna rezultatul măsurătorii sub formă de șiruri de biți sau numărători pentru mai multe circuite într-un singur job.
...
for idx, pub_result in enumerate(result):
    print(
        f" > Primele zece rezultate pentru pub {idx}: {pub_result.data.meas.get_bitstrings()[:10]}"
    )
```
...
Rulează mai multe experimente într-un singur job, valorificând valorile parametrilor pentru a crește reutilizabilitatea circuitului.
...
# Definește trei seturi de parametri pentru circuit
rng = np.random.default_rng(1234)
parameter_values = [
...
# Obține rezultatele pentru primul (și singurul) PUB
pub_result = result[0]
# Obține numărătorile din registrul clasic "meas".
print(
    f" >> Primele zece rezultate pentru registrul de ieșire meas: {pub_result.data.meas.get_bitstrings()[:10]}"
)
```
