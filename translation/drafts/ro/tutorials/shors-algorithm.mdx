...
```python
# Rânduri care vor fi afișate în tabel
rows = []
# Faza corespunzătoare fiecărui șir de biți
measured_phases = []

for output in counts:
    decimal = int(output, 2)  # Convertește șirul de biți în zecimal
    phase = decimal / (2**num_control)  # Găsește valoarea proprie corespunzătoare
    measured_phases.append(phase)
    # Adaugă aceste valori la rândurile din tabel:
    rows.append(
        [
...
# Afișează rândurile într-un tabel
...
```python
# Obține fracția care aproximează cel mai bine 0.666
# cu numitorul < 15
...
```python
# Rânduri care vor fi afișate într-un tabel
rows = []
...
```python
# Primitiva Sampler pentru a obține distribuția de probabilitate
sampler = Sampler(backend)

# Activează decuplarea dinamică cu secvența XpXm
sampler.options.dynamical_decoupling.enable = True
sampler.options.dynamical_decoupling.sequence_type = "XpXm"
# Activează răsucirea de porți
...
```python
# Dicționar cu șiruri de biți și numărătorile lor de reținut
counts_keep = {}
# Prag de filtrare
...
while not FACTOR_FOUND:
    print(f"\nATTEMPT {num_attempt}:")
    # Aici, obținem șirul de biți iterând peste rezultatele
    # unei rulări hardware anterioare cu mai multe măsurători.
    # Alternativ, putem efectua o singură măsurătoare
    # aici, în buclă.
    bitstring = list(counts_keep.keys())[num_attempt]
    num_attempt += 1
    # Găsește faza din măsurătoare
    decimal = int(bitstring, 2)
    phase = decimal / (2**num_control)  # phase = k / r
    print(f"Phase: theta = {phase}")

    # Estimează ordinul din fază
    frac = Fraction(phase).limit_denominator(N)
    r = frac.denominator  # order = r
    print(f"Order of {a} modulo {N} estimated as: r = {r}")

    if phase != 0:
        # Estimările pentru factori sunt gcd(a^{r / 2} ± 1, 15)
        if r % 2 == 0:
            x = pow(a, r // 2, N) - 1
            d = gcd(x, N)
            if d > 1:
