...
# احفظ الملف في مسار سهل التذكر.
# يمكنك تحديد مسار كامل حسب جهازك.
with open("C:\\Users\\...ضع مسارك هنا...\\my_circuit.qpy", "wb") as f:
    qpy.dump(oracle, f)
```
...
# حمّل الدائرة من ملف qpy الخاص بشريكك من المجلد اللي حفظته فيه.
with open("C:\\Users\\...مسار الملف هنا...\\my_circuit.qpy", "rb") as f:
    circuits = qpy.load(f)

# qpy.load دائماً يرجع قائمة من الدوائر
oracle_partner = circuits[0]

# يمكنك عرض الدائرة بصرياً، لكن هذا يكسر نموذج خوارزمية الاستعلام.
...
```python
# حدّث هذا حسب عدد الحالات المستهدفة عند شريكك.
num_marked_states = 1
```
...
```python
# للتشغيل على العتاد، اختر الباكند الذي لديه أقل عدد من المهام في الطابور
service = QiskitRuntimeService()
backend = service.least_busy(operational=True, simulator=False)
backend.name
...
```python
# للتشغيل على حاسوب كمومي حقيقي (تم اختبار هذا على معالج Heron r2 واستغرق 4 ثوانٍ من وقت QPU)
