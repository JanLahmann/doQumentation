...
# تحديد هندسة الجزيء {#تحديد-هندسة-الجزيء}
...
## الخطوة 1: تحويل المدخلات الكلاسيكية إلى مسألة كمية {#الخطوة-1-تحويل-المدخلات-الكلاسيكية-إلى-مسألة-كمية}

بالبقاء مع مثالنا الأساسي على الهيدروجين ثنائي الذرة، المعامل الهندسي الوحيد الذي يمكن تغييره هو طول الرابطة. لتحقيق ذلك، نسير كما في السابق، لكن مع استخدام متغير في بناء الجزيء الأولي (طول الرابطة، *x*، في الوسيطة). هذا تغيير بسيط نسبيًا، لكنه يستلزم تضمين المتغير في الدوال طوال العملية، لأنه يبدأ في بناء هاميلتوني الفرميون وينتشر عبر عملية التعيين وصولًا إلى دالة التكلفة.

أولًا، نحمّل بعض الحزم التي استخدمناها سابقًا ونعرّف دالة Cholesky.
...
الآن لتعريف هاميلتونياننا، سنستخدم PySCF تمامًا كما في المثال السابق، لكننا سنضمّن متغيرًا، `x`، ليمثّل المسافة بين الذرتين. هذا سيُعيد طاقة النواة وطاقة الإلكترون الأحادي وطاقات الإلكترونين كما في السابق.
...
تذكّر أن البناء أعلاه يُنشئ هاميلتونيًا فيرميونيًا بناءً على الأنواع الذرية والهندسة والمدارات الإلكترونية. في الأسفل، نُعيّن هذا الهاميلتوني الفيرميوني على مؤثرات باولي. ستتضمن دالة `build_hamiltonian` هذه أيضًا متغيرًا هندسيًا كوسيطة.
...
سنُعرِّف دالة التكلفة من جديد، لكنها كانت دائمًا تأخذ هاميلتونيًا مبنيًا ومُعيَّنًا بالكامل كوسيطة، لذا لا يتغيّر شيء في هذه الدالة.
...
بما أن الهاميلتوني سيتغيّر مع كل هندسة جديدة، فإن عملية ترتيب المؤثر ستتغيّر في كل خطوة. بالرغم من ذلك، يمكننا تعريف مدير تمرير عام يُطبَّق في كل خطوة، مُخصَّص للعتاد الذي نريد استخدامه.

هنا سنستخدم الخلفية الأقل ازدحامًا المتاحة. سنستخدم تلك الخلفية كنموذج لـ AerSimulator الخاص بنا، مما يتيح لمحاكينا محاكاة سلوك الضوضاء للخلفية الحقيقية على سبيل المثال. هذه النماذج الضوضائية ليست مثالية، لكنها قد تساعدك على توقّع ما ستحصل عليه من العتاد الحقيقي.
...
في الكتلة البرمجية أدناه، نُعدّ مصفوفة لتخزين مخرجات كل خطوة من خطوات المسافة البينذرية $x$. اخترنا نطاق $x$ بناءً على معرفتنا بالقيمة التجريبية لطول الرابطة عند الاتزان: 0.74 أنغستروم. سنُشغّل هذا أولاً على محاكي، وبالتالي سنستورد المُقدِّر (BackendEstimator) من `qiskit.primitives`. لكل خطوة هندسية، نبني الهاميلتونيان ونسمح بعدد معين من خطوات التحسين (500 هنا) باستخدام المُحسِّن "cobyla". في كل خطوة هندسية، نُخزّن كلاً من الطاقة الكلية والطاقة الإلكترونية. نظراً لارتفاع عدد خطوات المُحسِّن، قد يستغرق هذا ساعة أو أكثر. قد ترغب في تعديل المدخلات أدناه لتقليل الوقت المطلوب.
...
نتائج هذا المخرج راح نناقشها تحت في قسم المعالجة اللاحقة؛ بس دلوقتي لاحظ بس إن المحاكاة نجحت. دلوقتي إنت جاهز تشغّل على هاردوير حقيقي. هنضبط الـ resilience على `1`، وده بيعني إن تخفيف الأخطاء TREX هيتستخدم. بما إننا شغّالين على هاردوير حقيقي، هنستخدم Qiskit Runtime وprimitives الخاصة بـ Runtime. لاحظ إن loop الـ geometry وكمان التجارب المتعددة للـ variational كلها جوه الـ session.

عشان في تكاليف وحدود زمنية مرتبطة بتشغيل الهاردوير الحقيقي، قللنا عدد خطوات الـ geometry وخطوات المُحسِّن هنا. اتأكد إنك بتعدّل الخطوات دي حسب أهداف الدقة والحدود الزمنية عندك.
...
سواء للمحاكي أو الهاردوير الحقيقي، ممكن نرسم طاقات الحالة الأساسية اللي اتحسبت لكل مسافة بين الذرات ونشوف فين بتتحقق أدنى طاقة. المفروض ده يكون المسافة الموجودة في الطبيعة، وفعلاً هي قريبة منها. ممكن تطلع منحنى أنعم لو جربت ansaetze ومُحسِّنات تانية، وشغّلت الحسابات أكتر من مرة عند كل خطوة geometry وعملت متوسط لعدة شروط ابتدائية عشوائية.
...
لاحظ إن مجرد زيادة عدد خطوات التحسين مش هيحسّن النتائج من المحاكي، لأن كل عمليات التحسين اتقاربت فعلياً للتفاوت المطلوب في عدد تكرارات أقل من الحد الأقصى.

النتائج من الهاردوير الحقيقي قابلة للمقارنة، باستثناء نطاق قيم مختلف قليلاً.
...
بالإضافة لتوقع طول رابطة H2 البالغ 0.74 أنجستروم، المفروض تكون الطاقة الكلية ‎-1.17 هارتري. نلاحظ إن نتائج الهاردوير الحقيقي اقتربت أكثر من هذه القيم مقارنةً بالمحاكي. على الأرجح ده لأن الضوضاء كانت موجودة (أو محاكاتها) في الحالتين، لكن تخفيف الأخطاء استُخدم بس في حالة الهاردوير الحقيقي.

### الخاتمة {#closing}

بكده نكون خلّصنا كورسنا عن VQE في الكيمياء الكمية. لو إنت مهتم تفهم بعض نظرية المعلومات الأساسية المستخدمة في الحوسبة الكمية، شوف كورس John Watrous عن [أساسيات المعلومات الكمية](/learning/courses/basics-of-quantum-information). لمثال مختصر إضافي عن سير عمل VQE، اطلع على [تعليم تقدير طاقة الحالة الأساسية لسلسلة Heisenberg باستخدام VQE](/tutorials/spin-chain-vqe). أو تصفّح [التعليمات البرمجية التعليمية](https://docs.quantum.ibm.com/tutorials) و[الكورسات](/learning) للقاء مزيد من المواد التعليمية عن أحدث تقنيات الحوسبة الكمية.

متنساش تاخد امتحان الكورس ده. لو حصلت على 80% أو أكثر هتاخد شارة Credly، وهتتبعتلك أوتوماتيك على إيميلك. شكراً لكونك جزء من شبكة IBM Quantum&reg;!
