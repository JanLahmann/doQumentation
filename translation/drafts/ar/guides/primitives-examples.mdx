---
title: "أمثلة على Primitives"
sidebar_label: "أمثلة على Primitives"
description: "أمثلة عملية على استخدام primitives في Qiskit Runtime."
...
الإصدار البيتا لنموذج تنفيذ جديد أصبح متاحاً الآن. يوفر نموذج التنفيذ الموجَّه مرونةً أكبر عند تخصيص سير عمل تخفيف الأخطاء. راجع دليل [نموذج التنفيذ الموجَّه](/guides/directed-execution-model) لمزيد من المعلومات.
</Admonition>
{/*
...
الأمثلة في هذا القسم توضح بعض الطرق الشائعة لاستخدام primitives. قبل تشغيل هذه الأمثلة، اتبع التعليمات الموجودة في [التثبيت والإعداد.](install-qiskit)

<Admonition type="note">
    جميع هذه الأمثلة تستخدم primitives من Qiskit Runtime، لكن يمكنك استخدام base primitives بدلاً منها.
</Admonition>

## أمثلة على Estimator {#estimator-examples}
...
سيُرجع كتلة الكود التالية خطأً لمستخدمي الخطة المجانية (Open Plan) لأنها تستخدم الجلسات. أعباء العمل على الخطة المجانية لا يمكن تشغيلها إلا في [وضع المهمة](/guides/execution-modes#job-mode) أو [وضع الدُّفعة](/guides/execution-modes#batch-mode).
</Admonition>
...
```
## أمثلة على Sampler {#sampler-examples}

أنشئ توزيعات احتمالات شبه كاملة مُخففة من الأخطاء، مأخوذة من مخرجات الدوائر الكمومية. استفد من إمكانيات Sampler في خوارزميات البحث والتصنيف مثل خوارزمية Grover وQVSM.

### تشغيل تجربة واحدة {#run-a-single-experiment-sampler}
...
# احصل على النتائج لأول (والوحيد) PUB
pub_result = result[0]
...
### تشغيل تجارب متعددة في وظيفة واحدة {#run-multiple-experiments-sampler}
...
### تشغيل دوائر ذات معاملات {#run-parameterized-circuits-sampler}
...
# حدّد ثلاث مجموعات من المعاملات للدائرة
rng = np.random.default_rng(1234)
parameter_values = [
...
# احصل على النتائج لأول (والوحيد) PUB
pub_result = result[0]
# احصل على التعدادات من السجل الكلاسيكي "meas".
print(
...
### استخدام الجلسات والخيارات المتقدمة {#use-sessions-and-advanced-options-sampler}

استكشف الجلسات والخيارات المتقدمة لتحسين أداء الدائرة على وحدات المعالجة الكمومية (QPUs).

<Admonition type="caution">
سيُعيد كتلة الكود التالية خطأً لمستخدمي الخطة المفتوحة (Open Plan)، لأنها تستخدم الجلسات. لا يمكن تشغيل الأحمال على الخطة المفتوحة إلا في [وضع الوظيفة](/guides/execution-modes#job-mode) أو [وضع الدُّفعة](/guides/execution-modes#batch-mode).
</Admonition>
...
<Admonition type="tip" title="توصيات">
    - [حدّد خيارات وقت التشغيل المتقدمة.](runtime-options-overview)
    ...
    - تعلّم كيفية التحويل محلياً في قسم [Transpile](./transpile/).
    - جرّب دليل [مقارنة إعدادات التحويل](/guides/circuit-transpilation-settings).
    - اقرأ [الانتقال إلى V2 primitives](/guides/v2-primitives).
    - تعرّف على [حدود الوظائف](/guides/job-limits) عند إرسال وظيفة إلى وحدة المعالجة الكمومية (QPU) من IBM&reg;.
</Admonition>
