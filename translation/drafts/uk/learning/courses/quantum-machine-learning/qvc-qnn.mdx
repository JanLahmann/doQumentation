...
```python
# Цей код визначає зображення для класифікації:

import numpy as np

# Загальна кількість "пікселів"/кубітів
size = 36
# Один вимір зображення (називається вертикальним, але це не принципово). Має бути дільником `size`
vert_size = 6
# Довжина лінії, яку потрібно виявити (жовта). Має бути меншою або рівною найменшому виміру зображення (`<=min(vert_size,size/vert_size)`)
line_size = 6
...
def generate_dataset(num_images):
    ...
    # Робимо два сусідніх елементи рівними pi/2, потім переходимо на наступний рядок. Обережно уникаємо "пікселів" на позиції size/vert_size - linesize, бо хочемо скласти цей список у сітку.
    ...
    # Робимо елементи рівними pi/2, з кроком, рівним довжині/рядкам, щоб після складання елементи опинилися один над одним.
    ...
    for n in range(num_images):
        ...
        if rng == 0:
            ...
            random_image = np.random.randint(0, len(hor_array))
            images.append(np.array(hor_array[random_image]))
            # Випадково вибираємо один із кількох рядків, створених вище.
        elif rng == 1:
            labels.append(1)
            random_image = np.random.randint(0, len(ver_array))
            images.append(np.array(ver_array[random_image]))
...
np.random.seed(42)
# Тут задаємо дуже малий набір даних. Збільш для реалістичності, але стеж за використанням квантового обчислювального часу.
...
# Генеруємо фігуру з вкладеними зображеннями за допомогою subplots.
...
# Один кубіт на кожну ознаку даних
num_qubits = len(train_images[0])

# Кодування даних
# Зауваж, що qiskit сортує параметри за алфавітом. Призначаємо префікс параметра "a", щоб кодування даних потрапило в першу частину схеми — feature map.
feature_map = z_feature_map(num_qubits, parameter_prefix="a")
```

```python
# Це створює схему з CX-вентилями у стиснутому порядку.
...
# CNOT-вентилі між горизонтально сусідніми кубітами.
for i in range(vert_size):
    for j in range(hor_size):
        if j < hor_size - 1:
            qnn_circuit.cx((i * hor_size) + j, (i * hor_size) + j + 1)

# CNOT-вентилі між вертикально сусідніми кубітами — за попередньою симуляцією, мабуть, не потрібні.
#        if i<vert_size-1:
...
```python
# Об'єднуємо feature map і варіаційну схему
ansatz = qnn_circuit

# Об'єднуємо feature map з ansatz
full_circuit = QuantumCircuit(num_qubits)
full_circuit.compose(feature_map, range(num_qubits), inplace=True)
full_circuit.compose(ansatz, range(num_qubits), inplace=True)

# Перевіряємо глибину повної схеми
...
# Щоб запустити на залізі, вибираємо найменш завантажений квантовий комп'ютер або вказуємо конкретний.
service = QiskitRuntimeService()
backend = service.least_busy(operational=True, simulator=False)
...
```python
# Запускаємо менеджер проходів кілька разів, оскільки евристики можуть повертати різні транспіляції для великих схем, а нам потрібна найменш глибока.
...
# Використовуємо найменш глибокий варіант
...
```python
# Це виконувалося на процесорі Eagle r3 04.10.24 і зайняло 7 хв.
...
# Випадкові початкові ваги для ansatz
np.random.seed(42)
# weight_params = np.random.rand(len(ansatz.parameters)) * 2 * np.pi
# Або завантажуємо ваги з попереднього розрахунку
...
# Виконання в сесії дозволяє уникнути повторного поставлення в чергу. Це доступно користувачам тарифних планів Premium Plan, Flex Plan та On-Prem (IBM Quantum Platform API).
...
with Session(backend=backend) as session:
    ...
    for epoch in range(num_epochs):
        for i in range((num_samples - 1) // batch_size + 1):
            ...
            target = train_labels_batch
            iter = 0
            # Можна збільшити maxiter для повної оптимізації.
            res = minimize(
                mse_loss_weights,
                weight_params,
                method="COBYLA",
...
# Користувачі відкритого доступу можуть виконати той самий розрахунок за допомогою batch, але повторне поставлення в чергу можливе.
