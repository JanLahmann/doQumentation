...
```python
# Квантова схема для створення стану Белла
...
```python
# Повторне виконання двокубітного стану Белла
sampler.options.default_shots = 1000

job = sampler.run([isa_circuit])
second_result = job.result()

# Побудова результатів з користувацькими опціями
plot_histogram(
    [
...
# Простий експеримент з оцінки для створення результатів
qc = QuantumCircuit(2)
qc.h(0)
qc.crx(1.5, 0, 1)
...
# Оскільки масив результатів структурований як 2D-масив, де кожен елемент є
# списком, що містить одне значення, потрібно вирівняти масив.

# Побудова за допомогою Matplotlib
