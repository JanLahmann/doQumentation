...
```python
# Обчислення математичних сподівань для двох PUB, обидва з точністю 0.05.
...
```python
# Вибірка двох схем по 128 shots кожна.
sampler.run([circuit1, circuit2], shots=128)

# Вибірка двох схем з різною кількістю shots.
# "None" необхідні як заповнювачі
# для відсутніх значень параметрів у цьому прикладі.
sampler.run([
  (circuit1, None, 123),
  (circuit2, None, 456),
])
...
* Об'єкти результатів Sampler V2 організують дані у термінах **імен класичних регістрів вхідних схем**, для сумісності з динамічними схемами. За замовчуванням ім'я класичного регістра — `meas`, як показано в наступному прикладі. При визначенні схеми, якщо ти створюєш один або кілька класичних регістрів з нестандартним іменем, використовуй це ім'я для отримання результатів. Ім'я класичного регістра можна дізнатись, запустивши `<circuit_name>.cregs`. Наприклад, `qc.cregs`.
       ...
       ```python
    # Визначення квантової схеми з 2 кубітами
...
<Tabs>
  <TabItem value="Estimator14" label="1 схема, 4 спостережувані">
    ```python
    # Estimator V1: Виконання 1 схеми з 4 спостережуваними
    job = estimator_v1.run([circuit] * 4, [obs1, obs2, obs3, obs4])
    evs = job.result().values

    # Estimator V2: Виконання 1 схеми з 4 спостережуваними
...
```
  ...
  <TabItem value="Estimator142" label="1 схема, 4 спостережувані, 2 набори параметрів">
    ```python
    # Estimator V1: Виконання 1 схеми з 4 спостережуваними і 2 наборами параметрів
    job = estimator_v1.run([circuit] * 8, [obs1, obs2, obs3, obs4] * 2, [vals1, vals2] * 4)
    evs = job.result().values

    # Estimator V2: Виконання 1 схеми з 4 спостережуваними і 2 наборами параметрів
...
```
  ...
  <TabItem value="Estimator22" label="2 схеми, 2 спостережувані">
    ```python
    # Estimator V1: Неможливо виконати 2 схеми з різними спостережуваними

    # Estimator V2: Виконання 2 схем з 2 різними спостережуваними.  Є
    # два PUB, бо кожен PUB може містити лише одну схему.
...
<Tabs>
  <TabItem value="Sampler13" label="1 схема, 3 набори параметрів">
    ```python
      # Sampler V1: Виконання 1 схеми з 3 наборами параметрів
      job = sampler_v1.run([circuit] * 3, [vals1, vals2, vals3])
      dists = job.result().quasi_dists

      # Sampler V2: Виконання 1 схеми з 3 наборами параметрів
...
```
  ...
  <TabItem value="Sampler21" label="2 схеми, 1 набір параметрів">
    ```python
    # Sampler V1: Виконання 2 схем з 1 набором параметрів
      job = sampler_v1.run([circuit1, circuit2], [vals1] * 2)
      dists = job.result().quasi_dists

      # Sampler V2: Виконання 2 схем з 1 набором параметрів
...
result = job.result()
# Отримання результатів для першого (і єдиного) PUB
pub_result = result[0]
...
# Встановлення опцій під час ініціалізації примітива
estimator = Estimator(backend, options={"resilience_level": 2})

# Встановлення опцій після ініціалізації примітива
# Використовує автодоповнення.
estimator.options.default_shots = 4000
# Масове оновлення.
estimator.options.update(default_shots=4000, resilience_level=2)

# Вивести у форматі словника.
# Для незаданих опцій використовуються серверні значення за замовчуванням.
...
# Увімкнення gate twirling. Потребує qiskit_ibm_runtime 0.23.0 або новішої версії.
sampler.options.twirling.enable_gates = True

# Масове оновлення.  Значення default_shots буде перевизначено, якщо вказати shots у run() або в PUB.
...
# Встановлення resilience_level на 0
estimator.options.resilience_level = 0

# Увімкнення пом'якшення помилок вимірювання
...
sampler = Sampler(backend)
# Увімкнення dynamical decoupling з послідовністю XpXm.
sampler.options.dynamical_decoupling.enable = True
...
# перевірити, чи ще виконується завдання
...
# Зміна форми масиву спостережуваних для broadcasting
...
# Визначення трьох наборів параметрів для схеми
rng = np.random.default_rng(1234)
parameter_values = [
...
# Отримання результатів для першого (і єдиного) PUB
pub_result = result[0]
# Отримання кількості відліків з класичного регістра "meas".
