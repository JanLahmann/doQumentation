...
description: "Зрозумій формат вхідних і вихідних даних (включно з Primitive Unified Blocs або PUB) примітивів"
notebook_path: "guides/primitive-input-output.ipynb"
# source_hash: 013a0bb4
---
...
Бета-версія нової моделі виконання вже доступна. Модель направленого виконання забезпечує більшу гнучкість при налаштуванні робочого процесу пом'якшення помилок. Дивись посібник [Модель направленого виконання](/guides/directed-execution-model) для отримання додаткової інформації.
</Admonition>
{/*
...
- Один `QuantumCircuit`, який може містити один або більше об'єктів [`Parameter`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.Parameter)
   *Примітка: ці схеми також повинні включати інструкції вимірювання для кожного кубіту, що підлягає вибірці.*
...
Наступний код демонструє приклад набору векторизованих вхідних даних для примітива `Estimator` і виконує їх на бекенді IBM&reg; як єдиний об'єкт `RuntimeJobV2`.
...
A1     (2d array):      2 x 1
A2     (3d array):  6 x 5 x 4 # Це спрацювало б, якби середній вимір дорівнював 2, але він дорівнює 5.
```
...
Після того як один або кілька PUB надіслано до QPU для виконання і завдання успішно завершено, дані повертаються як об'єкт-контейнер [`PrimitiveResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PrimitiveResult), доступний через виклик методу `RuntimeJobV2.result()`. `PrimitiveResult` містить ітерований список об'єктів [`PubResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PubResult), що містять результати виконання для кожного PUB. Залежно від використовуваного примітива, ці дані будуть або математичними сподіваннями та їхніми похибками у випадку Estimator, або вибірками виходів схеми у випадку Sampler.
...
Кожен із цих об'єктів `PubResult` має атрибути `data` і `metadata`. Атрибут `data` — це налаштований [`DataBin`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.DataBin), що містить фактичні значення вимірювань, стандартні відхилення тощо. Цей `DataBin` має різні атрибути залежно від форми або структури відповідного PUB, а також від параметрів пом'якшення помилок, зазначених у примітиві, що використовувався для подачі завдання (наприклад, [ZNE](./error-mitigation-and-suppression-techniques#zero-noise-extrapolation-zne) або [PEC](./error-mitigation-and-suppression-techniques#probabilistic-error-cancellation-pec)). Натомість атрибут `metadata` містить інформацію про параметри середовища виконання та пом'якшення помилок (пояснено пізніше в розділі [Метадані результату](#result-metadata) на цій сторінці).
...
#### Як Estimator обчислює похибку {#how-the-estimator-calculates-error}
...
Розглянемо одну спостережувану $\mathcal{O}$. За відсутності [ZNE](/guides/error-mitigation-and-suppression-techniques#zero-noise-extrapolation-zne) можна вважати, що кожен знімок виконання Estimator надає точкову оцінку математичного сподівання $\langle \mathcal{O} \rangle$. Якщо точкові оцінки знаходяться у векторі `Os`, то значення, що повертається в `ensemble_standard_error`, еквівалентне наступному (де $\sigma_{\mathcal{O}}$ — [стандартне відхилення оцінки математичного сподівання](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.BackendEstimatorV2), а $N_{shots}$ — кількість знімків):

$$\frac{ \sigma_{\mathcal{O}} }{ \sqrt{N_{shots}} },$$

де всі знімки розглядаються як частина одного ансамблю. Якщо ти запросив гейтове [twirling](/guides/error-mitigation-and-suppression-techniques#pauli-twirling) (`twirling.enable_gates = True`), можна відсортувати точкові оцінки $\langle \mathcal{O} \rangle$ на набори, що мають спільний twirl. Назвемо ці набори оцінок `O_twirls`, і їх є `num_randomizations` (кількість twirl). Тоді `stds` — це стандартна похибка середнього `O_twirls`, як у

$$\frac{ \sigma_{\mathcal{O}} }{ \sqrt{N_{twirls}} },$$

де $\sigma_{\mathcal{O}}$ — стандартне відхилення `O_twirls`, а $N_{twirls}$ — кількість twirl. Коли twirling не увімкнено, `stds` і `ensemble_standard_error` рівні.
...
Для завдань Sampler ти також можеш переглянути метадані результату, щоб зрозуміти, коли певні дані були запущені; це називається [_проміжком виконання (execution span)._](monitor-job#execution-spans)
