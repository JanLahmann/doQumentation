...
# データエンコーディング
# Qiskitはパラメータをアルファベット順に並べることに注意してください。パラメータプレフィックスとして "a" を指定することで、データエンコーディングが回路の最初の部分（特徴量マッピング）に配置されるようにします。
feature_map = z_feature_map(num_qubits, parameter_prefix="a")
```

```python
# 圧縮された順序でCNOTゲートを含む回路を作成します。
...
# ハードウェアで実行する場合は、最もビジーでない量子コンピューターを選択するか、特定のものを指定します。
service = QiskitRuntimeService()
backend = service.least_busy(operational=True, simulator=False)
...
```python
# ヒューリスティックは大規模回路に対してさまざまなトランスパイル結果を返す可能性があるため、パスマネージャーを複数回試して最も浅いものを使用します。
...
```python
# Eagle r3プロセッサー上で2024年10月4日に実行し、7分かかりました。
...
# セッション内で実行することで、繰り返しのキュー待ちを回避できます。これはプレミアムプラン、フレックスプラン、オンプレミス（IBM Quantum Platform API）プランのユーザーが利用できます。
...
# オープンユーザーはバッチを使用して同じ計算を実行できますが、繰り返しのキュー待ちが発生する場合があります。
