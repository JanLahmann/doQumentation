...
# ハードウェアで実行可能な基底ゲートに Circuit をトランスパイルします
from qiskit.transpiler.preset_passmanagers import generate_preset_pass_manager
...
```python
# Notebook のセッションが切断された場合は、以下のコードを実行してジョブのステータスを確認できます
from qiskit_ibm_runtime import QiskitRuntimeService
...
```python
# ステップ 1: 問題を量子Circuit と演算子にマッピングする
# 2 Qubit Circuit を作成する
qc = QuantumCircuit(2, 2)

# Eve が EPR ペアを生成し、q0 を Alice に、q1 を Bob に送る
qc.h(0)
qc.cx(0, 1)
qc.barrier()

# Alice が Bob に転送したいメッセージを設定する
...
qc.barrier()
# Bob が Alice から EPR Qubit を受け取り、ユニタリ演算を実行する
qc.cx(0, 1)
qc.h(0)
qc.barrier()
