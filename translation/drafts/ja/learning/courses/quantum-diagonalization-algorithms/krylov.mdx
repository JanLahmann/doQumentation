...
```python
# vknown は部分空間内の既存ベクトルです。vnext は追加しようとするベクトルで、vknown と直交していなければなりません。
...
# v は部分空間に追加する候補ベクトルです。s は既存の部分空間です。
...
```python
# マイクロ秒単位で時間を追跡するために必要なインポートと定義
import time

def time_mus():
    return int(time.time() * 1000000)

# この関数は、元の行列の全空間を張るクリロフ部分空間を構築します。
#     入力:
#       v0          : 初期ベクトル
#       matrix      : 対角化する元の行列
#     出力:
...
def krylov_full_build(v0, matrix):
    ...
    # クリロフベクトル、射影ハミルトニアン、固有値、および総所要時間を返す。
...
# 上で定義した関数にテスト行列と初期推定値を引数として渡し、出力を計算する。
...
plt.axhline(y=0, color="red", linestyle="--")  # y=0 に赤い破線を追加
plt.xlabel("Order of Krylov subspace")  # x軸ラベルを追加
plt.ylabel("Error in minimum eigenvalue")  # y軸ラベルを追加
plt.show()
```
...
plt.xlabel("Dimension of matrix")  # x軸ラベルを追加
plt.ylabel("Time to diagonalize (microsec)")  # y軸ラベルを追加
plt.show()
```
...
```python
# 許容できる絶対誤差を選択し、その誤差が達成されるクリロフ部分空間サイズを追跡するリストを作成する。
abserr = 0.05
accept_subspace_size = []

# クリロフ法に費やした合計時間と、その中で射影行列の対角化に費やした時間を格納するリスト。
...
for mm in range(0, num_matrix - 1):
    ...
    # 必要な誤差内の最小固有値を与えるクリロフ部分空間がまだ見つかっていない。
    found = 0
    for j in range(0, len(matrices[mm]) - 1):
        # まだ見つかっていない場合...
        if found == 0:
            # ...この部分空間が条件を満たすなら、すべてを記録する
            if (
...
plt.xlabel("Dimension of matrix")  # x軸ラベルを追加
plt.ylabel("Time to diagonalize (smoothed, microsec)")  # y軸ラベルを追加
plt.show()
```
...
for i in range(krylov_dim - 1):
    # 実験から期待値を取得します
    expval_real = results.data.evs[0][0][i]  # ZNE を使用する場合は自動外挿された evs
    expval_imag = results.data.evs[1][0][i]  # ZNE を使用する場合は自動外挿された evs

    # 期待値を取得します
    expval = expval_real + 1j * expval_imag
    S_first_row[i + 1] += prefactors[i] * expval

S_first_row_list = S_first_row.tolist()  # 保存用

S_circ = np.zeros((krylov_dim, krylov_dim), dtype=complex)

# 最初の行のエントリを行列全体に分配します:
