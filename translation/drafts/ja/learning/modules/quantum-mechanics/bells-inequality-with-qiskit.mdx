...
```python
# まず qiskit と可視化モジュールをインポートして、結果のヒストグラムをプロットできるようにします。
...
```python
# まず最初の量子 Circuit を宣言し、2つの Qubit（最初の "2"）と出力を格納するための2つの古典的ビット（2番目の "2"）を与えます。
# レジスタの定義
...
# 上記の解析から、各 Qubit（L と R）に X Gate が必要であることがわかっています。
qc1.x([0, 1])
# ルーカスの Qubit（0番目の Qubit）にアダマール Gate が必要です。
qc1.h(0)
# 制御 NOT Gate は0番目の Qubit（ルーカスの）を制御として、1番目の Qubit（リアナの）をターゲットとして使用します。
qc1.cx(0, 1)
# 回転 Gate は1番目の Qubit（リアナの）に作用し、引数は -2 pi/3 です。
qc1.ry(-2 * pi / 3, 1)
# 最後に、回路内のすべての Qubit を測定して測定確率を得て、結果を古典的ビットに格納します。
qc1.measure([0, 1], [0, 1])
# ベルの不等式を検証する3つの Circuit のうち最初のものを描画します。
qc1.draw(output="mpl")
```
...
# 100 個のパラメーター値セットすべてに対してショットを返すジョブを開始します。
for i in range(0, len(qcs)):
    pub = qcs[i]
    job = sampler.run([pub], shots=10000)
    # 0番目の pub の結果を抽出します（この例では pub が1つだけあります）。
    result = job.result()
...
# ここでは "None" を 0 に変換して合計できるようにします。
...
# ここでは、すべての結果を格納し、各 Circuit での出現回数を合計する辞書を作成します。
