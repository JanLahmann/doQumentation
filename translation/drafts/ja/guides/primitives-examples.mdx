---
title: "Primitivesの使用例"
sidebar_label: "Primitivesの使用例"
description: "Qiskit RuntimeでPrimitivesを使用する実践的な例。"
...
新しい実行モデルのベータリリースが公開されました。有向実行モデル（Directed Execution Model）は、エラー軽減ワークフローをカスタマイズする際の柔軟性を高めます。詳細については、[有向実行モデル](/guides/directed-execution-model)のガイドをご覧ください。
</Admonition>
{/*
...
このセクションの例では、Primitivesの一般的な使い方を紹介します。これらの例を実行する前に、[インストールとセットアップ](install-qiskit)の手順に従ってください。

<Admonition type="note">
    これらの例はすべてQiskit RuntimeのPrimitivesを使用していますが、代わりにベースPrimitivesを使用することもできます。
</Admonition>

## Estimatorの例 {#estimator-examples}
...
### パラメータ付き回路を実行する {#run-parameterized-circuits}
...
以下のコードブロックは、セッションを使用するため、Openプランのユーザーにはエラーが返されます。Openプランのワークロードは、[ジョブモード](/guides/execution-modes#job-mode)または[バッチモード](/guides/execution-modes#batch-mode)でのみ実行できます。
</Admonition>
...
### 1つのジョブで複数の実験を実行する {#run-multiple-experiments-in-a-single-job}
...
### パラメータ化された回路を実行する {#run-parameterized-circuits}
...
<Admonition type="caution">
以下のコードブロックは、セッションを使用しているため、Openプランのユーザーにはエラーが返されます。Openプランのワークロードは、[ジョブモード](/guides/execution-modes#job-mode)または[バッチモード](/guides/execution-modes#batch-mode)でのみ実行できます。
</Admonition>
...
<Admonition type="tip" title="推奨事項">
    - [高度なランタイムオプションを指定する。](runtime-options-overview)
    - IBM Quantum Learningの[コスト関数のレッスン](/learning/courses/variational-algorithm-design/cost-functions)を通じて、プリミティブを実践的に学びましょう。
    - [トランスパイル](./transpile/)セクションで、ローカルでのトランスパイル方法を学びましょう。
    - [トランスパイラー設定の比較](/guides/circuit-transpilation-settings)ガイドを試しましょう。
    - [V2プリミティブへの移行](/guides/v2-primitives)を読みましょう。
    - IBM&reg; QPUにジョブを送信する際の[ジョブ制限](/guides/job-limits)を理解しましょう。
</Admonition>
