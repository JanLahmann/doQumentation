...
# ハミルトニアンシミュレーション向けFunctionテンプレートの構築 {#build-a-function-template-for-hamiltonian-simulation}

このテンプレートは、ユーザー定義のスピンベースのハミルトニアンに対して初期状態の時間発展をシミュレートし、[AQCアドオン](./qiskit-addons-aqc)を使用して指定した期待値のセットを返すワークフローをカプセル化しています。

このテンプレートは以下のステップで構成されるQiskitパターンとして構造化されています。

#### 1. 入力の収集と問題のマッピング {#1-collecting-input-and-mapping-the-problem}

このセクションは、シミュレートするハミルトニアン、`QuantumCircuit`の形式による初期状態、期待値を推定するためのオブザーバブルのセット、およびAQCアドオンのオプション仕様を入力として受け取ります。このステップでは、必要な入力データがすべて存在し、正しい形式であることを検証します。

次に、入力引数を使用してワークフローの関連する量子回路と演算子を構築します。ターゲット回路が作成され、AQCアドオンを使用してこの回路の行列積状態表現が求められます。その後、アンザッツ回路が生成され、テンソルネットワーク法を用いて最適化され、時間発展の残りの部分を実行する最終回路が生成されます。

#### 2. 生成された回路を実行用に準備する {#2-prepare-the-generated-circuits-for-execution}

AQCアドオンから生成された回路は、選択したバックエンドで実行できるようにトランスパイルされます。回路実行を管理するために、デフォルトのエラー緩和オプションのセットとともに[`EstimatorV2`](../api/qiskit-ibm-runtime/estimator-v2)インスタンスが作成されます。

#### 3. 実行 {#3-execution}

最後に、アンザッツ回路がトランスパイルされてQPU上で実行され、指定されたすべての期待値の推定値が収集されます。これらはユーザーがアクセスできるシリアライズ可能な形式で返されます。

## Functionテンプレートの作成 {#write-the-function-template}

まず、[AQC-Tensor Qiskitアドオン](/guides/qiskit-addons-aqc)を使用して問題の説明をハードウェア実行用の回路深度を削減した回路にマッピングする、ハミルトニアンシミュレーション向けのFunctionテンプレートを作成します。

コードは`./source_files/template_hamiltonian_simulation.py`に保存されます。このファイルは、Qiskit Serverlessを使用してリモートでアップロードおよび実行できるFunctionテンプレートです。
...
まず、テンプレートの入力を取得します。この例では、ハミルトニアンシミュレーションに関連するドメイン固有の入力（ハミルトニアンやオブザーバブルなど）と、機能固有のオプション（AQC-Tensorを使用してトロッター回路の初期レイヤーをどれだけ圧縮するかや、この例の一部であるデフォルト設定を超えたエラー抑制・緩和の細かい調整のための高度なオプションなど）があります。
...
Functionテンプレートの実行中は、print文を使用してログに情報を返すと便利です。これにより、ワークロードの進捗状況をより適切に評価できます。以下は、実際に使用されるEstimatorオプションの記録が残るよう`estimator_options`を出力する簡単な例です。プログラム全体を通じて、AQC-Tensorの反復コンポーネント中の目的関数の値や、ハードウェアでの実行を想定した最終命令セットアーキテクチャ（ISA）回路の2量子ビット深度など、実行中の進捗を報告するための類似した例が多数あります。
...
テンプレートをさまざまな入力にわたって再利用できるようにする上で重要な側面の一つが入力検証です。以下のコードは、AQC-Tensor中の停止フィデリティが適切に指定されているかを検証し、そうでない場合にエラーを修正する方法についての有益なエラーメッセージを返す例です。
...
#### 関数出力の準備 {#prepare-the-function-outputs}

まず、Functionテンプレートの出力をすべて保持するためのディクショナリを準備します。ワークフロー全体を通じてこのディクショナリにキーが追加され、プログラムの最後に返されます。
...
### AQCによる問題のマッピングと回路の前処理 {#map-the-problem-and-pre-process-the-circuit-with-aqc}

AQC-Tensorの最適化は、Qiskitパターンのステップ1で行われます。まず、ターゲット状態が構築されます。この例では、AQC部分と同じハミルトニアンを同じ時間発展させるターゲット回路から構築されます。次に、より少ないトロッターステップ数の等価な回路からアンザッツが生成されます。ACQアルゴリズムのメイン部分では、そのアンザッツがターゲット状態に反復的に近づけられます。最後に、結果は目的の発展時間に達するために必要な残りのトロッターステップと組み合わされます。

以下のコードには、追加のログ記録の例が組み込まれている点にご注意ください。
...
### 実行用の最終回路の最適化 {#optimize-the-final-circuit-for-execution}

ワークフローのAQC部分が完了した後、`final_circuit`は通常通り[ハードウェア向けにトランスパイル](/guides/transpile#instruction-set-architecture)されます。
...
#### ドライランモード使用時の早期終了 {#exit-early-if-using-dry-run-mode}

ドライランモードが選択された場合、ハードウェアでの実行前にプログラムが停止されます。これは、たとえばISA回路の2量子ビット深度をまず確認してからハードウェアでの実行を決定したい場合などに便利です。
...
このFunctionテンプレートは、このハミルトニアンシミュレーションワークフローの関連するドメインレベルの出力（期待値）に加えて、途中で生成された重要なメタデータを返します。
...
## IBM Quantum PlatformへのFunctionのデプロイ {#deploy-the-function-to-ibm-quantum-platform}

前のセクションでリモートで実行するプログラムを作成しました。このセクションのコードでは、そのプログラムをQiskit Serverlessにアップロードします。
...
オプションとして`save_account()`を使用して認証情報を保存できます（[IBM Cloudアカウントの設定](/guides/cloud-setup#cloud-save)ガイドを参照してください）。これは[`QiskitRuntimeService.save_account()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/qiskit-runtime-service#save_account)と同じファイルに認証情報を書き込む点にご注意ください。
...
このプログラムはカスタムの`pip`依存関係を持っています。`QiskitFunction`インスタンスを構築する際に`dependencies`配列に追加します。
...
## Functionテンプレートのリモート実行 {#run-the-function-template-remotely}

Functionテンプレートがアップロードされたので、Qiskit Serverlessを使用してリモートで実行できます。まず、名前でテンプレートを読み込みます。

```python
template = serverless.load("template_hamiltonian_simulation")
```

次に、ハミルトニアンシミュレーションのドメインレベルの入力でテンプレートを実行します。この例では、ランダムカップリングを持つ50量子ビットのXXZモデルと、初期状態およびオブザーバブルを指定します。
...
ジョブが実行中になった後は、`print()` の出力から作成されたログを取得できます。これらのログは、ハミルトニアンシミュレーションワークフローの進捗に関する有用な情報を提供します。たとえば、AQC の反復コンポーネント中の目的関数の値や、ハードウェア実行を意図した最終 ISA 回路の 2 量子ビット深度などが含まれます。
...
結果が得られるまでプログラムの残りの処理をブロックします。ジョブ完了後に結果を取得できます。取得できる結果には、ハミルトニアンシミュレーションのドメインレベルの出力（期待値）と有用なメタデータが含まれます。
...
AQC-Tensor Qiskit アドオンについてより深く理解するには、[近似量子コンパイルによるトロッタライズド時間発展の改善](/tutorials/approximate-quantum-compilation-for-time-evolution)チュートリアルまたは [qiskit-addon-aqc-tensor リポジトリ](https://github.com/Qiskit/qiskit-addon-aqc-tensor)をご覧ください。
...
<details>
<summary><b>完全なプログラムソースコード</b></summary>

以下は `./source_files/template_hamiltonian_simulation.py` のソース全体を 1 つのコードブロックとして示したものです。
