...
```python
def cholesky(V, eps):
    # zobacz https://arxiv.org/pdf/1711.02242.pdf sekcja B2
    # zobacz https://arxiv.org/abs/1808.02625
    # zobacz https://arxiv.org/abs/2104.08957
    no = V.shape[0]
    chmax, ng = 20 * no, 0
...
def build_hamiltonian(ecore: float, h1e: np.ndarray, h2e: np.ndarray) -> SparsePauliOp:
    ...
    # niskorangowa dekompozycja Hamiltonianu
    Lop, ng = cholesky(h2e, 1e-6)
    t1e = h1e - 0.5 * np.einsum("pxxr->pr", h2e)

    H = ecore * identity(2 * ncas)
    # składnik jednociałowy (one-body term)
    for p in range(ncas):
        for r in range(p, ncas):
            H += t1e[p, r] * Exc[p][r - p]
    # składnik dwuciałowy (two-body term)
    for g in range(ng):
...
```python
# Zastąp te zmienne właściwymi odległościami:
a = 1
b = 1
c = 1
...
def convert_openfermion_to_qiskit(
    ...
    for term, constant in terms.items():
        # Domyślnie ustawione na tożsamość
        operator = list("I" * num_qubits)

        # Iteracja po PauliSum i zastąpienie I operatorem Pauli
