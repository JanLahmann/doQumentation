#### Faedah kluster untuk mengurangkan overhed pensampelan bagi masa pengiraan klasik yang ditetapkan {#benefits-of-clusters-for-reducing-sampling-overhead-for-a-given-classical-compute-time}
...
### c. Laksanakan Circuit yang dikehendaki dengan antinoise {#c-execute-the-circuit-of-interest-with-antinoise}

#### c.i. Sediakan template circuit menggunakan `samplex` {#c-i-prepare-template-circuit-using-samplex}

`samplex` ialah output daripada kaedah `build` milik Samplomatic, yang memuatkan semua maklumat yang diperlukan untuk menjana parameter rawak bagi `template_circuit`. Parameter-parameter ini kemudiannya digunakan untuk menyediakan objek-objek `QuantumProgram`, yang seterusnya dijalankan pada QPU dengan primitif `Executor`. Setiap `QuantumProgram` boleh mengandungi beberapa item, yang boleh kamu fikirkan sebagai pasangan `template` dan `samplex`.

Rujuk tutorial [Hello samplomatic](https://github.com/qiskit-community/qdc-challenges-2025/blob/main/day3_tutorials/Track_A/hello_samplomatic/Samplomatic%20-%20Hello%20World.ipynb) untuk maklumat lanjut.
...
#### c.ii. Sediakan `QuantumProgram` {#c-ii-set-up-the-quantumprogram}
...
#### c.iii. Laksanakan program dengan primitif `Executor` {#c-iii-execute-program-with-the-executor-primitive}
...
Semasa kita mengira nilai jangkaan akhir yang dikehendaki menggunakan `expectation_values`, kita akan melaksanakan beberapa teknik pasca-proses yang bermanfaat untuk memastikan kita mendapat keputusan berkualiti terbaik yang mungkin. Pertama, kita guna [twirled readout mitigation, TREX](https://quantum.cloud.ibm.com/docs/guides/error-mitigation-and-suppression-techniques#twirled-readout-error-extinction-trex), yang mengambil kira sebarang ralat yang berlaku semasa proses pembacaan. Kemudian, kita betulkan ralat yang disebabkan oleh bunyi bising bukan-Markov pada backend Heron kita menggunakan kaedah pemilihan pasca. Kaedah ini mengukur qubit aktif dan qubit penonton (spectator), kemudian menerapkan putaran perlahan pada setiap qubit, lalu mengukur semula. Dalam kes di mana dua pengukuran tersebut tidak mengesahkan qubit yang terbalik seperti yang dijangkakan, shot-shot tersebut akan dibuang dengan menggunakan `mask` daripada fungsi `PostSelector`. Dalam pengiraan mask, strategi tertentu boleh ditetapkan untuk menapis berdasarkan nod satu-qubit atau tepi spektator jiran, yang boleh mempengaruhi bilangan shot yang ditapis dan kualiti keputusan.
...
Daripada pemeriksaan keputusan eksperimen, kita boleh membandingkan secara terus tingkah laku pendekatan yang berbeza: PEC, PEC yang digabungkan dengan SLC, dan garis dasar keputusan tanpa mitigasi. Beberapa butiran khusus yang perlu diketengahkan:

- Keputusan tanpa mitigasi kekal di luar julat bias yang dikehendaki dan tidak terjejas oleh overhed pensampelan.
- Memandangkan kos pensampelan yang tinggi (~10k), PEC sahaja tidak menumpu dalam had rawak yang digunakan.
- PEC + SLC pula menumpu dengan lebih cepat.
- Had ralat juga berkurangan dengan jauh lebih pantas untuk PEC + SLC berbanding PEC biasa.
