...
## Sokongan Qiskit Runtime {#qiskit-runtime-support}

Walaupun pelukis garis masa yang dibina dalam Qiskit berguna untuk litar statik, ia mungkin tidak mencerminkan pemasaan [litar dinamik](/guides/classical-feedforward-and-control-flow) dengan tepat kerana operasi tersirat seperti penyiaran dan penentuan cabang. Sebagai sebahagian daripada sokongan litar dinamik, Qiskit Runtime mengembalikan maklumat pemasaan litar yang tepat di dalam keputusan kerja apabila diminta.

<Admonition type="note" title="Nota">
- Ini adalah fungsi eksperimen. Ia dalam status keluaran pratonton dan oleh itu tertakluk kepada perubahan.
- Fungsi ini hanya terpakai kepada kerja Sampler Qiskit Runtime.
...
### Dayakan pengambilan data pemasaan {#enable-timing-data-retrieval}

Untuk mendayakan pengambilan data pemasaan, tetapkan bendera eksperimen `scheduler_timing` kepada `True` semasa menjalankan kerja primitif.
...
### Akses data pemasaan litar {#access-the-circuit-timing-data}

Apabila diminta, data pemasaan litar bagi setiap PUB dikembalikan dalam metadata keputusan kerja, di bawah `["compilation"]["scheduler_timing"]["timing"]`. Medan ini mengandungi maklumat pemasaan mentah. Untuk memaparkan maklumat pemasaan, gunakan alat visualisasi terbina, seperti yang diterangkan dalam bahagian [Visualisasikan pemasaan](#visualize-timings).

Gunakan kod berikut untuk mengakses data pemasaan litar bagi PUB pertama:
...
Walaupun memvisualisasikan data pemasaan litar menggunakan kaedah `draw_circuit_schedule_timing` adalah kes penggunaan yang paling biasa, mungkin berguna untuk memahami struktur data pemasaan mentah yang dikembalikan. Ini boleh membantu anda, sebagai contoh, mengekstrak maklumat secara aturcara.

Data pemasaan yang dikembalikan dalam `["compilation"]["scheduler_timing"]["timing"]` adalah senarai rentetan. Setiap rentetan mewakili satu arahan pada beberapa saluran dan dipisahkan dengan koma kepada jenis data berikut:

- `Branch` - Menentukan sama ada arahan berada dalam aliran kawalan (then / else) atau cabang utama.
- `Instruction` - Gate dan qubit untuk dioperasikan.
- `Channel` - Saluran yang ditugaskan dengan arahan. Ia boleh menjadi salah satu daripada berikut:
      - `Qubit x` - Saluran pemacu untuk qubit _x_.
      - `AWGRx_y` (penjana gelombang sewenang-wenangnya readout) - Digunakan oleh saluran readout untuk berkomunikasi semasa mengukur qubit. Argumen _x_ dan _y_ masing-masing sepadan dengan ID instrumen readout dan nombor qubit.
- `T0` - Masa mula arahan dalam jadual penuh
- `Duration` - Tempoh arahan, dalam unit _dt_ saat, di mana 1 dt = 1 kitaran penjadualan. Anda boleh mencari nilai `dt` bagi sebuah backend menggunakan [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt).
- `Pulse` - Jenis operasi denyutan yang digunakan.
...
#### Fahami rajah yang dijana {#understand-the-generated-figure}

Imej data pemasaan litar yang dihasilkan oleh `draw_circuit_schedule_timing` menyampaikan maklumat berikut:

- Paksi X ialah masa dalam unit _dt_ saat, di mana 1 dt = 1 kitaran penjadualan. Anda boleh mencari nilai `dt` bagi sebuah backend menggunakan [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt).
- Paksi Y ialah saluran (bayangkan saluran sebagai instrumen yang memancarkan denyutan).
    - `Receive channel` - Satu-satunya saluran yang bukan instrumen sendiri. Ia adalah arahan yang dimainkan pada semua saluran yang terlibat dalam prosedur komunikasi dengan hab pada masa tersebut.
    - `Qubit x` - Saluran pemacu untuk qubit x.
    - `AWGRx_y` (penjana gelombang sewenang-wenangnya readout) - Digunakan oleh saluran readout untuk berkomunikasi semasa mengukur qubit. Argumen _x_ dan _y_ masing-masing sepadan dengan ID instrumen readout dan nombor qubit.
    - `Hub` - Mengawal penyiaran.

Selain itu, setiap arahan mempunyai format *X_Y*, di mana *X* adalah nama arahan dan *Y* adalah jenis denyutan. Jenis `play` mengaplikasikan denyutan kawalan, dan `capture` merekodkan keadaan qubit. Anda juga boleh mengarahkan kursor ke atas setiap arahan untuk mendapatkan butiran lanjut. Sebagai contoh, rajah sebelumnya menunjukkan denyutan kawalan untuk Gate X yang diaplikasikan pada qubit 10 pada 1161 dt.
### Contoh hujung ke hujung {#end-to-end-example}

Contoh ini menunjukkan cara mendayakan pilihan tersebut, mendapatkan maklumat pemasaan litar daripada metadata, dan memaparkannya sebagai imej.

Pertama, sediakan persekitaran, tentukan litar dan tukarkannya kepada litar ISA, dan tentukan serta jalankan kerja.
...
Seterusnya, dapatkan pemasaan jadual litar:
...
Akhir sekali, anda boleh memvisualisasikan dan menyimpan pemasaan:
...
<Admonition type="tip" title="Cadangan">
  - [Maklum balas klasik dan aliran kawalan](/guides/classical-feedforward-and-control-flow) (litar dinamik)
