...
이 출력 결과는 후처리 섹션에서 아래에서 논의됩니다. 지금은 시뮬레이션이 성공적으로 완료되었다는 점만 확인하면 됩니다. 이제 실제 하드웨어에서 실행할 준비가 되었습니다. resilience를 `1`로 설정하여 TREX 오류 경감(error mitigation)이 사용됨을 나타냅니다. 이제 실제 하드웨어를 사용하므로 Qiskit Runtime과 Runtime 프리미티브를 사용합니다. 기하학적 구조에 관한 for 루프와 여러 변분 시도(variational trial) 모두 Session 내부에 포함되어 있다는 점에 유의하세요.
