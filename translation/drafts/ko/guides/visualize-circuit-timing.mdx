...
## Qiskit Runtime 지원 {#qiskit-runtime-support}
...
- 이것은 실험적인 함수입니다. 미리보기 출시 상태이므로 변경될 수 있습니다.
...
요청 시, 각 PUB에 대한 Circuit 타이밍 데이터는 작업 결과 메타데이터의 `["compilation"]["scheduler_timing"]["timing"]` 아래에 반환됩니다. 이 필드에는 원시 타이밍 정보가 포함되어 있습니다. 타이밍 정보를 표시하려면 [타이밍 시각화](#visualize-timings) 섹션에 설명된 내장 시각화 도구를 사용하세요.

첫 번째 PUB의 Circuit 타이밍 데이터에 접근하려면 다음 코드를 사용하세요:
...
#### 원시 타이밍 데이터 이해하기 {#understand-the-raw-timing-data}

`draw_circuit_schedule_timing` 메서드를 사용하여 Circuit 타이밍 데이터를 시각화하는 것이 가장 일반적인 사용 사례이지만, 반환된 원시 타이밍 데이터의 구조를 이해하면 유용할 수 있습니다. 이를 통해 예를 들어 정보를 프로그래밍 방식으로 추출하는 데 도움이 될 수 있습니다.

`["compilation"]["scheduler_timing"]["timing"]`에 반환되는 타이밍 데이터는 문자열 목록입니다. 각 문자열은 특정 채널의 단일 명령어를 나타내며, 다음 데이터 타입으로 쉼표로 구분됩니다:

- `Branch` - 명령어가 제어 흐름(then / else) 또는 메인 브랜치에 있는지 결정합니다.
- `Instruction` - 작동할 Gate와 Qubit입니다.
- `Channel` - 명령어가 할당된 채널입니다. 다음 중 하나일 수 있습니다:
      - `Qubit x` - Qubit _x_의 드라이브 채널입니다.
      - `AWGRx_y` (임의 파형 생성기 판독) - Qubit 측정 시 판독 채널에서 사용됩니다. _x_ 및 _y_ 인수는 각각 판독 장비 ID와 Qubit 번호에 해당합니다.
- `T0` - 전체 스케줄 내에서 명령어의 시작 시간입니다.
- `Duration` - _dt_ 초 단위의 명령어 지속 시간으로, 1 dt = 1 스케줄링 사이클입니다. Backend의 `dt` 값은 [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt)를 사용하여 확인할 수 있습니다.
- `Pulse` - 사용 중인 펄스 작업의 유형입니다.
...
`qiskit-ibm-runtime` v0.43.0 이상에서 Circuit 타이밍을 시각화할 수 있습니다. 타이밍을 시각화하려면 먼저 [`draw_circuit_schedule_timing` 메서드](https://github.com/Qiskit/qiskit-ibm-runtime/blob/3d1bf1e1d49e5123841639fce259859c90ce9314/qiskit_ibm_runtime/visualization/draw_circuit_schedule_timings.py#L26)를 사용하여 결과 메타데이터를 `fig`로 변환해야 합니다. 이 메서드는 `plotly` 그림을 반환하며, 직접 표시하거나, 파일로 저장하거나, 둘 다 할 수 있습니다. 사용할 `plotly` 명령에 대한 자세한 정보는 [`fig.show()`](https://plotly.com/python-api-reference/generated/plotly.io.show.html)와 [`fig.write_image("<path.format>")`](https://plotly.com/python-api-reference/generated/plotly.io.write_image.html)를 참조하세요.
...
![출력 위에 마우스를 올리면 시작, 종료 및 지속 시간 등의 정보가 표시됩니다.](/docs/images/guides/visualize-circuit-timing/image_1.svg '생성된 그림 예시')
#### 생성된 그림 이해하기 {#understand-the-generated-figure}

`draw_circuit_schedule_timing`이 출력하는 Circuit 타이밍 데이터 이미지는 다음 정보를 전달합니다:

- X축은 _dt_ 초 단위의 시간이며, 1 dt = 1 스케줄링 사이클입니다. [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt)를 사용하여 Backend의 `dt` 값을 확인할 수 있습니다.
- Y축은 채널입니다(채널은 펄스를 방출하는 장치로 생각하면 됩니다).
    - `Receive channel` - 그 자체로 장치가 아닌 유일한 채널입니다. 해당 시점에 허브와의 통신 절차에 참여하는 모든 채널에서 재생되는 명령입니다.
    - `Qubit x` - Qubit x의 드라이브 채널입니다.
    - `AWGRx_y` (임의 파형 발생기 읽기) - Qubit를 측정할 때 통신하기 위해 읽기 채널에서 사용됩니다. _x_와 _y_ 인수는 각각 읽기 장치 ID와 Qubit 번호에 해당합니다.
    - `Hub` - 브로드캐스팅을 제어합니다.

또한 각 명령은 *X_Y* 형식을 가지며, *X*는 명령의 이름이고 *Y*는 펄스 유형입니다. `play` 유형은 제어 펄스를 적용하고, `capture`는 Qubit의 상태를 기록합니다. 각 명령 위에 마우스를 올려 더 자세한 정보를 확인할 수도 있습니다. 예를 들어, 이전 그림은 1161 dt에서 Qubit 10에 적용된 X Gate의 제어 펄스를 보여줍니다.
### 전체 예제 {#end-to-end-example}

이 예제에서는 옵션을 활성화하고, 메타데이터에서 Circuit 타이밍 정보를 가져오고, 이미지로 표시하는 방법을 보여줍니다.

먼저 환경을 설정하고, Circuit을 정의하고 ISA Circuit으로 변환한 다음, 작업을 정의하고 실행합니다.
