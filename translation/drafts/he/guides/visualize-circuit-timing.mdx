...
## תמיכת Qiskit Runtime {#qiskit-runtime-support}
...
- זו פונקציה ניסיונית. היא נמצאת בסטטוס שחרור תצוגה מקדימה ולכן עשויה להשתנות.
...
### הפעלת אחזור נתוני תזמון {#enable-timing-data-retrieval}
...
### גישה לנתוני תזמון ה-Circuit {#access-the-circuit-timing-data}

כאשר מתבקש, נתוני תזמון ה-Circuit עבור כל PUB מוחזרים ב-metadata של תוצאת ה-job, תחת `["compilation"]["scheduler_timing"]["timing"]`. שדה זה מכיל את מידע התזמון הגולמי. כדי להציג את מידע התזמון, השתמש בכלי ויזואליזציה המובנה, כפי שמתואר בסעיף [ויזואליזציה של התזמונים](#visualize-timings).

השתמש בקוד הבא כדי לגשת לנתוני תזמון ה-Circuit עבור ה-PUB הראשון:
...
בעוד שביצוע ויזואליזציה של נתוני תזמון ה-Circuit באמצעות מתודת `draw_circuit_schedule_timing` הוא מקרה השימוש הנפוץ ביותר, עשוי להיות שימושי להבין את מבנה נתוני התזמון הגולמיים המוחזרים. זה יכול לעזור לך, למשל, לחלץ מידע בצורה פרוגרמטית.

נתוני התזמון המוחזרים ב-`["compilation"]["scheduler_timing"]["timing"]` הם רשימה של מחרוזות. כל מחרוזת מייצגת הוראה בודדת על ערוץ מסוים ומופרדת בפסיקים לסוגי הנתונים הבאים:

- `Branch` - קובע האם ההוראה נמצאת בזרימת בקרה (then / else) או בענף ראשי.
- `Instruction` - ה-Gate וה-Qubit לפעול עליו.
- `Channel` - הערוץ המוקצה להוראה. יכול להיות אחד מהבאים:
      - `Qubit x` - ערוץ הנהגה עבור Qubit _x_.
      - `AWGRx_y` (arbitrary waveform generator readout) - משמש לערוצי קריאה לתקשורת בעת מדידת Qubit-ים. ארגומנטי _x_ ו-_y_ מתאימים למזהה מכשיר הקריאה ומספר ה-Qubit, בהתאמה.
- `T0` - זמן התחלת ההוראה בתוך לוח הזמנים המלא
- `Duration` - משך ההוראה, ביחידות של _dt_ שניות, כאשר 1 dt = מחזור תזמון אחד. ניתן למצוא את ערך ה-`dt` של Backend באמצעות [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt).
- `Pulse` - סוג פעולת ה-pulse המשמשת.
...
### ויזואליזציה של התזמונים {#visualize-the-timings}

עם `qiskit-ibm-runtime` v0.43.0 ואילך, אפשר לבצע ויזואליזציה של תזמוני Circuit. כדי לבצע ויזואליזציה של התזמונים, תחילה יש להמיר את ה-metadata של התוצאה ל-`fig` באמצעות [מתודת `draw_circuit_schedule_timing`.](https://github.com/Qiskit/qiskit-ibm-runtime/blob/3d1bf1e1d49e5123841639fce259859c90ce9314/qiskit_ibm_runtime/visualization/draw_circuit_schedule_timings.py#L26) מתודה זו מחזירה תרשים `plotly`, שניתן להציג ישירות, לשמור לקובץ, או שניהם. למידע נוסף על פקודות ה-`plotly` לשימוש, ראה [`fig.show()`](https://plotly.com/python-api-reference/generated/plotly.io.show.html) ו-[`fig.write_image("<path.format>")`.](https://plotly.com/python-api-reference/generated/plotly.io.write_image.html)
...
![ריחוף מעל הפלט מציג מידע כמו התחלה, סיום ומשך.](/docs/images/guides/visualize-circuit-timing/image_1.svg 'Example of a generated figure')
#### הבנת התרשים שנוצר {#understand-the-generated-figure}

התמונה של פלט נתוני תזמון ה-Circuit שנוצרה על ידי `draw_circuit_schedule_timing` מעבירה את המידע הבא:

- ציר ה-X הוא זמן ביחידות של _dt_ שניות, כאשר 1 dt = מחזור תזמון אחד. ניתן למצוא את ערך ה-`dt` של Backend באמצעות [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt).
- ציר ה-Y הוא הערוץ (חשוב על ערוצים כמכשירים הפולטים דפקים).
    - `Receive channel` - הערוץ היחיד שאינו מכשיר בפני עצמו. זוהי הוראה המושמעת על כל הערוצים שהם חלק מנוהל תקשורת עם ה-hub בזמן נתון.
    - `Qubit x` - ערוץ הנהגה עבור Qubit x.
    - `AWGRx_y` (arbitrary waveform generator readout) - משמש לערוצי קריאה לתקשורת בעת מדידת Qubit-ים. ארגומנטי _x_ ו-_y_ מתאימים למזהה מכשיר הקריאה ומספר ה-Qubit, בהתאמה.
    - `Hub` - שולט ב-broadcasting.

בנוסף, לכל הוראה יש פורמט של *X_Y*, כאשר *X* הוא שם ההוראה ו-*Y* הוא סוג ה-pulse. סוג `play` מחיל דפקי בקרה, ו-`capture` מתעד את מצב ה-Qubit. אפשר גם לרחף מעל כל הוראה לקבלת פרטים נוספים. לדוגמה, התרשים הקודם מציג דפק בקרה עבור Gate X המוחל על Qubit 10 ב-1161 dt.
### דוגמה מקצה לקצה {#end-to-end-example}

דוגמה זו מראה לך כיצד להפעיל את האפשרות, לקבל את מידע תזמון ה-Circuit מה-metadata ולהציג אותו כתמונה.

ראשית, הגדר את הסביבה, הגדר את ה-Circuits והמר אותם ל-ISA circuits, ולאחר מכן הגדר והרץ את ה-jobs.
...
לאחר מכן, קבל את תזמון ה-Circuit:
...
לבסוף, אפשר לבצע ויזואליזציה ולשמור את התזמון:
...
<Admonition type="tip" title="המלצות">
  - [משוב קלאסי וזרימת בקרה](/guides/classical-feedforward-and-control-flow) (Circuits דינמיים)
