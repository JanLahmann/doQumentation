...
# בניית תבנית פונקציה לסימולציית המילטוניאן {#build-a-function-template-for-hamiltonian-simulation}

תבנית זו עוטפת זרימת עבודה לסימולציית התפתחות זמן של מצב התחלתי מול המילטוניאן מבוסס ספין שמוגדר על ידי המשתמש, ומחזירה קבוצת ערכי ציפייה מוגדרים בעזרת [תוסף AQC](./qiskit-addons-aqc).
...
ראשית, כתוב תבנית פונקציה לסימולציית המילטוניאן שמשתמשת ב-[תוסף AQC-Tensor של Qiskit](/guides/qiskit-addons-aqc) כדי למפות את תיאור הבעיה למעגל בעומק מצומצם לביצוע על חומרה.

לאורך כל הדרך, הקוד נשמר ב-`./source_files/template_hamiltonian_simulation.py`. קובץ זה הוא תבנית הפונקציה שאתה יכול להעלות ולהריץ מרחוק עם Qiskit Serverless.
...
התחל בקבלת הקלטים לתבנית. לדוגמה זו יש קלטים ספציפיים לתחום הרלוונטיים לסימולציית המילטוניאן (כגון ההמילטוניאן והאופרטור) ואפשרויות ספציפיות ליכולת (כגון כמה רוצים לדחוס את השכבות הראשוניות של מעגל Trotter בעזרת AQC-Tensor, או אפשרויות מתקדמות לכוונון עדין של דיכוי ומיפחתת שגיאות מעבר לברירות המחדל שהן חלק מדוגמה זו).
...
ניתן לבחרית להשתמש ב-`save_account()` לשמירת האישורים שלך (ראה את מדריך [הגדרת חשבון IBM Cloud שלך](/guides/cloud-setup#cloud-save)). שים לב שפעולה זו כותבת את האישורים שלך לאותו הקובץ כמו [`QiskitRuntimeService.save_account()`.](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/qiskit-runtime-service#save_account)
...
לצלילה עמוקה יותר לתוסף AQC-Tensor של Qiskit, עיין בהדרכה [שיפור התפתחות זמן Trotterized עם קומפילציה קוונטית מקורבת](/tutorials/approximate-quantum-compilation-for-time-evolution) או ב[מאגר qiskit-addon-aqc-tensor](https://github.com/Qiskit/qiskit-addon-aqc-tensor).
...
<details>
<summary><b>קוד המקור המלא של התוכנית</b></summary>
