...
description: "לאורך השיעור הזה נבחן את הרעש וכיצד ניתן להפחית אותו במחשבים קוונטיים."
...
# רעש קוונטי והפחתת שגיאות

<Admonition type="note">

Toshinari Itoko (28 יוני 2024)

[הורד את ה-PDF](https://ibm.ent.box.com/public/static/a0zgies7bh91hm2lwev9o0bfeybxc6n6.zip) של ההרצאה המקורית. שים לב שחלק מקטעי הקוד עשויים להפוך למיושנים מאחר שהם תמונות סטטיות.
...
לאורך השיעור הזה נבחן את הרעש וכיצד ניתן להפחית אותו במחשבים קוונטיים. נתחיל בבחינת השפעות הרעש באמצעות סימולטור שיכול לדמות רעש בכמה דרכים, כולל שימוש בפרופילי רעש ממחשבים קוונטיים אמיתיים. לאחר מכן נעבור למחשבים קוונטיים אמיתיים, שבהם הרעש הוא מובנה. נבחן את השפעות הפחתת השגיאות, כולל שילובים של דברים כמו אקסטרפולציה לאפס-רעש (ZNE) ו-gate-twirling.
...
## 2. סימולציה עם רעש ללא הפחתת שגיאות {#noisy-simulation-without-error-mitigation}

Qiskit Aer הוא סימולטור קלאסי לחישוב קוונטי. הוא יכול לדמות לא רק ביצוע אידיאלי אלא גם ביצוע עם רעש של Circuit-ים קוונטיים. ה-notebook הזה מדגים כיצד להריץ סימולציה עם רעש באמצעות Qiskit Aer:

1. בנה מודל רעש
2. בנה Sampler עם רעש (סימולטור) עם מודל הרעש
3. הרץ Circuit קוונטי על ה-Sampler עם הרעש
...
אנו שוקלים Circuit-ים צעצוע של Qubit בודד שפשוט חוזרים על Gate-ים X פעמים `d` (`d`=0 ... 100) ומודדים את האובסרבבל `Z`.
...
כדי לבצע סימולציה עם רעש, עלינו לציין `NoiseModel`. נראה כיצד לבנות `NoiseModel` בחלק הזה.
תחילה עלינו להגדיר שגיאות קוונטיות (או שגיאות קריאה) להוספה למודל הרעש.
...
### 2.3 בנה Sampler עם רעש עם מודל הרעש {#build-a-noisy-sampler-with-the-noise-model}
...
### 2.4 הרץ Circuit-ים קוונטיים על ה-Sampler עם הרעש {#run-quantum-circuits-on-the-noisy-sampler}
...
על ידי שינוי הקוד למטה,
- [ ] נסה 25 פעמים יותר shots (= 10,000 shots) וודא שמתקבל גרף חלק יותר
...
### 2.8 סימולציה עם רעש ריאליסטי יותר {#more-realistic-noisy-simulation}
...
## 3. חישוב קוונטי אמיתי עם הפחתת שגיאות {#real-quantum-computation-with-error-mitigation}

בחלק זה נדגים כיצד לקבל תוצאות עם הפחתת שגיאות (ערכי ציפייה) באמצעות Qiskit Estimator.
אנו מתייחסים ל-Circuit של מודל איזינג חד-ממדי עם Trotterization בעל 6 Qubit ורואים כיצד השגיאה מתרחבת ביחס למספר צעדי הזמן.
...
כדי לדעת מראש מה הפלט האידיאלי, אנו משתמשים ב-Circuit-ים מסוג compute-uncompute המורכבים משלב ראשון שבו מיושם ה-Circuit המקורי $U$, ושלב שני שבו הוא מהופך $U^\dagger$.
שימו לב שהתוצאה האידיאלית של Circuit-ים כאלה תהיה באופן טריוויאלי מצב הקלט $|000000\rangle$, שיש לו ערכי ציפייה טריוויאליים לכל אובזרבבל Pauli, למשל, $\langle IIIIIZ \rangle = 1$.
...
הערה: כפי שמוצג למעלה, ל-Circuit עם $k$ צעדי זמן יהיו $4k$ שכבות של Gate-ים דו-Qubit.
...
אנו מבצעים Transpile של ה-Circuit-ים עבור ה-Backend עם אופטימיזציה (`optimization_level=1`).
...
### 3.3 הרצה באמצעות Estimator (עם רמות חוסן שונות) {#33-execute-using-estimator-with-different-resilience-levels}

הגדרת רמת החוסן (`estimator.options.resilience_level`) היא הדרך הקלה ביותר ליישם הפחתת שגיאות בעת שימוש ב-Qiskit Estimator. ה-Estimator תומך ברמות החוסן הבאות (נכון ל-28/06/2024). ראו פרטים נוספים במדריך [הגדרת הפחתת שגיאות](/guides/configure-error-mitigation).
...
### 3.4 הצגת תוצאות {#34-plot-results}
...
ניתן להתאים אישית את יישום טכניקות הפחתת השגיאות דרך אפשרויות כפי שמוצג להלן.
...
ראו את המדריכים הבאים ואת ה-API reference לפרטים על אפשרויות הפחתת השגיאות.
