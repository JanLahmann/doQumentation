---
title: "Primeros pasos con OBP"
sidebar_label: "Primeros pasos con OBP"
description: "Primeros pasos con el addon de Qiskit para retropropagación de operadores (OBP)"
...
y el valor esperado a medir será $\langle Z_0 \rangle$.

El siguiente fragmento de código genera el Hamiltoniano en forma de `SparsePauliOp` usando el módulo `qiskit_addons_utils.problem_generators` y un `CouplingMap`. Establece las constantes de acoplamiento en $J_x=\pi/8$, $J_y=\pi/4$, $J_z=\pi/2$ y los campos magnéticos externos en $h_x=\pi/3$, $h_y=\pi/6$, $h_z=\pi/9$, y luego genera un circuito que modela su evolución temporal.
...
![Salida de la celda de código anterior](/docs/images/guides/qiskit-addons-obp-get-started/extracted-outputs/5b4fd077-2d8f-46e8-9631-40cadbe07879-0.svg)

### Preparar las entradas para retropropagar {#prepare-inputs-to-backpropagate}
...
Aquí, especifica que la retropropagación debe detenerse cuando el número de grupos de Pauli que conmutan qubit a qubit en el operador supere $8$, y asigna un presupuesto de error de $0.005$ por segmento.
...
### Retropropagar los segmentos {#backpropagate-slices}
...
<Admonition title="Nota" type="note">
  Por defecto, `backpropagate()` usa la norma $L_1$ de los coeficientes truncados para acotar el error total incurrido por el truncamiento, pero se pueden usar otras normas $L_p$ si deseas modificar cómo se calcula el error de truncamiento.
</Admonition>
...
![Salida de la celda de código anterior](/docs/images/guides/qiskit-addons-obp-get-started/extracted-outputs/243681df-1234-4b83-9139-16b2b8c88308-1.svg)
...
![Salida de la celda de código anterior](/docs/images/guides/qiskit-addons-obp-get-started/extracted-outputs/ab852775-229d-4652-9456-56c4dc8e628c-1.svg)

### Transpilar y ejecutar la carga de trabajo cuántica {#transpile-and-execute-quantum-workload}
...
- Prueba el tutorial sobre el uso de [OBP para mejorar los valores esperados.](/tutorials/operator-back-propagation)
</Admonition>
