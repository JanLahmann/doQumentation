---
title: "Exemplos de primitivas"
sidebar_label: "Exemplos de primitivas"
description: "Exemplos práticos de uso de primitivas no Qiskit Runtime."
...
A versão beta de um novo modelo de execução já está disponível. O modelo de execução direcionada oferece mais flexibilidade para personalizar seu fluxo de mitigação de erros. Consulte o guia [Modelo de execução direcionada](/guides/directed-execution-model) para mais informações.
</Admonition>
{/*
...
Os exemplos nesta seção ilustram algumas formas comuns de usar primitivas. Antes de executar esses exemplos, siga as instruções em [Instalar e configurar.](install-qiskit)

<Admonition type="note">
    Todos esses exemplos utilizam as primitivas do Qiskit Runtime, mas você pode usar as primitivas base no lugar delas.
</Admonition>

## Exemplos com o Estimator {#estimator-examples}

Calcule e interprete com eficiência os valores esperados dos operadores quânticos necessários para muitos algoritmos com o Estimator. Explore usos em modelagem molecular, aprendizado de máquina e problemas de otimização complexos.

### Executar um único experimento {#run-a-single-experiment}

Use o Estimator para determinar o valor esperado de um único par circuito-observável.
...
Use o Estimator para determinar os valores esperados de múltiplos pares circuito-observável.
...
### Executar circuitos parametrizados {#run-parameterized-circuits}

Use o Estimator para executar três experimentos em um único job, aproveitando valores de parâmetros para aumentar a reutilização dos circuitos.
...
Explore sessões e opções avançadas para otimizar o desempenho dos circuitos em QPUs.

<Admonition type="caution">
O bloco de código a seguir retornará um erro para usuários do Plano Open porque ele usa sessões. As cargas de trabalho do Plano Open só podem ser executadas no [modo de job](/guides/execution-modes#job-mode) ou no [modo de lote](/guides/execution-modes#batch-mode).
</Admonition>
...
## Exemplos com o Sampler {#sampler-examples}

Gere distribuições completas de quase-probabilidade com mitigação de erros, amostradas a partir das saídas de circuitos quânticos. Aproveite as capacidades do Sampler para algoritmos de busca e classificação como o de Grover e o QVSM.

### Executar um único experimento {#run-a-single-experiment-sampler}

Use o Sampler para retornar os resultados de medição como bitstrings ou contagens de um único circuito.
...
Use o Sampler para retornar os resultados de medição como bitstrings ou contagens de múltiplos circuitos em um único job.
...
### Executar circuitos parametrizados {#run-parameterized-circuits-sampler}

Execute vários experimentos em um único job, aproveitando valores de parâmetros para aumentar a reutilização dos circuitos.
...
<Admonition type="tip" title="Recomendações">
    ...
    - Pratique com primitivas trabalhando na [lição de funções de custo](/learning/courses/variational-algorithm-design/cost-functions) no IBM Quantum Learning.
    - Aprenda como transpor localmente na seção [Transpile](./transpile/).
    ...
    - Entenda os [Limites de jobs](/guides/job-limits) ao enviar um job para um QPU da IBM&reg;.
</Admonition>
