...
description: "Entenda o formato de entrada e saída (incluindo os Primitive Unified Blocs ou PUBs) dos primitivos"
...
A versão beta de um novo modelo de execução já está disponível. O modelo de execução dirigida oferece mais flexibilidade ao personalizar seu fluxo de trabalho de mitigação de erros. Consulte o guia [Modelo de execução dirigida](/guides/directed-execution-model) para mais informações.
</Admonition>
{/*
...
A1     (array 1d):      1
A2     (array 2d):  3 x 5
Result (array 2d):  3 x 5

A1     (array 3d):  11 x 2 x 7
A2     (array 3d):  11 x 1 x 7
Result (array 3d):  11 x 2 x 7
```
...
A1     (array 2d):      2 x 1
A2     (array 3d):  6 x 5 x 4 # Funcionaria se a dimensão do meio fosse 2, mas é 5.
```
...
#### Como o Estimator calcula o erro {#how-the-estimator-calculates-error}
...
Além dos resultados de execução, os objetos `PrimitiveResult` e `PubResult` contêm um atributo de metadados sobre o job submetido. Os metadados contendo informações para todos os PUBs submetidos (como as diversas [opções de runtime](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options) disponíveis) podem ser encontrados em `PrimitiveResult.metatada`, enquanto os metadados específicos de cada PUB são encontrados em `PubResult.metadata`.
...
Para jobs do Sampler, você também pode revisar os metadados do resultado para entender quando certos dados foram executados; isso é chamado de [_intervalo de execução._](monitor-job#execution-spans)
