...
Vamos usar essas instâncias de backend para obter algumas das informações que você vê na página de [Recursos de computação](https://quantum.cloud.ibm.com/computers)
na IBM Quantum&reg; Platform. Primeiro, criamos uma instância de backend para um dispositivo de interesse. No exemplo a seguir, escolhemos `"ibm_kyoto"`, `"ibm_kawasaki"` ou a máquina Eagle menos ocupada. Seu acesso a QPUs pode ser diferente; atualize o nome do backend conforme necessário.

```python
service = QiskitRuntimeService()
# backend = service.backend("ibm_kawasaki") # um Eagle, se você tiver acesso ao ibm_kawasaki
...
```python
a_heron = service.backend("ibm_strasbourg")  # um Heron
...
```python
# Esta função requer que o Graphviz esteja instalado. Se você precisar instalar o Graphviz, consulte:
# https://graphviz.org/download/#executable-packages para instruções.
try:
...
Tendo compreendido as estruturas de dados, estamos prontos para calcular os erros medianos para as portas `'sx'` e `'ecr'`. Mais uma vez, compare os resultados com os exibidos para o dispositivo na [IBM Quantum Platform.](https://quantum.cloud.ibm.com/)
