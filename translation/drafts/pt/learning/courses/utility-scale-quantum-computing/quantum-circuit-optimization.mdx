...
# Coleta blocos de 2q e consolida em unitária quando se espera reduzir a contagem de portas de 2q para essa unitária
...
```python
sampler.run([circ])  # IBMInputValueError será lançado
```
...
```python
# SUBSTITUA PELOS SEUS PRÓPRIOS JOB IDs
...
```python
# Circuito GHZ original (síntese ingênua)
ghz_circ.draw("mpl")
```
...
```python
# Um circuito GHZ melhor (síntese mais inteligente), que você aprendeu em uma aula anterior
ghz_circ2 = QuantumCircuit(5)
ghz_circ2.h(2)
ghz_circ2.cx(2, 1)
ghz_circ2.cx(2, 3)
