...
#### Benefícios dos clusters para reduzir o overhead de amostragem para um dado tempo de computação clássica
...
### c. Execute o circuito de interesse com antirruído
#### c.i. Prepare o circuito template usando `samplex`
O `samplex` é uma saída do método `build` do Samplomatic, que codifica todas as informações necessárias para gerar parâmetros aleatorizados para o `template_circuit`. Esses parâmetros são então usados para configurar os objetos `QuantumProgram`, que por sua vez são executados em uma QPU com o primitivo `Executor`. Cada `QuantumProgram` pode conter vários itens, que você pode imaginar como um par de `template` e `samplex`.
...
#### c.iii. Execute o programa com o primitivo `Executor`
