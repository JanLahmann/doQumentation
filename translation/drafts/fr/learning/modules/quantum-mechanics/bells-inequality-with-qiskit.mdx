...
Mache dasselbe für $P_{--}$. Verifiziere, dass es ebenfalls $\frac{1}{2}\sin^2(\theta)$ ergibt.
...
Comment pourrais-tu vérifier les maths de ce résultat ? Pour être clair, on ne te demande pas encore de vérifier qu'il correspond à la nature, juste de s'assurer que rien ne s'est mal passé dans tous les calculs.
...
(2) Vérifie un cas connu. Insère $\theta = 0$. Alors $P_{\text{same}}$ correspond aux deux observateurs mesurant chacun leur spin selon le même axe, ce qui violerait la conservation du moment cinétique. Donc on s'attendrait à ce que cette probabilité soit nulle, et en effet l'insertion de $\theta = 0$ donne $\sin^2(0/2) = 0$.

(3) Vérifie un autre cas connu. Essaie $\theta = \pi$. Que dois-tu obtenir ? Attention à ce $\frac{1}{2}$.
</details>

On était spécifiquement en train d'esquisser le cas où les axes sont à $120\deg$ les uns des autres. Rappelle-toi, quelle que soit la direction ($\pm a$, $\pm b$, ou $\pm c$) que Lucas obtient, on l'appelle $z$. Ensuite Rihanna choisit aléatoirement de mesurer selon $\pm a$, $\pm b$, ou $\pm c$. Si son choix est le même que celui de Lucas (à un signe près), ils mesurent tous deux selon $z$, et la probabilité que Rihanna mesure aussi $+z$ est nulle. Cela devrait se produire 1/3 du temps, puisque le choix d'axe de Rihanna est indépendant de celui de Lucas. Pour tout autre choix, Rihanna mesurera selon un axe soit à $120\deg = 2\pi/3$ radians de $z$ (1/3 du temps) soit à $240\deg = 4\pi/3$ radians de $z$ (1/3 du temps). Et bien sûr, selon l'un ou l'autre de ces axes, le spin pourrait être mesuré dans la direction positive ou négative. Cela nous donne une probabilité totale que Lucas et Rihanna obtiennent le même signe :
...
Les options d'Einstein et de Born semblaient devoir toujours donner les mêmes résultats, puisqu'elles ne différaient que dans leur description de ce qui se passe avant la mesure. Et pourtant, en supposant qu'il y avait des instructions prédéterminant le signe de la mesure de spin selon certains axes, on a obtenu une contrainte sur la probabilité que les mesures donnent le même signe $(P_{\text{same}})_\text{Einstein}\leq\frac{4}{9}$. Puis on a supposé des distributions de probabilité comme en mécanique quantique... et on a obtenu une valeur différente pour $(P_{\text{same}})_\text{Born}=\frac{1}{2}$. La prédiction de la mécanique quantique est plus élevée que celle permise par le traitement par variables cachées. On peut donc faire une expérience et découvrir si les états quantiques sont déterminés par la nature avant la mesure, ou s'ils sont véritablement dans une superposition probabiliste d'états possibles.

Cette expérience a été réalisée de nombreuses fois en utilisant de nombreux systèmes physiques différents, souvent des photons. Il y a de nombreuses considérations subtiles, comme les biais de mesure, la simultanéité des mesures, et bien d'autres. Au fil des décennies, les préoccupations concernant ces subtilités se sont régulièrement érodées. Des tests sont encore réalisés, à mesure qu'on en apprend davantage sur la réalité, mais il y a maintenant un large consensus que la réponse que tu obtiendras ici, en utilisant les ordinateurs quantiques IBM&reg;, est correcte.
## Teste avec de vrais ordinateurs quantiques ! {#test-using-real-quantum-computers}

Conformément à notre traitement ci-dessus, définissons la direction de la mesure de Lucas comme étant $+z$. C'était pratique même dans l'approche algébrique, mais c'est particulièrement pratique pour le calcul quantique, car ce qui est généralement mesuré est la projection du Qubit selon $z$.
On veut créer un Circuit quantique qui nous donne les mêmes conditions de probabilité que celles ci-dessus pour $P_{++}$. On est libre d'orienter notre plan de sorte que $\phi=0$, et on obtient
$$
P_{++}=\left|  \left(_L\langle+|_R\left(\cos(\theta/2)\langle+|+\sin(\theta/2)\langle-|\right)\right)  \frac{1}{\sqrt{2}}\left(|+\rangle_L|-\rangle_R-|-\rangle_L|+\rangle_R\right)    \right|^2
$$
On doit savoir quelques choses sur les ordinateurs quantiques IBM, pour guider notre discussion. Premièrement, les Qubits sont initialisés dans l'état $|0\rangle = |+\rangle_z$. Comme mentionné précédemment, lorsque des mesures sont effectuées, elles sont selon l'axe $z$. Donc l'objectif est de déterminer quels opérateurs on peut insérer entre les états de base de mesure $\langle 0|\langle 0|$ et les états initiaux des Qubits $|0\rangle |0\rangle$ pour obtenir l'expression compliquée ci-dessus. Pour cela, on devra passer en revue quelques Gates de base en calcul quantique.

Gate $X$ : Équivalent à une opération NOT. Gate à un seul Qubit.
$$
X|0\rangle = |1\rangle,\\X|1\rangle=|0\rangle
$$
$$
...
Gate de Hadamard $H$ : Crée un état de superposition. Gate à un seul Qubit.
$$
H|0\rangle = \frac{1}{2}\left(|0\rangle+|1\rangle\right),
$$
$$
...
Gate CNOT (NOT contrôlé) : Ce Gate utilise deux Qubits : un contrôle et une cible. Il vérifie l'état d'un Qubit de contrôle qui n'est pas modifié. Mais si le Qubit de contrôle est dans l'état $|1\rangle$, le Gate change l'état du Qubit cible ; si l'état du Qubit de contrôle est $|0\rangle$, aucun changement n'est effectué. Dans la notation ci-dessous, suppose que le premier Qubit est le contrôle, et le second est la cible.
$$
CNOT|00\rangle = |00\rangle, \\
CNOT|01\rangle = |01\rangle \\
CNOT|10\rangle = |11\rangle \\
...
Un Gate CNOT a une apparence un peu différente dans un Circuit, car il nécessite deux Qubits. Voici comment il est implémenté :
...
Note que le premier Qubit listé dans `qc.cx(0,1)` est le contrôle, et le second est la cible. Diagrammatiquement, la cible est celle avec le signe « + » ou la croix dessus.

Gate de rotation Y $R_y(\theta)$ : Fait tourner l'état autour de l'axe y. C'est un Gate à un seul Qubit.
$$
R_y(\theta)|0\rangle = \cos(\theta/2)|0\rangle+\sin(\theta/2)|1\rangle,\\R_y(\theta)|0\rangle = -\sin(\theta/2)|0\rangle+\cos(\theta/2)|1\rangle
$$
$$
...
Enfin, les Gates de rotation sont implémentées en spécifiant le type de Gate, la quantité de rotation, et le Qubit sur lequel le Gate est placé, dans cet ordre :
...
Le nom du Gate `ry` spécifie l'axe autour duquel la rotation se produit. Le premier argument $\pi/2$ fait référence à la quantité de rotation, et le second argument spécifie le Qubit sur lequel le Gate doit être placé.
...
En utilisant la syntaxe introduite ou rappelée ci-dessus, crée n'importe quel Circuit quantique impliquant quatre types différents de Gates quantiques.

</summary>

__Réponse :__

Il y a bien sûr infiniment de possibilités. Voici un exemple :
...
D'après les opérations de ces Gates, on peut voir, par exemple, que les kets dans les expressions de $P_{++}$ :
$$
\frac{1}{\sqrt{2}}\left(|+\rangle_L|-\rangle_R-|-\rangle_L|+\rangle_R\right)
$$
sont susceptibles de faire intervenir un Gate de Hadamard pour obtenir la superposition, et un Gate CNOT pour créer l'intrication.

On va maintenant utiliser les Gates H, X et CNOT pour transformer $|0\rangle_L|0\rangle_R$ en $\frac{1}{\sqrt{2}}\left(|+\rangle_L|-\rangle_R-|-\rangle_L|+\rangle_R\right)$ :
...
\frac{1}{\sqrt{2}}CNOT_{LR}\left(|0\rangle_L|1\rangle_R-|1\rangle_L|1\rangle_R\right)
$$
Ici $CNOT_{LR}$ signifie un Gate CNOT utilisant L comme contrôle et R comme cible. On peut maintenant factoriser la partie R de l'état :
$$
\text{CNOT}_{LR}\frac{1}{\sqrt{2}}\left(|0\rangle_L-|1\rangle_L\right)|1\rangle_R
$$
$$
...
On a maintenant écrit le ket entièrement sous forme de Gates quantiques opérant sur l'état de départ par défaut des Qubits.

On peut maintenant utiliser le $R_y(\theta)$ agissant sur $\vphantom{p}_L\langle 0|_R\langle 1|$ pour obtenir le bra dans l'expression de $P_{++}$.
...
En combinant ces résultats, on peut écrire la probabilité $P_{++}$ comme

$$
p_{++}=\left|\vphantom{p}_L\langle0|_R\langle0|R_{y,R}(-\theta)\text{CNOT}_{LR} H_L X_L X_R|0\rangle_L|0\rangle_R\right|^2
$$

Cela nous donne des instructions explicites pour construire notre Circuit quantique. On va appliquer des Gates X, H, CNOT et $R_y$ sur des Qubits représentant les états quantiques des particules mesurées par Lucas et Rihanna, et faire des mesures pour obtenir la probabilité.

IBM Quantum recommande de s'attaquer aux problèmes de calcul quantique en utilisant un cadre qu'on appelle les patterns Qiskit. Il se compose des étapes suivantes.
- Étape 1 : Associer ton problème à un Circuit quantique
- Étape 2 : Optimiser ton Circuit pour le faire tourner sur du vrai matériel quantique
...
Fondamentalement, tout le travail qu'on a fait ci-dessus était l'étape 1. Construisons le Circuit résultant à l'aide de Qiskit !
### Étape 1 : Associer nos résultats à un Circuit quantique {#step-1-mapping-our-results-to-a-quantum-circuit}
...
Rappelle-toi que 1/3 du temps l'axe choisi par Rihanna sera à $2\pi/3$ radians de celui de Lucas, 1/3 du temps il sera à $4 \pi/3$ radians de celui de Lucas, et 1/3 du temps, ils choisiront le même axe. On doit donc créer 3 Circuits quantiques pour ces 3 cas, et additionner les résultats. On va soigneusement expliquer le premier, et les deux derniers on va simplement les énoncer.
...
Le code ci-dessous construit rapidement les trois Circuits de manière plus rationalisée. Note que la seule différence entre les trois Circuits est de combien on fait tourner les deux Qubits autour de l'axe $y$.
...
On va maintenant utiliser une primitive Qiskit appelée le StatevectorSampler. Un sampler est une primitive conçue pour échantillonner tous les états possibles d'un système et retourner des probabilités (ou dans certains cas, des quasi-probabilités) d'obtenir chaque état. On peut spécifier un nombre de « shots », et examiner les « counts » pour chaque état.
...
Si on regarde les counts de chaque Circuit, on voit que deux d'entre eux étaient quasiment identiques, et le troisième était assez différent.
...
Faisons une liste des résultats possibles et additionnons tous les counts de chaque état de chacun des trois Circuits pour obtenir les probabilités globales.
...
On peut maintenant afficher les counts totaux pour chaque résultat, et tracer l'histogramme.
...
Non ! On utilisait un simulateur ! C'est un ordinateur programmé pour se comporter selon les lois de la mécanique quantique probabiliste. Si on propose une règle, puis programme un ordinateur pour suivre cette règle, sa capacité à suivre la règle n'est pas une preuve que la règle est correcte ! La seule façon de le prouver est d'utiliser un vrai ordinateur quantique !

</details>
### Étape 2 : Optimiser ton Circuit quantique pour le faire tourner sur du vrai matériel {#step-2-optimize-your-quantum-circuit-for-running-on-real-hardware}

Bien qu'on ait d'abord utilisé un simulateur pour déboguer notre code, on veut vraiment faire tourner sur du vrai matériel. Après tout, un simulateur fait juste semblant d'être de la mécanique quantique, d'après les équations ci-dessus. Si le simulateur nous disait que ces équations étaient correctes, cela ne nous convaincrait pas beaucoup. On veut qu'un vrai ordinateur quantique nous dise ce qui se passe ! On va donc sélectionner l'ordinateur quantique qu'on veut utiliser. Parfois il peut être important de choisir un dispositif spécifique qui a les propriétés voulues, mais souvent on veut simplement utiliser le dispositif le moins occupé.
...
### Étape 3 : Exécuter ton job sur les ordinateurs quantiques IBM en utilisant les Runtime Primitives {#step-3-execute-your-job-on-ibm-quantum-computers-using-runtime-primitives}

Maintenant qu'on a optimisé nos Circuits pour les faire tourner sur du vrai matériel quantique, et débogué notre code en utilisant des simulateurs, on est prêts à recueillir des statistiques d'un vrai ordinateur quantique, et à régler le désaccord entre Einstein et Born.
...
Prenons du recul et résumons : En utilisant un traitement par variables cachées et les 3 axes décalés, on a obtenu une contrainte sur la probabilité que les mesures donnent le même signe $P_{same,hv} =4/9$. Puis on a supposé des distributions de probabilité comme en mécanique quantique et on a obtenu une __valeur différente__ pour cette probabilité : $P_{same,gm} = 1/2$. La prédiction de la mécanique quantique est plus élevée que celle permise par le traitement par variables cachées. On peut donc savoir expérimentalement si les états quantiques sont déterminés par la nature avant la mesure, ou s'ils sont véritablement dans une superposition probabiliste d'états possibles.

On a conçu nos Circuits quantiques de sorte qu'il y ait quatre résultats possibles correspondant à Lucas et Rihanna mesurant un signe ou l'autre de la projection du spin : 00, 01, 10 et 11. Dans les cas de 00 et 11, Lucas et Rihanna mesurent le même signe, et dans les cas de 01 et 10, ils mesurent des signes opposés. On voit qu'avec une très bonne approximation, la chance que Lucas et Rihanna obtiennent le même signe est d'environ 50%, définitivement supérieure à $4/9$. Cela signifie qu'il n'existe pas d'ensemble de variables cachées pouvant expliquer cette distribution de probabilités, et le traitement par variables cachées n'est pas compatible avec l'expérience.

Il y a différentes interprétations des résultats expérimentaux de la mécanique quantique, et il y a de nombreuses subtilités dans les dispositifs expérimentaux qui sont réexaminées de temps en temps. Mais jusqu'à présent, les principes de la mécanique quantique et l'interprétation probabiliste des états quantiques ont décrit avec précision les résultats. Max Born semble avoir eu raison.

Prenons juste un moment de plus pour réfléchir à l'importance de ceci. Deux particules émergent d'un événement de désintégration, et les deux particules voyagent dans des directions différentes, possiblement pendant longtemps. Leurs spins ne sont dans aucun état bien défini, et elles ne transportent pas d'instructions de variables cachées pour déterminer les résultats de futures mesures. Mais une mesure de l'une (selon, disons $+z$) détermine nécessairement le résultat d'une expérience sur le spin de l'autre particule selon la direction $z$ (il doit être $-z$). Cela signifie que quelque chose dans la physique d'une particule est déterminé par ce qui est fait à l'autre particule, potentiellement loin. C'est l'une des situations qui a amené les gens à qualifier la réalité de « non-locale ».

Deux particules comme celles qu'on a décrites sont en quelque sorte « connectées » dans le sens où les mesures sur l'une peuvent influencer l'autre. On appelle ces particules « intriquées ». L'intrication est plus que de simples corrélations. Par exemple, on pourrait construire une machine classique qui crache un aimant d'un côté avec son pôle nord vers le haut et un aimant de l'autre côté avec son pôle nord vers le bas. Ces aimants pourraient être parfaitement anti-corrélés. Mais une mesure de l'un ne ferait rien à l'autre. Dans l'intrication mécanique quantique, la particule A pourrait être dans un état indéfini (ou un mélange de nombreux états), et on peut la fixer dans un état défini grâce à des mesures sur une particule totalement différente (disons, B). Rien de tel n'existe dans le monde classique.

Cela soulève souvent tout un nouveau monde de questions et de possibilités. Certaines des idées que cela évoque sont réelles, comme l'utilisation de l'intrication pour calculer, comme dans les ordinateurs quantiques ! Certaines sont séduisantes en apparence, mais s'avèrent échouer, comme tenter d'utiliser l'intrication pour envoyer des informations plus vite que la lumière. On t'encourage à poser toutes les questions qui te viennent à l'esprit, et à lire comment d'autres ont investigué ces phénomènes. Il y a tout un monde de mécanique quantique à explorer, mais voici quelques ressources que tu pourrais consulter :

Cours IBM Quantum :
- [Le calcul quantique en pratique](/learning/courses/quantum-computing-in-practice)
- [Les bases de l'information quantique](/learning/courses/basics-of-quantum-information)

Articles intéressants de mécanique quantique :
- [Le paradoxe Einstein Podolsky et Rosen](https://cds.cern.ch/record/405662/files/PhysRev.47.777.pdf)
- [L'article original de John Bell de 1964](https://cds.cern.ch/record/111654/files/vol1p195-200_001.pdf)
- [Article de 2019 sur la capture et l'inversion d'un « saut quantique » en pleine transition](https://www.nature.com/articles/s41586-019-1287-z)

Quelques ressources pédagogiques de mécanique quantique :
- [Matériaux du cours Quantum I](https://www.colorado.edu/sei/departments/physics/activities/courses/quantum-i-course-materials) de l'Université du Colorado.

Quelques recherches pédagogiques en mécanique quantique :
- [Revue des difficultés des étudiants en mécanique quantique de niveau avancé](https://journals.aps.org/prper/pdf/10.1103/PhysRevSTPER.11.020117) par C. Singh et E. Marshman
## Questions {#questions}

Les enseignant·e·s peuvent demander des versions de ces notebooks avec des corrigés et des conseils sur le placement dans les programmes habituels en remplissant ce [court sondage](https://ibm.biz/classrooms_instructor_key_request) sur la façon dont les notebooks sont utilisés.

### Concepts critiques : {#critical-concepts}

- Il y avait un désaccord historique sur la question de savoir si les états quantiques étaient simplement inconnus ou indéterminés par la nature avant la mesure, si la mécanique quantique est déterministe ou probabiliste.
- Les variables cachées et donc le réalisme local ne sont pas cohérents avec les observations de la mécanique quantique. C'est-à-dire que les corrélations observées en mécanique quantique ne peuvent pas être expliquées par des variables bien définies qui nous sont simplement inconnues.
- La mécanique quantique est probabiliste.
- L'intrication est réelle et observable.
- L'intrication n'est pas seulement des corrélations.
- On peut associer des scénarios du monde réel à des ordinateurs quantiques.
- Les variables cachées font référence à des quantités spécifiées par la nature, mais inconnues des humains ; elles n'existent pas dans ce contexte.

### Questions V/F : {#tf-questions}

1. V/F Albert Einstein a soutenu que la mécanique quantique était incomplète, en tant que théorie, parce qu'elle ne décrivait que les probabilités des résultats, et non le mécanisme sous-jacent qui déterminait ces résultats.
2. V/F « Variables cachées » fait référence à l'idée que deux particules mécaniques quantiques peuvent être intriquées.
3. V/F Deux systèmes corrélés quelconques sont quantiquement intriqués.
4. V/F L'intrication mécanique quantique est importante pour avoir des maths justes, mais on ne peut pas la voir dans une expérience.
5. V/F Dans la plupart des cas, la mécanique quantique ne peut pas te donner un résultat exact d'une expérience, seulement les probabilités que certains résultats seront mesurés.
...
7. V/F On peut associer des expériences du monde réel à des Circuits quantiques.

### Questions à choix multiples : {#mc-questions}

1. Suppose qu'une particule de spin-0 se désintègre en deux particules de spin-1/2 A et B. Une mesure est effectuée sur la particule A qui révèle que son spin a une projection selon $+z$. La particule B
    - a. a définitivement une projection de spin selon $-z$
    - b. a définitivement une projection de spin selon $-x$
    - c. a définitivement une projection de spin selon $-y$
    - d. a définitivement une projection de spin négative selon n'importe quel axe mesuré.

2. Une particule de spin-0 se désintègre en deux particules de spin-1/2 A et B. Si la particule A est mesurée avec une projection selon $+z$, lesquelles des projections suivantes sont possibles pour une mesure de la particule B ? Encercle toutes celles qui s'appliquent.
    - a. $+x$
    - b. $-x$
    - c. $+y$
    - d. $-y$
...
3. Suppose qu'une particule de spin-0 se désintègre en deux particules de spin-1/2 A et B. Qu'est-ce qui décrit le mieux l'état de la particule A avant toute mesure ?
    - a. Le spin de la particule A est selon $+z$.
    - b. Le spin de la particule A est selon $-z$.
    - c. Le spin de la particule A est selon $+x$.
    - d. Le spin de la particule A est défini selon certaines directions, mais pas d'autres.
    - e. L'orientation du spin de la particule A est indéterminée par la nature avant toute mesure.

4. Lesquelles des affirmations suivantes sont vraies du Gate de Hadamard ? Sélectionne toutes celles qui s'appliquent.
    - a. $H|0\rangle = \frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)$
    - b. $H|0\rangle = \frac{1}{\sqrt{2}}(|0\rangle-|1\rangle)$
    - c. $H \frac{1}{\sqrt{2}}(|0\rangle-|1\rangle)=|0\rangle$
    - d. $H \frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)=|0\rangle$

5. Lesquelles des affirmations suivantes sont vraies du Gate X ? Sélectionne toutes celles qui s'appliquent.
    - a. $X|0\rangle = |1\rangle$
    - b. $X|0\rangle = \frac{1}{\sqrt{2}}(|0\rangle-|1\rangle)$
    - c. $X|0\rangle = -|0\rangle$
    - d. $X|1\rangle = |0\rangle$
...
6. Lequel des éléments suivants est un Gate à deux Qubits ?
    - a. X
    - b. $R_y(\theta)$
    - c. H
    - d. CNOT

7. Suppose qu'un Qubit est dans l'état $|0\rangle$. Quelle est la probabilité de le mesurer dans l'état $|1\rangle$ ?
    - a. Exactement 100% sur un simulateur sans bruit, proche de 100% sur un vrai ordinateur quantique
    - b. Proche de 100% sur un simulateur sans bruit, exactement 100% sur un vrai ordinateur quantique
    - c. Exactement 0% sur un simulateur sans bruit, proche de 0% sur un vrai ordinateur quantique
    - d. Proche de 0% sur un simulateur sans bruit, exactement 0% sur un vrai ordinateur quantique

### Questions de discussion : {#discussion-questions}

1. Les ami·e·s A, B et C discutent des résultats de ce laboratoire, liés à l'inégalité de Bell. Plus précisément, ils regardent l'image qui montre que la probabilité mécanique quantique de mesurer le même signe selon les axes est supérieure à celle permise par un traitement par variables cachées : $(P_\text{same})_\text{max,QM}>(P_\text{same})_\text{max,HV}$. L'ami·e A dit : « Cela signifie qu'on ne connaissait pas les états de spin avant une mesure. » L'ami·e B dit : « Non, c'est plus que ça. Cela signifie que les spins ne pointent pas déjà dans une direction particulière, avant la mesure. Bien que l'état de spin puisse être d'une certaine manière déterminé ou stocké quelque part. » L'ami·e C dit : « Non, c'est encore plus que ça. Cela signifie que le futur état de spin n'était même pas décidé par la nature avant la mesure. » Avec qui es-tu d'accord, et pourquoi ?

2. Explique comment des phénomènes mécaniques quantiques comme l'intrication indiquent que la réalité est non-locale.

3. Quelles expériences supplémentaires voudrais-tu faire pour te convaincre des résultats obtenus ici ?

4. L'inégalité de Bell ne pourrait-elle être explorée qu'avec 3 axes équidistants $a$, $b$ et $c$ ? Pourrait-on le faire avec un autre nombre d'axes ? À quoi cela ressemblerait-il ? Donnerait-il toujours une différence de probabilités prédites par les variables cachées par rapport à la mécanique quantique probabiliste ?
