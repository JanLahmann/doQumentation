...
Porte $H$ de Hadamard : Crée un état de superposition. Gate à un seul Qubit.
$$
...
Porte $R_y(\theta)$ (rotation Y) : Fait pivoter l'état autour de l'axe $y$. C'est une Gate à un seul Qubit.
$$
...
Enfin, les gates de rotation sont implémentées en spécifiant le type de porte, l'angle de rotation, et le Qubit sur lequel la porte est appliquée, dans cet ordre :
...
En utilisant la syntaxe introduite ou rappelée ci-dessus, construis un Circuit quantique quelconque faisant intervenir quatre types différents de gates quantiques.
...
D'après les opérations de ces Gates, nous pouvons constater, par exemple, que les kets présents dans les expressions de $P_{++}$ :
$$
...
Nous allons maintenant utiliser les Gates H, X et CNOT pour transformer $|0\rangle_L|0\rangle_R$ en $\frac{1}{\sqrt{2}}\left(|+\rangle_L|-\rangle_R-|-\rangle_L|+\rangle_R\right)$ :
...
Nous avons maintenant écrit le ket entièrement sous la forme de gates quantiques agissant sur l'état initial par défaut des Qubits.
...
Cela nous donne des instructions explicites pour construire notre Circuit quantique. Nous allons appliquer les Gates X, H, CNOT et $R_y$ à des Qubits représentant les états quantiques des particules mesurées par Lucas et Rihanna, puis effectuer des mesures pour obtenir la probabilité.
