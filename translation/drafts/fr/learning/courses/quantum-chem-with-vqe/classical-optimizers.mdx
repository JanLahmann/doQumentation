...
Cette fonction minimize a plusieurs arguments, mais les arguments les plus pertinents pour la chimie quantique sont :
- La fonction de coût (`cost_func`). Elle est liée au Hamiltonien, mais inclut aussi certaines complexités, comme la détermination de la valeur d'expectation en utilisant Estimator, et dans le cas des calculs d'états excités, pourrait inclure des conditions d'orthogonalité.
- Un état initial (x0) pour le système, souvent l'état de Hartree-Fock
- D'autres arguments, y compris les arguments de la fonction de coût elle-même
- La méthode définie à l'optimiseur classique sélectionné
