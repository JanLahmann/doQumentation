...
L'opération effectuée sur $\rho$ pour obtenir $\rho_{\mathsf{A}}$ dans cette équation est connue sous le nom de *trace partielle*, et plus précisément on dit que la trace partielle est effectuée sur $\mathsf{B},$ ou que $\mathsf{B}$ est *tracé dehors*.
