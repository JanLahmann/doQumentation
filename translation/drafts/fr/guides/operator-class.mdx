...
Note que dans l'exemple ci-dessus, l'opérateur est initialisé à partir d'un objet `Pauli`. Cependant, l'objet `Pauli` peut aussi être directement inséré dans le circuit lui-même et sera converti en une séquence de gates Pauli à qubit unique :
...
Par exemple, pour composer une gate à deux qubits avec un opérateur à trois qubits :
