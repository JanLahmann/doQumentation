...
variational_form.decompose().draw("mpl")
```

![Output of the previous code cell](/learning/images/courses/variational-algorithm-design/examples-and-applications/extracted-outputs/9f6e1a9f-0a53-4c21-93e6-d8edbd7b4e60-0.avif)
...
exact_eigenvalues = eigvalsh(observable_1.to_matrix())
print("Exact eigenvalues: ", exact_eigenvalues)

solution_eigenvalue = exact_eigenvalues[0]
print("Solution eigenvalue: ", solution_eigenvalue)
```

```text
Exact eigenvalues:  [-6.  4.  4.  6.]
Solution eigenvalue:  -6.0
```
...
```python
cost_func_vqe(x0, raw_ansatz, observable_1, estimator)
```
...
```python
experimental_min_eigenvalue = cost_func_vqe(
...
print(
    f"Percent error: {100*abs((experimental_min_eigenvalue - solution_eigenvalue)/solution_eigenvalue):.2e}"
)
```
