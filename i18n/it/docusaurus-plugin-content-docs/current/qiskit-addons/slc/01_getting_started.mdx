...
a. CPU — Limitare l'impatto per errore degli errori Pauli a 1 e 2 qubit
  ...
  1. Propagazione forward (limitare l'effetto sull'osservabile). Propagare ogni errore fino alla fine del circuito e calcolarne il commutatore con l'osservabile.  
  ...
  2. Propagazione backward (limitare l'effetto sullo stato iniziale). Propagare ogni errore fino all'inizio del circuito e calcolarne il commutatore con lo stato iniziale.
...
c. CPU — Stabilire le priorità di mitigazione
  ...
  1. Aggiornare i limiti uniti con i tassi di rumore appresi. Combinare i limiti forward e backward calcolati in precedenza e aggiornarli con i tassi di rumore appresi.  
...
d. QPU — Inserire l'anti-rumore ed eseguire. Eseguire il circuito di interesse con l'anti-rumore (rumore inverso) specificato usando le annotazioni `Box`.

e. CPU — Stimare l'osservabile. Calcolare il valore di aspettazione, applicando la post-selezione basata sulle misure per ridurre l'impatto del rumore non-Markoviano.
...
## Passo 0. Configurazione {#step-0-setup}
...
Per semplicità di dimostrazione, selezioniamo una catena di Ising speculare 1D. La catena di Ising 1D offre una struttura del circuito ben densa, utile per illustrare le implementazioni PEC. Un circuito speculare rende semplice conoscere il risultato atteso (in particolare, si dovrebbe misurare un osservabile pari a 1).

Inoltre, poiché vogliamo eseguire un circuito speculare, per ogni gate nella seconda metà del circuito deve esserci un gate inverso nella prima metà. Poiché l'osservabile misurato **$<X_6 Z_{13}>$** ha misurazioni in base non-Z, e l'Executor tiene conto della base desiderata alla fine del circuito, forniamo una funzione `prepare_basis` che inserisce i gate appropriati all'inizio del circuito speculare. Questo dettaglio è specifico alla nostra dimostrazione con Circuit speculare. La funzione `get_measurement_bases` ci permette di identificare facilmente quali gate sono necessari e dove aggiungerli, oltre a tenere traccia delle sottigliezze degli indici dei qubit derivanti dalle convenzioni dell'annotazione `box` discusse nella sezione "Prepara le misurazioni nelle basi canoniche".
...
### Flusso di lavoro per lo shading del cono di luce, l'apprendimento del rumore e l'iniezione anti-rumore {#workflow-for-lightcone-shading-noise-learning-and-anti-noise-injection-1}
...
##### a. Calcolare e stringere i limiti forward {#a-compute-and-tighten-forward-bounds}
...
#### b. Calcola e restringi i limiti forward {#b-compute-and-tighten-forward-bounds}
Successivamente restringiamo i bounds utilizzando la funzione `tighten_with_speed_limit`, che traccia come l'osservabile si propaga all'indietro attraverso il circuito e utilizza tale propagazione per porre dei limiti superiori sull'effetto di ogni operatore di rumore, prendendo il minore tra il limite forward appena calcolato e il limite di propagazione backward.
...
#### Visualizza l'SLC per un'ispezione manuale {#visualize-the-slc-for-manual-inspection-2}
...
### b.i. Aggiornare i limiti uniti con i tassi di rumore effettivamente appresi {#bi-update-merged-bounds-with-actual-learned-noise-rates}
...
### c. Eseguire il circuito di interesse con il rumore inverso {#c-execute-the-circuit-of-interest-with-antinoise}

#### c.i. Preparare il circuito template usando `samplex` {#ci-prepare-template-circuit-by-using-samplex}
