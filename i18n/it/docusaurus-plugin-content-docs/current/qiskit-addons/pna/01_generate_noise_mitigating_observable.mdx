...
- Creare un circuito quantistico trotterizzato e un osservabile. Transpilarlo sul Backend e includere le misurazioni di post-selezione.
- Usare ``samplomatic`` per applicare il twirling ai layer di gate a 2Q e alle misurazioni. Trovare i layer a 2Q unici per ridurre il costo dell'apprendimento del rumore.
...
![Esperimento rumoroso](/img/qiskit-addons/images/noisy_expt.png)
Se apprendiamo il modello di rumore, possiamo applicarne l'inverso e cancellare il rumore.
![Esperimento con mitigazione del rumore](/img/qiskit-addons/images/noise_mitigated_expt.png)
...
![Panoramica PNA](/img/qiskit-addons/images/pna_overview.png)
### Generare il circuito di Trotter speculare e l'osservabile {#generate-the-mirrored-trotter-circuit-and-observable}
...
dove $J>0$ descrive il coupling degli spin primi vicini, $i<j$, e il campo trasverso globale, $h$, è impostato a $\frac{\pi}{8}$. Più $h$ si allontana da un angolo di Clifford (cioè $\theta=n\frac{\pi}{2}, n \in \mathbb{Z}$), più diventa difficile propagare i generatori anti-rumore attraverso il circuito.
...
![Diagramma del circuito quantistico](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_1.png)

Successivamente, sceglieremo una catena di qubit su ``ibm_kingston`` che riportano tassi di errore bassi e transpileremo il circuito sul Backend.
...
![Diagramma del circuito quantistico](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_2.png)
...
Qui ci assicuriamo che il pass manager annoti le box con le annotazioni ``Twirl`` e ``InjectNoise``, che ci consentono di apprendere il rumore che influirà sul nostro circuito e di associarlo al layer del circuito corrispondente.

- ``enable_gates/enable_measure: True``: Raggruppa in box tutti i layer di gate a 2q e le misurazioni terminali. I gate a singolo qubit verranno inclusi con dressing a sinistra (left-dressed) all'interno delle box.
- ``measure_annotations: all`` Includi le annotazioni `Twirl` e `ChangeBasis` sulla box di misurazione
- ``twirling_strategy: active``: Twirl di tutti i qubit attivi in ogni box contenente gate entangling
- ``inject_noise_targets: gates``: Le annotazioni ``InjectNoise`` devono essere aggiunte a tutte le box con annotazione ``Twirl`` contenenti gate entangling
...
![Diagramma del circuito quantistico](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_3.png)

### Generare il circuito template e il samplex, definire come verrà campionato il circuito {#generate-the-template-circuit-and-samplex-define-how-the-circuit-will-be-sampled}
...
![Diagramma del circuito quantistico](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_4.png)
...
Prima di apprendere il rumore, dobbiamo trovare i layer a 2 qubit unici nel nostro circuito, così da minimizzare il numero di shot necessari per apprendere il rumore per l'intero Circuit. Usiamo ``find_unique_box_instructions`` da ``samplomatic`` per ottenere i layer unici dal Circuit inscatolato, incluso il layer di misurazione. Questi sono i layer che passiamo al noise learner.

Una volta noti i layer, possiamo apprendere il rumore. Ci sono alcuni parametri da considerare:

- `num_randomizations`: Il numero di Circuit casuali da usare per ogni configurazione del circuito di apprendimento
- `shots_per_randomization`: Numero totale di shot da usare per ogni circuito di apprendimento casuale
...
#### Associare le box del circuito al rumore appreso {#associate-circuit-boxes-with-learned-noise}
...
Come discusso sopra, questo avviene in due passi. Prima, propaghiamo un generatore di anti-rumore fino alla fine del circuito. Dopodiché, propaghiamo l'osservabile attraverso quel generatore evoluto. Questo processo viene ripetuto per ogni generatore di anti-rumore nel Circuit. In questa implementazione, ogni generatore in un dato layer viene propagato fino alla fine del circuito in parallelo. Inoltre, il multiprocessing di Python viene usato per eseguire in parallelo sia la propagazione in avanti dell'anti-rumore sia la back-propagation dell'osservabile. Questo evita l'accumulo di generatori evoluti in memoria e massimizza le risorse di calcolo.

Quando si esegue PNA, dovrai sempre fornire un circuito rumoroso e un osservabile. Se il tuo Circuit rumoroso è un circuito inscatolato con annotazioni `InjectNoise`, dovrai fornire la mappatura creata nel passo precedente. È anche possibile passare un circuito non inscatolato contenente istruzioni ``PauliLindbladError`` da ``qiskit-aer``. In quel caso, non è necessario fornire ``refs_to_noise_models``. Oltre agli input primari, gli utenti vorranno considerare:
...
- ``template_circuit``: Il circuito contenente tutti i gate necessari per implementare tutte le randomizzazioni desiderate (dalle randomizzazioni di twirling, dai parametri, ecc.).
- ``samplex``: Un oggetto che definisce una distribuzione di probabilità su tutte le possibili randomizzazioni del circuito da cui campionare.
- ``samplex_arguments``: Le associazioni (binding) necessarie per definire completamente il samplex
...
#### Campiona il circuito utilizzando il prototipo della primitiva ``Executor`` {#sample-the-circuit-using-the-executor-primitive-prototype}
...
#### Post-processa i campioni per calcolare un valore atteso con mitigazione degli errori {#post-process-the-samples-to-calculate-an-error-mitigated-expectation-value}

Per calcolare un valore atteso con mitigazione degli errori, eseguiremo:
...
- Utilizzo della funzione ``executor_expectation_values`` da ``qiskit-addon-utils`` per combinare tutti i dati in un valore atteso con mitigazione degli errori.
