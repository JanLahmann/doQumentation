...
Quando si ottimizza la strategia di mitigazione degli errori di un carico di lavoro, è spesso utile filtrare le misurazioni che si sa essere state contaminate da processi di rumore non-Markoviani (correlati). Un metodo per farlo prevede l'aggiunta a un Circuit di un passaggio di post-elaborazione che misura i Qubit attivi e "spettatori" adiacenti, applica una rotazione lenta a ogni Qubit e poi li misura di nuovo. Nei casi in cui le due misurazioni non confermano un Qubit capovolto come previsto, lo shot viene scartato applicando una maschera ai risultati.
...
Inizia preparando il Circuit da eseguire e transpilare su un backend che supporta i Gate frazionari.
...
Poi, prepara un oggetto `QuantumProgram` contenente il Circuit da eseguire.
