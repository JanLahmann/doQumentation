...
Il modo consigliato per transpilare un circuito è creare uno staged pass manager e poi eseguirne il metodo `run` con il circuito come input. Questa pagina spiega come transpilare circuiti quantistici in questo modo.
...
In questi esempi si usa il backend simulato `FakeSherbrooke` di `qiskit_ibm_runtime`, ma puoi provarlo su qualsiasi backend reale o fittizio compatibile con Qiskit. I tuoi risultati potrebbero essere diversi.
</Admonition>
...
Le [funzioni generatrici di fasi](https://docs.quantum.ibm.com/api/qiskit/transpiler_preset#stage-generator-functions) possono essere utili per costruire pass manager personalizzati.
