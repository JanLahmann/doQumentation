...
description: "Comprendi il formato di input e output (inclusi i Primitive Unified Bloc, o PUB) delle primitive Qiskit"
...
Questa pagina offre una panoramica degli input e degli output delle primitive Qiskit. Con queste primitive puoi usare una struttura dati nota come **Primitive Unified Bloc (PUB)** per definire in modo efficiente workload vettorizzati. I PUB sono l'unità fondamentale di lavoro per l'esecuzione dei workload. Vengono usati come input al metodo `run()` per le primitive Sampler ed Estimator, che eseguono il workload definito come job. Una volta completato il job, i risultati vengono restituiti in un formato che dipende dai PUB usati e dalle eventuali opzioni specificate.
<span id="pubs"></span>
## Panoramica dei PUB {#overview-of-pubs}

Quando si richiama il metodo `run()` di una primitiva, l'argomento principale richiesto è una `list` di una o più tuple — una per ogni circuito eseguito dalla primitiva. Ognuna di queste tuple è considerata un PUB, e gli elementi richiesti in ciascuna tupla dipendono dalla primitiva usata. I dati forniti a queste tuple possono anche essere disposti in una varietà di forme per offrire flessibilità nel workload tramite broadcasting — le cui regole sono descritte in una [sezione successiva](#broadcasting-rules).
...
- Un singolo `QuantumCircuit`, che può contenere uno o più oggetti [`Parameter`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.Parameter)
   *Nota: questi circuiti devono includere istruzioni di misurazione per ciascuno dei qubit da campionare.*
...
Il codice seguente mostra un esempio di input vettorizzati per la primitiva `Estimator`.
...
`Estimator` restituisce una stima del valore di aspettazione per ciascun elemento della forma trasmessa (broadcasted).
...
![Questa immagine illustra diverse rappresentazioni visive del broadcasting di array.](/docs/images/guides/primitive-input-output/broadcasting.avif "Rappresentazione visiva del broadcasting")
...
Una volta che uno o più PUB vengono inviati a una QPU per l'esecuzione e un job viene completato con successo, i dati vengono restituiti come oggetto contenitore [`PrimitiveResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PrimitiveResult). Il `PrimitiveResult` contiene una lista iterabile di oggetti [`PubResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PubResult) che contengono i risultati di esecuzione per ciascun PUB. Per esempio, un job inviato con 20 PUB restituirà un oggetto `PrimitiveResult` che contiene una lista di 20 `PubResult`, uno per ogni PUB.

Ognuno di questi oggetti `PubResult` possiede sia un attributo `data` sia un attributo `metadata` opzionale. L'attributo `data` è un [`DataBin`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.DataBin) personalizzato che contiene le stime dei valori di aspettazione nel caso dell'Estimator, o i campioni dell'output del circuito nel caso del Sampler.

L'attributo `data` può anche includere altre informazioni specifiche dell'implementazione, come le deviazioni standard. L'attributo `metadata` può contenere ulteriori informazioni specifiche dell'implementazione sull'esecuzione del PUB associato.
...
<Tabs>
    ...
    ```
    <Admonition type="note">
    Quello sopra è un esempio dei dati che potrebbero essere restituiti. I dati effettivamente restituiti dipendono dall'implementazione.
    </Admonition>
    </TabItem>
...
Come indicato in precedenza, i dati restituiti nel `PubResult` per la primitiva Estimator dipendono dall'implementazione. Ad esempio, potrebbe contenere un array di valori di aspettazione (`PubResult.data.evs`) e le deviazioni standard associate (`PubResult.data.stds`).
...
Oltre ai risultati dell'esecuzione, gli oggetti `PrimitiveResult` e `PubResult` contengono un attributo metadata opzionale relativo al job inviato. I metadati restituiti (se presenti) sono specifici dell'implementazione.
...
<Admonition type="tip" title="Consigli">
  ...
  -  Consulta l'API delle [primitive Qiskit](https://docs.quantum.ibm.com/api/qiskit/primitives).
  -  Consulta l'API delle [primitive Qiskit Aer](https://qiskit.github.io/qiskit-aer/apidocs/aer_primitives.html).
  -  Scopri di più sulle [primitive Qiskit Runtime](/guides/qiskit-runtime-primitives).
  -  Consulta l'API di [Qiskit Runtime Estimator](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2).
  -  Consulta l'API di [Qiskit Runtime Sampler](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2).
</Admonition>
