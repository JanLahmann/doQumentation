---
description: 'Comprendi il formato di input e output (inclusi i Primitive Unified Bloc, o PUB) delle primitive Qiskit SDK'
...
{/* doqumentation-source-hash: c37e29ce */}
...
This page gives an overview of the inputs and outputs of the Qiskit SDK primitives. With these primitives you can use a data structure known as a **Primitive Unified Bloc (PUB)** to efficiently define vectorized workloads. These PUBs are the fundamental unit of work for workload execution. They are used as inputs to the `run()` method for the Sampler and Estimator primitives, which execute the defined workload as a job. Then, after the job has completed, the results are returned in a format that depends on the PUBs used and any specified options.
<span id="pubs"></span>
...
Una volta che uno o più PUB sono stati inviati a una QPU per l'esecuzione e un job si completa con successo, i dati vengono restituiti come oggetto contenitore [`PrimitiveResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PrimitiveResult). Il `PrimitiveResult` contiene una lista iterabile di oggetti [`PubResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PubResult) che contengono i risultati di esecuzione per ciascun PUB. Ad esempio, un job inviato con 20 PUB restituirà un oggetto `PrimitiveResult` che contiene una lista di 20 `PubResults`, uno corrispondente a ciascun PUB.
...
<Admonition type="tip" title="Consigli">
  ...
  -  Consulta l'API delle [primitive Qiskit SDK](https://docs.quantum.ibm.com/api/qiskit/primitives).

  -  Consulta l'API delle [primitive Qiskit Aer](https://qiskit.github.io/qiskit-aer/apidocs/aer_primitives.html).

  -  Scopri di più sulle [primitive IBM Quantum](/guides/qiskit-runtime-primitives).

  -  Consulta l'API di [`qiskit-ibm-runtime` Estimator](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2).

  -  Consulta l'API di [`qiskit-ibm-runtime` Sampler](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2).
</Admonition>
