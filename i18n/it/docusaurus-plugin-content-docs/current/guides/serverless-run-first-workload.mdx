...
<Admonition type="tip">
**Qiskit Serverless sta ricevendo un aggiornamento e le sue funzionalità stanno cambiando rapidamente.** Durante questa fase di sviluppo, trovi le note di rilascio e la documentazione più recente nella pagina [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html).
</Admonition>
...
Assicurati di aver seguito il flusso di lavoro in [Scrivi il tuo primo programma Qiskit Serverless](/guides/serverless-first-program) prima di iniziare.

## Elenca i programmi disponibili {#list-programs-available}

Puoi usare `QiskitServerless.list()` per recuperare un elenco dei programmi disponibili da eseguire con Qiskit Serverless. Questo include il precedentemente caricato [`transpile_remote_serverless`](/guides/serverless-first-program).
...
Innanzitutto, configura i tuoi input. Il tuo programma ha tre input: `circuits`, `backend` e `optimization_level`. Puoi usare `random_circuit` per creare 30 circuiti casuali:
...
Successivamente, passa i tuoi input ed eseguilo in stile pythonico come segue:
...
- Esplora gli [strumenti di gestione del calcolo e dei dati](/guides/serverless-manage-resources) disponibili per il tuo programma, incluso il parallelismo.
