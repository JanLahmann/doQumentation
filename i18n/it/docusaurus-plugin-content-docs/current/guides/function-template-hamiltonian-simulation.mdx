...
sidebar_label: 'Costruire un template di Qiskit Function per la simulazione hamiltoniana'
title: 'Costruire un template di Qiskit Function per la simulazione hamiltoniana'
---
...
Questo template racchiude un workflow per simulare l'evoluzione temporale di uno stato iniziale rispetto a un'Hamiltoniana spin-based definita dall'utente, e restituisce un insieme di valori di aspettazione specificati usando il componente aggiuntivo Qiskit [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/).

Il template è strutturato come un pattern Qiskit con i seguenti passaggi:

#### 1. Raccolta degli input e mappatura del problema {#1-collecting-input-and-mapping-the-problem}
...
#### 2. Preparare i circuiti generati per l'esecuzione {#2-prepare-the-generated-circuits-for-execution}

I circuiti generati dal componente aggiuntivo AQC vengono poi transpilati per essere eseguiti su un backend scelto. Viene creata un'istanza di [`EstimatorV2`](../api/qiskit-ibm-runtime/estimator-v2) con un insieme predefinito di opzioni di mitigazione degli errori per gestire l'esecuzione del circuito.

#### 3. Esecuzione {#3-execution}
...
## Scrivere il template di funzione {#write-the-function-template}

Prima di tutto, scrivi un template di funzione per la simulazione hamiltoniana che usa il [componente aggiuntivo AQC-Tensor di Qiskit](https://qiskit.github.io/qiskit-addon-aqc-tensor/) per mappare la descrizione del problema su un circuito a profondità ridotta per l'esecuzione su hardware.

Se scarichi questa pagina e la visualizzi localmente in un editor di notebook, vedrai che alcune celle di codice contengono il [magic command](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Questo magic command salva il codice in `./source_files/template_hamiltonian_simulation.py`, che è il template di funzione che puoi caricare ed eseguire da remoto con Qiskit Serverless.
...
### Raccogliere e validare gli input {#collect-and-validate-the-inputs}
...
#### Validare gli input {#validate-the-inputs}
...
#### Preparare gli output della funzione {#prepare-the-function-outputs}
...
### Mappare il problema e pre-elaborare il circuito con AQC {#map-the-problem-and-pre-process-the-circuit-with-aqc}
...
### Ottimizzare il circuito finale per l'esecuzione {#optimize-the-final-circuit-for-execution}

Dopo la porzione AQC del workflow, il `final_circuit` viene [transpilato per l'hardware](/guides/transpile#instruction-set-architecture) come di consueto.
...
#### Uscire prima se si usa la modalità dry run {#exit-early-if-using-dry-run-mode}
...
#### Eseguire il circuito sull'hardware {#execute-the-circuit-on-hardware}
...
#### Salvare l'output {#save-the-output}
...
## Distribuire la funzione su IBM Quantum Platform {#deploy-the-function-to-ibm-quantum-platform}
...
Usa `qiskit-ibm-catalog` per autenticarti a `QiskitServerless` con la tua chiave API, che puoi trovare nella dashboard di [IBM Quantum Platform](https://quantum.cloud.ibm.com), e carica il programma.

Puoi facoltativamente usare `save_account()` per salvare le tue credenziali (vedi la guida [Configurare il tuo account IBM Cloud](/guides/cloud-setup#cloud-save)). Nota che questo scrive le tue credenziali nello stesso file di [`QiskitRuntimeService.save_account()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/qiskit-runtime-service#save_account).
...
## Eseguire il template di funzione da remoto {#run-the-function-template-remotely}
...
Per un approfondimento sul componente aggiuntivo AQC-Tensor di Qiskit, consulta il tutorial [Improved Trotterized Time Evolution with Approximate Quantum Compilation](/tutorials/approximate-quantum-compilation-for-time-evolution) o il [repository qiskit-addon-aqc-tensor](https://github.com/Qiskit/qiskit-addon-aqc-tensor).
