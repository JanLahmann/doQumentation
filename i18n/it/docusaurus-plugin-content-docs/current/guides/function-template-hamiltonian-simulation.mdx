...
# Costruire un template di funzione per la simulazione hamiltoniana {#build-a-function-template-for-hamiltonian-simulation}

Questo template racchiude un workflow per simulare l'evoluzione temporale di uno stato iniziale rispetto a un'Hamiltoniana spin-based definita dall'utente, e restituisce un insieme di valori di aspettazione specificati usando il [componente aggiuntivo AQC](./qiskit-addons-aqc).
...
Prima di tutto, scrivi un template di funzione per la simulazione hamiltoniana che usa il [componente aggiuntivo AQC-Tensor di Qiskit](/guides/qiskit-addons-aqc) per mappare la descrizione del problema su un circuito a profondità ridotta per l'esecuzione su hardware.
...
<details>
<summary><b>Codice sorgente completo del programma</b></summary>
