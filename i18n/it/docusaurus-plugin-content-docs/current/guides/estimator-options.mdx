...
Non è necessario istanziare la classe options di `EstimatorV2` ([`EstimatorOptions`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-estimator-options)) se stai impostando le opzioni dopo l'inizializzazione della primitiva.
...
Ad esempio, se precision è specificato in tutti e quattro i punti, viene usato quello con la priorità più alta (precision specificato nel PUB).
...
Quanta resilienza costruire contro gli errori. Livelli più alti generano risultati più accurati a scapito di tempi di elaborazione più lunghi. Consulta la sezione [livelli di resilienza](/guides/estimator-noise-management#resilience) nel topic Gestione del rumore per saperne di più.
...
**Scelte**: Lista di interazioni orientate (dirette) a due qubit
