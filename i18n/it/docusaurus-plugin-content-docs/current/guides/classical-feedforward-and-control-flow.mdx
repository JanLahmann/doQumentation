...
* Preparazione efficiente dello stato quantistico, come [stato GHZ](https://journals.aps.org/prxquantum/abstract/10.1103/PRXQuantum.5.030339), [stato W](https://arxiv.org/abs/2403.07604) (per maggiori informazioni sullo stato W, consulta anche ["State preparation by shallow circuits using feed forward"](https://arxiv.org/abs/2307.14840)), e un'ampia classe di [stati a matrice di prodotto](https://arxiv.org/abs/2404.16083)

* [Entanglement efficiente a lungo raggio](https://journals.aps.org/prxquantum/abstract/10.1103/PRXQuantum.5.030339) tra qubit sullo stesso chip usando circuiti superficiali

* Campionamento efficiente di [circuiti simili a IQP](https://arxiv.org/pdf/2505.04705)
Qiskit supporta quattro costrutti di controllo di flusso per il feedforward classico, ciascuno implementato come metodo su [`QuantumCircuit`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.QuantumCircuit). I costrutti e i rispettivi metodi sono:
...
L'esempio seguente mostra come usare il calcolo della parità per creare uno stato GHZ a n qubit usando i circuiti dinamici. Prima, si generano $n/2$ coppie di Bell su qubit adiacenti. Poi, si collegano queste coppie tramite un layer di gate CNOT tra le coppie. Si misura quindi il qubit target di tutti i gate CNOT precedenti e si reimposta ogni qubit misurato allo stato $\vert 0 \rangle$. Si applica $X$ a ogni sito non misurato per il quale la parità di tutti i bit precedenti è dispari. Infine, i gate CNOT vengono applicati ai qubit misurati per ristabilire l'entanglement perso nella misura.
