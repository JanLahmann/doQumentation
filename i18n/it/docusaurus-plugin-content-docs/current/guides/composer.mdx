...
description: IBM Quantum Composer è uno strumento di programmazione grafico. Trascina e rilascia le operazioni per costruire circuiti ed eseguirli su hardware quantistico.
...
Esegui i tuoi circuiti su hardware quantistico reale per comprendere gli effetti del rumore del dispositivo.
...
6. **Visualizzazioni** — Le visualizzazioni caratterizzano il tuo circuito mentre lo costruisci. Usano un simulatore del vettore di stato a singolo shot, che è diverso dalla QPU specificata nelle impostazioni "Esegui circuito". Nota che le visualizzazioni ignorano qualsiasi operazione di misurazione aggiunta. Accedi e clicca su **Esegui circuito** per ottenere i risultati dal Backend specificato.
...
1. (Facoltativo) Se non hai ancora effettuato l'accesso a IBM Quantum, seleziona **Accedi** nell'angolo in alto a destra. Puoi quindi accedere oppure **Creare un account IBM Cloud**.
...
Ora puoi trascinare e rilasciare la nuova operazione nel circuito. Nota che l'operazione è salvata in questo circuito ma non compare nel catalogo delle operazioni degli altri circuiti.
...
## Eseguire circuiti e visualizzare i risultati {#run-circuits-and-view-results}
...
<Admonition type="note" title="Casualità nel simulatore">
Le visualizzazioni live provengono da un simulatore del vettore di stato a singolo shot, che è diverso dalla QPU specificata nelle impostazioni di esecuzione, che può avere più shot. Il simulatore introduce casualità generando risultati basati su un *seed*. Il seed è il valore iniziale introdotto nell'algoritmo che genera numeri pseudocasuali. Puoi vedere il numero del seed selezionando "Seed delle visualizzazioni" dal menu Modifica. Puoi anche impostare il seed manualmente cambiando il valore nella casella.
</Admonition>
...
![un circuito quantistico](/docs/images/guides/composer/2H-circuit.svg "Un circuito composto da una colonna di gate Hadamard che crea una sovrapposizione uniforme degli stati della base computazionale, seguita da un gate a due qubit controlled-Z (CZ).")

![Vista Probabilità del circuito precedente](/docs/images/guides/composer/probabilities.svg "Vista Probabilità del circuito precedente.")
...
![un circuito quantistico](/docs/images/guides/composer/2H-circuit.svg)

![vista q-sphere](/docs/images/guides/composer/q-sphere.avif)
...
![vista vettore di stato](/docs/images/guides/composer/state-vector.svg "Il vettore di stato alla fine del circuito sopra. La ruota dei colori mappa l'angolo di fase al colore. Lo stato di output è espresso come un elenco di numeri complessi.")
...
Il gate di Toffoli semplificato implementa il gate di Toffoli a meno di fasi relative. Nota che il gate di Toffoli semplificato non è equivalente al gate di Toffoli, ma può essere usato nei contesti in cui il gate di Toffoli viene successivamente annullato (uncomputed).
