...
Il risultante `NoiseLearnerResult.data` è una lista di oggetti [`LayerError`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/results-layer-error) contenenti il [modello di rumore](https://arxiv.org/abs/2201.09866) per ogni singolo layer di entanglement appartenente ai circuiti target. Ogni `LayerError` memorizza le informazioni del layer, sotto forma di circuito e insieme di etichette di qubit, insieme al [`PauliLindbladError`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/results-pauli-lindblad-error) per il modello di rumore appreso per il layer in questione.
...
### Opzioni per l'apprendimento del rumore {#noise-learning-options-1}
...
Il circuito nell'esempio precedente passa attraverso il boxing pass manager, che raggruppa le istruzioni in box annotati. Il codice rilevante è aggiunto qui per facilitare la comprensione.
- `inject_noise_targets=”gates”` specifica di aggiungere le annotazioni `InjectNoise` ai box che contengono entangler.
- `inject_noise_strategy="uniform_modification"` specifica di assegnare lo stesso `ref` e `modifier_ref` a tutti i box equivalenti con annotazioni `InjectNoise`.
      ...
      - `InjectNoise.modifier_ref` permette di scalare il modello di rumore assegnato a un box tramite fattori moltiplicativi.
...
- Scopri di più sulle [tecniche disponibili di mitigazione e soppressione degli errori](error-mitigation-and-suppression-techniques).
