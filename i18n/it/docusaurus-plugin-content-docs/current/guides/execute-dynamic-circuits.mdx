...
I circuito dinamici sono strumenti potenti con cui puoi misurare i qubit nel mezzo dell'esecuzione di un circuito quantistico e poi eseguire operazioni di logica classica all'interno del circuito, in base al risultato di quelle misurazioni mid-circuit. Questo processo è anche noto come _feedforward classico_. Sebbene questi siano i primi giorni per capire come sfruttare al meglio i circuito dinamici, la comunità di ricerca quantistica ha già identificato una serie di casi d'uso, come i seguenti:
...
Prima di `qiskit-ibm-runtime` v0.43.0, `measure` era l'unica istruzione di misurazione in Qiskit. Le misurazioni mid-circuit, tuttavia, hanno requisiti di tuning diversi rispetto alle misurazioni _terminali_ (misurazioni che avvengono alla fine di un circuito). Ad esempio, è necessario considerare la durata dell'istruzione durante il tuning di una misurazione mid-circuit perché le istruzioni più lunghe causano Circuit più rumorosi. Non è necessario considerare la durata dell'istruzione per le misurazioni terminali perché non ci sono istruzioni dopo le misurazioni terminali.
...
* La primitiva Sampler richiede le misurazioni del circuito. Puoi aggiungere misurazioni del circuito con la primitiva Estimator, ma vengono ignorate.
...
Il codice seguente crea ed esegue un circuito dinamico sull'hardware IBM&reg;.
...
- A causa della memoria fisica limitata sull'elettronica di controllo, c'è anche un limite al numero di istruzioni `if` e alla dimensione dei loro operandi. Questo limite è una funzione del numero di broadcast e del numero di bit trasmessi in un job (non in un circuito).
