...
Autenticati usando la tua [chiave API](https://quantum.cloud.ibm.com/) e seleziona la Qiskit Function come segue. (Questo snippet presuppone che tu abbia già [salvato il tuo account](/guides/functions-get-started#install-qiskit-functions-catalog-client) nell'ambiente locale.)
...
### 3. Definisci le impostazioni dell'ottimizzatore e dell'ansatz (Facoltativo) {#3-define-the-optimizer-and-ansatz-settings-optional}
...
Nel caso del Real Amplitudes Ansatz, puoi seguire una relazione lineare tra ``population_size`` e il numero di qubit nel circuito. Come regola empirica approssimata, si raccomanda di usare un **minimo** di ``population_size ~ 0.8 * n_qubits`` per l'ansatz `real_amplitudes`.
...
![Visualizzazione della soluzione dell'ottimizzazione](/docs/images/guides/global-data-quantum-optimizer/cost_distribution.avif)
