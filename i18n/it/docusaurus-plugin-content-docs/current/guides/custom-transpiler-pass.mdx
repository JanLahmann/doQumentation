...
<Accordion>
  <AccordionItem title="Approfondimento: rappresentazione DAG">

Prima di creare un pass, è importante introdurre la rappresentazione interna dei circuiti quantistici in Qiskit, il [grafo aciclico diretto (DAG)](../api/qiskit/qiskit.dagcircuit.DAGCircuit) (consulta [questo tutorial](https://qiskit.org/ecosystem/rustworkx/tutorial/dags.html) per una panoramica). Per seguire questi passaggi, installa la [libreria `graphviz`](https://graphviz.org/download/) per le funzioni di disegno del DAG.

In Qiskit, all'interno delle fasi di transpilazione, i circuiti sono rappresentati usando un DAG. In generale, un DAG è composto da *vertici* (detti anche "nodi") e da *archi* diretti che collegano coppie di vertici in un particolare orientamento. Questa rappresentazione viene memorizzata usando oggetti `qiskit.dagcircuit.DAGCircuit` composti da singoli oggetti `DagNode`. Il vantaggio di questa rappresentazione rispetto a un semplice elenco di Gate (cioè una *netlist*) è che il flusso di informazioni tra le operazioni è esplicito, rendendo più facile prendere decisioni di trasformazione.
...
Usa la funzione `qiskit.tools.visualization.dag_drawer()` per visualizzare il DAG di questo circuito. Esistono tre tipi di nodi del grafo: nodi Qubit/clbit (verdi), nodi operazione (blu) e nodi di output (rossi). Ogni arco indica il flusso di dati (o la dipendenza) tra due nodi.
