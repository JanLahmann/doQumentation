...
Lo stadio successivo riguarda il layout, ovvero la connettività del backend a cui verrà inviato il circuito. In generale, i circuiti quantistici sono entità astratte i cui qubit sono rappresentazioni "virtuali" o "logiche" dei qubit reali utilizzati nei calcoli. Per eseguire una sequenza di gate, è necessaria una mappatura uno-a-uno dai qubit "virtuali" ai qubit "fisici" di un dispositivo quantistico reale. Questa mappatura è memorizzata come oggetto `Layout` e fa parte dei vincoli definiti all'interno dell'[instruction set architecture (ISA)](/guides/transpile#instruction-set-architecture) di un backend.

![Questa immagine illustra come i qubit vengono mappati dalla rappresentazione a filo a un diagramma che rappresenta come i qubit sono connessi sulla QPU.](/docs/images/guides/transpiler-stages/layout-mapping.svg "Mappatura dei qubit")

La scelta della mappatura è estremamente importante per minimizzare il numero di operazioni SWAP necessarie per mappare il circuito di input sulla topologia del dispositivo e garantire che vengano utilizzati i qubit meglio calibrati.  Data l'importanza di questa fase, i pass manager preimpostati provano alcuni metodi diversi per trovare il layout migliore.  Tipicamente questo comporta due passaggi: prima, provare a trovare un layout "perfetto" (un layout che non richiede alcuna operazione SWAP), e poi, un pass euristico che cerca di trovare il layout migliore da usare se non è possibile trovare un layout perfetto.  Ci sono due `Passes` tipicamente usati per questo primo passaggio:
...
- `DenseLayout`: Trova il sottografo del dispositivo con la maggiore connettività e che ha lo stesso numero di qubit del circuito  (usato per il livello di ottimizzazione 1 se sono presenti nel circuito operazioni di controllo del flusso (come IfElseOp)).
...
![Output della cella di codice precedente](/docs/images/guides/transpiler-stages/extracted-outputs/62479697-cef1-4a89-ac2a-20051dd294f4-1.svg)
...
![Output della cella di codice precedente](/docs/images/guides/transpiler-stages/extracted-outputs/ab89c4ea-06c4-4320-b493-feb691b3570d-0.svg)
...
![Output della cella di codice precedente](/docs/images/guides/transpiler-stages/extracted-outputs/f6a3a92a-8656-4518-ba2c-c3b0b038f507-0.svg)
...
Quando scrivi un circuito quantistico, sei libero di usare qualsiasi gate quantistico (operazione unitaria) che preferisci, insieme a una raccolta di operazioni non-gate come istruzioni di misurazione o reset dei qubit.  Tuttavia, la maggior parte dei dispositivi quantistici supporta nativamente solo una manciata di operazioni gate e non-gate.  Questi gate nativi fanno parte della definizione dell'[ISA](/guides/transpile#instruction-set-architecture) di un target e questa fase dei `PassManagers` preimpostati traduce (o *srotola*) i gate specificati in un circuito nei gate di base nativi di un backend specificato.  Questo è un passaggio importante, poiché consente al circuito di essere eseguito dal backend, ma tipicamente porta a un aumento della profondità e del numero di gate.
...
![Output della cella di codice precedente](/docs/images/guides/transpiler-stages/extracted-outputs/d4d1f65a-3336-4d70-9189-65ba010f2366-1.svg)
...
![Output della cella di codice precedente](/docs/images/guides/transpiler-stages/extracted-outputs/4552b367-75e9-4faa-a59d-8317e25ac145-0.svg)
...
![Output della cella di codice precedente](/docs/images/guides/transpiler-stages/extracted-outputs/2700405a-f559-45d3-99a9-5b4447621743-0.svg)
...
![Output della cella di codice precedente](/docs/images/guides/transpiler-stages/extracted-outputs/ae2d9390-5c26-46f3-9418-a684ba8a406a-0.svg)
...
![Vista timeline.draw() dello stesso circuito](/docs/images/guides/transpiler-stages/transpiler-17.svg)
