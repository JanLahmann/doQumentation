---
description: 'Introduzione a IBM Quantum Compute Service, Qiskit Functions, Qiskit Serverless e IBM Quantum Platform'
...
Qiskit Functions (`qiskit-ibm-catalog.QiskitFunctionsCatalog`) sono servizi ad alto livello di astrazione, progettati per accelerare la scoperta di algoritmi e la prototipazione di applicazioni. Esplora il [Catalogo di Qiskit Functions](https://quantum.cloud.ibm.com/functions), che include:
...
- **Application functions**: Servizi che includono interi flussi di lavoro quantistici, dalla mappatura dal classico al quantistico, all'ottimizzazione per l'hardware, all'esecuzione sull'hardware e alla post-elaborazione. Gli utenti possono prototipare applicazioni industriali con input e output familiari a chi lavora nel dominio.


Per ulteriori dettagli, consulta la [documentazione di Qiskit Functions](/guides/functions).

I membri dei piani Premium, Flex e On-Prem (tramite IBM Quantum Platform API) possono accedere immediatamente alle funzioni fornite da IBM, oppure acquistare licenze per le funzioni fornite dai partner direttamente da quei partner.
