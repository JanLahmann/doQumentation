---
description: 'Spiegazione delle tecniche di mitigazione e soppressione degli errori disponibili tramite IBM Quantum Compute Service'
...
La versione beta di un nuovo modello di esecuzione è ora disponibile. Il modello di esecuzione diretta offre maggiore flessibilità nella personalizzazione del flusso di lavoro di mitigazione degli errori. Consulta la guida [Modello di esecuzione diretta](/guides/directed-execution-model) per maggiori informazioni.
</Admonition>
{/*
...
![Rappresentazione del twirling di Pauli](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)
...
![Rappresentazione del twirling della misura](/docs/images/guides/error-mitigation-explanation/measurement_twirling.svg)

In presenza di errori di readout, il twirling della misura ha l'effetto di diagonalizzare la matrice di trasferimento degli errori di readout, rendendone facile l'inversione. Un termine di riscalamento dal canale di rumore diagonale viene appreso effettuando il benchmarking di circuiti casuali inizializzati nello stato zero. Questo consente al servizio di rimuovere il bias dai valori di aspettazione che risultano dal rumore di readout. La stima della matrice di trasferimento degli errori di readout richiede l'esecuzione di circuiti di calibrazione aggiuntivi, il che introduce un piccolo overhead.

### Abilitare TREX usando l'opzione `measure_mitigation` {#enable-trex-by-using-the-measure-mitigation-option}

TREX può essere abilitato impostando `measure_mitigation` a `True` nelle [opzioni di resilienza di IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) per Estimator. Le opzioni per l'apprendimento del rumore di misura sono descritte [qui](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-measure-noise-learning-options). Come per il twirling dei gate, puoi impostare il numero di randomizzazioni del circuito e il numero di shot per randomizzazione.
...
### Abilitare TREX usando l'opzione `resilience_level` {#enable-trex-by-using-the-resilience-level-option}

Puoi abilitare TREX impostando il [livello di resilienza](/guides/estimator-noise-management#resilience) a `1`, come mostrato nell'esempio seguente.
...
L'overhead di questo metodo scala con il numero di fattori di rumore. Le impostazioni predefinite campionano il valore di aspettazione a tre fattori di rumore, portando a un overhead di circa 3x.

### Abilitare ZNE usando l'opzione `zne_mitigation` {#enable-zne-by-using-the-zne-mitigation-option}

La ZNE può essere abilitata impostando `zne_mitigation` a `True` nelle [opzioni di resilienza di IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) per Estimator.
Le opzioni per la ZNE sono descritte [qui](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options). Le seguenti opzioni sono degne di nota:
...
### Abilitare ZNE usando l'opzione `resilience_level` {#enable-zne-by-using-the-resilience-level-option}

Puoi abilitare ZNE (insieme a TREX e gate twirling) impostando il [livello di resilienza](/guides/estimator-noise-management#resilience) a `2`, come mostrato nell'esempio seguente.
...
- [Configura la gestione del rumore con Estimator](/guides/estimator-noise-management).

- [Configura la gestione del rumore con Sampler](/guides/sampler-noise-management).

- Esplora le altre [opzioni](/guides/runtime-options-overview) per le primitive di IBM Quantum.
