...
*Consulta il [riferimento API](https://docs.quantum.ibm.com/api/functions/qunova-chemistry)*
...
Poi, autenticati con la tua [chiave API IBM Quantum&reg;](http://quantum.cloud.ibm.com/) e, avendo già [salvato il tuo account](/guides/functions-get-started#install-qiskit-functions-catalog-client) nell'ambiente locale, seleziona la Qiskit Function come segue:
...
Per accedere all'energia dello stato fondamentale, usa la chiave "energy". La chiave "eigenvector" fornisce i coefficienti CI con la corrispondente notazione a bitstring della configurazione elettronica memorizzata con "states" dei risultati.
...
Si noti che l'utilizzo di questa Qiskit Function è limitato ai problemi che richiedono al massimo 20 qubit, salvo l'ottenimento di una licenza che conceda un limite superiore.

Invia un'e-mail a [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) per richieste relative alla licenza.

### Esempio di uso di una funzione con licenza {#example-of-licensed-function-use}

Agli utenti con licenza viene rilasciato un token di licenza, che deve poi essere inviato alla funzione tramite una libreria wrapper.
La libreria wrapper può essere installata [da PyPI](https://pypi.org/project/hivqe-qiskit-function-utils/) con `pip install hivqe-qiskit-function-utils`.
L'esempio seguente mostra come questa libreria dovrebbe essere usata per inviare il tuo token di licenza quando chiami la funzione.
...
- Visita il [riferimento API](https://docs.quantum.ibm.com/api/functions/qunova-chemistry) per questa Qiskit Function.
