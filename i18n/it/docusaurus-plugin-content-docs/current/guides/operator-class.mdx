...
This page shows how to use the [`Operator`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Operator) class. For a high-level overview of operator representations in Qiskit, including the `Operator` class and others, see [Overview of operator classes](/guides/operators-overview).
...
Diverse altre classi di Qiskit possono essere convertite direttamente in un oggetto `Operator` usando il metodo di inizializzazione dell'operatore. Per esempio:
...
Gli `Operators` unitari possono essere inseriti direttamente in un `QuantumCircuit` usando il metodo `QuantumCircuit.append`. Questo converte l'`Operator` in un oggetto `UnitaryGate`, che viene aggiunto al circuito.
