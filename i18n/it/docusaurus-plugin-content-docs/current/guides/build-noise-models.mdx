...
Questa pagina mostra come usare il modulo [`noise`](https://qiskit.org/ecosystem/aer/apidocs/aer_noise.html) di Qiskit Aer per costruire modelli di rumore per simulare Circuit quantistici in presenza di errori. Questo è utile per emulare processori quantistici rumorosi e per studiare gli effetti del rumore sull'esecuzione di algoritmi quantistici.
...
Vedremo ora alcuni esempi di modelli di rumore. Per le nostre dimostrazioni usiamo un semplice Circuit di test che genera uno stato GHZ a n Qubit:
...
<Admonition type="tip" title="Suggerimenti">
  - Per simulare Circuit rumorosi, consulta [Simulazione esatta e rumorosa con le primitive di Qiskit Aer](./simulate-with-qiskit-sdk-primitives).
