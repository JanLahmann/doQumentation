...
![Immagine che illustra il circuito seguente](/docs/images/guides/stretch/stretch.svg)
...
:::note
 ...
 Quando si usa `stretch` con IBM Quantum Compute Service, qualsiasi resto risultante dalla risoluzione di uno stretch viene aggiunto al primo delay che usa lo stretch.
...
## Visualizzare i valori stretch in IBM Quantum Compute Service {#view-stretch-values-in-ibm-quantum-compute-service}

Il valore effettivo di una durata stretch viene risolto in fase di compilazione, dopo la schedulazione del circuito. Quando si esegue un job Sampler in IBM Quantum Compute Service, puoi visualizzare i valori stretch risolti nei metadati del risultato del job. Il supporto per `stretch` in Quantum Compute è attualmente sperimentale, quindi devi prima impostare un'opzione sperimentale per abilitarne il recupero e poi accedere ai dati direttamente dai metadati, come segue:
...
Sebbene il tempo totale del circuito venga restituito nei metadati "compilation", questo NON è il tempo utilizzato per la fatturazione (QPU time).
</Admonition>
...
Puoi usare una visualizzazione per aiutarti a comprendere e verificare il timing.
...
Usa i cursori in basso, i controlli in alto (passa il mouse sull'immagine di output per visualizzarli) e la legenda sul lato dell'output per personalizzare la vista. Passa il cursore sull'immagine per visualizzare i dati esatti.

Per tutti i dettagli, consulta l'argomento [Visualizzare il timing del circuito](/guides/qiskit-runtime-circuit-timing).

<span id="qiskit-runtime-limitations"></span>

## Limitazioni di Quantum Compute {#quantum-compute-limitations}

Il supporto per `stretch` in Quantum Compute è attualmente sperimentale e presenta i seguenti vincoli:
...
- L'area delimitata da un insieme di barrier è chiamata _regione di barrier_. Una variabile stretch non può essere usata in più regioni di barrier.
    <Tabs>
        ...
        ![Illustrazione dell'output del codice precedente](/docs/images/guides/stretch/bad-barrier-region.svg 'Uso non valido di stretch nelle regioni di barrier')
     ...
     <TabItem value="Valid" label="Valido">
        ...
        ![Illustrazione dell'output del codice precedente](/docs/images/guides/stretch/good-barrier-region.svg 'Uso valido di stretch in una regione di barrier')
