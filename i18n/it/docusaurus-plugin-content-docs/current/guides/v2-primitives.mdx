...
The output is now in the [`PubResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PubResult) format. A `PubResult` is the data and metadata resulting from a single PUB’s execution.
...
The V2 primitives have a new `RuntimeJobV2` class, which inherits from `BasePrimitiveJob`. The `status()` method of this new class returns a string instead of a JobStatus enum from Qiskit. See the [RuntimeJobV2 API reference](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-job-v2) for details.
