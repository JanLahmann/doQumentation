...
Quando crei circuiti quantistici, devi anche considerare che tipo di dati vuoi ricevere dopo l'esecuzione. Qiskit offre due modi per restituire dati: puoi ottenere l'output di campionamento per un insieme di qubit che scegli di misurare, oppure puoi ottenere il valore di aspettazione di un osservabile. Prepara il tuo workload per misurare il tuo circuito in uno di questi due modi con le primitive Qiskit (spiegate in dettaglio nel [Passaggio 3](#step-3-execute-using-the-quantum-primitives)).
