...
description: Migra dall'utilizzo dei simulatori cloud di IBM Quantum alla modalità di test locale di Qiskit Runtime.
...
<Accordion>
<AccordionItem title="<a>I simulatori hanno delle limitazioni</a>">
...
<AccordionItem title="<a>Usare l'hardware quantistico sviluppa competenze uniche</a>">
...
- Lavorare con l'hardware quantistico offre una comprensione concreta delle sfide e delle opportunità del calcolo quantistico, migliorando la capacità degli sviluppatori di guidare l'innovazione nel settore.

</AccordionItem>
<AccordionItem title="<a>Gli algoritmi devono essere adattati all'hardware quantistico</a>">
...
La modalità di test locale non dispone di soppressione o mitigazione degli errori integrata. Devi invece specificare esplicitamente tali opzioni. Consulta [Configura la mitigazione degli errori per Qiskit Runtime](/guides/error-mitigation-and-suppression-techniques) per i dettagli.
</Admonition>

## Migra ai simulatori locali {#migrate-to-local-simulators}

Con `qiskit-ibm-runtime` 0.22.0 o versioni successive, puoi usare la modalità di test locale per sostituire i simulatori cloud. In base alle tue esigenze, esistono diversi modi per usare la modalità di test locale. Per iniziare, specifica uno dei fake backend in ``qiskit_ibm_runtime.fake_provider`` oppure specifica un backend Qiskit Aer quando istanzi una primitiva o una sessione.
...
| **Modello di rumore** | Applica automaticamente il modello di rumore dagli snapshot QPU | Personalizzato o basato sui dati di calibrazione di QPU reali | Ideale per simulazioni senza rumore          |
...
Puoi usare la modalità di test locale con i simulatori di [Qiskit Aer](/guides/simulate-with-qiskit-aer), che offre una simulazione ad alte prestazioni in grado di gestire circuiti più grandi e [modelli di rumore personalizzati](/guides/build-noise-models). Supporta inoltre la modalità di simulazione di Clifford, che può simulare in modo efficiente circuiti di Clifford con un grande numero di qubit.
