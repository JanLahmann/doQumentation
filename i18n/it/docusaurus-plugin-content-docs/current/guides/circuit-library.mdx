...
Puoi anche usare questo per assegnare questi parametri a valori reali usando un dizionario nella forma `{ Parameter: number }`. Per dimostrarlo, la seguente cella di codice assegna ogni parametro nel circuito a `0`.
...
<Admonition type="tip" title="Raccomandazioni">
  -  Scopri metodi avanzati per la creazione di circuiti nell'argomento [Costruire circuiti](/guides/construct-circuits).
