...
Nota che `print` restituisce `None` ma ha l'effetto collaterale di stampare il diagramma, mentre `QuantumCircuit.draw` restituisce il diagramma senza effetti collaterali. Poiché i notebook Jupyter mostrano l'output dell'ultima riga di ogni cella, i due sembrano avere lo stesso effetto.
...
L'argomento `fold` imposta una larghezza massima per l'output. Nel renderer `text`, questo imposta la lunghezza delle righe del diagramma prima che vada a capo. Quando si usa il renderer 'mpl', questo è il numero di livelli (visivi) prima di andare a capo.
