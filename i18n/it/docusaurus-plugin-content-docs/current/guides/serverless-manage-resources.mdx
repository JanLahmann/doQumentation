...
<Admonition type="tip">
**Qiskit Serverless sta ricevendo un aggiornamento e le sue funzionalità stanno cambiando rapidamente.** Durante questa fase di sviluppo, trova le note di rilascio e la documentazione più recente nella pagina [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html).
</Admonition>
...
Se stai leggendo le celle di codice localmente in un notebook, vedrai il [comando magic](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. L'esecuzione di celle con questo comando magic le salva su disco invece di eseguirle.
</Admonition>
...
Di seguito viene mostrato come caricare i file. Assicurati di esserti autenticato a Qiskit Serverless con il tuo account IBM Quantum (consulta [Carica su Qiskit Serverless](/guides/serverless-first-program#upload-to-qiskit-serverless) per le istruzioni).
...
- Consulta un esempio completo che [porta il codice esistente su Qiskit Serverless](/guides/serverless-port-code).
