...
Utilizza la primitiva `StatevectorSampler` di Qiskit per ricostruire una distribuzione di quasi-probabilità degli stati ottenuti dal campionamento del circuito. Per il compito di generare una matrice kernel, siamo particolarmente interessati alla probabilità di misurare lo stato |0>.
...
Calcola la probabilità di misurare $|0 \rangle$ sul circuito di sovrapposizione e popola la matrice kernel nella posizione corrispondente ai campioni rappresentati da questo particolare circuito di sovrapposizione (riga 15, colonna 20).
