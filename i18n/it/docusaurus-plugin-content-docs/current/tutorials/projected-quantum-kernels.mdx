...
- Come funzionano i kernel quantistici proiettati (PQK) e quando offrono un potenziale vantaggio quantistico.
- Come eseguire un PQK su hardware reale utilizzando un dataset del mondo reale.

## Prerequisiti {#prerequisites}

Si suggerisce agli utenti di avere familiarità con i seguenti argomenti prima di procedere con questo tutorial:
- [Kernel quantistici](/learning/courses/quantum-machine-learning/quantum-kernel-methods) dal [corso di apprendimento automatico quantistico](/learning/courses/quantum-machine-learning) in IBM Quantum&reg; Learning
...
I principali blocchi costitutivi dei PQK sono le matrici densità ridotte (RDM), ottenute attraverso misurazioni proiettive della mappa di caratteristiche quantistica. In particolare, si calcolano tipicamente le matrici densità ridotte a singolo qubit (1 RDM) per ogni qubit. Queste quantità misurate vengono poi utilizzate come input per una funzione kernel classica, come un kernel esponenziale, per costruire la matrice kernel finale.

I PQK offrono potenziali vantaggi rispetto ai [kernel quantistici](/tutorials/quantum-kernel-training) standard, in particolare per l'hardware quantistico a breve termine. I kernel quantistici standard si basano tipicamente sulla stima delle sovrapposizioni di stato globali, che diventano sempre più difficili da misurare accuratamente con l'aumentare del numero di qubit e sono altamente sensibili al rumore. Al contrario, i PQK utilizzano osservabili locali come le matrici densità ridotte a singolo qubit (1 RDM), portando a un minore overhead di campionamento, una maggiore robustezza al rumore hardware e una migliore scalabilità. Proiettando gli stati quantistici su caratteristiche di misurazione locali prima di applicare una funzione kernel classica, i PQK possono mantenere correlazioni quantistiche utili pur rimanendo più pratici per i dispositivi a breve termine.
...
Omettiamo l'esempio su simulatore su piccola scala in questo tutorial, poiché il nostro obiettivo principale è dimostrare come i kernel quantistici proiettati possano scalare a sistemi più grandi e hardware reale.
...
### Misurare le 1-RDM {#measure-1-rdms}
In questo passaggio, otteniamo tutte le matrici densità ridotte a singolo qubit (1-RDM) attraverso misurazioni proiettive della mappa di caratteristiche quantistica, che verranno successivamente fornite alla funzione kernel esponenziale classica.
...
### Definire il kernel quantistico proiettato {#define-the-projected-quantum-kernel}
...
### Macchina a Vettori di Supporto (SVM) {#support-vector-machine-svm}
...
### Benchmarking classico {#classical-benchmarking}
...
## Appendice: Verificare il potenziale vantaggio quantistico del dataset nei compiti di apprendimento {#appendix-verify-the-datasets-potential-quantum-advantage-in-learning-tasks}
...
- Corso approfondito di [apprendimento automatico quantistico](/learning/courses/quantum-machine-learning) da IBM Quantum Learning
- Tutorial sul [training del kernel quantistico](/tutorials/quantum-kernel-training)
</Admonition>
## References
