...
* Dimostrare l'uso della [backpropagation degli operatori (OBP)](https://qiskit.github.io/qiskit-addon-obp/) per ridurre la profondità del circuito. Questa tecnica elimina operazioni dalla fine del circuito al costo di più misurazioni di operatori.
...
2. Traspilazione consapevole del rumore - genera e valuta mappature alternative di qubit, insiemi di operazioni e basi di misurazione, selezionando la variante che minimizza il tempo di esecuzione QPU stimato, con parallelizzazione opzionale per accelerare la raccolta dei dati.
...
Successivamente, autenticatevi utilizzando la vostra chiave API dal dashboard della [IBM Quantum Platform](http://quantum.cloud.ibm.com/). Quindi, selezionate la Qiskit Function come segue. (Notate che per motivi di sicurezza, è meglio [salvare le credenziali del vostro account](/guides/functions-get-started#install-qiskit-functions-catalog-client) nel vostro ambiente locale, se vi trovate su una macchina affidabile, in modo da non dover inserire la vostra chiave API ogni volta che vi autenticate.)
...
Ora utilizzeremo la backpropagation degli operatori (OBP). (Consultate la [documentazione OBP](https://qiskit.github.io/qiskit-addon-obp/) per maggiori dettagli sull'addon Qiskit OBP.) Creeremo una funzione che genera le sezioni del circuito per la backpropagation:
