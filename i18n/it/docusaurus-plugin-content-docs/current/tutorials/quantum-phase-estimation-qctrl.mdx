...
Per prima cosa, autenticati utilizzando la tua [chiave API IBM Quantum](http://quantum.cloud.ibm.com/). Quindi, seleziona la Funzione Qiskit come segue. (Questo codice presuppone che tu abbia già [salvato il tuo account](/guides/functions#install-qiskit-functions-catalog-client) nel tuo ambiente locale.)
