...
Questo tutorial dimostra come costruire un `pattern Qiskit` per tagliare le porte in un circuito quantistico al fine di ridurre la profondità del circuito. Per una discussione più approfondita sul taglio dei circuiti, visita la [documentazione dell'addon Qiskit per il taglio di circuiti](https://qiskit.github.io/qiskit-addon-cutting/).
