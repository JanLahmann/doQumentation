---
description: "Eseguire un esperimento di mitigazione degli errori su scala di utilità con estrapolazione a rumore zero e amplificazione probabilistica dell'errore."
notebook_path: tutorials/probabilistic-error-amplification.ipynb
sidebar_label: "Mitigazione degli errori su scala di utilità con amplificazione probabilistica dell'errore"
title: "Mitigazione degli errori su scala di utilità con amplificazione probabilistica dell'errore"
---
...
*Stima di utilizzo: 14 minuti su un processore Heron r3 (NOTA: Questa è solo una stima. Il vostro tempo di esecuzione potrebbe variare.)*

## Risultati di apprendimento {#learning-outcomes}
Dopo aver completato questo tutorial, gli utenti dovrebbero comprendere:
- La teoria alla base dell'*estrapolazione a rumore zero* (ZNE), i diversi metodi per amplificare il rumore e perché l'*amplificazione probabilistica dell'errore* (PEA) è preferita per esperimenti su scala di utilità.
- Come implementare la ZNE con PEA in pratica utilizzando Qiskit.

## Prerequisiti {#prerequisites}
Si suggerisce che gli utenti abbiano familiarità con i seguenti argomenti prima di seguire questo tutorial:
- La [lezione sulla mitigazione degli errori](/learning/courses/utility-scale-quantum-computing/error-mitigation) del corso *Utility-scale quantum computing* per conoscenze di base sull'utilizzo della mitigazione degli errori in Qiskit.
- La [lezione Utility-I](/learning/courses/utility-scale-quantum-computing/utility-i) del corso *Utility-scale quantum computing* per ulteriori informazioni sull'esperimento su scala di utilità utilizzato come esempio in questo tutorial.

## Contesto {#background}

Questo tutorial dimostra come eseguire un esperimento di mitigazione degli errori su scala di utilità con IBM Quantum Compute Service utilizzando una versione sperimentale dell'*estrapolazione a rumore zero* (ZNE) con *amplificazione probabilistica dell'errore* (PEA).
...
* Lo stretching degli impulsi presuppone che il rumore del gate sia proporzionale alla durata, il che tipicamente non è vero. Anche la calibrazione è costosa.

* Il gate folding richiede grandi fattori di stretch che limitano notevolmente la profondità dei circuiti che possono essere eseguiti.

* La PEA può essere applicata a qualsiasi circuito che può essere eseguito con fattore di rumore nativo ($\lambda=1$) ma richiede l'apprendimento del modello di rumore.


### Apprendere il modello di rumore per la PEA {#learn-the-noise-model-for-pea}
La PEA presuppone lo stesso modello di rumore basato su layer della *probabilistic error cancellation* (PEC); tuttavia, evita l'overhead di campionamento che scala esponenzialmente con il rumore del circuito.
...
**Riferimento**: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, *Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors* [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)
...
Nella cella sottostante, importiamo i pacchetti rilevanti e creiamo alcune funzioni di supporto per costruire i circuiti per l'evoluzione temporale trotterizzata di un modello di Ising bidimensionale a campo trasverso che aderisce alla topologia del backend.
...
Salteremo questo passaggio poiché la mitigazione degli errori in runtime non è supportata sui simulatori.
...
##### Stabilire un backend {#establish-a-backend}
...
* Scegli un layout dei qubit che mappa i qubit virtuali del tuo circuito su qubit fisici sull'hardware.

* Inserisci gate swap secondo necessità per instradare le interazioni tra qubit che non sono connessi.

* Traduci i gate del nostro circuito in istruzioni [Instruction Set Architecture (ISA)](/guides/transpile#instruction-set-architecture) che possono essere eseguite direttamente sull'hardware.

* Esegui ottimizzazioni del circuito per minimizzare la profondità del circuito e il numero di gate.
...
Notate che ogni osservabile è racchiusa in una lista nel seguente blocco di codice. È fatto per effettuare il *broadcast* con i valori dei parametri in modo che ogni osservabile di qubit venga misurata per ogni valore di theta. Trovate le regole di broadcasting per le primitive nella [documentazione delle primitive](/guides/primitives).
...
* **amplifier**: Il metodo da utilizzare per amplificare il rumore ai fattori di rumore previsti.
  ...
  modello di rumore Pauli-twirled per gli strati di gate base a due qubit twirled. Sono disponibili anche le opzioni `"gate_folding_front"` e `"gate_folding_back"`, spiegate nella [documentazione API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier).

* **extrapolated\_noise\_factors**: Specificare uno o più valori di fattore di rumore ai quali valutare i
...
1. `pub_result.data.evs`: Valori di aspettazione corrispondenti al rumore zero (basati sull'estrapolazione euristicamente migliore).
   ...
   * Il secondo asse indicizza il valore del parametro per $\theta$ ($12$ valori del parametro)
...
- Un [tutorial](/tutorials/combine-error-mitigation-techniques) focalizzato sulla combinazione di tecniche di mitigazione degli errori.
- Documentazione dettagliata [documentazione](/guides/error-mitigation-and-suppression-techniques) sulle tecniche di mitigazione degli errori disponibili in Qiskit.
- Lezioni aggiuntive sugli esperimenti su scala di utilità: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) e [Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
