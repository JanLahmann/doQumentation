...
* Conoscenza di base di [Qiskit SDK](https://docs.quantum.ibm.com/guides) e [IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime), e accesso a un [account IBM Quantum](/guides/cloud-setup).
...
Questo tutorial dimostra l'esperimento su un dispositivo IBM Quantum&reg; per illustrare come i circuiti dinamici possono essere utilizzati per generare e valutare l'entanglement tra qubit distanti. Mapperemo tutte le catene lineari a quattro qubit sul dispositivo, eseguiremo il circuito di teletrasporto su ciascuna, e poi visualizzeremo la distribuzione dei valori MSE. Questa procedura end-to-end mostra come sfruttare IBM Quantum Compute Service e le funzionalità dei circuiti dinamici per informare scelte consapevoli dell'hardware per tagliare circuiti o distribuire algoritmi quantistici attraverso un sistema modulare.
...
Prima di eseguire sulla QPU reale, verifichiamo che il circuito produca una coppia di Bell testandolo su un simulatore privo di rumore con una catena a quattro qubit `[0, 1, 2, 3]`. Utilizziamo il `Sampler` di IBM Quantum con `AerSimulator` come modalità backend per eseguire i circuiti.
...
I risultati rivelano un'ampia gamma di qualità dell'entanglement nel dispositivo. Questo conferma la scoperta del paper secondo cui può esserci una variazione di oltre un ordine di grandezza nella fedeltà dello stato di Bell a seconda di quali qubit fisici vengono utilizzati. In termini pratici, questo significa che certe regioni o collegamenti nel chip sono molto migliori nell'eseguire operazioni di misurazione a metà circuito e feedforward rispetto ad altri. Fattori come l'errore di lettura del qubit, la durata del qubit e il crosstalk contribuiscono probabilmente a queste differenze. Ad esempio, se una catena include un qubit di lettura particolarmente rumoroso, la misurazione a metà circuito potrebbe essere inaffidabile, portando a una scarsa fedeltà per quella coppia entangled (MSE alto).
Infine, visualizziamo le prestazioni complessive tracciando la funzione di distribuzione cumulativa (CDF) dei valori di MSE per tutte le catene. Il grafico CDF mostra la soglia di MSE sull'asse x, e la frazione di coppie di qubit che hanno al massimo quell'MSE sull'asse y. Questa curva inizia da zero e si avvicina a uno man mano che la soglia cresce fino a comprendere tutti i punti dati. Una salita ripida vicino a un MSE basso indicherebbe che molte coppie hanno alta fedeltà; una salita lenta significa che molte coppie hanno errori maggiori. Annotiamo la CDF con le identità delle migliori coppie. Nel grafico, ogni punto nella CDF corrisponde all'MSE di una catena a quattro qubit, ed etichettiamo il punto con la coppia di indici di qubit $[q0, q3]$ che erano entangled in quell'esperimento. Questo rende facile individuare quali coppie di qubit fisici sono le migliori (i punti più a sinistra nella CDF).
