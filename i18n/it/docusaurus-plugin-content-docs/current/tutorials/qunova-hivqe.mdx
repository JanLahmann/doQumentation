---
description: 'Utilizzate Qunova HiVQE per calcolare le curve della superficie di energia potenziale (PES) di dissociazione.'
notebook_path: tutorials/qunova-hivqe.ipynb
sidebar_label: 'Curve PES di dissociazione con Qunova HiVQE'
...
Le Qiskit Functions sono una funzionalità sperimentale disponibile solo per gli utenti IBM Quantum&reg; Premium Plan, Flex Plan e On-Prem (tramite IBM Quantum Platform API) Plan. Sono in stato di rilascio di anteprima e soggette a modifiche.
</Admonition>
...
Definite le geometrie di $Li_2S$ in formato dizionario per diverse distanze di legame Li-S per calcolare la curva PES. Queste geometrie sono ottimizzate utilizzando calcoli B3LYP/631g.
...
### Passo 2 e 3: Ottimizzare il problema per l'esecuzione sull'hardware quantistico ed eseguirlo utilizzando la funzione HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}
...
L'intero runtime della QPU utilizzato nel job può essere tracciato effettuando l'accesso a [IBM Quantum Platform](https://quantum.cloud.ibm.com/) e visualizzando i job inviati con il tag `qunova-chemistry-hivqe`.

### Passo 4: Post-elaborazione e confronto con i metodi classici {#step-4-post-process-and-compare-with-classical-methods}
...
### Tracciare la curva di dissociazione per Li\_2S {#plotting-the-dissociation-curve-for-li-2s}

Tracciamo e confrontiamo i risultati HiVQE con HF e CASCI.
Potete osservare che tutti i calcoli HiVQE corrispondono bene al risultato di riferimento classico (CASCI).
...
Notate che per eseguire la funzione sul sistema FeP-NO con le impostazioni mostrate in questo esempio, è necessaria una licenza che consenta l'utilizzo della funzione con almeno 44 qubit. Inviate un'email a [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) per informazioni su come ottenere una licenza.

### Passo 1: Mappare gli input classici a un problema quantistico {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
### Passo 2 e 3: Ottimizzare il problema per l'esecuzione su hardware quantistico ed eseguire utilizzando la funzione HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

Sulla base della configurazione di HiVQE e delle geometrie, ottenete i risultati in sequenza.

#### Inviate il calcolo per d(Fe-N) = 1.75 $\AA$. {#submit-dfe-n-175-aa-calculation}
...
Tracciate il job e recuperate il risultato per il calcolo d(Fe-N) = 1.75 $\AA$.
...
#### Inviate il calcolo per d(Fe-N) = 2.00 $\AA$. {#submit-dfe-n-200-aa-calculation}
...
Tracciate il job e recuperate il risultato per il calcolo d(Fe-N) = 2.00 $\AA$.
...
#### Inviate il calcolo per d(Fe-N) = 5.00 $\AA$. {#submit-dfe-n-500-aa-calculation}
...
Tracciate il job e recuperate il risultato per il calcolo d(Fe-N) = 5.00 $\AA$.
...
### Passo 4: Post-elaborare e confrontare con i metodi classici {#step-4-post-process-and-compare-with-classical-methods-1}
...
Vi preghiamo di completare questo breve sondaggio per fornire un feedback su questo tutorial. Le vostre osservazioni ci aiuteranno a migliorare i nostri contenuti e l'esperienza utente.

> **Nota:** Questo sondaggio è fornito da **IBM Quantum** e riguarda i **contenuti del tutorial** (scritti da IBM). doQumentation fornisce il sito web, le traduzioni e l'esecuzione del codice — per feedback su questi aspetti, apri una [segnalazione su GitHub](https://github.com/JanLahmann/doQumentation/issues).
