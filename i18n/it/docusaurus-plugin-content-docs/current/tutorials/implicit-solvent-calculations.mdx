---
description: 'Impara a calcolare gli effetti del solvente implicito su hardware quantistico usando SQD e IEF-PCM.'
notebook_path: tutorials/implicit-solvent-calculations.ipynb
sidebar_label: 'Calcoli con solvente implicito usando Qiskit Serverless'
title: 'Calcoli con solvente implicito usando Qiskit Serverless'
---
...
*Stima di utilizzo: 2 minuti su un processore Heron r2 (NOTA: questa è solo una stima. Il tuo tempo di esecuzione può variare.)*
...
Dopo aver seguito questo tutorial, gli utenti dovrebbero capire:

* Come configurare ed eseguire un flusso di lavoro remoto usando [Qiskit Serverless](/guides/serverless)

* Come calcolare gli effetti del solvente implicito usando un computer quantistico


## Prerequisiti {#prerequisites}

Ti consigliamo che gli utenti abbiano familiarità con il seguente argomento prima di seguire questo tutorial:
...
I calcoli con solvente implicito sono frequentemente utilizzati nella biofisica computazionale. Questi modelli descrivono come un composto soluto interagisce con un solvente, senza modellare direttamente il sistema del solvente. Invece, viene fatta un'approssimazione in cui il modello del sistema del soluto è avvolto in una rappresentazione matematica di un mezzo dielettrico caratterizzato empiricamente. Questa approssimazione dielettrica interagisce quindi con il soluto, che è esso stesso modellato direttamente. Il mezzo dielettrico influenza le caratteristiche del sistema del soluto, come la sua energia dello stato fondamentale, interagendo con il suo campo elettronico. Questo è importante per i modelli biofisici usati, ad esempio, nella scoperta di farmaci, perché i composti si comportano in modo diverso in diversi ambienti dielettrici. Modellare un composto nell'aria (in vacuo) descriverà un comportamento diverso rispetto a modellarlo in acqua. Poiché i composti farmaceutici devono entrare nel corpo umano, che è esso stesso composto principalmente di acqua, è utile modellare un composto in una soluzione come l'acqua invece che in vacuo. Con i modelli di solvente implicito, possiamo ottenere questo comportamento a basso costo, sebbene il risultato finale sarà generalmente più approssimato rispetto alla controparte: i modelli di solvente esplicito, più costosi computazionalmente, che creano rappresentazioni dirette sia del soluto che delle molecole di solvente.

In questo tutorial, dimostriamo come un algoritmo quantistico, la Sample-based Quantum Diagonalization (SQD), può essere integrato in un modello di solvente implicito relativamente economico dal punto di vista computazionale. Nell'esempio, descriviamo come si comporta la metilammina quando si dissolve in acqua. Confrontiamo l'algoritmo quantistico con un metodo di confronto classico allo stato dell'arte chiamato CASCI e dimostriamo una stretta concordanza tra questi calcoli. Mostriamo un'architettura di supercalcolo quantum-centrica in miniatura, delegando l'elaborazione classica computazionalmente costosa della parte di campionamento quantistico della routine a un ambiente basato su cloud all'interno di Qiskit Serverless. Il codice dimostra anche la parallelizzazione tra i core CPU disponibili da remoto per migliorare il tempo di calcolo.

Qiskit Serverless è un framework per eseguire carichi di lavoro quantistici e classici distribuiti senza gestire l'infrastruttura. Non c'è provisioning del server (nessun avvio di EC2, cluster, container Docker), nessuno strumento di orchestrazione (Kubernetes, Docker Swarm) e nessun monitoraggio/manutenzione. Ogni job Serverless viene eseguito in un container pulito, esegue il tuo codice, e poi si spegne. Non c'è memoria tra i job. Semplicemente scrivi il tuo codice e poi invii il tuo job. All'interno di un job Serverless, un programma può accedere senza problemi ai backend IBM Quantum&reg; ed elaborare classicamente i risultati al suo interno. Con Qiskit Serverless, gli utenti possono accedere a core CPU e memoria remoti sempre attivi, il che consente la distribuzione di certi carichi di lavoro classici su risorse remote. Gli utenti ottengono anche qualche vantaggio nell'elaborazione parallela dei programmi evitando i comuni problemi legati allo spegnimento del dispositivo durante l'esecuzione. Per maggiori informazioni su Qiskit Serverless, vedi la sua [documentazione](/guides/serverless), così come materiale aggiuntivo su [GitHub](https://qiskit.github.io/qiskit-serverless/index.html).

Questo tutorial mostra un'applicazione rilevante di quanto segue:

* Sample-based quantum diagonalization

* Modelli computazionali client-server per il calcolo quantistico


Questo tutorial è ispirato e basato sulla ricerca condotta presso la Cleveland Clinic, come descritto in [Kaliakin, Danil, et al. "Implicit solvent sample-based quantum diagonalization." The Journal of Physical Chemistry B 129.23 (2025): 5788-5796](https://pubs.acs.org/doi/10.1021/acs.jpcb.5c01030), che espone il flusso di lavoro completo per i calcoli con solvente implicito ed lo estende con l'auto-consistenza iterativa del solvente ("The Heartwood Algorithm", M. Motta, T. Pellegrini, 2025), l'ottimizzazione della geometria, e la selezione automatica del layout dei qubit. Fai riferimento al [modello di funzione Qiskit SQD IEF-PCM](/guides/function-template-chemistry-workflow), sviluppato congiuntamente da Cleveland Clinic e IBM&reg; sulla base della ricerca di Cleveland Clinic, per un'interfaccia black-box semplificata per eseguire calcoli con solvente implicito.
...
* Addon Qiskit: Sample-based quantum diagonalization (SQD) v0.12.0 `pip install qiskit_addon_sqd`
...
Crea una directory esattamente accanto al programma notebook principale chiamata `source_files`. Metterai i file Python in questa directory che intendi condividere con l'ambiente di calcolo remoto. Devi creare due file:
...
Clicca per espandere il testo di ogni script qui sotto, poi copia e incolla il contenuto in un file locale con questi nomi di percorso.

<Accordion>
  <AccordionItem title="Clicca per visualizzare `source_files\diagonalization_engine.py`">
    
  </AccordionItem>

  <AccordionItem title="Clicca per visualizzare `source_files\classical_simulation.py`">
...
Dobbiamo condividere il programma destinato all'esecuzione nell'ambiente cloud, e ricaricarlo ogni volta che modifichiamo il suo codice sorgente:
...
Questo tutorial non fa uso di un simulatore su piccola scala perché l'intento è dimostrare un'applicazione quantistica scalabile che va oltre il regime dell'esplorazione con simulatore. Invece, mostriamo più avanti come questo metodo può essere implementato usando un metodo di confronto classico allo stato dell'arte chiamato CASCI.
...
### 1.1: Inizializza l'oggetto molecola usando la geometria molecolare $\textit{a priori}$ nota {#11-initialize-molecule-object-using-known-textita-priori-molecular-geometry}
...
### 1.2: Definisci gli effetti di solvatazione usando il Modello a Continuo Polarizzabile (PCM) {#12-define-solvation-effects-using-the-polarizable-continuum-model-pcm}
...
### 1.3: Ottimizzazione della geometria usando TRIC {#13-geometry-optimization-using-tric}
...
### 1.4: Prepara il modello a continuo e l'oggetto campo medio con le variabili rilevanti {#14-prepare-continuum-model-and-mean-field-object-with-relevant-variables}
...
Per maggiori informazioni sulle funzioni di supporto dimostrate qui, vedi il tutorial [Sample-based quantum diagonalization di un'Hamiltoniana chimica](/tutorials/sample-based-quantum-diagonalization).
...
## Passi 3 e 4: Esegui usando Qiskit ed elabora usando Qiskit Serverless {#steps-3-4-execute-using-qiskit-and-post-process-using-qiskit-serverless}

Qui, combiniamo il Passo 3 (Esegui) e il Passo 4 (Elabora) perché il contesto applicativo del modello di solvente implicito richiede un ciclo di feedback iterativo che esegue diversi cicli di esecuzione ed elaborazione per migliorare il calcolo finale.

### 3.1 Calcola l'energia di Hartree-Fock ristretta {#31-compute-the-restricted-hartree-fock-energy}
...
### 3.2: Stabilisci l'energia di riferimento classica con CASCI {#32-establish-classical-reference-energy-with-casci}
...
## Configura i parametri dell'applicazione {#configure-application-parameters}
...
<Admonition type="tip" title="Raccomandazioni">
  Se hai trovato interessante questo lavoro, potresti essere interessato alle seguenti risorse:

  * L'[Interfaccia di Gestione delle Risorse Quantistiche (QRMI)](https://github.com/qiskit-community/qrmi) — integra risorse quantistiche e classiche in gestori di carichi di lavoro HPC come Slurm, estendendo il pattern di calcolo distribuito dimostrato in questo tutorial

  * [Guida a Qiskit Serverless](/guides/serverless)

  * [Qiskit Serverless su GitHub](https://qiskit.github.io/qiskit-serverless/index.html)

  * Tutorial [Sample-based quantum diagonalization di un'Hamiltoniana chimica](/tutorials/sample-based-quantum-diagonalization)

  * [Panoramica della Sample-based quantum diagonalization (SQD)](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd)

  * [Distribuisci ed esegui un modello per la simulazione della struttura elettronica con un modello di solvente implicito](/guides/function-template-chemistry-workflow) (modello di funzione Qiskit SQD IEF-PCM, sviluppato congiuntamente da Cleveland Clinic e IBM)
</Admonition>
