...
description: "Confronta i metodi di compilazione SABRE, transpiler AI e Rustiq su circuiti di simulazione hamiltoniana da Hamlib."
...
*Stima di utilizzo: meno di 1 minuto su un processore IBM Heron (NOTA: questa è solo una stima. Il tempo effettivo potrebbe variare.)*
...
Al termine di questo tutorial, capirai:
- Come usare il Qiskit transpiler con SABRE per l'ottimizzazione del layout e del routing
- Come sfruttare il transpiler AI per un'ottimizzazione avanzata dei circuiti
- Come usare il plugin Rustiq per sintetizzare le operazioni `PauliEvolutionGate` nei circuiti di simulazione hamiltoniana
- Come eseguire benchmark e confrontare i metodi di compilazione usando la profondità a due qubit, il conteggio totale dei gate e il tempo di esecuzione

## Prerequisiti {#prerequisites}
Prima di seguire questo tutorial, ti consigliamo di avere familiarità con i seguenti argomenti:
- [Concetti di transpilazione](/guides/transpile)
- [Fasi del transpiler](/guides/transpiler-stages)
- [Transpilazione con pass manager](/guides/transpile-with-pass-managers)
## Contesto {#background}

La compilazione dei circuiti quantistici trasforma un algoritmo quantistico ad alto livello in un circuito fisico che rispetta i vincoli dell'hardware target. Una compilazione efficace può ridurre significativamente la profondità del circuito e il numero di gate, entrambi i quali influiscono direttamente sulla qualità dei risultati sui dispositivi quantistici a breve termine.

Questo tutorial mette a benchmark tre metodi di compilazione su circuiti di simulazione hamiltoniana costruiti con `PauliEvolutionGate`. Questi circuiti modellano interazioni a coppie tra qubit (come i termini $ZZ$, $XX$ e $YY$) e sono comuni nella chimica quantistica, nella fisica della materia condensata e nella scienza dei materiali.

I circuiti di benchmark provengono dalla collezione [Hamlib](https://github.com/SRI-International/QC-App-Oriented-Benchmarks/tree/master/qedcbench/hamlib#hamlib-simulation---benchmark-program), accessibile tramite il repository [Benchpress](https://github.com/Qiskit/benchpress). Hamlib fornisce un insieme standardizzato di Hamiltoniani rappresentativi, rendendo possibile confrontare le strategie di compilazione su carichi di lavoro di simulazione realistici.

### Panoramica dei metodi di compilazione {#compilation-methods-overview}

#### Qiskit transpiler con SABRE {#qiskit-transpiler-with-sabre}
Il Qiskit transpiler usa l'algoritmo SABRE (SWAP-based BidiREctional heuristic search) per ottimizzare il layout e il routing del circuito. SABRE si concentra sulla minimizzazione dei gate SWAP e del loro impatto sulla profondità del circuito, rispettando i vincoli di connettività dell'hardware. È un metodo general-purpose che offre un buon equilibrio tra prestazioni e tempo di compilazione. Per ulteriori dettagli, vedi [\[1\]](https://arxiv.org/abs/2409.08368). I vantaggi e l'esplorazione dei parametri di SABRE sono trattati in profondità in un [tutorial](/tutorials/transpilation-optimizations-with-sabre) separato.

#### Transpiler AI {#ai-powered-transpiler}
Il transpiler AI usa il machine learning per prevedere strategie di transpilazione ottimali analizzando i pattern nella struttura del circuito e i vincoli hardware. Può anche applicare il pass `AIPauliNetworkSynthesis`, che si rivolge ai circuiti di rete Pauli usando un approccio di sintesi basato sull'apprendimento per rinforzo. Per ulteriori informazioni, vedi [\[2\]](https://arxiv.org/abs/2405.13196) e [\[3\]](https://arxiv.org/abs/2503.14448).

#### Plugin Rustiq {#rustiq-plugin}
Il plugin Rustiq fornisce tecniche avanzate di sintesi specificamente per le operazioni `PauliEvolutionGate`, che rappresentano rotazioni di Pauli comunemente utilizzate nelle dinamiche trotterizzate. È progettato per produrre decomposizioni di circuiti a bassa profondità per i carichi di lavoro di simulazione hamiltoniana. Per ulteriori dettagli, vedi [\[4\]](https://arxiv.org/abs/2404.03280).

### Metriche chiave {#key-metrics}

Confrontiamo i tre metodi sulle seguenti metriche:
- **Profondità a due qubit**: La profondità del circuito contando solo i gate a due qubit. Questo è spesso il collo di bottiglia per la fedeltà sull'hardware reale.
- **Dimensione del circuito (conteggio totale dei gate)**: Il numero totale di gate nel circuito transpilato.
- **Tempo di esecuzione**: Il tempo reale per la transpilazione.
...
### Connessione a un backend {#connect-to-a-backend}

Seleziona un backend che verrà usato sia per gli esempi su piccola scala che per quelli su larga scala. Il backend determina la mappa di connettività e i gate base che il transpiler utilizza come target.
...
### Definire i pass manager {#define-pass-managers}

Configura i tre metodi di compilazione.
...
### Definire le funzioni di supporto {#define-helper-functions}

La seguente funzione transpila una lista di circuiti usando un dato pass manager e registra le metriche chiave (profondità a due qubit, dimensione del circuito e tempo di esecuzione) per ciascun circuito.
...
### Caricare i circuiti hamiltoniani da Hamlib {#load-hamiltonian-circuits-from-hamlib}

Carichiamo un insieme rappresentativo di Hamiltoniani dal repository Benchpress e costruiamo circuiti `PauliEvolutionGate`. I circuiti che superano il numero di qubit del backend vengono rimossi, insieme ai circuiti la cui dimensione decomposta supera 1.500 gate (per mantenere i tempi di transpilazione ragionevoli).
...
Dividiamo i circuiti in gruppi su piccola scala (meno di 20 qubit) e su larga scala (20 o più qubit).
...
Anteprima di uno dei circuiti hamiltoniani su piccola scala prima della transpilazione.
...
In questa sezione, eseguiamo il benchmark dei tre metodi di compilazione su circuiti hamiltoniani con meno di 20 qubit. Questi circuiti si transpilano rapidamente e forniscono una visione chiara di come ciascun metodo gestisce circuiti di complessità moderata.
### Passo 1: Mappare gli input classici a un problema quantistico {#step-1-map-classical-inputs-to-a-quantum-problem}

Ciascun Hamiltoniano è codificato come un circuito `PauliEvolutionGate`. I circuiti sono già stati costruiti nella sezione di configurazione dai dati di benchmark Hamlib.
...
Transpiliamo tutti i circuiti su piccola scala usando ciascuno dei tre pass manager, poi raccogliamo le metriche.
...
La tabella seguente riassume la media e la deviazione standard di ciascuna metrica su tutti i circuiti su piccola scala, insieme alla percentuale di miglioramento rispetto a SABRE. Poiché le dimensioni dei circuiti variano notevolmente, la deviazione standard fornisce un contesto importante per interpretare le medie.
...
La tabella per circuito mostra come ciascun metodo si confronta su singoli circuiti. Il valore migliore per ciascuna metrica è contrassegnato con un asterisco. Nota che per i circuiti più semplici, tutti e tre i metodi spesso convergono allo stesso risultato.
...
#### Visualizzare i risultati {#visualize-results}

I grafici seguenti confrontano i tre metodi su ciascuna metrica per singolo circuito. I circuiti sono ordinati per numero di qubit e identificati per indice sull'asse x, poiché più circuiti possono condividere lo stesso numero di qubit.
...
A questa scala, tutti e tre i pass manager si comportano bene e i loro risultati medi sono vicini tra loro. Ciò è dovuto principalmente al fatto che i circuiti piccoli lasciano poco spazio per ulteriori ottimizzazioni, quindi i metodi tendono a convergere verso soluzioni simili.

In questo esempio, Rustiq produce i risultati più variabili, con i maggiori outlier sia nella profondità a due qubit che nel conteggio dei gate. Sebbene questa variabilità significhi che a volte rimane indietro, significa anche che Rustiq trova occasionalmente soluzioni migliori degli altri due metodi. Il transpiler AI è più stabile nei suoi risultati rispetto a SABRE e Rustiq, seguendo da vicino la maggior parte dei circuiti senza molti outlier.

Per il tempo di esecuzione, SABRE e Rustiq sono entrambi veloci, mentre il transpiler AI è notevolmente più lento su certi circuiti.
#### Metodo con le migliori prestazioni per metrica {#best-performing-method-by-metric}

Il grafico seguente mostra con quale frequenza ciascun metodo ha raggiunto il valore migliore (più basso) per ciascuna metrica. I pareggi sono possibili: per i circuiti più semplici, più metodi possono raggiungere la stessa profondità a due qubit o conteggio di gate ottimale. Quando si verifica un pareggio, tutti i metodi in pareggio ricevono credito, quindi le percentuali per una data metrica possono sommare a più del 100%.
...
In questo esempio, i tre metodi si comportano in modo molto simile sui circuiti su piccola scala. Sulla profondità a due qubit e sul conteggio dei gate, la quota di circuiti in cui ciascun metodo è il migliore è simile (circa 35–55%), e molti circuiti finiscono in pareggio perché i circuiti più semplici spesso hanno un'unica soluzione ottimale che più metodi trovano. La differenza più evidente è il tempo di esecuzione: SABRE e Rustiq sono ciascuno i più veloci su circa metà dei circuiti, mentre il transpiler AI è raramente il più rapido. Considerando tutte e tre le metriche insieme, Rustiq ha un leggero vantaggio complessivo &nbsp; è il vincitore più frequente sulla profondità a due qubit e rimane competitivo sul conteggio dei gate e sul tempo di esecuzione.
### Passo 3: Eseguire usando le primitive di Qiskit {#step-3-execute-using-qiskit-primitives}

Per valutare come la qualità della transpilazione influisce sull'esecuzione sotto rumore, usiamo una tecnica di **circuito specchio**. Per ogni circuito transpilato $U$, aggiungiamo il suo inverso $U^\dagger$ in modo che il circuito combinato $U^\dagger U$ sia teoricamente l'identità. Partendo dallo stato $|0\rangle$, un'esecuzione perfetta (senza rumore) restituirebbe la stringa di bit tutti-zeri con probabilità 1.

In pratica, gli errori dei gate si accumulano durante il circuito, quindi la probabilità di recuperare $|0\rangle^{\otimes n}$ diminuisce. Un metodo di compilazione che produce un circuito più superficiale con meno gate accumula meno rumore.

L'approccio del circuito specchio è semplice e scala a qualsiasi dimensione del circuito, poiché l'output atteso è sempre $|0\rangle^{\otimes n}$ e non è richiesta alcuna simulazione classica dello stato ideale. Tuttavia, nota le seguenti avvertenze: il circuito specchio è un proxy per il circuito effettivo (non il circuito stesso), raddoppia il conteggio dei gate (il che esagera l'effetto del rumore) e può sottostimare certi errori quando il rumore si annulla simmetricamente attraverso il confine specchio.

Selezioniamo il circuito all'indice 6 dall'insieme su piccola scala ed eseguiamo i circuiti specchio su un simulatore Aer con un semplice modello di rumore depolarizzante.
...
Costruiamo i circuiti specchio (aggiungendo $U^\dagger$), rimappiamo agli indici di qubit contigui in modo che il simulatore gestisca solo i qubit attivi, ed eseguiamo su un simulatore Aer rumoroso.
...
Il metodo con la profondità a due qubit più bassa e il minor numero di gate raggiunge la fedeltà più alta, coerentemente con l'aspettativa che i circuiti più corti accumulino meno rumore. Anche differenze modeste nella profondità e nel conteggio dei gate si traducono in differenze misurabili nella fedeltà sotto il modello di rumore depolarizzante.

Tieni presente che questi risultati si riferiscono a un singolo circuito. Il ranking relativo dei metodi può cambiare da circuito a circuito a seconda della struttura dell'Hamiltoniano.
## Esempio su larga scala con hardware reale {#large-scale-hardware-example}

In questa sezione, eseguiamo il benchmark degli stessi tre metodi di compilazione su circuiti hamiltoniani con 20 o più qubit. Questi circuiti sono più rappresentativi dei carichi di lavoro pratici di simulazione hamiltoniana e testano come ciascun metodo scala in termini di qualità del circuito e tempo di compilazione.
### Passi 1-4 combinati {#steps-1-4-combined}

Il flusso di lavoro segue la stessa struttura dell'esempio su piccola scala. Transpiliamo tutti i circuiti su larga scala con ciascun metodo, raccogliamo le metriche e inviamo un circuito specchio all'hardware quantistico reale.
...
## Analisi dei risultati di compilazione {#analysis-of-compilation-results}

I benchmark sopra confrontano SABRE, il transpiler AI e Rustiq su circuiti di simulazione hamiltoniana dalla collezione Hamlib, sia su piccola che su larga scala.

### Profondità a due qubit e conteggio dei gate {#two-qubit-depth-and-gate-count}

Su larga scala, SABRE e il transpiler AI sono i due performer più forti, e ciascuno guida su una metrica diversa. Come mostra il grafico *metodo con le migliori prestazioni per metrica*, SABRE produce il minor conteggio di gate sulla grande maggioranza dei circuiti ed è il metodo più veloce su quasi tutti, coerentemente con un'euristica progettata per minimizzare i gate SWAP inseriti e con le recenti ottimizzazioni al suo layout e routing. Il transpiler AI produce la profondità a due qubit più bassa sulla maggior parte dei circuiti, coerentemente con la parte del suo obiettivo di apprendimento per rinforzo che punta alla profondità del circuito. La tabella riepilogativa riflette la stessa divisione: SABRE ha il minor conteggio medio di gate, mentre il transpiler AI ha la minor profondità media a due qubit. Entrambi i metodi sono coerenti e affidabili sull'intera gamma di circuiti.

Rustiq, costruito appositamente per la sintesi di `PauliEvolutionGate`, produce il singolo miglior risultato solo su una piccola frazione dei circuiti su larga scala. Le sue metriche medie sono fortemente distorte da alcuni outlier significativi, visibili come grandi picchi nel grafico di confronto della compilazione, dove Rustiq produce una profondità e un conteggio dei gate sostanzialmente superiori agli altri metodi. Senza questi outlier, le sue prestazioni medie sarebbero molto più vicine a SABRE e al transpiler AI.

L'osservazione chiave è che nessun metodo domina su tutti i circuiti. Ciascun metodo supera gli altri in casi specifici, il che rende utile provare tutti gli strumenti disponibili e selezionare il miglior risultato per ciascun circuito.

### Tempo di esecuzione {#runtime}

SABRE è costantemente il metodo più veloce. Rustiq generalmente gira a una velocità simile, ma può produrre outlier dove la compilazione richiede significativamente più tempo. Ciò è particolarmente visibile nei risultati su larga scala, dove alcuni circuiti causano picchi nel tempo di esecuzione di Rustiq. Questi outlier influenzano fortemente il tempo medio, quindi la mediana può essere un riepilogo più rappresentativo per Rustiq. Il transpiler AI è il più lento dei tre, con un tempo di esecuzione che cresce notevolmente su circuiti più grandi e complessi.

### Risultati del circuito specchio {#mirror-circuit-results}

Gli esperimenti del circuito specchio confermano la tendenza attesa: i metodi che producono una profondità a due qubit inferiore e meno gate raggiungono una fedeltà più alta sotto rumore. Questo vale sia sul simulatore rumoroso (piccola scala) che sull'hardware reale (larga scala).

Tieni presente che ciascun grafico del circuito specchio riflette un singolo circuito, non l'aggregato. L'esempio hardware sopra usa un circuito `tfim` a 26 qubit, che si rivela essere un caso in cui SABRE produce una profondità a due qubit molto più alta rispetto al transpiler AI e a Rustiq, quindi la sua fedeltà è corrispondentemente molto più bassa. Questo non è rappresentativo dei risultati più ampi: sull'intero insieme dei circuiti su larga scala, la profondità a due qubit di SABRE è solitamente vicina a quella del transpiler AI, e i due metodi guidano ciascuno su metriche diverse (il transpiler AI sulla profondità a due qubit, SABRE sul conteggio dei gate e sul tempo di esecuzione). Un singolo risultato specchio testa una versione raddoppiata di un circuito anziché l'intero carico di lavoro, quindi non dovrebbe essere letto come un verdetto sulla qualità complessiva del metodo.

### Raccomandazioni {#recommendations}

Non esiste una singola strategia di transpilazione migliore per tutti i circuiti. La scelta migliore dipende dalla struttura del circuito, dall'obiettivo di ottimizzazione e dal budget di tempo di compilazione disponibile:

- **SABRE** è il default raccomandato. È veloce e affidabile, e produce risultati solidi su un'ampia gamma di circuiti. Per una regolazione ulteriore, gli utenti possono aumentare le prove di layout e routing (vedi il [tutorial di ottimizzazione SABRE](/tutorials/transpilation-optimizations-with-sabre)).
- **Il transpiler AI** vale la pena provare quando il tempo di compilazione non è un vincolo, specialmente quando la minimizzazione della profondità a due qubit è la priorità: ha prodotto la profondità a due qubit più bassa sulla maggior parte dei circuiti su larga scala in questo benchmark.
- **Rustiq** è costruito appositamente per i circuiti `PauliEvolutionGate` e può trovare soluzioni a profondità molto bassa e basso conteggio di gate, in particolare su circuiti più piccoli. Su circuiti più grandi può occasionalmente produrre risultati molto più grandi, quindi è meglio usarlo come uno dei vari metodi da provare piuttosto che come default.

In pratica, il miglior approccio è eseguire tutti i metodi disponibili e scegliere il miglior risultato per ciascun circuito. Il costo di compilazione nel provare più metodi è piccolo rispetto al potenziale miglioramento nella qualità di esecuzione sull'hardware reale.
## Passi successivi {#next-steps}

Se hai trovato utile questo tutorial, potresti essere interessato a quanto segue:
...
- [Pass del transpiler AI](/guides/ai-transpiler-passes)
- [Creare un plugin transpiler](/guides/create-transpiler-plugin)
</Admonition>
