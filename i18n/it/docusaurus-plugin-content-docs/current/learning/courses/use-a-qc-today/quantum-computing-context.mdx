...
Un altro equivoco comune è che più qubit ha un computer quantistico, più deve essere potente. Sebbene la scala computazionale sia importante, il numero di qubit da solo non fornisce un quadro completo delle prestazioni del quantum computing. L'hardware quantistico moderno viene tipicamente valutato lungo tre dimensioni: scala, qualità e velocità.

La scala è misurata dal numero di qubit programmabili disponibili agli utenti. Questi sono i qubit che possono essere controllati, manipolati, misurati e incorporati direttamente negli algoritmi quantistici. Aumentare il numero di qubit programmabili espande la dimensione dei problemi che possono essere esplorati sull'hardware quantistico.

La qualità è misurata dalle operazioni per qubit (QuOps), che descrivono quante delle operazioni quantistiche più computazionalmente impegnative un sistema può eseguire in modo affidabile prima che gli errori accumulati sopraffacciano il calcolo. Miglioramenti nei tempi di coerenza, nelle fedeltà dei Gate, nelle procedure di calibrazione e nell'architettura contribuiscono tutti a una qualità computazionale più elevata. I tempi di coerenza e defasamento (comunemente indicati come $T_1$ e $T_2$) rimangono indicatori importanti delle prestazioni dei qubit.

La velocità è misurata dal numero massimo di circuiti al secondo, noto anche come throughput dei circuiti. Questa metrica cattura quanto calcolo utile un sistema quantistico può eseguire nel tempo e riflette sia l'elaborazione quantistica che quella classica di supporto.

Potresti anche incontrare metriche di benchmarking aggiuntive come CLOPS (Circuit Layer Operations Per Second) ed EPLG (Errors Per Layered Gate). CLOPS misura la velocità di esecuzione di un carico di lavoro di benchmark standardizzato, mentre EPLG fornisce informazioni sui tassi di errore a livello di Gate e sulla fedeltà del processore. Queste metriche rimangono utili per comprendere caratteristiche hardware e carichi di lavoro specifici, ma i qubit programmabili, le operazioni per qubit e il numero massimo di circuiti al secondo forniscono un quadro più ampio per confrontare i sistemi.
...
È necessario un progresso continuo lungo tutte e tre le dimensioni delle prestazioni, scala, qualità e velocità, per costruire un computer quantistico universale e tollerante ai guasti. Ecco perché, guardando la roadmap di IBM Quantum&reg;, noterai che alcuni salti tra processori non hanno grandi aumenti nel conteggio dei qubit programmabili. Per esempio, nota il modesto aumento di qubit programmabili tra Heron e Nighthawk, perché quello non è il vero focus di quel miglioramento. Nighthawk implementa invece una nuova topologia di connettività che consentirà diversi codici di correzione degli errori.
...
In teoria, certi approcci quantistici possono offrire speedup esponenziali o polinomiali in presenza di ipotesi specifiche (come una preparazione e una lettura efficienti dello stato). In pratica, si prevede che la risoluzione delle PDE sarà un'applicazione a più lungo termine, strettamente legata ai progressi nel quantum computing tollerante ai guasti e nell'integrazione quantistico-classica con sistemi di calcolo ad alte prestazioni (HPC).
