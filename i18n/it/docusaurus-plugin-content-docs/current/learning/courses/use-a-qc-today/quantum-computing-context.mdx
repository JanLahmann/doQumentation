...
Tutti e tre questi elementi insieme sono necessari per continuare a costruire il percorso verso un computer quantistico tollerante ai guasti e universale. Ecco perché, guardando la roadmap di IBM Quantum&reg;, noterai che alcuni salti tra processori non hanno grandi aumenti nel conteggio dei qubit. Per esempio, nota il modesto aumento di qubit tra Heron e Nighthawk, perché quello non è il vero focus di quel miglioramento. Nighthawk implementa invece una nuova topologia di connettività che consentirà diversi codici di correzione degli errori.
