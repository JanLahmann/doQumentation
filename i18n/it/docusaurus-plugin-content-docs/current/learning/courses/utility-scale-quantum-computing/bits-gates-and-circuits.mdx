...
### 3.2 Stato quantistico a qubit singolo ed evoluzione unitaria {#32-single-qubit-quantum-state-and-unitary-evolution}
...
### 3.3 Misurazione {#33-measurement}
...
Simuliamo prima l'azione di questo Gate a due qubit quando q0 e q1 sono entrambi $|0\rangle$, e otteniamo il vettore di stato in uscita. La sintassi Qiskit usata è ```qc.cx(control qubit, target qubit)```.
...
Se non hai `qiskit-ibm-runtime`, dovrai installarlo prima. Per maggiori informazioni, [consulta la documentazione API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-service).
...
Puoi vedere che nella transpilazione il circuito è stato riscritto usando nuovi gate. Per maggiori informazioni, consulta la documentazione su [ECRGate](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ECRGate#ecrgate).
