...
3. Creare un Circuit quantistico che preparerà lo stato fondamentale di questo Hamiltoniano tramite un processo simile al quantum annealing.

**Nota:** Nella metodologia QAOA, si vuole avere un operatore (**Hamiltoniano**) che rappresenti la **funzione di costo** dell'algoritmo ibrido, nonché un Circuit parametrizzato (**Ansatz**) che rappresenti gli stati quantistici con le soluzioni candidate al problema. È possibile campionare questi stati candidati e poi valutarli usando la funzione di costo.
...
#### Hamiltoniano &rarr; Circuit quantistico {#hamiltonian-quantum-circuit}
L'Hamiltoniano $H_C$ contiene la definizione quantistica del problema. Ora è possibile creare un Circuit quantistico che aiuterà a *campionare* buone soluzioni dal computer quantistico. Il QAOA è ispirato al quantum annealing e applica livelli alternati di operatori nel Circuit quantistico.
...
Il Circuit quantistico generato è **parametrizzato** da $\gamma_i$ e $\beta_i$, così si possono provare diversi valori di $\gamma_i$ e $\beta_i$ e campionare dallo stato risultante.
...
Il Circuit sopra contiene una serie di astrazioni utili per ragionare sugli algoritmi quantistici, ma non eseguibili sull'hardware. Per poter essere eseguito su una QPU, il Circuit deve subire una serie di operazioni che costituiscono il passo di **transpilazione** o **ottimizzazione del circuit** del pattern.

La libreria Qiskit offre una serie di **pass di transpilazione** che coprono un'ampia gamma di trasformazioni dei circuit. È necessario assicurarsi che il Circuit sia **ottimizzato** per il proprio scopo.

La transpilazione può comportare diversi passi, ad esempio:

* **Mappatura iniziale** dei qubit nel Circuit (come le variabili decisionali) ai qubit fisici sul dispositivo.
* **Srotolamento** (unrolling) delle istruzioni nel Circuit quantistico alle istruzioni native dell'hardware che il Backend comprende.
* **Routing** dei qubit nel Circuit che interagiscono verso qubit fisici adiacenti tra loro.
...
Il codice seguente trasforma e ottimizza il Circuit astratto in un formato pronto per l'esecuzione su uno dei dispositivi accessibili tramite cloud usando il **servizio Qiskit IBM&reg; Runtime**.
...
Nel flusso di lavoro QAOA, i parametri QAOA ottimali vengono trovati in un ciclo di ottimizzazione iterativo, che esegue una serie di valutazioni del Circuit e usa un ottimizzatore classico per trovare i parametri ottimali $\beta_k$ e $\gamma_k$. Questo ciclo di esecuzione viene svolto tramite i seguenti passi:

1. Definire i parametri iniziali
2. Istanziare una nuova `Session` contenente il ciclo di ottimizzazione e la primitiva usata per campionare il Circuit
3. Una volta trovato un insieme ottimale di parametri, eseguire il Circuit un'ultima volta per ottenere una distribuzione finale che verrà utilizzata nel passo di post-elaborazione.
#### Definire il Circuit con i parametri iniziali {#define-circuit-with-initial-parameters}
...
L'ottimizzatore è riuscito a ridurre il costo e trovare parametri migliori per il Circuit.
...
Una volta trovati i parametri ottimali per il Circuit, è possibile assegnare questi parametri e campionare la distribuzione finale ottenuta con i parametri ottimizzati. Qui è dove va utilizzata la primitiva *Sampler*, poiché è la distribuzione di probabilità delle misurazioni delle bitstringhe che corrisponde al taglio ottimale del grafo.
