...
## 3. Ottimizzazione quantistica con i pattern Qiskit {#3-quantum-optimization-with-qiskit-patterns}
...
### 3.1 Pattern Qiskit (su piccola scala) per l'ottimizzazione {#31-small-scale-qiskit-pattern-for-optimization}
...
### 3.2 Passo 1. Mappare gli input classici in un problema quantistico {#32-step-1-map-classical-inputs-to-a-quantum-problem}
...
### 3.3 Passo 2. Ottimizzare i circuit per l'esecuzione su hardware quantistico {#33-step-2-optimize-circuits-for-quantum-hardware-execution}
...
### 3.4 Passo 3. Eseguire usando le primitive IBM Quantum {#34-step-3-execute-using-ibm-quantum-primitives}
...
Usa le **primitive IBM Quantum** per interagire con i Backend IBM&reg;. Le due primitive sono Sampler ed Estimator, e la scelta della primitiva dipende dal tipo di misura che si vuole eseguire sul computer quantistico. Per la minimizzazione di $H_C$, si utilizza l'Estimator poiché la misura della funzione di costo è semplicemente il valore atteso di $\langle H_C \rangle$.
...
### 3.5 Passo 4. Post-elaborazione, restituzione del risultato in formato classico {#35-step-4-post-process-return-result-in-classical-format}
