...
#### Prepara il Circuit quantistico 2 (Circuit singolo per l'evoluzione temporale) per il problema a 70 qubit {#prepare-the-quantum-circuit-2-single-circuit-for-time-evolution-for-the-70-qubit-problem}
Qui costruiamo un Circuit per un singolo passo temporale usando Lie–Trotter.
...
Prova da solo, partendo dall'esempio del caso a 20 qubit. Come in precedenza, conta le operazioni per questo Circuit.
...
Come in tutte le lezioni precedenti, implementeremo il framework Qiskit patterns. La parte della lezione fino a questo punto si è concentrata sulla creazione dei Circuit quantistici corretti per descrivere il nostro problema. Questo corrisponde effettivamente al Passo 1.
...
Traspiliamo i Circuit e li raccogliamo in una lista. Questa operazione potrebbe richiedere alcuni minuti.
...
Utilizzeremo questi dati per il confronto più avanti. Prima però, vediamo se riusciamo a ottimizzare ulteriormente i nostri Circuit.
