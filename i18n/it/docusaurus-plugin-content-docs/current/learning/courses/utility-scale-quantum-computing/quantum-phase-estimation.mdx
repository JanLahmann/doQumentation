...
## 2. Basi della Trasformata di Fourier Quantistica (QFT) {#2-basic-of-quantum-fourier-transformation-qft}
...
### 2.1. Intuizione <a id="intuition"></a> {#21-intuition}

La trasformata quantistica di Fourier (QFT) trasforma tra due basi, la base computazionale (Z) e la base di Fourier. Ma cosa significa la base di Fourier in questo contesto? Probabilmente ricorderai che la trasformata di Fourier $F(\omega)$ di una funzione $f(x)$ descrive la convoluzione di $f(x)$ su una funzione sinusoidale con frequenza $\omega$. In parole semplici: la trasformata di Fourier è una funzione che descrive quanto di ogni frequenza $\omega$ avremmo bisogno per costruire una funzione $f(x)$ a partire da funzioni seno (o coseno). Per farti un'idea migliore di cosa significhi la QFT in questo contesto, considera le immagini a gradini sottostanti che mostrano un numero codificato in binario, usando quattro qubit:
...
Nella base computazionale, memorizziamo i numeri in binario usando gli stati $|0\rangle$ e $|1\rangle$.

Nota la frequenza con cui i diversi qubit cambiano; il qubit più a sinistra si inverte a ogni incremento del numero, il successivo ogni 2 incrementi, il terzo ogni 4 incrementi, e così via.

Se applichiamo una trasformata quantistica di Fourier a questi stati, mappiamo
$$
...
(Spesso indichiamo gli stati nella base di Fourier usando la tilde (~)).
...
## 3. Basi della Stima della Fase Quantistica (QPE) {#3-basics-of-quantum-phase-estimation-qpe}
...
$$
Inizializziamo $\vert\psi\rangle = \vert1\rangle$ dell'autovettore del Gate $T$ applicando un Gate $X$:
...
Applichiamo la trasformata quantistica di Fourier inversa per convertire lo stato del registro di conteggio, poi misuriamo il registro di conteggio:
...
Vediamo che otteniamo un risultato (`001`) con certezza, che si traduce nel decimale: `1`. Dobbiamo ora dividere il nostro risultato (`1`) per $2^n$ per ottenere $\theta$:

$$
\theta = \frac{1}{2^3} = \frac{1}{8}
$$
 L'algoritmo di Shor ci permette di fattorizzare un numero costruendo un circuito con $\theta$ sconosciuto e ottenendo $\theta$.
...
#### Soluzione dell'esercizio: $\theta = 1/3$ {#solution-of-the-exercise-theta-13}
...
## 4. Esecuzione con la primitiva Sampler di IBM Quantum {#4-execution-using-the-ibm-quantum-sampler-primitive}
