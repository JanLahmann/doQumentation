...
La QFT può essere implementata con $O(N^2)$ operazioni tramite porte quantistiche come le porte di Hadamard e le porte di Phase controllata per $N$ qubit, consentendo un'accelerazione esponenziale rispetto alla trasformata di Fourier classica.
...
Questa operazione è il risultato dell'applicazione della porta di Hadamard ($H$).
...
Applica un'operazione di porta di Hadamard a $n$ bit $H^{\otimes n}$ sul registro di conteggio:
...
### 3.2 Esempio: QPE con la porta T {#32-example-t-gate-qpe}

Usiamo la porta $T$ come esempio di QPE e stimiamo la sua fase $\theta$.
...
Inizializziamo $\vert\psi\rangle = \vert1\rangle$ come autovettore della porta $T$ applicando una porta $X$:
...
Successivamente, applichiamo le porte di Hadamard ai qubit di conteggio:
...
## 4. Esecuzione con il primitivo Sampler di Qiskit Runtime {#4-execution-using-the-qiskit-runtime-sampler-primitive}
