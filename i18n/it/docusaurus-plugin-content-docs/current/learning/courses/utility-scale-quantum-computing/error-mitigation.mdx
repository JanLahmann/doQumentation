...
---
{/* doqumentation-source-hash: 4a121616 */}
...
Qiskit Aer è un simulatore classico per il calcolo quantistico. Può simulare non solo l'esecuzione ideale, ma anche l'esecuzione rumorosa dei Circuit quantistici. Questo notebook mostra come eseguire una simulazione rumorosa usando Qiskit Aer:

1. Costruire un modello di rumore
2. Costruire un sampler rumoroso (simulatore) con il modello di rumore
3. Eseguire un Circuit quantistico sul sampler rumoroso
...
Consideriamo dei Circuit a 1 Qubit che ripetono semplicemente le gate X `d` volte (`d`=0 ... 100) e misurano l'osservabile `Z`.
...
### 2.4 Eseguire i Circuit quantistici sul sampler rumoroso {#24-run-quantum-circuits-on-the-noisy-sampler}
...
Consideriamo Circuit Trotterizzati a 6 Qubit per simulare l'evoluzione temporale di un modello di Ising unidimensionale e osserviamo come l'errore scala in funzione del numero di passi temporali.
...
Per conoscere in anticipo l'output ideale, utilizziamo Circuit di tipo compute-uncompute, composti da una prima fase in cui viene applicato il Circuit originale $U$ e una seconda fase in cui viene invertito $U^\dagger$.
Nota che l'esito ideale di tali Circuit sarà banalmente lo stato di input $|000000\rangle$, che ha valori di aspettazione banali per qualsiasi osservabile di Pauli, ad esempio $\langle IIIIIZ \rangle = 1$.
...
Nota: come mostrato sopra, il Circuit con $k$ passi temporali avrà $4k$ layer di gate a due Qubit.
...
Transpiliamo i Circuit per il Backend con ottimizzazione (`optimization_level=1`).
