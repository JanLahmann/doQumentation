...
Un __metodo del sottospazio di Krylov__ può riferirsi a uno qualsiasi tra diversi metodi costruiti attorno a ciò che viene chiamato __sottospazio di Krylov__. Una revisione completa di questi va oltre lo scopo di questa lezione, ma i riferimenti [\[2-4\]](#references) possono fornire un contesto molto più approfondito. Qui ci concentreremo su cosa sia un sottospazio di Krylov, come e perché sia utile nel risolvere problemi agli autovalori, e infine come possa essere implementato su un computer quantistico.
__Definizione:__ data una matrice $N\times N$ simmetrica, semidefinita positiva $A$, lo spazio di Krylov $\mathcal{K}^r$ di ordine $r$ è lo spazio generato dai vettori ottenuti moltiplicando potenze crescenti di una matrice $A$, fino a $r-1\leq N$, per un vettore di riferimento $\vert v \rangle$.
...
### Verifica della comprensione {#check-your-understanding-1}
...
### 4. 2 Passo 2. Ottimizzare i circuiti e gli operatori per l'hardware target {#4-2-step-2-optimize-circuits-and-operators-for-target-hardware}
...
#### Circuiti template per il calcolo degli elementi di matrice di $\tilde{S}$ e $\tilde{H}$ tramite il test di Hadamard {#template-circuits-for-calculating-matrix-elements-of-tildes-and-tildeh-via-hadamard-test}
...
### 4.3 Passo 3. Eseguire usando una primitiva di IBM Quantum {#43-step-3-execute-using-an-ibm-quantum-primitive}
