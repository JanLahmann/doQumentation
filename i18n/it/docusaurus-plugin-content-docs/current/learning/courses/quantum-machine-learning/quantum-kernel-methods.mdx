...
$$
Questo è esattamente il valore che vogliamo (a meno di $||^2$). Il livello di misurazione del nostro circuito restituirà probabilità di misurazione (o cosiddette "quasi-probabilità", se vengono usati certi metodi di mitigazione degli errori). La probabilità di interesse è quella dello stato zero, $|0\rangle^{\otimes N}$.
...
Usa la primitiva `Sampler` di IBM Quantum per ricostruire una distribuzione di probabilità degli stati ottenuti dal campionamento del circuito. Nota che potresti vedere questo riferito come "distribuzione di quasi-probabilità", un termine applicabile quando il rumore è un problema e vengono introdotti passaggi aggiuntivi, come nella mitigazione degli errori. In tali casi, la somma di tutte le probabilità potrebbe non essere esattamente uguale a 1; da qui "quasi-probabilità".
...
Calcola la probabilità di misurare $|0\rangle^{\otimes N}$ sul Circuit quantistico, e popola la matrice del kernel nella posizione corrispondente ai due vettori di dati usati. Per riempire l'intera matrice del kernel, dobbiamo eseguire un esperimento quantistico per ogni voce. Una volta ottenuta una matrice del kernel, possiamo usarla in molti algoritmi classici di machine learning che accettano `pre-calculated kernels`. Per esempio: `qml_svc = SVC(kernel="precomputed")`. Possiamo quindi usare flussi di lavoro classici per applicare il nostro modello ai nostri dati di test, e ottenere un punteggio di accuratezza. A seconda della nostra soddisfazione riguardo al punteggio di accuratezza, potremmo dover rivedere aspetti del nostro calcolo, come la nostra feature map.
...
### Passo 1: Mappare gli input classici su un problema quantistico {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
Le due unitarie sopra corrispondono esattamente a $U_1$ e $U_2$ descritte nell'introduzione. Possiamo combinarle usando `unitary_overlap`. Come sempre, vogliamo tenere d'occhio la profondità del nostro circuito.
...
### Passo 4: Post-elaborazione, restituire il risultato in formato classico {#step-4-post-process-return-result-in-classical-format-1}
...
Vale la pena notare che abbiamo usato la `z_feature_map` che ha prodotto una bassa profondità traspilata a due qubit (profondità 1, infatti). Se i tuoi circuiti diventano troppo profondi, ciò causerà sicuramente molto rumore, e questo ridurrà la probabilità di misurare lo stato zero anche se la tua feature map è ben adatta ai tuoi dati. Ad esempio, una ripetizione del processo precedente usando ```zz_feature_map``` e ```, entanglement='linear', reps=1``` ha prodotto ```dist.get(0,0.0) = 0.0015``` usando gli stessi punti dati. Ciò è dovuto alle profondità di circuito e a due qubit molto maggiori della ```zz_feature_map```. La figura seguente mostra la distribuzione di probabilità per quel calcolo.
...
Supponiamo che qualcuno alle prime armi con il quantum computing tenti di usare un Circuit quantistico che ha una profondità a due qubit molto alta, e non usi la mitigazione degli errori. Supponiamo inoltre che questo comporti un tasso di errore del 10% su ogni qubit. Se l'elemento vero (privo di errori) della matrice del kernel corrispondente a questo circuito è molto grande, diciamo 1.0, quale sarebbe la probabilità di misurare tutti e 10 i qubit nello stato con ogni qubit $|0\rangle$?
...
### Passo 1: Mappare gli input classici in un problema quantistico {#step-1-map-classical-inputs-to-a-quantum-problem-3}
...
### Passo 2: Ottimizzare il problema per l'esecuzione quantistica {#step-2-optimize-problem-for-quantum-execution-2}
...
### Passo 4: Post-elaborazione, restituzione del risultato in formato classico {#step-4-post-process-return-result-in-classical-format-3}
