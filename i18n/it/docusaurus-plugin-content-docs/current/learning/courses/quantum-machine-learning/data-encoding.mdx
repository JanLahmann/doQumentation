...
usando la codifica in ampiezza.
<Accordion>
...
Un modo per comprendere l'unitaria sopra riportata è tramite un'analogia con il propagatore in un sistema fisico. L'unitaria sopra è un operatore di evoluzione unitaria, $\exp(it\mathcal{H})$, per un'Hamiltoniana, $\mathcal{H}$, simile al modello di Ising, dove il parametro temporale, $t$, è sostituito con valori di dati per guidare l'evoluzione. L'espansione di questo operatore unitario dà origine al circuito PFM. Le connettività di entanglement in $S$ possono essere interpretate come accoppiamenti di Ising in un reticolo di spin.
Consideriamo un esempio di operatori di Pauli $Y$ e $XX$ che rappresentano quelle interazioni di tipo Ising. Qiskit fornisce una classe `pauli_feature_map` per istanziare una PFM con una scelta di gate a uno e $n$ qubit, che in questo esempio saranno passati come stringhe di Pauli `‘Y’` e `‘XX’`. Tipicamente, $n$ è 1 o 2 rispettivamente per interazioni a uno e due qubit. Lo schema di entanglement è "lineare", il che significa che sono accoppiati solo i qubit primi vicini nel circuito quantistico. Nota che questo non corrisponde ai qubit primi vicini sul computer quantistico stesso, poiché questo circuito quantistico è un livello di astrazione.
