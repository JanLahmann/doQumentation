...
L'insieme $\{\vert+\rangle,\vert-\rangle\}$ è una base ortonormale per lo spazio $2$-dimensionale corrispondente a un singolo qubit, e la base di Bell $\{\vert\phi^+\rangle, \vert\phi^-\rangle, \vert\psi^+\rangle, \vert\psi^-\rangle\}$ è una base ortonormale per lo spazio $4$-dimensionale corrispondente a due qubit.
...
Quindi, $\mathsf{Y}$ memorizza una copia del risultato della misurazione e $\mathsf{X}$ cambia esattamente come farebbe se la misurazione proiettiva descritta da $\{\Pi_0,\ldots,\Pi_{m-1}\}$ fosse stata eseguita direttamente su $\mathsf{X}$.
