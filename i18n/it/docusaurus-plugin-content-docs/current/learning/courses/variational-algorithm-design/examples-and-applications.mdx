...
Nell'esempio precedente non abbiamo usato alcun operatore di riferimento $U_R$. Proviamo ora a capire come si può ottenere lo stato ideale $\frac{1}{\sqrt{2}}(|00\rangle + |11\rangle)$. Considera il seguente Circuit.
...
Possiamo verificare rapidamente che questo Circuit ci restituisce lo stato desiderato.
...
Ora che abbiamo visto come si presenta un Circuit che prepara lo stato soluzione, sembra ragionevole usare un Gate di Hadamard come circuit di riferimento, in modo che l'ansatz completo diventi:
...
Per questo nuovo Circuit, la soluzione ideale potrebbe essere raggiunta con tutti i parametri impostati a $0$, il che conferma che la scelta del circuit di riferimento è ragionevole.
