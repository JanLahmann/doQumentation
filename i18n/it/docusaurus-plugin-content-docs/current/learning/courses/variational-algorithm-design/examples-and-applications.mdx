...
Useremo la seguente funzione per calcolare la penalità di sovrapposizione. Nota che questo fa ancora parte della mappatura del problema sui circuiti quantistici. Tuttavia, come discusso nella lezione precedente, questa funzione calcola la sovrapposizione tra un circuito variazionale attuale e il circuito ottimizzato dallo stato precedente a energia/costo inferiore ottenuto. Il nuovo circuito generato deve anch'esso essere traspilato per essere eseguito su hardware reale. Abbiamo già visto questa funzione, usata su un simulatore. Qui, dobbiamo già considerare la traspilazione e l'ottimizzazione correlata per quando usiamo un Backend reale, da cui le righe intorno a `if realbackend == 1`. Questo mescola un po' il passo 2, ma richiameremo esplicitamente il passo 2 più avanti.
...
Ora aggiungiamo la funzione di costo di VQD. Nota che rispetto alla lezione precedente, ora abbiamo due argomenti aggiuntivi (`realbackend` e `backend`) per aiutarci con la traspilazione quando usiamo Backend reali.
...
Ancora una volta, useremo simulatori per il debug, e poi passeremo all'hardware reale.
...
Questi risultati sono abbastanza vicini a quelli attesi, a parte l'errore di approssimazione e la fase globale. Potremmo regolare la tolleranza sull'ottimizzatore classico e/o le penalità per la sovrapposizione del vettore di stato per ottenere valori più precisi.
...
$$
L'operatore è una combinazione lineare di termini con operatori Z sui nodi collegati da un arco (ricorda che lo 0-esimo qubit è il più a destra): $IIZZ + IZIZ + IZZI + ZIIZ + ZZII$. Una volta costruito l'operatore, l'ansatz per l'algoritmo QAOA può essere facilmente costruito usando il circuito `QAOAAnsatz` dalla libreria di circuiti Qiskit.
...
Ora impostiamo un insieme iniziale di parametri casuali:
...
Qualsiasi ottimizzatore classico può essere usato per minimizzare la funzione di costo. Su un sistema quantistico reale, un ottimizzatore progettato per paesaggi di funzioni di costo non uniformi solitamente funziona meglio. Qui usiamo la [routine COBYLA](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-cobyla.html#optimize-minimize-cobyla) di SciPy tramite la funzione minimize.

Poiché eseguiamo iterativamente molte chiamate a Runtime, utilizziamo una Session per eseguire tutte le chiamate all'interno di un unico blocco. Inoltre, per QAOA, la soluzione è codificata nella distribuzione di output del circuito ansatz vincolato con i parametri ottimali ottenuti dalla minimizzazione. Pertanto, avremo bisogno di una primitiva Sampler, e la istanzieremo con la stessa session
Ed eseguiamo la nostra routine di minimizzazione:
...
Il vettore soluzione degli angoli dei parametri (`x`), quando inserito nel circuito ansatz, produce il partizionamento del grafo che stavamo cercando.
...
Procedi all'ultima lezione per sostenere la tua valutazione e ottenere il tuo badge!
