...
Per scrivere il problema QUBO come Hamiltoniano di Ising, tutto ciò che dobbiamo fare è effettuare un semplice cambio di variabili da $x \in \{0, 1\}^n$ a $z \in \{-1, 1\}^n$:
...
Innanzitutto, nota che possiamo riscrivere la nostra espressione QUBO come la somma di termini matriciali:
...
che può essere riorganizzato come segue.

$$
\sum_{ij}\frac{Q_{ij}}{4}z_iz_j - \frac{Q_{ij}}{4}z_i - \frac{Q_{ij}}{4}z_j + \frac{Q_{ij}}{4}
$$

Possiamo eliminare il termine costante perché non influenzerà quale $z$ minimizza il problema QUBO. Inoltre, possiamo moltiplicare questa espressione per quattro per recuperare il valore originale di $Q$, poiché anche questo non influenza la scelta ottimale di $z$.

$$
\sum_{ij}Q_{ij}z_iz_j - Q_{ij}z_i - Q_{ij}z_j = z^TQz + (-\sum_{ij} Q_{ij}z_i + Q_{ij}z_j)
$$

Raggruppando i termini lineari simili, possiamo definire il nostro vettore di coefficienti lineari $b$ usando l'equazione

$$
b_i = -\sum_{j} Q_{ij} + Q_{ji}
$$

che completa la trasformazione delle variabili in $z \in \{-1, 1\}^n$ dopo la sostituzione nell'equazione precedente.
...
Alla fine, la minimizzazione dell'espressione QUBO equivale alla minimizzazione di questa espressione, dove $b$ è un coefficiente scalare reale:

$$
\min_{x\in\{0,1\}^n} x^TQx\Longleftrightarrow \min_{z\in\{-1,1\}^n}z^TQz + b^Tz
$$

Riscrivendo ancora leggermente otteniamo il nostro Hamiltoniano della funzione di costo, dove il minimo dell'espressione rappresenta lo stato fondamentale e `Z` è l'operatore di Pauli Z:
...
#### Costruire il circuito quantistico {#construct-the-quantum-circuit}
...
![Diagramma del circuito con livelli QAOA](/learning/images/courses/quantum-computing-in-practice/utility-scale-qaoa/circuit-diagram.svg)
