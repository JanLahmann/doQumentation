...
Ogni processore può essere caratterizzato usando tre metriche di prestazione fondamentali che ne descrivono la scala, la qualità e la velocità: qubit programmabili, operazioni sui qubit, e numero massimo di circuiti al secondo.

* **Qubit programmabili:** questi sono i qubit che gli utenti possono controllare direttamente e incorporare negli algoritmi quantistici. Questa metrica cattura la scala computazionale disponibile per le applicazioni.


* **Operazioni sui qubit (QuOps):** questa metrica descrive quante delle operazioni quantistiche più computazionalmente impegnative un processore può eseguire in modo affidabile prima che gli errori accumulati limitino il calcolo. Serve come misura della qualità e della capacità computazionale.


* **Numero massimo di circuiti al secondo:** questa metrica misura il throughput dei circuiti, ovvero quanti circuiti quantistici un sistema può eseguire al secondo. Un throughput più elevato consente di completare più lavoro utile in una determinata quantità di tempo.


Potresti anche incontrare metriche aggiuntive del processore nella documentazione tecnica, negli studi di benchmarking e nei report sulle prestazioni:

EPLG (Errors Per Layered Gate). Una misura dell'errore medio introdotto dalle operazioni di gate di entanglement a strati. Un EPLG più basso generalmente indica una qualità del gate più alta e un accumulo di errori inferiore.

CLOPS (Circuit Layer Operations Per Second). Un benchmark che misura quanto rapidamente un sistema quantistico può eseguire un carico di lavoro standardizzato che include sia operazioni quantistiche sia l'elaborazione classica associata.

Queste metriche aggiuntive forniscono informazioni più dettagliate sul comportamento dell'hardware, mentre i qubit programmabili, le operazioni sui qubit e il numero massimo di circuiti al secondo forniscono una visione di più alto livello della scala, della qualità e della velocità di un sistema.
...
Successivamente, dovremo scegliere un processore quantistico che abbia abbastanza qubit di qualità sufficientemente alta da poter eseguire il nostro circuito quantistico. Prendi queste decisioni guidato dalle tre metriche primarie di cui abbiamo discusso nella sezione hardware: qubit programmabili, operazioni sui qubit, e numero massimo di circuiti al secondo. A seconda dell'applicazione, metriche aggiuntive come EPLG e CLOPS possono anche fornire informazioni utili sulla qualità del circuito attesa e sulle prestazioni di esecuzione.
...
Ora siamo pronti per eseguire il programma quantistico. Le primitive di IBM Quantum forniscono un'interfaccia con l'hardware IBM Quantum, e astraggono anche la soppressione e la mitigazione degli errori dall'utente. Ci sono due primitive tra cui scegliere: il Sampler e l'Estimator.
...
[IBM Quantum Estimator](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator) usa un processo algebrico complesso per stimare il valore di aspettazione su un dispositivo quantistico reale, scomponendo l'osservabile in una combinazione di altri osservabili con autobasi note.
