...
Caricheremo ora i pacchetti rimanenti necessari per eseguire il VQE vero e proprio, come l'ansatz `efficient_su2` e i minimizzatori di SciPy:
...
## Passo 3: Esegui con le primitive di Qiskit {#step-3-execute-using-qiskit-primitives}
...
I risultati di questo output sono discussi di seguito nella sezione di post-elaborazione; per ora, nota semplicemente che la simulazione è andata a buon fine. Ora sei pronto per eseguire su hardware reale. Imposteremo la resilienza a `1`, il che indica che verrà utilizzata la mitigazione degli errori TREX. Ora che lavoriamo con hardware reale, utilizzeremo Qiskit Runtime e le primitive di Runtime. Nota che sia il ciclo `for` relativo alla geometria sia i multipli tentativi variazionali sono all'interno della sessione.
