...
sidebar_label: 'Geometria molecolare'
...
# Determinare la geometria molecolare {#determining-a-molecular-geometry}

Nella sezione precedente abbiamo implementato il VQE per determinare l'energia dello stato fondamentale di una molecola. Si tratta di un utilizzo valido del calcolo quantistico, ma ancora più utile sarebbe determinare la struttura di una molecola.

## Passo 1: Mappa gli input classici su un problema quantistico {#step-1-map-classical-inputs-to-a-quantum-problem}

Rimanendo con il nostro esempio base dell'idrogeno biatomico, l'unico parametro geometrico da variare è la lunghezza del legame. Per farlo, procediamo come prima, ma usando una variabile nella costruzione iniziale della molecola (una lunghezza di legame, *x*, come argomento). Si tratta di una modifica abbastanza semplice, ma richiede che la variabile sia inclusa nelle funzioni lungo tutto il processo, poiché parte dalla costruzione dell'Hamiltoniano fermionico e si propaga attraverso il mapping e infine fino alla funzione di costo.

Per prima cosa, carichiamo alcuni dei pacchetti usati in precedenza e definiamo la funzione di Cholesky.
...
Ora, per definire il nostro Hamiltoniano, utilizzeremo PySCF esattamente come nell'esempio precedente, ma questa volta includeremo una variabile, `x`, che rappresenta la distanza interatomica. Questa funzione restituirà come prima l'energia del nucleo, l'energia a un elettrone e le energie a due elettroni.
...
Ricorda che la costruzione precedente crea un Hamiltoniano fermionico basato sulla specie atomica, la geometria e gli orbitali elettronici. Di seguito mappiamo questo Hamiltoniano fermionico sugli operatori di Pauli. Anche questa funzione `build_hamiltonian` includerà una variabile geometrica come argomento.
...
Caricheremo i pacchetti rimanenti per eseguire VQE stesso, come l'ansatz efficient_su2 e i minimizzatori di SciPy:
...
Definiremo nuovamente la funzione di costo, ma poiché questa ha sempre ricevuto un Hamiltoniano già costruito e mappato come argomento, questa funzione non cambia.
...
Poiché l'Hamiltoniano cambierà ad ogni nuova geometria, anche la transpilazione dell'operatore varierà ad ogni passo. Possiamo comunque definire un pass manager generale da applicare ad ogni passo, specifico per l'hardware che vogliamo utilizzare.

Qui utilizzeremo il backend meno occupato disponibile. Useremo quel backend come modello per il nostro AerSimulator, consentendo al simulatore di imitare, ad esempio, il comportamento del rumore del backend reale. Questi modelli di rumore non sono perfetti, ma possono aiutarti a sapere cosa aspettarti dall'hardware reale.
...
Importiamo il pass manager e i pacchetti correlati per ottimizzare il circuito. Questo passo, e quello precedente, sono indipendenti dall'Hamiltoniano e quindi rimangono invariati rispetto alla lezione precedente.
...
Nel blocco di codice seguente, configuriamo un array per memorizzare i risultati di ogni passo della distanza interatomica $x$. Abbiamo scelto l'intervallo di $x$ sulla base della nostra conoscenza del valore sperimentale della lunghezza del legame di equilibrio: 0,74 Ångström. Eseguiremo prima su un simulatore, quindi importeremo il nostro stimatore (BackendEstimator) da `qiskit.primitives`. Per ogni passo geometrico, costruiamo l'Hamiltoniano e consentiamo un certo numero di passi di ottimizzazione (qui 500) usando l'ottimizzatore "cobyla". Ad ogni passo geometrico, memorizziamo sia l'energia totale che l'energia elettronica. A causa dell'elevato numero di passi dell'ottimizzatore, questa operazione potrebbe richiedere un'ora o più. Potresti voler modificare gli input qui sotto per ridurre il tempo richiesto.
...
I risultati di questo output vengono discussi più avanti nella sezione di post-elaborazione; per ora, nota semplicemente che la simulazione ha avuto successo. Ora sei pronto per eseguire su hardware reale. Imposteremo la resilienza a `1`, indicando che verrà utilizzata la mitigazione degli errori TREX. Ora che stiamo lavorando con hardware reale, useremo IBM Quantum Compute Service e le primitive IBM Quantum. Nota che sia il ciclo for relativo alla geometria sia i molteplici tentativi variazionali si trovano all'interno della sessione.

Poiché i costi e i limiti di tempo associati alle esecuzioni su hardware reale sono significativi, abbiamo ridotto il numero di passi geometrici e di passi dell'ottimizzatore. Assicurati di adattare questi parametri in base ai tuoi obiettivi di precisione e ai limiti di tempo.
...
Sia per il simulatore che per l'hardware reale, possiamo tracciare le energie dello stato fondamentale calcolate per ogni distanza interatomica e vedere dove si raggiunge l'energia più bassa. Questa dovrebbe corrispondere alla distanza interatomica che si trova in natura, e in effetti è vicina. Una curva più regolare potrebbe essere ottenuta provando altri ansatz, altri ottimizzatori, ed eseguendo il calcolo più volte ad ogni passo geometrico e calcolando la media su diverse condizioni iniziali casuali.
...
Nota che semplicemente aumentare il numero di passi di ottimizzazione probabilmente non migliorerà i risultati del simulatore, poiché tutte le ottimizzazioni sono in realtà già convergite alla tolleranza richiesta in meno del numero massimo di iterazioni.

I risultati dell'hardware reale sono comparabili, a parte un intervallo di valori campionati leggermente diverso.
...
Oltre ad attendersi una lunghezza del legame H₂ di 0,74 Ångström, l'energia totale dovrebbe essere di -1,17 Hartree. Vediamo che i risultati dell'hardware reale si sono avvicinati maggiormente a questi valori rispetto al simulatore. Ciò è probabilmente dovuto al fatto che il rumore era presente (o simulato) in entrambi i casi, ma solo nel caso dell'hardware reale è stata applicata la mitigazione degli errori.

### Conclusioni {#closing}

Questo conclude il nostro corso su VQE per la chimica quantistica. Se sei interessato a comprendere alcune delle basi di teoria dell'informazione utilizzate nel calcolo quantistico, dai un'occhiata al corso di John Watrous sulle [Basi dell'informazione quantistica](/learning/courses/basics-of-quantum-information). Per un ulteriore esempio breve di un flusso di lavoro VQE, consulta il nostro [tutorial sulla stima dell'energia dello stato fondamentale della catena di Heisenberg con VQE](/tutorials/spin-chain-vqe). Oppure sfoglia i [tutorial](https://docs.quantum.ibm.com/tutorials) e i [corsi](/learning) per trovare altri materiali educativi sulle ultime tecnologie nel calcolo quantistico.

Non dimenticare di sostenere l'esame di questo corso. Un punteggio dell'80% o superiore ti farà guadagnare un badge Credly, che ti verrà inviato automaticamente via email. Grazie per far parte della rete IBM Quantum&reg;!
