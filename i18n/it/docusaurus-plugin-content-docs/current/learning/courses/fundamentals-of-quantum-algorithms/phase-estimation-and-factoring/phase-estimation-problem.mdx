...
Nel *problema della stima di fase*, ci viene dato uno stato quantistico $\vert \psi\rangle$ di $n$ qubit, insieme a un Circuit quantistico unitario che agisce su $n$ qubit.
Ci viene *promesso* che $\vert \psi\rangle$ è un autovettore della matrice unitaria $U$ che descrive l'azione del Circuit, e il nostro obiettivo è calcolare o approssimare l'autovalore $\lambda$ a cui $\vert \psi\rangle$ corrisponde.
...
Input: un Circuit quantistico unitario per un'operazione $U$ su $n$ qubit insieme a uno stato quantistico $\vert\psi\rangle$ di $n$ qubit \
