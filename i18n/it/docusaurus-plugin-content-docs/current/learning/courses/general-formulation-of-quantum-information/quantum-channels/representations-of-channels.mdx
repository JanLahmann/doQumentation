...
Ecco una rappresentazione di Stinespring del canale di dephasing qubit $\Delta$.
...
![Un diagramma di circuito quantistico che rappresenta il canale di dephasing completo](/learning/images/courses/general-formulation-of-quantum-information/quantum-channels/dephasing-circuit.svg)

Per verificare che l'effetto di questo circuito sul qubit di input sia effettivamente descritto dal canale di dephasing completo, possiamo percorrere il circuito passo dopo passo, utilizzando la rappresentazione matriciale esplicita della traccia parziale discussa nella lezione precedente.
...
Stiamo usando matrici densità anziché vettori di stato quantistico, ma vengono messe in prodotto tensoriale in modo simile a quanto fatto nella formulazione semplificata dell'informazione quantistica.
...
### Canale di dephasing completo (alternativo) {#completely-dephasing-channel-alternative}

Il circuito descritto sopra non è l'unico modo per implementare il canale di dephasing completo.
Ecco un modo diverso di farlo.

![Un diagramma di circuito quantistico alternativo che rappresenta il canale di dephasing completo](/learning/images/courses/general-formulation-of-quantum-information/quantum-channels/dephasing-alternative.svg)
...
il dephasing è equivalente a non fare nulla (ovvero applicare un'operazione identità) oppure applicare un gate $\sigma_z$, ciascuno con probabilità $1/2$.
...
Ovvero, il canale di dephasing completo è un esempio di canale unitario misto, e più specificamente, un canale di Pauli.
...
### Canale di dephasing completo {#completely-dephasing-channel-2}
...
### Canale di reset del qubit {#qubit-reset-channel-2}
