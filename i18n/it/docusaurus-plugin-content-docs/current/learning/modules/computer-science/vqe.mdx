...
(a) Normalizza la funzione d'onda di prova determinando $A$ tale che $$ \int_{-\infty}^{\infty} |\Psi_\text{trial}|^2 dx = 1 $$.

<Accordion>
<AccordionItem title="Answer">

Per normalizzare la funzione d'onda di prova data:
...
(b) Calcola il valore di aspettazione dell'hamiltoniano $\hat{H}$ dato da $$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V(x)$$ dove $V(x)  = \frac{1}{2}m\omega^2x^2$, che corrisponde a un potenziale dell'oscillatore armonico semplice.

<Accordion>
<AccordionItem title="Answer">

L'hamiltoniano per un oscillatore armonico è:
...
Ottimizza $ \alpha $ per l'energia minima
...
Riempi i vuoti con i termini corretti per completare il riepilogo del processo VQE, poi clicca per verificare le tue risposte.
...
Iniziamo caricando alcuni pacchetti necessari, incluse le primitive di IBM Quantum. Selezioneremo anche il computer quantistico meno occupato disponibile.
...
Quando eseguiamo un calcolo su un computer quantistico reale, non ci interessa solo la logica del Circuit quantistico. Ci interessano anche cose come quali operazioni possano essere eseguite da quel particolare computer quantistico, e dove si trovano sul computer quantistico i qubit che stiamo usando. Sono uno accanto all'altro? Sono lontani? Pertanto, il passo successivo è riscrivere il nostro circuito usando Gate naturali per il computer quantistico che useremo, tenendo conto della disposizione dei qubit. Questo può essere fatto tramite `transpilation` - dopo questo processo, puoi vedere il nostro semplice ansatz convertito in un diverso insieme di Gate, e i nostri qubit astratti verranno mappati su qubit fisici su un computer quantistico reale.
...
Nota: puoi ottenere un risultato migliore impostando un livello di mitigazione degli errori usando [`resilience_level`](/guides/error-mitigation-and-suppression-techniques). Il valore predefinito è 1, e se imposti un valore più alto, verrà utilizzato più tempo QPU ma potrebbe restituire un risultato migliore.
...
### Check your understanding {#check-your-understanding-3}
...
Ora che abbiamo esaminato il processo complessivo di VQE usando atomi $H$, calcoleremo ora più rapidamente l'energia dello stato fondamentale della molecola $H_2$.

### Passo 1: Mappare il problema su circuiti e operatori quantistici {#step-1-map-the-problem-to-quantum-circuits-and-operators-1}

Qui ti forniamo anche un'Hamiltoniana a un qubit che usa la base STO-6G e la trasformazione di Jordan-Wigner, con riduzione dei qubit sfruttando una simmetria dell'Hamiltoniana. Nota che abbiamo usato una distanza atomica tra due atomi di idrogeno di `0.735` $\mathring A$.

A differenza del calcolo di un singolo atomo di idrogeno ($H$), per calcolare lo stato fondamentale di una molecola di idrogeno ($H_2$), dobbiamo anche considerare la forza repulsiva che agisce tra i nuclei dei due atomi di idrogeno, oltre all'energia associata agli orbitali elettronici. In questo passo, daremo questo valore come costante, e calcoleremo effettivamente questo valore nel problema di verifica.
...
### Passo 2: Ottimizza per l'hardware di destinazione {#step-2-optimize-for-target-hardware-1}

Poiché il numero di qubit utilizzati dal precedente VQE e dall'Hamiltoniana è lo stesso del backend da utilizzare per l'esecuzione, useremo l'ansatz esistente e la sua forma ottimizzata.

```python
h2_hamiltonian_isa = h2_hamiltonian.apply_layout(layout=ansatz_isa.layout)
```

### Passo 1: Mappa il problema su circuiti quantistici e operatori {#step-3-execute-on-target-hardware-1}

Ora è il momento di fare i calcoli sulla QPU reale. Quasi tutto è uguale, ma useremo il punto iniziale appropriato per adattare l'Hamiltoniana. Inoltre, in una parte iterativa, alcune delle impostazioni dell'`Estimator`, che viene utilizzato per calcolare le aspettative dell'Hamiltoniana per l'ansatz nella QPU, saranno impostate leggermente in modo diverso rispetto ai calcoli precedenti. Discuteremo questo cambiamento più approfonditamente in una domanda di verifica.
...
Sebbene il VQE fornisca teoricamente un limite superiore alla vera energia dello stato fondamentale, le implementazioni pratiche su hardware quantistico reale o rumoroso simulato, così come le approssimazioni fatte nel preparare l'Hamiltoniana (come i set di basi o la riduzione dei qubit), possono introdurre errori che a volte danno come risultato un'energia misurata leggermente inferiore rispetto al valore teorico esatto o a un riferimento numerico specifico. Sebbene ci siano alcuni errori, i risultati sembrano essere soddisfacenti, specialmente dato il piccolo numero di passi. Ora, concludiamo questo calcolo VQE osservando come ha funzionato l'ottimizzatore.

### Passo 2: Ottimizza per l'hardware di destinazione {#step-4-post-process-1}
...
### Passo 3: Esegui sull'hardware di destinazione {#check-your-understanding-4}
Calcoliamo l'energia di repulsione nucleare della molecola di $H_2$, che abbiamo incluso come valore costante (0.71997 Hartree).

![H2 molecule](/learning/images/modules/computer-science/vqe/h2.avif)

Utilizza la [legge di Coulomb](https://en.wikipedia.org/wiki/Coulomb%27s_law) e le [unità atomiche](https://en.wikipedia.org/wiki/Atomic_units) per assicurarti di ottenere il valore in `Hartree`.
...
Per calcolare questa energia in Hartree, dobbiamo convertire l'equazione precedente nel sistema di Unità Atomiche (AU). In AU, $e^2 = 1$, $4\pi\epsilon_0=1$ e il raggio di Bohr ($a_0$) è 1 e diventa la scala di lunghezza fondamentale in AU. Con queste semplificazioni, la legge di Coulomb si riduce a:
...
## Calcola l'energia di reazione di $H + H = H_2$ {#compute-reaction-energy-of-h-h-h-2}

Ora usiamo quello che abbiamo ottenuto! Hai usato il VQE, un variational quantum eigensolver, per calcolare l'energia dello stato fondamentale dell'atomo di $H$ e della molecola di $H_2$. Quello che resta da fare è usare i valori calcolati per ottenere l'energia di reazione del processo $H+H=H_2$.

L'energia di reazione è il cambiamento di energia che avviene quando le sostanze reagiscono per formare nuove sostanze. Immagina di costruire qualcosa: a volte devi immettere energia (come impilare blocchi), e a volte l'energia viene rilasciata (come una palla che rotola in discesa). In chimica, le reazioni assorbono energia (endotermiche) o rilasciano energia (esotermiche).

L'energia di reazione del processo $H+H = H_2$ può essere calcolata con la seguente formula:

L'energia di reazione del processo $H+H = H_2$ può essere calcolata con la seguente formula:

Eseguendo la cella seguente, vediamo questo visivamente. Qui useremo il valore esatto dello stato fondamentale di ciascuna Hamiltoniana, e confronteremo l'energia di reazione della soluzione esatta e dei risultati del VQE.
...
Come mostrato nella figura, sebbene ci siano alcuni errori, l'energia esatta dello stato fondamentale delle Hamiltoniane e l'energia di reazione calcolata usando i risultati del VQE sono simili, vicine a -0.2 Hartree.
