...
Vorremmo scoprire quale di queste funzioni (1-4) sia la nostra $f(x)$. Classicamente, dovremmo eseguire la funzione due volte — una per $x=0$, una per $x=1$. Ma vediamo se possiamo fare meglio con un Circuit quantistico. Possiamo apprendere informazioni sulla funzione con il seguente Gate:
...
Usando Qiskit selezioneremo casualmente una delle quattro funzioni possibili sopra indicate ed eseguiremo il Circuit. Il tuo compito è usare le misurazioni del Circuit quantistico per apprendere la funzione nel minor numero di esecuzioni possibile.
...
Nel Circuit sopra, il Gate di Hadamard "H" porta il qubit 0, che si trova inizialmente nello stato $|0\rangle$, allo stato di sovrapposizione $\frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)$. Quindi, $U_f$ valuta la funzione $f(x)$ e la applica al qubit 1.

Ora dobbiamo ottimizzare e traspilare il Circuit per eseguirlo sul computer quantistico:
...
Infine, eseguiamo il nostro Circuit traspilato sul computer quantistico e visualizziamo i risultati:
...
Sopra è mostrato un istogramma dei nostri risultati. A seconda del numero di shots scelti per eseguire il Circuit nel passaggio 3 sopra, potresti vedere una o due barre, che rappresentano gli stati misurati dei due qubit in ogni shot. Come sempre con Qiskit e in questo notebook, usiamo la notazione "little endian", il che significa che gli stati dei qubit da 0 a n sono scritti in ordine crescente da destra a sinistra, quindi il qubit 0 si trova sempre all'estrema destra.

Quindi, poiché il qubit 0 era in uno stato di sovrapposizione, il Circuit ha valutato la funzione per *entrambi* $x=0$ e $x=1$ *allo stesso tempo* — qualcosa che i computer classici non possono fare! Ma il problema emerge quando vogliamo apprendere informazioni sulla funzione $f(x)$ — quando misuriamo i qubit, il loro stato collassa. Se selezioni "shots = 1" per eseguire il Circuit una sola volta, vedrai solo una barra nell'istogramma sopra e le tue informazioni sulla funzione saranno incomplete.
