...
L'amplificazione dell'ampiezza è un algoritmo quantistico di uso generale, o sottoalgoritmo, che può essere usato per ottenere un incremento quadratico rispetto a un certo numero di algoritmi classici. [L'algoritmo di Grover](https://arxiv.org/abs/quant-ph/9605043) è stato il primo a dimostrare questo incremento sui problemi di ricerca non strutturata. La formulazione di un problema di ricerca di Grover richiede una funzione oracolo che contrassegna uno o più stati della base computazionale come gli stati che vogliamo trovare, e un Circuit di amplificazione che aumenta l'ampiezza degli stati contrassegnati, sopprimendo di conseguenza gli stati rimanenti.

Qui mostriamo come costruire gli oracoli di Grover e usare il `GroverOperator` dalla libreria di Circuit di Qiskit per impostare facilmente un'istanza di ricerca di Grover. La primitiva `Sampler` di runtime consente un'esecuzione fluida dei Circuit di Grover.
...
![Un diagramma di un Circuit quantistico che mostra la struttura di base dell'algoritmo di Grover. Questo esempio usa quattro qubit.](/learning/images/modules/computer-science/grovers/grover-circuit-diagram-2.avif)
...
A un livello molto alto e astratto, puoi pensare ai passi nel Circuit nei seguenti modi:
...
Ecco il diagramma del Circuit con gli stati quantistici etichettati in vari punti. Nota che con soli due qubit, ci sono solo quattro stati possibili che potrebbero essere misurati in qualsiasi circostanza: $|00\rangle$, $|01\rangle$, $|10\rangle$ e $|11\rangle$.

![Un diagramma di un Circuit quantistico che implementa l'algoritmo di Grover su due qubit.](/learning/images/modules/computer-science/grovers/grover-circuit-diagram-2-q-ex.avif)
...
Ora scegliamo uno stato "contrassegnato" specifico come bersaglio e applichiamo la funzione che abbiamo appena definito. Vediamo che tipo di Circuit ha creato.
...
Se i qubit 1-3 sono nello stato $|1\rangle$ e il qubit 0 si trova inizialmente nello stato $|0\rangle$, il primo gate X invertirà il qubit 0 a $|1\rangle$ e tutti i qubit saranno in $|1\rangle.$ Ciò significa che il gate MCMT applicherà un cambio di segno complessivo o inversione di fase, come desiderato. Per qualsiasi altro caso, o i qubit 1-3 sono nello stato $|0\rangle$, oppure il qubit 0 viene invertito allo stato $|0\rangle$, e l'inversione di fase non verrà applicata. Vediamo che questo Circuit contrassegna effettivamente il nostro stato desiderato $|0111\rangle,$ o la bitstring `{1110}`.
...
Costruiamo ora un Circuit che include i Hadamard gate iniziali per creare una sovrapposizione di tutti gli stati possibili, e applichiamo l'operatore di Grover il numero ottimale di volte.
...
Abbiamo costruito il nostro Circuit di Grover!
...
Ora usiamo un pass manager preimpostato per ottimizzare il nostro Circuit quantistico per il backend selezionato.
...
Vale la pena notare che la profondità del Circuit quantistico transpilato è sostanziale.
...
## Esecuzione con i primitivi Qiskit {#execute-using-qiskit-primitives}

Vogliamo effettuare molte misurazioni e vedere quale stato è il più probabile. Un'amplificazione dell'ampiezza di questo tipo è un problema di campionamento adatto all'esecuzione con il primitivo Qiskit Runtime `Sampler`.
...
## Passo 3: Esecuzione con i primitivi Qiskit {#step-3-execute-using-qiskit-primitives}
