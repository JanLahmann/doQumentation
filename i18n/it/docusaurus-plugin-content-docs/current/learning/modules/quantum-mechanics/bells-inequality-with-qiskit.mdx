...
dove $\vec{a}$ e $\vec{b}$ sono gli stati di spin del tuo sistema e $\vec{c}$ e $\vec{d}$ sono stati di spin di riferimento (qualsiasi altro possibile stato di spin del sistema). Questa è una delle tante diseguaglianze ora note come "disuguaglianze di Bell". Non utilizzeremo questa forma generale qui. Invece, ci concentreremo su una specifica configurazione sperimentale, in modo da poterla mappare su un Circuit quantistico.
