...
Il codice seguente esegue i circuito GHZ su un vero computer IBM Quantum. Questo ti mostrerà come le prestazioni degradano su un dispositivo reale.
...
Ora usa un simulatore rumoroso basato su `FakeVigo` (che ha tempi T1 di ~50-100 µs) per eseguire questi circuito. Il simulatore applicherà automaticamente gli errori T1/T2 durante le istruzioni `delay`. Esegui il transpiling dei Circuit per questo backend ed eseguili.
