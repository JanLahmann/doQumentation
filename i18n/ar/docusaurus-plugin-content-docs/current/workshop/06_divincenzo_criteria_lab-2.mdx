...
حدّد الفيزيائي ديفيد ديفينتسنزو خمسة متطلبات رئيسية لأي تطبيق فيزيائي لحاسوب كمي، إضافةً إلى معيارين إضافيين للاتصالات الكمية. في هذا الكتيّب، سنـ**ختبر كل معيار من معايير ديفينتسنزو من خلال عروض توضيحية عملية باستخدام Qiskit**. بدلاً من التعمق في النظرية، يشرح كل قسم باختصار معياراً واحداً ثم يقدم تمارين برمجية باستخدام Qiskit 2. ستتمكن من تشغيل الدائرة على المحاكيات وأجهزة IBM Quantum الحقيقية لـ**استكشاف كل مبدأ بطريقة تطبيقية**.

**معايير ديفينتسنزو الخمسة للحوسبة الكمية**:

1. **نظام فيزيائي قابل للتوسع ذو كيوبتات محددة الخصائص جيداً.**

2. **القدرة على تهيئة الكيوبتات** إلى حالة مرجعية بسيطة (مثل |00…0〉).

3. **أزمنة تماسك طويلة** (زمن تماسك الكيوبت أطول بكثير من زمن تشغيل البوابة).

4. **مجموعة شاملة من البوابات الكمية** (قادرة على تنفيذ أي عملية وحدوية اعتباطية).

5. **قدرة قياس خاصة بكل كيوبت** (قراءة حالة كل كيوبت على حدة).


*(وصف ديفينتسنزو أيضاً معيارين للاتصالات الكمية: القدرة على التحويل بين الكيوبتات الثابتة و"الطائرة"، وإرسال الكيوبتات الطائرة بدقة بين المواقع. وقد أدرجنا هذين المعيارين في نشاط موصى به في نهاية هذا الكتيّب.)*

يتوافق كل قسم من الأقسام التالية مع معيار واحد. سنستخدم Qiskit لتوضيح المفهوم بالكود و**تجارب تفاعلية** يمكنك تجربتها. على سبيل المثال، سنرى كيف يؤثر توسيع عدد الكيوبتات وعمق الدائرة على النتائج (المعيار 1)، وكيفية إعادة تعيين حالات الكيوبتات وتهيئتها (المعيار 2)، وكيفية قياس الكيوبتات على المحاكيات مقابل الأجهزة الحقيقية (المعيار 4)، وكيف يؤلف Qiskit البوابات الشاملة (المعيار 3)، وكيف يؤثر التماسك المحدود (T₁، T₂) على العمليات الحسابية (المعيار 5). بنهاية هذا الكتيّب، ستمتلك حدساً أعمق حول ما يعنيه كل معيار من معايير ديفينتسنزو عملياً وكيف يتيح Qiskit التجريب بها.
...
## 1. المعيار 1 – كيوبتات قابلة للتوسع ومحددة الخصائص جيداً {#1-criterion-1-scalable-well-characterized-qubits}

**المعيار 1:** *"نظام فيزيائي قابل للتوسع ذو كيوبتات محددة الخصائص جيداً."* يعني هذا أننا بحاجة إلى منصة أجهزة كمية يمكننا فيها **زيادة عدد الكيوبتات** والتحكم فيها بشكل موثوق. ينبغي أن تكون خصائص كل كيوبت (مستويات الطاقة، معدلات الخطأ، الترابط، وما إلى ذلك) مفهومة جيداً. في جوهر الأمر، نريد بناء دوائر أكبر دون أن ينهار النظام. في الممارسة العملية، مع توسع عدد الكيوبتات أو عمق الدائرة، تتراكم الأخطاء وظاهرة فقدان التماسك، لذا فإن إثبات *قابلية التوسع* يعني أيضاً فهم كيفية تأثير الحجم المتزايد على الأداء.

**هدف العرض التوضيحي:** استخدام Qiskit لإظهار تأثير توسيع دائرة (من حيث عدد الكيوبتات أو عمق البوابات) على دقة المخرجات. سنحاكي سيناريو مثالياً مقابل سيناريو مشوّش لنرى كيف يستسلم النظام الأكبر أو الدائرة الأعمق لفقدان التماسك والأخطاء.

أولاً، لنبنِ حالة متشابكة صغيرة (حالة GHZ) على 3 كيوبتات، ثم حالة أكبر على 5 كيوبتات، كاختبار توسع بسيط. حالة GHZ لـ *n* كيوبت هي $\frac{1}{\sqrt{2}}(|0...0\rangle + |1...1\rangle)$. في المحاكاة المثالية، قياس حالة GHZ ذات n كيوبت يُنتج فقط نتيجتين (إما الكل 0 أو الكل 1) باحتمال متساوٍ. سنقارن **المخرج المثالي** بـ**المخرج المشوَّش** مع زيادة n أو عمق الدائرة.
...
**النتيجة المتوقعة (الحالة المثالية):** تُنتج حالة GHZ ذات 3 كيوبتات مثالياً نحو 50% `000` و50% `111` في التعدادات. أما حالة GHZ ذات 5 كيوبتات فتُنتج نحو 50% `00000` و50% `11111`. لا تظهر سلاسل بت أخرى لأن الحالة في الوضع المثالي متماسكة ومتشابكة تماماً. ينبغي أن ترى شريطين طويلين في المخطط الرسومي لكل دائرة يتوافقان مع نتيجتَي الكل-أصفار والكل-آحاد.

بعد ذلك، دعونا نرى ما يحدث في **بيئة مشوَّشة**. سنستخدم قدرات نموذج الضوضاء في Qiskit Aer لمحاكاة أخطاء جهاز حقيقي. على سبيل المثال، يمكننا أخذ خصائص Backend من IBM لإنشاء نموذج ضوضاء يتضمن أخطاء البوابات، وأزمنة البوابات المحدودة، وانحلال الكيوبت (T₁)، وتبدد الطور (T₂)، وأخطاء القراءة. هنا، سنستخدم **Backend وهمية** تمثل جهاز IBM Quantum Brisbane لتوليد نموذج ضوضاء، وإعادة تشغيل دوائر GHZ من خلاله.
...
**النتيجة المتوقعة (المشوَّشة مقابل المثالية):** مع الضوضاء، سواء في المحاكاة أو على جهاز حقيقي، تكون حالة GHZ **أقل دقةً**. ستشهد نتائج إضافية تتجاوز الكل-أصفار والكل-آحاد. بالنسبة لـ 3 كيوبتات، بدلاً من 100% في `000`/`111`، يتسرب بعض الاحتمال إلى سلاسل بت أخرى (مثل `001`، `010`، وما إلى ذلك) بسبب أخطاء البوابات أو فقدان التماسك الذي يُقلب بعض الكيوبتات. أما بالنسبة لـ 5 كيوبتات، فالتأثير أكثر وضوحاً؛ إذ تتراكم في الدائرة الأكبر (المزيد من الكيوبتات وبوابات CNOT) مزيداً من الأخطاء، فتنخفض قمم الكل-أصفار والكل-آحاد، وتظهر نتائج أخرى كثيرة. يُوضح هذا النمط تحدي *قابلية التوسع*: كلما توسعنا، أصبح الحفاظ على دقة عالية أصعب من دون تصحيح الأخطاء.

**الاستنتاج:** يحتاج الحاسوب الكمي القابل للتوسع إلى الحفاظ على الترابطات الكمية مع نمو النظام. تُظهر أمثلتنا كيف أن زيادة عدد الكيوبتات وعمق البوابات تتسبب في انخفاض دقة المخرجات عند وجود ضوضاء. ستتناول المعايير المتبقية الحفاظ على سلوك الكيوبتات (خطأ منخفض، قابلة للتهيئة، إلخ) مع التوسع.

## 2. المعيار 2 – تهيئة الكيوبت {#2-criterion-2-qubit-initialization}

**المعيار 2:** *"القدرة على تهيئة حالة الكيوبتات إلى حالة مرجعية بسيطة، مثل |000…〉."* ينبغي أن تبدأ جميع الكيوبتات بشكل موثوق في حالة مرجعية معروفة (عادةً حالة الأساس |0〉 لكل كيوبت). التهيئة ضرورية لضمان بدء الخوارزميات من صفحة نظيفة. في الممارسة العملية، على أجهزة IBM الكمية، تُعاد كل كيوبت تلقائياً إلى |0〉 عند بدء تنفيذ كل دائرة. يوفر Qiskit أيضاً تعليمات لإعادة تعيين الكيوبتات أو تحضير حالات مخصصة أثناء الحساب.

**هدف العرض التوضيحي:** إظهار كيفية تهيئة الكيوبتات في Qiskit، سواء في البداية أو في منتصف الدائرة. سنوضح استخدام تعليمة `reset` وأساليب تحضير الحالة.

### التمرين 2: تحضير حالة محددة {#exercise-2-prepare-a-specific-state}
في كتلة الكود أدناه، أكمل الـ `QuantumCircuit` لتحضير الحالة $|10\rangle$. يعني هذا أن الكيوبت 0 ينبغي أن تكون في الحالة $|0\rangle$ والكيوبت 1 في الحالة $|1\rangle$. استخدم البوابة والتعليمة المناسبتين لتحقيق ذلك.
...
ينبغي أن ترى `10` (ثنائي لـ qubit1=1، qubit0=0) باحتمال 100% من المحاكاة، مما يعني أن الكيوبت 1 جرى تحضيرها بنجاح في |1〉 والكيوبت 0 في |0〉.

الآن، لتحضير حالة أعم، يتيح Qiskit التهيئة إلى حالات اعتباطية باستخدام الأسلوب `initialize`. على سبيل المثال، لنحضّر كيوبت في الحالة $|+\rangle = (|0\rangle+|1\rangle)/\sqrt{2}$، وهي حالة تراكب، وزوجاً من الكيوبتات في حالة Bell $(|00\rangle+|11\rangle)/\sqrt{2}$:
...
**النتائج المتوقعة:** ستُنتج حالة |+〉 للكيوبت الواحدة، عند قياسها، `0` و`1` بنسبة 50% تقريباً لكل منهما. ينبغي أن يُعطي قياس حالة Bell نحو 50% `00` و50% `11`. إذا رأيت هذه النتائج، فهذا يؤكد نجاح تهيئتنا لتلك الحالات.

**التهيئة في منتصف الدائرة:** يمكن استخدام `reset` في Qiskit في منتصف الدائرة لإعادة تهيئة الكيوبت إلى |0〉 أثناء التنفيذ. على سبيل المثال، في أكواد تصحيح الأخطاء أو الخوارزميات التكرارية، كثيراً ما يُقاس الكيوبت ثم يُعاد تعيينه لإعادة استخدامه. عملية `reset` حتمية؛ إذ تُفرغ أي حالة موجودة وتُعيد الكيوبت إلى حالة الأساس.

**مثال على الجهاز:** على أجهزة مثل **ibmq_brisbane** (127 كيوبت) أو أي جهاز IBM، تبدأ جميع الكيوبتات في |0〉 بشكل افتراضي عند تشغيل مهمة. إذا احتجت إلى حالة بداية مختلفة، ستطبّق بوابات في البداية (كما فعلنا باستخدام X للحصول على |1〉). إعادة التهيئة المستمرة (لتصحيح الأخطاء الكمية) موضوع بحث نشط لأن تنفيذها بسرعة أمر صعب. لحسن الحظ، للاستخدام الأساسي، تتوفر القدرة على البدء من جديد في |0…0〉 وأوضحنا كيفية تحقيق حالات بداية مرغوبة أخرى أيضاً.
...
**المعيار الثالث:** *"أزمنة تبدد ترابط طويلة وذات صلة، أطول بكثير من زمن تشغيل البوابة."* يتناول هذا المعيار ضرورة أن تحافظ الكيوبتات على حالتها الكمية لفترة كافية لإجراء العمليات اللازمة. لكل كيوبت **زمن T₁** (زمن استرخاء الطاقة، وهو مدى سرعة تحلل الحالة |1〉 إلى |0〉) و**زمن T₂** (زمن تبدد الطور، وهو مدى سرعة فقدان الترابط الطوري النسبي). لكي يعمل الحاسوب الكمي بشكل صحيح، يجب أن تتجاوز هذه الأزمنة بكثير مدة عمليات البوابة.
...
**لإظهار تأثير الترابط المحدود**، سنحاكي تجربة تحلل T1. سنهيئ الكيوبت في الحالة |1〉، ننتظر لبعض الوقت باستخدام تعليمة `delay`، ثم نقيس. نتوقع أن ينخفض احتمال قياس |1〉 مع زيادة التأخير.
...
![مخطط دائرة كمية](/img/workshop/06-divincenzo-criteria-lab-2/output_3.png)
...
**المعيار الخامس:** *"قدرة قياس خاصة بكل كيوبت."* يجب أن تكون حالة كل كيوبت قابلة للقياس (عادةً في الأساس الحسابي، |0〉 أو |1〉). بعبارة أخرى، بعد تشغيل دائرة كمية، نحتاج إلى قراءة كل كيوبت كبت كلاسيكي 0/1. يتعلق هذا المعيار بامتلاك كاشفات موثوقة لكل كيوبت والقدرة على تحديد أي كيوبتات يجري قياسها.
...
![مخطط دائرة كمية](/img/workshop/06-divincenzo-criteria-lab-2/output_4.png)
...
من خلال إتمام التمارين الواردة في هذا الكتيّب، لم تقتصر على رؤية تعريفات معايير ديفينتسنزو، بل *لمستها* عبر الكود؛ مما أرسى لديك حدساً عميقاً بمعنى كل متطلب في سياق أجهزة الحوسبة الكمية والخوارزميات الفعلية. لا تتردد في توسيع هذه التجارب، ومع تمنياتنا لك بحوسبة كمية ممتعة!
