...
يُستخدم الـ mock backend من النوع `FakeSherbrooke` من `qiskit_ibm_runtime` في هذه الأمثلة، لكن يمكنك تجربته على أي backend حقيقي أو وهمي متوافق مع Qiskit. قد تكون نتائجك مختلفة.
</Admonition>
