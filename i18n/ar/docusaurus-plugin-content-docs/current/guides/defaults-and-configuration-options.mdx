---
title: "إعدادات Transpilation الافتراضية وخيارات الضبط"
sidebar_label: "إعدادات Transpilation الافتراضية وخيارات الضبط"
...
<Accordion>
<AccordionItem title="**معاملات وحدة QPU (`Backend`)**">
...
<AccordionItem title="**معاملات التخطيط والطوبولوجيا**">
...
- `coupling_map` (CouplingMap | List[List[int]]) - خريطة اقتران موجَّهة (ربما مخصصة) تُستهدف في عملية التعيين. إذا كانت خريطة الاقتران متماثلة، يجب تحديد الاتجاهين. الصيغ المدعومة هي:
    - كائن CouplingMap
    - قائمة (List) - يجب إعطاؤها كمصفوفة تجاور، حيث يُحدد كل إدخال جميع التفاعلات الثنائية الموجَّهة بين الـ qubits التي تدعمها وحدة QPU. مثلاً: [[0, 1], [0, 3], [1, 2], [1, 5], [2, 5], [4, 1], [5, 3]]
...
<Accordion>
<AccordionItem title="**مرحلة التهيئة**">
...
<AccordionItem title="**مرحلة التخطيط (Layout)**">
...
<AccordionItem title="**مرحلة التوجيه (Routing)**">
...
<AccordionItem title="**مرحلة الترجمة (Translation)**">
...
<AccordionItem title="**مرحلة التحسين (Optimization)**">
...
<AccordionItem title="**مرحلة الجدولة (Scheduling)**">
...
<AccordionItem title="**تنفيذ الـ Transpiler**">
...
- تعلم كيفية [ضبط مستوى التحسين عند استخدام Qiskit Runtime](/guides/runtime-options-overview).
- اطلع على موضوع [الـ Transpile باستخدام pass managers](/guides/transpile-with-pass-managers).
- للأمثلة، راجع [تمثيل الحواسيب الكمومية](/guides/represent-quantum-computers).
