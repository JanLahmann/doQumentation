...
يُغلّف هذا القالب سير عمل لمحاكاة التطور الزمني لحالة ابتدائية مقارنةً بهاميلتونيان قائم على السبين يُعرّفه المستخدم، ويُعيد مجموعة من قيم التوقع المحددة باستخدام إضافة Qiskit [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/).

هذا القالب مُهيكل كنمط Qiskit يتضمن الخطوات التالية:

#### 1. جمع المدخلات وتعيين المسألة {#1-collecting-input-and-mapping-the-problem}
...
#### 2. إعداد الدوائر المولَّدة للتنفيذ {#2-prepare-the-generated-circuits-for-execution}
...
أولاً، اكتب قالب دالة لمحاكاة هاميلتونيان يستخدم [إضافة AQC-Tensor لـ Qiskit](https://qiskit.github.io/qiskit-addon-aqc-tensor/) لتعيين وصف المسألة إلى دائرة ذات عمق مخفَّض للتنفيذ على العتاد.

إذا قمت بتنزيل هذه الصفحة وعرضتها محليًا في محرر notebook، فستلاحظ أن بعض خلايا الكود تحتوي على [الأمر السحري](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. يحفظ هذا الأمر السحري الكود في `./source_files/template_hamiltonian_simulation.py`، وهو قالب الدالة الذي يمكنك رفعه وتشغيله عن بُعد مع Qiskit Serverless.
...
<Accordion>
<AccordionItem title="**الكود المصدري الكامل للبرنامج**">
