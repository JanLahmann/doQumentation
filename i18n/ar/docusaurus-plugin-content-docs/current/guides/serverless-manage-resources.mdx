...
<Admonition type="tip">
**يشهد Qiskit Serverless تحسينات متسارعة، وميزاته في تطوّر مستمر.** خلال مرحلة التطوير هذه، يمكنك الاطلاع على ملاحظات الإصدار وأحدث التوثيقات في صفحة [Qiskit Serverless على GitHub](https://qiskit.github.io/qiskit-serverless/index.html).
</Admonition>
...
إذا كنت تقرأ خلايا الكود محليًا في دفتر ملاحظات، فستلاحظ الأمر السحري `%%writefile` [magic command](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile). تشغيل الخلايا التي تحتوي على هذا الأمر السحري يحفظها على القرص بدلًا من تنفيذها.
</Admonition>
...
بالنسبة للمهام الكلاسيكية القابلة للتوازي، استخدم مزيّن `@distribute_task` لتحديد متطلبات الحوسبة اللازمة لتنفيذ المهمة. ابدأ باسترجاع مثال `transpile_remote.py` من موضوع [كتابة أول برنامج Qiskit Serverless](/guides/serverless-first-program) بالكود التالي.
...
يوضح ما يلي كيفية رفع الملفات. تأكد من أنك قمت بالمصادقة على Qiskit Serverless بحساب IBM Quantum الخاص بك (راجع [رفع البرنامج إلى Qiskit Serverless](/guides/serverless-first-program#upload-to-qiskit-serverless) للحصول على التعليمات).
...
- اطّلع على مثال كامل يقوم بـ [نقل الكود الموجود إلى Qiskit Serverless](/guides/serverless-port-code).
