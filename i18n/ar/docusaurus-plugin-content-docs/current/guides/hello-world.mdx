...
sidebar_label: 'شغّل أول دائرة لك على العتاد الفعلي'
title: 'شغّل أول دائرة لك على العتاد الفعلي'
---
...
1. إذا لم تكن قد ثبّتَ Qiskit بعد، فستجد التعليمات في دليل [البداية السريعة](/guides/quick-start).
    ...
    - ثبّت عميل IBM Quantum Compute لتشغيل المهام على أجهزة الكم:
...
2. أعدّ مصادقتك للوصول إلى أجهزة الكم من خلال [الخطة المجانية المفتوحة](/guides/plans-overview#open-plan).
    ...
    - اذهب إلى [منصة IBM Quantum](https://quantum.cloud.ibm.com/) لتسجيل الدخول أو إنشاء حساب.
         ...
         إذا كنت تتصل عبر خادم وكيل (proxy)، يجب عليك استخدام `qiskit-ibm-runtime` الإصدار v0.44.0 أو أحدث.
         </Admonition>
...
3. الآن يمكنك استخدام كود Python هذا في أي وقت تريد فيه المصادقة على IBM Quantum Compute Service:
...
عند إنشاء الدوائر الكمومية، يجب أيضاً أن تفكر في نوع البيانات التي تريد إرجاعها بعد التنفيذ. يوفر Qiskit طريقتين لإرجاع البيانات: يمكنك الحصول على نتائج أخذ العيّنات لمجموعة من Qubits تختار قياسها، أو يمكنك الحصول على قيمة التوقع لمؤثِّر ما. جهّز حِملك لقياس دائرتك بإحدى هاتين الطريقتين باستخدام الدوال الأساسية (primitives) (مشروح بالتفصيل في [الخطوة 3](#step-3-execute-using-the-quantum-primitives)).
...
أرسل المهمة وفعّل قمع الأخطاء باستخدام تقنية تُسمى [الفصل الديناميكي (dynamical decoupling)](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-dynamical-decoupling-options) للحدّ من الأخطاء. يحدّد مستوى المرونة (resilience level) مقدار الحماية المبنية ضد الأخطاء؛ فكلما ارتفع المستوى، كانت النتائج أدق، لكن على حساب وقت معالجة أطول. لمزيد من التفصيل حول الخيارات المضبوطة في الكود التالي، راجع [تقنيات تخفيف الأخطاء وتثبيطها](/guides/error-mitigation-and-suppression-techniques).
