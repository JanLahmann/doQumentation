---
title: الترقية إلى أوليات Qiskit Runtime الإصدار الثاني (V2 primitives)
...
# الترقية إلى أوليات Qiskit Runtime الإصدار الثاني (V2 primitives) {#migrate-to-the-qiskit-runtime-v2-primitives}
...
4. جمِّع كل دائرة تريد تشغيلها مع المراصد وقيم المعاملات المراد تطبيقها عليها في صف (PUB). مثلًا، استخدم `(circuit1, observable1, parameter_set1)` إذا أردت تشغيل `circuit1` مع `observable1` و`parameter_set1`.

  1. قد تحتاج إلى إعادة تشكيل مصفوفات المراصد أو مجموعات المعاملات إذا أردت حساب جداءها الخارجي. على سبيل المثال، مصفوفة مراصد بشكل (4, 1) ومصفوفة مجموعات معاملات بشكل (1, 6) ستعطيك نتيجة بشكل (4, 6) قيمة توقع. راجع [قواعد البث في NumPy](https://numpy.org/doc/stable/user/basics.broadcasting.html) لمزيد من التفاصيل.
  2. يمكنك اختياريًا تحديد الدقة التي تريدها لهذا الـ PUB تحديدًا.
5. حدِّث طريقة `run()` في Estimator لتمرير قائمة الـ PUBs. مثلًا: `run([(circuit1, observable1, parameter_set1)])`.
...
استخدم Estimator لتحديد قيمة التوقع لزوج واحد من Circuit والمرصد.
...
استخدم Estimator لتحديد قيم التوقع لأزواج متعددة من الـ Circuit والمراصد.
...
4. اجمع كل دائرة تريد تشغيلها مع المراصد (observables) وقيم المعاملات في tuple واحدة (وهي ما يُعرف بـ PUB). مثلًا، استخدم `(circuit1, parameter_set1)` إذا أردت تشغيل `circuit1` مع `parameter_set1`.
   يمكنك اختياريًا تحديد عدد القياسات (shots) لكل PUB بشكل منفصل.
5. حدِّث طريقة `run()` في Sampler لتمرير قائمة PUBs. مثلًا، `run([(circuit1, parameter_set1)])`.
...
<Admonition type="caution">
سيُرجع مقطع الكود التالي خطأً للمستخدمين الذين يستخدمون الخطة المفتوحة (Open Plan)، لأنه يستخدم الجلسات. لا يمكن تشغيل أعباء العمل على الخطة المفتوحة إلا في [وضع المهمة (job mode)](/guides/execution-modes#job-mode) أو [وضع الدُفعات (batch mode)](/guides/execution-modes#batch-mode).
</Admonition>
...
- تعلَّم المزيد عن ضبط الخيارات في أدلة [خيارات Sampler](/guides/sampler-options) و[خيارات Estimator](/guides/estimator-options) و[خيارات Executor](/guides/executor-options).
- اطَّلع على مزيد من التفاصيل حول [مدخلات ومخرجات Primitives](/guides/primitive-input-output#pubs).
