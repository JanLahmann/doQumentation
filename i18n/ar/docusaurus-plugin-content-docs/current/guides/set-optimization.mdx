---
title: "ضبط مستوى تحسين المحوّل"
sidebar_label: "ضبط مستوى تحسين المحوّل"
description: "كيفية ضبط مستوى التحسين لتحويل الدوائر الكمومية في Qiskit."
...
## مستوى التحسين عملياً {#optimization-level-in-action}
...
يُستخدم في هذه الأمثلة backend المحاكى `FakeSherbrooke` من `qiskit_ibm_runtime`، لكن يمكنك تجربته على أي backend حقيقي أو محاكى متوافق مع Qiskit. قد تختلف نتائجك.
</Admonition>

أولاً، لنُجرِّب التحويل باستخدام مستوى التحسين 0.
...
ضبط مستوى التحسين يمكن أن يُغيّر جوانب أخرى من الدائرة أيضاً، ليس فقط عدد بوابات ECR. للاطلاع على أمثلة حول كيفية تأثير مستوى التحسين على التخطيط، راجع [تمثيل الحواسيب الكمومية](/guides/represent-quantum-computers).

## الخطوات التالية {#next-steps}

<Admonition type="tip" title="توصيات">
- لمعرفة المزيد عن دالة `generate_preset_passmanager`، ابدأ بموضوع [الإعدادات الافتراضية وخيارات تهيئة التحويل](defaults-and-configuration-options).
- واصل التعلم عن التحويل مع موضوع [مراحل المحوّل](transpiler-stages).
- جرّب دليل [مقارنة إعدادات المحوّل](/guides/circuit-transpilation-settings).
- جرّب درس [بناء رموز التكرار](/tutorials/repetition-codes).
- اطلع على [توثيق Transpile API](https://docs.quantum.ibm.com/api/qiskit/transpiler).
</Admonition>
