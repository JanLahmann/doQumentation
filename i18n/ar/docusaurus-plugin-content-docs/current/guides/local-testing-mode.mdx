---
description: 'استخدم وضع الاختبار المحلي لمحاكاة الأجهزة الكمومية مع IBM Quantum Compute'
notebook_path: guides/local-testing-mode.ipynb
sidebar_label: 'وضع الاختبار المحلي'
title: 'وضع الاختبار المحلي'
---
...
لا يدعم primitive Executor وضع الاختبار المحلي.
</Admonition>

لاستخدام وضع الاختبار المحلي، حدد إحدى الخلفيات الوهمية من ``qiskit_ibm_runtime.fake_provider`` أو حدد خلفية Qiskit Aer عند إنشاء IBM Quantum primitive أو جلسة.
...
- **محاكي Aer**: توفر المحاكيات من [Qiskit Aer](/guides/simulate-with-qiskit-aer) محاكاة عالية الأداء تتعامل مع دوائر أكبر و[نماذج ضوضاء مخصصة](/guides/build-noise-models). تتوفر قائمة بخيارات طرق المحاكاة عند استخدام `AerSimulator` في وضع الاختبار المحلي. اطلع على [مثال وضع محاكاة Clifford](#clifford-sim) الذي يوضح كيفية محاكاة دوائر Clifford ذات عدد كبير من الكيوبتات بكفاءة.
    ...
    <Accordion>
    <AccordionItem title="**قائمة طرق المحاكاة المتاحة من Qiskit Aer**">
...
- يمكنك تحديد جميع خيارات IBM Quantum Compute في وضع الاختبار المحلي. غير أن جميع الخيارات باستثناء shots تُتجاهل عند التشغيل على محاكي محلي.
...
## أمثلة على AerSimulator {#aersimulator-examples}
...
- استخدم [فئة `Neat`](/guides/debug-qiskit-runtime-jobs) لتحليل تأثير الضوضاء على حمل عمل Estimator.
- راجع [أمثلة تفصيلية على Sampler](/guides/sampler-examples).
- راجع [أمثلة تفصيلية على Estimator](/guides/estimator-examples).
