...
تتمحور حلقة التحسين حول ضبط معاملات دائرة كمومية لتوليد فضاء جزئي عالي الجودة. يوفّر HI-VQE ثلاثة خيارات للدائرة الكمومية: [`excitation_preserving`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.excitation_preserving) و[efficient_su2](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.efficient_su2) و[LUCJ](https://qiskit-community.github.io/ffsim/explanations/lucj.html). يُهيّأ التحسين قريبًا من حالة مرجعية Hartree-Fock لملاءمتها العامة. تُنفَّذ الدائرة بعد ذلك على جهاز كمومي وتُؤخذ عيّنات من التهيئات من الحالة الكمومية الناتجة وتُعاد كسلاسل ثنائية. نتيجةً لضوضاء الجهاز الكمومي، قد تكون بعض التهيئات المُعيَّنة غير صالحة فيزيائيًا، إذ لا تحفظ عدد الإلكترونات أو الدوران. يتعامل HI-VQE مع هذا باستخدام عملية استرداد التهيئة من حزمة [qiskit-addon-sqd](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd)، ليتمكن المستخدمون إما من تصحيح التهيئات غير الصالحة أو تجاهلها.
...
ثم قم بالمصادقة باستخدام [مفتاح IBM Quantum&reg; API](http://quantum.cloud.ibm.com/) الخاص بك، وبافتراض أنك [حفظت حسابك](/guides/functions-get-started#install-qiskit-functions-catalog-client) في بيئتك المحلية، اختر دالة Qiskit كما يلي:
...
تحقق من [الحالة](/guides/functions-get-started#check-job-status) لعمل Qiskit Function الخاص بك أو استرجع [النتائج](/guides/functions-get-started#retrieve-results) على النحو التالي:
...
لاحظ أن استخدام دالة Qiskit هذه مقيّد بالمسائل التي تتطلب 20 Qubit كحد أقصى، ما لم يُحصل على ترخيص يمنح حدًا أعلى.

أرسل بريدًا إلكترونيًا إلى [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) للاستفسارات المتعلقة بالترخيص.

### مثال على استخدام دالة مرخصة {#example-of-licensed-function-use}

يُصدَر للمستخدمين المرخّصين رمز ترخيص (license token)، ويجب عليهم بعد ذلك استخدام مكتبة غلاف (wrapper library) لإرسال رمز الترخيص الخاص بهم إلى الدالة.
يمكن تثبيت مكتبة الغلاف [من PyPI](https://pypi.org/project/hivqe-qiskit-function-utils/) باستخدام `pip install hivqe-qiskit-function-utils`.
يوضح المثال أدناه كيفية استخدام هذه المكتبة لإرسال رمز الترخيص الخاص بك عند استدعاء الدالة.
