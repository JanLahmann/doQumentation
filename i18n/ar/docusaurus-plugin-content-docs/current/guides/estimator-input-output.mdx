...
- قائمة من مراقب واحد أو أكثر، تُحدِّد قيم التوقع المُراد تقديرها، مُرتَّبة في مصفوفة (مثلاً مراقب واحد مُمثَّل كمصفوفة 0-d، وقائمة مراقبات كمصفوفة 1-d، وما إلى ذلك). يمكن أن تكون البيانات بأي تنسيق من تنسيقات `ObservablesArrayLike` كـ `Pauli` أو `SparsePauliOp` أو `PauliList` أو `str`.
   ...
   - المراقبات المتبادلة في PUBs مختلفة، حتى لو كانت لها نفس الـ دائرة، لا تُقدَّر باستخدام نفس القياس. كل PUB يُمثِّل أساسًا مختلفًا للقياس، وبالتالي تكون قياسات منفصلة مطلوبة لكل PUB.
   - لضمان تقدير المراقبات المتبادلة باستخدام نفس القياس، جمِّعها داخل نفس PUB.
   </Admonition>
- مجموعة من قيم المعاملات لربط الـ دائرة بها. يمكن تحديد هذا كائن واحد شبيه بالمصفوفة حيث الفهرس الأخير هو على كائنات `Parameter` في الـ دائرة أو حذفه (أو بشكل مكافئ، ضبطه على `None`) إذا لم يكن في الـ دائرة كائنات `Parameter`.
