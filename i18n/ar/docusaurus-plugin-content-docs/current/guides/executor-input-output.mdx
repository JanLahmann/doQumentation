...
يحتوي العنصر الأول على نتائج تشغيل المهمة الأولى (`CircuitItem`) في البرنامج. يحتوي على مفتاح واحد، `meas`، وهو اسم السجل الكلاسيكي في الدائرة المدخلة. قيمة هذا المفتاح تُعيَّن إلى `np.ndarray` بالشكل `(مجموعات المعاملات، التشغيلات، بتات السجل)`، وهو (10، 1024، 3) في المثال أعلاه.
