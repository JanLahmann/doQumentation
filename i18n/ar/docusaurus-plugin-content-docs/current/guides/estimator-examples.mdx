---
title: "أمثلة على Estimator"
sidebar_label: "أمثلة على Estimator"
description: "أمثلة عملية على استخدام Estimator primitive في Qiskit Runtime."
...
تستعرض الأمثلة في هذا القسم بعض الطرق الشائعة لاستخدام Estimator. قبل تشغيل هذه الأمثلة، اتبع التعليمات الواردة في [تثبيت Qiskit.](install-qiskit)

<Admonition type="note">
جميع هذه الأمثلة تستخدم الـ primitives من Qiskit Runtime، غير أنه يمكنك استخدام الـ base primitives بدلًا من ذلك.
</Admonition>

احسب قيم التوقع للمعاملات الكمية المطلوبة لكثير من الخوارزميات وفسِّرها بكفاءة باستخدام Estimator. استكشف التطبيقات في نمذجة الجزيئات، والتعلم الآلي، ومسائل التحسين المعقدة.

## تشغيل تجربة واحدة {#run-a-single-experiment}

استخدم Estimator لتحديد قيمة التوقع لزوج Circuit-مراقِب واحد.
...
استخدم Estimator لتحديد قيم التوقع لأزواج متعددة من Circuit-مراقِب.
...
## تشغيل Circuits ذات معاملات {#run-parameterized-circuits}

استخدم Estimator لتشغيل ثلاث تجارب في مهمة واحدة، مستفيدًا من قيم المعاملات لزيادة إمكانية إعادة استخدام الـ Circuit.
...
## استخدام الـ Batches والخيارات المتقدمة {#use-batches-and-advanced-options}

استكشف [وضع التنفيذ](/guides/execution-modes) batch والخيارات المتقدمة لتحسين أداء الـ Circuit على معالجات الكم (QPUs).
...
- [حدِّد خيارات Runtime المتقدمة](runtime-options-overview).
- تدرَّب على الـ primitives بالعمل على [درس دالة التكلفة](/learning/courses/variational-algorithm-design/cost-functions) في IBM Quantum&reg; Learning.
- تعلَّم كيفية عمل Transpile محليًا في قسم [Transpile](/guides/transpile/).
- جرِّب دليل [مقارنة إعدادات Transpiler](/guides/circuit-transpilation-settings).
- اقرأ [الانتقال إلى V2 primitives](/guides/v2-primitives).
- افهم [حدود المهمة](/guides/job-limits) عند إرسال مهمة إلى معالج IBM&reg; الكمي.
</Admonition>
