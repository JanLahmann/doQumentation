...
![صورة توضيحية للدائرة التالية](/docs/images/guides/stretch/stretch.svg)
...
:::note
 ...
 عند استخدام `stretch` مع خدمة IBM Quantum Compute، يُضاف أي باقٍ ناتج عن حل stretch إلى أول تأخير يستخدم stretch.
...
## عرض قيم stretch في خدمة IBM Quantum Compute {#view-stretch-values-in-ibm-quantum-compute-service}

تُحسم القيمة الفعلية لمدة stretch في وقت الترجمة، بعد جدولة الدائرة. عند تشغيل مهمة Sampler في IBM Quantum Compute Service، يمكنك عرض قيم stretch المحسومة في بيانات التعريف لنتيجة المهمة. دعم `stretch` في Quantum Compute تجريبي حالياً، لذا تحتاج أولاً إلى تعيين خيار تجريبي لتفعيل استرجاعه، ثم الوصول إلى البيانات مباشرةً من بيانات التعريف كما يلي:
...
على الرغم من أن إجمالي وقت الدائرة يُعاد في بيانات التعريف "compilation"، إلا أن هذا ليس الوقت المستخدم في الفوترة (وقت QPU).
</Admonition>
...
للاطلاع على التفاصيل الكاملة، راجع موضوع [تصور توقيت الدائرة](/guides/qiskit-runtime-circuit-timing).

<span id="qiskit-runtime-limitations"></span>

## قيود Quantum Compute {#quantum-compute-limitations}

دعم `stretch` في Quantum Compute تجريبي حالياً وله القيود التالية:
...
- المنطقة المحاطة بمجموعة من الحواجز تُسمى _منطقة الحاجز_. لا يمكن استخدام متغير stretch في مناطق حاجز متعددة.
    <Tabs>
        ...
        ![توضيح لمخرج الكود السابق](/docs/images/guides/stretch/bad-barrier-region.svg 'استخدام غير صالح لـ stretch في مناطق الحاجز')
     ...
     <TabItem value="Valid" label="صالح">
        ...
        ![توضيح لمخرج الكود السابق](/docs/images/guides/stretch/good-barrier-region.svg 'استخدام صالح لـ stretch في منطقة حاجز')
