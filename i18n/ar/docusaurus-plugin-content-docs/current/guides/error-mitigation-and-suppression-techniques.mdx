---
description: 'شرح لتقنيات تخفيف الأخطاء وقمعها المتاحة عبر IBM Quantum Compute Service'
...
تُظهر خلية الكود التالية كيفية تفعيل الفصل الديناميكي لـ Estimator واختيار تسلسل الفصل الديناميكي.
...
![توضيح تلوية باولي](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)
...
تُظهر خلية الكود التالية كيفية تفعيل تلوية باولي وضبط هذه الخيارات لـ Estimator. لا يُشترَط تحديد أيٍّ من هذه الخيارات صراحةً.
...
![توضيح تلوية القياس](/docs/images/guides/error-mitigation-explanation/measurement_twirling.svg)

في حضور خطأ القراءة، يُحوِّل تلوية القياس مصفوفة نقل خطأ القراءة إلى صيغة قُطرية، مما يسهّل عكسها. يُتعلَّم مصطلح إعادة القياس من القناة الضوضاء القُطرية عن طريق معايرة دوائر عشوائية مُهيَّأة في الحالة الصفرية. يُتيح ذلك للخدمة إزالة التحيز من قيم التوقع الناجمة عن ضوضاء القراءة. يتطلب تقدير مصفوفة نقل خطأ القراءة تنفيذ دوائر معايرة إضافية، مما يُضيف عبئًا صغيرًا.

### تفعيل TREX باستخدام خيار `measure_mitigation` {#enable-trex-by-using-the-measure-mitigation-option}

يمكن تفعيل TREX عن طريق ضبط `measure_mitigation` على `True` في [خيارات مرونة IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) للـ Estimator. خيارات تعلم ضوضاء القياس موصوفة [هنا](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-measure-noise-learning-options). كما هو الحال مع تلوية البوابات، يمكنك ضبط عدد التلويات العشوائية للدائرة وعدد اللقطات لكل تلوية.

تُظهر خلية الكود التالية كيفية تفعيل TREX وضبط هذه الخيارات لـ Estimator. لا يُشترَط تحديد أيٍّ من هذه الخيارات صراحةً.
...
### تفعيل TREX باستخدام خيار `resilience_level` {#enable-trex-by-using-the-resilience-level-option}

يمكنك تفعيل TREX عن طريق ضبط [مستوى المرونة](/guides/estimator-noise-management#resilience) على `1`، كما هو موضح في المثال التالي.
...
2. _الاستقراء_: تُقدَّر النتيجة المثالية عن طريق الاستقراء من نتائج قيم التوقع المشوشة نحو حد الضوضاء الصفرية.


يمكن تنفيذ كلتا مرحلتي تضخيم الضوضاء والاستقراء بطرق مختلفة متعددة. ينفذ IBM Quantum Compute تضخيم الضوضاء عن طريق "طي البوابات الرقمي"، مما يعني أن البوابات ثنائية الـ qubit تُستبدَل بتسلسلات مكافئة من البوابة ومعكوسها. على سبيل المثال، استبدال وحدوية $U$ بـ $U U^\dagger U$ يعطي عامل تضخيم ضوضاء مقداره 3. بالنسبة للاستقراء، يمكنك الاختيار من بين عدة أشكال دالية، منها ملاءمة خطية أو ملاءمة أسية.
...
يتناسب عبء هذه الطريقة مع عدد عوامل الضوضاء. تأخذ الإعدادات الافتراضية عينات من قيمة التوقع عند ثلاثة عوامل ضوضاء، مما يُفضي إلى عبء يبلغ تقريبًا ثلاثة أضعاف.

### تفعيل ZNE باستخدام خيار `zne_mitigation` {#enable-zne-by-using-the-zne-mitigation-option}

يمكن تفعيل ZNE عن طريق ضبط `zne_mitigation` على `True` في [خيارات مرونة IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) للـ Estimator.
خيارات ZNE موصوفة [هنا](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options). الخيارات التالية جديرة بالملاحظة:
...
تُظهر خلية الكود التالية كيفية تفعيل ZNE وضبط هذه الخيارات لـ Estimator. لا يُشترَط تحديد أيٍّ من هذه الخيارات صراحةً.
...
### تفعيل ZNE باستخدام خيار `resilience_level` {#enable-zne-by-using-the-resilience-level-option}

يمكنك تفعيل ZNE (إلى جانب TREX وتلوية البوابات) عن طريق ضبط [مستوى المرونة](/guides/estimator-noise-management#resilience) على `2`، كما هو موضح في المثال التالي.
...
يمكن تفعيل PEC عن طريق ضبط `pec_mitigation` على `True` في [خيارات مرونة IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) للـ Estimator.
خيارات PEC موصوفة [هنا](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-pec-options). يمكن ضبط حد لعبء أخذ العينات باستخدام خيار `max_overhead`. لاحظ أن تحديد عبء أخذ العينات قد يتسبب في تجاوز دقة النتيجة للدقة المطلوبة. القيمة الافتراضية لـ `max_overhead` هي 100.

تُظهر خلية الكود التالية كيفية تفعيل PEC وضبط خيار `max_overhead` لـ Estimator.
...
- [ضبط إدارة الضوضاء مع Estimator](/guides/estimator-noise-management).

- [ضبط إدارة الضوضاء مع Sampler](/guides/sampler-noise-management).

- استكشف [خيارات](/guides/runtime-options-overview) أخرى لـ primitives الخاصة بـ IBM Quantum.

- حدد [وضع التنفيذ](/guides/execution-modes) الذي تريد تشغيل وظيفتك فيه.
