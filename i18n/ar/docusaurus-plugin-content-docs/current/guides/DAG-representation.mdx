...
في الرسم البياني الموجه (DAG)، توجد ثلاثة أنواع من عُقد الرسم البياني: عُقد إدخال qubit/clbit (خضراء)، وعُقد العمليات (زرقاء)، وعُقد الإخراج (حمراء). تشير كل حافة إلى تدفق البيانات (أو الاعتماد) بين عُقدتين. استخدم الدالة qiskit.tools.visualization.dag_drawer() لعرض DAG الخاص بهذه الدائرة. (ثبّت [مكتبة Graphviz](https://graphviz.org/download/) لتشغيل ذلك.)
...
- راجع الدليل حول إنشاء [مرحلة Transpiler مخصصة](/guides/custom-transpiler-pass)
- تعلّم كيفية [إنشاء Backends مخصصة والـ Transpile إليها](/guides/custom-backend)
