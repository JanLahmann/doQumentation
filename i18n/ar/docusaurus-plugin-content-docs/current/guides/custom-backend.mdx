...
يمكنك عرض رسم بياني للاتصال لهذا الكلاس الجديد باستخدام طريقة [`plot_gate_map()`](../api/qiskit/qiskit.visualization.plot_gate_map) من وحدة `qiskit.visualization`. تُعد هذه الطريقة، إلى جانب [`plot_coupling_map()`](../api/qiskit/qiskit.visualization.plot_coupling_map) و[`plot_circuit_layout()`](../api/qiskit/qiskit.visualization.plot_circuit_layout)، أدوات مفيدة لتصور ترتيب الـ Qubits في الـ Backend، بالإضافة إلى كيفية توزيع دارة عبر Qubits الـ Backend. ينشئ هذا المثال Backend يحتوي على ثلاث شرائح صغيرة سداسية كثيفة. يحدد مجموعة من الإحداثيات لترتيب الـ Qubits، بالإضافة إلى مجموعة من الألوان المخصصة لبوابات الـ Qubit المزدوج المختلفة.
