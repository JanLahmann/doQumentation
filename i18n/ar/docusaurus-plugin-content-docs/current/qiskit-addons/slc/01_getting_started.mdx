...
{/* doqumentation-source-hash: 343b4d11 */}
...
a. CPU — تقييد تأثير الخطأ لكل خطأ من أخطاء Pauli أحادي وثنائي qubit
  ...
  1. الانتشار للأمام (تقييد التأثير على العنصر القابل للرصد). انشر كل خطأ إلى نهاية الدائرة واحسب معامله مع العنصر القابل للرصد.

      - اقتطع حدود المؤثر خلال التطور للحفاظ على قابلية الحساب.

      - شد هذه الحدود بشكل أكبر عن طريق انتشار خلفي فضفاض للعنصر القابل للرصد بناءً على حدود السرعة الكمية.
...
| ![paulitwirling.png](/img/qiskit-addons/slc/exp_data/paulitwirl.png) | ![learnlayer.png](/img/qiskit-addons/slc/exp_data/learnlayer.png) | ![curvefit.png](/img/qiskit-addons/slc/exp_data/curvefit.png) |

### نظرة عامة على المعالجة اللاحقة {#post-processing-overview}
بعد التنفيذ على الأجهزة الكمية باستخدام إطار عمل Samplomatic و Executor، نحول قياسات السلاسل البتية إلى قيمة العنصر القابل للرصد المطلوبة. في حالة دائرة Ising المعكوسة لدينا، سنحصل بشكل مثالي على عنصر قابل للرصد مقاس يساوي 1، إذ يجب أن تعود جميع qubit مثالياً إلى نقطة بدايتها $\ket{0}$. عند حساب قيمة العنصر القابل للرصد بدالة `expectation_values` لدينا، سنطبق بعض تقنيات المعالجة اللاحقة التي تقلل تأثير الضوضاء. يشمل ذلك إزالة اللقطات المتأثرة بالضوضاء غير الماركوفية، وتخفيف أخطاء القراءة، فضلاً عن مراعاة تفاصيل تطبيق PEC لدينا. تناقش التفاصيل في الخطوة 4 أدناه.
...
لتسهيل العرض، نختار سلسلة Ising مرآتية أحادية البعد. تمنح سلسلة Ising أحادية البعد بنية دائرة كثيفة بشكل لطيف، مما يُعدّ مناسباً لعرض تطبيقات PEC. تجعل الدائرة المرآتية من السهل معرفة النتيجة المتوقعة (أي يجب أن نقيس عنصراً قابلاً للرصد يساوي 1).

علاوة على ذلك، نريد تشغيل دائرة مرآتية، لذا لكل بوابة في النصف الثاني من الدائرة يجب أن تكون هناك بوابة عكسية في النصف الأول. بما أن العنصر القابل للرصد المقاس **$<X_6 Z_{13}>$** يحتوي على قياسات غير Z-أساسية، والمنفِّذ يراعي الأساس المطلوب في نهاية الدائرة، نوفر دالة `prepare_basis` تدرج البوابات المناسبة في بداية الدائرة المرآتية. هذه التفصيلة خاصة بعرض الدائرة المرآتية لدينا. تتيح لنا دالة `get_measurement_bases` تحديد البوابات المطلوبة ومكان إلحاقها بسهولة، فضلاً عن متابعة الدقائق الخاصة بفهرسة qubit الناشئة عن الاصطلاحات في تعليق `box` كما نوقش في قسم "تحضير قياسات الأساسيات القانونية".
...
سنقوم بتحسين التفاصيل المتعلقة بالدائرة المراد تشغيلها، والعنصر القابل للرصد المراد قياسه، ومعاملات تعلم الضوضاء. كنقطة انطلاق، نتأكد من إنشاء Backend مع تفعيل البوابات الكسرية كخيار. ستتيح هذه البوابات الكسرية حساسية أكبر في بعض عمليات تصفية الانتقاء اللاحق.
...
### سير عمل التظليل بالمخروط الضوئي وتعلّم الضوضاء وحقن مضاد الضوضاء {#workflow-for-lightcone-shading-noise-learning-and-anti-noise-injection-1}
...
تُقيّم دالة `compute_forward_bounds` علاقات التبديل بين البوابات في كل طبقة وحدود باولي المولَّدة أعلاه، من حيث تأثير أخطاء الانتشار الأمامي على العنصر القابل للرصد المطلوب $A$. فالبوابات المتبادلة مع حدود باولي لا تستدعي أي إجراء، أما البوابات من نوع كليفورد فتُدفع نحو بداية الدائرة، وبالنسبة إلى البوابات غير الكليفوردية فيُقدَّر تأثيرها على العنصر القابل للرصد المستهدف لإعطائها الأولوية في إلغاء الضوضاء لاحقًا (بعد دمج جميع الحدود). ويُحقَّق هذا الحد بتطبيق معيار L2 أولاً (أي الجذر التربيعي لمجموع مربعات معاملات حدود باولي ذات الصلة). وحين يكثر عدد qubit المتضمنة، يُلجأ إلى حدٍّ أرخى يعتمد متراجحة المثلث.
...
- يمكن تمييز المخروطين الضوئيين الناشئين عن حدَّي باولي غير الهويّة في العنصر القابل للرصد بوضوح.
...
نضيّق الحدود بعد ذلك باستخدام دالة `tighten_with_speed_limit`، التي تتتبع كيفية انتشار العنصر القابل للرصد في الاتجاه الخلفي عبر الدائرة وتستخدم هذا الانتشار لوضع حدود عليا لتأثير كل مؤثر ضوضاء، مختارةً القيمة الأصغر بين الحد الأمامي المحسوب للتو والحد الناتج عن الانتشار الخلفي.
...
تحدد دالة `merged_bounds` النقطة في الدائرة التي يُقلّل عندها التبديل من الحدود الخلفية إلى الحدود الأمامية من إجمالي الانحياز المُقدَّر على العنصر القابل للرصد المطلوب. يُحسب هذا الانحياز كمجموع مساهمات الحدود الخلفية لجميع مواقع الضوضاء السابقة لتلك النقطة، ومساهمات الحدود الأمامية لجميع مواقع الضوضاء اللاحقة لها. يُطبَّق ذلك حاليًا بصورة موحدة على جميع qubit.
...
#### ب.ب. حساب `local_scales` لتنفيذ الأجهزة {#bii-compute-the-local-scales-for-the-hardware-execution}
...
### ج. تنفيذ الدائرة المطلوبة مع مضاد الضوضاء {#c-execute-the-circuit-of-interest-with-antinoise}
