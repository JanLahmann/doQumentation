---
title: "تعزيز تصنيف الميزات باستخدام نوى الكم المسقطة"
sidebar_label: "تعزيز تصنيف الميزات باستخدام نوى الكم المسقطة"
description: "درس تعليمي حول تعزيز تصنيف الميزات باستخدام نوى الكم المسقطة"
...
يفترض هذا الدرس التعليمي أيضاً إلماماً عاماً بأساليب QML. لمزيد من الاستكشاف في مجال QML، يمكن الرجوع إلى دورة [التعلم الآلي الكمومي](/learning/courses/quantum-machine-learning) في IBM Quantum Learning.
...
يمكننا الآن التكرار على مجموعة بيانات التدريب بأكملها للحصول على جميع مصفوفات الكثافة المختزلة أحادية الجسيم (1-RDMs).
كما نوفر النتائج من تجربة أجريناها على عتاد كمومي حقيقي. يمكنك إما تشغيل التدريب بنفسك عن طريق ضبط العلامة أدناه على `True`، أو استخدام نتائج الإسقاط التي نوفرها.
...
بمجرد اكتمال المهام، يمكننا استرجاع النتائج.
...
نكرر هذه العملية لمجموعة الاختبار.
...
يمكننا استرجاع النتائج كما فعلنا سابقاً.
...
### تعريف النواة الكمومية المسقطة
تُعرَّف النواة الكمومية المسقطة بدالة النواة التالية: $$k^{\textrm{PQ}}(x_i, x_j) = \textrm{exp} \Big(-\gamma \sum_k \sum_{P \in \{ X,Y,Z \}} (\textrm{Tr}[P \rho_k(x_i)] - \textrm{Tr}[P \rho_k(x_j)])^2 \Big) $$
في المعادلة أعلاه، $\gamma>0$ هو معامل ضبط قابل للتعديل (hyperparameter). وتمثل $K^{\textrm{PQ}}_{ij} = k^{\textrm{PQ}}(x_i, x_j)$ عناصر مصفوفة النواة $K^{\textrm{PQ}}$.
باستخدام تعريف مصفوفات الكثافة المختزلة أحادية الجسيم (1-RDMs)، يمكننا أن نرى أن الحدود الفردية داخل دالة النواة يمكن تقييمها على النحو التالي: $\textrm{Tr}[P \rho_k (x_i)] = \braket P$، حيث $P \in \{ X,Y,Z \}$. وهذه القيم المتوقعة هي بالضبط ما قسنا أعلاه.
باستخدام ``scikit-learn``، يمكننا في الواقع حساب النواة بطريقة أسهل. ويعود ذلك إلى دالة النواة الأساسية الشعاعية (``'rbf'``) المتاحة مباشرة: $ \textrm{exp} (-\gamma \lVert x - x' \rVert^2)$. أولاً، نحتاج فقط إلى إعادة تشكيل مجموعتَي بيانات التدريب والاختبار المسقطتين الجديدتين إلى مصفوفات ثنائية الأبعاد.
تجدر الإشارة إلى أن المرور على مجموعة البيانات بأكملها قد يستغرق نحو 80 دقيقة على وحدة المعالجة الكمومية (QPU). ولضمان سهولة تنفيذ بقية البرنامج التعليمي، نوفر أيضاً إسقاطات من تجربة أُجريت مسبقاً (مضمَّنة في الملفات التي نزّلتها في كتلة الكود `Download dataset`). إذا أجريت التدريب بنفسك، يمكنك متابعة البرنامج التعليمي بنتائجك الخاصة.
...
يمكننا الآن تشغيل SVM كلاسيكي على هذه النواة المحسوبة مسبقاً، واستخدام النواة بين مجموعتَي الاختبار والتدريب للتنبؤ.
...
### المعيار القياسي الكلاسيكي
يمكننا تشغيل SVM كلاسيكي باستخدام دالة الأساس الشعاعي (radial basis function) كنواة دون إجراء أي إسقاط كمومي. وتُمثّل هذه النتيجة المعيارَ القياسي الكلاسيكي لدينا.
...
## الملحق: التحقق من الميزة الكمومية المحتملة لمجموعة البيانات في مهام التعلم

لا تُتيح جميع مجموعات البيانات ميزةً محتملة من استخدام PQKs. ثمة حدود نظرية يمكن استخدامها كاختبار أولي لمعرفة ما إذا كانت مجموعة بيانات معينة قادرة على الاستفادة من PQKs. ولقياس ذلك، يُعرِّف مؤلفو [Power of data in quantum machine learning](https://www.nature.com/articles/s41467-021-22539-9) [2] كمياتٍ يُشار إليها بتعقيد النموذج الكلاسيكي والكمومي والفصل الهندسي بين النموذجين الكلاسيكي والكمومي. لتوقع ميزة كمومية محتملة من PQKs، ينبغي أن يكون الفصل الهندسي بين النواتَين الكلاسيكية والكمومية المُسقَطة في حدود $\sqrt{N}$ تقريباً، حيث $N$ هو عدد عينات التدريب. إذا تحقق هذا الشرط، ننتقل إلى فحص تعقيدات النماذج. إذا كان تعقيد النموذج الكلاسيكي في حدود $N$، في حين كان تعقيد النموذج الكمومي المُسقَط أصغر بكثير من $N$، فيمكن توقع ميزة محتملة من PQK.
