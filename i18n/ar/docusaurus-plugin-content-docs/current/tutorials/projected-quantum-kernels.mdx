---
description: 'درس تعليمي حول تحسين تصنيف الميزات باستخدام نوى الكم المسقطة'
notebook_path: tutorials/projected-quantum-kernels.ipynb
sidebar_label: 'تحسين تصنيف الميزات باستخدام نوى الكم المسقطة'
title: 'تحسين تصنيف الميزات باستخدام نوى الكم المسقطة'
---
...
بعد اجتياز هذا الدرس التعليمي، ينبغي أن يفهم المستخدمون:
- كيف تعمل نوى الكم المسقطة (PQK) ومتى تقدم ميزةً كمومية محتملة.
- كيفية تشغيل PQK على عتاد حقيقي باستخدام مجموعة بيانات من العالم الحقيقي.

## المتطلبات الأساسية {#prerequisites}

نقترح أن يكون المستخدمون على دراية بالمواضيع التالية قبل الشروع في هذا الدرس التعليمي:
- [نوى الكم](/learning/courses/quantum-machine-learning/quantum-kernel-methods) من [دورة التعلم الآلي الكمومي](/learning/courses/quantum-machine-learning) في IBM Quantum&reg; Learning

## الخلفية {#background}

في هذا الدرس التعليمي، نستعرض كيفية تشغيل [نواة الكم المسقطة](https://www.nature.com/articles/s41467-021-22539-9) (PQK) مع Qiskit على مجموعة بيانات بيولوجية من العالم الحقيقي، استناداً إلى الورقة البحثية [Enhanced Prediction of CAR T-Cell Cytotoxicity with Quantum-Kernel Methods](https://arxiv.org/abs/2507.22710) [[1]](#references).
...
اللبنات الأساسية لنوى الكم المسقطة هي مصفوفات الكثافة المختزلة (RDMs)، التي تُحصَل عليها من خلال القياسات الإسقاطية لخريطة الميزات الكمومية. ويُحسَب بشكل عام مصفوفات الكثافة المختزلة لكيوبت واحد (1 RDMs) لكل كيوبت. تُستخدم هذه الكميات المقاسة بعد ذلك كمدخلات لدالة نواة كلاسيكية، كالنواة الأسية، لبناء مصفوفة النواة النهائية.

تتمتع نوى الكم المسقطة بمزايا محتملة على [نوى الكم](/tutorials/quantum-kernel-training) القياسية، ولا سيما للعتاد الكمومي قريب المدى. تعتمد نوى الكم القياسية عادةً على تقدير تداخلات الحالة الكلية، التي يصعب قياسها بدقة مع زيادة عدد الكيوبتات وتتأثر بشدة بالضوضاء. في المقابل، تستخدم نوى الكم المسقطة مُراقَبات محلية كمصفوفات الكثافة المختزلة أحادية الكيوبت (1 RDMs)، مما يؤدي إلى تقليل عبء أخذ العينات، وتحسين المتانة في مواجهة ضوضاء العتاد، وتوسيع النطاق بصورة أفضل. من خلال إسقاط الحالات الكمومية على ميزات القياس المحلية قبل تطبيق دالة النواة الكلاسيكية، يمكن لنوى الكم المسقطة الاحتفاظ بالارتباطات الكمومية المفيدة مع بقائها أكثر عملية للأجهزة قريبة المدى.
...
## مثال المحاكي الصغير النطاق {#small-scale-simulator-example}
نتجاوز مثال المحاكي الصغير النطاق في هذا الدرس التعليمي، إذ يتمثّل هدفنا الرئيسي في توضيح كيفية قدرة نوى الكم المسقطة على التوسع نحو أنظمة أكبر وعتاد حقيقي.

## مثال العتاد الكبير النطاق {#large-scale-hardware-example}

### الخطوة 1: تعيين المدخلات الكلاسيكية إلى مسألة كمومية {#step-1-map-classical-inputs-to-a-quantum-problem}

### تحضير مجموعة البيانات {#dataset-preparation}

في هذا الدرس التعليمي نستخدم مجموعة بيانات بيولوجية من العالم الحقيقي لمهمة تصنيف ثنائي، تم توليدها بواسطة Daniels et al. (2022) ويمكن تنزيلها من [المواد التكميلية](https://www.science.org/doi/full/10.1126/science.abq0225#supplementary-materials) المرفقة بالورقة البحثية. تتألف البيانات من خلايا CAR T، وهي خلايا T مهندسة وراثياً تُستخدم في العلاج المناعي لعلاج أنواع معينة من السرطان. يتم تعديل خلايا T، وهي نوع من خلايا المناعة، في المختبر لتعبّر عن مستقبلات المستضد الخيمري (CARs) التي تستهدف بروتينات محددة على الخلايا السرطانية. يمكن لخلايا T المعدَّلة هذه التعرف على الخلايا السرطانية وتدميرها بفاعلية أكبر. ميزات البيانات هي بنيات خلايا CAR T، التي تشير إلى المكوّن البنيوي أو الوظيفي المحدد للـ CAR المهندَس في خلايا T. بناءً على هذه البنيات، تتمثل مهمتنا في التنبؤ بسُمِّية خلية CAR T المعطاة، وتصنيفها إما كسامة أو غير سامة.
يوضح ما يلي الدوال المساعدة لمعالجة هذه المجموعة من البيانات.
...
نُحوِّل أيضاً مجموعة البيانات بحيث يُمثَّل $1$ على شكل $\pi/2$ لأغراض التحجيم.
...
### الخطوة 2: تحسين المسألة لتنفيذها على عتاد الكم {#step-2-optimize-problem-for-quantum-hardware-execution}

### الدائرة الكمومية {#quantum-circuit}

نبني الآن خريطة الميزات التي تُضمِّن مجموعة بياناتنا الكلاسيكية في فضاء ميزات أعلى الأبعاد. لهذا التضمين، نستخدم [`ZZFeatureMap`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ZZFeatureMap) من Qiskit.
...
### قياس مصفوفات الكثافة المختزلة أحادية الكيوبت (1-RDMs) {#measure-1-rdms}
في هذه الخطوة، نحصل على جميع مصفوفات الكثافة المختزلة أحادية الكيوبت (1-RDMs) من خلال القياسات الإسقاطية لخريطة الميزات الكمومية، والتي ستُقدَّم لاحقاً إلى دالة النواة الأسية الكلاسيكية.
لنتأمل كيفية حساب 1-RDM لنقطة بيانات واحدة من مجموعة البيانات قبل التشغيل على جميع البيانات. إن 1-RDMs هي مجموعة من قياسات الكيوبت الواحد لعوامل باولي ``X`` و``Y`` و``Z`` على جميع الكيوبتات. وذلك لأن مصفوفة الكثافة المختزلة لكيوبت واحد يمكن التعبير عنها كاملةً على النحو التالي: $$\rho = \frac{1}{2} \big( I + \braket \sigma_x \sigma_x  + \braket \sigma_y \sigma_y + \braket \sigma_z \sigma_z  \big)$$
...
يمكننا استرجاع النتائج كما كان من قبل.
...
### تعريف نواة الكم المسقطة {#define-the-projected-quantum-kernel}
تُعرَّف نواة الكم المسقطة بدالة النواة التالية: $$k^{\textrm{PQ}}(x_i, x_j) = \textrm{exp} \Big(-\gamma \sum_k \sum_{P \in \{ X,Y,Z \}} (\textrm{Tr}[P \rho_k(x_i)] - \textrm{Tr}[P \rho_k(x_j)])^2 \Big) $$
في المعادلة أعلاه، $\gamma>0$ هو معامل فرط المعلمات القابل للضبط. وتُشكِّل $K^{\textrm{PQ}}_{ij} = k^{\textrm{PQ}}(x_i, x_j)$ عناصر مصفوفة النواة $K^{\textrm{PQ}}$.
باستخدام تعريف 1-RDMs، يمكننا رؤية أن الحدود الفردية داخل دالة النواة يمكن تقييمها كـ $\textrm{Tr}[P \rho_k (x_i)] = \braket P$، حيث $P \in \{ X,Y,Z \}$. هذه القيم المتوقعة هي تماماً ما قسناه أعلاه.
باستخدام ``scikit-learn``، يمكننا في الواقع حساب النواة بسهولة أكبر. يعود ذلك إلى توفر دالة القاعدة الشعاعية (``'rbf'``) جاهزة: $ \textrm{exp} (-\gamma \lVert x - x' \rVert^2)$. أولاً، نحتاج فقط إلى إعادة تشكيل مجموعتَي بيانات التدريب والاختبار المُسقَطتَين الجديدتَين إلى مصفوفات ثنائية الأبعاد.
...
### آلة المتجهات الداعمة (SVM) {#support-vector-machine-svm}
...
### المقارنة المرجعية الكلاسيكية {#classical-benchmarking}
...
## الملحق: التحقق من إمكانية تحقيق ميزة كمية للمجموعة في مهام التعلم {#appendix-verify-the-datasets-potential-quantum-advantage-in-learning-tasks}

لا توفر جميع المجموعات ميزة محتملة من استخدام PQKs. ثمة بعض الحدود النظرية التي يمكن استخدامها كاختبار أولي لمعرفة ما إذا كانت مجموعة بيانات معينة يمكن الاستفادة من PQKs. لقياس ذلك، يُعرِّف مؤلفو [قوة البيانات في التعلم الآلي الكمي](https://www.nature.com/articles/s41467-021-22539-9) [2] كميات يُشار إليها بتعقيدات النموذج الكلاسيكي والكمي والفصل الهندسي للنموذجين الكلاسيكي والكمي. لتوقع ميزة كمية محتملة من PQKs، ينبغي أن يكون الفصل الهندسي بين النواتَين الكلاسيكية والكمية المسقطة تقريباً في حدود $\sqrt{N}$، حيث $N$ هو عدد عينات التدريب. إذا تحقق هذا الشرط، ننتقل إلى التحقق من تعقيدات النموذج. إذا كان تعقيد النموذج الكلاسيكي في حدود $N$ بينما تعقيد النموذج الكمي المسقط أصغر بكثير من $N$، يمكننا توقع ميزة محتملة من PQK.
يُعرَّف الفصل الهندسي على النحو التالي (F19 في [[2]](#references)):
...
يُعرَّف تعقيد النموذج على النحو التالي (M1 في [[2]](#references)):
...
- دورة [التعلم الآلي الكمومي](/learning/courses/quantum-machine-learning) المتعمقة من IBM Quantum Learning
- درس تعليمي حول [تدريب نواة الكم](/tutorials/quantum-kernel-training)
</Admonition>
