...
عرّف بيانات اعتماد [IBM Quantum Platform](https://quantum.cloud.ibm.com/) الخاصة بك، والتي ستُستخدم طوال هذا البرنامج التعليمي للمصادقة على IBM Quantum Compute Service وQiskit Functions.
...
يُعيد Solver قاموساً يحتوي على الحل والبيانات الوصفية المرتبطة به، مثل سلسلة البت الخاصة بالحل، وعدد التكرارات، وتعيين المتغيرات إلى سلسلة البت. للاطلاع على تعريف كامل لمدخلات Solver ومخرجاته، راجع [التوثيق](https://docs.quantum.ibm.com/api/functions/q-ctrl-optimization-solver).
...
بما أن هدف خوارزمية التحسين هذه هو إيجاد الحالة الأساسية الدنيا لنموذج Ising، فإن القيم الأقل تشير إلى حلول أفضل. لذلك، يتضح بصرياً أن الحلول التي يولّدها Fire Opal Optimization Solver تتفوق بشكل كبير على الاختيار العشوائي.
