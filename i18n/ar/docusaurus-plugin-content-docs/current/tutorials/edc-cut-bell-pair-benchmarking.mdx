...
بعد اجتياز هذا الدرس، ينبغي للمستخدمين أن يفهموا:
* كيفية بناء دوائر كمومية ديناميكية مع قياسات أثناء الدارة وتغذية أمامية كلاسيكية لنقل التشابك بين الكيوبتات البعيدة؛
* كيفية حساب وتفسير مقياس الخطأ لتحديد دقة زوج بيل عبر الجهاز؛
* كيفية تحديد أزواج الكيوبتات الأنسب للعمليات المعتمدة على LOCC باستخدام نتائج القياس.

## المتطلبات الأساسية {#prerequisites}
نقترح أن يكون المستخدمون على دراية بالمواضيع التالية قبل الشروع في هذا الدرس:
* [مفاهيم الحوسبة الكمومية الأساسية](/learning/courses/basics-of-quantum-information)، بما فيها حالات بيل والتشابك والبوابات الكمومية؛
* الإلمام بـ[الدوائر الكمومية الديناميكية](/guides/classical-feedforward-and-control-flow) ([القياسات أثناء الدارة](/guides/execute-dynamic-circuits#mid-circuit-measurements) والتغذية الأمامية الكلاسيكية)؛
* المعرفة الأساسية بـ[Qiskit SDK](https://docs.quantum.ibm.com/guides) و[IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime)، والوصول إلى [حساب IBM Quantum](/guides/cloud-setup).
...
قبل التشغيل على وحدة المعالجة الكمومية الفعلية، نتحقق من أن الدارة تُنتج زوج بيل باختبارها على محاكٍ خالٍ من الضجيج مع سلسلة رباعية الكيوبتات `[0, 1, 2, 3]`. نستخدم `Sampler` الخاص بـ IBM Quantum مع `AerSimulator` كوضع backend لتنفيذ الدوائر.

### الخطوة 1: تعيين المدخلات الكلاسيكية إلى مسألة كمومية {#step-1-map-classical-inputs-to-a-quantum-problem}

الخطوة الأولى هي إنشاء مجموعة من الدوائر الكمومية لقياس أداء جميع روابط أزواج بيل المرشحة بما يتوافق مع طوبولوجيا الجهاز. نبدأ ببناء هذه الدوائر مع سلسلة رباعية الكيوبتات `[0, 1, 2, 3]`.

تُنفّذ الروتين `create_bell_stab()` ما يلي لكل سلسلة:
...
لكل تخطيط ابتدائي رباعي، تُعيد الدالة أعلاه دارتين (واحدة لقياس $ZZ$، وأخرى لقياس $XX$). تتضمن هذه الدوائر قياسات أثناء الدارة وعمليات شرطية (if/else)، وهي التعليمات الأساسية للدوائر الكمومية الديناميكية.
...
قبل تنفيذ دوائرنا، نحتاج إلى نقلها (transpile) إلى عمليات بوابات مدعومة على الـbackend المحدد. ستُعيّن عملية النقل الدارة المجردة على الكيوبتات المادية ومجموعة البوابات الخاصة بالـbackend المختار. بما أننا اخترنا بالفعل كيوبتات مادية محددة لكل سلسلة (بتحديد `initial_layout` في مولّد الدوائر)، نستخدم `optimization_level=0` في عملية النقل مع ذلك التخطيط الثابت. يُخبر هذا Qiskit بعدم إعادة تخصيص الكيوبتات أو تطبيق أي تحسينات مكثفة قد تُغيّر بنية الدارة. نريد الحفاظ على تسلسل العمليات (لا سيما البوابات الشرطية) كما هو محدد بالضبط.
...
يمكننا الآن تشغيل التجربة على المحاكي الخالي من الضجيج.
...
تساوي هذه القيمة 0 لزوج بيل مثالي، وترتفع كلما ازداد ضجيج الحالة المتشابكة (إذ تُعطي النتائج العشوائية توقعًا قريبًا من 0، ليقترب MSE من 1). يحسب الكود MSE لكل مجموعة رباعية من الكيوبتات. مع انعدام الضجيج، نرى MSE = 0 في مثال المحاكي الصغير النطاق هذا، كما هو متوقع.

## مثال على أجهزة حقيقية كبيرة النطاق {#large-scale-hardware-example}

هنا نجمع كل هذه التفاصيل معًا في سير عمل موحد على نطاق أوسع، يُشغَّل بعد ذلك على أجهزة كمومية حقيقية.
...
بعد ذلك، نبني الدارة لكل شريحة من 16 كيوبت باستخدام دالة `create_bell_stab()`. في نهاية هذه الخطوة، تكون لدينا قائمة من الدوائر تغطي كل سلسلة رباعية في الجهاز. ثم ننقل الدوائر ونُشغّلها على الـbackend الفعلي ونُعالج النتائج.
...
- تعلّم كيفية تنفيذ [التشابك بعيد المدى مع الدوائر الكمومية الديناميكية](/tutorials/long-range-entanglement)
- تعلّم كيفية محاكاة [هاملتوني Kicked Ising مع الدوائر الكمومية الديناميكية](/tutorials/dc-hex-ising)
</Admonition>
