...
* المعرفة الأساسية بـ[Qiskit SDK](https://docs.quantum.ibm.com/guides) و[Qiskit Runtime](/guides/compute-services#qiskit-runtime)، والوصول إلى [حساب IBM Quantum&reg;](/guides/cloud-setup).
...
قبل التشغيل على وحدة المعالجة الكمومية الفعلية، نتحقق من أن الدارة تُنتج زوج بيل باختبارها على محاكٍ خالٍ من الضجيج مع سلسلة رباعية الكيوبتات `[0, 1, 2, 3]`. نستخدم `Sampler` الخاص بـ Qiskit Runtime مع `AerSimulator` كوضع backend لتنفيذ الدوائر.
