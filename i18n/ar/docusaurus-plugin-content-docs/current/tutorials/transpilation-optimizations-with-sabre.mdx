...
## الخلفية {#background}
يُعدّ التحويل البرمجي (Transpilation) خطوة محورية في Qiskit تُحوِّل الدوائر الكمية إلى أشكال متوافقة مع عتاد كمي محدد. وينطوي على مرحلتين رئيسيتين: **تخطيط الكيوبتات** (qubit layout) — وهو تعيين الكيوبتات المنطقية إلى الكيوبتات الفيزيائية على الجهاز — و**توجيه البوابات** (gate routing) — وهو ضمان احترام البوابات متعددة الكيوبتات لاتصالية الجهاز عبر إدراج بوابات SWAP حسب الحاجة.

SABRE (*خوارزمية البحث الإرشادي ثنائي الاتجاه القائمة على SWAP*) هو أداة تحسين قوية لكلٍّ من التخطيط والتوجيه. وهو فعّال بشكل خاص في **الدوائر واسعة النطاق** (أكثر من 100 كيوبت) والأجهزة ذات خرائط الاقتران المعقدة، كجهاز **IBM&reg; Heron**، حيث يستلزم النمو الأسي في احتمالات تعيين الكيوبتات حلولًا كفؤة.
...
ستتمكن من:
- تحسين SABRE للدوائر التي تحتوي على أكثر من 100 كيوبت، متجاوزًا إعدادات التحويل البرمجي الافتراضية مثل `optimization_level=3`.
...
يمكن استخدام SABRE في Qiskit لتحسين الدوائر الكمية عبر التعامل مع مرحلتي تخطيط الكيوبتات وتوجيه البوابات. في هذا القسم، سنرشدك خلال **المثال الأدنى** لاستخدام SABRE مع أنماط Qiskit، مع التركيز الأساسي على الخطوة الثانية من التحسين.

لتشغيل SABRE، تحتاج إلى:
- تمثيل **DAG** (الرسم البياني الموجَّه اللادوري) لدائرتك الكمية.
- **خريطة الاقتران** من الواجهة الخلفية، التي تحدد كيفية الاتصال الفيزيائي بين الكيوبتات.
...
دائرة **GHZ (Greenberger-Horne-Zeilinger)** هي دائرة كمية تُعدّ حالة متشابكة تكون فيها جميع الكيوبتات إما في الحالة `|0...0⟩` أو الحالة `|1...1⟩`. تُمثَّل حالة GHZ لـ $n$ كيوبت رياضيًا على النحو التالي:
$$ |\text{GHZ}\rangle = \frac{1}{\sqrt{2}} \left( |0\rangle^{\otimes n} + |1\rangle^{\otimes n} \right) $$

وتُبنى باتباع الخطوات الآتية:
1. تطبيق بوابة Hadamard على الكيوبت الأول لإنشاء التراكب.
2. تطبيق سلسلة من بوابات CNOT لتشابك الكيوبتات المتبقية مع الأول.

في هذا المثال، نبني عمدًا **دائرة GHZ ذات الطوبولوجيا النجمية** بدلًا من الطوبولوجيا الخطية. في الطوبولوجيا النجمية، يعمل الكيوبت الأول بوصفه "المحور"، وتتشابك كل الكيوبتات الأخرى معه مباشرةً عبر بوابات CNOT. هذا الاختيار مقصود لأنه بينما يمكن نظريًا تنفيذ **حالة GHZ الخطية** بعمق $ O(N) $ على خريطة اقتران خطية دون أي بوابات SWAP، فإن SABRE سيجد تافهًا حلًّا مثاليًا بتعيين دائرة GHZ من 100 كيوبت إلى رسم بياني جزئي من خريطة اقتران heavy-hex للواجهة الخلفية.
...
لأغراض هذا البرنامج التعليمي، نستثني كلًّا من HighLevelSynthesis وStarPrerouting لعزل التأثير المباشر لضبط SABRE على وقت التشغيل وعمق الدائرة وإبرازه. بقياس قيمة التوقع $ \langle Z_0 Z_i \rangle $ لكل زوج من الكيوبتات، نُحلّل:
...
بعد ذلك، سنُعيّن العوامل ذات الاهتمام لتقييم سلوك النظام. تحديدًا، سنستخدم عوامل `ZZ` بين الكيوبتات لفحص كيفية تدهور التشابك كلما ابتعدت الكيوبتات عن بعضها. هذا التحليل بالغ الأهمية لأن الأخطاء في قيم التوقع $\langle Z_0 Z_i \rangle$ للكيوبتات البعيدة يمكن أن تكشف عن أثر الضوضاء والأخطاء في تنفيذ الدائرة. بدراسة هذه الانحرافات، نكتسب رؤية حول مدى قدرة الدائرة على الحفاظ على التشابك في ظل ضبطيات SABRE المختلفة، ومدى فاعلية SABRE في تقليص أثر قيود العتاد.
...
في هذه الخطوة، نركّز على تحسين تخطيط الدائرة للتنفيذ على جهاز عتاد كمي محدد يحتوي على 127 كيوبتًا. وهذا هو المحور الرئيسي للبرنامج التعليمي، إذ نُجري **تحسينات SABRE والتحويل البرمجي** لتحقيق أفضل أداء للدائرة. باستخدام تمريرة `SabreLayout`، نُحدّد تعيينًا أوليًا للكيوبتات يُقلّص الحاجة إلى بوابات SWAP أثناء التوجيه. وبتمرير `coupling_map` للواجهة الخلفية المستهدفة، تتكيّف `SabreLayout` مع قيود الاتصالية في الجهاز.
...
بعد ضبط كل مدير تمريرات، سنُنفّذ الآن عملية التحويل البرمجي لكل منها. للمقارنة بين النتائج، سنتتبّع المقاييس الرئيسية، بما فيها وقت التحويل البرمجي، وعمق الدائرة (مقاسًا بعمق البوابة ثنائية الكيوبت)، وإجمالي عدد البوابات في الدوائر المحوَّلة برمجيًا.
...
في هذه الخطوة، نستخدم الأولية `Estimator` لحساب قيم التوقع $\langle Z_0 Z_i \rangle$ لعوامل `ZZ`، لتقييم التشابك وجودة التنفيذ للدوائر المحوَّلة برمجيًا. لتتوافق مع سير عمل المستخدمين النموذجيين، نُقدّم الوظيفة للتنفيذ ونُطبّق قمع الأخطاء باستخدام **الإزالة الديناميكية للاقتران** (dynamical decoupling)، وهي تقنية تُخفّف إزالة الترابط بإدراج تسلسلات بوابات للحفاظ على حالات الكيوبتات. علاوةً على ذلك، نُحدّد مستوى المرونة لمواجهة الضوضاء، إذ تُوفّر المستويات الأعلى نتائج أدق على حساب زيادة وقت المعالجة. يُقيّم هذا النهج أداء كل ضبطية من ضبطيات مدير التمريرات في ظروف تنفيذ واقعية.
...
بعد اكتمال الوظيفة، نُحلّل النتائج بتصوير قيم التوقع $\langle Z_0 Z_i \rangle$ لكل كيوبت بيانيًا. في المحاكاة المثالية، يجب أن تكون جميع قيم $\langle Z_0 Z_i \rangle$ مساوية للواحد، مما يعكس تشابكًا مثاليًا عبر الكيوبتات. غير أنه بسبب الضوضاء وقيود العتاد، عادةً ما تتناقص قيم التوقع مع زيادة `i`، مما يكشف كيف يتدهور التشابك مع المسافة.
