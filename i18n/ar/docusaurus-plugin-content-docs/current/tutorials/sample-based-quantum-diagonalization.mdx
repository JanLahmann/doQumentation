---
title: "التقطير الكمي القائم على العينات لهاميلتوني كيميائي"
sidebar_label: "التقطير الكمي القائم على العينات لهاميلتوني كيميائي"
description: "استخدام خوارزمية التقطير الكمي القائم على العينات لمحاكاة جزيئة النيتروجين باستخدام أجهزة كمية ذات ضوضاء."
...
في هذا البرنامج التعليمي، نوضح كيفية المعالجة اللاحقة للعينات الكمية الضوضائية للحصول على تقريب لحالة الأساس لجزيئة النيتروجين $\text{N}_2$ عند طول الرابطة التوازنية، باستخدام [خوارزمية التقطير الكمي القائم على العينات (SQD)](https://arxiv.org/abs/2405.05068)، لعينات مأخوذة من دائرة كمية مكونة من 59 كيوبت (52 كيوبت نظام + 7 كيوبت مساعد). تتوفر تطبيقات مبنية على Qiskit في [إضافات SQD لـ Qiskit](https://github.com/Qiskit/qiskit-addon-sqd)، ويمكن الاطلاع على مزيد من التفاصيل في [التوثيق المقابل](/guides/qiskit-addons-sqd) مع [مثال بسيط](/guides/qiskit-addons-sqd-get-started) للبدء.
...
1. اختيار نموذج الدائرة (circuit ansatz) وتطبيقه على حاسوب كمي انطلاقًا من حالة مرجعية (في هذه الحالة، حالة [هارتري-فوك](https://en.wikipedia.org/wiki/Hartree%E2%80%93Fock_method)).
2. أخذ عينات من السلاسل الثنائية (bitstrings) من الحالة الكمية الناتجة.
3. تشغيل إجراء *استرداد التهيئات بصورة ذاتية الاتساق* على السلاسل الثنائية للحصول على تقريب لحالة الأساس.

من المعروف أن SQD يعمل بشكل جيد عندما تكون الحالة الذاتية المستهدفة متفرقة: أي أن دالة الموجة تُدعم في مجموعة من الحالات الأساسية $\mathcal{S} = \{|x\rangle \}$ التي لا يزداد حجمها أسيًا مع حجم المسألة.

### الكيمياء الكمية {#quantum-chemistry}

تتحدد خصائص الجزيئات إلى حد بعيد بالبنية الإلكترونية داخلها. باعتبارها جسيمات فيرميونية، يمكن وصف الإلكترونات باستخدام صياغة رياضية تُعرف بالتكميم الثاني. تقوم هذه الفكرة على وجود عدد من *المدارات*، يمكن لكل منها أن يكون إما خاليًا أو محتلًا بفيرميون. يُوصف نظام من $N$ مدارًا بمجموعة من مؤثرات الإفناء الفيرميونية $\{\hat{a}_p\}_{p=1}^N$ التي تحقق علاقات التبادل الفيرميوني المضادة:
...
يُسمى المرافق $\hat{a}_p^\dagger$ مؤثر الإنشاء.

حتى الآن، لم يأخذ شرحنا بعين الاعتبار السبين، وهو خاصية أساسية للفيرميونات. عند مراعاة السبين، تأتي المدارات في أزواج تُسمى *المدارات المكانية*. يتكوّن كل مدار مكاني من *مداري سبين* اثنين، أحدهما يُصنَّف "سبين-$\alpha$" والآخر يُصنَّف "سبين-$\beta$". نكتب حينئذٍ $\hat{a}_{p\sigma}$ لمؤثر الإفناء المرتبط بمدار السبين ذي السبين $\sigma$ ($\sigma \in \{\alpha, \beta\}$) في المدار المكاني $p$. إذا أخذنا $N$ ليمثل عدد المدارات المكانية، فإن المجموع الكلي لمدارات السبين هو $2N$. يمتد فضاء هيلبرت لهذا النظام على $2^{2N}$ متجه أساسي متعامد تامٍّ يُصنَّف بسلاسل ثنائية مزدوجة من الشكل $\lvert z \rangle = \lvert z_\beta z_\alpha \rangle = \lvert z_{\beta, N} \cdots z_{\beta, 1} z_{\alpha, N} \cdots z_{\alpha, 1} \rangle$.
...
حيث $h_{pr}$ و$h_{prqs}$ أعداد مركبة تُعرف بالتكاملات الجزيئية، ويمكن حسابها من مواصفات الجزيئة باستخدام برنامج حاسوبي. في هذا البرنامج التعليمي، نحسب هذه التكاملات باستخدام حزمة البرامج [PySCF](https://pyscf.org/).

للاطلاع على تفاصيل اشتقاق الهاميلتوني الجزيئي، يُرجع إلى كتاب مرجعي في الكيمياء الكمية (مثلًا، كتاب *Modern Quantum Chemistry* لـ Szabo و Ostlund). للحصول على شرح رفيع المستوى حول كيفية تحويل مسائل الكيمياء الكمية إلى كيوبتات، يمكن الرجوع إلى المحاضرة [*Mapping Problems to Qubits*](https://youtube.com/watch?v=TyFU6r8uEsE&t=900) من Qiskit Global Summer School 2024.

### نموذج التجمع الأحادي الموضعي (LUCJ) {#local-unitary-cluster-jastrow-lucj-ansatz}

يستلزم SQD وجود نموذج دائرة كمية لأخذ العينات منه. في هذا البرنامج التعليمي، سنستخدم نموذج [التجمع الأحادي الموضعي (LUCJ)](https://pubs.rsc.org/en/content/articlelanding/2023/sc/d3sc02516k) نظرًا لجمعه بين الدوافع الفيزيائية وملاءمته للأجهزة الكمية.
...
حيث $\lvert \Phi_0 \rangle$ هي الحالة المرجعية، وغالبًا ما تكون حالة هارتري-فوك، و$\hat{K}_\mu$ و$\hat{J}_\mu$ يأخذان الشكل:
...
المؤثر $e^{\hat{K}_\mu}$ هو دوران مداري يمكن تطبيقه باستخدام خوارزميات معروفة بعمق خطي وبترابط خطي.
يستلزم تطبيق حد $e^{i \mathcal{J}_k}$ من نموذج UCJ إما ترابطًا كليًا بين الكيوبتات أو استخدام شبكة مبادلة فيرميونية، مما يجعله تحديًا لمعالجات كمية قبل تحقيق الخطأ المتسامح به وذات الترابط المحدود. الفكرة الجوهرية لنموذج *LUCJ* هي فرض قيود تفرق على مصفوفتي $\mathbf{J}^{\alpha\alpha}$ و$\mathbf{J}^{\alpha\beta}$، مما يسمح بتطبيقهما بعمق ثابت على هياكل كيوبت ذات ترابط محدود. تُحدد هذه القيود بقائمة من الفهارس التي تشير إلى مدخلات المصفوفة في المثلث العلوي المسموح لها بأن تكون غير صفرية (نظرًا لتماثل المصفوفات، يكفي تحديد المثلث العلوي فقط). يمكن تفسير هذه الفهارس على أنها أزواج من المدارات المسموح لها بالتفاعل.

على سبيل المثال، لنأخذ هيكل كيوبت على شبكة مربعة. يمكننا وضع المدارات $\alpha$ و$\beta$ في خطوط متوازية على الشبكة، مع وصلات بين هذين الخطين تشكّل "عوارض" تشبه شكل السلّم، كما يلي:

![مخطط ربط الكيوبتات لنموذج LUCJ على شبكة مربعة](/docs/images/tutorials/improving-energy-estimation-of-a-fermionic-hamiltonian-with-sqd/baad4e53-5bfd-4cb4-8027-2ec26d27ecdd.avif)

بهذا الإعداد، ترتبط المدارات ذات السبين المتماثل بهيكل خطي، بينما ترتبط المدارات ذات السبين المختلف عندما تشترك في المدار المكاني ذاته. ينتج عن ذلك قيود الفهارس التالية على مصفوفتي $\mathbf{J}$:
...
بعبارة أخرى، إذا كانت مصفوفتا $\mathbf{J}$ غير صفريتين فقط عند الفهارس المحددة في المثلث العلوي، فيمكن حينئذٍ تطبيق حد $e^{i \mathcal{J}_k}$ على هيكل مربع دون الحاجة إلى أي بوابات مبادلة، وبعمق ثابت. بطبيعة الحال، فرض مثل هذه القيود على النموذج يجعله أقل تعبيرية، وبالتالي قد يستلزم مزيدًا من تكرارات النموذج.

يمتلك الجهاز IBM&reg; هيكل كيوبت على شبكة "السداسي الثقيل" (heavy-hex)، ويمكن في هذه الحالة اعتماد نمط "متعرج" (zig-zag) كما هو موضح أدناه. في هذا النمط، تُربط المدارات ذات السبين المتماثل بكيوبتات ذات هيكل خطي (الدوائر الحمراء والزرقاء)، وتتوفر وصلة بين مدارات السبين المختلف عند كل رابع مدار مكاني، حيث تُتيح هذه الوصلة كيوبت مساعد (الدوائر البنفسجية). في هذه الحالة، قيود الفهارس هي:
...
![مخطط ربط الكيوبتات لنموذج LUCJ على شبكة السداسي الثقيل](/docs/images/tutorials/improving-energy-estimation-of-a-fermionic-hamiltonian-with-sqd/7e0ee7e1-2d24-417f-ac59-25c58db79aa9.avif)

### استرداد التهيئات بصورة ذاتية الاتساق {#self-consistent-configuration-recovery}

صُمِّم إجراء استرداد التهيئات بصورة ذاتية الاتساق لاستخلاص أكبر قدر ممكن من الإشارة من العينات الكمية الضوضائية. نظرًا لأن الهاميلتوني الجزيئي يحفظ عدد الجسيمات والسبين Z، فمن المنطقي اختيار نموذج دائرة يحفظ هذين التماثلين أيضًا. عند تطبيق النموذج على حالة هارتري-فوك، تكون الحالة الناتجة ذات عدد جسيمات وسبين Z ثابتين في الحالة الخالية من الضوضاء. لذلك، يجب أن يكون لنصفَي السبين-$\alpha$ والسبين-$\beta$ في أي سلسلة ثنائية مأخوذة من هذه الحالة نفس [الوزن الهاميلي](https://en.wikipedia.org/wiki/Hamming_weight) كما في حالة هارتري-فوك. بسبب وجود الضوضاء في المعالجات الكمية الحالية، ستنتهك بعض السلاسل الثنائية المقيسة هذه الخاصية. أبسط صورة للانتقاء اللاحق هي استبعاد هذه السلاسل الثنائية، غير أن ذلك مُضيِّع لأن تلك السلاسل قد تحتوي بعض الإشارة. يسعى إجراء الاسترداد ذاتي الاتساق إلى استعادة بعض تلك الإشارة في مرحلة ما بعد المعالجة. الإجراء تكراري ويتطلب كمدخلات تقديرًا لمتوسط الإشغال لكل مدار في حالة الأساس، والذي يُحسب أولًا من العينات الخام. يُشغَّل الإجراء في حلقة تكرارية، وتتضمن كل دورة الخطوات التالية:

1. لكل سلسلة ثنائية تنتهك التماثلات المحددة، يتم قلب بتاتها باستخدام إجراء احتمالي مصمم لتقريب السلسلة الثنائية من التقدير الحالي لمتوسط إشغال المدارات، للحصول على سلسلة ثنائية جديدة.
2. تجميع كل السلاسل الثنائية القديمة والجديدة التي تحقق التماثلات، وأخذ عينات فرعية من مجموعات بحجم ثابت محدد مسبقًا.
3. لكل مجموعة فرعية من السلاسل الثنائية، يُسقَط الهاميلتوني في الفضاء الجزئي الذي تمتد عليه المتجهات الأساسية المقابلة (انظر [القسم السابق](#quantum-chemistry) لوصف هذه المتجهات الأساسية)، وتُحسب تقدير حالة الأساس للهاميلتوني المسقوط على حاسوب كلاسيكي.
4. تحديث تقدير متوسط إشغال المدارات بتقدير حالة الأساس ذي الطاقة الأدنى.
...
- Qiskit SDK الإصدار 1.0 أو أحدث، مع دعم [الرسوميات](https://docs.quantum.ibm.com/api/qiskit/visualization)
...
## الخطوة الأولى: تحويل المدخلات الكلاسيكية إلى مسألة كمية {#step-1-map-classical-inputs-to-a-quantum-problem}
...
قبل بناء دائرة نموذج LUCJ، نُجري أولًا حساب CCSD في خلية الكود التالية. ستُستخدم [سعات $t_1$ و$t_2$](https://en.wikipedia.org/wiki/Coupled_cluster#Cluster_operator) من هذا الحساب لتهيئة معاملات النموذج.
...
الآن، نستخدم [ffsim](https://github.com/qiskit-community/ffsim) لإنشاء دائرة النموذج. بما أن جزيئتنا تمتلك حالة هارتري-فوك ذات قشرة مغلقة، نستخدم المتغير المتوازن للسبين من نموذج UCJ، وهو [UCJOpSpinBalanced](https://qiskit-community.github.io/ffsim/api/ffsim.html#ffsim.UCJOpSpinBalanced). نمرر أزواج التفاعل المناسبة لهيكل كيوبت شبكة السداسي الثقيل (انظر [قسم الخلفية النظرية حول نموذج LUCJ](#local-unitary-cluster-jastrow-lucj-ansatz)). نضبط `optimize=True` في دالة `from_t_amplitudes` لتفعيل التحليل المضغوط المزدوج لسعات $t_2$ (انظر [نموذج التجمع الأحادي الموضعي (LUCJ)](https://qiskit-community.github.io/ffsim/explanations/lucj.html#Parameter-initialization-from-CCSD) في توثيق ffsim للمزيد من التفاصيل).
...
## الخطوة الثانية: تحسين المسألة لتنفيذها على الأجهزة الكمية {#step-2-optimize-problem-for-quantum-hardware-execution}
...
- اختيار الكيوبتات الفيزيائية (`initial_layout`) من الجهاز المستهدف وفق النمط المتعرج (zig-zag) الموصوف سابقًا. يؤدي ترتيب الكيوبتات بهذا النمط إلى دائرة متوافقة مع الجهاز وذات بوابات أقل. نُدرج هنا بعض التعليمات البرمجية لأتمتة اختيار نمط "المتعرج" باستخدام تقييم إرشادي يُقلل أخطاء التخطيط المختارة.
- إنشاء مدير ممرات متعدد المراحل باستخدام دالة [generate_preset_pass_manager](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.generate_preset_pass_manager) من Qiskit مع اختيارك من `backend` و`initial_layout`.
- ضبط مرحلة `pre_init` في مدير الممرات على `ffsim.qiskit.PRE_INIT`. يتضمن `ffsim.qiskit.PRE_INIT` ممرات ترجمة Qiskit التي تُفكك البوابات إلى دورانات مدارية ثم تدمجها، مما ينتج عنه عدد أقل من البوابات في الدائرة النهائية.
- تشغيل مدير الممرات على الدائرة.
<details>
<summary>كود الاختيار التلقائي لتخطيط "المتعرج"</summary>
...
بعد تحسين الدائرة لتنفيذها على العتاد، أصبحنا جاهزين لتشغيلها على العتاد المستهدف وجمع العينات لتقدير طاقة الحالة الأساسية. بما أن لدينا دائرة واحدة فقط، سنستخدم [وضع تنفيذ Job](/guides/execution-modes) في Qiskit Runtime وننفّذ دائرتنا.
...
يُظهر الرسم البياني الأول أنه بعد بضع تكرارات نقدّر طاقة الحالة الأساسية في حدود ``~41 mH`` (تُقبل الدقة الكيميائية عادةً عند ``1 kcal/mol`` $\approx$ ``1.6 mH``). يمكن تحسين الطاقة بالسماح بمزيد من تكرارات استرداد التهيئة أو بزيادة عدد العينات لكل دُفعة.
