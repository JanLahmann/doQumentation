...
يجب تعليم الدائرة عبر إدراج **CutWire** في المواضع المطلوبة لتقسيمها. في هذا الدليل التعليمي، نختار تقسيمًا متساويًا. صُمِّمت دائرة MBL بحيث يؤدي ضبط `use_cut=True` في الدالة إلى إدراج التعليم في الموضع الصحيح بعد $\frac{n}{2}$ كيوبت، حيث $n$ هو عدد الكيوبتات في الدائرة الأصلية. كما قمنا بتعيين المعاملات المُولّدة عشوائيًا للدائرة.
...
#### بناء المؤثر المرصود وتوسيعه {#construct-and-expand-the-observable}

المؤثر المرصود، كما عُرِّف سابقًا، هو متوسط $Z$ على كل كيوبت. غير أنه بعد إدراج بوابة `Move` الافتراضية، يزداد العدد الفعلي للكيوبتات في الدائرة. يجب توسيع المؤثر المرصود وفقًا لذلك لمراعاة هذا التغيير في عدد الكيوبتات. لاحظ أن المؤثر المرصود يعمل دائمًا بصورة تافهة (أي $I$) على الكيوبت الإضافي المُضاف لبوابة `Move` الافتراضية.
...
يمكن الآن تقسيم الدائرة عند بوابة `Move`، فنحصل على الدوائر الفرعية وكذلك على المؤثر المرصود الفرعي، وهو الجزء من المؤثر المرصود الأصلي المرتبط بكل دائرة فرعية.
...
يستلزم توسيع المؤثر المرصود باستخدام عملية `Move` بنية بيانات `PauliList`. لإعادة بناء قيمة توقع الدائرة الأصلية، نحتاج إلى المؤثر المرصود بصيغة `SparsePauliOp`.
...
كما نوقش سابقًا، يجب عند كل قطع قياس الدائرة السابقة (المنبع) في أساس باولي، وتحضير الدائرة اللاحقة (المصب) في الحالة الذاتية لذلك الأساس. تُنشئ الدالة `generate_cutting_experiments` جميع هذه الدوائر الضرورية والمعاملات المرتبطة بكل دائرة المطلوبة لإعادة البناء. تفاصيل إضافية متاحة في [هذه الورقة البحثية](https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.125.150504).
...
#### التحويل البرمجي للدوائر إلى الواجهة الخلفية {#transpile-the-circuits-onto-the-backend}

في المثال الأول الذي يتضمن المحاكاة فقط، نُحوِّل الدائرة برمجيًا إلى مجموعة البوابات الأساسية للواجهة الخلفية:
...
نُظهر الآن قطع الأسلاك لدائرة MBL ذات 60 كيوبت. ستُنفَّذ الدائرة غير المقطوعة وكذلك الدائرة المقطوعة على أجهزة IBM Quantum&reg;:
