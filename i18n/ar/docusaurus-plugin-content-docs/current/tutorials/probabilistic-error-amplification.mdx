---
description: 'تشغيل تجربة تخفيف أخطاء على نطاق الاستخدام باستخدام استقراء الضوضاء الصفرية وتضخيم الخطأ الاحتمالي.'
notebook_path: tutorials/probabilistic-error-amplification.ipynb
sidebar_label: 'تخفيف الأخطاء على نطاق الاستخدام مع تضخيم الخطأ الاحتمالي'
title: 'تخفيف الأخطاء على نطاق الاستخدام مع تضخيم الخطأ الاحتمالي'
---
...
*تقدير الاستخدام: 14 دقيقة على معالج Heron r3 (ملاحظة: هذا تقدير فحسب. قد يختلف وقت تشغيلك.)*

## نتائج التعلم {#learning-outcomes}
بعد إكمال هذا البرنامج التعليمي، ينبغي للمستخدمين أن يفهموا:
- النظرية الكامنة وراء *استقراء الضوضاء الصفرية* (ZNE)، والطرق المختلفة لتضخيم الضوضاء، وسبب تميّز *تضخيم الخطأ الاحتمالي* (PEA) في التجارب على نطاق الاستخدام.
- كيفية تطبيق ZNE مع PEA عملياً باستخدام Qiskit.

## المتطلبات الأساسية {#prerequisites}
نقترح أن يكون المستخدمون على دراية بالموضوعات التالية قبل البدء بهذا البرنامج التعليمي:
- [درس تخفيف الأخطاء](/learning/courses/utility-scale-quantum-computing/error-mitigation) من دورة *الحوسبة الكمومية على نطاق الاستخدام* لاكتساب المعرفة الأساسية باستخدام تخفيف الأخطاء في Qiskit.
- [درس Utility-I](/learning/courses/utility-scale-quantum-computing/utility-i) من دورة *الحوسبة الكمومية على نطاق الاستخدام* لمزيد من الخلفية حول التجربة على نطاق الاستخدام المُستخدَمة مثالاً في هذا البرنامج التعليمي.

## الخلفية النظرية {#background}

يوضح هذا البرنامج التعليمي كيفية تشغيل تجربة تخفيف أخطاء على نطاق الاستخدام باستخدام IBM Quantum Compute Service، مع إصدار تجريبي من *استقراء الضوضاء الصفرية* (ZNE) مع *تضخيم الخطأ الاحتمالي* (PEA).
...
### استقراء الضوضاء الصفرية (ZNE) {#zero-noise-extrapolation-zne}
استقراء الضوضاء الصفرية (ZNE) هو تقنية تخفيف أخطاء تُزيل تأثيرات الضوضاء *المجهولة* أثناء تنفيذ الدائرة، والتي يمكن تحجيمها بطريقة *معروفة*.
...
| **توسيع النبضات** | **طي البوابات** | **تضخيم الخطأ الاحتمالي** |
|-----------|-----------|-----------|
| تحجيم مدة النبضة عبر المعايرة | تكرار البوابات في دورات الهوية $U\mapsto U(U^{-1}U)^{\lambda-1}/2$ | إضافة ضوضاء عبر أخذ عينات من قنوات Pauli |
...
بالنسبة للتجارب على نطاق الاستخدام، يُعدّ *تضخيم الخطأ الاحتمالي* (PEA) الأسلوب الأكثر جاذبية.
...
يفترض PEA نفس نموذج الضجيج القائم على الطبقات مثل *إلغاء الخطأ الاحتمالي* (PEC)؛ ومع ذلك، فإنه يتجنب الحمل الزائد للعينات الذي يتناسب أُسّياً مع ضجيج الدارة.

يفترض PEA نموذج الضوضاء المستند إلى الطبقات المستخدم في *إلغاء الخطأ الاحتمالي* (PEC)؛ غير أنه يتجنب الحمل الزائد في أخذ العينات الذي يتزايد بشكل أسي مع ضوضاء الدائرة.
| **الخطوة 1** | **الخطوة 2** | **الخطوة 3** |
|-----------|-----------|-----------|
| لف طبقات Pauli من بوابات ثنائية الكيوبت | تكرار أزواج الطبقات المتطابقة وتعلّم الضوضاء | اشتقاق الإخلاص (الخطأ لكل قناة ضوضاء) |
...
**المرجع**: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, *Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors* [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)

## المتطلبات {#requirements}
قبل البدء بهذا البرنامج التعليمي، تأكد من تثبيت ما يلي:

- Qiskit SDK الإصدار v2.0 أو أحدث، مع دعم [visualization](https://docs.quantum.ibm.com/api/qiskit/visualization)

- Qiskit Runtime الإصدار v0.22 أو أحدث (`pip install qiskit-ibm-runtime`)


## الإعداد {#setup}
في الخلية أدناه، نستورد الحزم ذات الصلة وننشئ بعض الدوال المساعدة لبناء الدوائر الخاصة بالتطور الزمني المُقسَّم بـ Trotter لنموذج Ising المستعرض ثنائي الأبعاد بما يتوافق مع تضولوجيا backend.
...
## مثال على نطاق صغير باستخدام المحاكي {#small-scale-simulator-example}
سنتخطى هذه الخطوة إذ لا يدعم تخفيف أخطاء وقت التشغيل المحاكيات.

## مثال على نطاق كبير باستخدام الأجهزة الفعلية {#large-scale-hardware-example}
...
##### اختيار backend {#establish-a-backend}
أولاً، اختر backend للعمل عليه. يعمل هذا المثال التوضيحي على backend من 127 qubit، غير أنه يمكنك تعديله ليتناسب مع أي backend متاح لديك.
...
#### المقاييس الرصدية لـ ISA {#isa-observables}
...
لاحظ أن كل مقياس رصدي مُغلَّف في قائمة في كتلة الكود التالية. يتم ذلك لـ *البث* مع قيم المعاملات بحيث يُقاس كل مقياس رصدي للكيوبت لكل قيمة من قيم theta. يمكن الاطلاع على قواعد البث الخاصة بالأوليات في [توثيق الأوليات](/guides/primitives).
...
#### ضبط خيارات Estimator {#configure-estimator-options}
...
##### شرح خيارات ZNE {#explanation-of-zne-options}
...
* **amplifier**: الأسلوب المستخدم عند تضخيم الضوضاء إلى معاملات الضوضاء المقصودة.
  ...
  لطبقات بوابات الأساس ثنائية الكيوبت المُلتوية. الخيارات الإضافية هي `"gate_folding_front"` و`"gate_folding_back"` المشروحة في [توثيق API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier).

* **extrapolated\_noise\_factors**: حدِّد قيمة واحدة أو أكثر من قيم معاملات الضوضاء التي تريد تقييم
...
#### تشغيل التجربة {#run-the-experiment}
...
#### الأشكال العامة للنتائج والبيانات الوصفية {#general-result-shapes-and-metadata}
...
1. `pub_result.data.evs`: قيم التوقع المقابلة للضوضاء الصفرية (استناداً إلى أفضل استقراء وفق الاستدلال العملي).
   ...
   * المحور الأول هو فهرس الكيوبت الافتراضي للمُلاحَظ $\langle Z_i\rangle$ ($124$ كيوبت افتراضي/مُلاحَظ)

   * المحور الثاني يُفهرس قيمة المعامل $\theta$ ($12$ قيمة معامل)
...
2. `pub_result.data.evs_extrapolated`: قيم التوقع لمعاملات الضوضاء المُستقرأة لكل مستقرئ. يحتوي هذا المصفوف على محورين إضافيين.
   ...
   * المحور الثالث يُفهرس طرق الاستقراء ($2$ مستقرئان، `exponential` و`linear`)

   * المحور الأخير يُفهرس `extrapolated_noise_factors` ($20$ نقطة استقراء محددة في الخيار)

3. `pub_result.data.evs_noise_factors`: قيم التوقع الخام لكل معامل ضوضاء.

   * المحور الثالث يُفهرس `noise_factors` الخام ($3$ عوامل)
...
يحتوي كائن `PubResult` على بيانات وصفية إضافية حول مرونة نماذج الضوضاء المتعلّمة المستخدمة في التخفيف.
...
### رسم نتائج محاكاة Trotter {#plot-trotter-simulation-results}

يُنشئ الكود التالي رسمًا بيانيًا لمقارنة نتائج التجربة الخام والمخففة مع الحل الدقيق.
...
بينما تُظهر القيم المشوشة (عامل الضوضاء `nf=1.0`) انحرافًا عاليًا عن القيم الدقيقة، فإن القيم المخففة قريبة من القيم الدقيقة، مما يُظهر فائدة تقنية التخفيف القائمة على PEA.

### رسم نتائج الاستقراء للكيوبتات الفردية {#plot-extrapolation-results-for-individual-qubits}

أخيرًا، يُنشئ الكود التالي رسمًا بيانيًا لعرض منحنيات الاستقراء لقيم مختلفة من ثيتا على كيوبت محدد.
...
إن كان هذا العمل قد أثار اهتمامك، فقد يُفيدك الاطلاع على المواد التالية:
- [برنامج تعليمي](/tutorials/combine-error-mitigation-techniques) يركز على دمج تقنيات تخفيف الأخطاء.
- [توثيق مفصّل](/guides/error-mitigation-and-suppression-techniques) حول تقنيات تخفيف الأخطاء المتاحة في Qiskit.
- دروس إضافية تغطي التجارب على نطاق الاستخدام: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) و[Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
