...
description: "يوضح هذا الدرس التطبيقي كيفية تنفيذ AQC-Tensor مع Qiskit لتحسين أداء الدوائر الكمية"
...
الهدف الرئيسي هو محاكاة تطور الزمن لـ Hamiltonian نموذجي بعمق دائرة مُخفَّض. يتحقق ذلك باستخدام الإضافة **AQC-Tensor** لـ Qiskit، [qiskit-addon-aqc-tensor](https://github.com/Qiskit/qiskit-addon-aqc-tensor)، التي تستغل الشبكات الترابطية، وتحديدًا حالات جداء المصفوفات (MPS)، لضغط الدائرة الأولية وتحسينها. من خلال التعديلات التكرارية، تحافظ دائرة ansatz المضغوطة على الأمانة مع الدائرة الأصلية مع بقائها قابلة للتطبيق على العتاد الكمي قريب المدى. يمكن الاطلاع على مزيد من التفاصيل في [التوثيق](/guides/qiskit-addons-aqc) المقابل مع [مثال بسيط](/guides/qiskit-addons-aqc-get-started) للبدء.
...
* Qiskit SDK الإصدار 1.0 أو أحدث، مع دعم [التصور المرئي](https://docs.quantum.ibm.com/api/qiskit/visualization)
...
## الجزء الأول. مثال صغير النطاق {#part-i-small-scale-example}
...
### Hamiltonian النموذج والكمية القابلة للرصد {#model-hamiltonian-and-observable}
...
حيث $J_{i,(i+1)}$ هو معامل عشوائي يقابل الحافة $(i, i+1)$، و $L=10$ هو عدد المواقع.

من خلال محاكاة تطور هذا النظام بعمق دائرة مخفّض، يمكننا اكتساب رؤى حول استخدام AQC-Tensor لضغط الدوائر وتحسينها.
#### إعداد Hamiltonian والكمية القابلة للرصد {#set-up-the-hamiltonian-and-observable}
...
#### تحديد جزء تطور الزمن المراد محاكاته كلاسيكيًا {#determine-the-portion-of-time-evolution-to-simulate-classically}
...
- **القطعة الأولية**: هذا الجزء الأولي من التطور، من $ t_i = 0.0 $ إلى $ t_f = 0.2 $، قابل للمحاكاة باستخدام MPS ويمكن "تصريفه" بكفاءة باستخدام AQC-Tensor. باستخدام [الإضافة AQC-Tensor لـ Qiskit](https://github.com/Qiskit/qiskit-addon-aqc-tensor)، نولّد دائرة مضغوطة لهذه القطعة، يُشار إليها بـ `aqc_target_circuit`. نظرًا لأن هذه القطعة ستُحاكى على محاكي الشبكة الترابطية، يمكننا استخدام عدد أكبر من طبقات Trotter دون التأثير بشكل ملحوظ على موارد العتاد. نضع `aqc_target_num_trotter_steps = 32` لهذه القطعة.

- **القطعة اللاحقة**: هذا الجزء المتبقي من التطور، من $ t = 0.2 $ إلى $ t = 0.4 $، سيُنفَّذ على العتاد الكمي، يُشار إليه بـ `subsequent_circuit`. نظرًا لقيود العتاد، نسعى إلى استخدام أقل عدد ممكن من طبقات Trotter للحفاظ على عمق دائرة قابل للإدارة. نستخدم لهذه القطعة `subsequent_num_trotter_steps = 3`.

#### اختيار وقت التقسيم {#choose-the-split-time}
نختار $t = 0.2$ وقتًا للتقسيم لتحقيق التوازن بين قابلية المحاكاة الكلاسيكية وجدوى العتاد. في مراحل مبكرة من التطور، يظل التشابك في نموذج XXZ منخفضًا بما يكفي للأساليب الكلاسيكية كـ MPS للتقريب بدقة.
...
#### توليد ansatz والمعاملات الأولية من دائرة Trotter بخطوات أقل {#generate-an-ansatz-and-initial-parameters-from-a-trotter-circuit-with-fewer-steps}
...
#### اختيار إعدادات محاكاة الشبكة الترابطية {#choose-settings-for-tensor-network-simulation}
...
لاحظ أنه باختيار عدد أكبر من خطوات Trotter للحالة الهدف، قللنا فعليًا من خطأ Trotter مقارنةً بالدائرة الأولية. يمكننا تقييم الأمانة ($ |\langle \psi_1 | \psi_2 \rangle|^2 $) بين الحالة المُعدَّة بالدائرة الأولية والحالة الهدف لقياس هذا الفرق.
...
#### تحسين معاملات ansatz باستخدام حسابات MPS {#optimize-the-parameters-of-the-ansatz-using-mps-calculations}
...
اختَر العتاد المناسب. سنستخدم هنا أيًّا من أجهزة IBM Quantum&reg; المتاحة التي تضم ما لا يقل عن 127 كيوبت.
...
## الجزء الثاني: التوسع إلى أنظمة أكبر {#part-ii-scale-it-up}
...
حيث $J_{i,(i+1)}$ هو معامل عشوائي يقابل الحافة $(i, i+1)$، و$L=50$ هو عدد المواقع.
عرِّف خريطة الاقتران والحواف الخاصة بالهاميلتوني.
...
### الخطوة 1: تعيين المدخلات الكلاسيكية على مسألة كمية {#step-1-map-classical-inputs-to-a-quantum-problem}
...
للاطلاع على القائمة الكاملة لخيارات التنفيذ المتاحة، راجع [توثيق qiskit-ibm-runtime](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-estimator-options).
...
- **ضغط الدائرة بصورة ملحوظة**: نجحت AQC-Tensor في تقليل عمق الدائرة اللازم لتطور زمني معقد، مما عزّز جدواها على الأجهزة الحالية.
