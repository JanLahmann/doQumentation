...
بعد إتمام هذا البرنامج التعليمي، ستكون قد تعلمت ما يلي:
* كيفية تنفيذ بوابة CNOT بعيدة المدى باستخدام الدوائر الديناميكية مع قياسات منتصف الدائرة (MCMs) والتغذية الكلاسيكية الأمامية؛
* كيفية تنفيذ البوابة المكافئة باستخدام نهج موحد قائم على SWAP؛
* كيفية مقارنة النهجين عن طريق قياس إخلاص البوابة كدالة لمسافة الكيوبت.

## المتطلبات الأساسية {#prerequisites}

نقترح أن يكون المستخدمون على دراية بالموضوعات التالية قبل الشروع في هذا البرنامج التعليمي:
* [المفاهيم الأساسية للحوسبة الكمومية](/learning/courses/basics-of-quantum-information)، بما في ذلك حالات بيل والتشابك والبوابات الكمومية؛
* الإلمام بـ [الدوائر الديناميكية](/guides/classical-feedforward-and-control-flow) (قياسات منتصف الدائرة والتغذية الكلاسيكية الأمامية)؛
* معرفة أساسية بـ [Qiskit SDK](https://docs.quantum.ibm.com/guides) و[IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime)، والوصول إلى [حساب IBM Quantum](/guides/cloud-setup).
...
في هذا الدفتر، نُكيّف البروتوكول لأجهزة IBM Quantum ونقيس أداءه كدالة لمسافة الفصل بين التحكم والهدف، مقارنةً بخط أساس موحد قائم على SWAP.
...
- Qiskit Runtime الإصدار 0.37 أو أحدث (`pip install qiskit-ibm-runtime`)
...
قبل التشغيل على وحدة المعالجة الكمومية الفعلية، نتحقق من أن كلًّا من الدوائر الديناميكية والموحدة تُنتجان حالة بيل مثالية على محاكٍ غير مُضوَّش. نستخدم IBM Quantum `Sampler` مع `AerSimulator` كوضع للخلفية، عند مسافة صغيرة قدرها 6.
...
#### تهيئة الدائرة {#initialize-circuit}
...
#### إعداد أزواج بيل {#prepare-bell-pairs}
...
#### قياس أزواج الكيوبتات المجاورة في أساس بيل {#measure-neighboring-qubit-pairs-in-the-bell-basis}
...
#### تطبيق تصحيحات التغذية الأمامية لتصحيح عوامل باولي الجانبية {#apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
#### قياس كيوبتي التحكم والهدف {#measure-control-and-target-qubits}
...
نجمع الخطوات المختلفة المحددة أعلاه لإنشاء بوابة CX بعيدة المدى على طرفي خط أحادي البعد (1D). تتضمن الخطوات ما يلي:
- تهيئة كيوبت التحكم في $|+\rangle$
- إعداد أزواج بيل
...
الآن نبني الدوائر التي تقيس في أسس $XX$ و$YY$ و$ZZ$، تمامًا كما فعلنا للدوائر الديناميكية أعلاه.
...
الآن بعد أن بنينا كلًّا من الدوائر الديناميكية والموحدة لمثال صغير النطاق بـ `distance=6`، نُحوّلها لتشغيلها أولًا على محاكٍ غير مُضوَّش.
...
يمكننا الآن تشغيل التجربة على خلفية المحاكي غير المُضوَّش. نستخدم Qiskit Runtime Sampler مع AerSimulator كوضع للخلفية لتنفيذ الدوائر.
...
في محاكاة غير مُضوَّشة، سنتحقق من أن مقياس الإخلاص يساوي $1$ للدوائر المُنشأة. وفي التجارب على وحدات المعالجة الكمومية الفعلية، سيوفر هذا التحليل صورة واضحة عن مدى أداء الدوائر الديناميكية مقارنةً بتنفيذ الأساس الموحد.
...
كما هو متوقع في محاكاة غير مُضوَّشة، يساوي الإخلاص $1$ في كلٍّ من الدوائر الديناميكية والموحدة.

## مثال واسع النطاق على الأجهزة الفعلية {#large-scale-hardware-example}

نجمع هنا كل هذه التفاصيل في سير عمل واحد على نطاق أوسع، ثم نشغّله على أجهزة كمومية فعلية.

### توليد دوائر لمسافات مختلفة {#generate-circuits-for-different-distances}

نُولّد الآن دوائر CX بعيدة المدى لمجموعة من مسافات الفصل بين الكيوبتات تصل إلى 60 كيوبتًا. لكل مسافة، نبني دوائر تقيس في أسس $XX$ و$YY$ و$ZZ$، والتي ستُستخدم لاحقًا لحساب قيم الإخلاص.
...
الآن وقد أصبحت لدينا الدارات الديناميكية والدارات الأحادية لمجموعة من المسافات، أصبحنا جاهزين لعملية التصريف (transpilation). نحتاج أولًا إلى اختيار جهاز Backend.
...
تضمن الخطوة التالية أن يدعم الـ Backend التعليمة `if_else`، وهي مطلوبة للإصدار الأحدث من الدارات الديناميكية. وبما أن هذه الميزة لا تزال في مرحلة الوصول المبكر، فإننا نضيف `IfElseOp` صراحةً إلى هدف الـ Backend إذا لم تكن متوفرة بالفعل.
...
بعد ذلك، ننفّذ التجربة على الخلفية الفعلية. نستخدم أيضًا المعالجة الدُفعية لتشغيل التجربة بكفاءة عبر تجارب متعددة. يتيح تشغيل تجارب متكررة حساب المتوسطات للحصول على مقارنة أكثر دقة بين الطريقتين الموحدة والديناميكية، فضلًا عن قياس تباينها من خلال مقارنة الانحرافات عبر عمليات التشغيل.
...
إن وجدت هذا العمل مثيرًا للاهتمام، قد تجد المواد التالية مفيدة:
<Admonition type="tip" title="توصيات">
- [قياس الدوائر الديناميكية بأزواج بيل المقطوعة](/tutorials/edc-cut-bell-pair-benchmarking)
- [محاكاة هاميلتونيان Ising المُفعَّل بالركلة مع الدوائر الديناميكية](/tutorials/dc-hex-ising)
</Admonition>
