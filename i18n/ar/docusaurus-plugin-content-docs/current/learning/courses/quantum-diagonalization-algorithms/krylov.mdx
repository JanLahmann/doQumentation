---
description: 'يُشرح قطرنة كريلوف الكمومية (KQD)، بدءاً من أساليب كريلوف الكلاسيكية. ويتم مناقشة التقارب وكثافة الموارد المطلوبة.'
notebook_path: learning/courses/quantum-diagonalization-algorithms/krylov.ipynb
sidebar_label: 'قطرنة كريلوف الكمومية'
title: 'قطرنة كريلوف الكمومية'
---
...
<IBMVideo id="134325510" title="في هذا الفيديو، يقدم كريس بورتر لمحة عامة عن أساليب كريلوف الكلاسيكية ويشرح سبب أهميتها. ويوضح كيف يمكن للحوسبة الكمومية أن تلعب دوراً في هذا النهج لعملية القطرنة. يوفر النص أدناه مزيداً من التفاصيل."/>

## 1. مقدمة إلى أساليب كريلوف {#1-introduction-to-krylov-methods}

يمكن أن تشير __طريقة فضاء كريلوف__ إلى أي من عدة طرق مبنية حول ما يُسمى __فضاء كريلوف__. مراجعة كاملة لهذه الطرق تتجاوز نطاق هذا الدرس، لكن المرجع [\[2-4\]](#references) يمكن أن يقدم خلفية أكثر شمولًا. هنا، سنركز على ما هو فضاء كريلوف، وكيف ولماذا يكون مفيدًا في حل مسائل القيم الذاتية، وأخيرًا كيف يمكن تنفيذه على حاسوب كمومي.
__تعريف:__ بالنظر إلى مصفوفة متماثلة، شبه معرّفة موجبة $N\times N$ تُسمى $A$، فإن فضاء كريلوف $\mathcal{K}^r$ من الرتبة $r$ هو الفضاء الممتد بواسطة متجهات يتم الحصول عليها بضرب قوى أعلى للمصفوفة $A$، حتى $r-1\leq N$، بمتجه مرجعي $\vert v \rangle$.
...
(ب) تتألف عملية التعامد من طرح إسقاط متجه جديد على جميع المتجهات السابقة. إذا كانت جميع المتجهات السابقة تمتد على فضاء المتجهات الكامل، فإن طرح الإسقاطات على الفضاء الفرعي الكامل سيترك لنا دائمًا متجهًا صفريًا.

</AccordionItem>
</Accordion>

لنفترض أن زميلًا باحثًا يعرض طريقة كريلوف مطبقة على مصفوفة صغيرة توضيحية. هل هناك خطأ ما في اختياره للمصفوفة $A$ والمتجه الأولي $|\psi\rangle$؟
...
اختار زميلك عن طريق الخطأ متجهًا ذاتيًا كمتجهه الأولي. سيؤدي تطبيق المصفوفة على المتجه الأولي إلى إعادة نفس المتجه فقط، مضروبًا في القيمة الذاتية. هذا لن يُولّد فضاءً فرعيًا ذا بُعد متزايد. انصح زميلك باختيار متجه أولي مختلف، والتأكد من أنه ليس متجهًا ذاتيًا.

</AccordionItem>
</Accordion>

طبّق طريقة كريلوف على المصفوفة المعطاة، باختيار متجه أولي جديد مناسب. اكتب تقديرات القيمة الذاتية الدنيا عند الرتبة صفر والرتبة الأولى من فضاء كريلوف الخاص بك.
...
توجد إجابات ممكنة عديدة بحسب اختيار المتجه الأولي. سنختار:
$$
|v_0\rangle=\frac{1}{\sqrt{3}}\begin{pmatrix}1\\1\\1\end{pmatrix}.
$$
للحصول على $|v_1\rangle$ نطبّق $A$ مرة واحدة على $|v_0\rangle$، ثم نجعل $|v_1\rangle$ متعامدًا مع $|v_0\rangle.$
$$
...
$$
عند الرتبة صفر، الإسقاط على فضاء كريلوف الخاص بنا هو
...
$$
عند الرتبة الأولى، الإسقاط على فضاء كريلوف هذا هو
...
$$
يمكن القيام بهذا يدويًا، لكن يكون أسهل عادةً باستخدام numpy:
...
نحن مهتمون بكيفية تصرف خطئنا عند تحويله بواسطة مصفوفتنا. لذا لنحسب معيار $A$ للخطأ. أي
$$
\begin{aligned}
∥|e_0\rangle−|p_0\rangle∥_A^2&=\left(\langle e_0|A−\langle p_0|A\right)\left(|e_0\rangle−|p_0\rangle\right)\\
 ...
 \end{aligned}
$$
حيث استخدمنا تماثل $A$ وأيضًا أن $A |e_0\rangle = |R_0\rangle.$ هنا $d$ ثابت ما مستقل عن $|p_0\rangle$. كما ذُكر في القسم 1.2، معيار $A$ للخطأ ليس الكمية الوحيدة التي قد نختار تصغيرها، لكنه كمية جيدة. نريد أن نرى كيف تتغير هذه الكمية مع اختيارنا لمتجهات التصحيح $|p_0\rangle.$ لذا نُعرّف الدالة $f$ بضبط
$$
f(|p_0\rangle)=\langle p_0|A|p_0\rangle−2\langle R_0|p_0\rangle+d.
$$
$f$ هو فقط الخطأ $|e_1\rangle$ كدالة للتصحيح $|p_0\rangle$ مقاسًا بمعيار $A$. وبالتالي، نريد اختيار $|p_0\rangle$ بحيث تكون $f(|p_0\rangle)$ أصغر ما يمكن. لهذا الغرض، نحسب تدرج $f$. باستخدام تماثل $A$ لدينا
$$
\nabla f(|p_0\rangle) = 2(A|p_0\rangle−|R_0\rangle).
$$
يشير التدرج إلى اتجاه أكبر صعود، مما يعني أن عكسه يعطينا الاتجاه الذي تتناقص فيه الدالة أكثر: اتجاه __أكبر انحدار__. عند تخميننا الأولي $|x_0\rangle$، حيث $|p_0\rangle=0$، لدينا أن
$\nabla f(0) = -2|R_0\rangle.$
وبالتالي، تتناقص الدالة $f$ أكثر في اتجاه المتبقي $|R_0\rangle.$ لذا سيستفيد اختيارنا الأولي أكثر من إضافة المتجه $|p_0\rangle=\alpha_0 |R_0\rangle$ لبعض العدد السلمي $\alpha_0$.
...
في سير العمل أعلاه، اقترحنا تصغير معيار $A$ للخطأ. ما هي الكميات الأخرى التي قد يفكر المرء في تصغيرها سعيًا للحصول على الحالة الأرضية وقيمتها الذاتية؟
...
ارجع إلى الرسم البياني الممهد لأوقات القطرنة مقابل بُعد المصفوفة أعلاه.

(أ) عند أي بُعد تقريبي للمصفوفة أصبحت طريقة كريلوف أسرع، وفقًا لهذا الرسم البياني؟

(ب) ما جوانب الحساب التي يمكن أن تغيّر ذلك البُعد الذي تصبح عنده طريقة كريلوف أسرع؟
...
(ب) ثمة إجابات كثيرة محتملة. من أهم العوامل: الدقة المطلوبة وتفرُّق المصفوفات المراد قطرنةها.
...
$$
هنا $P_\alpha$ هو سلسلة باولي من الشكل $P_\alpha \sim IZIXII...YZXIX$ أو مجموعة من سلاسل باولي المتبادلة التبديل فيما بينها. بما أنه يمكننا كتابة $H$ كمجموع من العوامل القابلة للقياس، يمكن تحقيق التعبيرات التالية لعناصر مصفوفة $\tilde{H}$ باستخدام IBM Quantum primitive Estimator.
...
لماذا لا يمكننا ببساطة قطرنة $\tilde{H}$؟ بما أن $\tilde{S}$ تحتوي على معلومات حول هندسة قاعدة كريلوف (وهي غير متعامدة في جميع الحالات باستثناء حالات خاصة جدًا)، فإن $\tilde{H}$ بمفردها لا تصف إسقاط الهاميلتوني الكامل، لذا فإن قيمها الذاتية ليس لها علاقة معينة بقيم الهاميلتوني الكامل -- يمكن أن تكون أي قيم عشوائية. حل مسألة القيم الذاتية المعممة مطلوب للحصول على القيم الذاتية والمتجهات الذاتية التقريبية المقابلة لإسقاط الهاميلتوني الكامل في فضاء كريلوف..
![مخطط دائرة به طبقات كثيرة يشير إلى أنه يجب استخدام الدائرة مرات عديدة مع حالات مختلفة لتنفيذ اختبار هادامارد المعدّل.](/learning/images/courses/quantum-diagonalization-algorithms/krylov/kqd-fig4.avif)
...
## 4. قطرنة كريلوف الكمومية على حاسوب كمومي {#4-krylov-quantum-diagonalization-on-a-quantum-computer}
...
هنا $P$ هو حدّ باولي في تحليل الهاميلتوني (لاحظ أنه لا يمكن أن يكون تركيباً خطياً من عدة حدود باولي متبادلة التبديل، لأن ذلك لن يكون تآلفياً — التجميع ممكن باستخدام بناء مختلف سنعرضه لاحقاً) $\text{Prep} \; \psi_i$، $\text{Prep} \; \psi_j$ هي عمليات متحكَّم بها تُحضّر متجهات $|\psi_i\rangle$ و$|\psi_j\rangle$ من فضاء كريلوف الوحداني، حيث $|\psi_k\rangle = e^{-i H k dt } \vert \psi \rangle = e^{-i H k dt } U_{\psi} \vert 0 \rangle^N$. يُحسب قياس $X$ و$Y$ على هذه الدائرة الجزءَ الحقيقي والجزءَ التخيلي على التوالي من عناصر المصفوفة التي نحتاجها.
...
![صورة لمخطط دائرة كمومية بطبقات متعددة تشير إلى أنه يجب تقييم الدائرة لعدد كبير من المؤثرات الوحدانية المختلفة لإجراء اختبار هادامار المعدّل والكفء.](/learning/images/courses/quantum-diagonalization-algorithms/krylov/kqd-fig6.avif)
...
![ناتج خلية الكود السابقة](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/8d35e6cd-c0f6-4871-83da-86472d66be2c-0.avif)
...
![ناتج خلية الكود السابقة](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/d1d0b9de-65d4-4a46-975d-6cfaaac05f9a-0.avif)

#### دوائر قالبية لحساب عناصر المصفوفة لـ $\tilde{S}$ و$\tilde{H}$ عبر اختبار هادامار {#template-circuits-for-calculating-matrix-elements-of-tildes-and-tildeh-via-hadamard-test}
...
![ناتج خلية الكود السابقة](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/37382668-1999-4475-b50f-2887449d5c93-0.avif)
...
![ناتج خلية الكود السابقة](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/054bcfba-fde3-4213-bbd7-d7a6dd6c03c8-1.avif)

بعد التحسين، انخفض عمق البوابات ثنائية الكيوبت للدائرة المُترجمة بشكل أكبر.

### 4.3 الخطوة 3. التنفيذ باستخدام أداة IBM Quantum بدائية {#43-step-3-execute-using-an-ibm-quantum-primitive}
...
نرى أن قطرنة كريلوف الكمية قد تكون مفيدة في الحالات التي يكون فيها عدد مجموعات Pauli المتبادلة التبديل في المؤثر الهاميلتوني صغيراً نسبياً. تُشير تبعيات التوسع هذه إلى بعض التطبيقات التي يمكن فيها لأسلوب كريلوف أن يكون مفيداً، وإلى أخرى لن يكون فيها كذلك على الأرجح.
...
### الملحق الأول: فضاء كريلوف الجزئي من التطورات الزمنية الحقيقية {#appendix-i-krylov-subspace-from-real-time-evolutions}
...
لخطوة زمنية $dt$ سنحددها لاحقاً. لنفترض مؤقتاً أن $r$ زوجي: نعرّف حينئذٍ $d=r/2$. لاحظ أنه حين نسقط المؤثر الهاميلتوني على فضاء كريلوف أعلاه، لا يمكن تمييزه عن فضاء كريلوف
...
نريد أن نُثبت أن ثمة حالة ذات طاقة منخفضة مضمونة الوجود في مكان ما من فضاء كريلوف هذا. نفعل ذلك بالاستناد إلى النتيجة التالية، المستنبطة من المبرهنة 3.1 في [\[3\]](#references):
...
يصح هذا لأي $\delta>0$، لذا إذا ضبطنا $\delta$ مساوياً لخطأ هدفنا، فإن حد الخطأ أعلاه يتقارب نحوه أسياً مع بُعد كريلوف $2d=r$. لاحظ أيضاً أنه إذا كان $\delta<E_1-E_0$ فإن الحد $\delta$ يختفي كلياً من الحد أعلاه.

لاستكمال الحجة، نلاحظ أولاً أن ما سبق هو مجرد خطأ الطاقة للحالة المحددة $f(H)|\psi\rangle$، لا خطأ الطاقة للحالة الأدنى طاقةً في فضاء كريلوف. غير أنه وفق المبدأ المتغيري (Rayleigh-Ritz)، فإن خطأ الطاقة للحالة الأدنى طاقةً في فضاء كريلوف يكون محدوداً من الأعلى بخطأ الطاقة لأي حالة في فضاء كريلوف، لذا فإن ما سبق هو أيضاً حد أعلى لخطأ الطاقة للحالة الأدنى طاقةً، أي ناتج خوارزمية قطرنة كريلوف الكمية.
...
نريد تحويل هذا إلى دالة يمكن التعبير عنها بطريقة طبيعية باستخدام الأسيات المركبة، لأن تلك هي التطورات الزمنية الحقيقية التي تُولّد فضاء كريلوف الكمي.
