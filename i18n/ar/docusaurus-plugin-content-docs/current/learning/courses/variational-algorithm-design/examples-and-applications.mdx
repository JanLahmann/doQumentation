...
تخيّل أننا نريد استخدام خوارزمية تغايرية لإيجاد القيمة الذاتية للمرصود التالي:

$$
\hat{O}_1 = 2 II - 2 XX + 3 YY - 3 ZZ,
$$

يمتلك هذا المرصود القيم الذاتية التالية:
...
بدلًا من البحث عن أصغر قيمة ذاتية فقط للمرصودات، سنبحث هنا عن جميع القيم الـ $4$ (حيث $k=4$).
...
علاوةً على ذلك، في تنفيذ Qiskit لـ VQD، وبدلًا من اعتبار المؤثرات الفعالة المُوضَّحة في الدرس السابق، تُحسَب الإخلاصات $|\langle \psi(\vec{\theta})| \psi(\vec{\theta^j})\rangle  |^2$ مباشرةً عبر خوارزمية `ComputeUncompute`، التي تستعين بأداة `Sampler` الأولية لأخذ عينات من احتمال الحصول على $|0\rangle$ للدائرة
...
سنستخدم الدالة التالية لحساب غرامة التداخل. لاحظ أن هذا ما زال جزءًا من تعيين المسألة إلى دارات كمومية. ومع ذلك، كما نوقش في الدرس السابق، تحسب هذه الدالة التداخل بين الدارة التبايُنية الحالية والدارة المحسَّنة من حالة سابقة أقل طاقة/تكلفة تم الحصول عليها. يجب أيضًا ترجمة الدارة الجديدة التي يتم إنشاؤها لتشغل على عتاد حقيقي. لقد رأينا هذه الدالة من قبل، مُستخدَمة على محاكي. هنا، يجب أن نأخذ بالفعل في الاعتبار الترجمة والتحسين المرتبط بها عندما نستخدم backend حقيقي، ومن هنا الأسطر حول `if realbackend == 1`. هذا يمزج قليلاً من الخطوة 2، لكننا سنشير إلى الخطوة 2 صراحةً لاحقًا.
...
الآن نضيف دالة التكلفة الخاصة بـ VQD. لاحظ أنه مقارنةً بالدرس السابق، لدينا الآن معاملان إضافيان (`realbackend` و`backend`) لمساعدتنا في الترجمة عندما نستخدم backend حقيقي.
...
مرة أخرى، سنستخدم المحاكيات لأغراض التصحيح، ثم ننتقل إلى الأجهزة الحقيقية.
...
هذه النتائج قريبة جداً من النتائج المتوقعة باستثناء خطأ التقريب والطور الكلي. يمكننا تعديل مدى التسامح في المُحسِّن الكلاسيكي و/أو العقوبات المتعلقة بتداخل متجه الحالة للحصول على قيم أكثر دقة.
...
سنُترجم دائرتنا باستخدام مُدير مسار جاهز ومستوى تحسين 3.
...
يمكننا تصغير دالة التكلفة هذه لحساب المعاملات المثلى، ويمكننا التحقق من كودنا أولاً باستخدام محاكي محلي.
...
سنستخدم مدير تمريرات جاهزاً للترجمة، وسنُحسّن دائرتنا إلى أقصى حد باستخدام مستوى التحسين 3.
...
لحل هذه المسألة بحاسوب كمي، سنُعبِّر عن دالة التكلفة على شكل القيمة المتوقعة للمرصود. غير أن المرصودات التي يقبلها Qiskit بشكل أصلي تتكون من مؤثرات باولي، التي تمتلك القيم الذاتية $1$ و$-1$ بدلاً من $0$ و$1$. لهذا السبب سنجري تغيير المتغير التالي:
...
وبالتالي المرصود الذي سنأخذه بالاعتبار هو:
...
انتقل إلى الدرس الأخير لإجراء تقييمك والحصول على شارتك!
