...
ابنِ دائرة GHZ لـ 20 كيوبت أو أكثر بحيث تستوفي نتيجة القياس المعايير التالية: دقة حالة GHZ > 0.5.

- يجب عليك استخدام جهاز Eagle (مثل `ibm_brisbane`) وتعيين عدد اللقطات إلى 40,000.
- يجب عليك تنفيذ دائرة GHZ باستخدام الدالة `execute_ghz_fidelity`، وحساب الدقة باستخدام الدالة `check_ghz_fidelity_from_jobs`.
