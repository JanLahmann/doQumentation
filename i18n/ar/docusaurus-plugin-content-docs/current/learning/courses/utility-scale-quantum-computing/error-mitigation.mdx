...
<OpenInLabBanner notebookPath="learning/courses/utility-scale-quantum-computing/error-mitigation.ipynb" />
  ...
  # الضوضاء الكمومية وتخفيف الأخطاء
...
## 2. المحاكاة الصاخبة بدون تخفيف للأخطاء {#2-noisy-simulation-without-error-mitigation}
...
### 2.1 بناء دائرة اختبار {#21-build-a-test-circuit}

نأخذ بعين الاعتبار دوائر بسيطة بكيوبت واحد تكرر بوابات X عدد `d` من المرات (`d`=0 ... 100) وتقيس المُشاهَد `Z`.
...
### 2.2 بناء نموذج الضوضاء {#22-build-a-noise-model}
...
### 2.3 بناء أداة أخذ عينات صاخبة باستخدام نموذج الضوضاء {#23-build-a-noisy-sampler-with-the-noise-model}
...
### 2.4 تشغيل الدوائر الكمية على أداة أخذ العينات الصاخبة {#24-run-quantum-circuits-on-the-noisy-sampler}
...
### 2.5 رسم النتائج {#25-plot-results}
...
### 2.6 المحاكاة المثالية {#26-ideal-simulation}
...
### 2.8 محاكاة أكثر واقعية للضوضاء {#28-more-realistic-noisy-simulation}
...
## 3. الحوسبة الكمومية الحقيقية مع تخفيف الأخطاء {#3-real-quantum-computation-with-error-mitigation}
...
### 3.1 بناء الدوائر {#31-build-circuits}
...
لاحظ أن الناتج المثالي لمثل هذه الدوائر سيكون بشكل بديهي هو الحالة المدخلة $|000000\rangle$، التي لها قيم توقع تافهة لأي مُشاهَدات Pauli، مثلاً $\langle IIIIIZ \rangle = 1$.
...
### 3.2 تحويل الدوائر {#32-transpile-the-circuits}
...
### 3.3 التنفيذ باستخدام Estimator (مع مستويات مرونة مختلفة) {#33-execute-using-estimator-with-different-resilience-levels}

ضبط مستوى المرونة (`estimator.options.resilience_level`) هو أسهل طريقة لتطبيق تخفيف الأخطاء عند استخدام Qiskit Estimator. يدعم Estimator مستويات المرونة التالية (اعتبارًا من 2024/06/28). راجع المزيد من التفاصيل في دليل [ضبط تخفيف الأخطاء](/guides/error-mitigation-and-suppression-techniques).
...
### 3.4 رسم النتائج {#34-plot-results}
...
## 4. (اختياري) تخصيص خيارات تخفيف الأخطاء {#4-optional-customize-error-mitigation-options}
...
- [ضبط تخفيف الأخطاء](/guides/error-mitigation-and-suppression-techniques)
