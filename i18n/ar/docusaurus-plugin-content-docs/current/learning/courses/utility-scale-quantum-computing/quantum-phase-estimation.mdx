...
### 2.1. الحدس <a id="intuition"></a> {#21-intuition}
...
لنعمم تحويلاً لـ $N = 2^n$، $QFT_{N}$ الذي يعمل على الحالة $\vert x \rangle = \vert x_1\ldots x_n \rangle$.
$$
\begin{aligned}
...
### 2.4 مثال: بناء دائرة تحويل فورييه الكمي لـ 3 كيوبتات {#24-example-circuit-construction-of-3-qubits-qft}
...
### 2.5 تمرين: تحويل فورييه الكمي {#25-exercise-qft}
...
$$
نُهيّئ $\vert\psi\rangle = \vert1\rangle$ من المتجه الذاتي لبوابة $T$ عن طريق تطبيق بوابة $X$:
...
$$
 تتيح لنا خوارزمية شور تحليل عدد إلى عوامل عن طريق بناء دارة بمعامل $\theta$ مجهول والحصول على $\theta$.
...
## 4. التنفيذ باستخدام أداة Sampler البدائية في IBM Quantum {#4-execution-using-the-ibm-quantum-sampler-primitive}
