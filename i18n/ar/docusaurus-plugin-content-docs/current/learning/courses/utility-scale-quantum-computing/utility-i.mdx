...
## 1. مقدمة إلى ورقة الفائدة {#1-introduction-to-the-utility-paper}
...
بشكل ملموس، مطلوب منك بناء وتشغيل الدارة المقابلة للشكل 4ب من الورقة البحثية، ورسم النقاط "غير المُخفَّفة" الخاصة بك.  كما ترى، إنها دارة 127-كيوبت $\times$ 60-طبقة (20 خطوة تروتر) بـ $\langle Z_{62} \rangle$ كمرصود.
![image.png](/learning/images/courses/utility-scale-quantum-computing/utility-i/utility1.avif)
يبدو الأمر مهمًا كبيرة؟  &nbsp; لا تقلق. توفر الدروس الثلاثة الأخيرة من هذه الدورة نقاط انطلاق. للبدء، سنعرض تجربة أصغر نطاقًا وهي بناء وتشغيل على جهاز وهمي دارة 27-كيوبت $\times$ 6-طبقة (2 خطوة تروتر) بـ $\langle Z_{13} \rangle$ كمرصود.
...
## 2. التحضير {#2-preparation}

### 2.1 بناء RZZ(-$\pi$ / 2) {#21-construct-rzz-pi-2}
...
### 2.2 تلوين خريطة الاقتران {#22-color-the-coupling-map}
...
## 3. حل التطور الزمني بتروتر لنموذج إيزنغ ثنائي الأبعاد {#3-solve-the-trotterized-time-evolution-of-a-2d-ising-model}
...
لنمارس هذه الروتينات.  يجب أن ترى دارة من 27 كيوبت $\times$ 15 طبقة (5 خطوات تروتر). بما أن الجهاز الوهمي لديه 28 حافة، يجب أن يكون هناك 28*5 بوابة متشابكة.
...
## 4. حل نسخة 27 كيوبت من المسألة {#4-solve-the-27-qubit-version-of-the-problem}
...
### 4.1 الخطوة 1: الربط {#41-step-1-map}
...
### 4.2 الخطوتان 2 و3: التحسين والتنفيذ (المحاكي) {#42-steps-2-and-3-optimize-and-execute-simulator}
...
### 4.3 الخطوتان 2 و3: التحسين والتنفيذ (الجهاز الافتراضي) {#43-steps-2-and-3-optimize-and-execute-fake-device}
...
عندما شغّل نفس المستخدم الخلية التالية بنفس البيئة أعلاه، استغرق الأمر 2 دقيقة و19 ثانية بالتوقيت الفعلي.  تنفيذ دارة على جهاز وهمي يستدعي محاكاة ضوضائية تستغرق وقتًا أطول بكثير من المحاكاة الدقيقة.   نوصي بألا تنفذ دارة أكبر (مثل 27-كيوبت $\times$ 9 طبقات مع 3 خطوات تروتر) على جهاز وهمي.
...
### 4.4 الخطوة 4: المعالجة اللاحقة {#44-step-4-post-process}
...
## 5. حل نسخة 127 كيوبت من المسألة {#5-solve-the-127-qubit-version-of-the-problem}
...
### 5.1 الخطوة 1: الربط {#51-step-1-map}
...
### 5.2 الخطوتان 2 و3: التحسين والتنفيذ {#52-steps-2-and-3-optimize-and-execute}
...
### 5.3 المعالجة اللاحقة {#53-post-process}
