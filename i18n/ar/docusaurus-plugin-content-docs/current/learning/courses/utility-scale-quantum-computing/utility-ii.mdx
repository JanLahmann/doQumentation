...
description: "يتبع هذا الدفتر أساليب وتقنيات الدرس 7. هدفنا هو حل معادلة شرودنغر المعتمدة على الزمن عدديًا."
...
# تجربة على نطاق المنفعة II {#utility-scale-experiment-ii}

<Admonition type="note">

Yukio Kawashima (12 July 2024)

[تحميل ملف PDF](https://ibm.ent.box.com/s/bipgoms7gr6b6vhkoc1uw6oi4wsanfoq) للمحاضرة الأصلية. لاحظ أن بعض مقتطفات الشيفرة قد تصبح قديمة لأنها صور ثابتة.

*الوقت التقريبي لوحدة المعالجة الكمية لتشغيل هذه التجربة هو 2 دقيقة و30 ثانية.*

(لاحظ أن هذا الدفتر استخدم نصوصًا ورسومًا توضيحية وأكوادًا من [دفتر تعليمي](https://github.com/qiskit-community/qiskit-algorithms/blob/main/docs/tutorials/13_trotterQRTE.ipynb) تم إيقافه الآن لـ Qiskit Algorithms.)
...
انطلاقًا من معادلة شرودنغر، يأخذ التطور الزمني لنظام في الحالة الابتدائية $\vert\psi(0)\rangle$ الشكل التالي:

$$
\vert \psi(t) \rangle = e^{-i H t} \vert \psi(0) \rangle \text{,}
$$

حيث $H$ هو الهاملتوني المستقل عن الزمن الذي يحكم النظام. نعتبر هاملتوني يمكن كتابته كمجموع موزون لحدود Pauli $H=\sum_j a_j P_j$، حيث يمثل $P_j$ حاصل ضرب تنسوري لحدود Pauli تعمل على $n$ كيوبت. على وجه الخصوص، قد تكون هذه الحدود تبادلية فيما بينها أو لا. بمعلومية حالة عند الزمن $t=0$، كيف نحصل على حالة النظام في وقت لاحق $|\psi(t)\rangle$ باستخدام حاسوب كمي؟ يمكن فهم الأسّي لمؤثر بسهولة أكبر من خلال متسلسلة تايلور الخاصة به:

$$
e^{-i H t} = 1-iHt-\frac{1}{2}H^2t^2+...
$$

بعض الأسّيات البسيطة جدًا، مثل $e^{iZ}$، يمكن تنفيذها بسهولة على الحواسيب الكمية باستخدام مجموعة مدمجة من البوابات الكمية. معظم الهاملتونيات المثيرة للاهتمام لن تحتوي على حد واحد فقط، بل ستحتوي على حدود كثيرة. لاحظ ما يحدث إذا كان $H = H_1+H_2$:

$$
e^{-i H t} = 1-i(H_1+H_2)t-\frac{1}{2}(H_1+H_2)^2t^2+...
$$

عندما يكون $H_1$ و $H_2$ تبادليين، لدينا الحالة المألوفة (وهي صحيحة أيضًا للأعداد والمتغيرات $a$ و $b$ أدناه):
...
أحد الحلول هو اعتبار زمن صغير جدًا $t$، بحيث يهيمن الحد من الرتبة الأولى في تمدد تايلور. بناءً على هذا الافتراض:
...
هنا $t/r$ هي الشريحة الزمنية (خطوة التطور) التي نختارها. ونتيجة لذلك، يتم إنشاء بوابة يتم تطبيقها $r$ مرة. تؤدي خطوة زمنية أصغر إلى تقريب أكثر دقة. ومع ذلك، يؤدي هذا أيضًا إلى دوائر أعمق مما يؤدي عمليًا إلى تراكم أكبر للأخطاء (وهو مصدر قلق غير قابل للإهمال على الأجهزة الكمية في المدى القريب).

سندرس اليوم التطور الزمني لـ [نموذج Ising](https://en.wikipedia.org/wiki/Ising_model) على شبكات خطية مكونة من $N=2$ و $N=6$ موقعًا. تتكون هذه الشبكات من مصفوفة من اللفات المغزلية $\sigma_i$ التي تتفاعل فقط مع جيرانها الأقرب. يمكن أن يكون لهذه اللفات اتجاهان: $\uparrow$ و $\downarrow$، واللذان يقابلان مغنطة $+1$ و $-1$ على التوالي.

$$
H = - J \sum_{i=0}^{N-2} Z_i Z_{i+1} - h \sum_{i=0}^{N-1} X_i  \text{,}
$$

حيث يصف $J$ طاقة التفاعل، و $h$ مقدار الحقل الخارجي (في الاتجاه x أعلاه، لكننا سنعدل هذا). لنكتب هذا التعبير باستخدام مصفوفات Pauli، مع الأخذ بعين الاعتبار أن الحقل الخارجي له زاوية $\alpha$ بالنسبة للاتجاه العرضي:
...
## 2. تعريف هاملتوني Ising ذو الحقل العرضي {#2-defining-the-transverse-field-ising-hamiltonian}

نعتبر هنا نموذج Ising أحادي البعد ذو الحقل العرضي.

أولاً، سننشئ دالة تأخذ معاملات النظام $N$ و $J$ و $h$، وتُرجع الهاملتوني الخاص بنا كـ `SparsePauliOp`. إن [SparsePauliOp](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.SparsePauliOp) هو تمثيل متناثر لمؤثر بدلالة حدود [Pauli](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Pauli) الموزونة.
### 2.1 النشاط 1 {#21-activity-1}
قم ببناء دالة لإنشاء هاملتوني Ising ذو الحقل العرضي (انظر المعادلة أعلاه) بمعاملات "عدد الكيوبتات" و "معامل J" و "معامل h". جرب ذلك بنفسك باستخدام الأمثلة السابقة. مرر لأسفل لرؤية الحل.
...
سنبدأ بدراسة التطور الزمني لنظام كمي، مع تتبع المغنطة.
نقارن هنا بين نتائج محاكيات Statevector و Matrix Product State.

#### تعريف الهاملتوني {#define-the-hamiltonian}
النظام الذي نعتبره الآن له حجم $N=20$.
...
#### ضبط معاملات محاكاة التطور الزمني {#set-the-parameters-of-the-time-evolution-simulation}
سنعتبر هنا صيغة Lie-Trotter (من الرتبة الأولى).
...
#### تحضير الدائرة الكمية (الحالة الابتدائية) {#prepare-the-quantum-circuit-initial-state}
قم بإنشاء حالة ابتدائية. سنبدأ من الحالة الأساسية، وهي حالة فيرومغناطيسية (الكل لأعلى أو الكل لأسفل). هنا نستخدم مثالاً للكل لأعلى (وهو كل '0').
...
#### تحضير الدائرة الكمية 2 (دائرة واحدة للتطور الزمني) {#prepare-the-quantum-circuit-2-single-circuit-for-time-evolution}
نبني هنا دائرة لخطوة زمنية واحدة باستخدام Lie-Trotter.
صيغة حاصل ضرب Lie (من الرتبة الأولى) مُنفّذة في صنف [LieTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter). تتكون صيغة الرتبة الأولى من التقريب المذكور في المقدمة، حيث يتم تقريب الأسّي المصفوفي لمجموع بحاصل ضرب أسّيات مصفوفية:

$$
e^{H_1+H_2} \approx e^{H_1} e^{H_2}
$$
لنحسب العمليات لهذه الدائرة.
...
#### ضبط المؤثرات المراد قياسها {#set-the-operators-to-be-measured}
لنعرّف *مؤثر المغنطة* $\sum_i Z_i  / N$.
...
سنراقب المغنطة (القيمة المتوقعة لمؤثر المغنطة). سنستخدم محاكيات Statevector و MPS ونقارن النتائج.
...
#### رسم التطور الزمني للقيم المرصودة {#plot-the-time-evolution-of-the-observables}
نرسم القيم المتوقعة التي قسناها مقابل الزمن. تأكد من تطابق نتائج محاكيات statevector و matrix product state.
...
سنبدأ بدراسة التطور الزمني لنظام كمي، مع تتبع الخصائص.
نقارن هنا بين نتائج محاكي Matrix Product State والجهاز الكمي الفعلي.

### 2.2 النشاط 2 {#22-activity-2}

#### تعريف الهاملتوني {#define-the-hamiltonian-1}
النظام الذي نعتبره الآن له حجم $N=70$. لاحظ أن الشروط الأخرى هي نفسها من مسألة الـ 20 كيوبت. جرب ذلك بنفسك؛ مرر لأسفل لرؤية الحل.
...
قم بإنشاء حالة ابتدائية. سنبدأ من الحالة الأساسية، وهي حالة فيرومغناطيسية (الكل لأعلى أو الكل لأسفل). هنا نستخدم مثالاً للكل لأعلى (وهو كل '0'). جرب ذلك بنفسك؛ مرر لأسفل لرؤية الحل.
...
#### تحضير الدائرة الكمية 2 (دائرة واحدة للتطور الزمني) لمسألة الـ 70 كيوبت {#prepare-the-quantum-circuit-2-single-circuit-for-time-evolution-for-the-70-qubit-problem}
نبني هنا دائرة لخطوة زمنية واحدة باستخدام Lie-Trotter.
تمامًا كما في حالة الـ 20 كيوبت، صيغة حاصل ضرب Lie (من الرتبة الأولى) مُنفّذة في صنف [LieTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter). مرة أخرى، تتكون صيغة الرتبة الأولى من التقريب المذكور أعلاه:

$$
e^{H_1+H_2} \approx e^{H_1} e^{H_2}
$$

جرب ذلك بنفسك، بناءً على مثال حالة الـ 20 كيوبت. كما سبق، احسب العمليات لهذه الدائرة.
...
#### ضبط المؤثرات المراد قياسها {#set-the-operators-to-be-measured-1}
نعرّف *مؤثر المغنطة* المماثل تمامًا لحالة الـ 20 كيوبت: $\sum_i Z_i  / N$. جرب ذلك بنفسك عن طريق تعديل حل الـ 20 كيوبت.
...
#### تنفيذ محاكاة التطور الزمني {#perform-time-evolution-simulation-1}
سنراقب المغنطة (القيمة المتوقعة لمؤثر المغنطة). سنستخدم محاكي MPS للحصول على القيمة المرجعية لمقارنة النتائج المحسوبة من الجهاز. لقد استخدمت محاكي MPS سابقًا في هذا البرنامج التعليمي. عدّل ذلك المثال حسب الحاجة ليناسب هذا الحساب الجديد.
...
كما في جميع الدروس السابقة، سننفذ إطار عمل Qiskit patterns. كان تركيز الدرس حتى هذه النقطة على إنشاء الدوائر الكمية الصحيحة لوصف مسألتنا. هذا هو فعليًا الخطوة 1.

#### الخطوة 2: التحسين للأجهزة المستهدفة {#step-2-optimize-for-target-hardware}

نبدأ بتعريف الخلفية المستهدفة.
...
نقوم بتحويل الدوائر وجمعها في قائمة. قد يستغرق هذا بضع دقائق.
...
الآن يجب استخراج القيم المتوقعة من هذه النتائج.
...
سنستخدم هذا للمقارنة أدناه. أولاً، دعنا نرى إذا كان بإمكاننا تحسين دوائرنا بشكل أكبر.

## 3. الحل باستخدام حاسوب كمي حقيقي II {#3-solution-using-a-real-quantum-computer-ii}

لنعد إلى الخطوة 1 من Qiskit patterns، ونرى إذا كان بإمكاننا تقليل عمق دائرتنا.

### 3.1 الخطوة 1. تعيين المسألة إلى دوائر ومؤثرات كمية {#31-step-1-map-the-problem-to-quantum-circuits-and-operators}

#### النشاط 7 {#activity-7}

قم ببناء دائرة تطور زمني. استخدم معرفتك من الدروس السابقة لمحاولة تقليل عمق الدائرة.
...
كان هذا ناجحًا جدًا. يمكننا الآن المتابعة مع بقية خطوات Qiskit patterns.

### 3.2 الخطوة 2. التحسين للأجهزة المستهدفة {#32-step-2-optimize-for-target-hardware}

قم بتحويل الدوائر وجمعها في قائمة. مرة أخرى، قد يستغرق هذا بضع دقائق.
...
عرّف Runtime Estimator وابنِ قائمة PUBs.
...
### 3.3 الخطوة 3. التنفيذ على الأجهزة المستهدفة {#33-step-3-execute-on-target-hardware}
شغّل المهمة.
...
استخرج القيم المتوقعة من النتائج.
...
حوّل القائمة إلى مصفوفات numpy للرسم البياني.
...
لنرسم الآن النتائج ونقارن نتائج الأجهزة (الدائرة الافتراضية والضحلة) مع محاكي MPS. كيف يؤثر الخطأ في الأجهزة الحقيقية على النتائج؟
...
تهانينا! لقد تقدمت خطوة أخرى في رحلتك نحو الحوسبة الكمية على نطاق المنفعة. لم يتبقَ سوى درس واحد!
