...
### الخطوة 3: التنفيذ باستخدام أوليات IBM Quantum {#step-3-execute-using-ibm-quantum-primitives}
...
### الخطوة 1: تعيين المدخلات الكلاسيكية إلى مسألة كمومية {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
### الخطوة 2: تحسين المسألة للتنفيذ الكمي {#step-2-optimize-problem-for-quantum-execution-1}
...
### الخطوة 3: التنفيذ باستخدام أوليات IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-1}
...
### الخطوة 4: المعالجة اللاحقة وإرجاع النتيجة بالصيغة الكلاسيكية {#step-4-post-process-return-result-in-classical-format-1}
...
لنفترض أن شخصًا جديدًا في الحوسبة الكمومية يحاول استخدام دارة كمومية ذات عمق ثنائي القيوبت عالٍ جدًا، ولا يستخدم تخفيف الأخطاء. لنفترض كذلك أن هذا يؤدي إلى معدل خطأ 10% على كل كيوبت. إذا كانت مدخلة مصفوفة النواة الحقيقية (الخالية من الأخطاء) المقابلة لهذه الدارة كبيرة جدًا، لنقل 1.0، فما احتمال قياس جميع الـ 10 كيوبتات في الحالة التي يكون فيها كل كيوبت $|0\rangle$؟
...
### الخطوة 1: تعيين المدخلات الكلاسيكية إلى مسألة كمية {#step-1-map-classical-inputs-to-a-quantum-problem-2}
...
### الخطوة 4: المعالجة اللاحقة وإرجاع النتيجة بصيغة كلاسيكية {#step-4-post-process-return-result-in-classical-format-2}
...
### الخطوة 2: تحسين المسألة للتنفيذ الكمومي {#step-2-optimize-problem-for-quantum-execution-2}
...
في المهام صغيرة النطاق، غالبًا ما يعيد مدير التمريرات المُعدّ مسبقًا نفس الدارة بنفس العمق، بشكل موثوق. لكن في الدارات الكبيرة والمعقدة جدًا، يمكن لمدير التمريرات أن يعيد دارات مُترجَمة مختلفة في كل مرة يعمل فيها. هذا لأنه يستخدم أساليب استكشافية، ولأن الدارات الكبيرة جدًا سيكون لها مشهد معقد من التحسينات الممكنة. من المفيد غالبًا الترجمة عدة مرات وأخذ أضحل دارة. هذا يُدخل فقط نفقات عامة كلاسيكية وقد يحسّن النتائج من الحاسوب الكمومي بشكل كبير.
...
هنا يمكنك أن ترى وجود بعض التباين في عمق البوابة الإجمالي مع تمريرات ترجمة مختلفة. دارتنا ليست عميقة/واسعة بما فيه الكفاية بعد لرؤية تباين في أعماق التمرير الثنائي القيوبت. سنستخدم `transpiled_qcs[1]`، الذي له عمق 60، أقل قليلًا من عمق أعمق دارة تم الحصول عليها، والتي كانت 77.

```python
overlap_ibm = transpiled_qcs[1]
```

### الخطوة 3: التنفيذ باستخدام أوليات IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-2}

بينما نتوسع أكثر نحو النفعية، لن تكون المحاكيات مفيدة. تظهر هنا فقط الصياغة النحوية للحواسيب الكمومية الحقيقية.
...
### الخطوة 4: المعالجة اللاحقة، إرجاع النتيجة بتنسيق كلاسيكي {#step-4-post-process-return-result-in-classical-format-3}
