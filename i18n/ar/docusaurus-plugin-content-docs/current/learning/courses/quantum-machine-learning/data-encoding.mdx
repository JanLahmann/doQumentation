...
هناك العديد من الإجابات الممكنة. إليك شفرة تطبع بعض الخطوات على طول الطريق:
...
هل ترى مزايا لترميز السعة على ترميز الأساس؟ إذا كان الأمر كذلك، اشرح.
...
مثال شائع ومفيد على الترميز مع التشابك الكمي هو دائرة [`efficient_su2`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.EfficientSU2) في Qiskit. والمثير للإعجاب أن هذه الدائرة تستطيع، على سبيل المثال، ترميز 8 ميزات على 2 كيوبت فقط. دعنا نشاهد ذلك ثم نحاول أن نفهم كيف يكون هذا ممكنًا.
...
نرمّز هنا متجه البيانات $\vec{x}$ ذا 12 ميزة على دائرة `efficient_su2` ذات 3 كيوبتات، مستخدمين كل بوابة معلمة لترميز ميزة مختلفة.
...
كما أُشير سابقًا، ثمة مقايضة: الدوائر التي تحتوي على كيوبتات أكثر أو تكرارات أكثر لطبقات التشابك والدوران قد تخزّن معاملات أكثر، لكنها تفعل ذلك بعمق دائرة أكبر. سنعود إلى أعماق بعض خرائط الميزات المدمجة لاحقًا.
...
### خريطة ميزات $Z$ {#z-feature-map}

يمكن تفسير خريطة ميزات $Z$ (ZFM) باعتبارها امتدادًا طبيعيًا لترميز الطور. تتكوّن ZFM من طبقات متناوبة من بوابات الكيوبت الواحد: طبقات بوابة هادامار وطبقات بوابة الطور. ليكن متجه البيانات $\vec{x}$ يحتوي على $N$ ميزة. تُمثَّل الدائرة الكمومية التي تنفّذ تعيين الميزات كمؤثر أحادي يعمل على الحالة الابتدائية:

$$
\mathscr{U}_{\text{ZFM}}(\vec{x})|0\rangle^{\otimes N}=|\phi(\vec{x})\rangle
$$
حيث $|0\rangle^{\otimes N}$ هي الحالة الأرضية لـ $N$ كيوبت. يُستخدم هذا الترميز للتناسق مع المرجع [\[4\]](#references) لـ Havlicek et al. تُعيَّن ميزات البيانات $x_i$ بصورة واحد-لواحد مع الكيوبتات المقابلة. على سبيل المثال، إن كانت لديك 8 ميزات في متجه بيانات، فستستخدم 8 كيوبتات. تتكوّن دائرة ZFM من $r$ تكرار لدائرة فرعية مؤلفة من طبقات بوابة هادامار وطبقات بوابة الطور. طبقة هادامار تتألف من بوابة هادامار تعمل على كل كيوبت في سجلّ $n$-كيوبت، $H \otimes H \otimes \dots \otimes H = H^{\otimes n}$، ضمن المرحلة ذاتها من الخوارزمية. ينطبق هذا الوصف أيضًا على طبقة بوابة الطور حيث يتأثر الكيوبت $i$ بـ $P(\vec{x}_i)$. كل بوابة $P$ لها ميزة واحدة كمعطى، لكن طبقة بوابة الطور ($P(\vec{x}_1)\otimes\ldots P(\vec{x}_k)\otimes\ldots P(\vec{x}_N)$) هي دالة لمتجه البيانات. المؤثر الأحادي الكامل لدائرة ZFM بتكرار واحد هو:
$$
...
$$
ميزات البيانات، $x_k$، تُعيَّن إلى بوابات الطور بالطريقة ذاتها في جميع التكرارات الـ $r$. حالة خريطة ميزات ZFM هي حالة جداء وهي فعّالة للمحاكاة الكلاسيكية[\[4\]](#references).

للبدء بمثال صغير، نبرمج دائرة ZFM ذات كيوبتين في Qiskit ونرسمها لإظهار البنية البسيطة للدائرة. في هذا المثال، يُنفَّذ تكرار واحد، $r=1$، مع متجه البيانات $\vec{x} = \left(\textstyle\frac{1}{2}\pi, \textstyle\frac{1}{3}\pi\right)$. لاحظ أن هذا مكتوب بالترتيب المعياري لمتجه في Python، أي أن العنصر $0$ هو $\textstyle\frac{1}{2}\pi.$ لنا حرية ترميز هذه الميزة $0$ على الكيوبت $0$، أو على الكيوبت $N$. مجددًا، لا يمكن دائمًا وجود تعيين 1:1 واحد من ترتيب الميزات إلى ترتيب الكيوبتات، إذ تُرمَّز أعداد مختلفة من الميزات على كل كيوبت في خرائط ميزات مختلفة. ومجددًا ما يهم هو أن نكون واعين بمكان ترميز كل ميزة. عند تقديم قائمة معاملات لخريطة ميزات $Z$، ستُرمَّز الميزة 0 من القائمة على الكيوبت الأقل أهمية الذي يحتوي بوابة معلمة، أي الكيوبت 0. لذا سنتبع هذا الاصطلاح حين نفعل ذلك يدويًا. سنرمّز $\textstyle\frac{1}{2}\pi$ على الكيوبت $0$، و$\textstyle\frac{1}{3}\pi$ على الكيوبت $1$.
...
إحدى طرق فهم المؤثر الأحادي أعلاه هي من خلال القياس مع المُنتشر (propagator) في نظام فيزيائي. المؤثر الأحادي أعلاه هو مؤثر تطور أحادي، $\exp(it\mathcal{H})$، لهاملتوني، $\mathcal{H}$، مشابه لنموذج إيزنغ، حيث يُستبدل معامل الزمن، $t$، بقيم البيانات لدفع التطور. توسيع هذا المؤثر الأحادي يعطي دارة PFM. يمكن تفسير الروابط المتشابكة في $S$ على أنها اقترانات إيزنغ في شبكة مغزلية.
لنعتبر مثالًا على مؤثري باولي $Y$ و$XX$ اللذين يمثلان تلك التفاعلات من نوع إيزنغ. يوفر Qiskit فئة `pauli_feature_map` لإنشاء PFM مع اختيار بوابات أحادية و$n$-كيوبت، والتي في هذا المثال ستُمرَّر كسلاسل باولي `‘Y’` و`‘XX’`. عادةً، تكون $n$ تساوي 1 أو 2 للتفاعلات أحادية القيوبت وثنائية القيوبت على التوالي. مخطط التشابك هو "خطي"، مما يعني أن الكيوبتات المتجاورة فقط في الدارة الكمومية متصلة. لاحظ أن هذا لا يتوافق مع الكيوبتات المتجاورة على الحاسوب الكمومي نفسه، حيث أن هذه الدارة الكمومية هي طبقة تجريد.
...
تطبق الشفرة أدناه هذه الخرائط الميزاتية الأربع المدمجة على ترميز مجموعة ميزات، وترسم عمق الكيوبتين الثنائي للدارة الناتجة. بما أن معدلات خطأ الكيوبتين الثنائي أعلى بكثير من معدلات خطأ البوابة أحادية الكيوبت، قد يكون من المعقول أن يكون المرء مهتمًا أكثر بعمق البوابات ثنائية القيوبت. في الشفرة أدناه، نحصل على عدد جميع البوابات في دارة عن طريق تفكيك الدارة أولاً ثم استخدام count_ops()، كما هو موضح أدناه. هنا البوابات ثنائية القيوبت التي نهتم بها هي بوابات 'cx':
