...
كتوضيح أخير لكيفية استخدام خوارزمية غروفر كـ subroutine، تخيل أننا نحتاج حالات كمية بعدد معين من الـ `1`s في تمثيل سلسلة البتات. هذا شائع في المواقف التي تتضمن قوانين الحفظ. مثلاً، في سياق الكيمياء الكمية، يُعيَّن الـ `1` لكيوبت لشغل مدار إلكتروني. لأن الشحنة يجب أن تتحفظ، لا بد أن يظل العدد الكلي للـ `1`s ثابتاً. القيود مثل هذه شائعة لدرجة أنها سُمّيت باسم خاص: **قيود وزن هامينج**، والعدد الكلي للـ `1`s في الحالة يُسمى **وزن هامينج**.
