...
#### $\mathbb{Z}_N$ و$\mathbb{Z}_N^*$ {#mathbbz-n-and-mathbbz-n}
...
لنعد كتابة تأثير $M_2$ على الحالات بالنظام الثنائي:
...
يمكن تحقيق كل من هذه الإجراءات بواسطة عملية SWAP بسيطة. $M_2|0001\rangle$ يتحقق بتبديل حالتي الكيوبتين $0$ و $1$. $M_2|0010\rangle$ يتحقق بتبديل حالتي الكيوبتين $1$ و $2$. وهكذا. لذا، يمكننا تفكيك مصفوفة $M_2$ إلى السلسلة التالية من بوابات SWAP:

$$
M_2 = SWAP(0,1)SWAP(1,2)SWAP(2,3)
$$

مع تذكّر أن المؤثرات تعمل من اليمين إلى اليسار، دعنا نتحقق من أن لهذا التأثير الذي نريده على كل حالة من الحالات:
...
يمكننا الآن كتابة كود الدائرة المكافئة لهذا المؤثر في Qiskit.
...
تستخدم خوارزمية QPE بوابة $U$ متحكماً بها. لذا، بما أن لدينا الآن دائرة $M_2$، نحتاج إلى جعلها دائرة $M_2$ *متحكماً بها*:
...
الآن لدينا بوابة $U$ المتحكم بها. لكن من أجل تشغيل خوارزمية تقدير الطور الكمومي، سنحتاج إلى $U^2$ متحكماً بها، و$U^4$ متحكماً بها، وصولاً إلى $U^{2^{m-1}}$ متحكماً بها، حيث $m$ هو عدد الكيوبتات المستخدمة لتقدير الطور. كلما زاد عدد الكيوبتات، زادت دقة تقدير الطور. سنستخدم $m=8$ كيوبتات تحكم لإجراء تقدير الطور. لذا، نحتاج إلى:

$$
M_{a^{2^k}}|y\rangle \equiv |a^{2^k} y \bmod N \rangle
$$

حيث الفهرس $k$، مع $0 \le k \le m-1 = 7$، يقابل كيوبت التحكم. الآن دعنا نحسب $a^{2^k}\bmod N $ لكل قيمة من $k$:
...
**ملاحظة:** هذا التبسيط يعمل هنا فقط لأن رتبة $2 \bmod 15 $ هي $4$. بمجرد أن يصبح $k=2$ (أي $2^k = 4$)، تكون كل قوة لاحقة للمؤثر هي عملية الهوية. بشكل عام، بالنسبة لأعداد $N$ أكبر أو اختيارات مختلفة لـ $a$، لا يمكنك تخطي بناء القوى الأعلى. هذا أحد الأسباب التي تجعل هذا يُعتبر *مثالاً توضيحياً بسيطاً*: الأعداد الصغيرة تسمح باختصارات لن تعمل في الحالات الأكبر.
...
وكما في السابق، نجعله مؤثر $M_4$ *متحكماً به*:
...
الآن، يمكننا جمع كل ذلك لإيجاد رتبة $2\bmod 15$ باستخدام دائرة كمومية، عبر تقدير الطور:
...
2. باتباع الإجراء الذي أوضحناه هنا لتحليل 15 إلى عوامل، حاول الآن تحليل 21 إلى عوامل.
