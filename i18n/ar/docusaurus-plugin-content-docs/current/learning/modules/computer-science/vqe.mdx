...
محلل القيم الذاتية الكمومي التباديلي (VQE) هو الأسلوب الرئيسي الذي سنستخدمه لاستكشاف عملية $H+H = H_2$، وهنا سنلقي نظرة على ما هو VQE وكيف يعمل. لكن لنتوقف أولاً ونتأمل شيئاً بالغ الأهمية من خلال سؤال المراجعة التالي.
...
على سبيل المثال، تحتاج محاكاة الكافيين باستخدام أبسط مجموعة أساسية (STO-3G) إلى $10^{48}$ بت، وهو عدد أكبر بكثير من إجمالي عدد النجوم في الكون المرئي ($10^{24}$) [المرجع 2]. يستطيع الحاسوب الكمومي وصف المدارات الإلكترونية للكافيين باستخدام 160 كيوبت.
...
#### (كمومي) الـ Ansatz: دالة الموجة التجريبية (كيفية بناء حالة كمومية بسيطة باستخدام دائرة كمومية) {#quantum-ansatz-the-trial-wave-function-how-to-build-a-trivial-quantum-state-with-a-quantum-circuit}
...
#### (كلاسيكي) المحسّن: الضبط الدقيق للدائرة الكمومية {#classical-optimizer-fine-tuning-the-quantum-circuit}
...
لنبدأ بتحميل بعض الحزم الضرورية، بما فيها أوليات IBM Quantum. سنختار أيضاً الحاسوب الكمومي الأقل ازدحاماً المتاح لنا.
...
### الخطوة 1: رسم خريطة المسألة إلى دوائر كمومية ومؤثرات {#step-1-map-the-problem-to-quantum-circuits-and-operators}
...
نلاحظ أن أول ansatz يُعيد حالات كمومية موزعة على شكل حلقة على كرة بلوخ. هذا منطقي، لأننا أعطينا الـ ansatz معاملاً دورانياً واحداً فقط. لذا يستطيع إنتاج حالات مُدارة حول محور واحد فقط. الانطلاق من النقطة $(0,0,1)$ والدوران حول محور واحد سيُنتج دائماً حلقة. لنتحقق الآن من ansatz الثاني، الذي يضم بوابتي دوران متعامدتين - `Rx` و `Rz`.
...
### الخطوة 1: تحويل المسألة إلى دوائر كمومية ومؤثرات {#step-1-map-the-problem-to-quantum-circuits-and-operators-1}
...
### الخطوة 2: التحسين للأجهزة المستهدفة {#step-2-optimize-for-target-hardware-1}
...
### الخطوة 3: التنفيذ على الأجهزة المستهدفة {#step-3-execute-on-target-hardware-1}
...
### الخطوة 4: ما بعد المعالجة {#step-4-post-process-1}
...
## حساب طاقة التفاعل لـ $H + H = H_2$ {#compute-reaction-energy-of-h-h-h-2}

الآن دعنا نستخدم ما حصلنا عليه! لقد استخدمت VQE، محلل القيم الذاتية الكمومي التباديلي، لحساب طاقة الحالة الأساسية لذرة $H$ ولجزيء $H_2$. ما تبقى هو استخدام القيم المحسوبة للحصول على طاقة التفاعل لعملية $H+H=H_2$.
...
تجدر الإشارة هنا إلى أن طاقة التفاعل لهذه العملية لها قيمة سالبة، مما يعني أن الطاقة تُطلق من خلال العملية، وأن الجزيء الناتج له طاقة أقل من ذرتين منفردتين.
6. الخلاصة
...
بعد ذلك، استكشفنا VQE، وهي خوارزمية واسعة الاستخدام لحساب طاقة الحالة الأساسية لنظام كمومي. شغّلنا كوداً لحساب طاقات الحالة الأساسية للهيدروجين الذري ($H$) وجزيء الهيدروجين ($H_2$). على وجه الخصوص، تعلمنا أنه من الضروري الحصول على الهاملتوني الجزيئي المناسب للنظام وتحويله إلى شكل قابل للتنفيذ على الحاسوب الكمومي. رأينا أيضاً أن الـ ansatz، وهو دائرة كمومية مُعلمَّة، ضروري لإعداد حالات كمومية تجريبية داخل VQE، وناقشنا أهمية اختيار بنية دائرة ansatz مناسبة. تعلمنا أيضاً أن VQE تعتمد على عملية تحسين تكرارية باستخدام حاسوب كلاسيكي، توجه الدائرة الكمومية للعثور على حالة الطاقة الأدنى، ورأينا كيف تتقارب العملية.
...
2. مجموعة الأساس هي مجموعة من الدوال تُستخدم لتقريب الدوال الموجية الكمومية.

3. VQE هي خوارزمية كمومية تُستخدم لحل معادلة شرودنغر بشكل دقيق لهاملتوني معين.

4. في VQE، تُستخدم دائرة كمومية مُعلمَّة (أنساتز) لإعداد دوال موجية تجريبية.
...
3. كم عدد الحالات الكمومية التي تم توليدها في هذا الدفتر لمقارنة الـ ansatz؟
...
هنا سنلقي نظرة على هذه التحديات باستخدام VQE لجزيء $H_2$، مع نوعين مختلفين من الـ ansätze.
...
يواصل الباحثون تطوير منهجيات متقدمة متنوعة يمكنها استخدام الحواسيب الكمومية لمشاكل الكيمياء. يمكنك الوصول إلى مجموعة متنوعة من المواد التعليمية في [IBM Quantum Learning](/learning).
