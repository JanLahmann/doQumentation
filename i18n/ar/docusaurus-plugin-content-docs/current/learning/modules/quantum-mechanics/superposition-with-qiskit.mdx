...
إذًا، تطبيق بوابة هادامار يعادل فعليًا رمي عملة. تمامًا كما رمينا العملة 1000 مرة لدراسة إحصائيات وقوعها صورة للأعلى أو للأسفل، يمكننا فعل شيء مماثل على Qiskit بـ"عملتنا الكمية". يمكننا استخدام أداة أساسية تُسمى `Sampler`، التي تكرر دائرة كمية عدة مرات لأخذ عينات من إحصائيات الحالة الناتجة.

أولاً، نُحمّل IBM Quantum Compute Service والأدوات الأساسية، ثم نختار واجهة خلفية لتشغيل الدائرة عليها.
...
لقد ألغى بوابة هادامارد الثانية أثر الأولى، وعُدنا إلى الحالة 0 التي بدأنا منها! يحدث هذا لأن بوابة هادامارد تحوّل كلاً من حالتي $|0\rangle$ و$|1\rangle$ إلى حالات متشابهة، لكن بأطوار متعاكسة. بعد بوابة هادامارد الثانية، نرى أن حدود $|0\rangle$ تتداخل بنّاءً، لكن حدود $|1\rangle$ تتداخل هداماً وتُلغي بعضها البعض.

لنستكشف أكثر: يمكننا تغيير الطور باستخدام بوابة «PHASE». إذاً لنُطبق الآن بوابة هادامارد لإنشاء التراكب، ثم نُطبق إزاحة طور مقدارها $\pi$ راديان، ثم نُطبق بوابة هادامارد الثانية:
...
والآن يُقاس الكيوبت في الحالة 1 بدلاً من 0.

إذاً، حتى من هذه التجارب القليلة والبسيطة، يمكنك أن ترى بالفعل الفرق الجذري الذي يمكن أن يُحدثه هذا الطور على دائرة كمومية. يمكن أن يكون دقيقاً وصعب الرؤية في البداية. في تجربتنا الأولى، حيث ألقينا ببساطة عملتنا الكمومية ببوابة هادامارد وقسنا نتيجة الرمي، لم يكشف ذلك عن أي أثر للطور. لم نكتشف الفرق الذي يمكن أن يُحدثه الطور إلا عندما تعمّقنا أكثر. يمكن أن يجعل نفس البوابة الكمومية لها أثر معاكس تماماً، حرفياً، على كيوبت.
...
هذا المتجه الذي يشير في اتجاه "وجه" العملة يُعرف بـ"متجه بلوخ". رسميًا، يمكن تمثيل حالة كيوبت اعتباطية (معزولة) بمتجه يقع على سطح كرة نصف قطرها 1 بإحداثيات $(r, \theta, \phi)$. وبكتابتها بدلالة هذه الإحداثيات، تكون حالة الكيوبت:
...
- يمكن تصوّر حالة كيوبت منفرد معزول كنقطة على ما يُسمى "كرة بلوخ"، حيث تحدد السعات النسبية لمركّبتَي $|0\rangle$ و$|1\rangle$ الزاوية القطبية $\theta$، والأطوار النسبية بين المركّبتَين تحدد الزاوية السمتية $\phi$.
...
1. استخدم Qiskit لإنشاء دائرة تُحوّل الحالة $|0\rangle$ إلى الحالة $\frac{\sqrt{3}}{2}|0\rangle + \frac{1}{2}e^{i \frac{5\pi}{6}}|1\rangle$
