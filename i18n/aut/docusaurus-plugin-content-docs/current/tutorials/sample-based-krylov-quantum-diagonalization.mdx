---
title: "Sample-basierte Krylov-Quantendiagonalisierung von'm fermionischen Gittermodell"
sidebar_label: "Sample-basierte Krylov-Quantendiagonalisierung von'm fermionischen Gittermodell"
description: "Verwendung vom sample-basierten Quantendiagonalisierungs-Algorithmus zur Simulation vom Single-Impurity-Anderson-Modell auf verrauschter Quantenhardware."
...
Das Tutorial folgt einem ähnlichen Workflow wie das verwandte Tutorial [Sample-basierte Quantendiagonalisierung von'm Chemie-Hamiltonian](/tutorials/sample-based-quantum-diagonalization). Der wesentliche Unterschied liegt aber darin, wie die Quantenschaltkreise aufgebaut san. Das andere Tutorial verwendet einen heuristischen Variationsansatz, der für Chemie-Hamiltonians mit potenziell Millionen von Wechselwirkungstermen attraktiv ist. Dieses Tutorial hingegen verwendet Schaltkreise, die Zeitentwicklung durch den Hamiltonian approximieren. Solche Schaltkreise können tief sein, was diesen Ansatz besser für Anwendungen auf Gittermodelle geeignet macht. Die von diesen Schaltkreisen vorbereiteten Zustandsvektoren bilden die Basis für einen [Krylov-Unterraum](https://en.wikipedia.org/wiki/Krylov_subspace), und daher konvergiert der Algorithmus nachweislich und effizient zum Grundzustand, unter geeigneten Annahmen.

Der in diesem Tutorial verwendete Ansatz lässt sich als Kombination der in SQD und [Krylov-Quantendiagonalisierung (KQD)](https://arxiv.org/abs/2407.14431) verwendeten Techniken betrachten. Der kombinierte Ansatz wird manchmal als sample-basierte Krylov-Quantendiagonalisierung (SQKD) bezeichnet. Schau dir [Krylov-Quantendiagonalisierung von Gitter-Hamiltonians](/tutorials/krylov-quantum-diagonalization) für ein Tutorial zur KQD-Methode an.

Dieses Tutorial basiert auf der Arbeit [„Quantum-Centric Algorithm for Sample-Based Krylov Diagonalization"](https://arxiv.org/abs/2501.09702), auf die du für weitere Details verweisen kannst.
...
Dabei san $c^\dagger_{\mathbf{j},\sigma}/c_{\mathbf{j},\sigma}$ die fermionischen Erzeugungs- und Vernichtungsoperatoren für die $\mathbf{j}^{\textrm{th}}$ Bad-Stelle mit Spin $\sigma$, $\hat{d}^\dagger_{\sigma}/\hat{d}_{\sigma}$ san die Erzeugungs- und Vernichtungsoperatoren für den Verunreinigungsmodus, und $\hat{n}_{d\sigma} = \hat{d}^\dagger_{\sigma} \hat{d}_{\sigma}$. $t$, $U$ und $V$ san reelle Zahlen, die die Hüpf-, Onsite- und Hybridisierungswechselwirkungen beschreiben, und $\varepsilon$ ist eine reelle Zahl, die das chemische Potential angibt.
...
wobei $H_1$ aus Einteilchen-Termen besteht, die quadratisch in den fermionischen Erzeugungs- und Vernichtungsoperatoren san, und $H_2$ aus Zweiteilchen-Termen, die quartisch san. Für das SIAM gilt:
$$
...
Aufgrund der näherungsweisen Translationssymmetrie in $H_\textrm{bath}$ erwarten wir nicht, dass der Grundzustand in der Ortsbasis dünn besetzt ist (die Orbitalbasis, in der der Hamiltonian oben angegeben ist). Die Leistung von SQD ist nur dann garantiert, wenn der Grundzustand dünn besetzt ist, das heißt, er hat signifikantes Gewicht nur auf einer kleinen Anzahl von Rechenbasiszuständen. Um die Dünnbesetztheit des Grundzustands zu verbessern, führen wir die Simulation in der Orbitalbasis durch, in der $H_\textrm{bath}$ diagonal ist. Diese Basis nennen wir die *Impulsbasis*. Da $H_\textrm{bath}$ ein quadratischer fermionischer Hamiltonian ist, kann er effizient durch eine Orbitalrotation diagonalisiert werden.

### Näherungsweise Zeitentwicklung durch den Hamiltonian {#approximate-time-evolution-by-the-hamiltonian}
...
## Schritt 1: Problem auf'n Quantenschaltkreis abbilden {#step-1-map-problem-to-a-quantum-circuit}
Zuerst erzeugen wir den SIAM-Hamiltonian in der Ortsbasis. Der Hamiltonian wird durch die Matrix $h_{pq}$ und den Tensor $h_{pqrs}$ dargestellt. Dann rotieren wir ihn in die Impulsbasis. In der Ortsbasis platzieren wir die Verunreinigung an der ersten Stelle. Wenn wir aber in die Impulsbasis rotieren, verschieben wir die Verunreinigung an eine zentrale Stelle, um Wechselwirkungen mit anderen Orbitalen zu erleichtern.
...
Für jede Spinspezies ist der Anfangszustand $\ket{\psi_0}$ durch die Überlagerung aller möglichen Anregungen der drei Elektronen, die dem Fermi-Niveau am nächsten san, in die 4 nächsten leeren Moden ausgehend vom Zustand $|00\cdots 0011 \cdots 11\rangle$ gegeben, und wird durch die Anwendung von sieben [XXPlusYYGate](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.XXPlusYYGate)s realisiert.
...
Für eine detailliertere Beschreibung dieses Modells und wie die Schaltkreise entworfen san, schau dir [„Quantum-Centric Algorithm for Sample-Based Krylov Diagonalization"](https://arxiv.org/abs/2501.09702) an.
...
Jetzt, wo wir die Schaltkreise erstellt haben, können wir sie für eine Zielhardware optimieren. Wir wählen den am wenigsten ausgelasteten QPU mit mindestens 127 Qubits. Schau dir die [Qiskit IBM&reg; Runtime-Dokumentation](/guides/get-started-with-primitives#get-started-with-sampler) für mehr Informationen an.
...
## Schritt 3: Mit Qiskit-Primitiven ausführen {#step-3-execute-by-using-qiskit-primitives}
...
## Schritt 4: Nachbearbeiten und Ergebnis ins gewünschte klassische Format zurückgeben {#step-4-post-process-and-return-result-to-desired-classical-format}
Jetzt führen wir den SQD-Algorithmus mit der Funktion `diagonalize_fermionic_hamiltonian` aus. Schau dir die [API-Dokumentation](https://qiskit.github.io/qiskit-addon-sqd/apidocs/qiskit_addon_sqd.fermion.html#qiskit_addon_sqd.fermion.diagonalize_fermionic_hamiltonian) für Erklärungen zu den Argumenten dieser Funktion an.
...
### Energie verifizieren {#verifying-the-energy}
