---
title: "Kompilierungsmethoden für Hamiltonian-Simulations-Schaltkreise"
sidebar_label: "Kompilierungsmethoden für Hamiltonian-Simulations-Schaltkreise"
description: "Dieses Tutorial gibt a vergleichende Übersicht über drei Kompilierungsmethoden in Qiskit für Hamiltonian-Simulations-Workloads."
...
### Was du lernst {#what-you-will-learn}
...
- Qiskit SDK v1.3 oder später, mit [Visualisierungs](https://docs.quantum.ibm.com/api/qiskit/visualization)-Support
...
#### Analyse der SU2-Schaltkreis-Kompilierungsergebnisse {#analysis-of-su2-circuit-compilation-results}
...
#### Verwendete Hamiltonians im Benchmark {#hamiltonians-used-in-the-benchmark}

Die in diesem Benchmark verwendeten Hamiltonians beschreiben paarweise Wechselwirkungen zwischen Qubits, einschließlich Terme wie $ZZ$, $XX$ und $YY$. Diese Hamiltonians werden häufig in der Quantenchemie, der Physik kondensierter Materie und der Materialwissenschaft verwendet, wo sie Systeme interagierender Teilchen modellieren.

Als Referenz können Benutzer a breitere Menge von Hamiltonians in diesem Papier erkunden: [Efficient Hamiltonian Simulation on Noisy Quantum Devices](https://arxiv.org/pdf/2306.13126).

#### Benchmark-Quelle: Hamlib und Benchpress {#benchmark-source-hamlib-and-benchpress}

Die in diesem Benchmark verwendeten Schaltkreise stammen aus dem [Hamlib-Benchmark-Repository](https://github.com/SRI-International/QC-App-Oriented-Benchmarks/tree/master/hamlib), das realistische Hamiltonian-Simulations-Workloads enthält.

Diese selben Schaltkreise wurden zuvor mit [Benchpress](https://github.com/Qiskit/benchpress) gebencht, a Open-Source-Framework zur Bewertung der Quantentranspilierungsleistung. Durch die Verwendung dieses standardisierten Schaltkreissatzes können wir die Effektivität verschiedener Kompilierungsstrategien bei repräsentativen Simulationsproblemen direkt vergleichen.
...
#### Analyse der Hamiltonian-Schaltkreis-Kompilierungsergebnisse {#analysis-of-hamiltonian-circuit-compilation-results}
...
- **KI-Transpiler** ist typischerweise zuverlässig und liefert tiefenoptimierte Schaltkreise, bringt aber Trade-offs in der Laufzeit mit sich und hat auch andere Einschränkungen, darunter unterstützte Coupling-Maps und Synthesefähigkeiten. Diese sind in der [Qiskit-Transpiler-Service-Dokumentation](/guides/qiskit-transpiler-service) beschrieben.

- In manchen Fällen, insbesondere bei sehr großen oder hardware-spezifischen Schaltkreisen, ist der KI-Transpiler möglicherweise nicht so effektiv. In diesen Fällen bleibt der Standard-SABRE-Transpiler extrem zuverlässig und kann durch Anpassung seiner Parameter weiter optimiert werden (sieh dir das [SABRE-Optimierungs-Tutorial](/tutorials/transpilation-optimizations-with-sabre) an).
