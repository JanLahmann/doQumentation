...
Nagbibigay ang Qiskit ng parameter, $\alpha$, sa Pauli feature maps para kontrolin ang scaling ng Pauli rotations.

$$
U(\bar{x}) = \exp\left(i \alpha \sum_{S\subseteq[n]} \phi_S(\bar{x}) \prod_{i \in S} \sigma_i \right)
$$

Ang default na halaga ng $\alpha$ ay $2$. Sa pamamagitan ng pag-optimize ng halaga nito sa interval, halimbawa, $[0,4],$ mas mahusay na maiaalign ng isa ang isang quantum kernel sa data.
