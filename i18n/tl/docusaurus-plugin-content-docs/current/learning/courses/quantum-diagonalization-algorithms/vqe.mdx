...
Kailangan natin ng paraan para tantiyahin ang expectation value ng ating Hamiltonian sa isang partikular na variational state $|\psi(\vec{\theta})\rangle$. Kung direkta nating masusukat ang buong operator $H$, ito ay magiging kasingsimple ng paggawa ng maraming (halimbawa $N$) sukat at pagkuha ng average ng mga nasukat na halaga:

$$
\langle \psi(\vec{\theta})|H|\psi(\vec{\theta})\rangle _N \approx \frac{1}{N}\sum_{j=1}^N {E_j}
$$
Dito, ang simbolong $\approx$ ay nagpapaalala sa atin na ang expectation value na ito ay magiging eksaktong tama lamang sa limitasyon kung saan $N\rightarrow \infty$. Ngunit sa libu-libong sukat na ginagawa sa isang circuit, ang sampling error ng expectation value ay medyo mababa. May iba pang konsiderasyon tulad ng noise na nagiging isyu para sa napakaprecise na mga kalkulasyon.

Gayunpaman, sa pangkalahatan ay hindi posible na sukatin ang $H$ nang sabay-sabay. Ang $H$ ay maaaring maglaman ng maraming non-commuting na Pauli X, Y, at Z operator. Kaya ang Hamiltonian ay dapat hatiin sa mga grupo ng mga operator na maaaring sabay na sukatin, at ang bawat ganitong grupo ay dapat tantiyahin nang hiwalay, at ang mga resulta ay pinagsama upang makakuha ng isang expectation value. Babalik tayo rito nang mas detalyado sa susunod na aralin, kapag tinatalakay natin ang scaling ng mga classical at quantum na pamamaraan. Ang kumplikasyon sa pagsukat na ito ay isa sa mga dahilan kaya kailangan natin ng napaka-episyenteng code para sa pagsasagawa ng ganitong pagtatantya. Sa araling ito at higit pa, gagamitin natin ang IBM Quantum primitive Estimator para sa layuning ito.
...
## 3. Halimbawang Hamiltonian {#3-example-hamiltonian}
...
### 3.1 Hakbang 1: I-map ang problema sa mga quantum circuit at operator {#31-step-1-map-the-problem-to-quantum-circuits-and-operators}
...
#### 3.3.1 Pag-ulat ng mga halaga {#331-reporting-out-values}
...
### 3.4 Hakbang 4: I-post-process ang mga resulta {#34-step-4-post-process-results}
...
## 4. Relasyon ng VQE sa iba pang mga pamamaraan {#4-vqes-relationship-to-other-methods}
...
### 4.1 Ang mga kalakasan at kahinaan ng VQE {#41-the-strengths-and-weaknesses-of-vqe}
...
### 4.2 Relasyon sa iba pang mga approach {#42-relationship-to-other-approaches}
