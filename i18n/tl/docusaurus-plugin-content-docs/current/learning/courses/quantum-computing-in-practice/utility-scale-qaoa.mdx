...
Para maisulat ang QUBO problem bilang Ising Hamiltonian, ang kailangan lang nating gawin ay gumawa ng simpleng pagbabago ng mga variable mula sa $x \in \{0, 1\}^n$ tungo sa $z \in \{-1, 1\}^n$:
...
Una, tandaan na maaari nating isulat muli ang ating QUBO expression bilang kabuuan ng mga matrix term:

$$
x^TQx = \sum_{ij}Q_{ij}x_ix_j
$$

Ang pag-apply ng pagbabago ng mga variable ay nagbubunga ng

$$
\sum_{ij}Q_{ij}(\frac{1-z_i}{2})(\frac{1-z_j}{2}) = \sum_{ij}\frac{Q_{ij}}{4}(1 - z_i - z_j + z_iz_j)
$$

na maaaring muling ayusin gaya ng sumusunod.

$$
\sum_{ij}\frac{Q_{ij}}{4}z_iz_j - \frac{Q_{ij}}{4}z_i - \frac{Q_{ij}}{4}z_j + \frac{Q_{ij}}{4}
$$

Maaari nating alisin ang constant term dahil hindi ito makakaapekto kung aling $z$ ang nagpapaliit sa QUBO problem. Bukod pa rito, maaari nating i-multiply ang expression na ito ng apat para mabawi ang orihinal na halaga ng $Q$, dahil hindi rin ito nakakaapekto sa pinakamainam na pagpili ng $z$.

$$
\sum_{ij}Q_{ij}z_iz_j - Q_{ij}z_i - Q_{ij}z_j = z^TQz + (-\sum_{ij} Q_{ij}z_i + Q_{ij}z_j)
$$

Sa pamamagitan ng pagpapangkat ng magkatulad na linear terms, maaari nating i-define ang ating linear coefficient vector $b$ gamit ang equation

$$
b_i = -\sum_{j} Q_{ij} + Q_{ji}
$$

na kumukumpleto sa transformation ng mga variable tungo sa $z \in \{-1, 1\}^n$ pagkatapos i-substitute sa naunang equation.
...
Sa huli, ang minimization ng QUBO expression ay katulad ng minimization ng expression na ito, kung saan ang $b$ ay isang tunay na scalar coefficient:

$$
\min_{x\in\{0,1\}^n} x^TQx\Longleftrightarrow \min_{z\in\{-1,1\}^n}z^TQz + b^Tz
$$

Isusulat muli nang bahagya at mayroon na tayong cost function Hamiltonian, kung saan ang minimum ng expression ay kumakatawan sa ground state at ang `Z` ay ang Pauli Z operator:
