...
Kung mukhang baliktad ito sa iyo (lalo na ang sign sa pagitan ng $X$ at $iY$), tingnan ang opsyonal na note sa ibaba tungkol sa mga convention kung aling state $|0\rangle$ o $|1\rangle$ ang tumutugma sa "occupied" at alin sa "unoccupied".
...
Medyo standardized na ito ngayon sa konteksto ng quantum computing, at sa mga built-in mapper tulad ng Qiskit addon `FermionicMapper` na nagtatakda ng
$$
\begin{aligned}
|0\rangle & = \text{ empty}\\
...
Katumbas nito, ang operator na lumilikha ng elektron na sumasakop sa orbital $k$ (kinakatawan ng qubit $q_k$) ay dapat kumilos bilang:

$$
O_k |q_N,...,q_k=0,...,q_0\rangle = c |q_N,...,q_k=1,...,q_0\rangle
$$

Ngayon tingnan natin ang mga kahihinatnan ng pagpiling iyon. Tandaan na sa mga standard bases na ginagamit sa quantum computing,
...
at sa katulad na paraan, maaari nating buuin ang buong set:
...
Malinaw dito na sa pagpili ng tila makatwirang convention na $|0\rangle$ ay tumutugma sa walang elektron, at $|1\rangle$ ay tumutugma sa isang elektron, natatapos tayo sa nakalilitong sitwasyon kung saan lumalabas ang $\sigma^+$ sa annihilation operator, at $\sigma^-$ sa creation operator. Kaya ang convention sa `Qiskit Fermions` (kung saan ang $|0\rangle$ ay tumutugma sa occupied, at $|1\rangle$ ay tumutugma sa unoccupied) ay nagreresulta sa fermionic creation operator $f_k^\dagger$ na naglalaman ng $\sigma_k^-$, at ang annihilation operator $f_k$ ay naglalaman ng $\sigma_k^+$.

Kung makakita ka ng mga source na may kabaligtaran na sign sa pagitan ng $X$ at $iY$ na terms sa creation at annihilation operators, malamang na ginagamit nila ang kabaligtarang convention kung aling state ang tumutugma sa isang orbital na puno.
...
kung saan ang $\sigma^+$ ay ang Pauli operator na $X + iY$ at ang $\sigma^{-}$ ay $X - iY.$ Kapag mayroon na kaming Hamiltonian na nakasulat sa format na ito, tapos na ang mahirap na bahagi ng mapping stage, at madali na itong masusulat sa isang circuit sa mga Pauli operator.
