...
## Bagong uri ng resource: QPU (Quantum Processing Unit) {#new-type-of-resource-qpu-quantum-processing-unit}
...
Sa quantum computing, tinutukoy ang isang "layer" ng mga gate na maaaring lahat isagawa nang sabay-sabay. Sa maraming aplikasyon, kapaki-pakinabang na magsagawa ng set ng mga rotation sa lahat ng qubit at pagkatapos ay mga entangling gate sa pagitan ng mga pares ng qubit. Sa mga kontekstong ito, tinutukoy ang isang "rotation layer" (isang layer ng mga gate tulad ng `R_x`, `R_y`, at/o `R_z`) at isang "entangling layer" (tulad ng isa na may mga `CNOT` gate). Ang bilang ng mga layer sa isang circuit ay ang "circuit depth", isang mahalagang sukatan dahil ang higit na depth ay nangangahulugan ng higit pang mga layer ng compounding noise at errors.
...
Sa quantum computing, ang mga kakayahan ng isang quantum system ay kadalasang inilalarawan gamit ang tatlong pangunahing dimensyon: scale, quality, at speed. Ang IBM Quantum ay tumutukoy sa mga dimensyong ito gamit ang tatlong hardware metrics na maaaring ilapat sa iba't ibang quantum-computing modalities: programmable qubits (scale), qubit operations (quality), at maximum circuits per second (speed). Ang mga metrics na ito ay nagbibigay ng high-level view ng computational capability at efficiency ng isang system sa iba't ibang hardware implementation.

* **Scale** ay sinusukat ng bilang ng programmable qubits. Ang programmable qubits ay ang mga qubit na maaaring direktang kontrolin, manipulahin, sukatin, at isama ng mga user sa mga quantum algorithm.


* **Quality** ay sinusukat ng qubit operations (QuOps), isang metric na naglalarawan kung ilan sa mga pinaka-computationally demanding na operations ang maaaring maisagawa nang maaasahan ng isang system bago madaig ng naipong errors ang computation. Mas mataas na kakayahan sa QuOps ay karaniwang nagbibigay-daan sa mas malalim na circuit at mas sopistikadong mga quantum algorithm.


* **Speed** ay sinusukat ng maximum circuits per second, na kilala rin bilang circuit throughput. Sinusukat ng metric na ito kung ilang kapaki-pakinabang na quantum circuit ang maaaring isagawa ng isang system sa paglipas ng panahon at nagbibigay ito ng indikasyon ng pangkalahatang computational efficiency.


Ang mga karagdagang hardware metrics ay kadalasang ginagamit upang tukuyin ang mga tiyak na aspeto ng pag-uugali ng processor. Halimbawa, ang EPLG (errors per layered gate) at layer-fidelity measurements ay nagbibigay ng insight sa kalidad ng gate at pag-ipon ng error, habang ang CLOPS (Circuit Layer Operations Per Second) ay sumusukat sa bilis ng pagsasagawa ng isang standardized quantum benchmarking workload. Ang mga metrics na ito ay nananatiling kapaki-pakinabang para sa pagsusuri ng partikular na workloads at mga detalye ng implementation, ngunit ang programmable qubits, qubit operations, at maximum circuits per second ay nagbibigay ng mas malawak na framework para sa paghahambing ng mga quantum system.

### Mga pagkakaiba sa pagitan ng quantum at klasikal na sukatan {#differences-between-quantum-and-classical-metrics}

Ang mga classical computing system ay kadalasang tinutukoy gamit ang mga metric tulad ng FLOPS (floating-point operations per second), memory capacity, memory bandwidth, at processor clock rates. May ilang maluwag na analogies na maaaring gawin sa pagitan ng mga ito at ng mga metric para sa quantum computers na ipinakilala sa itaas, ngunit mahalaga ang mga pagkakaiba sa metrics.

Ang maximum circuits per second ay maaaring tingnan bilang isang maluwag na analog sa throughput-oriented performance metrics sa classical computing dahil sinusukat nito kung gaano karaming kapaki-pakinabang na trabaho ang maaaring gawin ng isang quantum system sa paglipas ng panahon. Hindi tulad ng FLOPS, gayunpaman, sinusukat nito ang pagsasagawa ng mga quantum circuit sa halip na mga arithmetic operations.

Ang CLOPS ay minsang inihahambing sa mga classical throughput benchmarks tulad ng FLOPS, ngunit mas mainam itong intindihin bilang isang workload-specific benchmark sa halip na isang universal na sukatan ng quantum-computing performance. Ang EPLG at mga kaugnay na fidelity measurements ay nagbibigay ng insight kung gaano katumpak ang isang processor sa pagsasagawa ng mga layer ng quantum gates. Ang mga metric na ito ay kapaki-pakinabang para sa pag-unawa sa mga tiyak na katangian ng hardware, habang ang pangunahing scale-quality-speed framework ay nagbibigay ng mas pangkalahatang batayan para sa paghahambing ng mga quantum system. Maraming paglalarawan ng performance ng quantum computer ang matatagpuan sa [Compute resources](https://quantum.cloud.ibm.com/computers) page sa IBM Quantum Platform. Tandaan na habang ang programmable qubit count ay isang fixed na numero para sa isang partikular na QPU, ang mga speed at quality metrics ay maaaring bahagyang magbago-bago sa paglipas ng panahon dahil sa mga calibration update, maintenance, operational conditions, at mga pagpapahusay sa nakapaligid na hardware at software infrastructure.

Sa kabuuan, ang mga complementary metrics na ito ay tumutulong sa paggabay ng resource allocation, scheduling, at application design sa mga quantum computing system.

Sa maraming kaso, ang buong quantum system ay tinatrato bilang isang yunit. Ngunit kapag ang isang gawain ay nalampas ang kapasidad ng isang yunit — uman man sa bilang ng programmable qubit, circuit depth, o circuit throughput — ang mga pamamaraan tulad ng circuit cutting/knitting ay maaaring gamitin. Ang circuit cutting ang proseso ng paghahati ng malalaking quantum na gawain sa mas maliliit at napapamahalaang sub-gawain na maaaring ipamahagi sa maraming quantum chip, na nagbibigay-daan sa scalable quantum computation sa kabila ng mga limitasyon ng hardware. Ang circuit knitting naman ang prosesong sumusunod sa circuit cutting — ang klasikal na post-processing na hakbang na "nag-ti-knit" o pinagsasama ang mga resulta mula sa mas maliliit na subcircuit.
...
* Ang mga job ay isinusumite ng gumagamit, gamit ang IBM Quantum Compute Service, at karaniwang gumagamit ng isang IBM Quantum primitive, tulad ng `Sampler`, `Estimator`, o iba pa.
...
* Sa pagsusumite ng job sa pamamagitan ng Quantum Compute, ang job ay pumapasok sa queue para sa napiling QPU. Walang kontrol ang gumagamit sa oras ng queue, bagama't maaaring naiiba ito para sa mga nakatuong sistema.
...
Ipagpalagay na sinusubukan mong maghanap sa isang unsorted database upang mahanap ang isang element na tatawagin nating 'target'. Para sa bawat isa sa mga sumusunod na aksyon, sabihin kung aling stage ng resource management ito tumutugma:

(a) Pagpaplano
(b) Pagkontrol/pamamahala
(c) Paglalaan/pag-schedule,

(b) Tinitiyak na ang paghahanap sa target sa isang GPU ay itititigil ang proseso sa ibang mga GPU upang mapalaya ang mga ito para sa susunod na problema

(c) Paghahati-hati sa search space sa mga rehiyon para sa bawat isa sa iyong (halimbawa 10) mga GPU na maghahanap
...
(c) Paglalaan/pagsasaayos ng schedule
...
(d) Oo, maaari. Ginagamit nito ang quantum partition.
