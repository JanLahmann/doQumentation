...
# Quantum algorithms: Pagtatantya ng phase {#quantum-algorithms-phase-estimation}
...
$$
Sinisimulan natin ang $\vert\psi\rangle = \vert1\rangle$ ng eigenvector ng $T$ gate sa pamamagitan ng pag-apply ng $X$ gate:
...
Susunod, ina-apply natin ang mga Hadamard gate sa mga counting qubit:
...
Isinasagawa natin ang mga controlled unitary operation:
...
Ina-apply natin ang inverse quantum Fourier transformation upang i-convert ang estado ng counting register, pagkatapos ay sinusukat ang counting register:
...
Maaari nating i-simulate gamit ang Aer simulator:
...
Nakikita natin na nakakakuha tayo ng isang resulta (`001`) nang may katiyakan, na nagsasalin sa decimal: `1`. Kailangan na nating hatiin ang ating resulta (`1`) sa $2^n$ upang makuha ang $\theta$:

$$
\theta = \frac{1}{2^3} = \frac{1}{8}
$$
 Pinapayagan tayo ng algorithm ni Shor na i-factorize ang isang numero sa pamamagitan ng paggawa ng circuit na may hindi kilalang $\theta$ at pagkuha ng $\theta$.
...
## 4. Pagpapatakbo gamit ang IBM Quantum Sampler Primitive {#4-execution-using-the-ibm-quantum-sampler-primitive}
