...
Kaya, ang isang arbitrary na quantum state ay kinakatawan din bilang
...
Mula dito, makikita natin na ang estado ng isang quantum bit ay unit vector sa isang two-dimensional complex inner product space na may orthonormal basis na $|0\rangle$ at $|1\rangle$. Ito ay normalized sa 1.
...
Ang isang single-qubit quantum state ay kinakatawan din bilang
...
Ang superposition state na ito ay napakakaraniwan at mahalaga, kaya't binigyan ito ng sarili nitong simbolo:

$$
|+\rangle \equiv \frac{1}{\sqrt{2}}(|0\rangle+|1\rangle).
$$

Sa pag-apply ng $H$ gate sa $|0\rangle$, nakalikha tayo ng superposition ng $|0\rangle$ at $|1\rangle$ kung saan ang pagsukat sa computational basis (kasama ang z sa Bloch sphere picture) ay magbibigay sa iyo ng bawat estado na may pantay na posibilidad.

#### Estado na $|-\rangle$ {#rangle-state}

Maaaring naisip mo na mayroong kaukulang $|-\rangle$ state:
$$
|-\rangle \equiv \frac{|0\rangle -|1\rangle}{\sqrt{2}}.
$$
Upang likhain ang estadong ito, unang i-apply ang X gate upang makagawa ng $|1\rangle$, pagkatapos ay i-apply ang H gate.
...
Kung wala kang `qiskit-ibm-runtime`, kailangan mo itong i-install muna. Para sa karagdagang impormasyon, [tingnan ang API reference](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-service).
