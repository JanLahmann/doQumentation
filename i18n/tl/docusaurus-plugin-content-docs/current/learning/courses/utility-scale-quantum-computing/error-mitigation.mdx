---
title: "Pagpapagaan ng error"
sidebar_label: "Pagpapagaan ng error"
description: "Sa araling ito, titingnan natin ang ingay at kung paano ito mapagagaan sa mga quantum computer."
...
[I-download ang pdf](https://ibm.ent.box.com/public/static/a0zgies7bh91hm2lwev9o0bfeybxc6n6.zip) ng orihinal na lektura. Tandaan na maaaring maging deprecated ang ilang code snippet dahil mga static na imahe ang mga ito.
...
## 2. Maingay na simulation nang walang pagpapagaan ng error {#2-noisy-simulation-without-error-mitigation}
...
1. Bumuo ng noise model
2. Bumuo ng maingay na sampler (simulator) gamit ang noise model
3. Patakbuhin ang quantum circuit sa maingay na sampler
...
```
### 2.1 Bumuo ng test circuit {#21-build-a-test-circuit}
...
### 2.2 Bumuo ng noise model {#22-build-a-noise-model}
...
### 2.3 Bumuo ng maingay na sampler gamit ang noise model {#23-build-a-noisy-sampler-with-the-noise-model}
...
### 2.4 Patakbuhin ang mga quantum circuit sa maingay na sampler {#24-run-quantum-circuits-on-the-noisy-sampler}
...
### 2.5 I-plot ang mga resulta {#25-plot-results}
...
### 2.6 Ideal na simulation {#26-ideal-simulation}
...
### 2.8 Mas makatotohanang maingay na simulation {#28-more-realistic-noisy-simulation}
...
## 3. Tunay na quantum computation na may pagpapagaan ng error {#3-real-quantum-computation-with-error-mitigation}

Sa bahaging ito, ipinakita namin kung paano makakuha ng mga resulta na may pagpapagaan ng error (mga expected value) gamit ang Qiskit Estimator.
Isinasaalang-alang natin ang 6-qubit Trotterized circuits para sa pag-simulate ng time evolution ng one-dimensional Ising model at tinitingnan kung paano lumalaki ang error kaugnay ng bilang ng mga time step.
...
### 3.1 Bumuo ng mga circuit {#31-build-circuits}
...
Para malaman nang maaga ang ideal na output, gumagamit tayo ng compute-uncompute circuits na binubuo ng unang yugto kung saan inilalapat ang orihinal na circuit $U$, at pangalawang yugto kung saan binaliktad ito $U^\dagger$.
Tandaan na ang ideal na resulta ng ganitong mga circuit ay palaging magiging ang input state $|000000\rangle$, na may trivial na expected value para sa anumang Pauli observable, halimbawa, $\langle IIIIIZ \rangle = 1$.
...
Tandaan: Tulad ng makikita sa itaas, ang circuit na may $k$ time steps ay magtataglay ng $4k$ na two-qubit gate layers.
...
### 3.2 I-transpile ang mga circuit {#32-transpile-the-circuits}
...
### 3.4 I-plot ang mga resulta {#34-plot-results}
...
## 4. (Opsyonal) I-customize ang mga opsyon ng pagpapagaan ng error {#4-optional-customize-error-mitigation-options}
...
- [I-configure ang pagpapagaan ng error](/guides/error-mitigation-and-suppression-techniques)
- [Panimula sa mga opsyon](/guides/runtime-options-overview)
