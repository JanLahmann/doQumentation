...
Ang `optimization_level` ay may apat na opsyon, mula 0 hanggang 3. Kung mas mataas ang optimization level, mas malaki ang computational effort na ginagastos upang i-optimize ang circuit. Ang level 0 ay walang isinasagawang optimization at ginagawa lang ang minimal na halaga ng trabaho upang gumana ang circuit sa napiling backend. Ang level 3 ay gumagastos ng pinakamalaking effort (at karaniwan ay runtime) upang subukang i-optimize ang circuit. Ang level 1 ang default na optimization level.
Tina-transpile natin ang circuit nang walang optimization (`optimization_level=0`) at may optimization (`optimization_level=2`).
Makikita natin ang malaking pagkakaiba sa haba ng circuit ng mga na-transpile na circuit.
...
Susunod, ihahambing natin ang mga resulta ng pagpapatakbo ng dalawang magkaibang paraan ng pag-synthesize ng 5-qubit na GHZ state ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$) na mga preparation circuit.
...
Ang ginawa natin sa bagong circuit ay maaaring ituring bilang manual na "coupling constraint-aware" circuit synthesis. Sa madaling salita: manual na paglutas ng circuit synthesis at circuit mapping nang sabay-sabay.
...
Tina-transpile natin ang circuit nang walang optimization (`optimization_level=0`) at may optimization (`optimization_level=2`).
Tulad ng makikita mo, may malaking pagkakaiba sa haba ng circuit ng mga na-transpile na circuit.
