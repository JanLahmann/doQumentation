...
### Teoretikal na layout {#theoretical-layout-1}
...
Ang positibong value na $\beta_0$ ay dapat mas malaki kaysa sa $\lambda_1-\lambda_0$.

Ipinapakilala nito ang isang bagong cost function na maaaring ituring bilang isang constrained problem, kung saan mini-minimize natin ang $C_\text{VQE}(\vec{\theta}) = \langle \psi(\vec{\theta}) | \hat{H} | \psi(\vec{\theta})\rangle$ na napapailalim sa constraint na dapat orthogonal ang estado sa naunang nakuhang $|\psi(\vec{\theta^0})\rangle$, na kung saan ang $\beta_0$ ay kumikilos bilang isang penalty term kung hindi natutugunan ang constraint.

Bilang alternatibo, ang bagong problemang ito ay maaaring bigyang-kahulugan bilang pagpapatakbo ng VQE sa bagong observable:
...
Ipagpalagay na ang solusyon sa bagong problema ay $|\psi(\vec{\theta^1})\rangle$, ang expected value ng $\hat{H}$ (hindi $\hat{H_1}$) ay dapat na $ \langle \psi(\vec{\theta^1}) | \hat{H} | \psi(\vec{\theta^1})\rangle = \lambda_1$.
Upang makuha ang ikatlong eigenvalue na $\lambda_2$, ang cost function na io-optimize ay:
...
kung saan ang $\beta_1$ ay isang positibong constant na malaki nang sapat upang ipatupad ang orthogonality ng solution state sa parehong $|\psi(\vec{\theta^0})\rangle$ at $|\psi(\vec{\theta^1})\rangle$. Pinaparusahan nito ang mga estado sa search space na hindi nakakatugon sa requirement na ito, epektibong ihinihigpit ang search space. Kaya, ang optimal na solusyon ng bagong problema ay dapat ang eigenstate na kaukulan sa $\lambda_2$.

Tulad ng naunang kaso, ang bagong problemang ito ay maaari ring bigyang-kahulugan bilang VQE na may observable:
...
Kung ang solusyon sa bagong problemang ito ay $|\psi(\vec{\theta^2})\rangle$, ang expected value ng $\hat{H}$ (hindi $\hat{H_2}$) ay dapat na $ \langle \psi(\vec{\theta^2}) | \hat{H} | \psi(\vec{\theta^2})\rangle = \lambda_2$.
Sa katulad na paraan, upang makuha ang ika-$k$ na eigenvalue na $\lambda_{k-1}$, mini-minimize mo ang cost function:
...
Ang problemang ito ay katumbas ng isang VQE na may observable:
...
Tulad ng makikita mo sa proseso, upang makuha ang ika-$k$ na eigenvalue, kailangan mo ng (approximate) mga eigenstate ng naunang $k-1$ eigenvalue, kaya kailangan mong patakbuhin ang VQE nang kabuuang $k$ na beses. Kaya, ang cost function ng VQD ay ang sumusunod:
...
### Teoretikal na layout {#theoretical-layout-2}
...
- Gumamit ng classical optimizer upang piliin ang susunod na set ng mga parameter na $\Theta_{i+1}$.

- Ulitin ang prosesong ito hanggang maabot ang convergence.


### Implementasyon {#implementation-1}
...
### Teoretikal na layout {#theoretical-layout-3}
...
- Kunin ang sample ng cost function na $C(\vec\theta) := a_0 + \sum_{k=1}^S[a_k\cos(k\theta)+ b_k\sin(k\theta)]$ nang hindi bababa sa $T$ na beses.
