...
- Una, matututunan natin ang tungkol sa [IBM Quantum primitives](/guides/primitives)

- I-define ang isang *cost function* na $C(\vec\theta)$. Ito ay isang function na naka-depende sa problema na nagtatakda ng layunin ng problema na i-minimize (o i-maximize) ng optimizer

- Pagtatakda ng estratehiya sa pagsukat gamit ang IBM Quantum primitives upang ma-optimize ang bilis kumpara sa katumpakan
...
* `Estimator`: Binibigyan ng isang quantum observable na $\hat{H}$ at isang estado na $|\psi\rangle$, kinakalkula ng primitive na ito ang expectation value ng $\hat{H}$.
...
Kinakalkula ng `Estimator` primitive ang expectation value ng isang observable $\hat{H}$ para sa isang quantum state $|\psi\rangle$; kung saan ang mga probabilidad ng observable ay maaaring ipahayag bilang $p_\lambda = |\langle\lambda|\psi\rangle|^2$, na ang $|\lambda\rangle$ ay ang mga eigenstate ng observable $\hat{H}$. Ang expectation value ay tinutukoy bilang average ng lahat ng posibleng resulta $\lambda$ (ibig sabihin, ang mga eigenvalue ng observable) ng isang pagsukat ng estadong $|\psi\rangle$, na binibigyang-timbang ng kaukulang mga probabilidad:

$$
\langle\hat{H}\rangle_\psi := \sum_\lambda p_\lambda \lambda = \langle \psi | \hat{H} | \psi \rangle
$$

Gayunpaman, ang pagkalkula ng expectation value ng isang observable ay hindi laging posible, dahil kadalasan ay hindi natin alam ang eigenbasis nito. Ang IBM Quantum [`Estimator`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2) ay gumagamit ng kumplikadong algebraic na proseso upang tantiyahin ang expectation value sa isang tunay na quantum device sa pamamagitan ng paghahati-hati ng observable sa kombinasyon ng ibang mga observable na alam natin ang eigenbasis.

Sa mas simpleng salita, hinahati-hati ng `Estimator` ang anumang observable na hindi nito alam kung paano sukatin sa mas simple, nasusukat na mga observable na tinatawag na <DefinitionTooltip definition="Set of matrices commonly used in quantum computing to represent and manipulate quantum states, consisting of the identity matrix and the three Pauli matrices (X, Y, and Z).">Pauli operators</DefinitionTooltip>.
Anumang operator ay maaaring ipahayag bilang kombinasyon ng $4^n$ Pauli operators.
...
kung saan ang $n$ ay ang bilang ng mga qubit, $k \equiv k_{n-1} \cdots k_0$ para sa $k_l \in \mathbb{Z}_4 \equiv \{0, 1, 2, 3\}$ (ibig sabihin, mga integer base $4$), at $(\sigma_0, \sigma_1, \sigma_2, \sigma_3) := (I, X, Y, Z)$.

Pagkatapos isagawa ang decomposition na ito, kinukuha ng `Estimator` ang isang bagong circuit $V_k|\psi\rangle$ para sa bawat observable $\hat{P}_k$ (mula sa orihinal na circuit), upang epektibong *i-diagonalize* ang Pauli observable sa computational basis at sukatin ito. Madali nating masusukat ang mga Pauli observable dahil alam na natin ang $V_k$ nang maaga, na hindi karaniwang totoo para sa ibang mga observable.

Para sa bawat $\hat{P}_{k}$, pinapatakbo ng `Estimator` ang kaukulang circuit sa isang quantum device nang maraming beses, sinusukat ang output state sa computational basis, at kinakalkula ang probabilidad na $p_{kj}$ ng pagkuha ng bawat posibleng output na $j$. Pagkatapos ay hinahanap nito ang eigenvalue na $\lambda_{kj}$ ng $P_k$ na kaukulan sa bawat output $j$, pinaparami ng $w_k$, at idinaragdag ang lahat ng resulta upang makuha ang expected value ng observable $\hat{H}$ para sa ibinigay na estadong $|\psi\rangle$.

$$
\langle\hat{H}\rangle_\psi =
\sum_{k=0}^{4^n-1} w_k \sum_{j=0}^{2^n-1}p_{kj} \lambda_{kj},
$$
Dahil hindi praktikal ang pagkalkula ng expectation value ng $4^n$ Paulis (ibig sabihin, tumataas nang exponential), ang `Estimator` ay maaari lamang maging episyente kapag ang malaking bilang ng $w_k$ ay zero (ibig sabihin, *sparse* na Pauli decomposition sa halip na *dense*). Pormal nating sasabihin na, upang ang computation na ito ay maging *episyenteng malulutas*, ang bilang ng mga non-zero na termino ay dapat tumaas nang polynomial lamang kasama ng bilang ng mga qubit na $n$: $\hat{H} = \sum^{\text{Poly}(n)}_k w_k \hat{P}_k$.
...
### Patnubay na halimbawa para sa pagkalkula ng mga expectation value {#guided-example-to-calculate-expectation-values}
...
na may sumusunod na teoretikal na expectation value na $\langle\hat{H}\rangle_+ = \langle+|\hat{H}|+\rangle = 2$.

Dahil hindi natin alam kung paano sukatin ang observable na ito, hindi natin direktang makalkula ang expectation value nito, at kailangan nating muling ipahayag ito bilang $\langle\hat{H}\rangle_+ = 2\langle X \rangle_+ - \langle Z \rangle_+ $. Maaaring ipakita na ito ay nagbibigay ng parehong resulta sa pamamagitan ng pagpansin na $\langle+|X|+\rangle = 1$, at $\langle+|Z|+\rangle = 0$.
...
Ipinapahiwatig nito na ang expectation value ay maaaring muling isulat bilang:
...
Dahil kung ang isang sistema ay nasa estado na $|\phi\rangle = V |\psi\rangle$ ang posibilidad ng pagsukat ng $| j\rangle$ ay $p_j = |\langle j|\phi \rangle|^2$, ang expectation value sa itaas ay maaaring ipahayag bilang:
...
\end{array}
$$
Kaya't isulat natin muli ang $\hat{H}$ ayon sa mga Pauli at identity:
...
kung saan ang $k = \sum_{l=0}^{n-1} 4^l k_l \equiv k_{n-1}...k_0$ para sa $k_{n-1},...,k_0\in \{0,1,2,3\}$ (ibig sabihin, base $4$), at $\hat{P}_{k} := \sigma_{k_{n-1}}\otimes ... \otimes \sigma_{k_0}$:
...
kung saan ang $V_k := V_{k_{n-1}}\otimes ... \otimes V_{k_0}$ at $\Lambda_k := \Lambda_{k_{n-1}}\otimes ... \otimes \Lambda_{k_0}$, gayong: $\hat{P_k}=V_k^\dagger \Lambda_k V_k$.
...
Sa kabuuan, magsasagawa tayo ng classical optimization loop ngunit maaasahan ang pagsusuri ng cost function sa isang quantum computer. Mula sa perspektibong ito, maaaring isipin ng isa ang optimization bilang isang purong klasikal na gawain kung saan tinatawagan natin ang ilang <DefinitionTooltip definition="Isang haka-hakang device o software component na gumaganap ng isang tiyak na function, ngunit hindi alam ang panloob na gawain nito. Alam lamang ng isang user ang mga input at output ng black box, at wala siyang kaalaman kung paano pinoproseso ng black box ang mga input upang makabuo ng mga output.">black-box quantum oracle</DefinitionTooltip> sa tuwing kailangang suriin ng optimizer ang cost function.
...
Magpapatuloy na tayo sa pagpapatakbo sa tunay na quantum computer. Pansinin ang mga pagbabago sa syntax. Ang mga hakbang na kinasasangkutan ng pass_manager ay tatalakayin pa sa susunod na halimbawa. Ang isang hakbang na partikular na mahalaga sa mga variational algorithm ay ang paggamit ng isang IBM Quantum Compute Service session. Ang pagsisimula ng isang session ay nagbibigay-daan sa iyo na magpatakbo ng maraming iteration ng isang variational algorithm nang hindi naghihintay sa bagong pila sa bawat oras na ma-update ang mga parameter. Mahalaga ito kung mahaba ang mga oras ng pila at/o kailangan ng maraming iteration. Tandaan na ang mga Open Plan user ay hindi makakagamit ng Quantum Compute session. Kung wala kang access sa mga session, maaari mong bawasan ang bilang ng mga iteration na isusumite mo sa isang pagkakataon, at i-save ang pinakabagong mga parameter para gamitin sa mga susunod na run. Kung magsumite ka ng masyadong maraming iteration o makatagpo ng masyadong mahabang oras ng pila, maaari kang makatagpo ng error code 1217, na tumutukoy sa matagal na pagkaantala sa pagitan ng mga pagsusumite ng job.
...
Ang maximum cut (max-cut) problem ay isang combinatorial optimization problem na kinapapalooban ng paghahati sa mga vertex ng isang graph sa dalawang disjoint na set upang mapalaki ang bilang ng mga edge sa pagitan ng dalawang set. Sa mas pormal na paraan, ibinigay ang isang undirected graph na $G=(V,E)$, kung saan ang $V$ ay ang set ng mga vertex at ang $E$ ay ang set ng mga edge, hinihiling ng max-cut problem na hatiin ang mga vertex sa dalawang disjoint na subset, $S$ at $T$, upang mapalaki ang bilang ng mga edge na may isang endpoint sa $S$ at ang isa naman ay sa $T$.

Maaari nating gamitin ang max-cut upang lutasin ang iba't ibang problema kabilang ang: clustering, network design, phase transitions, at iba pa. Magsisimula tayo sa paglikha ng isang problem graph:
...
Ang problemang ito ay maaaring ipahayag bilang isang binary optimization problem. Para sa bawat node $0 \leq i < n$, kung saan ang $n$ ay ang bilang ng mga node ng graph (sa kasong ito $n=4$), isasaalang-alang natin ang binary variable na $x_i$. Ang variable na ito ay magkakaroon ng value na $1$ kung ang node $i$ ay isa sa mga grupong lalagyan natin ng label na $1$ at $0$ kung ito ay nasa ibang grupo, na lalagyan natin ng label na $0$. Tatawagin din nating $w_{ij}$ (element $(i,j)$ ng adjacency matrix $w$) ang timbang ng edge na nagmumula sa node $i$ patungo sa node $j$. Dahil undirected ang graph, $w_{ij}=w_{ji}$. Pagkatapos ay maaari nating i-formulate ang ating problema bilang pagmamaximize ng sumusunod na cost function:
...
Upang lutasin ang problemang ito gamit ang isang quantum computer, ipapahayag natin ang cost function bilang expected value ng isang observable. Gayunpaman, ang mga observable na natural na tinatanggap ng Qiskit ay binubuo ng mga Pauli operator, na may mga eigenvalue na $1$ at $-1$ sa halip na $0$ at $1$. Kaya't gagawa tayo ng sumusunod na pagbabago ng variable:

Kung saan $\vec{x}=(x_0,x_1,\cdots ,x_{n-1})$. Maaari nating gamitin ang adjacency matrix na $w$ upang komportableng ma-access ang mga timbang ng lahat ng edge. Ito ay gagamitin upang makuha ang ating cost function:
...
Bukod dito, ang natural na tendensya ng isang quantum computer ay ang paghahanap ng minima (karaniwang ang pinakamababang enerhiya) sa halip na maxima kaya't sa halip na i-maximize ang $C(\vec{z})$ ay i-minimize natin ang:

$$
-C(\vec{z}) =  \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2} z_iz_j -  \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2}
$$

Ngayong mayroon na tayong cost function na i-minimize na ang mga variable ay maaaring magkaroon ng values na $-1$ at $1$, maaari na tayong gumawa ng sumusunod na analohiya sa Pauli $Z$:

$$
z_i \equiv Z_i = \overbrace{I}^{n-1}\otimes ... \otimes \overbrace{Z}^{i} \otimes ... \otimes \overbrace{I}^{0}
$$

Sa madaling salita, ang variable na $z_i$ ay magiging katumbas ng isang $Z$ gate na kumikilos sa qubit $i$. Bukod dito:
...
Kaya ang observable na isasaalang-alang natin ay:

$$
\hat{H} = \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2} Z_iZ_j
$$

kung saan kailangan nating idagdag ang independent na termino pagkatapos:

$$
\texttt{offset} = - \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2}
$$
Ang operator ay isang linear combination ng mga termino na may Z operator sa mga node na konektado ng isang edge (tandaan na ang ika-0 qubit ay pinakamalayo sa kanan): $IIZZ + IZIZ + IZZI + ZIIZ + ZZII$. Kapag naitayo na ang operator, ang ansatz para sa QAOA algorithm ay madaling maitatayo gamit ang `QAOAAnsatz` circuit mula sa Qiskit circuit library.
...
Dahil direktang tumatanggap ang IBM Quantum Estimator ng Hamiltonian at parameterized ansatz at nagbabalik ng kinakailangang enerhiya, ang cost function para sa isang QAOA instance ay medyo simple:
...
Maaari nating gamitin ang mga opsyon ng error suppression at error mitigation ng IBM Quantum primitive upang tugunan ang ingay at mapakinabangan ang utility ng mga quantum computer ngayon.

### Error Suppression {#error-suppression}

Ang [Error suppression](/guides/error-mitigation-and-suppression-techniques) ay tumutukoy sa mga teknik na ginagamit upang i-optimize at baguhin ang isang circuit sa panahon ng compilation upang mabawasan ang mga error. Ito ay isang pangunahing teknik sa pangangasiwa ng error na karaniwang nagdudulot ng ilang klasikal na pre-processing na <DefinitionTooltip definition="Mga karagdagang gastos na dulot ng mga bagong teknik, kumpara sa isang base implementation.">overhead</DefinitionTooltip> sa kabuuang runtime. Ang overhead ay kinabibilangan ng pag-transpile ng mga circuit upang mapatakbo sa quantum hardware sa pamamagitan ng:
...
Para sa kursong ito, susuriin natin ang mga modelong ito ng error mitigation sa mataas na antas upang ilarawan ang error mitigation na maaaring isagawa ng IBM Quantum primitives nang hindi nangangailangan ng buong detalye ng implementasyon.
...
### Paggamit ng mga opsyon sa Quantum Compute para sa mitigation at suppression {#using-the-options-in-quantum-compute-for-mitigation-and-suppression}

Narito kung paano kalkulahin ang isang expectation value habang ginagamit ang error mitigation at suppression sa IBM Quantum Compute Service. Maaari tayong gumamit ng eksaktong parehong circuit at observable tulad ng dati, ngunit sa pagkakataong ito ay pinanatili ang optimization level sa level 2, at ngayon ay ina-tune ang _resilience_ o ang mga teknik ng error mitigation na ginagamit. Ang prosesong ito ng error mitigation ay nangyayari nang maraming beses sa buong optimization loop.
...
Tulad ng dati, maaari nating i-plot ang mga nagresultang expectation value bilang function ng phase angle para sa tatlong antas ng error mitigation na ginamit. Sa kabila ng kahirapan, makikita ng isa na ang error mitigation ay bahagyang nagpapabuti ng mga resulta. Muli, ang epektong ito ay mas kapansin-pansin sa mas malalim at mas kumplikadong circuit.
...
- Kung paano gamitin ang IBM Quantum primitives upang mabawasan at mapigilan ang ingay
