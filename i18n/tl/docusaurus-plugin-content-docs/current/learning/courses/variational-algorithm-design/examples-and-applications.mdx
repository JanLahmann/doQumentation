...
May isa pang isyu: ang energy ng unang excited state ay tila mas mababa kaysa sa energy ng ground state! Subukan mong ipaliwanag kung paano ito maaaring mangyari. Pahiwatig: ito ay may kaugnayan sa convergence point na ating tinalakay lamang. Ang ganitong gawi ay ipinaliwanag nang detalyado sa ibaba pagkatapos mailapat ang VQD sa H2 molecule.
...
Magtatakda tayo ngayon ng paunang set ng random na mga parameter:
...
Anumang classical optimizer ay maaaring gamitin upang i-minimize ang cost function. Sa isang tunay na quantum system, karaniwang mas mahusay ang isang optimizer na dinisenyo para sa mga non-smooth na cost function landscape. Dito, ginagamit natin ang [COBYLA routine](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-cobyla.html#optimize-minimize-cobyla) mula sa SciPy sa pamamagitan ng minimize function.

Dahil paulit-ulit tayong nagsasagawa ng maraming tawag sa Runtime, ginagamit natin ang isang session upang isagawa ang lahat ng tawag sa loob ng iisang block. Bukod dito, para sa QAOA, ang solusyon ay naka-encode sa output distribution ng ansatz circuit na naka-bind sa optimal na mga parameter mula sa minimization. Kaya, kakailanganin natin ng Sampler primitive, at ito ay ida-instantiate natin gamit ang parehong session
At patatakbuhin natin ang ating minimization routine:
...
Ang solution vector ng mga parameter angle (`x`), kapag ipinasok sa ansatz circuit, ay nagbubunga ng graph partitioning na hinahanap natin.
...
Magpatuloy sa huling aralin upang kunin ang iyong assessment at makuha ang iyong badge!
