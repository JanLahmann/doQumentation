---
description: 'Sa araling ito, binabago natin ang geometry ng isang simpleng molekula at mino-minimize ang enerhiya sa bawat hakbang.'
notebook_path: learning/courses/quantum-chem-with-vqe/geometry.ipynb
sidebar_label: 'Molecular na geometry'
title: 'Molecular na geometry'
---

{/* doqumentation-source-hash: 532dc9c8 */}
...
# Pagtukoy ng molecular na geometry {#determining-a-molecular-geometry}

Sa nakaraang seksyon, ginamit natin ang VQE para matukoy ang ground state energy ng isang molekula. Ito ay isang wastong paggamit ng quantum computing, ngunit mas kapaki-pakinabang pa ang matukoy ang istruktura ng isang molekula.

## Hakbang 1: I-map ang mga classical na input sa isang quantum na problema {#step-1-map-classical-inputs-to-a-quantum-problem}

Ostaying sa ating pangunahing halimbawa ng diatomic hydrogen, ang tanging geometric na parameter na babaguhin ay ang bond length. Para magawa ito, gagawin natin ang parehong proseso tulad ng dati, pero gagamit ng variable sa ating unang paggawa ng molekula (isang bond length, *x*, sa argument). Ito ay medyo simpleng pagbabago, ngunit kailangan na isama ang variable sa mga function sa buong proseso, dahil nagsisimula ito sa pagbuo ng fermionic Hamiltonian at dumadaan sa mapping at sa wakas sa cost function.

Una, i-load natin ang ilan sa mga package na ginamit natin dati at i-define ang Cholesky function.
...
Ngayon, para i-define ang ating Hamiltonian, gagamitin natin ang PySCF tulad ng sa nakaraang halimbawa, pero isasama na natin ngayon ang isang variable, `x`, bilang kinatawan ng ating interatomic distance. Ibabalik nito ang core energy, single-electron energy, at two-electron energies tulad ng dati.
...
Tandaan na ang pagbuo sa itaas ay gumagawa ng fermionic Hamiltonian batay sa atomic species, geometry, at electronic orbitals. Sa ibaba, imi-map natin ang fermionic Hamiltonian na ito sa mga Pauli operator. Ang `build_hamiltonian` na function na ito ay magsasama rin ng geometric variable bilang argument.
...
I-load natin ang natitirang mga package para patakbuhin ang VQE mismo, tulad ng efficient_su2 ansatz at mga SciPy minimizer:
...
Ide-define natin ulit ang cost function, ngunit lagi itong tumatanggap ng fully-built at na-map na Hamiltonian bilang argument, kaya walang nagbabago sa function na ito.
...
## Hakbang 2: I-optimize ang problema para sa quantum na pagpapatakbo {#step-2-optimize-problem-for-quantum-execution}

Dahil magbabago ang Hamiltonian sa bawat bagong geometry, magbabago rin ang transpiling ng operator sa bawat hakbang. Maaari pa rin tayong mag-define ng pangkalahatang pass manager na ilalapat sa bawat hakbang, na nakatutok sa hardware na gusto nating gamitin.

Dito, gagamitin natin ang pinaka-hindi abala (least busy) na backend na available. Gagamitin natin ang backend na iyon bilang modelo para sa ating AerSimulator, na nagbibigay-daan sa ating simulator na tularan, halimbawa, ang gawi ng ingay ng tunay na backend. Hindi perpekto ang mga noise model na ito, ngunit maaari silang makatulong sa iyo na malaman kung ano ang aasahan sa tunay na hardware.
...
Iina-import natin ang pass manager at mga kaugnay na package para matulungan tayong i-optimize ang ating Circuit. Ang hakbang na ito, at ang isa bago nito, ay hindi nakasalalay sa Hamiltonian, kaya hindi nagbabago mula sa nakaraang aralin.
...
## Hakbang 3: Patakbuhin gamit ang mga IBM Quantum primitive. {#step-3-execute-using-ibm-quantum-primitives}

Sa code block sa ibaba, nagse-set up tayo ng array para iimbak ang ating mga output mula sa bawat hakbang sa ating interatomic distance na $x$. Pinili natin ang hanay ng $x$ batay sa ating kaalaman sa eksperimental na halaga ng equilibrium bond length: 0.74 Angstrom. Patatakbuhin muna natin ito sa isang simulator, at kaya iina-import natin ang ating estimator (BackendEstimator) mula sa `qiskit.primitives`. Para sa bawat geometry step, binubuo natin ang Hamiltonian at pinapayagan ang isang tiyak na bilang ng mga optimization step (dito 500) gamit ang optimizer na "cobyla". Sa bawat geometry step, iniimbak natin ang parehong kabuuang enerhiya at electronic na enerhiya. Dahil sa mataas na bilang ng mga optimizer step, maaaring tumagal ito ng isang oras o higit pa. Maaari mong baguhin ang mga input sa ibaba para bawasan ang kinakailangang oras.
...
Ang mga resulta ng output na ito ay tatalakayin sa ibaba sa seksyon ng post-processing; sa ngayon, tandaan lamang na matagumpay ang simulation. Handa ka na ngayong patakbuhin sa tunay na hardware. Itatakda natin ang resilience sa `1`, na nagpapahiwatig na gagamitin ang TREX error mitigation. Dahil nagtatrabaho na tayo sa tunay na hardware, gagamitin natin ang IBM Quantum Compute Service at IBM Quantum primitives. Tandaan na parehong ang for loop na may kaugnayan sa geometry at ang maraming variational trial ay nasa loob ng session.

Dahil may mga gastos at limitasyon sa oras na nauugnay sa mga tunay na hardware run, binabawasan natin ang bilang ng mga geometry step at optimizer step sa ibaba. Siguraduhing iaangkop ang mga hakbang na ito ayon sa iyong mga layunin sa katumpakan at mga limitasyon sa oras.
...
Para sa parehong simulator at tunay na hardware, maaari nating i-plot ang mga ground state energy na kinakalkula para sa bawat inter-atomic distance at tingnan kung saan nakamit ang pinakamababang enerhiya. Iyon ang inter-atomic distance na makikita sa kalikasan, at katotohanan na malapit ito. Maaaring makakuha ng mas maayos na kurba sa pamamagitan ng pagsubok ng iba pang mga ansaetze, optimizer, at pagpapatakbo ng kalkulasyon nang maraming beses sa bawat geometry step at pag-average ng ilang random na panimulang kondisyon.
...
Tandaan na ang simpleng pagdaragdag ng bilang ng mga optimization step ay hindi malamang na mapabuti ang mga resulta mula sa simulator, dahil lahat ng mga optimization ay talagang na-converge sa kinakailangang tolerance sa mas mababa kaysa sa maximum na bilang ng mga iteration.

Ang mga resulta mula sa tunay na hardware ay maihahambing, maliban sa bahagyang naiibang hanay ng mga halaga na sinample.
...
Bukod sa pag-asa ng H2 bond length na 0.74 Angstrom, ang kabuuang enerhiya ay dapat -1.17 Hartrees. Makikita natin na ang mga resulta ng tunay na hardware ay mas malapit sa mga halagang ito kaysa sa simulator. Ito ay malamang dahil ang ingay ay naroroon (o nasimulate) sa parehong kaso, ngunit sa kaso lamang ng tunay na hardware ay ginamit ang error mitigation.

### Pagtatapos {#closing}

Dito nagtatapos ang ating kurso sa VQE para sa quantum chemistry. Kung interesado kang maunawaan ang ilan sa mga pinagbabatayan na information theory na ginagamit sa quantum computing, tingnan ang kurso ni John Watrous sa [Basics of Quantum Information](/learning/courses/basics-of-quantum-information). Para sa karagdagang maikling halimbawa ng isang VQE workflow, tingnan ang ating [Ground-state energy estimation of the Heisenberg chain with VQE tutorial](/tutorials/spin-chain-vqe). O mag-browse sa [mga tutorial](https://docs.quantum.ibm.com/tutorials) at [mga kurso](/learning) para makahanap ng higit pang mga educational na materyales tungkol sa pinakabagong teknolohiya sa quantum computing.

Huwag kalimutang kunin ang exam ng kursong ito. Ang iskor na 80% o mas mataas ay magbibigay sa iyo ng Credly badge, na awtomatikong ipapadala sa iyo sa pamamagitan ng email. Salamat sa pagiging bahagi ng IBM Quantum&reg; Network!
