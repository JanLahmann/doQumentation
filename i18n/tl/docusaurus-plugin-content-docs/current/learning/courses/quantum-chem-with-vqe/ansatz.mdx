...
Magsisimula tayo sa isang halimbawang four-qubit na `efficient_su2` circuit na may dalawang uri ng SU(2) gate, halimbawa `rx` at `y`. Tutukuyin din natin ang entanglement scheme at ang bilang ng mga pag-uulit. Kung basta `.draw()` mo lang ang mga circuit, makakakuha ka ng medyo abstract na representasyon. Mas malinaw na circuit diagram ang makukuha gamit ang `.decompose().draw()`, at dito gagamitin natin ang `output = "mpl"'.
