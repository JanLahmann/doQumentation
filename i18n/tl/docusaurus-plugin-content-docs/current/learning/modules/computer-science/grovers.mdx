...
Dito, ipinapakita namin kung paano bumuo ng mga Grover oracle at gamitin ang `GroverOperator` mula sa Qiskit circuit library para madaling mag-set up ng isang Grover search instance. Ang IBM Quantum `Sampler` primitive ay nagbibigay-daan sa maayos na pagpapatupad ng mga Grover circuit.
...
Pansinin na ang epekto nito ay binabaligtad ng $Z_f$ ang tanda ng isang target na estado kung saan ang $f(x) = 1$ at iniiwan ang iba pang mga estado nang hindi nababago.
...
Kung ang mga qubit 1-3 ay nasa estadong $|1\rangle$, at ang qubit 0 ay unang nasa estadong $|0\rangle$, ang unang X gate ay magpapalit sa qubit 0 patungong $|1\rangle$ at lahat ng qubit ay magiging $|1\rangle$. Nangangahulugan ito na ang MCMT gate ay mag-aaplay ng pangkalahatang pagbabago ng sign o phase flip, ayon sa nais. Para sa anumang ibang kaso, alinman sa mga qubit 1-3 ay nasa estadong $|0\rangle$, o ang qubit 0 ay napalitan patungong estadong $|0\rangle$, at hindi maa-apply ang phase flip. Makikita natin na ang circuit na ito ay talagang minamarkahan ang nais nating estadong $|0111\rangle$, o ang bitstring na `{1110}`.
Ang buong Grover operator ay binubuo ng phase query gate (oracle), mga Hadamard layer, at ang $Z_\text{OR}$ operator. Maaari nating gamitin ang built-in na `grover_operator` upang buuin ito mula sa oracle na naitakda natin sa itaas.
...
Tulad ng napag-usapan natin sa geometric picture sa itaas, maaaring kailanganin nating i-apply ang Grover operator nang maraming beses. Ang optimal na bilang ng mga iteration na $t$ upang mapalaki ang amplitude ng target state sa kawalan ng ingay ay
$$
t\approx \frac{\pi}{4} \sqrt{\frac{N}{|A_1|}}-\frac{1}{2}
$$
kung saan ang $|A_1|$ ay ang bilang ng mga solution state at ang $N=2^n$ ay ang kabuuang bilang ng mga estado. Sa mga modernong maiingay na quantum computer, maaaring iba ang experimentally optimal na bilang ng mga iteration &mdash; ngunit dito ay kinakalkula at ginagamit natin ang teoretikal, optimal na numerong ito gamit ang $|A_1|=1$.
...
Bumuo na tayo ngayon ng isang circuit na may kasamang paunang Hadamard gate upang lumikha ng superposition ng lahat ng posibleng estado, at i-apply ang Grover operator nang optimal na bilang ng beses.
...
Naitayo na natin ang ating Grover circuit!
...
### Hakbang 3: I-execute gamit ang IBM Quantum primitives {#step-3-execute-using-ibm-quantum-primitives}

Gusto naming gumawa ng maraming sukat at makita kung aling estado ang pinakamalamang. Ang ganitong amplitude amplification ay isang sampling na problema na angkop para sa pagpapatakbo gamit ang `Sampler` IBM Quantum primitive.

Pansinin na ang `run()` method ng IBM Quantum SamplerV2 ay tumatanggap ng isang iterable ng primitive unified blocks (PUBs). Para sa Sampler, ang bawat PUB ay isang iterable sa format na (circuit, parameter_values). Gayunpaman, sa pinakamababa, ito ay tumatanggap ng isang listahan ng quantum circuits.
...
### Suriin ang iyong pang-unawa {#check-your-understanding-1}
...
### Hakbang 2: I-optimize ang problema para sa pagpapatakbo sa quantum hardware {#step-2-optimize-problem-for-quantum-hardware-execution-1}
...
### Hakbang 3: I-execute gamit ang IBM Quantum primitives {#step-3-execute-using-ibm-quantum-primitives-1}
...
### Hakbang 4: I-post-process at ibalik ang resulta sa nais na klasikal na format {#step-4-post-process-and-return-result-in-desired-classical-format-1}
...
<Accordion>
<AccordionItem title="Mga Pahiwatig">
...
Ang cell na "2" ay nagsasabi na dalawa sa tatlong blangkong cell ang dapat maglaman ng mga mina. Dahil ang XOR ay isang parity na operasyon, ang $x_0 \oplus x_1 \oplus x_2$ ay nagbabalik ng true kapag *gansal* na bilang ng mga variable ay true. Gusto namin ng *pares* na bilang (partikular na dalawa) na maging true, kaya itinatanggi namin gamit ang $\lnot$:
$$
...
### Hakbang 3: I-execute gamit ang IBM Quantum primitives {#step-3-execute-using-ibm-quantum-primitives-2}
