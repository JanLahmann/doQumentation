...
# SPANK plugin para sa QRMI {#spank-plugin-for-qrmi}
...
Sa default, ang `CMakeLists.txt` na file ay inaasahang ang Slurm header file (`slurm.h`) ay matatagpuan sa `/usr/include/slurm`, ngunit maaari itong i-customize tulad ng sumusunod.
...
```
Halimbawa, kapag nakikipag-ugnayan sa mga quantum resource sa pamamagitan ng isang HTTP proxy, ang mga environment variable na `http_proxy`, `https_proxy`, at `no_proxy` ay kinakailangan. Maaari itong idagdag tulad ng sumusunod.
```bash
optional /usr/lib64/slurm/spank_qrmi.so /etc/slurm/qrmi_config.json \
