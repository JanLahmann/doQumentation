---
description: 'Unawain ang format ng input at output (kasama ang Primitive Unified Blocs o PUBs) ng mga Qiskit SDK primitives'
...
Kapag tinatawag ang `run()` method ng isang primitive, ang pangunahing argument na kinakailangan ay isang `list` ng isa o higit pang mga tuple -- isa para sa bawat circuit na pinapatakbo ng primitive. Ang bawat isa sa mga tuple na ito ay itinuturing na isang PUB, at ang mga kinakailangang elemento ng bawat tuple sa list ay depende sa primitive na ginamit. Ang data na ibinibigay sa mga tuple na ito ay maaari ring ayusin sa iba't ibang hugis para magbigay ng flexibility sa isang workload sa pamamagitan ng broadcasting -- ang mga panuntunan nito ay inilarawan sa [sumusunod na seksyon](#broadcasting-rules).
...
Kapag isa o higit pang mga PUB ay naipadala na sa isang QPU para sa pagpapatakbo at matagumpay na natapos ang isang job, ibinabalik ang data bilang isang [`PrimitiveResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PrimitiveResult) container object. Ang `PrimitiveResult` ay naglalaman ng isang iterable na list ng mga [`PubResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PubResult) object na naglalaman ng mga resulta ng pagpapatakbo para sa bawat PUB. Halimbawa, ang isang job na isinumite na may 20 PUB ay magbabalik ng isang `PrimitiveResult` object na naglalaman ng list ng 20 `PubResults`, isa para sa bawat PUB.
...
<Admonition type="tip" title="Mga Rekomendasyon">
  ...
  -  Suriin ang [Qiskit SDK primitives](https://docs.quantum.ibm.com/api/qiskit/primitives) API.

  -  Suriin ang [Qiskit Aer primitives](https://qiskit.github.io/qiskit-aer/apidocs/aer_primitives.html) API.

  -  Matuto pa tungkol sa [IBM Quantum primitives](/guides/qiskit-runtime-primitives).

  -  Suriin ang [`qiskit-ibm-runtime` Estimator](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2) API.

  -  Suriin ang [`qiskit-ibm-runtime` Sampler](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2) API.
</Admonition>
