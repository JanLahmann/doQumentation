---
description: 'Paano magpatakbo ng quantum computing job sa isang IBM Quantum Compute Service session.'
...
{/* doqumentation-source-hash: 93f10395 */}
...
Bago magsimula ng session, kailangan mong [i-set up ang IBM Quantum Compute client](/guides/install-qiskit-runtime) at i-initialize ito bilang isang service:
...
Para malaman ang max TTL o interactive TTL ng isang session, sundin ang mga tagubilin sa [Alamin ang mga detalye ng session](#session-details) at hanapin ang value ng `max_time` o `interactive_timeout`, ayon sa pagkakabanggit.
...
Halimbawa: Magpatakbo ng iterative workload na gumagamit ng klasikal na SciPy optimizer para i-minimize ang isang cost function. Sa modelong ito, ginagamit ng SciPy ang output ng cost function para kalkulahin ang susunod nitong input.
...
Maaari kang makakuha ng mas marami mula sa isang session sa pamamagitan ng pagpapatakbo ng maraming workload nang sabay-sabay. Ipinapakita ng sumusunod na halimbawa kung paano mo maaaring patakbuhin ang dalawang VQE algorithm, na ang bawat isa ay gumagamit ng ibang klasikal na optimizer, nang sabay-sabay sa loob ng iisang session. Ginagamit din ang mga job tag para matukoy kung aling job ang galing sa bawat workload.
