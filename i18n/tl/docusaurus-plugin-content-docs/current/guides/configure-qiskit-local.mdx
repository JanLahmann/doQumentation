...
- `min_qpy_version`: Tatanggihan ng pag-load ng QPY ang mga file na may QPY version na mas mababa kaysa sa tinukoy dito. Maaaring gamitin ang opsyong ito para limitahan ang mga pinahintulutang bersyon ng QPY kapag may natuklasang mga isyu sa seguridad ng deserialization sa mga mas lumang bersyon ng QPY, na pinipigilan ang pag-load ng mga potensyal na mapanganib na payload.


<Admonition type="note">
* Ang mga setting ng circuit drawer ay nalalapat sa [qiskit.circuit.QuantumCircuit.draw](../api/qiskit/qiskit.circuit.QuantumCircuit) at [qiskit.visualization.circuit_drawer](https://docs.quantum.ibm.com/api/qiskit/qiskit.visualization.circuit_drawer#qiskitvisualizationcircuit_drawer).
* Ang mga draw method ng state visualization ay [qiskit.quantum_info.Statevector.draw](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Statevector#statevector) at [qiskit.quantum_info.DensityMatrix.draw](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Statevector#statevector).
</Admonition>
...
- `QISKIT_FORCE_THREADS`: Tinutukoy na ang multithreaded na code ay dapat palaging tumakbo (isagawa) sa maraming thread. Sa default, kung nagpapatakbo ka ng multithreaded na code sa isang seksyon ng Qiskit na nasa parallel na proseso na, hindi naglulunsad ang Qiskit ng maraming thread kundi isinasagawa ang function na iyon nang sunud-sunod. Ginagawa ito para maiwasan ang potensyal na pag-overload ng limitadong CPU resources. Gayunpaman, kung gusto mong pilitin ang paggamit ng maraming thread kahit nasa multiprocess na konteksto, itakda ang `QISKIT_FORCE_THREADS=TRUE`.

- `QISKIT_SABRE_ALL_THREADS`: Kinokontrol ang gawi ng layout at routing pass sa preset pass manager ng Qiskit. Kapag nakatakda sa `1` o `TRUE`, ginagamit nito ang lahat ng available na CPU para sa pagpapatakbo ng maraming random na trial. Maaari nitong mapabuti ang kalidad ng mga resulta, lalo na para sa mga sistema na may higit sa 20 CPU/core; gayunpaman, ang kapalit ay hindi na maaaring i-reproduce ang mga resulta kapag pinatakbo sa ibang lokal na hardware.
