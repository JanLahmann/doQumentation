...
Maaaring direktang i-insert ang mga unitary na `Operators` sa isang `QuantumCircuit` gamit ang `QuantumCircuit.append` method. Kino-convert nito ang `Operator` sa isang `UnitaryGate` object, na idinaragdag sa circuit.
