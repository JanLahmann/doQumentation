...
{/* doqumentation-source-hash: fe7438b3 */}
...
!pip install -q hivqe-qiskit-function-utils qiskit-ibm-catalog qiskit-ibm-runtime
```
...
Pagkatapos, i-authenticate gamit ang iyong [IBM Quantum&reg; API key](http://quantum.cloud.ibm.com/) at, kung na-save mo na ang iyong [account](/guides/functions-get-started#install-qiskit-functions-catalog-client) sa iyong lokal na environment, piliin ang Qiskit Function tulad ng sumusunod:
...
Tandaan na ang paggamit ng Qiskit Function na ito ay limitado sa mga problemang nangangailangan ng hindi hihigit sa 20 qubit, maliban kung makakuha ng lisensya na nagbibigay ng mas mataas na limitasyon.

Mag-email sa [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) para sa mga tanong tungkol sa lisensya.

### Example of licensed function use {#example-of-licensed-function-use}

Ang mga licensed na user ay binibigyan ng license token, at pagkatapos ay dapat gumamit ng wrapper library para isumite ang kanilang license token sa function.
Ang wrapper library ay maaaring i-install [mula sa PyPI](https://pypi.org/project/hivqe-qiskit-function-utils/) gamit ang `pip install hivqe-qiskit-function-utils`.
Ipinapakita ng halimbawa sa ibaba kung paano dapat gamitin ang library na ito para isumite ang iyong license token kapag tinatawag ang function.
...
Kung mabigo ang iyong workload, ang status ay magiging `ERROR` at magre-raise ng exception ang pagtawag sa `job.result()`:
...
Kung gusto mong matulungan sa pag-troubleshoot ng isang partikular na error, ibigay ang Function job ID ng job na nakaranas ng error.
