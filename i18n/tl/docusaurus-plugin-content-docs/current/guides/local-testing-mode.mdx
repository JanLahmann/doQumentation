---
description: 'Gamitin ang lokal na testing mode para gayahin ang quantum hardware gamit ang IBM Quantum Compute'
notebook_path: guides/local-testing-mode.ipynb
sidebar_label: 'Lokal na testing mode'
title: 'Lokal na testing mode'
---
...
Hindi sinusuportahan ng Executor primitive ang lokal na testing mode.
</Admonition>

Para gamitin ang lokal na testing mode, tukuyin ang isa sa mga fake backend mula sa ``qiskit_ibm_runtime.fake_provider`` o tukuyin ang isang Qiskit Aer backend kapag nag-instantiate ng IBM Quantum primitive o session.
...
- Maaari mong tukuyin ang lahat ng opsyon ng IBM Quantum Compute sa lokal na testing mode. Gayunpaman, lahat ng opsyon maliban sa shots ay binabalewala kapag pinatakbo sa lokal na simulator.
...
- Gamitin ang [klase ng `Neat`](/guides/debug-qiskit-runtime-jobs) para suriin ang epekto ng noise sa isang Estimator workload.
- Suriin ang mga detalyadong [halimbawa ng Sampler](/guides/sampler-examples).
- Suriin ang mga detalyadong [halimbawa ng Estimator](/guides/estimator-examples).
