...
## Suporta ng Qiskit Runtime {#qiskit-runtime-support}
...
### I-access ang circuit timing data {#access-the-circuit-timing-data}

Kapag hiniling, ang circuit timing data para sa bawat PUB ay ibinababa sa job result metadata, sa ilalim ng `["compilation"]["scheduler_timing"]["timing"]`. Naglalaman ang field na ito ng raw na impormasyon ng timing. Para ipakita ang impormasyon ng timing, gamitin ang built-in na visualization tool, gaya ng inilarawan sa seksyong [I-visualize ang mga timing](#visualize-timings).

Gamitin ang sumusunod na code para ma-access ang circuit timing data para sa unang PUB:
...
Habang ang pag-visualize ng circuit timing data gamit ang `draw_circuit_schedule_timing` method ang pinakakaraniwang use case, maaaring maging kapaki-pakinabang na maunawaan ang istruktura ng raw timing data na ibinababa. Makakatulong ito sa iyo, halimbawa, para makuha ang impormasyon nang programatiko.

Ang timing data na ibinababa sa `["compilation"]["scheduler_timing"]["timing"]` ay isang listahan ng mga string. Ang bawat string ay kumakatawan sa isang instruksyon sa isang channel at pinaghihiwalay ng kuwit sa mga sumusunod na uri ng data:

- `Branch` - Tinutukoy kung ang instruksyon ay nasa control flow (then / else) o sa isang main branch.
- `Instruction` - Ang gate at ang qubit na dapat gamitin.
- `Channel` - Ang channel na itinalaga sa instruksyon. Maaari itong isa sa mga sumusunod:
      - `Qubit x` - Ang drive channel para sa qubit _x_.
      - `AWGRx_y` (arbitrary waveform generator readout) - Ginagamit ng mga readout channel para mag-communicate kapag sinusukat ang mga qubit. Ang mga argument na _x_ at _y_ ay tumutugma sa readout instrument ID at qubit number, ayon sa pagkakasunod.
- `T0` - Ang oras ng pagsisimula ng instruksyon sa loob ng kumpletong schedule
- `Duration` - Ang tagal ng instruksyon, sa mga yunit ng _dt_ segundo, kung saan ang 1 dt = 1 scheduling cycle. Mahahanap mo ang `dt` na halaga ng isang backend sa pamamagitan ng [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt).
...
Gamit ang `qiskit-ibm-runtime` v0.43.0 o mas bago, maaari kang mag-visualize ng mga timing ng circuit. Para ma-visualize ang mga timing, kailangan mo munang i-convert ang result metadata sa `fig` gamit ang [`draw_circuit_schedule_timing` method.](https://github.com/Qiskit/qiskit-ibm-runtime/blob/3d1bf1e1d49e5123841639fce259859c90ce9314/qiskit_ibm_runtime/visualization/draw_circuit_schedule_timings.py#L26) Nagbabalik ang method na ito ng `plotly` figure, na maaari mong ipakita nang direkta, i-save sa isang file, o pareho. Para sa higit pang impormasyon tungkol sa mga `plotly` command na gagamitin, tingnan ang [`fig.show()`](https://plotly.com/python-api-reference/generated/plotly.io.show.html) at [`fig.write_image("<path.format>")`.](https://plotly.com/python-api-reference/generated/plotly.io.write_image.html)
...
![Kapag nag-hover ka sa output, makikita mo ang impormasyon tulad ng simula, katapusan, at tagal.](/docs/images/guides/visualize-circuit-timing/image_1.svg 'Halimbawa ng isang na-generate na figure')
#### Unawain ang na-generate na figure {#understand-the-generated-figure}

Ang imahe ng output ng circuit timing data mula sa `draw_circuit_schedule_timing` ay nagdadala ng sumusunod na impormasyon:

- Ang X axis ay oras sa mga yunit ng _dt_ segundo, kung saan ang 1 dt = 1 scheduling cycle. Mahahanap mo ang `dt` na halaga ng isang backend sa pamamagitan ng [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt).
- Ang Y axis ay ang channel (isipin ang mga channel bilang mga instrumento na nagpapalabas ng mga pulse).
    - `Receive channel` - Ang tanging channel na hindi instrumento sa sarili nito. Ito ay isang instruksyon na pinaplays sa lahat ng channel na bahagi ng isang proseso ng komunikasyon kasama ang hub sa oras na iyon.
    - `Qubit x` - Ang drive channel para sa qubit x.
    - `AWGRx_y` (arbitrary waveform generator readout) - Ginagamit ng mga readout channel para mag-communicate kapag sinusukat ang mga qubit. Ang mga argument na _x_ at _y_ ay tumutugma sa readout instrument ID at qubit number, ayon sa pagkakasunod.
    - `Hub` - Kinokontrol ang broadcasting.

Bukod pa rito, ang bawat instruksyon ay may format na *X_Y*, kung saan ang *X* ay ang pangalan ng instruksyon at ang *Y* ay ang uri ng pulse. Ang uri na `play` ay nag-aaplika ng mga control pulse, at ang `capture` ay nagtatala ng estado ng qubit. Maaari ka ring mag-hover sa bawat instruksyon para makakuha ng higit pang detalye. Halimbawa, ang nakaraang figure ay nagpapakita ng isang control pulse para sa X gate na inilapat sa qubit 10 sa 1161 dt.
### End-to-end na halimbawa {#end-to-end-example}

Ipinapakita ng halimbawang ito kung paano i-enable ang opsyon, makuha ang impormasyon ng timing ng circuit mula sa metadata, at ipakita ito bilang isang imahe.

Una, i-set up ang environment, tukuyin ang mga circuit at i-convert ang mga ito sa ISA circuit, at tukuyin at patakbuhin ang mga job.
...
Susunod, kunin ang circuit schedule timing:
