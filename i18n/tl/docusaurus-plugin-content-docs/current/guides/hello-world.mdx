...
sidebar_label: 'Patakbuhin ang iyong unang Circuit sa hardware'
title: 'Patakbuhin ang iyong unang Circuit sa hardware'
---
...
1. Kung hindi mo pa nai-install ang Qiskit, hanapin ang mga instruksyon sa gabay na [Quickstart](/guides/quick-start).
    ...
    - I-install ang IBM Quantum Compute client para makapagpatakbo ng mga job sa quantum hardware:
...
2. I-set up ang iyong authentication para ma-access ang quantum hardware sa pamamagitan ng libreng [Open Plan](/guides/plans-overview#open-plan).
    ...
    - Pumunta sa [IBM Quantum Platform](https://quantum.cloud.ibm.com/) para mag-log in o lumikha ng account.
         ...
         Kung kumokonekta ka sa pamamagitan ng proxy server, kailangan mong gumamit ng `qiskit-ibm-runtime` v0.44.0 o mas bago.
         </Admonition>
...
3. Maaari mo na ngayong gamitin ang Python code na ito sa tuwing gusto mong mag-authenticate sa IBM Quantum Compute Service:
...
Kung ang state ay naka-entangle, ang pagsukat ng $\langle Z_1 Z_0 \rangle$ ay dapat na iba sa pagsukat ng $\langle I_1 \otimes Z_0 \rangle \langle Z_1 \otimes I_0 \rangle$. Para sa partikular na entangled state na ginawa ng ating circuit na inilarawan sa itaas, ang pagsukat ng $\langle Z_1 Z_0 \rangle$ ay dapat na 1 at ang pagsukat ng $\langle I_1 \otimes Z_0 \rangle \langle Z_1 \otimes I_0 \rangle$ ay dapat na zero.
</Admonition>
...
Ang sumusunod na code ay gumagawa ng instance ng tunay na device na pagsa-submit-an ng job at nagbabago ng circuit at mga observable para tumugma sa ISA ng backend na iyon. Kinakailangan na [na-save mo na ang iyong mga kredensyal](/guides/cloud-setup) bago ito.
...
Ang mga quantum computer ay maaaring mag-produce ng mga random na resulta, kaya karaniwang nangongolekta ka ng sample ng mga output sa pamamagitan ng paulit-ulit na pagpapatakbo ng circuit. Maaari mong i-estimate ang value ng observable gamit ang `Estimator` class. Ang `Estimator` ay isa sa dalawang [primitive](/guides/get-started-with-estimator); ang isa ay ang `Sampler`, na maaaring gamitin para makakuha ng data mula sa isang quantum computer. Ang mga object na ito ay may `run()` na method na nagpapatupad ng piniling mga circuit, observable, at parameter (kung applicable), gamit ang isang [primitive unified bloc (PUB)](/guides/get-started-with-sampler).
...
:::note[Alternatibo: patakbuhin ang halimbawa gamit ang simulator]


  Kapag pinatakbo mo ang iyong quantum program sa isang tunay na device, ang iyong workload ay dapat munang maghintay sa pila bago tumakbo. Para makatipid ng oras, maaari mong gamitin sa halip ang sumusunod na code para patakbuhin ang maliit na workload na ito sa [`fake_provider`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/fake-provider) gamit ang `qiskit-ibm-runtime` local testing mode. Tandaan na posible lang ito para sa maliit na circuit. Kapag nag-scale up ka sa susunod na seksyon, kailangan mong gumamit ng tunay na device.
...
I-submit ang job at i-enable ang error suppression sa pamamagitan ng paggamit ng teknik para mabawasan ang mga error na tinatawag na [dynamical decoupling](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-dynamical-decoupling-options). Tinutukoy ng resilience level kung gaano karaming resilience ang buuin laban sa mga error. Ang mas mataas na antas ay nagbibigay ng mas tumpak na mga resulta, ngunit nangangailangan ng mas mahabang oras ng pagproseso. Para sa karagdagang paliwanag ng mga opsyon na nakatakda sa sumusunod na code, tingnan ang [Error mitigation at suppression techniques](/guides/error-mitigation-and-suppression-techniques).
...
Ipinapakita ng nakaraang graph na habang lumalaki ang distansya sa pagitan ng mga qubit, humihina ang signal dahil sa presensya ng noise.
