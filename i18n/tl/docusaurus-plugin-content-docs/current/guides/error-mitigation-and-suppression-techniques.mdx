---
description: 'Paliwanag ng mga teknik ng error mitigation at suppression na available sa pamamagitan ng IBM Quantum Compute Service'
...
Sa presensya ng readout error, ang measurement twirling ay may epekto ng pag-diagonalize ng readout-error transfer matrix, na ginagawa itong madaling i-invert. Natututo ang isang rescaling term mula sa diagonal noise channel sa pamamagitan ng
benchmarking ng mga random na circuit na na-initialize sa zero state. Pinapayagan nito ang
serbisyo na alisin ang bias mula sa mga expectation value na nagmumula sa
readout noise. Ang pagtatantya ng readout-error transfer matrix ay nangangailangan ng pagpapatupad ng karagdagang mga calibration circuit, na nagdudulot ng maliit na overhead.

### I-enable ang TREX gamit ang opsyon na `measure_mitigation` {#enable-trex-by-using-the-measure-mitigation-option}

Maaaring i-enable ang TREX sa pamamagitan ng pag-set ng `measure_mitigation` sa `True` sa [mga opsyon ng IBM Quantum Compute resilience](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) para sa Estimator. Ang mga opsyon para sa measurement noise learning ay inilalarawan [dito](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-measure-noise-learning-options). Tulad ng gate twirling, maaari mong i-set ang bilang ng mga circuit randomization at ang bilang ng shots bawat randomization.
...
### I-enable ang TREX gamit ang opsyon na `resilience_level` {#enable-trex-by-using-the-resilience-level-option}

Maaari mong i-enable ang TREX sa pamamagitan ng pag-set ng [resilience level](/guides/estimator-noise-management#resilience) sa `1`, tulad ng ipinapakita sa sumusunod na halimbawa.
...
Ang overhead ng pamamaraang ito ay nag-e-scale kasabay ng bilang ng mga noise factor. Ang
mga default na setting ay nagsa-sample ng expectation value sa tatlong noise factor,
na humahantong sa halos 3x na overhead.

### I-enable ang ZNE gamit ang opsyon na `zne_mitigation` {#enable-zne-by-using-the-zne-mitigation-option}

Maaaring i-enable ang ZNE sa pamamagitan ng pag-set ng `zne_mitigation` sa `True` sa [mga opsyon ng IBM Quantum Compute resilience](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) para sa Estimator.
Ang mga opsyon para sa ZNE ay inilalarawan [dito](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options). Ang mga sumusunod na opsyon ay kapansin-pansin:
...
### I-enable ang ZNE gamit ang opsyon na `resilience_level` {#enable-zne-by-using-the-resilience-level-option}

Maaari mong i-enable ang ZNE (kasama ang TREX at gate twirling) sa pamamagitan ng pag-set ng [resilience level](/guides/estimator-noise-management#resilience) sa `2`, tulad ng ipinapakita sa sumusunod na halimbawa.
...
- [I-configure ang noise management gamit ang Estimator](/guides/estimator-noise-management).

- [I-configure ang noise management gamit ang Sampler](/guides/sampler-noise-management).

- Tuklasin ang iba pang [mga opsyon](/guides/runtime-options-overview) para sa mga IBM Quantum primitive.

- Magpasya kung anong [execution mode](/guides/execution-modes) ang patakbuhin ng iyong job.
