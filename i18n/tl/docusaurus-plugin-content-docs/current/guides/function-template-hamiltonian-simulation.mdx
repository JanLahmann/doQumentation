...
Ang template na ito ay sumasaklaw sa isang workflow para sa pag-simulate ng time evolution ng isang initial state sa ilalim ng isang user-defined spin-based Hamiltonian at nagbabalik ng isang hanay ng mga tinukoy na expectation values gamit ang [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/) Qiskit addon.

Ang template na ito ay nakabalangkas bilang isang Qiskit pattern na may mga sumusunod na hakbang:

#### 1. Pangongolekta ng input at pag-map ng problema {#1-collecting-input-and-mapping-the-problem}
...
#### 2. Paghahanda ng mga nabuong circuit para sa execution {#2-prepare-the-generated-circuits-for-execution}
...
Una, sumulat ng function template para sa Hamiltonian simulation na gumagamit ng [AQC-Tensor Qiskit addon](https://qiskit.github.io/qiskit-addon-aqc-tensor/) para i-map ang paglalarawan ng problema sa isang reduced-depth circuit para sa execution sa hardware.

Kung ida-download mo ang pahinang ito at titingnan nang lokal sa isang notebook editor, makikita mo na ang ilang code cell ay naglalaman ng [magic command](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) na `%%writefile`. Sine-save ng magic command na ito ang code sa `./source_files/template_hamiltonian_simulation.py`, na siyang function template na maaari mong i-upload at patakbuhin nang remote gamit ang Qiskit Serverless.
...
### Kolektahin at i-validate ang mga input {#collect-and-validate-the-inputs}

Magsimula sa pamamagitan ng pagkuha ng mga input para sa template. Ang halimbawang ito ay may mga domain-specific na input na may kaugnayan sa Hamiltonian simulation (tulad ng Hamiltonian at observable) at mga capability-specific na opsyon (tulad ng kung gaano kalaki ang gusto mong i-compress ang mga initial layer ng Trotter circuit gamit ang AQC-Tensor, o mga advanced na opsyon para sa fine-tuning ng error suppression at mitigation na lampas sa mga default na bahagi ng halimbawang ito).
...
<Accordion>
<AccordionItem title="**Buong source code ng programa**">
