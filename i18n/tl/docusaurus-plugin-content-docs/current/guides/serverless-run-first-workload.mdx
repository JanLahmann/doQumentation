...
<Admonition type="tip">
**May paparating na upgrade ang Qiskit Serverless, at mabilis na nagbabago ang mga feature nito.** Sa panahon ng development phase na ito, hanapin ang mga release notes at pinakabagong dokumentasyon sa [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html) na pahina.
</Admonition>
...
Siguruhing sinunod mo ang workflow sa [Sumulat ng iyong unang Qiskit Serverless program](/guides/serverless-first-program) bago magsimula.

## Mag-lista ng mga available na programa {#list-programs-available}

Maaari mong gamitin ang `QiskitServerless.list()` para makuha ang listahan ng mga available na programa na maaaring patakbuhin gamit ang Qiskit Serverless. Kasama dito ang dating na-upload na [`transpile_remote_serverless`](/guides/serverless-first-program).
...
- **`RUNNING`**: Tumatakbo na ang programa. Sa yugtong ito, kung mayroon kang mga `print()` na output sa iyong programa, maaari kang kumuha ng mga logs gamit ang `job.logs()`

- **`DONE`**: Tapos na ang programa, at maaari mong makuha ang data na naka-store sa `save_result()` gamit ang `job.results()`
...
Matagumpay mong napatakbo ang iyong unang Qiskit Serverless program!
...
- Galugarin ang [mga compute at data management tools](/guides/serverless-manage-resources) na available sa iyong programa, kabilang ang parallelization.
