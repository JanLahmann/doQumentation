...
Sa DAG, mayroong tatlong uri ng graph node: mga qubit/clbit input node (berde), mga operation node (asul), at mga output node (pula). Ang bawat edge ay nagpapahiwatig ng daloy ng data (o dependency) sa pagitan ng dalawang node. Gamitin ang function na qiskit.tools.visualization.dag_drawer() para tingnan ang DAG ng circuit na ito. (I-install ang [Graphviz library](https://graphviz.org/download/) para patakbuhin ito.)
...
- Suriin ang gabay sa paglikha ng [custom transpiler pass](/guides/custom-transpiler-pass)
- Alamin kung paano [Gumawa at mag-transpile para sa mga custom na backend](/guides/custom-backend)
