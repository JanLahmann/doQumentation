...
*Tansiya ng paggamit: Tatlong segundo sa isang Eagle r3 processor (TANDAAN: Ito ay isang tantiya lamang. Maaaring mag-iba ang iyong runtime.)*

## Mga resulta ng pag-aaral {#learning-outcomes}
Pagkatapos ng tutorial na ito, dapat maunawaan ng mga gumagamit ang:
- Ang mathematical background ng algoritmo ni Shor para sa integer factorization
- Kung paano magpatakbo ng isang halimbawa ng algorithm na ito sa hardware
...
- [Phase estimation and factoring](/learning/courses/fundamentals-of-quantum-algorithms/phase-estimation-and-factoring/introduction). Tinatakpan namin ang ilan sa materyal na ito sa tutorial na ito.
...
Tandaan na inalis natin ang controlled modular exponentiation operations mula sa natitirang control qubits dahil ang $M_1$ ay ang identity operator.
Tandaan na mamaya sa tutorial na ito, patatakbuhin natin ang circuit na ito sa `ibm_marrakesh` backend. Para gawin ito, itra-transpile natin ang circuit ayon sa partikular na backend na ito at iuulat ang circuit depth at gate counts.
...
Ngayon ay maaari na nating patakbuhin ang order finding circuit na na-transpile na natin dati para sa `ibm_marrakesh`. Dito, gagamit tayo ng [dynamical decoupling](/guides/error-mitigation-and-suppression-techniques#dynamical-decoupling) (DD) para sa error suppression, at [gate twirling](/guides/error-mitigation-and-suppression-techniques#pauli-twirling) para sa mga layunin ng error mitigation. Ang DD ay may kinalaman sa paglalapat ng mga sequence ng eksaktong tinamang control pulses sa isang quantum device, epektibong pinagagaling ang hindi gustong environmental interactions at decoherence. Ang gate twirling naman ay nagra-randomize ng mga partikular na quantum gates para i-transform ang coherent errors sa Pauli errors, na naiipon nang linear sa halip na quadratic. Kadalasan pinagsasama ang dalawang teknik para pahusayin ang coherence at fidelity ng quantum computations.
