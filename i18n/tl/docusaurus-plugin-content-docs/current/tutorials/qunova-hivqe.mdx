---
description: 'Gamitin ang Qunova HiVQE upang kalkulahin ang mga dissociation potential energy surface (PES) curve.'
notebook_path: tutorials/qunova-hivqe.ipynb
sidebar_label: 'Mga kurba ng Dissociation PES gamit ang Qunova HiVQE'
title: 'Mga kurba ng Dissociation PES gamit ang Qunova HiVQE'
---

{/* doqumentation-source-hash: 3e02f524 */}
...
Ang mga Qiskit Function ay isang eksperimentong feature na available lamang para sa mga user ng IBM Quantum&reg; Premium Plan, Flex Plan, at On-Prem (via IBM Quantum Platform API) Plan. Ang mga ito ay nasa preview release status at maaaring magbago.
</Admonition>

*Tantiya ng paggamit (TANDAAN: Ito ay tantiya lamang. Maaaring mag-iba ang iyong runtime.)*

* Li2S: Limang minutong QPU time sa isang Heron r2 processor

* FeP-NO: Limang minutong QPU time sa isang Heron r2 processor


## Panimula {#background}

Ang tumpak na pagkalkula ng mga enerhiya ng kemikal na reaksyon ay mahalaga para sa siyentipikong pag-unlad sa materials science, chemical engineering, drug discovery, at iba pang larangan. Sa iba't ibang kemikal na sistema, ang Li-S system ay nakakuha ng malaking interes para sa pag-unawa at pagbuo ng mga bagong komposisyon ng baterya. Ang tutorial na ito ay nagbibigay ng hands-on na karanasan sa pagkalkula ng Li-S bond dissociation potential energy surface (PES) ng isang $Li_2S$ system sa pamamagitan ng pag-alis ng isang lithium atom gamit ang mga kalkulasyon ng HiVQE. Ang mga resulta ay maaaring ikumpara sa reference calculation (CASCI) pati na rin sa mga klasikal na pamamaraan tulad ng Hartree-Fock (HF) para sa isang 20-qubit na problema.

## Mga Kinakailangan {#requirements}

I-install ang mga sumusunod na dependency upang patakbuhin ang code sa tutorial na ito.
...
Upang patakbuhin ang tutorial na ito, i-import ang `qunova/hivqe-chemistry` function sa pamamagitan ng `QiskitFunctionCatalog`. Kailangan ninyo ng IBM Quantum Premium Plan, Flex Plan, o On-Prem (IBM Quantum Platform API) Plan account na may lisensya mula sa Qunova upang patakbuhin ang function na ito.
...
## Bahagi 1: Li2S (20Q) {#part-1-li2s-20q}

### Hakbang 1: I-map ang mga klasikal na input sa isang quantum problem {#step-1-map-classical-inputs-to-a-quantum-problem}

Tukuyin ang mga $Li_2S$ geometry sa dictionary format para sa iba't ibang bond distance ng Li-S upang kalkulahin ang PES curve. Ang mga geometry na ito ay na-optimize gamit ang mga kalkulasyon ng B3LYP/631g.
...
Ang mga kalkulasyon ng HiVQE ay isasagawa gamit ang mga opsyon na tinukoy sa ibaba. Gamit ang sto3g basis para sa $Li_2S$, mayroong 19 spatial orbital na may 22 elektron. Upang patakbuhin ang (10o,10e) case gamit ang HiVQE calculation, maaari ninyong tukuyin ang 10 active orbital at anim na frozen orbital. Sa bawat iteration, gagamitin ang 100 shot upang mag-sample ng electron configuration na nabuo ng ExcitationPreserving quantum circuit (`epa`) na may `circular` entanglement at dalawang repetition (`reps`). Ang maximum na bilang ng iteration ay nakatakda sa 30 upang masiguro ang pagtatapos ng iteration na may energy convergence.
...
### Hakbang 2 at 3: I-optimize ang problema para sa quantum hardware execution at isagawa gamit ang HiVQE Chemistry function {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}

I-setup ang `for` loop upang patakbuhin ang mga kalkulasyon ng HiVQE gamit ang mga geometry na may mga opsyon na tinukoy sa ibaba. Ang mga job ay isinusubmit sa `for` loop. Sa tutorial na ito, magsusubmit kayo ng anim na geometry at kukuhanin ang mga resulta kapag nakumpleto na ang lahat. Sa main function run, kailangan ninyong tukuyin ang `max_states` at `max_expansion_states` upang kontrolin ang maximum na laki ng subspace matrix at upang kontrolin kung ilang estado ang maaaring mabuo gamit ang mga klasikal na pamamaraan ng CI expansion bawat iteration. Ang mga function job ID ay iimbak sa dictionary na may bawat geometry label upang higit pang subaybayan at iproseso ang output.
...
Suriin natin kung ang lahat ng mga job ay tumatakbo pa o nakumpleto na.
...
Kapag nakumpleto na ang lahat ng mga job, kunin natin ang lahat ng mga resulta ng kalkulasyon.
...
Ang kabuuang QPU runtime na ginamit sa job ay maaaring subaybayan sa pamamagitan ng pag-log in sa [IBM Quantum Platform](https://quantum.cloud.ibm.com/) at pagtingin sa mga naisumiteng job na may `qunova-chemistry-hivqe` tag.

### Hakbang 4: Post-process at ikumpara sa mga klasikal na pamamaraan {#step-4-post-process-and-compare-with-classical-methods}

Ang klasikal na reference calculation (CASCI) ay maaaring isagawa para sa (10o,10e) upang i-validate ang mga resulta ng HiVQE.
...
### Pag-plot ng dissociation curve para sa Li\_2S {#plotting-the-dissociation-curve-for-li-2s}

I-plot at ikumpara natin ang mga resulta ng HiVQE sa HF at CASCI.
Makikita ninyo na ang lahat ng mga kalkulasyon ng HiVQE ay tumutugma nang mabuti sa klasikal na reference result (CASCI).
...
## Bahagi 2: FeP-NO (44Q) {#part-2-fep-no-44q}

Tandaan na upang mapatakbo ang function sa FeP-NO system na may mga setting na ipinapakita sa halimbawang ito, kailangan mo ng lisensya na nagpapahintulot ng paggamit ng function na may hindi bababa sa 44 na qubits. Mag-email sa [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) para magtanong tungkol sa pagkuha ng lisensya.

### Hakbang 1: I-map ang mga klasikal na input sa isang quantum problem {#step-1-map-classical-inputs-to-a-quantum-problem-1}

Tukuyin ang mga opsyon para sa mga kalkulasyon ng HiVQE
...
Tukuyin ang mga FeP-NO geometry sa dictionary format para sa iba't ibang bond distance ng Fe-N upang kalkulahin ang PES curve.
...
### Hakbang 2 at 3: I-optimize ang problema para sa quantum hardware execution at isagawa gamit ang HiVQE Chemistry function {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

Batay sa setup ng HiVQE at mga geometry, kunin ang mga resulta nang sunud-sunod.

#### Isumite ang d(Fe-N) = 1.75 $\AA$ calculation. {#submit-dfe-n-175-aa-calculation}
...
Subaybayan ang job at kunin ang resulta para sa d(Fe-N) = 1.75 $\AA$ calculation.
...
#### Isumite ang d(Fe-N) = 2.00 $\AA$ calculation. {#submit-dfe-n-200-aa-calculation}
...
Subaybayan ang job at kunin ang resulta para sa d(Fe-N) = 2.00 $\AA$ calculation.
...
#### Isumite ang d(Fe-N) = 5.00 $\AA$ calculation. {#submit-dfe-n-500-aa-calculation}
...
Subaybayan ang job at kunin ang resulta para sa d(Fe-N) = 5.00 $\AA$ calculation.
...
### Hakbang 4: Post-process at ikumpara sa mga klasikal na pamamaraan {#step-4-post-process-and-compare-with-classical-methods-1}

Ang mga resulta ng klasikal na reference calculation (CASCI-DMRG, maxM=800) ay ibinigay para sa (22o,22e) upang i-validate ang mga resulta ng HiVQE.
...
Pakisagutan ang maikling survey na ito upang magbigay ng feedback sa tutorial na ito. Ang iyong mga pananaw ay makakatulong sa amin na mapabuti ang aming mga alok na nilalaman at karanasan ng user.

> **Tandaan:** Ang survey na ito ay mula sa **IBM Quantum** at sumasaklaw sa **tutorial content** (isinulat ng IBM). Ibinibigay ng doQumentation ang website, mga pagsasalin, at pagpapatakbo ng code — para sa feedback tungkol dito, mangyaring [mag-open ng GitHub issue](https://github.com/JanLahmann/doQumentation/issues).

[Link papunta sa survey](https://your.feedback.ibm.com/jfe/form/SV_9AizRyKFHEGzVs2)
