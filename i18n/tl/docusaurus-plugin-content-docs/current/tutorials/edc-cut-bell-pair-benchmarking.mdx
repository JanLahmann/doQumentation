...
* Pangunahing kaalaman sa [Qiskit SDK](https://docs.quantum.ibm.com/guides) at [Qiskit Runtime](/guides/compute-services#qiskit-runtime), at access sa isang [IBM Quantum&reg; account](/guides/cloud-setup).
...
Bago tumakbo sa tunay na QPU, bine-verify natin na ang circuit ay gumagawa ng Bell pair sa pamamagitan ng pagsubok nito sa isang noiseless simulator na may apat na qubit chain na `[0, 1, 2, 3]`. Ginagamit natin ang Qiskit Runtime `Sampler` na may `AerSimulator` bilang backend mode upang isagawa ang mga circuit.
