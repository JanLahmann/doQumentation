...
## Setup {#setup}
Una, mag-authenticate gamit ang iyong [IBM Quantum API key](http://quantum.cloud.ibm.com/). Pagkatapos, piliin ang Qiskit Function tulad ng sumusunod. (Ang code na ito ay umaasa na [na-save mo na ang iyong account](/guides/functions-get-started#install-qiskit-functions-catalog-client) sa iyong local environment.)
...
Upang i-simulate ang dynamics ng TFIM, bubuo tayo ng mga circuits na tumantiya sa time evolution operator.
...
### Estimate single-qubit magnetizations $\langle Z_i \rangle$ {#estimate-single-qubit-magnetizations-langle-z-i-rangle}
...
Sinusuri natin ang magnetization dynamics sa tunay na quantum hardware. Ang Fire Opal ay nagbibigay ng isang Qiskit Function na pinalalawig ang karaniwang IBM Quantum Estimator primitive na may automated error suppression at performance management. Isinusubmit natin ang Trotterized circuits nang direkta sa isang IBM&reg; backend habang hinahawakan ng Fire Opal ang noise-aware execution.
...
Mangyaring maglaan ng isang minuto upang magbigay ng feedback sa tutorial na ito. Ang iyong mga pananaw ay makakatulong sa amin na mapahusay ang aming mga alok sa nilalaman at karanasan ng user.
