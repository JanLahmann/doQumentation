...
*Tinatayang paggamit: wala pang isang minuto sa Heron r2 processor (PAALALA: Ito ay isang tantya lamang. Maaaring mag-iba ang iyong runtime.)*
...
- Sa pagtantya ng ${M}$, ipinagpalagay natin na tanging mga readout error lamang ang nangyari. Partikular,
  ipinagpalagay natin na walang mga error sa paghahanda ng state at quantum measurement —
  o kung hindi man, na ito ay otherwise mitigated.
  Hangga't ito ay isang magandang palagay, tunay na kumakatawan ang ${M}$
  sa readout error lamang. Ngunit kapag _ginamit_ natin ang ${M}$ upang itama ang isang measured distribution
  sa mga bitstring, wala tayong ginagawang ganitong palagay. Sa katunayan, inaasahan nating magdulot ng noise ang isang interesanteng
  circuit, halimbawa, mga gate error. Ang "tunay" na distribution
  ay may kasamang mga epekto pa rin mula sa anumang mga error na hindi otherwise mitigated.


Ang pamamaraang ito, bagama't kapaki-pakinabang sa ilang pagkakataon, ay may ilang limitasyon.

Ang space at time resources na kinakailangan upang tantyahin ang ${M}$ ay lumalaki nang exponential sa $n$:
- Ang pagtantya ng ${M}$ at ${\tilde{p}}$ ay napapailalim sa statistical error dahil sa finite sampling.
  Maaaring gawing kasing-liit hangga't gusto ang noise na ito
  sa halagang mas maraming shot (hanggang sa timescale ng driftng hardware parameter
  na nagreresulta sa systematic error sa ${M}$).
  Gayunpaman, kung walang ipinapalagay na mga bitstring na naobserbahan
  kapag isinasagawa ang mitigation, ang bilang ng shot na kinakailangan upang tantyahin ang ${M}$ ay lumalaki
  nang hindi bababa sa exponential sa $n$.
- Ang ${M}$ ay isang $2^n \times 2^n$ na matrix.
  Kapag $n>10$, ang dami ng memory na kinakailangan upang itago ang ${M}$ ay
  mas malaki kaysa sa memory na available sa isang makapangyarihang laptop.

Ang iba pang mga limitasyon ay:

- Ang nakuhang distribution na ${\vec{p}}$ ay maaaring magkaroon ng isa
  o higit pang negatibong probability (habang ang kabuuan ay isa pa rin). Isang solusyon
  ay ang i-minimize ang $||{M} {\vec{p}} - {\tilde{p}}||^2$ sa ilalim ng constraint na
  ang bawat entry sa ${\vec{p}}$ ay non-negative. Gayunpaman, ang runtime ng ganitong
  metodo ay orders of magnitude na mas mahaba kaysa sa direktang paglutas sa equation (2).

- Gumagana ang mitigation procedure na ito sa antas ng isang probability distribution
  sa mga bitstring. Sa partikular, hindi nito maitatama ang isang error sa isang indibidwal
  na naobserbahang bitstring.
