...
*Tinatayang paggamit: 50 minuto sa isang Heron r2 processor. (TANDAAN: Ito ay isang tantiya lamang. Maaaring mag-iba ang iyong runtime.)*

Tandaan na ang execution time ng function na ito ay karaniwang higit sa 20 minuto,
kaya maaaring nais mong hatiin ang tutorial na ito sa dalawang seksyon: ang una, kung saan
babasahin mo ito at ilulunsad ang mga jobs, at ang pangalawa ilang oras mamaya
...
Ituturo sa iyo ng tutorial na ito kung paano:
...
Bago simulan ang tutorial na ito, siguraduhing mayroon kang sumusunod na naka-install:
...
Mag-authenticate gamit ang iyong [API key](http://quantum.cloud.ibm.com/) at piliin ang function tulad ng sumusunod:
...
equation, isang convection–diffusion equation na naglalarawan ng mga phenomena na nangyayari sa
...
sa mga IBM&reg; QPUs gamit ang QUICK-PDE, na ibinibigay ng:
...
solusyon. Maaari mong isaayos ang $a$ at $b$ at tingnan kung paano nila naimpluwensyahan ang
proseso ng paglutas at ang solusyon. Para sa pagpili ng initial condition na ito,
admit ng equation ang sumusunod na analytical solution:

$$u(t, x) = \frac{ax + b}{at + 1},$$

na totoo hangga't walang shock na nabuo, tulad ng, para sa $t < -1/a$ kapag $a < 0$.
...
Depende sa configuration na sinusubukan mong lutasin, maaaring kailangang isaayos ang
...
Makikita rin kung paano tukuyin ang backend at ang execution mode.

Bilang karagdagan, ang physically-informed parameters ay maaaring mag-steer ng optimization process
sa maling direksyon; sa kasong iyan, maaari mong i-disable ito sa pamamagitan ng pagtakda ng
...
Maaari mong ikumpara ang convergence process ng aming solusyon (HDES) ng job_2 sa performance ng isang physics-informed neural networks (PINN) algorithm at solver (tingnan ang [paper](https://arxiv.org/abs/1711.10561) at ang kaugnay na [GitHub repository](https://github.com/314arhaam/burger-pinn)).
...
Maaari mong gawin ang pareho gamit ang iyong data mula sa job_2, at mag-plot ng pagkukumpara sa PINN solution.
...
Sa iyong solusyon, maaari na mong piliin kung ano ang gagawin dito. Ipinapakita ng sumusunod kung paano i-plot ang resulta.
...
Maglaan ng isang minuto upang magbigay ng feedback sa tutorial na ito. Tutulungan kami ng iyong mga pananaw na pahusayin ang aming mga content offerings at user experience:
