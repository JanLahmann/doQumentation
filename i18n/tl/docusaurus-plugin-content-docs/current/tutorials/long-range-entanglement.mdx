...
*Tinatayang paggamit: 4 na minuto sa isang Heron r2 processor. (TANDAAN: Tantiya lamang ito. Maaaring mag-iba ang iyong runtime.)*

## Mga Resultang Matututunan {#learning-outcomes}
Pagkatapos makumpleto ang tutorial na ito, matututunan mo ang mga sumusunod:
* Paano ipatupad ang isang long-range CNOT gate gamit ang mga dynamic na circuit na may mid-circuit measurement (MCM) at klasikal na feedforward;
* Paano ipatupad ang katumbas na gate gamit ang isang unitary SWAP-based na pamamaraan;
* Paano ihambing ang parehong pamamaraan sa pamamagitan ng pagsukat ng gate fidelity bilang function ng distansya ng qubit.

## Mga Kinakailangan {#prerequisites}

Iminumungkahi namin na pamilyar ang mga gumagamit sa mga sumusunod na paksa bago dumaan sa tutorial na ito:
* [Mga pangunahing konsepto ng quantum computing](/learning/courses/basics-of-quantum-information), kabilang ang Bell state, entanglement, at mga quantum gate;
* Kaalaman sa [mga dynamic na circuit](/guides/classical-feedforward-and-control-flow) (mid-circuit measurement at klasikal na feedforward);
...
Sa notebook na ito, inaangkop namin ang protokol para sa IBM Quantum hardware at binibigyang-benchmark ang pagganap nito bilang function ng pagkakahiwalay ng control–target, inihahambing ito laban sa isang unitary SWAP-based na baseline.
...
Bago tumakbo sa tunay na QPU, bine-verify namin na ang parehong dynamic at unitary circuit ay gumagawa ng ideal na Bell state sa isang noiseless na simulator. Gumagamit kami ng IBM Quantum `Sampler` na may `AerSimulator` bilang backend mode, sa maliit na distansya na 6.
...
2. Magsagawa ng mga Bell measurement sa pagitan ng mga kalapit na qubit na hindi naka-entangle, na sunud-sunod na inililipat ang entanglement hanggang maibahagi ng control at target ang isang Bell pair.
...
#### I-initialize ang circuit {#initialize-circuit}
...
#### Maghanda ng mga Bell pair {#prepare-bell-pairs}
...
#### Mag-apply ng mga feedforward correction upang itama ang mga Pauli byproduct operator {#apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
#### Sukatin ang control at target na qubit {#measure-control-and-target-qubits}
...
Pinagsasama namin ang iba't ibang hakbang na tinukoy sa itaas upang lumikha ng isang long-range CX gate sa dalawang dulo ng isang isang-dimensyon (1D) na linya. Kasama sa mga hakbang ang sumusunod:
- Pag-initialize ng control qubit sa $|+\rangle$
...
Ngayon itayo ang mga circuit na sumusukat sa $XX$, $YY$, at $ZZ$ na mga batayan, tulad ng ginawa natin para sa mga dynamic circuit sa itaas.
...
Ngayong naitayo na natin ang parehong dynamic at unitary circuit para sa maliit na halimbawa na may `distance=6`, ita-transpile natin ang mga ito upang patakbuhin muna sa isang noiseless na simulator.
...
Maaari na nating patakbuhin ang eksperimento sa noiseless na simulator backend. Gumagamit kami ng Qiskit Runtime Sampler na may AerSimulator bilang backend mode upang isagawa ang mga circuit.
...
Sa hakbang na ito, gagawin natin ang sumusunod:
...
Sa isang noiseless na simulation, ibe-verify natin na ang fidelity metric ay $1$ para sa mga nagtayong circuit. Sa mga eksperimento sa tunay na QPU, ang pagsusuring ito ay magbibigay ng malinaw na larawan kung gaano kahusay ang pagganap ng mga dynamic na circuit kumpara sa unitary baseline na implementasyon.
...
Tulad ng inaasahan sa isang noiseless na simulation, ang mga fidelity sa parehong dynamic at unitary circuit ay $1$.

## Malaking Halimbawa sa Hardware {#large-scale-hardware-example}

Dito ngayon ay pinagsasama-sama natin ang lahat ng mga detalyeng ito sa isang solong workflow sa mas malaking sukat, na pagkatapos ay pinapatakbo sa tunay na quantum hardware.

### Bumuo ng mga circuit para sa iba't ibang distansya {#generate-circuits-for-different-distances}

Bumubuo tayo ngayon ng mga long-range CX circuit para sa iba't ibang pagitan ng qubit na hanggang 60 qubit ang pagitan. Para sa bawat distansya, nagtatayo tayo ng mga circuit na sumusukat sa $XX$, $YY$, at $ZZ$ na mga batayan, na gagamitin sa kalaunan para sa pagkalkula ng fidelity.
...
Tinitiyak ng sumusunod na hakbang na sinusuportahan ng backend ang `if_else` na instruksyon, na kailangan para sa mas bagong bersyon ng dynamic na circuit. Dahil nasa early access pa ang feature na ito, tahasan nating idinaragdag ang `IfElseOp` sa backend target kung wala pa ito.
...
#### Gamitin ang Layer Fidelity string para sa pagpili ng 1D chain {#use-layer-fidelity-string-for-selecting-1d-chain}
Dahil nais naming ihambing ang pagganap ng mga dynamic at unitary circuit sa isang 1D chain, ginagamit namin ang Layer Fidelity string upang pumili ng isang linear na topology ng pinakamahusay na hanay ng mga qubit mula sa device. Tinitiyak nito na ang parehong uri ng circuit ay nati-transpile sa ilalim ng parehong mga limitasyon sa koneksyon, na nagbibigay-daan sa isang makatarungang paghahambing ng kanilang pagganap.
...
Kung natagpuan mong kawili-wili ang gawaing ito, maaaring interesado ka sa mga sumusunod na materyales:
