---
description: 'Gamitin ang fractional gates, mga parameterized gates na direktang nagpapatupad ng arbitrary-angle rotations, upang bawasan ang depth at duration ng mga quantum kernel circuit.'
notebook_path: tutorials/fractional-gates.ipynb
sidebar_label: 'Quantum kernels na may fractional gates'
title: 'Quantum kernels na may fractional gates'
---
...
Pagkatapos dumaan sa tutorial na ito, dapat mong maunawaan:
- Ano ang fractional gates at kung paano nila binabawasan ang circuit depth at duration sa mga IBM&reg; QPU
- Ang mga hadlang na kaugnay sa paggamit ng fractional gates (partikular, ang RZZ angle range)
- Kung paano bumuo ng quantum kernel workflow na gumagamit ng fractional gates gamit ang IBM Quantum Compute Service
- Kung paano ihambing ang mga hardware-execution metrics (depth, duration, non-local gate count, fidelity) na may at walang fractional gates
- Kung paano gumamit lamang ng fractional RX gates habang pinapanatili ang standard Qiskit patterns workflow

## Prerequisites {#prerequisites}
Iminumungkahi namin na pamilyar ka sa mga sumusunod na paksa bago dumaan sa tutorial na ito:
- Ang [Qiskit patterns](/guides/intro-to-patterns) workflow
- Ang [Fractional gates](/guides/fractional-gates) guide
- Ang [Quantum kernel training](/tutorials/quantum-kernel-training) tutorial at ang [Quantum kernels](/learning/courses/quantum-machine-learning/quantum-kernel-methods) lesson ng Quantum machine learning course
...
Sa pamamagitan ng paggamit ng native interactions sa pagitan ng mga physical qubit, maaari mong mas epektibong magpatupad ang ilang unitaries sa hardware.
...
### Workflow approaches for the RZZ angle constraint {#workflow-approaches-for-the-rzz-angle-constraint}
...
May dalawang paraan upang matiyak na natutugunan ang kundisyong ito, gaya ng tatalakayin natin sa ibaba. Inirerekomenda namin ang pangalawang paraan, at sa tutorial na ito, ipinapakita namin ito sa pamamagitan ng isang halimbawang inspirasyon mula sa quantum kernel method.
Upang mas maunawaan kung saan malamang na magiging kapaki-pakinabang ang quantum kernels, inirerekomenda naming basahin ang [Liu, Arunachalam & Temme (2021)](https://www.nature.com/articles/s41567-021-01287-z).

Maaari rin ninyong gawin ang [Quantum kernel training](/tutorials/quantum-kernel-training) tutorial at ang [Quantum kernels](/learning/courses/quantum-machine-learning/quantum-kernel-methods) lesson sa Quantum machine learning course sa IBM Quantum&reg; Learning.
...
```
Upang maiwasan ang error na ito, gamitin ang pangalawang paraan na inilarawan sa ibaba.
...
Ang approach na ito ay hinihiling sa inyo na mag-assign ng parameter values sa mga quantum circuit bago ang transpilation.
...
Tandaan na ang workflow na ito ay nagdudulot ng mas mataas na computational cost kaysa sa unang paraan, dahil kinasasangkutan nito ang pag-assign ng parameter values sa mga quantum circuit at pag-imbak ng parameter-bound circuits nang lokal.

:::caution


**Pansinin ang isang kilalang isyu sa `qiskit-ibm-runtime` v0.47.0** kung saan ang mga RZZ gate na may invalid angles ay maaaring manatili sa mga circuit kahit pagkatapos ng transpilation sa ilang partikular na sitwasyon.

Tingnan ang [qiskit-ibm-runtime#2441](https://github.com/Qiskit/qiskit-ibm-runtime/issues/2441) upang masubaybayan ang progreso sa isyung ito.
Inirerekomenda namin ang sumusunod na workaround hanggang sa ito ay malutas.
...
Sa seksyong ito, dadaanan natin ang apat na hakbang ng [Qiskit patterns](/guides/intro-to-patterns) workflow sa isang simulator, gamit ang quantum kernel circuit bilang isang praktikal na halimbawa.
...
Bagamat nagbibigay ang Qiskit ng built-in `zz_feature_map`, hindi pa nito sinusuportahan ang RZZ gates simula Qiskit v2.4.1 ([tingnan ang issue](https://github.com/Qiskit/qiskit/issues/14469)).
...
### Paghahambing ng depths, durations, at fidelities {#comparison-of-depths-durations-and-fidelities}
...
Sa seksyong ito, bina-benchmark natin ang quantum kernel workflow na may at walang fractional gates sa quantum hardware na may hanggang 40 qubits.

### Step 1-4 combined {#step-1-4-combined}

Ang workflow ay sumusunod sa parehong istruktura ng small-scale example. Iti-transpile natin ang lahat ng circuit na may at walang fractional gates, kokolektahin ang mga metrics, at pagkatapos ay isusumite ang mga circuit sa tunay na quantum hardware.
...
Pagkatapos ay ihahambing natin ang mga metrics.
...
Sa seksyong ito, ipinapakita natin kung paano i-optimize ang mga U gate gamit ang fractional RX gates, batay sa parehong circuit na ipinakilala sa nakaraang seksyon.
...
Kung nakakita ka ng kawili-wili ang gawaing ito, maaaring interesado ka sa mga sumusunod na materyal:
...
- Ang [Quantum kernel training](/tutorials/quantum-kernel-training) tutorial
- Ang [Quantum kernels](/learning/courses/quantum-machine-learning/quantum-kernel-methods) lesson sa Quantum machine learning course
</Admonition>
