...
중첩 페널티를 계산하기 위해 다음 함수를 사용합니다. 이는 여전히 문제를 양자 회로에 매핑하는 과정의 일부라는 점에 유의하세요. 하지만 이전 레슨에서 논의했듯이, 이 함수는 현재 변분 회로와 이전에 얻은 더 낮은 에너지/비용 상태의 최적화된 회로 사이의 중첩을 계산합니다. 새로 생성된 회로 역시 실제 하드웨어에서 실행하기 위해 트랜스파일링되어야 합니다. 이 함수는 시뮬레이터에서 사용하는 것을 이전에 본 적이 있습니다. 여기서는 실제 백엔드를 사용할 때의 트랜스파일링과 관련된 최적화를 이미 고려해야 하므로, `if realbackend == 1` 주변의 라인이 있는 것입니다. 이는 2단계와 다소 혼합되지만, 나중에 2단계를 명시적으로 다룰 것입니다.
...
이제 VQD의 비용 함수를 추가합니다. 이전 레슨과 비교했을 때, 실제 백엔드를 사용할 때 트랜스파일에 도움이 되는 두 개의 추가 인수(`realbackend`와 `backend`)가 있다는 점에 유의하세요.
...
다시 한 번, 디버깅을 위해 시뮬레이터를 사용한 다음 실제 하드웨어로 넘어가겠습니다.
...
여기서는 계산하고자 하는 상태의 수, 페널티, 그리고 초기 매개변수 집합 x0를 소개합니다.
...
이 비용 함수를 최소화하여 최적의 매개변수를 계산할 수 있으며, 먼저 로컬 시뮬레이터를 사용하여 코드를 확인할 수 있습니다.
...
$$
이 연산자는 엣지로 연결된 노드에 대한 Z 연산자를 가진 항들의 선형 결합입니다(0번째 큐비트가 가장 오른쪽임을 기억하세요): $IIZZ + IZIZ + IZZI + ZIIZ + ZZII$. 연산자가 구성되면, QAOA 알고리즘의 안자츠는 Qiskit 회로 라이브러리의 `QAOAAnsatz` 회로를 사용하여 쉽게 만들 수 있습니다.
...
어떤 고전적인 최적화기든 비용 함수를 최소화하는 데 사용할 수 있습니다. 실제 양자 시스템에서는 매끄럽지 않은 비용 함수 지형을 위해 설계된 최적화기가 대체로 더 나은 성능을 보입니다. 여기서는 minimize 함수를 통해 SciPy의 [COBYLA 루틴](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-cobyla.html#optimize-minimize-cobyla)을 사용합니다.

Runtime에 대한 많은 호출을 반복적으로 실행하고 있으므로, 단일 블록 내에서 모든 호출을 실행하기 위해 세션을 사용합니다. 게다가 QAOA의 경우, 해는 최소화로부터 얻은 최적 매개변수와 결합된 안자츠 회로의 출력 분포에 인코딩되어 있습니다. 따라서 Sampler primitive가 필요하며, 동일한 세션으로 이를 인스턴스화합니다
그리고 최소화 루틴을 실행합니다:
...
매개변수 각도의 해 벡터(`x`)를 안자츠 회로에 대입하면, 우리가 찾던 그래프 분할이 나옵니다.
