...
`options.resilience_level = 2`로 설정하면 됩니다. `noise_factors`, `noise_amplifiers`, `extrapolators`를 다양하게 탐색하여 더욱 최적화할 수 있지만, 이는 이 과정의 범위를 벗어납니다. [여기에 설명된 옵션](/guides/configure-error-mitigation)을 직접 실험해 보세요.
