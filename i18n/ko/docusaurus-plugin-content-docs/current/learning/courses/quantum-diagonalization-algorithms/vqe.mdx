...
![원자 오비탈 이미지와 수많은 노드와 연결로 이루어진 네트워크 이미지.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig1.svg)
...
![변분 매개변수 "theta"가 표시된 양자 Circuit 이미지.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig2.svg)
...
![에너지를 나타내는 곡선과 최솟값을 찾기 위해 여러 지점을 테스트하는 모습을 보여주는 이미지.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig3.svg)
...
그러나 그 파울리 연산자들을 모두 동시에 측정할 수는 없습니다. (항등을 제외한) 파울리 연산자는 동일한 Qubit와 연관된 경우 서로 교환되지 않습니다. 예를 들어 `IZIZ`와 `ZZXZ`는 동시에 측정할 수 있는데, 세 번째 Qubit에 대해 I와 Z를 동시에 측정할 수 있고, 첫 번째 Qubit에 대해 I와 X를 동시에 알 수 있기 때문입니다. 그러나 `ZZZZ`와 `ZZZX`는 동시에 측정할 수 없습니다. Z와 X가 교환되지 않으며 둘 다 0번째 Qubit에 작용하기 때문입니다. 경험이 많은 독자라면 두 파울리 연산자 그룹이 집합으로서는 교환 가능할 수 있지만, 각 개별 Qubit의 측정은 교환 불가능할 수도 있다는 점을 떠올릴 수 있을 것입니다. Estimator는 Qubit별로 교환 가능한 연산자를 그룹화하는 방식에 해당하는 텐서곱 파울리 측정(기저 회전을 통해)을 가정합니다. 따라서 Estimator를 사용하여 두 개의 파울리 연산자 문자열(A와 B)을 동시에 추정하려면, A와 B의 각 Qubit에 대한 파울리 연산자들이 교환 가능해야 합니다. 이는 `ZZZZ`와 `ZZXX`도 동시에 측정할 수 없음을 의미합니다.

![다양한 파울리 문자열이 표시된 표로, 일부는 교환 가능하고 나머지는 그렇지 않습니다.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig4.svg)
...
![최솟값을 찾는 두 가지 방식을 보여주는 곡선 이미지 두 개. 하나는 최솟값을 찾기 위해 무작위로 점을 확인하고, 다른 하나는 인접한 두 점 사이의 직선을 그려 기울기를 추정합니다.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig5.svg)
...
![고전적 변분 접근과 양자 변분 접근을 비교하는 표. 두 방법 모두 좋은 초기 추측값이 필요합니다. 고전적 방법에서 비용은 행렬 차원의 제곱에 비례하고, 양자 접근에서는 교환 가능한 파울리 연산자 그룹의 수에 따라 달라집니다.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig6.svg)
