...
기댓값 추정에 필요한 과정들은 Estimator라는 Qiskit Runtime 프리미티브에 통합되어 있습니다. Estimator에 대한 자세한 내용은 IBM Quantum&reg; Documentation의 [API 참조](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2)를 참고하세요. Estimator를 직접 사용할 수 있지만, Estimator는 가장 낮은 에너지 고유값 이상의 정보를 반환합니다. 예를 들어, 앙상블 표준 오차에 대한 정보도 반환합니다. 따라서 최소화 문제의 맥락에서, Estimator가 비용 함수 안에 포함된 경우를 자주 볼 수 있습니다. Estimator의 입력 및 출력에 대한 자세한 내용은 IBM Quantum Documentation의 이 [가이드](/guides/primitive-input-output)를 참고하세요.
