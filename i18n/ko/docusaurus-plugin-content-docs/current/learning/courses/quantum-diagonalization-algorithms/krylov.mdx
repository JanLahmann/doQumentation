---
description: '크릴로프 양자 대각화(KQD)를 소개합니다. 고전적 크릴로프 방법에서 출발하여 수렴성과 계산 비용을 다룹니다.'
...
__Krylov 부분공간 방법__은 __Krylov 부분공간__이라고 불리는 것을 중심으로 구축된 여러 방법 중 하나를 가리킬 수 있습니다. 이에 대한 완전한 검토는 이 강의의 범위를 벗어나지만, 참고문헌 [\[2-4\]](#references)에서 훨씬 더 많은 배경 지식을 얻을 수 있습니다. 여기서는 Krylov 부분공간이 무엇인지, 고유값 문제를 풀 때 어떻게 그리고 왜 유용한지, 마지막으로 이를 양자 컴퓨터에서 어떻게 구현할 수 있는지에 초점을 맞추겠습니다.
__정의:__ 대칭이고 양의 준정부호인 $N\times N$ 행렬 $A$가 주어졌을 때, 차수 $r$의 Krylov 공간 $\mathcal{K}^r$은 참조 벡터 $\vert v \rangle$에 행렬 $A$의 더 높은 거듭제곱을 $r-1\leq N$까지 곱하여 얻은 벡터들로 생성되는 공간입니다.
...
관심 있는 행렬의 차원 $N$을 넘어 Krylov 부분공간의 차원 $r$을 확장하는 것이 (a) 왜 유용하지 않은지, 그리고 (b) 왜 불가능한지 설명하세요.

<Accordion>
<AccordionItem title="Answer">

(a) 벡터를 생성하면서 정규직교화하고 있으므로, $N$개의 그러한 벡터 집합은 완전한 기저를 형성하게 되며, 이는 그들의 선형 결합을 사용하여 공간 내의 어떤 벡터든 만들 수 있음을 의미합니다.

(b) 직교화 과정은 새로운 벡터를 이전의 모든 벡터에 투영한 것을 빼는 작업으로 구성됩니다. 이전의 모든 벡터가 전체 벡터 공간을 생성한다면, 전체 부분공간에 대한 투영을 빼는 것은 항상 영벡터를 남기게 됩니다.
...
여러분의 동료가 실수로 초기 벡터로 고유벡터를 선택했습니다. 초기 벡터에 행렬을 작용시키면 고유값으로 스케일된 동일한 벡터가 그대로 반환될 뿐입니다. 이는 차원이 증가하는 부분공간을 생성하지 않습니다. 동료에게 고유벡터가 아닌 다른 초기 벡터를 선택하도록 조언하세요.
...
### 1.2 Krylov 방법의 유형 {#12-types-of-krylov-methods}
...
### 1.3 Krylov 부분공간 방법이 효과적인 이유 {#13-why-the-krylov-subspace-method-works}
...
마지막으로, 이 접근 방식의 스케일링에 대한 수치적 주장은 하지 않았으며, 희소 행렬에 대한 비교 이점도 논의하지 않았음을 다시 한번 강조합니다. 이는 Krylov 부분공간 방법을 사용하는 이유를 설득하고 직관적인 이해를 돕기 위한 것일 뿐입니다. 이제 이 방법들의 수치적 거동을 탐구해 보겠습니다.

### 이해도 확인 {#check-your-understanding-1}

위의 워크플로우에서, 우리는 오류의 $A$-노름을 최소화하는 것을 제안했습니다. 바닥 상태와 그 고유값을 찾는 데 있어 최소화를 고려할 수 있는 다른 양은 무엇일까요?
...
위 그래프에서 차원 약 125 부근의 이상하게 높은 시간은 행렬의 무작위성 또는 사용된 고전 프로세서의 구현 방식 때문일 수 있으며, 재현되지 않습니다. 코드를 다시 실행하면 이상 피크의 위치가 다른 프로파일이 나타납니다.
...
Krylov 부분공간을 구축하는 데 필요한 시간은 처음에는 numpy의 완전 대각화에 필요한 시간을 초과한다는 점에 유의하세요. 하지만 행렬의 크기가 커질수록 Krylov 방법이 유리해집니다. 이는 허용 가능한 오류를 낮추더라도 마찬가지이지만, 그 이점은 더 큰 행렬 크기에서 나타납니다. 이는 자세히 살펴볼 가치가 있습니다.

수치 대각화의 시간 복잡도는 $O(n^3)$입니다(알고리즘에 따라 다소 차이가 있음). $n$개 벡터의 정규직교 기저를 생성하는 시간 복잡도도 $O(n^3)$입니다. 따라서 Krylov 방법의 이점은 $\it{어떤}$ 정규직교 기저를 사용하는 것과 __관련이 없으며__, 관심 있는 고유값을 효과적으로 골라내는 특정한 정규직교 기저를 사용하는 것과 관련이 있습니다. 우리는 이미 이 강의의 첫 번째 섹션에서 증명의 개요를 통해 이를 확인했으며, 이는 Krylov 방법의 수렴 보장에 있어 매우 중요합니다.
지금까지의 진행 상황을 정리해 봅시다:
* 매우 큰 행렬의 경우, Krylov 부분공간 방법은 전통적인 대각화 알고리즘보다 더 빠르게 필요한 허용 오차 내의 근사 고유값을 산출할 수 있습니다.
* 이러한 매우 큰 행렬의 경우, Krylov 부분공간을 생성하는 것이 Krylov 부분공간 방법에서 가장 시간이 많이 걸리는 부분입니다.
* 따라서 Krylov 부분공간을 생성하는 효율적인 방법은 매우 가치 있을 것입니다.
마침내 여기서 양자 컴퓨터가 등장하게 됩니다.

### 이해도 확인 {#check-your-understanding-2}

위의 행렬 차원 대비 대각화 시간의 평활화된 그래프를 참조하세요.
...
(b) 계산의 어떤 측면이 변화하면 Krylov 방법이 더 빨라지는 차원이 바뀔 수 있을까요?
...
$$
여기서 $P_\alpha$는 $P_\alpha \sim IZIXII...YZXIX$ 형태의 Pauli 문자열이거나 서로 가환인 Pauli 문자열들의 집합입니다. $H$를 이러한 측정 가능한 연산자들의 합으로 쓸 수 있다면, $\tilde{H}$의 행렬 원소에 대한 다음 식들을 IBM Quantum primitive Estimator를 사용해 구현할 수 있습니다.
...
![서로 다른 상태에서 수정된 Hadamard 테스트를 수행하기 위해 회로를 여러 번 사용해야 함을 보여주는 여러 층의 회로 다이어그램.](/learning/images/courses/quantum-diagonalization-algorithms/krylov/kqd-fig4.avif)

이 그림은 서로 다른 양자 상태 간의 겹침을 계산하는 데 사용되는 수정된 Hadamard 테스트의 회로 표현을 보여줍니다. 각 행렬 원소 $\tilde{H}_{i,j}$에 대해 상태 $\vert \psi_i \rangle$와 $\vert \psi_j \rangle$ 사이의 Hadamard 테스트가 수행됩니다. 이는 그림의 행렬 원소에 대한 색 구분과 대응하는 $\text{Prep} \; \psi_i$, $\text{Prep} \; \psi_j$ 연산으로 강조되어 있습니다. 따라서 사영된 해밀토니안 $\tilde{H}$의 모든 행렬 원소를 계산하려면 Krylov 공간 벡터들의 가능한 모든 조합에 대한 Hadamard 테스트 집합이 필요합니다. Hadamard 테스트 회로의 맨 위 와이어는 보조(ancilla) 큐비트로, X 또는 Y 기저에서 측정되며, 그 기댓값이 상태 간 겹침의 값을 결정합니다. 아래 와이어는 시스템 해밀토니안의 모든 큐비트를 나타냅니다. $\text{Prep} \; \psi_i$ 연산은 보조 큐비트의 상태에 의해 제어되어 시스템 큐비트를 상태 $\vert \psi_i \rangle$로 준비하며 ($\text{Prep} \; \psi_j$도 마찬가지), 연산 $P$는 시스템 해밀토니안의 Pauli 분해 $H = \sum_i P_i$를 나타냅니다. 양자 컴퓨터에서의 구현은 아래에서 보다 자세히 설명합니다.
...
초기 벤치마킹 단계에서는 수렴 거동을 확인하기 위해 정확한 고전적 해를 알고 있는 것이 유용합니다. 아래 함수는 해밀토니안과 큐비트 수를 인수로 받아 해밀토니안의 바닥 상태 에너지를 고전적으로 계산합니다.
...
### 4.1 단계 1: 문제를 양자 회로 및 연산자로 매핑하기 {#41-step-1-map-problem-to-quantum-circuits-and-operators}
...
사용할 최대 Krylov 차원을 지정합니다(더 작은 차원에서의 수렴 여부도 확인할 예정입니다). 시간 진화에 사용할 트로터(Trotter) 단계 수도 지정합니다. 이 예제에서는 최대 Krylov 차원을 4로 지정합니다. 실제 응용에는 충분히 크지 않지만 이 예제에는 충분합니다. 이후 레슨에서는 해밀토니안을 더 큰 부분공간으로 확장하고 투영할 수 있는 방법을 살펴볼 것입니다.
...
$\tilde{H}$와 그람 행렬 $\tilde{S}$의 행렬 원소를 모두 아다마르 테스트로 계산하려 한다는 점을 상기하세요. 먼저 $\tilde{H}$의 구성에 초점을 맞추어 이 과정이 어떻게 동작하는지 살펴보겠습니다. 전체 과정은 아래에 도식으로 표현되어 있습니다. 색으로 표시된 상태 준비 블록 $\text{Prep}|\psi_i\rangle$의 여러 레이어는 이 과정이 부분공간 내의 $|\psi_i\rangle$와 $|\psi_j\rangle$의 모든 조합에 대해 수행됨을 나타냅니다.
...
위의 단계 4에서 시작하여 영번째 큐비트에 아다마르 게이트 $H$를 적용합니다.
...
트로터 회로에서 발생하는 깊이(depth) 문제에 대해 이미 경고한 바 있습니다. 이 조건에서 아다마르 테스트를 수행하면 기본 게이트로 분해한 후 회로가 훨씬 더 깊어질 수 있으며, 소자의 토폴로지(topology)를 고려하면 더욱 심해집니다. 따라서 양자 컴퓨터에서 시간을 사용하기 전에 회로의 2-큐비트 깊이를 확인하는 것이 좋습니다.
...
![수정된 효율적인 Hadamard 테스트를 수행하기 위해 여러 다양한 유니터리 연산자에 대해 회로를 평가해야 함을 나타내는 다층 레이어의 양자 회로 다이어그램 이미지](/learning/images/courses/quantum-diagonalization-algorithms/krylov/kqd-fig6.avif)
...
게이트 $\text{Prep} \; \psi_0$가 원하는 기준 상태 $\ket{\psi_0} = \text{Prep} \; \psi_0 \ket{0} = e^{-i H 0 dt} U_{\psi_0} \ket{0}$를 준비한다고 할 때, 예를 들어 화학의 HF 상태를 준비하기 위해 $\text{Prep} \; \psi_0$는 단일 큐비트 NOT의 곱이 되므로, controlled-$\text{Prep} \; \psi_0$는 CNOT의 곱이 됩니다.
...
![이전 코드 셀의 출력](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/8d35e6cd-c0f6-4871-83da-86472d66be2c-0.avif)
...
![이전 코드 셀의 출력](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/d1d0b9de-65d4-4a46-975d-6cfaaac05f9a-0.avif)

#### Hadamard 테스트를 통한 $\tilde{S}$와 $\tilde{H}$의 행렬 원소 계산을 위한 템플릿 회로 {#template-circuits-for-calculating-matrix-elements-of-tildes-and-tildeh-via-hadamard-test}

Hadamard 테스트에 사용되는 회로 간의 유일한 차이점은 시간 진화 연산자의 위상과 측정되는 관측량입니다. 따라서 시간 진화 연산자에 의존하는 게이트의 자리 표시자를 포함하는 Hadamard 테스트의 일반적인 회로를 나타내는 템플릿 회로를 준비할 수 있습니다.
...
![이전 코드 셀의 출력](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/37382668-1999-4475-b50f-2887449d5c93-0.avif)
...
양자 Krylov 계산을 실행할 백엔드를 선택하여 해당 양자 컴퓨터에서 실행할 수 있도록 회로를 트랜스파일합니다.
...
![이전 코드 셀의 출력](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/054bcfba-fde3-4213-bbd7-d7a6dd6c03c8-1.avif)

최적화 후 트랜스파일된 2-큐비트 깊이가 더욱 줄어들었습니다.

### 4.3 단계 3. IBM Quantum 프리미티브를 사용하여 실행하기 {#43-step-3-execute-using-an-ibm-quantum-primitive}
...
### 4.4 단계 4. 후처리 및 결과 분석 {#44-step-4-post-process-and-analyze-results}
...
일부 해밀토니안은 큐비트에 매핑될 때 높은 복잡성을 가지며, 몇 개의 교환 그룹으로 쉽게 분리되지 않는 많은 비가환 파울리 문자열을 포함합니다. 이는 예를 들어 양자 화학 문제에서 흔히 나타납니다. 이러한 복잡성은 근시일 내(near-term) 양자 컴퓨터에 두 가지 주요 어려움을 야기합니다:
...
위의 두 문제는 양자 컴퓨터가 내결함성에 도달하면 덜 심각해지겠지만, 근시일 내에는 반드시 고려해야 합니다. 양자 화학보다 "단순한" 매핑을 가진 시스템이라도 해밀토니안에 비가환 항이 너무 많으면 동일한 장애를 겪을 수 있습니다.
...
이해하기 쉬운 상한으로 바꾸기 위해, 먼저 분자의 합을 $E_k-E_0\le\delta$인 항과 $E_k-E_0>\delta$인 항으로 나눕니다:
...
여기서 첫 번째 단계는 합계에 있는 모든 $E_k$에 대해 $E_k-E_0\le\delta$이기 때문이고, 두 번째 단계는 분자의 합이 분모의 합의 부분집합이기 때문입니다. 두 번째 항에 대해서는, 먼저 $f(E_0)^2=1$이므로 분모를 $|\gamma_0|^2$로 하한을 잡습니다: 모든 것을 다시 합치면 다음을 얻습니다

$$
\text{energy error} \le \delta + \frac{1}{|\gamma_0|^2}\sum_{E_k>E_0+\delta}|\gamma_k|^2f(E_k)^2(E_k-E_0).
$$

남은 부분을 단순화하기 위해, 이러한 모든 $E_k$에 대해 $f$의 정의에 의해 $f(E_k)^2 \le 4\left(1 + \delta\right)^{-2d}$임을 알 수 있습니다. 추가로 $E_k-E_0<2\|H\|$의 상한과 $\sum_{E_k>E_0+\delta}|\gamma_k|^2<1$의 상한을 적용하면 다음을 얻습니다

$$
\text{energy error} \le \delta + \frac{8}{|\gamma_0|^2}\|H\|\left(1 + \delta\right)^{-2d}.
$$

이는 임의의 $\delta>0$에 대해 성립하므로, $\delta$를 목표 오류와 같게 설정하면 위의 오류 한계는 Krylov 차원 $2d=r$에 따라 지수적으로 그 값에 수렴합니다. 또한 $\delta<E_1-E_0$이면 위 한계에서 $\delta$ 항이 완전히 사라진다는 점에 유의하세요.

논증을 완성하기 위해, 먼저 위의 내용은 Krylov 공간에서 가장 낮은 에너지 상태의 에너지 오류가 아니라 특정 상태 $f(H)|\psi\rangle$의 에너지 오류일 뿐이라는 점에 유의합니다. 하지만 (Rayleigh-Ritz) 변분 원리에 의해, Krylov 공간에서 가장 낮은 에너지 상태의 에너지 오류는 Krylov 공간의 어떤 상태의 에너지 오류로도 상한이 정해지므로, 위의 내용은 또한 가장 낮은 에너지 상태, 즉 Krylov 양자 대각화 알고리즘의 출력에 대한 에너지 오류의 상한이기도 합니다.

위와 비슷한 분석을 수행하여 노트북에서 논의된 노이즈와 임계값 설정 절차를 추가로 고려할 수 있습니다. 이 분석에 대해서는 [\[2\]](#references) 및 [\[4\]](#references)를 참조하세요.

### 부록 II: 주장 1의 증명 {#appendix-ii-proof-of-claim-1}

다음은 대부분 [\[3\]](#references)의 정리 3.1에서 유도된 것입니다: $0 < a < b$이고 $\Pi^*_d$를 차수가 최대 $d$인 잔차 다항식(0에서의 값이 1인 다항식)의 공간이라고 합시다. 다음의 해는
...
이를 복소 지수로 자연스럽게 표현할 수 있는 함수로 변환하고자 합니다. 왜냐하면 그것들이 양자 Krylov 공간을 생성하는 실제 시간 진화이기 때문입니다.
이를 위해, 해밀토니안의 스펙트럼 범위 내 에너지를 $[0,1]$ 범위의 숫자로 변환하는 것이 편리합니다: 다음을 정의합니다
...
$g(E_0)=0$이고 $g(E)$는 $E$가 $E_0$에서 멀어질수록 증가한다는 점에 유의하세요.
...
여기서 $E_0$는 바닥 상태 에너지입니다. $\cos(x)=\frac{e^{ix}+e^{-ix}}{2}$를 대입하면 $f(E)$가 차수 $d$의 삼각 다항식, 즉 $j=-d,-d+1,...,d-1,d$에 대한 $e^{ijE\,dt}$의 선형 결합임을 알 수 있습니다. 또한 위의 $p^*(x)$의 정의로부터 $f(E_0)=p(0)=1$이며, $\vert E-E_0 \vert > \delta$인 스펙트럼 범위 내의 임의의 $E$에 대해 다음이 성립함을 알 수 있습니다
