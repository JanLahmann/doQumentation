...
이 샘플 기반 크릴로프 양자 대각화(SKQD) 단원은 이전 방법들에서 설명한 방법들을 결합합니다. Qiskit 패턴 프레임워크를 활용한 단일 예제로 구성되어 있습니다:
    ...
    - 단계 3: IBM Quantum 프리미티브를 사용하여 실행하기

    - 단계 4: 후처리


샘플 기반 양자 대각화 방법의 중요한 단계는 부분공간에 대한 고품질 벡터를 생성하는 것입니다. 이전 단원에서는 LUCJ 앤셰츠를 사용하여 화학 해밀토니안의 부분공간 벡터를 생성했습니다. 이 단원에서는 단원 2에서 논의한 양자 크릴로프 상태[[1]](#references)를 사용합니다. 먼저 시간 진화 연산을 사용하여 양자 컴퓨터에서 크릴로프 공간을 만드는 방법을 검토합니다. 그 다음 이것으로부터 샘플을 추출합니다. 시스템 해밀토니안을 샘플링된 부분공간에 투영하고 대각화하여 바닥상태 에너지를 추정합니다. 이 알고리즘은 단원 2에서 설명한 가정 하에서 바닥상태로 증명 가능하고 효율적으로 수렴합니다.
...
행렬 $A$가 해밀토니안 $H$인 경우, 해당 공간을 _거듭제곱 크릴로프 공간_ $\mathcal{K}_P$라고 합니다. $A$가 해밀토니안으로 생성된 시간 진화 연산자 $U=e^{-iH(dt)}$인 경우, 이 공간을 유니터리 크릴로프 공간 $\mathcal{K}_U$라고 합니다. 거듭제곱 크릴로프 부분공간은 $H$가 유니터리 연산자가 아니기 때문에 양자 컴퓨터에서 직접 생성할 수 없습니다. 대신, 거듭제곱 크릴로프 공간과 유사한 [수렴 보장](https://arxiv.org/abs/2110.07492)을 제공할 수 있는 시간 진화 연산자 $U = e^{-iH(dt)}$를 사용할 수 있습니다. 그러면 $U$의 거듭제곱은 서로 다른 시간 단계 $U^k = e^{-iH(k dt)}$ (여기서 $k = 0, 1, 2, ..., (r-1)$)가 됩니다.
...
이 단원에서는 주기 경계 조건을 갖는 $L = 22$ 사이트의 반강자성 XX-Z 스핀-1/2 체인에 대한 해밀토니안을 고려합니다:
$$
...
다음으로, 바닥상태와의 겹침을 갖는 기준 상태 $\vert \psi \rangle$를 선택해야 합니다. 이 해밀토니안의 경우, 교대하는 1과 0 $\vert ...101...010...101 \rangle$을 가진 _Neel_ 상태를 기준 상태로 사용합니다.
...
이제 사전 설정된 패스 매니저(pass manager)를 사용하여 회로를 대상 백엔드로 변환합니다.
...
해밀토니안을 투영하고 대각화하기 위해 [`qiskit-addon-sqd`](https://qiskit.github.io/qiskit-addon-sqd/)의 기능을 사용합니다. 애드온은 Pauli 문자열 기반 해밀토니안을 부분공간에 투영하고 `SciPy`를 사용하여 고유값을 해결하기 위한 기능을 제공합니다.
...
원칙적으로, 부분공간을 확장하기 전에 부정확한 패턴을 가진 비트 문자열을 필터링할 수 있습니다. 예를 들어, 이 단원의 반강자성 해밀토니안의 바닥상태는 일반적으로 "상향" 및 "하향" 스핀의 개수가 같으므로, 비트 문자열의 "1"의 개수는 시스템의 비트(스핀) 총 개수의 정확히 절반이어야 합니다. 다음 함수는 개수에서 지정된 개수("1" 비트)를 갖지 않는 비트 문자열을 필터링합니다.
...
SKQD의 주요 장점은 (a) SQD와 (b) KQD에 비해 무엇입니까?
   ...
   (a) SKQD는 SQD가 갖지 못한 수렴 보장을 제공합니다. SQD에서는 계산 기저에서 바닥 상태 지지 집합과 매우 잘 겹치는 훌륭한 앤자츠 추측을 해야 하거나, 앤자츠 계열을 샘플링하기 위해 계산에 변분적 요소를 도입해야 합니다.
...
- 크릴로프 기저 상태 샘플링을 통한 바닥상태 에너지 추정은 스핀 시스템, 응축 물질 문제, 격자 게이지 이론을 포함한 격자 모델에 매우 적합합니다. 이 접근법은 VQE보다 훨씬 더 나은 스케일링을 이루는데, VQE처럼 변분 앤셰츠의 많은 매개변수에 대한 최적화가 필요하거나, 발견적 앤셰츠 기반 SQD(예: 이전 단원의 화학 문제)가 필요하지 않기 때문입니다.
    ...
    - 회로 깊이를 낮게 유지하기 위해, 내결함성 이전 하드웨어에 적합한 격자 문제를 다루는 것이 현명합니다.

- SKQD는 VQE처럼 양자 측정 문제를 야기하지 않습니다. 추정할 가환(commuting) 파울리 연산자 그룹이 없습니다.
