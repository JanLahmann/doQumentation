...
이 개념을 바탕으로 나이마크 정리를 증명할 준비가 되었습니다$.\mathsf{X}$가 $n$개의 고전 상태를 갖는다고 가정하면, 쌍 $(\mathsf{Y},\mathsf{X})$에 대한 유니터리 연산 $U$는 $nm\times nm$ 행렬로 표현될 수 있으며, 이를 블록이 $n\times n$인 $m\times m$ 블록 행렬로 볼 수 있습니다.
