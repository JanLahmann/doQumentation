...
숫자 $\lambda_0,\ldots,\lambda_{n-1}$은 $\rho$의 고유값이며$,\rho$가 양의 준정치이기 때문에, 이 숫자들은 음이 아닌 실수여야 합니다$.\rho$의 대각합이 $1$이라는 사실로부터 $\lambda_0 + \cdots + \lambda_{n-1} = 1$임을 결론 내릴 수 있습니다.
