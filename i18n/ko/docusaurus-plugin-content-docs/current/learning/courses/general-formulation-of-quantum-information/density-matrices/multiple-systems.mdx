...
Alice가 큐비트 $\mathsf{A}$를 보유하고 Bob이 $\mathsf{B}$를 보유한다고 상상할 수 있습니다. 즉, 함께 그들은 e-비트를 공유합니다.
우리는 Bob이 자신의 큐비트를 가지고 별로 여행을 떠나 다시는 보이지 않을 경우, 고립된 Alice의 큐비트 $\mathsf{A}$에 대한 밀도 행렬 설명을 원합니다.

먼저, Bob이 여정 어딘가에서 표준 기저 측정으로 자신의 큐비트를 측정하기로 결정했다면 어떻게 될지 생각해봅시다.
만약 그렇게 한다면, 그는 다음 확률로 결과 $0$을 얻게 됩니다
...
따라서 Bob의 측정 결과를 무시하고 Alice의 큐비트에 초점을 맞추면, 그녀가 상태 $\vert 0\rangle$을 확률 $1/2$로, 상태 $\vert 1\rangle$을 확률 $1/2$로 얻는다고 결론 내릴 수 있습니다.
...
우리는 또한 Bob의 큐비트 $\mathsf{B}$의 상태를 고려할 수 있으며, 이는 완전히 혼합된 상태이기도 합니다.
...
이제 방금 논의한 예제를 두 임의 시스템 $\mathsf{A}$와 $\mathsf{B}$로 일반화합시다. 반드시 상태 $\vert \phi^+\rangle$의 큐비트일 필요는 없습니다.
...
$\mathsf{B}$가 아니라 $\mathsf{A}$가 트레이스 아웃되도록, $\mathsf{A}$에 대해서도 부분 트레이스를 다음과 같이 정의할 수 있습니다.
...
이는 $\mathsf{A}$가 아니라 $\mathsf{B}$가 단독으로 있을 때의 상태에 대한 밀도 행렬 설명 $\rho_{\mathsf{B}}$를 제공합니다.

요약하면, $(\mathsf{A},\mathsf{B})$가 어떤 시스템 쌍이고 $(\mathsf{A},\mathsf{B})$의 상태를 설명하는 밀도 행렬 $\rho$가 있을 때, 시스템 $\mathsf{A}$와 $\mathsf{B}$의 *환산 상태(reduced state)*는 다음과 같습니다.
...
$\rho$가 밀도 행렬이라면, $\rho_{\mathsf{A}}$와 $\rho_{\mathsf{B}}$ 역시 반드시 밀도 행렬이 됩니다.

이러한 개념은 두 개 대신 임의의 수의 시스템으로 자연스러운 방식으로 일반화될 수 있습니다.
일반적으로, 밀도 행렬 $\rho$의 아래첨자에 우리가 선택한 어떤 시스템의 이름이든 넣어서 그 시스템들만의 환산 상태를 나타낼 수 있습니다.
예를 들어, $\mathsf{A}$, $\mathsf{B}$, $\mathsf{C}$가 시스템이고 $\rho$가 $(\mathsf{A},\mathsf{B},\mathsf{C})$의 상태를 설명하는 밀도 행렬이라면, 다음과 같이 정의할 수 있습니다
...
예를 들어, 큐비트 쌍 $(\mathsf{A},\mathsf{B})$의 이 상태를 고려합시다.
...
### 두 큐비트에 대한 부분 대각합 {#the-partial-trace-for-two-qubits}

부분 대각합은 또한 행렬 관점에서 명시적으로 설명될 수 있습니다.
여기서 우리는 두 큐비트만에 대해 이를 수행하지만, 이것은 더 큰 시스템으로도 일반화될 수 있습니다.
두 큐비트 $(\mathsf{A},\mathsf{B})$를 가진다고 가정합시다. 그래서 이 두 큐비트의 상태를 설명하는 모든 밀도 행렬을 다음과 같이 쓸 수 있습니다
...
다음은 첫 번째가 아니라 두 번째 시스템이 트레이스 아웃될 때의 공식입니다.
...
이전과 유사한 형태의 블록 행렬로 표현하면, 다음 공식을 얻습니다.
...
이러한 함수들의 블록 행렬 설명은 큐비트보다 큰 시스템으로 자연스럽고 직접적인 방식으로 확장될 수 있습니다.
...
그리고 두 번째 시스템 $\mathsf{B}$의 환산 상태는
