...
위에 나열된 문장들을 번호로 참조하면, 증명해야 할 첫 번째 함의는 1 $\Rightarrow$ 2입니다.
이 함의는 채널의 Choi 상태 맥락에서 이미 논의되었습니다.
여기서는 수학적 세부 사항을 요약하겠습니다.

입력 시스템 $\mathsf{X}$의 고전 상태 집합이 $\Sigma$이고 $n = \vert\Sigma\vert$라고 가정합니다.
$\mathsf{X}$의 두 복사본 중 두 번째에 $\Phi$가 다음 상태로 함께 적용되는 상황을 고려합니다
...
모든 밀도 행렬과 마찬가지로 이는 양의 준정부호(positive semidefinite)이어야 하며, 양의 준정부호 행렬에 양의 실수를 곱하면 또 다른 양의 준정부호 행렬이 나오므로, $J(\Phi) \geq 0$입니다.

더욱이 $\Phi$가 채널이라는 가정 하에, 이는 대각합을 보존해야 하므로
...
그런데 벡터 $\vert\psi_k\rangle$와 행렬 $A_k$ 사이에는 간단하고 직관적인 관계가 있습니다. 바로 $A_k$를 *벡터화(vectorizing)*하면 $\vert\psi_k\rangle$을 얻는다는 것입니다$.A_k$를 벡터화한다는 것은 열들을 (가장 왼쪽 열을 맨 위로, 가장 오른쪽 열을 맨 아래로) 차례로 쌓아 열벡터를 만드는 것을 의미합니다.
...
잠시 $U$가 유니터리라는 우려는 접어두고 다음 식에 집중해 봅시다

$$
\operatorname{Tr}_{\mathsf{G}} \bigl( U (\vert 0\rangle \langle 0 \vert_{\mathsf{W}} \otimes \rho)U^{\dagger}\bigr)
$$

이는 우리의 Stinespring 표현에 대해 $\mathsf{X}$의 입력 상태 $\rho$가 주어졌을 때 $\mathsf{Y}$의 출력 상태를 설명합니다.
대안적으로 다음과 같이 쓸 수 있습니다
...
따라서 매핑 $\Phi$에 대한 올바른 표현을 갖게 되었으며, 이제 $U$를 유니터리로 선택할 수 있는지 확인하는 일이 남았습니다.

위 패턴에 따라 $U$가 선택될 때 $U$의 처음 $n$개 열을 고려해 봅시다.
이 열들만 취하면 블록 행렬을 얻습니다
...
$\mathsf{X}$의 각 고전 상태마다 하나씩 총 $n$개의 열이 있으며, 벡터로서 이 열들을 각 $a\in\Sigma$에 대해 $\vert \gamma_a \rangle$라고 명명합시다.
다음은 위의 블록 행렬 표현과 일치시킬 수 있는 이 벡터들에 대한 공식입니다.

$$
\vert \gamma_a\rangle = \sum_{k = 0}^{N-1} \vert k\rangle \otimes A_k \vert a \rangle
$$

이제 이 벡터들 중 임의의 두 개, 즉 $a,b\in\Sigma$의 임의의 선택에 해당하는 벡터들 사이의 내적을 계산해 봅시다.
...
$n$개의 열 벡터 $\{\vert\gamma_a\rangle\,:\,a\in\Sigma\}$가 정규직교 집합을 이룬다는 결론을 얻습니다:
...
이는 $U$의 나머지 열들을 채워서 유니터리 행렬로 만드는 것이 가능함을 의미합니다.
특히, Gram-Schmidt 직교화 과정을 사용하여 나머지 열들을 선택할 수 있습니다. 상태 판별 문제의 맥락에서 "양자 정보의 기초"의 *양자 회로* 강의에서 이와 비슷한 작업이 수행된 바 있습니다.
