...
비국소 게임(nonlocal game)이 작동하는 방식은 심판이 먼저 Alice와 Bob 각각에게 질문을 하는 것입니다.
문자 $x$는 Alice의 질문을 나타내고 $y$는 Bob의 질문을 나타내는 데 사용하겠습니다.
여기서 $x$와 $y$는 고전 상태로 생각하며, CHSH 게임에서 $x$와 $y$는 비트입니다.
...
$(x,y) = (0,1)$일 때, Alice와 Bob은 $\alpha = 0$과 $\beta = -\pi/8$을 선택하며, 이들의 기저를 겹쳐보면 다음 그림을 얻습니다.
...
Alice와 Bob의 결과가 일치할 확률은 다시 이 각도의 코사인 제곱입니다.
...
$(x,y) = (1,0)$일 때, Alice와 Bob은 $\alpha = \pi/4$와 $\beta = \pi/8$을 선택하며, 이들의 기저를 겹쳐보면 다음 그림을 얻습니다.

![Alice and Bob bases case 1](/learning/images/courses/basics-of-quantum-information/entanglement-in-action/strategy10.avif)

기저는 바뀌었지만 각도는 바뀌지 않았습니다 — 같은 색의 벡터 사이의 각도는 여전히 $\pi/8$입니다.
Alice와 Bob의 결과가 일치할 확률은 다음과 같습니다.
...
$(x,y) = (1,1)$일 때, Alice와 Bob은 $\alpha = \pi/4$와 $\beta = -\pi/8$을 선택합니다. 이들의 기저를 겹쳐보면 무언가 다른 일이 일어난 것을 알 수 있습니다.

![Alice and Bob bases case 1](/learning/images/courses/basics-of-quantum-information/entanglement-in-action/strategy11.avif)

각도가 선택된 방식 때문에, 이번에는 같은 색의 벡터 사이의 각도가 $\pi/8$이 아니라 $3\pi/8$입니다.
Alice와 Bob의 결과가 일치할 확률은 여전히 이 각도의 코사인 제곱이지만, 이번에는 값이 다음과 같습니다.

$$
\cos^2\Bigl(\frac{3\pi}{8}\Bigr) = \frac{2 - \sqrt{2}}{4}.
$$

결과가 일치하지 않을 확률은 이 각도의 사인 제곱이며, 이 경우 다음과 같습니다.
