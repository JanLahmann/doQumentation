...
우리가 다룰 첫 번째 한계 — 실제로는 실제 한계라기보다는 양자 상태가 양자 상태 벡터로 표현되는 방식에서의 약간의 축퇴(degeneracy)에 가까운 것입니다 — 는 *전역 위상(global phase)*의 개념에 관한 것입니다.

전역 위상이 의미하는 바는 다음과 같습니다.
$\vert \psi \rangle$와 $\vert \phi \rangle$가 어떤 시스템의 양자 상태를 나타내는 단위 벡터이고, 단위원 위의 복소수 $\alpha$가 존재하여, 즉 $\vert \alpha \vert = 1$이거나 다르게 표현하면 어떤 실수 $\theta$에 대해 $\alpha = e^{i\theta}$이며, 다음이 성립한다고 가정합시다.

$$
\vert \phi \rangle = \alpha \vert \psi \rangle.
$$

그러면 벡터 $\vert \psi \rangle$와 $\vert \phi \rangle$는 *전역 위상만큼 다르다*고 합니다.
또한 우리는 때때로 $\alpha$를 *전역 위상*이라고도 부르지만, 이는 맥락에 따라 다릅니다.
단위원 위의 어떤 숫자든 단위 벡터에 곱해질 때 전역 위상으로 생각할 수 있습니다.

시스템이 두 양자 상태 $\vert\psi\rangle$와 $\vert\phi\rangle$ 중 하나에 있고, 시스템이 표준 기저 측정을 겪을 때 어떤 일이 일어나는지 생각해 보세요.
첫 번째 경우, 즉 시스템이 상태 $\vert\psi\rangle$에 있는 경우, 주어진 고전 상태 $a$를 측정할 확률은 다음과 같습니다.

$$
\bigl\vert \langle a \vert \psi \rangle \bigr\vert^2.
$$

두 번째 경우, 즉 시스템이 상태 $\vert\phi\rangle$에 있는 경우, 고전 상태 $a$를 측정할 확률은 다음과 같습니다.
...
이제 두 상태 모두에 임의의 유니터리 연산 $U$를 적용할 때 어떤 일이 일어나는지 생각해 보세요.
첫 번째 경우, 즉 초기 상태가 $\vert \psi \rangle$인 경우, 상태는 다음과 같이 됩니다.

$$
U \vert \psi \rangle,
$$

그리고 두 번째 경우, 즉 초기 상태가 $\vert \phi\rangle$인 경우, 다음과 같이 됩니다.
