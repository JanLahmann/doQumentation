...
![네 개의 수평선과 많은 게이트가 있는 회로 다이어그램](/learning/images/courses/integrating-quantum-and-high-performance-computing/programming-models/checkin-circuit.avif)
...
분자의 전자를 모델링한다고 가정합시다. (a) 분자의 바닥 상태 에너지와 (b) 분자의 바닥 상태에서 어떤 계산 기저 상태가 가장 우세한지를 근사하려고 합니다. 각 경우에 Estimator와 Sampler 프리미티브 중 어느 것을 사용하겠습니까?
...
`mapping.py` 프로그램은 양자 머신러닝 문헌과 양자 벤치마크 문헌에서 자주 사용되는 `PauliTwoDesign` 회로를 구축하도록 설계되었으며, $n$큐비트 시스템의 $(n-1)^\text{th}$ 큐비트를 $Z$ 방향으로 측정하는 간단한 관측 가능량과 무작위 초기 매개변수를 사용합니다. 이 각각(qasm 파일로 변환된 양자 회로, 관측 가능량, 매개변수)은 데이터 디렉터리 아래 별도의 파일로 저장되며 최적화 단계의 입력으로 사용됩니다.
