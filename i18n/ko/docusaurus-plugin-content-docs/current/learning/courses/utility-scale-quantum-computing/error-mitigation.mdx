...
원본 강의 [PDF 다운로드](https://ibm.ent.box.com/public/static/a0zgies7bh91hm2lwev9o0bfeybxc6n6.zip). 일부 코드 스니펫은 정적 이미지이므로 더 이상 지원되지 않을 수 있다는 점에 유의해 주세요.
...
이번 강의 전체에 걸쳐 양자 컴퓨터에서 잡음과 이를 완화하는 방법을 살펴봅니다. 먼저 시뮬레이터로 잡음의 영향을 살펴봅니다. 이 시뮬레이터는 실제 양자 컴퓨터의 잡음 프로파일을 불러오는 방식을 비롯해 여러 방법으로 잡음을 재현할 수 있습니다. 그런 다음 잡음이 본질적으로 존재하는 실제 양자 컴퓨터로 이동합니다. ZNE(영잡음 외삽)와 게이트 트워링(gate twirling) 같은 기법들의 조합을 포함한 오류 완화의 효과를 살펴보겠습니다.
...
최적화(`optimization_level=1`)를 적용하여 Backend용으로 Circuit을 트랜스파일합니다.
...
### 3.3 Estimator를 사용한 실행 (서로 다른 복원력 레벨 적용) {#33-execute-using-estimator-with-different-resilience-levels}

복원력 레벨(`estimator.options.resilience_level`) 설정은 Qiskit Estimator를 사용할 때 오류 완화를 적용하는 가장 쉬운 방법입니다. Estimator는 다음과 같은 복원력 레벨을 지원합니다(2024/06/28 기준). 자세한 내용은 [오류 완화 구성](/guides/error-mitigation-and-suppression-techniques) 가이드에서 확인할 수 있습니다.
...
- [오류 완화 구성](/guides/error-mitigation-and-suppression-techniques)
