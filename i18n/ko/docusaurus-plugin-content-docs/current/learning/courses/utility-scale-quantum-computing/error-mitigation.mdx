...
복원력 레벨(`estimator.options.resilience_level`) 설정은 Qiskit Estimator를 사용할 때 오류 완화를 적용하는 가장 쉬운 방법입니다. Estimator는 다음과 같은 복원력 레벨을 지원합니다(2024/06/28 기준). 자세한 내용은 [오류 완화 구성](/guides/configure-error-mitigation) 가이드에서 확인할 수 있습니다.
