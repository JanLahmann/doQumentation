...
연산자(예: $Z \otimes Z$)의 기댓값을 추정하고 싶다면 Estimator를 사용합니다. 시스템의 상태를 살펴보고 싶다면 Sampler를 사용합니다.
...
**조합 최적화** 문제에 이 패턴을 적용하여, 하이브리드(양자-고전) 반복 방법인 **양자 근사 최적화 알고리즘(QAOA)**을 사용해 **Max-Cut** 문제를 푸는 방법을 보여주겠습니다.
...
이 경우 $f(x)$의 최솟값은 컷이 통과하는 엣지의 수가 최대일 때입니다. 보면 알 수 있듯이, 아직 양자 컴퓨팅과 관련된 내용은 없습니다. 이 문제를 양자 컴퓨터가 이해할 수 있는 형태로 재정식화해야 합니다.
...
### 3.4 3단계. IBM Quantum primitives를 사용하여 실행 {#34-step-3-execute-using-ibm-quantum-primitives}
...
IBM&reg; Backend와 상호작용하려면 **IBM Quantum 프리미티브**를 사용하세요. 두 가지 프리미티브는 Sampler와 Estimator이며, 양자 컴퓨터에서 실행할 측정 유형에 따라 프리미티브를 선택합니다. $H_C$의 최솟값 구하기에는 비용 함수 측정이 단순히 $\langle H_C \rangle$의 기댓값이므로 Estimator를 사용합니다.

#### 실행하기 {#run}

primitive는 양자 장치에서 워크로드를 스케줄링하기 위한 다양한 [실행 모드](/guides/execution-modes)를 제공하며, QAOA 워크플로우는 세션 내에서 반복적으로 실행됩니다.
!["실행 모드"](/learning/images/courses/utility-scale-quantum-computing/variational-quantum-algorithms/execution-mode.avif)
...
최적화기는 비용을 줄이고 회로에 대한 더 나은 매개변수를 찾을 수 있었습니다.
...
회로에 대한 최적의 매개변수를 찾은 후에는 이러한 매개변수를 할당하고 최적화된 매개변수로 얻은 최종 분포를 샘플링할 수 있습니다. 이때 *Sampler* primitive를 사용해야 하는데, 이는 그래프의 최적 절단에 해당하는 비트스트링 측정의 확률 분포이기 때문입니다.

**참고:** 이는 컴퓨터에서 양자 상태 $\psi$를 준비한 다음 이를 측정하는 것을 의미합니다. 측정은 상태를 단일 계산 기저 상태로 붕괴시킵니다 - 예를 들어, `010101110000...` - 이는 우리의 초기 최적화 문제(작업에 따라 $\max f(x)$ 또는 $\min f(x)$)에 대한 후보 해 $x$에 해당합니다.
