...
양자 알고리즘을 사용하려면 고전 데이터를 어떤 방식으로든 양자 Circuit에 가져와야 합니다. 이를 보통 데이터 *인코딩*이라고 하며, 데이터 *로딩*이라고도 부릅니다. 이전 단원에서 다룬 특징 매핑(feature mapping) 개념을 떠올려 보세요. 특징 매핑은 데이터의 특징을 한 공간에서 다른 공간으로 매핑하는 것입니다. 고전 데이터를 양자 컴퓨터로 단순히 옮기는 것도 일종의 매핑이며, 이를 특징 매핑이라고 부를 수 있습니다. 실제로 Qiskit에 내장된 특징 매핑(예: `z_feature_map`이나 `zz_feature_map`)은 대개 회전 레이어와 얽힘 레이어를 포함하여 상태를 힐베르트 공간의 여러 차원으로 확장합니다. 이 인코딩 과정은 양자 머신러닝 알고리즘의 핵심 부분으로, 알고리즘의 계산 능력에 직접적인 영향을 미칩니다.
...
위의 유니터리를 이해하는 한 가지 방법은 물리적 시스템에서의 전파자와의 유사성을 통한 것입니다. 위의 유니터리는 이징 모델과 유사한 해밀토니안 $\mathcal{H}$에 대한 유니터리 시간 진화 연산자 $\exp(it\mathcal{H})$이며, 여기서 시간 매개변수 $t$는 진화를 유도하기 위해 데이터 값으로 대체됩니다. 이 유니터리 연산자를 전개하면 PFM 회로가 나옵니다. $S$에 있는 얽힘 연결성은 스핀 격자에서의 이징 커플링으로 해석할 수 있습니다.
이러한 이징 유형 상호작용을 나타내는 Pauli $Y$와 $XX$ 연산자의 예를 살펴보겠습니다. Qiskit은 단일 및 $n$-큐비트 게이트 선택을 통해 PFM을 인스턴스화하기 위한 `pauli_feature_map` 클래스를 제공하며, 이 예제에서는 Pauli 문자열 `‘Y’`와 `‘XX’`로 전달됩니다. 일반적으로 $n$은 단일 및 이중 큐비트 상호작용의 경우 각각 1 또는 2입니다. 얽힘 방식은 "선형"이며, 이는 양자 회로에서 가장 가까운 이웃 큐비트만 결합됨을 의미합니다. 이 양자 회로는 추상화 계층이므로, 이는 양자 컴퓨터 자체의 가장 가까운 이웃 큐비트와는 일치하지 않는다는 점에 유의하세요.
