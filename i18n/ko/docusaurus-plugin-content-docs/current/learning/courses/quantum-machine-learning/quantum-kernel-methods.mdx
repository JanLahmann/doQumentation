...
이러한 상태는 데이터를 고차원으로 매핑한 것이므로, 원하는 커널 항목은 다음 내적입니다
$$
\langle\psi(\vec{x}_j)|\psi(\vec{x}_i)\rangle = \langle 0 |^{\otimes N}\Phi^\dagger(\vec{x}_j)\Phi(\vec{x}_i)|0\rangle^{\otimes N}.
$$
기본 초기 상태 $|0\rangle^{\otimes N}$에 두 회로 $\Phi^\dagger(\vec{x}_j)$와 $\Phi(\vec{x}_i)$를 모두 적용하여 연산하면, 그 다음 상태 $|0\rangle^{\otimes N}$을 측정할 확률은
$$
P_0 = |\langle0|^{\otimes N}\Phi^\dagger(\vec{x}_j)\Phi(\vec{x}_i)|0\rangle^{\otimes N}|^2.
$$
입니다. 이것이 바로 우리가 원하는 값입니다($||^2$까지). 우리 회로의 측정 계층은 측정 확률(또는 특정 오류 완화 방법을 사용하는 경우 이른바 "준확률(quasi-probabilities)")을 반환합니다. 관심 있는 확률은 0 상태 $|0\rangle^{\otimes N}$의 확률입니다.
...
IBM Quantum `Sampler` 프리미티브를 사용하여 Circuit을 샘플링한 상태의 확률 분포를 재구성합니다. 이것이 "준확률 분포"로 불리는 경우가 있는데, 이 용어는 잡음이 문제가 되고 오류 완화와 같은 추가 단계가 도입될 때 적용됩니다. 이러한 경우, 모든 확률의 합이 정확히 1이 되지 않을 수 있으므로 "준확률"이라고 합니다.
...
### Step 1: 고전적 입력을 양자 문제로 매핑하기 {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
### Step 2: 양자 실행을 위한 문제 최적화하기 {#step-2-optimize-problem-for-quantum-execution-1}
...
### Step 4: 후처리 및 고전적 형식으로 결과 반환 {#step-4-post-process-return-result-in-classical-format-1}
...
### 1단계: 고전 입력을 양자 문제로 변환 {#step-1-map-classical-inputs-to-a-quantum-problem-2}
...
### 4단계: 후처리 및 고전 형식으로 결과 반환 {#step-4-post-process-return-result-in-classical-format-2}
...
훈련된 모델의 정확도가 100%였다는 것을 확인했습니다. 이는 훌륭한 결과이며, QKE가 작동할 수 있음을 보여줍니다. 하지만 이는 양자 우위와는 매우 다릅니다. 고전 커널 역시 이 분류 문제를 100% 정확도로 해결할 수 있었을 가능성이 높습니다. 현재 유용성 시대에서 양자 커널이 가장 유용하게 사용될 수 있는 곳을 파악하기 위해서는 다양한 데이터 유형과 데이터 관계를 특성화하는 데 많은 작업이 필요합니다.
이 워크플로우의 일부를 수정하여 다양한 양자 특징 맵의 효과를 연구하는 것은 학습자에게 맡깁니다. 다음은 고려할 몇 가지 사항입니다:
* 정확도는 얼마나 견고한가요? 광범위한 유형의 데이터에 대해서도 유지되나요, 아니면 이 특정 훈련 데이터에만 해당하나요?
* 데이터의 어떤 구조가 양자 특징 맵이 유용할 것이라고 생각하게 만드나요?
* 훈련 데이터의 양을 늘리거나 줄이면 정확도는 어떻게 영향을 받나요?
* 어떤 특징 맵을 사용할 수 있으며, 특징 맵에 따라 결과는 어떻게 달라지나요?
* 특징 수를 늘리면 정확도와 실행 시간은 어떻게 영향을 받나요?
...
### 1단계: 고전 입력을 양자 문제로 매핑하기 {#step-1-map-classical-inputs-to-a-quantum-problem-3}
...
### 2단계: 양자 실행을 위한 문제 최적화 {#step-2-optimize-problem-for-quantum-execution-2}
...
### 4단계: 후처리 및 고전 형식으로 결과 반환하기 {#step-4-post-process-return-result-in-classical-format-3}
...
양자 커널 방법은 양자 친화적인 피처를 가진 데이터셋과 적합한 양자 피처 맵이 올바르게 결합될 때 강력한 도구가 될 수 있습니다. 양자 커널이 유용할 가능성이 높은 곳을 더 잘 이해하려면 [Liu, Arunachalam & Temme (2021)](https://www.nature.com/articles/s41567-021-01287-z)을 읽어보세요.
