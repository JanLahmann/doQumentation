...
선택한 하드웨어에서 실행하기 전에, 간단한 디버깅과 때로는 오류 추정을 위해 시뮬레이터를 사용하는 것이 좋은 생각입니다. 이러한 이유로, VQE를 시뮬레이터에서 실행하는 방법을 간단히 보여드릴게요. 하지만 **고전 컴퓨터, 시뮬레이터 또는 GPU**도 고도로 얽힌 127-Qubit 양자 컴퓨터의 전체 기능을 정확하게 시뮬레이션할 수 없다는 점은 매우 중요합니다. 현재의 양자 유용성 시대에, 시뮬레이터의 사용은 제한적입니다.
