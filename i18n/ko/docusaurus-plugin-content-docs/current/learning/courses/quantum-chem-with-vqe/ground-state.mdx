...
# IBM Quantum Compute Service로 모두 함께 정리하기 {#bringing-it-all-together-with-ibm-quantum-compute-service}
...
코드가 사용하는 하드웨어에서 최대한 효율적으로 실행되기를 원합니다. 따라서 최적화 단계를 시작하기 위해 Backend를 선택해야 합니다. 아래 코드는 여러분이 사용할 수 있는 Backend 중 가장 한가한 것을 선택합니다.
...
회로를 실제 Backend에서 실행하기 위해 최적화하는 것은 풍부하고 중요한 주제입니다. 하지만 VQE에만 국한되지 않습니다. 지금은 두 가지 중요한 용어를 간단히 짚고 넘어가겠습니다:

* **[optimization\_level](https://docs.quantum.ibm.com/api/qiskit/transpiler#preset-pass-managers)**: 선택한 Backend의 레이아웃에 맞춰진 회로 정도를 설명합니다. 가장 낮은 최적화 수준은 회로를 장치에서 실행하는 데 필요한 최소한의 작업만 수행합니다. 회로 Qubit을 장치 Qubit에 매핑하고 모든 2-Qubit 연산을 허용하도록 Swap Gate를 추가합니다. 가장 높은 최적화 수준은 훨씬 더 스마트하고 전체 Gate 개수를 줄이기 위해 많은 트릭을 사용합니다. 다중 Qubit Gate는 높은 오류율을 가지고 있고 Qubit은 시간이 지나면서 결어긋남(decoherence)이 일어나므로, 더 짧은 회로가 더 나은 결과를 제공해야 합니다.

* **[동적 디커플링 (Dynamical Decoupling)](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.passes.PadDynamicalDecoupling)**: 유휴 Qubit에 Gate 수열을 적용할 수 있습니다. 이는 환경과의 원치 않는 상호작용 중 일부를 상쇄합니다.
...
## 3단계: IBM Quantum 프리미티브를 사용하여 실행하기 {#step-3-execute-using-ibm-quantum-primitives}

선택한 하드웨어에서 실행하기 전에, 간단한 디버깅과 때로는 오류 추정을 위해 시뮬레이터를 사용하는 것이 좋은 생각입니다. 이러한 이유로, VQE를 시뮬레이터에서 실행하는 방법을 간단히 살펴봐요. 하지만 **고전 컴퓨터, 시뮬레이터 또는 GPU**도 고도로 얽힌 127-Qubit 양자 컴퓨터의 전체 기능을 정확하게 시뮬레이션할 수 없다는 점은 매우 중요합니다. 현재의 양자 유용성 시대에, 시뮬레이터의 사용은 제한적입니다.

변분 회로의 각 매개변수 선택에 대해 기댓값을 계산해야 한다는 점을 기억하세요. 최소화할 값이기 때문입니다. 이미 짐작하셨을 수도 있듯이, 가장 효율적인 방법은 Qiskit SDK Primitive인 Estimator를 사용하는 것입니다. 로컬 시뮬레이터를 사용하면서 시작하겠습니다. 로컬 버전의 Estimator인 BackendEstimator를 사용해야 합니다.
...
이 코드는 올바르게 평가되었지만 수렴하지 않았습니다. 이는 우리가 예상한 바입니다. 실제 하드웨어에서 계산을 실행한 후 출력을 논의할 것입니다. 실제 Backend의 경우 IBM Quantum Estimator를 사용합니다. 이를 Session 내에서 실행하고 그 Session에 대한 옵션을 지정하고 싶을 것입니다.
...
다른 것들 중에서도, Session을 사용하는 것은 우리의 작업이 시작하기 위해 오직 한 번만 대기열에 있다는 의미입니다. 고전 최적화기의 후속 반복은 대기열에 들어가지 않습니다. Session에서 복원력 및 최적화 수준을 설정할 수 있습니다. 이 도구들은 충분히 중요하므로, 각각에 대한 간단한 설명과 VQE에서의 중요성, 그리고 더 알아볼 수 있는 링크를 아래에 정리했습니다:
...
* Estimator의 내장 복원력: 최적화와 마찬가지로, Estimator는 오류에 대한 복원력을 위한 내장 설정을 가지고 있으며, 다양한 오류 완화 접근 방식에 해당합니다. 복원력 수준 설정에 대해 배우려면 [문서](/guides/error-mitigation-and-suppression-techniques)를 참고하세요.


오류 완화는 VQE 계산의 수렴에서 미묘한 역할을 한다는 점은 주목할 가치가 있습니다. 고전 최적화기는 에너지를 최소화하는 매개변수를 찾기 위해 매개변수 공간을 검색합니다. 최적 매개변수에서 매우 멀리 떨어져 있을 때, 오류가 있는 경우에도 고전 최적화기에는 가파른 기울기가 명확할 수 있습니다. 하지만 계산이 수렴하고 최적 값에 접근할수록 기울기가 더 작아져 오류에 더 쉽게 묻혀 버립니다. 얼마나 많은 오류 완화를 사용해야 할까요? 수렴의 어느 지점에서? 이들은 특정 사용 사례에 대해 결정해야 할 선택입니다.

이 첫 번째 하드웨어 실행의 경우, 우리는 상대적으로 빠른 실행을 용이하게 하기 위해 복원력을 0으로 설정했습니다. 어떤 심각한 응용 프로그램의 경우, 오류 완화를 사용하고 싶을 것입니다. 아래 셀에는 (1) Session을 위한 옵션 (우리가 "session\_options"라고 명명한)과 (2) 고전 최적화기를 위한 옵션 (여기서는 단순히 "options"라고 불림)의 두 가지 옵션 세트가 있다는 점에 유의하세요.
