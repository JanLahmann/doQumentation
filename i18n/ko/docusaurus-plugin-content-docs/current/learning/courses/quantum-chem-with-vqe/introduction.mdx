...
이 모든 요소는 IBM Quantum 프리미티브, 특히 Estimator를 기반으로 VQE 계산을 구축하기 위해 함께 사용됩니다. primitives에 대한 정보는 [문서](/guides/primitives)를 참조해 주세요. VQE 계산을 설정할 때, 다음과 같은 표준 Qiskit patterns 프레임워크를 따릅니다:

* 고전적 입력을 양자 문제로 매핑

* 양자 실행을 위한 문제 최적화

* IBM Quantum primitives를 이용한 실행
