...
두 가지 유형의 SU(2) gate, 즉 `rx`와 `y`를 사용하는 4-qubit 예제 `efficient_su2` circuit으로 시작하겠습니다. 또한 얽힘(entanglement) 스킴과 반복 횟수를 지정합니다. circuit을 단순히 `.draw()`하면 상당히 추상적인 표현을 얻게 됩니다. 더 이해하기 쉬운 circuit diagram은 `.decompose().draw()`를 사용하여 얻을 수 있으며, 여기서는 `output = "mpl"'을 사용하겠습니다.
