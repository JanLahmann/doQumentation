...
# 분자 기하학 결정하기 {#determining-a-molecular-geometry}
...
## Step 3: IBM Quantum 프리미티브를 사용하여 실행하기 {#step-3-execute-using-ibm-quantum-primitives}

아래 코드 블록에서는 원자 간 거리 $x$의 각 단계별 출력값을 저장할 배열을 설정합니다. $x$의 범위는 평형 결합 길이의 실험값인 0.74 옹스트롬(Angstrom)에 대한 사전 지식을 바탕으로 선택했습니다. 먼저 시뮬레이터에서 실행하며, 이를 위해 `qiskit.primitives`에서 Estimator(BackendEstimator)를 임포트합니다. 각 기하학적 단계마다 해밀토니안을 구성하고, 옵티마이저 "cobyla"를 사용하여 일정 횟수(여기서는 500회)의 최적화 단계를 허용합니다. 각 기하학적 단계에서 전체 에너지와 전자 에너지를 모두 저장합니다. 옵티마이저 단계 수가 많기 때문에 1시간 이상 소요될 수 있습니다. 필요한 시간을 줄이려면 아래 입력값을 수정하는 것을 권장합니다.
...
이 출력 결과는 후처리 섹션에서 아래에서 논의됩니다. 지금은 시뮬레이션이 성공적으로 완료되었다는 점만 확인하면 됩니다. 이제 실제 하드웨어에서 실행할 준비가 되었습니다. resilience를 `1`로 설정하여 TREX 오류 경감(error mitigation)이 사용됨을 나타냅니다. 이제 실제 하드웨어를 사용하므로 IBM Quantum Compute Service와 IBM Quantum 프리미티브를 사용합니다. 기하학적 구조에 관한 for 루프와 여러 변분 시도(variational trial) 모두 Session 내부에 포함되어 있다는 점에 유의하세요.

실제 하드웨어 실행에는 비용과 시간 제한이 따르기 때문에, 아래에서 기하학적 단계 수와 옵티마이저 단계 수를 줄였습니다. 정밀도 목표와 시간 제한에 맞게 이러한 단계들을 반드시 조정하세요.
