...
벨의 논문이 발표된 당시, 관련 통계를 실험으로 검증하는 일은 물리학 최전선의 연구자들이 아니고서는 접근할 수 없었습니다. 하지만 오늘날 IBM Quantum 덕분에, 전 세계 학생들이 클라우드를 통해 무료로 실제 양자 장치를 원격으로 사용하며 양자 상태의 본질을 탐구할 수 있게 되었습니다. 오늘 여러분이 할 일이 바로 그것입니다.
...
![EPR 실험 설정 다이어그램. 선원(입자원)이 왼쪽으로 입자 하나, 오른쪽으로 입자 하나를 방출합니다. 각 입자는 슈테른-게를라흐 장치, 즉 불균일 자기장과 측정 스크린을 통과합니다.](/learning/images/modules/quantum-mechanics/bells-inequality-with-qiskit/epr-pic.avif)
...
**참고:** Einstein과 Born의 옵션에 대해 서로 다른 결과를 산출하는 실험을 고안할 수 있다면 매우 인상적일 것입니다. 사람이 그러한 실험을 고안하는 데 수십 년이 걸렸습니다.

<Accordion>
<AccordionItem title="Answer">

지금까지 설명한 실험(즉, 양전자와 전자의 스핀이 반대로 정렬되어 순 스핀이 없는 경우)을 그대로 유지한다면, 각운동량 보존에 따라 어느 옵션이 옳든지 상관없이 두 스핀을 $\pm x$, $\pm y$, 또는 $\pm z$ 방향으로 측정하면 항상 반대 부호를 얻게 됩니다. 한 입자(예를 들어 전자)의 스핀을 한 방향(예를 들어 $+z$)으로 측정하면, 다른 입자인 양전자의 스핀은 $-z$ 방향으로 측정됨을 의미합니다. 대신 양전자의 스핀을 $x$ 방향으로 측정하면, $+x$ 또는 $-x$가 나올 확률이 동일합니다. 이는 숨겨진 지침이 그렇게 말하기 때문일 수도 있고(Einstein의 옵션 1), 전자 스핀 측정 후 양전자 스핀의 확률 분포가 업데이트되어 새로운 확률 분포가 $\pm x$ 사이의 50대 50 분할과 일치하기 때문일 수도 있습니다(Born의 옵션 2). 이 점들은 아래에서 더 자세히 설명합니다.
...
이 강의의 나머지 부분은 Einstein과 Born의 옵션을 구별할 수 있는 실험에 할애되므로, 여기서는 자세히 다루지 않겠습니다. 하지만 트릭의 일부는 두 입자를 서로 다른 방향(예를 들어 $x$와 $z$, 또는 전통적인 데카르트 축 사이의 어떤 방향)으로 측정하는 것입니다. 나머지는 양자역학의 예측과 숨은 변수와 같은 고전 정보의 예측이 주는 서로 다른 결과의 정확한 확률을 신중하게 고려하는 데서 나옵니다.

</AccordionItem>
</Accordion>

두 옵션 모두에서, 두 관찰자 Lucas와 Rihanna가 같은 축을 따라 측정한다면, 어느 옵션이 맞든 상관없이 반대로 정렬된 스핀을 얻을 것으로 예상됩니다. 그 이유를 알기 위해 아래 다이어그램을 살펴보세요.
...
위 그림은 Einstein의 옵션을 보여줍니다. 스핀의 방향은 반대이며 결정되어 있습니다. $z$ 축을 따라 측정하면 하나는 $+z$를 따르고 다른 하나는 $-z$를 따를 것입니다. 양전자가 $+z$를 따르고 전자가 $-z$를 따를 것이라고 가정할 이유는 없습니다. 이 이미지는 단지 스핀이 반대 방향으로 측정될 것임을 보여줄 뿐입니다. 사실 Einstein의 옵션의 경우, 주어진 스핀이 결국 측정되는 방향을 따라 스핀 성분을 실제로 가질 필요는 없습니다. Einstein의 옵션에 대한 가장 약한 진술은, 스핀에 저장된 어떤 지침 세트가 있어 어떤 축을 따라 측정하든 측정 결과가 무엇이 될지 결정한다는 것입니다. 이 지침이 단순한 벡터 형태(아래 다이어그램 참조)라고 상상할 필요는 없습니다. 이 부분은 나중에 다시 다루겠습니다.
...
아래 그림은 Born의 옵션을 보여주는데, 여기서는 양전자와 전자 스핀의 방향이 확률 분포로 퍼져 있으며 명확한 방향이 없습니다. 분포의 모양에 너무 큰 의미를 두지 마세요. 각 스핀은 서로 반대이기만 하면 어떤 방향을 가리킬 확률도 실제로 0이 아닐 수 있습니다. 우리는 단지 논의를 위해 시각적으로 구별할 수 있도록 원의 조각으로 그렸을 뿐입니다. Born의 옵션의 경우에도 각운동량은 여전히 보존되어야 한다는 점에 유의하세요. 따라서 만약 하나의 확률 파동이 "붕괴"되어 스핀이 $+z$를 가리키게 되면, 다른 하나는 $-z$를 가리키며 반대 방향으로 편향됩니다. 두 옵션은 동일하게 보입니다.
...
하지만 관찰자 L과 R이 그림 4와 5에 나타난 것처럼 각각 120도 간격의 세 축 중 하나를 따라 측정할 수 있다면 어떻게 될까요? 각 관찰자는 스핀을 측정할 축(a, b, c)을 무작위로 결정할 수 있습니다. 두 사람이 같은 축을 따라 측정할 필요는 없습니다. 각 관찰자가 측정할 때, 선택한 축에서 양의 투영을 발견할 수도 있고 음의 투영을 발견할 수도 있습니다. 예를 들어 Lucas와 Rihanna는 +a와 -b, 또는 +b와 +c를 측정할 수 있습니다. 만약 같은 축을 따라 측정하기로 선택한다면, 반드시 투영에서 반대 부호를 얻어야 함에 유의하세요: +a와 -a, +b와 -b, 또는 +c와 -c. 둘 다 예를 들어 +a를 발견할 수는 없습니다. 다음 섹션에서는 Lucas와 Rihanna가 측정한 축에서 같은 부호(++ 또는 --)를 얻을 확률과 반대 부호(+- 또는 -+)를 얻을 확률을 계산하는 방법을 다루겠습니다.
...
위의 두 그림은 이 새로운 3축 측정 시나리오에서 가능한 숨은 변수 해석을 보여줍니다. 즉, 스핀이 벡터로서 이미 결정되어 있거나, 모든 가능한 측정의 결과가 실험자가 측정 전에는 알 수 없더라도 미리 결정되도록 시스템에 어떤 방식으로든 내장된 물리적 지침 세트가 존재한다는 것입니다. 그 대안은 아래에 나와 있습니다. 결과에 대한 어떤 확률 분포가 존재하며, 이 분포는 서로 다른 측정 결과의 가능성에 대해 몇 가지를 알려줄 수 있지만, 결과는 측정 전에는 자연에 의해 결정되지 않습니다.
...
우리는 스스로에게 물을 수 있습니다. "두 참가자가 스핀 투영의 같은 부호를 얼마나 자주 찾게 될까요?" 즉, 우리는 그들이 측정하기로 선택한 축조차 기록하지 않고, 단지 같은 부호를 찾았는지 다른 부호를 찾았는지만 기록하고 있습니다. Einstein과 Born의 옵션이 이 더 복잡한 측정 방식에서 같은 결과를 산출할지는 명확하지 않습니다. 하지만 그림 4와 5에서 차이가 있을 $가능성$이 있다는 것은 분명해야 합니다. Einstein의 옵션에서 보여준 경우, 축 $a$에서 $e+$ 스핀의 투영 측정은 반드시 $+a$를 산출하며, 축 $b$에서 $e-$ 스핀의 투영은 $-b$를 산출합니다(간신히). 하지만 Born의 옵션에서는 가능성이 활짝 열려 있습니다. 각운동량이 여전히 보존된다는 것은 사실입니다. 하지만 두 자기장이 같은 축을 따라 정렬되어 있지 않기 때문에, 우리는 입자들이 (장과의 상호작용을 통해) 서로 다른 축으로 붕괴되어야 하는 상황으로 강제합니다. 다음 섹션에서는 양자역학을 사용하여 Born의 옵션이 주어졌을 때, Lucas와 Rihanna가 측정한 축에서 같은 부호(++ 또는 --)를 얻을 확률과 반대 부호(+- 또는 -+)를 얻을 확률이 어떻게 되어야 하는지 결정하겠습니다.
...
위 표의 각 경우에 대해, 루카스와 리아나가 축을 선택하는 방법은 $aa$, $ab$, $ac$, $ba$, $bb$, $bc$, $ca$, $cb$, $cc$로 총 9가지입니다. 이 표를 읽으면, 1행과 8행에서 두 관측자가 같은 부호를 측정할 확률은 0임을 알 수 있습니다. 2행부터 7행까지는 같은 부호를 얻을 수 있는 경우가 4가지 있으며, 이를 2행에 대해서만 살펴봐요.
...
위 표의 2행에 대해, 우리는 Lucas와 Rihanna가 측정에서 같은 부호를 얻는 모든 가능한 방법과 다른 부호를 얻는 모든 방법을 나열했습니다. 세 번째 행에 대해서도 이를 반복해 보세요.
...
Lucas는 어떤 축을 선택해 입자 하나의 스핀이 양의 방향인지 음의 방향인지를 측정합니다. 그가 어떤 결과를 얻든, $z$ 축이 그 방향이 되도록 좌표계를 설정합시다. 그러면 중간자가 붕괴된 직후, 아무런 측정이 이루어지기 전의 초기 상태를 다음과 같이 쓸 수 있습니다.
$$
...
우리는 구체적으로 세 축이 서로 $120\deg$ 간격으로 배치된 경우를 살펴보고 있습니다. Lucas가 어떤 방향($\pm a$, $\pm b$, 또는 $\pm c$)을 측정하든, 그 방향을 $z$라고 부르겠습니다. 그러면 Rihanna는 $\pm a$, $\pm b$, 또는 $\pm c$ 중 하나를 무작위로 선택해 측정합니다. Rihanna의 선택이 Lucas의 선택과 같은 축이라면(부호는 달라도 무방합니다), 두 사람 모두 $z$를 측정하는 것이므로 Rihanna가 $+z$를 측정할 확률은 0입니다. 이는 Rihanna의 축 선택이 Lucas의 선택과 독립적이기 때문에, 전체 경우의 1/3에 해당합니다. 그 외의 경우, Rihanna는 $z$로부터 $120\deg = 2\pi/3$ 라디안 떨어진 축(전체의 1/3)이나, $240\deg = 4\pi/3$ 라디안 떨어진 축(전체의 1/3)을 측정하게 됩니다. 물론 각 축에서 스핀은 양 또는 음의 방향으로 측정될 수 있습니다. 이를 통해 Lucas와 Rihanna가 같은 부호를 얻을 전체 확률을 구할 수 있습니다.
...
### 이해도 확인 {#check-your-understanding-5}
...
이제 StatevectorSampler라는 Qiskit reference 프리미티브를 사용하겠습니다. Sampler는 시스템의 모든 가능한 상태를 샘플링하고 각 상태를 얻을 확률(또는 경우에 따라 준확률)을 반환하도록 설계된 프리미티브입니다. "shots" 수를 지정하고 각 상태의 "counts"를 확인할 수 있습니다.
...
### 이해도 확인 {#check-your-understanding-6}
...
2. 스핀이 0인 입자가 두 개의 스핀 1/2 입자 A와 B로 붕괴합니다. 입자 A가 $+z$ 방향의 투영값을 갖는다고 측정되었을 때, 입자 B에 대한 측정에서 가능한 투영값은 무엇입니까? 해당하는 것을 모두 고르세요.
