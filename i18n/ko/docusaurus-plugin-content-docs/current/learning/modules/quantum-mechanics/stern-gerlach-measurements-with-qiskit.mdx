...
### 이해 확인 {#check-your-understanding-1}
...
실제 양자 컴퓨터에서 실험을 실행하려면, IBM Quantum Compute Service를 로드하고 양자 컴퓨터(또는 "Backend")를 선택해야 합니다. 아래에서는 사용 가능한 가장 여유로운 양자 컴퓨터를 간단히 선택합니다.
...
### 1단계: 고전적 입력을 양자 문제로 매핑하기 {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
측정 확률을 내적의 제곱과 연관시키려는 우리의 시도는 각 벡터의 크기가 1이어야 함을 의미합니다. 즉, 모든 $|v\rangle$에 대해 $|\langle v|v\rangle|^2 = 1$이어야 하는데, 이는 상태 $|v\rangle$에 있는 것이 상태 $|v\rangle$에 있을 확률이 100%이기 때문입니다. 이를 "정규화 조건"이라고 합니다.
...
그렇지 않습니다. 처음에 $|0\rangle = \begin{pmatrix} 1 \\0 \end{pmatrix}$를 선택해야 할 이유는 없습니다. 오히려 이는 관례입니다. 하지만 일단 그 선택을 하면, 이후의 선택에 특정 제약이 부과됩니다. 아래를 참조하세요.

</AccordionItem>
</Accordion>

위의 실험에서, 처음에 상태 $|0\rangle$에 있던 큐비트가 이후 측정에서도 그 상태에 머무른다는 것을 발견했음을 기억하세요. $|1\rangle$에 대해서도 마찬가지입니다. 이는 $|0\rangle$에 있는 큐비트가 상태 $|1\rangle$로 측정될 확률이 0임을 의미합니다(노이즈의 영향은 제외). 내적과 측정 확률 사이에 필요한 관계는 다음을 알려줍니다

$$
P_{01} = P_{10}=0.
$$

일반성을 잃지 않고, 상태 벡터를 $|1\rangle = \begin{pmatrix} a \\ b \end{pmatrix}$로 쓸 수 있습니다. 그러면 다음과 같이 쓸 수 있습니다

$$
P_{01} = |\langle 0| 1 \rangle|^2 = \vert \begin{pmatrix} 1 & 0\end{pmatrix} \begin{pmatrix} a \\ b\end{pmatrix}\vert ^2 = |a|^2 = 0.
$$

$P_{11} = 1$이라는 요구사항, 즉 이른바 "정규화 조건"은 $|b|^2=1$임을 알려줍니다. 이 자체만으로는 실제로 $\phi \in \mathbb{R}$에 대해 $b=e^{i\phi}$로 제한할 뿐입니다. $b=1$을 선택하는 다른 이유들이 있지만 이는 이 주제에 대한 소개 범위를 벗어납니다. 지금은 $b=1$이 하나의 받아들일 수 있는 해라는 것으로 충분합니다.

우리의 분석은 상당히 진전되었습니다. 상태 벡터의 형태를 선택하면 여기서 작용하는 물리적 현상에 대한 무언가를 설명하는 행렬을 구성할 수 있습니다. 특히, 원래의 Stern-Gerlach 실험은 $z$ 축을 따른 스핀 각운동량 성분을 기반으로 궤적의 분리를 측정했기 때문에, 정확히 그것을 설명하는 연산자, 즉 $S_z$를 원합니다. 실험과의 또 다른 핵심 연결은 편향량, 이동 시간, 알려진 자기장 세기로부터 스핀의 $z$ 성분의 크기를 결정할 수 있다는 것입니다. 이는 실험 설정의 정밀도에 대한 많은 가정을 필요로 하지만, 여기서는 단순히 측정된 스핀 각운동량의 z 성분이 $\pm \hbar/2$라는 것을 재확인하겠습니다.

그러면 우리는 실수 고유값(Hermitian 행렬로 충족됨)을 가지며, 이 고유값이 실험적으로 관찰된 스핀 성분에 해당하는 행렬을 찾고 있습니다. 일반성을 잃지 않고, $S_z = \begin{pmatrix} s_{11} & s_{12} \\ s_{21} & s_{22}\end{pmatrix}$로 쓸 수 있으며, 다음을 요구할 수 있습니다:
...
결합하고 전체 인수 $\hbar/2$를 빼내면, 다음을 얻습니다

$$
S_z = \frac{\hbar}{2} \begin{pmatrix} 1 & 0 \\ 0 & -1\end{pmatrix}.
$$
이것은 모든 양자역학 교과서에서 볼 수 있는 잘 알려진 spin-z 연산자입니다. $\hbar/2$가 제거된 형태로 자주 볼 수 있는데, 이 경우 "Pauli-z" 연산자라고 하며, 일반적으로 $\sigma_z$로 표기됩니다:

$$
\sigma_z = \begin{pmatrix} 1 & 0 \\ 0 & -1\end{pmatrix}.
$$

이는 유용한데, 이 Pauli 연산자(및 관련 행렬)는 양자 컴퓨터의 트랜스몬 큐비트 상태를 포함하여 두 준위 또는 두 가지 가능한 측정 결과를 포함하는 많은 물리 시스템을 설명할 수 있기 때문입니다.

이것과 관련된 다른 연산자와 상태로 넘어가기 전에, 많은 사람들이 잘못 이해하는 한 가지 사실을 짚고 넘어가야 합니다. __연산자 작용은 측정과 같지 않습니다.__ 어떤 의미에서는 "당연하지! 하나는 수학적 계산의 일부로 종이 위에서 일어나고, 다른 하나는 실험실에서 물리적 시스템에 일어나는 일이니까"라고 말할 수 있습니다. 네, 맞는 말이지만, 그 이상의 의미가 있습니다. $z$ 성분의 스핀을 측정하면 시스템의 초기 상태와 무관하게 항상 "스핀-업" 또는 "스핀-다운" 상태를 산출합니다. 우리는 양자 컴퓨팅의 유사물인 $|0\rangle$과 $|1\rangle$에서 이를 확인했습니다. 수백 가지의 무작위 방향으로 상태를 초기화했지만, 측정은 항상 $|0\rangle$ 또는 $|1\rangle$을 산출했습니다. 이는 측정이 상태를 고유상태로 "붕괴"시킨다고 알려져 있습니다. 상태에 행렬을 적용할 때는 이런 일이 일어나지 않습니다. 아래 질문들을 통해 이를 탐구해 보세요.

### 이해도 확인 {#check-your-understanding-3}

입자가 스핀 상태로 시작한다고 가정해 봅시다
...
### 이해도 확인하기 {#check-your-understanding-4}
...
강사는 이 [간단한 설문](https://ibm.biz/classrooms_instructor_key_request)을 작성하여 노트북 사용 방법을 알려주시면 답안 키와 일반 교육과정 배치 지침이 포함된 노트북 버전을 요청할 수 있습니다.
