...
(a) 다음 조건을 만족하는 $A$를 구하여 시도 파동 함수를 규격화하세요. $$ \int_{-\infty}^{\infty} |\Psi_\text{trial}|^2 dx = 1 $$.
...
(b) 다음과 같이 주어진 해밀토니안 $\hat{H}$의 기댓값을 계산하세요. $$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V(x)$$ 여기서 $V(x)  = \frac{1}{2}m\omega^2x^2$로, 단순 조화 진동자 퍼텐셜에 해당합니다.
...
빈칸에 알맞은 용어를 채워 VQE 과정 요약을 완성하고, 클릭하여 정답을 확인하세요.
...
- 3단계: IBM Quantum 기본 요소(Primitive)를 사용하여 실행
...
먼저 IBM Quantum 기본 요소를 포함한 필요한 패키지들을 불러오겠습니다. 또한 현재 사용 가능한 양자 컴퓨터 중 가장 여유 있는 것을 선택합니다.
...
VQE 계산은 특정 결합 거리에서 수소 분자($H_2$)의 해밀토니안을 정의하는 것으로 시작합니다. 이 해밀토니안은 표준 절차를 통해 분자 시스템에서 생성·매핑된 qubit 연산자 형태로 시스템의 전체 에너지를 표현합니다. 구체적으로는: 1) STO-6G 기저 집합(전자 오비탈을 근사하는 수학 함수들의 특정 모음)을 사용하고, 2) Jordan-Wigner 매핑(전자를 기술하는 페르미온 연산자를 qubit 연산자로 변환하는 기법)을 적용하며, 3) 해밀토니안의 패리티(parity)를 이용한 qubit 감소를 수행하여 문제를 단순화합니다.
...
여기서는 세 가지 서로 다른 단일 qubit 앤사츠를 시도하여 어느 앤사츠가 더 넓은 "커버리지(coverage)"를 제공하는지 확인해 보겠습니다. 커버리지란 앤사츠 Circuit이 매개변수를 변화시켜 생성할 수 있는 양자 상태의 범위를 의미합니다.
...
세 가지 앤사츠로 얻은 VQE 결과를 비교함으로써, 앤사츠의 유연성과 상태 공간 커버리지가 단순화된 해밀토니안의 실제 바닥 상태 에너지를 찾는 능력에 어떤 영향을 미치는지 알 수 있습니다. 더 유연한 앤사츠는 더 나은 근사값을 찾을 *가능성*이 있지만, 고전 최적화기가 다루기 더 어려울 수도 있습니다.
...
실제 양자 컴퓨터에서 계산을 실행할 때는 양자 회로의 논리만 고려하는 것이 아닙니다. 특정 양자 컴퓨터가 수행할 수 있는 연산이 무엇인지, 그리고 사용 중인 Qubit이 양자 컴퓨터의 어느 위치에 있는지도 고려해야 합니다. 서로 인접해 있나요? 멀리 떨어져 있나요? 따라서 다음 단계는 사용할 양자 컴퓨터에 자연스러운 Gate를 사용하여 회로를 재작성하고, qubit 레이아웃을 반영하는 것입니다. 이는 `transpilation`(트랜스파일)을 통해 수행할 수 있습니다. 이 과정을 거치면 단순했던 앤사츠가 다른 Gate 집합으로 변환되고, 추상적인 Qubit이 실제 양자 컴퓨터의 물리적 Qubit에 매핑되는 것을 확인할 수 있습니다.
...
참고: [`resilience_level`](/guides/error-mitigation-and-suppression-techniques)을 사용하여 오류 완화 수준을 설정하면 더 나은 결과를 얻을 수 있습니다. 기본값은 1이며, 더 높은 값을 설정하면 QPU 시간은 더 소요되지만 더 나은 결과를 반환할 수 있습니다.

### Step 4: 후처리하기 {#step-4-post-process}

이제 고전 최적화기가 어떻게 동작했는지 살펴볼 차례입니다. 아래 셀을 실행하여 수렴 패턴을 확인해 보세요.
...
상당히 좋은 초기값으로 시작하여 단 10단계만에 좋은 최종값을 얻었습니다. 크고 작은 피크가 보이는데, 이것은 COBYLA 최적화기의 전형적인 특징입니다. COBYLA는 마치 경관을 볼 수 없는 것처럼 공간을 탐색하면서 각 측정마다 탐색 보폭을 조정합니다.

### 이해도 확인하기 {#check-your-understanding-3}
...
더 복잡한 기저를 선택하면 해밀토니안에서 사용되는 qubit 수가 늘어나므로, 화학 문제에 더 복잡하고 적합한 앤사츠를 선택해야 합니다.
...
마지막으로, 다른 최적화기를 시도하여 더 나은 수렴 성능을 달성할 수도 있습니다.
...
### Step 1: 문제를 양자 회로와 연산자로 매핑하기 {#step-1-map-the-problem-to-quantum-circuits-and-operators-1}
...
### Step 2: 대상 하드웨어를 위한 최적화 {#step-2-optimize-for-target-hardware-1}
...
### Step 3: 대상 하드웨어에서 실행하기 {#step-3-execute-on-target-hardware-1}
...
### Step 4: 후처리 {#step-4-post-process-1}
...
[쿨롱 법칙](https://en.wikipedia.org/wiki/Coulomb%27s_law)과 [원자 단위계](https://en.wikipedia.org/wiki/Atomic_units)를 사용하여 값이 `Hartree` 단위로 나오도록 계산해 보세요.
...
## $H + H = H_2$의 반응 에너지 계산 {#compute-reaction-energy-of-h-h-h-2}

지금까지 얻은 결과를 활용해 봅시다! 여러분은 VQE(변분 양자 고유값 분해기)를 이용해 $H$ 원자와 $H_2$ 분자의 바닥 상태 에너지를 각각 계산했습니다. 이제 남은 것은 계산된 값을 사용하여 $H+H=H_2$ 과정의 반응 에너지를 구하는 것입니다.
...
아래 셀을 실행하여 이를 시각적으로 확인해 봅시다. 여기서는 각 해밀토니안의 정확한 바닥 상태 값을 사용하고, 정확한 해와 VQE 결과의 반응 에너지를 비교합니다.
...
그림에서 볼 수 있듯이, 다소 오차가 있지만 해밀토니안의 정확한 바닥 상태 에너지와 VQE 결과를 이용해 계산한 반응 에너지는 모두 -0.2 Hartree에 근접하여 유사한 결과를 보여줍니다.

여기서 주목할 점은 이 과정의 반응 에너지가 음수 값을 가진다는 것입니다. 이는 이 과정을 통해 에너지가 방출되며, 생성된 분자가 두 개의 단일 원자보다 더 낮은 에너지 상태에 있음을 의미합니다.

지금까지 배운 내용을 정리해 봅시다.

먼저 양자 화학 문제를 풀기 위해 필요한 두 가지 중요한 근사 기법인 변분 원리와 기저 집합 선택을 살펴봤습니다. 이 두 가지는 모두 VQE의 근본적인 요소입니다. 단순 조화 진동자의 바닥 상태 에너지를 직접 계산하며 변분 원리를 탐구했습니다.

다음으로 양자 시스템의 바닥 상태 에너지를 계산하는 데 널리 사용되는 알고리즘인 VQE를 살펴봤습니다. 수소 원자($H$)와 수소 분자($H_2$)의 바닥 상태 에너지를 계산하는 코드를 실행했습니다. 특히, 해당 시스템에 적합한 분자 해밀토니안을 구하고 이를 양자 컴퓨터에서 실행 가능한 형태로 변환하는 것이 필요하다는 것을 배웠습니다. 또한 VQE에서 시험 양자 상태를 준비하기 위해 매개변수화된 양자 Circuit인 앤사츠(ansatz)가 필요하며, 적절한 앤사츠 Circuit 구조를 선택하는 것이 중요하다는 점도 살펴봤습니다. 아울러 VQE는 고전 컴퓨터를 이용한 반복적인 최적화 과정에 의존하여 양자 Circuit이 최저 에너지 상태를 찾도록 유도하며, 이 과정이 어떻게 수렴되는지도 확인했습니다.

마지막으로 VQE를 통해 계산된 $H$와 $H_2$의 바닥 상태 에너지를 사용하여 $H + H \rightarrow H_2$ 과정의 반응 에너지를 계산했습니다.

VQE는 강력한 근거리 양자 알고리즘이지만 그 한계도 인식하는 것이 중요합니다. VQE의 성능은 앤사츠 선택에 크게 의존하는데, 더 크고 복잡한 분자의 경우 실제 바닥 상태를 정확하게 표현할 수 있으면서도 효율적으로 준비 가능한 앤사츠를 찾는 것이 어려워집니다. 또한 현재의 양자 하드웨어는 노이즈에 취약하여, 특히 더 깊은 Circuit이나 더 많은 수의 Qubit을 다루는 경우 VQE 결과의 정확도에 영향을 줄 수 있습니다. 이러한 한계에도 불구하고, VQE는 기초 알고리즘으로서의 역할을 하며, 현재 진행 중인 연구들은 근거리 양자 컴퓨터에서 양자 화학의 가능성을 확장하기 위한 더 정교한 변분 방법과 오류 완화 기법을 탐색하고 있습니다. 예를 들어, SQD(샘플 기반 양자 대각화)와 같은 알고리즘이 개발 중인데, 이는 양자 Circuit에서 얻은 샘플과 부분 공간에서의 고전적 대각화를 결합하여 에너지 추정을 개선하고, 특히 측정 효율성과 노이즈 강건성 측면에서 VQE의 한계를 해결하고자 합니다.
...
- VQE에서는 시스템의 해밀토니안으로 시작하여 이를 양자 컴퓨터에서 실행할 수 있도록 Qubit에 매핑합니다. 매개변수화된 양자 Circuit인 앤사츠를 선택하고, 최저 에너지 값에 도달할 때까지 앤사츠의 매개변수를 변경하면서 반복적으로 측정합니다. 매개변수 공간 탐색은 고전 최적화기를 사용하여 수행됩니다. 좋은 결과를 얻으려면 적절한 앤사츠와 최적화기를 선택하는 것이 필요합니다.

- 반응 에너지는 화학 반응에서의 총 에너지 변화로, 반응물과 생성물의 에너지 차이로 결정됩니다.


### 참/거짓 {#truefalse}
1. 변분 원리는 임의의 시험 파동함수에 대한 에너지의 기댓값이 항상 실제 바닥 상태 에너지보다 크거나 같다고 말합니다.

2. 기저 집합은 양자 파동함수를 근사하는 데 사용되는 함수들의 모음입니다.

3. VQE는 주어진 해밀토니안에 대해 슈뢰딩거 방정식을 정확하게 풀기 위한 양자 알고리즘입니다.

4. VQE에서는 매개변수화된 양자 Circuit(앤사츠)을 사용하여 시험 파동함수를 준비합니다.
...
3. 이 노트북에서 앤사츠를 비교하기 위해 생성되는 양자 상태는 몇 개인가요?
...
- B) 에너지를 최소화하도록 앤사츠 매개변수를 업데이트하기 위해
...
5. 앤사츠가 매개변수화되도록 설계되는 이유는 무엇인가요?
...
6. 좋은 앤사츠를 선택하는 것에 대해 가장 올바른 설명은 무엇인가요?

- A) 앤사츠는 블로흐 구 위에 균등하게 분포된 상태를 생성해야 하며, 그렇지 않으면 실패합니다.

- B) 앤사츠는 바닥 상태에 가까운 상태를 생성할 수 있도록 시스템에 맞게 설계되어야 합니다.

- C) 앤사츠는 변분 매개변수를 사용하여 무작위 상태를 생성해야 합니다.

- D) 더 좋은 앤사츠는 항상 더 많은 변분 매개변수를 가집니다.


## (Optional) 부록: Ansatz 복잡도에 따른 최적화기 오버헤드 {#optional-appendix-optimizer-overhead-by-ansatz-complexity}
...
VQE의 최적화 지형에는 기울기가 지수적으로 소실되는 영역(barren plateau)이 존재할 수 있어, 고전 최적화기가 변분 매개변수를 효율적으로 업데이트하기 어렵게 만듭니다. 이를 위해 연구자들은 기울기 기반 및 기울기 없는 방식 등 다양한 유형의 최적화기를 시도해 왔지만, 두 방식 모두 과제를 안고 있습니다. 기울기 기반 최적화기는 barren plateau 문제를 겪고, 기울기 없는 방법은 많은 수의 함수 평가를 필요로 합니다.

3. 최적화기 오버헤드


또 다른 잘 알려진 과제는 최적화기 오버헤드로, 이는 문제의 규모와 관련이 있습니다. VQE에 필요한 양자 Circuit은 문제 크기가 증가함에 따라 깊이와 복잡도가 커지며, 이는 일반적으로 최적화해야 할 매개변수의 수도 증가시킵니다. 매개변수 수가 늘어날수록 최적화 과정은 다루기 어려워지고, 수렴이 느려지며 최적해를 찾는 데 어려움이 생깁니다.
...
단순한 단일 qubit Circuit과 간단한 Ansatz에 의존하는 것과 달리, 더 큰 양자 Circuit과 복잡한 구조의 Ansatz가 필요할 때 최적화의 복잡성이 증가합니다. 이는 VQE의 잘 알려진 과제인 최적화기 오버헤드를 잘 보여줍니다.
