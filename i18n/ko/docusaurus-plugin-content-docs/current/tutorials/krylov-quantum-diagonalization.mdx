...
### 효율적인 아다마르 테스트 {#efficient-hadamard-test}
일부 근사를 도입하고 모델 해밀토니안에 대한 특정 가정에 의존함으로써, 앞서 얻은 아다마르 테스트의 심층 회로를 최적화할 수 있습니다. 예를 들어, 다음과 같은 아다마르 테스트 회로를 고려해 보겠습니다:
...
### 아다마르 테스트를 통한 $\tilde{S}$ 및 $\tilde{H}$ 행렬 요소 계산을 위한 템플릿 회로 {#template-circuits-for-calculating-matrix-elements}
아다마르 테스트에서 사용되는 회로 간의 유일한 차이는 시간 진화 연산자의 위상과 측정되는 관측량입니다. 따라서 시간 진화 연산자에 의존하는 게이트에 대한 플레이스홀더를 포함하여 아다마르 테스트의 일반 회로를 나타내는 템플릿 회로를 준비할 수 있습니다.
...
Trotter 근사와 비제어 유니타리를 조합하여 아다마르 테스트의 깊이를 상당히 줄였습니다.
...
## 부록: 실수 시간 진화를 통한 크릴로프 부분 공간 {#appendix-krylov-subspace-from-real-time-evolutions}

유니터리 크릴로프 공간은 다음과 같이 정의됩니다.

$$
\mathcal{K}_U(H, |\psi\rangle) = \text{span}\left\{ |\psi\rangle,  e^{-iH\,dt} |\psi\rangle, \dots, e^{-irH\,dt} |\psi\rangle \right\}
$$

이때 $dt$는 나중에 결정할 시간 간격입니다. 잠시 $r$이 짝수라고 가정하면, $d=r/2$로 정의할 수 있습니다. 해밀토니안을 위의 크릴로프 공간에 투영할 때, 아래의 크릴로프 공간과 구별할 수 없다는 점에 주목하세요.
...
이 크릴로프 공간 어딘가에 저에너지 상태가 반드시 존재함을 보이고자 합니다. 이를 위해 [\[3\]](#references)의 정리 3.1에서 도출된 다음 결과를 사용합니다.
...
아래에 증명을 제공하지만, 완전하고 엄밀한 논증을 이해하고 싶지 않다면 건너뛰어도 됩니다. 지금은 위 주장의 함의에 집중하겠습니다. 위의 성질 3에 의해, 위에서 이동된 크릴로프 공간에는 상태 $f(H)|\psi\rangle$이 포함된다는 것을 알 수 있습니다. 이것이 저에너지 상태입니다. 이를 확인하기 위해 에너지 고유 기저로 $|\psi\rangle$을 전개하면:
...
이는 임의의 $\delta>0$에 대해 성립하므로, $\delta$를 목표 오차와 같게 설정하면 위의 오차 상한은 크릴로프 차원 $2d=r$에 따라 지수적으로 수렴합니다. 또한 $\delta<E_1-E_0$이면 위의 상한에서 $\delta$ 항이 완전히 사라집니다.

논증을 완성하기 위해, 위는 크릴로프 공간에서 가장 낮은 에너지 상태의 에너지 오차가 아니라 특정 상태 $f(H)|\psi\rangle$의 에너지 오차임을 먼저 주목합니다. 그러나 (Rayleigh-Ritz) 변분 원리에 의해, 크릴로프 공간에서 가장 낮은 에너지 상태의 에너지 오차는 크릴로프 공간의 어떤 상태의 에너지 오차로도 상한을 구할 수 있으므로, 위는 가장 낮은 에너지 상태, 즉 크릴로프 양자 대각화 알고리즘의 출력의 에너지 오차에 대한 상한이기도 합니다.
...
이것을 복소 지수 함수로 자연스럽게 표현할 수 있는 함수로 변환하고자 합니다. 복소 지수 함수는 양자 크릴로프 공간을 생성하는 실수 시간 진화이기 때문입니다.
