---
title: "Global Data Quantum의 Portfolio Optimizer를 활용한 동적 포트폴리오 최적화"
sidebar_label: "Global Data Quantum의 Portfolio Optimizer를 활용한 동적 포트폴리오 최적화"
...
양자 실행은 `ibm_torino` Backend에서 수행됩니다. 문제 공식화, 방법론 및 성능 평가에 대한 자세한 설명은 게재된 논문 [\[1\]](#references)을 참조하세요.
## 요구 사항 {#requirements}
...
Yahoo Finance API에서 데이터가 올바르게 다운로드되었는지 확인하기 위해 다음 셀을 실행하는 것을 권장합니다. 다운로드가 실패하면 대신 대체 온라인 데이터베이스에서 데이터를 가져온다는 알림이 표시됩니다.
</Admonition>
...
`ansatz_settings` 딕셔너리는 양자 회로 앤사츠(ansatz)를 구성합니다. `ansatz` 파라미터는 금융 최적화 문제를 위해 설계된 하드웨어 효율적 앤사츠인 `"optimized_real_amplitudes"` 방식의 사용을 지정합니다. 여러 패스 매니저(기본 로컬 Qiskit 패스 매니저 및 Qiskit AI 기반 Transpiler 서비스 포함)의 사용을 가능하게 하여 회로 최적화 성능과 효율성을 향상시키는 `multiple_passmanager` 옵션은 이 기능이 일시적으로 비활성화되어 있어 현재 false로 설정되어 있습니다.
