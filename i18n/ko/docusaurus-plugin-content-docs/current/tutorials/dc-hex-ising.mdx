...
DD 시퀀스는 함수 `add_stretch_dd()`에 의해 추가되며, 이 함수는 [`stretch` durations](/guides/stretch)를 사용하여 DD 게이트 사이의 시간 간격을 결정합니다. `stretch` duration은 `delay` 연산에 대해 신축 가능한 시간 길이를 지정하는 방법으로, 지연 시간이 큐비트 유휴 시간을 채우도록 늘어날 수 있게 합니다. `stretch`로 지정된 duration 변수는 컴파일 시점에 특정 제약을 충족하는 원하는 duration으로 해석됩니다. 이는 DD 시퀀스의 타이밍이 우수한 오류 억제 성능을 달성하는 데 필수적일 때 매우 유용합니다. `stretch` 타입에 대한 자세한 내용은 [OpenQASM](https://openqasm.com/language/delays.html#duration-and-stretch-types) 문서를 참조하세요. 현재 `stretch` 타입에 대한 지원은 실험적입니다. 사용 제약에 대한 자세한 내용은 `stretch` 문서의 [제한 사항 섹션](/guides/stretch#qiskit-runtime-limitations)을 참조하세요.
...
`MidCircuitMeasure`는 [중간 Circuit 측정](/guides/execute-dynamic-circuits#midcircuit)을 수행하기 위해 특별히 보정된 추가 측정 연산입니다. `MidCircuitMeasure` 명령어는 Backend가 지원하는 `measure_2` 명령어에 매핑됩니다. `measure_2`가 모든 Backend에서 지원되는 것은 아닙니다. `service.backends(filters=lambda b: "measure_2" in b.supported_instructions)`를 사용하여 이를 지원하는 Backend를 찾을 수 있습니다. 여기서는 Backend가 지원하는 경우, Circuit에 정의된 중간 Circuit 측정이 `MidCircuitMeasure` 연산을 사용하여 실행되도록 Circuit을 트랜스파일하는 방법을 보여줍니다.
...
작업이 완료된 후, 작업 결과 메타데이터에서 Circuit 실행 시간을 가져와 Circuit 스케줄 정보를 시각화할 수 있습니다. Circuit의 스케줄링 정보를 시각화하는 방법에 대한 자세한 내용은 [이 페이지](/guides/qiskit-runtime-circuit-timing)를 참조하세요.
