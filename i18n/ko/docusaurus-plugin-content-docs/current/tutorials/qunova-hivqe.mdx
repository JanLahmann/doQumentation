---
description: 'Qunova HiVQE를 사용하여 해리 퍼텐셜 에너지 표면(PES) 곡선을 계산합니다.'
notebook_path: tutorials/qunova-hivqe.ipynb
sidebar_label: 'Qunova HiVQE로 해리 PES 곡선 계산하기'
title: 'Qunova HiVQE로 해리 PES 곡선 계산하기'
---
...
Qiskit Functions는 IBM Quantum&reg; Premium Plan, Flex Plan, On-Prem(IBM Quantum Platform API) Plan 사용자에게만 제공되는 실험적 기능입니다. 현재 미리 보기 릴리스 상태로, 변경될 수 있습니다.
</Admonition>

*사용량 예상치 (참고: 이는 추정치이며, 실제 실행 시간은 다를 수 있습니다.)*

* Li2S: Heron r2 프로세서에서 약 5분 QPU 시간

* FeP-NO: Heron r2 프로세서에서 약 5분 QPU 시간


## 배경 {#background}

화학 반응 에너지를 정확하게 계산하는 것은 재료 과학, 화학 공학, 신약 개발 등 다양한 분야의 과학적 발전에 매우 중요합니다. 다양한 화학 시스템 중에서 Li-S 시스템은 새로운 배터리 조성을 이해하고 개발하는 데 큰 주목을 받고 있습니다. 이 튜토리얼에서는 HiVQE 계산을 이용하여 리튬 원자 하나를 제거하는 방식으로 $Li_2S$ 시스템의 Li-S 결합 해리 퍼텐셜 에너지 표면(PES)을 계산하는 실습 경험을 제공합니다. 20 Qubit 문제에 대한 결과를 참조 계산(CASCI) 및 Hartree-Fock(HF)과 같은 고전적 방법과 비교할 수 있습니다.

## 요구 사항 {#requirements}

이 튜토리얼의 코드를 실행하려면 아래 의존성을 설치하세요.
...
이 튜토리얼을 실행하려면 `QiskitFunctionCatalog`를 통해 `qunova/hivqe-chemistry` 함수를 임포트하세요. 이 함수를 실행하려면 IBM Quantum Premium Plan, Flex Plan, 또는 On-Prem(IBM Quantum Platform API) Plan 계정과 Qunova의 라이선스가 필요합니다.
...
## Part 1: Li2S (20Q) {#part-1-li2s-20q}

### 1단계: 고전적 입력을 양자 문제로 매핑하기 {#step-1-map-classical-inputs-to-a-quantum-problem}

PES 곡선을 계산하기 위해 Li-S의 다양한 결합 거리에 대한 $Li_2S$ 기하 구조를 딕셔너리 형식으로 정의합니다. 이 기하 구조들은 B3LYP/631g 계산을 이용하여 최적화된 것입니다.
...
HiVQE 계산은 아래에 정의된 옵션을 사용하여 수행됩니다. $Li_2S$에 sto3g 기저를 사용하면 22개의 전자를 가진 19개의 공간 오비탈이 존재합니다. HiVQE 계산에서 (10o,10e) 경우를 실행하려면 10개의 활성 오비탈과 6개의 동결 오비탈을 정의할 수 있습니다. 각 반복에서 `circular` 얽힘과 두 번의 반복(`reps`)을 가진 ExcitationPreserving 양자 Circuit(`epa`)으로 생성된 전자 배치를 샘플링하기 위해 100번의 샷이 사용됩니다. 에너지 수렴으로 반복이 종료될 수 있도록 최대 반복 횟수는 30으로 설정되어 있습니다.
...
### 2~3단계: 양자 하드웨어 실행을 위한 문제 최적화 및 HiVQE Chemistry 함수로 실행하기 {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}

아래에 정의된 옵션을 사용하여 기하 구조에 대한 HiVQE 계산을 실행하기 위해 `for` 루프를 설정합니다. 작업은 `for` 루프 안에서 제출됩니다. 이 튜토리얼에서는 6개의 기하 구조를 제출하고 모두 완료되면 결과를 가져옵니다. 메인 함수 실행 시, `max_states`와 `max_expansion_states`를 정의하여 부분 공간 행렬의 최대 크기와 반복당 고전적 CI 확장 방법으로 생성할 수 있는 상태의 수를 제어해야 합니다. 함수 작업 ID는 각 기하 구조 레이블과 함께 딕셔너리에 저장되어 출력을 추적하고 처리하는 데 사용됩니다.
...
모든 작업이 아직 실행 중인지 또는 완료되었는지 확인해 보겠습니다.
...
모든 작업이 완료되면, 모든 계산 결과를 가져와 보겠습니다.
...
작업에 사용된 전체 QPU 런타임은 [IBM Quantum Platform](https://quantum.cloud.ibm.com/)에 로그인하여 `qunova-chemistry-hivqe` 태그가 붙은 제출된 작업을 확인함으로써 추적할 수 있습니다.

### 4단계: 후처리 및 고전적 방법과의 비교 {#step-4-post-process-and-compare-with-classical-methods}

HiVQE 결과를 검증하기 위해 (10o,10e)에 대한 고전적 참조 계산(CASCI)을 수행할 수 있습니다.
...
### Li\_2S 해리 곡선 플롯 {#plotting-the-dissociation-curve-for-li-2s}

HiVQE 결과를 HF 및 CASCI와 비교하여 플롯해 보겠습니다.
모든 HiVQE 계산 결과가 고전적 참조 결과(CASCI)와 잘 일치하는 것을 확인할 수 있습니다.
...
## Part 2: FeP-NO (44Q) {#part-2-fep-no-44q}

이 예제에 표시된 설정으로 FeP-NO 시스템에서 함수를 실행하려면, 최소 44 Qubit을 허용하는 라이선스가 필요합니다. 라이선스 취득에 대한 문의는 [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com)으로 이메일을 보내세요.

### 1단계: 고전적 입력을 양자 문제로 매핑하기 {#step-1-map-classical-inputs-to-a-quantum-problem-1}

HiVQE 계산을 위한 옵션을 정의합니다.
...
Fe-N 결합 거리에 따른 퍼텐셜 에너지 곡선(PES)을 계산하기 위해, 서로 다른 결합 거리에 해당하는 FeP-NO 기하 구조를 딕셔너리 형식으로 정의합니다.
...
### 2단계 및 3단계: 양자 하드웨어 실행을 위한 문제 최적화 및 HiVQE Chemistry 함수를 사용한 실행 {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

HiVQE 설정과 기하 구조를 기반으로 결과를 순차적으로 얻습니다.

#### d(Fe-N) = 1.75 $\AA$ 계산 제출 {#submit-dfe-n-175-aa-calculation}
...
d(Fe-N) = 1.75 $\AA$ 계산에 대한 작업을 추적하고 결과를 가져옵니다.
...
#### d(Fe-N) = 2.00 $\AA$ 계산 제출 {#submit-dfe-n-200-aa-calculation}
...
d(Fe-N) = 2.00 $\AA$ 계산에 대한 작업을 추적하고 결과를 가져옵니다.
...
#### d(Fe-N) = 5.00 $\AA$ 계산 제출 {#submit-dfe-n-500-aa-calculation}
...
d(Fe-N) = 5.00 $\AA$ 계산에 대한 작업을 추적하고 결과를 가져옵니다.
...
### 4단계: 후처리 및 고전적 방법과의 비교 {#step-4-post-process-and-compare-with-classical-methods-1}

HiVQE 결과를 검증하기 위해 (22o,22e)에 대한 고전적 참조 계산(CASCI-DMRG, maxM=800) 결과를 제공합니다.
...
![이전 코드 셀의 출력](/docs/images/tutorials/qunova-hivqe/extracted-outputs/49a9bf81-1.avif)

## 튜토리얼 설문조사 {#tutorial-survey}

이 튜토리얼에 대한 피드백을 제공하기 위해 간단한 설문조사에 참여해 주세요. 여러분의 의견은 콘텐츠 제공 방식과 사용자 경험을 개선하는 데 큰 도움이 됩니다.
...
[설문조사 링크](https://your.feedback.ibm.com/jfe/form/SV_9AizRyKFHEGzVs2)
