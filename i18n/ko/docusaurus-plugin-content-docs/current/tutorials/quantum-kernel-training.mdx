...
이를 위해 위의 소스 코드를 `./source/generate_kernel_entry.py` 파일로 이동하고, 입력을 받아 최종 솔루션을 반환하는 스크립트로 코드를 감싼 다음, `Qiskit Serverless`의 `QiskitFunction` 클래스를 사용하여 원격 클러스터에 업로드합니다. 외부 종속성 지정, 입력 인수 전달 등에 대한 지침은 [Qiskit Serverless 가이드](https://qiskit.github.io/qiskit-serverless/function_features/index.html#function-features)를 참조하세요.
