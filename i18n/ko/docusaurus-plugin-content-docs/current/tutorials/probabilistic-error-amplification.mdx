...
이 튜토리얼은 *확률적 오류 증폭* (PEA)을 적용한 *영잡음 외삽* (ZNE)의 실험적 버전을 사용하여 IBM Quantum Compute Service로 유틸리티 규모의 오류 완화 실험을 실행하는 방법을 설명합니다.
...
유틸리티 규모 실험에서는 *확률적 오류 증폭* (PEA)이 가장 유망한 선택입니다.
...
PEA는 *확률적 오류 상쇄* (PEC)와 동일한 레이어 기반 잡음 모델을 가정합니다. 다만, Circuit 잡음에 따라 지수적으로 증가하는 샘플링 오버헤드를 피할 수 있습니다.
...
**참고 문헌**: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, *희소 파울리-린드블라드 모델을 사용한 잡음 양자 프로세서에서의 확률적 오류 상쇄* [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)
...
##### Backend 설정 {#establish-a-backend}
...
##### 엔탱글링 레이어 커플링 정의 {#define-entangling-layer-couplings}
Trotterized Ising 시뮬레이션을 구현하기 위해 디바이스에서 각 Trotter 단계마다 반복될 2-큐비트 게이트 커플링의 세 레이어를 정의합니다. 이는 잡음을 학습하여 완화를 구현하는 데 필요한 세 가지 트월링된 레이어를 정의합니다.
...
##### 메인 Trotter Circuit 생성 {#main-trotter-circuit-generation}
...
#### 나중에 할당할 매개변수 값 목록 생성 {#create-a-list-of-parameter-values-to-be-assigned-later}
...
#### Estimator 옵션 구성 {#configure-estimator-options}
다음으로 완화 실험을 실행하는 데 필요한 `Estimator` 옵션을 구성합니다. 여기에는 엔탱글링 레이어의 잡음 학습 옵션과 ZNE 외삽 옵션이 포함됩니다.
...
##### ZNE 옵션 설명 {#explanation-of-zne-options}
다음은 실험 브랜치의 추가 옵션에 대한 세부 사항입니다. 이러한 옵션과 이름은 확정되지 않았으며, 공식 릴리스 전에 변경될 수 있습니다.

* **amplifier**: 의도한 잡음 인자로 잡음을 증폭할 때 사용하는 방법입니다.
  허용되는 값은 2-큐비트 기저 게이트를 반복하여 증폭하는 `"gate_folding"`,
  그리고 트월링된 2-큐비트 기저 게이트 레이어에 대한 Pauli-트월링된 잡음 모델을 학습한 후 확률론적 샘플링으로 증폭하는 `"pea"`입니다. 추가 옵션으로 `"gate_folding_front"` 및 `"gate_folding_back"`이 있으며, 이는 [API 문서](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier)에서 설명됩니다.

* **extrapolated\_noise\_factors**: 외삽 모델을 평가할 하나 이상의 잡음 인자 값을 지정합니다. 값의 시퀀스를 지정하면 반환된 결과는 외삽 모델에 대해 지정된 잡음 인자로 배열 값이 됩니다. 값
  0은 영잡음 외삽에 해당합니다.


#### 실험 실행 {#run-the-experiment}
...
1. `pub_result.data.evs`: 영 잡음(휴리스틱적으로 최적의 외삽 기반)에 해당하는 기댓값입니다.
   ...
   * 첫 번째 축은 관측량 $\langle Z_i\rangle$에 대한 가상 큐비트 인덱스입니다 ($124$개의 가상 큐비트/관측량)

   * 두 번째 축은 $\theta$에 대한 매개변수 값을 인덱싱합니다 ($12$개의 매개변수 값)

2. `pub_result.data.evs_extrapolated`: 모든 외삽기(extrapolator)에 대한 외삽된 잡음 인자의 기댓값입니다. 이 배열에는 두 개의 추가 축이 있습니다.

   * 세 번째 축은 외삽 방법을 인덱싱합니다 ($2$개의 외삽기, `exponential`과 `linear`)

   * 마지막 축은 `extrapolated_noise_factors`를 인덱싱합니다 (옵션에 지정된 $20$개의 외삽 지점)

3. `pub_result.data.evs_noise_factors`: 각 잡음 인자에 대한 원시 기댓값입니다.

   * 세 번째 축은 원시 `noise_factors`를 인덱싱합니다 ($3$개의 인자)
...
* `resilience/zne/noise_factors`: 원시 잡음 인자
...
![이전 코드 셀의 출력](/docs/images/tutorials/probabilistic-error-amplification/extracted-outputs/e466736a-0.avif)
...
![이전 코드 셀의 출력](/docs/images/tutorials/probabilistic-error-amplification/extracted-outputs/bea9695a-0.avif)

## 다음 단계 {#next-steps}
<Admonition type="tip" title="권장 사항">
이 내용이 흥미로우셨다면 다음 자료를 참고해 보세요:
- 오류 완화 기법을 결합하는 방법에 초점을 맞춘 [튜토리얼](/tutorials/combine-error-mitigation-techniques).
- Qiskit에서 사용 가능한 오류 완화 기법에 대한 상세한 [문서](/guides/error-mitigation-and-suppression-techniques).
- 유틸리티 규모 실험을 다루는 추가 레슨: [유틸리티 II](/learning/courses/utility-scale-quantum-computing/utility-ii) 및 [유틸리티 III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
