...
이 회로의 모든 게이트가 기저 게이트 집합에 속하도록 ```BasisTranslator```와 그다음 ```Optimize1qGatesDecomposition``` 패스를 적용하겠습니다. 라우팅 및 초기 레이아웃 선택과 같은 다른 단계들은 다시 수행되지 않으므로, 이 두 패스를 사용하는 것이 전체 트랜스파일 과정보다 빠릅니다.
...
> **참고:** 이 설문 조사는 **IBM Quantum**에서 제공하며 (IBM이 작성한) **튜토리얼 콘텐츠**를 다룹니다. doQumentation은 웹사이트, 번역 및 코드 실행을 제공합니다 — 이에 대한 피드백은 [GitHub 이슈를 열어](https://github.com/JanLahmann/doQumentation/issues) 주세요.

[설문 조사 링크](https://your.feedback.ibm.com/jfe/form/SV_3fQQYAIjTxvIChg)
