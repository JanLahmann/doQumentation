...
- 실제로는 ${M}$의 역행렬을 구하는 방식으로 방정식 (2)를 풀지 않습니다. 소프트웨어 라이브러리의 선형 대수
...
- 복원된 분포 ${\vec{p}}$에는 하나 이상의 음수 확률이 있을 수 있습니다(합이 여전히 1인 경우에도). 한 가지 해결책은
  ...
  방법의 실행 시간은 방정식 (2)를 직접 푸는 것보다 수십~수백 배(자릿수 단위로) 더 깁니다.
...
문제에 대해 아래에서 제시하는 것과 유사한 기법을 통해 양자 속도 향상이 가능한 큰 함수 부류(class) 가운데 가장 단순한 것입니다.
...
이 인용문은 논문 [_고도로 비선형인 불리언 함수에 대한 양자 알고리즘_](https://arxiv.org/abs/0811.3208)에서 가져온 것으로,
여러 종류의 벤트 함수에 대한 효율적인 숨겨진 이동 알고리즘을 제공합니다.
이 튜토리얼의 알고리즘은 해당 논문의 3.1절에 나옵니다.
...
먼저, 숨겨진 이동(hidden shift) 문제를 `QuantumCircuit`으로 구현하는 함수를 작성합니다.
...
추가적인 복잡성은, 더 단순한(사전 트랜스파일된) Circuit을 갖기 위해 인접한 qubit 쌍 사이에 필요한 CZ 게이트를 삽입했다는 점입니다. 이는 비트열 $a$와 $b$를 $a1 b1 a2 b2 \ldots$와 같이 교차 배열하는 것과 같습니다.
...
두 비트열 사이의 해밍 거리(Hamming distance)는 비트가 다른 인덱스의 수입니다.
...
M3를 사용한 판독 오류 완화를 적용하기 전에 가장 확률이 높은 비트열의 확률을 기록해 둡시다.
...
#### M3 보정 적용 전후의 숨겨진 이동 문자열 식별 비교 {#compare-identifying-the-hidden-shift-string-before-and-after-applying-m3-correction}
...
올바른 숨겨진 이동 문자열이 발견되었음을 확인할 수 있습니다. 또한 다음으로 확률이 높은 아홉 개의 비트열은 단 한 위치에서만 틀렸습니다.
가장 높은 확률을 기록합니다:
