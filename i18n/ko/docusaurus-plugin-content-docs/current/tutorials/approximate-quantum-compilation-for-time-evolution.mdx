...
- **얽힘이 고전적으로 처리 가능한 경우.** 시간 진화된 상태의 총 얽힘은 주로 Trotter 스텝 수 $k$가 아닌 시간 $t$에 따라 달라집니다. 즉 $t$가 채권 차원을 관리 가능한 수준으로 유지할 만큼 충분히 짧다면, $10k$개의 스텝을 갖는 목표 회로는 일반적으로 $k$개의 스텝을 갖는 회로보다 MPS로 표현하기 어렵지 않습니다.
