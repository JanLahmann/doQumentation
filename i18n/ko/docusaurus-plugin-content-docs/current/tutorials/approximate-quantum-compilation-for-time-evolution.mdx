...
그 결과 단일 Trotter 스텝의 깊이를 유지하면서도 많은 스텝의 정확도를 달성하는 회로가 생성되어, 가까운 미래의(단기) 양자 하드웨어에서 실행 가능해집니다.
...
이 섹션에서는 10사이트 시스템을 사용하여 AQC-Tensor 워크플로우를 단계별로 설명합니다. 우리는 스핀 상호작용과 자기적 특성을 조사하기 위해 널리 연구되는 모델인 10사이트 XXZ 스핀 체인의 동역학을 시뮬레이션합니다.
