...
두 번째 Qubit를 $Z$ 기저와 $X$ 기저에서 측정합니다. 첫 번째 Qubit도 직교 기저에서 측정하지만, 두 번째 Qubit에 대한 각도를 $0$에서 $2\pi$ 사이로 스윕합니다. 보면 알 수 있듯이, Estimator 프리미티브를 사용하면 매개변수화된 Circuit 실행이 매우 간편합니다. 일련의 CHSH Circuit을 생성하는 대신, 측정 각도를 지정하는 매개변수와 해당 매개변수의 위상 값 목록을 가진 *하나의* CHSH Circuit만 생성하면 됩니다.
