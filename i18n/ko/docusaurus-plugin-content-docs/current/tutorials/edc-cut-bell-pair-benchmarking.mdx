...
* [Qiskit SDK](https://docs.quantum.ibm.com/guides) 및 [Qiskit Runtime](/guides/compute-services#qiskit-runtime)에 대한 기본 지식과 [IBM Quantum&reg; 계정](/guides/cloud-setup) 접근 권한.
## 배경 {#background}
...
실제 QPU에서 실행하기 전에, 노이즈 없는 시뮬레이터에서 4-Qubit 체인 `[0, 1, 2, 3]`으로 Circuit을 테스트하여 Bell 쌍을 생성하는지 확인합니다. Qiskit Runtime `Sampler`와 백엔드 모드로 `AerSimulator`를 사용하여 Circuit을 실행합니다.
