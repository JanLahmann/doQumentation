...
이 튜토리얼에서 수행할 작업은 다음과 같습니다. 그리드 안정성 데이터셋을 준비해 균형을 맞추고, 고전 AdaBoost 기준선을 세우고, 앙상블 너비와 정규화를 달리한 여러 양자 구성을 실행하고, Qiskit Serverless를 통해 IBM&reg; 시뮬레이터 또는 QPU에서 실행하고, 모든 실행에 걸쳐 정확도, 정밀도, 재현율, F1을 비교합니다. 과정에서 함수의 액션 패턴(`create`, `fit`, `predict`, `fit_predict`, `create_fit_predict`)과 주요 제어 항목을 사용합니다:
...
- 옵티마이저 옵션: 시뮬레이터 대 QPU, 반복 횟수, 고전 옵티마이저 및 해당 옵션, 트랜스파일 깊이, 런타임 Sampler/Estimator 설정
