...
<Admonition type="tip" title="추천 자료">
  ...
  * **샘플 기반 양자 대각화(SQD) 시도:** 이 튜토리얼에서 보여주듯이, VQE는 척박한 고원과 높은 측정 오버헤드로 인해 대규모에서 어려움을 겪습니다. IBM은 더 확장 가능한 대안으로 [샘플 기반 양자 대각화(SQD)](/guides/qiskit-addons-sqd)를 개발했습니다. VQE와 달리 SQD는 변분 최적화를 완전히 피합니다. 대신 양자 컴퓨터가 샘플을 생성하고 고전 컴퓨터가 그 샘플이 걸친 부분 공간에 해밀토니안을 사영하여 대각화합니다. 이를 통해 훨씬 적은 측정 횟수로, 척박한 고원에 영향받지 않고 바닥 상태 에너지의 상한을 제공합니다. [SQD 튜토리얼](/tutorials/sample-based-quantum-diagonalization)에서 이 접근 방식을 실제로 확인해 보세요.
