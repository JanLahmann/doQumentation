...
## 설정 {#setup}
먼저 [IBM Quantum API 키](http://quantum.cloud.ibm.com/)를 사용하여 인증합니다. 그런 다음 아래와 같이 Qiskit Function을 선택합니다. (이 코드는 이미 [계정을 저장](/guides/functions-get-started#install-qiskit-functions-catalog-client)했다고 가정합니다.)
...
### 단일 qubit 자화량 추정 $\langle Z_i \rangle$ {#estimate-single-qubit-magnetizations-langle-z-i-rangle}
...
실제 양자 하드웨어에서 자화 동역학을 평가합니다. Fire Opal은 자동화된 오류 억제 및 성능 관리로 표준 IBM Quantum Estimator 프리미티브를 확장하는 Qiskit 함수를 제공합니다. Trotterized Circuit을 IBM&reg; Backend에 직접 제출하고, Fire Opal이 노이즈 인식 실행을 처리합니다.
