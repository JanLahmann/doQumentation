---
title: Qiskit Runtime V2 프리미티브로 마이그레이션
...
```
  ...
  <TabItem value="SamplerOut" label="Convert V2 output to V1 format">
    V1 출력 형식은 각 회로에 대해 비트 문자열(정수)을 키로, 유사 확률을 값으로 하는 딕셔너리였습니다. V2 형식은 동일한 키(문자열)와 카운트를 값으로 합니다. V2 형식을 V1으로 변환하려면, 카운트를 샷 수로 나누세요. 선택한 샷 수는 [Sampler 옵션](/guides/sampler-options#shots) 가이드에 설명되어 있습니다.
...
5. PUB 목록을 전달하도록 Estimator의 `run()` 메서드를 업데이트하세요. 예: `run([(circuit1, observable1, parameter_set1)])`.
...
5. PUB 목록을 전달하도록 Sampler의 `run()` 메서드를 업데이트하세요. 예: `run([(circuit1, parameter_set1)])`.
...
- [Sampler 옵션](/guides/sampler-options), [Estimator 옵션](/guides/estimator-options), [Executor 옵션](/guides/executor-options) 가이드에서 옵션 설정에 대해 자세히 알아보세요.
- [프리미티브 입력과 출력](/guides/primitive-input-output#pubs)에 대해 더 자세히 알아보세요.
