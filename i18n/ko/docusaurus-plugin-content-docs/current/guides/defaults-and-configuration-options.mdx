...
- `backend`(Backend) - 이 설정이 지정되면 트랜스파일러는 입력 Circuit을 이 디바이스에 맞게 컴파일합니다. `coupling_map`과 같이 이러한 설정에 영향을 주는 다른 옵션이 설정되면, `backend`의 설정을 덮어씁니다.
- `target`(Target) - Backend 트랜스파일러 타겟입니다. 일반적으로 이는 backend 인수의 일부로 지정되지만, Target 객체를 수동으로 구성한 경우 여기에서 지정할 수 있습니다. 이는 `backend`의 타겟을 덮어씁니다.
- `backend_properties`(BackendProperties) - QPU가 반환하는 속성으로, Gate 오류, 판독 오류, Qubit 결맞음 시간 등에 대한 정보를 포함합니다. `backend.properties()`를 실행하여 이 정보를 제공하는 QPU를 찾으세요.
- `timing_constraints`(Dict[str, int] | None) - 명령 시간 해상도에 대한 선택적 제어 하드웨어 제한입니다. 이 정보는 QPU 구성에 의해 제공됩니다. QPU에 명령 시간 할당에 대한 제한이 없는 경우, `timing_constraints`는 `None`이며 조정이 수행되지 않습니다. QPU는 다음과 같은 제한 사항을 보고할 수 있습니다:
    - `granularity`: dt 단위의 최소 펄스 Gate 해상도를 나타내는 정수 값입니다. 사용자 정의 펄스 Gate의 기간은 이 세분성 값의 배수여야 합니다.
    - `min_length`: dt 단위의 최소 펄스 Gate 길이를 나타내는 정수 값입니다. 사용자 정의 펄스 Gate는 이 길이보다 길어야 합니다.
...
- `inst_map`(List[InstructionScheduleMap] | None) - Circuit 연산을 펄스 스케줄에 매핑합니다. `None`인 경우 QPU의 `instruction_schedule_map`이 사용됩니다.
</AccordionItem>
</Accordion>
...
- `unitary_synthesis_plugin_config`(dict) - 유니터리 합성 플러그인에 직접 전달되는 선택적 구성 딕셔너리입니다. 기본 유니터리 합성 방법은 사용자 정의 구성을 받지 않으므로 기본적으로 이 설정은 효과가 없습니다. 사용자 정의 구성을 적용하는 것은 `unitary_synthesis` 인수로 유니터리 합성 플러그인이 지정된 경우에만 필요합니다. 이는 각 유니터리 합성 플러그인마다 사용자 정의되므로, 이 옵션을 사용하는 방법에 대해서는 플러그인의 문서를 참조하세요.
</AccordionItem>
...
- `layout_method`(str) - 레이아웃 선택 패스의 이름입니다(`default`, `dense`, `sabre`, `trivial`). 이는 레이아웃 단계에 사용할 외부 플러그인 이름일 수도 있습니다. `stage_name` 인수에 `layout`을 지정하여 `list_stage_plugins()`를 실행하면 설치된 플러그인 목록을 볼 수 있습니다. 기본값은 `sabre`입니다.
</AccordionItem>
<AccordionItem title="**라우팅 단계**">

- `routing_method`(str) - 라우팅 패스의 이름입니다(`basic`, `lookahead`, `default`, `sabre`, `none`). 이는 라우팅 단계에 사용할 외부 플러그인 이름일 수도 있습니다. `stage_name` 인수에 `routing`을 지정하여 `list_stage_plugins()`를 실행하면 설치된 플러그인 목록을 볼 수 있습니다. 기본값은 `sabre`입니다.
</AccordionItem>
<AccordionItem title="**변환 단계**">

- `translation_method`(str) - 변환 패스의 이름입니다(`default`, `synthesis`, `translator`, `ibm_backend`, `ibm_dynamic_circuits`, `ibm_fractional`). 이는 변환 단계에 사용할 외부 플러그인 이름일 수도 있습니다. `stage_name` 인수에 `translation`을 지정하여 `list_stage_plugins()`를 실행하면 설치된 플러그인 목록을 볼 수 있습니다. 기본값은 `translator`입니다.
</AccordionItem>
<AccordionItem title="**최적화 단계**">

- `approximation_degree`(float, 0-1 범위 | None) - Circuit 근사에 사용되는 휴리스틱 다이얼입니다(1.0 = 근사 없음, 0.0 = 최대 근사). 기본값은 1.0입니다. `None`을 지정하면 근사 정도가 보고된 오류율로 설정됩니다. 자세한 내용은 [근사 정도 섹션](common-parameters#approx-degree)을 참조하세요.

- `optimization_method`(str) - 최적화 단계에 사용할 플러그인 이름입니다. 기본적으로 외부 플러그인은 사용되지 않습니다. `stage_name` 인수에 `optimization`을 지정하여 `list_stage_plugins()`를 실행하면 설치된 플러그인 목록을 볼 수 있습니다.
</AccordionItem>
<AccordionItem title="**스케줄링 단계**">

- `scheduling_method`(str) - 스케줄링 패스의 이름입니다. 이는 스케줄링 단계에 사용할 외부 플러그인 이름일 수도 있습니다. `stage_name` 인수에 `scheduling`을 지정하여 `list_stage_plugins()`를 실행하면 설치된 플러그인 목록을 볼 수 있습니다.

  * 'as_soon_as_possible': Qubit 리소스에서 가능한 한 빨리 명령을 탐욕적으로 스케줄링합니다(별칭: `asap`).

  * 'as_late_as_possible': 가능한 경우 Qubit을 바닥 상태로 유지하면서 명령을 늦게 스케줄링합니다(별칭: `alap`). 이것이 기본값입니다.
</AccordionItem>
<AccordionItem title="**트랜스파일러 실행**">

- `seed_transpiler`(int) - 트랜스파일러의 확률적 부분에 대한 랜덤 시드를 설정합니다.
</AccordionItem>
</Accordion>
...
<Admonition type="tip" title="권장 사항">
    ...
    - [최적화 수준 설정](/guides/runtime-options-overview) 방법을 알아보세요.
    - [패스 매니저로 트랜스파일하기](/guides/transpile-with-pass-managers) 주제를 방문하세요.
    - 예시는 [양자 컴퓨터 표현](/guides/represent-quantum-computers)을 참조하세요.
    - Qiskit 패턴 워크플로의 일부로 [Circuit 트랜스파일 방법](/guides/circuit-transpilation-settings)을 알아보세요.
