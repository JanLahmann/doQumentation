...
DAG에는 세 가지 종류의 그래프 노드가 있습니다: Qubit/Clbit 입력 노드(녹색), 연산 노드(파란색), 출력 노드(빨간색)입니다. 각 엣지는 두 노드 간의 데이터 흐름(또는 종속성)을 나타냅니다. 이 Circuit의 DAG를 보려면 qiskit.tools.visualization.dag_drawer() 함수를 사용하세요. (이를 실행하려면 [Graphviz 라이브러리](https://graphviz.org/download/)를 설치하세요.)
