...
이 예제에서는 `qiskit_ibm_runtime`의 `FakeSherbrooke` 모의 Backend를 사용하지만, Qiskit과 호환되는 실제 또는 가상 Backend에서도 시도해 볼 수 있습니다. 결과는 다를 수 있습니다.
</Admonition>

먼저 최적화 레벨 0으로 트랜스파일해 보겠습니다.
...
최적화 레벨을 조정하면 ECR Gate 수뿐만 아니라 Circuit의 다른 측면도 변경될 수 있습니다. 최적화 레벨 설정이 레이아웃을 어떻게 변경하는지에 대한 예시는 [양자 컴퓨터 표현](/guides/represent-quantum-computers)을 참조하세요.
