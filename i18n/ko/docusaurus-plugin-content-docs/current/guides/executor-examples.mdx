---
description: 'qiskit-ibm-runtime에서 Executor 프리미티브를 사용하는 실용적인 예제들.'
...
Executor는 `QuantumProgram` 객체를 실행합니다. 각 `QuantumProgram`은 여러 항목을 포함할 수 있습니다. 이 예제는 실행을 위해 Circuit 항목과 samplex 항목을 추가합니다. 자세한 내용은 [Executor 입력 및 출력](/guides/executor-input-output)을 참조하세요.
...
### circuit 설정 {#set-up-the-circuit-1}
...
### circuit 실행 {#execute-the-circuits-1}

`Executor`는 `QuantumProgram` 객체를 실행합니다. 각 `QuantumProgram`은 프로그램에 추가되는 여러 *항목*을 포함할 수 있습니다. 각 항목은 프로그램이 수행할 작업입니다.
