---
description: 'qiskit-ibm-runtime 디버깅 도구 모듈의 Neat 클래스를 사용하여 작업을 디버그하고 분석합니다.'
notebook_path: guides/debug-qiskit-runtime-jobs.ipynb
sidebar_label: 'NEAT로 작업 디버그하기'
title: 'NEAT로 작업 디버그하기'
---
...
`Neat` 클래스를 사용하여 Estimator 워크로드에 대한 노이즈 영향을 분석할 수 있습니다. 구문 검증을 위해서는 [로컬 테스트 모드](/guides/local-testing-mode)를 사용하세요.

## `Neat` 클래스 사용법 {#neat-class-usage}

리소스를 많이 사용하는 워크로드를 하드웨어에서 실행하기 위해 제출하기 전에, IBM Quantum Compute [NEAT (Noisy Estimator Analyzer Tool)](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/debug-tools-neat#neat) 클래스를 사용하여 Estimator 워크로드가 올바르게 설정되어 있는지, 정확한 결과를 반환할 가능성이 높은지, 지정된 문제에 가장 적합한 옵션을 사용하는지 등을 확인할 수 있습니다.
...
먼저, 관련 패키지를 가져오고 [IBM Quantum Compute Service에 인증합니다](/guides/cloud-setup).
...
- [Qiskit 디버깅 도구](/guides/debugging-tools) 개요를 읽어보세요.
- [Qiskit Aer 프리미티브를 이용한 정확한 시뮬레이션 및 노이즈 시뮬레이션](/guides/simulate-with-qiskit-aer)에 대해 알아보세요.
- [사용 가능한 IBM Quantum Compute 옵션](/guides/runtime-options-overview)에 대해 알아보세요.
...
- Qiskit 패턴 워크플로우의 일부로 [Circuit을 Transpile하는 방법](/guides/circuit-transpilation-settings#compare-transpiler-settings)을 알아보세요.
