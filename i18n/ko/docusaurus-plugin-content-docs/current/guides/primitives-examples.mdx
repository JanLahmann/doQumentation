...
<span id="sampler-examples"></span>
## Sampler 예제 {#sampler-examples}

양자 Circuit 출력에서 샘플링된 전체 오류 완화 유사 확률 분포를 생성합니다. Grover 및 QVSM과 같은 검색 및 분류 알고리즘을 위해 Sampler의 기능을 활용하세요.

### 단일 실험 실행 {#run-a-single-experiment}

Sampler를 사용하여 단일 Circuit의 측정 결과를 비트문자열 또는 카운트로 반환합니다.
...
Sampler를 사용하여 하나의 작업에서 여러 Circuit의 측정 결과를 비트문자열 또는 카운트로 반환합니다.
...
### 매개변수화된 Circuit 실행 {#run-parameterized-circuits}

매개변수 값을 활용하여 Circuit 재사용성을 높이면서 단일 작업에서 여러 실험을 실행합니다.
...
Session 및 고급 옵션을 탐색하여 QPU에서 Circuit 성능을 최적화하세요.

<Admonition type="caution">
다음 코드 블록은 Session을 사용하기 때문에 Open Plan 사용자에게는 오류를 반환합니다. Open Plan의 워크로드는 [작업 모드](/guides/execution-modes#job-mode) 또는 [배치 모드](/guides/execution-modes#batch-mode)에서만 실행할 수 있습니다.
</Admonition>
...
<Admonition type="tip" title="권장 사항">
    - [고급 런타임 옵션 지정하기](runtime-options-overview)
    ...
    - IBM&reg; QPU에 작업을 보낼 때의 [작업 제한](/guides/job-limits)을 이해하세요.
</Admonition>
