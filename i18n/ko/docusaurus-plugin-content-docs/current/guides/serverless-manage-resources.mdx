...
<Admonition type="tip">
**Qiskit Serverless가 업그레이드 중이며 기능이 빠르게 변화하고 있습니다.** 이 개발 단계 동안, 릴리스 노트 및 최신 문서는 [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html) 페이지에서 확인하세요.
</Admonition>
...
노트북에서 코드 셀을 로컬로 읽고 있다면 `%%writefile` [매직 명령어](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile)가 표시됩니다. 이 매직 명령어가 포함된 셀을 실행하면 코드를 실행하는 대신 디스크에 저장됩니다.
</Admonition>
...
병렬화할 수 있는 클래식 작업의 경우, `@distribute_task` 데코레이터를 사용하여 작업 수행에 필요한 컴퓨팅 요구 사항을 정의하세요. [첫 번째 Qiskit Serverless 프로그램 작성](/guides/serverless-first-program) 주제에서 다음 코드로 `transpile_remote.py` 예시를 다시 확인하는 것으로 시작합니다.
...
다음은 파일을 업로드하는 방법을 보여줍니다. IBM Quantum 계정으로 Qiskit Serverless에 인증되어 있는지 확인하세요 (지침은 [Qiskit Serverless에 업로드](/guides/serverless-first-program#upload-to-qiskit-serverless)를 참조하세요).
...
- [기존 코드를 Qiskit Serverless로 포팅](/guides/serverless-port-code)하는 전체 예시를 살펴보세요.
