...
그런 다음 [IBM Quantum&reg; API 키](http://quantum.cloud.ibm.com/)를 사용하여 인증하고, 이미 [계정을 저장](/guides/functions-get-started#install-qiskit-functions-catalog-client)했다고 가정하면, 다음과 같이 Qiskit Function을 선택합니다:
...
라이선스 문의는 [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com)으로 이메일을 보내세요.

### 라이선스 함수 사용 예시 {#example-of-licensed-function-use}

라이선스가 있는 사용자는 라이선스 토큰을 발급받으며, 함수에 라이선스 토큰을 제출하기 위해 wrapper 라이브러리를 사용해야 합니다.
이 wrapper 라이브러리는 `pip install hivqe-qiskit-function-utils` 명령으로 [PyPI에서](https://pypi.org/project/hivqe-qiskit-function-utils/) 설치할 수 있습니다.
아래 예시는 함수를 호출할 때 라이선스 토큰을 제출하기 위해 이 라이브러리를 어떻게 사용해야 하는지 보여줍니다.
...
- 이 Qiskit Function의 [API 참조](https://docs.quantum.ibm.com/api/functions/qunova-chemistry)를 방문하세요.
