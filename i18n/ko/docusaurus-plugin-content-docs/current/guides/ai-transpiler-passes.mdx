...
AI 기반 Transpiler 패스는 일부 트랜스파일링 작업에서 "전통적인" Qiskit 패스를 대체하여 사용할 수 있는 패스입니다. 기존 휴리스틱 알고리즘(낮은 깊이 및 CNOT 수 등)보다 더 나은 결과를 자주 도출하며, 불리언 만족도 해석기(Boolean satisfiability solver)와 같은 최적화 알고리즘보다 훨씬 빠릅니다. AI Transpiler 패스는 로컬 환경에서 실행됩니다.
...
AI Transpiler 패스를 사용하려면 먼저 `qiskit-ibm-transpiler` 패키지를 설치하세요. 사용 가능한 다양한 옵션에 대한 자세한 내용은 [qiskit-ibm-transpiler API 문서](https://docs.quantum.ibm.com/api/qiskit-ibm-transpiler)를 참고하세요.

## 로컬 또는 클라우드에서 AI Transpiler 패스 실행 {#run-the-ai-transpiler-passes-locally-or-on-the-cloud}

아래와 같이 추가 의존성을 포함하여 `qiskit-ibm-transpiler`를 먼저 설치하세요:

```sh
pip install qiskit-ibm-transpiler[ai-local-mode]
```

추가 의존성을 설치하면 AI 기반 Transpiler 패스의 기본 실행 모드는 로컬 머신을 사용하는 방식으로 전환됩니다.
...
인용
연구에서 Qiskit Transpiler Service의 AI 기반 기능을 사용하는 경우 다음 권장 인용 방식을 사용하세요:
