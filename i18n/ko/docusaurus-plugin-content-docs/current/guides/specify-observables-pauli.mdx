...
여기서 인덱스 $\langle i, j\rangle$는 상호작용하는 스핀들을 나타내며, 스핀들은 $X$ 방향의 횡방향 자기장(transverse field)에 놓여 있습니다.
...
여기서 합산은 가능한 모든 $n$-Qubit Pauli 항에 대해 이루어지며, $\mathrm{Tr}(\cdot)$는 내적으로 작용하는 행렬의 대각합(trace)입니다. 인수 $1/2^n$은 $n$-Qubit Pauli 문자열의 Hilbert--Schmidt 정규화를 나타냅니다.
