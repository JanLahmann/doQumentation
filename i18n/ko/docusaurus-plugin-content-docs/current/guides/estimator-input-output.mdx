...
선택적 `parameter values`는 리스트나 단일 매개변수일 수 있습니다. observables와 parameter values의 요소는 [Primitive 입력 및 출력](primitive-input-output#broadcasting-rules) 주제에서 설명하는 NumPy 브로드캐스팅 규칙에 따라 결합되며, 브로드캐스트된 형태의 각 요소에 대해 하나의 기댓값 추정치가 반환됩니다.
...
다음 코드는 `Estimator` primitive에 대한 벡터화된 입력 예제 세트를 보여주며, 이를 단일 `RuntimeJobV2 ` 객체로 IBM&reg; backend에서 실행합니다.
