...
{/* doqumentation-source-hash: 2b726936 */}
...
2-Qubit 유니터리 합성(모든 레벨의 초기 단계 및 최적화 레벨 3의 최적화 단계에서 사용됨)에서 이 값은 출력 분해의 목표 충실도를 지정합니다. 즉, Circuit의 행렬 표현이 이산 Gate로 변환될 때 도입되는 오류의 양입니다. 근사 정도가 낮을수록(더 많은 근사) 합성으로 생성된 출력 Circuit은 입력 행렬과 더 많이 달라지지만, 게이트 수는 더 적어질 가능성이 높고(임의의 2-Qubit 연산은 최대 세 개의 CX Gate로 완벽하게 분해될 수 있기 때문입니다), 그만큼 실행하기도 쉬워집니다.
...
트랜스파일 전에 Circuit에 포함된 Qubit은 대상 Backend의 물리적 Qubit에 반드시 대응되지 않는 가상 Qubit입니다. `initial_layout` 인수를 사용하여 가상 Qubit을 물리적 Qubit에 초기 매핑하는 방법을 지정할 수 있습니다. 최종 qubit 레이아웃은 Transpiler가 스왑 Gate나 다른 방법으로 Qubit을 재배치할 수 있기 때문에 초기 레이아웃과 다를 수 있습니다.
...
이 예시에서는 `qiskit_ibm_runtime`의 `FakeSherbrooke` 모의 Backend를 사용하지만, Qiskit과 호환되는 실제 또는 모의 Backend 어디서나 사용해 볼 수 있습니다. 결과는 다를 수 있습니다.
</Admonition>
...
* `scheduling_method` (str) - 스케줄링 패스 이름. 스케줄링 단계에 사용할 외부 플러그인 이름을 지정할 수도 있습니다.
  * `as_soon_as_possible`: 명령어를 탐욕적으로 스케줄링합니다. 즉, qubit 리소스에서 가능한 한 이른 시점에 배치합니다 (별칭: `asap`).
