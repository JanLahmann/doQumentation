---
description: 'Qiskit SDK 프리미티브의 입출력 형식(Primitive Unified Bloc, PUB 포함)을 이해합니다'
...
하나 이상의 PUB가 실행을 위해 QPU로 전송되고 작업이 성공적으로 완료되면, 데이터는 [`PrimitiveResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PrimitiveResult) 컨테이너 객체로 반환됩니다. `PrimitiveResult`는 각 PUB의 실행 결과를 포함하는 [`PubResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PubResult) 객체의 반복 가능한 리스트를 포함합니다. 예를 들어, 20개의 PUB로 제출된 작업은 20개의 `PubResults`로 구성된 리스트를 포함하는 `PrimitiveResult` 객체를 반환하며, 각 PubResult는 하나의 PUB에 대응합니다.
...
<Admonition type="tip" title="권장 사항">
  ...
  -  [Qiskit SDK 프리미티브](https://docs.quantum.ibm.com/api/qiskit/primitives) API를 검토하세요.

  -  [Qiskit Aer 프리미티브](https://qiskit.github.io/qiskit-aer/apidocs/aer_primitives.html) API를 검토하세요.

  -  [IBM Quantum 프리미티브](/guides/qiskit-runtime-primitives)에 대해 자세히 알아보세요.

  -  [`qiskit-ibm-runtime` Estimator](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2) API를 검토하세요.

  -  [`qiskit-ibm-runtime` Sampler](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2) API를 검토하세요.
</Admonition>
