...
**시드 최적화**: `seed_transpiler`는 기본적으로 `None`으로 설정되어 있어요. 이를 통해 트랜스파일러의 자동 최적화 프로세스가 활성화돼요. `None`으로 설정하면 시스템이 여러 시드로 시도를 시작하고, `max_trials` 매개변수의 전체 기능을 각 트랜스파일 레벨마다 활용하여 최상의 회로 깊이를 생성하는 시드를 선택해요.

**트랜스파일 레벨 성능**: `transpilation_level` 값을 높이면서 `max_trials` 수를 늘리면 트랜스파일 시간이 불가피하게 증가하지만, 최종 회로가 항상 변경되지는 않아요. 이는 특정 회로 구조와 복잡도에 크게 의존해요. 그러나 일부 회로/문제의 경우 10번 시도(레벨 1)와 50번 시도(레벨 5)의 차이가 극적일 수 있으므로, 이러한 매개변수를 탐색하는 것이 솔루션을 성공적으로 찾는 핵심이 될 수 있어요.
...
다음 예제는 150개 노드의 Maxcut 인스턴스를 실행합니다.
...
- 이 Qiskit Function에 대한 [API 레퍼런스](https://docs.quantum.ibm.com/api/functions/kipu-optimization)를 방문하세요.

- [Kipu Quantum의 Iskay Quantum Optimizer로 Market Split 문제 해결하기](/tutorials/solve-market-split-problem-with-iskay-quantum-optimizer) 튜토리얼을 시도해보세요.
