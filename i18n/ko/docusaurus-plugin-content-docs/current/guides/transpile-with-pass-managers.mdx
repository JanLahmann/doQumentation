...
이 예제에서는 `qiskit_ibm_runtime`의 `FakeSherbrooke` 모의 백엔드를 사용하지만, Qiskit과 호환되는 실제 또는 모의 백엔드에서도 시도해 볼 수 있습니다. 결과는 다를 수 있습니다.
</Admonition>
...
`generate_preset_pass_manager` 함수에 전달할 수 있는 인수에 대한 설명은 [트랜스파일 기본값 및 구성 옵션](defaults-and-configuration-options)을 참조하세요. `generate_preset_pass_manager`의 인수는 [`transpile`](https://docs.quantum.ibm.com/api/qiskit/compiler#qiskit.compiler.transpile) 함수의 인수와 동일합니다.

사전 설정된 pass manager가 필요에 맞지 않는 경우, (staged) pass manager 또는 트랜스파일 pass를 직접 생성하여 트랜스파일을 사용자 정의할 수 있습니다. 이 페이지의 나머지 부분에서는 pass manager를 생성하는 방법을 설명합니다. 트랜스파일 pass를 생성하는 방법은 [나만의 Transpiler pass 작성하기](custom-transpiler-pass)를 참조하세요.
...
dynamical decoupling이라고 알려진 오류 억제 기법을 구현하기 위한 pass manager를 생성하는 방법을 보여주는 고급 예시는 [dynamical decoupling을 위한 pass manager 생성하기](dynamical-decoupling-pass-manager)를 참조하세요.
...
- [dynamical decoupling을 위한 pass manager 생성하기](dynamical-decoupling-pass-manager).
