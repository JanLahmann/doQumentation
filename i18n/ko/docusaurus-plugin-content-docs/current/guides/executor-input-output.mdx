...
첫 번째 항목은 프로그램에서 첫 번째 작업(`CircuitItem`)을 실행한 결과를 포함합니다. 여기에는 입력 Circuit의 클래식 레지스터 이름인 `meas`라는 단일 키가 포함됩니다. 이 키의 값은 `(parameter sets, shots, register bits)` 형태의 `np.ndarray`에 매핑되며, 위 예제에서는 (10, 1024, 3)입니다.
...
두 번째 항목은 프로그램에서 두 번째 작업(`SamplexItem`)을 실행한 결과를 포함합니다. 이 항목에는 여러 키가 포함됩니다. 입력 Circuit의 클래식 레지스터 이름인 `meas` 키는 해당 레지스터의 결과 배열에 매핑됩니다. 이 배열은 `(randomizations, parameter sets, shots, classical bits)` 형태를 가지며, 이 예제에서는 (28, 10, 1024, 3)입니다. 또한, 출력에는 `meas` 레지스터에 대한 측정 트월링을 원상복구하기 위한 비트 플립 보정인 `measurement_flips.meas` 키가 포함됩니다. 비트 플립을 수행하는 데 단 하나의 샷만 필요하기 때문에 이 예제에서 이 출력 형태는 (28, 10, 1, 3)이 됩니다.
