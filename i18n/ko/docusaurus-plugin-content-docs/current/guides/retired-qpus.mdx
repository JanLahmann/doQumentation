---
title: "퇴역한 클라우드 QPU"
sidebar_label: "퇴역한 클라우드 QPU"
description: "현재 퇴역한 IBM 양자 처리 장치(QPU) 목록"
notebook_path: "guides/retired-qpus.ipynb"
---

{/* doqumentation-source-hash: 70a7a5e5 */}

다음 IBM&reg; 양자 처리 장치(QPU)는 퇴역했습니다. 사용 가능한 QPU 전체 목록은 [컴퓨팅 리소스](https://quantum.cloud.ibm.com/computers) 페이지를 참조하세요.

<Admonition type="note">
    퇴역한 QPU에서 작업을 가져오려면 [이 안내를 참조하세요.](#retrieve)
</Admonition>

## 퇴역한 클라우드 QPU 표 {#table-of-retired-cloud-qpus}

| QPU 이름          | 큐비트 수    | 퇴역 날짜 (연도 - 월 - 일)            |
| ----------------- | ----------- | --------------- |
...
## 퇴역한 프로세서 패밀리 {#retired-processor-families}

- **Canary**: Canary 패밀리는 5개에서 16개의 큐비트를 포함하는 소형 설계로 구성되어 있습니다.
- **Falcon**: Falcon 패밀리 장치는 중규모 회로를 위한 유용한 플랫폼을 제공했으며, 더 큰 장치에 적용하기 전에 성능 및 확장성 개선을 시연하는 플랫폼으로도 활용되었습니다.
- **Egret**: Egret은 33큐비트 플랫폼에 조정 가능한 커플러(tunable coupler)의 혁신을 도입하여, 더 빠르고 높은 충실도의 2큐비트 게이트를 구현했습니다.
- **Hummingbird**: 헤비-헥사고날(heavy-hexagonal) 큐비트 레이아웃을 사용하여, Hummingbird 패밀리는 최대 65개의 큐비트를 지원했습니다.
- **Osprey**: Osprey는 433큐비트로 Eagle의 거의 4배 크기였습니다. 더 큰 칩 크기에는 장치 패키징의 추가적인 개선이 필요했으며, 동일한 배선 공간 내에서 더 많은 I/O 요구 사항을 수용하기 위해 극저온 장치(cryostat)에 맞춤형 플렉스 케이블이 사용되었습니다.
- **Eagle**: Eagle 프로세서 패밀리는 이전 세대보다 더 확장 가능한 패키징 기술을 도입했습니다. 특히 신호가 여러 칩 레이어를 통과함으로써, 성능을 저하시키지 않고 고밀도 I/O가 가능해졌습니다.

<span id="retrieve"></span>
## 퇴역한 QPU에서 작업 가져오기 {#retrieve-a-job-from-a-retired-qpu}

다음 코드를 사용하여 퇴역한 QPU에서 작업을 가져올 수 있습니다.

<Admonition type="note">
다음 코드에서 Cloud Resource Name(CRN)을 전달하는 것은 선택 사항입니다. CRN을 전달하면 해당 인스턴스의 백엔드와 작업만 사용할 수 있습니다. 인스턴스를 지정하지 않으면 계정 내 모든 인스턴스의 백엔드와 작업을 모두 사용할 수 있습니다. 이 경우 백엔드를 지정하면, 해당 백엔드에 접근할 수 있는 인스턴스가 자동으로 검색되어 사용됩니다.
</Admonition>
