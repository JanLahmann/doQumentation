...
이 템플릿은 사용자가 정의한 스핀 기반 해밀토니안에 대해 초기 상태의 시간 진화를 시뮬레이션하는 워크플로를 캡슐화하며, [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/) Qiskit 애드온을 사용하여 지정된 기댓값 집합을 반환합니다.

이 템플릿은 다음 단계를 포함하는 Qiskit 패턴 구조로 이루어져 있습니다.

#### 1. 입력 수집 및 문제 매핑 {#1-collecting-input-and-mapping-the-problem}
...
#### 2. 생성된 Circuit을 실행에 맞게 준비 {#2-prepare-the-generated-circuits-for-execution}
...
먼저, [AQC-Tensor Qiskit addon](https://qiskit.github.io/qiskit-addon-aqc-tensor/)을 사용하여 문제 설명을 하드웨어 실행에 적합한 깊이가 줄어든 Circuit으로 변환하는 해밀토니안 시뮬레이션용 Function 템플릿을 작성합니다.

이 페이지를 다운로드하여 노트북 편집기에서 로컬로 실행하면, 일부 코드 셀에 [매직 명령어](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`이 포함된 것을 확인할 수 있습니다. 이 매직 명령어는 코드를 `./source_files/template_hamiltonian_simulation.py`에 저장하며, 이 파일이 Qiskit Serverless에 업로드하고 원격으로 실행할 수 있는 Function 템플릿입니다.
...
<Accordion>
<AccordionItem title="**전체 프로그램 소스 코드**">
