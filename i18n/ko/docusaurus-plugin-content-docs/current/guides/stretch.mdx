...
![다음 Circuit을 설명하는 이미지](/docs/images/guides/stretch/stretch.svg)
...
:::note
 ...
 IBM Quantum Compute Service에서 `stretch`를 사용할 때, stretch 해석에서 발생하는 나머지 값은 해당 stretch를 사용하는 첫 번째 delay에 추가됩니다.
...
## IBM Quantum Compute Service에서 stretch 값 확인하기 {#view-stretch-values-in-ibm-quantum-compute-service}

stretch 지속 시간의 실제 값은 Circuit이 스케줄된 이후 컴파일 시점에 해석됩니다. IBM Quantum Compute Service에서 Sampler 작업을 실행할 때, 작업 결과 메타데이터에서 해석된 stretch 값을 확인할 수 있습니다. Quantum Compute에서 `stretch` 지원은 현재 실험적이므로, 먼저 실험적 옵션을 설정하여 검색을 활성화한 다음 아래와 같이 메타데이터에서 직접 데이터에 접근해야 합니다.
...
"compilation" 메타데이터에 총 Circuit 시간이 반환되지만, 이는 과금(QPU 시간)에 사용되는 시간이 아닙니다.
</Admonition>
...
자세한 내용은 [Circuit 타이밍 시각화](/guides/qiskit-runtime-circuit-timing) 항목을 참조하세요.

<span id="qiskit-runtime-limitations"></span>

## Quantum Compute 제한 사항 {#quantum-compute-limitations}

Quantum Compute에서 `stretch` 지원은 현재 실험적이며 다음과 같은 제약 사항이 있습니다.
...
- barrier 집합으로 둘러싸인 영역을 _barrier 영역_이라고 합니다. stretch 변수는 여러 barrier 영역에서 사용될 수 없습니다.
    <Tabs>
        ...
        ![이전 코드 출력의 일러스트레이션](/docs/images/guides/stretch/bad-barrier-region.svg 'barrier 영역에서 stretch를 잘못 사용한 예')
     ...
     <TabItem value="Valid" label="올바른 예">
        ...
        ![이전 코드 출력의 일러스트레이션](/docs/images/guides/stretch/good-barrier-region.svg 'barrier 영역에서 stretch를 올바르게 사용한 예')
