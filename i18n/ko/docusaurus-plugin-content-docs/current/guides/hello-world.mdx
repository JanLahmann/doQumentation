...
sidebar_label: '하드웨어에서 첫 번째 Circuit 실행하기'
title: '하드웨어에서 첫 번째 Circuit 실행하기'
---
...
이 예제는 두 부분으로 구성되어 있습니다. 먼저 간단한 "Hello world" 양자 프로그램을 만들고 QPU(양자 처리 장치)에서 실행합니다. 실제 양자 연구에는 훨씬 더 견고한 프로그램이 필요하므로, 두 번째 섹션([대규모 Qubit로 확장하기](#scale-to-large-numbers-of-qubits))에서는 이 간단한 프로그램을 유틸리티 수준으로 확장합니다.
...
1. Qiskit을 아직 설치하지 않았다면 [빠른 시작](/guides/quick-start) 가이드에서 설치 방법을 확인하세요.
    ...
    - 양자 하드웨어에서 작업을 실행하려면 IBM Quantum Compute 클라이언트를 설치하세요:
...
2. 무료 [Open Plan](/guides/plans-overview#open-plan)을 통해 양자 하드웨어에 접근하기 위한 인증을 설정하세요.
    ...
    - [IBM Quantum Platform](https://quantum.cloud.ibm.com/)에서 로그인하거나 계정을 만드세요.
         ...
         프록시 서버를 통해 연결하는 경우 `qiskit-ibm-runtime` v0.44.0 이상을 사용해야 합니다.
         </Admonition>
...
3. 이제 IBM Quantum Compute Service에 인증할 때마다 이 Python 코드를 사용할 수 있습니다:
...
양자 Circuit을 만들 때 실행 후 반환받을 데이터 유형도 고려해야 합니다. Qiskit은 데이터를 반환하는 두 가지 방법을 제공합니다: 측정하려는 Qubit 집합에 대한 샘플링 출력을 얻거나, 관측 가능량의 기댓값을 얻을 수 있습니다. 프리미티브([3단계](#step-3-execute-using-the-quantum-primitives)에서 자세히 설명)를 사용하여 이 두 가지 방법 중 하나로 Circuit을 측정하도록 워크로드를 준비하세요.
...
양자 컴퓨터는 무작위 결과를 생성할 수 있으므로, 보통 Circuit를 여러 번 실행하여 출력 샘플을 수집합니다. `Estimator` 클래스를 사용하여 관측 가능량의 값을 추정할 수 있습니다. `Estimator`는 두 가지 [프리미티브](/guides/get-started-with-estimator) 중 하나이며, 다른 하나는 양자 컴퓨터에서 데이터를 얻는 데 사용할 수 있는 `Sampler`입니다. 이 객체들은 [PUB(primitive unified bloc)](/guides/get-started-with-sampler)을 사용하여 선택한 Circuit, 관측 가능량, 매개변수(해당하는 경우)를 실행하는 `run()` 메서드를 가지고 있습니다.
...
:::note[대안: 시뮬레이터를 사용하여 예제 실행하기]


  실제 디바이스에서 양자 프로그램을 실행하면 실행 전에 대기열에서 기다려야 합니다. 시간을 절약하기 위해 `qiskit-ibm-runtime` 로컬 테스트 모드에서 [`fake_provider`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/fake-provider)를 사용하여 이 소규모 워크로드를 실행하는 다음 코드를 대신 사용할 수 있습니다. 이 방법은 소규모 Circuit에서만 가능하다는 점에 유의하세요. 다음 섹션에서 규모를 확장할 때는 실제 디바이스를 사용해야 합니다.
...
Estimator에 지정한 관측량에 대한 기댓값과 표준 편차는 작업 결과의 `PubResult.data.evs` 및 `PubResult.data.stds` 속성을 통해 확인할 수 있습니다. Sampler에서 결과를 얻으려면 `PubResult.data.meas.get_counts()` 함수를 사용하세요. 이 함수는 비트 문자열을 키로, 카운트를 값으로 하는 `dict` 형태의 측정 결과를 반환합니다. 자세한 내용은 [Sampler 빠른 시작](/guides/get-started-with-sampler) 가이드를 참조하세요.
...
### 1단계. 문제 매핑 {#step-1-map-the-problem}
...
### 2단계. 양자 하드웨어 실행을 위한 최적화 {#step-2-optimize-the-problem-for-execution-on-quantum-hardware}
...
### 3단계. 하드웨어에서 실행 {#step-3-execute-on-hardware}

작업을 제출하고, [동적 디커플링(dynamical decoupling)](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-dynamical-decoupling-options)이라는 오류 억제 기법을 사용하여 오류 억제를 활성화합니다. 복원력 수준(resilience level)은 오류에 대한 복원력의 정도를 지정합니다. 수준이 높을수록 더 정확한 결과를 얻을 수 있지만, 처리 시간이 더 길어집니다. 다음 코드에서 설정된 옵션에 대한 자세한 설명은 [오류 완화 및 억제 기법](/guides/error-mitigation-and-suppression-techniques)을 참조하세요.
