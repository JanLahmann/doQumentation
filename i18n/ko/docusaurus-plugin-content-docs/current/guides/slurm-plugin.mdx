...
기본적으로 `CMakeLists.txt` 파일은 Slurm 헤더 파일(`slurm.h`)이 `/usr/include/slurm`에 있다고 가정하지만, 다음과 같이 사용자 지정할 수 있습니다.
...
```
예를 들어 HTTP 프록시를 통해 양자 리소스와 상호작용할 때는 `http_proxy`, `https_proxy`, `no_proxy` 환경 변수가 필요합니다. 다음과 같이 추가할 수 있습니다.
```bash
optional /usr/lib64/slurm/spank_qrmi.so /etc/slurm/qrmi_config.json \
