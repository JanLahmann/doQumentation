...
$\hat{a}^\dagger_{p\sigma}$/$\hat{a}_{p\sigma}$는 $p$번째 기저 집합 원소와 스핀 $\sigma$에 연관된 페르미온 생성/소멸 연산자입니다. $h_{pr}$과 $(pr|qs)$는 1체 및 2체 전자 적분(각각 1전자/2전자 적분)입니다.
...
- 위에서 설명한 지그재그 패턴을 따르도록 목표 하드웨어의 물리적 qubit(`initial_layout`)을 선택합니다. 이 패턴으로 Qubit을 배치하면 게이트 수가 적은 효율적인 하드웨어 호환 Circuit을 얻을 수 있습니다.
...
SQD 애드온에 포함된 솔버는 selected CI(선택 CI)에 대한 PySCF의 구현, 구체적으로 [pyscf.fci.selected_ci.kernel_fixed_space](https://pyscf.org/pyscf_api_docs/pyscf.fci.html#pyscf.fci.selected_ci.kernel_fixed_space)를 사용합니다. 아래 예제는 포함된 솔버를 통해 해당 함수에 키워드 인수를 전달하는 방법도 보여줍니다. 여기서는 `max_cycle` 인수를 전달합니다.
