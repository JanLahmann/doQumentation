...
***2-큐비트 게이트에 영향을 미치는 노이즈 채널의 역(inverse)을 통해 Observable을 전파함으로써 게이트 오류를 완화하는 기법으로, 노이즈 완화 Observable을 생성합니다.***
...
여기서 $J>0$은 최근접 이웃 스핀($i<j$)의 결합을 나타내고, 전역 횡방향 자기장 $h$는 $\frac{\pi}{8}$로 설정됩니다. $h$가 Clifford 각도(즉, $\theta=n\frac{\pi}{2}, n \in \mathbb{Z}$)에서 멀어질수록, Circuit을 통해 안티-노이즈 생성자를 전파하기가 더 어려워집니다.
...
여기서는 ``Executor``에서 출력된 샘플에 대해 사후 선택을 수행하는 데 필요한 스펙테이터(spectator) 측정과 사후 선택 측정도 추가합니다.
...
#### Circuit 박스를 학습된 노이즈와 연결 {#associate-circuit-boxes-with-learned-noise}
...
#### 학습된 안티-노이즈를 통해 Observable을 전파하여 노이즈 완화 Observable 얻기 {#propagate-the-observable-through-the-learned-anti-noise-to-get-a-noise-mitigating-observable}
...
#### 측정 기저를 표준 형태로 변환 {#transform-the-measurement-bases-to-canonical-form}
...
#### ``QuantumProgram``에서 샘플링 방법 지정 {#specify-how-to-sample-in-the-quantumprogram}
...
#### ``Executor`` 프리미티브 프로토타입을 사용하여 Circuit 샘플링 {#sample-the-circuit-using-the-executor-primitive-prototype}
...
#### 샘플을 후처리하여 오류 완화된 기댓값 계산 {#post-process-the-samples-to-calculate-an-error-mitigated-expectation-value}
