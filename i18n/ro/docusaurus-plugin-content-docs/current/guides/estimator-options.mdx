...
Note :
<Admonition type="note" title="Note despre specificarea opțiunilor în primitivele Estimator">
...
### Inițializarea primitivului {#primitive-initialization}
...
Clasa de opțiuni `EstimatorV2` ([`EstimatorOptions`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-estimator-options)) nu trebuie instanțiată dacă setezi opțiunile după inițializarea primitivului.
...
Acești parametri de precizie sunt doar pentru specificarea preciziei _țintă_, iar rezultatele nu sunt garantate să atingă precizia specificată.
</Admonition>
...
3. Dacă `twirling` este activat (True implicit), se folosește produsul dintre `num_randomizations` și `shots_per_randomization`, specificat ca [opțiunile `twirling`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-twirling-options).
...
<Accordion>
  <AccordionItem title="Porți fracționale">
    Incompatibil cu:
  ...
  <AccordionItem title="Twirling de porți">
    Incompatibil cu:
  - Porți fracționale
  - Stretch-uri
