...
![Imagine care ilustrează circuitul următor](/docs/images/guides/stretch/stretch.svg)
...
:::note
 ...
 Când folosești `stretch` cu IBM Quantum Compute Service, orice rest rezultat din rezolvarea unui stretch este adăugat la prima întârziere care folosește stretch-ul.
...
## Vizualizează valorile stretch în IBM Quantum Compute Service {#view-stretch-values-in-ibm-quantum-compute-service}

Valoarea reală a unei durate stretch este rezolvată la momentul compilării, după ce circuitul este programat. Când rulezi un job Sampler în IBM Quantum Compute Service, poți vizualiza valorile stretch rezolvate în metadatele rezultatului jobului. Suportul pentru `stretch` în Quantum Compute este în prezent experimental, deci trebuie mai întâi să setezi o opțiune experimentală pentru a activa recuperarea acestuia, apoi să accesezi datele direct din metadate după cum urmează:
...
Deși timpul total al circuitului este returnat în metadatele „compilation", acesta NU este timpul folosit pentru facturare (timp QPU).
</Admonition>
...
Pentru detalii complete, consultă subiectul [Vizualizarea temporizării circuitului](/guides/qiskit-runtime-circuit-timing).

<span id="qiskit-runtime-limitations"></span>

## Limitările Quantum Compute {#quantum-compute-limitations}

Suportul pentru `stretch` în Quantum Compute este în prezent experimental și are următoarele constrângeri:
...
- Zona înconjurată de un set de bariere se numește _regiune de barieră_. O variabilă stretch nu poate fi folosită în mai multe regiuni de barieră.
    <Tabs>
        ...
        ![Ilustrarea rezultatului codului anterior](/docs/images/guides/stretch/bad-barrier-region.svg 'Invalid use of stretch in barrier regions')
     ...
     <TabItem value="Valid" label="Valid">
        ...
        ![Ilustrarea rezultatului codului anterior](/docs/images/guides/stretch/good-barrier-region.svg 'Valid use of stretch in a barrier region')
