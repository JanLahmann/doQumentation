...
# Începe {#get-started}
Autentifică-te folosind [cheia ta API](https://quantum.cloud.ibm.com/) și selectează Qiskit Function-ul după cum urmează. (Acest fragment presupune că ți-ai [salvat deja contul](/guides/functions-get-started#install-qiskit-functions-catalog-client) în mediul tău local.)
...
### 1. Citește activele incluse în portofoliu. {#1-read-the-assets-included-in-the-portfolio}
...
### 2. Definește problema. {#2-define-the-problem}
...
### 3. Definește setările Optimizer-ului și ale ansatz-ului (Opțional) {#3-define-the-optimizer-and-ansatz-settings-optional}
...
În cazul Real Amplitudes Ansatz, poți urma o relație liniară între ``population_size`` și numărul de qubiți din circuit. Ca regulă empirică aproximativă, se recomandă să folosești un **minim** ``population_size ~ 0.8 * n_qubits`` pentru ansatz-ul `real_amplitudes`.
...
### 4. Rulează problema. {#4-run-the-problem}
...
### 5. Obține rezultatele {#5-retrieve-results}
...
![Vizualizarea soluției optimizării](/docs/images/guides/global-data-quantum-optimizer/cost_distribution.avif)
