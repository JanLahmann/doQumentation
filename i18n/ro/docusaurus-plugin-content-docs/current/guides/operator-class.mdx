...
Această pagină arată cum să folosești clasa [`Operator`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Operator). Pentru o prezentare generală a reprezentărilor de operatori în Qiskit, inclusiv clasa `Operator` și altele, consultă [Prezentare generală a claselor de operatori](/guides/operators-overview).
...
`Operators` unitare pot fi inserate direct într-un `QuantumCircuit` folosind metoda `QuantumCircuit.append`. Aceasta convertește `Operator` într-un obiect `UnitaryGate`, care este adăugat la circuit.
...
Reține că în exemplul de mai sus operatorul este inițializat dintr-un obiect `Pauli`. Totuși, obiectul `Pauli` poate fi inserat direct în Circuit și va fi convertit într-o secvență de poartă-uri Pauli cu un singur qubit:
...
## Combine Operators {#combine-operators}

Operatorii pot fi combinați folosind mai multe metode.

### Produs tensorial {#tensor-product}
...
### Expansiune tensorială {#tensor-expansion}
...
### Compoziție pe subsisteme {#subsystem-composition}
...
De exemplu, pentru a compune o poartă pe doi qubiți cu un operator pe trei qubiți:
...
### Combinații liniare {#linear-combinations}
...
### Conversie implicită la Operators {#implicit-conversion-to-operators}
...
## Compară Operators {#compare-operators}
...
### Fidelitate de proces {#process-fidelity}
