...
4. Dacă `twirling` este activat, se folosește produsul dintre `num_randomizations` și `shots_per_randomization`, specificat ca [opțiuni `twirling`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-twirling-options).
...
<Accordion>
  <AccordionItem title="Circuite dinamice">
    Incompatibil cu:
  - Decuplarea dinamică
  ...
  <AccordionItem title="Decuplare dinamică">
    Incompatibil cu:
  - Circuite dinamice
  </AccordionItem>
  ...
  - Twirling-ul de măsurători este incompatibil cu instrucțiunea [`store`](https://docs.quantum.ibm.com/api/qiskit/circuit#store).
