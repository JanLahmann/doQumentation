...
Rezultatul `NoiseLearnerResult.data` este o listă de obiecte [`LayerError`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/results-layer-error) care conțin [modelul de zgomot](https://arxiv.org/abs/2201.09866) pentru fiecare strat de entanglare individual ce aparține Circuit-elor țintă. Fiecare `LayerError` stochează informațiile despre strat, sub forma unui Circuit și a unui set de etichete de qubit, alături de [`PauliLindbladError`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/results-pauli-lindblad-error) pentru modelul de zgomot care a fost învățat pentru stratul respectiv.
...
Modelul de zgomot învățat pe Circuit poate fi folosit și ca intrare pentru primitiva IBM Quantum `EstimatorV2`. Acesta poate fi transmis primitivei în câteva moduri diferite. Următoarele trei exemple arată cum poți transmite modelul de zgomot atributului `estimator.options` direct, folosind un obiect `ResilienceOptionsV2` înainte de a instanția o primitivă Estimator, și prin transmiterea unui dicționar formatat corespunzător.
...
### Prezentare generală {#overview-1}
...
### Opțiuni pentru învățarea zgomotului {#noise-learning-options-1}
...
Circuitul din exemplul anterior trece prin pass manager-ul de boxing, care grupează instrucțiunile în cutii adnotate. Codul relevant este adăugat aici pentru ușurința înțelegerii.
- `inject_noise_targets=”gates”` specifică adăugarea adnotărilor `InjectNoise` la cutiile care conțin entangleri.
- `inject_noise_strategy="uniform_modification"` specifică atribuirea acelorași `ref` și `modifier_ref` tuturor cutiilor echivalente cu adnotări `InjectNoise`.
...
- Află mai multe despre [tehnicile disponibile de atenuare și suprimare a erorilor](error-mitigation-and-suppression-techniques).
