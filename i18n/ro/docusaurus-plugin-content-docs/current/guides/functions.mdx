...
3. <span id="functions-untrusted"></span>**Evită să execuți cod pe o mașină care nu este de încredere sau într-un mediu Python cloud extern pentru a minimiza riscurile de securitate.** Dacă trebuie să folosești un mediu care nu este de încredere (de exemplu, pe un calculator public), schimbă cheia API după fiecare utilizare ștergând-o de pe pagina [IBM Cloud API keys](https://cloud.ibm.com/iam/apikeys) pentru a reduce riscul. Află mai multe în subiectul [Managing user API keys](https://cloud.ibm.com/docs/account?topic=account-userapikey&interface=ui). Pentru a inițializa serviciul în această situație, extinde secțiunea următoare pentru a vedea codul pe care îl poți folosi:
    ...
    <Accordion>
        <AccordionItem title="Inițializează serviciul într-un mediu care nu este de încredere">
...
- [Explorează Circuit functions](/guides/ibm-circuit-function) pentru a construi algoritmi și aplicații noi, fără a fi nevoie să gestionezi transpilarea sau tratarea erorilor.
- [Explorează application functions](/guides/q-ctrl-optimization-solver) pentru a rezolva sarcini specifice unui domeniu, cu intrări și ieșiri clasice.
- Consultă [documentația de referință API](https://docs.quantum.ibm.com/api/functions/index) pentru Qiskit Functions.
- Pentru experiență practică, încearcă câteva [tutoriale](/tutorials/index#leverage-qiskit-capabilities) care demonstrează Qiskit Functions.
</Admonition>
