...
Poți folosi clasa `Neat` pentru a analiza impactul zgomotului asupra unui workload Estimator. Pentru verificarea sintaxei, folosește [modul de testare locală](/guides/local-testing-mode).
## Utilizarea clasei `Neat` {#neat-class-usage}
...
### Pregătirea mediului {#prepare-the-environment}
...
### Inițializarea unui Circuit țintă {#initialize-a-target-circuit}

Consideră un circuit cu șase qubit-uri care are următoarele proprietăți:

* Alternează între rotații `RZ` aleatorii și straturi de poartă-uri `CNOT`.
...
### Cliffordizarea Circuit-urilor {#cliffordize-the-circuits}
...
## Application 1: Analizează impactul zgomotului asupra rezultatelor Circuit-ului {#application-1-analyze-the-impact-of-noise-on-the-circuit-outputs}
...
## Application 2: Benchmark different strategies {#application-1-analyze-the-impact-of-noise-on-the-circuit-outputs}
...
- Citește o prezentare generală a [instrumentelor de depanare Qiskit](/guides/debugging-tools).
