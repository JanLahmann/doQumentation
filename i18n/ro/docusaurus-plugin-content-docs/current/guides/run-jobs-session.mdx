---
description: 'Cum să rulezi un job de calcul cuantic într-o sesiune IBM Quantum Compute Service.'
...
Înainte de a porni o sesiune, trebuie să [configurezi clientul IBM Quantum Compute](/guides/install-qiskit-runtime) și să-l inițializezi ca serviciu:
...
## Verifică starea Session-ului {#check-session-status}
...
## Tipare de utilizare {#determine-session-details}

Pentru o prezentare completă a configurației și stării unei sesiuni, folosește metoda `session.details() method`.
...
## Modele de utilizare {#usage-patterns}

Sesiunile sunt deosebit de utile pentru algoritmii care necesită comunicare frecventă între resursele clasice și cele cuantice.
...
## Rulează doi algoritmi VQE într-un Session folosind threading {#run-two-vqe-algorithms-in-a-session-by-using-threading}
