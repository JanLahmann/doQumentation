...
{/* doqumentation-source-hash: 09db66ad */}
...
- *AICliffordSynthesis*: Sinteză pentru Circuit-uri [Clifford](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Clifford) (blocuri de poartă-uri `H`, `S` și `CX`). În prezent până la blocuri de nouă Qubiți.
- *AILinearFunctionSynthesis*: Sinteză pentru Circuit-uri de [Funcție Liniară](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.LinearFunction) (blocuri de poartă-uri `CX` și `SWAP`). În prezent până la blocuri de nouă Qubiți.
- *AIPermutationSynthesis*: Sinteză pentru Circuit-uri de [Permutare](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.Permutation#permutation) (blocuri de poartă-uri `SWAP`). Disponibil în prezent pentru blocuri de 65, 33 și 27 de Qubiți.
- *AIPauliNetworkSynthesis*: Sinteză pentru Circuit-uri de rețea Pauli (blocuri de poartă-uri `H`, `S`, `SX`, `CX`, `RX`, `RY` și `RZ`). În prezent până la blocuri de șase Qubiți.
...
Consultă [documentația Qiskit Transpiler Service](/guides/qiskit-transpiler-service#limits-of-the-qiskit-transpiler-service) pentru mai multe informații despre limitele care se aplică trecerilor de Transpiler alimentate de AI.

## Citare {#citation}

Dacă folosești orice funcționalitate alimentată de AI din Qiskit Transpiler Service în cercetarea ta, folosește [citarea recomandată](/guides/qiskit-transpiler-service#citation).
