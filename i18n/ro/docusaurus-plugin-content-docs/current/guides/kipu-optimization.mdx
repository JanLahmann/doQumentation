...
{/* doqumentation-source-hash: 7e6ffdcf */}
...
Algoritmul se bazează pe evoluția temporală a unui sistem cuantic care se transformă în timp, unde soluția problemei este codificată în starea fundamentală a sistemului cuantic la sfârșitul evoluției. Conform [teoremei adiabatice](https://en.wikipedia.org/wiki/Adiabatic_theorem), această evoluție trebuie să fie lentă pentru a garanta că sistemul rămâne în starea sa fundamentală. Digitizarea acestei evoluții stă la baza calculului cuantic adiabatic digitizat (DQA) și a binecunoscutului algoritm QAOA. Cu toate acestea, evoluția lentă necesară nu este fezabilă pentru dimensiuni crescânde ale problemei, deoarece duce la o adâncime crescândă a circuitului. Prin utilizarea protocoalelor contradiabatice, poți suprima excitațiile nedorite care apar în timpul evoluțiilor scurte, rămânând totodată în starea fundamentală. Digitizarea acestui timp de evoluție mai scurt duce la circuite cuantice cu adâncime mai mică și mai puține porți de entanglare.

Circuitele algoritmilor bf-DCQO utilizează în mod tipic de până la zece ori mai puține porți de entanglare decât DQA și de trei până la patru ori mai puține porți de entanglare decât implementările standard QAOA. Datorită numărului mai mic de porți, apar mai puține erori în timpul execuției circuitului pe hardware. Prin urmare, Optimizer-ul nu necesită utilizarea tehnicilor de suprimare sau de atenuare a erorilor. Implementarea lor în versiunile viitoare poate îmbunătăți și mai mult calitatea soluției.
...
$$
unde $C$ este funcția obiectiv, $C_{\textrm{min}}$, $C_{\textrm{max}}$ sunt valorile sale minime și maxime, iar $C^{*}$ este costul celei mai bune soluții găsite. Prin urmare, AR=100\% înseamnă că s-a obținut starea fundamentală a problemei.
...
The following code assumes that you have saved your credentials. If you have not, follow the instructions in [save your IBM Cloud account](/guides/functions-get-started#install-qiskit-functions-catalog-client) to authenticate with your API key.
</Admonition>
...
**Optimizarea seed-ului**: Reține că `seed_transpiler` este setat implicit la `None`. Acest lucru activează procesul automat de optimizare al transpilatorului. Când este `None`, sistemul pornește o serie de încercări cu mai multe seed-uri și îl selectează pe cel care produce cea mai bună adâncime a circuitului, folosind la maximum parametrul `max_trials` pentru fiecare nivel de transpilare.

**Performanța nivelului de transpilare**: Creșterea numărului de `max_trials` împreună cu valori mai mari pentru `transpilation_level` va mări inevitabil timpul de transpilare, dar s-ar putea să nu schimbe întotdeauna circuitul final — depinde puternic de structura și complexitatea circuitului. Totuși, pentru unele circuite/probleme, diferența dintre 10 încercări (nivelul 1) și 50 de încercări (nivelul 5) poate fi dramatică, așa că explorarea acestor parametri poate fi cheia găsirii unei soluții.

## Exemplul 1: Funcție de cost simplă {#example-1-simple-cost-function}
...
### 1. Creează funcția obiectiv {#1-create-the-objective-function-1}
...
### 2. Rulează Optimizer-ul {#2-run-the-optimizer-1}
...
### 3. Obține rezultatul {#3-retrieve-the-result-1}
...
Următorul exemplu rulează instanța Maxcut cu 150 de noduri.
...
- Încearcă tutorialul [Rezolvă problema Market Split cu Iskay Quantum Optimizer de la Kipu Quantum](/tutorials/solve-market-split-problem-with-iskay-quantum-optimizer).
