...
Opționalul `parameter values` poate fi o listă sau un singur parametru. Elementele din observabile și valorile parametrilor sunt combinate urmând regulile de broadcasting NumPy, așa cum sunt descrise în subiectul [Intrări și ieșiri ale primitivelor](primitive-input-output#broadcasting-rules), și se returnează o estimare a valorii așteptate pentru fiecare element al formei broadcast.
...
Codul următor demonstrează un exemplu de set de intrări vectorizate pentru primitiva `Estimator` și le execută pe un backend IBM&reg; ca un singur obiect `RuntimeJobV2 `.
