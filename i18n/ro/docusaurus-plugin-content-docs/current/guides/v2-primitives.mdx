---
title: Migrează la primitivele Qiskit Runtime V2
...
# Migrează la primitivele Qiskit Runtime V2 {#migrate-to-the-qiskit-runtime-v2-primitives}
...
## Modificări majore {#major-changes}
...
### Intrare și ieșire {#input-and-output}
...
#### Exemple Estimator (intrare și ieșire) {#estimator-examples-input-and-output}
...
#### Exemple Sampler (intrare și ieșire) {#sampler-examples-input-and-output}
...
### Atenuarea și suprimarea erorilor {#error-mitigation-and-suppression}
...
## Pași pentru migrarea la Estimator V2 {#steps-to-migrate-to-estimator-v2}
...
5. Actualizează metoda `run()` a Estimator pentru a transmite lista de PUB-uri. De exemplu, `run([(circuit1, observable1, parameter_set1)])`.
...
6. Rezultatele job-urilor Estimator V2 sunt grupate pe PUB-uri. Poți vedea valoarea așteptată și eroarea standard pentru fiecare PUB indexând la acesta. De exemplu:
  ...
  ```
## Exemple complete cu Estimator {#estimator-full-examples}

### Rulează un singur experiment {#run-a-single-experiment}
...
### Rulează mai multe experimente într-un singur job {#run-multiple-experiments-in-a-single-job}
...
### Rularea circuitelor parametrizate {#run-parameterized-circuits}
...
### Folosește sesiuni și opțiuni avansate {#use-sessions-and-advanced-options}
...
## Pași pentru migrarea la Sampler V2 {#steps-to-migrate-to-sampler-v2}
...
## Exemple complete cu Sampler {#sampler-full-examples}

### Rularea unui singur experiment {#run-a-single-experiment}
...
### Rulează circuite parametrizate {#run-parameterized-circuits}
...
- Află mai multe despre setarea opțiunilor în ghidurile [Opțiunile Sampler](/guides/sampler-options), [Opțiunile Estimator](/guides/estimator-options) și [Opțiunile Executor](/guides/executor-options).
- Află mai multe detalii despre [Intrările și ieșirile Primitive](/guides/primitive-input-output#pubs).
