...
Aici, demonstrăm cum să construim oracole Grover și să folosim `GroverOperator` din biblioteca de circuite Qiskit pentru a configura ușor o instanță de căutare Grover. Primitiva IBM Quantum `Sampler` permite execuția fără probleme a circuitelor Grover.
...
* Un ultim strat de poartă-uri Hadamard este aplicat, după care se efectuează măsurătorile.
...
Să presupunem că oracolul ($Z_f$, necunoscut nouă) marchează starea $|01\rangle$. Vom parcurge acțiunile fiecărui set de poartă-uri cuantice, inclusiv oracolul, și vom vedea ce distribuție de stări posibile apare la momentul măsurătorii.
La bun început, avem
$$
...
- Pasul 3: Execuție folosind primitivele IBM Quantum
...
$$
Putem folosi poarta cu mai mulți controali și mai multe ținte (`MCMTGate`) pentru a aplica o poartă Z controlată de toți Qubiții (inversarea fazei dacă toți Qubiții sunt în starea $|1\rangle$). Desigur, unii dintre Qubiții din starea dorită pot fi $|0\rangle$. Prin urmare, pentru acei Qubiți trebuie mai întâi să aplicăm o poartă X, apoi să realizăm Gate Z multi-controlată, și apoi să aplicăm din nou Gate X pentru a anula modificarea. `MCMTGate` arată astfel:
...
### Pasul 3: Execuție folosind primitivele IBM Quantum {#step-3-execute-using-ibm-quantum-primitives}

Dorim să facem multe măsurători și să vedem care stare este cea mai probabilă. O astfel de amplificare de amplitudine este o problemă de eșantionare potrivită pentru execuție cu primitiva IBM Quantum `Sampler`.

Observă că metoda `run()` a IBM Quantum SamplerV2 acceptă un iterabil de blocuri unificate primitive (PUB-uri). Pentru Sampler, fiecare PUB este un iterabil în formatul (circuit, valori_parametri). Cu toate acestea, la minimum, acceptă o listă de circuite cuantice.
...
### Pasul 1: Maparea intrărilor clasice la o problemă cuantică {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
### Pasul 2: Optimizarea problemei pentru execuția pe hardware cuantic {#step-2-optimize-problem-for-quantum-hardware-execution-1}
...
### Pasul 3: Execuție folosind primitivele IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-1}
...
### Pasul 4: Post-procesare și returnarea rezultatului în formatul clasic dorit {#step-4-post-process-and-return-result-in-desired-classical-format-1}
...
* Dacă starea ta țintă are deja un $|1\rangle$ pe un anumit qubit, nu trebuie să faci nimic cu acel qubit. Dacă ținta ta are un $|0\rangle$ pe un anumit qubit și vrei ca MCMTGate să inverseze semnul, trebuie să aplici un poartă `X` acelui qubit în oracolul tău (și apoi să anulezi Gate-ul `X` după MCMTGate).
...
### Pasul 1: Mapează intrările clasice la o problemă cuantică {#step-1-map-classical-inputs-to-a-quantum-problem-2}
...
### Pasul 2: Optimizează problema pentru execuția pe hardware cuantic {#step-2-optimize-problem-for-quantum-hardware-execution-2}
...
### Pasul 3: Execuție folosind primitivele IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-2}
...
### Pasul 4: Post-procesare și returnarea rezultatului în formatul clasic dorit {#step-4-post-process-and-return-result-in-desired-classical-format-2}
