...
Să începem prin a încărca câteva pachete necesare, inclusiv primitivele IBM Quantum. Vom selecta și calculatorul cuantic cel mai puțin ocupat disponibil.
...
## Algoritmul Deutsch-Jozsa {#the-deutsch-jozsa-algorithm-1}
...
2. Apoi, poarta $U_f$ este aplicată qubiților. Această poartă va lua primii n qubiți ca intrare (care se află acum într-o suprapunere egală a tuturor șirurilor posibile de n biți) și aplică funcția $f(x)=s \cdot x$ qubitului de ieșire, astfel încât acest qubit se află acum în starea: $ |- \oplus f(x)\rangle$. Datorită mecanismului de reacție inversă de fază (phase kickback), starea acestui qubit rămâne neschimbată, dar unii dintre termenii din starea qubitului de intrare capătă un semn minus:
...
3. Acum, următorul set de poartă-uri Hadamard este aplicat Qubiților de la 0 la $n-1$. Urmărirea semnelor minus în acest caz poate fi dificilă. Este util să știi că aplicarea unui strat de poartă-uri Hadamard la $n$ Qubiți aflați într-o stare de bază standard $|x\rangle$ poate fi scrisă astfel:
