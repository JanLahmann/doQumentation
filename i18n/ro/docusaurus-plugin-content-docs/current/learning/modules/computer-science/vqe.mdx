...
(a) Normalizează funcția de undă de încercare determinând $A$ astfel încât $$ \int_{-\infty}^{\infty} |\Psi_\text{trial}|^2 dx = 1 $$.

<Accordion>
<AccordionItem title="Answer">

Pentru a normaliza funcția de undă de încercare dată:
...
(b) Calculează valoarea de așteptare a Hamiltonianului $\hat{H}$ dat de $$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V(x)$$ unde $V(x)  = \frac{1}{2}m\omega^2x^2$, care corespunde unui potențial de oscilator armonic simplu.

<Accordion>
<AccordionItem title="Answer">

Hamiltonianul pentru un oscilator armonic este:
...
Optimizează $ \alpha $ pentru energia minimă
...
Pentru VQE, ansatz-ul (plural: ansätze) constă din două componente cheie. Prima este pregătirea stării inițiale, care configurează starea Qubitului prin aplicarea de poartă-uri cuantice fără parametri variationali. A doua componentă este circuit-ul cuantic parametrizat, un circuit cuantic special cu parametri ajustabili, similar cu butoanele unui radio. Acești parametri vor fi folosiți pentru ultima parte — optimizatorul clasic — pentru a ne ajuta să atingem cea mai bună stare fundamentală posibilă.
...
Completează spațiile libere cu termenii corecți pentru a completa rezumatul procesului VQE, apoi dă click pentru a-ți verifica răspunsurile.
...
Să începem prin încărcarea unor pachete necesare, inclusiv primitivele IBM Quantum. Vom selecta, de asemenea, cel mai puțin ocupat calculator cuantic disponibil.
...
Vom folosi trei ansatz-uri bazate pe combinații diferite de poartă-uri de rotație pentru un singur qubit:

- Ansatz cu un poartă de rotație pe un singur ax: Acest ansatz folosește rotații doar în jurul unui singur ax ($R_x(\theta)$). Pe sfera Bloch, aceasta corespunde deplasării doar de-a lungul unui cerc specific. Este cel mai puțin flexibil și acoperă un set limitat de stări.
...
Poți observa că Gate-urile `rx, rz` ale ansatz-ului nostru au fost convertite într-o serie de poartă-uri `rz, sx`, care sunt Gate-urile native ale Backend-ului nostru. De asemenea, poți vedea că `q0` este acum mapat pe al cincilea qubit fizic. Trebuie să mapăm și Hamiltonianul conform acestor modificări, ca în codul următor:
...
Notă: Poți obține un rezultat mai bun setând un nivel de atenuare a erorilor folosind [`resilience_level`](/guides/error-mitigation-and-suppression-techniques). Valoarea implicită este 1, iar dacă setezi o valoare mai mare, va utiliza mai mult timp QPU, dar ar putea returna un rezultat mai bun.
...
### Pasul 1: Maparea problemei pe Circuit-uri cuantice și operatori {#step-1-map-the-problem-to-quantum-circuits-and-operators-1}
...
### Pasul 2: Optimizează pentru hardware-ul țintă {#step-2-optimize-for-target-hardware-1}
...
### Pasul 3: Execută pe hardware-ul țintă {#step-3-execute-on-target-hardware-1}
...
### Pasul 4: Post-procesare {#step-4-post-process-1}
...
Te rog folosește [legea Coulomb](https://en.wikipedia.org/wiki/Coulomb%27s_law) și [unitățile atomice](https://en.wikipedia.org/wiki/Atomic_units) pentru a te asigura că obții valoarea în `Hartree`.
