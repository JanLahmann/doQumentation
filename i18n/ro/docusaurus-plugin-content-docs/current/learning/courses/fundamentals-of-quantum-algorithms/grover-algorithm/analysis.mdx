...
Deși nu este deloc evident la prima vedere, matricea $M$ este ceea ce obținem prin *ridicarea la pătrat* a unei matrice cu aspect mai simplu.
...
Acest unghi $\theta$ va juca un rol foarte important în analiza care urmează, deci merită subliniată importanța sa aici, când îl vedem pentru prima dată.

În lumina acestei exprimări a acestei matrice, observăm că
...
Acest lucru se întâmplă deoarece rotirea cu unghiul $\theta$ de două ori este echivalentă cu rotirea cu unghiul $2\theta$.
...
împreună cu formulele de *unghi dublu* din trigonometrie:
...
\end{aligned}
$$
În concluzie, starea registrului $\mathsf{Q}$ la începutul pasului 2 este
...
iar efectul aplicării $G$ asupra acestei stări este de a o roti cu un unghi $2\theta$ în spațiul generat de $\vert A_0\rangle$ și $\vert A_1\rangle$.
