...
Unele exponențiale foarte simple, cum ar fi $e^{iZ}$, pot fi implementate ușor pe calculatoare cuantice folosind un set compact de poartă-uri cuantice. Majoritatea Hamiltonienilor de interes nu vor avea un singur termen, ci vor avea în schimb mulți termeni. Observă ce se întâmplă dacă $H = H_1+H_2$:
...
Aici $t/r$ este feliuța de timp (pasul de evoluție) pe care o alegem. Ca rezultat, se creează un poartă care urmează să fie aplicat de $r$ ori. Un pas de timp mai mic duce la o aproximare mai precisă. Cu toate acestea, aceasta duce și la circuit-uri mai adânci care, în practică, duc la o acumulare mai mare de erori (o preocupare non-neglijabilă pe dispozitivele cuantice din generația actuală).
...
#### Definirea Hamiltonianului {#define-the-hamiltonian-1}
...
Aici construim un circuit pentru un singur pas de timp folosind Lie–Trotter.
Exact ca în cazul cu 20 de qubiți, formula produsului Lie (de ordinul întâi) este implementată în clasa [LieTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter). Din nou, formula de ordinul întâi constă în aproximarea menționată mai sus:
...
#### Setarea operatorilor de măsurat {#set-the-operators-to-be-measured-1}
...
#### Efectuarea simulației de evoluție temporală {#perform-time-evolution-simulation-1}
...
Felicitări! Ai avansat cu încă un pas în călătoria ta cuantică la scară de utilitate. Mai există o singură lecție!
