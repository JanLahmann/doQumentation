...
## 2. Bazele Transformatei Fourier Cuantice (QFT) {#2-basic-of-quantum-fourier-transformation-qft}
...
### 2.1. Intuiție <a id="intuition"></a> {#21-intuition}
...
### 2.2 Exemplu: QFT cu 1 qubit {#22-example-1-qubit-qft}
...
### 2.3 Reprezentarea ca produs a QFT {#23-product-representation-of-qft}
...
### 2.4 Exemplu: Construcția Circuit-ului QFT cu 3 qubiți {#24-example-circuit-construction-of-3-qubits-qft}
...
### 2.5 Exercițiu: QFT {#25-exercise-qft}
...
## 3. Bazele Estimării Fazei Cuantice (QPE) {#3-basics-of-quantum-phase-estimation-qpe}
...
### 3.1 Procedură {#31-procedure}

#### 1. Configurare {#1-setup}
...
#### 3. Operații unitare controlate {#3-controlled-unitary-operations}
...
### 3.2 Exemplu: QPE cu Gate-ul T {#32-example-t-gate-qpe}
...
$$
Inițializăm $\vert\psi\rangle = \vert1\rangle$ al vectorului propriu al porții $T$ aplicând o poartă $X$:
...
Apoi, aplicăm porți Hadamard qubiților de numărare:
...
Aplicăm transformarea Fourier cuantică inversă pentru a converti starea registrului de numărare, apoi măsurăm registrul de numărare:
...
Vedem că obținem un rezultat (`001`) cu certitudine, care se traduce în zecimal ca: `1`. Acum trebuie să împărțim rezultatul nostru (`1`) la $2^n$ pentru a obține $\theta$:
...
#### Soluția exercițiului: $\theta = 1/3$ {#solution-of-the-exercise-theta-13}
...
## 4. Execuție folosind primitiva IBM Quantum Sampler {#4-execution-using-the-ibm-quantum-sampler-primitive}
...
### 4.1 Pasul 1: Maparea problemei în Circuit-uri cuantice și operatori {#41-step-1-map-problem-to-quantum-circuits-and-operators}
...
### 4.3 Pasul 3: Execuție pe hardware-ul țintă {#43-step-3-execute-on-target-hardware}
...
### 4.4 Pasul 4: Post-procesarea rezultatelor {#44-step-4-post-process-the-results}
