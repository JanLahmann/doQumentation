...
<OpenInLabBanner notebookPath="learning/courses/utility-scale-quantum-computing/error-mitigation.ipynb" />
  ...
  # Zgomot cuantic și atenuarea erorilor
...
## 2. Simulare cu zgomot fără atenuarea erorilor {#2-noisy-simulation-without-error-mitigation}
...
### 2.1 Construirea unui Circuit de test {#21-build-a-test-circuit}
...
### 2.2 Construirea unui model de zgomot {#22-build-a-noise-model}
...
### 2.3 Construirea unui Sampler cu zgomot pe baza modelului de zgomot {#23-build-a-noisy-sampler-with-the-noise-model}
...
### 2.4 Rularea circuitelor cuantice pe Sampler-ul cu zgomot {#24-run-quantum-circuits-on-the-noisy-sampler}
...
### 2.5 Reprezentarea grafică a rezultatelor {#25-plot-results}
...
### 2.6 Simulare ideală {#26-ideal-simulation}
...
### 2.8 Simulare cu zgomot mai realistă {#28-more-realistic-noisy-simulation}
...
## 3. Calcul cuantic real cu atenuarea erorilor {#3-real-quantum-computation-with-error-mitigation}
...
### 3.1 Construirea circuitelor {#31-build-circuits}
...
Notă: Așa cum se arată mai sus, circuitul cu $k$ pași de timp va avea $4k$ straturi de poartă-uri cu doi qubiți.
...
### 3.2 Transpilarea circuitelor {#32-transpile-the-circuits}
...
### 3.3 Executare cu Estimator (cu niveluri de reziliență diferite) {#33-execute-using-estimator-with-different-resilience-levels}

Setarea nivelului de reziliență (`estimator.options.resilience_level`) este cea mai simplă modalitate de a aplica atenuarea erorilor când utilizezi Qiskit Estimator. Estimator acceptă următoarele niveluri de reziliență (începând cu 28.06.2024). Vezi mai multe detalii în ghidul [Configurarea atenuării erorilor](/guides/error-mitigation-and-suppression-techniques).
...
### 3.4 Reprezentarea grafică a rezultatelor {#34-plot-results}
...
## 4. (Opțional) Personalizarea opțiunilor de atenuare a erorilor {#4-optional-customize-error-mitigation-options}
...
- [Configurarea atenuării erorilor](/guides/error-mitigation-and-suppression-techniques)
