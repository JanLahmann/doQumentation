...
Să simulăm mai întâi acțiunea acestei porți cu doi qubiți atunci când q0 și q1 sunt ambele $|0\rangle$, și să obținem vectorul de stare de ieșire. Sintaxa Qiskit folosită este ```qc.cx(control qubit, target qubit)```.
...
O stare încurcată este o stare $|\psi_{AB}\rangle$ formată din stările cuantice $|\psi_A\rangle$ și $|\psi_B\rangle$ care nu poate fi reprezentată printr-un produs tensorial al stărilor cuantice individuale.
...
dar nu există coeficienți $a_0, a_1, b_0, $ și $b_1$ care să satisfacă aceste două ecuații. Prin urmare, $|\psi_{AB}\rangle$ nu este reprezentată printr-un produs tensorial al stărilor cuantice individuale, $|\psi\rangle_A$ și $|\psi\rangle_B$, iar aceasta înseamnă că $|\psi_{AB}\rangle =  \frac{1}{\sqrt{2}}(|00\rangle +|11\rangle)$ este o stare încurcată.

Să creăm starea Bell și să o rulăm pe un calculator cuantic real. Acum vom urma cei patru pași pentru a scrie un program cuantic, numiți **modele Qiskit**:
...
Dacă nu ai `qiskit-ibm-runtime`, va trebui să îl instalezi mai întâi. Pentru mai multe informații, [consultă referința API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-service).
