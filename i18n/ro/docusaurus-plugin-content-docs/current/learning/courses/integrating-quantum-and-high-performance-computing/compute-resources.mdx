...
Atât în calculul clasic, cât și în cel cuantic, procesele pot fi executate secvențial sau în paralel. Deoarece qubiții au un spațiu de stare bogat față de biții clasici, uneori are sens ca mai multe Gate-uri cu un singur qubit să fie executate pe un qubit în secvență (cum ar fi un poartă `R_x` urmat de un poartă `R_z`). Deoarece entanglementul între qubiți este esențial pentru calculul cuantic, este de asemenea obișnuit ca un circuit cuantic să aibă un set de poartă-uri de entanglare acționând pe mulți qubiți. Acești factori și alții fac obișnuită identificarea proceselor care pot fi executate în paralel la scara operațiunilor individuale de poartă dintr-un Circuit cuantic. În calculul clasic, paralelismul la nivel de bit este de asemenea posibil, dar mai puțin frecvent considerat la nivel de poartă; este mai obișnuit să se facă referire la procesele paralele și secvențiale la o scară mai mare.

În calculul cuantic, se face referire la un „strat” de porți care pot fi executate toate simultan. În multe aplicații este util să se efectueze un set de rotații pe toți qubiții și apoi porți de întrețesere între perechi de qubiți. În aceste contexte, se face referire la un „strat de rotație” (un strat de porți precum `R_x`, `R_y`, și/sau `R_z`) și un „strat de întrețesere” (ca unul cu porți `CNOT`). Numărul de straturi dintr-un circuit este „adâncimea circuitului”, o măsură importantă deoarece o adâncime mai mare înseamnă mai multe straturi de zgomot și erori compuse.

Poate fi dificil să identifici vizual straturile de poartă-uri atunci când acestea nu sunt aliniate folosind bariere. În Qiskit, o barieră servește ca o instrucțiune în circuitele cuantice care acționează ca un separator vizual și o constrângere în timpul compilării. Atât la desenarea circuitului, cât și la executarea lui, niciun Gate nu va fi mutat dincolo de barieră. Acest lucru poate fi important în contexte precum decuplarea dinamică, în care se implementează intenționat Gate-uri care se simplifică la o identitate pentru a suprima anumite tipuri de erori. Pentru mai multe informații despre decuplarea dinamică, consultă [acest ghid](/guides/error-mitigation-and-suppression-techniques). Pentru efectul vizual al barierelor, compară aceste două imagini ale aceluiași circuit, prima fără bariere și a doua cu bariere pentru a forța alinierea straturilor.
...
În calculul cuantic, capacitățile unui sistem cuantic sunt adesea descrise folosind trei dimensiuni fundamentale: scală, calitate și viteză. IBM Quantum caracterizează aceste dimensiuni folosind trei metrici hardware care pot fi aplicate pe diferite modalități de calcul cuantic: qubiți programabili (scală), operații pe qubiți (calitate) și numărul maxim de circuite pe secundă (viteză). Aceste metrici oferă o imagine de ansamblu a capacității computaționale și eficienței unui sistem în diferite implementări hardware.

* **Scala** este măsurată prin numărul de qubiți programabili. Qubiții programabili sunt qubiții pe care utilizatorii îi pot controla, manipula, măsura și încorpora direct în algoritmi cuantici.


* **Calitatea** este măsurată prin operații pe qubiți (QuOps), o metrică care descrie câte dintre cele mai solicitante operații din punct de vedere computațional poate executa în mod fiabil un sistem înainte ca erorile acumulate să copleșească calculul. O capacitate QuOps mai mare permite, în general, circuite mai adânci și algoritmi cuantici mai sofisticați.


* **Viteza** este măsurată prin numărul maxim de circuite pe secundă, cunoscut și ca debit de circuite. Această metrică surprinde câte circuite cuantice utile poate executa un sistem într-un interval de timp și oferă o indicație a eficienței computaționale generale.


Metrici hardware suplimentare sunt adesea folosite pentru a caracteriza aspecte specifice ale comportamentului procesorului. De exemplu, EPLG (erori pe poartă stratificată) și măsurătorile de fidelitate a stratului oferă informații despre calitatea porților și acumularea erorilor, în timp ce CLOPS (Circuit Layer Operations Per Second) măsoară viteza de execuție a unei sarcini standardizate de referință cuantică. Aceste metrici rămân utile pentru evaluarea unor sarcini specifice și a detaliilor de implementare, dar qubiții programabili, operațiile pe qubiți și numărul maxim de circuite pe secundă oferă un cadru mai larg pentru compararea sistemelor cuantice.

### Diferențe între metricile cuantice și clasice {#differences-between-quantum-and-classical-metrics}

Sistemele de calcul clasic sunt adesea caracterizate folosind metrici precum FLOPS (operații în virgulă mobilă pe secundă), capacitate de memorie, lățime de bandă a memoriei și frecvențe de ceas ale procesorului. Se pot face unele analogii aproximative între acestea și metricile pentru calculatoare cuantice introduse mai sus, dar diferențele dintre metrici sunt importante.

Numărul maxim de circuite pe secundă poate fi văzut ca o analogie aproximativă cu metricile de performanță orientate spre debit din calculul clasic, deoarece măsoară cât de multă muncă utilă poate efectua un sistem cuantic într-un interval de timp. Spre deosebire de FLOPS, totuși, aceasta măsoară execuția circuitelor cuantice, nu a operațiilor aritmetice.

CLOPS este uneori comparat cu benchmark-uri clasice de debit precum FLOPS, dar este mai bine înțeles ca un benchmark specific unei sarcini de lucru decât ca o măsură universală a performanței calculului cuantic. EPLG și măsurătorile de fidelitate conexe oferă informații despre cât de precis execută un procesor straturi de porți cuantice. Aceste metrici sunt utile pentru înțelegerea caracteristicilor hardware specifice, în timp ce cadrul principal scală-calitate-viteză oferă o bază mai generală pentru compararea sistemelor cuantice. Multe descrieri ale performanței calculatoarelor cuantice pot fi găsite pe pagina [Resurse de calcul](https://quantum.cloud.ibm.com/computers) de pe IBM Quantum Platform. Reține că, deși numărul de qubiți programabili este un număr fix pentru un anumit QPU, metricile de viteză și calitate pot varia ușor în timp datorită actualizărilor de calibrare, întreținerii, condițiilor de funcționare și îmbunătățirilor infrastructurii hardware și software înconjurătoare.

Împreună, aceste metrici complementare ajută la ghidarea alocării resurselor, planificării și proiectării aplicațiilor pe sistemele de calcul cuantic.

În multe cazuri, întregul sistem cuantic este tratat ca o singură unitate. Totuși, când o sarcină depășește capacitatea unei unități — fie în termeni de număr de qubiți programabili, adâncime a circuitului sau throughput al circuitului — tehnici precum tăierea/îmbinarea circuitelor pot fi utilizate. Tăierea circuitelor este procesul de descompunere a sarcinilor cuantice mari în sub-sarcini mai mici și gestionabile, care pot fi distribuite pe mai multe cipuri cuantice, permițând calcul cuantic scalabil în ciuda limitărilor hardware-ului. Îmbinarea circuitelor se referă la procesul care urmează după tăierea circuitelor — pasul de post-procesare clasică care „îmbină" sau combină rezultatele din sub-circuitele mai mici.
...
QPU-urile pot fi conectate la infrastructura clasică prin rețele și diverse interfețe de programare a aplicațiilor (API-uri) care permit dezvoltatorilor de software să interacționeze programatic cu QPU-urile. Aceste API-uri sunt de obicei ascunse în spatele kit-urilor de dezvoltare software (SDK-uri) și bibliotecilor (precum Qiskit) și expuse oamenilor de știință computaționali sub forma unor abstracții de programare (precum primitive, despre care vom vorbi în Capitolul 3: modele de programare).
...
* Joburile sunt trimise de utilizator, valorificând IBM Quantum Compute Service, și folosind de obicei o primitivă IBM Quantum, precum `Sampler`, `Estimator` sau altele.
...
* Execuția sarcinii: Algoritmul de factorizare în numere prime rulează, efectuând diviziuni sau alți pași de factorizare pe resursele alocate până la finalizarea sarcinii.
...
* La trimiterea jobului prin Quantum Compute, jobul intră în coada pentru QPU-ul selectat. Utilizatorul nu are control asupra timpului de așteptare în coadă, deși acest lucru poate fi diferit pentru sistemele dedicate.
...
Să presupunem că încerci să cauți într-o bază de date nesortată pentru a găsi un element pe care îl vom numi „țintă”. Pentru fiecare dintre următoarele acțiuni, indică cărei etape de management al resurselor îi corespunde:

(a) Estimarea dimensiunii bazei de date și a timpului necesar pentru verificarea fiecărui element

(b) Asigurarea faptului că găsirea țintei pe un GPU oprește procesul pe celelalte GPU-uri pentru a le elibera pentru următoarea problemă

(c) Divizarea spațiului de căutare în regiuni pentru fiecare dintre cele (să spunem 10) GPU-uri ale tale să caute
...
(d) Da, ar putea. Folosește partiția cuantică.
...
Să analizăm fișierul de trimitere și să vedem ce se întâmplă acolo. Avem o opțiune nouă, care este `gres`. `gres` este o opțiune Slurm pentru a defini resurse computaționale suplimentare. În cazul nostru, această nouă resursă ar fi resursa noastră cuantică. Deoarece am specificat resursele și partiția clusterului nostru unde sunt disponibile resursele cuantice, primitivele noastre IBM Quantum vor folosi aceste resurse alocate pentru a executa sarcina cuantică.
