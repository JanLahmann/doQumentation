...
### 1.1 Operatorul (Hamiltonianul) {#11-the-operator-hamiltonian}
...
Cuvântul „ansatz” provine din germană și înseamnă „abordare”. Pluralul corect în germană este „ansätze”, deși se întâlnește adesea „ansatzes” sau „ansatze”. În contextul VQE, un ansatz este circuitul cuantic pe care îl folosești pentru a crea o funcție de undă multi-qubit care aproximează cel mai bine starea fundamentală a sistemului pe care îl studiezi și care produce astfel cea mai mică valoare de așteptare a operatorului tău. Acest circuit cuantic va conține parametri variaționali (adesea grupați în vectorul de variabile $\vec{\theta}$).
...
Se alege un set inițial de valori $\vec{\theta_0}$ ale parametrilor variaționali. Vom numi operația unitară a ansatz-ului pe circuit $U_{\text{var}}(\vec{\theta_0})$. În mod implicit, toți qubiții din calculatoarele cuantice IBM&reg; sunt inițializați în starea $|0\rangle$. Când circuitul este rulat, starea qubiților va fi

$$
|\psi(\vec{\theta_0})\rangle=U_{\text{var}}(\vec{\theta_0})|0\rangle^{\otimes N}
$$

Dacă tot ce ne-ar trebui ar fi cea mai mică energie (folosind limbajul sistemelor fizice), am putea estima acest lucru pur și simplu măsurând energia de multe ori și luând valoarea minimă. Dar de obicei vrem, de asemenea, configurația care produce acea energie sau valoare proprie minimă. Așadar, următorul pas este estimarea valorii de așteptare a Hamiltonianului, ceea ce se realizează prin măsurători cuantice. Multe lucruri intervin în acest proces. Dar putem înțelege acest proces calitativ observând că probabilitatea $P_j$ de a măsura o energie $E_j$ (din nou folosind limbajul sistemelor fizice) este legată de valoarea de așteptare prin:
...
Astfel, prin efectuarea multor măsurători ale operatorilor Pauli care compun Hamiltonianul nostru, putem estima valoarea de așteptare a Hamiltonianului în starea curentă a sistemului $|\psi(\vec{\theta_0})\rangle$. Următorul pas este să variem parametrii $\vec{\theta}$ și să încercăm să ne apropiem mai mult de starea de energie minimă (fundamentală) a sistemului. Din cauza parametrilor variaționali din ansatz, acesta este uneori numit __forma variațională__.
...
### 1.5 Principiul variațional {#15-the-variational-principle}
...
## 2. Comparație cu fluxul de lucru clasic {#2-comparison-with-classical-workflow}
...
### 2.1 Fluxul de lucru clasic {#21-classical-workflow}
...
### 2.2 Fluxul de lucru cuantic și grupurile de Pauli comutativi {#22-quantum-workflow-and-commuting-pauli-groups}
...
Procesele necesare pentru estimarea valorii de așteptare sunt colectate în primitivul IBM Quantum numit Estimator. Pentru a afla mai multe despre Estimator, vezi [referința API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2) în Documentația IBM Quantum®. Se poate folosi Estimator direct, dar Estimator returnează mult mai mult decât valoarea proprie minimă a energiei. De exemplu, returnează și informații despre eroarea standard a ansamblului. Astfel, în contextul problemelor de minimizare, Estimator se găsește adesea în interiorul unei funcții de cost. Pentru a afla mai multe despre intrările și ieșirile Estimator, vezi acest [ghid](/guides/primitive-input-output#pubs) din Documentația IBM Quantum.
...
### 2.3 Factorii care determină costul computațional {#23-factors-that-determine-computational-cost}
...
## 3. Exemplu de Hamiltonian {#3-example-hamiltonian}
...
### 3.1 Pasul 1: Maparea problemei pe circuite cuantice și operatori {#31-step-1-map-the-problem-to-quantum-circuits-and-operators}
...
### 3.3 Pasul 3: Execuție pe hardware-ul țintă {#33-step-3-execute-on-target-hardware}

#### 3.3.1 Raportarea valorilor {#331-reporting-out-values}
...
### 3.4 Pasul 4: Post-procesarea rezultatelor {#34-step-4-post-process-results}
...
## 4. Relația VQE cu alte metode {#4-vqes-relationship-to-other-methods}
...
### 4.1 Punctele forte și slabe ale VQE {#41-the-strengths-and-weaknesses-of-vqe}
...
### 4.2 Relația cu alte abordări {#42-relationship-to-other-approaches}
