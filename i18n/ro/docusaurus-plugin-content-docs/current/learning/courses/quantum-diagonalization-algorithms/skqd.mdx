...
## 0. Spațiul Krylov {#0-the-krylov-space}
...
## 1. Mapează problema la circuite cuantice și operatori {#1-map-problem-to-quantum-circuits-and-operators}
...
Pe lângă dimensiunea Krylov și pasul de timp, trebuie să setăm numărul de pași Trotter pentru evoluția temporală. Folosirea prea puținilor pași duce la erori de Trotterizare mai mari, în timp ce prea mulți pași duc la circuite mai adânci. În această lecție, setăm numărul de pași Trotter la $6$.
...
## 2. Optimizează pentru hardware-ul țintă {#2-optimize-for-target-hardware}
...
## 3. Execută pe hardware-ul țintă {#3-execute-on-target-hardware}
...
## 4. Post-procesează rezultatele {#4-post-process-results}
...
<Accordion>
<AccordionItem title="Ce s-ar putea face pentru a îmbunătăți convergența în graficul de mai sus?">
...
<AccordionItem title="Care sunt principalele avantaje ale SKQD față de (a) SQD, și (b) KQD?">

__Answer:__

Care sunt principalele avantaje ale SKQD față de (a) SQD și (b) KQD?

   (a) SKQD vine cu garanții de convergență pe care SQD nu le are. În SQD trebuie fie să faci o presupunere foarte bună pentru ansatz-ul tău, care are o suprapunere excelentă cu suportul stării fundamentale în baza computațională, fie trebuie să introduci o componentă variațională în calcul pentru a eșantiona o familie de ansaetze.
