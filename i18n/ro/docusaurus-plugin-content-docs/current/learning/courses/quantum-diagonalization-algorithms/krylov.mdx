...
(b) Procesul de ortogonalizare constă în scăderea proiecției unui vector nou pe toți vectorii anteriori. Dacă toți vectorii anteriori acoperă întregul spațiu vectorial, atunci scăderea proiecțiilor pe întregul subspațiu va lăsa întotdeauna un vector zero.

</AccordionItem>
</Accordion>

Să presupunem că un coleg cercetător demonstrează metoda Krylov aplicată unei matrice mici, de tip demonstrativ. Există ceva greșit în alegerea matricei $A$ și a vectorului inițial $|\psi\rangle$?
...
Colegul tău a ales din greșeală un vector propriu drept vector inițial. Aplicarea matricei asupra vectorului inițial va returna pur și simplu același vector, scalat cu valoarea proprie. Aceasta nu va genera un subspațiu de dimensiune crescândă. Sfătuiește-ți colegul să aleagă un alt vector inițial, asigurându-se că nu este un vector propriu.

</AccordionItem>
</Accordion>

Aplică metoda Krylov matricei date, selectând un vector inițial nou adecvat. Notează estimările valorii proprii minime la ordinul 0 și la ordinul 1 ale subspațiului tău Krylov.
...
$$
Acest lucru poate fi făcut manual, dar este mai ușor de făcut folosind numpy:
...
Suntem interesați de modul în care se comportă eroarea noastră atunci când este transformată de matricea noastră. Așadar, să calculăm norma $A$ a erorii. Adică
$$
\begin{aligned}
∥|e_0\rangle−|p_0\rangle∥_A^2&=\left(\langle e_0|A−\langle p_0|A\right)\left(|e_0\rangle−|p_0\rangle\right)\\
 ...
 \end{aligned}
$$
unde am folosit simetria lui $A$ și, de asemenea, faptul că $A |e_0\rangle = |R_0\rangle.$ Aici $d$ este o constantă independentă de $|p_0\rangle$. Așa cum s-a menționat în secțiunea 1.2, norma $A$ a erorii nu este singura cantitate pe care am putea alege să o minimizăm, dar este una bună. Vrem să vedem cum variază această cantitate în funcție de alegerea vectorilor de corecție $|p_0\rangle.$ Așadar, definim funcția $f$ prin
$$
f(|p_0\rangle)=\langle p_0|A|p_0\rangle−2\langle R_0|p_0\rangle+d.
$$
$f$ este pur și simplu eroarea $|e_1\rangle$ ca funcție a corecției $|p_0\rangle$ măsurată în norma $A$. Prin urmare, vrem să alegem $|p_0\rangle$ astfel încât $f(|p_0\rangle)$ să fie cât mai mic posibil. În acest scop, calculăm gradientul lui $f$. Folosind simetria lui $A$, avem
$$
\nabla f(|p_0\rangle) = 2(A|p_0\rangle−|R_0\rangle).
$$
Gradientul indică direcția de creștere maximă, ceea ce înseamnă că opusul său ne oferă direcția în care funcția scade cel mai mult: direcția de __coborâre cea mai abruptă__. La estimarea noastră inițială $|x_0\rangle$, unde $|p_0\rangle=0$, avem că
$\nabla f(0) = -2|R_0\rangle.$
Astfel, funcția $f$ scade cel mai mult în direcția reziduului $|R_0\rangle.$ Așadar, alegerea noastră inițială ar beneficia cel mai mult de adăugarea vectorului $|p_0\rangle=\alpha_0 |R_0\rangle$ pentru un anumit scalar $\alpha_0$.
...
### Verifică-ți înțelegerea {#check-your-understanding-1}

În fluxul de lucru de mai sus, am propus minimizarea normei $A$ a erorii. Ce alte cantități ar putea lua în considerare cineva pentru minimizare atunci când caută starea fundamentală și valoarea proprie a acesteia?

<Accordion>
<AccordionItem title="Răspuns">

S-ar putea imagina folosirea vectorului reziduu în locul normei $A$ a erorii. Ar putea exista cazuri în care ar fi util să se ia în considerare vectorul de eroare în sine.
...
Observă că timpul necesar pentru construirea unui subspațiu Krylov depășește inițial timpul necesar pentru diagonalizarea completă cu numpy. Dar pe măsură ce dimensiunea matricei crește, metoda Krylov devine avantajoasă. Acest lucru este valabil chiar dacă reducem eroarea acceptabilă, dar avantajul apare la o dimensiune mai mare a matricei. Merită să analizăm acest lucru mai atent.

Complexitatea temporală a diagonalizării numerice este $O(n^3)$ (cu unele variații între algoritmi). Complexitatea temporală a generării unei baze ortonormale de $n$ vectori este de asemenea $O(n^3)$. Așadar, avantajul metodei Krylov __nu__ este legat de utilizarea $\it{unei}$ baze ortonormale oarecare, ci de utilizarea unei baze ortonormale particulare care selectează eficient valorile proprii de interes. Am văzut deja acest lucru din schița unei demonstrații din prima secțiune a acestei lecții, iar acest lucru este esențial pentru garanțiile de convergență în metodele Krylov.
Să trecem în revistă progresul nostru de până acum:
* Pentru matrice foarte mari, metoda subspațiului Krylov poate produce valori proprii aproximative în cadrul toleranțelor necesare mai rapid decât algoritmii tradiționali de diagonalizare.
* Pentru astfel de matrice foarte mari, generarea unui subspațiu Krylov este partea cea mai costisitoare din punct de vedere al timpului a metodei subspațiului Krylov.
* Prin urmare, o modalitate eficientă de a genera un subspațiu Krylov ar fi extrem de valoroasă.
Aici intervine, în sfârșit, calculatorul cuantic.

### Verifică-ți înțelegerea {#check-your-understanding-2}

Consultă graficul netezit al timpilor de diagonalizare în funcție de dimensiunea matricei de mai sus.
...
## 3. Krylov prin evoluție temporală {#3-krylov-via-time-evolution}
...
### 3.1 Evoluție temporală {#31-time-evolution}
...
### 3.2 Cum se implementează pe un calculator cuantic {#32-how-to-implement-on-a-quantum-computer}
...
$$
Aici $P_\alpha$ este un șir Pauli de forma $P_\alpha \sim IZIXII...YZXIX$ sau un set de astfel de șiruri Pauli care comută unele cu altele. Dat că putem scrie $H$ ca o astfel de sumă de operatori măsurabili, următoarele expresii pentru elementele matriceale ale lui $\tilde{H}$ pot fi realizate folosind primitivul Estimator IBM Quantum.
...
#### Circuite template pentru calcularea elementelor de matrice ale $\tilde{S}$ și $\tilde{H}$ prin testul Hadamard {#template-circuits-for-calculating-matrix-elements-of-tildes-and-tildeh-via-hadamard-test}
...
### 4.3 Pasul 3. Executarea folosind o primitivă IBM Quantum {#43-step-3-execute-using-an-ibm-quantum-primitive}
...
## 5. Discuție și extindere {#5-discussion-and-extension}
