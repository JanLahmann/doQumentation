---
description: 'Aici descriem cum pot fi aplicate algoritmii cuantici variaționali la probleme de chimie, Max-Cut și altele!'
...
## Definiții ale problemelor {#problem-definitions}
...
Vom explora mai întâi cum să construim o instanță VQE manual pentru a găsi cea mai mică valoare proprie a lui $\hat{O}_1$. Aceasta va încorpora o varietate de tehnici pe care le-am abordat pe parcursul acestui curs.
...
## Experimentând pentru a îmbunătăți viteza și acuratețea {#experimenting-to-improve-speed-and-accuracy}

### Adaugă starea de referință {#add-reference-state}
...
Pentru acest nou circuit, soluția ideală ar putea fi atinsă cu toți parametrii setați la $0$, deci aceasta confirmă că alegerea circuitului de referință este rezonabilă.
...
### Schimbă punctul inițial {#change-the-initial-point}
...
### Experimentând cu optimizatori diferiți {#experimenting-with-different-optimizers}

Putem ajusta optimizatorul folosind argumentul `method` al funcției `minimize` din SciPy, cu mai multe opțiuni [găsite aici](https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.minimize.html). Am folosit inițial un minimizator cu constrângeri (`COBYLA`). În acest exemplu, vom explora folosirea unui minimizator fără constrângeri (`BFGS`)
...
Acum, în loc să căutăm doar cea mai mică valoare proprie a observabilelor noastre, vom căuta toate cele $4$ (unde $k=4$).
...
De asemenea, în implementarea VQD din Qiskit, în loc să se ia în considerare observabilele efective descrise în lecția anterioară, fidelitățile $|\langle \psi(\vec{\theta})| \psi(\vec{\theta^j})\rangle  |^2$ sunt calculate direct prin algoritmul `ComputeUncompute`, care utilizează o primitivă `Sampler` pentru a eșantiona probabilitatea de a obține $|0\rangle$ pentru circuitul
...
### Modificarea valorilor beta {#change-betas}
După cum s-a menționat în lecția anterioară, valorile lui $\vec{\beta}$ ar trebui să fie mai mari decât diferența dintre valorile proprii. Să vedem ce se întâmplă atunci când nu satisfac această condiție cu $\hat{O}_2$
...
De data aceasta, optimizatorul returnează aceeași stare $|\phi_0\rangle = \frac{1}{\sqrt{2}}(|00\rangle + |11\rangle)$ ca soluție propusă pentru toate stările proprii: ceea ce este în mod evident greșit. Acest lucru se întâmplă deoarece valorile beta erau prea mici pentru a penaliza starea proprie minimă în funcțiile de cost succesive. Prin urmare, aceasta nu a fost exclusă din spațiul de căutare efectiv în iterațiile ulterioare ale algoritmului și a fost aleasă întotdeauna ca cea mai bună soluție posibilă.

Recomandăm să experimentezi cu valorile lui $\vec{\beta}$ și să te asiguri că sunt mai mari decât diferența dintre valorile proprii.
...
## Chimie cuantică: rezolvator de stare fundamentală și stări excitate {#quantum-chemistry-ground-state-and-excited-energy-solver}
...
Putem folosi și VQD pentru a rezolva $k=2$ stări totale (starea fundamentală și prima stare excitată).
...
### Hardware real și un avertisment final {#real-hardware-and-a-final-cautionary-message}
...
Problema tăieturii maxime (max-cut) este o problemă de optimizare combinatorică care implică împărțirea vârfurilor unui graf în două mulțimi disjuncte astfel încât numărul de muchii dintre cele două mulțimi să fie maximizat. Mai formal, dat un graf neorientat $G=(V,E)$, unde $V$ este mulțimea vârfurilor și $E$ este mulțimea muchiilor, problema max-cut cere partiționarea vârfurilor în două submulțimi disjuncte, $S$ și $T$, astfel încât numărul de muchii cu un capăt în $S$ și celălalt în $T$ să fie maximizat.

Putem aplica max-cut pentru a rezolva diverse probleme, precum clusterizarea, proiectarea de rețele și tranzițiile de fază. Vom începe prin a crea un graf problemă:
...
Această problemă poate fi exprimată ca o problemă de optimizare binară. Pentru fiecare nod $0 \leq i < n$, unde $n$ este numărul de noduri ale grafului (în acest caz $n=4$), vom considera variabila binară $x_i$. Această variabilă va avea valoarea $1$ dacă nodul $i$ face parte din grupul pe care îl vom eticheta $1$ și $0$ dacă face parte din celălalt grup, pe care îl vom eticheta ca $0$. Vom nota de asemenea cu $w_{ij}$ (elementul $(i,j)$ al matricei de adiacență $w$) greutatea muchiei care merge de la nodul $i$ la nodul $j$. Deoarece graful este neorientat, $w_{ij}=w_{ji}$. Apoi putem formula problema noastră ca maximizarea următoarei funcții de cost:
...
Pentru a rezolva această problemă cu un calculator cuantic, vom exprima funcția de cost ca valoarea așteptată a unui observabil. Totuși, observabilele pe care Qiskit le admite nativ constau din operatori Pauli, care au valori proprii $1$ și $-1$ în loc de $0$ și $1$. De aceea vom face următoarea schimbare de variabilă:

Unde $\vec{x}=(x_0,x_1,\cdots ,x_{n-1})$. Putem folosi matricea de adiacență $w$ pentru a accesa comod greutățile tuturor muchiilor. Aceasta va fi folosită pentru a obține funcția noastră de cost:
...
Mai mult, tendința naturală a unui calculator cuantic este să găsească minime (de obicei energia cea mai mică) în loc de maxime, așa că în loc să maximizăm $C(\vec{z})$ vom minimiza:

$$
-C(\vec{z}) =  \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2} z_iz_j -  \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2}
$$

Acum că avem o funcție de cost de minimizat ale cărei variabile pot avea valorile $-1$ și $1$, putem face următoarea analogie cu $Z$ Pauli:

$$
z_i \equiv Z_i = \overbrace{I}^{n-1}\otimes ... \otimes \overbrace{Z}^{i} \otimes ... \otimes \overbrace{I}^{0}
$$

Cu alte cuvinte, variabila $z_i$ va fi echivalentă cu o poartă $Z$ care acționează pe qubitul $i$. Mai mult:
...
$$
Operatorul este o combinație liniară de termeni cu operatori Z pe nodurile conectate printr-o muchie (amintește-ți că qubitul al 0-lea este cel mai din dreapta): $IIZZ + IZIZ + IZZI + ZIIZ + ZZII$. Odată ce operatorul este construit, ansatz-ul pentru algoritmul QAOA poate fi ușor construit folosind circuitul `QAOAAnsatz` din biblioteca de circuite Qiskit.
...
Continuă la lecția finală pentru a susține evaluarea și a-ți obține insigna!
