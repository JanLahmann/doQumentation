...
{/* source-hash: sha256:lines1-198:examples-and-applications.mdx */}
...
Problema tăieturii maxime (Max-Cut) este o problemă de optimizare combinatorie care implică împărțirea vârfurilor unui graf în două mulțimi disjuncte astfel încât numărul de muchii dintre cele două mulțimi să fie maximizat. Mai formal, dat un graf neorientat $G=(V,E)$, unde $V$ este mulțimea vârfurilor și $E$ este mulțimea muchiilor, problema Max-Cut cere partiționarea vârfurilor în două submulțimi disjuncte, $S$ și $T$, astfel încât numărul de muchii cu un capăt în $S$ și celălalt în $T$ să fie maximizat.
...
Această problemă poate fi exprimată ca o problemă de optimizare binară. Pentru fiecare nod $0 \leq i < n$, unde $n$ este numărul de noduri ale grafului (în acest caz $n=4$), vom considera variabila binară $x_i$. Această variabilă va avea valoarea $1$ dacă nodul $i$ se află în unul dintre grupuri, pe care îl vom eticheta $1$, și $0$ dacă se află în celălalt grup, pe care îl vom eticheta $0$. Vom nota de asemenea cu $w_{ij}$ (elementul $(i,j)$ al matricei de adiacență $w$) ponderea muchiei care merge de la nodul $i$ la nodul $j$. Deoarece graful este neorientat, $w_{ij}=w_{ji}$. Atunci putem formula problema noastră ca maximizarea următoarei funcții de cost:
...
Pentru a rezolva această problemă cu un calculator cuantic, vom exprima funcția de cost ca valoarea de așteptare a unui observabil. Totuși, observabilele pe care Qiskit le acceptă nativ constau din operatori Pauli, care au valorile proprii $1$ și $-1$ în loc de $0$ și $1$. De aceea vom face următoarea schimbare de variabilă:

Unde $\vec{x}=(x_0,x_1,\cdots ,x_{n-1})$. Putem folosi matricea de adiacență $w$ pentru a accesa comod ponderile tuturor muchiilor. Aceasta va fi utilizată pentru a obține funcția noastră de cost:
...
Mai mult, tendința naturală a unui calculator cuantic este de a găsi minime (de obicei, energia cea mai mică) în loc de maxime, deci în loc să maximizăm $C(\vec{z})$ vom minimiza:
...
Cu alte cuvinte, variabila $z_i$ va fi echivalentă cu o poartă $Z$ care acționează pe qubit-ul $i$. Mai mult:
...
Operatorul este o combinație liniară de termeni cu operatori Z pe noduri conectate printr-o muchie (reamintește-ți că qubit-ul 0 se află cel mai în dreapta): $IIZZ + IZIZ + IZZI + ZIIZ + ZZII$. Odată ce operatorul este construit, ansatz-ul pentru algoritmul QAOA poate fi ușor construit folosind circuitul `QAOAAnsatz` din biblioteca de circuite Qiskit.
