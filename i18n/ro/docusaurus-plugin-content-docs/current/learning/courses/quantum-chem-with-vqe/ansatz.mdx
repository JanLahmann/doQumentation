...
Vom începe cu un exemplu de circuit `efficient_su2` de patru qubiți cu două tipuri de porți SU(2), să spunem `rx` și `y`. De asemenea, specificăm o schemă de întrețesere și numărul de repetiții. Dacă pur și simplu faci `.draw()` circuitelor, vei obține o reprezentare destul de abstractă. O diagramă de circuit mai ușor de înțeles se obține folosind `.decompose().draw()`, iar aici vom folosi `output = "mpl"'.
