...
Înainte de efectuarea porții controlled-NOT, starea perechii $(\mathsf{W},\mathsf{X})$ este reprezentată de următoarea matrice densitate.
...
Următorul pas este efectuarea operației controlled-NOT, unde $\mathsf{X}$ este controlul iar $\mathsf{W}$ este ținta.
Ținând în continuare cont de convenția de ordonare Qiskit, reprezentarea matricială a acestei porți este următoarea.
...
Aceasta este o operație unitară, și pentru a o aplica unei matrice densitate conjugăm cu matricea unitară.
Conjugata-transpusă nu întâmplă să schimbe această matrice particulară, deci rezultatul este următorul.
...
În final, urma parțială este efectuată pe $\mathsf{W}$.
Amintindu-ne acțiunea acestei operații pe matrice $4\times 4$, care a fost descrisă în lecția anterioară, obținem următoarea matrice densitate rezultată.
...
Putem alternativ calcula urma parțială convertind mai întâi la notația Dirac.
...
Efectuarea urmei parțiale pe qubitul din partea stângă produce același răspuns ca înainte.
...
Un mod intuitiv de a gândi despre acest circuit este că operația controlled-NOT copiază efectiv starea clasică a qubitului de intrare, iar când copia este aruncată la gunoi qubitul de intrare „colapsează” probabilistic la una dintre cele două stări clasice posibile, ceea ce este echivalent cu defazarea completă.

### Canalul complet de defazare (alternativă) {#completely-dephasing-channel-alternative}

Circuitul descris mai sus nu este singurul mod de a implementa canalul de defazare completă.
Iată un mod diferit de a-l face.
...
După efectuarea porții Hadamard avem această stare de doi qubiți ca matrice densitate:
...
Poarta controlled-$\sigma_z$ funcționează prin conjugare astfel.
...
### Canalul de defazare completă {#completely-dephasing-channel-1}
...
### Canalul de resetare a qubitului {#qubit-reset-channel-1}
...
### Canalul de defazare completă {#completely-dephasing-channel-2}
...
### Canalul de depolarizare completă {#completely-depolarizing-channel-1}
...
### Canalul de resetare a qubitului {#qubit-reset-channel-2}
