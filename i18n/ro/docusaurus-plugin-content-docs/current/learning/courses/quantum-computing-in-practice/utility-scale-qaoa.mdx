...
sidebar_label: 'QAOA la scară de utilitate'
...
Începem cu un graf, care constă dintr-o colecție de vârfuri (sau noduri), dintre care unele sunt conectate prin muchii. În problemă, ni se cere să împărțim nodurile grafului în două subseturi „tăind” muchiile care le conectează. Vrem să găsim partiția care maximizează numărul de muchii tăiate în acest fel – de aici și numele, „max-cut”.
...
De exemplu, figura de mai sus arată un graf cu cinci noduri, cu o soluție max-cut în dreapta. Aceasta taie cinci muchii, ceea ce este cel mai bun rezultat posibil pentru acest graf.
...
Pentru a scrie problema QUBO ca Hamiltonianul Ising, tot ce trebuie să facem de fapt este o simplă schimbare de variabile din $x \in \{0, 1\}^n$ în $z \in \{-1, 1\}^n$:
...
<Accordion>
<AccordionItem title="Detalii">

Mai întâi, observă că putem rescrie expresia QUBO ca sumă de termeni matriceali:

$$
x^TQx = \sum_{ij}Q_{ij}x_ix_j
$$

Aplicând schimbarea de variabile, obținem
...
Putem elimina termenul constant deoarece acesta nu va afecta care $z$ minimizează problema QUBO. În plus, putem înmulți această expresie cu patru pentru a recupera valoarea originală a lui $Q$, deoarece nici aceasta nu afectează alegerea optimă a lui $z$.

$$
\sum_{ij}Q_{ij}z_iz_j - Q_{ij}z_i - Q_{ij}z_j = z^TQz + (-\sum_{ij} Q_{ij}z_i + Q_{ij}z_j)
$$

Grupând termenii liniari similari, putem defini vectorul de coeficienți liniari $b$ folosind ecuația

$$
b_i = -\sum_{j} Q_{ij} + Q_{ji}
$$

care finalizează transformarea variabilelor în $z \in \{-1, 1\}^n$ după substituirea în ecuația anterioară.
...
În cele din urmă, minimizarea expresiei QUBO este echivalentă cu minimizarea acestei expresii, unde $b$ este un coeficient scalar real:
...
Rescriind din nou ușor, obținem Hamiltonianul funcției de cost, unde minimul expresiei reprezintă starea fundamentală și `Z` este operatorul Pauli Z:
...
Deși Circuit-ul pe care tocmai l-am creat pare destul de simplu și este util pentru a construi o înțelegere intuitivă, ține minte că cipul cuantic nu înțelege ce este poartă-ul QAOA. Trebuie să transformăm aceasta într-o serie de poartă-uri native cu un singur qubit și cu doi qubiți, care pot fi executate direct pe hardware. Gate-urile native sunt cele care pot fi executate direct pe qubiți. Astfel de circuit-uri se spune că sunt scrise în Arhitectura Setului de Instrucțiuni (ISA) a Backend-ului.
...
Acum că ai avut o primă experiență cu procesul de rezolvare a unei probleme mici de max-cut pe un calculator cuantic, te provoc să o faci la scară. Urmărește [tutorialul](/tutorials/quantum-approximate-optimization-algorithm) legat pentru a vedea câte tăieturi poți obține într-un graf cu 125 de noduri.
