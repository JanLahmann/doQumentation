...
### Crearea unui Circuit pentru a rula pe Backend {#create-a-circuit-to-run-on-the-backend}
...
### Specificarea unui Backend {#specify-a-backend}
...
`cut_gates` va înlocui Gate-urile de la indicii specificați cu `TwoQubitQPDGate` și va returna, de asemenea, o listă de instanțe `QPDBasis` — câte una pentru fiecare descompunere de poartă.
...
### Generarea subexperimentelor pentru a rula pe Backend {#generate-the-subexperiments-to-run-on-the-backend}
...
## Pasul 3: Executare {#step-3-execute}

### Rularea subexperimentelor folosind primitiva Qiskit Runtime Sampler {#run-the-subexperiments-using-the-qiskit-runtime-sampler-primitive}
