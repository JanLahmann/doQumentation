...
Circuitul TwoLocal permite repetarea `rotation_blocks` și a `entangler map` de mai multe ori. În acest caz, numărul de repetiții determină numărul de porți periodice care trebuie tăiate. Deoarece overhead-ul de eșantionare crește exponențial cu numărul de tăieturi (vezi tutorialul [Wire cutting for expectation values estimation](/tutorials/wire-cutting) pentru mai multe detalii), vom fixa numărul de repetiții la 2 în acest notebook.
...
> **Notă:** Acest sondaj este realizat de **IBM Quantum** și acoperă **conținutul tutorialului** (scris de IBM). doQumentation oferă site-ul web, traducerile și execuția codului — pentru feedback despre acestea, te rugăm [să deschizi un issue pe GitHub](https://github.com/JanLahmann/doQumentation/issues).

[Link către sondaj](https://your.feedback.ibm.com/jfe/form/SV_3fQQYAIjTxvIChg)
