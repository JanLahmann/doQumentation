...
{/* doqumentation-source-hash: 14310d20 */}
...
După finalizarea acestui tutorial, vei fi învățat următoarele:
* Cum să implementezi o poartă CNOT pe distanță lungă folosind circuite dinamice cu măsurători mid-circuit (MCM) și feedforward clasic;
* Cum să implementezi poarta echivalentă folosind o abordare unitară bazată pe SWAP;
* Cum să compari ambele abordări prin măsurarea fidelității porții în funcție de distanța dintre qubiți.

## Cerințe preliminare {#prerequisites}

Sugerăm că utilizatorii să fie familiarizați cu următoarele subiecte înainte de a parcurge acest tutorial:
* [Concepte de bază ale calculului cuantic](/learning/courses/basics-of-quantum-information), inclusiv stările Bell, entanglement-ul și porțile cuantice;
* Familiarizare cu [circuitele dinamice](/guides/classical-feedforward-and-control-flow) (măsurători mid-circuit și feedforward clasic);
* Cunoștințe de bază despre [Qiskit SDK](https://docs.quantum.ibm.com/guides) și [IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime), și acces la un [cont IBM Quantum](/guides/cloud-setup).
...
În acest notebook, adaptăm protocolul pentru hardware-ul IBM Quantum și evaluăm performanța sa în funcție de separarea control–țintă, comparând-o cu o referință unitară bazată pe SWAP.
...
- Qiskit Runtime v0.37 sau mai recent (`pip install qiskit-ibm-runtime`)

- Qiskit Aer v0.17 sau mai recent (`pip install qiskit-aer`)
...
Înainte de a rula pe QPU-ul real, verificăm că atât circuitele dinamice, cât și cele unitare produc o stare Bell ideală pe un simulator fără zgomot. Folosim IBM Quantum `Sampler` cu `AerSimulator` ca mod de backend, la o distanță mică de 6.
...
#### Inițializarea circuitului {#initialize-circuit}
...
#### Pregătirea perechilor Bell {#prepare-bell-pairs}
...
#### Măsurarea perechilor de qubiți vecini în baza Bell {#measure-neighboring-qubit-pairs-in-the-bell-basis}
...
#### Aplicarea corecțiilor feedforward pentru a corecta operatorii Pauli secundari {#apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
#### Măsurarea qubiților de control și țintă {#measure-control-and-target-qubits}
...
Combinăm diferiții pași definiți mai sus pentru a crea o poartă CX pe distanță lungă la cele două capete ale unei linii unidimensionale (1D). Pașii includ următoarele:
- Inițializarea qubitului de control în $|+\rangle$
...
Acum construiește circuitele care măsoară în bazele $XX$, $YY$ și $ZZ$, exact cum am făcut pentru circuitele dinamice de mai sus.
...
Acum că am construit atât circuitele dinamice, cât și circuitele unitare pentru un exemplu la scară mică cu `distance=6`, le transpilăm pentru a rula mai întâi pe un simulator fără zgomot.
...
Putem acum rula experimentul pe backend-ul simulator fără zgomot. Folosim Qiskit Runtime Sampler cu AerSimulator ca mod de backend pentru a executa circuitele.
...
În acest pas, facem următoarele:
...
Într-o simulare fără zgomot, vom verifica că metrica de fidelitate este $1$ pentru circuitele construite. În experimentele pe QPU-urile reale, această analiză va oferi o imagine clară despre cât de bine performează circuitele dinamice față de implementarea unitară de referință.

#### Metrici de calitate {#quality-metrics}

Pentru a evalua succesul protocolului CX pe distanță lungă, măsurăm cât de apropiată este starea de ieșire de starea Bell ideală. O modalitate convenabilă de a cuantifica acest lucru este calcularea fidelității stării folosind valorile așteptate ale operatorilor Pauli. Putem calcula fidelitatea pentru o stare Bell pe starea de control și pe cea țintă după ce se cunosc $\braket{XX}$, $\braket{YY}$ și $\braket{ZZ}$. În particular,
...
Conform așteptărilor într-o simulare fără zgomot, fidelitățile atât pentru circuitele dinamice, cât și pentru cele unitare sunt $1$.

## Exemplu la scară mare pe hardware {#large-scale-hardware-example}

Acum reunim toate aceste detalii într-un singur flux de lucru la scară mai mare, care este apoi rulat pe hardware cuantic real.

### Generează circuite pentru distanțe diferite {#generate-circuits-for-different-distances}

Acum generăm circuite CX pe distanțe lungi pentru o gamă de separări între qubiți de până la 60 de qubiți distanță. Pentru fiecare distanță, construim circuite care măsoară în bazele $XX$, $YY$ și $ZZ$, care vor fi folosite ulterior pentru a calcula fidelitățile.

Lista de distanțe include atât separări scurte, cât și lungi, `distance = 0` corespunzând unui CX între vecini apropiați. Aceleași distanțe vor fi folosite și pentru a genera circuitele unitare corespunzătoare, ulterior, în scop de comparație.
...
Acum că avem atât circuite dinamice, cât și unitare pentru o gamă de distanțe, suntem gata pentru transpilare. Mai întâi trebuie să selectăm un dispozitiv backend.
...
Următorul pas se asigură că backend-ul suportă instrucțiunea `if_else`, care este necesară pentru versiunea mai nouă a circuitelor dinamice. Deoarece această funcționalitate se află încă în acces timpuriu, adăugăm explicit `IfElseOp` la ținta backend-ului dacă nu este deja disponibilă.
...
#### Folosește șirul Layer Fidelity pentru a selecta lanțul 1D {#use-layer-fidelity-string-for-selecting-1d-chain}
Deoarece vrem să comparăm performanța circuitelor dinamice și unitare pe un lanț 1D, folosim șirul Layer Fidelity pentru a selecta o topologie liniară a celui mai bun lanț de qubiți de pe dispozitiv. Acest lucru asigură că ambele tipuri de circuite sunt transpilate sub aceleași constrângeri de conectivitate, permițând o comparație echitabilă a performanței lor.
...
Dacă ai găsit acest material interesant, s-ar putea să fii interesat și de următoarele materiale:
