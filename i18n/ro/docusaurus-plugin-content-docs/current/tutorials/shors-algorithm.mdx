...
- Fundamentele matematice ale algoritmului lui Shor pentru factorizarea numerelor întregi
- Cum să ruleze un exemplu al acestui algoritm pe hardware
...
- [Estimarea fazei și factorizarea](/learning/courses/fundamentals-of-quantum-algorithms/phase-estimation-and-factoring/introduction). Acoperim o parte din acest material în acest tutorial.
...
Încheiem tutorialul cu o discuție despre alte demonstrații ale algoritmului lui Shor pe hardware real, atât implementări generice, cât și unele adaptate factorizării unor numere întregi specifice, precum 15 și 21.
Notă: Acest tutorial se concentrează mai degrabă pe implementarea și demonstrarea circuitelor legate de algoritmul lui Shor. Pentru o resursă educațională aprofundată, consultă cursul [Fundamentele algoritmilor cuantici](/learning/courses/fundamentals-of-quantum-algorithms/phase-estimation-and-factoring/introduction) al Dr. John Watrous și lucrările din secțiunea [Referințe](#references).
...
Să considerăm ce ar rezulta din circuit dacă pregătim starea computațională $\ket{1}$ ca stare inițială. Aceasta nu este o stare proprie a lui $M_a$, dar este superpoziția uniformă a stărilor proprii pe care tocmai le-am descris. Cu alte cuvinte, următoarea relație este valabilă. $$ \ket{1} = \frac{1}{\sqrt{r}} \sum^{r-1}_{k=0} \ket{\psi_k} $$
...
Putem face o pauză aici pentru a discuta un exemplu specific și a construi circuitul de găsire a ordinului pentru $N=15$. Reține că posibilele valori netriviale $a \in \mathbb{Z}_N^*$ pentru $N=15$ sunt $a \in \{2, 4, 7, 8, 11, 13, 14 \}$. Pentru acest exemplu, alegem $a=2$. Vom construi operatorul $M_2$ și operatorii de exponențiere modulară $M_2^k$.
...
Prin observație, putem vedea că stările bazei sunt amestecate, deci avem o matrice de permutare. Putem construi această operație pe patru qubiți cu porți swap. Mai jos, construim operațiile $M_2$ și $M_2$ controlat.
...
Porțile care acționează pe mai mult de doi qubiți vor fi în continuare descompuse în porți cu doi qubiți.
...
După cum putem vedea din lista valorilor $b$, pe lângă $M_2$ pe care l-am construit anterior, trebuie să construim și $M_4$ și $M_1$. Reține că $M_1$ acționează trivial asupra stărilor bazei computaționale, deci este pur și simplu operatorul identitate.
...
Acum suntem gata să construim circuitul complet de găsire a ordinului folosind operatorii de exponențiere modulară controlați definiți anterior. În codul următor, importăm și circuitul [QFT](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.QFT) din biblioteca de circuite Qiskit, care utilizează porți Hadamard pe fiecare qubit, o serie de porți controlate-U1 (sau Z, în funcție de fază) și un strat de porți swap.
...
Reține că am omis operațiile de exponențiere modulară controlate de la qubiții de control rămași, deoarece $M_1$ este operatorul identitate.
Reține că mai târziu în acest tutorial vom rula acest circuit pe backend-ul `ibm_marrakesh`. Pentru a face acest lucru, transpilăm circuitul conform acestui backend specific și raportăm adâncimea circuitului și numărul de porți.
...
După cum se poate vedea, am obținut aceleași șiruri de biți cu cele mai multe apariții (cele mai mari valori de contorizare). Deoarece hardware-ul cuantic are zgomot, există o scurgere către alte șiruri de biți, pe care le putem filtra statistic.
...
Până acum, am discutat cum putem implementa problema găsirii ordinului folosind un circuit de estimare a fazei. Acum, conectăm problema găsirii ordinului la factorizarea întregilor, ceea ce completează algoritmul lui Shor. Reține că această parte a algoritmului este clasică.
Demonstrăm acum aceasta folosind exemplul nostru cu $N = 15$ și $a = 2$. Reamintește-ți că faza pe care am măsurat-o este $k / r$, unde $$a^r \; (\textrm{mod} \; N) = 1$$ și $k$ este un întreg aleatoriu între $0$ și $r - 1$. Din această ecuație, avem $$(a^r - 1) \; (\textrm{mod} \; N) = 0,$$ ceea ce înseamnă că $N$ trebuie să dividă $a^r-1$. Dacă $r$ este de asemenea par, atunci putem scrie $$a^r -1 = (a^{r/2}-1)(a^{r/2}+1).$$ Dacă $r$ nu este par, nu putem merge mai departe și trebuie să încercăm din nou cu o altă valoare pentru $a$; altfel, există o probabilitate mare ca cel mai mare divizor comun al lui $N$ și fie $a^{r/2}-1$, fie $a^{r/2}+1$ să fie un factor propriu al lui $N$.
