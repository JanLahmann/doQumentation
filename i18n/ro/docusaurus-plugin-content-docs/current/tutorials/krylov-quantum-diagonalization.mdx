...
{/* doqumentation-source-hash: 90a60dd2 */}
...
unde primul pas rezultă din faptul că $E_k-E_0\le\delta$ pentru fiecare $E_k$ din sumă, iar al doilea pas rezultă din faptul că suma din numărător este un subset al sumei din numitor. Pentru al doilea termen, mărginim inferior numitorul prin $|\gamma_0|^2$, deoarece $f(E_0)^2=1$: adunând totul înapoi, obținem
...
Aceasta este valabilă pentru orice $\delta>0$, deci dacă setăm $\delta$ egal cu eroarea noastră țintă, atunci marginea de eroare de mai sus converge exponențial spre aceasta cu dimensiunea Krylov $2d=r$. De asemenea, reține că dacă $\delta<E_1-E_0$ atunci termenul $\delta$ dispare complet în marginea de mai sus.
...
O analiză similară cu cea de mai sus poate fi efectuată ținând cont suplimentar de zgomot și de procedura de prag discutată în notebook. Consultă [\[2\]](#references) și [\[4\]](#references) pentru această analiză.
