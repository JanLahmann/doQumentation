...
description: "Implementează algoritmul de diagonalizare cuantică Krylov (KQD) în contextul tiparelor Qiskit."
...
În multe discipline, suntem interesați să cunoaștem proprietățile stării fundamentale ale sistemelor cuantice. Exemple includ înțelegerea naturii fundamentale a particulelor și forțelor, predicția și înțelegerea comportamentului materialelor complexe, precum și înțelegerea interacțiunilor și reacțiilor biochimice. Din cauza creșterii exponențiale a spațiului Hilbert și a corelațiilor care apar în sistemele entanglate, algoritmii clasici se confruntă cu dificultăți în rezolvarea acestei probleme pentru sisteme cuantice de dimensiuni tot mai mari. La un capăt al spectrului se află abordarea existentă care valorifică hardware-ul cuantic, concentrată pe metodele cuantice variaționale (de exemplu, [eigensolver-ul cuantic variațional](/tutorials/spin-chain-vqe)). Aceste tehnici se confruntă cu provocări pe dispozitivele actuale din cauza numărului mare de apeluri de funcție necesare în procesul de optimizare, ceea ce adaugă un supracost mare de resurse odată ce sunt introduse tehnici avansate de mitigare a erorilor, limitând astfel eficacitatea lor la sisteme mici. La celălalt capăt al spectrului se află metodele cuantice tolerante la defecte cu garanții de performanță (de exemplu, [estimarea fazei cuantice](https://arxiv.org/abs/quant-ph/0604193)), care necesită circuite adânci ce pot fi executate doar pe un dispozitiv tolerant la defecte. Din aceste motive, introducem aici un algoritm cuantic bazat pe metode de subspațiu (descris în această [lucrare de sinteză](https://arxiv.org/abs/2312.00178)), algoritmul de diagonalizare cuantică Krylov (KQD). Acest algoritm funcționează bine la scară largă [\[1\]](#references) pe hardware-ul cuantic existent, are [garanții de performanță](https://arxiv.org/abs/2110.07492) similare cu estimarea fazei, este compatibil cu tehnici avansate de mitigare a erorilor și ar putea furniza rezultate inaccesibile pe cale clasică.

## Cerințe {#requirements}

Înainte de a începe acest tutorial, asigură-te că ai instalate următoarele:

- Qiskit SDK v2.0 sau versiune ulterioară, cu suport pentru [vizualizare](https://docs.quantum.ibm.com/api/qiskit/visualization)
...
### Spațiul Krylov {#the-krylov-space}

Spațiul Krylov $\mathcal{K}^r$ de ordin $r$ este spațiul generat de vectorii obținuți prin înmulțirea puterilor mai mari ale unei matrice $A$, până la $r-1$, cu un vector de referință $\vert v \rangle$.

$$
\mathcal{K}^r = \left\{ \vert v \rangle, A \vert v \rangle, A^2 \vert v \rangle, ..., A^{r-1} \vert v \rangle \right\}
$$

Dacă matricea $A$ este Hamiltonianul $H$, vom numi spațiul corespunzător spațiul Krylov al puterilor $\mathcal{K}_P$. În cazul în care $A$ este operatorul de evoluție temporală generat de Hamiltonian $U=e^{-iHt}$, vom numi spațiul spațiu Krylov unitar $\mathcal{K}_U$. Subspațiul Krylov al puterilor pe care îl folosim clasic nu poate fi generat direct pe un calculator cuantic, deoarece $H$ nu este un operator unitar. În schimb, putem folosi operatorul de evoluție temporală $U = e^{-iHt}$, care poate fi demonstrat că oferă [garanții de convergență](https://arxiv.org/abs/2110.07492) similare cu metoda puterilor. Puterile lui $U$ devin atunci diferiți pași de timp $U^k = e^{-iH(kt)}$.
...
### Algoritmul de diagonalizare cuantică Krylov {#krylov-quantum-diagonalization-algorithm}

Dat un Hamiltonian $H$ pe care dorim să-l diagonalizăm, considerăm mai întâi spațiul Krylov unitar corespunzător $\mathcal{K}_U$. Scopul este de a găsi o reprezentare compactă a Hamiltonianului în $\mathcal{K}_U$, pe care o vom numi $\tilde{H}$. Elementele de matrice ale $\tilde{H}$, proiecția Hamiltonianului în spațiul Krylov, pot fi calculate prin calcularea valorilor de așteptare următoare
...
Unde $\vert \psi_n \rangle = e^{-i H t_n} \vert \psi \rangle$ sunt vectorii spațiului Krylov unitar și $t_n = n dt$ sunt multiplii pasului de timp $dt$ ales. Pe un calculator cuantic, calculul fiecărui element de matrice poate fi realizat cu orice algoritm care permite obținerea suprapunerii dintre stările cuantice. Acest tutorial se concentrează pe testul Hadamard. Dat că $\mathcal{K}_U$ are dimensiunea $r$, Hamiltonianul proiectat în subspațiu va avea dimensiunile $r \times r$. Cu $r$ suficient de mic (în general $r<<100$ este suficient pentru a obține convergența estimărilor valorilor proprii) putem apoi să diagonalizăm ușor Hamiltonianul proiectat $\tilde{H}$. Cu toate acestea, nu putem diagonaliza direct $\tilde{H}$ din cauza ne-ortogonalității vectorilor spațiului Krylov. Va trebui să măsurăm suprapunerile acestora și să construim o matrice $\tilde{S}$
...
Se pot obține astfel estimări ale valorilor proprii și stărilor proprii ale $H$ examinând valorile proprii ale $\tilde{H}$. De exemplu, estimarea energiei stării fundamentale se obține luând cea mai mică valoare proprie $c$, iar starea fundamentală din vectorul propriu corespunzător $\vec{c}$. Coeficienții din $\vec{c}$ determină contribuția diferiților vectori care generează $\mathcal{K}_U$.

![fig1.png](/docs/images/tutorials/krylov-subspace-diagonalization/fc662b76-8ad7-4a6c-8c49-5f08c125aee8.avif)

Figura prezintă o reprezentare circuitară a testului Hadamard modificat, o metodă folosită pentru a calcula suprapunerea dintre diferite stări cuantice. Pentru fiecare element de matrice $\tilde{H}_{i,j}$, se efectuează un test Hadamard între stările $\vert \psi_i \rangle$, $\vert \psi_j \rangle$. Aceasta este evidențiată în figură prin schema de culori pentru elementele de matrice și operațiile corespunzătoare $\text{Prep} \; \psi_i$, $\text{Prep} \; \psi_j$. Astfel, este necesar un set de teste Hadamard pentru toate combinațiile posibile de vectori ai spațiului Krylov pentru a calcula toate elementele de matrice ale Hamiltonianului proiectat $\tilde{H}$. Firul de sus din circuitul testului Hadamard este un Qubit ansilă care este măsurat fie în baza X, fie în baza Y; valoarea sa de așteptare determină valoarea suprapunerii dintre stări. Firul de jos reprezintă toți Qubiții Hamiltonianului sistemului. Operația $\text{Prep} \; \psi_i$ pregătește Qubitul sistemului în starea $\vert \psi_i \rangle$ controlată de starea Qubitului ansilă (similar pentru $\text{Prep} \; \psi_j$), iar operația $P$ reprezintă descompunerea Pauli a Hamiltonianului sistemului $H = \sum_i P_i$. O derivare mai detaliată a operațiilor calculate de testul Hadamard este prezentată mai jos.

#### Definirea Hamiltonianului {#define-hamiltonian}

Să considerăm Hamiltonianul Heisenberg pentru $N$ Qubiți pe un lanț liniar: $H= \sum_{i,j}^N X_i X_j + Y_i Y_j - J Z_i Z_j$
...
#### Stabilirea parametrilor algoritmului {#set-parameters-for-the-algorithm}

Alegem euristic o valoare pentru pasul de timp `dt` (bazată pe limite superioare ale normei Hamiltonianului). Ref [\[2\]](#references) a arătat că un pas de timp suficient de mic este $\pi/\vert \vert H \vert \vert$, și că este preferabil, până la un punct, să subestimăm această valoare mai degrabă decât să o supraestimăm, deoarece supraestimarea poate permite contribuțiilor din stările de energie înaltă să corupă chiar și starea optimă din spațiul Krylov. Pe de altă parte, alegerea unui $dt$ prea mic duce la un condiționament mai slab al subspațiului Krylov, deoarece vectorii bazei Krylov diferă mai puțin de la un pas de timp la altul.
...
Alege o stare de referință $\vert \psi \rangle$ care are o suprapunere cu starea fundamentală. Pentru acest Hamiltonian, folosim o stare cu o excitație în Qubitul din mijloc $\vert 00..010...00 \rangle$ ca stare de referință.
...
Putem realiza operatorul de evoluție temporală generat de un Hamiltonian dat: $U=e^{-iHt}$ prin [aproximarea Lie-Trotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter).
...
Unde $P$ este unul dintre termenii din descompunerea Hamiltonianului $H=\sum P$, iar $\text{Prep} \; \psi_i$, $\text{Prep} \; \psi_j$ sunt operații controlate care pregătesc vectorii $|\psi_i\rangle$, $|\psi_j\rangle$ ai spațiului Krylov unitar, cu $|\psi_k\rangle = e^{-i H k dt } \vert \psi \rangle = e^{-i H k dt } U_{\psi} \vert 0 \rangle^N$. Pentru a măsura $X$, se aplică mai întâi $H$...
...
Din identitatea $|a + b\|^2 = \langle a + b | a + b \rangle = \|a\|^2 + \|b\|^2 + 2\text{Re}\langle a | b \rangle$. Similar, măsurarea $Y$ produce
...
### Test Hadamard eficient {#efficient-hadamard-test}
...
Să presupunem că putem calcula clasic $E_0$, valoarea proprie a lui $|0\rangle^N$ sub Hamiltonianul $H$.
Această condiție este satisfăcută atunci când Hamiltonianul păstrează simetria U(1). Deși poate părea o ipoteză puternică, există multe cazuri în care este sigur să presupunem că există o stare de vid (în acest caz aceasta corespunde stării $|0\rangle^N$) care nu este afectată de acțiunea Hamiltonianului. Acest lucru este adevărat, de exemplu, pentru Hamiltonieni de chimie care descriu molecule stabile (unde numărul de electroni este conservat).
Dat fiind că poarta $\text{Prep} \; \psi$ pregătește starea de referință dorită $\ket{psi} = \text{Prep} \; \psi \ket{0} = e^{-i H 0 dt} U_{\psi} \ket{0}$, de exemplu, pentru a pregăti starea HF în chimie, $\text{Prep} \; \psi$ ar fi un produs de operații NOT pe qubiți individuali, deci $\text{Prep} \; \psi$ controlată este doar un produs de porți CNOT.
Astfel, circuitul de mai sus implementează următoarea stare înainte de măsurare:
...
unde am folosit deplasarea de fază calculabilă clasic $ U\ket{0}^N = e^{i\phi}\ket{0}^N$ pe a treia linie. Prin urmare, valorile de așteptare se obțin ca
...
Folosind aceste ipoteze am reușit să scriem valorile de așteptare ale operatorilor de interes cu mai puține operații controlate. De fapt, trebuie să implementăm doar pregătirea controlată a stării $\text{Prep} \; \psi$, nu și evoluțiile temporale controlate. Reformulând calculul nostru ca mai sus, vom putea reduce considerabil adâncimea circuitelor rezultante.
...
În loc să implementăm operatorul de evoluție temporală exact, putem folosi descompunerea Trotter pentru a implementa o aproximație a acestuia. Repetând de mai multe ori o descompunere Trotter de un anumit ordin obținem o reducere suplimentară a erorii introduse de aproximație. În continuare, construim direct implementarea Trotter în cel mai eficient mod pentru graful de interacțiune al Hamiltonianului pe care îl considerăm (numai interacțiuni între vecini direcți). În practică, inserăm rotații Pauli $R_{xx}$, $R_{yy}$, $R_{zz}$ cu un unghi parametrizat $t$, care corespund implementării aproximative a lui $e^{-i (XX + YY + ZZ) t}$. Dată diferența dintre definiția rotațiilor Pauli și evoluția temporală pe care încercăm să o implementăm, va trebui să folosim parametrul $2*dt$ pentru a obține o evoluție temporală de $dt$. În plus, inversăm ordinea operațiilor pentru un număr impar de repetări ale pașilor Trotter, ceea ce este funcțional echivalent, dar permite sinteza operațiilor adiacente într-un singur unitar $SU(2)$. Aceasta produce un circuit mult mai puțin adânc decât cel obținut cu funcționalitatea generică `PauliEvolutionGate()`.
...
### Folosește un circuit optimizat pentru pregătirea stării {#use-an-optimized-circuit-for-state-preparation}
...
### Circuite șablon pentru calculul elementelor de matrice ale $\tilde{S}$ și $\tilde{H}$ prin testul Hadamard {#template-circuits-for-calculating-matrix-elements-of-tilde-s-and-tilde-h-via-hadamard-test}
...
### Transpilare către un QPU {#transpiling-to-a-qpu}
...
### Creează PUB-uri pentru execuție cu Estimator {#create-pubs-for-execution-with-estimator}
...
```
### Rularea circuitelor {#run-circuits}

Circuitele pentru $t=0$ sunt calculabile clasic
...
Execută circuitele pentru $S$ și $\tilde{H}$ cu Estimator
...
### Calcularea matricelor Hamiltonianului efectiv și de suprapunere {#calculate-effective-hamiltonian-and-overlap-matrices}
...
Odată ce avem rezultatele execuțiilor circuitelor, putem post-procesa datele pentru a calcula elementele matricei $S$
...
![Rezultatul celulei de cod anterioare](/docs/images/tutorials/krylov-quantum-diagonalization/extracted-outputs/4bc52594-0376-497f-8a61-0949415a1fe0-0.avif)

## Anexă: Subspațiul Krylov din evoluții în timp real {#appendix-krylov-subspace-from-real-time-evolutions}
...
adică acolo unde toate evoluțiile temporale sunt deplasate înapoi cu $d$ pași de timp.
Motivul pentru care este indistinguibil constă în faptul că elementele de matrice

$$
\tilde{H}_{j,k} = \langle\psi|e^{i\,j\,H\,dt}He^{-i\,k\,H\,dt}|\psi\rangle=\langle\psi|He^{i(j-k)H\,dt}|\psi\rangle
$$

sunt invariante la deplasări globale ale timpului de evoluție, deoarece evoluțiile temporale comutează cu Hamiltonianul. Pentru $r$ impar, putem folosi analiza pentru $r-1$.

Dorim să arătăm că undeva în acest spațiu Krylov există garantat o stare de energie scăzută. Facem asta prin intermediul următorului rezultat, derivat din Teorema 3.1 din [\[3\]](#references):
...
1. $f(E_0)=1$
2. $|f(E)|\le2\left(1 + \delta\right)^{-d}$ pentru toate valorile $E$ aflate la distanță $\ge\delta$ față de $E_0$, adică este suprimată exponențial
3. $f(E)$ este o combinație liniară de $e^{ijE\,dt}$ pentru $j=-d,-d+1,...,d-1,d$

Oferim mai jos o demonstrație, care poate fi sărită în siguranță dacă nu dorim să înțelegem argumentul complet și riguros. Acum ne concentrăm pe implicațiile afirmației de mai sus. Prin proprietatea 3 de mai sus, putem vedea că spațiul Krylov deplasat de mai sus conține starea $f(H)|\psi\rangle$. Aceasta este starea noastră de energie scăzută. Pentru a înțelege de ce, scriem $|\psi\rangle$ în baza proprie de energie:

$$
|\psi\rangle = \sum_{k=0}^{N}\gamma_k|E_k\rangle,
$$

unde $|E_k\rangle$ este cea de-a k-a stare proprie de energie și $\gamma_k$ este amplitudinea sa în starea inițială $|\psi\rangle$. Exprimat în acești termeni, $f(H)|\psi\rangle$ este dat de

$$
f(H)|\psi\rangle = \sum_{k=0}^{N}\gamma_kf(E_k)|E_k\rangle,
$$

folosind faptul că putem înlocui $H$ cu $E_k$ atunci când acționează asupra stării proprii $|E_k\rangle$. Eroarea de energie a acestei stări este, prin urmare,
...
unde primul pas urmează deoarece $E_k-E_0\le\delta$ pentru fiecare $E_k$ din sumă, iar al doilea pas urmează deoarece suma din numărător este un subset al sumei din numitor. Pentru al doilea termen, mărginim inferior numitorul prin $|\gamma_0|^2$, deoarece $f(E_0)^2=1$: adunând totul înapoi, obținem

$$
\text{energy error} \le \delta + \frac{1}{|\gamma_0|^2}\sum_{E_k>E_0+\delta}|\gamma_k|^2f(E_k)^2(E_k-E_0).
$$

Pentru a simplifica restul, observăm că pentru toți acești $E_k$, prin definiția lui $f$ știm că $f(E_k)^2 \le 4\left(1 + \delta\right)^{-2d}$. Mărginind suplimentar $E_k-E_0<2\|H\|$ și mărginind $\sum_{E_k>E_0+\delta}|\gamma_k|^2<1$ obținem

$$
\text{energy error} \le \delta + \frac{8}{|\gamma_0|^2}\|H\|\left(1 + \delta\right)^{-2d}.
$$

Aceasta este valabilă pentru orice $\delta>0$, deci dacă setăm $\delta$ egal cu eroarea noastră țintă, atunci marginea de eroare de mai sus converge exponențial spre aceasta cu dimensiunea Krylov $2d=r$. De asemenea, rețineți că dacă $\delta<E_1-E_0$ atunci termenul $\delta$ dispare complet în marginea de mai sus.

Pentru a completa argumentul, menționăm că cele de mai sus reprezintă doar eroarea de energie a stării particulare $f(H)|\psi\rangle$, nu eroarea de energie a stării cu energia cea mai mică din spațiul Krylov. Cu toate acestea, prin principiul variațional (Rayleigh-Ritz), eroarea de energie a stării cu energia cea mai mică din spațiul Krylov este mărginită superior de eroarea de energie a oricărei stări din spațiul Krylov, deci cele de mai sus reprezintă și o margine superioară pentru eroarea de energie a stării cu energia cea mai mică, adică rezultatul algoritmului de diagonalizare cuantică Krylov.

O analiză similară cu cea de mai sus poate fi efectuată ținând cont suplimentar de zgomot și de procedura de prag discutată în notebook. Consultați [\[2\]](#references) și [\[4\]](#references) pentru această analiză.

## Anexă: demonstrația Afirmației 1 {#appendix-proof-of-claim-1}

Următoarele sunt derivate în mare parte din [\[3\]](#references), Teorema 3.1: fie $ 0 < a < b $ și fie $ \Pi^*_d $ spațiul polinoamelor reziduale (polinoame a căror valoare în 0 este 1) de grad cel mult $ d $. Soluția la
...
Dorim să convertim aceasta într-o funcție care poate fi exprimată natural în termeni de exponențiale complexe, deoarece acestea sunt evoluțiile în timp real care generează spațiul cuantic Krylov.
...
Observăm că $g(E_0)=0$ și $g(E)$ crește pe măsură ce $E$ se îndepărtează de $E_0$.
