---
description: 'Învață diagonalizarea cuantică Krylov pentru Hamiltonienii de rețea cu conceptul de funcție Hamiltoniană.'
notebook_path: tutorials/krylov-quantum-diagonalization.ipynb
sidebar_label: 'Diagonalizarea cuantică Krylov a Hamiltonienilor de rețea'
title: 'Diagonalizarea cuantică Krylov a Hamiltonienilor de rețea'
---
...
Se recomandă să te familiarizezi cu următoarele subiecte:
- [Primitivele Qiskit](/guides/primitives)
- [Estimarea energiei stării fundamentale a lanțului Heisenberg cu VQE](/tutorials/spin-chain-vqe)
- [Cursul de algoritmi de diagonalizare cuantică](/learning/courses/quantum-diagonalization-algorithms/krylov)
...
Estimarea proprietăților de energie joasă ale Hamiltonienilor cu mai multe corpuri este o sarcină centrală în simularea cuantică.
De exemplu, energiile stării fundamentale și excitațiile de energie joasă sunt legate direct de stabilitatea chimică, ordinea magnetică, tranzițiile de fază cuantice și răspunsul materialelor.
Pe un computer clasic, dimensiunea spațiului Hilbert crește exponențial cu numărul de orbitali sau spinuri, astfel încât diagonalizarea directă devine rapid impracticabilă.

Există mai multe abordări de calcul cuantic pentru această problemă.
Metodele variaționale pe termen scurt, cum ar fi [rezolvatorul variațional cuantic de valori proprii (VQE)](/tutorials/spin-chain-vqe), utilizează circuite parametrizate relativ puțin adânci, dar necesită o buclă de optimizare clasică neliniară cu multe evaluări de circuite cuantice.
La cealaltă extremă, [estimarea cuantică a fazei (QPE)](https://arxiv.org/abs/quant-ph/0604193) oferă o rută mai directă către estimarea valorilor proprii cu garanții riguroase, dar QPE standard necesită circuite coerente lungi și este potrivit în principal pentru computere cuantice tolerante la erori.
KQD se situează între aceste două abordări: folosește evoluția Hamiltoniană în timp real, ca în algoritmii bazați pe estimarea fazei, dar înlocuiește estimarea completă a fazei cu o problemă compactă de valori proprii proiectată, care poate fi rezolvată clasic.

Considerăm un Hamiltonian de $n$ qubiți $H$ și o stare de referință $\lvert \psi_{0}\rangle$.
Metoda KQD construiește un subspațiu Krylov din stări evoluate în timp real,
...
unde $r$ este dimensiunea Krylov iar $\Delta t$ este pasul de timp.
Orice stare din subspațiul Krylov este apoi reprezentată ca o combinație liniară a acestor stări de bază,
...
Aici, matricele $\mathcal{S}$ și $\mathcal{H}$,
...
definesc matricele de suprapunere proiectată și cea Hamiltoniană proiectată.
Elementele lor sunt estimate folosind măsurători de circuite cuantice.

Ne propunem să găsim coeficientul $\mathbf{c}$ care dă minimul $E(\mathbf{c})$:
...
Rețineți că dimensiunea $r$ poate fi suficient de mică pentru ca un computer clasic să rezolve GEVP.

Acesta este același principiu variațional folosit în diagonalizarea subspațiilor clasice, dar aici stările de bază sunt generate prin evoluție cuantică în timp.
Comparativ cu VQE, KQD necesită de obicei circuite mai adânci deoarece se bazează pe evoluția în timp real.
În schimb, KQD evită optimizarea neliniară a parametrilor și execuția iterativă pe hardware cuantic, și se îmbunătățește sistematic pe măsură ce subspațiul proiectat este extins.
Algoritmul a fost demonstrat la scară largă pe hardware cuantic existent [\[2\]](#references), iar performanța sa poate fi analizată cu garanții demonstrabile [\[1\]](#references).

## Cerințe {#requirements}

Înainte de a începe acest tutorial, asigură-te că ai instalate următoarele:

- Qiskit SDK v2.3 sau versiune ulterioară cu suport pentru [vizualizare](https://docs.quantum.ibm.com/api/qiskit/visualization)
...
Execuția pe hardware necesită `qiskit-ibm-runtime` și acces la un cont IBM Quantum&reg;.

## Configurare {#setup}

Celula de configurare importă modulele necesare și definește funcții ajutătoare pentru flux de lucru:

1. construiește Hamiltonianul Heisenberg;

2. rezolvă GEVP-ul prin prag;

3. evaluează filtrul Krylov învățat;

4. convertește valorile filtrului în ponderi spectrale;

5. reprezintă grafic distribuțiile de energie de referință și filtrată.
...
#### Hamiltonianul și starea de referință {#hamiltonian-and-reference-state}
Acest exemplu folosește un lanț Heisenberg cu 12 qubiți și margini deschise ($n=12$),
...
#### Stabilirea parametrilor algoritmului {#set-the-parameters-for-the-algorithm}

Pe baza limitelor superioare ale normei Hamiltoniene, Ref. [\[1\]](#references) sugerează euristic pasul de timp $\Delta t$ ca $\pi/\|H\|$.
Deoarece norma spectrală $\|H\|$ este greu de calculat, folosim în schimb limita ei superioară:
...
Setăm dimensiunea Krylov la $r=10$ și numărul de pași Trotter per pas de timp la $5$: un spațiu Krylov suficient de mare pentru a rezolva spectrul de energie joasă, menținând totodată circuitul cel mai adânc ($t_{\max}=(r-1)\Delta t$) accesibil, și suficienți pași Trotter pentru a menține mică eroarea de discretizare la acel circuit cel mai adânc.
...
#### Construirea circuitului {#build-circuit}

Aici, construim circuitele pentru a estima elementele matriceale $\mathcal{H}_{k\ell}$ și $\mathcal{S}_{k\ell}$.
Deoarece toate puterile lui $H$ comută, avem
$$
\begin{equation*}
\mathcal{H}_{k\ell}
=
...
\end{equation*}
$$
unde $|\psi_d\rangle=e^{-id\Delta tH}|\psi_{0}\rangle$.

#### Starea de referință {#reference-state}

Pregătim starea de referință $|\psi_0\rangle$.
...
Realizăm operatorul de evoluție temporală generat de Hamiltonian, aproximat prin simpla [Lie-Trotterizare](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter).
...
#### Testul de swap extins [\[3\]](#references) {#extended-swap-test-circuit-3references}

Circuitul pregătește mai întâi starea de referință pe registrul de sistem, în timp ce ancila rămâne în $|0\rangle$:
...
doar când ancila se află în ramura $|1\rangle$. Prin urmare,
...
\end{equation*}
$$
care va fi atribuit ca $t=d\Delta t$ pentru $d=1,\cdots r-1$ în pasul de execuție.
...
Pentru orice observabilă hermitiană de sistem $O$, stabilim aici observabilele de calculat:
...
Aceasta se întâmplă deoarece $O=I$ dă elementul de suprapunere $\mathcal{S}_{0d}=\langle\psi_0|\psi_d\rangle$, în timp ce $O=H$ dă elementul Hamiltonian $\mathcal{H}_{0d}=\langle\psi_0|H|\psi_d\rangle$.

Folosind $X=|0\rangle\langle 1|+|1\rangle\langle 0|$, avem
...
În mod similar, folosind $Y=-i|0\rangle\langle 1|+i|1\rangle\langle 0|$,
...
În final, pentru fiecare stare $|\Psi_{0d}\rangle$, trebuie să măsurăm:
$$
\begin{align*}
X \otimes I \text{ pentru } \operatorname{Re}\mathcal{S}_{0d},\\
Y \otimes I \text{ pentru } \operatorname{Im}\mathcal{S}_{0d},\\
X \otimes H \text{ pentru } \operatorname{Re}\mathcal{H}_{0d},\\
Y \otimes H \text{ pentru } \operatorname{Im}\mathcal{H}_{0d}.\\
\end{align*}
$$
...
Pentru estimarea elementelor matriceale $\mathcal{H}$, numărul de termeni Pauli este mult mai mare decât cel pentru elementele matriceale $\mathcal{S}$.

Putem acum reduce numărul de termeni Hamiltonieni măsurați folosind o tehnică de deplasare [\[4\]](#references).
Împărțim Hamiltonianul astfel:
...
unde $T$ este ales astfel încât starea de referință să fie starea sa proprie,
...
este elementul matriceal Hamiltonian deplasat.
Prin urmare, trebuie doar să măsurăm $X\otimes (H-T)$ și $Y\otimes (H-T)$.
Contribuția de la $T$ este reconstruită clasic folosind elementul matriceal de suprapunere deja măsurat $\mathcal{S}_{0d}$.
...
Deoarece starea de referință este o stare de bază computațională, ea este o stare proprie pentru fiecare termen $Z_iZ_{i+1}$.

Totuși, o alegere mai avantajoasă este să includem nu doar termenii diagonali $ZZ$, ci și termenii $XX+YY$ care anulează starea de referință.

Pentru fiecare pereche vecină, operatorul $XX+YY$ satisface
...
Prin urmare, termenul $XX+YY$ contribuie doar atunci când cei doi qubiți vecini au ocupări diferite în șirul de biți de referință.
Dacă ambii qubiți sunt fie $0$, fie $1$, termenul anulează starea de referință și poate fi de asemenea deplasat în afară.

Fie $|\psi_{\rm ref}\rangle = |z_0\cdots z_{n-1}\rangle$, unde $z_i \in \{0,1\}$.
Astfel, putem alege
$$
\begin{equation*}
...
deoarece termenii $ZZ$ acționează diagonal pe $|\psi_0\rangle$, în timp ce termenii deplasați $XX+YY$ dau zero.
Valoarea proprie corespunzătoare este așadar determinată doar de termenii $ZZ$,
$$
\begin{equation*}
\tau = \sum_{i=0}^{n-2}
(-1)^{z_i}
...
Ca rezultat, trebuie măsurate doar muchiile cu ocupări diferite în starea de referință.
Toți termenii $ZZ$ și toți termenii $XX+YY$ inactivi sunt reconstruiți prin contribuția de suprapunere $\tau \mathcal{S}_{0d}$, sau dau o contribuție zero prin construcție.

Aceasta oferă o observabilă mai mică decât deplasarea doar a părții diagonale.
În special, pentru o stare de referință de bază computațională cu o excitație localizată, doar muchiile adiacente excitației rămân în $H-T$.
Prin urmare, numărul de termeni Pauli din $X\otimes(H-T)$ și $Y\otimes(H-T)$ poate fi redus substanțial, în timp ce elementul matriceal reconstruit
$$
\begin{equation*}
\widetilde{\mathcal{H}}_{0d}
+
...
Acum transformăm circuitul abstract extended-swap-test într-un șablon orientat pe hardware.
Înainte de asta, optimizăm mai întâi circuitul suplimentar la nivel abstract.

#### Compararea ordinii termenilor Hamiltonieni {#compare-hamiltonian-term-ordering}

Mai întâi, comparăm ordini diferite ale termenilor Pauli din Hamiltonianul Heisenberg pentru simularea Hamiltoniană.
Hamiltonianul în sine rămâne neschimbat, dar ordinea afectează modul în care este generat circuitul cu formulă produs și cât de mult poate fi paralelizată structura în circuit.
De exemplu, ordinea naivă listează mai întâi toți termenii $XX$ ai celor mai apropiați vecini, apoi toți termenii $YY$, apoi toți termenii $ZZ$.
Aceasta plasează muchii adiacente precum $(0,1)$ și $(1,2)$ una lângă alta, deci nu pot fi executate în paralel.
Ordinea pară-apoi-impară vizitează mai întâi muchiile pare disjuncte, urmate de cele impare, ceea ce expune straturi paralele cu doi qubiți.
Ordinea grupată pe muchii pară-impară merge un pas mai departe: pentru fiecare muchie, păstrează împreună termenii locali $XX$, $YY$ și $ZZ$, vizitând totuși muchiile pare înaintea celor impare.
Ne așteptăm ca ordinile pară-apoi-impară și grupată pe muchii pară-impară să reducă adâncimea circuitului expunând straturi paralele cu doi qubiți, iar ordinea grupată pe muchii să reducă suplimentar eroarea Trotter deoarece interacțiunea locală de doi qubiți pe aceeași muchie este tratată ca un bloc compact.

![Pauli evolutions with different term ordering](/docs/images/tutorials/krylov-quantum-diagonalization/ham_ordering.avif)

Ordinea Hamiltonianului afectează de asemenea eroarea Trotter.
Dacă plasăm termeni care nu comută unul lângă altul, tranzițiile de bază apar mai des, ceea ce induce mai multă eroare Trotter.
Grupând termenii care necesită aceeași transformare de bază Pauli, se pot evita schimbările de bază redundante.

Aici, comparația folosește cel mai mare timp de evoluție care apare în estimările Krylov din primul rând,
$t_{\max} = (r-1)\Delta t,$
folosind aceeași condiție de transpilare.
...
Mai întâi menținem regula de sinteză fixată la un singur pas Trotter de ordinul întâi și variem doar ordinea termenilor Pauli.
Obiectivul acestei comparații este în principal să vedem cât de mult pot fi reduse adâncimea circuitului și costul de doi qubiți prin expunerea muchiilor disjuncte ale celor mai apropiați vecini către transpiler.
...
#### Compararea sintezei prin formulă produs {#compare-product-formula-synthesis}

În continuare, explorăm diferite setări avansate ale Trotterizării, cu ordinea Hamiltoniană fixată la ordinea grupată pe muchii pară-impară.
Considerăm Lie-Trotter de ordinul întâi, Suzuki-Trotter de ordinul doi și Suzuki-Trotter de ordinul patru.
...
Lie-Trotter oferă cel mai puțin adânc circuit, dar are cea mai mare eroare, în timp ce Suzuki-Trotter de ordinul patru este mai precis, dar crește adâncimea circuitului.
Pentru restul tutorialului, alegem Suzuki-Trotter de ordinul doi deoarece oferă un circuit cu adâncime mică, reducând totodată substanțial eroarea Trotter comparativ cu formula de ordinul întâi.

#### Eliminarea porții controlate de evoluție temporală {#remove-the-controlled-time-evolution-gate}

În testul de swap extins, controlarea evoluției temporale cu un singur qubit ancilă necesită ca ancila să controleze multe porți pe tot sistemul.
Aceasta poate introduce o supraîncărcare substanțială de rutare și, în cel mai rău caz, necesită practic conectivitate de la toate la unul.
Pentru a evita acest lucru, este posibilă o optimizare suplimentară prin înlocuirea porții controlate de evoluție temporală cu o versiune fără control, exploatând simetria Hamiltonianului.
Să observăm următorul circuit.

![circuit optimization](/docs/images/tutorials/krylov-quantum-diagonalization/circuit_opt.avif)

Aici, $B_{\rm ref}$ pregătește starea de referință, $B_{\rm ref}|0^n\rangle = |\psi_{0}\rangle$.

În loc să pregătim mai întâi $\frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)|\psi_0\rangle$ și apoi să aplicăm $U(t)$ doar pe ramura $|1\rangle$, circuitul pregătește direct cele două ramuri ca
...
Circuitul aplică mai întâi o poartă Hadamard asupra ancilei și pregătește starea de referință doar pe ramura $|1\rangle$:
...
Deoarece Hamiltonianul păstrează numărul de excitații, putem vedea că $|0^n\rangle$ este starea sa proprie, și astfel operatorul de evoluție acumulează doar o fază sub Hamiltonian:
$$
\begin{equation*}
U(t)|0^n\rangle
=
...
În continuare, $B_{\rm ref}$ este aplicat doar pe ramura $|0\rangle$.
...
#### Transpilarea {#transpilation}

Acum, transpilăm circuitele controlate și fără control pentru a deveni executabile pe hardware.
Să comparăm rezultatul circuitelor transpilate.
...
Următorul pas este să transmitem același circuit parametrizat pentru mai multe valori ale $d$. Pentru fiecare $d$, estimăm patru valori de așteptare: $X\otimes I$, $Y\otimes I$, $X\otimes (H-T)$ și $Y\otimes (H-T)$. Aceste patru numere sunt apoi combinate în elementele complexe de pe primul rând $\mathcal{S}_{0d}$ și $\mathcal{H}_{0d}$.

Aici, $\mathcal{S}_{00}=1$ și $\mathcal{H}_{00}=\langle\psi_{0}|(H-T)|\psi_{0}\rangle=\langle\psi_{0}|H|\psi_{0}\rangle-\tau$ pot fi calculate clasic deoarece $\lvert\psi_0\rangle$ este rar, deci omitem cazul $d=0$.
...
Acum rezolvăm problema generalizată de valori proprii folosind matricele reconstruite din estimările circuitului.
Într-un calcul ideal de stare vectorială cu evoluție exactă în timp real, aceasta ar trebui să reproducă rezultatul proiectat exact.
În practică, abaterile pot proveni din Trotterizare, eroarea de eșantionare și instabilitatea numerică a matricei de suprapunere.
...
Acum reunim toate aceste detalii într-un flux de lucru unic la o scară mai mare, care este apoi rulat pe hardware-ul nostru cuantic real. În această secțiune, aplicăm setări realiste de atenuare a erorilor pentru a îmbunătăți fiabilitatea rezultatelor. Deoarece elementele matriceale corespunzătoare diferitelor valori ale $d$ pot fi evaluate în paralel, folosim modul `Batch` pentru a le executa eficient.
...
## Anexă: Perspectiva funcției Hamiltoniene (filtru spectral) {#appendix-the-hamiltonian-function-spectral-filter-viewpoint}

Fluxul de lucru principal a prezentat KQD în mod operațional: construirea unei baze Krylov din stări evoluate în timp real, estimarea matricelor proiectate $\mathcal{H}$ și $\mathcal{S}$, și rezolvarea GEVP.
Această anexă revizitează *același* calcul dintr-un unghi complementar care explică **de ce** funcționează KQD: perspectiva *funcției Hamiltoniene*, sau a *filtrului spectral* [\[3\]](#references), [\[5\]](#references).
Aceasta reutilizează modelul de 12 qubiți, pasul de timp $\Delta t$ și soluția Krylov deja obținută mai sus; nu este necesară nicio execuție nouă de circuit.

### Starea de referință ca distribuție de energie {#the-reference-state-as-an-energy-distribution}
...
cu stările proprii de energie $|E_m\rangle$.
Orice stare de referință poate fi extinsă în această bază proprie,
...
astfel încât poartă o **pondere spectrală** $p_m=|a_m|^2$ la fiecare energie $E_m$.
Energia de referință este media acestei distribuții, $\langle H\rangle_{0}=\sum_m p_m E_m$.

Descompunerea proprie a unui Hamiltonian general de $n$ qubiți este costisitoare exponențial, deci această imagine este doar un diagnostic, care *nu* face parte din algoritm.
Aici, totuși, o putem calcula ieftin pentru *aceeași* problemă cu $n=12$: Hamiltonianul Heisenberg conservă numărul total de excitații, iar referința $|000001000000\rangle$ poartă o singură excitație, deci întregul său conținut spectral se află în subspațiul cu o singură excitație, a cărui dimensiune crește doar liniar cu $n$.
Prin urmare, reutilizăm blocul exact cu o singură excitație (deja folosit mai sus ca reper) și citim distribuția de referință $\{(E_m, p_m)\}$ în interiorul acelui subspațiu.
...
### KQD învață un filtru care remodelează această distribuție {#kqd-learns-a-filter-that-reshapes-this-distribution}

O **funcție Hamiltoniană** $f(H)$ este definită prin calcul spectral,
...
sau, cu alte cuvinte, o sumă ponderată de proiectori proprii.
Aplicarea ei asupra referinței remodelează fiecare amplitudine spectrală, $a_m \to a_m f(E_m)$:
...
Dacă $f$ ar fi puternic concentrată la energia cea mai joasă ($f(E_0)=1$ și $f(E_m)\approx 0$ în rest), atunci $f(H)$ ar acționa ca un proiector pe starea fundamentală, iar ieșirea normalizată ar fi (aproape) starea fundamentală.
Prin urmare, un **filtru spectral** trece-jos bun în energie este exact ceea ce ne dorim.

KQD nu prescrie $f$ în avans. În schimb, extinde filtrul în baza evoluției în timp real,
...
o funcție *trigonometrică* a energiei ale cărei coeficienți $\{c_\ell\}$ sunt exact vectorul propriu GEVP rezolvat mai sus.
Minimizarea raportului Rayleigh $\mathbf{c}^\dagger\mathcal{H}\mathbf{c}/\mathbf{c}^\dagger\mathcal{S}\mathbf{c}$ este așadar echivalentă cu *învățarea* filtrului care suprimă cel mai bine ponderea stărilor excitate ale referinței.
O dimensiune Krylov $r$ mai mare oferă filtrului mai multe grade de libertate și un vârf mai ascuțit la energia stării fundamentale.
...
### Vizualizarea filtrului și a flexibilității sale {#visualize-the-filter-and-its-flexibility}

Arătăm mai întâi filtrul învățat la dimensiunea Krylov completă folosită mai sus, apoi urmărim cum se ascute pe măsură ce dimensiunea $r$ crește.

Barele arată ponderile spectrale de referință $p_m$ (înainte) și ponderile filtrate $p_m|f_{\rm KQD}(E_m)|^2$ (după), împreună cu intensitatea filtrului învățat $|f_{\rm KQD}(E)|^2$ pe o axă de energie continuă.
Filtrul concentrează ponderea pe cea mai joasă energie a subspațiului cu o singură excitație &mdash; aceeași energie la care a convergent estimarea KQD în exemplul la scară mică.
Rețineți că aceasta este starea fundamentală *în interiorul sectorului cu o singură excitație*, care este ținta relevantă pentru această stare de referință care conservă excitația, nu starea fundamentală globală.
...
### Creșterea dimensiunii Krylov: flexibilitatea funcției învățate {#increasing-the-krylov-dimension-flexibility-of-the-learned-function}

Reamintim că filtrul învățat este un polinom trigonometric în energie cu $r$ coeficienți,
...
Dimensiunea Krylov $r$ este exact numărul de coeficienți liberi, deci controlează **flexibilitatea** funcției.
Un $r$ mic poate produce doar un filtru larg, care variază lin și lasă să se scurgă pondere în stările excitate de joasă energie; când $r$ crește, filtrul poate forma un vârf mai îngust la energia țintă și poate suprima mai agresiv ponderea rămasă a stărilor excitate.
Aceasta este contrapartida în filtru spectral a convergenței energiei observate în exemplul la scară mică: pe măsură ce $r$ crește, distribuția filtrată se prăbușește pe starea fundamentală a subspațiului, iar energia estimată scade către aceasta.

Reutilizăm matricele proiectate deja estimate mai sus și pur și simplu rezolvăm GEVP la fiecare bloc principal $r\times r$, apoi evaluăm și reprezentăm grafic filtrul corespunzător.
...
- [Diagonalizare Krylov cuantică bazată pe eșantioane a unui model de rețea fermionic](/tutorials/sample-based-krylov-quantum-diagonalization)
- [Estimarea energiei stării fundamentale a lanțului Heisenberg cu VQE](/tutorials/spin-chain-vqe)
- [Curs despre algoritmi de diagonalizare cuantică](/learning/courses/quantum-diagonalization-algorithms/krylov)
</Admonition>
...
[5] G. Lee, M. Kang, J. Hong, S. Fomichev și J. Huh, *Filtered Quantum Phase Estimation*, [arXiv:2510.04294](https://arxiv.org/abs/2510.04294) (2025).
