---
description: 'Folosește Qunova HiVQE pentru a calcula curbe ale suprafeței de energie potențială (PES) la disociere.'
notebook_path: tutorials/qunova-hivqe.ipynb
sidebar_label: 'Curbe PES de disociere cu Qunova HiVQE'
title: 'Curbe PES de disociere cu Qunova HiVQE'
---
...
Funcțiile Qiskit sunt o funcționalitate experimentală disponibilă exclusiv pentru utilizatorii IBM Quantum&reg; Premium Plan, Flex Plan și On-Prem (prin IBM Quantum Platform API) Plan. Sunt în stare de previzualizare și pot suferi modificări.
</Admonition>

*Estimare de utilizare (NOTĂ: Aceasta este doar o estimare. Timpii de execuție pot varia.)*

* Li2S: Cinci minute de timp QPU pe un procesor Heron r2

* FeP-NO: Cinci minute de timp QPU pe un procesor Heron r2


## Context {#background}

Calcularea cu precizie a energiilor de reacție chimică este esențială pentru progresele științifice din știința materialelor, ingineria chimică, descoperirea de medicamente și alte domenii. Printre diversele sisteme chimice, sistemul Li-S a suscitat un interes semnificativ pentru înțelegerea și dezvoltarea de noi compoziții de baterii. Acest tutorial oferă experiență practică în calcularea suprafeței de energie potențială (PES) la disocierea legăturii Li-S a unui sistem $Li_2S$, prin eliminarea unui atom de litiu folosind calcule HiVQE. Rezultatele pot fi comparate cu calcule de referință (CASCI) și cu metode clasice precum Hartree-Fock (HF) pentru o problemă de 20 de qubiți.

## Cerințe {#requirements}

Instalează următoarele dependențe pentru a rula codul din acest tutorial.
...
Pentru a rula acest tutorial, importă funcția `qunova/hivqe-chemistry` prin `QiskitFunctionCatalog`. Ai nevoie de un cont IBM Quantum Premium Plan, Flex Plan sau On-Prem (IBM Quantum Platform API) Plan cu o licență de la Qunova pentru a rula această funcție.
...
## Partea 1: Li2S (20Q) {#part-1-li2s-20q}

### Pasul 1: Maparea intrărilor clasice la o problemă cuantică {#step-1-map-classical-inputs-to-a-quantum-problem}

Definește geometriile $Li_2S$ în format dicționar pentru diferite distanțe de legătură Li-S, pentru a calcula curba PES. Aceste geometrii sunt optimizate folosind calcule B3LYP/631g.
...
Calculele HiVQE vor fi efectuate cu opțiunile definite mai jos. Folosind baza sto3g pentru $Li_2S$, există 19 orbitali spațiali cu 22 de electroni. Pentru a rula cazul (10o,10e) cu calculul HiVQE, poți defini 10 orbitali activi și șase orbitali înghețați. La fiecare iterație, 100 de shots vor fi folosite pentru a eșantiona configurația de electroni generată de circuitul cuantic ExcitationPreserving (`epa`) cu entanglement `circular` și două repetări (`reps`). Numărul maxim de iterații este setat la 30 pentru a garanta terminarea iterației la convergența energiei.
...
### Pașii 2 și 3: Optimizarea problemei pentru execuția pe hardware cuantic și execuția cu funcția HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}

Configurează bucla `for` pentru a rula calculele HiVQE cu geometriile și opțiunile definite mai jos. Joburile sunt trimise în bucla `for`. În acest tutorial, vei trimite șase geometrii și vei recupera rezultatele când toate sunt finalizate. În execuția principală a funcției, trebuie să definești `max_states` și `max_expansion_states` pentru a controla dimensiunea maximă a matricei de subspațiu și pentru a controla câte stări pot fi generate folosind metode clasice de expansiune CI per iterație. ID-urile joburilor funcției vor fi stocate în dicționar cu eticheta fiecărei geometrii pentru a urmări și procesa ulterior rezultatele.
...
Să verificăm dacă toate joburile sunt încă în execuție sau s-au finalizat.
...
Odată ce toate joburile sunt finalizate, să recuperăm toate rezultatele calculelor.
...
Timpul total de execuție QPU utilizat în job poate fi urmărit prin autentificarea pe [IBM Quantum Platform](https://quantum.cloud.ibm.com/) și vizualizarea joburilor trimise cu eticheta `qunova-chemistry-hivqe`.

### Pasul 4: Post-procesare și comparare cu metodele clasice {#step-4-post-process-and-compare-with-classical-methods}

Calculul clasic de referință (CASCI) poate fi efectuat pentru (10o,10e) pentru a valida rezultatele HiVQE.
...
### Reprezentarea grafică a curbei de disociere pentru Li\_2S {#plotting-the-dissociation-curve-for-li-2s}

Să reprezentăm grafic și să comparăm rezultatele HiVQE cu HF și CASCI.
Poți observa că toate calculele HiVQE se potrivesc bine cu rezultatul clasic de referință (CASCI).
...
## Partea 2: FeP-NO (44Q) {#part-2-fep-no-44q}

Reține că pentru a rula funcția pe sistemul FeP-NO cu setările prezentate în acest exemplu, ai nevoie de o licență care să permită utilizarea funcției cu cel puțin 44 de qubiți. Trimite un e-mail la [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) pentru a solicita informații despre obținerea unei licențe.

### Pasul 1: Mapează intrările clasice la o problemă cuantică {#step-1-map-classical-inputs-to-a-quantum-problem-1}

Definește opțiunile pentru calculele HiVQE
...
Definește geometriile FeP-NO în format dicționar pentru diferite distanțe de legătură Fe-N, în vederea calculării curbei PES.
...
### Pașii 2 și 3: Optimizarea problemei pentru execuție pe hardware cuantic și execuția folosind funcția HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

Pe baza configurației HiVQE și a geometriilor, obține rezultatele secvențial.

#### Trimite calculul pentru d(Fe-N) = 1.75 $\AA$. {#submit-dfe-n-175-aa-calculation}
...
Urmărește job-ul și preia rezultatul pentru calculul d(Fe-N) = 1.75 $\AA$.
...
#### Trimite calculul pentru d(Fe-N) = 2.00 $\AA$. {#submit-dfe-n-200-aa-calculation}
...
Urmărește job-ul și preia rezultatul pentru calculul d(Fe-N) = 2.00 $\AA$.
...
#### Trimite calculul pentru d(Fe-N) = 5.00 $\AA$. {#submit-dfe-n-500-aa-calculation}
...
Urmărește job-ul și preia rezultatul pentru calculul d(Fe-N) = 5.00 $\AA$.
...
### Pasul 4: Post-procesează și compară cu metodele clasice {#step-4-post-process-and-compare-with-classical-methods-1}

Rezultatele calculului clasic de referință (CASCI-DMRG, maxM=800) sunt furnizate pentru (22o,22e) în vederea validării rezultatelor HiVQE.
...
![Rezultatul celulei de cod anterioare](/docs/images/tutorials/qunova-hivqe/extracted-outputs/49a9bf81-1.avif)
...
> **Notă:** Acest sondaj este realizat de **IBM Quantum** și acoperă **conținutul tutorialului** (scris de IBM). doQumentation oferă site-ul web, traducerile și execuția codului — pentru feedback despre acestea, te rugăm [deschide un issue pe GitHub](https://github.com/JanLahmann/doQumentation/issues).

[Link către sondaj](https://your.feedback.ibm.com/jfe/form/SV_9AizRyKFHEGzVs2)
