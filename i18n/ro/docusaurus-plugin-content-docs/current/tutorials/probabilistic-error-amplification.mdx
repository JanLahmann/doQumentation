---
description: 'Rulează un experiment de atenuare a erorilor la scară utilitară cu extrapolarea zgomotului zero și amplificarea probabilistică a erorilor.'
notebook_path: tutorials/probabilistic-error-amplification.ipynb
sidebar_label: 'Atenuarea erorilor la scară utilitară cu amplificarea probabilistică a erorilor'
title: 'Atenuarea erorilor la scară utilitară cu amplificarea probabilistică a erorilor'
---
...
*Estimare de utilizare: 14 minute pe un procesor Heron r3 (NOTĂ: Aceasta este doar o estimare. Timpul tău de execuție poate varia.)*

## Rezultate de învățare {#learning-outcomes}
După parcurgerea acestui tutorial, utilizatorii ar trebui să înțeleagă:
- Teoria din spatele *extrapolării zgomotului zero* (ZNE), diferitele metode de amplificare a zgomotului și de ce *amplificarea probabilistică a erorilor* (PEA) este preferată pentru experimentele la scară utilitară.
- Cum să implementeze ZNE cu PEA în practică folosind Qiskit.
...
- [Lecția despre atenuarea erorilor](/learning/courses/utility-scale-quantum-computing/error-mitigation) din cursul *Informatică cuantică la scară utilitară* pentru cunoștințe de bază despre utilizarea atenuării erorilor în Qiskit.
- [Lecția Utility-I](/learning/courses/utility-scale-quantum-computing/utility-i) din cursul *Informatică cuantică la scară utilitară* pentru mai mult context despre experimentul la scară utilitară utilizat ca exemplu în acest tutorial.

## Background {#background}

Acest tutorial demonstrează cum să rulezi un experiment de atenuare a erorilor la scară utilitară cu IBM Quantum Compute Service, folosind o versiune experimentală a *extrapolării zgomotului zero* (ZNE) cu *amplificarea probabilistică a erorilor* (PEA).
...
unde $\lambda$ parametrizează intensitatea zgomotului și poate fi amplificată.

Putem implementa ZNE prin următorii pași:
...
#### Amplificarea zgomotului pentru ZNE {#amplify-noise-for-zne}
...
### Învățarea modelului de zgomot pentru PEA {#learn-the-noise-model-for-pea}
...
**Referință**: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, *Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors* [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)
...
## Configurare {#setup}
În celula de mai jos, importăm pachetele relevante și creăm câteva funcții ajutătoare pentru a construi Circuit-urile pentru evoluția temporală Trotterizată a unui model Ising transversal bidimensional care respectă topologia Backend-ului.
...
Vom omite acest pas deoarece atenuarea erorilor la runtime nu este acceptată pe simulatoare.
...
#### Crearea unui Circuit Ising parametrizat {#create-a-parameterized-ising-model-circuit}

##### Stabilirea unui backend {#establish-a-backend}
...
##### Definirea cuplajelor stratului de înlănțuire {#define-entangling-layer-couplings}
...
##### Eliminarea qubiților defectuoși {#remove-bad-qubits}
...
##### Generarea principalului Circuit Trotter {#main-trotter-circuit-generation}
...
#### Crearea unei liste de valori de parametri pentru atribuire ulterioară {#create-a-list-of-parameter-values-to-be-assigned-later}
...
### Pasul 2: Optimizează problema pentru execuție pe hardware cuantic {#step-2-optimize-problem-for-quantum-hardware-execution}
...
* Traduce porțile din Circuit-ul nostru în instrucțiuni [Instruction Set Architecture (ISA)](/guides/transpile#instruction-set-architecture) care pot fi executate direct pe hardware.
...
#### Observable-uri ISA {#isa-observables}
...
Observă că fiecare observable este învelit într-o listă în blocul de cod următor. Acest lucru este necesar pentru a realiza *broadcasting* cu valorile parametrilor, astfel încât fiecare observable de qubit să fie măsurat pentru fiecare valoare theta. Regulile de broadcasting pentru primitive se găsesc în [documentația primitivelor](/guides/primitives).
...
#### Configurează opțiunile Estimator {#configure-estimator-options}
...
##### Explicarea opțiunilor ZNE {#explanation-of-zne-options}
...
* **amplifier**: Metoda utilizată pentru amplificarea zgomotului la factorii de zgomot doriți.
  Valorile permise sunt `"gate_folding"`, care amplifică prin repetarea porților de bază cu doi qubiți,
  și `"pea"`, care amplifică prin eșantionare probabilistică după învățarea modelului de zgomot Pauli-twirled
  pentru straturi de porți de bază cu doi qubiți twirled. Opțiuni suplimentare sunt `"gate_folding_front"` și `"gate_folding_back"`, care sunt explicate în [documentația API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier).

* **extrapolated\_noise\_factors**: Specifică una sau mai multe valori de factori de zgomot la care se evaluează
  modelele extrapolate. Dacă este o secvență de valori, rezultatele returnate vor fi de tip array cu factorul de zgomot specificat evaluat pentru modelul de extrapolare. O valoare
  de 0 corespunde extrapolării la zgomot zero.


#### Rulează experimentul {#run-the-experiment}
...
#### Forme generale ale rezultatelor și metadate {#general-result-shapes-and-metadata}
...
1. `pub_result.data.evs`: Valorile de așteptare corespunzătoare zgomotului zero (bazate pe cea mai bună extrapolare euristică).
   ...
   * Prima axă este indicele qubitului virtual pentru observabila $\langle Z_i\rangle$ ($124$ qubiți virtuali/observabile)

   * A doua axă indexează valoarea parametrului pentru $\theta$ ($12$ valori ale parametrului)
...
### Reprezentarea grafică a rezultatelor simulării Trotter {#plot-trotter-simulation-results}
...
![Rezultatul celulei de cod anterioare](/docs/images/tutorials/probabilistic-error-amplification/extracted-outputs/e466736a-0.avif)
...
### Reprezentarea grafică a rezultatelor extrapolării pentru qubiți individuali {#plot-extrapolation-results-for-individual-qubits}
...
![Rezultatul celulei de cod anterioare](/docs/images/tutorials/probabilistic-error-amplification/extracted-outputs/bea9695a-0.avif)

## Pași următori {#next-steps}
<Admonition type="tip" title="Recomandări">
Dacă ți s-a părut interesant acest material, s-ar putea să fii interesat și de:
- Un [tutorial](/tutorials/combine-error-mitigation-techniques) dedicat combinării tehnicilor de atenuare a erorilor.
- [Documentație](/guides/error-mitigation-and-suppression-techniques) detaliată despre tehnicile de atenuare a erorilor disponibile în Qiskit.
- Lecții suplimentare despre experimentele la scară utilitară: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) și [Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
