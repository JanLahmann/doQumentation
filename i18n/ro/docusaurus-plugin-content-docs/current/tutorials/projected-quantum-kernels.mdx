---
title: "Îmbunătățirea clasificării caracteristicilor cu nuclee cuantice proiectate"
sidebar_label: "Îmbunătățirea clasificării caracteristicilor cu nuclee cuantice proiectate"
description: "Tutorial despre îmbunătățirea clasificării caracteristicilor cu nuclee cuantice proiectate"
...
*Estimare utilizare: 80 de minute pe un procesor Heron r3 (NOTĂ: Aceasta este doar o estimare. Timpul tău de execuție poate varia.)*

## Rezultate ale învățării {#learning-outcomes}

După parcurgerea acestui tutorial, utilizatorii ar trebui să înțeleagă:
- Cum funcționează nucleele cuantice proiectate (PQK) și când oferă un potențial avantaj cuantic.
- Cum să rulezi un PQK pe hardware folosind un set de date din lumea reală.

## Condiții prealabile {#prerequisites}

Sugerăm ca utilizatorii să fie familiarizați cu următoarele subiecte înainte de a parcurge acest tutorial:
- [Nuclee cuantice](/learning/courses/quantum-machine-learning/quantum-kernel-methods) din [cursul de învățare automată cuantică](/learning/courses/quantum-machine-learning) din IBM Quantum&reg; Learning

## Context {#background}

În acest tutorial, demonstrăm cum să rulezi un [nucleu cuantic proiectat](https://www.nature.com/articles/s41467-021-22539-9) (PQK) cu Qiskit pe un set de date biologice din lumea reală, bazat pe articolul [Enhanced Prediction of CAR T-Cell Cytotoxicity with Quantum-Kernel Methods](https://arxiv.org/abs/2507.22710) [[1]](#references).

PQK este o metodă utilizată în învățarea automată cuantică (QML) pentru a codifica date clasice într-un spațiu de caracteristici cuantic și a le proiecta înapoi în domeniul clasic, folosind calculatoare cuantice pentru a îmbunătăți selecția caracteristicilor. Implică codificarea datelor clasice în stări cuantice cu ajutorul unui Circuit cuantic, de obicei printr-un proces numit mapare a caracteristicilor, în care datele sunt transformate într-un spațiu Hilbert de înaltă dimensionalitate. Aspectul „proiectat" se referă la extragerea informațiilor clasice din stările cuantice, prin măsurarea unor observabile specifice, pentru a construi o matrice nucleu care poate fi utilizată în algoritmi clasici bazați pe nucleu, cum ar fi mașinile cu vectori suport. Această abordare valorifică avantajele computaționale ale sistemelor cuantice pentru a obține potențial performanțe mai bune la anumite sarcini comparativ cu metodele clasice.

Principalele componente ale PQK-urilor sunt matricele de densitate redusă (RDM), obținute prin măsurători proiective ale hărții de caracteristici cuantice. În special, se calculează de obicei matricele de densitate redusă pe un singur qubit (1 RDM) pentru fiecare qubit. Aceste cantități măsurate sunt apoi utilizate ca intrări pentru o funcție clasică de nucleu, cum ar fi un nucleu exponențial, pentru a construi matricea finală de nucleu.

PQK-urile oferă avantaje potențiale față de [nucleele cuantice](/tutorials/quantum-kernel-training) standard, în special pentru hardware-ul cuantic de termen scurt. Nucleele cuantice standard se bazează de obicei pe estimarea suprapunerilor globale ale stărilor, care devin din ce în ce mai dificil de măsurat cu precizie pe măsură ce numărul de qubiți crește și sunt foarte sensibile la zgomot. În schimb, PQK-urile utilizează observabile locale, cum ar fi matricele de densitate redusă pe un singur qubit (1 RDM), conducând la un overhead de eșantionare mai mic, o robustețe îmbunătățită la zgomotul hardware și o scalabilitate mai bună. Prin proiectarea stărilor cuantice pe caracteristici de măsurare locale înainte de aplicarea unei funcții clasice de nucleu, PQK-urile pot păstra corelații cuantice utile rămânând în același timp mai practice pentru dispozitivele de termen scurt.
...
Omitem exemplul la scară mică cu simulator în acest tutorial, deoarece scopul nostru principal este să demonstrăm cum nucleele cuantice proiectate pot scala la sisteme mai mari și pe hardware real.
...
### Pregătirea setului de date {#dataset-preparation}

În acest tutorial folosim un set de date biologice din lumea reală pentru o sarcină de clasificare binară, generat de Daniels et al. (2022) și care poate fi descărcat din [materialul suplimentar](https://www.science.org/doi/full/10.1126/science.abq0225#supplementary-materials) inclus în articol. Datele constau din celule CAR T, care sunt celule T modificate genetic utilizate în imunoterapie pentru tratarea anumitor tipuri de cancer. Celulele T, un tip de celule imune, sunt modificate în laborator pentru a exprima receptori antigenici chimerici (CARs) care vizează proteine specifice de pe celulele canceroase. Aceste celule T modificate pot recunoaște și distruge celulele canceroase mai eficient. Caracteristicile datelor sunt motivele celulelor CAR T, care se referă la componentele structurale sau funcționale specifice ale CAR-ului inginerat în celulele T. Pe baza acestor motive, sarcina noastră este să prezicem citotoxicitatea unei celule CAR T date, etichentând-o fie ca toxică, fie ca non-toxică.
Cele de mai jos prezintă funcțiile auxiliare pentru preprocesarea acestui set de date.
...
Poți rula acest tutorial executând celula de mai jos, care creează automat structura de foldere necesară și descarcă atât fișierele de antrenament, cât și cele de test direct în mediul tău. Dacă ai deja aceste fișiere local, acest pas le va suprascrie în siguranță pentru a asigura consistența versiunii.
...
De asemenea, transformăm setul de date astfel încât $1$ să fie reprezentat ca $\pi/2$ în scopuri de scalare.
...
Verificăm dimensiunile și formele seturilor de date de antrenament și de test.
...
Construim acum harta de caracteristici care înglobează setul nostru de date clasice într-un spațiu de caracteristici de dimensionalitate mai mare. Pentru această încorporare, folosim [`ZZFeatureMap`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ZZFeatureMap) din Qiskit.
...
O altă opțiune de încorporare cuantică este ansatz-ul de evoluție a hamiltonianului Heisenberg 1D. Poți sări peste rularea acestei secțiuni dacă dorești să continui cu `ZZFeatureMap`.
...
### Măsurarea 1-RDM-urilor {#measure-1-rdms}
În acest pas, obținem toate matricele de densitate redusă pe un singur qubit (1-RDM) prin măsurători proiective ale hărții de caracteristici cuantice, care vor fi furnizate ulterior funcției clasice de nucleu exponențial.
Să vedem cum se calculează 1-RDM pentru un singur punct de date din set, înainte de a itera peste toate datele. 1-RDM-urile sunt o colecție de măsurători pe un singur qubit ale operatorilor Pauli ``X``, ``Y`` și ``Z`` pe toți qubiții. Aceasta deoarece o RDM pe un singur qubit poate fi exprimată complet ca: $$\rho = \frac{1}{2} \big( I + \braket \sigma_x \sigma_x  + \braket \sigma_y \sigma_y + \braket \sigma_z \sigma_z  \big)$$
Mai întâi selectăm Backend-ul de utilizat.
...
Apoi rulăm circuitul cuantic și măsurăm proiecțiile. Rețineți că activăm atenuarea erorilor, inclusiv Extrapolarea Zgomotului Zero (ZNE).
...
Afișăm dimensiunea circuitului și adâncimea porților cu doi qubiți.
...
Acum putem itera peste întregul set de antrenament pentru a obține toate 1-RDM-urile.
De asemenea, furnizăm rezultatele unui experiment pe care l-am rulat pe hardware cuantic real. Poți fie să rulezi antrenamentul tu însuți setând indicatorul de mai jos la `True`, fie să folosești rezultatele proiecțiilor pe care le furnizăm noi.
...
Odată ce joburile sunt complete, putem recupera rezultatele.
...
Repetăm procesul pentru setul de test.
...
Putem recupera rezultatele ca mai înainte.
...
### Definirea kernelului cuantic proiectat {#define-the-projected-quantum-kernel}
Kernelul cuantic proiectat este definit prin următoarea funcție kernel: $$k^{\textrm{PQ}}(x_i, x_j) = \textrm{exp} \Big(-\gamma \sum_k \sum_{P \in \{ X,Y,Z \}} (\textrm{Tr}[P \rho_k(x_i)] - \textrm{Tr}[P \rho_k(x_j)])^2 \Big) $$
În ecuația de mai sus, $\gamma>0$ este un hiperparametru reglabil. $K^{\textrm{PQ}}_{ij} = k^{\textrm{PQ}}(x_i, x_j)$ reprezintă elementele matricei kernel $K^{\textrm{PQ}}$.
Folosind definiția 1-RDM-urilor, putem observa că termenii individuali din funcția kernel pot fi evaluați ca $\textrm{Tr}[P \rho_k (x_i)] = \braket P$, unde $P \in \{ X,Y,Z \}$. Aceste valori de așteptare sunt exact ceea ce am măsurat mai sus.
Prin utilizarea ``scikit-learn``, putem de fapt să calculăm kernelul și mai ușor. Aceasta se datorează kernelului cu funcție de bază radială (``'rbf'``) disponibil imediat: $ \textrm{exp} (-\gamma \lVert x - x' \rVert^2)$. Mai întâi, trebuie pur și simplu să reformatăm noile seturi de date de antrenare și de test proiectate în tablouri bidimensionale.
Rețineți că parcurgerea întregului set de date poate dura aproximativ 80 de minute pe QPU. Pentru a te asigura că restul tutorialului este ușor de executat, furnizăm suplimentar proiecții dintr-un experiment rulat anterior (care sunt incluse în fișierele descărcate în blocul de cod `Download dataset`). Dacă ai efectuat antrenarea singur, poți continua tutorialul cu propriile rezultate.
...
### Mașina cu vectori suport (SVM) {#support-vector-machine-svm}
Putem acum rula un SVM clasic pe acest kernel precalculat și să folosim kernelul dintre seturile de test și antrenare pentru predicție.
...
### Benchmarking clasic {#classical-benchmarking}
Putem rula un SVM clasic cu funcția de bază radială ca kernel, fără a efectua o proiecție cuantică. Acest rezultat reprezintă referința noastră clasică.
...
## Anexă: Verificarea potențialului avantaj cuantic al setului de date în sarcinile de învățare {#appendix-verify-the-datasets-potential-quantum-advantage-in-learning-tasks}

Nu toate seturile de date oferă un avantaj potențial din utilizarea PQK-urilor. Există câteva limite teoretice pe care le poți folosi ca test preliminar pentru a vedea dacă un anumit set de date poate beneficia de PQK-uri. Pentru a cuantifica acest lucru, autorii articolului [Power of data in quantum machine learning](https://www.nature.com/articles/s41467-021-22539-9) [2] definesc cantități numite complexități ale modelelor clasic și cuantic, respectiv separarea geometrică a modelelor clasic și cuantic. Pentru a anticipa un potențial avantaj cuantic din PQK-uri, separarea geometrică dintre kernelurile clasic și cuantic-proiectat ar trebui să fie aproximativ de ordinul $\sqrt{N}$, unde $N$ este numărul de eșantioane de antrenare. Dacă această condiție este satisfăcută, trecem la verificarea complexităților modelelor. Dacă complexitatea modelului clasic este de ordinul $N$, în timp ce complexitatea modelului cuantic-proiectat este substanțial mai mică decât $N$, putem anticipa un avantaj potențial din PQK.
Separarea geometrică este definită astfel (F19 în [[2]](#references)):
...
Complexitatea modelului este definită astfel (M1 în [[2]](#references)):
...
Dacă ai găsit acest material interesant, s-ar putea să fii interesat de următoarele resurse:
- Cursul aprofundat de [quantum machine learning](/learning/courses/quantum-machine-learning) de la IBM Quantum Learning
- Tutorialul [Quantum kernel training](/tutorials/quantum-kernel-training)
</Admonition>
