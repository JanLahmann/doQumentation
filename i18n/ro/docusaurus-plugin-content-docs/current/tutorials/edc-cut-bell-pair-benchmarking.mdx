...
* [Concepte de bază ale calculului cuantic](/learning/courses/basics-of-quantum-information), inclusiv stări Bell, entanglement și porți cuantice;
* Familiarizare cu [circuitele dinamice](/guides/classical-feedforward-and-control-flow) ([măsurători în mijlocul circuitului](/guides/execute-dynamic-circuits#mid-circuit-measurements) și feedforward clasic);
...
Înainte de a rula pe QPU-ul real, verificăm că circuitul produce o pereche Bell testând-ul pe un simulator fără zgomot cu un lanț de patru Qubiți `[0, 1, 2, 3]`. Folosim `Sampler`-ul IBM Quantum cu `AerSimulator` ca mod backend pentru a executa circuitele.
...
Pentru fiecare aranjament inițial de patru Qubiți, funcția de mai sus returnează două circuite (unul pentru măsurarea stabilizatorului $ZZ$, unul pentru $XX$). Aceste circuite includ măsurători în mijlocul circuitului și operații condiționale (if/else), care sunt instrucțiunile cheie ale circuitului dinamic.
...
Rezultatele dezvăluie o gamă largă de calitate a entanglementului pe întregul dispozitiv. Acest lucru confirmă concluzia lucrării conform căreia poate exista o variație de peste un ordin de mărime în fidelitatea stării Bell, în funcție de qubiții fizici utilizați. În termeni practici, aceasta înseamnă că anumite regiuni sau legături din cip sunt mult mai bune la efectuarea măsurătorilor și operațiilor de feedforward în mijlocul circuitului decât altele. Factori precum eroarea de citire a qubitului, durata de viață a qubitului și crosstalk-ul contribuie probabil la aceste diferențe. De exemplu, dacă un lanț include un qubit de citire deosebit de zgomotos, măsurătoarea în mijlocul circuitului ar putea fi nesigură, ducând la o fidelitate slabă pentru acea pereche încurcată (MSE ridicat).
În final, vizualizăm performanța generală trasând funcția de distribuție cumulativă (CDF) a valorilor MSE pentru toate lanțurile. Graficul CDF arată pragul MSE pe axa x și fracția de perechi de qubiți care au cel mult acel MSE pe axa y. Această curbă începe de la zero și se apropie de unu pe măsură ce pragul crește pentru a cuprinde toate punctele de date. O creștere abruptă în apropierea unui MSE scăzut ar indica faptul că multe perechi au fidelitate ridicată; o creștere lentă înseamnă că multe perechi au erori mai mari. Adnotăm CDF-ul cu identitățile celor mai bune perechi. În grafic, fiecare punct din CDF corespunde MSE-ului unui lanț de patru qubiți, iar eticheta punctului este perechea de indici de qubiți $[q0, q3]$ care au fost încurcați în acel experiment. Acest lucru facilitează identificarea perechilor fizice de qubiți care sunt cele mai performante (punctele din extrema stângă a CDF-ului).
...
- Află cum să implementezi [entanglementul pe distanță lungă cu circuite dinamice](/tutorials/long-range-entanglement)
- Află cum să simulezi [hamiltonianul Ising cu kick cu circuite dinamice](/tutorials/dc-hex-ising)
</Admonition>
