...
În acest pas, folosim primitiva `Estimator` pentru a calcula valorile de așteptare $\langle Z_0 Z_i \rangle$ pentru operatorii `ZZ`, evaluând calitatea entanglementului și a execuției circuitelor transpirate. Pentru a se alinia cu fluxurile tipice de lucru ale utilizatorilor, trimitem job-ul spre execuție și aplicăm suprimarea erorilor folosind **decuplarea dinamică**, o tehnică ce atenuează decoerența prin inserarea unor secvențe de poartă-uri pentru a păstra stările qubit-urilor. De asemenea, specificăm un nivel de reziliență pentru a contrabalansa zgomotul, nivelurile mai ridicate oferind rezultate mai precise cu prețul unui timp de procesare mai mare. Această abordare evaluează performanța fiecărei configurații de pass manager în condiții realiste de execuție.
