---
title: "Stichprobenbasierte Krylov-Quantendiagonalisierung vu am fermionische Gittermodell"
sidebar_label: "Stichprobenbasierte Krylov-Quantendiagonalisierung vu am fermionische Gittermodell"
description: "Verwended Se dr stichprobenbasierte Quantendiagonalisierungsalgorithmus, um s Einzelstörstellen-Anderson-Modell mit verrauschter Quantenhardware z simuliere."
...
*Nutzungsschätzung: Nein Sekunde auf am Heron r2 Prozessor (HINWEIS: Des isch nur a Schätzung. Ihre Laufzeit ka variiere.)*
## Hintergrund {#hintergrund}

Des Tutorial zeigt, wie Se die stichprobenbasierte Quantendiagonalisierung (SQD) verwende, um d Grundzustandsenergie vun am fermionische Gittermodell z schätze. Konkret untersuche mir s eindimensionale Einzelstörstellen-Anderson-Modell (SIAM), des zur Beschreibung magnetischer Störstelle in Metalle verwendet wird.

Des Tutorial folgt amm ähnliche Arbeitsablauf wie s verwandte Tutorial [Stichprobenbasierte Quantendiagonalisierung eines Chemie-Hamiltonians](/tutorials/sample-based-quantum-diagonalization). A wesentlicher Unterschied liegt aber drin, wie d Quantenschaltkreise aufgebaut werde. S andere Tutorial verwendet an heuristische Variationsansatz, dr für Chemie-Hamiltonians mit potenziell Millione vo Wechselwirkungstermen attraktiv isch. Des Tutorial hingege verwendet Schaltkreise, die d Zeitentwicklung durch dr Hamiltonian approximiere. Solche Schaltkreise köne tief sei, was des Vorgehe besser für Anwendunge auf Gittermodelle macht. D Zustandsvektore, die von denne Schaltkreise präpariert werde, bilde d Basis für an [Krylov-Unterraum](https://en.wikipedia.org/wiki/Krylov_subspace), und als Ergebnis konvergiert dr Algorithmus nachweislich und effizient zum Grundzustand unter geeignete Annahme.

Dr in dem Tutorial verwendete Ansatz ka als a Kombination vun dr in SQD und [Krylov-Quantendiagonalisierung (KQD)](https://arxiv.org/abs/2407.14431) verwendete Technike betrachtet werde. Dr kombinierte Ansatz wird manchmal als stichprobenbasierte Krylov-Quantendiagonalisierung (SQKD) bezeichnet. Luage Se [Krylov-Quantendiagonalisierung von Gitter-Hamiltonians](/tutorials/krylov-quantum-diagonalization) für a Tutorial zur KQD-Methode.

Des Tutorial basiert auf dr Arbeit ["Quantum-Centric Algorithm for Sample-Based Krylov Diagonalization"](https://arxiv.org/abs/2501.09702), auf die für weitere Details verwiese werde ka.

### Einzelstörstellen-Anderson-Modell (SIAM) {#einzelstoerstellen-anderson-modell-siam}

Dr eindimensionale SIAM-Hamiltonian isch a Summe aus drei Terme:
...
Do sind $c^\dagger_{\mathbf{j},\sigma}/c_{\mathbf{j},\sigma}$ d fermionische Erzeugungs-/Vernichtungsoperatore für d $\mathbf{j}^{\textrm{te}}$ Bad-Stell mit Spin $\sigma$, $\hat{d}^\dagger_{\sigma}/\hat{d}_{\sigma}$ sind Erzeugungs-/Vernichtungsoperatore für dr Störstellenmodus, und $\hat{n}_{d\sigma} = \hat{d}^\dagger_{\sigma} \hat{d}_{\sigma}$. $t$, $U$ und $V$ sind reelle Zahle, die d Hüpf-, Vor-Ort- und Hybridisierungswechselwirkunge beschreibe, und $\varepsilon$ isch a reelle Zahl, die s chemische Potenzial spezifiziert.

Beachde Se, dass dr Hamiltonian a spezifische Instanz vum generische Wechselwirkungs-Elektronen-Hamiltonian isch,
...
Wege dr approximative Translationssymmetrie in $H_\textrm{bath}$ erwarte mir ned, dass dr Grundzustand in dr Ortsbasis (dr Orbitalbasis, in dr dr Hamiltonian obe spezifiziert isch) dünn besetzt isch. D Leistung vun SQD isch nur garantiert, wenn dr Grundzustand dünn besetzt isch, des heißt, wenn er signifikantes Gewicht auf nur aner kleinen Anzahl von Rechenbasis-Zuständ hend. Um d Dünnbesetztheit vum Grundzustands z verbessere, führe mir d Simulation in dr Orbitalbasis durch, in dr $H_\textrm{bath}$ diagonal isch. Mir nenne diese Basis d *Impulsbasis*. Da $H_\textrm{bath}$ a quadratischer fermionischer Hamiltonian isch, ka er effizient durch a Orbitalrotation diagonalisiert werde.

### Approximative Zeitentwicklung durch dr Hamiltonian {#approximative-zeitentwicklung-durch-den-hamiltonian}

Um d Zeitentwicklung durch dr Hamiltonian z approximiere, verwende mir a Trotter-Suzuki-Zerlegung zweiter Ordnung,

$$
  e^{-i \Delta t H} \approx e^{-i\frac{\Delta t}{2} H_2} e^{-i\Delta t H_1} e^{-i\frac{\Delta t}{2} H_2}.
$$

Unter dr [Jordan-Wigner-Transformation](https://en.wikipedia.org/wiki/Jordan%E2%80%93Wigner_transformation) entspricht d Zeitentwicklung durch $H_2$ amm einzelne [CPhase](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.CPhaseGate)-Gate zwische de Spin-up- und Spin-down-Orbitale an dr Störstell. Da $H_1$ a quadratischer fermionischer Hamiltonian isch, entspricht d Zeitentwicklung durch $H_1$ aner Orbitalrotation.

D Krylov-Basiszuständ $\{ |\psi_k\rangle \}_{k=0}^{D-1}$, wobei $D$ d Dimension vum Krylov-Unterraums isch, werde durch wiederholte Anwendung vun amm einzelne Trotter-Schritt gebildet, sodass

$$
  |\psi_k\rangle \approx \left[e^{-i\frac{\Delta t}{2} H_2} e^{-i\Delta t H_1} e^{-i\frac{\Delta t}{2} H_2} \right]^k\ket{\psi_0}.
$$

Im folgende SQD-basierte Arbeitsablauf werde mir Stichproben aus dem Satz vo Schaltkreise zie und dr kombinierte Satz vo Bitfolge mit SQD nachbearbeite. Des Vorgehe steht im Gegensatz zu dem im verwandte Tutorial [Stichprobenbasierte Quantendiagonalisierung eines Chemie-Hamiltonians](/tutorials/sample-based-quantum-diagonalization) verwendete, wo Stichproben aus amm einzelne heuristische Variationsschaltkreis gezogt worde sind.
## Anforderungen {#anforderungen}

Stell Se vor Beginn vun dem Tutorial sicher, dass Se Folgendes installiert hend:
...
Für jede Spin-Spezies isch dr Anfangszustand $\ket{\psi_0}$ durch d Superposition aller mögliche Anregunge vun de drei Elektronen, die dem Fermi-Niveau am nächsten sind, in d 4 nächste leere Mode gegeben, ausgehend vom Zustand $|00\cdots 0011 \cdots 11\rangle$, und wird durch d Anwendung vun siebe [XXPlusYYGate](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.XXPlusYYGate)s realisiert.
D zeitentwickelte Zuständ werde durch sukzessive Anwendunge vun amm Trotter-Schritt zweiter Ordnung erzeugt.

Für a detailliertere Beschreibung vun dem Modell und wie d Schaltkreise entworfe worde sind, luage Se ["Quantum-Centric Algorithm for Sample-Based Krylov Diagonalization"](https://arxiv.org/abs/2501.09702).
...
Nachdem mir d Schaltkreise erstellt hend, köne mir se für a Ziel-Hardware optimiere. Mir wähle d am wenigste ausgelastete QPU mit mindestens 127 Qubits. Weitere Informationen finde Se in dr [Qiskit IBM&reg; Runtime-Dokumentation](/guides/get-started-with-primitives#get-started-with-sampler).
...
Nachdem mir d Schaltkreise für d Hardware-Ausführung optimiert hend, sind mir bereit, se auf dr Ziel-Hardware auszuführe und Stichproben für d Grundzustandsenergieschätzung z sammle. Nachdem mir d Sampler-Primitive verwendet hend, um Bitfolge aus jedem Schaltkreis z zie, kombiniere mir alle Ergebnisse in amm einzelne Zähler-Wörterbuch und zeiche d 20 am häufigste gezogte Bitfolge auf.
...
Jetzt führe mir dr SQD-Algorithmus mit dr Funktion `diagonalize_fermionic_hamiltonian` aus. Erläuterunge zu de Argumente vun dere Funktion finde Se in dr [API-Dokumentation](https://qiskit.github.io/qiskit-addon-sqd/apidocs/qiskit_addon_sqd.fermion.html#qiskit_addon_sqd.fermion.diagonalize_fermionic_hamiltonian).
...
D folgende Code-Zell zeicht d Ergebnisse auf. D erste Grafik zeigt d berechnete Energie als Funktion vun dr Anzahl dr Konfigurationswiederherstellungsiterationen, und d zweite Grafik zeigt d durchschnittliche Besetzung vun jedem räumliche Orbital nach dr letzten Iteration. Für d Referenzenergie verwende mir d Ergebnisse vun ara [DMRG](https://en.wikipedia.org/wiki/Density_matrix_renormalization_group)-Berechnung, die separat durchgeführt worden isch.
...
D von SQD zurückgegebene Energie isch garantiert a obere Schranke für d wahre Grundzustandsenergie. Dr Wert vun dr Energie ka verifiziert werde, da SQD au d Koeffizienten vum Zustandsvektor zurückgibt, dr dr Grundzustand approximiert. Se köne d Energie aus dem Zustandsvektor unter Verwendung vun seine 1- und 2-Teilchen-reduzierte Dichtematrize berechne, wie in dr folgende Code-Zell demonstriert wird.
