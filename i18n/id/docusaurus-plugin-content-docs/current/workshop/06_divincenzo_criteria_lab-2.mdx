...
**Inisialisasi di tengah Circuit:** `reset` Qiskit dapat digunakan di tengah Circuit untuk menginisialisasi ulang qubit ke |0〉 secara langsung. Misalnya, dalam kode koreksi kesalahan atau algoritma iteratif, seseorang sering mengukur qubit lalu meresetnya untuk digunakan kembali. Operasi `reset` bersifat deterministik; ia membuang keadaan yang ada dan mendinginkan qubit kembali ke keadaan dasar.
...
![Diagram Circuit kuantum](/img/workshop/06-divincenzo-criteria-lab-2/output_3.png)
...
![Diagram Circuit kuantum](/img/workshop/06-divincenzo-criteria-lab-2/output_4.png)
...
Dengan menguasai teleportasi kuantum, kamu akan memahami cara meng-encode, mengirimkan, dan memulihkan informasi kuantum antara node yang berbeda—meletakkan dasar untuk jaringan kuantum, sistem repeater, skema komunikasi aman, dan komputasi kuantum modular yang dapat diskalakan.
**Bagaimana ini berkaitan dengan kriteria 6 & 7:** Dalam jaringan kuantum yang sesungguhnya, pasangan terjerat (entangled) yang dibagikan akan dibuat dengan mendistribusikan qubit "terbang" (seperti foton) antara lokasi Alice dan Bob (Kriteria 7: transmisi yang setia). Protokol teleportasi itu sendiri kemudian berfungsi sebagai cara untuk memetakan keadaan qubit stasioner milik Alice ke separuh pasangan terjerat miliknya, secara efektif 'mengirimkannya' ke Bob (Kriteria 6: interkonversi). Qiskit memungkinkan kita untuk mensimulasikan logika protokol ini secara sempurna, memberikan model konseptual tentang bagaimana kriteria-kriteria ini dipenuhi dalam arsitektur komunikasi.
