...
Untuk menjalankan tutorial ini, impor fungsi `qunova/hivqe-chemistry` melalui `QiskitFunctionCatalog`. Anda memerlukan akun IBM Quantum Premium Plan, Flex Plan, atau On-Prem (IBM Quantum Platform API) Plan dengan lisensi dari Qunova untuk menjalankan fungsi ini.
...
## Bagian 1: Li2S (20Q) {#part-1-li2s-20q}
...
Perhitungan HiVQE akan dilakukan dengan opsi yang didefinisikan di bawah ini. Menggunakan basis sto3g untuk $Li_2S$, terdapat 19 orbital spasial dengan 22 elektron. Untuk menjalankan kasus (10o,10e) dengan perhitungan HiVQE, Anda dapat mendefinisikan 10 orbital aktif dan enam orbital beku. Pada setiap iterasi, 100 shots akan digunakan untuk men-sample konfigurasi elektron yang dihasilkan oleh Circuit kuantum ExcitationPreserving (`epa`) dengan entanglement `circular` dan dua pengulangan (`reps`). Jumlah maksimum iterasi diatur menjadi 30 untuk memastikan terminasi iterasi dengan konvergensi energi.
...
### Langkah 2 dan 3: Optimalkan masalah untuk eksekusi pada hardware kuantum dan jalankan menggunakan fungsi HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}

Siapkan loop `for` untuk menjalankan perhitungan HiVQE dengan geometri dan opsi yang telah didefinisikan. Job-job disubmit dalam loop `for`. Dalam tutorial ini, Anda akan mensubmit enam geometri dan mengambil hasilnya ketika semuanya sudah selesai. Dalam eksekusi fungsi utama, Anda perlu mendefinisikan `max_states` dan `max_expansion_states` untuk mengontrol ukuran maksimum matriks subspace dan mengontrol berapa banyak state yang dapat dihasilkan menggunakan metode ekspansi CI klasik per iterasi. ID job fungsi akan disimpan dalam dictionary dengan label geometri masing-masing untuk melacak dan memproses output lebih lanjut.
...
### Langkah 4: Pasca-proses dan bandingkan dengan metode klasik {#step-4-post-process-and-compare-with-classical-methods}
...
### Plotting the dissociation curve for Li\_2S {#plotting-the-dissociation-curve-for-li-2s}

Mari kita plot dan bandingkan hasil HiVQE dengan HF dan CASCI.
Anda dapat melihat bahwa semua perhitungan HiVQE cocok dengan hasil referensi klasik (CASCI).
...
## Bagian 2: FeP-NO (44Q) {#part-2-fep-no-44q}

Perlu dicatat bahwa untuk menjalankan fungsi ini pada sistem FeP-NO dengan pengaturan yang ditampilkan dalam contoh ini, Anda memerlukan lisensi yang mengizinkan penggunaan fungsi dengan minimal 44 qubit. Kirim email ke [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) untuk menanyakan cara mendapatkan lisensi.
...
### Langkah 2 dan 3: Optimalkan masalah untuk eksekusi perangkat keras kuantum dan jalankan menggunakan fungsi HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

Berdasarkan pengaturan HiVQE dan geometri yang ada, dapatkan hasil secara berurutan.

#### Kirim perhitungan d(Fe-N) = 1.75 $\AA$. {#submit-dfe-n-175-aa-calculation}
...
#### Kirim perhitungan d(Fe-N) = 2.00 $\AA$. {#submit-dfe-n-200-aa-calculation}
...
#### Kirim perhitungan d(Fe-N) = 5.00 $\AA$. {#submit-dfe-n-500-aa-calculation}
...
### Langkah 4: Pasca-pemrosesan dan perbandingan dengan metode klasik {#step-4-post-process-and-compare-with-classical-methods-1}
...
Silakan isi survei singkat ini untuk memberikan masukan mengenai tutorial ini. Pendapat Anda akan membantu kami meningkatkan konten dan pengalaman pengguna.

> **Catatan:** Survei ini dibuat oleh **IBM Quantum** dan mencakup **konten tutorial** (yang ditulis oleh IBM). doQumentation menyediakan website, terjemahan, dan eksekusi kode — untuk masukan mengenai hal-hal tersebut, silakan [buka GitHub issue](https://github.com/JanLahmann/doQumentation/issues).
