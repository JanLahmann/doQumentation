...
Setelah menyelesaikan tutorial ini, kamu akan mempelajari hal-hal berikut:
* Cara mengimplementasikan gate CNOT jarak jauh menggunakan dynamic circuits dengan pengukuran mid-circuit (MCM) dan classical feedforward;
* Cara mengimplementasikan gate yang setara menggunakan pendekatan berbasis unitary SWAP;
* Cara membandingkan kedua pendekatan dengan mengukur fidelity gate sebagai fungsi dari jarak qubit.

## Prasyarat {#prerequisites}

Kami sarankan pengguna sudah familiar dengan topik-topik berikut sebelum mengikuti tutorial ini:
* [Konsep dasar komputasi kuantum](/learning/courses/basics-of-quantum-information), termasuk Bell state, entanglement, dan quantum gates;
* Familiar dengan [dynamic circuits](/guides/classical-feedforward-and-control-flow) (pengukuran mid-circuit dan classical feedforward);
* Pengetahuan dasar tentang [Qiskit SDK](https://docs.quantum.ibm.com/guides) dan [IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime), serta akses ke [akun IBM Quantum](/guides/cloud-setup).
...
Dalam notebook ini, kita mengadaptasi protokol tersebut untuk hardware IBM Quantum dan membandingkan performanya sebagai fungsi dari pemisahan kontrol–target, membandingkannya dengan baseline berbasis unitary SWAP.
...
Sebelum menjalankan pada QPU nyata, kita memverifikasi bahwa sirkuit dinamis maupun unitary menghasilkan Bell state ideal pada simulator tanpa noise. Kita menggunakan IBM Quantum `Sampler` dengan `AerSimulator` sebagai mode backend, pada jarak kecil yaitu 6.
...
#### Inisialisasi sirkuit {#initialize-circuit}
...
#### Menyiapkan Bell pair {#prepare-bell-pairs}
...
#### Mengukur pasangan qubit tetangga dalam basis Bell {#measure-neighboring-qubit-pairs-in-the-bell-basis}
...
#### Terapkan koreksi feedforward untuk memperbaiki operator byproduct Pauli {#apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
#### Ukur qubit kontrol dan target {#measure-control-and-target-qubits}
...
Kita menggabungkan berbagai langkah yang didefinisikan di atas untuk membuat gate CX jarak jauh di dua ujung sebuah garis satu dimensi (1D). Langkah-langkahnya meliputi hal-hal berikut:
- Menginisialisasi qubit kontrol dalam $|+\rangle$
...
Sekarang bangun sirkuit yang mengukur dalam basis $XX$, $YY$, dan $ZZ$, sama seperti yang kita lakukan untuk sirkuit dinamis di atas.
...
Sekarang setelah kita membangun sirkuit dinamis dan sirkuit unitary untuk contoh skala kecil dengan `distance=6`, kita transpilasi keduanya untuk dijalankan terlebih dahulu pada simulator tanpa noise.
...
Kita sekarang bisa menjalankan eksperimen pada backend simulator tanpa noise. Kita menggunakan Qiskit Runtime Sampler dengan AerSimulator sebagai mode backend untuk mengeksekusi sirkuit.
...
Pada langkah ini, kita melakukan hal berikut:
...
Dalam simulasi tanpa noise, kita akan memverifikasi bahwa metrik fidelity adalah $1$ untuk sirkuit yang dibuat. Dalam eksperimen pada QPU nyata, analisis ini akan memberikan gambaran yang jelas tentang seberapa baik Circuit dinamis bekerja dibandingkan implementasi baseline unitary.

#### Metrik kualitas {#quality-metrics}

Untuk mengevaluasi keberhasilan protokol LRCX jarak jauh, kita mengukur seberapa dekat state keluaran dengan Bell state ideal. Cara yang mudah untuk mengkuantifikasi ini adalah dengan menghitung fidelity state menggunakan nilai ekspektasi operator Pauli. Kita dapat menghitung fidelity untuk Bell state pada state kontrol dan target setelah mengetahui $\braket{XX}$, $\braket{YY}$, dan $\braket{ZZ}$. Secara khusus,
...
Seperti yang diharapkan dalam simulasi tanpa noise, fidelity pada sirkuit dinamis maupun unitary adalah $1$.

## Contoh hardware skala besar {#large-scale-hardware-example}

Di sini kita menggabungkan semua detail tersebut menjadi satu alur kerja skala yang lebih besar, yang kemudian dijalankan pada hardware kuantum nyata.

### Membuat sirkuit untuk berbagai jarak {#generate-circuits-for-different-distances}

Sekarang kita membuat sirkuit CX jarak jauh untuk berbagai pemisahan qubit hingga 60 qubit. Untuk setiap jarak, kita membangun sirkuit yang mengukur dalam basis $XX$, $YY$, dan $ZZ$, yang nantinya akan digunakan untuk menghitung fidelitas.
...
Sekarang bahwa kita telah memiliki sirkuit dinamis dan unitary untuk berbagai jarak, kita siap untuk transpilasi. Pertama-tama kita perlu memilih perangkat backend.
...
Langkah berikut memastikan bahwa backend mendukung instruksi `if_else`, yang diperlukan untuk versi terbaru sirkuit dinamis. Karena fitur ini masih dalam akses awal, kita secara eksplisit menambahkan `IfElseOp` ke target backend jika belum tersedia.
...
Karena kita ingin membandingkan performa sirkuit dinamis dan unitary pada rantai 1D, kita menggunakan string Layer Fidelity untuk memilih topologi linear dari rantai qubit terbaik pada perangkat. Ini memastikan bahwa kedua jenis sirkuit ditranspilasi dengan batasan konektivitas yang sama, sehingga memungkinkan perbandingan performa yang adil.
...
Jika kamu menemukan pekerjaan ini menarik, kamu mungkin tertarik dengan materi-materi berikut:
