...
3. Dekomposisi Circuit:
   Setiap operasi permutasi didekomposisi menjadi set gate native yang kompatibel dengan hardware kuantum target. Kita menganalisis kedalaman dan jumlah Gate dua-Qubit (gate nonlokal) untuk setiap sirkuit yang didekomposisi.
...
Transpilasi setiap sirkuit permutasi menggunakan AI synthesis passes dan metode sintesis generik.
...
Catat metrik (kedalaman, jumlah Gate, waktu) untuk setiap sirkuit setelah transpilasi.
...
Grafik ini menyoroti hasil individual untuk setiap sirkuit (`qc_1` hingga `qc_10`) di berbagai metode sintesis:
