...
Sebelum menjalankan pada QPU nyata, kita memverifikasi bahwa Circuit menghasilkan Bell pair dengan mengujinya pada simulator tanpa noise menggunakan rantai empat-Qubit `[0, 1, 2, 3]`. Kita menggunakan IBM Quantum `Sampler` dengan `AerSimulator` sebagai mode backend untuk mengeksekusi Circuit.
