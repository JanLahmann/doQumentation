...
1. **Keseragaman:** Digest yang dihasilkan oleh CHF harus terdistribusi secara seragam dan terlihat acak. Tujuannya adalah memastikan output tidak membocorkan informasi apa pun tentang input.
...
Fakta bahwa ukuran domain input $Σ^*$ dari $H$ tidak terbatas sementara ko-domain $\{0,1\}^n$ terbatas berarti bahwa $H$ secara inheren merupakan pemetaan banyak-ke-satu yang memetakan tak hingga banyak input ke string n-bit mana pun.

Sifat uniformitas dan determinisme terangkum dengan baik dalam <DefinitionTooltip definition="Model kriptografi yang diidealisasikan yang sering digunakan dalam diskusi teoritis dan analisis protokol serta skema kriptografi; memungkinkan analisis sifat keamanan yang jelas dan ketat.">*model oracle acak*</DefinitionTooltip> dari hashing kriptografi.
...
![Gambar 1: Hashing aman untuk pemeriksaan integritas data](/learning/images/courses/quantum-safe-cryptography/cryptographic-hash-functions/data-integrity.svg)
...
![Gambar 2: Tanda tangan digital](/learning/images/courses/quantum-safe-cryptography/cryptographic-hash-functions/digital-signature.svg)
...
<AccordionItem title="<em>Detail matematis</em>">

Diberikan CHF $H$ dan digest $g$, seharusnya tidak mungkin secara komputasi untuk menemukan input $m$ mana pun dari pre-image $g$ sedemikian sehingga $H(m) = g$.
...
<AccordionItem title="<em>Detail matematis collision hash</em>">
