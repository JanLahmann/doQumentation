...
(b) Proses ortogonalisasi terdiri dari mengurangi proyeksi vektor baru terhadap semua vektor sebelumnya. Jika semua vektor sebelumnya membentang seluruh ruang vektor, maka mengurangi proyeksi terhadap seluruh subruang akan selalu menyisakan vektor nol bagi kita.

</AccordionItem>
</Accordion>

Misalkan seorang rekan peneliti sedang mendemonstrasikan metode Krylov yang diterapkan pada matriks mainan kecil. Apakah ada yang salah dengan pilihan matriks $A$ dan vektor awal $|\psi\rangle$ mereka?
...
Rekanmu secara tidak sengaja telah memilih eigenvector untuk vektor awalnya. Beroperasi dengan matriks pada vektor awal hanya akan mengembalikan vektor yang sama, diskalakan dengan eigenvalue. Ini tidak akan menghasilkan subruang berdimensi meningkat. Sarankan rekanmu untuk memilih vektor awal yang berbeda, pastikan itu bukan eigenvector.

</AccordionItem>
</Accordion>

Terapkan metode Krylov pada matriks yang diberikan, dengan memilih vektor awal baru yang tepat. Tuliskan perkiraan eigenvalue minimum pada orde ke-0 dan ke-1 dari subruang Krylov kamu.
...
$$
Untuk mendapatkan $|v_1\rangle$ kita menerapkan $A$ sekali pada $|v_0\rangle$, kemudian membuat $|v_1\rangle$ ortogonal terhadap $|v_0\rangle.$
$$
...
$$
Ini bisa dilakukan secara manual, tetapi paling mudah dilakukan menggunakan numpy:
...
```
Perkiraan eigenvalue minimum adalah -0.414.
...
Kita tertarik dengan bagaimana error kita berperilaku saat ditransformasikan oleh matriks kita. Jadi mari kita hitung norma-$A$ dari error tersebut. Yaitu
$$
\begin{aligned}
∥|e_0\rangle−|p_0\rangle∥_A^2&=\left(\langle e_0|A−\langle p_0|A\right)\left(|e_0\rangle−|p_0\rangle\right)\\
 ...
 \end{aligned}
$$
di mana kita telah menggunakan simetri dari $A$ dan juga bahwa $A |e_0\rangle = |R_0\rangle.$ Di sini $d$ adalah suatu konstanta yang tidak bergantung pada $|p_0\rangle$. Seperti disebutkan pada Bagian 1.2, norma-$A$ dari error bukan satu-satunya kuantitas yang bisa kita pilih untuk diminimalkan, tetapi ini adalah kuantitas yang baik. Kita ingin melihat bagaimana kuantitas ini bervariasi terhadap pilihan vektor koreksi kita $|p_0\rangle.$ Jadi kita definisikan fungsi $f$ dengan menetapkan
$$
f(|p_0\rangle)=\langle p_0|A|p_0\rangle−2\langle R_0|p_0\rangle+d.
$$
$f$ hanyalah error $|e_1\rangle$ sebagai fungsi dari koreksi $|p_0\rangle$ yang diukur dalam norma-$A$. Oleh karena itu, kita ingin memilih $|p_0\rangle$ sedemikian rupa sehingga $f(|p_0\rangle)$ sekecil mungkin. Untuk tujuan ini, kita menghitung gradien dari $f$. Dengan menggunakan simetri dari $A$ kita punya
$$
\nabla f(|p_0\rangle) = 2(A|p_0\rangle−|R_0\rangle).
$$
Gradien menunjuk ke arah kenaikan tercuram, artinya kebalikannya memberi kita arah di mana fungsi menurun paling banyak: arah __penurunan tercuram__. Pada tebakan awal kita $|x_0\rangle$, di mana $|p_0\rangle=0$, kita punya bahwa
$\nabla f(0) = -2|R_0\rangle.$
Jadi, fungsi $f$ menurun paling banyak pada arah residual $|R_0\rangle.$ Jadi pilihan awal kita akan paling diuntungkan dengan penambahan vektor $|p_0\rangle=\alpha_0 |R_0\rangle$ untuk suatu skalar $\alpha_0$.
...
### Periksa pemahamanmu {#check-your-understanding-1}

Pada alur kerja di atas, kita mengusulkan untuk meminimalkan norma-$A$ dari error. Kuantitas lain apa yang mungkin dipertimbangkan seseorang untuk diminimalkan dalam mencari ground state dan eigenvalue-nya?

<Accordion>
<AccordionItem title="Jawaban">

Seseorang bisa membayangkan menggunakan vektor residual alih-alih norma-$A$ dari error. Ada kasus-kasus di mana mempertimbangkan vektor error itu sendiri berguna.
...
Lihat plot halus dari waktu diagonalisasi versus dimensi matriks di atas.
...
Alih-alih mengimplementasikan operator evolusi waktu secara tepat kita bisa menggunakan dekomposisi Trotter untuk mengimplementasikan aproksimasinya. Mengulangi dekomposisi Trotter orde tertentu beberapa kali memberi kita pengurangan lebih lanjut dari error yang diperkenalkan dari aproksimasi. Berikut ini, kita langsung membangun implementasi Trotter dengan cara paling efisien untuk grafik interaksi dari Hamiltonian yang kita pertimbangkan (hanya interaksi tetangga terdekat). Dalam praktiknya kita menyisipkan rotasi Pauli $R_{xx}$, $R_{yy}$, $R_{zz}$ dengan kekuatan kopling $J_x,$ $J_y,$ dan $J_z$ dan sudut terparametrisasi $t$, yang berkorespondensi dengan implementasi aproksimasi dari $e^{-i (J_x XX + J_y YY + J_z ZZ) t}$. Mengingat perbedaan dalam definisi rotasi Pauli dan evolusi waktu yang kita coba implementasikan, kita harus menggunakan parameter $2*dt$ untuk mencapai evolusi waktu $dt$. Selanjutnya, kita membalik urutan operasi untuk jumlah pengulangan langkah Trotter yang ganjil, yang secara fungsional setara tetapi memungkinkan sintesis operasi yang berdekatan dalam satu unitary $SU(2)$. Ini menghasilkan circuit yang jauh lebih dangkal dibandingkan dengan yang diperoleh menggunakan fungsionalitas generik `PauliEvolutionGate()`.
...
#### Template Circuit untuk menghitung elemen matriks dari $\tilde{S}$ dan $\tilde{H}$ melalui uji Hadamard {#template-circuits-for-calculating-matrix-elements-of-tildes-and-tildeh-via-hadamard-test}
...
### 4.3 Langkah 3. Eksekusi menggunakan primitif IBM Quantum {#43-step-3-execute-using-an-ibm-quantum-primitive}
