...
`optimization_level` memiliki empat opsi, dari 0 hingga 3. Semakin tinggi tingkat optimisasi, semakin banyak upaya komputasional yang dikeluarkan untuk mengoptimalkan circuit. Level 0 tidak melakukan optimisasi dan hanya melakukan pekerjaan minimal agar circuit dapat dijalankan pada backend yang dipilih. Level 3 mengeluarkan upaya (dan biasanya runtime) paling banyak untuk mencoba mengoptimalkan circuit. Level 1 adalah tingkat optimisasi default.
Kita mentranspilasi circuit tanpa optimisasi (`optimization_level=0`) dan dengan optimisasi (`optimization_level=2`).
Kita melihat perbedaan besar dalam panjang circuit dari circuit yang ditranspilasi.
...
Selanjutnya kita membandingkan hasil dari menjalankan dua circuit persiapan GHZ state 5-qubit ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$) yang disintesis secara berbeda.
...
Apa yang telah kita lakukan dalam circuit baru dapat dilihat sebagai sintesis circuit "sadar batasan kopling" secara manual. Dengan kata lain: secara manual menyelesaikan sintesis circuit dan pemetaan circuit pada saat yang sama.
...
Secara umum, sintesis circuit bergantung pada aplikasi dan terlalu sulit bagi software untuk mencakup semua kemungkinan aplikasi. Transpiler Qiskit kebetulan tidak memiliki fungsi untuk mensintesis circuit persiapan GHZ state. Dalam kasus seperti itu, sintesis circuit manual seperti yang ditunjukkan di atas patut dipertimbangkan.
Dalam bagian ini, kita melihat lebih dalam bagaimana transpiler Qiskit bekerja menggunakan contoh circuit mainan berikut.
...
Kita mentranspilasi circuit tanpa optimisasi (`optimization_level=0`) dan dengan optimisasi (`optimization_level=2`).
Seperti yang bisa kamu lihat, ada perbedaan besar dalam panjang circuit dari circuit yang ditranspilasi.
