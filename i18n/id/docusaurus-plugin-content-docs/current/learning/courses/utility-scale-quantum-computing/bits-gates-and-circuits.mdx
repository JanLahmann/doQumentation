...
# Quantum bit, Gate, dan Circuit {#quantum-bits-gates-and-circuits}
...
Mari kita pertama-tama mensimulasikan aksi dari gate dua-qubit ini ketika q0 dan q1 keduanya $|0\rangle$, dan memperoleh statevector output. Sintaks Qiskit yang digunakan adalah ```qc.cx(control qubit, target qubit)```.
...
Jika kamu belum memiliki `qiskit-ibm-runtime`, kamu perlu menginstalnya terlebih dahulu. Untuk informasi lebih lanjut, [lihat referensi API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-service).
