...
- Bukan berarti komputer kuantum tidak berguna sebelum kita mencapai toleransi kesalahan atau koreksi error
...
Kesalahpahaman umum lainnya adalah bahwa semakin banyak qubit yang dimiliki komputer kuantum, semakin kuat komputer itu seharusnya. Meskipun skala komputasional itu penting, jumlah qubit saja tidak memberikan gambaran lengkap tentang kinerja komputasi kuantum. Perangkat keras kuantum modern biasanya dievaluasi di tiga dimensi: skala, kualitas, dan kecepatan.

Skala diukur dengan jumlah qubit yang dapat diprogram yang tersedia bagi pengguna. Ini adalah qubit yang bisa dikontrol, dimanipulasi, diukur, dan dimasukkan langsung ke dalam algoritma kuantum. Meningkatkan jumlah qubit yang dapat diprogram memperluas ukuran masalah yang bisa dijelajahi pada perangkat keras kuantum.

Kualitas diukur dengan operasi qubit (QuOps), yang mendeskripsikan berapa banyak operasi kuantum yang paling menuntut secara komputasional yang bisa dijalankan sistem secara andal sebelum error yang terakumulasi membebani perhitungan. Peningkatan pada waktu koherensi, fidelitas gate, prosedur kalibrasi, dan arsitektur semuanya berkontribusi pada kualitas komputasional yang lebih tinggi. Waktu koherensi dan dephasing (umumnya disebut sebagai $T_1$ dan $T_2$) tetap menjadi indikator penting kinerja qubit.

Kecepatan diukur dengan maksimum circuit per detik, juga dikenal sebagai throughput circuit. Metrik ini menangkap seberapa banyak komputasi berguna yang bisa dilakukan sistem kuantum sepanjang waktu dan mencerminkan baik pemrosesan kuantum maupun pemrosesan klasik pendukungnya.

Kamu mungkin juga menemukan metrik benchmarking tambahan seperti CLOPS (Circuit Layer Operations Per Second) dan EPLG (Errors Per Layered Gate). CLOPS mengukur kecepatan eksekusi dari beban kerja benchmark yang distandarkan, sementara EPLG memberikan wawasan tentang tingkat error lapisan gate dan fidelitas prosesor. Metrik-metrik ini tetap berharga untuk memahami karakteristik perangkat keras dan beban kerja tertentu, tetapi qubit yang dapat diprogram, operasi qubit, dan maksimum circuit per detik memberikan kerangka kerja yang lebih luas untuk membandingkan sistem.
...
Kemajuan berkelanjutan di ketiga dimensi kinerja, skala, kualitas, dan kecepatan, diperlukan untuk membangun komputer kuantum universal yang toleran terhadap kesalahan. Itulah mengapa, ketika melihat peta jalan IBM Quantum&reg;, kamu akan melihat bahwa beberapa lompatan antara prosesor tidak memiliki peningkatan besar dalam jumlah qubit yang dapat diprogram. Misalnya, perhatikan peningkatan qubit yang dapat diprogram yang sederhana antara Heron dan Nighthawk, karena itu bukan fokus utama dari peningkatan tersebut. Sebaliknya, Nighthawk menerapkan topologi konektivitas baru yang akan memungkinkan kode koreksi error yang berbeda.
...
Mitigasi error, di sisi lain, sudah digunakan secara rutin untuk meningkatkan hasil komputasi kuantum. Ide di balik mitigasi error adalah bahwa kita menerima bahwa error akan terjadi, dan mencoba memprediksi perilakunya untuk mengurangi efek error tersebut. Ada banyak teknik mitigasi error; banyak yang memerlukan beberapa kali menjalankan komputer kuantum ditambah beberapa pasca-pemrosesan klasik. Kecil kemungkinan koreksi error akan sepenuhnya menggantikan mitigasi error. Sebaliknya, kita memprediksi keduanya akan digunakan bersama untuk mengembalikan hasil terbaik yang mungkin dari komputer kuantum.
...
Dalam praktiknya, komputer kuantum saat ini paling baik dipahami sebagai sistem hibrida. Komputer klasik mengatur alur kerja &mdash; menyiapkan input, mengompilasi Circuit kuantum, menjadwalkan pekerjaan, dan melakukan pascapemrosesan hasil &mdash; sementara QPU hanya mengeksekusi bagian kuantum dari perhitungan. Bahkan ketika hardware kuantum berkembang, pembagian kerja ini diperkirakan akan bertahan, dengan kemajuan berfokus pada integrasi yang lebih ketat dan komunikasi yang lebih cepat antara sistem klasik dan QPU daripada menghilangkan komponen klasik sepenuhnya.

## Area aplikasi yang mungkin untuk komputasi kuantum {#likely-application-areas-of-quantum-computing}

Kami secara garis besar mengelompokkan bidang-bidang yang menurut kami paling akan terdampak oleh komputasi kuantum menjadi empat kategori: optimasi, simulasi Hamiltonian, Persamaan Diferensial Parsial (PDP), dan pembelajaran mesin.
...
Banyak masalah optimasi dikategorikan sebagai NP-hard dalam teori kompleksitas, artinya algoritma klasik biasanya mengandalkan heuristik atau aproksimasi untuk instans berukuran besar. Karena qubit berperilaku berbeda dari bit klasik, kita bisa memodelkan solusi secara berbeda. Ini mungkin memungkinkan kita menjelajahi ruang solusi lebih cepat atau lebih lengkap daripada algoritma klasik.

Pendekatan kuantum yang umum meliputi:

- Algoritma variasional, seperti Quantum Approximate Optimization Algorithm (QAOA)
...
- Teknik optimasi variasional

- Kernel kuantum yang secara implisit beroperasi dalam ruang fitur berdimensi tinggi


Namun, pembelajaran mesin adalah area yang sangat menantang untuk keunggulan kuantum. Pembelajaran mesin klasik sangat matang, dan model kuantum harus mengatasi kendala seperti pemuatan data, noise, dan penskalaan.
