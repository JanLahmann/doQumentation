...
Untuk menulis masalah QUBO sebagai Hamiltonian Ising, yang sebenarnya kita lakukan hanyalah melakukan perubahan variabel sederhana dari $x \in \{0, 1\}^n$ menjadi $z \in \{-1, 1\}^n$:
...
Pertama, perlu dicatat bahwa kita dapat menulis ulang ekspresi QUBO kita sebagai jumlah dari suku-suku matriks:

$$
x^TQx = \sum_{ij}Q_{ij}x_ix_j
$$

Menerapkan perubahan variabel menghasilkan

$$
\sum_{ij}Q_{ij}(\frac{1-z_i}{2})(\frac{1-z_j}{2}) = \sum_{ij}\frac{Q_{ij}}{4}(1 - z_i - z_j + z_iz_j)
$$

yang dapat disusun ulang sebagai berikut.

$$
\sum_{ij}\frac{Q_{ij}}{4}z_iz_j - \frac{Q_{ij}}{4}z_i - \frac{Q_{ij}}{4}z_j + \frac{Q_{ij}}{4}
$$

Kita dapat menghilangkan suku konstan karena tidak akan memengaruhi $z$ mana yang meminimalkan masalah QUBO. Selain itu, kita dapat mengalikan ekspresi ini dengan empat untuk memulihkan nilai asli $Q$, karena ini juga tidak memengaruhi pilihan optimal $z$.

$$
\sum_{ij}Q_{ij}z_iz_j - Q_{ij}z_i - Q_{ij}z_j = z^TQz + (-\sum_{ij} Q_{ij}z_i + Q_{ij}z_j)
$$

Dengan mengelompokkan suku-suku linear yang serupa, kita dapat mendefinisikan vektor koefisien linear $b$ menggunakan persamaan

$$
b_i = -\sum_{j} Q_{ij} + Q_{ji}
$$

yang menyelesaikan transformasi variabel ke $z \in \{-1, 1\}^n$ setelah disubstitusikan ke dalam persamaan sebelumnya.
...
Pada akhirnya, minimasi ekspresi QUBO sama dengan minimasi ekspresi ini, di mana $b$ adalah koefisien skalar real:

$$
\min_{x\in\{0,1\}^n} x^TQx\Longleftrightarrow \min_{z\in\{-1,1\}^n}z^TQz + b^Tz
$$

Menulis ulang sedikit lagi dan kita memiliki Hamiltonian fungsi biaya, di mana minimum ekspresi mewakili ground state dan `Z` adalah operator Pauli Z:
...
Sejauh ini, kita telah mentranspilasi circuit dengan membiarkan parameter gamma dan beta apa adanya — tapi kita sebenarnya tidak bisa menjalankan circuit tanpa menentukan parameter ini. Dalam alur kerja QAOA, parameter QAOA yang optimal ditemukan dalam loop optimisasi iteratif, di mana kita menjalankan serangkaian evaluasi circuit dan kemudian menggunakan optimizer klasik untuk menemukan parameter 𝛽 dan 𝛾 yang optimal. Namun, kita perlu memulai dari suatu titik, jadi kita membuat perkiraan awal $\gamma=\pi/2$ dan $\beta=\pi$.
