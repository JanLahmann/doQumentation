...
<IBMVideo id="134063416" title="Dalam video ini, Olivia Lanes menjelaskan Circuit kuantum dan primitif. Ia mencakup primitif IBM Quantum Estimator dan Sampler."/>
...
Setiap prosesor dapat dikarakterisasi menggunakan tiga metrik kinerja mendasar yang menggambarkan skala, kualitas, dan kecepatannya: qubit yang dapat diprogram, operasi qubit, dan jumlah maksimum Circuit per detik.

* **Qubit yang dapat diprogram:** Ini adalah qubit yang dapat dikontrol langsung dan dimasukkan pengguna ke dalam algoritma kuantum. Metrik ini menangkap skala komputasi yang tersedia untuk aplikasi.


* **Operasi qubit (QuOps):** Metrik ini menggambarkan berapa banyak operasi kuantum yang paling menuntut secara komputasi yang dapat dijalankan secara andal oleh prosesor sebelum error yang terakumulasi membatasi komputasi. Ini berfungsi sebagai ukuran kualitas dan kemampuan komputasi.


* **Jumlah maksimum Circuit per detik:** Metrik ini mengukur throughput Circuit, atau berapa banyak Circuit kuantum yang dapat dijalankan sistem per detik. Throughput yang lebih tinggi memungkinkan lebih banyak pekerjaan berguna diselesaikan dalam jumlah waktu tertentu.


Anda mungkin juga menemukan metrik prosesor tambahan pada dokumentasi teknis, studi benchmarking, dan laporan kinerja:

EPLG (Errors Per Layered Gate). Ukuran error rata-rata yang diperkenalkan oleh operasi Gate entangling berlapis. EPLG yang lebih rendah umumnya menunjukkan kualitas Gate yang lebih tinggi dan akumulasi error yang lebih rendah.

CLOPS (Circuit Layer Operations Per Second). Benchmark yang mengukur seberapa cepat sistem kuantum dapat menjalankan beban kerja terstandardisasi yang mencakup operasi kuantum dan pemrosesan klasik terkait.

Metrik-metrik tambahan ini memberikan informasi yang lebih rinci tentang perilaku perangkat keras, sementara qubit yang dapat diprogram, operasi qubit, dan jumlah maksimum Circuit per detik memberikan gambaran tingkat tinggi tentang skala, kualitas, dan kecepatan suatu sistem.
...
## Perangkat lunak: Qiskit dan IBM Quantum Compute Service {#software-qiskit-and-ibm-quantum-compute-service}

Untuk mengubah masalah kuantummu menjadi instruksi untuk komputer kuantum, kamu akan menggunakan Qiskit, kit pengembangan perangkat lunak open-source yang dirancang untuk bekerja pada komputer kuantum yang dikembangkan oleh IBM. Ada juga Ekosistem Qiskit — kumpulan tutorial perangkat lunak dan fungsi yang membangun atau memperluas fungsionalitas inti Qiskit — dan Quantum Compute — layanan komputasi kuantum dan model pemrograman yang memungkinkan pengguna merancang dan mengoptimalkan beban kerja kuantum mereka dan mengeksekusinya secara efisien menggunakan primitif IBM Quantum.
...
3.	**Eksekusi.** Primitif IBM Quantum menyediakan antarmuka ke perangkat keras IBM Quantum yang memungkinkan Circuit yang telah ditranspilasi untuk dijalankan. Langkah ini juga mencakup penggunaan teknik penekanan dan mitigasi kesalahan, yang sebagian besar bisa diabstraksikan dari pengguna.
...
Selanjutnya, kita perlu memilih prosesor kuantum yang memiliki cukup qubit berkualitas tinggi agar kita bisa menjalankan Circuit kuantum kita. Buat keputusan ini dipandu oleh tiga metrik utama yang dibahas di bagian perangkat keras: qubit yang dapat diprogram, operasi qubit, dan jumlah maksimum Circuit per detik. Bergantung pada aplikasinya, metrik tambahan seperti EPLG dan CLOPS juga dapat memberikan informasi berguna tentang kualitas Circuit dan kinerja eksekusi yang diharapkan.
...
Sekarang kita siap untuk mengeksekusi program kuantum. Primitif IBM Quantum menyediakan antarmuka ke perangkat keras IBM Quantum, dan mereka juga mengabstraksikan penekanan dan mitigasi kesalahan dari pengguna. Ada dua primitif yang bisa dipilih: Sampler dan Estimator.
...
[IBM Quantum Estimator](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator) menggunakan proses aljabar yang kompleks untuk memperkirakan nilai ekspektasi pada perangkat kuantum nyata dengan memecah observable menjadi kombinasi observable lain dengan eigenbasis yang diketahui.
