...
Salah satu cara untuk memahami unitary di atas adalah melalui analogi dengan propagator dalam sistem fisik. Unitary di atas adalah operator evolusi unitary, $\exp(it\mathcal{H})$, untuk Hamiltonian, $\mathcal{H}$, mirip dengan model Ising, di mana parameter waktu, $t$, digantikan dengan nilai data untuk menggerakkan evolusi. Ekspansi dari operator unitary ini menghasilkan circuit PFM. Konektivitas entangling dalam $S$ dapat diinterpretasikan sebagai kopling Ising dalam kisi spin.
Mari kita pertimbangkan sebuah contoh operator Pauli $Y$ dan $XX$ yang merepresentasikan interaksi tipe Ising tersebut. Qiskit menyediakan kelas `pauli_feature_map` untuk menginstansiasi PFM dengan pilihan gate satu- dan $n$-qubit, yang dalam contoh ini akan diteruskan sebagai string Pauli `‘Y’` dan `‘XX’`. Biasanya, $n$ adalah 1 atau 2 untuk interaksi masing-masing satu- dan dua-qubit. Skema entanglement adalah "linear," artinya hanya qubit tetangga terdekat dalam circuit kuantum yang dikopel. Perhatikan bahwa ini tidak berkorespondensi dengan qubit tetangga terdekat pada komputer kuantum itu sendiri, karena circuit kuantum ini adalah lapisan abstraksi.
