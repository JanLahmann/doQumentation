...
Momen magnetik kecil akan berputar menuju kesejajaran dengan medan eksternal. Namun ketika sejenak sejajar, ia akan memiliki energi kinetik rotasional yang akan membuatnya terus berputar melampaui medan dan kembali keluar dari kesejajaran. Perilaku ini bahkan bisa dilihat pada magnet klasik besar. Namun dalam sistem klasik tersebut, osilasi momen magnetik kecil akhirnya berhenti karena energi dibuang menjadi panas. Namun dalam sistem tanpa mekanisme disipasi seperti itu, osilasi seharusnya berlanjut tanpa batas.
...
- Langkah 3: Jalankan menggunakan IBM Quantum primitives
...
Sekarang kita akan menggunakan sudut-sudut di atas sebagai parameter dalam sirkuit kuantum. Kita hanya mempertimbangkan satu partikel sekaligus, jadi kita hanya akan menggunakan satu qubit dalam sirkuit kita, dan kita hanya memerlukan satu register klasik.
...
Untuk menjalankan eksperimen kita pada komputer kuantum sungguhan, kita perlu memuat IBM Quantum Compute Service, dan memilih komputer kuantum (atau "backend"). Di bawah ini, kita cukup memilih komputer kuantum yang paling tidak sibuk yang tersedia bagi kita.
...
Kita sekarang harus men-transpile sirkuit, artinya kita harus memetakan sirkuit kita ke gate dasar yang tersedia pada komputer kuantum yang dipilih, dan kita ingin mengoptimalkan sirkuit kita untuk dijalankan pada komputer kuantum tersebut.
...
### Langkah 3: Jalankan menggunakan IBM Quantum primitives {#step-3-execute-using-ibm-quantum-primitives}
...
Sirkuit yang ingin kita buat adalah sama seperti sebelumnya. Satu-satunya perbedaan adalah bahwa kali ini, kita akan membangun sirkuit menggunakan gate dengan parameter bebas $\theta$ dan $\phi$. Nilai numerik untuk parameter-parameter tersebut kemudian akan ditetapkan untuk setiap run baru.
...
Menjalankan sirkuit kuantum hanya untuk satu shot dan melakukannya untuk banyak konfigurasi acak adalah alur kerja yang tidak biasa untuk komputer kuantum. Ini tentu bisa dilakukan, tetapi demi kesederhanaan, kita akan menggunakan simulator lokal di sini.
...
Ini mungkin terlihat sepele. Bagaimanapun, jika sebuah qubit diukur dalam satu keadaan, dan kemudian saya mengukurnya lagi, dengan cepat, mengapa tidak akan tetap dalam keadaan itu? Tapi jika kita benar-benar memperhatikan seluk-beluk ini, itu mungkin membantu kita memilih alat matematika untuk mendeskripsikan fenomena ini.
...
- Ketika kita menginisialisasi keadaan sistem biner kita secara acak (seperti partikel spin-1/2 yang keluar dari oven), salah satu dari dua hasil pengukuran biner mungkin terjadi.
...
Ingat bahwa dalam eksperimen di atas, kita menemukan bahwa qubit yang awalnya dalam keadaan $|0\rangle$ tetap dalam keadaan itu saat pengukuran berikutnya. Hal yang sama berlaku untuk $|1\rangle$. Ini berarti qubit yang ada dalam $|0\rangle$ memiliki probabilitas nol untuk diukur dalam keadaan $|1\rangle$ (kecuali efek noise). Hubungan yang kita butuhkan antara hasil kali dalam dan probabilitas pengukuran kemudian memberi tahu kita bahwa
...
Kita sudah cukup jauh dalam analisis kita. Memilih bentuk untuk vektor keadaan kita memungkinkan kita membangun sebuah matriks yang mendeskripsikan sesuatu tentang fenomena fisika yang terjadi di sini. Secara khusus, karena eksperimen Stern-Gerlach asli mengukur pemisahan lintasan berdasarkan komponen momentum sudut spin sepanjang sumbu $z$, kita ingin sebuah operator yang mendeskripsikan tepatnya itu: $S_z$. Hubungan penting lainnya ke eksperimen adalah bahwa dari jumlah defleksi, waktu perjalanan, dan kekuatan medan magnet yang diketahui, kita dapat menentukan besaran komponen $z$ dari spin. Meskipun ini membutuhkan banyak asumsi tentang presisi pengaturan eksperimen, di sini kita hanya akan menegaskan kembali bahwa komponen $z$ dari momentum sudut spin yang diukur adalah $\pm \hbar/2$.
...
Buat kode sirkuit kuantum untuk membuat keadaan awal yang diinginkan, lalu gunakan hardware nyata atau simulator untuk memeriksa hasil dari satu pengukuran, dan dari ensemble ratusan atau bahkan ribuan pengukuran.
