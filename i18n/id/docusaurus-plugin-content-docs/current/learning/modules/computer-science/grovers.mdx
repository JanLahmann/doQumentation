...
Di sini, kita mendemonstrasikan cara membuat oracle Grover dan menggunakan `GroverOperator` dari library Circuit Qiskit untuk dengan mudah mengatur instance pencarian Grover. Primitif `Sampler` IBM Quantum memungkinkan eksekusi Circuit Grover yang mulus.
...
- Langkah 3: Eksekusi menggunakan primitif IBM Quantum
...
### Langkah 3: Eksekusi menggunakan primitif IBM Quantum {#step-3-execute-using-ibm-quantum-primitives}

Kita ingin melakukan banyak pengukuran dan melihat state mana yang paling mungkin. Amplifikasi amplitudo seperti ini adalah masalah sampling yang cocok untuk dieksekusi dengan primitif `Sampler` IBM Quantum.

Perlu dicatat bahwa metode `run()` dari IBM Quantum SamplerV2 menerima iterable dari primitive unified blocks (PUB). Untuk Sampler, setiap PUB adalah iterable dalam format (circuit, parameter_values). Namun, minimal, ia menerima daftar Circuit kuantum.
...
### Uji pemahamanmu {#check-your-understanding-1}
...
### Langkah 1: Petakan input klasik ke masalah kuantum {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
### Langkah 2: Optimalkan masalah untuk eksekusi hardware kuantum {#step-2-optimize-problem-for-quantum-hardware-execution-1}
...
### Langkah 3: Eksekusi menggunakan primitif IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-1}
...
### Langkah 4: Post-process dan kembalikan hasil dalam format klasik yang diinginkan {#step-4-post-process-and-return-result-in-desired-classical-format-1}
...
### Langkah 3: Eksekusi menggunakan primitif IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-2}
