...
(a) Normalisasi fungsi gelombang trial dengan menentukan $A$ sedemikian sehingga $$ \int_{-\infty}^{\infty} |\Psi_\text{trial}|^2 dx = 1 $$.

<Accordion>
<AccordionItem title="Jawaban">

Untuk menormalisasi fungsi gelombang trial yang diberikan:
...
(b) Hitung nilai ekspektasi Hamiltonian $\hat{H}$ yang diberikan oleh $$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V(x)$$ di mana $V(x)  = \frac{1}{2}m\omega^2x^2$, yang berhubungan dengan potensial osilator harmonik sederhana.

<Accordion>
<AccordionItem title="Jawaban">

Hamiltonian untuk osilator harmonik adalah:
...
Optimasi $ \alpha $ untuk energi minimum
...
Untuk VQE, ansatz (jamak: ansätze) terdiri dari dua komponen utama. Yang pertama adalah persiapan keadaan awal, yang menyiapkan keadaan qubit dengan menerapkan Gate kuantum tanpa parameter variasional. Komponen kedua adalah Circuit kuantum yang diparameterisasi, sebuah sirkuit kuantum khusus dengan parameter yang dapat disesuaikan, mirip dengan kenop di radio. Parameter-parameter ini akan digunakan untuk bagian terakhir — optimizer klasik — untuk membantu kita mencapai ground state terbaik yang mungkin.
...
Mari mulai dengan memuat beberapa paket yang diperlukan, termasuk primitif IBM Quantum. Kita juga akan memilih komputer kuantum yang paling tidak sibuk yang tersedia.
...
Catatan: Kamu bisa mendapatkan hasil yang lebih baik dengan menetapkan tingkat mitigasi error menggunakan [`resilience_level`](/guides/error-mitigation-and-suppression-techniques). Nilai defaultnya adalah 1, dan jika kamu menetapkan nilai yang lebih tinggi, ini akan menggunakan lebih banyak waktu QPU tetapi mungkin menghasilkan hasil yang lebih baik.
...
Berbeda dengan perhitungan atom hidrogen tunggal ($H$), untuk menghitung ground state dari molekul hidrogen ($H_2$), kita juga harus mempertimbangkan gaya tolak-menolak yang bekerja antara inti dari kedua atom hidrogen, selain energi yang terkait dengan orbital elektronik. Pada langkah ini, kita akan memberikan nilai ini sebagai konstanta, dan kita akan benar-benar menghitung nilai ini pada soal pemeriksaan.
...
### Periksa pemahamanmu {#check-your-understanding-4}
...
Gunakan [hukum Coulomb](https://en.wikipedia.org/wiki/Coulomb%27s_law) dan [satuan atom](https://en.wikipedia.org/wiki/Atomic_units) untuk memastikan kamu mendapatkan nilai dalam `Hartree`.
...
## Hitung energi reaksi $H + H = H_2$ {#compute-reaction-energy-of-h-h-h-2}
