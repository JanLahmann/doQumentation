...
2. Selanjutnya, Gate $U_f$ diterapkan ke qubit-Qubit tersebut. Gate ini akan menerima $n$ qubit pertama sebagai input (yang sekarang berada dalam superposisi merata dari semua string $n$-bit yang mungkin) dan menerapkan fungsi $f(x)=s \cdot x$ ke qubit output, sehingga qubit ini sekarang berada dalam state: $ |- \oplus f(x)\rangle$. Berkat mekanisme phase kickback, state qubit ini tetap tidak berubah, tapi beberapa suku dalam state qubit input mendapatkan tanda minus:
