...
Sebagian kekuatan komputasi kuantum berasal dari "paralelisme kuantum," yang pada dasarnya adalah kemampuan untuk melakukan operasi pada banyak input sekaligus, karena state input qubit bisa berada dalam superposisi dari banyak state yang diperbolehkan secara klasik. NAMUN, meskipun sebuah sirkuit kuantum mungkin bisa mengevaluasi banyak state input sekaligus, mengekstrak semua informasi itu dalam satu kali pengukuran adalah hal yang mustahil.
...
Mari mulai dengan memuat beberapa paket yang diperlukan, termasuk primitif IBM Quantum. Kita juga akan memilih komputer kuantum yang paling tidak sibuk yang tersedia untuk kita.
...
Algoritma ini bekerja dengan cara yang sama seperti algoritma Deutsch: phase-kickback memungkinkan kita membaca state qubit 0 untuk menentukan apakah fungsinya constant atau balanced. Ini sedikit lebih sulit untuk dilihat dibanding kasus algoritma Deutsch dua-Qubit, karena state-nya akan mencakup penjumlahan atas $n$ qubit, sehingga menghitung state tersebut akan ditinggalkan sebagai latihan opsional untukmu di akhir modul. Algoritma akan mengembalikan bitstring semua 0 jika fungsinya constant, dan bitstring yang mengandung setidaknya satu 1 jika fungsinya balanced.
...
Kita bisa menentukan apakah fungsinya balanced atau constant dengan melihat Circuit di atas, tapi ingat, untuk keperluan masalah ini, kita menganggap fungsi ini sebagai "kotak hitam." Kita tidak bisa mengintip ke dalam kotak untuk melihat diagram Circuit. Sebagai gantinya, kita perlu melakukan kueri ke kotak tersebut.

Untuk melakukan kueri ke kotak, kita menggunakan algoritma Deutsch-Jozsa dan menentukan apakah fungsinya constant atau balanced:
...
Berapa banyak kueri yang dibutuhkan komputer klasik untuk menentukan dengan kepastian 100% apakah sebuah fungsi constant atau balanced? Ingat, secara klasik, satu kueri hanya memungkinkan kamu menerapkan fungsi ke satu bitstring.
...
Bagaimana jawabanmu di atas berubah jika kamu hanya perlu mengukur sampai salah satu hasil (balanced atau constant) lebih mungkin dari yang lain? Berapa banyak kueri yang dibutuhkan dalam kasus ini?
...
1. Pertama, Gate Hadamard diterapkan ke $n$ qubit input, dan Gate NOT plus Hadamard diterapkan ke qubit output, menghasilkan state:
...
$$
   ...
   State qubit 1 hingga $n$ bisa ditulis lebih sederhana sebagai penjumlahan atas semua $2^n$ state basis $n$-Qubit $|00...00\rangle, |00...01\rangle, |000...11\rangle, ..., |111...11\rangle$. Kita sebut himpunan state basis ini $\Sigma^n$. (Lihat [Fundamentals of Quantum Algorithms](/learning/courses/fundamentals-of-quantum-algorithms/quantum-query-algorithms/deutsch-jozsa-algorithm) untuk detail lebih lanjut.)

$$
|\Psi\rangle = |-\rangle \otimes \frac{1}{\sqrt{2^n}}\sum\limits_{x \in \Sigma^n}{|x\rangle}
$$

2. Selanjutnya, gate $U_f$ diterapkan pada qubit. Gate ini akan mengambil n qubit pertama sebagai input (yang sekarang berada dalam superposisi setara dari semua kemungkinan string n-bit) dan menerapkan fungsi $f(x)=s \cdot x$ pada qubit output, sehingga qubit ini sekarang berada dalam state: $ |- \oplus f(x)\rangle$. Berkat mekanisme phase kickback, state dari qubit ini tetap tidak berubah, tetapi beberapa suku dalam state qubit input mendapatkan tanda minus:
...
3. Sekarang, serangkaian Gate Hadamard berikutnya diterapkan ke qubit 0 hingga $n-1$. Melacak tanda minus dalam kasus ini bisa rumit. Berguna untuk mengetahui bahwa menerapkan lapisan Hadamard ke $n$ qubit dalam state basis standar $|x\rangle$ bisa ditulis sebagai:
...
Jadi, hanya dengan satu kueri, algoritma Deutsch-Jozsa akan mengembalikan string $s$ yang digunakan dalam fungsi: $f(x)=x \cdot s$ ketika kita terapkan ke masalah Bernstein-Vazirani. Dengan algoritma klasik, seseorang membutuhkan $n$ kueri untuk menyelesaikan masalah yang sama.
...
### Soal tantangan {#challenge-problem}
1. Algoritma Deutsch-Jozsa: Ingat bahwa kamu punya pertanyaan di atas yang memintamu menghitung state qubit perantara $\pi_1$ dan $\pi_2$ dari algoritma Deutsch. Lakukan hal yang sama untuk state $(n+1)$-Qubit perantara $\pi_1$ dan $\pi_2$ dari algoritma Deutsch-Jozsa, untuk kasus khusus $n=2$. Kemudian, verifikasi bahwa $\pi_3 = |-\rangle \otimes \sum\limits_{x_0...x_n}(-1)^{f(x_0...x_n)}|x_0 ... x_n\rangle$, sekali lagi, untuk kasus khusus $n=2$.
