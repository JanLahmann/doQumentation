---
title: "Menentukan opsi Sampler"
sidebar_label: "Menentukan opsi Sampler"
description: "Menentukan opsi saat membangun dengan primitif Sampler."
notebook_path: "guides/sampler-options.ipynb"
---

{/* doqumentation-source-hash: 817c424a */}

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

<OpenInLabBanner notebookPath="guides/sampler-options.ipynb" />




{/*
  DO NOT EDIT THIS CELL!!!
  This cell's content is generated automatically by a script. Anything you add
  here will be removed next time the notebook is run. To add new content, create
  a new cell before or after this one.
*/}

<Accordion>
<AccordionItem title="Versi paket">

Kode di halaman ini dikembangkan menggunakan persyaratan berikut.
Kami sarankan pakai versi ini atau yang lebih baru.

```
qiskit[all]~=2.4.0
qiskit-ibm-runtime~=0.46.1
```
</AccordionItem>
</Accordion>

Kamu bisa pakai opsi untuk menyesuaikan primitif Sampler. Bagian ini fokus pada cara menentukan opsi primitif Qiskit Runtime. Meskipun antarmuka metode `run()` primitif sama di semua implementasi, opsinya tidak demikian. Lihat referensi API yang sesuai untuk info tentang opsi [`qiskit.primitives.BackendSamplerV2`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.BackendSamplerV2) dan [`qiskit_aer.primitives.SamplerV2`](https://qiskit.github.io/qiskit-aer/stubs/qiskit_aer.primitives.SamplerV2.html).

<span id="pass-options"></span>
## Mengatur opsi Sampler {#set-sampler-options}

Kamu bisa mengatur opsi saat menginisialisasi Sampler, setelah menginisialisasi Sampler, atau kamu bisa memperbarui opsi setelah Sampler diinisialisasi. Untuk instruksi penggunaan teknik-teknik ini, lihat topik [Pengantar opsi](/guides/runtime-options-overview#options-precedence).

Selain itu, kamu bisa mengatur nilai `shots` di metode `run()`, seperti dijelaskan di bagian berikut.

<span id="run-method"></span>
### Metode Run() {#run-method}

Satu-satunya nilai yang bisa kamu masukkan ke `run()` adalah yang didefinisikan di antarmuka. Yaitu, `shots`. Ini menimpa nilai apa pun yang diatur untuk `default_shots` pada run saat ini.

```python
# Added by doQumentation — required packages for this notebook
!pip install -q qiskit qiskit-ibm-runtime
```

```python
from qiskit_ibm_runtime import QiskitRuntimeService
from qiskit_ibm_runtime import SamplerV2 as Sampler
from qiskit.circuit.library import random_iqp
from qiskit.transpiler import generate_preset_pass_manager

service = QiskitRuntimeService()
backend = service.least_busy(operational=True, simulator=False)

circuit1 = random_iqp(3)
circuit1.measure_all()
circuit2 = random_iqp(3)
circuit2.measure_all()

pass_manager = generate_preset_pass_manager(
    optimization_level=3, backend=backend
)

transpiled1 = pass_manager.run(circuit1)
transpiled2 = pass_manager.run(circuit2)

sampler = Sampler(mode=backend)
# Default shots to use if not specified in run()
sampler.options.default_shots = 500
# Sample two circuits at 128 shots each.
sampler.run([transpiled1, transpiled2], shots=128)
```

```text
<RuntimeJobV2('d8286680bvlc73d1vmu0', 'sampler')>
```

### Kasus khusus {#special-cases}
<span id="shots"></span>
#### Shots {#shots}

Metode `SamplerV2.run` menerima dua argumen: daftar PUB, di mana setiap PUB bisa menentukan nilai shots khusus untuk PUB tersebut, dan argumen keyword shots. Nilai shots ini adalah bagian dari antarmuka eksekusi Sampler, dan tidak bergantung pada opsi Runtime Sampler. Nilai ini diutamakan di atas nilai apa pun yang ditentukan sebagai opsi agar sesuai dengan abstraksi Sampler.

Namun, jika `shots` tidak ditentukan oleh PUB mana pun atau dalam argumen keyword run (atau semuanya `None`), maka nilai shots dari opsi yang digunakan, terutama `default_shots`.

Untuk merangkum, ini adalah urutan prioritas untuk menentukan shots di Sampler, untuk PUB tertentu:

1. Jika PUB menentukan shots, gunakan nilai itu.
2. Jika argumen keyword `shots` ditentukan di `run`, gunakan nilai itu.
4. Jika `twirling` diaktifkan (True secara default), maka hasil perkalian `num_randomizations` dan `shots_per_randomization`, yang ditentukan sebagai [opsi `twirling`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-twirling-options), yang digunakan.
5. Jika `sampler.options.default_shots` ditentukan, gunakan nilai itu.

Jadi, jika shots ditentukan di semua tempat yang mungkin, yang memiliki prioritas tertinggi (shots yang ditentukan di PUB) yang digunakan.

Contoh:

```python
from qiskit_ibm_runtime import QiskitRuntimeService
from qiskit_ibm_runtime import SamplerV2 as Sampler
from qiskit.circuit.library import random_iqp
from qiskit.transpiler import generate_preset_pass_manager

service = QiskitRuntimeService()
backend = service.least_busy(operational=True, simulator=False)

circuit1 = random_iqp(3)
circuit1.measure_all()
circuit2 = random_iqp(3)
circuit2.measure_all()

pass_manager = generate_preset_pass_manager(
    optimization_level=3, backend=backend
)

transpiled1 = pass_manager.run(circuit1)
transpiled2 = pass_manager.run(circuit2)

# Setting shots during primitive initialization
sampler = Sampler(mode=backend, options={"default_shots": 4096})

# Setting options after primitive initialization
# This uses auto-complete.
sampler.options.default_shots = 2000

# This does bulk update.  The value for default_shots is overridden if you specify shots with run() or in the PUB.
sampler.options.update(
    default_shots=1024, dynamical_decoupling={"sequence_type": "XpXm"}
)

# Sample two circuits at 128 shots each.
sampler.run([transpiled1, transpiled2], shots=128)
```

```text
<RuntimeJobV2('d82868ugbeec73alfa80', 'sampler')>
```
<span id="options-table"></span>
## Opsi yang tersedia {#available-options}

Tabel berikut mendokumentasikan opsi dari versi terbaru `qiskit-ibm-runtime`. Untuk melihat versi opsi yang lebih lama, kunjungi [referensi API `qiskit-ibm-runtime`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime) dan pilih versi sebelumnya.

<Accordion>
<AccordionItem title="`default_shots`">

Jumlah total shots yang digunakan per sirkuit per konfigurasi.

**Pilihan**: Integer >= 0

**Default**: None

[Dokumentasi API `default_shots`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-sampler-options#default_shots)
  </AccordionItem>

<AccordionItem title="`dynamical_decoupling`">

Mengontrol pengaturan mitigasi error dynamical decoupling.

[Dokumentasi API `dynamical_decoupling`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-sampler-options#dynamical_decoupling)
<Accordion>
<AccordionItem title="`dynamical_decoupling.enable`">

**Pilihan**: `True`, `False`

**Default**: `False`
  </AccordionItem>

<AccordionItem title="`dynamical_decoupling.extra_slack_distribution`">

**Pilihan**: `middle`, `edges`

**Default**: `middle`
  </AccordionItem>

<AccordionItem title="`dynamical_decoupling.scheduling_method`">

Pilihan: `asap`, `alap`
Default: `alap`
  </AccordionItem>

<AccordionItem title="`dynamical_decoupling.sequence_type`">

Pilihan: `XX`, `XpXm`, `XY4`
Default: `XX`
  </AccordionItem>

<AccordionItem title="`dynamical_decoupling.skip_reset_qubits`">

Pilihan: `True`, `False`
Default: `False`
  </AccordionItem>

</Accordion>

  </AccordionItem>

<AccordionItem title="`environment`">
[Dokumentasi API `environment`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-sampler-options#environment)
<Accordion>
<AccordionItem title="`environment.job_tags`">

Daftar tag.

**Pilihan**: None

**Default**: None
  </AccordionItem>

<AccordionItem title="`environment.log_level`">

**Pilihan**: DEBUG, INFO, WARNING, ERROR, CRITICAL

**Default**: WARNING
  </AccordionItem>

<AccordionItem title="`environment.private`">

**Pilihan**: `True`, `False`

**Default**: `False`
  </AccordionItem>

</Accordion>

  </AccordionItem>

<AccordionItem title="`execution`">
[Dokumentasi API `execution`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-sampler-options#execution)
<Accordion>
<AccordionItem title="`execution.init_qubits`">
Apakah mereset qubit ke ground state untuk setiap shot.

**Pilihan**: `True`, `False`

**Default**: `True`
  </AccordionItem>

<AccordionItem title="`execution.rep_delay`">
Jeda antara pengukuran dan sirkuit kuantum berikutnya.

**Pilihan**: Nilai dalam rentang yang disediakan oleh `backend.rep_delay_range`

**Default**: Diberikan oleh `backend.default_rep_delay`
  </AccordionItem>

<AccordionItem title="`execution.meas_type`">

**Pilihan**: `classified`, `kerneled`, `avg_kerneled`

**Default**: `classified`
  </AccordionItem>

</Accordion>

  </AccordionItem>

<AccordionItem title="`max_execution_time`">
Membatasi berapa lama job bisa berjalan, dalam detik. Lihat panduan [waktu eksekusi maksimum](/guides/max-execution-time) untuk detail.

**Pilihan**: Bilangan bulat detik dalam rentang [1, 10800]

**Default**: 10800 (3 jam)

[Dokumentasi API `max_execution_time`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-sampler-options#max_execution_time)
  </AccordionItem>

<AccordionItem title="`simulator`">

Opsi untuk dimasukkan saat mensimulasikan Backend

[Dokumentasi API `simulator`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-simulator-options)
<Accordion>
<AccordionItem title="`simulator.basis_gates`">

**Pilihan**: Daftar nama gate basis untuk di-unroll ke

**Default**: Sekumpulan semua gate basis yang didukung oleh [simulator Qiskit Aer](https://qiskit.github.io/qiskit-aer/stubs/qiskit_aer.AerSimulator.html)

  </AccordionItem>

<AccordionItem title="`simulator.coupling_map`">

**Pilihan**: Daftar interaksi dua qubit terarah

**Default**: None, yang berarti tidak ada kendala konektivitas (konektivitas penuh).

  </AccordionItem>

<AccordionItem title="`simulator.noise_model`">

**Pilihan**: [Qiskit Aer NoiseModel](/guides/build-noise-models), atau representasinya

**Default**: None

  </AccordionItem>

<AccordionItem title="`simulator.seed_simulator`">

**Pilihan**: Integer

**Default**: None

  </AccordionItem>

</Accordion>

  </AccordionItem>

<AccordionItem title="`twirling`">

Opsi Twirling

[Dokumentasi API `twirling`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-twirling-options)
<Accordion>
<AccordionItem title="`twirling.enable_gates`">

**Pilihan**: True, False

**Default**: False

  </AccordionItem>

<AccordionItem title="`twirling.enable_measure`">

**Pilihan**: True, False

**Default**: False

  </AccordionItem>

<AccordionItem title="`twirling.num_randomizations`">

**Pilihan**: `auto`, Integer >= 1

**Default**: `auto`

  </AccordionItem>

<AccordionItem title="`twirling.shots_per_randomization`">

**Pilihan**: `auto`, Integer >= 1

**Default**: `auto`

  </AccordionItem>

<AccordionItem title="`twirling.strategy`">

**Pilihan**: `active`, `active-circuit`, `active-accum`, `all`

**Default**: `active-accum`

  </AccordionItem>

</Accordion>

  </AccordionItem>

<AccordionItem title="`experimental`">

Opsi eksperimental, jika tersedia.

  </AccordionItem>

</Accordion>

<span id="options-compatibility-table"></span>
## Kompatibilitas fitur {#feature-compatibility}

Fitur-fitur Runtime tertentu tidak bisa digunakan bersama dalam satu job. Klik tab yang sesuai untuk daftar fitur yang tidak kompatibel dengan fitur yang dipilih:

<Tabs>
  <TabItem value="Dynamic" label="Sirkuit dinamis">
  Tidak kompatibel dengan:
  - Dynamical decoupling

  Catatan lain:
  - Gate twirling bisa diterapkan ke sirkuit dinamis, tapi hanya pada gate yang tidak berada di dalam blok kondisional. Measurement twirling hanya bisa diterapkan ke pengukuran terminal.
  - Kompatibel dengan gate fraksional saat menggunakan `qiskit-ibm-runtime` v0.42.0 atau yang lebih baru.

  </TabItem>
  <TabItem value="DD" label="Dynamical decoupling">
  Tidak kompatibel dengan sirkuit dinamis.

  </TabItem>

  <TabItem value="Fractional" label="Gate fraksional">
  Tidak kompatibel dengan:
  - Gate twirling

  Kompatibel dengan sirkuit dinamis saat menggunakan `qiskit-ibm-runtime` v0.42.0 atau yang lebih baru.

  </TabItem>

  <TabItem value="Twirling" label="Gate twirling">
  Tidak kompatibel dengan gate fraksional atau dengan stretches.

  Catatan lain:
  - Gate twirling bisa diterapkan ke sirkuit dinamis, tapi hanya pada gate yang tidak berada di dalam blok kondisional. Measurement twirling hanya bisa diterapkan ke pengukuran terminal.
  - Tidak bekerja dengan entangler non-Clifford.

  </TabItem>

</Tabs>

## Langkah berikutnya {#next-steps}

<Admonition type="tip" title="Rekomendasi">
- Tinjau panduan [Pengantar opsi](/guides/runtime-options-overview).
- Temukan detail lebih lanjut tentang metode `SamplerV2` di [referensi API Sampler](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2).
- Tentukan [mode eksekusi](execution-modes) mana yang akan digunakan untuk job kamu.
- Pelajari tentang [manajemen noise dengan Sampler](/guides/sampler-noise-management).
</Admonition>
