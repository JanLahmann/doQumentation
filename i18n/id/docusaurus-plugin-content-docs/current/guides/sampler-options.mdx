...
Kamu bisa mengatur opsi saat menginisialisasi Sampler, setelah menginisialisasi Sampler, atau kamu bisa memperbarui opsi setelah Sampler diinisialisasi. Untuk instruksi menggunakan teknik-teknik ini, lihat topik [Introduction to options](/guides/runtime-options-overview#options-precedence).

Selain itu, kamu bisa mengatur nilai `shots` dalam metode `run()`, seperti yang dijelaskan pada bagian berikut.
...
4. Jika `twirling` diaktifkan, maka hasil perkalian `num_randomizations` dan `shots_per_randomization`, yang ditentukan sebagai [opsi `twirling`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-twirling-options), yang digunakan.
...
<Accordion>
  ...
  - Measurement twirling tidak kompatibel dengan instruksi [`store`](https://docs.quantum.ibm.com/api/qiskit/circuit#store).
