---
description: 'Cara menginisialisasi akun IBM Quantum Compute Service, membuat instance akun, dan memuat kredensial yang tersimpan.'
title: 'Inisialisasi akun IBM Quantum Compute Service kamu'
---

{/* doqumentation-source-hash: ab5dc076 */}

# Inisialisasi akun IBM Quantum Compute Service kamu {#initialize-your-ibm-quantum-compute-service-account}

Sebelum menggunakan IBM Quantum&reg; Compute Service, kamu harus menginisialisasi (membuat instance) akunmu dengan mengirimkan (memuat) kredensial.  Kredensial ini bisa dikirimkan secara manual setiap kali kamu menginisialisasi IBM Quantum Compute Service, atau kamu bisa menyimpannya untuk digunakan kembali.
...
1. Pastikan kamu bekerja di lingkungan Python aktif dengan [Qiskit SDK dan `qiskit-ibm-runtime` yang sudah terinstal.](/guides/install-qiskit#local)
...
## Hubungkan Qiskit dengan instance IBM Quantum Compute Service kamu {#connect-qiskit-with-your-ibm-quantum-compute-service-instance}
...
Berikut adalah kode dasar untuk menghubungkan Qiskit dengan instance Quantum Compute kamu. Namun, ada beberapa cara berbeda untuk menyesuaikan opsi `QiskitRuntimeService`, tergantung kebutuhanmu. Opsi-opsi ini dijelaskan di bagian berikut.
...
Cara tercepat untuk menjalankan instance `QiskitRuntimeService` adalah metode instansiasi langsung: berikan API token (key) dan CRN (identifier instance) secara eksplisit setiap kali kamu perlu menginisialisasi IBM Quantum Compute Service. Lihat [Temukan kredensial akses kamu](#access-credentials) jika perlu.
...
3. Jika akunmu memiliki beberapa instance yang bisa mengakses QPU yang diminta, sistem menggunakan preferensi paket yang kamu tentukan untuk memilih paket dan instance. Untuk `qiskit-ibm-runtime` v0.42 dan yang lebih baru, paket gratis diprioritaskan secara default.
...
### Pengujian lokal dengan `qiskit-ibm-runtime` {#local-testing-with-qiskit-ibm-runtime}
