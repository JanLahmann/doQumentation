...
<Admonition type="tip">
**Qiskit Serverless sedang mendapatkan peningkatan, dan fitur-fiturnya berubah dengan cepat.** Selama fase pengembangan ini, temukan catatan rilis dan dokumentasi terbaru di halaman [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html).
</Admonition>
...
Jika kamu membaca sel-sel kode ini secara lokal di sebuah notebook, kamu akan melihat [perintah magic](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Menjalankan sel dengan perintah magic ini akan menyimpannya ke disk alih-alih menjalankannya.
</Admonition>
...
Untuk tugas-tugas klasik yang bisa diparalelkan, gunakan dekorator `@distribute_task` untuk mendefinisikan kebutuhan komputasi yang diperlukan untuk menjalankan sebuah tugas. Mulai dengan mengingat kembali contoh `transpile_remote.py` dari topik [Tulis program Qiskit Serverless pertamamu](/guides/serverless-first-program) dengan kode berikut.
...
Berikut ini menunjukkan cara mengunggah file. Pastikan kamu sudah mengautentikasi ke Qiskit Serverless dengan akun IBM Quantum kamu (lihat [Unggah ke Qiskit Serverless](/guides/serverless-first-program#upload-to-qiskit-serverless) untuk instruksinya).
...
- Lihat contoh lengkap yang [memindahkan kode yang sudah ada ke Qiskit Serverless](/guides/serverless-port-code).
