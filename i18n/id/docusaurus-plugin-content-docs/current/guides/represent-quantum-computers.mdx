---
title: "Merepresentasikan komputer kuantum untuk transpiler"
sidebar_label: "Merepresentasikan komputer kuantum untuk transpiler"
...
- [`Target`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.Target) berisi semua batasan yang relevan dari sebuah perangkat, seperti set instruksi yang didukungnya, konektivitas qubit, dan informasi pulse atau timing.
- [`Backend`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2) memiliki `Target` secara default, berisi informasi tambahan -- seperti [`InstructionScheduleMap`](https://docs.quantum.ibm.com/api/qiskit/1.4/qiskit.pulse.InstructionScheduleMap), dan menyediakan antarmuka untuk mengirimkan job Circuit kuantum.
...
Halaman ini menunjukkan beberapa contoh pengiriman informasi QPU ke transpiler.

<span id="default-config"></span>
## Konfigurasi default {#default-configuration}

Penggunaan transpiler yang paling sederhana adalah memberikan semua informasi QPU dengan menyediakan `Backend` atau `Target`. Untuk lebih memahami cara kerja transpiler, buat sebuah sirkuit dan transpilasikan dengan informasi yang berbeda, seperti berikut.
...
Topik ini menunjukkan beberapa contoh pengiriman informasi ke transpiler.

<Admonition type="note">
Contoh-contoh ini menggunakan target dari backend mock [`FakeSherbrooke`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/fake-provider-fake-sherbrooke#fakesherbrooke) `qiskit_ibm_runtime`. Namun, kamu bisa mencobanya di backend nyata atau palsu apa pun yang kompatibel dengan Qiskit. Hasilmu mungkin berbeda.
</Admonition>
...
Contoh ini digunakan di bagian-bagian selanjutnya dari topik ini untuk mengilustrasikan bahwa coupling map dan set instruksi yang didukung adalah informasi esensial yang perlu diberikan ke transpiler untuk konstruksi Circuit yang optimal. QPU biasanya bisa memilih pengaturan default untuk informasi lain yang tidak diberikan, seperti timing dan scheduling.
...
## Set instruksi yang didukung {#supported-instructions}

Setiap komputer kuantum mendukung set instruksi terbatas. Setiap Gate dalam Circuit harus diterjemahkan ke elemen-elemen set ini. Set ini harus terdiri dari Gate satu dan dua qubit yang menyediakan set Gate universal, artinya operasi kuantum apa pun bisa didekomposisi menjadi Gate-Gate tersebut. Ini dilakukan oleh [BasisTranslator](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.passes.BasisTranslator), dan `basis_gates` bisa ditentukan sebagai argumen kata kunci ke transpiler untuk memberikan informasi ini.
...
Gate satu qubit default di `FakeSherbrooke` adalah `rz`, `x`, dan `sx`, dan Gate dua qubit default adalah `ecr` (echoed cross-resonance). Gate CX dibangun dari Gate `ecr`, jadi di beberapa QPU `ecr` ditentukan sebagai basis Gate dua qubit, sementara di QPU lain `cx` adalah defaultnya. Gate `ecr` adalah bagian _entangling_ dari Gate `cx`. Selain Gate kontrol, ada juga instruksi `delay` dan `measurement`.

<Admonition type="note">
QPU memiliki basis Gate default, tapi kamu bisa memilih Gate apa pun yang kamu mau, selama kamu menyediakan instruksi atau menambahkan pulse Gate (lihat [Membuat transpiler pass](/guides/custom-transpiler-pass).) Basis Gate default adalah yang kalibrasinya telah dilakukan di QPU, sehingga tidak perlu memberikan instruksi/pulse Gate lebih lanjut. Misalnya, di beberapa QPU `cx` adalah Gate dua qubit default dan `ecr` di QPU lainnya. Lihat daftar [Gate dan operasi native yang mungkin](/guides/qpu-information#native-gates) untuk detail lebih lanjut.
</Admonition>
