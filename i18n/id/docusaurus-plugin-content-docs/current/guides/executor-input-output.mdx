...
Seperti yang disebutkan sebelumnya, input ke primitif Executor adalah [`QuantumProgram`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/quantum-program-quantum-program), yang merupakan iterable dari objek [`QuantumProgramItem`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/quantum-program-quantum-program-item). Objek-objek ini bisa berupa dua tipe:
...
Item kedua berisi hasil menjalankan tugas kedua (sebuah `SamplexItem`) dalam program. Item ini berisi beberapa key. Key `meas`, yang merupakan nama register klasik sirkuit input, dipetakan ke array hasil register tersebut. Array ini memiliki bentuk `(randomizations, parameter sets, shots, classical bits)`, atau (28, 10, 1024, 3) dalam contoh ini. Selain itu, output berisi key `measurement_flips.meas`, yang merupakan koreksi bit-flip untuk membatalkan measurement twirling untuk register `meas`. Bentuk output ini akan menjadi (28, 10, 1, 3) untuk contoh kita karena hanya satu shot yang diperlukan untuk melakukan bit-flip.
