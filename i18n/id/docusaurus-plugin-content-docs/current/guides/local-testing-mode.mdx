---
description: 'Gunakan mode pengujian lokal untuk mensimulasikan hardware kuantum dengan IBM Quantum Compute'
notebook_path: guides/local-testing-mode.ipynb
sidebar_label: 'Mode pengujian lokal'
...
Primitif Executor tidak mendukung mode pengujian lokal.
</Admonition>

Untuk menggunakan mode pengujian lokal, tentukan salah satu fake backend dari ``qiskit_ibm_runtime.fake_provider`` atau tentukan Backend Qiskit Aer saat membuat instance primitif IBM Quantum atau sebuah Session.
...
- **Simulator Aer**: Simulator dari [Qiskit Aer](/guides/simulate-with-qiskit-aer) memberikan simulasi berkinerja lebih tinggi yang dapat menangani Circuit yang lebih besar dan [model noise kustom](/guides/build-noise-models). Daftar opsi metode simulasi tersedia saat kamu menggunakan `AerSimulator` dalam mode pengujian lokal. Lihat [contoh mode simulasi Clifford](#clifford-sim), yang mendemonstrasikan cara mensimulasikan Circuit Clifford dengan jumlah qubit yang besar secara efisien.
    ...
    <Accordion>
    <AccordionItem title="**Daftar metode simulasi yang tersedia dari Qiskit Aer**">
...
- Kamu bisa menentukan semua opsi IBM Quantum Compute dalam mode pengujian lokal. Namun, semua opsi kecuali shots diabaikan saat dijalankan di simulator lokal.
...
- Gunakan [kelas `Neat`](/guides/debug-qiskit-runtime-jobs) untuk menganalisis dampak noise pada beban kerja Estimator.
- Tinjau [contoh Sampler](/guides/sampler-examples) secara detail.
- Tinjau [contoh Estimator](/guides/estimator-examples) secara detail.
