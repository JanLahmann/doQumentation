---
description: 'Penjelasan teknik mitigasi dan supresi error yang tersedia melalui IBM Quantum Compute Service'
...
Pauli twirling diimplementasikan dengan menjepit sekumpulan Gate yang dipilih dengan Gate Pauli satu qubit yang dipilih secara acak sedemikian rupa sehingga efek ideal dari Gate tetap sama. Hasilnya adalah satu sirkuit digantikan dengan ansambel Circuit acak, semuanya dengan efek ideal yang sama. Saat mengambil sampel Circuit, sampel diambil dari beberapa instance acak, bukan hanya satu.

![Gambaran Pauli twirling](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)

Karena sebagian besar error pada perangkat keras kuantum saat ini berasal dari Gate dua qubit, teknik ini sering diterapkan secara eksklusif pada Gate dua qubit (asli). Diagram berikut menggambarkan beberapa Pauli twirl untuk Gate CNOT dan ECR. Setiap sirkuit dalam satu baris memiliki efek ideal yang sama.
...
![Gambaran measurement twirling](/docs/images/guides/error-mitigation-explanation/measurement_twirling.svg)

Dengan adanya readout error, measurement twirling berfungsi untuk mendiagonalisasi matriks transfer readout-error, sehingga mudah untuk diinversikan. Sebuah suku penskalaan ulang dari kanal noise diagonal dipelajari dengan melakukan benchmarking Circuit acak yang diinisialisasi dalam keadaan nol. Hal ini memungkinkan layanan untuk menghilangkan bias dari nilai ekspektasi yang dihasilkan oleh readout noise. Mengestimasi matriks transfer readout-error memerlukan eksekusi Circuit kalibrasi tambahan, yang menimbulkan sedikit overhead.

### Aktifkan TREX dengan menggunakan opsi `measure_mitigation` {#enable-trex-by-using-the-measure-mitigation-option}

TREX dapat diaktifkan dengan mengatur `measure_mitigation` ke `True` pada [opsi resiliensi IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) untuk Estimator. Opsi untuk pembelajaran noise pengukuran dijelaskan [di sini](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-measure-noise-learning-options). Seperti gate twirling, kamu dapat mengatur jumlah randomisasi Circuit dan jumlah shot per randomisasi.
...
### Aktifkan TREX dengan menggunakan opsi `resilience_level` {#enable-trex-by-using-the-resilience-level-option}

Kamu bisa mengaktifkan TREX dengan mengatur [resilience level](/guides/estimator-noise-management#resilience) ke `1`, seperti yang ditunjukkan dalam contoh berikut.
...
Overhead dari metode ini berskala dengan jumlah faktor noise. Pengaturan default mengambil sampel nilai ekspektasi pada tiga faktor noise, sehingga menghasilkan overhead sekitar 3x.

### Aktifkan ZNE dengan menggunakan opsi `zne_mitigation` {#enable-zne-by-using-the-zne-mitigation-option}

ZNE dapat diaktifkan dengan mengatur `zne_mitigation` ke `True` pada [opsi resiliensi IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) untuk Estimator.
Opsi untuk ZNE dijelaskan [di sini](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options). Opsi-opsi penting berikut ini:
...
### Aktifkan ZNE dengan menggunakan opsi `resilience_level` {#enable-zne-by-using-the-resilience-level-option}

Kamu bisa mengaktifkan ZNE (bersama dengan TREX dan gate twirling) dengan mengatur [resilience level](/guides/estimator-noise-management#resilience) ke `2`, seperti yang ditunjukkan dalam contoh berikut.
...
- [Konfigurasi manajemen noise dengan Estimator](/guides/estimator-noise-management).

- [Konfigurasi manajemen noise dengan Sampler](/guides/sampler-noise-management).

- Jelajahi [opsi](/guides/runtime-options-overview) lain untuk primitif IBM Quantum.

- Tentukan [mode eksekusi](/guides/execution-modes) mana yang akan digunakan untuk menjalankan job kamu.
