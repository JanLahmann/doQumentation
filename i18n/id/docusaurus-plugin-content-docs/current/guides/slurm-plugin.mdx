...
Secara default, file `CMakeLists.txt` mengharapkan file header Slurm (`slurm.h`) berada di `/usr/include/slurm`, tapi ini bisa dikustomisasi sebagai berikut.
...
```
Misalnya, saat berinteraksi dengan sumber daya kuantum melalui proxy HTTP, variabel lingkungan `http_proxy`, `https_proxy`, dan `no_proxy` diperlukan. Ini bisa ditambahkan sebagai berikut.
```bash
optional /usr/lib64/slurm/spank_qrmi.so /etc/slurm/qrmi_config.json \
