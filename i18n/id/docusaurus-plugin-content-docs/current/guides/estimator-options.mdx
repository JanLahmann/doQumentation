...
Kamu bisa mengatur opsi saat menginisialisasi Estimator, setelah menginisialisasi Estimator, atau kamu bisa memperbarui opsi setelah Estimator diinisialisasi. Untuk instruksi penggunaan teknik-teknik ini, lihat topik [Pengantar opsi](/guides/runtime-options-overview#options-precedence).

Selain itu, kamu bisa mengatur nilai `precision` di metode `run()`, seperti dijelaskan di bagian berikut.
...
3. Jika `twirling` diaktifkan (True secara default), maka hasil perkalian `num_randomizations` dan `shots_per_randomization`, yang ditentukan dalam [opsi `twirling`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-twirling-options), yang digunakan.
...
<Tabs>
  ...
  Mungkin tidak bekerja saat menggunakan gate kustom.
  </TabItem>
