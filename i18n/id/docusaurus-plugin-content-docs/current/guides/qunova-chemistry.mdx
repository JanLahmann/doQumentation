...
Kemudian, autentikasi menggunakan [API key IBM Quantum&reg;](http://quantum.cloud.ibm.com/) kamu dan, dengan asumsi kamu sudah [menyimpan akun](/guides/functions-get-started#install-qiskit-functions-catalog-client) ke lingkungan lokalmu, pilih Qiskit Function sebagai berikut:
...
Periksa [status](/guides/functions-get-started#check-job-status) beban kerja Qiskit Function kamu atau ambil [hasil](/guides/functions-get-started#retrieve-results) sebagai berikut:
...
Kirim email ke [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) untuk pertanyaan terkait lisensi.

### Contoh penggunaan fungsi berlisensi {#example-of-licensed-function-use}

Pengguna berlisensi akan diberikan license token, dan kemudian harus menggunakan wrapper library untuk mengirimkan license token mereka ke fungsi tersebut.
Wrapper library ini bisa diinstal [dari PyPI](https://pypi.org/project/hivqe-qiskit-function-utils/) dengan `pip install hivqe-qiskit-function-utils`.
Contoh di bawah ini menunjukkan bagaimana library ini seharusnya digunakan untuk mengirimkan license token kamu saat memanggil fungsi tersebut.
