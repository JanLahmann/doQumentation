...
Backend tiruan `FakeSherbrooke` dari `qiskit_ibm_runtime` digunakan dalam contoh-contoh ini, tetapi kamu bisa mencobanya pada backend nyata atau tiruan apa pun yang kompatibel dengan Qiskit. Hasilmu mungkin berbeda.
</Admonition>

Pertama, mari transpilasi menggunakan level optimasi 0.
...
Mengatur level optimasi juga bisa mengubah aspek lain dari sirkuit, tidak hanya jumlah gate ECR. Untuk contoh bagaimana pengaturan level optimasi mengubah layout, lihat [Merepresentasikan komputer kuantum](/guides/represent-quantum-computers).
