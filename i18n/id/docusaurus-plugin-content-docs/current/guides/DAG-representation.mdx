...
Pada DAG, ada tiga jenis node graf: node input qubit/clbit (hijau), node operasi (biru), dan node output (merah). Setiap edge menunjukkan aliran data (atau dependensi) antara dua node. Gunakan fungsi qiskit.tools.visualization.dag_drawer() untuk melihat DAG Circuit ini. (Install [Graphviz library](https://graphviz.org/download/) untuk menjalankan ini.)
...
Setelah semua transformasi selesai, DAG bisa dikonversi kembali menjadi objek `QuantumCircuit` biasa. Inilah cara pipeline Transpiler bekerja. Sebuah sirkuit diambil, diproses dalam bentuk DAG, dan Circuit yang sudah ditransformasi dihasilkan sebagai output.
...
<Admonition type="tip" title="Rekomendasi">
    - Tinjau panduan tentang membuat [transpiler pass kustom](/guides/custom-transpiler-pass)
    - Pelajari cara [Membuat dan mentranspilasi terhadap Backend kustom](/guides/custom-backend)
