...
Template ini merangkum sebuah alur kerja untuk mensimulasikan evolusi waktu dari suatu keadaan awal terhadap Hamiltonian berbasis spin yang ditentukan pengguna, dan mengembalikan sekumpulan nilai ekspektasi yang ditentukan menggunakan Qiskit addon [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/).
...
Pertama, tulis template function untuk simulasi Hamiltonian yang menggunakan [addon AQC-Tensor Qiskit](https://qiskit.github.io/qiskit-addon-aqc-tensor/) untuk memetakan deskripsi masalah ke Circuit berkedalaman lebih rendah untuk dieksekusi pada hardware.

Jika kamu mengunduh halaman ini dan melihatnya secara lokal di editor notebook, kamu akan melihat beberapa sel kode berisi [magic command](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Magic command ini menyimpan kode ke `./source_files/template_hamiltonian_simulation.py`, yang merupakan template function yang bisa kamu upload dan jalankan secara remote dengan Qiskit Serverless.
...
Untuk pemahaman yang lebih mendalam tentang addon Qiskit AQC-Tensor, cek tutorial [Improved Trotterized Time Evolution with Approximate Quantum Compilation](/tutorials/approximate-quantum-compilation-for-time-evolution) atau [repositori qiskit-addon-aqc-tensor](https://github.com/Qiskit/qiskit-addon-aqc-tensor).
...
<Accordion>
<AccordionItem title="**Kode sumber program lengkap**">
