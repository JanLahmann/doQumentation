...
`parameter values` opsional bisa berupa daftar atau satu parameter tunggal. Elemen dari observables dan parameter values digabungkan dengan mengikuti aturan broadcasting NumPy seperti yang dijelaskan di topik [Primitive inputs and outputs](primitive-input-output#broadcasting-rules), dan satu estimasi nilai ekspektasi dikembalikan untuk setiap elemen dari bentuk hasil broadcast.
...
Kode berikut mendemonstrasikan contoh set input yang divektorisasi untuk primitif `Estimator` dan mengeksekusinya pada backend IBM&reg; sebagai satu objek `RuntimeJobV2 ` tunggal.
