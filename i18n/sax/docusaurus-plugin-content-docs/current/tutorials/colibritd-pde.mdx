...
description: "Nutz die QUICK-PDE-Funktion for die Modelliering vun ere fließndn nisch-viskosn Flüssichkeet."
...
Beachtn, dass die Ausfiehrungszeet vun derscher Funktion im Allgemeenen mehr wie 20 Minudn betracht,
drum mechtn de des Tutorial villeicht in zwee Abschnitde ufdeeln: dr erschde, in däm
de's durchlesn un die Jobs staadn, un dr zweete e paar Stundn schbäter
(um de Jobs genuch Zeet zur Ferdischstellung zu gewwn), um mit de Ergebnisse vun de Jobs zu schaffe.

## Hindergrundd {#background}

Des Tutorial is druf aus, uf enem einfiehrenden Niveau zu zeechn, wie de die QUICK-PDE-Funktion
nutzn gonnst, um gomplexe Multi-Physik-Probleme uf 156Q Heron R2 QPUs mit'm
...
Beacht, dass derscher Solver ooch nisch-lineare Gleichungen leesn kann.
...
* Zugang zur QUICK-PDE-Funktion. Fülln des [Formular aus, um Zugang anzufordern.](https://forms.cloud.microsoft/e/3Wi9cbjQPK)

## Setup {#setup}

Authentifizir dich mit deim [API-Schlüssel](http://quantum.cloud.ibm.com/) un wähl die Funktion so aus:
...
Leesung beeinflussn. De gönnst $a$ un $b$ abassn un sehn, wie se
...
Abhängich vun dr Gonfiguration, die de zu leesn versuchst, missn die Parameter vum Algorithmus
...
in die falsche Richtung länkn; in däm Fall gönnst de des deaktivirn, indem de die
`initialization`-Strategie uf `"RANDOM"` setzst.
...
De gönnst dän Konvergenzprozess vun unsrer Leesung (HDES) vun job_2 mit dr Leistung vun enem Physics-Informed Neural Networks (PINN)-Algorithmus un -Solver verglichn (sehn dir des [Paper](https://arxiv.org/abs/1711.10561) un des zugeheerige [GitHub-Repository](https://github.com/314arhaam/burger-pinn)).
...
Beacht dän Unterschied vun dr Anfangsbedingung for dän zweetn Durchlauf un sei Auswirkung uf
des Ergebnis:
...
Nimm dir bitte e Minudd Zeet, um Feedback zu däm Tutorial zu gewwn. Dei Erkenntnisse helfn uns, unser Inhaltsbott un unsre Benutzererfahrung zu verbessern:
