---
title: "Binäre Obdimierung höherer Ordnung mit Q-CTRLs Optimization Solver"
sidebar_label: "Binäre Obdimierung höherer Ordnung mit Q-CTRLs Optimization Solver"
description: "Eene Utility-Scale Quanden-Obdimierungsbroblem lösen mit'm Optimization Solver, eener Qiskit Function von Q-CTRL Fire Opal"
...
Qiskit Functions is ne experimentelle Funggzjon, die bloß fer IBM Quantum&reg; Premium Plan, Flex Plan un On-Prem (ieber IBM Quantum Platform API) Plan Nutzer verfichbar is. Se sin im Preview-Status un könn sich ännern.
</Admonition>
...
## Hinnergrunnd {#background}

Das Tutorial zeicht, wie mer e higher-order binary optimization (HOBO) Problem mid dem [Optimization Solver, ner Qiskit Function von Q-CTRL Fire Opal](/guides/q-ctrl-optimization-solver) lösn kann. Das Beischbiel ausm Tutorial is e Obdimierungsproblem, was dafier gedachd is, die Grundzustandsenergie vonn'n 156-Qubit Ising-Modell mid gubischen Termen ze findn. Der Optimization Solver kann fer allgemeine Obdimierungsprobleme gebrucht wern, die als Zielfunggzjon definiert sin.

Der Optimization Solver aatomadisiert vollschdännich die Hardware-bewussten Implementierungsschritte fer Obdimierungsprobleme uff Quantnhardware, un durch's Ausnutzn von [Performance Management](/guides/q-ctrl-performance-management) fer die Quantnausfiehrung, erreichd er genaue Lösungn uff utility scale. Fer ne dedaillierde Zusammmfassung vomm Optimization Solver Workflow un Benchmarking-Ergebnisse, guggde mol uff [das veröffendlichte Manusgript](https://arxiv.org/abs/2406.01743).

Das Tutorial jeht durch die Schritte:

1. Das Problem als Zielfunggzjon definieren
2. Dän Hybridalgorithmus mid dem Fire Opal Optimization Solver laafn lassen
3. Ergebnisse evaluiern
## Voraussetznge {#requirements}
...
De brauchsd ooch Zugriff uff die Optimization Solver Funggzjon. [Füll das Formular aus](https://quantum.cloud.ibm.com/functions?id=q-ctrl-optimization-solver) fer Zugriff aanzufordern.
## Einrichtung {#setup}
...
Definier deine [IBM Quantum Platform](https://quantum.cloud.ibm.com/) Zugangsdan, die während dem Tutorial fer Authentifizierung bei Qiskit Runtime un Qiskit Functions gebrucht wern.
...
## Schridd 1: Das Problem als Zielfunggzjon definieren {#step-1-define-the-problem-as-an-objective-function}
Der Optimization Solver aggzebtiert entweder ne Zielfunggzjon oder'n Graph als Eingabe. In dem Tutorial is das Ising spin glass Minimierungsproblem als Zielfunggzjon definiert, un es is uff die heavy-hex Dobologie von IBM&reg; Gerätn aanjebassdt.
...
Fer ne dedaillierte Disgussion ieber die Gonstrugzjon von der Problemdefinizjon un frühere Ergebnisse vomm Optimization Solver guggde mol uff [das dechnische Manusgript](https://arxiv.org/abs/2406.01743). Das Problem wurde urspringlich definiert un evaluiert als Deil vonn'n [Paper vomm Los Alamos National Laboratory](https://arxiv.org/abs/2312.00997), un es wurde aanjebassdt fer die volle Gerätebreide von de 156-Qubit IBM Quantum Heron Brozessorn ze nutzn.
...
## Schridd 2: Dän Hybridalgorithmus mid dem Fire Opal Optimization Solver laafn lassen {#step-2-run-the-hybrid-algorithm-using-the-fire-opal-optimization-solver}
...
De gannsde die beganndn [Qiskit Serverless APIs](/guides/serverless) bruchen fer dein Qiskit Function Workload Status ze priefn:
...
## Schridd 3: Ergebnisse evaluiern {#step-3-evaluate-results}
