...
### Krok 3: Wykonanie z użyciem prymitywów IBM Quantum {#step-3-execute-using-ibm-quantum-primitives}
...
### Krok 2: Zoptymalizuj problem dla wykonania kwantowego {#step-2-optimize-problem-for-quantum-execution-1}
...
### Krok 3: Wykonanie z użyciem prymitywów IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-1}
...
### Krok 4: Przetwarzanie końcowe, zwrócenie wyniku w formacie klasycznym {#step-4-post-process-return-result-in-classical-format-1}
...
Warto zauważyć, że użyliśmy `z_feature_map`, co zaowocowało niską dwukubitową głębokością po transpilacji (w rzeczywistości głębokością 1). Jeśli Twoje obwody staną się zbyt głębokie, z pewnością spowoduje to duży szum, co sprawi, że prawdopodobieństwo pomiaru stanu zerowego będzie bardzo niskie, nawet jeśli Twoje mapowanie cech jest dobrze dopasowane do Twoich danych. Na przykład powtórzenie powyższego procesu z użyciem ```zz_feature_map``` oraz ```, entanglement='linear', reps=1``` dało ```dist.get(0,0.0) = 0.0015``` dla tych samych punktów danych. Wynika to ze znacznie większych głębokości obwodu i głębokości dwukubitowych w ```zz_feature_map```. Poniższy rysunek pokazuje rozkład prawdopodobieństwa dla tego obliczenia.
...
Warto poeksperymentować z kilkoma punktami danych z tej samej kategorii, aby zobaczyć, jak niska musi być Twoja głębokość, aby uzyskać dobre wyniki. Poniższe porady są ogólne i z pewnością mają wyjątki. Generalnie dwukubitowa głębokość po transpilacji wynosząca 10 lub mniej nie powinna stanowić problemu. Dwukubitowa głębokość po transpilacji wynosząca 50-60 jest na poziomie najnowszych osiągnięć i będzie wymagać zaawansowanego ograniczania błędów i innych narzędzi. Pomiędzy tymi wartościami Twoje wyniki mogą się różnić w zależności od podobieństwa danych, ekspresyjności mapowania cech, szerokości obwodu i innych czynników.
...
Załóżmy, że ktoś nowy w obliczeniach kwantowych próbuje użyć obwodu kwantowego o bardzo dużej głębokości dwuqubitowej i nie stosuje mitygacji błędów. Załóżmy dalej, że skutkuje to współczynnikiem błędu 10% na każdym Qubicie. Jeśli prawdziwy (bezbłędny) element macierzy jądra odpowiadający temu obwodowi jest bardzo duży, powiedzmy 1.0, jakie byłoby prawdopodobieństwo zmierzenia wszystkich 10 Qubitów w stanie, w którym każdy Qubit jest $|0\rangle$?
...
### Krok 1: Mapowanie klasycznych danych wejściowych na problem kwantowy {#step-1-map-classical-inputs-to-a-quantum-problem-2}
...
### Krok 4: Przetwarzanie końcowe, zwrócenie wyniku w formacie klasycznym {#step-4-post-process-return-result-in-classical-format-2}
...
### Krok 1: Odwzorowanie klasycznych danych wejściowych na problem kwantowy {#step-1-map-classical-inputs-to-a-quantum-problem-3}
...
### Krok 2: Zoptymalizuj problem pod kątem wykonania kwantowego {#step-2-optimize-problem-for-quantum-execution-2}
...
### Krok 3: Wykonanie z użyciem prymitywów IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-2}
...
### Krok 4: Przetwarzanie końcowe, zwrócenie wyniku w formacie klasycznym {#step-4-post-process-return-result-in-classical-format-3}
