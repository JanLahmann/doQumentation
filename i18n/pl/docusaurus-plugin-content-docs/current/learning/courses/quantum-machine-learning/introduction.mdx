...
Wiele zbiorów danych używanych w aplikacjach QML opisanych w literaturze jest „inżynierowanych pod kątem cech" (ang. feature engineered), co oznacza, że zbiór danych jest wybierany lub generowany specjalnie w celu pokazania wąskiego przypadku użycia, w którym obliczenia kwantowe są przydatne. Jeśli wydaje się to oszustwem, to źle rozumiemy zadanie, które mamy do wykonania. __Nie__ jest tak, że niektóre kwantowe feature mapy pozwalają nam rozwiązywać wszystkie lub wiele zadań klasyfikacyjnych efektywniej lub skalowalniej niż klasyczne algorytmy uczenia maszynowego. Raczej: niektóre kwantowe feature mapy (nie wszystkie) zachowują się inaczej niż klasyczne feature mapy. Zadaniem jest zatem eksploracja obwodów kwantowych w kontekście złożonych struktur danych. Kilka konkretnych pytań, na które trzeba odpowiedzieć:
1.	Które obwody kwantowe mają największe szanse zachowywać się w nowatorski sposób w porównaniu z klasycznymi alternatywami?
2.	Czy istnieją rzeczywiste problemy obejmujące dane o właściwościach najlepiej eksplorowanych za pomocą takich nowatorskich obwodów kwantowych?
3.	Czy te obwody kwantowe skalują się na komputerach kwantowych bliskiej przyszłości?
...
W kolejnych lekcjach przedstawiamy przepływy pracy umożliwiające włączanie obwodów kwantowych do zadań uczenia maszynowego – i robimy to z wyraźnym celem ułatwienia eksploracji możliwości obliczeń kwantowych. Żaden feature map ani algorytm w tym kursie nie jest przedstawiany jako szybka droga do lepszych wyników uczenia maszynowego dla ogólnych problemów, bo taki feature map ani algorytm nie istnieje. Zamiast tego prezentujemy szeroki wachlarz narzędzi kwantowych do wykorzystania w poszukiwaniu użytecznych zastosowań obliczeń kwantowych.
...
Nawet jeśli algorytm nie może zostać zdekwantyzowany, nie oznacza to, że jest bardziej wydajny lub skalowalny niż wszystkie algorytmy klasyczne. Jako skrajny, sztuczny przykład: wyobraź sobie algorytm wybierający j największych elementów ze zbioru o rozmiarze k. Można by napisać algorytm kwantowy, który wykorzystuje algorytm Shora do rozłożenia każdego z k elementów na czynniki pierwsze, a następnie określa największe elementy na podstawie tych czynników. Taki algorytm prawdopodobnie nie może zostać zdekwantyzowany, ale jest drastycznie mniej wydajny niż klasyczne algorytmy do wykonania tego samego wyboru największych elementów (choć nie zbędnej części z rozkładem na czynniki).
...
Fałsz. Większość problemów uczenia maszynowego jest rozwiązywana bardzo skutecznie przez algorytmy klasyczne, a algorytmy kwantowe prawdopodobnie nie zaoferują istotnego przyspieszenia. Celem w QML jest znalezienie zbiorów danych z cechami dobrze opisanymi przez stany kwantowe i/lub znalezienie odwzorowań cech danych, które optymalizują dokładność modeli.
...
5. Implementować kwantowe metody kernel i wariacyjne klasyfikatory kwantowe z wykorzystaniem IBM Quantum primitives oraz zgodnie z wzorcami Qiskit.
...
- Lekcja 5: Wariacyjne klasyfikatory kwantowe / sieci neuronowe
...
* Krok 3: Wykonanie z użyciem prymitywów IBM Quantum
...
W powyższym procesie obliczyliśmy element macierzy jądra dla 14. i 19. punktu danych. Jaką wartość powinniśmy otrzymać, jeśli użyjemy tego samego punktu danych dwukrotnie (na przykład 14. i ponownie 14.)? Innymi słowy, jakie powinny być elementy diagonalne macierzy jądra? Odpowiedz na to pytanie przy braku szumu, ale zauważ, że w obecności szumu możliwe są odchylenia od Twojej odpowiedzi.

<Accordion>
<AccordionItem title="Odpowiedź">

Elementy diagonalne powinny wynosić 1.0. Ten proces powinien obliczać znormalizowany iloczyn skalarny wektora z samym sobą, który zawsze musi wynosić jeden.
