...
3. Wierność między stanami nie maleje pod działaniem żadnego kanału. Oznacza to, że jeśli $\rho$ i $\sigma$ są macierzami gęstości, a $\Phi$ jest kanałem, który może przyjąć te dwa stany jako wejście, to z konieczności zachodzi
   $$
   \operatorname{F}(\rho,\sigma) \leq \operatorname{F}(\Phi(\rho),\Phi(\sigma)).
   $$
4. *Nierówności Fuchsa-van de Graafa* ustanawiają bliski (choć niedokładny) związek między wiernością a odległością śladu: dla dowolnych dwóch stanów $\rho$ i $\sigma$ mamy
   $$
...
Zatem $\langle \psi \vert P_0 \vert \psi \rangle$ jest zawsze liczbą rzeczywistą między $0$ a $1$, co oznacza, że każda wartość własna $P_0$ jest liczbą rzeczywistą między $0$ a $1$, ponieważ możemy wybrać $\vert\psi\rangle$ specjalnie jako jednostkowy wektor własny odpowiadający dowolnej interesującej nas wartości własnej.
...
Gratulacje ukończenia tego kursu! Prosimy o chwilę, aby pomóc nam ulepszyć nasz kurs, wypełniając następującą [krótką ankietę](https://your.feedback.ibm.com/jfe/form/SV_bNoS9Adav7L8z9s). Twoje opinie zostaną wykorzystane do poprawy naszej oferty treści i doświadczenia użytkownika. Dziękujemy!
