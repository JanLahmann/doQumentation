...
### Przykład 2: nieprojekcyjny pomiar na kubicie {#example-2-a-non-projective-qubit-measurement}

Załóżmy, że $\mathsf{X}$ jest kubitem, i zdefiniujmy dwie macierze w następujący sposób.
...
Mamy również, że $P_0 + P_1 = \mathbb{I},$ a zatem $\{P_0,P_1\}$ opisuje pomiar.

Jeśli stan $\mathsf{X}$ jest opisany przez macierz gęstości $\rho$ i wykonamy ten pomiar, to prawdopodobieństwo uzyskania wyniku $0$ wynosi $\operatorname{Tr}(P_0 \rho),$ a prawdopodobieństwo uzyskania wyniku $1$ wynosi
$\operatorname{Tr}(P_1 \rho).$
Na przykład, jeśli $\rho = \vert + \rangle \langle + \vert,$ to prawdopodobieństwa dwóch wyników $0$ i $1$ są następujące.
...
### Przykład 3: pomiar tetraedryczny {#example-3-tetrahedral-measurement}

Zdefiniujmy cztery wektory stanu kwantowego jednego kubitu w następujący sposób.
...
co można zweryfikować, wyrażając reprezentacje tych stanów jako macierze gęstości w postaci kombinacji liniowych macierzy Pauliego.
...
Zdefiniujmy teraz pomiar $\{P_0,P_1,P_2,P_3\}$ kubitu, przyjmując $P_a$ w następujący sposób dla każdego $a=0,\ldots,3.$
...
Kanał ten jest analogiczny do całkowicie defazującego kanału kwantowego kubitu $\Delta$ z poprzedniej lekcji.
...
Załóżmy, że mamy kanał z $\mathsf{X}$ do $\mathsf{Y}$ o własności
...
Tak jak każdy kanał, możemy wyrazić $\Phi$ w postaci Krausa dla pewnego wyboru macierzy Krausa
$A_0,\ldots,A_{N-1}.$
...
Zatem dla tych samych macierzy $P_0,\ldots,P_{m-1}$ możemy wyrazić kanał $\Phi$ następująco.

$$
\Phi(\rho) = \sum_{a = 0}^{m-1} \operatorname{Tr}(P_a \rho) \vert a\rangle\langle a\vert
$$

Wyrażenie to jest zgodne z naszym opisem ogólnych pomiarów w kategoriach macierzy, ponieważ widzimy, że każdy wynik pomiaru pojawia się z prawdopodobieństwem $\operatorname{Tr}(P_a \rho).$

Zauważmy teraz, że dwie własności wymagane od zbioru macierzy $\{P_0,\ldots,P_{m-1}\}$ do opisu ogólnego pomiaru są istotnie spełnione.
...
Ostatnia równość wynika z faktu, że $\Phi$ jest kanałem, więc jego macierze Krausa muszą spełniać ten warunek.
...
jest istotnie prawidłowym kanałem z $\mathsf{X}$ do $\mathsf{Y}.$
...
więc wnioskujemy, że $\Phi$ jest kanałem.
...
(Układ po prawej nazywamy $\mathsf{Z},$ ponieważ $\mathsf{Y}$ weźmiemy jako układ reprezentujący klasyczne wyjście pomiaru, gdy będziemy go traktować jako kanał.)
...
$\{P_0,\ldots,P_{m-1}\}.$
Ten pomiar może być alternatywnie opisany jako kanał $\Phi$ z $\mathsf{X}$ do $\mathsf{Y},$ gdzie $\mathsf{Y}$ jest nowym układem o zbiorze stanów klasycznych $\{0,\ldots,m-1\}.$
...
Rozważamy pomiar układu $\mathsf{X},$ więc prawdopodobieństwa, z jakimi uzyskiwane są różne wyniki pomiaru, mogą zależeć tylko od $\rho_{\mathsf{X}},$ zredukowanego stanu $\mathsf{X}.$
...
Wychodząc od pomiaru na $\mathsf{X},$ efektywnie definiujemy pomiar układu $(\mathsf{X},\mathsf{Z}),$ w którym po prostu odrzucamy $\mathsf{Z}$ i mierzymy $\mathsf{X}.$
...
Jeśli chcemy wyznaczyć nie tylko prawdopodobieństwa różnych wyników, ale także powstały stan $\mathsf{Z}$ warunkowany każdym wynikiem pomiaru, możemy odwołać się do opisu pomiaru za pomocą kanału.
...
Zauważmy, że jest to macierz gęstości, ponieważ $\Phi$ jest kanałem, więc każda macierz
...
Podsumowując, oto co się dzieje, gdy pomiar $\{P_0,\ldots,P_{m-1}\}$ jest wykonywany na $\mathsf{X},$ gdy $(\mathsf{X},\mathsf{Z})$ jest w stanie $\rho.$
...
Ogólnie, jeśli mamy $r$ układów $\mathsf{X}_1,\ldots,\mathsf{X}_r,$ stan układu złożonego $(\mathsf{X}_1,\ldots,\mathsf{X}_r)$ to $\rho,$ a pomiar $\{P_0,\ldots,P_{m-1}\}$ jest wykonywany na $\mathsf{X}_k$, dzieje się co następuje.
