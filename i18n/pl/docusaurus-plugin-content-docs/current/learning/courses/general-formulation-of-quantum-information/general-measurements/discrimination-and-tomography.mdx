...
2. *Tomografia stanów kwantowych*
   ...
   Celem jest znalezienie przybliżenia nieznanego stanu, jako macierz gęstości,
   poprzez pomiar tych układów.

## Rozróżnianie dwóch stanów {#discriminating-between-two-states}
...
### Optymalny pomiar {#an-optimal-measurement}
...
Możemy zmaksymalizować prawdopodobieństwo poprawnego odgadnięcia, wybierając pomiar rzutowy $\{\Pi_0,\Pi_1\}$ w następujący sposób.
...
Możemy wówczas wybrać *pomiar rzutowy* w następujący sposób.
...
### Prawdopodobieństwo poprawności {#correctness-probability}
...
Dla *dowolnego* pomiaru $\{P_0,P_1\}$ (niekoniecznie rzutowego) możemy zapisać prawdopodobieństwo poprawności następująco.
...
Mówiąc wprost, chodzi o znalezienie klasycznego opisu macierz gęstości $\tilde{\rho}$ możliwie jak najbliższej $\rho.$

Możemy też opisać tę sytuację w następujący sposób.
Wybierana jest nieznana macierz gęstości $\rho,$ a my mamy dostęp do $N$ układów kwantowych $\mathsf{X}_1,\ldots,\mathsf{X}_N,$ z których każdy został *niezależnie* przygotowany w stanie $\rho.$
...
Celem jest wykonanie pomiarów na układach $\mathsf{X}_1,\ldots,\mathsf{X}_N$ i — na podstawie wyników tych pomiarów — obliczenie macierz gęstości $\tilde{\rho}$ blisko aproksymującej $\rho.$
...
Rozważymy teraz kwantową tomografię stanów w prostym przypadku, gdy $\rho$ jest qubitową macierz gęstości.
...
W granicy, gdy $N$ dąży do nieskończoności, ta aproksymacja zbiega według prawdopodobieństwa do prawdziwej macierz gęstości $\rho$ na mocy *prawa wielkich liczb,* a dobrze znane ograniczenia statystyczne (takie jak *nierówność Hoeffdinga*) mogą być używane do ograniczania prawdopodobieństwa, że aproksymacja $\tilde{\rho}$ odchyla się od $\rho$ o zadaną wielkość.

Ważną rzeczą do dostrzeżenia jest jednak to, że macierz $\tilde{\rho}$ uzyskana w ten sposób może nie być macierz gęstości.
W szczególności, choć zawsze będzie miała ślad równy $1,$ może nie być dodatnio półokreślona.
Istnieją różne znane strategie „zaokrąglania" takiej aproksymacji $\tilde{\rho}$ do macierz gęstości — jedna z nich polega na obliczeniu rozkładu spektralnego, zastąpieniu ujemnych wartości własnych zerami, a następnie renormalizacji (przez podzielenie otrzymanej macierzy przez jej ślad).
### Tomografia qubitów przy użyciu pomiaru tetraedrycznego {#qubit-tomography-using-the-tetrahedral-measurement}
...
To przybliżenie zawsze będzie macierzą hermitowską o śladzie równym jeden, jednak może nie być dodatnio półokreślone.
W takim przypadku przybliżenie to musi zostać „zaokrąglone" do macierzy gęstości, podobnie jak w strategii stosowanej przy pomiarach Pauliego.
