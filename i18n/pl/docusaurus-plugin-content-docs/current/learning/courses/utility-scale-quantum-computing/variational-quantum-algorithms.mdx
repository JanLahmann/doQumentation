...
Jeśli chcemy oszacować wartość oczekiwaną operatora (jak $Z \otimes Z$), użylibyśmy Estimator. Jeśli chcemy przyjrzeć się stanom systemu, używamy Sampler.
...
Możesz następnie przeformułować problem QUBO jako **Hamiltonian** (tutaj, macierz reprezentującą energię systemu):

$$
H_C=\sum_{ij}Q_{ij}Z_iZ_j + \sum_i b_iZ_i.
$$

**Kroki przeformułowania problemu QAOA na Hamiltonian**

Aby pokazać, jak problem QAOA można przepisać w ten sposób, najpierw zamień zmienne binarne $x_i$ na nowy zbiór zmiennych $z_i\in\{-1, 1\}$ za pomocą

$$
x_i = \frac{1-z_i}{2}.
$$

Tutaj widzisz, że jeśli $x_i$ wynosi $0$, to $z_i$ musi wynosić $1$. Gdy $x_i$'y zostaną podstawione za $z_i$'y w problemie optymalizacyjnym ($x^TQx$), można uzyskać równoważne sformułowanie.

$$
x^TQx=\sum_{ij}Q_{ij}x_ix_j \\ =\frac{1}{4}\sum_{ij}Q_{ij}(1-z_i)(1-z_j) \\=\frac{1}{4}\sum_{ij}Q_{ij}z_iz_j-\frac{1}{4}\sum_{ij}(Q_{ij}+Q_{ji})z_i + \frac{n^2}{4}.
$$

Teraz, jeśli zdefiniujemy $b_i=-\sum_{j}(Q_{ij}+Q_{ji})$, usuniemy współczynnik i wyraz stały $n^2$, otrzymamy dwa równoważne sformułowania tego samego problemu optymalizacyjnego.

$$
min_{x\in\{0,1\}^n} x^TQx\Longleftrightarrow \min_{z\in\{-1,1\}^n}z^TQz + b^Tz
$$

Tutaj $b$ zależy od $Q$. Zauważ, że aby uzyskać $z^TQz + b^Tz$, pominęliśmy czynnik 1/4 i stały przesunięcie $n^2$, które nie odgrywają roli w optymalizacji.
...
### 3.4 Krok 3. Wykonanie przy użyciu prymitywów IBM Quantum {#34-step-3-execute-using-ibm-quantum-primitives}
