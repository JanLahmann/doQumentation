...
## 2. Podstawy Kwantowej Transformacji Fouriera (QFT) {#2-basic-of-quantum-fourier-transformation-qft}
...
### 2.1. Intuicja <a id="intuition"></a> {#21-intuition}
...
### 2.2 Przykład: 1-kubitowa QFT {#22-example-1-qubit-qft}
...
### 2.3 Reprezentacja produktowa QFT {#23-product-representation-of-qft}
...
### 2.4 Przykład: konstrukcja obwodu QFT dla 3 kubitów {#24-example-circuit-construction-of-3-qubits-qft}
...
Mając operację unitarną $U$, QPE estymuje $\theta$ w $U\vert\psi \rangle =e^{\boldsymbol{2\pi i} \theta }|\psi \rangle$, ponieważ $U$ jest unitarna, wszystkie jej wartości własne mają normę 1.
...
Musimy wprowadzić kontrolowaną operację unitarną $CU$, która stosuje operator unitarny $U$ na rejestrze docelowym tylko wtedy, gdy odpowiadający mu bit sterujący ma wartość $|1\rangle$. Ponieważ $U$ jest operatorem unitarnym z wektorem własnym $|\psi\rangle$ takim, że $U|\psi \rangle =e^{\boldsymbol{2\pi i} \theta }|\psi \rangle$, oznacza to:
...
$$
Inicjalizujemy $\vert\psi\rangle = \vert1\rangle$ jako wektor własny bramki $T$, stosując bramkę $X$:
...
## 4. Wykonanie z użyciem prymitywu Sampler IBM Quantum {#4-execution-using-the-ibm-quantum-sampler-primitive}
...
### 4.3 Krok 3: Wykonanie na docelowym sprzęcie {#43-step-3-execute-on-target-hardware}
...
### 4.4 Krok 4: Przetwarzanie końcowe wyników {#44-step-4-post-process-the-results}
