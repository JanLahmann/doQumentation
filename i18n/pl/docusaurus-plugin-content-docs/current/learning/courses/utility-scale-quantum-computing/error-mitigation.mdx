...
Ustawienie poziomu resilience (`estimator.options.resilience_level`) jest najłatwiejszym sposobem zastosowania mitygacji błędów przy użyciu Qiskit Estimator. Estimator obsługuje następujące poziomy resilience (stan na 2024/06/28). Więcej szczegółów znajdziesz w przewodniku [Configure error mitigation](/guides/error-mitigation-and-suppression-techniques).
