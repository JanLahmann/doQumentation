---
description: 'Opisana jest kwantowa diagonalizacja Kryłowa (KQD), zaczynając od klasycznych metod Krylov. Omówiono zbieżność i zużycie zasobów.'
...
<IBMVideo id="134325510" title="W tym nagraniu Chris Porter przedstawia przegląd klasycznych metod Krylov i wyjaśnia, dlaczego są one użyteczne. Tłumaczy, jak obliczanie kwantowe może odgrywać rolę w tym podejściu do diagonalizacji. Poniższy tekst zawiera więcej szczegółów."/>

## 1. Wprowadzenie do metod Krylov {#1-introduction-to-krylov-methods}

__Metoda podprzestrzeni Krylov__ może odnosić się do dowolnej z kilku metod zbudowanych wokół tak zwanej __podprzestrzeni Krylov__. Pełny przegląd tych metod wykracza poza zakres tej lekcji, lecz Ref [\[2-4\]](#references) mogą dostarczyć znacznie szerszego kontekstu. Skupimy się tutaj na tym, czym jest podprzestrzeń Krylov, jak i dlaczego jest użyteczna w rozwiązywaniu problemów wartości własnych, a w końcu jak może być zaimplementowana na komputerze kwantowym.
__Definicja:__ Dla symetrycznej, dodatnio półokreślonej macierzy $A$ o wymiarze $N\times N$, przestrzeń Krylov $\mathcal{K}^r$ rzędu $r$ jest przestrzenią rozpinaną przez wektory otrzymane przez mnożenie wyższych potęg macierzy $A$, aż do $r-1\leq N$, przez wektor odniesienia $\vert v \rangle$.
...
$$
Jeśli powtórzymy ten proces dla wszystkich $r$ wektorów, otrzymamy kompletną ortonormalną bazę dla przestrzeni Krylov. Zauważ, że proces ortogonalizacji w tym miejscu daje zero, gdy $r>m$, ponieważ $m$ ortogonalnych wektorów z konieczności rozpina pełną przestrzeń. Proces również da zero, jeśli dowolny wektor jest wektorem własnym $A$, ponieważ wszystkie kolejne wektory będą wielokrotnościami tego wektora.

### 1.1 Prosty przykład: Krylov ręcznie {#11-a-simple-example-krylov-by-hand}

Przejdźmy krok po kroku przez generowanie podprzestrzeni Krylov na trywialnie małej macierzy, tak aby zobaczyć ten proces. Zaczynamy od interesującej nas początkowej macierzy $A$:
$$
...
\end{aligned}
$$
Chcielibyśmy zbadać, jak ten proces działa (lub zawodzi), w miarę jak zwiększamy wymiar naszej podprzestrzeni Krylov, $r$. W tym celu zastosujemy następujący proces:
...
Jeśli nie jest jeszcze znormalizowany, znormalizuj go. W tym przypadku wektor był już znormalizowany, więc
$$
...
$$
Teraz wyznaczamy wartości własne:
$$
...
$$
Te wartości własne są dokładnie wartościami własnymi oryginalnej macierzy $A$. Tak musi być, ponieważ rozszerzyliśmy naszą podprzestrzeń Kryłowa tak, by obejmowała całą przestrzeń wektorową oryginalnej macierzy $A$.

W tym przykładzie metoda Kryłowa może nie wydawać się szczególnie prostsza niż bezpośrednia diagonalizacja. Rzeczywiście, jak zobaczymy w dalszych sekcjach, metoda Kryłowa jest korzystna dopiero powyżej pewnego wymiaru macierzy; ma to pomóc nam rozwiązywać problemy wartości/wektorów własnych dla bardzo dużych macierzy.
...
#### Wyjaśnienie terminów {#clarification-of-terms}
...
Wyjaśnij, dlaczego (a) nieużyteczne, oraz (b) niemożliwe jest rozszerzenie wymiaru podprzestrzeni Kryłowa $r$ poza wymiar $N$ interesującej nas macierzy.

<Accordion>
<AccordionItem title="Odpowiedź">

(a) Ponieważ ortonormalizujemy wektory w miarę ich tworzenia, zbiór $N$ takich wektorów utworzy pełną bazę, co oznacza, że kombinacja liniowa z nich może być użyta do utworzenia dowolnego wektora w przestrzeni.

(b) Proces ortogonalizacji polega na odejmowaniu rzutu nowego wektora na wszystkie poprzednie wektory. Jeśli wszystkie poprzednie wektory rozpinają całą przestrzeń wektorową, to odjęcie rzutów na pełną podprzestrzeń zawsze pozostawi nam wektor zerowy.
...
Twój współpracownik przypadkowo wybrał wektor własny jako swój wektor początkowy. Działanie macierzą na wektor początkowy po prostu zwróci ten sam wektor, przeskalowany przez wartość własną. To nie wygeneruje podprzestrzeni o rosnącym wymiarze. Poradź współpracownikowi wybór innego wektora początkowego, upewniając się, że nie jest to wektor własny.
...
### 1.2 Rodzaje metod Krylov {#12-types-of-krylov-methods}
...
### 1.3 Dlaczego metoda podprzestrzeni Krylov działa {#13-why-the-krylov-subspace-method-works}
...
![Wynik poprzedniej komórki kodu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/0912e12b-63bd-4026-ac8e-56bc5db8736e-0.avif)
...
![Wynik działania poprzedniej komórki kodu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/d9cb6e36-9ad3-403f-823f-0072e9cf53fe-0.avif)
...
Zauważ, że czas potrzebny na zbudowanie podprzestrzeni Kryłowa początkowo przewyższa czas potrzebny na pełną diagonalizację przy użyciu numpy. Jednak w miarę wzrostu rozmiaru macierzy metoda Kryłowa staje się korzystniejsza. Jest to prawdą nawet jeśli obniżymy nasz akceptowalny błąd, ale przewaga pojawia się przy większym rozmiarze macierzy. Warto to przeanalizować.

Złożoność czasowa diagonalizacji numerycznej wynosi $O(n^3)$ (z pewnymi różnicami między algorytmami). Złożoność czasowa generowania ortonormalnej bazy $n$ wektorów również wynosi $O(n^3)$. Zatem przewaga metody Kryłowa __nie__ jest związana z użyciem $\it{jakiejkolwiek}$ ortonormalnej bazy, lecz z użyciem konkretnej ortonormalnej bazy, która skutecznie wyodrębnia interesujące nas wartości własne. Widzieliśmy to już w szkicu dowodu w pierwszej sekcji tej lekcji, i jest to kluczowe dla gwarancji zbieżności w metodach Kryłowa.
Podsumujmy nasze postępy do tej pory:
* Dla bardzo dużych macierzy metoda podprzestrzeni Kryłowa może dawać przybliżone wartości własne w wymaganych tolerancjach szybciej niż tradycyjne algorytmy diagonalizacji.
* Dla tak bardzo dużych macierzy generowanie podprzestrzeni Kryłowa jest najbardziej czasochłonną częścią metody podprzestrzeni Kryłowa.
* Zatem wydajny sposób generowania podprzestrzeni Kryłowa byłby bardzo cenny.
Tutaj właśnie wkracza komputer kwantowy.

### Sprawdź swoje zrozumienie {#check-your-understanding-2}

Odnieś się do wygładzonego wykresu czasów diagonalizacji w funkcji wymiaru macierzy powyżej.
...
(b) Jakie aspekty obliczeń mogłyby zmienić wymiar, przy którym metoda Kryłowa staje się szybsza?
...
## 3. Krylov poprzez ewolucję w czasie {#3-krylov-via-time-evolution}
...
### 3.1 Ewolucja w czasie {#31-time-evolution}
...
#### Uogólnione zagadnienie własne {#generalized-eigenvalue-problem}
...
![Wynik poprzedniej komórki kodu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/ac536079-96f2-435b-a15f-72c207fa24d1-1.avif)
...
Zamiast implementować operator ewolucji czasowej dokładnie, możemy użyć dekompozycji Trottera, aby zaimplementować jego przybliżenie. Kilkukrotne powtórzenie dekompozycji Trottera pewnego rzędu daje nam dalsze zmniejszenie błędu wprowadzonego przez to przybliżenie. W dalszej części bezpośrednio budujemy implementację Trottera w najbardziej efektywny sposób dla grafu oddziaływań rozważanego przez nas Hamiltonianu (oddziaływania jedynie najbliższych sąsiadów). W praktyce wstawiamy obroty Pauli $R_{xx}$, $R_{yy}$, $R_{zz}$ z siłami sprzężenia $J_x$, $J_y$, $J_z$ oraz parametryzowanym kątem $t$, które odpowiadają przybliżonej implementacji $e^{-i (J_x XX + J_y YY + J_z ZZ) t}$. Biorąc pod uwagę różnicę w definicji obrotów Pauli i ewolucji czasowej, którą próbujemy zaimplementować, będziemy musieli użyć parametru $2*dt$, aby osiągnąć ewolucję czasową o $dt$. Ponadto odwracamy kolejność operacji dla nieparzystej liczby powtórzeń kroków Trottera, co jest funkcjonalnie równoważne, ale pozwala na syntezę sąsiednich operacji w pojedynczym unitarnym $SU(2)$. Daje to znacznie płytszy obwód niż ten uzyskany za pomocą ogólnej funkcjonalności `PauliEvolutionGate()`.
...
![Wynik poprzedniej komórki kodu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/8d35e6cd-c0f6-4871-83da-86472d66be2c-0.avif)
...
![Wynik poprzedniej komórki kodu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/d1d0b9de-65d4-4a46-975d-6cfaaac05f9a-0.avif)
...
![Wynik poprzedniej komórki kodu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/37382668-1999-4475-b50f-2887449d5c93-0.avif)
...
![Wynik poprzedniej komórki kodu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/054bcfba-fde3-4213-bbd7-d7a6dd6c03c8-1.avif)

Po optymalizacji nasza stranspilowana głębokość dwukubitowa jest dodatkowo zredukowana.

### 4.3 Krok 3. Wykonanie przy użyciu prymitywu IBM Quantum {#43-step-3-execute-using-an-ibm-quantum-primitive}
...
![Wynik poprzedniej komórki kodu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/a5d4a983-1a30-4cea-b695-3e6a67338633-0.avif)

## 5. Dyskusja i rozszerzenie {#5-discussion-and-extension}
