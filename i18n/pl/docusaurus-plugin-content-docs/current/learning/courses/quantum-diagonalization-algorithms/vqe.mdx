...
* __Operator__: Często Hamiltonian, który będziemy oznaczać jako $H$, opisujący właściwość systemu, którą chcesz zoptymalizować. Inaczej mówiąc, poszukujesz wektora własnego tego operatora odpowiadającego minimalnej wartości własnej. Często nazywamy ten wektor własny „stanem podstawowym".
* __„Ansatz"__ (niemieckie słowo oznaczające „podejście"): jest to obwód kwantowy, który przygotowuje stan kwantowy przybliżający poszukiwany wektor własny. W rzeczywistości ansatz to rodzina obwodów kwantowych, ponieważ niektóre z bramek w ansatzu są sparametryzowane, to znaczy przyjmują parametr, który możemy zmieniać. Ta rodzina obwodów kwantowych może przygotować rodzinę stanów kwantowych przybliżających stan podstawowy.
...
Powyższy Hamiltonian jest operatorem macierzowym $16\times 16$. Diagonalizacja tego operatora w celu znalezienia jego minimalnej energetycznej wartości własnej nie jest trudna.
...
* Jałowe płaskowyże: Te obszary zanikających gradientów[\[2-3\]](#references) istnieją nawet przy braku szumu, ale szum czyni je bardziej problematycznymi, ponieważ zmiana wartości oczekiwanych z powodu szumu może być większa niż zmiana spowodowana aktualizacją parametrów w tych jałowych obszarach.
