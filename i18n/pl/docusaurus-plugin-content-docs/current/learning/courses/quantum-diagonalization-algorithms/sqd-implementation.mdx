...
LUCJ jest sparametryzowanym ansatzem i musimy zainicjować parametry przed wykonaniem na sprzęcie. Jednym ze sposobów inicjalizacji ansatzu jest użycie amplitud `t1` i `t2` z klasycznej metody coupled cluster singles and doubles (CCSD), gdzie amplitudy `t1` są współczynnikami operatorów wzbudzeń pojedynczych, a amplitudy `t2` operatorów wzbudzeń podwójnych.
...
![Schemat pokazujący warstwy ansatz LUCJ.](/learning/images/courses/quantum-diagonalization-algorithms/sqd2/sqd2-fig9.avif)
...
Po zoptymalizowaniu obwodu pod kątem wykonania sprzętowego jesteśmy gotowi uruchomić go na docelowym sprzęcie i zebrać próbki do oszacowania energii stanu podstawowego. Ponieważ mamy tylko jeden obwód, użyjemy [trybu wykonania Job](/guides/execution-modes) z `qiskit-ibm-runtime` i wykonamy nasz obwód.
...
### 4.1 Odzyskiwanie konfiguracji: przegląd {#41-configuration-recovery-overview}
Każdy bit w ciągu bitów (wyznaczniku Slatera) reprezentuje orbital spinowy. Prawa połowa ciągu bitów reprezentuje orbitale ze spinem w górę, a lewa połowa orbitale ze spinem w dół. `1` oznacza, że orbital jest zajęty przez elektron, a `0` oznacza, że orbital jest pusty. Znamy z góry prawidłową liczbę cząstek (zarówno elektronów ze spinem w górę, jak i w dół). Załóżmy, że mamy wyznacznik $x$ z $N_x$ elektronami (to znaczy, że w ciągu bitów jest $N_x$ liczb $1$). Prawidłowa liczba cząstek to $N$. Jeśli $N_x \neq N$, to wiemy, że ciąg bitów jest zniekształcony przez szum. Procedura samouzgodnionej konfiguracji próbuje skorygować ciąg bitów, probabilistycznie odwracając $|N_x - N|$ bitów, wykorzystując informacje o średniej obsadzie orbitali. Średnia obsada orbitalu ($n$) mówi nam, jak prawdopodobne jest, że orbital będzie zajęty przez elektron. Jeśli $N_x < N$, mamy mniej elektronów i musimy odwrócić niektóre $0$ na $1$, i odwrotnie.
...
### 4.2 Estymacja stanu podstawowego {#42-ground-state-estimation}
...
### 4.3 Omówienie wyników {#43-discussion-of-results}
...
#### Ćwiczenie dla czytelnika {#exercise-for-the-reader}
