...
# Kwantowa diagonalizacja próbkowana (SQD) {#sample-based-quantum-diagonalization-sqd}
...
![Diagram komponentów kwantowych i klasycznych SQD. Po stronie kwantowej przygotowujesz stan i próbkujesz z jego nośnika; klasycznie rzutujesz macierz na próbkowaną podprzestrzeń i diagonalizujesz rzutowaną macierz.](/learning/images/courses/quantum-diagonalization-algorithms/sqd1/sqd1-fig1.svg)
Istnieje wiele sposobów przygotowania takiego stanu kwantowego — mogą być one wariacyjne lub niewariacyjne, w zależności od problemu.
...
2. W Lekcji 5 będziemy próbkować ze stanów bazy Krylova, aby rozpiąć podprzestrzeń dla problemu z fizyki materii skondensowanej. Podejście to ma charakter niewariacyjny.
...
![Diagramy dobrego i złego nośnika ansatzu. Dobry ansatz ma nośnik całkowicie zawierający nośnik ground state. Słaby ansatz zawiera tylko część nośnika ground state lub wcale go nie zawiera.](/learning/images/courses/quantum-diagonalization-algorithms/sqd1/sqd1-fig3.svg)
...
Przepaść między wymiarem pełnej przestrzeni Hilberta a wymiarem wykonalnej podprzestrzeni odsłania kolejny ważny aspekt SQD — rzadkość (sparsity) wave function. Podejście SQD działa dobrze dla rzadkich lub skoncentrowanych wave functions, w których jedynie niewielka część stanów bazowych ma niepomijalne (znaczące) amplitudy. Stoją za tym dwa powody:

1. Jeśli wave function jest rozległa (tzn. wiele stanów bazowych ma niepomijalne/znaczące amplitudy) i nie uwzględnimy wektorów z nośnikiem na stanie docelowym w podprzestrzeni, możemy skończyć z błędnymi wartościami własnymi i wektorami własnymi.
