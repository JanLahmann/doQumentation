...
Aby zapisać problem QUBO jako Hamiltonian Ising, musimy jedynie wykonać prostą zmianę zmiennych z $x \in \{0, 1\}^n$ na $z \in \{-1, 1\}^n$:
...
<Accordion>
<AccordionItem title="Szczegóły">

Po pierwsze, zauważ, że możemy przepisać nasze wyrażenie QUBO jako sumę wyrazów macierzowych:

$$
x^TQx = \sum_{ij}Q_{ij}x_ix_j
$$

Zastosowanie zmiany zmiennych daje

$$
\sum_{ij}Q_{ij}(\frac{1-z_i}{2})(\frac{1-z_j}{2}) = \sum_{ij}\frac{Q_{ij}}{4}(1 - z_i - z_j + z_iz_j)
$$

co można przekształcić w następujący sposób.

$$
\sum_{ij}\frac{Q_{ij}}{4}z_iz_j - \frac{Q_{ij}}{4}z_i - \frac{Q_{ij}}{4}z_j + \frac{Q_{ij}}{4}
$$

Możemy pominąć wyraz stały, ponieważ nie wpłynie on na to, które $z$ minimalizuje problem QUBO. Dodatkowo możemy pomnożyć to wyrażenie przez cztery, aby odzyskać pierwotną wartość $Q$, ponieważ to również nie wpływa na optymalny wybór $z$.

$$
\sum_{ij}Q_{ij}z_iz_j - Q_{ij}z_i - Q_{ij}z_j = z^TQz + (-\sum_{ij} Q_{ij}z_i + Q_{ij}z_j)
$$

Grupując podobne wyrazy liniowe, możemy zdefiniować nasz wektor współczynników liniowych $b$ za pomocą równania

$$
b_i = -\sum_{j} Q_{ij} + Q_{ji}
$$

co kończy transformację zmiennych do $z \in \{-1, 1\}^n$ po podstawieniu do poprzedniego równania.
...
Ostatecznie minimalizacja wyrażenia QUBO jest tożsama z minimalizacją tego wyrażenia, gdzie $b$ jest rzeczywistym skalarnym współczynnikiem:

$$
\min_{x\in\{0,1\}^n} x^TQx\Longleftrightarrow \min_{z\in\{-1,1\}^n}z^TQz + b^Tz
$$

Zapisując jeszcze raz nieco inaczej, otrzymujemy nasz Hamiltonian funkcji kosztu, gdzie minimum wyrażenia reprezentuje stan podstawowy, a `Z` to operator Pauli Z:
...
Zanim jednak przeprowadzimy transpilację, musimy wybrać backend, na którym uruchomimy nasz obwód, ponieważ transpilator optymalizuje w różny sposób dla różnych procesorów. To kolejny powód, dla którego ważne jest używanie automatycznego transpilatora — nie chciałbyś przechodzić przez czasochłonny proces ręcznego optymalizowania swojego Circuit, żeby potem zdać sobie sprawę, że chcesz go uruchomić na zupełnie innym procesorze o innych właściwościach.
...
Dla eksperymentu QAOA sesja byłaby dobrym wyborem — jeśli masz do niej dostęp — ponieważ musimy wielokrotnie próbkować nasz obwód z różnymi wartościami parametrów, aby znaleźć optimum.
...
Teraz, gdy udało nam się zmniejszyć naszą funkcję kosztu, znajdując lepsze wartości parametrów, uruchomimy nasz obwód z nowymi wartościami gamma i beta. Podałem tutaj konkretne wartości, których używam, ale pamiętaj — gdy sam spróbujesz lub nawet po prostu ponownie uruchomisz ten sam notatnik samouczka, wartości te mogą się nieznacznie zmienić. Teraz uruchomimy nasz zoptymalizowany Circuit z tymi wartościami i znajdziemy kandydujące rozwiązanie naszego problemu Max-Cut.
...
Teraz, gdy miałeś przedsmak procesu rozwiązywania małego problemu Max-Cut na komputerze kwantowym, rzucam ci wyzwanie, by zrobić to na skalę użytkową. Śledź powiązany [samouczek](/tutorials/quantum-approximate-optimization-algorithm), aby zobaczyć, ile cięć uda ci się uzyskać w grafie o 125 węzłach.
