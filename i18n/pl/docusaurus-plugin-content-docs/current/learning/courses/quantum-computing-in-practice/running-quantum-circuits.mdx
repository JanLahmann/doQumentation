...
<IBMVideo id="134063416" title="W tym filmie Olivia Lanes wyjaśnia obwody kwantowe i prymitywy. Omawia prymitywy IBM Quantum: Estimator i Sampler."/>
...
Każdy procesor można scharakteryzować za pomocą trzech podstawowych metryk wydajności opisujących jego skalę, jakość i szybkość: programowalnych kubitów, operacji na kubitach oraz maksymalnej liczby obwodów na sekundę.

* **Programowalne kubity:** Są to kubity, którymi użytkownicy mogą bezpośrednio sterować i włączać do algorytmów kwantowych. Ta metryka odzwierciedla skalę obliczeniową dostępną dla aplikacji.


* **Operacje na kubitach (QuOps):** Ta metryka opisuje, ile najbardziej wymagających obliczeniowo operacji kwantowych procesor może niezawodnie wykonać, zanim zgromadzone błędy ograniczą obliczenia. Służy jako miara jakości i możliwości obliczeniowych.


* **Maksymalna liczba obwodów na sekundę:** Ta metryka mierzy przepustowość obwodów, czyli ile obwodów kwantowych system może wykonać w ciągu sekundy. Wyższa przepustowość pozwala wykonać więcej użytecznej pracy w danym czasie.


Możesz również napotkać dodatkowe metryki procesora w dokumentacji technicznej, badaniach benchmarkowych i raportach wydajności:

EPLG (Errors Per Layered Gate). Miara średniego błędu wprowadzanego przez warstwowe operacje bramek splątujących. Niższe EPLG generalnie oznacza wyższą jakość bramek i mniejszą akumulację błędów.

CLOPS (Circuit Layer Operations Per Second). Benchmark, który mierzy, jak szybko system kwantowy może wykonać znormalizowane obciążenie robocze obejmujące zarówno operacje kwantowe, jak i powiązane przetwarzanie klasyczne.

Te dodatkowe metryki dostarczają bardziej szczegółowych informacji o zachowaniu sprzętu, podczas gdy programowalne kubity, operacje na kubitach i maksymalna liczba obwodów na sekundę dają bardziej ogólny obraz skali, jakości i szybkości systemu.
...
3.	**Execute.** Prymitywy IBM Quantum zapewniają interfejs do sprzętu IBM Quantum, który umożliwia uruchamianie transpilowanych obwodów. Ten krok obejmuje również stosowanie technik tłumienia i mitygacji błędów, które w dużej mierze mogą być ukryte przed użytkownikiem.
...
Następnie będziemy musieli wybrać procesor kwantowy, który ma wystarczającą liczbę kubitów o odpowiednio wysokiej jakości, abyśmy mogli uruchomić nasz obwód kwantowy. Kieruj się przy tym trzema podstawowymi metrykami omówionymi w sekcji o sprzęcie: programowalnymi kubitami, operacjami na kubitach i maksymalną liczbą obwodów na sekundę. W zależności od zastosowania, dodatkowe metryki, takie jak EPLG i CLOPS, mogą również dostarczyć przydatnych informacji o oczekiwanej jakości obwodu i wydajności wykonania.
...
Teraz jesteśmy gotowi do uruchomienia programu kwantowego. Prymitywy IBM Quantum zapewniają interfejs do sprzętu IBM Quantum i jednocześnie ukrywają przed użytkownikiem mechanizmy tłumienia oraz mitygacji błędów. Do wyboru są dwa prymitywy: Sampler i Estimator.
...
[IBM Quantum Estimator](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator) używa złożonego procesu algebraicznego do szacowania wartości oczekiwanej na rzeczywistym urządzeniu kwantowym, rozkładając obserwowalną na kombinację innych obserwowalnych o znanych eigenbazach.
...
Szczególnym rodzajem ZNE jest Probabilistic Error Amplification (PEA). Po nauczeniu się modelu szumu dla danego Gate'a PEA działa poprzez próbkowanie błędów z tego modelu szumu i celowe wstrzykiwanie ich do obwód. Nie jest to jeszcze dostępne w Qiskit, ale pojawi się w późniejszym czasie w tym roku.
...
Następnym razem przyjrzymy się konkretnemu przykładowi tego, jak wykorzystać przepływ pracy Qiskit patterns do zaprojektowania i uruchomienia obwodu kwantowego rozwiązującego klasyczny problem z teorii grafów zwany MaxCut.
