...
Jeśli wydaje Ci się to odwrotne (szczególnie znak między $X$ a $iY$), zobacz opcjonalną notatkę poniżej dotyczącą konwencji określającej, który stan $|0\rangle$ lub $|1\rangle$ odpowiada „zajętemu”, a który „niezajętemu”.

<Accordion>
<AccordionItem title="Opcjonalne uwagi na temat konwencji">

W wielu kontekstach fizycznych nie ma dwuznaczności co do interpretacji operatorów $\sigma^\pm$; zwiększają/zmniejszają one rzut z momentu pędu spinu systemu. $\sigma^+$, na przykład, przekształca stan momentu pędu spinu $|m_s\rangle$ w stan $|m_s+1\rangle$. Jednak przy stosowaniu mappera do wyrażania fermionowych operatorów kreacji i anihilacji za pomocą tych operatorów momentu pędu (kombinacji operatorów Pauliego), zwiększenie/zmniejszenie odpowiada teraz obsadzeniu orbitalu elektronowego. „Podnoszenie” i „obniżanie” nie budzi wątpliwości, jeśli działanie polega na zwiększeniu lub zmniejszeniu rzutu momentu pędu na daną oś i odpowiadającej mu etykiecie. W obsadzeniu orbitali elektronowych można powiązać $|0\rangle$ z pustym, a $|1\rangle$ z zajętym. Może się to wydawać oczywistą konwencją i rzeczywiście jest to konwencja stosowana w dodatkach Qiskit. Ale zauważ, że możliwe jest wybranie $|0\rangle$ jako „zajęty” w sensie relaksacji elektronu do stanu podstawowego systemu, a $|1\rangle$ jako „niezajęty”. Jest to wolny wybór i potencjalna dwuznaczność w etykietowaniu: który stan odpowiada zajętemu orbitalowi? $|0\rangle$ czy $|1\rangle$?

Obecnie jest to nieco ustandaryzowane w kontekście obliczeń kwantowych oraz we wbudowanych mapperach, takich jak dodatek Qiskit `FermionicMapper`, gdzie przyjmuje się
$$
\begin{aligned}
|0\rangle & = \text{ empty}\\
...
Równoważnie, operator, który tworzy elektron zajmujący orbital $k$ (reprezentowany przez kubit $q_k$), musi zachowywać się jak:

$$
O_k |q_N,...,q_k=0,...,q_0\rangle = c |q_N,...,q_k=1,...,q_0\rangle
$$

Zobaczmy teraz konsekwencje tego wyboru. Zauważ, że w standardowych bazach stosowanych w obliczeniach kwantowych,
...
i podobnym rozumowaniem możemy skonstruować pełny zbiór:
...
Z tego jasno wynika, że wybierając pozornie rozsądną konwencję, że $|0\rangle$ odpowiada brakowi elektronu, a $|1\rangle$ odpowiada jednemu elektronowi, otrzymujemy mylącą sytuację, w której $\sigma^+$ pojawia się w operatorze anihilacji, a $\sigma^-$ w operatorze kreacji. Zatem konwencja w `Qiskit Fermions` (gdzie $|0\rangle$ odpowiada zajętemu, a $|1\rangle$ odpowiada niezajętemu) skutkuje tym, że fermionowy operator kreacji $f_k^\dagger$ zawiera $\sigma_k^-$, a operator anihilacji $f_k$ zawiera $\sigma_k^+$.

Jeśli znajdziesz źródła z przeciwnymi znakami między terminami $X$ a $iY$ w operatorach kreacji i anihilacji, prawdopodobnie stosują one przeciwną konwencję dotyczącą tego, który stan odpowiada zajętemu orbitalowi.
...
gdzie $\sigma^+$ jest operatorem Pauli $X + iY$, a $\sigma^{-}$ jest $X - iY.$ Gdy już mamy Hamiltonian zapisany w tym formacie, trudna część etapu mapowania dobiegła końca i można go teraz łatwo zapisać jako obwód w operatorach Pauli.
...
Wiele z tego materiału jest bardzo techniczne i jeśli nie spotkałeś się z nim wcześniej, może wydawać się bardzo onieśmielające. Jednak z czasem staje się łatwiejsze — właśnie dlatego ten kurs nosi nazwę Quantum Computing in Practice. Nie jest to coś, co ktokolwiek może po prostu podchwycić i od razu stosować — wymaga nauki, zastanowienia i zapewne chwil frustracji. Zachęcam jednak do zaakceptowania tego dyskomfortu i naprawdę zgłębiania pytań, które będą pojawiać się w trakcie dalszej nauki. To jedyna droga do opanowania tej wiedzy.
