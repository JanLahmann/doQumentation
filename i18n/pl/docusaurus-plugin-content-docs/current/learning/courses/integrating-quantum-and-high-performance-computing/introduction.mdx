...
Teraz mamy kolejne ważne narzędzie dla postępu naukowego: komputery kwantowe. Choć wciąż jest to technologia wschodząca, komputery kwantowe mają potencjał, by znacząco zmienić rodzaje problemów obliczeniowych, które możemy efektywnie rozwiązywać. Ten kurs dotyczy zrozumienia, w jaki sposób te technologie, działając razem, mogą rozszerzać granice tego, co jest obliczeniowo możliwe.
...
Jednym z pierwszych znanych superkomputerów był CDC 6600 (1964), zbudowany przez Seymoura Craya, często nazywanego ojcem superkomputerów. CDC 6600 był najszybszym komputerem swojej epoki, wykorzystując innowacyjną architekturę obejmującą równoległe jednostki funkcjonalne i przetwarzanie potokowe – koncepcje stosowane w HPC do dziś.
...
## Nowa architektura {#new-architecture}

IBM wprowadził nową referencyjną architekturę superkomputerów kwantowo-centrycznych (QCSC), która określa, jak procesory kwantowe (QPU) mogą bezpośrednio integrować się z procesorami CPU i GPU w nowoczesnych środowiskach HPC. Podejście to jest napędzane przez udane hybrydowe przepływy pracy kwantowo-klasyczne&mdash;takie jak zastosowanie metody kwantowej diagonalizacji opartej na próbkach (SQD) do symulacji molekularnych&mdash;które już teraz wykazują dokładność porównywalną z zaawansowanymi technikami klasycznymi.

Architektura jest zaprojektowana jako otwarta, modułowa i kompozycyjna, tak aby zasoby kwantowe mogły być włączone do istniejących przepływów pracy HPC bez konieczności tworzenia zupełnie nowego stosu. Wczesne integracje z systemami takimi jak RIKEN i Fugaku pokazują, że hybrydowe przepływy pracy na poziomie produkcyjnym są już wykonalne. Architektura ta stanowi długoterminowe ramy dla ewoluującego wspólnego projektowania kwantowo-klasycznego na najbliższą dekadę.

![„Obraz przedstawiający pełną architekturę kwantową i HPC."](/learning/images/courses/integrating-quantum-and-high-performance-computing/introduction/qcsc-arch.avif)

Stos obejmuje warstwę aplikacyjną, która rozkłada problemy na komponenty klasyczne i kwantowe, oprogramowanie pośredniczące łączące klasyczne modele programowania z generowaniem obwodów kwantowych (na przykład poprzez zaktualizowane interfejsy Qiskit) oraz warstwę orkiestracji systemowej zarządzającą harmonogramowaniem zasobów hybrydowych przy użyciu narzędzi takich jak Quantum Resource Management Interface (QRMI).

Na poziomie sprzętowym architektura definiuje trzy poziomy:
* System kwantowy z QPU i klasycznymi środowiskami uruchomieniowymi o niskich opóźnieniach (FPGA/ASICs).
* Systemy CPU/GPU zlokalizowane w pobliżu, połączone za pomocą łączy niemal czasu rzeczywistego do zadań takich jak mitygacja błędów.
* Systemy skalowania poziomego zapewniające elastyczne klasyczne obliczenia do wstępnego przetwarzania, końcowego przetwarzania i hybrydowych przepływów pracy.

Ogólnie rzecz biorąc, IBM pozycjonuje architekturę QCSC jako praktyczny plan działania dla centrów HPC, który umożliwia rozpoczęcie integracji prawdziwego sprzętu kwantowego, tworzenie bardziej wydajnych algorytmów hybrydowych i przygotowanie infrastruktury na przyszłe odporne na błędy systemy kwantowe. Więcej informacji na temat tej architektury znajdziesz w [tym artykule](https://research.ibm.com/blog/quantum-centric-supercomputing-system-reference-architecture).
...
Ten kurs będzie korzystać z podstawowych narzędzi i konceptów obliczeń kwantowych z minimalnym wprowadzeniem. Zalecamy, aby uczestnicy posiadali co najmniej roboczą znajomość Qiskit, znajomość bramek i obwodów kwantowych, oraz pewne zapoznanie z algorytmami opartymi na próbkowaniu. Wymienione poniżej zasoby powinny zapewnić pomocne przygotowanie.

* [Przewodnik IBM Quantum&reg; Composer](/guides/composer): Przewodnik po Composerze, graficznym narzędziu do programowania kwantowego, które pozwala przeciągać i upuszczać operacje, aby budować obwody kwantowe i uruchamiać je na sprzęcie kwantowym.
...
Potraktuj ten kurs jako swoją wyrzutnię – miejsce, w którym budujesz mentalny zestaw narzędzi i pewność siebie, by odkrywać kwantowo-klasyczną granicę długo po ukończeniu ostatniej lekcji.
