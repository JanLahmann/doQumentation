...
W obliczeniach kwantowych możliwości systemu kwantowego są często opisywane za pomocą trzech podstawowych wymiarów: skali, jakości i szybkości. IBM Quantum charakteryzuje te wymiary za pomocą trzech metryk sprzętowych, które można zastosować w różnych modalnościach obliczeń kwantowych: programowalne kubity (skala), operacje na kubitach (jakość) oraz maksymalna liczba obwodów na sekundę (szybkość). Metryki te zapewniają ogólny widok na możliwości obliczeniowe i wydajność systemu w różnych implementacjach sprzętowych.

* **Skala** jest mierzona liczbą programowalnych kubitów. Programowalne kubity to kubity, którymi użytkownicy mogą bezpośrednio sterować, manipulować, mierzyć i włączać do algorytmów kwantowych.


* **Jakość** jest mierzona za pomocą operacji na kubitach (QuOps), metryki opisującej, ile najbardziej wymagających obliczeniowo operacji system może niezawodnie wykonać, zanim skumulowane błędy przytłoczą obliczenia. Wyższa zdolność QuOps zazwyczaj umożliwia głębsze obwody i bardziej wyrafinowane algorytmy kwantowe.


* **Szybkość** jest mierzona za pomocą maksymalnej liczby obwodów na sekundę, znanej również jako przepustowość obwodów. Ta metryka wychwytuje, ile użytecznych obwodów kwantowych system może wykonać w czasie i stanowi wskazówkę co do ogólnej wydajności obliczeniowej.


Dodatkowe metryki sprzętowe są często używane do charakteryzowania konkretnych aspektów zachowania procesora. Na przykład EPLG (errors per layered gate) i pomiary wierności warstwy zapewniają wgląd w jakość bramek i akumulację błędów, podczas gdy CLOPS (Circuit Layer Operations Per Second) mierzy szybkość wykonywania standaryzowanego obciążenia benchmarkowego kwantowego. Te metryki pozostają użyteczne do oceny konkretnych obciążeń i szczegółów implementacji, ale programowalne kubity, operacje na kubitach i maksymalna liczba obwodów na sekundę zapewniają szerszą strukturę do porównywania systemów kwantowych.

### Różnice między metrykami kwantowymi a klasycznymi {#differences-between-quantum-and-classical-metrics}

Klasyczne systemy obliczeniowe są często charakteryzowane za pomocą metryk takich jak FLOPS (operacje zmiennoprzecinkowe na sekundę), pojemność pamięci, przepustowość pamięci oraz częstotliwości taktowania procesora. Można dostrzec pewne luźne analogie między nimi a metrykami dla komputerów kwantowych wprowadzonymi powyżej, ale różnice w metrykach są istotne.

Maksymalną liczbę obwodów na sekundę można postrzegać jako luźny odpowiednik metryk wydajności zorientowanych na przepustowość w obliczeniach klasycznych, ponieważ mierzy ona, ile użytecznej pracy może wykonać system kwantowy w czasie. W przeciwieństwie do FLOPS, mierzy jednak wykonanie obwodów kwantowych, a nie operacji arytmetycznych.

CLOPS jest czasami porównywany do klasycznych benchmarków przepustowości, takich jak FLOPS, ale najlepiej rozumieć go jako benchmark specyficzny dla danego obciążenia, a nie uniwersalną miarę wydajności obliczeń kwantowych. EPLG i powiązane pomiary wierności dają wgląd w to, jak dokładnie procesor wykonuje warstwy bramek kwantowych. Te metryki są przydatne do zrozumienia konkretnych charakterystyk sprzętowych, podczas gdy podstawowa struktura skala-jakość-szybkość zapewnia bardziej ogólną podstawę do porównywania systemów kwantowych. Wiele opisów wydajności komputera kwantowego można znaleźć na stronie [Compute resources](https://quantum.cloud.ibm.com/computers) w IBM Quantum Platform. Zwróć uwagę, że choć liczba programowalnych kubitów jest wartością stałą dla danego QPU, metryki szybkości i jakości mogą się nieznacznie zmieniać w czasie z powodu aktualizacji kalibracji, konserwacji, warunków eksploatacyjnych oraz ulepszeń otaczającej infrastruktury sprzętowej i programowej.

Razem te uzupełniające się metryki pomagają kierować alokacją zasobów, planowaniem i projektowaniem aplikacji w systemach obliczeń kwantowych.

W wielu przypadkach cały system kwantowy jest traktowany jako pojedyncza jednostka. Jednak gdy zadanie przekracza pojemność jednej jednostki — czy to pod względem liczby qubitów do programowania, głębokości obwodu czy przepustowości obwodu — można zastosować techniki takie jak circuit cutting/knitting. Circuit cutting to proces dzielenia dużych zadań kwantowych na mniejsze, łatwiejsze do zarządzania podzadania, które można rozdzielić na wiele chipów kwantowych, umożliwiając skalowalne obliczenia kwantowe pomimo ograniczeń sprzętowych. Circuit knitting odnosi się do procesu następującego po circuit cutting — klasycznego kroku przetwarzania końcowego, który „zszywa" lub łączy wyniki z mniejszych podobwodów z powrotem razem.
...
QPU mogą być połączone z infrastrukturą klasyczną poprzez sieci i różne interfejsy programowania aplikacji (API), które pozwalają programistom oprogramowania na programistyczną interakcję z QPU. Te API są zwykle ukryte za zestawami narzędzi programistycznych (SDK) i bibliotekami (jak Qiskit) i udostępniane naukowcom obliczeniowym w formie abstrakcji programistycznych (jak prymitywy, o których będziemy mówić w Rozdziale 3: modele programowania).
...
* Jobs są zgłaszane przez użytkownika, z wykorzystaniem IBM Quantum Compute Service, zazwyczaj przy użyciu prymitywu IBM Quantum, takiego jak `Sampler`, `Estimator` lub innych.
...
* Po zgłoszeniu job poprzez Quantum Compute, job trafia do queue dla wybranego QPU. Użytkownik nie ma kontroli nad czasem queue, choć może to być inaczej w przypadku systemów dedykowanych.
...
(b) Zapewnienie, że znalezienie celu na jednym GPU zatrzymuje proces na innych GPU, aby zwolnić je na potrzeby kolejnego problemu

(c) Podzielenie przestrzeni przeszukiwania na regiony dla każdego z Twoich (powiedzmy 10) GPU do przeszukania
...
(b) Sterowanie/zarządzanie

(c) Alokowanie/planowanie
...
(d) Tak, mogłoby. Używa partycji kwantowej.
