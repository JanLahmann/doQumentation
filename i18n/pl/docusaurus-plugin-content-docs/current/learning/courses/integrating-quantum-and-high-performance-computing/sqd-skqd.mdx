...
Nazwij zbiór wszystkich stanów bazowych obliczeniowych tworzących Twój ansatz A. Nazwij zbiór wszystkich stanów bazowych obliczeniowych tworzących prawdziwy stan podstawowy Twojego systemu G. Który z poniższych odpowiada „dobremu” ansatzowi? Zaznacz wszystkie, które mają zastosowanie:
...
#### Krok 3: Rzutuj i diagonalizuj klasycznie {#step-3-project-and-diagonalize-classically-1}
...
### Mapowanie (`mapping.py` i `mapping.sh`) {#mapping-mappingpy-and-mappingsh}
...
### Optymalizacja (`optimization.py` i `optimization.sh`) {#optimization-optimizationpy-and-optimizationsh}
...
### Wykonanie (`execution.py` i `execution.sh`) {#execution-executionpy-and-executionsh}
...
### Przetwarzanie końcowe (`postprocessing.py` i `postprocessing.sh`) {#post-processing-postprocessingpy-and-postprocessingsh}
