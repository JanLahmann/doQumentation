...
![Diagram obwodu z czterema poziomymi liniami i wieloma bramkami.](/learning/images/courses/integrating-quantum-and-high-performance-computing/programming-models/checkin-circuit.avif)
...
Załóżmy, że modelujesz elektrony w cząsteczce. Chcesz oszacować (a) energię stanu podstawowego cząsteczki oraz (b) które stany bazowe obliczeniowe są najbardziej dominujące w stanie podstawowym cząsteczki. W każdym przypadku, którego prymitywu użyłbyś: Estimator czy Sampler?
...
* Główny przepływ sterowania programu jest determinowany przez rangę procesu. Jest to zasada SPMD w praktyce: program używa logiki warunkowej (na przykład if (rank == 0)), aby zapewnić, że tylko określone, zrównoleglone sekcje kodu są wykonywane przez procesy robocze, podczas gdy proces główny (często Rank 0) zajmuje się inicjalizacją i końcową agregacją.
...
Cały kod i skrypty użyte w tym rozdziale są dostępne dla ciebie w tym [repozytorium Github](https://github.com/christopherporter1/hpc-course-demos).
</div>
