...
Ta funkcja minimize ma kilka argumentów, ale najistotniejsze z nich dla chemii kwantowej to:
- Funkcja kosztu (`cost_func`). Jest związana z Hamiltonianem, ale obejmuje też pewne komplikacje, takie jak wyznaczanie wartości oczekiwanej za pomocą Estimatora, a w przypadku obliczeń stanów wzbudzonych może obejmować warunki ortogonalności.
- Stan początkowy (x0) układu, często stan Hartree-Focka
- Inne argumenty, w tym argumenty samej funkcji kosztu
- Metoda (method) ustawiona na wybrany przez Ciebie klasyczny optymalizator
- Opcje (options) dla klasycznego optymalizatora (nie mylić z opcjami Session omówionymi w następnej sekcji)

Przykładowy kod pokazano poniżej. Ograniczymy tu naszą dyskusję do dwóch ostatnich argumentów.
