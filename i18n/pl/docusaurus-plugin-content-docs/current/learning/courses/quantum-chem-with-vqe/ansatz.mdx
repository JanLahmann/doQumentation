...
Zaczniemy od czterokubitowego przykładowego obwodu `efficient_su2` z dwoma typami bramek SU(2), powiedzmy `rx` i `y`. Określamy też schemat splątania oraz liczbę powtórzeń. Jeśli po prostu wywołasz `.draw()` na obwodach, otrzymasz dość abstrakcyjną reprezentację. Bardziej zrozumiały diagram obwodu uzyskuje się, używając `.decompose().draw()`, a tutaj użyjemy `output = "mpl"'.
