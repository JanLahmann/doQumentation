...
Najpierw skonfigurujemy nasz problem, zanim przejdziemy do eksploracji każdej klasy optymalizatorów. Zaczniemy od obwodu zawierającego osiem parametrów wariacyjnych:
...
Gdy optymalizatory lokalne zbiegają do nieoptymalnych minimów lokalnych, możemy spróbować globalnego bootstrappingu optymalizacji i lokalnego doprecyzowania zbieżności. Choć wymaga to skonfigurowania dwóch wariacyjnych workloadów, pozwala optymalizatorowi znaleźć bardziej optymalne rozwiązanie niż sam optymalizator lokalny.
...
Niezależnie od rodzaju zastosowanej metody optymalizacji, jeśli krajobraz kosztu jest stosunkowo płaski, metodzie może być trudno ustalić właściwy kierunek poszukiwań. Ten scenariusz określa się mianem <DefinitionTooltip definition="When gradients of parametrized quantum circuits become exponentially small with respect to the number of qubits, making optimization difficult and potentially impossible.">jałowego płaskowyżu (barren plateau),</DefinitionTooltip> gdzie krajobraz kosztu staje się stopniowo coraz płaszy (a tym samym coraz trudniej jest ustalić kierunek do minimum). Dla szerokiego zakresu sparametryzowanych obwodów kwantowych prawdopodobieństwo, że gradient w dowolnym rozsądnym kierunku jest niezerowy z określoną precyzją, maleje wykładniczo wraz ze wzrostem liczby qubitów.
...
- **Eksperymentowanie z tłumieniem błędów i mitygacją błędów**: prymitywy IBM Quantum zapewniają prosty interfejs do eksperymentowania z różnymi wartościami odpowiednio `optimization_level` i `resilience_setting`. Może to zmniejszyć wpływ szumu i sprawić, że proces optymalizacji będzie bardziej wydajny.
...
Nasz wysokopoziomowy wariacyjny workload jest kompletny:
...
Następnie zbadamy konkretne algorytmy wariacyjne, mając na uwadze ten framework.
