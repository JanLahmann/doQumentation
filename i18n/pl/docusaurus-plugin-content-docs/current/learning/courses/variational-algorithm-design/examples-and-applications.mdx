...
## Definicje problemów {#problem-definitions}
...
### Eksperymentowanie z różnymi optymalizatorami {#experimenting-with-different-optimizers}
...
Problem maksymalnego cięcia (max-cut) jest problemem optymalizacji kombinatorycznej, który polega na podzieleniu wierzchołków grafu na dwa rozłączne zbiory w taki sposób, aby liczba krawędzi między tymi dwoma zbiorami była maksymalna. Bardziej formalnie, mając nieskierowany graf $G=(V,E)$, gdzie $V$ jest zbiorem wierzchołków, a $E$ jest zbiorem krawędzi, problem max-cut polega na podzieleniu wierzchołków na dwa rozłączne podzbiory, $S$ i $T$, tak aby liczba krawędzi mających jeden koniec w $S$, a drugi w $T$ była maksymalna.

Możemy zastosować max-cut do rozwiązania różnych problemów, takich jak klastrowanie, projektowanie sieci i przejścia fazowe. Zaczniemy od utworzenia grafu problemu:
...
Ten problem można wyrazić jako problem optymalizacji binarnej. Dla każdego węzła $0 \leq i < n$, gdzie $n$ jest liczbą węzłów grafu (w tym przypadku $n=4$), rozważymy zmienną binarną $x_i$. Zmienna ta będzie miała wartość $1$, jeśli węzeł $i$ należy do jednej z grup, którą oznaczymy jako $1$, i $0$, jeśli należy do drugiej grupy, którą oznaczymy jako $0$. Będziemy też oznaczać jako $w_{ij}$ (element $(i,j)$ macierzy sąsiedztwa $w$) wagę krawędzi biegnącej od węzła $i$ do węzła $j$. Ponieważ graf jest nieskierowany, $w_{ij}=w_{ji}$. Możemy więc sformułować nasz problem jako maksymalizację następującej funkcji kosztu:
...
Aby rozwiązać ten problem za pomocą komputera kwantowego, wyrazimy funkcję kosztu jako wartość oczekiwaną obserwabli. Jednak obserwable, które Qiskit natywnie dopuszcza, składają się z operatorów Pauliego, które mają wartości własne $1$ i $-1$ zamiast $0$ i $1$. Dlatego dokonamy następującej zmiany zmiennej:
...
Zatem nowa funkcja kosztu, którą chcemy zmaksymalizować, wynosi:
...
Ponadto, naturalną tendencją komputera kwantowego jest znajdowanie minimów (zazwyczaj najniższej energii) zamiast maksimów, więc zamiast maksymalizować $C(\vec{z})$, zminimalizujemy:

$$
-C(\vec{z}) =  \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2} z_iz_j -  \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2}
$$

Teraz, gdy mamy funkcję kosztu do zminimalizowania, której zmienne mogą przyjmować wartości $-1$ i $1$, możemy dokonać następującej analogii z bramką Pauliego $Z$:

$$
z_i \equiv Z_i = \overbrace{I}^{n-1}\otimes ... \otimes \overbrace{Z}^{i} \otimes ... \otimes \overbrace{I}^{0}
$$

Innymi słowy, zmienna $z_i$ będzie odpowiadać bramce $Z$ działającej na kubicie $i$. Ponadto:
