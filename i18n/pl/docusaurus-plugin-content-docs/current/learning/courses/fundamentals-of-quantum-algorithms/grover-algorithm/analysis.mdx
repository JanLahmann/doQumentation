...
dla każdego ciągu $x\in\Sigma^n$, i dlatego

$$
Z_g = - Z_f.
$$

To oznacza, że gdybyśmy mieli zastąpić funkcję $f$ funkcją $g$, algorytm Grovera nie działałby inaczej — ponieważ stany, które otrzymujemy z algorytmu w obu przypadkach są koniecznie równoważne aż do globalnej fazy.

To nie jest problem!
Mówiąc intuicyjnie, algorytmowi nie zależy na tym, które ciągi są rozwiązaniami, a które nie — musi jedynie umieć *odróżnić* rozwiązania od nierozwiązań, aby działać poprawnie.

## Działanie operacji Grover {#action-of-the-grover-operation}

Rozważmy teraz działanie $G$ na wektory stanu kwantowego $\vert A_0\rangle$ i $\vert A_1\rangle$.
...
ponownie dla każdego ciągu $x\in\Sigma^n$, i wygodnym alternatywnym sposobem wyrażenia tej operacji jest taki:

$$
Z_{\mathrm{OR}} = 2 \vert 0^n \rangle \langle 0^n \vert - \mathbb{I}.
$$

Prostym sposobem na sprawdzenie, że to wyrażenie zgadza się z definicją $Z_{\mathrm{OR}}$, jest obliczenie jego działania na standardowych stanach bazowych.
...
używając tej samej notacji $\vert u \rangle$, którą użyliśmy powyżej dla jednolitej superpozycji po wszystkich $n$-bitowych ciągach.

Teraz mamy to, czego potrzebujemy do obliczenia działania $G$ na $\vert A_0\rangle$ i $\vert A_1\rangle$.
Najpierw obliczmy działanie $G$ na $\vert A_0\rangle$.
...
Po drugie, obliczmy działanie $G$ na $\vert A_1\rangle$.
...
W obu przypadkach używamy równania
...
Wyjaśnia to, w kategoriach geometrycznych, dlaczego efektem operacji Grovera jest obrót kombinacji liniowych $\vert A_0\rangle$ i $\vert A_1\rangle$ o kąt $2\theta$.
