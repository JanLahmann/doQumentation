...
# Problem estymacji fazy {#the-phase-estimation-problem}
...
### Macierze normalne {#normal-matrices}
...
Twierdzenie spektralne: Niech $M$ będzie normalną macierzą zespoloną wymiaru $N\times N$.
Istnieje ortonormalna baza $N$-wymiarowych wektorów zespolonych $\bigl\{ \vert\psi_1\rangle,\ldots,\vert\psi_N\rangle \bigr\}$ oraz liczby zespolone $\lambda_1,\ldots,\lambda_N$ takie, że
$$
...
Wynika to z faktu, że $\bigl\{ \vert\psi_1\rangle,\ldots,\vert\psi_N\rangle \bigr\}$ jest ortonormalna:
...
Innymi słowy, każda liczba $\lambda_j$ jest *wartością własną* macierzy $M,$ a $\vert\psi_j\rangle$ jest *wektorem własnym* odpowiadającym tej wartości własnej.
...
- **Przykład 1**.
  ...
  Co więcej, możemy wybrać $\{\vert\psi_1\rangle,\vert\psi_2\rangle\}$ jako *dowolną* bazę ortonormalną i równanie nadal będzie prawdziwe. Na przykład:
...
Czyli $\lambda$ jest wartością własną macierzy $U,$ a $\vert\psi\rangle$ jest wektorem własnym odpowiadającym tej wartości własnej.
...
## Sformułowanie problemu estymacji fazy {#phase-estimation-problem-statement}

W *problemie estymacji fazy* dane są: stan kwantowy $\vert \psi\rangle$ złożony z $n$ kubitów oraz unitarny obwód kwantowy działający na $n$ kubitach.
*Zakładamy*, że $\vert \psi\rangle$ jest wektorem własnym macierzy unitarnej $U$ opisującej działanie tego obwodu, a naszym celem jest obliczenie lub przybliżenie wartości własnej $\lambda$, której odpowiada $\vert \psi\rangle$.
Dokładniej, ponieważ $\lambda$ leży na zespolonym okręgu jednostkowym, możemy zapisać
...
<Figure title="Problem estymacji fazy">
Wejście: unitarny obwód kwantowy dla $n$-kubitowej operacji $U$ wraz z $n$-kubitowym stanem kwantowym $\vert\psi\rangle$ \
Założenie: $\vert\psi\rangle$ jest wektorem własnym $U$ \
...
1. Problem estymacji fazy różni się od innych problemów, które poznałeś dotychczas w tym kursie, tym że wejście zawiera stan kwantowy. Zazwyczaj skupiamy się na problemach z klasycznymi wejściami i wyjściami, ale nic nie stoi na przeszkodzie, by rozważać takie kwantowe stany wejściowe jak tutaj. Jeśli chodzi o praktyczne zastosowania, problem estymacji fazy najczęściej pojawia się jako *podproblem* w ramach większego obliczenia — tak jak zobaczymy w kontekście faktoryzacji liczb całkowitych w dalszej części lekcji.

2. Powyższe sformułowanie problemu estymacji fazy nie precyzuje, czym dokładnie jest przybliżenie $\theta,$ jednak możemy formułować bardziej szczegółowe wersje problemu w zależności od naszych potrzeb i zainteresowań. W kontekście faktoryzacji liczb całkowitych będziemy wymagać bardzo dokładnego przybliżenia $\theta,$ lecz w innych przypadkach może wystarczyć nam bardzo przybliżona wartość. Wkrótce omówimy, jak wymagana precyzja wpływa na koszt obliczeniowy rozwiązania.

3. Zauważmy, że przechodząc od $\theta = 0$ do $\theta = 1$ w problemie estymacji fazy, okrążamy cały okrąg jednostkowy, zaczynając od $e^{2\pi i \cdot 0} = 1$ i poruszając się przeciwnie do ruchu wskazówek zegara w kierunku $e^{2\pi i \cdot 1} = 1.$ Oznacza to, że po osiągnięciu $\theta = 1$ wracamy do punktu, od którego zaczęliśmy, czyli $\theta = 0.$ Dlatego przy ocenie dokładności przybliżeń wartości $\theta$ bliskie 1 powinny być traktowane jako bliskie 0. Na przykład przybliżenie $\theta = 0{,}999$ należy uznać za różniące się od $\theta = 0$ o mniej niż $1/1000.$
