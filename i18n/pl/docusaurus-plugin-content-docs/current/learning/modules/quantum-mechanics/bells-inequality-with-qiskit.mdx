...
**Uwaga:** Byłoby bardzo imponujące, gdybyś potrafił wymyślić eksperyment, który dawałby różne wyniki dla opcji Einsteina i Borna; ludziom zajęło dekady, aby taki wymyślić.

<Accordion>
<AccordionItem title="Odpowiedź">

Trzymając się dotychczas opisanego eksperymentu (czyli braku netto spinu przy antyrównoległym ustawieniu pozytonu i elektronu), pomiar obu spinów wzdłuż $\pm x$, $\pm y$ lub $\pm z$ zawsze dawałby przeciwne znaki ze względu na zachowanie momentu pędu, niezależnie od tego, która opcja jest prawdziwa. Zmierzenie spinu jednej cząstki (powiedzmy elektronu) wzdłuż jednego kierunku (powiedzmy $+z$) oznacza, że spin drugiej cząstki, pozytonu, zostałby zmierzony wzdłuż $-z$. Jeśli zamiast tego zmierzysz spin pozytonu wzdłuż kierunku $x$, będzie równie prawdopodobne, że wyjdzie $+x$ lub $-x$. Może to wynikać z tego, że tak mówią ukryte instrukcje (opcja 1 Einsteina) lub z tego, że rozkład prawdopodobieństwa spinu pozytonu aktualizuje się po pomiarze spinu elektronu, a nowy rozkład prawdopodobieństwa jest zgodny z podziałem 50-50 między $\pm x$ (opcja 2 Borna). Punkty te są wyjaśnione bardziej szczegółowo poniżej.
...
Reszta tej lekcji jest poświęcona eksperymentowi, który pozwala odróżnić opcje Einsteina i Borna, więc nie będziemy tutaj wchodzić w zbyt wiele szczegółów. Jednak część sztuczki polega na zmierzeniu obu cząstek wzdłuż różnych kierunków (np. $x$ i $z$, a nawet jakiegoś kierunku pomiędzy tradycyjnymi osiami kartezjańskimi). Reszta wynika z dokładnego rozważenia precyzyjnego prawdopodobieństwa uzyskania różnych wyników z uwzględnieniem przewidywań mechaniki kwantowej oraz przewidywań informacji klasycznej, jak w przypadku zmiennych ukrytych.

</AccordionItem>
</Accordion>

W obu opcjach, jeśli dwoje obserwatorów, Lucas i Rihanna, zmierzą wzdłuż tej samej osi, spodziewalibyśmy się, że uzyskają antyrównoległe spiny, niezależnie od tego, która opcja jest prawdziwa. Aby zobaczyć dlaczego, rozważ poniższe diagramy.
...
Powyższy rysunek pokazuje opcję Einsteina. Kierunki spinów są przeciwne i ustalone. Jeśli zmierzymy wzdłuż osi $z$, jeden będzie wzdłuż $+z$, a drugi wzdłuż $-z$. Nie mamy powodu zakładać, że pozyton byłby wzdłuż $+z$, a elektron wzdłuż $-z$; obraz pokazuje jedynie, że spiny zostaną zmierzone jako skierowane w przeciwne strony. W rzeczywistości dany spin nie musi w istocie mieć składowej spinu wzdłuż kierunku ostatecznie zmierzonego, w przypadku opcji Einsteina. Najsłabszym stwierdzeniem opcji Einsteina jest to, że istnieje pewien zestaw instrukcji przechowywanych w spinie, który określa, jakie będą wyniki pomiarów, gdy zostaną wykonane wzdłuż dowolnej osi. Nie musimy wyobrażać sobie, że te instrukcje mają postać prostego wektora (patrz diagram poniżej); wrócimy do tego później.
...
Poniższy rysunek pokazuje opcję Borna, w której kierunki spinów pozytonu i elektronu są rozmyte w rozkładzie prawdopodobieństwa i nie mają określonego kierunku. Nie należy przywiązywać zbyt dużej wagi do kształtu rozkładu. Każdy spin mógłby w rzeczywistości mieć niezerowe prawdopodobieństwo skierowania w dowolną stronę, dopóki są one przeciwne względem siebie; narysowaliśmy je po prostu jako fragmenty okręgu, abyśmy mogli wizualnie je odróżnić na potrzeby dyskusji. Zauważ, że w przypadku opcji Borna nadal prawdą jest, że moment pędu musi być zachowany. Zatem jeśli jedna fala prawdopodobieństwa zostanie „skolapsowana” tak, że spin wskazuje wzdłuż $+z$, druga wskaże wzdłuż $-z$ i zostanie odchylona w przeciwnym kierunku. Opcje wyglądają identycznie.
...
Ale co się dzieje, gdy obserwatorzy L i R mogą mierzyć wzdłuż jednej z trzech osi, z każdą parą oddaloną o 120 stopni, jak pokazano na Rysunkach 4 i 5. Każdy obserwator może losowo zdecydować, wzdłuż jakiej osi zmierzy spin (a, b lub c). Nie muszą mierzyć wzdłuż tej samej osi. Gdy każdy obserwator dokonuje pomiaru, może znaleźć dodatnią projekcję na wybranej osi lub może znaleźć ujemną projekcję. Na przykład, Lucas i Rihanna mogą zmierzyć +a i -b lub +b i +c. Zauważ, że jeśli zdarzy im się wybrać pomiar wzdłuż tej samej osi, to MUSZĄ uzyskać przeciwne znaki w swoich projekcjach: +a i -a, +b i -b lub +c i -c; nie mogą oboje znaleźć na przykład +a. W następnej sekcji przeanalizujemy, jak obliczyć prawdopodobieństwo, że Lucas i Rihanna otrzymają ten sam znak na swoich zmierzonych osiach (++ lub --) oraz przeciwne znaki (+-) lub (-+).
...
Powyższe dwa rysunki ilustrują możliwe interpretacje zmiennych ukrytych w tym nowym scenariuszu pomiaru w trzech osiach. To znaczy, albo spiny są już określone jako wektory, albo pewien zestaw fizycznych instrukcji istnieje w jakiś sposób osadzony w systemie, tak że wyniki wszystkich możliwych pomiarów są z góry określone, nawet jeśli są nieznane eksperymentatorom przed pomiarem. Alternatywa jest zilustrowana poniżej. Istnieje pewien rozkład prawdopodobieństwa wyników, który może powiedzieć nam pewne rzeczy o prawdopodobieństwie różnych wyników pomiaru, ale wyniki są nieokreślone przez naturę przed pomiarem.
...
Możemy zadać sobie pytanie: „Jak często obaj gracze powinni znaleźć ten sam znak projekcji spinu?” To znaczy, nawet nie rejestrujemy, wzdłuż której osi wybrali pomiar; po prostu rejestrujemy, czy znaleźli ten sam znak czy różny znak. Nie jest oczywiste, czy opcje Einsteina i Borna dadzą taki sam wynik w tym bardziej złożonym schemacie pomiarowym. Ale z Rysunków 4 i 5 powinno być jasne, że $możliwe$ jest wystąpienie różnicy. W przypadku pokazanym w opcji Einsteina, pomiar projekcji spinu $e+$ na osi $a$ z pewnością da $+a$, a projekcja spinu $e-$ na osi $b$ da $-b$ (ledwie). Ale w opcji Borna możliwości są szeroko otwarte. Prawdą jest, że moment pędu jest nadal zachowany. Ale ponieważ oba pola magnetyczne nie są zorientowane wzdłuż tej samej osi, zmuszamy cząstki do sytuacji, w której muszą skolapsować na różne osie (poprzez interakcje z polem). W następnej sekcji użyjemy mechaniki kwantowej, aby określić, jakie powinny być prawdopodobieństwa, przy założeniu opcji Borna, że Lucas i Rihanna uzyskają ten sam znak na swoich zmierzonych osiach (++ lub --) oraz prawdopodobieństwa, że uzyskają przeciwne znaki (+- lub -+).
...
### Sprawdź, czy rozumiesz {#check-your-understanding-1}
...
Zgodnie z naszym podejściem opisanym powyżej, zdefiniujmy kierunek pomiaru Lucasa jako $+z$. Było to wygodne już w podejściu algebraicznym, ale jest szczególnie wygodne w obliczeniach kwantowych, ponieważ to, co jest zazwyczaj mierzone, to rzut kubita wzdłuż $z$.
Chcemy zbudować obwód kwantowy, który daje nam te same warunki prawdopodobieństwa co powyżej dla $P_{++}$. Możemy dowolnie zorientować naszą płaszczyznę tak, by $\phi=0$, i otrzymujemy
$$
...
$$
Musimy wiedzieć kilka rzeczy o komputerach kwantowych IBM, które ukierunkują naszą dyskusję. Po pierwsze, kubity są inicjalizowane w stanie $|0\rangle = |+\rangle_z$. Jak wspomniano wcześniej, gdy dokonywane są pomiary, odbywają się one wzdłuż osi $z$. Celem jest więc ustalenie, jakie operatory możemy wstawić pomiędzy bazowe stany pomiarowe $\langle 0|\langle 0|$ a stany początkowe kubitów $|0\rangle |0\rangle$, aby uzyskać skomplikowane wyrażenie powyżej. W tym celu musimy przypomnieć sobie podstawowe bramki w obliczeniach kwantowych.

Bramka $X$: Odpowiednik operacji NOT. Bramka jednokubitowa.
$$
...
W Qiskit tworzenie obwodu kwantowego z bramką $X$ wygląda tak:
...
Bramka Hadamarda $H$: Tworzy stan superpozycji. Bramka jednokubitowa.
$$
...
Obwód kwantowy z bramką Hadamarda tworzy się następująco:
...
Bramka CNOT (Controlled-NOT): Ta bramka używa dwóch kubitów: kontrolnego i docelowego. Sprawdza stan kubita kontrolnego, który nie jest zmieniany. Jeśli jednak kubit kontrolny jest w stanie $|1\rangle$, bramka zmienia stan kubita docelowego; jeśli kubit kontrolny jest w stanie $|0\rangle$, żadna zmiana nie jest dokonywana. W poniższej notacji przyjmijmy, że pierwszy kubit jest kontrolnym, a drugi docelowym.
$$
...
Bramka CNOT wygląda nieco inaczej w obwodzie kwantowym, ponieważ wymaga dwóch kubitów. Oto jak się ją implementuje:
...
Zauważ, że pierwszy kubit podany w `qc.cx(0,1)` jest kontrolnym, a drugi docelowym. Na schemacie docelowy to ten ze znakiem „+" lub krzyżykiem.

Bramka obrotu $R_y(\theta)$: Obraca stan wokół osi y. Jest to bramka jednokubitowa.
$$
...
Wreszcie, bramki rotacji implementuje się, podając typ bramki, kąt obrotu i kubit, na którym bramka jest umieszczona — w tej kolejności:
...
Nazwa bramki `ry` określa oś, wokół której następuje obrót. Pierwszy argument $\pi/2$ oznacza kąt obrotu, a drugi argument wskazuje kubit, na którym bramka ma być umieszczona.

### Sprawdź swoje zrozumienie {#check-your-understanding-5}

Używając składni wprowadzonej lub przypomnianej powyżej, utwórz dowolny obwód kwantowy zawierający cztery różne typy bramek kwantowych.
...
### Od eksperymentu fizycznego do obwodów kwantowych {#from-physical-experiment-to-quantum-circuits}

Na podstawie działania tych bramek możemy dostrzec, że kety w wyrażeniach na $P_{++}$:
$$
\frac{1}{\sqrt{2}}\left(|+\rangle_L|-\rangle_R-|-\rangle_L|+\rangle_R\right)
$$
prawdopodobnie wymagają bramki Hadamarda do uzyskania superpozycji oraz bramki CNOT do stworzenia splątania.

Użyjemy teraz bramek H, X i CNOT, aby przekształcić $|0\rangle_L|0\rangle_R$ w $\frac{1}{\sqrt{2}}\left(|+\rangle_L|-\rangle_R-|-\rangle_L|+\rangle_R\right)$:
...
$$
Tutaj $CNOT_{LR}$ oznacza bramkę CNOT używającą L jako kontrolnego i R jako docelowego. Możemy teraz wyłączyć część R stanu:
$$
...
Zapisaliśmy teraz ket wyłącznie jako bramki kwantowe działające na domyślny stan początkowy kubitów.
...
Daje nam to precyzyjne instrukcje dotyczące budowy naszego obwodu kwantowego. Zastosujemy bramki X, H, CNOT i $R_y$ na kubitach reprezentujących stany kwantowe cząstek mierzonych przez Lucasa i Rihannę, a następnie wykonamy pomiary, aby uzyskać prawdopodobieństwo.
...
- Krok 2: Zoptymalizuj swój obwód kwantowy do uruchomienia na prawdziwym sprzęcie kwantowym
- Krok 3: Wykonaj zadanie na komputerach kwantowych IBM przy użyciu Runtime Primitives
- Krok 4: Przetwórz wyniki

Zasadniczo cała praca wykonana powyżej stanowiła krok 1. Zbudujmy teraz wynikowy obwód kwantowy używając Qiskit!

### Krok 1: Odwzorowanie naszych wyników na obwód kwantowy {#step-1-mapping-our-results-to-a-quantum-circuit}
...
Pamiętaj, że 1/3 czasu oś wybrana przez Rihannę będzie znajdować się w odległości $2\pi/3$ radianów od osi Lucasa, 1/3 czasu będzie w odległości $4 \pi/3$ radianów od osi Lucasa, a 1/3 czasu oboje wybiorą tę samą oś. Musimy więc zbudować 3 obwody kwantowe dla tych 3 przypadków i zsumować wyniki. Pierwszy z nich wyjaśnimy dokładnie, a dwa pozostałe po prostu podamy.
...
Poniższy kod szybko buduje wszystkie trzy obwody kwantowe w bardziej uproszczony sposób. Zauważ, że jedyną różnicą między tymi trzema obwodami kwantowymi jest to, o ile obracamy dwa kubity wokół osi $y$.
...
Gdy spojrzymy na liczby z każdego obwodu kwantowego, zobaczymy, że dwa z nich były zasadniczo identyczne, a trzeci był zupełnie inny.
...
Stwórzmy listę możliwych wyników i zsumujmy wszystkie liczby dla każdego stanu ze wszystkich trzech obwodów kwantowych, aby uzyskać ogólne prawdopodobieństwa.
...
### Sprawdź swoją wiedzę {#check-your-understanding-6}
...
### Krok 2: Zoptymalizuj swój obwód kwantowy pod kątem działania na prawdziwym sprzęcie {#step-2-optimize-your-quantum-circuit-for-running-on-real-hardware}
...
Teraz, gdy zoptymalizowaliśmy nasz obwód kwantowy pod kątem działania na prawdziwym sprzęcie kwantowym i zdebugowaliśmy kod przy użyciu symulatorów, jesteśmy gotowi zebrać statystyki z prawdziwego komputera kwantowego i rozstrzygnąć spór między Einsteinem a Bornem.
...
Zaprojektowaliśmy nasze obwody kwantowe tak, by istniały cztery możliwe wyniki odpowiadające temu, że Lucas i Rihanna mierzą jeden lub drugi znak rzutu spinu: 00, 01, 10 i 11. W przypadkach 00 i 11 Lucas i Rihanna mierzą ten sam znak, natomiast w przypadkach 01 i 10 mierzą przeciwne znaki. Widzimy, że z bardzo dobrym przybliżeniem szansa, że Lucas i Rihanna otrzymają ten sam znak, wynosi około 50% — zdecydowanie więcej niż $4/9$. Oznacza to, że żaden zestaw ukrytych zmiennych nie może wyjaśnić tego rozkładu prawdopodobieństw i że podejście z ukrytymi zmiennymi nie jest zgodne z eksperymentem.
...
6. Która z poniższych jest bramką dwukubitową?
...
7. Załóżmy, że kubit jest w stanie $|0\rangle$. Jakie jest prawdopodobieństwo zmierzenia go w stanie $1\rangle$?
