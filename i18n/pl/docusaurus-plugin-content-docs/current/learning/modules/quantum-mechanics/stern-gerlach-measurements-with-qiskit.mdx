...
### Sprawdź swoje rozumienie {#check-your-understanding-1}
...
### Jak kubity pomogą nam to przetestować? {#how-will-qubits-help-us-test-this}

Większość komputerów kwantowych używa „kubitów" – kwantowych analogów klasycznych bitów. Dokładniej, są one zaprojektowane jako układy dwupoziomowe, analogiczne do stanów „włączony"/"wyłączony" klasycznych bitów. Istnieją paradygmaty obliczeń kwantowych wykorzystujące układy trzypoziomowe (tzw. „kutrity") lub układy wielopoziomowe (zwane „kuditami"). Jednak większość prac skupia się na kubitach. W szczególności komputery kwantowe IBM&reg; używają tak zwanych kubitów transmonowych o stałej częstotliwości. Różnią się one dość znacznie od orbitalnego lub spinowego momentu pędu atomów. Ale podobnie jak spin elektronu, kubity IBM&reg; są układami kwantowomechanicznymi, które mogą oddziaływać ze światłem i na których można wykonywać pomiary. W rzeczywistości często można znaleźć analogie między stanami kwantowomechanicznego spinu a stanami obliczeniowymi kubitu. Na przykład często można zobaczyć stan „spin w górę" powiązany ze stanem obliczeniowym 0, a „spin w dół" powiązany ze stanem obliczeniowym 1:
...
Teraz użyjemy powyższych kątów jako parametrów w obwodzie kwantowym. Rozważamy tylko jedną cząstkę na raz, więc użyjemy w naszym obwodzie tylko jednego kubitu i będziemy potrzebować tylko jednego rejestru klasycznego.
...
Aby uruchomić nasz eksperyment na prawdziwym komputerze kwantowym, musimy załadować IBM Quantum Compute Service i wybrać komputer kwantowy (czyli „Backend"). Poniżej po prostu wybieramy najmniej zajęty dostępny komputer kwantowy.
...
Musimy teraz dokonać transpilacji obwodu, co oznacza, że musimy odwzorować nasz obwód na bazowe bramki dostępne w wybranym komputerze kwantowym i zoptymalizować go pod kątem działania na tym komputerze.
...
W eksperymencie Sterna-Gerlacha badacze nie mogli określić kąta, pod jakim wektor momentu pędu wyłaniałby się z pieca. Orientacje były losowe (a może nawet bardziej tajemnicze! Zobacz moduł Qiskit Classrooms dotyczący twierdzenia Bella). Rozsądnym analogiem tego eksperymentu byłoby losowe inicjowanie stanów naszego kubitu i wielokrotne wykonywanie pomiarów.

### Krok 1: Mapowanie klasycznych danych wejściowych na problem kwantowy {#step-1-map-classical-inputs-to-a-quantum-problem-1}

Obwód, który chcemy zbudować, jest taki sam jak poprzednio. Jedyna różnica polega na tym, że tym razem skonstruujemy obwód przy użyciu bramek z wolnymi parametrami $\theta$ i $\phi$. Wartości liczbowe tych parametrów będą następnie przypisywane dla każdego nowego uruchomienia.
...
Uruchamianie obwodu kwantowego tylko dla jednego strzału i robienie tego dla wielu losowych konfiguracji to nietypowy przepływ pracy dla komputera kwantowego. Z pewnością można to zrobić, ale dla uproszczenia użyjemy tutaj lokalnego symulatora.
...
Widzimy więc, że losowy stan początkowy kubitu (odpowiadający losowym orientacjom momentu pędu w eksperymencie Sterna-Gerlacha) daje podobną liczbę stanów 0 i 1 (podobnie jak podobna liczba stanów spin-w-górę i spin-w-dół). Jest to dokładnie to, co pokazał oryginalny eksperyment Sterna-Gerlacha.

## Czwarty eksperyment: Powtórzone pomiary {#fourth-experiment-repeated-measurements}

Gdy kubit zaczyna w losowym stanie, widzimy, że istnieje mniej więcej 50-procentowa szansa zmierzenia jednego ekstremum zamiast drugiego. Ale co dzieje się ze stanem kubitu (lub momentem pędu cząstki) po dokonaniu pomiaru? Aby to zbadać, musimy zdefiniować obwód umożliwiający wykonanie wielu pomiarów na tych samych kubitach. Zdefiniujmy obwód, którego możemy użyć do zbadania tej kwestii. Chcemy dopuścić możliwość zmierzenia obu stanów $|0\rangle$ i $|1\rangle$, więc potrzebujemy czegoś, co obróci początkowy stan kubitu ze stanu domyślnego $|0\rangle$. W tym przypadku użyjemy bramki Hadamarda $H$, ponieważ $H|0\rangle = \frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)$. Zauważ, że domyślnie oba te pomiary będą wykonywane wzdłuż osi $z$.
...
Na powyższym rysunku pojemniki (biny) są oznaczone „00", „01" i tak dalej. Liczby te odnoszą się do „wynik 2. pomiaru, wynik 1. pomiaru". Zatem „00" oznacza, że oba pomiary dały stan $|0\rangle$, a „01" oznacza, że pierwszy pomiar dał $|1\rangle$, a drugi $|0\rangle$. Zdecydowana większość kubitów była mierzona albo dwukrotnie jako $|0\rangle$, albo dwukrotnie jako $|1\rangle$. Przypadki, w których kubit był mierzony w jednym stanie, a następnie w drugim, były bardzo rzadkie, a te nieliczne przypadki (~1%) wynikają z szumów. W tym przypadku szum ten został zasymulowany zgodnie z zachowaniem rzeczywistego układu kwantowego. Obserwowana korelacja między kolejnymi pomiarami jest również obserwowana w pomiarach typu Sterna–Gerlacha cząstek z spinowym momentem pędu. Jeśli cząstka zostanie zmierzona jako „spin w górę", kolejny pomiar wykonany wkrótce potem ponownie da wynik „spin w górę" (z wyjątkiem niewielkich odchyleń spowodowanych szumami).

Może to wydawać się oczywiste. W końcu, jeśli kubit zostaje zmierzony w pewnym stanie, a ja zmierzę go ponownie, szybko, dlaczego miałby nie pozostać w tym stanie? Ale jeśli naprawdę skupimy się na tej subtelności, może ona pomóc nam wybrać narzędzia matematyczne do opisu tego zjawiska.
...
- Pomiar układu kwantowego daje tylko jedną z dozwolonych wartości. Dla układów dwupoziomowych, takich jak kubit lub cząstka o spinie 1/2, pomiar zawsze daje jeden z dwóch możliwych wyników.
...
Przypomnij sobie, że w powyższych eksperymentach odkryliśmy, że kubit w stanie $|0\rangle$ pozostaje w tym stanie przy kolejnych pomiarach. To samo dotyczy $|1\rangle$. Oznacza to, że kubit w stanie $|0\rangle$ ma zerowe prawdopodobieństwo bycia zmierzonym w stanie $|1\rangle$ (pomijając efekty szumów). Wymagany przez nas związek między iloczynami skalarnymi a prawdopodobieństwami pomiarów mówi nam wtedy, że
...
Jest to użyteczne, ponieważ ten operator Pauliego (i macierze pokrewne) może opisywać wiele układów fizycznych obejmujących dwa poziomy lub dwa możliwe wyniki pomiaru, w tym stany kubitów transmonowych w komputerze kwantowym.
...
Zweryfikuj odpowiedzi do części (b) i (c) poprzedniego pytania, korzystając z analogii między stanami spinu skierowanego w górę atomu a stanami kubitu:

$$
|\uparrow\rangle\sim|0\rangle.
$$

Zakoduj obwody kwantowe, które tworzą żądany stan początkowy, a następnie użyj prawdziwego sprzętu lub symulatora, aby sprawdzić wyniki pojedynczego pomiaru oraz rozkład wyników z zespołu złożonego z setek lub nawet tysięcy pomiarów.
...
### Sprawdź swoje rozumienie {#check-your-understanding-4}
...
Wynik ten pokazuje, że dla kubitu początkowo w stanie $|0\rangle$ (lub równoważnie, spinu w stanie $|\uparrow\rangle$) prawdopodobieństwo zmierzenia rzutu wzdłuż $+x$ lub $-x$ wynosi mniej więcej 50-50. Pod pewnym względem jest to całkowicie zrozumiałe. W końcu, jeśli coś wskazuje wzdłuż kierunku z, nie powinno preferować ani $+x$, ani $-x$. Można oczekiwać tego samego dla stanu $|1\rangle$ (lub $|\downarrow\rangle$). Sprawdźmy to:
...
- Kubity można zmierzyć tylko w jednym z dwóch stanów, często oznaczanych jako $|0\rangle$ i $|1\rangle$.
