...
{/* doqumentation-source-hash: 74131ded */}
...
- **Niepowodzenie dla małych instancji**: Konwencjonalne solwery mieszanego programowania całkowitoliczbowego (Mixed Integer Programming) zawodzą już dla instancji z zaledwie siedmioma produktami przy limicie czasu jednej godziny [\[4\]](#references)
...
**Wydajność obwodów**: Algorytm bf-DCQO osiąga znaczną redukcję bramek [\[1\]](#references):
- Do **10 razy mniej bramek splątujących** niż Digital Quantum Annealing (DQA)
- Znacznie płytsze obwody umożliwiają:
  ...
  - Brak konieczności stosowania technik łagodzenia błędów
...
**Protokoły kontradiabatyczne**: Algorytm wykorzystuje człony kontradiabatyczne, które tłumią niechciane wzbudzenia kwantowe podczas krótkich czasów ewolucji, umożliwiając systemowi pozostanie blisko stanu podstawowego nawet przy szybkich przejściach [\[1\]](#references).
...
3. **Redukują głębokość obwodu**: Krótsza ewolucja prowadzi do mniejszej liczby bramek i mniejszej liczby błędów
...
W przeciwieństwie do algorytmów wariacyjnych, które optymalizują parametry obwodu przez wiele iteracji, bf-DCQO stosuje **podejście prowadzone przez pole bias**, które zbiega w około 10 iteracjach [1]:

**Proces iteracyjny:**

1. **Początkowa ewolucja kwantowa**: Rozpoczęcie od obwodu kwantowego implementującego protokół ewolucji kontradiabatycznej


2. **Pomiar**: Zmierzenie stanu kwantowego w celu uzyskania rozkładu prawdopodobieństwa nad ciągami bitów


3. **Obliczenie pola bias**: Analiza statystyk pomiarów i obliczenie optymalnego pola bias $h_i$ dla każdego kubitu:
...
2. **Brak potrzeby łagodzenia błędów**: Wbudowana wydajność algorytmu eliminuje konieczność stosowania kosztownych technik łagodzenia błędów [\[1\]](#references)

3. **Skalowalność**: Może obsługiwać problemy z maksymalnie 156 kubitami (156 zmiennych binarnych) przy bezpośrednim mapowaniu kubitów [\[1\]](#references)
...
Algorytm bf-DCQO automatycznie obsługuje optymalizację obwodów, tworząc płytkie obwody kwantowe z członami kontradiabatycznymi zaprojektowanymi specjalnie pod kątem docelowego backendu.
...
- **Wybór backendu**: Wybierz spośród dostępnych QPU (na przykład `"ibm_fez"`) na podstawie swoich potrzeb i instancji zasobów obliczeniowych.
...
| **direct_qubit_mapping** | `bool` | False | Mapuj wirtualne kubity bezpośrednio na fizyczne kubity |
...
| **transpilation_level** | `int` | 0 | Próby optymalizacji transpilera (0-5) — szczegóły poniżej |
...
1. Skonstruuje płytkie obwody kwantowe z członami kontradiabatycznymi
...
- Demonstruje to przewagę kwantową w przypadku problemu, z którym klasyczne solwery mają trudności [\[4\]](#references)
...
3. **Wykorzystaliśmy zaawansowane algorytmy kwantowe**: Użyliśmy algorytmu bf-DCQO firmy Kipu Quantum z członami kontradiabatycznymi [1]
...
- **Około 10 iteracji** zamiast około 100 dla metod wariacyjnych
- **Wbudowana odporność na błędy** dzięki wydajności obwodów

**Człony kontradiabatyczne**: Umożliwiają szybką ewolucję kwantową przy zachowaniu wierności stanu podstawowego, dzięki czemu optymalizacja kwantowa staje się praktyczna na dzisiejszym zaszumionym sprzęcie [\[1\]](#references).
