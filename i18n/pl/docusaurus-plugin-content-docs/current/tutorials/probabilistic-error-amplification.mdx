...
1. Wzmocnij szum Circuit dla kilku współczynników szumu $\lambda_1, \lambda_2, ... $
2. Uruchom każdy wzmocniony szumowo Circuit, aby zmierzyć $\langle A(\lambda_1)\rangle, ...$
...
### Tworzenie sparametryzowanego Circuit modelu Isinga {#create-a-parameterized-ising-model-circuit}
...
#### Funkcje pomocnicze do konstrukcji Circuit {#helper-functions-for-circuit-construction}
...
##### Usuwanie wadliwych Qubit {#remove-bad-qubits}
...
##### Główna generacja Circuit Trottera {#main-trotter-circuit-generation}
...
- Przetłumacz Gate w twoim Circuit na instrukcje [Architektury zestawu instrukcji (ISA)](/guides/transpile#instruction-set-architecture), które można bezpośrednio wykonać na sprzęcie.
...
Zauważ, że każde obserwable jest opakowane w listę w poniższym bloku kodu. Robi się to, aby _rozgłaszać_ (broadcast) z wartościami parametrów, tak aby każde obserwable Qubit było mierzone dla każdej wartości theta. Zasady rozgłaszania dla prymitywów można znaleźć [tutaj](/guides/primitives).
