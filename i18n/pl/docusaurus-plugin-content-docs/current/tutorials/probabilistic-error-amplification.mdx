---
description: 'Przeprowadź eksperyment mitygacji błędów w skali użytkowej z ekstrapolacją zerowego szumu i probabilistycznym wzmacnianiem błędów.'
notebook_path: tutorials/probabilistic-error-amplification.ipynb
sidebar_label: 'Mitygacja błędów w skali użytkowej z probabilistycznym wzmacnianiem błędów'
title: 'Mitygacja błędów w skali użytkowej z probabilistycznym wzmacnianiem błędów'
---
...
- [Lekcja dotycząca mitygacji błędów](/learning/courses/utility-scale-quantum-computing/error-mitigation) kursu *Obliczenia kwantowe w skali użytkowej* — podstawowa wiedza na temat stosowania mitygacji błędów w Qiskit.
- [Lekcja Utility-I](/learning/courses/utility-scale-quantum-computing/utility-i) kursu *Obliczenia kwantowe w skali użytkowej* — dodatkowe informacje na temat eksperymentu w skali użytkowej używanego jako przykład w tym samouczku.

## Tło {#background}

Ten samouczek pokazuje, jak uruchomić eksperyment mitygacji błędów w skali użytkowej za pomocą IBM Quantum Compute Service, używając eksperymentalnej wersji *ekstrapolacji zerowego szumu* (ZNE, ang. *zero noise extrapolation*) z *probabilistycznym wzmacnianiem błędów* (PEA, ang. *probabilistic error amplification*).
...
### Ekstrapolacja zerowego szumu (ZNE) {#zero-noise-extrapolation-zne}
...
$$
gdzie $\lambda$ parametryzuje siłę szumu i może być wzmacniana.
ZNE można zaimplementować w następujących krokach:

gdzie $\lambda$ parametryzuje siłę szumu i może być amplifikowane.

Możemy zaimplementować ZNE za pomocą następujących kroków:
...
#### Wzmacnianie szumu dla ZNE {#amplify-noise-for-zne}
...
* Rozciąganie impulsów (pulse stretching) zakłada, że szum bramki jest proporcjonalny do czasu trwania, co zazwyczaj nie jest prawdą. Kalibracja jest również kosztowna.

* Zwijanie bramek (gate folding) wymaga dużych współczynników rozciągnięcia, które znacznie ograniczają głębokość obwodów, które można uruchomić.

* PEA może być zastosowane do dowolnego obwodu, który można uruchomić z natywnym współczynnikiem szumu ($\lambda=1$), ale wymaga uczenia modelu szumu.


### Uczenie modelu szumu dla PEA {#learn-the-noise-model-for-pea}
PEA zakłada ten sam model szumu oparty na warstwach co *probabilistyczne anulowanie błędów* (PEC); jednak unika narzutu próbkowania, który skaluje się wykładniczo z szumem obwodu.
...
__Źródło__: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, _Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors_ [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)
...
## Przykład sprzętowy w skali użytkowej {#large-scale-hardware-example}

### Krok 1: Mapowanie klasycznych danych wejściowych na problem kwantowy {#step-1-map-classical-inputs-to-a-quantum-problem}

#### Tworzenie sparametryzowanego obwodu modelu Isinga {#create-a-parameterized-ising-model-circuit}

##### Ustal backend {#establish-a-backend}
...
##### Definiowanie połączeń warstw splątujących {#define-entangling-layer-couplings}
...
##### Usuwanie wadliwych kubitów {#remove-bad-qubits}
...
##### Główna generacja obwodu Trottera {#main-trotter-circuit-generation}
...
#### Tworzenie listy wartości parametrów do późniejszego przypisania {#create-a-list-of-parameter-values-to-be-assigned-later}
...
### Krok 2: Optymalizacja problemu dla wykonania na sprzęcie kwantowym {#step-2-optimize-problem-for-quantum-hardware-execution}
...
* Wstaw bramki swap w razie potrzeby, aby trasować interakcje między kubitami, które nie są połączone.

* Przetłumacz bramki w naszym obwodzie na instrukcje [Architektury Zestawu Instrukcji (ISA)](/guides/transpile#instruction-set-architecture), które mogą być bezpośrednio wykonane na sprzęcie.
...
Zauważ, że każde obserwable jest opakowane w listę w poniższym bloku kodu. Robi się to, aby _rozgłaszać_ (broadcast) z wartościami parametrów, tak aby każde obserwable kubitu było mierzone dla każdej wartości theta. Zasady rozgłaszania dla prymitywów można znaleźć [tutaj](/guides/primitives).
...
#### Konfiguracja opcji Estimatora {#configure-estimator-options}
...
##### Wyjaśnienie opcji ZNE {#explanation-of-zne-options}
...
* **amplifier**: Metoda używana do amplifikacji szumu do zamierzonych współczynników szumu.
  Dozwolone wartości to `"gate_folding"`, który amplifikuje przez powtarzanie dwukubitowych bramek bazowych,
  oraz `"pea"`, który amplifikuje przez próbkowanie probabilistyczne po wyuczeniu twirlingowanego przez Pauliego
  modelu szumu dla warstw twirlingowanych dwukubitowych bramek bazowych. Dodatkowe opcje to `"gate_folding_front"` i `"gate_folding_back"`, które są wyjaśnione w [dokumentacji API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier).

* **extrapolated\_noise\_factors**: Określ jedną lub więcej wartości współczynnika szumu, przy których ma być oceniany
  wyekstrapolowany model. W przypadku sekwencji wartości zwrócone wyniki będą tablicowe, z podanym współczynnikiem szumu ocenianym dla modelu ekstrapolacji. Wartość
  0 odpowiada ekstrapolacji do zerowego szumu.


#### Uruchomienie eksperymentu {#run-the-experiment}
...
### Krok 4: Post-przetwarzanie i zwracanie wyniku w pożądanym formacie klasycznym {#step-4-post-process-and-return-result-in-desired-classical-format}
...
#### Ogólne kształty wyników i metadane {#general-result-shapes-and-metadata}
...
1. `pub_result.data.evs`: Wartości oczekiwane odpowiadające zerowemu szumowi (oparte na heurystycznie najlepszej ekstrapolacji).
   ...
   * Pierwsza oś to indeks kubitu wirtualnego dla obserwabla $\langle Z_i\rangle$ ($124$ kubity wirtualne/obserwable)

   * Druga oś indeksuje wartość parametru $\theta$ ($12$ wartości parametru)
...
* `resilience/zne/extrapolator`: Ekstrapolatory użyte dla każdego wyniku
...
### Wykres wyników symulacji Trottera {#plot-trotter-simulation-results}
...
### Wykres wyników ekstrapolacji dla poszczególnych qubitów {#plot-extrapolation-results-for-individual-qubits}
...
- [Samouczek](/tutorials/combine-error-mitigation-techniques) skupiony na łączeniu technik mitygacji błędów.
- Szczegółowa [dokumentacja](/guides/error-mitigation-and-suppression-techniques) dotycząca technik mitygacji błędów dostępnych w Qiskit.
- Dodatkowe lekcje obejmujące eksperymenty w skali użytkowej: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) i [Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
