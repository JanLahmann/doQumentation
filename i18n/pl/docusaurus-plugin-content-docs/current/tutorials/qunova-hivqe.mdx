---
title: "Krzywe PES dysocjacji z Qunova HiVQE"
sidebar_label: "Krzywe PES dysocjacji z Qunova HiVQE"
description: "Użyj Qunova HiVQE do obliczenia krzywych potencjalnej powierzchni energii (PES) dysocjacji."
...
Funkcje Qiskit to funkcja eksperymentalna dostępna wyłącznie dla użytkowników planów IBM Quantum&reg; Premium Plan, Flex Plan oraz On-Prem (przez IBM Quantum Platform API). Są one w fazie podglądu i mogą ulec zmianie.
</Admonition>

*Szacunkowe zużycie zasobów (UWAGA: to tylko szacunek. Rzeczywisty czas wykonania może się różnić.)*

* Li2S: Pięć minut czasu QPU na procesorze Heron r2
* FeP-NO: Pięć minut czasu QPU na procesorze Heron r2
## Wprowadzenie {#background}

Dokładne obliczanie energii reakcji chemicznych ma kluczowe znaczenie dla postępu naukowego w dziedzinie materiałoznawstwa, inżynierii chemicznej, odkrywania leków i innych dziedzin. Wśród różnych układów chemicznych szczególnym zainteresowaniem cieszy się układ Li-S, istotny dla zrozumienia i opracowania nowych składów baterii. Ten samouczek umożliwia zdobycie praktycznego doświadczenia w obliczaniu potencjalnej powierzchni energii (PES) dysocjacji wiązania Li-S w układzie $Li_2S$ poprzez usunięcie jednego atomu litu przy użyciu obliczeń HiVQE. Wyniki można porównać z obliczeniami referencyjnymi (CASCI) oraz klasycznymi metodami, takimi jak Hartree-Fock (HF), dla problemu 20-qubitowego.
## Wymagania {#requirements}

Zainstaluj następujące zależności, aby uruchomić kod z tego samouczka.
...
Aby uruchomić ten samouczek, zaimportuj funkcję `qunova/hivqe-chemistry` przez `QiskitFunctionCatalog`. Do uruchomienia tej funkcji potrzebujesz konta IBM Quantum Premium Plan, Flex Plan lub On-Prem (IBM Quantum Platform API) Plan oraz licencji od Qunova.
...
## Część 1: Li2S (20Q) {#part-1-li2s-20q}

### Krok 1: Odwzorowanie klasycznych danych wejściowych na problem kwantowy {#step-1-map-classical-inputs-to-a-quantum-problem}

Zdefiniuj geometrie $Li_2S$ w formacie słownikowym dla różnych odległości wiązania Li-S, aby obliczyć krzywą PES. Geometrie te są zoptymalizowane przy użyciu obliczeń B3LYP/631g.
...
Obliczenia HiVQE zostaną wykonane z opcjami zdefiniowanymi poniżej. Używając bazy sto3g dla $Li_2S$, mamy 19 orbitali przestrzennych z 22 elektronami. Aby uruchomić przypadek (10o,10e) z obliczeniami HiVQE, możesz zdefiniować 10 aktywnych orbitali i sześć zamrożonych orbitali. W każdej iteracji 100 strzałów zostanie użytych do próbkowania konfiguracji elektronowej generowanej przez obwód kwantowy ExcitationPreserving (`epa`) ze splątaniem `circular` i dwoma powtórzeniami (`reps`). Maksymalna liczba iteracji wynosi 30, aby zapewnić zakończenie iteracji po osiągnięciu zbieżności energii.
...
### Kroki 2 i 3: Optymalizacja problemu do wykonania na sprzęcie kwantowym i wykonanie za pomocą funkcji HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}

Skonfiguruj pętlę `for`, aby uruchomić obliczenia HiVQE z geometriami przy użyciu opcji zdefiniowanych poniżej. Zadania są przesyłane w pętli `for`. W tym samouczku prześlesz sześć geometrii i pobierzesz wyniki po ich wszystkich ukończeniu. W głównym uruchomieniu funkcji musisz zdefiniować `max_states` i `max_expansion_states`, aby kontrolować maksymalny rozmiar macierzy podprzestrzeni oraz liczbę stanów, które mogą być generowane przy użyciu klasycznych metod rozszerzania CI na iterację. Identyfikatory zadań funkcji zostaną zapisane w słowniku z etykietą każdej geometrii, aby umożliwić dalsze śledzenie i przetwarzanie wyników.
...
Sprawdźmy, czy wszystkie zadania nadal się wykonują, czy zostały ukończone.
...
Po ukończeniu wszystkich zadań pobierzmy wszystkie wyniki obliczeń.
...
Całkowity czas QPU wykorzystany w zadaniu można śledzić, logując się do [IBM Quantum Platform](https://quantum.cloud.ibm.com/) i przeglądając przesłane zadania z tagiem `qunova-chemistry-hivqe`.
### Krok 4: Przetwarzanie końcowe i porównanie z metodami klasycznymi {#step-4-post-process-and-compare-with-classical-methods}

Klasyczne obliczenia referencyjne (CASCI) można przeprowadzić dla przypadku (10o,10e), aby zweryfikować wyniki HiVQE.
...
### Wykres krzywej dysocjacji dla Li\_2S {#plotting-the-dissociation-curve-for-li_2s}

Narysujmy wykres i porównajmy wyniki HiVQE z HF i CASCI.
Możesz zaobserwować, że wszystkie obliczenia HiVQE dobrze zgadzają się z klasycznym wynikiem referencyjnym (CASCI).
...
## Część 2: FeP-NO (44Q) {#part-2-fep-no-44q}

Aby uruchomić funkcję na układzie FeP-NO z ustawieniami pokazanymi w tym przykładzie, potrzebujesz licencji zezwalającej na używanie funkcji z co najmniej 44 kubitami. Napisz na adres [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com), aby dowiedzieć się, jak uzyskać licencję.
...
Zdefiniuj opcje dla obliczeń HiVQE.
...
Zdefiniuj geometrie FeP-NO w formacie słownika dla różnych odległości wiązania Fe-N, aby obliczyć krzywą PES.
...
### Kroki 2 i 3: Optymalizacja problemu pod kątem wykonania na sprzęcie kwantowym i uruchomienie za pomocą funkcji HiVQE Chemistry {#kroki-2-i-3-optymalizacja-problemu-pod-katem-wykonania-na-sprzecie-kwantowym-i-uruchomienie-za-pomoca-funkcji-hivqe-chemistry}

Na podstawie konfiguracji HiVQE i geometrii uzyskaj wyniki sekwencyjnie.

#### Prześlij obliczenia dla d(Fe-N) = 1.75 $\AA$. {#przeslij-obliczenia-dla-d-fe-n-1-75}
...
Śledź zadanie i pobierz wynik dla obliczeń d(Fe-N) = 1.75 $\AA$.
...
#### Prześlij obliczenia dla d(Fe-N) = 2.00 $\AA$. {#przeslij-obliczenia-dla-d-fe-n-2-00}
...
Śledź zadanie i pobierz wynik dla obliczeń d(Fe-N) = 2.00 $\AA$.
...
#### Prześlij obliczenia dla d(Fe-N) = 5.00 $\AA$. {#przeslij-obliczenia-dla-d-fe-n-5-00}
...
Śledź zadanie i pobierz wynik dla obliczeń d(Fe-N) = 5.00 $\AA$.
...
### Krok 4: Przetwarzanie końcowe i porównanie z metodami klasycznymi {#krok-4-przetwarzanie-koncowe-i-porownanie-z-metodami-klasycznymi}

Klasyczne wyniki referencyjne (CASCI-DMRG, maxM=800) dla (22o,22e) są dostarczone w celu walidacji wyników HiVQE.
...
[Link do ankiety](https://your.feedback.ibm.com/jfe/form/SV_9AizRyKFHEGzVs2)

> **Uwaga:** Ta ankieta jest udostępniona przez **IBM Quantum** i dotyczy **treści samouczka** (napisanych przez IBM). doQumentation dostarcza stronę internetową, tłumaczenia i wykonywanie kodu — aby przekazać opinię na ich temat, [otwórz zgłoszenie na GitHubie](https://github.com/JanLahmann/doQumentation/issues).
