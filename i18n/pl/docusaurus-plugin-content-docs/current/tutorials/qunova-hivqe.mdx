...
Obliczenia HiVQE zostaną wykonane z opcjami zdefiniowanymi poniżej. Używając bazy sto3g dla $Li_2S$, mamy 19 orbitali przestrzennych z 22 elektronami. Aby uruchomić przypadek (10o,10e) z obliczeniami HiVQE, możesz zdefiniować 10 aktywnych orbitali i sześć zamrożonych orbitali. W każdej iteracji 100 strzałów zostanie użytych do próbkowania konfiguracji elektronowej generowanej przez obwód kwantowy ExcitationPreserving (`epa`) z splataniem `circular` i dwoma powtórzeniami (`reps`). Maksymalna liczba iteracji wynosi 30, aby zapewnić zakończenie iteracji po osiągnięciu zbieżności energii.
