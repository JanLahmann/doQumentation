...
Tutaj $c^\dagger_{\mathbf{j},\sigma}/c_{\mathbf{j},\sigma}$ są fermionowymi operatorami kreacji/anihilacji dla $\mathbf{j}^{\textrm{th}}$ węzła kąpielowego ze spinem $\sigma$, $\hat{d}^\dagger_{\sigma}/\hat{d}_{\sigma}$ są operatorami kreacji/anihilacji dla trybu domieszki, a $\hat{n}_{d\sigma} = \hat{d}^\dagger_{\sigma} \hat{d}_{\sigma}$. $t$, $U$ i $V$ są liczbami rzeczywistymi opisującymi oddziaływania przeskokowe, jednocentrowe i hybrydyzacyjne, a $\varepsilon$ jest liczbą rzeczywistą określającą potencjał chemiczny.
...
W ramach [transformacji Jordana-Wignera](https://en.wikipedia.org/wiki/Jordan%E2%80%93Wigner_transformation), ewolucja czasowa przez $H_2$ sprowadza się do pojedynczej bramki [CPhase](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.CPhaseGate) między orbitalami spinowymi w górę i w dół na miejscu domieszki. Ponieważ $H_1$ jest kwadratowym hamiltonianem fermionowym, ewolucja czasowa przez $H_1$ sprowadza się do obrotu orbitalnego.
...
Najpierw generujemy hamiltonian SIAM w bazie położeniowej. Hamiltonian jest reprezentowany przez macierz $h_{pq}$ oraz tensor $h_{pqrs}$. Następnie obracamy go do bazy pędowej. W bazie położeniowej umieszczamy domieszkę na pierwszym miejscu. Jednak gdy przechodzimy do bazy pędowej, przenosimy domieszkę na centralne miejsce, aby ułatwić oddziaływania z innymi orbitalami.
...
Następnie generujemy obwody do wytworzenia stanów bazowych Krylova.
Dla każdego gatunku spinowego stan początkowy $\ket{\psi_0}$ jest dany przez superpozycję wszystkich możliwych wzbudzeń trzech elektronów najbliższych poziomowi Fermiego do 4 najbliższych pustych modów zaczynając od stanu $|00\cdots 0011 \cdots 11\rangle$, i realizowany przez zastosowanie siedmiu bramek [XXPlusYYGate](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.XXPlusYYGate).
Stany ewoluowane w czasie są produkowane przez kolejne zastosowania kroku Trottera drugiego rzędu.

Aby uzyskać bardziej szczegółowy opis tego modelu i sposobu projektowania obwodów, zapoznaj się z ["Quantum-Centric Algorithm for Sample-Based Krylov Diagonalization"](https://arxiv.org/abs/2501.09702).
...
## Krok 2: Optymalizacja problemu pod kątem wykonania na sprzęcie kwantowym {#step-2-optimize-problem-for-quantum-execution}
Teraz, gdy mamy już utworzone obwody, możemy je zoptymalizować pod kątem docelowego sprzętu. Wybieramy najmniej obciążony QPU z co najmniej 127 qubitami. Więcej informacji znajdziesz w [dokumentacji Qiskit IBM&reg; Runtime](/guides/get-started-with-primitives#get-started-with-sampler).
...
## Krok 3: Wykonanie przy użyciu prymitywów Qiskit {#step-3-execute-by-using-qiskit-primitives}
...
## Krok 4: Przetwarzanie końcowe i zwrócenie wyniku w pożądanym formacie klasycznym {#step-4-post-process-and-return-result-to-desired-classical-format}
Teraz uruchamiamy algorytm SQD, używając funkcji `diagonalize_fermionic_hamiltonian`. Wyjaśnienia argumentów tej funkcji znajdziesz w [dokumentacji API](https://qiskit.github.io/qiskit-addon-sqd/apidocs/qiskit_addon_sqd.fermion.html#qiskit_addon_sqd.fermion.diagonalize_fermionic_hamiltonian).
...
### Weryfikacja energii {#verifying-the-energy}
