---
description: 'Ten samouczek pokazuje symulację 2D poprzeczno-polowego modelu Isinga z użyciem mitygacji błędów QESEM w połączeniu z modułem propagacji wstecznej operatorów Qiskit.'
...
Zamiast symulować dynamikę ciągłą w czasie, przyjmujemy ściśle powiązany model Isinga z kopnięciami. Dynamika może być wyrażona dokładnie jako periodyczny obwód kwantowy, w którym każdy krok ewolucji składa się z trzech warstw ułamkowych bramek dwukubitowych $ R_{ZZ} (\alpha_{ZZ}) $, przeplatanych warstwami bramek jednokubitowych $ R_X (\alpha_X) $ i $ R_Z (\alpha_Z) $.

Użyjemy ogólnych kątów, które są trudne zarówno dla klasycznej symulacji, jak i mitygacji błędów. Konkretnie, wybraliśmy $ \alpha_{ZZ} = 1.0 $, $ \alpha_X = 0.53 $ i $ \alpha_Z = 0.1 $, umieszczając model daleko od jakiegokolwiek punktu całkowalności.

W tym samouczku wykonamy następujące kroki:
* Oszacujemy oczekiwany czas działania QPU dla pełnej mitygacji błędów przy użyciu analitycznych i empirycznych funkcji szacowania czasu QESEM.
* Zbudujemy i zasymulujemy obwód modelu 2D Isinga z nachylonym polem, używając układów kubitów inspirowanych sprzętem i warstw bramek.
* Zwizualizujemy łączność kubitów urządzenia oraz wybrane podgrafy dla naszego eksperymentu.
* Zademonstrujemy użycie [propagacji wstecznej operatorów (OBP)](https://qiskit.github.io/qiskit-addon-obp/) w celu zmniejszenia głębokości obwodu. Technika ta usuwa operacje z końca obwodu kosztem większej liczby pomiarów operatorów.
* Wykonamy nieobciążoną mitygację błędów (EM) dla wielu obserwowalnych jednocześnie przy użyciu QESEM, porównując wyniki idealne, zaszumione i po mitygacji.
* Przeanalizujemy i wykreślimy wpływ mitygacji błędów na namagnesowanie dla różnych głębokości obwodu.
...
QESEM to niezawodne, wysokoprecyzyjne oprogramowanie oparte na charakteryzacji, implementujące wydajną, nieobciążoną kwazi-probabilistyczną mitygację błędów.
Jest zaprojektowane do mitygacji błędów w ogólnych obwodach kwantowych i jest niezależne od zastosowania. Zostało zwalidowane na różnych platformach sprzętowych, w tym w eksperymentach w skali użytkowej na urządzeniach IBM&reg; Eagle i Heron. Etapy przepływu pracy QESEM są następujące:

1. Charakteryzacja urządzenia — mapuje wierności bramek i identyfikuje błędy koherentne, dostarczając danych kalibracyjnych w czasie rzeczywistym. Ten etap zapewnia, że mitygacja korzysta z operacji o najwyższej dostępnej wierności.

2. Transpilacja uwzględniająca szum — generuje i ocenia alternatywne mapowania kubitów, zestawy operacji i bazy pomiarowe, wybierając wariant minimalizujący szacowany czas QPU, z opcjonalną równoległością w celu przyspieszenia zbierania danych.

3. Tłumienie błędów — redefiniuje natywne bramki, stosuje twirling Pauliego i optymalizuje sterowanie na poziomie impulsów (na obsługiwanych platformach) w celu poprawy wierności.

4. Charakteryzacja obwodu — buduje dostosowany lokalny model błędów i dopasowuje go do pomiarów QPU w celu kwantyfikacji resztkowego szumu.

5. Mitygacja błędów — konstruuje wielotypowe kwazi-probabilistyczne dekompozycje i próbkuje z nich w adaptacyjnym procesie minimalizującym czas QPU mitygacji i wrażliwość na fluktuacje sprzętowe, osiągając wysoką dokładność przy dużych objętościach obwodu.


Więcej informacji na temat QESEM i eksperymentu w skali użytkowej tego modelu na 103-kubitowym, wysokołącznościowym podgrafie natywnej geometrii heavy-hex `ibm_marrakesh` można znaleźć w artykule [Reliable high-accuracy error mitigation for utility-scale quantum circuits.](https://arxiv.org/abs/2508.10997)
...
## Krok 1: Mapowanie wejść klasycznych na problem kwantowy {#step-1-map-classical-inputs-to-a-quantum-problem}
Zaczynamy od zdefiniowania funkcji tworzącej obwód Trottera:
...
Zwróć uwagę, że w przypadku dużych obwodów (30 lub więcej kubitów) zalecamy używanie wstępnie obliczonych wartości z symulacji BP PEPS (ang. belief-propagation PEPS). Ten kod zawiera wstępnie obliczone wartości dla 35 kubitów jako przykład, oparty na podejściu BP do ewolucji sieci tensorowej PEPS wprowadzonym [w tym artykule](https://www.science.org/doi/10.1126/sciadv.adk4321) (który nazywamy PEPS-BP), przy użyciu pakietu Pythona do sieci tensorowych [quimb.](https://joss.theoj.org/papers/10.21105/joss.00819)
...
Używamy mapowania warstw $R_{ZZ}$ opartego na sprzęcie, zaczerpniętego z urządzenia Heron, z którego wycinamy warstwy zgodnie z liczbą kubitów, które chcemy symulować. Definiujemy podgrafy dla 10, 21, 28 i 35 kubitów, które zachowują strukturę 2D (możesz zmienić na swój ulubiony podgraf):
...
Teraz wizualizujemy układ kubitów na urządzeniu Heron dla wybranego podgrafu:
...
Zauważ, że łączność wybranego układu kubitów niekoniecznie jest liniowa i może obejmować duże obszary urządzenia Heron w zależności od wybranej liczby kubitów.

Teraz generujemy obwód Trottera i obserwowalną średniego namagnesowania dla wybranej liczby kubitów i parametrów:
...
Teraz użyjemy propagacji wstecznej operatora (OBP). (Szczegóły na temat dodatku Qiskit OBP znajdziesz w dokumentacji [OBP](https://qiskit.github.io/qiskit-addon-obp/).) Utworzymy funkcję generującą wycinki obwodu do propagacji wstecznej:
...
Widzimy, że OBP zmniejsza koszt czasowy mitygacji obwodu.

## Krok 3: Wykonanie przy użyciu prymitywów Qiskit {#step-3-execute-using-qiskit-primitives}

### Uruchomienie na prawdziwym backendzie {#run-with-real-backend}
...
Analizujemy cztery obwody z 5, 7 i 9 krokami Trottera przy precyzji 0,05, porównując ich idealne, zaszumione i poddane mitygacji błędów wartości oczekiwane:
...
Gdy wszystkie zadania zakończą działanie, możemy porównać ich zaszumione i zmitygowane wartości oczekiwane.
...
Na koniec możemy wykreślić namagnesowanie w zależności od liczby kroków. Podsumowuje to korzyści z używania funkcji Qiskit QESEM do bezstronnej mitygacji błędów na zaszumionych urządzeniach kwantowych.
...
Dziewiąty krok ma duży statystyczny pasek błędu, ponieważ ograniczyliśmy czas QPU do 5 minut. Jeśli uruchomisz ten krok przez 15 minut (zgodnie z sugestią empirycznego szacowania czasu), otrzymasz mniejszy pasek błędu. W rezultacie zmitygowana wartość będzie bliższa wartości idealnej.
</Admonition>
