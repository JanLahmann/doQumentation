...
* Podstawowa wiedza o [Qiskit SDK](https://docs.quantum.ibm.com/guides) i [Qiskit Runtime](/guides/compute-services#qiskit-runtime) oraz dostęp do [konta IBM Quantum&reg;](/guides/cloud-setup).
## Tło {#background}
...
Przed uruchomieniem na rzeczywistym QPU sprawdzamy, czy zarówno obwody dynamiczne, jak i unitarne dają idealny stan Bella na symulacie bez szumów. Używamy Qiskit Runtime `Sampler` z `AerSimulator` jako trybem backend, przy małej odległości 6.
...
Circuit w środku można zaimplementować w następujący sposób:
...
Teraz utwórz Circuit mierzące w bazach $XX$, $YY$ i $ZZ$, tak jak zrobiliśmy to dla dynamicznych Circuit powyżej.
...
Teraz, gdy zbudowaliśmy zarówno dynamiczne, jak i unitarne Circuit dla przykładu małej skali z `distance=6`, transpilujemy je, aby uruchomić najpierw na symulacie bez szumów.
...
W tej sekcji badamy, jak Circuit LRCX jest mapowany na sprzęt. Zaczynamy od wizualizacji fizycznych qubitów używanych w Circuit, a następnie analizujemy, jak odległość sterowanie–cel w układzie wpływa na liczbę operacji.
