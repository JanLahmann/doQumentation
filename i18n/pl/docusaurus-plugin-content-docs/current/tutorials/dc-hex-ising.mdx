...
Jesteśmy teraz gotowi do transpilacji obwodu na sprzęt. Przetranspilujemy zarówno standardową implementację unitarną, jak i implementację opartą na obwodach dynamicznych.
...
### Transpilacja dla obwodów dynamicznych {#transpilation-for-dynamic-circuits}

Najpierw transpilujemy obwody dynamiczne — z sekwencją DD i bez niej. Aby zapewnić spójność wyników, używamy tego samego zestawu fizycznych qubitów we wszystkich obwodach. W tym celu transpilujemy obwód jeden raz, a następnie używamy jego rozkładu dla wszystkich kolejnych obwodów, podając go przez parametr [`initial_layout`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.TranspileLayout#initial_layout) w menedżerze przebiegów. Następnie konstruujemy [primitive unified blocs](/guides/primitive-input-output) (PUBs) jako dane wejściowe dla prymitywu Sampler.
...
Poniżej możemy zwizualizować rozkład qubitów w przetranspilowanym obwodzie. Czarne kółka oznaczają qubity danych oraz qubity pomocnicze (ancilla) użyte w implementacji z obwodami dynamicznymi.
...
`MidCircuitMeasure` to rozszerzenie dostępnych operacji pomiarowych, skalibrowane specjalnie do wykonywania [pomiarów w trakcie obwodu](/guides/execute-dynamic-circuits#midcircuit). Instrukcja `MidCircuitMeasure` mapuje się na instrukcję `measure_2` obsługiwaną przez backendy. Należy zauważyć, że `measure_2` nie jest obsługiwane przez wszystkie backendy. Możesz użyć `service.backends(filters=lambda b: "measure_2" in b.supported_instructions)`, aby znaleźć backendy, które go obsługują. Poniżej pokazujemy, jak przetranspilować obwód tak, aby pomiary w trakcie jego trwania były wykonywane przy użyciu operacji `MidCircuitMeasure`, jeśli backend to obsługuje.
...
### Transpilacja dla obwodów unitarnych {#transpilation-for-unitary-circuits}

Aby zapewnić uczciwe porównanie między obwodami dynamicznymi a ich unitarnym odpowiednikiem, używamy tego samego zestawu fizycznych qubitów co w obwodach dynamicznych (dla qubitów danych) jako rozkładu przy transpilacji obwodów unitarnych.
...
Wizualizujemy rozkład qubitów przetranspilowanych obwodów unitarnych. Czarne kółka wskazują fizyczne qubity użyte do transpilacji obwodów unitarnych, a ich indeksy odpowiadają indeksom wirtualnych qubitów. Porównując to z rozkładem przedstawionym dla obwodów dynamicznych, możemy potwierdzić, że obwody unitarne używają tego samego zestawu fizycznych qubitów co qubity danych w obwodach dynamicznych.
...
Teraz dodajemy sekwencję DD do przetranspilowanych obwodów i konstruujemy odpowiadające im PUBs do przesłania zadania.
...
### Porównanie głębokości dwuqubitowej obwodów unitarnych i dynamicznych {#compare-two-qubit-gate-depth-of-unitary-and-dynamic-circuits}
...
Główną zaletą obwodu opartego na pomiarach jest to, że przy implementowaniu wielu interakcji ZZ warstwy CX mogą być zrównoleglone, a pomiary mogą odbywać się jednocześnie. Wynika to z faktu, że wszystkie interakcje ZZ komutują ze sobą, dzięki czemu obliczenia można wykonać przy głębokości pomiarowej równej 1. Po przetranspilowaniu obwodów obserwujemy, że podejście z dynamicznymi obwodami daje znacznie mniejszą głębokość dwuqubitową niż standardowe podejście unitarne, choć należy pamiętać, że dodatkowe pomiary w trakcie obwodu oraz klasyczne sprzężenie zwrotne same zajmują czas i wprowadzają własne źródła błędów.
...
Po zakończeniu zadań możemy pobrać czas trwania obwodu z metadanych wyników zadania i zwizualizować informacje o harmonogramie obwodu. Aby dowiedzieć się więcej o wizualizacji informacji o harmonogramowaniu obwodu, zajrzyj na [tę stronę](/guides/qiskit-runtime-circuit-timing).
...
Porównując wyniki eksperymentalne z symulacją, widzimy, że implementacja obwodu dynamicznego (linia przerywana z gwiazdkami) ogólnie osiąga lepsze wyniki niż standardowa implementacja unitarna (linia przerywana z kółkami). Podsumowując, przedstawiamy obwody dynamiczne jako rozwiązanie do symulacji modeli spinowych Isinga na siatce plastra miodu — topologii, która nie jest natywna dla sprzętu. Rozwiązanie z obwodami dynamicznymi umożliwia interakcje ZZ między qubitami, które nie są najbliższymi sąsiadami, przy krótszej głębokości bramek dwuqubitowych niż przy użyciu bramek SWAP, kosztem wprowadzenia dodatkowych qubitów pomocniczych i operacji klasycznego sprzężenia zwrotnego.
