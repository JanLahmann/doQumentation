...
Zoptymalizujemy szczegóły dotyczące uruchamianego układu, mierzonej obserwabli oraz parametrów uczenia szumów. Na początek upewniamy się, że instancjujemy backend z włączoną opcją bramek ułamkowych. Te bramki ułamkowe pozwolą na większą czułość w niektórych filtrach post-selekcji.
...
### Workflow for lightcone shading, noise learning, and anti-noise injection {#workflow-for-lightcone-shading-noise-learning-and-anti-noise-injection-1}
...
Funkcja `generate_noise_model_paulis` przechodzi przez każdą opakowaną warstwę dostarczonego obwodu i generuje wszystkie odpowiednie termy Pauliego wagowego jeden i dwa, uwzględniając łączność qubitów w obwodzie oraz wybierając termy istotne dla aktywnych węzłów i krawędzi. Termy te są następnie używane do obliczania granic szumu w przód i w tył.
...
### a. Uczenie wskaźników szumu {#a-learn-noise-rates}
...
### b.i. Aktualizacja scalonych granic przy użyciu rzeczywiście wyuczonych wskaźników szumu {#bi-update-merged-bounds-with-actual-learned-noise-rates}
...
#### b.ii. Obliczanie `local_scales` dla wykonania sprzętowego {#bii-compute-the-local-scales-for-the-hardware-execution}
...
#### Korzyści z klastrów w zakresie redukcji narzutu próbkowania dla danego czasu obliczeń klasycznych {#benefits-of-clusters-for-reducing-sampling-overhead-for-a-given-classical-compute-time}
...
### c. Wykonanie obwodu z antyszumem {#c-execute-the-circuit-of-interest-with-antinoise}

#### c.i. Przygotowanie szablonowego obwodu przy użyciu `samplex` {#ci-prepare-template-circuit-by-using-samplex}
