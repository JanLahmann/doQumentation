...
### Co to jest pochłanianie propagowanego szumu (PNA)? {#what-is-propagated-noise-absorption-pna}
...
![Zaszumiony eksperyment](/img/qiskit-addons/images/noisy_expt.png)
Jeśli nauczymy się modelu szumu, możemy zastosować jego odwrotność i anulować szum.
![Eksperyment z mitygacją szumu](/img/qiskit-addons/images/noise_mitigated_expt.png)
...
![Przegląd PNA](/img/qiskit-addons/images/pna_overview.png)

### Generowanie lustrzanego obwodu Trottera i obserwowalnego {#generate-the-mirrored-trotter-circuit-and-observable}
...
![Diagram obwodu kwantowego](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_1.png)
...
![Diagram obwodu kwantowego](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_2.png)
...
Tutaj upewniamy się, że pass manager adnotuje bloki za pomocą adnotacji ``Twirl`` i ``InjectNoise``, które pozwalają nam uczyć się szumu wpływającego na nasz obwód i powiązać ten szum z odpowiadającą mu warstwą układu.
...
Tutaj tworzymy odwzorowanie między identyfikatorami referencyjnymi ``InjectNoise`` każdego pola a wyuczonym modelem szumu (`PauliLindbladMap`) wpływającym na bramki splątujące w tym polu.
...
Podczas uruchamiania PNA zawsze będziesz musiał podać zaszumiony obwód i obserwablę. Jeśli twój zaszumiony obwód jest obwodem w polach z adnotacjami `InjectNoise`, musisz podać odwzorowanie, które utworzyliśmy w powyższym kroku. Można też przekazać obwód nieoparty na polach, zawierający instrukcje ``PauliLindbladError`` z ``qiskit-aer``. W takim przypadku nie trzeba podawać ``refs_to_noise_models``. Oprócz podstawowych danych wejściowych, użytkownicy będą chcieli rozważyć:
...
#### Próbkuj Circuit za pomocą prototypu prymitywu ``Executor`` {#sample-the-circuit-using-the-executor-primitive-prototype}
...
#### Przetwórz próbki, aby obliczyć wartość oczekiwaną z korekcją błędów {#post-process-the-samples-to-calculate-an-error-mitigated-expectation-value}
