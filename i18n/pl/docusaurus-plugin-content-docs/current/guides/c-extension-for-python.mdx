...
Wykonaj instrukcje w temacie [Instalacja](/guides/install-c-api), aby zainstalować następujące elementy:

- Standardowy zestaw narzędzi kompilatora C dla Twojej platformy

- Wersja Pythona, która zawiera swoje nagłówki C API. 


Powinieneś być również zaznajomiony lub być gotów wyszukać funkcje i obiekty dostępne
w [Qiskit C API](https://docs.quantum.ibm.com/api/qiskit-c) i powinieneś mieć pewną znajomość programowania w C.
...
Ustaw wersję runtime Qiskit w
`project.dependencies` tak, aby odpowiadała wersji podrzędnej używanej w czasie kompilacji.
