---
title: "Przykłady użycia prymitywów"
sidebar_label: "Przykłady użycia prymitywów"
description: "Praktyczne przykłady użycia prymitywów w Qiskit Runtime."
...
Dostępna jest już wersja beta nowego modelu wykonania. Kierowany model wykonania zapewnia większą elastyczność podczas dostosowywania przepływu pracy związanego z ograniczaniem błędów. Więcej informacji znajdziesz w przewodniku [Directed execution model](/guides/directed-execution-model).
</Admonition>
{/*
...
Przykłady w tej sekcji ilustrują typowe sposoby korzystania z prymitywów. Przed uruchomieniem tych przykładów postępuj zgodnie z instrukcjami zawartymi w [Instalacja i konfiguracja.](install-qiskit)

<Admonition type="note">
    Wszystkie poniższe przykłady korzystają z prymitywów z Qiskit Runtime, ale możesz zamiast nich użyć prymitywów bazowych.
</Admonition>

## Przykłady użycia Estimatora {#estimator-examples}

Efektywnie obliczaj i interpretuj wartości oczekiwane operatorów kwantowych wymaganych przez wiele algorytmów za pomocą Estimatora. Odkryj zastosowania w modelowaniu molekularnym, uczeniu maszynowym i złożonych problemach optymalizacji.

### Uruchamianie pojedynczego eksperymentu {#run-a-single-experiment}

Użyj Estimatora, aby wyznaczyć wartość oczekiwaną dla pojedynczej pary Circuit–operator.
...
### Uruchamianie wielu eksperymentów w ramach jednego zadania {#run-multiple-experiments-in-a-single-job}

Użyj Estimatora, aby wyznaczyć wartości oczekiwane dla wielu par Circuit–operator.
...
### Uruchamianie parametryzowanych Circuit {#run-parameterized-circuits}

Użyj Estimatora, aby uruchomić trzy eksperymenty w ramach jednego zadania, korzystając z wartości parametrów w celu zwiększenia możliwości ponownego użycia Circuit.
...
### Korzystanie z Session i zaawansowanych opcji {#use-sessions-and-advanced-options}

Odkryj możliwości Session i zaawansowanych opcji, aby zoptymalizować wydajność Circuit na QPU.

<Admonition type="caution">
Poniższy blok kodu zwróci błąd dla użytkowników planu Open Plan, ponieważ korzysta z Session. Obciążenia w planie Open Plan mogą być uruchamiane wyłącznie w [trybie zadania](/guides/execution-modes#job-mode) lub [trybie wsadowym](/guides/execution-modes#batch-mode).
</Admonition>
...
<span id="sampler-examples"></span>
## Przykłady Samplera {#sampler-examples}

Generuj kompletne rozkłady quasi-prawdopodobieństwa z mitygacją błędów, próbkowane z wyjść obwodów kwantowych. Wykorzystaj możliwości Samplera w algorytmach wyszukiwania i klasyfikacji, takich jak algorytm Grovera i QVSM.

### Uruchom pojedynczy eksperyment {#run-a-single-experiment}

Użyj Samplera, aby zwrócić wynik pomiaru jako ciągi bitów lub zliczenia dla pojedynczego Circuit.
...
### Uruchom wiele eksperymentów w jednym zadaniu {#run-multiple-experiments-in-a-single-job}

Użyj Samplera, aby zwrócić wyniki pomiarów jako ciągi bitów lub zliczenia dla wielu Circuit w jednym zadaniu.
...
### Uruchom sparametryzowane Circuit {#run-parameterized-circuits}

Uruchom kilka eksperymentów w jednym zadaniu, wykorzystując wartości parametrów do zwiększenia wielokrotnego użycia Circuit.
...
```
### Używaj sesji i zaawansowanych opcji {#use-sessions-and-advanced-options}

Poznaj sesje i zaawansowane opcje, aby zoptymalizować wydajność Circuit na QPU.

<Admonition type="caution">
Poniższy blok kodu zwróci błąd dla użytkowników planu Open Plan, ponieważ korzysta z sesji. Zadania w ramach planu Open Plan mogą być uruchamiane wyłącznie w [trybie zadania](/guides/execution-modes#job-mode) lub [trybie wsadowym](/guides/execution-modes#batch-mode).
</Admonition>
...
<Admonition type="tip" title="Zalecenia">
    - [Określ zaawansowane opcje środowiska uruchomieniowego.](runtime-options-overview)
    - Ćwicz z prymitywami, przechodząc przez [lekcję funkcji kosztu](/learning/courses/variational-algorithm-design/cost-functions) w IBM Quantum Learning.
    ...
    - Zapoznaj się z [limitami zadań](/guides/job-limits) podczas wysyłania zadania do QPU IBM&reg;.
</Admonition>
