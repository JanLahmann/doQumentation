...
Pakiet [Qiskit addon utilities](https://qiskit.github.io/qiskit-addon-utils/) dostarcza zestaw przejść transpilatora i funkcję postelekcji do zastosowania maski. Ta strona zawiera wskazówki dotyczące włączania postelekcji do obciążeń roboczych kwantowych, używając czterokubotowego stanu GHZ jako przykładu.
...
Te klucze odpowiadają aktywnym kubitom i kubytom obserwatora przed instrukcjami `rx` (`meas` i `spec`) oraz po instrukcjach `rx` (`meas_ps` i `spec_ps`). Każdy z nich jest tablicą tablic opartą na liczbie pomiarów i kubitów. W tym przypadku kształt to (1000, 4).
