...
<Accordion>
  <AccordionItem title="Tło: reprezentacja DAG">

Zanim zbudujesz przebieg, ważne jest, aby wprowadzić wewnętrzną reprezentację obwodów kwantowych w Qiskit, czyli [skierowany graf acykliczny (DAG)](../api/qiskit/qiskit.dagcircuit.DAGCircuit) (zobacz [ten samouczek](https://qiskit.org/ecosystem/rustworkx/tutorial/dags.html), aby uzyskać przegląd). Aby przejść przez te kroki, zainstaluj [bibliotekę `graphviz`](https://graphviz.org/download/) dla funkcji rysowania DAG.

W Qiskit, w ramach etapów transpilacji, obwody są reprezentowane za pomocą DAG. Ogólnie DAG składa się z *wierzchołków* (znanych też jako „węzły”) oraz skierowanych *krawędzi*, które łączą pary wierzchołków w określonej orientacji. Ta reprezentacja jest przechowywana za pomocą obiektów `qiskit.dagcircuit.DAGCircuit`, które składają się z pojedynczych obiektów `DagNode`. Zaletą tej reprezentacji w porównaniu z czystą listą bramek (czyli *netlistą*) jest to, że przepływ informacji między operacjami jest jawny, co ułatwia podejmowanie decyzji o transformacjach.
...
Użyj funkcji `qiskit.tools.visualization.dag_drawer()`, aby zobaczyć DAG tego obwodu. Istnieją trzy rodzaje węzłów grafu: węzły kubitów/bitów klasycznych (zielone), węzły operacji (niebieskie) i węzły wyjściowe (czerwone). Każda krawędź wskazuje przepływ danych (lub zależność) między dwoma węzłami.
