...
## Rysowanie obwodu kwantowego {#draw-a-quantum-circuit}
...
### Alternatywne renderery {#alternative-renderers}
...
### Dostosowania specyficzne dla renderera {#renderer-specific-customizations}

Niektóre dostępne opcje dostosowywania są specyficzne dla renderera.

Argument `fold` ustawia maksymalną szerokość wyjścia. W renderer'ze `text` ustawia to długość linii diagramu, zanim zostanie on zawinięty do następnej linii. W przypadku renderer'a 'mpl' jest to liczba (widocznych) warstw przed zawinięciem do następnej linii.
