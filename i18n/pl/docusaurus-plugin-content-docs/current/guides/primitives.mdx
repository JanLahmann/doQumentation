...
Wraz z coraz szerszym dostępem do QPU i rozwojem coraz większej liczby algorytmów kwantowych,
ponownie pojawiła się potrzeba abstrakcji wyższego poziomu. W odpowiedzi Qiskit wprowadził
interfejs primitives, zoptymalizowany pod kątem dwóch podstawowych zadań w rozwoju algorytmów kwantowych:
szacowania wartości oczekiwanej (`Estimator`) i próbkowania obwodów (`Sampler`). Celem jest
ponownie pomóc programistom skupić się bardziej na innowacji, a mniej na konwersji danych. Interfejs primitives zastępuje interfejs `backend.run`, ponieważ `Sampler` zapewnia ten sam bezpośredni dostęp do sprzętu, który oferował `backend.run`.
...
Na przykład zapoznaj się z tematami [Opcje Estimator](/guides/estimator-options) i [Opcje Sampler](/guides/sampler-options), aby poznać opcje prymitywów IBM Quantum, lub zajrzyj do [dokumentacji API Qiskit Aer](https://qiskit.github.io/qiskit-aer/apidocs/aer_primitives.html), aby sprawdzić opcje prymitywów Qiskit Aer.
...
dostęp do zaawansowanych możliwości sprzętowych danego dostawcy. Na przykład, korzystając z prymitywów IBM Quantum,
...
- Dowiedz się więcej o [prymitywach IBM Quantum](/guides/qiskit-runtime-primitives), które służą do uruchamiania Circuit na QPU IBM.
</Admonition>
