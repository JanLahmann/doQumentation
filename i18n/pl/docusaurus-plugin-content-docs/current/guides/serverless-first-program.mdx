...
Poniższy kod zakłada, że już wykonałeś procedurę zapisywania poświadczeń przy użyciu `QiskitRuntimeService.save_account` i załaduje Twoje domyślne zapisane konto, chyba że określisz inaczej. Więcej informacji znajdziesz w artykułach [Zapisz swoje dane logowania](/guides/save-credentials) i [Inicjalizacja konta IBM Quantum Compute Service](/guides/initialize-account).
