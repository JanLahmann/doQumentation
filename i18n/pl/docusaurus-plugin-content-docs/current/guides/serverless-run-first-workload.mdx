---
description: 'Jak wyświetlać listę programów dostępnych w Serverless, przekazywać dane wejściowe do tych programów, uruchamiać je zdalnie, sprawdzać status oraz pobierać wyniki i logi.'
notebook_path: guides/serverless-run-first-workload.ipynb
sidebar_label: 'Zdalne uruchamianie pierwszego zadania Qiskit Serverless'
title: 'Zdalne uruchamianie pierwszego zadania Qiskit Serverless'
---

{/* doqumentation-source-hash: 48f571ce */}


<OpenInLabBanner notebookPath="guides/serverless-run-first-workload.ipynb" />



<Accordion>
<AccordionItem title="Wersje pakietów">

Kod na tej stronie został opracowany przy użyciu poniższych wymagań.
Zalecamy używanie tych wersji lub nowszych.

```
qiskit[all]~=1.4.0
qiskit-ibm-runtime~=0.36.1
qiskit-ibm-catalog~=0.4
```
</AccordionItem>
</Accordion>
<Admonition type="tip">
**Qiskit Serverless jest w trakcie aktualizacji i jego funkcje szybko się zmieniają.** W tej fazie rozwoju notatki o wydaniach oraz najnowsza dokumentacja dostępne są na stronie [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html).
</Admonition>

Ta sekcja przedstawia, jak używać `qiskit-ibm-catalog` do wyświetlania listy programów dostępnych w Qiskit Serverless, przekazywania do nich danych wejściowych, zdalnego uruchamiania, sprawdzania statusu oraz pobierania wyników i logów.

Upewnij się, że wykonałeś kroki z sekcji [Napisz swój pierwszy program Qiskit Serverless](/guides/serverless-first-program) przed rozpoczęciem.

## Wyświetlanie listy dostępnych programów {#list-programs-available}

Możesz użyć `QiskitServerless.list()`, aby pobrać listę dostępnych programów do uruchomienia z Qiskit Serverless. Obejmuje to wcześniej przesłany [`transpile_remote_serverless`](/guides/serverless-first-program).

```python
# Added by doQumentation — required packages for this notebook
!pip install -q qiskit qiskit-ibm-catalog qiskit-ibm-runtime
```

```python
from qiskit_ibm_catalog import QiskitServerless

serverless = QiskitServerless()
next(
    program
    for program in serverless.list()
    if program.title == "transpile_remote_serverless"
)
```

```text
QiskitFunction(transpile_remote_serverless)
```

## Uruchamianie przesłanego programu i przekazywanie danych wejściowych {#run-an-uploaded-program-and-pass-inputs}

Najpierw skonfiguruj swoje dane wejściowe. Twój program ma trzy dane wejściowe: `circuits`, `backend` i `optimization_level`. Możesz użyć `random_circuit`, aby utworzyć 30 losowych obwodów:

```python
from qiskit.circuit.random import random_circuit

qc_random = [(random_circuit(4, 4, measure=True, seed=i)) for i in range(10)]
qc_random[0].draw(output="mpl", idle_wires=False)
```

![Output of the previous code cell](/docs/images/guides/serverless-run-first-workload/extracted-outputs/46108a95-11f8-4ecf-83bb-131c3e7ea4c2-0.svg)

Następnie użyj `QiskitRuntimeService` i `least_busy`, aby wybrać `backend`:

```python
from qiskit_ibm_runtime import QiskitRuntimeService

service = QiskitRuntimeService()
backend = service.least_busy(operational=True, simulator=False)
print(backend.name)
```

Ustaw swój poziom optymalizacji:

```python
optimization_level = 3
```

Wybierz swój program za pomocą `serverless.load('PROGRAM_NAME')`:

```python
transpile_remote_serverless = serverless.load("transpile_remote_serverless")
```

Następnie przekaż swoje dane wejściowe i uruchom go w sposób pythonowy w następujący sposób:

```python
job = transpile_remote_serverless.run(
    circuits=qc_random,
    backend=backend.name,
    optimization_level=optimization_level,
)
```

```python
job.job_id
```

```text
'118256c5-bbb0-4ea8-9e9f-51aac2220aef'
```

<span id="serverless-job-status"></span>

## Sprawdzanie statusu zadania {#check-job-status}

Mając `job_id` Qiskit Serverless, możesz sprawdzić status uruchomionych zadań. Obejmuje to następujące statusy:

- **`QUEUED`**: Zdalny program jest w kolejce Qiskit Serverless. Priorytet kolejki jest obecnie oparty na tym, jak dużo używałeś Qiskit Serverless

- **`INITIALIZING`**: Zdalny program jest uruchamiany; obejmuje to konfigurację zdalnego środowiska i instalację zależności

- **`RUNNING`**: Program jest uruchomiony. Na tym etapie, jeśli masz wyjścia `print()` w swoim programie, możesz pobrać logi za pomocą `job.logs()`

- **`DONE`**: Program jest zakończony i możesz pobrać dane zapisane w `save_result()` za pomocą `job.results()`


Możesz również ustawić bardziej szczegółowe statusy zadań w sekcji [Zarządzanie zasobami obliczeniowymi i danymi Qiskit Serverless](/guides/serverless-manage-resources).

```python
job.status()
```

```text
'QUEUED'
```

```python
# This cell is hidden from users, it checks the job status
assert _ in ["QUEUED", "INITIALIZING", "RUNNING", "DONE"]  # noqa: F821
```

<Admonition type="tip">
Currently, the IBM Quantum workloads table only reflects IBM Quantum Compute Service workloads. Use `job.status()` to see your Qiskit Serverless workload's current status.
</Admonition>

Udało ci się uruchomić swój pierwszy program Qiskit Serverless!

## Pobieranie logów i wyników {#retrieve-logs-and-results}

Jak wspomniano wcześniej, gdy program ma status `RUNNING`, możesz użyć `job.logs()`, aby pobrać logi utworzone z wyjść `print()`:

```python
logs = job.logs()
print(logs)
```

```text
No logs yet.
```

W dowolnym momencie możesz również anulować zadanie:

```python
job.stop()
```

```text
'Job has been stopped.'
```

Gdy program ma status `DONE`, możesz użyć `job.results()`, aby pobrać wynik zapisany w `save_result()`:

```python
# We can't get results from a cancelled job, so we'll fetch a completed one instead
completed_job = next(
    job for job in serverless.jobs() if job.status() == "DONE"
)
completed_job.result()
```

```text
{'transpiled_circuits': [<qiskit.circuit.quantumcircuit.QuantumCircuit at 0x7f93eca64810>,
  <qiskit.circuit.quantumcircuit.QuantumCircuit at 0x7f93ec5e5310>,
  <qiskit.circuit.quantumcircuit.QuantumCircuit at 0x7f93ec5d5310>,
  <qiskit.circuit.quantumcircuit.QuantumCircuit at 0x7f93ec58b490>,
  <qiskit.circuit.quantumcircuit.QuantumCircuit at 0x7f93ec57d310>,
  <qiskit.circuit.quantumcircuit.QuantumCircuit at 0x7f93ec535950>,
  <qiskit.circuit.quantumcircuit.QuantumCircuit at 0x7f93ec523c90>,
  <qiskit.circuit.quantumcircuit.QuantumCircuit at 0x7f93ec60a990>,
  <qiskit.circuit.quantumcircuit.QuantumCircuit at 0x7f93ec5527d0>,
  <qiskit.circuit.quantumcircuit.QuantumCircuit at 0x7f93ec4152d0>]}
```

## Wyświetlanie listy wcześniej uruchomionych zadań z Qiskit Serverless {#list-jobs-previously-run-with-qiskit-serverless}

Możesz użyć `jobs()`, aby wyświetlić listę wszystkich zadań przesłanych do Qiskit Serverless:

```python
old_jobs = serverless.jobs()
old_jobs
```

```text
[<Job | 118256c5-bbb0-4ea8-9e9f-51aac2220aef>,
 <Job | e9a36469-7d6b-4f00-bf91-78709ebdbbff>,
 <Job | 4efd601b-8f61-4c8e-b14a-0b8a9c649dc0>,
 <Job | 87cd22c7-8eb9-4606-bdb4-befe946e9e9b>,
 <Job | be9a6dfd-0830-4250-aa60-acdd05bb8818>,
 <Job | 479513dd-6a76-4c3e-ba49-bb21351b9a05>,
 <Job | f9c20c31-be46-41b3-97ac-99f7be61f89e>,
 <Job | 37fa2489-4449-4bfb-974e-9d9a9ec3cc21>,
 <Job | b754c4e8-6817-48db-9bb9-74c151d6349a>,
 <Job | 78bc6744-b417-48cb-8e01-59bb63bcc2be>]
```

## Kolejne kroki {#next-steps}

<Admonition type="info" title="Zalecenia">

- Poznaj [narzędzia do zarządzania obliczeniami i danymi](/guides/serverless-manage-resources) dostępne dla twojego programu, w tym równoległość.


</Admonition>
