...
Prosty obwód kwantowy to zbiór kubitów oraz lista instrukcji działających na tych kubitach. Aby to zademonstrować, poniższa komórka tworzy nowy obwód z dwoma nowymi kubitami, a następnie wyświetla atrybut [`qubits`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.QuantumCircuit#qubits) obwodu, który jest listą [`Qubits`](https://docs.quantum.ibm.com/api/qiskit/circuit#qiskit.circuit.Qubit) w kolejności od najmniej znaczącego bitu $q_0$ do najbardziej znaczącego bitu $q_n$.
...
Dodanie instrukcji do obwód dołącza ją do atrybutu [`data`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.QuantumCircuit#data) tego obwód. Poniższe dane wyjściowe komórki pokazują, że `data` jest listą obiektów [`CircuitInstruction`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.CircuitInstruction), z których każdy ma atrybut `operation` oraz atrybut `qubits`.
...
Najprostszym sposobem wyświetlenia tych informacji jest metoda [`draw`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.QuantumCircuit#draw), która zwraca wizualizację Circuit. Zobacz stronę [Wizualizacja Circuit](/guides/visualize-circuits), aby poznać różne sposoby wyświetlania kwantowych Circuit.
...
Instrukcje i obwód są podobne w tym sensie, że oba opisują operacje na bitach i Qubitach, ale służą różnym celom:
...
Pomiary służą do próbkowania stanów poszczególnych Qubitów i przesyłania wyników do rejestru klasycznego. Pamiętaj, że jeśli przesyłasz Circuit do prymitywu [Sampler](/guides/get-started-with-sampler), pomiary są wymagane. Natomiast Circuit przesyłane do prymitywu [Estimator](/guides/get-started-with-estimator) nie mogą zawierać pomiarów.

Qubity można mierzyć trzema metodami: [`measure`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.QuantumCircuit#qiskit.circuit.QuantumCircuit.measure), [`measure_all`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.QuantumCircuit#measure_all) i [`measure_active`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.QuantumCircuit#measure_active). Aby dowiedzieć się, jak wizualizować wyniki pomiarów, zobacz stronę [Wizualizacja wyników](/guides/visualize-results).
...
Poniższa komórka tworzy wiele wariantów tego obwód i wyświetla jeden z nich.
