---
description: 'Omówienie technik łagodzenia i tłumienia błędów dostępnych w IBM Quantum Compute Service'
...
Poniższa komórka kodu pokazuje, jak włączyć dynamiczne odsprzęganie dla Estimator i wybrać sekwencję dynamicznego odsprzęgania.
...
![Ilustracja splatania Pauliego](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)

Ponieważ większość błędów w obecnym sprzęcie kwantowym pochodzi z dwuQubitowych Gate'ów, technika ta jest często stosowana wyłącznie do (natywnych) dwuQubitowych Gate'ów. Poniższy diagram przedstawia kilka splatań Pauliego dla Gate'ów CNOT i ECR. Każdy obwód w wierszu ma ten sam idealny efekt.
...
Poniższa komórka kodu pokazuje, jak włączyć splatanie Pauliego i ustawić te opcje dla Estimator. Żadna z tych opcji nie musi być ustawiana jawnie.
...
![Ilustracja splatania pomiarów](/docs/images/guides/error-mitigation-explanation/measurement_twirling.svg)

W obecności błędu odczytu, splatanie pomiarów ma efekt diagonalizacji macierzy transferu błędów odczytu, co ułatwia jej odwrócenie. Czynnik skalowania z diagonalnego kanału szumu jest uczony poprzez
benchmarkowanie losowych Circuit zainicjalizowanych w stanie zero. Pozwala to
usłudze usunąć obciążenie z wartości oczekiwanych wynikających z
szumu odczytu. Szacowanie macierzy transferu błędów odczytu wymaga wykonania dodatkowych Circuit kalibracyjnych, co wprowadza niewielki narzut.

### Włącz TREX za pomocą opcji `measure_mitigation` {#enable-trex-by-using-the-measure-mitigation-option}

TREX można włączyć, ustawiając `measure_mitigation` na `True` w [opcjach odporności IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) dla Estimator. Opcje dla uczenia szumu pomiarowego opisano [tutaj](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-measure-noise-learning-options). Podobnie jak w przypadku splatania Gate'ów, możesz ustawić liczbę randomizacji Circuit i liczbę ujęć na randomizację.

Poniższa komórka kodu pokazuje, jak włączyć TREX i ustawić te opcje dla Estimator. Żadna z tych opcji nie musi być ustawiana jawnie.
...
### Włącz TREX za pomocą opcji `resilience_level` {#enable-trex-by-using-the-resilience-level-option}

Możesz włączyć TREX, ustawiając [poziom odporności](/guides/estimator-noise-management#resilience) na `1`, jak pokazano w poniższym przykładzie.
...
Narzut tej metody skaluje się wraz z liczbą współczynników szumu. Domyślne
ustawienia pobierają wartość oczekiwaną przy trzech współczynnikach szumu,
co prowadzi do około 3-krotnego narzutu.

### Włącz ZNE za pomocą opcji `zne_mitigation` {#enable-zne-by-using-the-zne-mitigation-option}

ZNE można włączyć, ustawiając `zne_mitigation` na `True` w [opcjach odporności IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) dla Estimator.
Opcje dla ZNE opisano [tutaj](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options). Godne uwagi opcje to:
...
Poniższa komórka kodu pokazuje, jak włączyć ZNE i ustawić te opcje dla Estimator. Żadna z tych opcji nie musi być ustawiana jawnie.
...
### Włącz ZNE za pomocą opcji `resilience_level` {#enable-zne-by-using-the-resilience-level-option}

Możesz włączyć ZNE (wraz z TREX i splataniem Gate'ów), ustawiając [poziom odporności](/guides/estimator-noise-management#resilience) na `2`, jak pokazano w poniższym przykładzie.
...
PEC można włączyć, ustawiając `pec_mitigation` na `True` w [opcjach odporności IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) dla Estimator.
Opcje dla PEC opisano [tutaj](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-pec-options). Limit narzutu próbkowania można ustawić za pomocą opcji `max_overhead`. Pamiętaj, że ograniczenie narzutu próbkowania może spowodować, że precyzja wyniku przekroczy żądaną precyzję. Domyślna wartość `max_overhead` wynosi 100.

Poniższa komórka kodu pokazuje, jak włączyć PEC i ustawić opcję `max_overhead` dla Estimator.
...
- [Skonfiguruj zarządzanie szumem z Estimator](/guides/estimator-noise-management).

- [Skonfiguruj zarządzanie szumem z Sampler](/guides/sampler-noise-management).

- Poznaj inne [opcje](/guides/runtime-options-overview) prymitywów IBM Quantum.
