...
Następnie uwierzytelnij się przy użyciu swojego [klucza API IBM Quantum®](http://quantum.cloud.ibm.com/) i — zakładając, że masz już [zapisane konto](/guides/functions-get-started#install-qiskit-functions-catalog-client) w środowisku lokalnym — załaduj funkcję Qiskit w następujący sposób:
...
Sprawdź [status](/guides/functions-get-started#check-job-status) zadania Qiskit Function lub pobierz [wyniki](/guides/functions-get-started#retrieve-results) w następujący sposób:
...
Aby uzyskać dostęp do energii stanu podstawowego, użyj klucza "energy". Klucz "eigenvector" dostarcza współczynniki CI wraz z odpowiadającą im notacją ciągu bitowego konfiguracji elektronowej, przechowywaną w "states" wyników.
...
Napisz na adres [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) z zapytaniami dotyczącymi licencji.

### Przykład użycia funkcji licencjonowanej {#example-of-licensed-function-use}

Licencjonowanym użytkownikom przydzielany jest token licencyjny, który następnie muszą przesłać do funkcji za pomocą biblioteki opakowującej.
Bibliotekę opakowującą można zainstalować [z PyPI](https://pypi.org/project/hivqe-qiskit-function-utils/) za pomocą `pip install hivqe-qiskit-function-utils`.
Poniższy przykład pokazuje, jak należy używać tej biblioteki do przesłania tokena licencyjnego podczas wywoływania funkcji.
