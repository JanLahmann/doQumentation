---
description: 'Jak używać prymitywu Executor w qiskit-ibm-runtime.'
...
### 1. Inicjalizacja konta {#1-initialize-the-account}

Ponieważ IBM Quantum Compute Service jest usługą zarządzaną, musisz najpierw zainicjować swoje konto. Następnie możesz wybrać QPU, którego chcesz użyć do obliczenia wartości oczekiwanej.
...
### 2. Tworzenie i transpilacja obwodu {#2-create-and-transpile-a-circuit}
...
### 3. Inicjalizacja `QuantumProgram` {#3-initialize-a-quantumprogram}
...
### 4. Opcjonalnie: Grupowanie bramek i pomiarów w adnotowane bloki {#4-optional-group-gates-and-measurements-into-annotated-boxes}
...
### 5. Opcjonalnie: Tworzenie obwodu szablonowego i samplex oraz dodanie ich do programu {#5-optional-build-a-template-circuit-and-samplex-and-add-them-to-the-program}
...
### 6. Wywołanie Executor i uzyskanie wyników {#6-invoke-executor-and-get-results}
...
Wynik jest typu [`QuantumProgramResult`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/results-quantum-program-result). Zobacz [Dane wejściowe i wyjściowe Executor](/guides/executor-input-output), aby dowiedzieć się o obiekcie wynikowym.
