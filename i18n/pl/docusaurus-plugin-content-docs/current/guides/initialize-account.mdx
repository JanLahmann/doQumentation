---
description: 'Jak zainicjalizować konto IBM Quantum Compute Service, utworzyć instancję konta i wczytać zapisane dane uwierzytelniające.'
title: 'Inicjalizacja konta IBM Quantum Compute Service'
---

{/* doqumentation-source-hash: ab5dc076 */}

# Inicjalizacja konta IBM Quantum Compute Service {#initialize-your-ibm-quantum-compute-service-account}

Przed użyciem IBM Quantum&reg; Compute Service musisz zainicjalizować (utworzyć instancję) swoje konto, przesyłając (wczytując) dane uwierzytelniające. Możesz je podawać ręcznie za każdym razem, gdy inicjalizujesz IBM Quantum Compute Service, lub zapisać je do ponownego użycia.
...
1. Upewnij się, że pracujesz w aktywnym środowisku Python z [zainstalowanym Qiskit SDK i `qiskit-ibm-runtime`.](/guides/install-qiskit#local)
...
## Połącz Qiskit z instancją IBM Quantum Compute Service {#connect-qiskit-with-your-ibm-quantum-compute-service-instance}
...
Poniżej znajduje się podstawowy kod do połączenia Qiskit z instancją IBM Quantum Compute Service. Istnieje jednak kilka różnych sposobów dostosowania opcji `QiskitRuntimeService` w zależności od potrzeb. Opcje te opisano w kolejnych sekcjach.
...
Najszybszym sposobem uruchomienia instancji `QiskitRuntimeService` jest metoda bezpośredniej inicjalizacji: jawne podawanie tokenu API (klucza) i CRN (identyfikatora instancji) za każdym razem, gdy trzeba zainicjalizować IBM Quantum Compute Service. W razie potrzeby zapoznaj się z sekcją [Znajdź swoje dane uwierzytelniające](#access-credentials).
...
3. Jeśli twoje konto ma wiele instancji, które mogą uzyskać dostęp do żądanego QPU, system używa twoich preferencji planu do wyboru planu i instancji. W przypadku `qiskit-ibm-runtime` w wersji 0.42 i nowszych plany bezpłatne są domyślnie traktowane priorytetowo.
...
### Lokalne testowanie z `qiskit-ibm-runtime` {#local-testing-with-qiskit-ibm-runtime}
