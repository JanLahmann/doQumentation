...
[IBM Quantum&reg; Composer](https://quantum.cloud.ibm.com/composer) to graficzne narzędzie do programowania kwantowego, które umożliwia przeciąganie i upuszczanie operacji w celu budowania obwodów i uruchamiania ich na sprzęcie kwantowym.
...
4. **Pasek narzędzi** — Dostęp do często używanych narzędzi: cofania i ponawiania akcji, zmiany wyrównania bramek oraz przełączania trybu inspekcji. W trybie inspekcji widzisz krok po kroku widok stanów kubitu w trakcie ewolucji obliczeń obwodu. Aby dowiedzieć się więcej, zobacz [Sprawdzaj swój obwód krok po kroku](#4-inspect-your-circuit-step-by-step).
...
## Budowanie, edytowanie i inspekcja obwodu {#build-edit-and-inspect-quantum-circuits}

<Admonition type="warning" title="Pobierz pliki obwodów kwantowych przed wyjściem z Composera">
...
2. Otwórz IBM Quantum Composer, klikając link na stronie nawigacyjnej [Nauka](/learning). Obszar roboczy wyświetla pusty obwód bez nazwy. Możesz utworzyć nowy obwód lub wgrać plik .qasm, aby kontynuować pracę z obwodem, który już stworzyłeś.
...
### 2. Budowanie obwodów metodą przeciągnij i upuść {#2-build-your-circuit-with-drag-and-drop}
...
Teraz możesz przeciągać i upuszczać nową operację w dowolne miejsce obwodu. Pamiętaj, że operacja jest zapisana w tym obwód i nie pojawia się w katalogu operacji innych obwód.
...
Aby dodać lub usunąć rejestry kwantowe lub klasyczne, kliknij Edytuj → Zarządzaj rejestrami. Możesz zwiększyć lub zmniejszyć liczbę kubit lub bitów w obwodzie i zmienić nazwy rejestrów. Kliknij **OK**, aby zastosować zmiany. Możesz też po prostu kliknąć nazwę rejestru (np. `q[0]`) i użyć opcji w menu kontekstowym, aby szybko dodać lub usunąć rejestry lub kubit.
...
Modyfikator sterujący daje bramka, którego oryginalna operacja jest teraz uzależniona od stanu kubitu sterującego. Aby uzyskać więcej szczegółów, kliknij prawym przyciskiem myszy symbol modyfikatora sterującego w katalogu operacji, a następnie kliknij Info.
...
#### Wizualizacja dyskami fazy w całym obwodzie {#visualize-with-phase-disks-throughout-your-circuit}

Aby zwizualizować stan wszystkich kubit w dowolnym miejscu obwodu, przeciągnij ikonę dysku fazy z katalogu operacji i umieść ją w dowolnym miejscu obwodu. Zostanie dodana kolumna operacji bariery oraz kolumna dysków fazy (jedna operacja bariery i jeden dysk fazy na każdy kubit). Najedź kursorem na każdy dysk fazy, aby odczytać stan kubit w tym punkcie obwód. Pamiętaj, że dodanie dysków fazy nie zmienia obwód; są one jedynie narzędziem wizualizacji.

Dowiedz się więcej o wizualizacji dysku fazy [tutaj.](#phase-disk)

#### Eksportowanie obrazu obwodu {#export-a-circuit-image}
...
![obwód kwantowy](/docs/images/guides/composer/58f83286-9707-4e43-893e-2bd161c744cd.avif "obwód złożony z kolumny bramek Hadamarda tworzących równą superpozycję stanów bazy obliczeniowej, po której następuje dwuqubitowa bramka controlled-Z (CZ).")
...
![obwód kwantowy](/docs/images/guides/composer/58f83286-9707-4e43-893e-2bd161c744cd.avif)
