...
Zalecanym sposobem transpilacji obwodu jest utworzenie etapowego menedżera przejść (staged pass manager), a następnie wywołanie jego metody `run` z obwodem jako argumentem. Ta strona wyjaśnia, jak transpilować obwody kwantowe w ten sposób.
## Co to jest (etapowy) menedżer przejść? {#what-is-a-staged-pass-manager}

W kontekście Qiskit SDK transpilacja oznacza proces przekształcania wejściowego obwodu do postaci nadającej się do wykonania na urządzeniu kwantowym. Transpilacja odbywa się zazwyczaj w sekwencji kroków zwanych przejściami transpilatora (transpiler passes). Obwód jest przetwarzany przez każde przejście transpilatora kolejno, a wyjście jednego przejścia staje się wejściem następnego. Na przykład jedno przejście może przejrzeć obwód i scalić wszystkie kolejne sekwencje bramek jednokubitowych, a następne przejście może zsyntetyzować te bramki w zbiorze bazowym docelowego urządzenia. Przejścia transpilatora dostarczone z Qiskit znajdują się w module [qiskit.transpiler.passes](https://docs.quantum.ibm.com/api/qiskit/transpiler_passes).

Menedżer przejść (pass manager) to obiekt przechowujący listę przejść transpilatora i mogący je wykonywać na obwodach. Utwórz menedżer przejść, inicjując obiekt [`PassManager`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.PassManager) z listą przejść transpilatora. Aby uruchomić transpilację na obwodzie, wywołaj metodę [`run`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.PassManager#run) z obwodem jako argumentem.

Etapowy menedżer przejść (staged pass manager) to szczególny rodzaj menedżera przejść reprezentujący wyższy poziom abstrakcji niż zwykły menedżer przejść. Podczas gdy zwykły menedżer przejść składa się z kilku przejść transpilatora, etapowy menedżer przejść składa się z kilku *menedżerów przejść*. Jest to przydatna abstrakcja, ponieważ transpilacja odbywa się zazwyczaj w odrębnych etapach, opisanych w [Etapach transpilatora](transpiler-stages), przy czym każdy etap reprezentowany jest przez menedżer przejść. Etapowe menedżery przejść są reprezentowane przez klasę [`StagedPassManager`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.StagedPassManager). Pozostała część tej strony opisuje, jak tworzyć i dostosowywać (etapowe) menedżery przejść.
...
W tych przykładach używany jest fikcyjny Backend `FakeSherbrooke` z `qiskit_ibm_runtime`, ale możesz wypróbować go na dowolnym rzeczywistym lub fikcyjnym Backendzie zgodnym z Qiskit. Twoje wyniki mogą być inne.
</Admonition>
...
Aby transpilować obwód lub listę obwodów za pomocą menedżera przejść, przekaż obwód lub listę obwodów do metody `run`. Zróbmy to na dwukubitowym obwodzie składającym się z bramki Hadamarda, po której następują dwie sąsiednie bramki CX:
...
Aby zademonstrować ten menedżer przejść w działaniu, przetestuj go na dwukubitowym obwodzie składającym się z bramki Hadamarda, po której następują dwie sąsiednie bramki CX:
...
Aby uruchomić menedżer przejść na obwodzie, wywołaj metodę `run`.
...
- [Napisz własne przejście transpilatora](custom-transpiler-pass).
...
- Zapoznaj się z [dokumentacją API transpilatora](https://docs.quantum.ibm.com/api/qiskit/transpiler).
</Admonition>
