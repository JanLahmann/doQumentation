...
<Admonition type="tip">
**Qiskit Serverless jest w trakcie ulepszania, a jego funkcje szybko się zmieniają.** W tej fazie rozwoju znajdziesz notatki o wydaniach i najnowszą dokumentację na stronie [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html).
</Admonition>
...
Jeśli czytasz komórki kodu lokalnie w notatniku, zobaczysz [polecenie magiczne](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Wykonanie komórek z tym poleceniem magicznym zapisuje je na dysk zamiast je wykonywać.
</Admonition>
...
W przypadku zadań klasycznych, które można zrównoleglić, użyj dekoratora `@distribute_task`, aby zdefiniować wymagania obliczeniowe potrzebne do wykonania zadania. Zacznij od przypomnienia sobie przykładu `transpile_remote.py` z tematu [Napisz swój pierwszy program Qiskit Serverless](/guides/serverless-first-program) z następującym kodem.
...
Poniżej pokazano, jak przesyłać pliki. Upewnij się, że uwierzytelniłeś się w Qiskit Serverless za pomocą swojego konta IBM Quantum (instrukcje znajdziesz w sekcji [Wdrażanie na platformę Qiskit Serverless](/guides/serverless-first-program#upload-to-qiskit-serverless)).
...
- Zobacz pełny przykład, który [przenosi istniejący kod do Qiskit Serverless](/guides/serverless-port-code).
