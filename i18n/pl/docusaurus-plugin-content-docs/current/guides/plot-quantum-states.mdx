...
W wielu sytuacjach – takich jak nauka czy debugowanie – pomocne jest zwizualizowanie stanu komputera kwantowego. Zakładamy tutaj, że masz już określony stan uzyskany z symulacji lub tomografii stanów. Możliwe jest jedynie wizualizowanie stanów małych układów kwantowych.
...
<TabItem value="Bloch" label="Bloch">
  ...
  Wektor Blocha stanu kubitu to jego wartość oczekiwana w obserwablach Pauliego X, Y i Z odwzorowana na osie X, Y i Z trójwymiarowej przestrzeni. Ten wykres rzutuje wielokubitowe stany kwantowe na przestrzeń jednokubitową i wykreśla każdy kubit na sferze Blocha. Ta wizualizacja pokazuje jedynie wartości oczekiwane poszczególnych kubitów. Nie może pokazać korelacji między kubitami i dlatego nie opisuje w pełni splątanych stanów kwantowych.
