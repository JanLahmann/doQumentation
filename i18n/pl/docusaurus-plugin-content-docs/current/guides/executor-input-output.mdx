...
Jak wspomniano wcześniej, dane wejściowe do prymitywu Executor to [`QuantumProgram`](https://qiskit.github.io/qiskit-ibm-runtime/stubs/QuantumProgram.html#QuantumProgram), który jest iterowalnym zbiorem obiektów
[`QuantumProgramItem`](https://qiskit.github.io/qiskit-ibm-runtime/stubs/QuantumProgramItem.html). Obiekty te mogą być dwojakiego rodzaju:
...
Wyjście Executor to [`QuantumProgramResult`](https://qiskit.github.io/qiskit-ibm-runtime/stubs/QuantumProgramResult.html#QuantumProgramResult), który jest iterowalny. Zawiera jeden wpis na wejściowy `QuantumProgramItem` w tej samej kolejności co elementy wejściowe. Każdy z tych elementów wyjściowych jest słownikiem, gdzie klucze to ciągi znaków odpowiadające nazwom klasycznych rejestrów w obwodach wejściowych (między innymi), więc nie musisz już zapamiętywać tych nazw tak jak w przypadku wyjść Sampler. Wartości słownika są typu `np.ndarray`.
