---
description: 'ใช้ fractional gates ซึ่งเป็น parameterized gates ที่รันการหมุนมุมอิสระได้โดยตรง เพื่อลดความลึกและระยะเวลาของ quantum kernel circuits'
notebook_path: tutorials/fractional-gates.ipynb
sidebar_label: 'Quantum kernels ด้วย fractional gates'
title: 'Quantum kernels ด้วย fractional gates'
---
...
หลังจากศึกษาบทแนะนำนี้แล้ว คุณควรเข้าใจ:
- fractional gates คืออะไร และช่วยลดความลึกและระยะเวลาของ circuit บน IBM&reg; QPU ได้อย่างไร
- ข้อจำกัดที่เกี่ยวข้องกับการใช้ fractional gates (โดยเฉพาะช่วงมุม RZZ)
- วิธีสร้าง quantum kernel workflow ที่ใช้ fractional gates กับ IBM Quantum Compute Service
- วิธีเปรียบเทียบตัวชี้วัดการรันบนฮาร์ดแวร์ (depth, duration, จำนวน non-local gate, fidelity) ทั้งแบบใช้และไม่ใช้ fractional gates
- วิธีใช้เฉพาะ fractional RX gates โดยยังคงใช้ workflow มาตรฐานของ Qiskit patterns

## ข้อกำหนดเบื้องต้น {#prerequisites}
เราแนะนำให้คุณคุ้นเคยกับหัวข้อต่อไปนี้ก่อนศึกษาบทแนะนำนี้:
- workflow [Qiskit patterns](/guides/intro-to-patterns)
- คู่มือ [Fractional gates](/guides/fractional-gates)
- บทแนะนำ [Quantum kernel training](/tutorials/quantum-kernel-training) และบทเรียน [Quantum kernels](/learning/courses/quantum-machine-learning/quantum-kernel-methods) ของคอร์ส Quantum machine learning
...
### แนวทาง Workflow สำหรับข้อจำกัดมุม RZZ {#workflow-approaches-for-the-rzz-angle-constraint}
...
มีสองวิธีเพื่อให้แน่ใจว่าเงื่อนไขนี้เป็นไปตามที่กำหนด ดังที่เราจะกล่าวถึงด้านล่าง เราแนะนำวิธีที่สอง และในบทแนะนำนี้ เราจะสาธิตผ่านตัวอย่างที่ได้แรงบันดาลใจจากวิธี quantum kernel
เพื่อทำความเข้าใจให้ดีขึ้นว่า quantum kernel มีแนวโน้มจะเป็นประโยชน์ในกรณีใด เราแนะนำให้อ่าน [Liu, Arunachalam & Temme (2021)](https://www.nature.com/articles/s41567-021-01287-z)
...
```
เพื่อหลีกเลี่ยงข้อผิดพลาดนี้ ให้ใช้วิธีที่สองที่อธิบายด้านล่าง
...
โปรดสังเกตว่า workflow นี้มีต้นทุนการคำนวณสูงกว่าวิธีแรก เนื่องจากเกี่ยวข้องกับการกำหนดค่าพารามิเตอร์ให้กับ quantum circuit และการจัดเก็บ circuit ที่ผูกพารามิเตอร์แล้วไว้ในเครื่อง

:::caution


**โปรดทราบถึงปัญหาที่ทราบกันดีใน `qiskit-ibm-runtime` v0.47.0** ที่ RZZ gates ที่มีมุมไม่ถูกต้องอาจยังคงอยู่ใน circuit แม้หลังการ transpile แล้วในบางสถานการณ์

ดู [qiskit-ibm-runtime#2441](https://github.com/Qiskit/qiskit-ibm-runtime/issues/2441) เพื่อติดตามความคืบหน้าของปัญหานี้
เราแนะนำวิธีแก้ปัญหาชั่วคราวต่อไปนี้จนกว่าจะได้รับการแก้ไข
...
ในส่วนนี้ เราจะไปทีละขั้นตอนทั้งสี่ของ workflow [Qiskit patterns](/guides/intro-to-patterns) บน simulator โดยใช้ quantum kernel circuit เป็นตัวอย่างที่นำไปใช้จริง

### ขั้นตอนที่ 1: แมปข้อมูล Classical ไปยังปัญหา Quantum {#step-1-map-classical-inputs-to-a-quantum-problem}
...
### การเปรียบเทียบความลึก ระยะเวลา และ Fidelity {#comparison-of-depths-durations-and-fidelities}
...
ในส่วนนี้ เราจะเปรียบเทียบประสิทธิภาพของ quantum kernel workflow ทั้งแบบใช้และไม่ใช้ fractional gates บนฮาร์ดแวร์ควอนตัมที่มี qubit สูงสุดถึง 40 qubit

### ขั้นตอนที่ 1-4 รวมกัน {#step-1-4-combined}

workflow นี้เป็นไปตามโครงสร้างเดียวกันกับตัวอย่างขนาดเล็ก เราทำการ transpile Circuit ทั้งหมดทั้งแบบมีและไม่มี fractional gates เก็บ metrics แล้วส่ง Circuit ไปยังฮาร์ดแวร์ควอนตัมจริง
...
ในหัวข้อนี้ เราสาธิตวิธีการปรับ U gates ให้เหมาะสมโดยใช้ fractional RX gates โดยต่อยอดจาก Circuit เดียวกันที่แนะนำในหัวข้อก่อนหน้า
...
- คู่มือ [Fractional gates](/guides/fractional-gates)
- [เมื่อไร*ไม่ควร*ใช้ fractional gates](/guides/fractional-gates#when-not-to-use)
- เอกสารอ้างอิง API ของ transpiler pass [`FoldRzzAngle`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/transpiler-passes-fold-rzz-angle)
- บทเรียน [Quantum kernel training](/tutorials/quantum-kernel-training)
- บทเรียน [Quantum kernels](/learning/courses/quantum-machine-learning/quantum-kernel-methods) ในคอร์ส Quantum machine learning
</Admonition>
