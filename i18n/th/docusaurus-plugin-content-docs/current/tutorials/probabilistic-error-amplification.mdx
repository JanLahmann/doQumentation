...
##### กำหนด Backend {#establish-a-backend}
...
1. `pub_result.data.evs`: Expectation values ที่ตรงกับ zero noise (อิงจาก extrapolation ที่ดีที่สุดตาม heuristic)
   ...
   * แกนแรกคือดัชนี virtual qubit สำหรับ observable $\langle Z_i\rangle$ ($124$ virtual-qubits/observables)

   * แกนที่สองคือดัชนีค่าพารามิเตอร์สำหรับ $\theta$ ($12$ ค่าพารามิเตอร์)
...
2. `pub_result.data.evs_extrapolated`: Expectation values สำหรับ noise factors ที่ extrapolated สำหรับทุก extrapolator โดย array นี้มีสองแกนเพิ่มเติม
   ...
   * แกนที่สามคือดัชนีของวิธี extrapolation ($2$ extrapolator คือ `exponential` และ `linear`)

   * แกนสุดท้ายคือดัชนีของ `extrapolated_noise_factors` ($20$ จุด extrapolation ที่ระบุในตัวเลือก)

3. `pub_result.data.evs_noise_factors`: Expectation values แบบ raw สำหรับแต่ละ noise factor

   * แกนที่สามคือดัชนีของ `noise_factors` ดิบ ($3$ ปัจจัย)
...
หากคุณสนใจงานนี้ อาจสนใจเนื้อหาต่อไปนี้:
- [Tutorial](/tutorials/combine-error-mitigation-techniques) ที่เน้นการรวมเทคนิคการลดข้อผิดพลาด
- [เอกสารรายละเอียด](/guides/error-mitigation-and-suppression-techniques) เกี่ยวกับเทคนิคการลดข้อผิดพลาดที่มีใน Qiskit
- บทเรียนเพิ่มเติมที่ครอบคลุมการทดลองระดับ utility-scale: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) และ [Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii)
</Admonition>
