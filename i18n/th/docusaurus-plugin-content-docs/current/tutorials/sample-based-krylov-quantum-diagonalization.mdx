...
sidebar_label: 'การวิเคราะห์เมทริกซ์เชิงควอนตัมแบบ Krylov โดยใช้การสุ่มตัวอย่างสำหรับโมเดลแลตทิซเฟอร์มิออน'
...
- วิธีใช้ [SQD Qiskit addon](https://github.com/Qiskit/qiskit-addon-sqd) เพื่อประมาณ ground state energy ของโมเดลแลตทิซโดยใช้ bitstring ที่สุ่มตัวอย่างจากหน่วยประมวลผลควอนตัม (QPU)
- วิธีใช้ [ffsim](https://github.com/qiskit-community/ffsim) เพื่อสร้าง time evolution circuit สำหรับการจำลองเฟอร์มิออน
- วิธีรวม sample จาก Circuit หลายตัวสำหรับการ post-process ด้วยอัลกอริทึม sample-based Krylov diagonalization (SKQD)

## ข้อกำหนดเบื้องต้น {#prerequisites}
เราแนะนำให้ผู้ใช้คุ้นเคยกับหัวข้อต่อไปนี้ก่อนเรียนบทแนะนำนี้:
...
- ffsim v0.0.72 หรือใหม่กว่า (`pip install ffsim`)


## ตัวอย่างขนาดเล็กด้วยซิมูเลเตอร์ {#small-scale-simulator-example}
...
ต่อไป เราจะ optimize วงจรสำหรับ hardware เป้าหมาย สำหรับตอนนี้ เราจะสร้าง backend ทั่วไปที่มีจำนวน Qubit ที่กำหนดและชุด gate ที่ time evolution circuit สามารถ decompose ได้โดยธรรมชาติ
...
code cell ต่อไปนี้ plot ผลลัพธ์ plot แรกแสดง energy ที่คำนวณได้เป็นฟังก์ชันของจำนวน configuration recovery iteration และ plot ที่สองแสดง average occupancy ของแต่ละ spatial orbital หลังจาก iteration สุดท้าย เนื่องจากปัญหานี้มีขนาดเล็กมาก iteration แรกจึงนำเราไปใกล้ energy ที่แน่นอนมากแล้ว (สังเกตสเกลของแกน y)
...
### ตรวจสอบ energy {#verify-the-energy}

energy ที่ SQD คืนมารับประกันว่าเป็น upper bound ของ true ground state energy จริง ค่า energy สามารถตรวจสอบได้เพราะ SQD ยังคืน coefficient ของ state vector ที่ประมาณ ground state ด้วย เราสามารถคำนวณ energy จาก state vector โดยใช้ reduced density matrix หนึ่งอนุภาคและสองอนุภาค ดังที่แสดงใน code cell ต่อไปนี้
...
ตอนนี้เราจะรันตัวอย่างที่ใหญ่ขึ้นบน QPU จริง
สำหรับ reference energy เราใช้ผลลัพธ์จากการคำนวณ [DMRG](https://en.wikipedia.org/wiki/Density_matrix_renormalization_group) ที่ทำแยกต่างหาก
...
หากคุณสนใจเนื้อหานี้ คุณอาจสนใจเนื้อหาต่อไปนี้:
- [Sample-based quantum diagonalization of a chemistry Hamiltonian](/tutorials/sample-based-quantum-diagonalization) - บทแนะนำที่เกี่ยวข้องซึ่งใช้ heuristic variational ansatz แทน Trotter circuit
- [Krylov quantum diagonalization of lattice Hamiltonians](/tutorials/krylov-quantum-diagonalization) - บทแนะนำเกี่ยวกับวิธี KQD
- [SQD addon API documentation](https://qiskit.github.io/qiskit-addon-sqd/apidocs/qiskit_addon_sqd.fermion.html#qiskit_addon_sqd.fermion.diagonalize_fermionic_hamiltonian) - เอกสารอ้างอิงสำหรับฟังก์ชัน `diagonalize_fermionic_hamiltonian`
- [*Quantum-Centric Algorithm for Sample-Based Krylov Diagonalization*](https://arxiv.org/abs/2501.09702) - บทความวิจัยที่บทแนะนำนี้อ้างอิง
</Admonition>
