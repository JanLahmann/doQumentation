---
description: 'เรียนรู้การคำนวณผลกระทบของ implicit solvent บนฮาร์ดแวร์ควอนตัมโดยใช้ SQD และ IEF-PCM'
notebook_path: tutorials/implicit-solvent-calculations.ipynb
sidebar_label: 'การคำนวณ implicit solvent โดยใช้ Qiskit Serverless'
title: 'การคำนวณ implicit solvent โดยใช้ Qiskit Serverless'
---
...
*ประมาณการเวลาใช้งาน: 2 นาทีบนโปรเซสเซอร์ Heron r2 (หมายเหตุ: นี่เป็นการประมาณเท่านั้น เวลาจริงอาจแตกต่างออกไป)*
...
หลังจากทำตามบทเรียนนี้แล้ว ผู้ใช้ควรเข้าใจ:

* วิธีการตั้งค่าและรัน workflow ระยะไกลโดยใช้ [Qiskit Serverless](/guides/serverless)

* วิธีการคำนวณผลกระทบของ implicit solvent โดยใช้คอมพิวเตอร์ควอนตัม


## ข้อกำหนดเบื้องต้น {#prerequisites}

เราแนะนำให้ผู้ใช้คุ้นเคยกับหัวข้อต่อไปนี้ก่อนทำตามบทเรียนนี้:
...
การคำนวณ implicit solvent ถูกใช้บ่อยในสาขาชีวฟิสิกส์เชิงคำนวณ โมเดลเหล่านี้อธิบายว่าสารละลาย (solute) มีปฏิสัมพันธ์กับตัวทำละลาย (solvent) อย่างไร โดยไม่ต้องจำลองระบบตัวทำละลายโดยตรง แทนที่จะเป็นเช่นนั้น จะใช้การประมาณโดยห่อโมเดลระบบสารละลายไว้ด้วยการแทนค่าทางคณิตศาสตร์ของตัวกลางไดอิเล็กทริกที่มีลักษณะเฉพาะเชิงประจักษ์ ตัวกลางไดอิเล็กทริกโดยประมาณนี้จะมีปฏิสัมพันธ์กับสารละลาย ซึ่งถูกจำลองโดยตรงด้วยตัวมันเอง ตัวกลางไดอิเล็กทริกมีอิทธิพลต่อคุณลักษณะของระบบสารละลาย เช่น พลังงานสถานะพื้น โดยการมีปฏิสัมพันธ์กับสนามอิเล็กตรอนของมัน สิ่งนี้สำคัญต่อโมเดลชีวฟิสิกส์ที่ใช้ เช่น ในการค้นพบยา เพราะสารประกอบมีพฤติกรรมแตกต่างกันในสภาพแวดล้อมไดอิเล็กทริกที่ต่างกัน การจำลองสารประกอบในอากาศ (in vacuo) จะอธิบายพฤติกรรมที่แตกต่างจากการจำลองในน้ำ เนื่องจากสารประกอบทางเภสัชกรรมต้องเข้าสู่ร่างกายมนุษย์ ซึ่งประกอบด้วยน้ำเป็นส่วนใหญ่ จึงเป็นประโยชน์ที่จะจำลองสารประกอบในสารละลายอย่างน้ำแทนที่จะเป็น in vacuo ด้วยโมเดล implicit solvent เราสามารถบรรลุพฤติกรรมนี้ได้อย่างประหยัด แม้ว่าผลลัพธ์สุดท้ายโดยทั่วไปจะมีความประมาณมากกว่าคู่เทียบ: โมเดล explicit solvent ที่มีค่าใช้จ่ายในการคำนวณสูงกว่า ซึ่งสร้างการแทนค่าโดยตรงของทั้งโมเลกุลสารละลายและตัวทำละลาย

ในบทเรียนนี้ เราสาธิตว่าอัลกอริทึมควอนตัม Sample-based Quantum Diagonalization (SQD) สามารถนำมาใช้ในโมเดล implicit solvent ที่มีค่าใช้จ่ายในการคำนวณค่อนข้างต่ำได้อย่างไร ในตัวอย่างนี้ เราอธิบายว่าเมทิลามีนมีพฤติกรรมอย่างไรเมื่อละลายในน้ำ เราเปรียบเทียบอัลกอริทึมควอนตัมกับวิธีการเปรียบเทียบแบบคลาสสิกที่ทันสมัยที่สุดที่เรียกว่า CASCI และแสดงให้เห็นถึงความสอดคล้องที่ใกล้เคียงกันระหว่างการคำนวณเหล่านี้ เราแสดงสถาปัตยกรรม quantum-centric supercomputing แบบย่อ โดยถ่ายโอนการประมวลผลหลังการทำงาน (post-processing) แบบคลาสสิกที่มีค่าใช้จ่ายในการคำนวณสูงของส่วน sampling ควอนตัมในกระบวนการไปยังสภาพแวดล้อมบนคลาวด์ภายใน Qiskit Serverless โค้ดยังแสดงการประมวลผลแบบขนานบนคอร์ CPU ที่เข้าถึงได้ระยะไกลเพื่อปรับปรุงเวลาในการคำนวณ

Qiskit Serverless เป็นเฟรมเวิร์กสำหรับรันงานเวิร์กโหลดควอนตัมและคลาสสิกแบบกระจายโดยไม่ต้องจัดการโครงสร้างพื้นฐาน ไม่มีการจัดสรรเซิร์ฟเวอร์ (ไม่ต้องตั้งค่า EC2, cluster, Docker container) ไม่มีเครื่องมือจัดการ orchestration (Kubernetes, Docker Swarm) และไม่มีการเฝ้าติดตาม/บำรุงรักษา งาน Serverless แต่ละงานรันในคอนเทนเนอร์ที่สะอาด ทำงานตามโค้ดของคุณ แล้วปิดตัวลง ไม่มีการจดจำข้อมูลระหว่างงาน คุณเพียงเขียนโค้ดแล้วส่งงานของคุณ ภายในงาน Serverless โปรแกรมสามารถเข้าถึง Backend ของ IBM Quantum&reg; ได้อย่างราบรื่นและประมวลผลผลลัพธ์แบบคลาสสิกในนั้นได้ ด้วย Qiskit Serverless ผู้ใช้สามารถเข้าถึงคอร์ CPU และหน่วยความจำระยะไกลที่พร้อมใช้งานตลอดเวลา ซึ่งช่วยให้กระจายงานเวิร์กโหลดคลาสสิกบางส่วนไปยังทรัพยากรระยะไกลได้ ผู้ใช้ยังได้เปรียบในด้านการประมวลผลแบบขนานของโปรแกรมในขณะที่หลีกเลี่ยงปัญหาทั่วไปจากการปิดอุปกรณ์ระหว่างการทำงาน สำหรับข้อมูลเพิ่มเติมเกี่ยวกับ Qiskit Serverless ดู[เอกสารประกอบ](/guides/serverless) รวมถึงเนื้อหาเพิ่มเติมบน [GitHub](https://qiskit.github.io/qiskit-serverless/index.html)

บทเรียนนี้แสดงการใช้งานที่เกี่ยวข้องกับสิ่งต่อไปนี้:

* Sample-based quantum diagonalization

* โมเดลการคำนวณแบบ client-server สำหรับการคำนวณควอนตัม


บทเรียนนี้ได้รับแรงบันดาลใจและอ้างอิงจากงานวิจัยที่ดำเนินการที่ Cleveland Clinic ตามที่อธิบายไว้ใน [Kaliakin, Danil, et al. "Implicit solvent sample-based quantum diagonalization." The Journal of Physical Chemistry B 129.23 (2025): 5788-5796](https://pubs.acs.org/doi/10.1021/acs.jpcb.5c01030) ซึ่งเปิดเผย workflow เต็มรูปแบบสำหรับการคำนวณ implicit solvent และขยายด้วยความสอดคล้องในตัวเอง (self-consistency) ของ solvent แบบวนซ้ำ ("The Heartwood Algorithm", M. Motta, T. Pellegrini, 2025), การปรับ geometry ให้เหมาะสม และการเลือก qubit layout อัตโนมัติ โปรดดู [SQD IEF-PCM Qiskit Function template](/guides/function-template-chemistry-workflow) ซึ่งพัฒนาร่วมกันโดย Cleveland Clinic และ IBM&reg; โดยอิงจากงานวิจัยของ Cleveland Clinic สำหรับอินเทอร์เฟซแบบ black-box ที่คล่องตัวสำหรับการรันการคำนวณ implicit solvent

## ข้อกำหนด {#requirements}

ก่อนเริ่มบทเรียนนี้ ตรวจสอบให้แน่ใจว่าคุณได้ติดตั้งสิ่งต่อไปนี้แล้ว:
* Qiskit SDK v2.0 หรือใหม่กว่าพร้อมการสนับสนุน[การแสดงผลภาพ](https://docs.quantum.ibm.com/api/qiskit/visualization)
...
สร้างไดเรกทอรีที่อยู่ถัดจากโปรแกรม notebook หลักโดยตรง เรียกว่า `source_files` คุณจะใส่ไฟล์ Python ลงในไดเรกทอรีนี้ที่คุณตั้งใจจะแชร์กับสภาพแวดล้อมการประมวลผลระยะไกล คุณต้องสร้างไฟล์สองไฟล์:
...
คลิกเพื่อขยายข้อความสำหรับแต่ละสคริปต์ด้านล่าง จากนั้นคัดลอกและวางเนื้อหาลงในไฟล์ในเครื่องที่มีชื่อพาธเหล่านี้
...
เราต้องแชร์โปรแกรมที่ตั้งใจจะรันในสภาพแวดล้อมคลาวด์ และอัปโหลดใหม่ทุกครั้งที่เราเปลี่ยนซอร์สโค้ดของมัน:
...
บทเรียนนี้ไม่ได้ใช้ซิมูเลเตอร์ขนาดเล็ก เนื่องจากมีวัตถุประสงค์เพื่อสาธิตแอปพลิเคชันควอนตัมที่ขยายขนาดได้ ซึ่งขยายเกินขอบเขตของการสำรวจด้วยซิมูเลเตอร์ แทนที่จะเป็นเช่นนั้น เราจะแสดงในภายหลังว่าวิธีนี้สามารถนำไปใช้โดยใช้วิธีการเปรียบเทียบแบบคลาสสิกที่ทันสมัยที่สุดที่เรียกว่า CASCI ได้อย่างไร
...
## ขั้นตอนที่ 1: แมปอินพุตแบบคลาสสิกเป็นปัญหาควอนตัม {#step-1-map-classical-inputs-to-a-quantum-problem}

### 1.1: กำหนดค่าเริ่มต้นออบเจกต์โมเลกุลโดยใช้ geometry ของโมเลกุลที่ทราบล่วงหน้า $\textit{a priori}$ {#11-initialize-molecule-object-using-known-textita-priori-molecular-geometry}
...
### 1.2: กำหนดผลกระทบของ solvation โดยใช้ Polarizable Continuum Model (PCM) {#12-define-solvation-effects-using-the-polarizable-continuum-model-pcm}
...
### 1.3: การปรับ geometry ให้เหมาะสมโดยใช้ TRIC {#13-geometry-optimization-using-tric}
...
### 1.4: เตรียม continuum model และออบเจกต์ mean field พร้อมตัวแปรที่เกี่ยวข้อง {#14-prepare-continuum-model-and-mean-field-object-with-relevant-variables}
...
สำหรับข้อมูลเพิ่มเติมเกี่ยวกับฟังก์ชันช่วยที่สาธิตไว้ที่นี่ ดูบทเรียน [Sample-based quantum diagonalization of a chemistry Hamiltonian](/tutorials/sample-based-quantum-diagonalization)
...
## ขั้นตอนที่ 3 และ 4: รันโดยใช้ Qiskit และประมวลผลหลังการทำงานโดยใช้ Qiskit Serverless {#steps-3-4-execute-using-qiskit-and-post-process-using-qiskit-serverless}

ที่นี่ เรารวมขั้นตอนที่ 3 (รัน) และขั้นตอนที่ 4 (ประมวลผลหลังการทำงาน) เข้าด้วยกัน เนื่องจากบริบทการใช้งานของโมเดล implicit solvent ต้องการลูปข้อเสนอแนะแบบวนซ้ำที่รันหลายรอบของการรันและการประมวลผลหลังการทำงานเพื่อปรับปรุงการคำนวณสุดท้าย

### 3.1 คำนวณพลังงาน restricted Hartree-Fock {#31-compute-the-restricted-hartree-fock-energy}
...
### 3.2: กำหนดพลังงานอ้างอิงแบบคลาสสิกด้วย CASCI {#32-establish-classical-reference-energy-with-casci}
...
## กำหนดค่าพารามิเตอร์ของแอปพลิเคชัน {#configure-application-parameters}
...
<Admonition type="tip" title="Recommendations">
  หากคุณสนใจงานนี้ คุณอาจสนใจแหล่งข้อมูลต่อไปนี้:

  * [Quantum Resource Management Interface (QRMI)](https://github.com/qiskit-community/qrmi) — ผสานทรัพยากรควอนตัมและคลาสสิกเข้ากับตัวจัดการเวิร์กโหลด HPC อย่าง Slurm ขยายรูปแบบการประมวลผลแบบกระจายที่สาธิตไว้ในบทเรียนนี้

  * [คู่มือ Qiskit Serverless](/guides/serverless)

  * [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html)

  * บทเรียน [Sample-based quantum diagonalization of a chemistry Hamiltonian](/tutorials/sample-based-quantum-diagonalization)

  * [ภาพรวม Sample-based quantum diagonalization (SQD)](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd)

  * [ปรับใช้และรันเทมเพลตสำหรับการจำลองโครงสร้างอิเล็กทรอนิกส์ด้วยโมเดล implicit solvent](/guides/function-template-chemistry-workflow) (เทมเพลต SQD IEF-PCM Qiskit Function ซึ่งพัฒนาร่วมกันโดย Cleveland Clinic และ IBM)
</Admonition>
