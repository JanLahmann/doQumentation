---
description: 'แก้ปัญหาการปรับแต่งควอนตัมระดับ utility scale ด้วย Optimization Solver ซึ่งเป็น Qiskit Function โดย Q-CTRL Fire Opal'
notebook_path: tutorials/solve-higher-order-binary-optimization-problems-with-q-ctrls-optimization-solver.ipynb
sidebar_label: 'การปรับแต่งไบนารีอันดับสูงด้วย Optimization Solver ของ Q-CTRL'
title: 'การปรับแต่งไบนารีอันดับสูงด้วย Optimization Solver ของ Q-CTRL'
---
...
บทเรียนนี้จะแสดงให้เห็นวิธีแก้ปัญหาการปรับแต่งไบนารีอันดับสูง (HOBO) โดยใช้ [Optimization Solver ซึ่งเป็น Qiskit Function โดย Q-CTRL Fire Opal](/guides/q-ctrl-optimization-solver) ตัวอย่างในบทเรียนนี้คือปัญหาการปรับแต่งที่ออกแบบมาเพื่อหาค่าพลังงานสภาวะพื้นฐานของโมเดล Ising แบบสุ่ม-พันธะ 156-Qubit ที่มีพจน์คิวบิก Optimization Solver ใช้ได้กับปัญหาการปรับแต่งทั่วไปที่กำหนดได้ในรูปของฟังก์ชันเป้าหมาย

Optimization Solver ทำให้ขั้นตอนการนำไปใช้งานบน hardware ที่ต้องรับรู้สภาพ hardware เป็นอัตโนมัติทั้งหมด และด้วยการใช้ประโยชน์จาก [Performance Management](/guides/q-ctrl-performance-management) สำหรับการประมวลผลควอนตัม จึงได้รับผลลัพธ์ที่แม่นยำในระดับ utility scale สำหรับสรุปรายละเอียดเต็มรูปแบบของ workflow ของ Optimization Solver และผลการเปรียบเทียบ โปรดดู [บทความที่ตีพิมพ์](https://arxiv.org/abs/2406.01743)

บทเรียนนี้จะอธิบายขั้นตอนต่อไปนี้:

1. กำหนดปัญหาในรูปของฟังก์ชันเป้าหมาย

2. รัน hybrid algorithm ด้วย Fire Opal Optimization Solver
...
นอกจากนี้ต้องได้รับสิทธิ์เข้าถึงฟังก์ชัน Optimization Solver ด้วย [กรอกแบบฟอร์ม](https://quantum.cloud.ibm.com/functions?id=q-ctrl-optimization-solver) เพื่อขอสิทธิ์เข้าถึง
...
กำหนดข้อมูลประจำตัวของ [IBM Quantum Platform](https://quantum.cloud.ibm.com/) ซึ่งจะใช้ตลอดบทเรียนนี้เพื่อยืนยันตัวตนกับ Qiskit Runtime และ Qiskit Functions
...
## ขั้นตอนที่ 1: กำหนดปัญหาในรูปของฟังก์ชันเป้าหมาย {#step-1-define-the-problem-as-an-objective-function}
Optimization Solver รับอินพุตเป็นฟังก์ชันเป้าหมายหรือกราฟก็ได้ ในบทเรียนนี้ปัญหาการหาค่าต่ำสุดของ Ising spin glass ถูกนิยามในรูปของฟังก์ชันเป้าหมาย และได้รับการปรับแต่งให้เหมาะกับ topology แบบ heavy-hex ของอุปกรณ์ IBM&reg;
...
สำหรับการอภิปรายโดยละเอียดเกี่ยวกับการสร้างนิยามปัญหาและผลลัพธ์ก่อนหน้าที่ได้จาก Optimization Solver โปรดดู [บทความวิชาการนี้](https://arxiv.org/abs/2406.01743) ปัญหาดังกล่าวถูกนิยามและประเมินครั้งแรกในฐานะส่วนหนึ่งของ [บทความที่ตีพิมพ์โดย Los Alamos National Laboratory](https://arxiv.org/abs/2312.00997) และได้รับการดัดแปลงให้ใช้ประโยชน์จากความกว้างเต็มอุปกรณ์ของโปรเซสเซอร์ IBM Quantum Heron 156-Qubit
...
## ขั้นตอนที่ 2: รัน hybrid algorithm โดยใช้ Fire Opal Optimization Solver {#step-2-run-the-hybrid-algorithm-using-the-fire-opal-optimization-solver}
...
สามารถใช้ [Qiskit Serverless APIs](/guides/serverless) ที่คุ้นเคยเพื่อตรวจสอบสถานะ workload ของ Qiskit Function ได้:
...
Solver คืนค่า dictionary ที่มีผลลัพธ์และ metadata ที่เกี่ยวข้อง เช่น bitstring ของผลลัพธ์ จำนวนการวนซ้ำ และการแมปตัวแปรไปยัง bitstring สำหรับนิยามครบถ้วนของ input และ output ของ Solver ดูได้ที่ [documentation](https://docs.quantum.ibm.com/api/functions/q-ctrl-optimization-solver)
...
## ขั้นตอนที่ 3: ประเมินผลลัพธ์ {#step-3-evaluate-results}
...
Solver พบผลลัพธ์ที่ถูกต้อง ซึ่งได้รับการยืนยันโดยใช้ซอฟต์แวร์ classical optimization ความซับซ้อนของปัญหาระดับ utility-scale นี้ต้องการซอฟต์แวร์ optimization ขั้นสูงในการแก้แบบ classical เช่น [IBM ILOG CPLEX Optimization Studio (CPLEX)](https://www.ibm.com/products/ilog-cplex-optimization-studio) หรือ [Gurobi Optimization](https://www.gurobi.com/)
ในการวิเคราะห์คุณภาพของผลลัพธ์ด้วยภาพ สามารถพล็อตผลลัพธ์โดยคำนวณค่าต้นทุนจาก bitstring และความน่าจะเป็นของแต่ละ bitstring เพื่อเปรียบเทียบ ให้พล็อตผลลัพธ์คู่กับการกระจายของ bitstring ที่สุ่มมา ซึ่งเทียบเท่ากับผลลัพธ์แบบ classical "brute-force" หาก algorithm หาค่าต้นทุนที่ต่ำกว่าได้อย่างสม่ำเสมอ แสดงว่า quantum algorithm กำลังแก้ปัญหา optimization ได้อย่างมีประสิทธิภาพ
