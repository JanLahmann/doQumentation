---
description: 'ใช้ Qunova HiVQE เพื่อคำนวณกราฟ potential energy surface (PES) การแยกพันธะ'
notebook_path: tutorials/qunova-hivqe.ipynb
sidebar_label: 'กราฟ PES การแยกพันธะด้วย Qunova HiVQE'
title: 'กราฟ PES การแยกพันธะด้วย Qunova HiVQE'
---
...
Qiskit Functions เป็นฟีเจอร์ทดลองที่ใช้ได้เฉพาะผู้ใช้แผน IBM Quantum&reg; Premium Plan, Flex Plan และ On-Prem (ผ่าน IBM Quantum Platform API) เท่านั้น อยู่ในสถานะ preview release และอาจมีการเปลี่ยนแปลง
</Admonition>
...
การคำนวณพลังงานของปฏิกิริยาเคมีอย่างแม่นยำมีความสำคัญอย่างยิ่งต่อความก้าวหน้าทางวิทยาศาสตร์ในด้านวัสดุศาสตร์ วิศวกรรมเคมี การค้นพบยา และสาขาอื่น ๆ ในบรรดาระบบเคมีต่าง ๆ ระบบ Li-S ได้รับความสนใจอย่างมากในการทำความเข้าใจและพัฒนาองค์ประกอบของแบตเตอรี่ใหม่ บทเรียนนี้ให้ประสบการณ์จริงในการคำนวณ potential energy surface (PES) การแยกพันธะ Li-S ของระบบ $Li_2S$ โดยการกำจัดอะตอมลิเทียมหนึ่งอะตอมโดยใช้การคำนวณ HiVQE สามารถเปรียบเทียบผลลัพธ์กับการคำนวณอ้างอิง (CASCI) รวมถึงวิธีการแบบ classical อย่าง Hartree-Fock (HF) สำหรับปัญหา 20 Qubit
...
## ส่วนที่ 1: Li2S (20Q) {#part-1-li2s-20q}

### ขั้นตอนที่ 1: แปลงข้อมูล input แบบ classical เป็นปัญหาควอนตัม {#step-1-map-classical-inputs-to-a-quantum-problem}

กำหนดรูปทรงเรขาคณิตของ $Li_2S$ ในรูปแบบ dictionary สำหรับระยะห่างพันธะ Li-S ที่แตกต่างกัน เพื่อคำนวณกราฟ PES รูปทรงเหล่านี้ได้รับการ optimize โดยใช้การคำนวณ B3LYP/631g
...
การคำนวณ HiVQE จะดำเนินการด้วยตัวเลือกที่กำหนดด้านล่าง โดยใช้ basis sto3g สำหรับ $Li_2S$ มี 19 spatial orbital กับ 22 อิเล็กตรอน เพื่อรันกรณี (10o,10e) ด้วยการคำนวณ HiVQE สามารถกำหนด 10 active orbital และ 6 frozen orbital ในแต่ละ iteration จะใช้ 100 shots เพื่อ sample การกำหนด electron configuration ที่สร้างโดย Circuit ExcitationPreserving (`epa`) ที่มี entanglement แบบ `circular` และสอง repetition (`reps`) จำนวน iteration สูงสุดกำหนดไว้ที่ 30 เพื่อให้มั่นใจว่า iteration จะสิ้นสุดเมื่อพลังงานมีการ converge
...
### ขั้นตอนที่ 2 และ 3: Optimize ปัญหาสำหรับการรันบน quantum hardware และดำเนินการโดยใช้ฟังก์ชัน HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}

ตั้งค่า `for` loop เพื่อรันการคำนวณ HiVQE กับรูปทรงเรขาคณิตตามตัวเลือกที่กำหนดด้านล่าง Job จะถูก submit ใน `for` loop ในบทเรียนนี้จะ submit รูปทรงเรขาคณิต 6 แบบและดึงผลลัพธ์เมื่อทั้งหมดเสร็จสิ้น ในการรันฟังก์ชันหลักต้องกำหนด `max_states` และ `max_expansion_states` เพื่อควบคุมขนาดสูงสุดของ subspace matrix และควบคุมว่าจะสร้าง state ได้กี่ state โดยใช้วิธี classical CI expansion ต่อ iteration ID ของ function job จะถูกเก็บใน dictionary พร้อม label ของแต่ละรูปทรงเรขาคณิตเพื่อติดตามและประมวลผล output
...
สามารถติดตามเวลา runtime QPU ทั้งหมดที่ใช้ใน job ได้โดยการ login เข้า [IBM Quantum Platform](https://quantum.cloud.ibm.com/) และดู job ที่ submit ด้วย tag `qunova-chemistry-hivqe`

### ขั้นตอนที่ 4: Post-process และเปรียบเทียบกับวิธีการแบบ classical {#step-4-post-process-and-compare-with-classical-methods}
...
### การ plot กราฟการแยกพันธะของ Li\_2S {#plotting-the-dissociation-curve-for-li-2s}

มา plot และเปรียบเทียบผลลัพธ์ HiVQE กับ HF และ CASCI
สังเกตได้ว่าการคำนวณ HiVQE ทั้งหมดสอดคล้องกับผลลัพธ์อ้างอิงแบบ classical (CASCI) อย่างดี
...
## ส่วนที่ 2: FeP-NO (44Q) {#part-2-fep-no-44q}

โปรดทราบว่าในการรัน function บนระบบ FeP-NO ด้วยการตั้งค่าที่แสดงในตัวอย่างนี้ คุณต้องมีใบอนุญาตที่อนุญาตให้ใช้ function กับอย่างน้อย 44 qubits ติดต่อสอบถามการขอรับใบอนุญาตได้ที่ [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com)

### ขั้นตอนที่ 1: แมปข้อมูล input แบบ classical ไปสู่ปัญหาควอนตัม {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
### ขั้นตอนที่ 2 และ 3: ปรับปัญหาให้เหมาะสมสำหรับการรันบนฮาร์ดแวร์ควอนตัม และรันด้วย HiVQE Chemistry function {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

จากการตั้งค่า HiVQE และ geometry ที่กำหนดไว้ ให้ดึงผลลัพธ์ทีละรายการตามลำดับ

#### ส่งการคำนวณสำหรับ d(Fe-N) = 1.75 $\AA$ {#submit-dfe-n-175-aa-calculation}
...
ติดตาม job และดึงผลลัพธ์สำหรับการคำนวณ d(Fe-N) = 1.75 $\AA$
...
#### ส่งการคำนวณสำหรับ d(Fe-N) = 2.00 $\AA$ {#submit-dfe-n-200-aa-calculation}
...
ติดตาม job และดึงผลลัพธ์สำหรับการคำนวณ d(Fe-N) = 2.00 $\AA$
...
#### ส่งการคำนวณสำหรับ d(Fe-N) = 5.00 $\AA$ {#submit-dfe-n-500-aa-calculation}
...
ติดตาม job และดึงผลลัพธ์สำหรับการคำนวณ d(Fe-N) = 5.00 $\AA$
...
### ขั้นตอนที่ 4: ประมวลผลหลังการคำนวณและเปรียบเทียบกับวิธี classical {#step-4-post-process-and-compare-with-classical-methods-1}
...
[ลิงก์ไปยังแบบสำรวจ](https://your.feedback.ibm.com/jfe/form/SV_9AizRyKFHEGzVs2)
