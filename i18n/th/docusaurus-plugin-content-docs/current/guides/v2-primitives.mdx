---
title: ย้ายไปใช้ Qiskit Runtime V2 primitives
...
```
  ...
  <TabItem value="SamplerOut" label="Convert V2 output to V1 format">
    รูปแบบ output ของ V1 คือ dictionary ของ bitstrings (เป็น int) เป็น key และ quasi-probabilities เป็น value สำหรับแต่ละ circuit รูปแบบ V2 มี key เดียวกัน (แต่เป็น string) และ counts เป็น value เพื่อแปลงรูปแบบ V2 เป็น V1 ให้หาร counts ด้วยจำนวน shots โดยจำนวน shots ที่เลือกอธิบายไว้ในคู่มือ [Sampler options](/guides/sampler-options#shots)
...
- เรียนรู้เพิ่มเติมเกี่ยวกับการตั้งค่า options ในคู่มือ [Sampler options](/guides/sampler-options), [Estimator options](/guides/estimator-options) และ [Executor options](/guides/executor-options)
- เรียนรู้รายละเอียดเพิ่มเติมเกี่ยวกับ [Primitive inputs and outputs](/guides/primitive-input-output#pubs)
