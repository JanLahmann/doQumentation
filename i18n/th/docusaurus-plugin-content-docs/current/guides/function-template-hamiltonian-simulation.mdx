...
template นี้ครอบคลุม workflow สำหรับจำลองการ time evolution ของ initial state กับ spin-based Hamiltonian ที่ผู้ใช้กำหนด และคืนค่า expectation values ที่ระบุไว้โดยใช้ [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/) Qiskit addon
...
ขั้นแรก ให้เขียน function template สำหรับ Hamiltonian simulation ที่ใช้ [AQC-Tensor Qiskit addon](https://qiskit.github.io/qiskit-addon-aqc-tensor/) เพื่อ map คำอธิบายปัญหาไปยัง circuit ที่มี depth ลดลงสำหรับการรันบน hardware

หากคุณดาวน์โหลดหน้านี้และเปิดในโปรแกรมแก้ไข notebook จะเห็นว่า code cell บางส่วนมี [magic command](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile` magic command นี้จะบันทึก code ลงไว้ที่ `./source_files/template_hamiltonian_simulation.py` ซึ่งเป็น function template ที่สามารถ upload และรันจากระยะไกลด้วย Qiskit Serverless
...
<Accordion>
<AccordionItem title="**ซอร์สโค้ดโปรแกรมฉบับเต็ม**">
