...
หน้านี้แสดงวิธีใช้คลาส [`Operator`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Operator) สำหรับภาพรวมระดับสูงของการแทนค่า operator ใน Qiskit รวมถึงคลาส `Operator` และคลาสอื่นๆ ดูที่ [ภาพรวมของคลาส operator](/guides/operators-overview)
...
`Operators` แบบ unitary สามารถแทรกเข้าไปใน `QuantumCircuit` ได้โดยตรงโดยใช้เมธอด `QuantumCircuit.append` วิธีนี้จะแปลง `Operator` เป็นอ็อบเจ็กต์ `UnitaryGate` ซึ่งจะถูกเพิ่มเข้าไปในวงจร
...
โปรดทราบว่าสำหรับเมธอดต่อไปนี้ทั้งหมด หากอ็อบเจ็กต์ที่สองยังไม่ใช่อ็อบเจ็กต์ `Operator` มันจะถูกแปลงเป็นหนึ่งโดยปริยายโดยเมธอดนั้น ซึ่งหมายความว่าเมทริกซ์สามารถส่งเข้าไปได้โดยตรงโดยไม่ต้องแปลงเป็น `Operator` อย่างชัดเจนก่อน หากไม่สามารถแปลงได้ จะเกิดข้อยกเว้นขึ้น
