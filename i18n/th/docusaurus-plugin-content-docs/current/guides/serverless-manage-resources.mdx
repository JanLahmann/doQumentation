...
<Admonition type="tip">
**Qiskit Serverless กำลังได้รับการอัปเกรด และฟีเจอร์ต่าง ๆ กำลังเปลี่ยนแปลงอย่างรวดเร็ว** ในช่วงการพัฒนานี้ ดู release notes และเอกสารล่าสุดได้ที่หน้า [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html)
</Admonition>
...
หากคุณอ่าน code cell เหล่านี้ในเครื่องผ่าน notebook คุณจะเห็น [magic command](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile` การรัน cell ที่มี magic command นี้จะบันทึกลงดิสก์แทนที่จะเรียกใช้งาน
</Admonition>
...
สำหรับงาน classical ที่สามารถประมวลผลแบบขนานได้ ให้ใช้ decorator `@distribute_task` เพื่อกำหนดข้อกำหนดการคำนวณที่จำเป็นในการทำงาน เริ่มต้นด้วยการเรียกดูตัวอย่าง `transpile_remote.py` จากหัวข้อ [เขียนโปรแกรม Qiskit Serverless แรกของคุณ](/guides/serverless-first-program) ด้วยโค้ดต่อไปนี้
...
ต่อไปนี้แสดงวิธีอัปโหลดไฟล์ ตรวจสอบให้แน่ใจว่าคุณได้ authenticate กับ Qiskit Serverless ด้วยบัญชี IBM Quantum ของคุณแล้ว (ดูคำแนะนำที่ [Upload to Qiskit Serverless](/guides/serverless-first-program#upload-to-qiskit-serverless))
...
- ดูตัวอย่างเต็มรูปแบบที่ [ย้ายโค้ดที่มีอยู่ไปยัง Qiskit Serverless](/guides/serverless-port-code)
