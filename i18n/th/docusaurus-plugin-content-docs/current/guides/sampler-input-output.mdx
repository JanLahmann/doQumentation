...
โค้ดต่อไปนี้แสดงตัวอย่างชุดอินพุตแบบเวกเตอร์ไปยัง primitive `Sampler` และรันบน backend ของ IBM&reg; ในรูปแบบอ็อบเจ็กต์ `RuntimeJobV2 ` เดียว
...
นอกเหนือจากผลลัพธ์การรันแล้ว ทั้งอ็อบเจ็กต์ `PrimitiveResult` และ `SamplerPubResult` ยังมีแอตทริบิวต์ข้อมูลเมตาเกี่ยวกับงานที่ถูกส่ง ข้อมูลเมตาที่มีข้อมูลสำหรับ PUB ที่ส่งทั้งหมด (เช่น [ตัวเลือกรันไทม์](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options) ต่างๆ ที่มี) สามารถพบได้ใน `PrimitiveResult.metatada` ในขณะที่ข้อมูลเมตาเฉพาะของแต่ละ PUB สามารถพบได้ใน `SamplerPubResult.metadata`
...
ผลลัพธ์ของงาน [`SamplerV2`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2) ที่รันใน IBM Quantum Compute Service มีข้อมูลเวลาการรันใน metadata
