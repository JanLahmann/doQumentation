---
description: 'อธิบายเทคนิคการลดและกดข้อผิดพลาดที่มีให้ใช้งานผ่าน IBM Quantum Compute Service'
...
![ภาพแสดง Pauli twirling](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)
...
![ภาพแสดง measurement twirling](/docs/images/guides/error-mitigation-explanation/measurement_twirling.svg)

เมื่อมี readout error การทำ measurement twirling มีผลในการทำให้ readout-error transfer matrix เป็นแบบ diagonal ทำให้ง่ายต่อการหาผกผัน มีการเรียนรู้เทอม rescaling จาก diagonal noise channel โดยการ benchmark Circuit สุ่มที่เริ่มต้นในสถานะ zero ซึ่งช่วยให้ service ลบอคติออกจากค่าความคาดหวังที่เกิดจาก readout noise ได้ การประมาณ readout-error transfer matrix ต้องรัน calibration circuits เพิ่มเติม ซึ่งทำให้มี overhead เล็กน้อย

### เปิดใช้ TREX โดยใช้ตัวเลือก `measure_mitigation` {#enable-trex-by-using-the-measure-mitigation-option}

สามารถเปิดใช้ TREX ได้โดยตั้ง `measure_mitigation` เป็น `True` ใน [IBM Quantum Compute resilience options](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) สำหรับ Estimator ตัวเลือกสำหรับ measurement noise learning อธิบายไว้[ที่นี่](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-measure-noise-learning-options) เช่นเดียวกับ gate twirling สามารถตั้งจำนวน circuit randomizations และจำนวน shots ต่อ randomization ได้
...
### เปิดใช้ TREX โดยใช้ตัวเลือก `resilience_level` {#enable-trex-by-using-the-resilience-level-option}

คุณสามารถเปิดใช้ TREX ได้โดยตั้ง [resilience level](/guides/estimator-noise-management#resilience) เป็น `1` ดังที่แสดงในตัวอย่างต่อไปนี้
...
ทั้งการขยาย noise และขั้นตอน extrapolation สามารถนำไปใช้ได้หลายวิธี IBM Quantum Compute นำ noise amplification ไปใช้โดย "digital gate folding" ซึ่งหมายความว่า two-qubit gates ถูกแทนที่ด้วยลำดับที่เทียบเท่าของ Gate และ inverse ของมัน ตัวอย่างเช่น การแทนที่ unitary $U$ ด้วย $U U^\dagger U$ จะให้ noise amplification factor เท่ากับ 3 สำหรับ extrapolation สามารถเลือกจากรูปแบบฟังก์ชันหลายแบบ รวมถึง linear fit หรือ exponential fit
...
overhead ของวิธีนี้ขยายตามจำนวน noise factors การตั้งค่าเริ่มต้นจะ sample ค่าความคาดหวังที่ noise factors สามค่า ทำให้มี overhead ราว 3 เท่า

### เปิดใช้ ZNE โดยใช้ตัวเลือก `zne_mitigation` {#enable-zne-by-using-the-zne-mitigation-option}

สามารถเปิดใช้ ZNE ได้โดยตั้ง `zne_mitigation` เป็น `True` ใน [IBM Quantum Compute resilience options](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) สำหรับ Estimator
ตัวเลือก IBM Quantum Compute สำหรับ ZNE อธิบายไว้[ที่นี่](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options) ตัวเลือกที่น่าสังเกตได้แก่:
...
### เปิดใช้ ZNE โดยใช้ตัวเลือก `resilience_level` {#enable-zne-by-using-the-resilience-level-option}

คุณสามารถเปิดใช้ ZNE (พร้อมกับ TREX และ gate twirling) ได้โดยตั้ง [resilience level](/guides/estimator-noise-management#resilience) เป็น `2` ดังที่แสดงในตัวอย่างต่อไปนี้
...
สามารถเปิดใช้ PEC ได้โดยตั้ง `pec_mitigation` เป็น `True` ใน [IBM Quantum Compute resilience options](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) สำหรับ Estimator
ตัวเลือก IBM Quantum Compute สำหรับ PEC อธิบายไว้[ที่นี่](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-pec-options) สามารถตั้งขีดจำกัดของ sampling overhead ได้โดยใช้ตัวเลือก `max_overhead` โปรดทราบว่าการจำกัด sampling overhead อาจทำให้ความแม่นยำของผลลัพธ์เกินค่าที่ร้องขอ ค่าเริ่มต้นของ `max_overhead` คือ 100
...
- [กำหนดค่าการจัดการ noise ด้วย Estimator](/guides/estimator-noise-management)

- [กำหนดค่าการจัดการ noise ด้วย Sampler](/guides/sampler-noise-management)

- สำรวจ[ตัวเลือก](/guides/runtime-options-overview)อื่นๆ สำหรับ IBM Quantum primitives

- ตัดสินใจว่าจะใช้[execution mode](/guides/execution-modes) ใดในการรัน job ของคุณ
