...
- ใน `qiskit-ibm-runtime` v0.30 หรือใหม่กว่า ให้รัน [`job.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-job-v2#usage) สำหรับ job เดียว หรือ [`batch.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/batch#usage) หรือ [`session.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/session#usage) เมื่อใช้ batch หรือ session mode  หากใช้เวอร์ชันเก่ากว่าของ `qiskit-ibm-runtime` (>= 0.23 และ < 0.30) ยังสามารถพบ usage ได้ใน `session.details()["usage_time"]` และ `batch.details()["usage_time"]`

- ใช้ [`GET /sessions/{id}`](https://docs.quantum.ibm.com/api/qiskit-runtime-rest/tags/sessions#tags__sessions__operations__get_session) เพื่อดู usage สำหรับ batch หรือ session เฉพาะ (endpoint เดียวกันสำหรับทั้งสอง execution modes)

- ใช้ [`GET /jobs/{id}`](https://docs.quantum.ibm.com/api/qiskit-runtime-rest/tags/jobs#tags__jobs__operations__get_job_metrics_jid) เพื่อดู usage สำหรับ job เดียว
