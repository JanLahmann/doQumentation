...
description: "ทำความเข้าใจรูปแบบอินพุตและเอาต์พุต (รวมถึง Primitive Unified Blocs หรือ PUBs) ของ primitives"
...
<Admonition type="note" title="โมเดลการประมวลผลใหม่ ขณะนี้อยู่ใน beta release">
beta release ของโมเดลการประมวลผลใหม่พร้อมให้ใช้งานแล้ว โมเดล directed execution ให้ความยืดหยุ่นมากขึ้นในการปรับแต่ง error mitigation workflow ดูคู่มือ [Directed execution model](/guides/directed-execution-model) สำหรับข้อมูลเพิ่มเติม
</Admonition>
{/*
...
![ภาพนี้แสดง visual representations หลายแบบของ array broadcasting](/docs/images/guides/primitive-input-output/broadcasting.avif "Visual representation of broadcasting")
...
#### วิธีที่ Estimator คำนวณ error {#how-the-estimator-calculates-error}
...
พิจารณา observable เดียว $\mathcal{O}$ หากไม่มี [ZNE](/guides/error-mitigation-and-suppression-techniques#zero-noise-extrapolation-zne) สามารถมองแต่ละ shot ของการรัน Estimator ว่าให้ point estimate ของ expectation value $\langle \mathcal{O} \rangle$ ถ้า pointwise estimates อยู่ใน vector `Os` ค่าที่คืนใน `ensemble_standard_error` เทียบเท่ากับสิ่งต่อไปนี้ (ซึ่ง $\sigma_{\mathcal{O}}$ คือ[standard deviation ของ expectation value](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.BackendEstimatorV2) estimate และ $N_{shots}$ คือจำนวน shots):

$$\frac{ \sigma_{\mathcal{O}} }{ \sqrt{N_{shots}} },$$

ซึ่งถือว่า shots ทั้งหมดเป็นส่วนหนึ่งของ ensemble เดียว ถ้าขอ gate [twirling](/guides/error-mitigation-and-suppression-techniques#pauli-twirling) (`twirling.enable_gates = True`) สามารถจัดเรียง pointwise estimates ของ $\langle \mathcal{O} \rangle$ เป็นชุดที่มี twirl เดียวกัน เรียกชุด estimates เหล่านี้ว่า `O_twirls` และมี `num_randomizations` (จำนวน twirls) ชุด จากนั้น `stds` คือ standard error of the mean ของ `O_twirls` เช่น

$$\frac{ \sigma_{\mathcal{O}} }{ \sqrt{N_{twirls}} },$$

ซึ่ง $\sigma_{\mathcal{O}}$ คือ standard deviation ของ `O_twirls` และ $N_{twirls}$ คือจำนวน twirls เมื่อไม่ได้เปิดใช้ twirling `stds` และ `ensemble_standard_error` จะเท่ากัน
...
นอกจากผลลัพธ์การประมวลผล ทั้ง `PrimitiveResult` และ `PubResult` objects ยังมี metadata attribute เกี่ยวกับ job ที่ส่งไป metadata ที่มีข้อมูลสำหรับ PUBs ทั้งหมดที่ส่ง (เช่น [runtime options](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options) ต่างๆ ที่มีให้) สามารถพบได้ใน `PrimitiveResult.metatada` ส่วน metadata เฉพาะสำหรับแต่ละ PUB อยู่ใน `PubResult.metadata`
...
สำหรับ Sampler jobs ยังสามารถตรวจสอบ result metadata เพื่อเข้าใจว่าข้อมูลบางส่วนถูกรันเมื่อไหร่ ซึ่งเรียกว่า [_execution span_](monitor-job#execution-spans)
