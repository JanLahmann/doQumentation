...
`NoiseLearnerResult.data` ที่ได้คือรายการของ [`LayerError`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/results-layer-error) objects ที่มี [noise model](https://arxiv.org/abs/2201.09866) สำหรับแต่ละ entangling layer ที่เป็นส่วนหนึ่งของ target circuits แต่ละ `LayerError` เก็บข้อมูล layer ในรูปแบบของ Circuit และชุด qubit labels ควบคู่กับ [`PauliLindbladError`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/results-pauli-lindblad-error) สำหรับ noise model ที่เรียนรู้สำหรับ layer นั้น
...
noise model ที่เรียนรู้บน Circuit ยังสามารถใช้เป็น input ให้กับ IBM Quantum `EstimatorV2` primitive ซึ่งสามารถส่งเข้า primitive ได้หลายวิธี ตัวอย่างสามตัวถัดไปแสดงวิธีที่คุณสามารถส่ง noise model ไปยัง attribute `estimator.options` โดยตรง โดยใช้ `ResilienceOptionsV2` object ก่อนสร้าง Estimator primitive และโดยการส่ง dictionary ที่จัดรูปแบบอย่างเหมาะสม
...
หลังจากส่ง noise model เข้าไปใน `EstimatorV2` object แล้ว สามารถใช้รัน workloads และทำ error mitigation ได้ตามปกติ
...
### ตัวเลือก noise learning {#noise-learning-options-1}
...
วงจรในตัวอย่างก่อนหน้าทำงานผ่าน boxing pass manager ซึ่งจัดกลุ่มคำสั่งเข้าไปในกล่องที่มีคำอธิบายประกอบ โค้ดที่เกี่ยวข้องถูกเพิ่มไว้ที่นี่เพื่อความเข้าใจง่าย
- `inject_noise_targets=”gates”` ระบุให้เพิ่ม annotation `InjectNoise` ลงในกล่องที่มี entangler
- `inject_noise_strategy="uniform_modification"` ระบุให้กำหนด `ref` และ `modifier_ref` เดียวกันให้กับกล่องที่เทียบเท่ากันทั้งหมดที่มี annotation `InjectNoise`
      - `InjectNoise.ref` เป็นตัวระบุที่ไม่ซ้ำกันซึ่งใช้เพื่อกำหนดโมเดลสัญญาณรบกวนให้กับกล่องนั้น
      - `InjectNoise.modifier_ref` ช่วยให้สามารถปรับขนาดโมเดลสัญญาณรบกวนที่กำหนดให้กับกล่องได้ด้วยตัวคูณ
...
- เรียนรู้เพิ่มเติมเกี่ยวกับ [Error mitigation and suppression techniques](error-mitigation-and-suppression-techniques)
