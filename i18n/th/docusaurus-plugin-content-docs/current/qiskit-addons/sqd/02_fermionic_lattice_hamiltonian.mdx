...
sidebar_label: 'การปรับปรุงการประมาณพลังงานของโมเดลตาข่าย Fermionic ด้วย SQD'
title: 'การปรับปรุงการประมาณพลังงานของโมเดลตาข่าย Fermionic ด้วย SQD'
---
...
ในบทแนะนำนี้ เราจะสร้าง [Qiskit pattern](https://quantum.cloud.ibm.com/docs/guides/intro-to-patterns) ที่แสดงวิธีการประมวลผลหลัง (post-process) ตัวอย่างควอนตัมที่มีสัญญาณรบกวน เพื่อหาการประมาณสถานะพื้น (ground state) ของ Hamiltonian ตาข่าย Fermionic ที่เรียกว่า single-impurity Anderson model เราจะใช้แนวทาง sample-based quantum diagonalization เพื่อประมวลผลตัวอย่างที่ได้จากชุดสถานะฐาน Krylov ขนาด ``16``-Qubit ในช่วงเวลาที่เพิ่มขึ้นเรื่อยๆ สถานะเหล่านี้ถูกสร้างบนอุปกรณ์ควอนตัมโดยใช้ Trotterization ของวิวัฒนาการตามเวลา เพื่อรับมือกับผลกระทบของสัญญาณรบกวนควอนตัม จึงใช้เทคนิค configuration recovery [แนวทางนี้ได้รับการพิสูจน์ว่าลู่เข้าอย่างมีประสิทธิภาพ](https://arxiv.org/abs/2501.09702) เมื่อสถานะเริ่มต้นดีและสถานะพื้นมีความเบาบาง (sparsity)

รูปแบบสามารถอธิบายได้ใน 4 ขั้นตอน:

1. **ขั้นตอนที่ 1: แปลงเป็นปัญหาควอนตัม**
...
3. **ขั้นตอนที่ 3: รันการทดลอง**

    - ดึงตัวอย่าง (sample) จาก circuit โดยใช้ primitive ``Sampler``
...
4. **ขั้นตอนที่ 4: ประมวลผลผลลัพธ์**
   ...
   - ลูป self-consistent configuration recovery
       ...
       - บันทึกพลังงานสถานะพื้นต่ำสุดที่พบในทุก batch และอัปเดตค่าเฉลี่ยการครอบครอง orbital


### ขั้นตอนที่ 1: แปลงปัญหาเป็น Circuit ควอนตัม {#step-1-map-problem-to-a-quantum-circuit}
...
ต่อไป เราจะสร้าง quantum Krylov subspace ด้วยชุด Circuit ควอนตัมแบบ Trotterized เราจะสร้าง helper สำหรับสร้างสถานะเริ่มต้น (reference) รวมถึงวิวัฒนาการตามเวลาสำหรับส่วนหนึ่งตัวและสองตัวของ Hamiltonian สำหรับคำอธิบายโดยละเอียดเพิ่มเติมเกี่ยวกับโมเดลนี้และวิธีออกแบบ Circuit โปรดดู [บทความ](https://arxiv.org/abs/2501.09702)
...
### ขั้นตอนที่ 3: รันการทดลอง {#step-3-execute-experiments}
...
***หมายเหตุ: ต้องใช้ [Qiskit Aer](https://qiskit.github.io/qiskit-aer/index.html) เพื่อจำลองตัวอย่างจาก Circuit ที่ผ่าน Transpile แล้ว***
...
ตอนนี้ เราจะรันอัลกอริทึม SQD โดยใช้ฟังก์ชัน `diagonalize_fermionic_hamiltonian` ดู [เอกสาร API](../apidocs/qiskit_addon_sqd.fermion.rst#qiskit_addon_sqd.fermion.diagonalize_fermionic_hamiltonian) สำหรับคำอธิบายของอาร์กิวเมนต์ของฟังก์ชันนี้
