...
(a) ทำให้ trial wave function เป็น normalize โดยหา $A$ ที่ทำให้ $$ \int_{-\infty}^{\infty} |\Psi_\text{trial}|^2 dx = 1 $$.

<Accordion>
<AccordionItem title="Answer">

การทำ normalize สำหรับ trial wave function ที่กำหนด:
...
(b) คำนวณค่าคาดหวังของ Hamiltonian $\hat{H}$ ที่กำหนดโดย $$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V(x)$$ โดยที่ $V(x)  = \frac{1}{2}m\omega^2x^2$ ซึ่งสอดคล้องกับศักย์ของ simple harmonic oscillator

<Accordion>
<AccordionItem title="Answer">

Hamiltonian สำหรับ harmonic oscillator คือ:
...
ปรับค่า $\alpha$ เพื่อให้พลังงานต่ำที่สุด
...
เติมช่องว่างด้วยคำที่ถูกต้องเพื่อให้สรุปกระบวนการ VQE สมบูรณ์ แล้วคลิกเพื่อตรวจสอบคำตอบของคุณ
...
มาเริ่มต้นด้วยการโหลดแพ็กเกจที่จำเป็นบางส่วน รวมถึง IBM Quantum primitives นอกจากนี้เราจะเลือกคอมพิวเตอร์ควอนตัมที่ว่างน้อยที่สุดที่มีอยู่
...
หมายเหตุ: คุณสามารถได้ผลลัพธ์ที่ดีขึ้นโดยการตั้งค่าระดับของ error mitigation โดยใช้ [`resilience_level`](/guides/error-mitigation-and-suppression-techniques) ค่าเริ่มต้นคือ 1 และถ้าคุณตั้งค่าที่สูงขึ้น มันจะใช้เวลา QPU มากขึ้นแต่อาจให้ผลลัพธ์ที่ดีขึ้น
...
### ขั้นตอนที่ 1: Map ปัญหาไปยัง quantum circuits และ operators {#step-1-map-the-problem-to-quantum-circuits-and-operators-1}
...
### ขั้นตอนที่ 2: ปรับให้เหมาะสมสำหรับฮาร์ดแวร์เป้าหมาย {#step-2-optimize-for-target-hardware-1}
...
### ขั้นตอนที่ 3: รันบนฮาร์ดแวร์เป้าหมาย {#step-3-execute-on-target-hardware-1}
...
### ขั้นตอนที่ 4: Post-process {#step-4-post-process-1}
...
ใช้ [กฎของ Coulomb](https://en.wikipedia.org/wiki/Coulomb%27s_law) และ [atomic unit](https://en.wikipedia.org/wiki/Atomic_units) เพื่อให้แน่ใจว่าคุณได้ค่าเป็น `Hartree`
...
## คำนวณพลังงานของปฏิกิริยา $H + H = H_2$ {#compute-reaction-energy-of-h-h-h-2}
