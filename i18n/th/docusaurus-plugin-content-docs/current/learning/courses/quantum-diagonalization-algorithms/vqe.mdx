...
* สร้าง guess state พร้อมพารามิเตอร์ $\vec{\theta}_i$ ที่จะเปลี่ยนแปลง: $|\psi(\vec{\theta}_i)\rangle$ แม้ว่า guess เริ่มต้นนี้อาจเป็นแบบสุ่ม แต่ไม่แนะนำให้ทำเช่นนั้น เราต้องการใช้ความรู้เกี่ยวกับปัญหาเพื่อปรับ guess ของเราให้มากที่สุดเท่าที่เป็นไปได้
...
H = \sum_{\alpha = 1}^T{c_\alpha h_\alpha}
$$
โดยที่ $h_\alpha$ คือ Pauli string ที่กระทำบน qubit ทั้งหมดที่อธิบายระบบของคุณ เช่น `IZZX…XIYX` หรือ string หลายตัวที่ commute กัน สังเกตว่า Qiskit ใช้ *little endian* notation โดยที่ Pauli operator ที่ $n$ จากด้านขวากระทำบน qubit ที่ $n$ ดังนั้นเราสามารถวัด operator ของเราได้โดยวัด Pauli operator ลำดับหนึ่ง
...
สามารถทำได้ใน 4 กลุ่ม สังเกตว่าคำตอบดังกล่าวโดยทั่วไปไม่ unique
...
ansatz ถูกเขียนโดยใช้ gate ที่คุ้นเคย แต่ circuit ของเราต้อง transpile เพื่อใช้ basis gate ที่สามารถนำไปใช้บนควอนตัมคอมพิวเตอร์แต่ละเครื่อง เราเลือก backend ที่ยุ่งน้อยที่สุด (คิวงานน้อยที่สุด)
...
สังเกตว่า gate ที่ใช้เปลี่ยนไป และ qubit ใน circuit นามธรรมของเราถูกแมปไปยัง qubit ที่มีหมายเลขต่างกันบนควอนตัมคอมพิวเตอร์ เราต้องแมป Hamiltonian ของเราให้เหมือนกันเพื่อให้ผลลัพธ์มีความหมาย
...
QPE มีวัตถุประสงค์คล้ายกับ VQE แต่การนำไปใช้งานต่างกันมาก QPE ต้องการควอนตัมคอมพิวเตอร์แบบ fault-tolerant เนื่องจาก quantum circuit ของมันมักมีความลึกมาก และต้องการ coherence ในระดับสูง เมื่อ QPE สามารถนำไปใช้งานได้ มันจะแม่นยำกว่า VQE วิธีหนึ่งในการอธิบายความแตกต่างคือผ่านความแม่นยำเป็นฟังก์ชันของ circuit depth QPE บรรลุความแม่นยำ $\epsilon$ ด้วย circuit depth ที่ scale เป็น $O(1/\epsilon)$ [\[10\]](#references) VQE ต้องการตัวอย่าง $O(1/\epsilon^2)$ เพื่อบรรลุความแม่นยำเดียวกัน [\[10,11\]](#references)
...
* มักใช้ configuration recovery [\[16\]](#references) และ symmetry postselection [\[15\]](#references) เป็นเทคนิค post processing
