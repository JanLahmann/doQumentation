...
![eigenvector จริงที่มีป้ายกำกับ x การเดาที่มีป้ายกำกับ x 0 และการแสดงกราฟิกของ error ระหว่างสองตัว](/learning/images/courses/quantum-diagonalization-algorithms/krylov/kqd-fig2.svg)
...
![eigenvector จริงและการอัปเดตการเดาเริ่มต้น การเดาที่อัปเดตใกล้เคียงกับ eigenvector จริงมากขึ้น](/learning/images/courses/quantum-diagonalization-algorithms/krylov/kqd-fig3.svg)
...
แทนที่จะ implement time-evolution operator อย่างแม่นยำ เราสามารถใช้ Trotter decomposition เพื่อ implement การประมาณของมัน การทำซ้ำ Trotter decomposition อันดับหนึ่งหลายครั้งช่วยลด error จาก approximation เพิ่มเติม ในต่อไปนี้ เราสร้าง Trotter implementation โดยตรงในวิธีที่มีประสิทธิภาพที่สุดสำหรับ interaction graph ของ Hamiltonian ที่พิจารณา (nearest neighbor interactions เท่านั้น) ในทางปฏิบัติเราแทรก Pauli rotations $R_{xx}$, $R_{yy}$, $R_{zz}$ ที่มีค่าความแรงของ coupling $J_x,$ $J_y,$ และ $J_z$ และมุมแบบ parameter $t$ ซึ่งสอดคล้องกับการ implement โดยประมาณของ $e^{-i (J_x XX + J_y YY + J_z ZZ) t}$ เมื่อพิจารณาความแตกต่างในนิยามของ Pauli rotations และ time evolution ที่เราพยายาม implement เราต้องใช้ parameter `2*dt` เพื่อให้ได้ time evolution ของ `dt` นอกจากนี้เราพลิกลำดับของการดำเนินการสำหรับ Trotter steps จำนวนคี่ ซึ่งเทียบเท่ากันทางฟังก์ชัน แต่ช่วยให้ synthesize การดำเนินการที่อยู่ติดกันเป็น $SU(2)$ unitary เดียว ทำให้ได้ circuit ที่ตื้นกว่ามากเมื่อเทียบกับที่ได้โดยใช้ `PauliEvolutionGate()` ทั่วไป
