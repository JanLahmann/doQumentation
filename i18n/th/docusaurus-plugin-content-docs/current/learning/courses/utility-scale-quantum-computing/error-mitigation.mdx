---
title: "การลดข้อผิดพลาด"
sidebar_label: "การลดข้อผิดพลาด"
description: "ตลอดบทเรียนนี้ เราจะศึกษา noise และวิธีที่สามารถลดได้บนคอมพิวเตอร์ควอนตัม"
...
[ดาวน์โหลด pdf](https://ibm.ent.box.com/public/static/a0zgies7bh91hm2lwev9o0bfeybxc6n6.zip) ของบรรยายต้นฉบับ โปรดทราบว่าโค้ดบางส่วนอาจล้าสมัยเนื่องจากเป็นภาพนิ่ง
...
## 2. การจำลอง noisy โดยไม่มีการลดข้อผิดพลาด {#2-noisy-simulation-without-error-mitigation}
...
```
### 2.1 สร้าง test circuit {#21-build-a-test-circuit}
...
### 2.2 สร้าง noise model {#22-build-a-noise-model}

เพื่อทำ noisy simulation เราต้องระบุ `NoiseModel` ส่วนนี้แสดงวิธีสร้าง `NoiseModel`
ก่อนอื่นเราต้องกำหนด quantum (หรือ readout) errors เพื่อเพิ่มใน noise model
...
### 2.3 สร้าง noisy sampler ด้วย noise model {#23-build-a-noisy-sampler-with-the-noise-model}
...
### 2.4 รัน quantum circuits บน noisy sampler {#24-run-quantum-circuits-on-the-noisy-sampler}
...
### 2.5 แสดงผลลัพธ์ {#25-plot-results}
...
### 2.6 การจำลองแบบ ideal {#26-ideal-simulation}
...
### 2.8 การจำลอง noisy ที่ใกล้เคียงความเป็นจริงมากขึ้น {#28-more-realistic-noisy-simulation}
...
## 3. การคำนวณควอนตัมจริงพร้อมการลดข้อผิดพลาด {#3-real-quantum-computation-with-error-mitigation}

ในส่วนนี้ เราแสดงวิธีรับผลลัพธ์ที่ผ่านการลดข้อผิดพลาด (expectation values) โดยใช้ Qiskit Estimator
เราพิจารณา 6-qubit Trotterized circuits สำหรับการจำลอง time evolution ของ one dimensional Ising model และดูว่า error ปรับขนาดตามจำนวน time steps อย่างไร
...
### 3.1 สร้าง circuits {#31-build-circuits}
...
เพื่อทราบผลลัพธ์ที่ ideal ล่วงหน้า เราใช้ compute-uncompute circuits ที่ประกอบด้วย stage แรกที่ใช้ circuit ต้นฉบับ $U$ และ stage ที่สองที่กลับด้าน $U^\dagger$
โปรดทราบว่าผลลัพธ์ ideal ของ circuits ดังกล่าวจะเป็น input state $|000000\rangle$ อย่างไม่สำคัญ ซึ่งมี expectation values ที่ไม่สำคัญสำหรับ Pauli observables ใด ๆ เช่น $\langle IIIIIZ \rangle = 1$
...
หมายเหตุ: ตามที่แสดงข้างต้น circuit ที่มี $k$ time steps จะมี two-qubit gate layers จำนวน $4k$ ชั้น
...
### 3.2 Transpile circuits {#32-transpile-the-circuits}
...
### 3.3 Execute โดยใช้ Estimator (ด้วย resilience levels ที่แตกต่างกัน) {#33-execute-using-estimator-with-different-resilience-levels}

การตั้งค่า resilience level (`estimator.options.resilience_level`) เป็นวิธีที่ง่ายที่สุดในการใช้การลดข้อผิดพลาดเมื่อใช้ Qiskit Estimator Estimator รองรับ resilience levels ต่อไปนี้ (ณ วันที่ 2024/06/28) ดูรายละเอียดเพิ่มเติมในคู่มือ [Configure error mitigation](/guides/error-mitigation-and-suppression-techniques)
...
### 3.4 แสดงผลลัพธ์ {#34-plot-results}
...
## 4. (Optional) ปรับแต่ง error mitigation options {#4-optional-customize-error-mitigation-options}
