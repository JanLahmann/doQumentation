...
- ขั้นแรก เราจะทำความรู้จักกับ [IBM Quantum primitives](/guides/primitives)

- นิยาม *ฟังก์ชันค่าใช้จ่าย* $C(\vec\theta)$ ซึ่งเป็นฟังก์ชันเฉพาะสำหรับปัญหาที่กำหนดเป้าหมายให้ optimizer นำไปลดค่า (หรือเพิ่มค่า)

- กำหนดกลยุทธ์การวัดด้วย IBM Quantum primitives เพื่อสมดุลระหว่างความเร็วและความแม่นยำ
...
IBM Quantum [`Sampler`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2) รัน Circuit หลายครั้งบนอุปกรณ์ควอนตัม ทำการวัดในแต่ละรอบ และสร้างการกระจายความน่าจะเป็นขึ้นใหม่จาก bitstrings ที่ได้ ยิ่งรันมากครั้ง (หรือ *shots*) ผลลัพธ์ก็ยิ่งแม่นยำ แต่ต้องใช้เวลาและทรัพยากรควอนตัมมากขึ้น
...
โชคดีที่มีวิธีแก้: เมทริกซ์ขนาด $2^n \times 2^n$ ใดๆ สามารถเขียนเป็นการรวมเชิงเส้นของ tensor products ของ Pauli matrices และ identities จำนวน $4^n$ ตัว ทั้งหมดล้วน Hermitian และ unitary โดยมี $V$ และ $\Lambda$ ที่รู้จัก นี่คือสิ่งที่ `Estimator` ทำภายในโดยแยก object [`Operator`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Operator) ใดๆ ออกเป็น [`SparsePauliOp`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.SparsePauliOp)
...
$$
ตัวดำเนินการนี้คือผลรวมเชิงเส้นของพจน์ที่มีตัวดำเนินการ Z บนโหนดที่เชื่อมต่อกันด้วยขอบ (โปรดจำไว้ว่าคิวบิตที่ 0 อยู่ขวาสุด): $IIZZ + IZIZ + IZZI + ZIIZ + ZZII$ เมื่อสร้างตัวดำเนินการนี้แล้ว ansatz สำหรับอัลกอริทึม QAOA สามารถสร้างได้อย่างง่ายดายโดยใช้วงจร `QAOAAnsatz` จากไลบรารีวงจรของ Qiskit
...
โดยที่ IBM Quantum Estimator ใช้ Hamiltonian และ parameterized ansatz โดยตรง และส่งกลับพลังงานที่จำเป็น cost function สำหรับ QAOA instance ค่อนข้างง่าย:
...
[การบรรเทาข้อผิดพลาด](/guides/error-mitigation-and-suppression-techniques) หมายถึงเทคนิคที่ช่วยให้ผู้ใช้ลดข้อผิดพลาดของ Circuit โดยการสร้างแบบจำลองสัญญาณรบกวนของอุปกรณ์ในขณะที่รัน โดยทั่วไปแล้ว สิ่งนี้ส่งผลให้มี overhead ในการประมวลผลล่วงหน้าเชิงควอนตัมที่เกี่ยวกับการฝึกโมเดล และ overhead ในการประมวลผลหลังเชิงคลาสสิกเพื่อบรรเทาข้อผิดพลาดในผลลัพธ์ดิบโดยใช้โมเดลที่สร้างขึ้น

ตัวเลือก `resilience_level` ของ IBM Quantum primitive ระบุปริมาณความยืดหยุ่นที่จะสร้างขึ้นต่อข้อผิดพลาด ระดับที่สูงขึ้นจะสร้างผลลัพธ์ที่แม่นยำกว่าแต่แลกมาด้วยเวลาประมวลผลที่นานขึ้นเนื่องจาก overhead ในการสุ่มตัวอย่างเชิงควอนตัม ระดับความยืดหยุ่นสามารถใช้กำหนดค่าการแลกเปลี่ยนระหว่างต้นทุนและความแม่นยำเมื่อนำการบรรเทาข้อผิดพลาดไปใช้กับ primitive query ของคุณ
...
สำหรับหลักสูตรนี้ เราจะสำรวจโมเดลการบรรเทาข้อผิดพลาดเหล่านี้ในระดับสูงเพื่อแสดงให้เห็นการบรรเทาข้อผิดพลาดที่ IBM Quantum primitive สามารถทำได้โดยไม่ต้องอธิบายรายละเอียดการใช้งานทั้งหมด
...
เราสามารถตั้งค่านี้ด้วย `options.resilience_level = 2` เราสามารถปรับปรุงเพิ่มเติมได้โดยสำรวจ `noise_factors`, `noise_amplifiers`, และ `extrapolators` หลากหลาย แต่สิ่งนี้อยู่นอกขอบเขตของหลักสูตรนี้ เราขอแนะนำให้ทดลองกับ[ตัวเลือกเหล่านี้ตามที่อธิบายไว้ที่นี่](/guides/error-mitigation-and-suppression-techniques)
...
### การใช้ตัวเลือกใน Quantum Compute สำหรับการบรรเทาและการระงับ {#using-the-options-in-quantum-compute-for-mitigation-and-suppression}
...
- วิธีใช้ IBM Quantum primitives เพื่อบรรเทาและระงับสัญญาณรบกวน
