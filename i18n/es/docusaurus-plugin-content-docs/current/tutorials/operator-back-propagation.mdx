...
En el ejemplo anterior, si $\{P,P_s\} = 0$, entonces necesitamos ejecutar dos circuitos cuánticos, en lugar de uno, para calcular el valor esperado. Por lo tanto, la retropropagación podría aumentar el número de términos en el observable, lo que lleva a un mayor número de ejecuciones de circuitos. Una forma de permitir una retropropagación más profunda en el circuito, evitando que el operador crezca demasiado, es truncar los términos con coeficientes pequeños, en lugar de agregarlos al operador. Por ejemplo, en el ejemplo anterior, se podría optar por truncar el término que involucra $P_sP$ siempre que $\theta_s$ sea suficientemente pequeño. Truncar términos puede reducir el número de circuitos cuánticos a ejecutar, pero al hacerlo se produce cierto error en el cálculo final del valor esperado proporcional a la magnitud de los coeficientes de los términos truncados.
...
### Paso 4: Posprocesar y devolver el resultado al formato clásico deseado {#step-4-post-process-and-return-result-in-desired-classical-format}
...
- [`pauli-prop`](https://github.com/Qiskit/pauli-prop), un paquete acelerado con Rust para la propagación de Pauli, con tutoriales que cubren OBP, estimación clásica de valores esperados y simulación ruidosa
</Admonition>
