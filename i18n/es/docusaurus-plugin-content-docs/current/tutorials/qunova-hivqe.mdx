---
description: 'Utilice Qunova HiVQE para calcular curvas de superficie de energía potencial (PES) de disociación.'
notebook_path: tutorials/qunova-hivqe.ipynb
sidebar_label: 'Curvas de disociación PES con Qunova HiVQE'
title: 'Curvas de disociación PES con Qunova HiVQE'
---
...
Qiskit Functions es una característica experimental disponible únicamente para usuarios del Plan Premium, Plan Flex y Plan On-Prem (a través de la API de IBM Quantum Platform) de IBM Quantum&reg;. Se encuentran en estado de versión preliminar y están sujetas a cambios.
</Admonition>

*Estimación de uso (NOTA: Esto es solo una estimación. Tu tiempo de ejecución puede variar.)*
...
Para ejecutar este tutorial, importa la función `qunova/hivqe-chemistry` a través de `QiskitFunctionCatalog`. Necesita una cuenta del Plan Premium, Plan Flex o Plan On-Prem (API de IBM Quantum Platform) de IBM Quantum con una licencia de Qunova para ejecutar esta función.
...
## Parte 1: Li2S (20Q) {#part-1-li2s-20q}

### Paso 1: Asignar entradas clásicas a un problema cuántico {#step-1-map-classical-inputs-to-a-quantum-problem}

Define las geometrías de $Li_2S$ en formato de diccionario para diferentes distancias de enlace Li-S con el fin de calcular la curva PES. Estas geometrías están optimizadas mediante cálculos B3LYP/631g.
...
### Pasos 2 y 3: Optimizar el problema para la ejecución en hardware cuántico y ejecutar utilizando la función HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}

Configura el bucle `for` para ejecutar los cálculos de HiVQE con las geometrías y las opciones definidas a continuación. Los trabajos se envían en el bucle `for`. En este tutorial, enviarás seis geometrías y recuperarás los resultados cuando todos estén completados. En la ejecución de la función principal, necesitas definir `max_states` y `max_expansion_states` para controlar el tamaño máximo de la matriz del subespacio y para controlar cuántos estados pueden generarse utilizando métodos de expansión CI clásicos por iteración. Los identificadores de trabajo de la función se almacenarán en el diccionario con cada etiqueta de geometría para el seguimiento y procesamiento posterior de la salida.
...
El tiempo total de ejecución en QPU utilizado en el trabajo puede rastrearse iniciando sesión en [IBM Quantum Platform](https://quantum.cloud.ibm.com/) y visualizando los trabajos enviados con la etiqueta `qunova-chemistry-hivqe`.

### Paso 4: Post-procesamiento y comparación con métodos clásicos {#step-4-post-process-and-compare-with-classical-methods}
...
### Representación gráfica de la curva de disociación para Li\_2S {#plotting-the-dissociation-curve-for-li-2s}

Grafiquemos y comparemos los resultados de HiVQE con HF y CASCI.
Puedes observar que todos los cálculos de HiVQE coinciden adecuadamente con el resultado de referencia clásico (CASCI).
...
## Parte 2: FeP-NO (44Q) {#part-2-fep-no-44q}

Ten en cuenta que para ejecutar la función en el sistema FeP-NO con los ajustes mostrados en este ejemplo, necesitas una licencia que permita usar la función con al menos 44 qubits. Envía un correo a [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) para consultar sobre cómo obtener una licencia.

### Paso 1: Mapear entradas clásicas a un problema cuántico {#step-1-map-classical-inputs-to-a-quantum-problem-1}

Define las opciones para los cálculos de HiVQE
...
Define las geometrías de FeP-NO en formato de diccionario para diferentes distancias de enlace Fe-N con el fin de calcular la curva PES.
...
### Pasos 2 y 3: Optimizar el problema para la ejecución en hardware cuántico y ejecutar usando la función HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

Según la configuración de HiVQE y las geometrías, obtén los resultados de forma secuencial.

#### Enviar el cálculo de d(Fe-N) = 1.75 $\AA$. {#submit-dfe-n-175-aa-calculation}
...
Rastrea el trabajo y recupera el resultado del cálculo de d(Fe-N) = 1.75 $\AA$.
...
#### Enviar el cálculo de d(Fe-N) = 2.00 $\AA$. {#submit-dfe-n-200-aa-calculation}
...
Rastrea el trabajo y recupera el resultado del cálculo de d(Fe-N) = 2.00 $\AA$.
...
#### Enviar el cálculo de d(Fe-N) = 5.00 $\AA$. {#submit-dfe-n-500-aa-calculation}
...
Rastrea el trabajo y recupera el resultado del cálculo de d(Fe-N) = 5.00 $\AA$.
...
### Paso 4: Postprocesar y comparar con métodos clásicos {#step-4-post-process-and-compare-with-classical-methods-1}
...
Por favor, realiza esta breve encuesta para proporcionar comentarios sobre este tutorial. Tus opiniones nos ayudarán a mejorar nuestras ofertas de contenido y la experiencia del usuario.

> **Nota:** Esta encuesta es de **IBM Quantum** y cubre el **contenido del tutorial** (escrito por IBM). doQumentation proporciona el sitio web, las traducciones y la ejecución de código — para comentarios sobre eso, por favor [abre un issue en GitHub](https://github.com/JanLahmann/doQumentation/issues).

[Enlace a la encuesta](https://your.feedback.ibm.com/jfe/form/SV_9AizRyKFHEGzVs2)
