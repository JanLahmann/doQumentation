---
description: 'Construye y resuelve el modelo de Ising con campo transversal utilizando Fire Opal y Qiskit'
...
## Configuración {#setup}
Primero, autentícate utilizando tu [clave API de IBM Quantum](http://quantum.cloud.ibm.com/). Luego, selecciona la función de Qiskit de la siguiente manera. (Este código asume que ya has [guardado tu cuenta](/guides/functions-get-started#install-qiskit-functions-catalog-client) en tu entorno local.)
...
Evaluamos la dinámica de magnetización en hardware cuántico real. Fire Opal proporciona una función de Qiskit que extiende la primitiva estándar Estimator de IBM Quantum con supresión automática de errores y gestión de rendimiento. Enviamos los circuitos trotterizados directamente a un backend de IBM® mientras Fire Opal se encarga de la ejecución con reconocimiento de ruido.
