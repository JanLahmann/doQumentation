---
description: 'Aprende a calcular efectos de solvente implícito en hardware cuántico usando SQD e IEF-PCM.'
notebook_path: tutorials/implicit-solvent-calculations.ipynb
sidebar_label: 'Cálculos de solvente implícito usando Qiskit Serverless'
title: 'Cálculos de solvente implícito usando Qiskit Serverless'
---
...
*Estimación de uso: 2 minutos en un procesador Heron r2 (NOTA: Esto es solo una estimación. Tu tiempo de ejecución puede variar.)*
...
* Cómo configurar y ejecutar un flujo de trabajo remoto usando [Qiskit Serverless](/guides/serverless)

* Cómo calcular efectos de solvente implícito usando una computadora cuántica


## Prerrequisitos {#prerequisites}

Sugerimos que los usuarios estén familiarizados con el siguiente tema antes de realizar este tutorial:

* [Diagonalización cuántica basada en muestreo](/learning/courses/quantum-diagonalization-algorithms/sqd-overview)


## Antecedentes {#background}

Los cálculos de solvente implícito se utilizan frecuentemente en biofísica computacional. Estos modelos describen cómo un compuesto soluto interactúa con un solvente, sin modelar el sistema solvente directamente. En cambio, se hace una aproximación en la que el modelo del sistema soluto se envuelve en una representación matemática de un medio dieléctrico caracterizado empíricamente. Esta aproximación dieléctrica luego interactúa con el soluto, que se modela directamente. El medio dieléctrico influye en las características del sistema soluto, como su energía de estado fundamental, al interactuar con su campo electrónico. Esto es importante para los modelos biofísicos utilizados, por ejemplo, en el descubrimiento de fármacos, porque los compuestos se comportan de manera diferente en varios entornos dieléctricos. Modelar un compuesto en el aire (in vacuo) describirá un comportamiento diferente al de modelarlo en agua. Dado que los compuestos farmacéuticos deben entrar en el cuerpo humano, que en sí mismo está compuesto principalmente de agua, es útil modelar un compuesto en una solución como el agua en lugar de in vacuo. Con los modelos de solvente implícito, podemos lograr este comportamiento de manera económica, aunque el resultado final generalmente será más aproximado que su contraparte: los modelos de solvente explícito, computacionalmente más costosos, que crean representaciones directas tanto del soluto como de las moléculas de solvente.

En este tutorial, demostramos cómo un algoritmo cuántico, la Diagonalización Cuántica Basada en Muestreo (SQD), se puede incorporar en un modelo de solvente implícito relativamente económico computacionalmente. En el ejemplo, describimos cómo se comporta la metilamina al disolverse en agua. Comparamos el algoritmo cuántico con un método de comparación clásico de última generación llamado CASCI y demostramos una concordancia cercana entre estos cálculos. Mostramos una arquitectura de supercomputación cuántica-céntrica en miniatura, transfiriendo el posprocesamiento clásico computacionalmente costoso de la parte de muestreo cuántico de la rutina a un entorno basado en la nube dentro de Qiskit Serverless. El código también demuestra la paralelización en los núcleos de CPU disponibles de forma remota para mejorar el tiempo de cálculo.

Qiskit Serverless es un marco de trabajo para ejecutar cargas de trabajo cuánticas y clásicas distribuidas sin gestionar infraestructura. No hay aprovisionamiento de servidores (sin levantar instancias EC2, clústeres, contenedores Docker), no hay herramientas de orquestación (Kubernetes, Docker Swarm) y no hay monitoreo/mantenimiento. Cada trabajo de Serverless se ejecuta en un contenedor limpio, ejecuta tu código y luego se apaga. No hay memoria entre trabajos. Simplemente escribes tu código y luego envías tu trabajo. Dentro de un trabajo de Serverless, un programa puede acceder sin problemas a los backends de IBM Quantum&reg; y posprocesar los resultados clásicamente allí mismo. Con Qiskit Serverless, los usuarios pueden acceder a núcleos de CPU y memoria remotos siempre activos, lo que permite la distribución de ciertas cargas de trabajo clásicas en recursos remotos. Los usuarios también obtienen alguna ventaja en el procesamiento paralelo de programas mientras evitan los dolores de cabeza comunes por el apagado de dispositivos a mitad de ejecución. Para obtener más información sobre Qiskit Serverless, consulta su [documentación](/guides/serverless), así como material adicional en [GitHub](https://qiskit.github.io/qiskit-serverless/index.html).

Este tutorial muestra una aplicación relevante de lo siguiente:

* Diagonalización cuántica basada en muestreo

* Modelos computacionales cliente-servidor para computación cuántica


Este tutorial está inspirado y basado en la investigación realizada en Cleveland Clinic, como se describe en [Kaliakin, Danil, et al. "Implicit solvent sample-based quantum diagonalization." The Journal of Physical Chemistry B 129.23 (2025): 5788-5796](https://pubs.acs.org/doi/10.1021/acs.jpcb.5c01030), exponiendo el flujo de trabajo completo para los cálculos de solvente implícito y extendiéndolo con autoconsistencia iterativa del solvente ("El Algoritmo Heartwood", M. Motta, T. Pellegrini, 2025), optimización de geometría y selección automática de disposición de qubits. Por favor consulta la [plantilla de función Qiskit SQD IEF-PCM](/guides/function-template-chemistry-workflow), desarrollada conjuntamente por Cleveland Clinic e IBM&reg; basándose en la investigación de Cleveland Clinic, para obtener una interfaz simplificada de caja negra para ejecutar cálculos de solvente implícito.
...
* Complemento de Qiskit: Diagonalización cuántica basada en muestreo (SQD) v0.12.0 `pip install qiskit_addon_sqd`
...
Crea un directorio exactamente junto al programa notebook principal llamado `source_files`. Colocarás archivos Python en este directorio que pretendes compartir con el entorno de cómputo remoto. Debes crear dos archivos:
...
Haz clic para expandir el texto de cada script a continuación, luego copia y pega el contenido en un archivo local con estos nombres de ruta.

<Accordion>
  <AccordionItem title="Haz clic para ver `source_files\diagonalization_engine.py`">
    
  </AccordionItem>

  <AccordionItem title="Haz clic para ver `source_files\classical_simulation.py`">
...
Necesitamos compartir el programa destinado a ejecutarse en el entorno de nube, y volver a subirlo cada vez que cambiemos su código fuente:
...
Este tutorial no hace uso de un simulador a pequeña escala porque la intención es demostrar una aplicación cuántica escalable que va más allá del ámbito de la exploración con simuladores. En cambio, mostramos más adelante cómo se puede implementar este método utilizando un método de comparación clásico de última generación llamado CASCI.
...
### 1.1: Inicializar el objeto molécula usando la geometría molecular conocida $\textit{a priori}$ {#11-initialize-molecule-object-using-known-textita-priori-molecular-geometry}
...
### 1.2: Definir efectos de solvatación usando el Modelo de Continuo Polarizable (PCM) {#12-define-solvation-effects-using-the-polarizable-continuum-model-pcm}
...
### 1.3: Optimización de geometría usando TRIC {#13-geometry-optimization-using-tric}
...
### 1.4: Preparar el modelo de continuo y el objeto de campo medio con las variables relevantes {#14-prepare-continuum-model-and-mean-field-object-with-relevant-variables}
...
Para obtener más información sobre las funciones auxiliares demostradas aquí, consulta el tutorial de [Diagonalización cuántica basada en muestreo de un hamiltoniano de química](/tutorials/sample-based-quantum-diagonalization).
...
## Pasos 3 y 4: Ejecutar usando Qiskit y posprocesar usando Qiskit Serverless {#steps-3-4-execute-using-qiskit-and-post-process-using-qiskit-serverless}

Aquí, combinamos el Paso 3 (Ejecutar) y el Paso 4 (Posprocesar) porque el contexto de aplicación del modelo de solvente implícito requiere un bucle de retroalimentación iterativo que ejecuta varios ciclos de ejecución y posprocesamiento para mejorar el cálculo final.

### 3.1 Calcular la energía Hartree-Fock restringida {#31-compute-the-restricted-hartree-fock-energy}
...
### 3.2: Establecer la energía de referencia clásica con CASCI {#32-establish-classical-reference-energy-with-casci}
...
## Configurar parámetros de la aplicación {#configure-application-parameters}
...
<Admonition type="tip" title="Recomendaciones">
  Si encontraste este trabajo interesante, podrían interesarte los siguientes recursos:

  * La [Quantum Resource Management Interface (QRMI)](https://github.com/qiskit-community/qrmi) — integra recursos cuánticos y clásicos en gestores de carga de trabajo de HPC como Slurm, extendiendo el patrón de computación distribuida demostrado en este tutorial

  * [Guía de Qiskit Serverless](/guides/serverless)

  * [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html)

  * Tutorial [Diagonalización cuántica basada en muestras de un Hamiltoniano químico](/tutorials/sample-based-quantum-diagonalization)

  * [Resumen de la diagonalización cuántica basada en muestras (SQD)](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd)

  * [Implementa y ejecuta una plantilla para la simulación de estructura electrónica con un modelo de solvente implícito](/guides/function-template-chemistry-workflow) (plantilla de Qiskit Function SQD IEF-PCM, desarrollada conjuntamente por Cleveland Clinic e IBM)
</Admonition>
