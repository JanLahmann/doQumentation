...
La compilación de circuitos cuánticos transforma un algoritmo cuántico de alto nivel en un circuito físico que respeta las restricciones del hardware objetivo. Una compilación eficaz puede reducir significativamente la profundidad del circuito y el número de compuertas, ambos de los cuales impactan directamente en la calidad de los resultados en dispositivos cuánticos de corto plazo.
...
En esta sección, comparamos los tres métodos de compilación en circuitos hamiltonianos con menos de 20 qubits. Estos circuitos se transpilar rápidamente y ofrecen una visión clara de cómo cada método maneja circuitos de complejidad moderada.
...
En la práctica, los errores de las compuertas se acumulan a lo largo del circuito, por lo que la probabilidad de recuperar $|0\rangle^{\otimes n}$ disminuye. Un método de compilación que produce un circuito más superficial con menos compuertas acumulará menos ruido.
...
Las comparaciones anteriores comparan SABRE, el transpilador potenciado por IA y Rustiq en circuitos de simulación hamiltoniana de la colección Hamlib tanto a pequeña como a gran escala.
...
- **SABRE** es el valor predeterminado recomendado. Es rápido y confiable, y produce resultados sólidos en una amplia gama de circuitos. Para una mayor sintonía, los usuarios pueden aumentar los intentos de distribución y enrutamiento (consulta el [tutorial de optimización con SABRE](/tutorials/transpilation-optimizations-with-sabre)).
