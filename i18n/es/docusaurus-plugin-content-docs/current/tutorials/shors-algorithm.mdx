...
- [Estimación de fase y factorización](/learning/courses/fundamentals-of-quantum-algorithms/phase-estimation-and-factoring/introduction). Cubrimos parte de este material en este tutorial.
...
Nota: Este tutorial se centra más en la implementación y demostración de los circuitos relacionados con el algoritmo de Shor. Para un recurso educativo en profundidad sobre el material, consulta el curso [Fundamentals of quantum algorithms](/learning/courses/fundamentals-of-quantum-algorithms/phase-estimation-and-factoring/introduction) del Dr. John Watrous, y los artículos en la sección de [Referencias](#references).
...
### Problema de estimación de fase {#phase-estimation-problem}
...
### Problema de búsqueda de orden {#order-finding-problem}
...
Para conectar el problema de búsqueda de orden con la estimación de fase, consideramos la operación definida sobre un sistema cuyos estados clásicos corresponden a $\mathbb{Z}_N$, donde multiplicamos por un elemento fijo $a \in \mathbb{Z}^*_N$. En particular, definimos este operador de multiplicación $M_a$ tal que $$M_a \ket{x} = \ket{ax \; (\mathrm{mod} \; N)}$$ para cada $x \in \mathbb{Z}_N$. Observa que está implícito que estamos tomando el producto módulo $N$ dentro del ket en el lado derecho de la ecuación. Un análisis matemático muestra que $M_a$ es un operador unitario. Además, resulta que $M_a$ tiene pares de vectores propios y valores propios que nos permiten conectar el orden $r$ de $a$ con el problema de estimación de fase. Específicamente, para cualquier elección de $j \in \{0, \dots, r-1\}$, tenemos que $$\ket{\psi_j} = \frac{1}{\sqrt{r}} \sum^{r-1}_{k=0} \omega^{-jk}_{r} \ket{a^k}$$ es un vector propio de $M_a$ cuyo valor propio correspondiente es $\omega^{j}_{r}$, donde $$\omega^{j}_{r} = e^{2 \pi i \frac{j}{r}}.$$
...
### Operadores de exponenciación modular {#modular-exponentiation-operators}
...
### Ejemplo específico con $N = 15$ y $a=2$ {#specific-example-with-n-15-and-a2}

Podemos hacer una pausa aquí para discutir un ejemplo específico y construir el circuito de búsqueda de orden para $N=15$. Observa que los posibles valores no triviales de $a \in \mathbb{Z}_N^*$ para $N=15$ son $a \in \{2, 4, 7, 8, 11, 13, 14 \}$. Para este ejemplo, elegimos $a=2$. Construiremos el operador $M_2$ y los operadores de exponenciación modular $M_2^k$.
...
Como podemos ver en la lista de valores de $b$, además de $M_2$ que construimos previamente, también necesitamos construir $M_4$ y $M_1$. Observa que $M_1$ actúa trivialmente sobre los estados de la base computacional, por lo que es simplemente el operador identidad.
...
Observa que omitimos las operaciones de exponenciación modular controlada de los qubits de control restantes porque $M_1$ es el operador identidad.
...
### Ejecuciones en hardware {#hardware-runs}
...
### Factorización de enteros {#integer-factorization}
Hasta ahora, discutimos cómo podemos implementar el problema de búsqueda de orden utilizando un circuito de estimación de fase. Ahora, conectamos el problema de búsqueda de orden con la factorización de enteros, lo que completa el algoritmo de Shor. Ten en cuenta que esta parte del algoritmo es clásica.
Ahora demostramos esto usando nuestro ejemplo de $N = 15$ y $a = 2$. Recuerda que la fase que medimos es $k / r$, donde $$a^r \; (\textrm{mod} \; N) = 1$$ y $k$ es un entero aleatorio entre $0$ y $r - 1$. De esta ecuación, tenemos $$(a^r - 1) \; (\textrm{mod} \; N) = 0,$$ lo que significa que $N$ debe dividir a $a^r-1$. Si $r$ también es par, entonces podemos escribir $$a^r -1 = (a^{r/2}-1)(a^{r/2}+1).$$ Si $r$ no es par, no podemos continuar y debemos intentar de nuevo con un valor diferente para $a$; de lo contrario, existe una alta probabilidad de que el máximo común divisor de $N$ y ya sea $a^{r/2}-1$ o $a^{r/2}+1$ sea un factor propio de $N$.

Dado que algunas ejecuciones del algoritmo fallarán estadísticamente, repetiremos este algoritmo hasta que se encuentra al menos un factor de $N$.
La celda a continuación repite el algoritmo hasta que se encuentra al menos un factor de $N=15$. Utilizaremos los resultados de la ejecución en hardware anterior para estimar la fase y el factor correspondiente en cada iteración.
...
### Trabajo relacionado {#related-work}
