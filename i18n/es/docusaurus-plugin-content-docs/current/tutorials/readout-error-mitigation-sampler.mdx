...
- Al estimar ${M}$, asumimos que solo ocurrieron errores de lectura. En particular,
  ...
  o al menos que estaban mitigados de alguna otra manera.
  ...
  sobre cadenas de bits, no hacemos tal suposición. De hecho, esperamos que un circuito
  interesante introduzca ruido, por ejemplo, errores de puerta. La distribución "verdadera"
  todavía incluye efectos de cualquier error que no esté mitigado de otra manera.


Este método, aunque útil en algunas circunstancias, sufre de algunas limitaciones.
...
- La estimación de ${M}$ y ${\tilde{p}}$ está sujeta a error estadístico debido al muestreo finito.
  ...
  a costa de más shots (hasta la escala de tiempo de los parámetros de hardware que van a la deriva
  que resultan en errores sistemáticos en ${M}$).
  Sin embargo, si no se hacen suposiciones sobre las cadenas de bits observadas
  al realizar la mitigación, el número de shots requeridos para estimar ${M}$ crece
...
- ${M}$ es una matriz de $2^n \times 2^n$.
  ...
  mayor que la memoria disponible en una laptop potente.

Otras limitaciones son:

- La distribución recuperada ${\vec{p}}$ puede tener una
  o más probabilidades negativas (aunque sigue sumando uno). Una solución
  es minimizar $||{M} {\vec{p}} - {\tilde{p}}||^2$ sujeto a la restricción de que
  cada entrada en ${\vec{p}}$ sea no negativa. Sin embargo, el tiempo de ejecución de dicho
  método es órdenes de magnitud más largo que resolver directamente la ecuación (2).

- Este procedimiento de mitigación funciona al nivel de una distribución de probabilidad
  sobre cadenas de bits. En particular, no puede corregir un error en una
  cadena de bits observada individual.
