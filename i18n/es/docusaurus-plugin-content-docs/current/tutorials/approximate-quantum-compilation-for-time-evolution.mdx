...
donde $J_{i,(i+1)}$ es un coeficiente aleatorio correspondiente al enlaza $(i, i+1)$, y $L=10$ es el número de sitios.
...
También necesitamos fusionar nuestro `aqc_comparison_circuit` con el circuito de evolución restante. Este circuito se utilizarás para comparar el rendimiento del circuito optimizado con AQC-Tensor con el circuito original.
...
Selecciona el hardware. Aquí utilizaremos cualquiera de los dispositivos de IBM Quantum&reg; disponibles que tengan al menos 127 qubits.
...
Realiza la transpilación para el circuito de comparación.
...
Realiza la ejecución para el circuito de comparación.
...
donde $J_{i,(i+1)}$ es un coeficiente aleatorio correspondiente al enlaza $(i, i+1)$, y $L=50$ es el número de sitios.
Define el mapa de acoplamiento y los enlaces para el hamiltoniano.
...
Genera el circuito objetivo AQC (el segmento inicial).
...
Genera el circuito posterior (el segmento restante).
...
Genera el circuito de comparación AQC (el segmento inicial, pero con el mismo número de pasos de Trotter que el circuito posterior).
...
Genera el circuito de referencia.
...
Genera un ansatz y parámetros iniciales a partir de un circuito Trotter con menos pasos.
...
Configura los ajustes para la simulación de redes de tensores y luego construye una representación de estado de producto matricial del estado objetivo para la optimización. Después, evalúe la fidelidad entre el circuito inicial y el estado objetivo para cuantificar la diferencia en el error de Trotter.
...
Para optimizar los parámetros del ansatz, minimizamos la función de costo `MaximizeStateFidelity` utilizando el optimizador L-BFGS de SciPy, con un criterio de parada establecido para superar la fidelidad del circuito inicial sin AQC-Tensor. Esto asegura que el circuito comprimido ten tanto un menor error de Trotter como una profundidad reducida.
...
Construye el circuito final para la transpilación ensamblando el ansatz optimizado con el circuito de evolución temporal restante.
...
Para una lista completa de las opciones de ejecución disponibles, consulta la [documentación de qiskit-ibm-runtime](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-estimator-options).
...
Grafica los resultados de los circuitos AQC, de comparación y exacto para el modelo XXZ de 50 sitios.
...
- **Optimización eficiente**: El enfoque MPS proporciónó un marco robusto para la optimización de parámetros, equilibrando la fidelidad con la eficiencia computacional.
...
Por favor, realiza esta breve encuesta para proporcionar retroalimentación sobre este tutorial. Sus opiniones nos ayudarán a mejorar nuestras ofertas de contenido y la experiencia del usuario.

[Enlaza a la encuesta](https://your.feedback.ibm.com/jfe/form/SV_eF01c2sfeSt6cqq)
