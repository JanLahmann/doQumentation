---
description: 'Resuelve un problema de optimización cuántica a escala de utilidad utilizando el Optimization Solver, una función de Qiskit de Q-CTRL Fire Opal'
...
El Optimization Solver automatiza completamente los pasos de implementación adaptados al hardware para resolver problemas de optimización en hardware cuántico y, al aprovechar [Performance Management](/guides/q-ctrl-performance-management) para la ejecución cuántica, logra soluciones precisas a escala de utilidad. Para un resumen detallado del flujo de trabajo completo del Optimization Solver y los resultados de las pruebas de rendimiento, consulta [el manuscrito publicado](https://arxiv.org/abs/2406.01743).
...
También necesitarás obtener acceso a la función Optimization Solver. [Completa el formulario](https://quantum.cloud.ibm.com/functions?id=q-ctrl-optimization-solver) para solicitar acceso.

## Configuración {#setup}

Primero, importa los paquetes y herramientas necesarios.
...
Define tus credenciales de [IBM Quantum Platform](https://quantum.cloud.ibm.com/), que se utilizarán a lo largo del tutorial para autenticarse en IBM Quantum Compute Service y Qiskit Functions.
...
Ahora utiliza la función de Qiskit del Optimization Solver para ejecutar el algoritmo. Detrás de escena, el Optimization Solver se encarga de mapear el problema a un algoritmo cuántico híbrido, ejecutar los circuitos cuánticos con supresión de errores y realizar la optimización clásica.
...
Verifica que el dispositivo elegido tenga al menos 156 qubits.
...
Puedes utilizar las conocidas [API de Qiskit Serverless](/guides/serverless) para verificar el estado de la carga de trabajo de tu función de Qiskit:
...
El Solver devuelve un diccionario con la solución y los metadatos asociados, como la cadena de bits de la solución, el número de iteraciones y la correspondencia entre variables y cadena de bits. Para una definición completa de las entradas y salidas del Solver, consulta la [documentación](https://docs.quantum.ibm.com/api/functions/q-ctrl-optimization-solver).
...
Como análisis visual de la calidad de los resultados, puedes graficar los resultados calculando los valores de costo a partir de las cadenas de bits y sus probabilidades. A modo de comparación, grafica los resultados junto con una distribución de cadenas de bits muestreadas aleatoriamente, lo cual es equivalente a una solución clásica por "fuerza bruta". Si el algoritmo encuentra consistentemente costos más bajos, esto sugiere que el algoritmo cuántico está resolviendo eficazmente el problema de optimización.
