...
Ten en cuenta que existen dos tipos de cortes: cortar el cable de un circuito (llamado `wire cutting`), o reemplazar una compuerta de 2 qubits con múltiples operaciones de un solo qubit (llamado `gate cutting`). En este cuaderno, nos centraremos en el corte de compuertas. Para más detalles sobre el corte de compuertas, consulta los <a href="https://qiskit.github.io/qiskit-addon-cutting/explanation/index.html">materiales explicativos</a> en `qiskit-addon-cutting`, y las referencias correspondientes. Para más detalles sobre el corte de cables, consulta el tutorial [Corte de cables para la estimación de valores esperados](/tutorials/wire-cutting), o los tutoriales en <a href='https://qiskit.github.io/qiskit-addon-cutting/tutorials/index.html'>qiskit-addon-cutting</a>.
...
Ten en cuenta que el circuito puede transpilarse en esta etapa o después del corte. Si transpilamos después del corte, eso requerirá que transpilemos cada uno de los subexperimentos generados debido a la sobrecarga de muestreo. Por lo tanto, es más prudente transpilar en esta etapa para reducir la sobrecarga de la transpilación.
...
Ahora cortamos las compuertas en el circuito transpilado. Ten en cuenta que las compuertas de 2 qubits que necesitan ser cortadas son las que conectan el último y el primer qubit del diseño.
...
Ten en cuenta que cortar las interacciones de largo alcance conduce a la ejecución de múltiples muestras del circuito que difieren en las bases de medición y preparación. Puedes encontrar más información sobre esto en <a href='https://arxiv.org/abs/1909.07534'>Constructing a virtual two-qubit gate by sampling single-qubit operations</a> y <a href='https://arxiv.org/abs/2312.11638'>Cutting circuits with multiple two-qubit unitaries</a>.
...
Observamos a partir de los resultados que el promedio de los observables de tipo $\langle Z \rangle$ de peso 1 y $\langle ZZ \rangle$ de peso 2 mejora significativamente al cortar las compuertas periódicas. Ten en cuenta que este estudio no incluye ninguna técnica de supresión o mitigación de errores. La mejora observada se debe únicamente al uso adecuado del corte de compuertas para este problema. Los resultados podrían haberse mejorado aún más utilizando técnicas de mitigación y supresión.
...
> **Nota:** Esta encuesta es de **IBM Quantum** y cubre el **contenido del tutorial** (escrito por IBM). doQumentation proporciona el sitio web, las traducciones y la ejecución de código; para comentarios sobre eso, [abre un issue de GitHub](https://github.com/JanLahmann/doQumentation/issues).
