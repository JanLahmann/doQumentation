...
* Cómo construir circuitos dinámicos con mediciones a mitad de circuito y realimentación clásica para teleportar el entrelazamiento entre qubits distantes;
* Cómo calcular e interpretar la métrica de error para cuantificar la fidelidad del par de Bell en un dispositivo;
* Cómo identificar qué pares de qubits son más adecuados para operaciones basadas en LOCC usando los resultados de la evaluación.
...
* Familiaridad con [circuitos dinámicos](/guides/classical-feedforward-and-control-flow) ([mediciones a mitad de circuito](/guides/execute-dynamic-circuits#mid-circuit-measurements) y realimentación clásica);
* Conocimientos básicos del [SDK de Qiskit](https://docs.quantum.ibm.com/guides) y [IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime), y acceso a una [cuenta de IBM Quantum](/guides/cloud-setup).
...
Antes de ejecutar en la QPU real, verificamos que el circuito produce un par de Bell probándolo en un simulador sin ruido con una cadena de cuatro qubits `[0, 1, 2, 3]`. Usamos el `Sampler` de IBM Quantum con `AerSimulator` como modo del backend para ejecutar los circuitos.

### Paso 1: Mapear las entradas clásicas a un problema cuántico {#step-1-map-classical-inputs-to-a-quantum-problem}

El primer paso es crear un conjunto de circuitos cuánticos para evaluar todos los enlaces candidatos de pares de Bell adaptados a la topología del dispositivo. Comenzamos construyendo dichos circuitos con una cadena de cuatro qubits `[0, 1, 2, 3]`.

La rutina `create_bell_stab()` realiza lo siguiente para cada cadena:
...
Para cada disposición inicial de cuatro qubits, la función anterior devuelve dos circuitos (uno para la medición del estabilizador $ZZ$, otro para $XX$). Estos circuitos incluyen mediciones a mitad de circuito y operaciones condicionales (if/else), que son las instrucciones clave del circuito dinámico.
...
Este valor es 0 para un par de Bell perfecto, y aumenta a medida que el estado entrelazado se vuelve más ruidoso (con resultados aleatorios que dan una expectativa cercana a 0, el MSE se aproximaría a 1). El código calcula este MSE para cada grupo de cuatro qubits. Sin ruido, vemos MSE = 0 en este ejemplo con simulador a pequeña escala, como era de esperar.

## Ejemplo de hardware a gran escala {#large-scale-hardware-example}

Aquí reunimos todos estos detalles en un flujo de trabajo único a mayor escala, que luego se ejecuta en hardware cuántico real.
...
A continuación, construimos el circuito para cada franja de 16 qubits usando la función `create_bell_stab()`. Al final de este paso, tenemos una lista de circuitos que cubren cada cadena de cuatro qubits en el dispositivo. Luego transpilamos y ejecutamos los circuitos en el backend real y post-procesamos los resultados.
...
Si este trabajo te resultó interesante, puede que te interesen los siguientes materiales:
- Aprende a implementar [entrelazamiento de largo alcance con circuitos dinámicos](/tutorials/long-range-entanglement)
- Aprende a simular el [hamiltoniano de Ising pulsado con circuitos dinámicos](/tutorials/dc-hex-ising)
</Admonition>
