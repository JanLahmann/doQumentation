...
*Estimación de uso: 50 minutos en un procesador Heron r2. (NOTA: Esto es solo una estimación. Tu tiempo de ejecución puede variar.)*

Ten en cuenta que el tiempo de ejecución de esta función es generalmente superior a 20 minutos,
por lo que podrías querer dividir este tutorial en dos secciones: la primera, en la que
tú lo lees y lanzas los trabajos, y la segunda unas horas después
...
Ten en cuenta que este solucionador también puede resolver ecuaciones no lineales.
...
* Acceso a la función QUICK-PDE. Completa el [formulario para solicitar acceso.](https://forms.cloud.microsoft/e/3Wi9cbjQPK)


## Configuración {#setup}

Autentícate usando tu [clave de API](http://quantum.cloud.ibm.com/) y selecciona la función de la siguiente manera:
...
en QPUs de IBM&reg; usando QUICK-PDE, dada por:
...
solución. Puedes ajustar $a$ y $b$ y observar cómo influyen en el
proceso de resolución y en la solución. Para esta elección de condición inicial,
la ecuación admite la siguiente solución analítica:

$$u(t, x) = \frac{ax + b}{at + 1},$$

lo cual se mantiene siempre que no se haya formado ningún choque, es decir, para $t < -1/a$ cuando $a < 0$.
...
Puedes comparar el proceso de convergencia de nuestra solución (HDES) de job_2 con el rendimiento de un algoritmo de redes neuronales informadas por la física (PINN) y su solucionador (consulta el [artículo](https://arxiv.org/abs/1711.10561) y el [repositorio de GitHub](https://github.com/314arhaam/burger-pinn) asociado).
...
Puedes hacer lo mismo con tus datos de job_2 y graficar una comparación con la solución PINN.
...
Con tu solución, puedes ahora elegir qué hacer con ella. A continuación se muestra cómo graficar el resultado.
...
Observa la diferencia en la condición inicial de la segunda ejecución y su efecto en
el resultado:
