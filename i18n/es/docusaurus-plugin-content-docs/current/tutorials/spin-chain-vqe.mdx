...
- Cómo ejecutar flujos de trabajo variacionales en hardware cuántico de IBM® usando las primitivas de IBM Quantum y sesiones
...
Minimiza la energía estimada del estado fundamental del sistema optimizando los parámetros del circuito. Utiliza la primitiva `Estimator` de IBM Quantum para evaluar la función de costo durante la optimización.

Como ya optimizamos el circuito para el backend en el Paso 2, podemos evitar la transpilación en el servidor de Quantum Compute estableciendo `skip_transpilation=True` y pasando el circuito optimizado. Para esta demostración, ejecutaremos en una QPU utilizando las primitivas de IBM Quantum. Para ejecutar con las primitivas basadas en statevector de `qiskit`, reemplaza el bloque de código que utiliza las primitivas de IBM Quantum con el bloque comentado.
