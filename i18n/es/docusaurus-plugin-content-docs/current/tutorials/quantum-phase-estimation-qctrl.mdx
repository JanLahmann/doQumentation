...
## Configuración {#setup}
Primero, autentíquese utilizando su [clave de API de IBM Quantum](http://quantum.cloud.ibm.com/). Luego, selecciona la función de Qiskit de la siguiente manera. (Este código asume que ya has [guardado tu cuenta](/guides/functions#install-qiskit-functions-catalog-client) en tu entorno local.)
...
Establecemos el número de disparos y qubits para el experimento, y codificamos la fase objetivo $\varphi = 1/6$ utilizando $m$ dígitos binarios. Con estos parámetros, construimos el circuito QPE que se ejecutarás en simulación, hardware predeterminado y backends mejorados con Fire Opal.
