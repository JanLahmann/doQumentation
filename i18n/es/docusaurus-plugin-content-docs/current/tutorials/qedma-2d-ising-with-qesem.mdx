...
A continuación, autentíquese utilizando su clave de API del panel de control de [IBM Quantum Platform](http://quantum.cloud.ibm.com/). Luego, selecciona la función de Qiskit de la siguiente manera. (Ten en cuenta que, por seguridad, es mejor [guardar las credenciales de tu cuenta](/guides/functions#install-qiskit-functions-catalog-client) en tu entorno local, si se encuentra en una máquina de confianza, para que no ten que ingresar su clave de API cada vez que se autentique.)
...
Ahora utilizaremos la retropropagación de operadores (OBP). (Consulta la guía de [OBP](/guides/qiskit-addons-obp) para obtener más detalles sobre el complemento OBP de Qiskit.) Crearemos una función que genera los segmentos del circuito para la retropropagación:
