...
description: "Construya y analice un conjunto híbrido cuántico-clásico para clasificación de estabilidad de red en QPUs de IBM utilizando la función Qiskit Singularity de Multiverse Computing."
...
De manera crucial, la escala de los datos no está limitada por los qubits. El trabajo pesado con los datos — preprocesamiento, entrenamiento del conjunto de aprendices y evaluación — sigue siendo clásico y puede manejar millones de ejemplos. Los qubits solo determinan el tamaño del conjunto utilizado en el paso de selección cuántica. Este desacoplamiento es lo que hace que el enfoque sea viable en el hardware actual: usted mantiene los flujos de trabajo familiares de scikit-learn para datos y entrenamiento de modelos mientras invoca el paso cuántico a través de una interfaz de acción limpia en Qiskit Functions.
...
Lo que usted hará aquí: preparar y balancear el conjunto de datos de estabilidad de red; establecer una línea base clásica con AdaBoost; ejecutar varias configuraciones cuánticas que varían el ancho del conjunto y la regularización; ejecutar en simuladores o QPUs de IBM&reg; a través de Qiskit Serverless; y comparar la exactitud, precisión, exhaustividad y F1 en todas las ejecuciones. A lo largo del proceso, usted utilizará el patrón de acciones de la función (`create`, `fit`, `predict`, `fit_predict`, `create_fit_predict`) y los controles clave:
...
- Auto-regularización: establezca `regularization="auto"` con una proporción de selección objetivo para adaptar la dispersión automáticamente
...
Los benchmarks en la documentación muestran que la exactitud mejora a medida que aumenta el número de aprendices (qubits) en problemas desafiantes, con el clasificador cuántico igualando o superando a un conjunto clásico comparable. En este tutorial, usted reproducirá el flujo de trabajo de principio a fin y examinará cuándo aumentar el ancho del conjunto o cambiar a regularización adaptativa produce un mejor F1 con un uso razonable de recursos. El resultado es una visión fundamentada de cómo un paso de optimización cuántica puede complementar, en lugar de reemplazar, el aprendizaje clásico por conjuntos en aplicaciones reales.
## Requisitos {#requirements}

Antes de comenzar este tutorial, asegúrese de tener los siguientes paquetes instalados en su entorno de Python:
...
Con Qiskit Serverless, usted puede ejecutar flujos de trabajo híbridos cuántico-clásicos en la infraestructura de nube gestionada por IBM sin preocuparse por la gestión de recursos.
Necesitará una clave API de IBM Quantum Platform y el nombre de su recurso en la nube (CRN) para autenticarse y acceder a las funciones Qiskit.
### Descargue el conjunto de datos {#download-the-dataset}
...
La siguiente celda crea automáticamente la estructura de carpetas necesaria y descarga tanto los archivos de entrenamiento como los de prueba directamente en su entorno utilizando `wget`.
Si usted ya tiene estos archivos localmente, este paso los sobrescribirá de forma segura para garantizar la consistencia de versiones.
...
### Importe los paquetes necesarios {#import-required-packages}
...
### Establezca las variables constantes {#set-constant-variables}
...
### Conéctese a IBM Quantum y cargue la función Singularity {#connect-to-ibm-quantum-and-load-the-singularity-function}
...
`QiskitRuntimeService` establece una conexión segura con IBM Quantum Platform utilizando su token API y el CRN de la instancia, permitiendo el acceso a los backends cuánticos.
...
Si la configuración es exitosa, usted verá un mensaje de confirmación indicando que la función se ha cargado correctamente.
...
### Defina las funciones auxiliares {#define-helper-functions}
...
## Paso 1: Mapee las entradas clásicas a un problema cuántico {#step-1-map-classical-inputs-to-a-quantum-problem}
...
Configured hardware optimization profile:
  ...
  sampler_options: None
```
## Paso 3: Ejecute utilizando primitivas de Qiskit {#step-3-execute-using-qiskit-primitives}
...
- `regularization_type = "alpha"` define cómo se aplica la penalización. A diferencia de `onsite`, que no tiene límites `[0, ∞]`, `alpha` está acotado entre `[0, 1]`, lo que facilita su ajuste e interpretación. El parámetro controla el equilibrio entre penalizaciones individuales y por pares, ofreciendo un rango de configuración más suave.
...
## Paso 4: Post-procese y devuelva el resultado en el formato clásico deseado {#step-4-post-process-and-return-result-in-desired-classical-format}
...
En la práctica:
- Utilice la **línea base clásica** para validación rápida y benchmarking en conjuntos de datos pequeños.
- Aplique **conjuntos cuánticos** cuando la dimensionalidad del modelo o la complejidad de las características crezcan — la búsqueda basada en QAOA escala con mayor elegancia en esos regímenes.
- Emplee **α-regularización adaptativa** para mantener la dispersión y la generalización sin aumentar el ancho del circuito.
- Monitoree el tiempo de QPU y la profundidad para equilibrar las ganancias de calidad con las restricciones del hardware actual.
...
### Visualice las tendencias de calidad a través de las configuraciones {#visualize-quality-trends-across-configurations}
