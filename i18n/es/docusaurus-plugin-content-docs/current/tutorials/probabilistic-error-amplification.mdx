...
*Estimación de uso: 14 minutos en un procesador Heron r3 (NOTA: Esto es solo una estimación. Su tiempo de ejecución puede variar.)*

## Resultados de aprendizaje {#learning-outcomes}
Después de completar este tutorial, los usuarios deberán entender:
- La teoría detrás de la *extrapolación a ruido cero* (ZNE), los diferentes métodos para amplificar el ruido y por qué la *amplificación probabilística de errores* (PEA) es preferida para experimentos a escala de utilidad.
- Cómo implementar ZNE con PEA en la práctica usando Qiskit.

## Requisitos previos {#prerequisites}
Sugerimos que los usuarios estén familiarizados con los siguientes temas antes de realizar este tutorial:
- La [lección de mitigación de errores](/learning/courses/utility-scale-quantum-computing/error-mitigation) del curso *Computación cuántica a escala de utilidad* para conocimientos básicos sobre el uso de la mitigación de errores en Qiskit.
- La [lección Utility-I](/learning/courses/utility-scale-quantum-computing/utility-i) del curso *Computación cuántica a escala de utilidad* para más contexto sobre el experimento a escala de utilidad utilizado como ejemplo en este tutorial.

## Antecedentes {#background}

Este tutorial demuestra cómo ejecutar un experimento de mitigación de errores a escala de utilidad con IBM Quantum Compute Service utilizando una versión experimental de *extrapolación a ruido cero* (ZNE) con *amplificación probabilística de errores* (PEA).
...
#### Amplificar ruido para ZNE {#amplify-noise-for-zne}
...
* El estiramiento de pulsos asume que el ruido de las puertas es proporcional a la duración, lo cual normalmente no es cierto. La calibración también es costosa.

* El plegado de puertas (gate folding) requiere factores de estiramiento grandes que limitan enormemente la profundidad de los circuitos que se pueden ejecutar.

* PEA se puede aplicar a cualquier circuito que se pueda ejecutar con el factor de ruido nativo ($\lambda=1$) pero requiere aprender el modelo de ruido.


### Aprender el modelo de ruido para PEA {#learn-the-noise-model-for-pea}
...
**Referencia**: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, *Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors* [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)
...
## Configuración {#setup}
En la celda a continuación, importamos los paquetes relevantes y creamos algunas funciones auxiliares para construir los circuitos para la evolución temporal trotterizada de un modelo de Ising de campo transverso bidimensional que se adhiere a la topología del backend.
...
Omitiremos este paso ya que la mitigación de errores de runtime no es compatible con simuladores.
...
#### Crear un circuito parametrizado del modelo de Ising {#create-a-parameterized-ising-model-circuit}

##### Establecer un backend {#establish-a-backend}
Primero, elige un backend en el cual ejecutar. Esta demostración se ejecuta en un backend de 127 qubits, pero puedes modificar esto para cualquier backend disponible.
...
##### Definir los acoplamientos de las capas de entrelazamiento {#define-entangling-layer-couplings}
Para implementar la simulación de Ising trotterizada, define tres capas de acoplamientos de compuertas de dos qubits para el dispositivo, que se repetirán en cada uno de los pasos de Trotter. Estas definen las tres capas con twirl que necesitas aprender para el ruido e implementar la mitigación.
...
##### Eliminar qubits defectuosos {#remove-bad-qubits}
Observa el mapa de acoplamiento del backend y verifica si algún qubit se conecta a acoplamientos con alto error. Elimina estos qubits "defectuosos" de tu experimento.
...
##### Generación del circuito principal de Trotter {#main-trotter-circuit-generation}
...
#### Crear una lista de valores de parámetros para asignar posteriormente {#create-a-list-of-parameter-values-to-be-assigned-later}
...
Antes de ejecutar el circuito en hardware, optimízalo para la ejecución en hardware. Este proceso involucra varios pasos:

* Elige una distribución de qubits (qubit layout) que mapee los qubits virtuales de tu circuito a los qubits físicos del hardware.

* Inserta puertas swap según sea necesario para enrutar interacciones entre qubits que no están conectados.

* Traduce las puertas de nuestro circuito a instrucciones de [Instruction Set Architecture (ISA)](/guides/transpile#instruction-set-architecture) que se pueden ejecutar directamente en el hardware.

* Realiza optimizaciones del circuito para minimizar la profundidad del circuito y el número de puertas.


Aunque el transpilador integrado en Qiskit puede realizar todos estos pasos, este tutorial demuestra la construcción del circuito de Trotter a escala de utilidad desde cero. Selecciona los qubits físicos adecuados y define capas de entrelazamiento en pares de qubits conectados a partir de esos qubits seleccionados. No obstante, aún necesita traducir las compuertas que no son ISA en el circuito y aprovechar cualquier optimización de circuito ofrecida por el transpilador.

Transpila tu circuito para el backend elegido creando un administrador de pases y luego ejecutando el administrador de pases en el circuito. Además, fija la disposición inicial del circuito a los `good_qubits` ya seleccionados. Una forma sencilla de crear un administrador de pases es usar la función [`generate_preset_pass_manager`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.generate_preset_pass_manager). Consulta [Transpilar con administradores de pases](/guides/transpile-with-pass-managers) para una explicación más detallada sobre la transpilación con administradores de pases.
...
A continuación, crea todos los observables $\langle Z \rangle$ de peso 1 para cada qubit virtual rellenando el número necesario de términos $\langle I \rangle$.
...
Observa que cada observable está envuelto en una lista en el siguiente bloque de código. Esto se hace para *transmitir* (broadcast) con los valores de parámetros de modo que cada observable de qubit se mida para cada valor de theta. Encuentra las reglas de transmisión para primitivas en la [documentación de primitivas](/guides/primitives).
...
#### Configurar las opciones del Estimator {#configure-estimator-options}
A continuación, configura las opciones del `Estimator` necesarias para ejecutar el experimento de mitigación. Esto incluye opciones para el aprendizaje de ruido de las capas de entrelazamiento y para la extrapolación ZNE.
...
##### Explicación de las opciones de ZNE {#explanation-of-zne-options}
A continuación se proporcionan detalles sobre las opciones adicionales en la rama experimental. Ten en cuenta que estas opciones y nombres no están finalizados, y todo aquí está sujeto a cambios antes de un lanzamiento oficial.

* **amplifier**: El método a utilizar al amplificar el ruido a los factores de ruido deseados.
  ...
  con twirl de Pauli para capas de compuertas base de dos qubits con twirl. Las opciones adicionales son `"gate_folding_front"` y `"gate_folding_back"`, que se explican en la [documentación de la API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier).

* **extrapolated\_noise\_factors**: Especifica uno o más valores de factor de ruido en los cuales evaluar los
...
#### Ejecutar el experimento {#run-the-experiment}
...
Una vez finalizado el experimento, puedes ver tus resultados. Obtén los valores esperados crudos y mitigados y compáralos con los resultados exactos. Luego, grafica los valores esperados, tanto mitigados (extrapolados) como crudos, promediados sobre todos los qubits para cada parámetro. Finalmente, grafica los valores esperados para los qubits individuales de tu elección.

```python
primitive_result = job.result()
```

#### Formas generales de los resultados y metadatos {#general-result-shapes-and-metadata}
...
### Graficar los resultados de la simulación de Trotter {#plot-trotter-simulation-results}
...
### Graficar los resultados de extrapolación para qubits individuales {#plot-extrapolation-results-for-individual-qubits}
...
Si este trabajo te ha parecido interesante, es posible que te interese el siguiente material:
- Un [tutorial](/tutorials/combine-error-mitigation-techniques) enfocado en combinar técnicas de mitigación de errores.
- [Documentación](/guides/error-mitigation-and-suppression-techniques) detallada sobre las técnicas de mitigación de errores disponibles en Qiskit.
- Lecciones adicionales que cubren experimentos a escala de utilidad: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) y [Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
