---
title: "Optimización dinámica de portafolios con el Optimizador de Portafolios de Global Data Quantum"
sidebar_label: "Optimización dinámica de portafolios con el Optimizador de Portafolios de Global Data Quantum"
description: "Resuelve un problema de optimización dinámica a escala de utilidad utilizando el Optimizador Cuántico de Portafolios de Global Data Quantum."
...
La ejecución cuántica se realiza en el backend `ibm_torino`. Para una explicación detallada de la formulación del problema, la metodología y la evaluación del rendimiento, consulta el manuscrito publicado [\[1\]](#references).
...
## Configuración {#setup}
Para utilizar el Optimizador Cuántico de Portafolios, selecciona la función a través del Catálogo de Funciones de Qiskit. Necesitas una cuenta del Plan Premium o Plan Flex de IBM Quantum con una licencia de Global Data Quantum para ejecutar esta función.

Primero, autentícate con tu [clave de API](https://quantum.cloud.ibm.com). Luego, carga la función deseada desde el Catálogo de Funciones de Qiskit. Aquí, accedes a la función `quantum_portfolio_optimizer` del catálogo utilizando la clase `QiskitFunctionsCatalog`. Esta función nos permite utilizar el solucionador predefinido de Optimización Cuántica de Portafolios.
...
Recomendamos ejecutar la siguiente celda para verificar que los datos se han descargado correctamente de la API de Yahoo Finance. Si la descarga falla, una notificación indicará que los datos se recuperarán en su lugar de una base de datos en línea alternativa.
</Admonition>
...
El diccionario `ansatz_settings` configura el ansatz del circuito cuántico. El parámetro `ansatz` especifica el uso del enfoque `"optimized_real_amplitudes"`, que es un ansatz eficiente en hardware diseñado para problemas de optimización financiera. La opción `multiple_passmanager`, que permite el uso de múltiples gestores de pases (incluyendo el gestor de pases local predeterminado de Qiskit y el servicio de transpilación impulsado por IA de Qiskit) para mejorar el rendimiento y la eficiencia de la optimización de circuitos, está actualmente desactivada porque esta función se encuentra temporalmente inhabilitada.
