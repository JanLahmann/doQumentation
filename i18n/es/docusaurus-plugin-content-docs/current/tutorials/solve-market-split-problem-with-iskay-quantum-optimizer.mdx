...
*Estimación de uso: 20 segundos en un procesador Heron r2. (NOTA: Esto es solo una estimación. Tu tiempo de ejecución puede variar.)*
...
También necesitarás obtener acceso a la [función Iskay Quantum Optimizer](https://quantum.cloud.ibm.com/functions?id=kipu-quantum-iskay-quantum-optimizer) del Catálogo de Qiskit Functions.
## Configuración {#setup}
...
Define tus credenciales de [IBM Quantum&reg; Platform](https://quantum.cloud.ibm.com/). Necesitarás:
- **Token de API**: Tu clave de API de 44 caracteres de IBM Quantum Platform
- **CRN de instancia**: Tu identificador de instancia de IBM Cloud&reg;
...
- **Selección de backend**: Elige entre las QPUs disponibles (por ejemplo, `"ibm_fez"`) según tus necesidades e instancia de recursos de cómputo.
...
**Análisis de ventas:**
- Compara las ventas objetivo versus las reales para cada producto
...
- Considera ajustar los parámetros del optimizador:
  - Aumenta `num_iterations` para más pasadas de optimización
  - Aumenta `postprocessing_level` para más refinamiento clásico
  - Aumenta `shots` para mejores estadísticas de medición
...
Para profundizar tu comprensión y explorar más:

1. **Probar diferentes instancias**: Experimenta con otras instancias de QOBLIB de tamaños variados
...
4. **Probar diferentes estrategias**: Intenta encontrar una mejor codificación para el problema o formúlalo como HUBO (si es posible)
5. **Aplicar a tu dominio**: Adapta las técnicas de formulación QUBO/HUBO a tus propios problemas de optimización
