...
*Estimación de uso: 20 minutos en un Heron r2 (NOTA: Esto es solo una estimación. Tu tiempo de ejecución puede variar.)*
...
Este tutorial demuestra cómo implementar el Algoritmo de Diagonalización Cuántica de Krylov (KQD) dentro del contexto de los patrones de Qiskit. Primero aprenderás sobre la teoría detrás del algoritmo y luego verás una demostración de su ejecución en una QPU.
...
### El espacio de Krylov {#the-krylov-space}
...
Consulta el Apéndice para una derivación detallada de cómo el espacio de Krylov unitario permite representar los estados de baja energía con precisión.
### Algoritmo de diagonalización cuántica de Krylov {#krylov-quantum-diagonalization-algorithm}
...
#### Establecer los parámetros del algoritmo {#set-parameters-for-the-algorithm}
...
Selecciona un estado de referencia $\vert \psi \rangle$ que tenga cierto solapamiento con el estado fundamental. Para este Hamiltoniano, utilizamos un estado con una excitación en el qubit central $\vert 00..010...00 \rangle$ como nuestro estado de referencia.
...
#### Prueba de Hadamard {#hadamard-test}
...
### Prueba de Hadamard eficiente {#efficient-hadamard-test}
Podemos optimizar los circuitos profundos para la prueba de Hadamard que hemos obtenido introduciendo algunas aproximaciones y basándonos en ciertas suposiciones sobre el Hamiltoniano del modelo. Por ejemplo, considera el siguiente circuito para la prueba de Hadamard:
...
### Utilizar un circuito optimizado para la preparación del estado {#use-an-optimized-circuit-for-state-preparation}
...
### Circuitos plantilla para calcular los elementos de la matriz de $\tilde{S}$ y $\tilde{H}$ mediante la prueba de Hadamard {#template-circuits-for-calculating-matrix-elements-of-tildes-and-tildeh-via-hadamard-test}
...
### Transpilación a una QPU {#transpiling-to-a-qpu}
...
### Crear PUBs para la ejecución con Estimator {#create-pubs-for-execution-with-estimator}
...
### Ejecutar circuitos {#run-circuits}
...
Ejecutar circuitos para $S$ y $\tilde{H}$ con Estimator
...
## Apéndice: Subespacio de Krylov a partir de evoluciones temporales reales {#appendix-krylov-subspace-from-real-time-evolutions}
...
para algún paso temporal $dt$ que determinaremos más adelante. Supón temporalmente que $r$ es par: entonces define $d=r/2$. Observa que cuando proyectamos el Hamiltoniano en el espacio de Krylov anterior, es indistinguible del espacio de Krylov
...
Para simplificar lo que queda, observa que para todos estos $E_k$, por la definición de $f$ sabemos que $f(E_k)^2 \le 4\left(1 + \delta\right)^{-2d}$. Adicionalmente, acotando superiormente $E_k-E_0<2\|H\|$ y acotando superiormente $\sum_{E_k>E_0+\delta}|\gamma_k|^2<1$ obtenemos
...
Se puede realizar un análisis similar al anterior que además tenga en cuenta el ruido y el procedimiento de umbralización discutido en el cuaderno. Consulta [\[2\]](#references) y [\[4\]](#references) para este análisis.

## Apéndice: demostración de la Afirmación 1 {#appendix-proof-of-claim-1}
