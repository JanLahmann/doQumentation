...
* Cómo implementar una compuerta CNOT de largo alcance utilizando circuitos dinámicos con mediciones a mitad de circuito (MCM) y retroalimentación clásica;
* Cómo implementar la compuerta equivalente utilizando un enfoque unitario basado en SWAP;
* Cómo comparar ambos enfoques midiendo la fidelidad de la compuerta en función de la distancia entre qubits.

## Requisitos previos {#prerequisites}

Sugerimos que los usuarios estén familiarizados con los siguientes temas antes de realizar este tutorial:
* [Conceptos básicos de computación cuántica](/learning/courses/basics-of-quantum-information), incluyendo estados de Bell, entrelazamiento y compuertas cuánticas;
* Familiaridad con los [circuitos dinámicos](/guides/classical-feedforward-and-control-flow) (mediciones a mitad de circuito y retroalimentación clásica);
* Conocimiento básico del [SDK de Qiskit](https://docs.quantum.ibm.com/guides) y [IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime), y acceso a una [cuenta de IBM Quantum](/guides/cloud-setup).
...
En este cuaderno, adaptamos el protocolo para hardware de IBM Quantum y evaluamos su rendimiento en función de la separación entre control y objetivo, comparándolo con una línea de base unitaria basada en SWAP.
...
## Ejemplo a pequeña escala en simulador {#small-scale-simulator-example}

Antes de ejecutar en el QPU real, verificamos que tanto los circuitos dinámicos como los unitarios produzcan un estado de Bell ideal en un simulador sin ruido. Usamos el `Sampler` de IBM Quantum con `AerSimulator` como modo de backend, con una distancia pequeña de 6.
...
#### Medir pares de qubits vecinos en la base de Bell {#measure-neighboring-qubit-pairs-in-the-bell-basis}
...
#### Aplicar correcciones de retroalimentación para corregir los operadores de subproducto de Pauli {#apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
Combinamos los diversos pasos definidos anteriormente para crear una compuerta CX de largo alcance en los dos extremos de una línea unidimensional (1D). Los pasos incluyen lo siguiente:
- Inicializar el qubit de control en $|+\rangle$
...
El circuito en el centro puede implementarse de la siguiente manera:
...
Ahora construimos los circuitos que miden en las bases $XX$, $YY$ y $ZZ$, tal como hicimos anteriormente para los circuitos dinámicos.
...
Ahora que hemos construido tanto los circuitos dinámicos como los unitarios para un ejemplo a pequeña escala con `distance=6`, los transpilamos para ejecutarlos primero en un simulador sin ruido.
...
Ahora podemos ejecutar el experimento en el backend del simulador sin ruido. Usamos el Sampler de Qiskit Runtime con AerSimulator como modo de backend para ejecutar los circuitos.
...
En una simulación sin ruido, verificaremos que la métrica de fidelidad sea $1$ para los circuitos construidos. En los experimentos en los QPU reales, este análisis proporcionará una imagen clara de qué tan bien se desempeñan los circuitos dinámicos en relación con la implementación de referencia unitaria.

#### Métricas de calidad {#quality-metrics}

Para evaluar el éxito del protocolo CX de largo alcance, medimos qué tan cerca está el estado de salida del estado de Bell ideal. Una forma conveniente de cuantificar esto es calculando la fidelidad del estado utilizando valores esperados de operadores de Pauli. Podemos calcular la fidelidad para un estado de Bell en el estado de control y objetivo después de conocer $\braket{XX}$, $\braket{YY}$ y $\braket{ZZ}$. En particular,
...
Como se esperaba en una simulación sin ruido, las fidelidades tanto en los circuitos dinámicos como en los unitarios son $1$.

## Ejemplo a gran escala en hardware {#large-scale-hardware-example}

Aquí reunimos todos estos detalles en un único flujo de trabajo a mayor escala, que luego se ejecuta en hardware cuántico real.

### Generar circuitos para diferentes distancias {#generate-circuits-for-different-distances}

Ahora generamos circuitos CX de largo alcance para un rango de separaciones entre qubits de hasta 60 qubits de distancia. Para cada distancia, construimos circuitos que miden en las bases $XX$, $YY$ y $ZZ$, que se utilizarán posteriormente para calcular las fidelidades.
...
Ahora que tenemos circuitos tanto dinámicos como unitarios para un rango de distancias, estamos listos para la transpilación. Primero necesitamos seleccionar un dispositivo backend.
...
El siguiente paso asegura que el backend admita la instrucción `if_else`, que es necesaria para la versión más nueva de los circuitos dinámicos. Dado que esta función todavía está en acceso anticipado, agregamos explícitamente el `IfElseOp` al target del backend si aún no está disponible.
...
Como queremos comparar el rendimiento de los circuitos dinámicos y unitarios en una cadena 1D, usamos la cadena de Layer Fidelity para seleccionar una topología lineal de la mejor cadena de qubits del dispositivo. Esto garantiza que ambos tipos de circuitos sean transpilados bajo las mismas restricciones de conectividad, lo que permite una comparación justa de su rendimiento.
...
Para apreciar los resultados visualmente, la siguiente celda grafica las fidelidades estimadas de la compuerta medidas a distancias variables entre qubits entrelazados para cada método.
...
Si encontraste este trabajo interesante, puede que te interesen los siguientes materiales:
