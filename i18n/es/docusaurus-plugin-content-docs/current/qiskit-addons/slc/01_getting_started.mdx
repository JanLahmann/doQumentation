...
|-----------|-----------|-----------|
| Twirling de Pauli de capas de puertas de dos qubits | Repetir pares de identidad de capas y aprender el ruido | Derivar una fidelidad (error para cada canal de ruido) |
...
### Flujo de trabajo para el sombreado de conos de luz, el aprendizaje de ruido y la inyección anti-ruido {#workflow-for-lightcone-shading-noise-learning-and-anti-noise-injection-1}
...
La función `compute_forward_bounds` evalúa las relaciones de conmutación entre las puertas de cada capa y los términos de Pauli generados anteriormente, en términos de cómo los errores de propagación hacia adelante afectan el observable deseado $A$. Para las puertas que conmutan con los términos de Pauli, no se realiza ninguna acción. Para las puertas de Clifford, se desplazan hacia el inicio del circuito. Para las puertas no Clifford, aproximamos su influencia sobre los observables objetivo para ser priorizadas posteriormente en la cancelación de ruido (una vez que se hayan fusionado todas las cotas). Esta cota se obtiene aplicando primero la norma L2 (es decir, la raíz cuadrada de la suma de los cuadrados de los coeficientes de los términos de Pauli relevantes). Cuando hay demasiados términos de qubits involucrados, recurrimos a una cota más laxa que utiliza la desigualdad triangular.
...
Al calcular las cotas hacia atrás, podemos ver cómo la estructura del estado inicial rige el comportamiento temprano de la propagación de errores:
...
#### Vista previa de las cotas combinadas sin tasas de ruido aprendidas {#preview-merged-bounds-without-learned-noise-rates}

La función `merged_bounds` determina el punto del circuito donde cambiar de cotas hacia atrás a cotas hacia adelante minimiza el sesgo total estimado en el observable deseado. Este sesgo se calcula como la suma de las contribuciones de las cotas hacia atrás para todas las ubicaciones de ruido antes de ese punto, más las contribuciones de las cotas hacia adelante para todas las ubicaciones de ruido después de él. Actualmente, esto se hace de manera uniforme para todos los qubits.

> **Nota importante**: El punto para cambiar de cotas hacia adelante a cotas hacia atrás depende de las tasas de ruido aprendidas.
...
Tras combinar las cotas hacia atrás y las cotas hacia adelante ajustadas, el comportamiento de los SLCs combinados se vuelve claro:

- La función anterior nos indica que se elige una partición en la que se produce el cambio de las cotas hacia atrás a las cotas hacia adelante ajustadas.

- Podemos ver a continuación que los SLCs ahora contienen cotas parciales hacia atrás y cotas parciales hacia adelante ajustadas.
...
#### Recursos a nivel de clúster {#cluster-level-resources}
...
### b.i. Actualizar las cotas fusionadas con las tasas de ruido aprendidas reales {#bi-update-merged-bounds-with-actual-learned-noise-rates}

Ahora que se ha aprendido el modelo de ruido específico, podemos aplicar las tasas de ruido aprendidas a las cotas de ruido predichas y obtener una determinación final de qué cotas tienen mayor impacto en la minimización del sesgo.
...
- Las cotas de error también disminuyen significativamente más rápido para PEC + SLC que para PEC solo.
