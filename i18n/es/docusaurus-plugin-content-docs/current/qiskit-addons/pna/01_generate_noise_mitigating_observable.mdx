...
![Noisy experiment](/img/qiskit-addons/images/noisy_expt.png)
Si aprendemos el modelo de ruido, podemos aplicar su inverso y cancelar el ruido.
![Noise-mitigated experiment](/img/qiskit-addons/images/noise_mitigated_expt.png)
...
![PNA overview](/img/qiskit-addons/images/pna_overview.png)
...
Antes de ejecutar los experimentos, aprendemos el modelo de ruido que afecta a las compuertas de entrelazamiento y las mediciones del circuito. Contar con un modelo de ruido preciso es necesario para mitigar los errores de forma eficaz. Aprender el ruido justo antes de ejecutar los experimentos ofrece la mejor oportunidad de que el modelo de ruido describa fielmente el ruido real que afecta a las compuertas durante la ejecución.
...
- `post_selection`: Usaremos post-selección basada en aristas durante el aprendizaje utilizando compuertas `rx` para implementar los pulsos post-medición
...
Aquí creamos un mapeo entre los IDs de referencia de ``InjectNoise`` de cada caja y el modelo de ruido aprendido (`PauliLindbladMap`) que afecta a las compuertas de entrelazamiento en esa caja.
...
- `num_to_measure`: Aunque esta variable no es una entrada de ``generate_noise_mitigating_observable``, la usamos para controlar cuántos términos de $\tilde{O}$ queremos medir realmente. Aquí solo mediremos los 30 términos principales, que son los términos originales de nuestro observable. Los términos han sido reescalados de modo que medirlos tenga el efecto de mitigar el ruido de compuertas aprendido. Aunque solo medimos 30 términos de $\tilde{O}$, a menudo sigue siendo útil permitir que crezca, ya que eso aumenta la precisión de los factores de escala de los términos principales.
...
- ``template_circuit``: El circuito que contiene todas las compuertas necesarias para implementar todas las aleatorizaciones deseadas (de las aleatorizaciones de twirling, parámetros, etc.).
