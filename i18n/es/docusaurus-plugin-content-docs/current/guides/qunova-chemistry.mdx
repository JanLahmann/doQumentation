...
Luego, autentícate con tu [clave API de IBM Quantum&reg;](http://quantum.cloud.ibm.com/) y, suponiendo que ya hayas [guardado tu cuenta](/guides/functions-get-started#install-qiskit-functions-catalog-client) en tu entorno local, selecciona la Función de Qiskit de la siguiente manera:
...
#### Definir la geometría molecular y las opciones {#define-the-molecular-geometry-and-options}
...
Verifica el [estado](/guides/functions-get-started#check-job-status) de tu carga de trabajo de Función de Qiskit u obtén los [resultados](/guides/functions-get-started#retrieve-results) de la siguiente manera:
...
Envía un correo electrónico a [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) para consultas sobre licencias.

### Ejemplo de uso de función con licencia {#example-of-licensed-function-use}

A los usuarios con licencia se les emite un token de licencia, y luego deben usar una biblioteca de envoltura (wrapper) para enviar su token de licencia a la función.
...
El siguiente ejemplo muestra cómo se debe usar esta biblioteca para enviar tu token de licencia al llamar a la función.
...
#### Curva de superficie de energía potencial de disociación para una molécula de Li2S con 24 qubits {#dissociation-potential-energy-surface-curve-for-an-li2s-molecule-with-24-qubits}
...
#### Curva PES de disociación para una molécula de N2 con 40 qubits {#dissociation-pes-curve-for-an-n2-molecule-with-40-qubits}
...
#### Curva PES de disociación para el hierro(II)-porfirina pentacoordinado con un sistema NO con 44 qubits {#dissociation-pes-curve-for-five-coordinated-ironii-porphyrin-with-an-no-system-with-44-qubits}
