---
description: 'Explicación de las técnicas de mitigación y supresión de errores disponibles a través de IBM Quantum Compute Service'
...
Te recomendamos usar estas versiones o más recientes.
...
El diagrama a continuación muestra el desacoplamiento dinámico con una secuencia de pulsos XX. El circuito abstracto de la izquierda se mapea en una secuencia temporal (programación) de pulsos de microondas en la parte superior derecha. La parte inferior derecha muestra la misma programación, pero con una secuencia de dos pulsos X insertados durante un período de inactividad del primer qubit.
...
![Representación del twirling de Pauli](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)
...
### Habilitar TREX usando la opción `resilience_level` {#enable-trex-by-using-the-resilience-level-option}

Puedes habilitar TREX estableciendo el [nivel de resiliencia](/guides/estimator-noise-management#resilience) en `1`, como se muestra en el siguiente ejemplo.
...
La sobrecarga de este método escala con el número de factores de ruido. La configuración predeterminada muestrea el valor esperado en tres factores de ruido, lo que lleva a una sobrecarga aproximada de 3x.

### Habilitar ZNE usando la opción `zne_mitigation` {#enable-zne-by-using-the-zne-mitigation-option}

ZNE puede habilitarse estableciendo `zne_mitigation` en `True` en las [opciones de resiliencia de IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) para el Estimator.
Las opciones para ZNE se describen [aquí](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options). Las siguientes opciones son destacables:
...
### Habilitar ZNE usando la opción `resilience_level` {#enable-zne-by-using-the-resilience-level-option}

Puedes habilitar ZNE (junto con TREX y el twirling de puertas) estableciendo el [nivel de resiliencia](/guides/estimator-noise-management#resilience) en `2`, como se muestra en el siguiente ejemplo.
...
La cancelación probabilística de errores (PEC) es una técnica para mitigar errores en la estimación de valores esperados de observables. A diferencia de ZNE, devuelve una estimación insesgada del valor esperado. Sin embargo, generalmente conlleva una mayor sobrecarga.
...
- [Configura la gestión de ruido con Estimator](/guides/estimator-noise-management).

- [Configura la gestión de ruido con Sampler](/guides/sampler-noise-management).

- Explora otras [opciones](/guides/runtime-options-overview) para los primitivos de IBM Quantum.
