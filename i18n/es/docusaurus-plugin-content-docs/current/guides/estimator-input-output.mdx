...
El `parameter values` opcional puede ser una lista o un solo parámetro. Los elementos de observables y parameter values se combinan siguiendo las reglas de difusión (broadcasting) de NumPy tal como se describe en el tema [Entradas y salidas de primitivos](primitive-input-output#broadcasting-rules), y se devuelve una estimación de valor esperado para cada elemento de la forma difundida.
...
El siguiente código demuestra un conjunto de ejemplo de entradas vectorizadas para el primitivo `Estimator` y las ejecuta en un backend IBM&reg; como un único objeto `RuntimeJobV2 `.
...
Después de que uno o más PUBs son enviados a una QPU para su ejecución y un trabajo se completa exitosamente, los datos se devuelven como un objeto contenedor [`PrimitiveResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PrimitiveResult) al que se accede llamando al método `RuntimeJobV2.result()`.
