...
## Construye, edita e inspecciona circuitos cuánticos {#build-edit-and-inspect-quantum-circuits}
...
### 1. Abre IBM Quantum Composer {#1-open-ibm-quantum-composer}
...
### 2. Construye tu circuito con arrastrar y soltar {#2-build-your-circuit-with-drag-and-drop}
...
#### Selecciona múltiples operaciones {#select-multiple-operations}
...
#### Construye una operación personalizada usando la función de grupo {#build-a-custom-operation-using-the-group-feature}
...
#### Desagrupa una operación personalizada o predefinida {#ungroup-a-custom-or-predefined-operation}
...
#### Expande la definición de una operación {#expand-an-operations-definition}
...
#### Renombra o elimina una operación personalizada {#rename-or-delete-a-custom-operation}
...
#### Agrega o quita registros {#add-or-remove-registers}
...
#### Agrega una condicional {#add-a-conditional}
...
#### Agrega un modificador de control {#add-a-control-modifier}
...
#### Visualiza con discos de fase a lo largo de tu circuito {#visualize-with-phase-disks-throughout-your-circuit}
...
Lee más sobre la [visualización de disco de fase](#phase-disk).

#### Exporta una imagen del circuito {#export-a-circuit-image}
...
### 3. Construye tu circuito con código OpenQASM {#3-build-your-circuit-with-openqasm-code}
...
### 4. Inspecciona tu circuito, paso a paso {#4-inspect-your-circuit-step-by-step}
...
* Para inspeccionar solo algunas operaciones, haz clic en las operaciones que quieres inspeccionar, y aparecerá una capa de color sobre cada una, indicando que se incluirán cuando ejecutes el modo Inspeccionar. Para deseleccionar una operación, haz clic en ella nuevamente, y la capa desaparece.
...
## Ejecuta circuitos y visualiza resultados {#run-circuits-and-view-results}

Sigue los pasos a continuación para ejecutar circuitos cuánticos en QPUs y ver los resultados.

### Elige la configuración de tu trabajo {#choose-your-job-settings}
...
### Haz clic en "Ejecutar en (nombre del QPU)" {#click-run-on-qpu-name}
...
### Ve los resultados {#view-results}
...
### Ve las visualizaciones {#view-visualizations}
...
### Descarga visualizaciones {#download-visualizations}
...
![un circuito cuántico](/docs/images/guides/composer/2H-circuit.svg "Un circuito que consiste en una columna de puertas Hadamard que crea una superposición igual de los estados de base computacional, seguida por una puerta Z controlada de dos qubits (CZ).")

![Vista de probabilidades del circuito anterior](/docs/images/guides/composer/probabilities.svg "Vista de probabilidades del circuito anterior.")
...
### Vista de q-esfera {#q-sphere-view}
...
![un circuito cuántico](/docs/images/guides/composer/2H-circuit.svg)

![Vista de q-esfera](/docs/images/guides/composer/q-sphere.avif)

Puedes seleccionar, mantener pulsado y arrastrar para rotar la q-esfera. Para devolver la q-esfera a su orientación predeterminada, selecciona el **botón de rebobinado** en la esquina superior derecha de la q-esfera.

<Admonition type="note" title="¿Cuál es la diferencia entre una esfera de Bloch y una q-esfera?">
Es importante destacar que la q-esfera **no** es lo mismo que la esfera de Bloch, ni siquiera para un solo qubit. De hecho, al igual que el [disco de fase](#phase-disk), la esfera de Bloch proporciona una vista *local* del estado cuántico, donde cada qubit se ve por su cuenta. Cuando intentas entender cómo se comportan los registros de qubits (estados de múltiples qubits) ante la aplicación de circuitos cuánticos, resulta más revelador tomar una vista *global* y examinar el estado cuántico en su totalidad. La q-esfera proporciona una representación visual del estado cuántico, y así este punto de vista *global*. Por lo tanto, cuando exploras aplicaciones cuánticas y algoritmos en pequeños números de qubits, la q-esfera debería ser el método de visualización principal.
</Admonition>

### Vista de vector de estado {#statevector-view}
...
![Vista de vector de estado](/docs/images/guides/composer/state-vector.svg "El vector de estado al final del circuito anterior. La rueda de colores mapea el ángulo de fase al color. El estado de salida se expresa como una lista de números complejos.")
...
## Glosario de operaciones de Composer {#composer-operations-glossary}
...
### Puertas clásicas {#classical-gates}
...
La puerta Toffoli junto con la puerta Hadamard forman un conjunto universal de puertas para la computación cuántica.
...
La puerta de fase (anteriormente llamada puerta U1) aplica una fase de $e^{i\theta}$ al estado $\left|1\right\rangle$. Para ciertos valores de $\theta$, es equivalente a otras puertas. Por ejemplo, P($\pi$)=Z, P($\pi$/$2$)=S, y P($\pi/4$)=T. Salvo una fase global de $e^{i\theta/2}$, es equivalente a RZ($\theta$).
...
### Puerta Hadamard {#hadamard-gate}

**Puerta H**

La puerta H, o puerta Hadamard, rota los estados $\left|0\right\rangle$ y $\left|1\right\rangle$ a $\left|+\right\rangle$ y $\left|-\right\rangle$, respectivamente. Es útil para hacer superposiciones. Si tienes un conjunto universal de puertas en una computadora clásica y agregas la puerta Hadamard, se convierte en un conjunto universal de puertas en una computadora cuántica.
...
La puerta Pauli Y es equivalente a Ry para el ángulo $\pi$. Es equivalente a aplicar X y Z, salvo un factor de fase global.
...
La puerta Toffoli simplificada implementa la puerta Toffoli salvo fases relativas. Ten en cuenta que el Toffoli simplificado no es equivalente al Toffoli, pero se puede usar en lugares donde la puerta Toffoli se descomputa nuevamente.
