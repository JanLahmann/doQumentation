...
<Admonition type="tip">
**Qiskit Serverless está siendo actualizado, y sus funcionalidades están cambiando rápidamente.** Durante esta fase de desarrollo, encuentra las notas de la versión y la documentación más reciente en la página de [Qiskit Serverless en GitHub](https://qiskit.github.io/qiskit-serverless/index.html).
</Admonition>
...
Para tareas clásicas que pueden paralelizarse, usa el decorador `@distribute_task` para definir los requisitos de cómputo necesarios para realizar una tarea. Comienza recordando el ejemplo `transpile_remote.py` del tema [Escribe tu primer programa con Qiskit Serverless](/guides/serverless-first-program) con el siguiente código.
...
A continuación se muestra cómo subir archivos. Asegúrate de haberte autenticado en Qiskit Serverless con tu cuenta de IBM Quantum (consulta [Subir a Qiskit Serverless](/guides/serverless-first-program#upload-to-qiskit-serverless) para obtener instrucciones).
...
- Consulta un ejemplo completo que [migra código existente a Qiskit Serverless](/guides/serverless-port-code).
