...
Para convertir un circuito abstracto en un circuito ISA que pueda ejecutarse en un procesador específico, el transpilador necesita cierta información sobre el procesador. Por lo general, esta información se almacena en el [`Backend`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.Backend#backend) o el [`Target`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.Target#target) proporcionados al transpilador, y no se necesita información adicional. Sin embargo, también puedes proporcionar información explícitamente para que la use el transpilador; por ejemplo, si tienes un caso de uso específico o si crees que dicha información ayudará al transpilador a generar un circuito más optimizado.
