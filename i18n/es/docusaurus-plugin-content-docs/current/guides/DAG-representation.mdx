...
Esta guía demuestra cómo trabajar con DAGs y usarlos para escribir pases del transpilador personalizados. Comenzará construyendo un circuito simple y examinando su representación DAG; después explorará las operaciones básicas de DAG e implementará un pase `BasicMapper` personalizado.
...
En este pase específico, el DAG se recorre capa por capa (donde cada capa contiene operaciones que actúan sobre conjuntos disjuntos de qubits y, por lo tanto, pueden ejecutarse de forma independiente). Para cada operación, si no se cumplen las restricciones del mapa de acoplamiento, se identifica una ruta de intercambio adecuada y se insertan los intercambios necesarios para hacer adyacentes los qubits implicados.
...
- Revisa la guía sobre cómo crear un [pase del transpilador personalizado](/guides/custom-transpiler-pass)
- Aprende a [crear backends personalizados y a transpilar para ellos](/guides/custom-backend)
