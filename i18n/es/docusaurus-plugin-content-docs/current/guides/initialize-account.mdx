---
description: 'Cómo inicializar tu cuenta del Servicio de Cómputo Cuántico de IBM, instanciar tu cuenta y cargar credenciales guardadas.'
title: 'Inicializa tu cuenta del Servicio de Cómputo Cuántico de IBM'
---

{/* doqumentation-source-hash: ab5dc076 */}

# Inicializa tu cuenta del Servicio de Cómputo Cuántico de IBM {#initialize-your-ibm-quantum-compute-service-account}

Antes de usar el Servicio de Cómputo Cuántico de IBM Quantum&reg;, debes inicializar (instanciar) tu cuenta enviando (cargando) las credenciales. Estas credenciales pueden enviarse manualmente cada vez que inicialices el Servicio de Cómputo Cuántico de IBM, o puedes guardarlas para reutilizarlas.
...
1. Asegúrate de estar trabajando en un entorno Python activo con el [Qiskit SDK y `qiskit-ibm-runtime` instalados](/guides/install-qiskit#local).
...
## Conecta Qiskit con tu instancia del Servicio de Cómputo Cuántico de IBM {#connect-qiskit-with-your-ibm-quantum-compute-service-instance}
...
A continuación se muestra el código básico para conectar Qiskit con tu instancia de Quantum Compute. Sin embargo, hay varias formas de personalizar las opciones de `QiskitRuntimeService` según tus necesidades. Estas opciones se describen en las secciones siguientes.
...
La forma más rápida de poner en marcha una instancia de `QiskitRuntimeService` es el método de instanciación directa: proporcionar explícitamente el token de API (clave) y el CRN (identificador de instancia) cada vez que necesites inicializar IBM Quantum Compute Service. Consulta [Encuentra tus credenciales de acceso](#access-credentials) si es necesario.
...
3. Si tu cuenta tiene múltiples instancias que pueden acceder al QPU solicitado, el sistema usa tus preferencias de plan especificadas para elegir un plan e instancia. Para `qiskit-ibm-runtime` v0.42 y versiones posteriores, los planes gratuitos tienen prioridad de forma predeterminada.
...
### Pruebas locales con `qiskit-ibm-runtime` {#local-testing-with-qiskit-ibm-runtime}
