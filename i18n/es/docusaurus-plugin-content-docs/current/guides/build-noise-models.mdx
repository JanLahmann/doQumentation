...
`NoiseModel` no soporta la adición de errores cuánticos de qubits no locales. Deben ser manejados fuera de `NoiseModel`. Eso sugiere que deberías [escribir tu propio pase del transpilador](./custom-transpiler-pass) (`TransformationPass`) para insertar estos errores en tu circuito, y ejecutarlo justo antes de usar el simulador.
