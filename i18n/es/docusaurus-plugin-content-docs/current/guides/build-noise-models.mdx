---
description: 'Aprende a construir modelos de ruido para la gestión de errores.'
...
## Inicializar un modelo de ruido desde un backend {#initialize-a-noise-model-from-a-backend}
...
El backend simulado `FakeSherbrooke` de `qiskit_ibm_runtime` se usa en estos ejemplos, pero puedes probarlo con cualquier backend real o simulado compatible con Qiskit.
</Admonition>
...
### Combinar errores cuánticos {#combine-quantum-errors}
...
### Convertir de y hacia operadores QuantumChannel {#convert-to-and-from-quantumchannel-operators}
...
### 1. Error cuántico de todos los qubits {#1-all-qubit-quantum-error}
...
### 2. Error cuántico de qubit específico {#2-specific-qubit-quantum-error}
...
`NoiseModel` no soporta la adición de errores cuánticos de qubits no locales. Deben ser manejados fuera de `NoiseModel`. Eso sugiere que deberías [escribir tu propio pase del transpilador](/guides/custom-transpiler-pass) (`TransformationPass`) para insertar estos errores en tu circuito, y ejecutarlo justo antes de usar el simulador.
...
<Admonition type="tip" title="Recomendaciones">
  - Para simular circuitos ruidosos, consulta [Simulación exacta y ruidosa con primitivas de Qiskit Aer](/guides/simulate-with-qiskit-sdk-primitives).
