...
Puedes establecer opciones al inicializar el Estimator, después de inicializarlo, o puedes actualizar las opciones una vez que el Estimator ha sido inicializado. Para instrucciones sobre cómo usar estas técnicas, consulta el tema [Introducción a las opciones](/guides/runtime-options-overview#options-precedence).

Además, puedes establecer el valor `precision` en el método `run()`, como se describe en la siguiente sección.
...
<Tabs>
  ...
  Puede no funcionar cuando se usan gates personalizados.
  </TabItem>
  <TabItem value="Twirling" label="Gate twirling">
  Incompatible con fractional gates o con stretches.
...
- Revisa el guía [Introducción a las opciones](/guides/runtime-options-overview).
