...
- `<overhead por sub-trabajo>` es un overhead de aproximadamente 2 segundos por sub-trabajo. Esto incluye operaciones como cargar el payload en la electrónica de control. Tu trabajo de primitiva puede dividirse en varios sub-trabajos si es demasiado grande para que el motor de ejecución lo procese de una sola vez.
...
- `<número de ejecuciones>` es el número total de circuitos multiplicado por el número de shots, donde los circuitos son los generados después de aplicar broadcasting a los elementos del PUB.
