---
description: 'Cómo crear un programa de transpilación paralela y desplegarlo en IBM Quantum Platform para usarlo como servicio remoto reutilizable.'
...
Esta plantilla encapsula un flujo de trabajo para simular la evolución temporal de un estado inicial bajo un Hamiltoniano de espines definido por el usuario, y devuelve un conjunto de valores de expectación especificados usando el complemento Qiskit [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/).

Esta plantilla está estructurada como un patrón de Qiskit con los siguientes pasos:

#### 1. Recopilar entradas y mapear el problema {#1-collecting-input-and-mapping-the-problem}
...
#### 2. Preparar los circuitos generados para su ejecución {#2-prepare-the-generated-circuits-for-execution}
...
Primero, escribe una plantilla de función para simulación de Hamiltoniano que use el [complemento AQC-Tensor de Qiskit](https://qiskit.github.io/qiskit-addon-aqc-tensor/) para mapear la descripción del problema a un circuito de profundidad reducida que se pueda ejecutar en hardware.

Si descargas esta página y la ves localmente en un editor de cuadernos, verás que algunas celdas de código contienen el [comando mágico](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Este comando mágico guarda el código en `./source_files/template_hamiltonian_simulation.py`, que es la plantilla de función que puedes subir y ejecutar de forma remota con Qiskit Serverless.
...
A continuación, ejecuta la plantilla con las entradas de nivel de dominio para la simulación de Hamiltoniano. Este ejemplo especifica un modelo XXZ de 50 qubits con acoplamientos aleatorios, y un estado inicial y un observable.
...
Bloquea el resto del programa hasta que haya un resultado disponible. Una vez finalizado el trabajo, puedes recuperar los resultados. Estos incluyen la salida de nivel de dominio de la simulación de Hamiltoniano (valores de expectación) y metadatos útiles.
