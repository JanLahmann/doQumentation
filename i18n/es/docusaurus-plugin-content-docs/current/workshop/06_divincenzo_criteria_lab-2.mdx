...
## 3. Criterio 3 – Tiempo de coherencia largo (decoherencia vs tiempo de puerta) {#3-criterion-3-long-coherence-time-decoherence-vs-gate-time}

**Criterio 3:** *"Tiempos de decoherencia relevantes largos, mucho más largos que el tiempo de operación de la puerta."* Esto aborda la necesidad de que los qubits mantengan su estado cuántico el tiempo suficiente para realizar las operaciones necesarias. Cada qubit tiene un **tiempo T₁** (tiempo de relajación de energía, con qué rapidez |1〉 decae a |0〉) y un **tiempo T₂** (tiempo de desfase, con qué rapidez se pierde la coherencia de fase relativa). Para que un ordenador cuántico funcione, estas escalas de tiempo deben superar ampliamente la duración de las operaciones de puerta.

**Objetivo de la demostración:** Investigar la coherencia de los qubits en Qiskit mostrando cómo la decoherencia impacta los resultados del circuito a medida que crece la longitud de ejecución. Usaremos un backend falso con tiempos T1/T2 conocidos para simular este efecto.
...
## 4. Criterio 4 – Conjunto universal de puertas cuánticas {#4-criterion-4-universal-set-of-quantum-gates}

**Criterio 4:** *"Un conjunto 'universal' de puertas cuánticas."* Esto significa que nuestro hardware debe permitirnos realizar *cualquier* cómputo cuántico componiendo un conjunto finito de puertas básicas. En la computación clásica, NAND es universal; en cuántica, hay muchas opciones de conjuntos de puertas universales (p. ej. \{H, T, CNOT\} o las puertas nativas de una máquina determinada). Los dispositivos IBM, por ejemplo, tienen un conjunto de operaciones nativas como rotaciones arbitrarias de un solo qubit y CNOTs entre ciertos qubits, que juntas son universales. El trabajo de Qiskit es a menudo **compilar puertas de alto nivel en estas puertas base**.

**Objetivo de la demostración:** Ilustrar la universalidad de las puertas mostrando cómo Qiskit descompone puertas. Tomaremos una puerta no nativa (como una puerta Toffoli de 3 qubits, CCX) y veremos cómo se descompone en las puertas base del dispositivo. Esto demuestra que el conjunto de puertas proporcionado es efectivamente *universal* – puede producir la operación más compleja.

Primero, veamos cuáles son las puertas base de un backend IBM típico. Consultaremos la configuración de un dispositivo (o su versión falsa). Por ejemplo, las puertas base de ibmq_brisbane:
...
**Impacto en los algoritmos:** Si intentas un algoritmo más largo (con muchas puertas secuenciales), el tiempo total de ejecución podría acercarse o superar T2, causando que el estado pierda coherencia antes del final. Por eso mejorar los tiempos de coherencia y hacer las puertas más rápidas son dos de los objetivos más críticos en la investigación de hardware cuántico.
...
Esto podría generar algo como `['id', 'rz', 'sx', 'x', 'ecr']`. Estas son las operaciones primitivas que el hardware soporta de forma nativa (Identidad/sin operación, rotación RZ, puerta sqrt(X), puerta X y X controlada). Cualquier otra puerta debe componerse a partir de estas. Se sabe que este conjunto es universal para la computación cuántica (esencialmente rotaciones de un solo qubit más una puerta de dos qubits de entrelazamiento forman un conjunto universal).

Ahora, tomemos una **puerta Toffoli (CCX)** como caso de prueba. CCX invierte un qubit objetivo solo si dos qubits de control son ambos 1. No es una puerta nativa en el hardware de IBM. Qiskit proporciona una instrucción `ccx`, pero internamente la descompondrá.
### Ejercicio 4: Descomponer una puerta Toffoli {#exercise-4-decompose-a-toffoli-gate}
Completa el código a continuación para construir un circuito con una puerta Toffoli (CCX) y luego usa Qiskit para descomponerla en las puertas base nativas del backend `FakeBrisbane`.
...
En la salida transpilada, deberías ver el CCX reemplazado por una secuencia de puertas más básicas como `rz`, `sx` y `ecr`. Esto prueba que las puertas nativas son suficientes para expresar el Toffoli.

**Universalidad en la práctica:** El ejercicio anterior muestra que una puerta compleja de 3 qubits fue construida a partir de otras más simples. En general, **cualquier** unitario de múltiples qubits puede componerse a partir de puertas de 1 y 2 qubits. El transpilador es un componente crucial de cualquier pila de software cuántico, ya que tiende el puente entre los algoritmos abstractos que queremos ejecutar y las operaciones físicas que un dispositivo cuántico específico puede realizar realmente.

**Ejemplo en dispositivo:** El dispositivo **ibmq_brisbane** usa la arquitectura Eagle con las puertas base mostradas arriba. Eso significa que cualquier algoritmo enviado a esas máquinas será convertido en secuencias de esas operaciones. Este criterio es esencialmente sobre **controlabilidad**; tenemos suficientes palancas de control para realizar cualquier operación necesaria en nuestros qubits.
...
- **Puertas universales**: transpilar operaciones complejas a las puertas base de una máquina, probando que podemos realizar cualquier cómputo.
