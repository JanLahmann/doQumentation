...
# Teletransportación cuántica {#quantum-teleportation}
...
Para lograrlo, necesitaremos conocer algunas operaciones cuánticas o "puertas cuánticas".
## Operadores cuánticos (puertas) {#quantum-operators-gates}

Siéntete libre de omitir esta sección si ya estás familiarizado con las puertas cuánticas. Si quieres entender mejor estas puertas, consulta [Fundamentos de la información cuántica](/learning/courses/basics-of-quantum-information), especialmente las dos primeras lecciones, en IBM Quantum Learning.

Para este protocolo de teletransportación utilizaremos principalmente dos tipos de puertas cuánticas: la puerta Hadamard y la puerta CNOT. Algunas otras tendrán un papel menor: la puerta $X$, la puerta $Z$ y la puerta SWAP.

Este módulo puede completarse con conocimientos muy limitados de álgebra lineal, pero a veces visualizar las puertas cuánticas mediante matrices y vectores puede ser útil. Por eso presentamos aquí también las formas matricial/vectorial de las puertas y estados cuánticos.
...
Con esta notación vectorial en mente, podemos introducir las puertas cuánticas que necesitamos, sus efectos sobre los estados cuánticos y sus formas matriciales.

__Puerta H (Hadamard):__ Crea un estado de superposición. Puerta de un solo qubit.
$$
...
Un circuito con una puerta Hadamard se construye de la siguiente manera:
...
Una puerta CNOT se ve algo diferente en un circuito, ya que requiere dos qubits. Así se implementa:
...
La mayoría de las puertas tienen la misma forma matricial en Qiskit que en cualquier otro lugar. Pero la puerta CNOT actúa sobre dos qubits, por lo que de repente el orden de los qubits se convierte en un problema. Los textos que ordenan los qubits como $|q_0,q_1,...\rangle$ mostrarán una forma matricial diferente para sus puertas CNOT. Verifica mediante multiplicación matricial explícita que la matriz CNOT anterior tiene la acción correcta sobre el estado $|01\rangle.$
...
**Puerta $X$**: Equivalente a una operación NOT. Puerta de un solo qubit.
...
En Qiskit, crear un circuito con una puerta $X$ se hace así:
...
**Puerta $Z$**: Agrega una "fase" a un estado (un prefactor que, en el caso de los autoestados $|0\rangle$ y $|1\rangle$ de la puerta Z, es 1 o -1, respectivamente). Puerta de un solo qubit.
...
En Qiskit, crear un circuito con una puerta $Z$ se hace así:
...
__Alice entrelaza los qubits A y Q:__ Alice opera ahora con una puerta CNOT con su propio qubit como control y Q como objetivo, y luego aplica una puerta Hadamard a Q. Calculemos el estado de tres qubits tras esa operación:
...
## Experimento 1: Teletransportación básica {#experiment-1-basic-teleportation}
...
- Paso 1: Transforma tu problema en un circuito cuántico
...
### Paso 1: Transforma tu problema en un circuito cuántico {#step-1-map-your-problem-to-a-quantum-circuit}

Toda la matemática que hicimos arriba fue describiendo el paso 1. Ahora lo implementaremos, ¡construyendo nuestro circuito cuántico con Qiskit! Comenzamos creando un circuito cuántico con tres qubits y entrelazando los dos qubits de Alice y Bob. Tomaremos estos como los qubits 1 y 2, y reservaremos el qubit 0 para el estado secreto.
...
Ahora debemos "transpilar" el circuito cuántico. Esto implica muchos subpasos y es un tema fascinante. Solo para dar un ejemplo de un subpaso: no todas las computadoras cuánticas pueden implementar directamente todas las puertas lógicas de Qiskit. Debemos reescribir las puertas de nuestro circuito en términos de puertas que la computadora cuántica pueda implementar. Podemos llevar a cabo ese proceso, y otros, usando un gestor de pases preconfigurado. Establecer ```optimization = 3``` (el nivel más alto de optimización) garantiza que el mapeo de nuestro circuito cuántico abstracto a las instrucciones dadas a la computadora cuántica sea tan eficiente como puede hacerlo nuestro pre-procesamiento.
...
¿Cuáles de los estados anteriores indican una teletransportación exitosa y cómo puedes saberlo?

</summary>

__Respuesta:__

Los estados $|000\rangle,$ $|001\rangle,$ $|010\rangle,$ $|011\rangle$ son todos consistentes con una teletransportación exitosa. Esto se debe a que agregamos una puerta para deshacer la preparación inicial del estado secreto. Si el estado secreto fue teletransportado correctamente al qubit de Bob, esa puerta adicional debería devolver el qubit de Bob al estado $|0\rangle.$ Así que cualquier estado anterior con el qubit de Bob (qubit 0, también medido en el componente 0 del registro clásico, y por lo tanto el más alto/más a la derecha) en el estado $|0\rangle$ indica éxito.

</details>

Este gráfico muestra todos los resultados de medición para los tres qubits, a lo largo de 5.000 pruebas o "shots". Señalamos antes que Alice mediría todos los estados posibles para los qubits A y Q con igual probabilidad. Asignamos los qubits 0-2 en el circuito a Q, A y B, en ese orden. En notación little-endian, el qubit de Bob es el más a la izquierda/inferior. Así que las cuatro barras de la izquierda corresponden al qubit de Bob en $|0\rangle$, y los otros dos qubits están en todas las combinaciones posibles con probabilidades aproximadamente iguales. Nota que casi todas (generalmente \~95%) las mediciones dan el qubit de Bob en el estado $|0\rangle$, ¡lo que significa que nuestra configuración fue exitosa! Hay un puñado de shots (\~5%) que dieron el qubit de Bob en el estado $|1\rangle.$ Eso no debería ser lógicamente posible. Sin embargo, todas las computadoras cuánticas modernas sufren de ruido y errores en una medida mucho mayor que las computadoras clásicas. Y la corrección de errores cuánticos es todavía un campo emergente.

## Experimento 2: Teletransportación a través de un procesador {#experiment-2-teleporting-across-a-processor}

Sin duda, la parte más interesante de la teletransportación cuántica es que un estado cuántico puede teletransportarse a grandes distancias de forma instantánea (aunque la comunicación clásica de las puertas adicionales no es instantánea). Como ya se dijo, no podemos romper los qubits del procesador y moverlos. Pero sí podemos mover la información de un qubit a otro, hasta que los qubits involucrados en la teletransportación estén en lados opuestos del procesador. Repitamos los pasos que dimos arriba, pero ahora haremos un circuito más grande con suficientes qubits para abarcar el procesador.

### Paso 1: Transforma tu problema en un circuito cuántico {#step-1-map-your-problem-to-a-quantum-circuit-2}
...
Puedes ver en el diagrama del circuito que los pasos lógicos son los mismos. La única diferencia es que usamos los swap gates para llevar el estado del qubit de Alice desde el qubit 6 ($A_5$) hasta el qubit 1 ($A_0$), justo al lado de Q. Y usamos swap gates para llevar el estado inicial de Bob desde el qubit 7 ($B_0$) hasta el qubit 12 ($B_5$). Nota que el estado en el qubit 12 ni siquiera está relacionado con el estado secreto de Q hasta que se realizan las mediciones en los lejanos qubits 0 y 1, y el estado en el qubit 12 no es igual al estado secreto hasta que se aplican las puertas condicionales $X$ y $Z$.

### Paso 2: Optimiza tu circuito {#step-2-optimize-your-circuit}

Normalmente, cuando usamos el gestor de pases para transpilar y optimizar nuestros circuitos, tiene sentido establecer ```optimization_level = 3```, porque queremos que nuestros circuitos sean lo más eficientes posible. En este caso, no hay razón computacional para que transfiramos estados de los qubits 6 y 7 a los qubits 1 y 12. Eso es solo algo que hicimos para demostrar la teletransportación a distancia. Si le pedimos al gestor de pases que optimice nuestro circuito, se dará cuenta de que no hay razón lógica para estos swap gates, y los eliminará y realizará las operaciones de las puertas en qubits adyacentes. Así que para este caso especial, usamos ```optimization_level = 0```.
...
- A través de una combinación de entrelazamientos y mediciones, podemos transferir un estado (que puede almacenar información) de un qubit a otro. Esta transferencia puede hacerse incluso a grandes distancias, y esto se llama teletransportación cuántica.
- La teletransportación cuántica depende de mediciones cuánticas, que son probabilísticas. Por lo tanto, puede ser necesaria comunicación clásica para ajustar los estados teletransportados. Esto impide que la teletransportación cuántica mueva información más rápido que la luz. La teletransportación cuántica no viola la relatividad ni la causalidad.
...
- Cuantas más puertas añadas en secuencia (especialmente puertas de 2 qubits), más errores y ruido puedes esperar.

### Preguntas de verdadero/falso {#truefalse-questions}

1. V/F La teletransportación cuántica puede usarse para enviar información más rápido que la luz.
...
1. Describe el protocolo de teletransportación cuántica, de principio a fin, a tu compañero/grupo. Comprueba si tienen algo que agregar o si tienen preguntas.
