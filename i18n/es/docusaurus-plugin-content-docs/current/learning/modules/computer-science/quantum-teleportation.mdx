...
Siéntete libre de omitir esta sección si ya estás familiarizado con los puertas cuánticos. Si quieres entender mejor estos puertas, consulta [Fundamentos de la información cuántica](/learning/courses/basics-of-quantum-information), especialmente las dos primeras lecciones, en IBM Quantum Learning.

Para este protocolo de teletransportación utilizaremos principalmente dos tipos de Gates cuánticos: el Gate Hadamard y el Gate CNOT. Algunos otros tendrán un papel menor: el puerta $X$, el puerta $Z$ y el puerta SWAP.

Este módulo puede completarse con conocimientos muy limitados de álgebra lineal, pero a veces visualizar los puertas mecánico-cuánticos mediante matrices y vectores puede ser útil. Por eso presentamos aquí también las formas matricial/vectorial de los puertas y estados cuánticos.
...
Con esta notación vectorial en mente, podemos introducir los puertas cuánticos que necesitamos, sus efectos sobre los estados cuánticos y sus formas matriciales.
...
Un circuito con un Gate Hadamard se construye de la siguiente manera:
...
Un Gate CNOT se ve algo diferente en un circuito, ya que requiere dos qubits. Así se implementa:
...
La mayoría de los puertas tienen la misma forma matricial en Qiskit que en cualquier otro lugar. Pero el Gate CNOT actúa sobre dos qubits, por lo que de repente el orden de los qubits se convierte en un problema. Los textos que ordenan los qubits como $|q_0,q_1,...\rangle$ mostrarán una forma matricial diferente para sus Gates CNOT. Verifica mediante multiplicación matricial explícita que la matriz CNOT anterior tiene la acción correcta sobre el estado $|01\rangle.$
...
En Qiskit, crear un circuito con un puerta $X$ se hace así:
...
**Gate $Z$**: Agrega una "fase" a un estado (un prefactor que, en el caso de los autoestados $|0\rangle$ y $|1\rangle$ del puerta Z, es 1 o -1, respectivamente). Gate de un solo qubit.
...
En Qiskit, crear un circuito con un puerta $Z$ se hace así:
...
Los estados $|000\rangle,$ $|001\rangle,$ $|010\rangle,$ $|011\rangle$ son todos consistentes con una teleportación exitosa. Esto se debe a que agregamos un puerta para deshacer la preparación inicial del estado secreto. Si el estado secreto fue teleportado correctamente al qubit de Bob, ese puerta adicional debería devolver el qubit de Bob al estado $|0\rangle.$ Así que cualquier estado anterior con el qubit de Bob (qubit 0, también medido en el componente 0 del registro clásico, y por lo tanto el más alto/más a la derecha) en el estado $|0\rangle$ indica éxito.
