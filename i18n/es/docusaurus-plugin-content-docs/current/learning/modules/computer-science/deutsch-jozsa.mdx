...
Nos gustaría saber cuál de estas funciones (1-4) es nuestra $f(x)$. De forma clásica, necesitaríamos ejecutar la función dos veces: una para $x=0$ y otra para $x=1$. Pero veamos si podemos hacerlo mejor con un circuito cuántico. Podemos aprender sobre la función con la siguiente Gate:
...
En el circuito anterior, la Gate Hadamard "H" toma el qubit 0, que inicialmente está en el estado $|0\rangle$, y lo lleva al estado de superposición $\frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)$. Luego, $U_f$ evalúa la función $f(x)$ y la aplica al qubit 1.
