...
Parte del poder de la computación cuántica proviene del "paralelismo cuántico", que es esencialmente la capacidad de realizar operaciones sobre múltiples entradas al mismo tiempo, ya que los estados de entrada de los qubits pueden estar en una superposición de múltiples estados clásicamente permitidos. Sin embargo, aunque un circuito cuántico puede evaluar múltiples estados de entrada a la vez, extraer toda esa información de una sola vez es imposible.
...
Nos gustaría saber cuál de estas funciones (1-4) es nuestra $f(x)$. De forma clásica, necesitaríamos ejecutar la función dos veces: una para $x=0$ y otra para $x=1$. Pero veamos si podemos hacerlo mejor con un circuito cuántico. Podemos aprender sobre la función con la siguiente puerta:
...
Usando Qiskit, seleccionaremos aleatoriamente una de las cuatro funciones posibles anteriores y ejecutaremos el circuito. Luego, tu tarea es usar las mediciones del circuito cuántico para aprender la función en el menor número de ejecuciones posible.
...
Comencemos cargando algunos paquetes necesarios, incluidas las primitivas de IBM Quantum. También seleccionaremos la computadora cuántica menos ocupada disponible.
...
En el circuito anterior, la puerta Hadamard "H" toma el qubit 0, que inicialmente está en el estado $|0\rangle$, y lo lleva al estado de superposición $\frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)$. Luego, $U_f$ evalúa la función $f(x)$ y la aplica al qubit 1.

A continuación, necesitamos optimizar y transpilar el circuito para ejecutarlo en la computadora cuántica:
...
Finalmente, ejecutamos nuestro circuito transpilado en la computadora cuántica y visualizamos los resultados:
...
Sin embargo, en el algoritmo cuántico de Deutsch, encontró una manera de obtener la información con una sola consulta. Hizo un ajuste al circuito de "paralelismo cuántico" anterior, preparando un estado de superposición en *ambos* qubits, en lugar de solo en el qubit 0. Luego, las dos salidas de la función, $f(0)$ y $f(1)$, interfirieron para devolver 0 si ambas eran 0 o ambas eran 1 (la función era constante), y devolvieron 1 si eran diferentes (la función era balanceada). De esta manera, Deutsch podía diferenciar entre una función constante y una balanceada con una sola consulta.

Aquí hay un diagrama de circuito del algoritmo de Deutsch:
...
## El algoritmo de Deutsch-Jozsa {#the-deutsch-jozsa-algorithm-1}
...
A continuación se muestra el diagrama del circuito del algoritmo:
...
Esta es la función oráculo, que puede ser balanceada o constante. ¿Puedes ver, mirando el circuito, si la salida del último qubit depende de los valores introducidos en los primeros $n$ qubits? Si la salida del último qubit depende de los primeros $n$ qubits, ¿puedes determinar si esa salida dependiente es balanceada o no?

Podemos saber si la función es balanceada o constante mirando el circuito anterior, pero recuerda: a efectos de este problema, tratamos la función como una "caja negra". No podemos espiar dentro de la caja para ver el diagrama del circuito. En cambio, necesitamos consultar la caja.
...
¿Cómo cambiaría tu respuesta anterior si solo tuvieras que medir hasta que un resultado (balanceado o constante) sea más probable que el otro? ¿Cuántas consultas harían falta en ese caso?
...
1. Primero, se aplica una puerta Hadamard a los $n$ qubits de entrada, y una puerta NOT más una Hadamard al qubit de salida, obteniendo el estado:
...
2. A continuación, se aplica la puerta $U_f$ a los qubits. Esta puerta tomará los primeros n qubits como entrada (que ahora están en una superposición equitativa de todas las posibles cadenas de n bits) y aplica la función $f(x)=s \cdot x$ al qubit de salida, de modo que este qubit ahora está en el estado: $ |- \oplus f(x)\rangle$. Gracias al mecanismo de retroceso de fase (phase kickback), el estado de este qubit permanece sin cambios, pero algunos de los términos en el estado del qubit de entrada adquieren un signo negativo:
...
Los instructores pueden solicitar versiones de estos notebooks con claves de respuesta y orientación sobre su integración en planes de estudio habituales completando esta [breve encuesta](https://ibm.biz/classrooms_instructor_key_request) sobre cómo se están utilizando los notebooks.
...
4. V/F El algoritmo de Deutsch-Jozsa requiere múltiples evaluaciones de la función para determinar si es constante o balanceada.
...
### Problema desafío {#challenge-problem}
1. El algoritmo de Deutsch-Jozsa: Recuerda que arriba tenías una pregunta en la que debías calcular los estados intermedios de qubits $\pi_1$ y $\pi_2$ del algoritmo de Deutsch. Haz lo mismo para los estados intermedios de $n+1$ qubits $\pi_1$ y $\pi_2$ del algoritmo de Deutsch-Jozsa, para el caso específico de $n=2$. Luego, verifica que $\pi_3 = |-\rangle \otimes \sum\limits_{x_0...x_n}(-1)^{f(x_0...x_n)}|x_0 ... x_n\rangle$, nuevamente, para el caso específico de $n=2$.
