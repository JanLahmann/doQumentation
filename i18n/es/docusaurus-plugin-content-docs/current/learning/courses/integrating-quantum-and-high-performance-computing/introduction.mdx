...
## Nueva arquitectura {#new-architecture}

IBM ha presentado una nueva arquitectura de referencia para la supercomputación cuántico‑céntrica (QCSC) que describe cómo los procesadores cuánticos (QPUs) pueden integrarse directamente con CPUs y GPUs en entornos HPC modernos. Este enfoque está impulsado por flujos de trabajo híbridos cuántico‑clásicos ya exitosos&mdash;como el uso del método de diagonalización cuántica basada en muestras (SQD) para simulaciones moleculares&mdash;que ya demuestran una precisión comparable a las técnicas clásicas avanzadas.

La arquitectura está diseñada para ser abierta, modular y componible, de modo que los recursos cuánticos puedan integrarse en los flujos de trabajo HPC existentes sin requerir una pila completamente nueva. Las primeras integraciones con sistemas como RIKEN y Fugaku demuestran que los flujos de trabajo híbridos a nivel de producción ya son factibles. Proporciona un marco a largo plazo para la evolución del co-diseño cuántico‑clásico durante la próxima década.
...
La pila incluye una capa de aplicación que descompone los problemas entre componentes clásicos y cuánticos, un middleware que vincula los modelos de programación clásica con la generación de circuitos cuánticos (por ejemplo, a través de las interfaces actualizadas de Qiskit), y una capa de orquestación del sistema que gestiona la planificación de recursos híbridos mediante herramientas como la Interfaz de Gestión de Recursos Cuánticos (QRMI).

A nivel de hardware, la arquitectura define tres niveles:
* Sistema cuántico con QPUs y runtimes clásicos de baja latencia (FPGAs/ASICs).
* Sistemas de escalado vertical CPU/GPU coubicados, conectados mediante interconexiones de tiempo cercano para tareas como la mitigación de errores.
* Sistemas de escalado horizontal que proporcionan cómputo clásico flexible para el preprocesamiento, posprocesamiento y cargas de trabajo híbridas.

En general, IBM posiciona la arquitectura QCSC como una hoja de ruta práctica para que los centros HPC comiencen a incorporar hardware cuántico real, permitiendo algoritmos híbridos más potentes y preparando la infraestructura para futuros sistemas cuánticos tolerantes a fallos. Para más información sobre esta arquitectura, consulta [este artículo](https://research.ibm.com/blog/quantum-centric-supercomputing-system-reference-architecture).
...
También se proporcionan recursos adicionales en este [repositorio de GitHub](https://github.com/christopherporter1/hpc-course-demos).
...
Los estudiantes de todos los niveles pueden encontrar útil [esta guía](/guides/slurm-plugin); cubre el plugin SPANK para la gestión de recursos cuánticos y algunas palabras sobre Slurm.
...
Al finalizar este curso, serás capaz de hacer algo más que repetir términos técnicos: comprenderás cómo gestionar un flujo de trabajo híbrido moderno que asigna subtareas específicas a un procesador cuántico mientras las CPUs y GPUs manejan el resto del trabajo. Aprenderás a escribir scripts para trabajos que transicionen sin problemas entre nodos clásicos y QPUs, a interpretar los resultados con precisión y a reconocer dónde la aceleración cuántica puede realmente mejorar los cálculos (y dónde no). Igualmente importante, practicarás mantener una mentalidad de crecimiento: en un campo nuevo y en rápida evolución, nadie lo aprende todo de una vez, y el verdadero progreso proviene de iterar, experimentar y hacer preguntas.
