...
Comenzaremos con un ejemplo de circuito `efficient_su2` de cuatro qubits con dos tipos de puertas SU(2), digamos `rx` y `y`. También especificamos un esquema de entrelazamiento y el número de repeticiones. Si simplemente haces `.draw()` de los circuitos, obtendrás una representación bastante abstracta. Se obtiene un diagrama de circuito más comprensible usando `.decompose().draw()`, y aquí usaremos `output = "mpl"'.
