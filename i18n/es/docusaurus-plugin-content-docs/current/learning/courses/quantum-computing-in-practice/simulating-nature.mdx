...
5. Optimización del circuito

6. Ejecución del circuito
...
Al simular partículas indistinguibles de fermiones o bosones, es necesario aplicar una transformación de codificación. Estas partículas se describen dentro de un marco matemático especial llamado `second quantization`, que rastrea el número de ocupación de cada estado cuántico introduciendo operadores de `creation` y `annihilation`, donde el operador de creación $\hat{a}_i^\dagger$ añade una partícula al estado $i$ mientras que el operador de aniquilación $\hat{a}_i$ elimina una partícula del estado $i$. Basándose en este marco de segunda cuantización, el fermión puede transformarse mediante Bravyi-Kitaev y Jordan-Wigner. La transformación de Jordan-Wigner define el operador de creación fermiónico
$\hat{f}_j^\dagger = \Bigl( \prod\limits_{k \lt j}{(-Z_k)} \Bigr)\Bigl( \frac{X_j + i Y_j}{2} \Bigr)$ que llena el $j$-ésimo estado cuántico con un fermión, y el operador de aniquilación fermiónico $\hat{f}_j= \Bigl( \prod\limits_{k \lt j}{(-Z_k)} \Bigr)\Bigl( \frac{X_j - i Y_j}{2} \Bigr)$ que vacía un fermión del $j$-ésimo estado. Puedes encontrar más detalles sobre esta transformación de Jordan-Wigner en nuestro [Quantum Computing in Practice, episode 5 - Mapping](/learning/courses/quantum-computing-in-practice/mapping). De manera similar, los bosones también requieren sus propios métodos de codificación, como la transformación de Holstein-Primakoff, para ser representados por qubits.
...
Además, un ansatz fácilmente preparado con un solapamiento significativo con el verdadero estado base puede servir como buen estado inicial para problemas de química, como encontrar la energía del estado base.
...
Sin embargo, construir un circuito cuántico que implemente directamente el operador completo $e^{-iHt}$ es típicamente imposible cuando nuestro Hamiltoniano es una suma de diferentes partes. Por lo tanto, necesitamos la Trotterización.
...
Un enfoque útil es aplicar de forma alterna sus exponenciales de evolución temporal durante pequeñas duraciones, $t/n$, un total de $n$ veces. En el caso de estas dos contribuciones que no conmutan, escribiríamos
...
Tras generar el circuito trotterizado, el paso de mapeo está completo y podemos proceder a la optimización del circuito. Este proceso implica varias tareas clave:

- __Establecer un diseño de qubits__ que mapee los qubits abstractos del circuito a los qubits físicos del hardware. Este paso es necesario porque la arquitectura del hardware a menudo tiene restricciones de conectividad específicas, mientras que los diseños de Circuit cuántico típicamente asumen que cualquier qubit puede interactuar con cualquier otro.
...
- __Traducir las puertas del circuito a instrucciones de la Arquitectura del Conjunto de Instrucciones (ISA)__ que el hardware pueda ejecutar directamente.

- __Realizar optimizaciones del Circuit__ para reducir la profundidad del circuito y el conteo de puertas. Esta optimización también puede aplicarse antes, en el circuito virtual antes de que los qubits sean asignados a conexiones específicas del hardware.
...
Después de completar el paso de optimización, estamos listos para ejecutar el circuito usando una primitiva. Estamos considerando un experimento de simulación cuyo objetivo es entender cómo ciertas propiedades del sistema cambian con el tiempo. Para este propósito, la primitiva Estimator es la elección más apropiada, ya que te permite medir los valores esperados de los observables que corresponden a estas propiedades.
...
Además, discutiremos cómo construir el circuito cuántico parametrizado que implementa la evolución temporal del sistema usando Trotterización. Una parte clave de este proceso es identificar las capas de entrelazamiento dentro del circuito, que desempeñan un papel crucial en la principal técnica de mitigación de errores.
...
* Crea un circuito del modelo de Ising parametrizado

* Crea capas de entrelazamiento y elimina qubits malos

* Genera una versión trotterizada del circuito
...
Es momento de construir nuestro circuito trotterizado. Como se discutió anteriormente, la Trotterización desglosa la evolución temporal en pasos discretos, por lo que debemos elegir cuántos pasos usar. Para este ejemplo, seleccionaremos seis pasos. En general, el enfoque implica equilibrar el error de Trotter —un error de aproximación introducido por el algoritmo— con los errores causados por la decoherencia. Aumentar el número de pasos de Trotter reduce el error de aproximación, pero requiere Circuits cuánticos más profundos, que son más susceptibles al ruido de decoherencia.

El circuito se definirá usando varios parámetros: el parámetro theta que representa la intensidad del campo magnético externo, los acoplamientos entre capas, el número de pasos, el número de qubits y, por supuesto, la elección del Backend del dispositivo. Dado que la magnetización del sistema depende de la intensidad del campo magnético externo, es valioso ejecutar la simulación con diferentes valores de campo magnético. Esta variación corresponde a diferentes ángulos de rotación para la puerta RX en el circuito.
...
1.	Amplificar el ruido del circuito para varios factores de ruido $λ_1$, $λ_2$, … .
2.	Ejecutar cada circuito con ruido amplificado para medir los valores esperados correspondientes $\langle A(\lambda_1)\rangle$, $\langle A(\lambda_2)\rangle$, ….
