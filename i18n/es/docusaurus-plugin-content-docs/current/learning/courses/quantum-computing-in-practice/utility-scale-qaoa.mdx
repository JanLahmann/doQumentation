...
Para escribir el problema QUBO como el Hamiltoniano de Ising, todo lo que necesitamos hacer es un simple cambio de variables de $x \in \{0, 1\}^n$ a $z \in \{-1, 1\}^n$:
...
Primero, nota que podemos reescribir nuestra expresión QUBO como la suma de términos matriciales:
...
lo cual puede reordenarse de la siguiente manera.

$$
\sum_{ij}\frac{Q_{ij}}{4}z_iz_j - \frac{Q_{ij}}{4}z_i - \frac{Q_{ij}}{4}z_j + \frac{Q_{ij}}{4}
$$

Podemos descartar el término constante porque no afectará a qué $z$ minimiza el problema QUBO. Además, podemos multiplicar esta expresión por cuatro para recuperar el valor original de $Q$, ya que esto tampoco afecta la elección óptima de $z$.

$$
\sum_{ij}Q_{ij}z_iz_j - Q_{ij}z_i - Q_{ij}z_j = z^TQz + (-\sum_{ij} Q_{ij}z_i + Q_{ij}z_j)
$$

Agrupando términos lineales similares, podemos definir nuestro vector de coeficientes lineales $b$ usando la ecuación

$$
b_i = -\sum_{j} Q_{ij} + Q_{ji}
$$

lo cual completa la transformación de variables a $z \in \{-1, 1\}^n$ tras sustituir en la ecuación anterior.
...
Al final, la minimización de la expresión QUBO es la misma que la minimización de esta expresión, donde $b$ es un coeficiente escalar real:

$$
\min_{x\in\{0,1\}^n} x^TQx\Longleftrightarrow \min_{z\in\{-1,1\}^n}z^TQz + b^Tz
$$

Reescribiendo de nuevo ligeramente, tenemos nuestro Hamiltoniano de función de costo, donde el mínimo de la expresión representa el estado base y `Z` es el operador de Pauli Z:
