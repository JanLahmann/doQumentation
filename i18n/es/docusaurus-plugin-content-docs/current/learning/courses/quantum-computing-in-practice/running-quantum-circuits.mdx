...
<IBMVideo id="134063416" title="En este video, Olivia Lanes explica circuitos cuánticos y primitivos. Cubre primitivos Estimator y Sampler de IBM Quantum."/>
...
Cada procesador puede caracterizarse mediante tres métricas de rendimiento fundamentales que describen su escala, calidad y velocidad: qubits programables, operaciones de qubits y circuitos máximos por segundo.

* **Qubits programables:** Son los qubits que los usuarios pueden controlar directamente e incorporar en algoritmos cuánticos. Esta métrica captura la escala computacional disponible para las aplicaciones.


* **Operaciones de qubits (QuOps):** Esta métrica describe cuántas de las operaciones cuánticas más exigentes computacionalmente puede ejecutar de manera confiable un procesador antes de que los errores acumulados limiten el cálculo. Sirve como medida de calidad y capacidad computacional.


* **Circuitos máximos por segundo:** Esta métrica mide el rendimiento de circuitos, o cuántos circuitos cuánticos puede ejecutar un sistema por segundo. Un mayor rendimiento permite completar más trabajo útil en una cantidad de tiempo determinada.


También puedes encontrar métricas adicionales de procesadores en documentación técnica, estudios de benchmarking e informes de rendimiento:

EPLG (Errores Por Puerta en Capas). Una medida del error promedio introducido por operaciones de puertas de entrelazamiento en capas. Un EPLG más bajo generalmente indica mayor calidad de puerta y menor acumulación de errores.

CLOPS (Operaciones de Capa de Circuito Por Segundo). Un benchmark que mide qué tan rápido puede ejecutar un sistema cuántico una carga de trabajo estandarizada que incluye tanto operaciones cuánticas como el procesamiento clásico asociado.

Estas métricas adicionales proporcionan información más detallada sobre el comportamiento del hardware, mientras que los qubits programables, las operaciones de qubits y los circuitos máximos por segundo proporcionan una vista de más alto nivel de la escala, calidad y velocidad de un sistema.
...
## Software: Qiskit y el IBM Quantum Compute Service {#software-qiskit-and-ibm-quantum-compute-service}

Para convertir su problema cuántico en instrucciones para una computadora cuántica, usará Qiskit, el kit de desarrollo de software de código abierto diseñado para trabajo en computadoras cuánticas desarrolladas por IBM. También está el Ecosistema Qiskit—una colección de tutoriales y funciones de software que construyen o extienden las funcionalidades principales de Qiskit—y Quantum Compute—un servicio de computación cuántica y modelo de programación que permite usuarios diseñar y optimizar sus cargas de trabajo cuánticas y ejecutarlas eficientemente usando primitivos IBM Quantum.
...
3.	**Ejecutar.** Los primitivos IBM Quantum proporcionan la interfaz al hardware IBM Quantum que permite que los circuitos transpilados corran. Este paso también incluye usar técnicas de supresión y mitigación de errores, que pueden ser en gran medida abstraídas del usuario.
...
A continuación, necesitaremos elegir un procesador cuántico que tenga suficientes qubits de calidad suficientemente alta como para ejecutar nuestro circuito cuántico. Toma estas decisiones guiándote por las tres métricas principales que discutimos en la sección de hardware: qubits programables, operaciones de qubits y circuitos máximos por segundo. Dependiendo de la aplicación, métricas adicionales como EPLG y CLOPS también pueden proporcionar información útil sobre la calidad de circuito y el rendimiento de ejecución esperados.
...
Ahora estamos listos para ejecutar el programa cuántico. Los primitivos IBM Quantum proporcionan una interfaz al hardware IBM Quantum, y también abstraen supresión y mitigación de errores del usuario. Hay dos primitivos para elegir: Sampler y Estimator.

[IBM Quantum Sampler](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler) ejecuta el circuito múltiples veces en un dispositivo cuántico, realizando mediciones en cada ejecución, y reconstruyendo la distribución de probabilidad de las cadenas de bits recuperadas. Cuantas más ejecuciones (o disparos) realice, más precisos serán los resultados, pero esto requiere más tiempo y recursos cuánticos. Específicamente, calcula la probabilidad de obtener cada posible estado de base estándar midiendo el estado preparado por el circuito.

[IBM Quantum Estimator](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator) usa un proceso algebraico complejo para estimar el valor esperado en un dispositivo cuántico real descomponiendo el observable en una combinación de otros observables con eigenbases conocidas.
