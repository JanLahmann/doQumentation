...
Aunque la estrategia concreta para mapear un problema a un computador cuántico depende del caso, todas persiguen en general un par de objetivos.
...
2.	¿Es nuestro problema continuo? Dado que las computadoras cuánticas son digitales, si el problema es continuo, necesitaremos encontrar una manera de discretizarlo.

3.	Por último, ¿la topología del problema se alinea con la topología del hardware? Si no, podríamos necesitar implementar algunos trucos para hacer que funcione.
...
Si eso te parece al revés (particularmente el signo entre $X$ e $iY$), consulta la nota opcional a continuación sobre las convenciones acerca de qué estado $|0\rangle$ o $|1\rangle$ corresponde a "ocupado" y cuál a "desocupado".

<Accordion>
<AccordionItem title="Notas opcionales sobre convenciones">

En muchos contextos de física no existe ambigüedad sobre la interpretación de los operadores $\sigma^\pm$; aumentan/disminuyen la proyección z del momento angular de espín de un sistema. $\sigma^+$, por ejemplo, lleva un estado de momento angular de espín $|m_s\rangle$ a uno con $|m_s+1\rangle$. Sin embargo, al aplicar un mapper para expresar operadores fermiónicos de creación y aniquilación usando estos operadores de momento angular (combinaciones de operadores de Pauli), el aumento/disminución ahora corresponde a la ocupación de un orbital electrónico. "Subir" y "bajar" no tiene ambigüedad si la acción es aumentar o disminuir la proyección del momento angular sobre un cierto eje, y la etiqueta correspondiente. En la ocupación de orbitales electrónicos, uno podría asociar $|0\rangle$ con vacío y $|1\rangle$ con ocupado. Eso podría parecer la convención obvia, y de hecho es la que se usa en los addons de Qiskit. Pero nota que es posible elegir $|0\rangle$ como "ocupado" en el sentido de un electrón relajándose al estado base de un sistema, y $|1\rangle$ como "desocupado". Es una elección libre, y una posible ambigüedad al etiquetar: ¿qué estado corresponde a un orbital ocupado? ¿$|0\rangle$ o $|1\rangle$?

Ahora está algo estandarizado en un contexto de computación cuántica, y en mappers integrados como el addon de Qiskit `FermionicMapper` se establece
$$
\begin{aligned}
|0\rangle & = \text{ vacío}\\
|1\rangle & = \text{ ocupado}
\end{aligned}
$$

De manera equivalente, un operador que crea un electrón ocupando el orbital $k$ (representado por el qubit $q_k$) debe comportarse como:

$$
O_k |q_N,...,q_k=0,...,q_0\rangle = c |q_N,...,q_k=1,...,q_0\rangle
$$

Ahora veamos las consecuencias de esa elección. Nota que en las bases estándar usadas en computación cuántica,
...
y con un trabajo similar, podemos construir el conjunto completo:
...
De esto queda claro que al elegir la convención aparentemente razonable de que $|0\rangle$ corresponda a ningún electrón, y $|1\rangle$ corresponda a un electrón, terminamos con la situación confusa de que $\sigma^+$ aparece en el operador de aniquilación, y $\sigma^-$ en el operador de creación. Así, la convención en `Qiskit Fermions` (que $|0\rangle$ corresponde a ocupado, y $|1\rangle$ corresponde a desocupado) resulta en que el operador de creación fermiónico $f_k^\dagger$ contiene $\sigma_k^-$, y el operador de aniquilación $f_k$ contiene $\sigma_k^+$.

Si encuentras fuentes con signos opuestos entre los términos $X$ e $iY$ en los operadores de creación y aniquilación, probablemente están usando la convención opuesta sobre qué estado corresponde a un orbital lleno.
...
Discutamos brevemente cómo implementaríamos el mapeo de Jordan-Wigner en Qiskit. Es importante entender cómo funcionan este tipo de transformaciones, pero sería poco práctico desarrollarlas a mano cada vez, sobre todo en sistemas grandes. Afortunadamente, Qiskit nos lo facilita con la función `SparsePauliOp`.

A un alto nivel, los pasos son:

1.	Usa la función `from_list` de SparsePauliOp para crear un operador identidad correspondiente al tamaño del espacio de parámetros a mapear.

2.	Siguiendo la definición de los operadores de creación y destrucción mostrada anteriormente, usa la función `from_list` de SparsePauliOp para definir los operadores de Pauli $X$, $Y$, $Z$. Esto mapeará los operadores fermiónicos de creación y aniquilación a operadores de espín de qubits, codificando el número de ocupación fermiónica en la base computacional de los qubits.

3.	Genera el Hamiltoniano deseado en la base de Pauli aplicando los operadores anteriores a los orbitales de interés. Esto usualmente corresponde a la creación de una matriz identidad que representa los orbitales core (no interactuantes), y luego aplicando los operadores de creación y aniquilación al espacio activo, con pesos que corresponden a las particularidades del problema en cuestión.
...
donde $\sigma^+$ es el operador de Pauli $X + iY$ y $\sigma^{-}$ es $X - iY$. Una vez que tenemos un hamiltoniano escrito en este formato, la parte difícil del estadio de mapeo es sobre, y ahora puede ser fácilmente escrito a un circuito en operadores de Pauli.
...
Gran parte de este material fue muy técnico, y si no lo has visto antes, puede resultar muy intimidante. Pero se vuelve más fácil cuanto más tiempo pases practicando -- por eso este curso se llama "Quantum Computing in Practice". Esto no es algo que se domine a la primera -- requiere algo de estudio, algo de rascarse la cabeza y probablemente algunos momentos frustrantes. Pero te animo a permanecer en esa incomodidad y explorar realmente las preguntas que surgen a medida que avanzas. Es la única forma de aprender.
