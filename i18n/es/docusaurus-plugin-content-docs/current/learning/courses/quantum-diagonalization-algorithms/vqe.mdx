...
### 1.1 El operador (hamiltoniano) {#11-the-operator-hamiltonian}
...
Un flujo de trabajo común (demostrado en la Sección 3.3) es usar uno de estos algoritmos como el método dentro de un minimizador como la función ```minimize``` de scipy. Esta toma como argumentos:
* Alguna función a minimizar. Esto suele ser el valor esperado de la energía. Pero estas generalmente se denominan "funciones de costo".
* Un conjunto de parámetros desde el cual comenzar la búsqueda. A menudo llamado $x_0$ o $\theta_0$.
* Argumentos, incluyendo argumentos de la función de costo. En computación cuántica con Qiskit, estos argumentos incluirán el ansatz, el Hamiltoniano y la primitiva Estimator, que se discute más en la siguiente subsección.
* Un 'método' de minimización. Esto se refiere al algoritmo específico usado para buscar el espacio de parámetros. Aquí es donde especificaríamos, por ejemplo, COBYLA o SLSQP.
* Opciones. Las opciones disponibles pueden diferir según el método. Pero un ejemplo que prácticamente todos los métodos incluirían es el número máximo de iteraciones del optimizador antes de finalizar la búsqueda: 'maxiter'.
...
### 1.5 El principio variacional {#15-the-variational-principle}
...
## 2. Comparación con el flujo de trabajo clásico {#2-comparison-with-classical-workflow}
...
### 2.1 Flujo de trabajo clásico {#21-classical-workflow}
...
### 2.2 Flujo de trabajo cuántico y grupos de Pauli conmutantes {#22-quantum-workflow-and-commuting-pauli-groups}
...
### 2.3 Factores que determinan el costo computacional {#23-factors-that-determine-computational-cost}
...
### Verifica tu comprensión {#check-your-understanding-1}
...
## 3. Hamiltoniano de ejemplo {#3-example-hamiltonian}
...
### 3.1 Paso 1: Mapear el problema a circuitos cuánticos y operadores {#31-step-1-map-the-problem-to-quantum-circuits-and-operators}
...
### 3.2 Paso 2: Optimizar para el hardware objetivo {#32-step-2-optimize-for-target-hardware}
...
### 3.3 Paso 3: Ejecutar en el hardware objetivo {#33-step-3-execute-on-target-hardware}

#### 3.3.1 Reportar valores {#331-reporting-out-values}
...
### 3.4 Paso 4: Posprocesar resultados {#34-step-4-post-process-results}
...
## 4. Relación de VQE con otros métodos {#4-vqes-relationship-to-other-methods}
...
### 4.1 Las fortalezas y debilidades de VQE {#41-the-strengths-and-weaknesses-of-vqe}
...
### 4.2 Relación con otros enfoques {#42-relationship-to-other-approaches}
...
### Verifica tu comprensión {#check-your-understanding-3}
