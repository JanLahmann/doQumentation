...
![Una imagen de orbitales atómicos y una imagen de una red con muchos nodos y conexiones.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig1.svg)
...
![Una imagen de un circuito cuántico con parámetros variacionales etiquetados como "theta".](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig2.svg)
...
![Una imagen de una línea curva que representa la energía, con varios puntos donde se prueba el valor para encontrar el mínimo.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig3.svg)
...
Supón que te interesa una matriz con N filas y N columnas. Imagina que tu matriz es tan grande que la diagonalización exacta no es una opción. Supón que conoces lo suficiente sobre tu problema como para hacer algunas suposiciones sobre la estructura general del estado propio objetivo, y deseas explorar estados similares a tu estimación inicial para ver si su costo/energía puede reducirse aún más. Este es un enfoque variacional y un método que se emplea cuando la diagonalización exacta no es una opción.

### 2.1 Flujo de trabajo clásico {#classical-workflow}

Con un computador clásico, esto funcionaría de la siguiente manera:
* Construye un estado estimado con algunos parámetros $\vec{\theta}_i$ que variará: $|\psi(\vec{\theta}_i)\rangle$. Aunque esta estimación inicial podría ser aleatoria, no es recomendable. Queremos aprovechar el conocimiento sobre el problema en cuestión para ajustar nuestra estimación tanto como sea posible.
* Calcula el valor esperado del operador cuando el sistema se encuentra en este estado: $\langle\psi(\vec{\theta}_i)|H|\psi(\vec{\theta}_i)\rangle$
* Cambia los parámetros variacionales y repite: $\vec{\theta}_i\rightarrow \vec{\theta}_{i+1}$.
* Utiliza la información recopilada sobre el paisaje de estados posibles en su subespacio variacional para hacer estimaciones cada vez mejores y acercarse al estado objetivo. El principio variacional garantiza que nuestro estado variacional no puede producir un valor propio menor que el del estado fundamental buscado. Por lo tanto, cuanto menor sea el valor esperado, mejor será nuestra aproximación del estado fundamental:
$$
...
Imagina ahora que transfieres esta parte del cálculo a un computador cuántico. En lugar de calcular este valor esperado, lo estimas preparando el estado $|\psi(\vec{\theta_i})\rangle$ en el computador cuántico con tu ansatz variacional y luego realizando mediciones.
...
H = \sum_{\alpha = 1}^T{c_\alpha h_\alpha}
$$
donde $h_\alpha$ es una cadena de Pauli que actúa sobre todos los qubits del sistema, como `IZZX…XIYX`, o varias de estas cadenas que conmutan entre sí. Nota que Qiskit utiliza la notación *little-endian*, donde el $n^\text{ésimo}$ operador de Pauli desde la derecha actúa sobre el $n^\text{ésimo}$ qubit. Por lo tanto, podemos medir nuestro operador midiendo una serie de operadores de Pauli.
...
![Una tabla de diferentes cadenas de Pauli, algunas de las cuales conmutan y otras no.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig4.svg)
...
![Dos imágenes de una línea curva con un valor mínimo. En una, se verifican puntos aleatoriamente para encontrar el mínimo; en la otra, se estima un gradiente trazando una línea entre dos puntos adyacentes.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig5.svg)
...
Considera un hamiltoniano de cuatro qubits que contiene los siguientes términos:
...
Es posible hacerlo en 5 grupos. Nota que estas soluciones típicamente no son únicas.
...
* Frecuentemente se emplean la reconstrucción de configuraciones[\[16\]](#references) y la postselección por simetría[\[15\]](#references) como técnicas de posprocesamiento.
