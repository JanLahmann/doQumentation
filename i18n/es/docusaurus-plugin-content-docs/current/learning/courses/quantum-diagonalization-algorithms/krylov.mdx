...
Un __método de subespacio de Krylov__ puede referirse a cualquiera de varios métodos construidos alrededor de lo que se llama el __subespacio de Krylov__. Una revisión completa de estos está fuera del alcance de esta lección, pero las Ref [\[2-4\]](#references) pueden dar mucho más contexto. Aquí, nos centraremos en qué es un subespacio de Krylov, cómo y por qué es útil para resolver problemas de valores propios, y finalmente cómo puede implementarse en una computadora cuántica.
__Definición:__ Dada una matriz simétrica, semidefinida positiva $N\times N$ $A$, el espacio de Krylov $\mathcal{K}^r$ de orden $r$ es el espacio generado por vectores obtenidos multiplicando potencias más altas de una matriz $A$, hasta $r-1\leq N$, con un vector de referencia $\vert v \rangle$.
...
(b) El proceso de ortogonalización consiste en restar la proyección de un nuevo vector sobre todos los vectores anteriores. Si todos los vectores anteriores abarcan el espacio vectorial completo, entonces restar las proyecciones sobre el subespacio completo siempre nos dejará con un vector cero.

</AccordionItem>
</Accordion>

Supongamos que un colega investigador está demostrando el método de Krylov aplicado a una pequeña matriz de ejemplo. ¿Hay algo mal con su elección de matriz $A$ y vector inicial $|\psi\rangle$?
...
Tu colega ha elegido accidentalmente un vector propio como su vector inicial. Actuar con la matriz sobre el vector inicial simplemente devolverá el mismo vector, escalado por el valor propio. Esto no generará un subespacio de dimensión creciente. Aconseja a tu colega que seleccione un vector inicial diferente, asegurándose de que no sea un vector propio.
...
$$
Aquí $P_\alpha$ es una cadena de Pauli de la forma $P_\alpha \sim IZIXII...YZXIX$ o un conjunto de tales cadenas de Pauli que conmutan entre sí. Dado que podemos escribir $H$ como tal suma de operadores medibles, las siguientes expresiones para elementos matriciales de $\tilde{H}$ pueden ser realizadas usando el primitivo IBM Quantum Estimator.
...
Elige un estado de referencia $\vert \psi \rangle$ que tenga algún solapamiento con el estado fundamental. Para este Hamiltoniano, usamos un estado con una excitación en el qubit central $\vert 00..010...00 \rangle$ como nuestro estado de referencia.
...
### 4. 2 Paso 2. Optimizar circuitos y operadores para el hardware objetivo {#4-2-step-2-optimize-circuits-and-operators-for-target-hardware}
...
#### Circuitos plantilla para calcular los elementos de matriz de $\tilde{S}$ y $\tilde{H}$ mediante el test de Hadamard {#template-circuits-for-calculating-matrix-elements-of-tildes-and-tildeh-via-hadamard-test}
...
### 4.3 Paso 3. Ejecutar con una primitiva de IBM Quantum {#43-step-3-execute-using-an-ibm-quantum-primitive}
...
## 5. Discusión y extensión {#5-discussion-and-extension}
