...
![Un diagrama que muestra cómo VQE usa el estado de referencia y el ansatz para estimar una función de coste, y luego itera usando parámetros variacionales.](/learning/images/courses/variational-algorithm-design/instances-and-extensions/instances-vqe.svg)
...
- Evaluar la función de coste $C(\vec\theta_{i,j}) := \langle \psi(\vec{\theta}) | \hat{H} | \psi(\vec{\theta})\rangle$ para todos los estados preparados en un ordenador cuántico.
...
Este es un ciclo de optimización clásico sencillo en el que evaluamos la función de coste. Algunos optimizadores pueden requerir varias evaluaciones para calcular un gradiente, determinar la siguiente iteración o verificar la convergencia.
...
Podemos usar esta función de coste para calcular los parámetros óptimos:
...
Ya estamos listos para ejecutar nuestro cálculo en hardware de IBM Quantum&reg;. Dado que la minimización de la función de coste es muy iterativa, iniciaremos una sesión de Runtime. De este modo, solo tendremos que esperar en la cola una vez. Una vez que el trabajo comience a ejecutarse, cada iteración con actualizaciones de parámetros se ejecutará de inmediato.
...
El valor de x obtenido es la mejor estimación actual de los parámetros que minimizan la función de coste. Si se itera para obtener mayor precisión, esos valores deben usarse en lugar del x0 inicial (un vector de unos).
...
Para implementar este algoritmo, necesitamos $k$ estados de referencia mutuamente ortogonales $\{ |\rho_j\rangle \}_{j=0}^{k-1}$, lo que significa que $\langle \rho_j | \rho_l \rangle = \delta_{jl}$ para $j,l<k$. Estos estados pueden construirse usando operadores de Pauli. La función de coste de este algoritmo es:
...
### Esquema teórico {#theoretical-layout-1}
...
- Evaluar la función de coste $C(\vec\theta_{i,j}) := \sum_{j=0}^{k-1} w_j \langle \psi_{j}(\vec{\theta}) | \hat{H} | \psi_{j}(\vec{\theta}) \rangle$ para todos los estados preparados en un ordenador cuántico.
  ...
  - A continuación, la función de coste devuelve la suma de todos los valores esperados ponderados.
...
Reconstruirás la función de coste de SSVQE en la evaluación, pero aquí tienes el siguiente fragmento para orientar tu solución:
...
### Diseño teórico {#theoretical-layout-2}
...
### Implementación {#implementation-1}
...
### Esquema teórico {#theoretical-layout-3}
...
- Muestrea la función de costo $C(\vec\theta) := a_0 + \sum_{k=1}^S[a_k\cos(k\theta)+ b_k\sin(k\theta)]$ al menos $T$ veces.
...
Estas ideas se pueden adaptar para formar un algoritmo variacional personalizado que se ajuste a tu problema. Te animamos a compartir tus resultados con la comunidad. La próxima lección explorará cómo usar un algoritmo variacional para resolver una aplicación.
