---
title: "Optimización de circuitos cuánticos"
sidebar_label: "Optimización de circuitos cuánticos"
description: "Esta lección cubre varios aspectos de la optimización de circuitos en la computación cuántica."
...
# Optimización de circuitos cuánticos {#quantum-circuit-optimization}

<Admonition type="note">

Toshinari Itoko (21 de junio de 2024)

[Descargar el PDF de la clase original](https://ibm.ent.box.com/public/static/0hvvgr1gnwx64x2ukgk04sss6sxc4zko.zip). Ten en cuenta que algunos fragmentos de código pueden estar desactualizados, ya que son imágenes estáticas.

*El tiempo aproximado de QPU para este experimento es de 15 s.*

(Nota: Algunas celdas de la Parte 2 provienen del notebook "Qiskit Deep Dive" de Matthew Treinish (mantenedor de Qiskit))
...
Esta lección cubre varios aspectos de la optimización de circuitos en la computación cuántica. En concreto, conoceremos la utilidad de la optimización de circuitos mediante las configuraciones de optimización integradas en Qiskit. Luego profundizaremos un poco más y mostraremos lo que puedes hacer como experto en tu área de aplicación para construir circuitos de manera dirigida. Finalmente, examinaremos en detalle lo que ocurre durante la transpilación para optimizar nuestros circuitos.
## 2. La optimización de circuitos es importante {#2-circuit-optimization-matters}

Primero comparamos los resultados de ejecutar circuitos de preparación de estados GHZ de 5 qubits ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$) con y sin optimización.
...
Primero utilizamos un circuito GHZ sintetizado de forma ingenua como se muestra a continuación.
...
### 2.1 Nivel de optimización {#21-optimization-level}
Hay 4 valores de `optimization_level` disponibles, de 0 a 3. Cuanto mayor sea el nivel de optimización, más esfuerzo computacional se dedica a optimizar el circuito. El nivel 0 no realiza optimización y solo hace lo necesario para que el circuito pueda ejecutarse en el backend seleccionado. El nivel 3 invierte el mayor esfuerzo (y típicamente el mayor tiempo de ejecución) para optimizar el circuito. El nivel 1 es el nivel de optimización predeterminado.
Transpilamos el circuito sin optimización (`optimization_level=0`) y con optimización (`optimization_level=2`).
Al hacerlo, vemos una diferencia significativa en la longitud de los circuitos transpilados.
...
### 2.2 Ejercicio {#22-exercise}
Prueba también `optimization_level=1` y compara el circuito resultante con los dos anteriores. Para ello, modifica el código de arriba.
...
Ejecución en un backend simulado (simulación con ruido). Cómo ejecutar en un backend real se describe en el Apéndice 1.
...
## 3. La síntesis de circuitos es importante {#3-circuit-synthesis-matters}
A continuación, comparamos los resultados de dos circuitos de preparación de estados GHZ de 5 qubits ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$) sintetizados de manera diferente.
...
La nueva síntesis produce un circuito más plano. ¿Por qué?

Esto se debe a que el nuevo circuito puede colocarse en qubits conectados linealmente y por tanto también funciona en el grafo de acoplamiento Heavy-Hexagon de Brisbane de IBM®, mientras que el circuito original requiere una conectividad en forma de estrella (un nodo de grado 4) y por lo tanto no puede colocarse en el grafo de acoplamiento Heavy-Hex, cuyos nodos tienen un grado máximo de 3. Como resultado, el circuito original requiere un enrutamiento de qubits que añade puertas SWAP y, por tanto, aumenta el número de puertas.

Lo que hicimos con el nuevo circuito puede considerarse como una síntesis de circuitos manual "consciente de las restricciones de acoplamiento". En otras palabras: la síntesis del circuito y el mapeo del circuito se resolvieron simultáneamente de forma manual.
...
En general, la síntesis de circuitos depende de la aplicación, y es demasiado difícil para el software cubrir todas las aplicaciones posibles. El transpilador de Qiskit casualmente no tiene funciones para sintetizar circuitos de preparación de estados GHZ. En tales casos, una síntesis manual de circuitos como la mostrada arriba es una opción sensata.
En esta sección examinamos cómo funciona el transpilador de Qiskit en detalle utilizando el siguiente circuito de ejemplo.
...
### 3.1 Visualizar el flujo completo de transpilación de Qiskit {#31-draw-the-entire-qiskit-transpilation-flow}
Observamos los passes (tareas) del transpilador para `optimization_level=1`.
...
### 3.2 Visualizar una etapa individual {#32-draw-an-individual-stage}

Primero visualicemos todas las tareas (passes del transpilador) de la etapa `init`.
...
Podemos ejecutar cada etapa individualmente. Ejecutemos la etapa `init` para nuestro circuito. Al activar el logger, podemos ver los detalles del proceso.
...
Visualiza los passes de la etapa `layout` y ejecuta la etapa para el circuito de salida de la etapa `init` (`init_out`), modificando las celdas utilizadas anteriormente.
...
Haz lo mismo para la etapa `translation`.
...
Nota: No todas las etapas individuales pueden ejecutarse siempre de forma independiente, ya que algunas necesitan información de una etapa anterior.
### 3.4 Etapa de optimización {#34-optimization-stage}

La última etapa estándar en la pipeline es la optimización. Después de que el circuito ha sido embebido para el sistema objetivo, ha crecido considerablemente. Esto se debe principalmente a ineficiencias en las relaciones de equivalencia de la traducción de base y la inserción de SWAPs. La etapa de optimización intenta minimizar el tamaño y la profundidad del circuito. Ejecuta una serie de passes en un bucle ```do while``` hasta alcanzar un resultado estable.
...
## 4. Ejemplos en profundidad {#4-in-depth-examples}
### 4.1 Optimización de bloques de dos qubits mediante síntesis unitaria de dos qubits {#41-two-qubit-block-optimization-using-two-qubit-unitary-synthesis}

Para los niveles 2 y 3 hay más passes (`Collect2qBlocks`, `ConsolidateBlocks`, `UnitarySynthesis`) para una optimización más profunda, concretamente la optimización de bloques de dos qubits. (Compara el flujo del nivel de optimización 2 con el de nivel 1 mostrado arriba)

La optimización de bloques de dos qubits consta de dos pasos: la recolección y consolidación de bloques de 2 qubits y la síntesis de las matrices unitarias de 2 qubits.
...
Hemos visto en la Parte 2 que el flujo real del compilador cuántico no es tan simple y consta de muchos passes (tareas). Esto se debe principalmente al esfuerzo de ingeniería de software necesario para garantizar un buen rendimiento para un amplio espectro de circuitos de aplicación y la mantenibilidad del software. El transpilador de Qiskit funciona bien en la mayoría de los casos, pero si descubres que tu circuito no está siendo optimizado de manera óptima, sería una buena oportunidad para desarrollar una optimización de circuitos específica para la aplicación como en la Parte 1. La tecnología de transpiladores continúa evolucionando — tu contribución de I+D es bienvenida.
...
### 4.2 La optimización de circuitos es importante {#42-circuit-optimization-matters}
...
Transpilamos el circuito sin optimización (`optimization_level=0`) y con optimización (`optimization_level=2`).
Como puedes ver, hay una diferencia significativa en la longitud de los circuitos transpilados.
...
### 4.3 La síntesis de circuitos es importante {#43-circuit-synthesis-matters}
A continuación, comparamos los resultados de dos circuitos de preparación de estados GHZ de 5 qubits ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$) sintetizados de manera diferente.
...
### 4.4 Descomposición general de puertas de 1 qubit {#44-general-1-qubit-gate-decomposition}
...
### 4.5 Optimización de bloques de 1 qubit {#45-one-qubit-block-optimization}
...
### 4.6 Descomposición de Toffoli {#46-toffoli-decomposition}
...
### 4.7 Descomposición de la puerta CU {#47-cu-gate-decomposition}
...
### 4.8 CX, ECR, CZ son equivalentes salvo Cliffords locales {#48-cx-ecr-cz-equal-up-to-local-cliffords}

Nota: $H$ (Hadamard), $S$ (rotación Z de $\pi/2$), $S^\dagger$ (rotación Z de $-\pi/2$) y $X$ (Pauli-X) son todas puertas Clifford.
...
Usando las puertas base de 1 qubit del backend de IBM "rz", "sx" y "x".
