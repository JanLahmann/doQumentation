---
description: 'Este curso trata sobre la simulación cuántica, incluyendo la trotterización para el hamiltoniano de Ising de campo transversal'
notebook_path: learning/courses/utility-scale-quantum-computing/quantum-simulation.ipynb
sidebar_label: 'Simulación cuántica'
...
[Descarga el PDF](https://ibm.ent.box.com/s/kzzsmxhw38vph1ioohczaet53euwi310) de la conferencia original. Ten en cuenta que algunos fragmentos de código podrían quedar obsoletos, ya que son imágenes estáticas.

*El tiempo aproximado de QPU para ejecutar este experimento es de 7 segundos.*

(Este notebook está basado en gran medida en un [notebook de tutorial](https://github.com/qiskit-community/qiskit-algorithms/blob/main/docs/tutorials/13_trotterQRTE.ipynb) para Qiskit Algorithms, actualmente obsoleto.)
...
Hoy estudiaremos la evolución temporal del [modelo de Ising](https://en.wikipedia.org/wiki/Ising_model) en redes lineales de $N=2$ y $N=6$ sitios. Estas redes consisten en un arreglo de espines $\sigma_i$ que interactúan únicamente con sus vecinos más cercanos. Estos espines pueden tener dos orientaciones: $\uparrow$ y $\downarrow$, que corresponden a una magnetización de $+1$ y $-1$ respectivamente.
...
### 1.2 Importar las bibliotecas {#12-import-the-libraries}
...
## 2. Mapeando tu problema {#2-mapping-your-problem}

### 2.1 Definición del hamiltoniano de Ising de campo transversal {#21-defining-the-transverse-field-ising-hamiltonian}

Aquí consideramos el modelo de Ising de campo transversal en 1D.

Primero, crearemos una función que recibe los parámetros del sistema $N$, $J$, $h$ y $\alpha$, y devuelve nuestro hamiltoniano como un `SparsePauliOp`. Un [SparsePauliOp](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.SparsePauliOp) es una representación dispersa de un operador como suma de términos de [Pauli](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Pauli) ponderados.
...
#### Definir el hamiltoniano {#define-the-hamiltonian}
...
### 2.2 Establecer los parámetros de la simulación de evolución temporal {#22-set-the-parameters-of-the-time-evolution-simulation}
...
### 2.3 Preparar el circuito cuántico 1 (estado inicial) {#23-prepare-the-quantum-circuit-1-initial-state}
...
![Salida de la celda de código anterior](/learning/images/courses/utility-scale-quantum-computing/quantum-simulation/extracted-outputs/252c2734-653d-4c8e-af6b-d3173845de88-1.avif)

### 2.5 Establecer los operadores a medir {#25-set-the-operators-to-measure}
...
### 2.6 Realizar la simulación de evolución temporal {#26-perform-time-evolution-simulation}
...
### 2.7 Graficar la evolución temporal de los observables {#27-plot-the-time-evolution-of-the-observables}
...
![Salida de la celda de código anterior](/learning/images/courses/utility-scale-quantum-computing/quantum-simulation/extracted-outputs/9c0f4b39-801a-448c-9dce-8a6d7c83fe76-1.avif)

## 3. Ejercicio 1. Realizar la simulación usando Suzuki–Trotter de segundo orden {#3-exercise-1-perform-simulation-using-second-order-suzukitrotter}
...
El Suzuki–Trotter de segundo orden se puede usar en Qiskit mediante la [clase SuzukiTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.SuzukiTrotter). Con esta fórmula, la descomposición de segundo orden es:

$$
e^{H_1+H_2} \approx e^{H_1/2}e^{H_2}e^{H_1/2}
$$

### 3.1 Construir un circuito para un solo paso temporal {#31-construct-a-circuit-for-a-single-time-step}
Usa product_formula_st2 (SuzukiTrotter(order=2)) y construye un circuito para un solo paso de tiempo usando Suzuki–Trotter de segundo orden. Además, cuenta el número de puertas y la profundidad del circuito y compara con Lie–Trotter.
...
![Salida de la celda de código anterior](/learning/images/courses/utility-scale-quantum-computing/quantum-simulation/extracted-outputs/9da4ff65-1989-4fc0-b63b-5e166a77f2d5-1.avif)

### 3.2 Realizar la simulación de evolución temporal {#32-perform-time-evolution-simulation}
...
### 3.3 Graficar los resultados de Suzuki–Trotter de segundo orden {#33-plot-the-second-order-suzukitrotter-results}
...
![Salida de la celda de código anterior](/learning/images/courses/utility-scale-quantum-computing/quantum-simulation/extracted-outputs/f859672f-cef3-4c2c-b087-9d36fa8162ff-0.avif)

### 3.4 Comparar con los resultados exactos {#34-compare-with-exact-results}
Los datos siguientes son los resultados exactos precalculados en el ordenador clásico.
...
![Salida de la celda de código anterior](/learning/images/courses/utility-scale-quantum-computing/quantum-simulation/extracted-outputs/c95e61c9-229c-4eed-b240-e6540f80956a-0.avif)

## 4. Ejecución en el hardware cuántico {#4-executing-on-the-quantum-hardware}
...
### 4.1  Paso 1. Mapear las entradas clásicas a un problema cuántico {#41-step-1-map-classical-inputs-to-a-quantum-problem}
...
![Salida de la celda de código anterior](/learning/images/courses/utility-scale-quantum-computing/quantum-simulation/extracted-outputs/f33796b1-7511-46b0-93ff-ee6e6188c412-0.avif)
...
![Salida de la celda de código anterior](/learning/images/courses/utility-scale-quantum-computing/quantum-simulation/extracted-outputs/8469258b-bb82-4e46-b3d6-43aee59d9474-1.avif)
...
#### Construir el circuito para el experimento en HW {#construct-circuit-for-hw-experiment}
...
![Salida de la celda de código anterior](/learning/images/courses/utility-scale-quantum-computing/quantum-simulation/extracted-outputs/64d9166e-296a-4a6d-a6d1-0b6fdd49399b-0.avif)

### 4.3 Paso 3. Ejecutar con primitivas de IBM Quantum {#43-step-3-execute-with-ibm-quantum-primitives}
...
### 4.4 Paso 4. Post-procesar los resultados {#44-step-4-post-process-results}
...
![Salida de la celda de código anterior](/learning/images/courses/utility-scale-quantum-computing/quantum-simulation/extracted-outputs/1f4e5fbd-1994-4f4d-8d35-ddf1a37664b7-1.avif)

A continuación mostramos un ejemplo para construir un circuito usando Suzuki–Trotter de orden superior (cuarto orden).
Ahora intenta construir una simulación de circuito con Suzuki–Trotter de cuarto orden siguiendo los ejemplos mostrados anteriormente.

El Suzuki–Trotter de cuarto orden puede usarse en Qiskit mediante la [clase SuzukiTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.SuzukiTrotter). El cuarto orden puede evaluarse usando la siguiente relación de recurrencia. Nótese que el orden de Suzuki–Trotter se denota como "2k" en las siguientes ecuaciones.
...
#### Construir un circuito para un único paso temporal {#construct-a-circuit-for-a-single-time-step}
Usa product_formula_st4 (SuzukiTrotter(order=4)) y construye un circuito para un solo paso de tiempo usando Suzuki–Trotter de cuarto orden. Además, cuenta el número de puertas y la profundidad del circuito y compara con Lie–Trotter y Suzuki–Trotter de segundo orden.
...
![Salida de la celda de código anterior](/learning/images/courses/utility-scale-quantum-computing/quantum-simulation/extracted-outputs/e20ea669-82d3-42a7-89f6-bab7d41689f1-1.avif)
