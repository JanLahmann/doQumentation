...
Como ya hemos señalado, el estado de $\mathsf{Q}$ justo antes del paso 2 se encuentra en el espacio bidimensional abarcado por $\vert A_0\rangle$ y $\vert A_1\rangle$, y acabamos de establecer que $G$ mapea cualquier vector en este espacio a otro vector en el mismo espacio.
Esto significa que, para propósitos del análisis, podemos enfocar nuestra atención exclusivamente en este subespacio.

Para entender mejor qué está sucediendo dentro de este espacio bidimensional, expresemos la acción de $G$ en este espacio como una matriz,
...
Aunque no es en absoluto obvio a primera vista, la matriz $M$ es lo que obtenemos al *elevar al cuadrado* una matriz de apariencia más simple.
...
Este ángulo $\theta$ va a desempeñar un papel muy importante en el análisis que sigue, así que vale la pena resaltar su importancia aquí al verlo por primera vez.

A la luz de esta expresión de esta matriz, observamos que
...
Esto ocurre porque rotar por el ángulo $\theta$ dos veces es equivalente a rotar por el ángulo $2\theta$.
Otra forma de ver esto es utilizar la expresión alternativa
...
y el efecto de aplicar $G$ a este estado es rotarlo por un ángulo $2\theta$ dentro del espacio abarcado por $\vert A_0\rangle$ y $\vert A_1\rangle$.
