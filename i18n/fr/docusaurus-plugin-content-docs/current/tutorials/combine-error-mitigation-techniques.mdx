---
description: "Combinez les options d'atténuation des erreurs pour des expériences à l'échelle utilitaire en utilisant des QPU IBM Quantum de plus de 100 qubits et la primitive Estimator d'IBM Quantum."
...
Ce tutoriel explore les options de suppression et d'atténuation des erreurs disponibles avec la primitive Estimator d'IBM Quantum. Ce tutoriel montre comment mettre en œuvre chacune des méthodes suivantes individuellement :
...
Ici, tu vas soumettre cinq tâches distinctes, en commençant sans suppression ni atténuation des erreurs, puis en activant successivement les différentes options de suppression et d'atténuation des erreurs disponibles dans IBM Quantum Compute Service. Pour plus d'informations sur les options, consulte les pages suivantes :
...
Comme ces tâches peuvent s'exécuter indépendamment les unes des autres, tu peux utiliser le [mode batch](/guides/run-jobs-batch) pour permettre à Quantum Compute d'optimiser la planification de leur exécution.
