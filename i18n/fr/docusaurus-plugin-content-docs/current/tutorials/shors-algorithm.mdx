...
Comme nous pouvons le voir dans la liste des valeurs de $b$, en plus de $M_2$ que nous avons précédemment construit, nous devons également construire $M_4$ et $M_1$. Notez que $M_1$ agit de manière triviale sur les états de la base computationnelle, c'est donc simplement l'opérateur identité.
