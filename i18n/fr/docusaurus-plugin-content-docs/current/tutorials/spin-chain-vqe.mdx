...
- Comment exécuter des workflows variationnels sur du matériel quantique IBM® à l'aide des primitives IBM Quantum et des sessions
...
Minimise l'énergie estimée de l'état fondamental du système en optimisant les paramètres du circuit. Utilise la primitive `Estimator` d'IBM Quantum pour évaluer la fonction de coût pendant l'optimisation.

Puisque nous avons optimisé le circuit pour le backend à l'étape 2, nous pouvons éviter la transpilation sur le serveur Quantum Compute en définissant `skip_transpilation=True` et en passant le circuit optimisé. Pour cette démonstration, nous exécuterons sur un QPU en utilisant les primitives IBM Quantum. Pour exécuter avec les primitives basées sur le vecteur d'état de `qiskit`, remplace le bloc de code utilisant les primitives IBM Quantum par le bloc commenté.
