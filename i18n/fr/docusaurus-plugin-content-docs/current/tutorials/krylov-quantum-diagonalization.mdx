...
*Estimation d'utilisation : 20 minutes sur un Heron r2 (REMARQUE : ceci est une estimation uniquement. Ton temps d'exécution peut varier.)*
...
Consulte l'annexe pour une dérivation détaillée montrant comment l'espace de Krylov unitaire permet de représenter avec précision les états propres de basse énergie.
...
Choisis un état de référence $\vert \psi \rangle$ qui possède un certain recouvrement avec l'état fondamental. Pour ce hamiltonien, nous utilisons un état avec une excitation sur le qubit central $\vert 00..010...00 \rangle$ comme état de référence.
...
où nous avons utilisé le déphasage calculable classiquement $ U\ket{0}^N = e^{i\phi}\ket{0}^N$ à la troisième ligne. Les valeurs moyennes sont donc obtenues comme suit
...
Ensuite, transpile le circuit virtuel vers la meilleure disposition physique dans cette nouvelle cible.
...
Calcule d'abord la phase accumulée par l'état $\vert 0 \rangle$ durant l'évolution temporelle non contrôlée
...
pour un pas de temps $dt$ que nous déterminerons ultérieurement. Supposons temporairement que $r$ est pair : définissons alors $d=r/2$. Remarque que lorsque nous projetons le Hamiltonien dans l'espace de Krylov ci-dessus, celui-ci est indiscernable de l'espace de Krylov
...
Nous fournissons une preuve ci-dessous, mais celle-ci peut être ignorée sans risque à moins que tu ne souhaites comprendre l'argument rigoureux complet. Pour l'instant, nous nous concentrons sur les implications de l'affirmation ci-dessus. Par la propriété 3 ci-dessus, nous pouvons voir que l'espace de Krylov décalé ci-dessus contient l'état $f(H)|\psi\rangle$. C'est notre état de basse énergie. Pour comprendre pourquoi, écrivons $|\psi\rangle$ dans la base des états propres d'énergie :
...
> **Note :** Cette enquête est proposée par **IBM Quantum** et porte sur le **contenu du tutoriel** (rédigé par IBM). doQumentation fournit le site web, les traductions et l'exécution du code — pour tout retour sur ces aspects, merci d'[ouvrir une issue GitHub](https://github.com/JanLahmann/doQumentation/issues).
