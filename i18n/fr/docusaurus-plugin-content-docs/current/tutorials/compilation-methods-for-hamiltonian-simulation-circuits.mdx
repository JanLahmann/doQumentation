...
En pratique, les erreurs de porte s'accumulent tout au long du circuit, de sorte que la probabilité de récupérer $|0\rangle^{\otimes n}$ diminue. Une méthode de compilation qui produit un circuit plus superficiel avec moins de portes accumulera moins de bruit.
...
- **Rustiq** est spécialement conçu pour les circuits `PauliEvolutionGate` et peut trouver des solutions à très faible profondeur et faible nombre de portes, en particulier sur les circuits plus petits. Sur les circuits plus grands, il peut parfois produire des résultats bien plus élevés, il est donc mieux utilisé comme l'une des plusieurs méthodes à essayer plutôt que comme choix par défaut.
