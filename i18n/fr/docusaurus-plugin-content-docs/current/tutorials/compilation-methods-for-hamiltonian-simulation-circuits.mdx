...
description: "Ce tutoriel propose une vue d'ensemble comparative de trois méthodes de compilation dans Qiskit pour les charges de travail de simulation hamiltonienne."
...
L'objectif de ce tutoriel est d'enseigner aux utilisateurs comment appliquer et évaluer trois méthodes de compilation dans Qiskit : le transpileur SABRE, le transpileur alimenté par l'IA et le plugin Rustiq. Tu apprendras à utiliser efficacement chaque méthode et à comparer leurs performances sur différents circuits quantiques. À l'issue de ce tutoriel, tu seras en mesure de choisir et d'adapter des stratégies de compilation en fonction d'objectifs d'optimisation spécifiques tels que la réduction de la profondeur du circuit, la minimisation du nombre de portes ou l'amélioration du temps d'exécution.

### Ce que tu apprendras {#what-you-will-learn}
...
- Qiskit SDK v1.3 ou version ultérieure, avec prise en charge de la [visualisation](https://docs.quantum.ibm.com/api/qiskit/visualization)
...
- Qiskit AI Transpiler mode local (`pip install qiskit_ibm_ai_local_transpiler`)
- Bibliothèque de graphes Networkx (`pip install networkx`)
## Configuration {#setup}
...
## Partie 1 : Circuit Efficient SU2 {#part-1-efficient-su2-circuit}
...
#### Transpiler et capturer les métriques {#transpile-and-capture-metrics}
...
#### Graphique des résultats {#results-graph}
...
#### Analyse des résultats de compilation du circuit SU2 {#analysis-of-su2-circuit-compilation-results}
...
## Partie 2 : Circuit de simulation hamiltonienne {#part-2-hamiltonian-simulation-circuit}

### Étape 1 : Étudier les circuits avec `PauliEvolutionGate` {#step-1-investigate-circuits-with-paulievolutiongate}
...
#### Hamiltoniens utilisés dans le benchmark {#hamiltonians-used-in-the-benchmark}

Les hamiltoniens utilisés dans ce benchmark décrivent des interactions par paires entre qubits, incluant des termes tels que $ZZ$, $XX$ et $YY$. Ces hamiltoniens sont couramment utilisés en chimie quantique, en physique de la matière condensée et en science des matériaux, où ils modélisent des systèmes de particules en interaction.

Pour référence, les utilisateurs peuvent explorer un ensemble plus large d'hamiltoniens dans cet article : [Efficient Hamiltonian Simulation on Noisy Quantum Devices](https://arxiv.org/pdf/2306.13126).

#### Source du benchmark : Hamlib et Benchpress {#benchmark-source-hamlib-and-benchpress}

Les circuits utilisés dans ce benchmark sont issus du [dépôt de benchmarks Hamlib](https://github.com/SRI-International/QC-App-Oriented-Benchmarks/tree/master/qedcbench/hamiltonian_simulation), qui contient des charges de travail réalistes de simulation hamiltonienne.

Ces mêmes circuits ont été précédemment évalués à l'aide de [Benchpress](https://github.com/Qiskit/benchpress), un framework open source pour l'évaluation des performances de transpilation quantique. En utilisant cet ensemble standardisé de circuits, nous pouvons comparer directement l'efficacité des différentes stratégies de compilation sur des problèmes de simulation représentatifs.
...
#### Analyse des résultats de compilation des circuits hamiltoniens {#analysis-of-hamiltonian-circuit-compilation-results}
...
Bien que le transpileur IA fournisse généralement de meilleurs résultats que SABRE, en particulier en termes de profondeur de circuit, la conclusion ne devrait pas simplement être « utilise toujours le transpileur IA ». Il y a des nuances importantes à prendre en compte :

- Le **transpileur IA** est généralement fiable et fournit des circuits optimisés en profondeur, mais il comporte des compromis en termes de temps d'exécution, ainsi que d'autres limitations, notamment les cartes de couplage prises en charge et les capacités de synthèse. Celles-ci sont détaillées dans la [documentation du Qiskit Transpiler Service](/guides/qiskit-transpiler-service).

- Dans certains cas, en particulier avec des circuits très grands ou spécifiques au matériel, le transpileur IA peut ne pas être aussi efficace. Dans ces cas, le transpileur SABRE par défaut reste extrêmement fiable et peut être davantage optimisé en ajustant ses paramètres (voir le [tutoriel d'optimisation SABRE](/tutorials/transpilation-optimizations-with-sabre)).

- Il est également important de considérer la structure du circuit lors du choix d'une méthode. Par exemple, `rustiq` est spécialement conçu pour les circuits impliquant `PauliEvolutionGate` et produit souvent les meilleures performances pour les problèmes de simulation hamiltonienne.
