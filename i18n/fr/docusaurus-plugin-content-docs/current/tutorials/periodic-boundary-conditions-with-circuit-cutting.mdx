...
description: "Utilise la découpe de circuits pour traiter un problème de chaîne périodique à grande échelle où les premier et dernier qubits ne sont pas voisins."
...
*Estimation d'utilisation : deux minutes sur un processeur Eagle (REMARQUE : il s'agit uniquement d'une estimation. Ton temps d'exécution peut varier.)*
## Contexte {#background}
...
Note qu'il existe deux types de découpes : couper le fil d'un circuit (appelé `wire cutting`), ou remplacer une porte à 2 qubits par plusieurs opérations à un seul qubit (appelé `gate cutting`). Dans ce notebook, nous nous concentrerons sur la découpe de portes. Pour plus de détails sur la découpe de portes, consulte les <a href="https://qiskit.github.io/qiskit-addon-cutting/explanation/index.html">documents explicatifs</a> de `qiskit-addon-cutting`, ainsi que les références correspondantes. Pour plus de détails sur la découpe de fils, consulte le tutoriel [Découpe de fils pour l'estimation des valeurs d'espérance](/tutorials/wire-cutting), ou les tutoriels de <a href='https://qiskit.github.io/qiskit-addon-cutting/tutorials/index.html'>qiskit-addon-cutting</a>.
...
Nous considérons une `carte d'intrication` adaptée au matériel (hardware-efficient) pour le circuit TwoLocal avec une connectivité périodique entre les dernier et premier qubits de la `carte d'intrication`. Cette interaction longue distance peut entraîner des portes SWAP supplémentaires lors de la transpilation, augmentant ainsi la profondeur du circuit.
#### Sélection du backend et du placement initial {#select-backend-and-initial-layout}
...
#### Construction de la carte d'intrication pour le circuit TwoLocal {#build-the-entangler-map-for-the-twolocal-circuit}
...
Le circuit TwoLocal permet la répétition des `rotation_blocks` et de la `carte d'intrication` plusieurs fois. Dans ce cas, le nombre de répétitions détermine le nombre de portes périodiques à découper. Étant donné que le surcoût d'échantillonnage augmente exponentiellement avec le nombre de découpes (consulte le tutoriel [Découpe de fils pour l'estimation des valeurs d'espérance](/tutorials/wire-cutting) pour plus de détails), nous fixons le nombre de répétitions à 2 dans ce notebook.
...
#### Sélection des observables {#select-observables}
Pour quantifier les bénéfices de la découpe de portes, nous mesurons les valeurs d'espérance des observables $\frac{1}{n}\sum_{i=1}^n \langle Z_i \rangle$ et $\frac{1}{n-1}\sum_{i=1}^{n-1} \langle Z_i Z_{i+1} \rangle$. Comme discuté précédemment, les valeurs d'espérance idéales sont respectivement $-1$ et $+1$.
...
<ul>
    ...
    <li>Sortie : Circuit cible et observables produites par la découpe des portes longue distance</li>
</ul>
#### Transpilation du circuit {#transpile-the-circuit}

Note que le circuit peut être transpilé à cette étape, ou après la découpe. Si nous transpilons après la découpe, cela nécessitera de transpiler chacune des sous-expériences générées en raison du surcoût d'échantillonnage. Il est donc plus judicieux de transpiler à cette étape pour réduire le surcoût de transpilation.
...
#### Découpe des connectivités périodiques longue distance {#cut-the-long-range-periodic-connectivities}
Nous découpons maintenant les portes dans le circuit transpilé. Note que les portes à 2 qubits qui doivent être découpées sont celles connectant les dernier et premier qubits du placement.
...
Note que la découpe des interactions longue distance entraîne l'exécution de multiples échantillons du circuit qui diffèrent par les bases de mesure et de préparation. Plus d'informations à ce sujet sont disponibles dans <a href='https://arxiv.org/abs/1909.07534'>Constructing a virtual two-qubit gate by sampling single-qubit operations</a> et <a href='https://arxiv.org/abs/2312.11638'>Cutting circuits with multiple two-qubit unitaries</a>.

Le nombre de portes périodiques à découper est égal au nombre de répétitions de la couche `TwoLocal`, défini par `num_reps` ci-dessus. Le surcoût d'échantillonnage de la découpe de portes est de 6. Par conséquent, le nombre total de sous-expériences sera $6^{num\_reps}$.
...
#### Transpilation des sous-expériences {#transpile-the-subexperiments}
...
Nous utilisons une primitive `SamplerV2` pour l'exécution des circuits découpés. Nous désactivons le `découplage dynamique` et le `twirling` afin que toute amélioration obtenue dans le résultat soit uniquement due à l'application efficace de la découpe de portes pour ce type de circuit.
...
### Vérification croisée : Obtenir les valeurs d'espérance sans découpe {#cross-verify-obtain-uncut-expectation-value}
Il est utile de vérifier l'avantage de la technique de découpe de circuits par rapport à l'absence de découpe. Ici, nous allons calculer les valeurs d'espérance sans découper le circuit. Note qu'un tel circuit non découpé souffrira d'un grand nombre de portes SWAP nécessaires pour implémenter l'opération à 2 qubits entre les premier et dernier qubits. Nous utiliserons la fonction `sampled_expectation_value` pour obtenir les valeurs d'espérance du circuit non découpé après avoir obtenu la distribution de probabilité via `SamplerV2`. Cela permet une utilisation homogène de la primitive sur toutes les instances. Note cependant que nous aurions également pu utiliser `EstimatorV2` pour calculer directement les valeurs d'espérance.
...
Nous allons maintenant calculer les valeurs d'espérance moyennes de toutes les observables Z de poids 1 et de poids 2 sans découpe.
...
Visualisons maintenant l'amélioration obtenue pour les observables de poids 1 et de poids 2 lors de l'utilisation de la découpe de portes pour un circuit à chaîne périodique.
...
- supprimé la connectivité longue distance du circuit transpilé via la découpe de portes.
...
Nous observons à partir des résultats que les moyennes des observables de type $\langle Z \rangle$ de poids 1 et $\langle ZZ \rangle$ de poids 2 sont significativement améliorées par la découpe des portes périodiques. Note que cette étude n'inclut aucune technique de suppression ou d'atténuation des erreurs. L'amélioration observée est uniquement due à l'utilisation appropriée de la découpe de portes pour ce problème. Les résultats auraient pu être davantage améliorés en utilisant des techniques d'atténuation et de suppression des erreurs.

Cette étude montre un exemple d'utilisation efficace de la découpe de portes pour améliorer les performances du calcul.
