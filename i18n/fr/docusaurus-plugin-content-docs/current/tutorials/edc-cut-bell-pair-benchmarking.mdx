...
* Des connaissances de base du [SDK Qiskit](https://docs.quantum.ibm.com/guides) et de [Qiskit Runtime](/guides/compute-services#qiskit-runtime), ainsi qu'un accès à un [compte IBM Quantum&reg;](/guides/cloud-setup).
## Contexte {#background}
...
Avant d'exécuter sur le QPU réel, nous vérifions que le circuit produit une paire de Bell en le testant sur un simulateur sans bruit avec une chaîne de quatre qubits `[0, 1, 2, 3]`. Nous utilisons le `Sampler` de Qiskit Runtime avec `AerSimulator` comme mode de backend pour exécuter les circuits.
