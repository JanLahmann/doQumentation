...
## Configuration {#setup}
Tout d'abord, authentifie-toi à l'aide de ta [clé API IBM Quantum](http://quantum.cloud.ibm.com/). Ensuite, sélectionne la fonction Qiskit comme suit. (Ce code suppose que tu as déjà [enregistré ton compte](/guides/functions-get-started#install-qiskit-functions-catalog-client) dans ton environnement local.)
...
Prends une minute pour donner ton avis sur ce tutoriel. Tes commentaires nous aideront à améliorer notre contenu et l'expérience utilisateur.
