...
Dans ce tutoriel, nous définirons `default_precision`, `optimization_level: 3` et `mitigation_level: 3`, ce qui activera le gate twirling et l'extrapolation de bruit zéro (ZNE) via l'amplification probabiliste des erreurs (PEA) en plus des paramètres par défaut du niveau 1.
