...
Configurez la boucle `for` pour exécuter les calculs HiVQE avec les géométries et les options définies ci-dessous. Les tâches sont soumises dans la boucle `for`. Dans ce tutoriel, vous soumettrez six géométries et récupérerez les résultats lorsqu'elles seront toutes terminées. Dans l'exécution de la fonction principale, vous devez définir `max_states` et `max_expansion_states` pour contrôler la taille maximale de la matrice de sous-espace et le nombre d'états pouvant être générés à l'aide de méthodes d'expansion CI classiques par itération. Les identifiants de tâches de la fonction seront stockés dans le dictionnaire avec chaque étiquette de géométrie pour suivre et traiter les résultats ultérieurement.
...
Le temps d'exécution QPU total utilisé dans la tâche peut être suivi en vous connectant à [IBM Quantum Platform](https://quantum.cloud.ibm.com/) et en consultant les tâches soumises avec le tag `qunova-chemistry-hivqe`.
