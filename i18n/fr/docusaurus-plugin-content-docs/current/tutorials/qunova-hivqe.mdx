---
description: "Utilisez Qunova HiVQE pour calculer les courbes de surface d'énergie potentielle (PES) de dissociation."
notebook_path: tutorials/qunova-hivqe.ipynb
sidebar_label: 'Courbes PES de dissociation avec Qunova HiVQE'
title: 'Courbes PES de dissociation avec Qunova HiVQE'
---
...
## Partie 1 : Li2S (20Q) {#part-1-li2s-20q}

### Étape 1 : Mapper les entrées classiques sur un problème quantique {#step-1-map-classical-inputs-to-a-quantum-problem}

Définissez les géométries de $Li_2S$ au format dictionnaire pour différentes distances de liaison Li-S afin de calculer la courbe PES. Ces géométries sont optimisées à l'aide de calculs B3LYP/631g.
...
Les calculs HiVQE seront effectués avec les options définies ci-dessous. En utilisant la base sto3g pour $Li_2S$, il y a 19 orbitales spatiales avec 22 électrons. Pour exécuter le cas (10o,10e) avec un calcul HiVQE, tu peux définir 10 orbitales actives et six orbitales gelées. À chaque itération, 100 shots seront utilisés pour échantillonner la configuration électronique générée par le circuit quantique ExcitationPreserving (`epa`) avec une intrication `circular` et deux répétitions (`reps`). Le nombre maximum d'itérations est fixé à 30 pour garantir la terminaison de l'itération avec convergence de l'énergie.
...
### Étapes 2 et 3 : Optimiser le problème pour l'exécution sur matériel quantique et exécuter avec la fonction HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}
...
Le temps d'exécution QPU total utilisé dans la tâche peut être suivi en te connectant à [IBM Quantum Platform](https://quantum.cloud.ibm.com/) et en consultant les tâches soumises avec le tag `qunova-chemistry-hivqe`.

### Étape 4 : Post-traitement et comparaison avec les méthodes classiques {#step-4-post-process-and-compare-with-classical-methods}
...
### Traçage de la courbe de dissociation pour Li\_2S {#plotting-the-dissociation-curve-for-li-2s}

Traçons et comparons les résultats HiVQE avec HF et CASCI.
Vous pouvez observer que tous les calculs HiVQE correspondent bien au résultat de référence classique (CASCI).
...
## Partie 2 : FeP-NO (44Q) {#part-2-fep-no-44q}

Note : pour exécuter la fonction sur le système FeP-NO avec les paramètres indiqués dans cet exemple, tu dois disposer d'une licence permettant l'utilisation de la fonction avec au moins 44 qubits. Envoie un e-mail à [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) pour obtenir des informations sur l'obtention d'une licence.

### Étape 1 : Associer les entrées classiques à un problème quantique {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
### Étapes 2 et 3 : Optimiser le problème pour l'exécution sur matériel quantique et exécuter à l'aide de la fonction HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

Sur la base de la configuration de HiVQE et des géométries, obtenez les résultats séquentiellement.

#### Soumettre le calcul pour d(Fe-N) = 1.75 $\AA$. {#submit-dfe-n-175-aa-calculation}
...
Suivez la tâche et récupérez le résultat pour le calcul d(Fe-N) = 1.75 $\AA$.
...
#### Soumettre le calcul pour d(Fe-N) = 2.00 $\AA$. {#submit-dfe-n-200-aa-calculation}
...
Suivez la tâche et récupérez le résultat pour le calcul d(Fe-N) = 2.00 $\AA$.
...
#### Soumettre le calcul pour d(Fe-N) = 5.00 $\AA$. {#submit-dfe-n-500-aa-calculation}
...
Suivez la tâche et récupérez le résultat pour le calcul d(Fe-N) = 5.00 $\AA$.
...
### Étape 4 : Post-traiter et comparer avec les méthodes classiques {#step-4-post-process-and-compare-with-classical-methods-1}
...
> **Remarque :** Cette enquête est réalisée par **IBM Quantum** et porte sur le **contenu du tutoriel** (rédigé par IBM). doQumentation fournit le site web, les traductions et l'exécution de code — pour tout retour sur ces aspects, veuillez [ouvrir une issue GitHub](https://github.com/JanLahmann/doQumentation/issues).

[Lien vers l'enquête](https://your.feedback.ibm.com/jfe/form/SV_9AizRyKFHEGzVs2)
