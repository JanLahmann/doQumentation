...
*Estimation d'utilisation : deux minutes sur un processeur Heron r2 (REMARQUE : il s'agit uniquement d'une estimation. Ton temps d'exécution peut varier.)*
## Contexte {#background}
...
Si tu souhaites approfondir la théorie, explore [l'intrication en action](/learning/courses/basics-of-quantum-information/entanglement-in-action/chsh-game) avec John Watrous.
Tu vas créer une paire intriquée entre deux qubits dans un ordinateur quantique en créant l'état de Bell $|\Phi^+\rangle = \frac{|00\rangle + |11\rangle}{\sqrt{2}}$. En utilisant la primitive Estimator, tu peux obtenir directement les valeurs d'espérance nécessaires ($\langle AB \rangle, \langle Ab \rangle, \langle aB \rangle$ et $\langle ab \rangle$) pour calculer les valeurs d'espérance des deux quantités CHSH $\langle S_1\rangle$ et $\langle S_2\rangle$. Avant l'introduction de la primitive Estimator, il fallait construire les valeurs d'espérance à partir des résultats de mesure.

Tu mesureras le second qubit dans les bases $Z$ et $X$. Le premier qubit sera également mesuré dans des bases orthogonales, mais avec un angle par rapport au second qubit, que nous allons faire varier entre $0$ et $2\pi$. Comme tu le verras, la primitive Estimator rend l'exécution de circuits paramétrés très facile. Plutôt que de créer une série de circuits CHSH, tu n'as besoin de créer qu'*un seul* circuit CHSH avec un paramètre spécifiant l'angle de mesure et une série de valeurs de phase pour ce paramètre.

Enfin, tu analyseras les résultats et les traceras en fonction de l'angle de mesure. Tu verras que pour certaines plages d'angles de mesure, les valeurs d'espérance des quantités CHSH $|\langle S_1\rangle| > 2$ ou $|\langle S_2\rangle| > 2$, ce qui démontre la violation de l'inégalité CHSH.
...
### Créer un circuit CHSH paramétré {#create-a-parameterized-chsh-circuit}
...
### Créer une liste de valeurs de phase à assigner ultérieurement {#create-a-list-of-phase-values-to-be-assigned-later}

Après avoir créé le circuit CHSH paramétré, tu vas créer une liste de valeurs de phase à assigner au circuit à l'étape suivante. Tu peux utiliser le code suivant pour créer une liste de 21 valeurs de phase allant de $0$ à $2 \pi$ avec un espacement régulier, c'est-à-dire $0$, $0.1 \pi$, $0.2 \pi$, ..., $1.9 \pi$, $2 \pi$.
...
Dans la figure, les lignes et les zones grises délimitent les bornes ; les lignes extérieures (en pointillés alternés) délimitent les bornes quantiques ($\pm 2\sqrt{2}$), tandis que les lignes intérieures (en tirets) délimitent les bornes classiques ($\pm 2$). Tu peux observer qu'il existe des régions où les quantités témoins CHSH dépassent les bornes classiques. Félicitations ! Tu as démontré avec succès la violation de l'inégalité CHSH dans un véritable système quantique !
