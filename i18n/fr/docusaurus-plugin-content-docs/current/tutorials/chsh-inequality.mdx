...
{/* doqumentation-source-hash: 16a99671 */}
...
Nous exécutons le balayage de paramètres avec `EstimatorV2` en mode `aer_sim`. La méthode `run()` de l'Estimator prend un itérable de PUBs. Chaque PUB a le format `(circuit, observables, parameter_values, precision)`. Nous passons les deux observables ensemble afin qu'elles partagent le même balayage de paramètres.
...
Un test CHSH est intrinsèquement une expérience à *deux qubits*, donc il ne passe pas à l'échelle en agrandissant un seul circuit. Il passe plutôt à l'échelle en exécutant **de nombreux tests en parallèle**. Ici, nous pavons le backend avec autant de paires de Bell disjointes que sa connectivité le permet (un *couplage* de la carte de connectivité) et nous exécutons un sous-circuit CHSH indépendant sur chaque paire, le tout en une seule tâche.
