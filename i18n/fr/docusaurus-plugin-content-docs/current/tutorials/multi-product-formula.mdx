...
1. Pour un budget fixe de pas de Trotter que tu peux exécuter, tu peux obtenir des résultats avec une erreur de Trotter qui est plus petite au total.
2. Étant donné un nombre cible de pas de Trotter qui est trop grand pour être exécuté, tu peux utiliser la MPF pour trouver une collection de circuits de profondeur inférieure à exécuter qui résulte en une erreur de Trotter similaire.
...
De plus, tu peux prendre en compte si la PF est symétrique ou non (voir [\[1\]](#references)) en définissant `symmetric=True/False`.
...
Si c'est le cas et que tu ne peux pas choisir un ensemble différent de valeurs $k_j$, tu peux utiliser une solution approximative au LSE au lieu d'une exacte.
...
Note que tu as une liberté complète sur la façon de résoudre ce problème d'optimisation, ce qui signifie que tu peux changer le solveur d'optimisation, ses seuils de convergence, etc.
...
Ensuite, crée l'état initial de ton système dans un format compatible avec TeNPy (par exemple, un `MPS_neel_state`=$\vert 0101...01 \rangle$). Cela configure la fonction d'onde à plusieurs corps que tu feras évoluer dans le temps $\lvert\psi_{\mathrm{in}}\rangle$ comme un tenseur.
