---
description: "Résoudre un problème d'optimisation quantique à l'échelle utilitaire en utilisant l'Optimization Solver, une fonction Qiskit par Q-CTRL Fire Opal"
notebook_path: tutorials/solve-higher-order-binary-optimization-problems-with-q-ctrls-optimization-solver.ipynb
sidebar_label: "Optimisation binaire d'ordre supérieur avec l'Optimization Solver de Q-CTRL"
title: "Optimisation binaire d'ordre supérieur avec l'Optimization Solver de Q-CTRL"
---
...
Les fonctions Qiskit sont une fonctionnalité expérimentale disponible uniquement pour les utilisateurs des plans IBM Quantum&reg; Premium, Flex et On-Prem (via l'API IBM Quantum Platform). Elles sont en version préliminaire et sujettes à modification.
</Admonition>
...
Ce tutoriel montre comment résoudre un problème d'optimisation binaire d'ordre supérieur (HOBO) en utilisant l'[Optimization Solver, une fonction Qiskit par Q-CTRL Fire Opal](/guides/q-ctrl-optimization-solver). L'exemple présenté dans ce tutoriel est un problème d'optimisation conçu pour trouver l'énergie de l'état fondamental d'un modèle d'Ising à liaisons aléatoires de 156 qubits possédant des termes cubiques. L'Optimization Solver peut être utilisé pour des problèmes d'optimisation généraux pouvant être définis comme une fonction objectif.

L'Optimization Solver automatise entièrement les étapes d'implémentation adaptées au matériel pour la résolution de problèmes d'optimisation sur du matériel quantique, et en exploitant le [Performance Management](/guides/q-ctrl-performance-management) pour l'exécution quantique, il atteint des solutions précises à l'échelle utilitaire. Pour un résumé détaillé du flux de travail complet de l'Optimization Solver et des résultats de benchmarking, consulte [le manuscrit publié](https://arxiv.org/abs/2406.01743).

Ce tutoriel te guide à travers les étapes suivantes :

1. Définir le problème comme une fonction objectif

2. Exécuter l'algorithme hybride en utilisant l'Optimization Solver de Fire Opal
...
Tu devras également obtenir l'accès à la fonction Optimization Solver. [Remplis le formulaire](https://quantum.cloud.ibm.com/functions?id=q-ctrl-optimization-solver) pour demander l'accès.

## Configuration {#setup}

Tout d'abord, importe les packages et outils nécessaires.
...
Définis tes identifiants [IBM Quantum Platform](https://quantum.cloud.ibm.com/), qui seront utilisés tout au long du tutoriel pour l'authentification auprès d'IBM Quantum Compute Service et des fonctions Qiskit.
...
## Étape 1 : Définir le problème comme une fonction objectif {#step-1-define-the-problem-as-an-objective-function}
L'Optimization Solver accepte soit une fonction objectif, soit un graphe en entrée. Dans ce tutoriel, le problème de minimisation du verre de spin d'Ising est défini comme une fonction objectif, et il a été adapté à la topologie heavy-hex des appareils IBM&reg;.
...
Pour une discussion détaillée de la construction de la définition du problème et des résultats précédents obtenus avec l'Optimization Solver, consulte [ce manuscrit technique](https://arxiv.org/abs/2406.01743). Le problème a été initialement défini et évalué dans le cadre d'un [article publié par le Los Alamos National Laboratory](https://arxiv.org/abs/2312.00997), et il a été adapté pour exploiter la pleine largeur des processeurs IBM Quantum Heron à 156 qubits.
...
## Étape 2 : Exécuter l'algorithme hybride en utilisant l'Optimization Solver de Fire Opal {#step-2-run-the-hybrid-algorithm-using-the-fire-opal-optimization-solver}
Utilise maintenant la fonction Qiskit Optimization Solver pour exécuter l'algorithme. En coulisses, l'Optimization Solver se charge de mapper le problème vers un algorithme quantique hybride, d'exécuter les circuits quantiques avec suppression d'erreurs, et de réaliser l'optimisation classique.
...
Vérifie que l'appareil choisi possède au moins 156 qubits.
...
Tu peux utiliser les [API Qiskit Serverless](/guides/serverless) habituelles pour vérifier le statut de ta charge de travail de fonction Qiskit :
...
## Étape 3 : Évaluer les résultats {#step-3-evaluate-results}
...
Le Solver a trouvé la solution correcte, qui a été validée à l'aide d'un logiciel d'optimisation classique. La complexité de ce problème à l'échelle utilitaire nécessite un logiciel d'optimisation avancé pour être résolu de manière classique, tel que [IBM ILOG CPLEX Optimization Studio (CPLEX)](https://www.ibm.com/products/ilog-cplex-optimization-studio) ou [Gurobi Optimization](https://www.gurobi.com/).
Pour une analyse visuelle de la qualité des résultats, tu peux tracer les résultats en calculant les valeurs de coût à partir des chaînes de bits et de leurs probabilités. À titre de comparaison, trace les résultats aux côtés d'une distribution de chaînes de bits échantillonnées aléatoirement, ce qui équivaut à une solution classique par « force brute ». Si l'algorithme trouve systématiquement des coûts plus bas, cela suggère que l'algorithme quantique résout efficacement le problème d'optimisation.
