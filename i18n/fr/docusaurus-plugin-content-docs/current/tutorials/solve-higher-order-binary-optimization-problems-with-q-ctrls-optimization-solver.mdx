...
L'Optimization Solver automatise entièrement les étapes d'implémentation adaptées au matériel pour la résolution de problèmes d'optimisation sur du matériel quantique, et en exploitant le [Performance Management](/guides/q-ctrl-performance-management) pour l'exécution quantique, il atteint des solutions précises à l'échelle utilitaire. Pour un résumé détaillé du flux de travail complet de l'Optimization Solver et des résultats de benchmarking, consultez [le manuscrit publié](https://arxiv.org/abs/2406.01743).
...
Vous devrez également obtenir l'accès à la fonction Optimization Solver. [Remplissez le formulaire](https://quantum.cloud.ibm.com/functions?id=q-ctrl-optimization-solver) pour demander l'accès.
## Configuration {#setup}
...
Définissez vos identifiants [IBM Quantum Platform](https://quantum.cloud.ibm.com/), qui seront utilisés tout au long du tutoriel pour l'authentification auprès de Qiskit Runtime et des fonctions Qiskit.
...
Vous pouvez utiliser les [API Qiskit Serverless](/guides/serverless) habituelles pour vérifier le statut de votre charge de travail de fonction Qiskit :
