...
* [Concepts de base de l'informatique quantique](/learning/courses/basics-of-quantum-information), notamment les états de Bell, l'intrication et les portes quantiques ;
* La maîtrise des [circuits dynamiques](/guides/classical-feedforward-and-control-flow) (mesures en milieu de circuit et feedforward classique) ;
...
- Qiskit Runtime v0.37 ou ultérieur (`pip install qiskit-ibm-runtime`)
...
Avant d'exécuter sur le QPU réel, nous vérifions que les circuits dynamiques et unitaires produisent un état de Bell idéal sur un simulateur sans bruit. Nous utilisons le `Sampler` d'IBM Quantum avec `AerSimulator` comme mode de backend, pour une petite distance de 6.
...
#### Initialiser le circuit {#initialize-circuit}
...
#### Préparer les paires de Bell {#prepare-bell-pairs}
...
#### Mesurer les paires de qubits voisins dans la base de Bell {#measure-neighboring-qubit-pairs-in-the-bell-basis}
...
#### Appliquer les corrections par feedforward pour corriger les opérateurs sous-produits de Pauli {#apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
#### Mesurer les qubits de contrôle et cible {#measure-control-and-target-qubits}
...
- Initialiser le qubit de contrôle dans $|+\rangle$
...
Nous construisons maintenant les circuits qui mesurent dans les bases $XX$, $YY$ et $ZZ$, de la même manière que pour les circuits dynamiques ci-dessus.
...
![Sortie de la cellule de code précédente](/docs/images/tutorials/long-range-entanglement/extracted-outputs/b899e143-0.avif)
...
Comme attendu dans une simulation sans bruit, les fidélités des circuits dynamiques et unitaires sont $1$.
...
Nous générons maintenant des circuits CX à longue portée pour une gamme de séparations entre qubits allant jusqu'à 60 qubits d'écart. Pour chaque distance, nous construisons des circuits qui mesurent dans les bases $XX$, $YY$ et $ZZ$, qui seront utilisés ultérieurement pour calculer les fidélités.
...
#### Utiliser la chaîne de fidélité de couche pour sélectionner une chaîne 1D {#use-layer-fidelity-string-for-selecting-1d-chain}
...
- [Benchmarker les circuits dynamiques avec des paires de Bell coupées](/tutorials/edc-cut-bell-pair-benchmarking)
- [Simulation du Hamiltonien d'Ising perturbé avec des circuits dynamiques](/tutorials/dc-hex-ising)
</Admonition>
