...
Vous devrez également obtenir l'accès à la [fonction Iskay Quantum Optimizer](https://quantum.cloud.ibm.com/functions?id=kipu-quantum-iskay-quantum-optimizer) depuis le catalogue Qiskit Functions.
## Configuration {#setup}
...
Définissez vos identifiants [IBM Quantum&reg; Platform](https://quantum.cloud.ibm.com/). Vous aurez besoin de :
- **Jeton API** : votre clé API de 44 caractères depuis IBM Quantum Platform
- **CRN de l'instance** : l'identifiant de votre instance IBM Cloud&reg;
...
- En choisissant `problem_type = "binary"`, vous spécifiez que la fonction de coût est au format `binary`, ce qui signifie que $D = \{0,  1\}^{n}$, c'est-à-dire que la fonction de coût est écrite sous forme QUBO/HUBO.
...
pour $i=j$ (puisque $x_i \in \{0,1\}$ implique $x_i \cdot x_i = x_i$). Ainsi, dans votre formulation QUBO, si vous avez à la fois des contributions linéaires $b_i x_i$ et des contributions quadratiques diagonales $c_{i,i} x_i^2$, ces termes doivent être combinés en un seul coefficient linéaire :
...
**Exemple :** Si votre QUBO contient $3x_1 + 2x_1^2 + 4x_1 x_2$, le dictionnaire Iskay doit contenir :
...
- **Sélection du Backend** : choisissez parmi les QPU disponibles (par exemple, `"ibm_fez"`) en fonction de vos besoins et de votre instance de ressources de calcul.
...
5. **Appliquer à votre domaine** : adaptez les techniques de formulation QUBO/HUBO à vos propres problèmes d'optimisation
