---
description: 'Apprends à calculer les effets de solvant implicite sur du matériel quantique en utilisant SQD et IEF-PCM.'
notebook_path: tutorials/implicit-solvent-calculations.ipynb
sidebar_label: 'Calculs de solvant implicite en utilisant Qiskit Serverless'
title: 'Calculs de solvant implicite en utilisant Qiskit Serverless'
---
...
*Estimation d'utilisation : 2 minutes sur un processeur Heron r2 (REMARQUE : ceci est une estimation uniquement. Ton temps d'exécution peut varier.)*
...
* Comment configurer et exécuter un flux de travail distant en utilisant [Qiskit Serverless](/guides/serverless)

* Comment calculer les effets de solvant implicite en utilisant un ordinateur quantique


## Prérequis {#prerequisites}

Nous suggérons que les utilisateurs se familiarisent avec le sujet suivant avant de suivre ce tutoriel :

* [Diagonalisation quantique basée sur l'échantillonnage](/learning/courses/quantum-diagonalization-algorithms/sqd-overview)


## Contexte {#background}

Les calculs de solvant implicite sont fréquemment utilisés en biophysique computationnelle. Ces modèles décrivent comment un composé soluté interagit avec un solvant, sans modéliser directement le système du solvant. À la place, une approximation est faite dans laquelle le modèle du système soluté est enveloppé dans une représentation mathématique d'un milieu diélectrique caractérisé empiriquement. Cette approximation diélectrique interagit alors avec le soluté, qui est lui-même modélisé directement. Le milieu diélectrique influence les caractéristiques du système soluté, comme son énergie de l'état fondamental, en interagissant avec son champ électronique. Cela est important pour les modèles biophysiques utilisés, par exemple, dans la découverte de médicaments, car les composés se comportent différemment dans divers environnements diélectriques. Modéliser un composé dans l'air (in vacuo) décrira un comportement différent que le modéliser dans l'eau. Puisque les composés pharmaceutiques doivent entrer dans le corps humain, qui est lui-même composé principalement d'eau, il est utile de modéliser un composé dans une solution comme l'eau plutôt que in vacuo. Avec les modèles de solvant implicite, nous pouvons obtenir ce comportement à faible coût, bien que le résultat final soit généralement plus approximatif que son équivalent : les modèles de solvant explicite, plus coûteux en calcul, qui créent des représentations directes à la fois du soluté et des molécules de solvant.

Dans ce tutoriel, nous démontrons comment un algorithme quantique, la diagonalisation quantique basée sur l'échantillonnage (SQD), peut être intégré dans un modèle de solvant implicite relativement peu coûteux en calcul. Dans l'exemple, nous décrivons comment la méthylamine se comporte lorsqu'elle se dissout dans l'eau. Nous comparons l'algorithme quantique à une méthode de comparaison classique de pointe appelée CASCI et démontrons une bonne concordance entre ces calculs. Nous présentons une architecture d'informatique supercalculée quantique-centrique en miniature, déchargeant le post-traitement classique coûteux en calcul de la partie échantillonnage quantique de la routine vers un environnement basé sur le cloud au sein de Qiskit Serverless. Le code démontre également la parallélisation à travers les cœurs de CPU disponibles à distance pour améliorer le temps de calcul.

Qiskit Serverless est un framework pour exécuter des charges de travail quantiques et classiques distribuées sans gérer d'infrastructure. Il n'y a pas de provisionnement de serveur (pas de démarrage d'instances EC2, de clusters, de conteneurs Docker), pas d'outils d'orchestration (Kubernetes, Docker Swarm) et pas de surveillance/maintenance. Chaque tâche Serverless s'exécute dans un conteneur propre, exécute ton code, puis s'arrête. Il n'y a pas de mémoire entre les tâches. Tu écris simplement ton code puis soumets ta tâche. Au sein d'une tâche Serverless, un programme peut accéder de manière transparente aux backends IBM Quantum&reg; et post-traiter les résultats de manière classique. Avec Qiskit Serverless, les utilisateurs peuvent accéder à des cœurs de CPU et à de la mémoire distants toujours actifs, ce qui permet la distribution de certaines charges de travail classiques sur des ressources distantes. Les utilisateurs bénéficient également d'un certain avantage dans le traitement parallèle des programmes tout en évitant les problèmes courants liés à l'arrêt des appareils en cours d'exécution. Pour plus d'informations sur Qiskit Serverless, consulte sa [documentation](/guides/serverless), ainsi que du matériel supplémentaire sur [GitHub](https://qiskit.github.io/qiskit-serverless/index.html).

Ce tutoriel montre une application pertinente de ce qui suit :

* Diagonalisation quantique basée sur l'échantillonnage

* Modèles de calcul client-serveur pour l'informatique quantique


Ce tutoriel est inspiré et basé sur des recherches menées à la Cleveland Clinic, comme décrit dans [Kaliakin, Danil, et al. "Implicit solvent sample-based quantum diagonalization." The Journal of Physical Chemistry B 129.23 (2025): 5788-5796](https://pubs.acs.org/doi/10.1021/acs.jpcb.5c01030), exposant le flux de travail complet pour les calculs de solvant implicite et l'étendant avec l'auto-cohérence itérative du solvant (« L'algorithme Heartwood », M. Motta, T. Pellegrini, 2025), l'optimisation de géométrie, et la sélection automatique de la disposition des qubits. Veuillez consulter le [modèle de fonction Qiskit SQD IEF-PCM](/guides/function-template-chemistry-workflow), développé conjointement par Cleveland Clinic et IBM&reg; sur la base des recherches de Cleveland Clinic, pour une interface simplifiée et prête à l'emploi pour exécuter des calculs de solvant implicite.
...
* Module complémentaire Qiskit : Diagonalisation quantique basée sur l'échantillonnage (SQD) v0.12.0 `pip install qiskit_addon_sqd`
...
Crée un répertoire exactement à côté du programme notebook principal appelé `source_files`. Tu placeras des fichiers Python dans ce répertoire que tu as l'intention de partager avec l'environnement de calcul distant. Tu dois créer deux fichiers :
...
Clique pour développer le texte de chaque script ci-dessous, puis copie et colle le contenu dans un fichier local avec ces noms de chemin.

<Accordion>
  <AccordionItem title="Cliquer pour voir `source_files\diagonalization_engine.py`">
    
  </AccordionItem>

  <AccordionItem title="Cliquer pour voir `source_files\classical_simulation.py`">
...
Nous devons partager le programme destiné à être exécuté dans l'environnement cloud, et le retélécharger chaque fois que nous modifions son code source :
...
Ce tutoriel n'utilise pas de simulateur à petite échelle car l'intention est de démontrer une application quantique évolutive qui va au-delà du domaine de l'exploration par simulateur. Au lieu de cela, nous montrons plus tard comment cette méthode peut être implémentée en utilisant une méthode de comparaison classique de pointe appelée CASCI.
...
### 1.1 : Initialiser l'objet molécule en utilisant une géométrie moléculaire connue $\textit{a priori}$ {#11-initialize-molecule-object-using-known-textita-priori-molecular-geometry}
...
### 1.2 : Définir les effets de solvatation en utilisant le modèle de continuum polarisable (PCM) {#12-define-solvation-effects-using-the-polarizable-continuum-model-pcm}
...
### 1.3 : Optimisation de géométrie en utilisant TRIC {#13-geometry-optimization-using-tric}
...
### 1.4 : Préparer le modèle de continuum et l'objet de champ moyen avec les variables pertinentes {#14-prepare-continuum-model-and-mean-field-object-with-relevant-variables}
...
Pour plus d'informations sur les fonctions d'aide présentées ici, consulte le tutoriel [Diagonalisation quantique basée sur l'échantillonnage d'un hamiltonien chimique](/tutorials/sample-based-quantum-diagonalization).
...
## Étapes 3 et 4 : Exécuter avec Qiskit et post-traiter avec Qiskit Serverless {#steps-3-4-execute-using-qiskit-and-post-process-using-qiskit-serverless}

Ici, nous combinons l'étape 3 (Exécuter) et l'étape 4 (Post-traiter) car le contexte applicatif du modèle de solvant implicite nécessite une boucle de rétroaction itérative qui exécute plusieurs cycles d'exécution et de post-traitement pour améliorer le calcul final.

### 3.1 Calculer l'énergie de Hartree-Fock restreinte {#31-compute-the-restricted-hartree-fock-energy}
...
### 3.2 : Établir l'énergie de référence classique avec CASCI {#32-establish-classical-reference-energy-with-casci}
...
## Configurer les paramètres de l'application {#configure-application-parameters}
...
<Admonition type="tip" title="Recommandations">
  ...
  * L'[Interface de gestion des ressources quantiques (QRMI)](https://github.com/qiskit-community/qrmi) — intègre les ressources quantiques et classiques dans les gestionnaires de charge de travail HPC comme Slurm, étendant le modèle de calcul distribué démontré dans ce tutoriel

  * [Guide de Qiskit Serverless](/guides/serverless)

  * [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html)

  * Tutoriel [Diagonalisation quantique basée sur l'échantillonnage d'un hamiltonien chimique](/tutorials/sample-based-quantum-diagonalization)

  * [Aperçu de la diagonalisation quantique basée sur l'échantillonnage (SQD)](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd)

  * [Déployer et exécuter un modèle pour la simulation de structure électronique avec un modèle de solvant implicite](/guides/function-template-chemistry-workflow) (modèle de fonction Qiskit SQD IEF-PCM, développé conjointement par Cleveland Clinic et IBM)
</Admonition>
