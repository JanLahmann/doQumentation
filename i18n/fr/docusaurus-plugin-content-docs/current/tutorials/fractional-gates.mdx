...
Pour utiliser les portes fractionnaires, vous pouvez obtenir un backend qui les prend en charge en définissant l'option `use_fractional_gates=True`.
Si le backend prend en charge les portes fractionnaires, vous verrez `rzz` et `rx` parmi ses portes de base.
...
Vous pouvez obtenir la valeur de la fonction de noyau $K(x, x)$ en mesurant la probabilité de la chaîne de bits tout-zéro `00...00` dans la sortie.
...
Cependant, si vous utilisez uniquement les portes RX fractionnaires et excluez les portes RZZ fractionnaires, vous pouvez continuer à suivre le flux de travail standard Qiskit patterns.
...
Dans cette section, nous démontrons comment optimiser vos circuits en utilisant uniquement les portes RX fractionnaires, tout en omettant les portes RZZ.
...
Vous devrez installer le [package](https://github.com/Qiskit/qiskit-basis-constructor) `qiskit-basis-constructor` pour cette section.
...
Consultez [ce problème](https://github.com/Qiskit/qiskit/issues/14318) pour plus de détails.
#### Échec lorsque les circuits contiennent certaines portes {#failure-when-circuits-contain-certain-gates}

Si votre circuit inclut certaines portes telles que `XXPlusYYGate`, le Transpiler Qiskit peut générer des portes RZZ avec des angles invalides.
Si vous rencontrez ce problème, consultez ce [problème GitHub](https://github.com/Qiskit/qiskit-ibm-runtime/issues/2256#issuecomment-2889487152) pour une solution de contournement.
