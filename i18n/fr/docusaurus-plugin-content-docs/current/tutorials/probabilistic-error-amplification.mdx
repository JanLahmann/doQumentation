---
description: "Exécute une expérience d'atténuation des erreurs à l'échelle utilitaire avec l'extrapolation au bruit nul et l'amplification probabiliste des erreurs."
...
{/* doqumentation-source-hash: d330b37d */}
...
*Estimation d'utilisation : 14 minutes sur un processeur Heron r3 (REMARQUE : ceci n'est qu'une estimation. Ton temps d'exécution peut varier.)*

## Résultats d'apprentissage {#learning-outcomes}
Après avoir suivi ce tutoriel, les utilisateurs devraient comprendre :
- La théorie derrière l'*extrapolation au bruit nul* (ZNE), les différentes méthodes d'amplification du bruit, et pourquoi l'*amplification probabiliste des erreurs* (PEA) est préférable pour les expériences à l'échelle utilitaire.
- Comment implémenter la ZNE avec la PEA en pratique à l'aide de Qiskit.

## Prérequis {#prerequisites}
Nous suggérons que les utilisateurs soient familiers avec les sujets suivants avant de suivre ce tutoriel :
- La [leçon sur l'atténuation des erreurs](/learning/courses/utility-scale-quantum-computing/error-mitigation) du cours *Informatique quantique à l'échelle utilitaire* pour une connaissance de base de l'utilisation de l'atténuation des erreurs dans Qiskit.
- La [leçon Utility-I](/learning/courses/utility-scale-quantum-computing/utility-i) du cours *Informatique quantique à l'échelle utilitaire* pour plus de contexte sur l'expérience à l'échelle utilitaire utilisée comme exemple dans ce tutoriel.

## Contexte {#background}

Ce tutoriel montre comment exécuter une expérience d'atténuation des erreurs à l'échelle utilitaire avec IBM Quantum Compute Service en utilisant une version expérimentale de l'*extrapolation au bruit nul* (ZNE) avec l'*amplification probabiliste des erreurs* (PEA).

![kim\_nature\_fig.png](/docs/images/tutorials/utility-scale-error-mitigation-with-probabilistic-error-amplification/e1e67c34-9d4d-4a88-9340-f0b2f3676770.avif)
...
![zne\_stages.png](/docs/images/tutorials/utility-scale-error-mitigation-with-probabilistic-error-amplification/5e63d706-82d8-4212-b802-c9191ce53341.avif)
...
**Référence** : E. van den Berg, Z. Minev, A. Kandala, et K. Temme, *Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors* [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)
...
## Configuration {#setup}
Dans la cellule ci-dessous, nous importons les packages nécessaires et créons quelques fonctions utilitaires pour construire les circuits pour l'évolution temporelle de Trotter d'un modèle d'Ising transverse bidimensionnel conforme à la topologie du backend.
...
Nous allons ignorer cette étape car l'atténuation d'erreurs d'exécution n'est pas prise en charge par les simulateurs.
...
##### Établir un backend {#establish-a-backend}
Tout d'abord, choisis un backend sur lequel exécuter. Cette démonstration s'exécute sur un backend de 127 qubits, mais tu peux adapter cela à tout backend disponible pour toi.
...
Pour implémenter la simulation d'Ising trottérisée, définis trois couches de couplages de portes à deux qubits pour le dispositif, qui seront répétées à chaque étape de Trotter. Celles-ci définissent les trois couches twirl dont tu dois apprendre le bruit pour implémenter l'atténuation.
...
Examine la carte de couplage du backend et vérifie si des qubits sont connectés à des couplages présentant une erreur élevée. Retire ces qubits « défectueux » de ton expérience.
...
![Output of the previous code cell](/docs/images/tutorials/probabilistic-error-amplification/extracted-outputs/fccef708-0.avif)
...
Avant d'exécuter le circuit sur le matériel, optimise-le pour l'exécution matérielle. Ce processus comprend plusieurs étapes :

* Choisissez une disposition de qubits qui associe les qubits virtuels de votre circuit à des qubits physiques sur le matériel.

* Insérez des portes swap si nécessaire pour router les interactions entre des qubits qui ne sont pas connectés.

* Traduisez les portes de notre circuit en instructions [Instruction Set Architecture (ISA)](/guides/transpile#instruction-set-architecture) qui peuvent être exécutées directement sur le matériel.

* Effectuez des optimisations de circuit pour minimiser la profondeur du circuit et le nombre de portes.


Bien que le transpiler intégré à Qiskit puisse effectuer toutes ces étapes, ce tutoriel montre comment construire le circuit de Trotter à l'échelle utilitaire de manière ascendante. Sélectionne les bons qubits physiques et définis les couches d'intrication sur les paires de qubits connectés parmi les qubits sélectionnés. Néanmoins, tu dois toujours traduire les portes non-ISA dans le circuit et exploiter toute optimisation de circuit offerte par le transpiler.

Transpile ton circuit pour le backend choisi en créant un gestionnaire de passes puis en exécutant le gestionnaire de passes sur le circuit. Fixe également la disposition initiale du circuit aux `good_qubits` déjà sélectionnés. Un moyen simple de créer un gestionnaire de passes est d'utiliser la fonction [`generate_preset_pass_manager`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.generate_preset_pass_manager). Consulte [Transpiler avec des gestionnaires de passes](/guides/transpile-with-pass-managers) pour une explication plus détaillée de la transpilation avec des gestionnaires de passes.
...
Ensuite, crée toutes les observables $\langle Z \rangle$ de poids 1 pour chaque qubit virtuel en complétant avec le nombre nécessaire de termes $\langle I \rangle$.
...
Le processus de transpilation a mappé les qubits virtuels de ton circuit vers les qubits physiques du matériel. L'information sur la disposition des qubits est stockée dans l'attribut `layout` du circuit transpilé. Ton observable est également définie en termes de qubits virtuels, tu dois donc appliquer cette disposition à l'observable. Cela se fait en utilisant la méthode `apply_layout` de `SparsePauliOp`.

Note que chaque observable est encapsulée dans une liste dans le bloc de code suivant. Cela permet le _broadcasting_ avec les valeurs de paramètres afin que chaque observable de qubit soit mesurée pour chaque valeur de theta. Les règles de broadcasting pour les primitives sont disponibles [ici](/guides/primitives).
...
Configure ensuite les options de l'`Estimator` nécessaires pour exécuter l'expérience d'atténuation. Cela inclut les options pour l'apprentissage du bruit des couches d'intrication et pour l'extrapolation ZNE.
...
Ce qui suit donne des détails sur les options supplémentaires dans la branche expérimentale. Note que ces options et noms ne sont pas finalisés, et tout ici est susceptible de changer avant une version officielle.

* **amplifier** : La méthode à utiliser lors de l'amplification du bruit aux facteurs de bruit prévus.
  Les valeurs autorisées sont `"gate_folding"`, qui amplifie en répétant les portes de base à deux qubits,
  et `"pea"`, qui amplifie par échantillonnage probabiliste après avoir appris le modèle de bruit à twirling de Pauli
  pour les couches de portes de base à deux qubits twirlées. Des options supplémentaires sont `"gate_folding_front"` et `"gate_folding_back"`, qui sont expliquées dans la [documentation de l'API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier).

* **extrapolated\_noise\_factors** : Spécifiez une ou plusieurs valeurs de facteur de bruit auxquelles évaluer les
  modèles extrapolés. S'il s'agit d'une séquence de valeurs, les résultats retournés seront des tableaux avec le facteur de bruit spécifié évalué pour le modèle d'extrapolation. Une valeur
  de 0 correspond à l'extrapolation à bruit nul.
...
Une fois l'expérience terminée, tu peux consulter tes résultats. Tu récupères les valeurs attendues brutes et atténuées et les compares avec les résultats exacts. Ensuite, trace les valeurs attendues, à la fois atténuées (extrapolées) et brutes, moyennées sur tous les qubits pour chaque paramètre. Enfin, trace les valeurs attendues pour les qubits individuels de ton choix.
...
1. `pub_result.data.evs` : Valeurs attendues correspondant au bruit nul (basées sur la meilleure extrapolation heuristique).
   ...
   * Le premier axe est l'index de qubit virtuel pour l'observable $\langle Z_i\rangle$ ($124$ qubits/observables virtuels)

   * Le deuxième axe indexe la valeur du paramètre pour $\theta$ ($12$ valeurs de paramètre)
...
![Output of the previous code cell](/docs/images/tutorials/probabilistic-error-amplification/extracted-outputs/e466736a-0.avif)
...
![Output of the previous code cell](/docs/images/tutorials/probabilistic-error-amplification/extracted-outputs/bea9695a-0.avif)
...
- Un [tutoriel](/tutorials/combine-error-mitigation-techniques) axé sur la combinaison de techniques d'atténuation des erreurs.
- Une [documentation détaillée](/guides/error-mitigation-and-suppression-techniques) sur les techniques d'atténuation des erreurs disponibles dans Qiskit.
- Des leçons supplémentaires couvrant les expériences à l'échelle utilitaire : [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) et [Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
