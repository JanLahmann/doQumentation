...
- D'abord, nous découvrirons les [primitives IBM Quantum](/guides/primitives)
...
- Nous définirons une stratégie de mesure avec les primitives IBM Quantum pour équilibrer vitesse et précision
...
Le [`Sampler`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2) d'IBM Quantum exécute le circuit plusieurs fois sur un dispositif quantique, effectue des mesures à chaque exécution, et reconstruit la distribution de probabilité à partir des chaînes de bits récupérées. Plus il y a d'exécutions (ou *shots*), plus les résultats seront précis, mais cela nécessite plus de temps et de ressources quantiques.
...
Nous allons maintenant procéder à l'exécution sur un véritable ordinateur quantique. Note les changements de syntaxe. Les étapes impliquant le pass_manager seront abordées plus en détail dans l'exemple suivant. Une étape particulièrement importante dans les algorithmes variationnels est l'utilisation d'une session IBM Quantum Compute Service. Démarrer une session te permet d'exécuter plusieurs itérations d'un algorithme variationnel sans attendre dans une nouvelle file d'attente à chaque mise à jour des paramètres. Cela est important si les temps d'attente sont longs et/ou si de nombreuses itérations sont nécessaires. Note que les utilisateurs du Open Plan ne peuvent pas utiliser les sessions Quantum Compute. Si tu n'as pas accès aux sessions, tu peux réduire le nombre d'itérations que tu soumets à un moment donné, et sauvegarder les paramètres les plus récents pour les utiliser lors de futures exécutions. Si tu soumets trop d'itérations ou si tu rencontres des temps d'attente trop longs, tu peux rencontrer le code d'erreur 1217, qui fait référence à de longs délais entre les soumissions de jobs.
...
Avec l'Estimator d'IBM Quantum prenant directement un Hamiltonien et un ansatz paramétré, et retournant l'énergie nécessaire, la fonction de coût pour une instance de QAOA est assez simple :
...
Nous pouvons utiliser les options de suppression et d'atténuation des erreurs des primitives d'IBM Quantum pour faire face au bruit et maximiser l'utilité des ordinateurs quantiques actuels.

### Suppression des erreurs {#error-suppression}

La [suppression des erreurs](/guides/error-mitigation-and-suppression-techniques) fait référence aux techniques utilisées pour optimiser et transformer un circuit pendant la compilation afin de minimiser les erreurs. Il s'agit d'une technique basique de gestion des erreurs qui introduit normalement un certain <DefinitionTooltip definition="Extra costs introduced by new techniques, relative to a base implementation.">surcoût</DefinitionTooltip> de prétraitement classique dans le temps d'exécution total. Ce surcoût inclut la transpilation des circuits pour les exécuter sur du matériel quantique en :
...
L'[atténuation des erreurs](/guides/error-mitigation-and-suppression-techniques) fait référence à des techniques qui permettent de réduire les erreurs d'un circuit en modélisant le bruit du dispositif au moment de l'exécution. En général, cela implique un surcoût de prétraitement quantique lié à l'entraînement du modèle et un surcoût de post-traitement classique pour atténuer les erreurs dans les résultats bruts à partir du modèle généré.

L'option `resilience_level` des primitives IBM Quantum spécifie le niveau de résilience face aux erreurs que l'on souhaite construire. Les niveaux plus élevés génèrent des résultats plus précis au prix de temps de traitement plus longs, dus au surcoût d'échantillonnage quantique plus important. Les niveaux de résilience peuvent être utilisés pour configurer l'équilibre entre coût et précision lors de l'application de l'atténuation des erreurs à ta requête de primitives.

Lors de l'implémentation de toute technique d'atténuation d'erreurs, nous nous attendons à ce que le <DefinitionTooltip definition="A systematic drift in the measured quantities, usually caused by errors.">biais</DefinitionTooltip> dans nos résultats soit réduit par rapport au biais précédent, non atténué. Dans certains cas, le biais peut même disparaître. Cependant, cela a un coût. Alors que nous réduisons le biais dans nos quantités estimées, la variabilité statistique augmentera (c'est-à-dire la variance), ce que nous pouvons compenser en augmentant davantage le nombre de shots par circuit dans notre processus d'échantillonnage. Cela introduira une surcharge au-delà de celle nécessaire pour réduire le biais, ce qui n'est donc pas fait par défaut. Nous pouvons facilement opter pour ce comportement en ajustant le nombre de shots par circuit dans options.executions.shots, comme montré dans l'exemple ci-dessous.
...
Dans ce cours, nous explorerons ces modèles d'atténuation des erreurs à un niveau élevé pour illustrer l'atténuation que peuvent réaliser les primitives IBM Quantum, sans avoir besoin de connaître tous les détails d'implémentation.
...
### Utilisation des options dans Quantum Compute pour l'atténuation et la suppression {#using-the-options-in-quantum-compute-for-mitigation-and-suppression}

Ci-dessous, nous montrons comment calculer une valeur d'espérance en utilisant l'atténuation et la suppression des erreurs dans IBM Quantum Compute Service. Nous pouvons exploiter exactement le même circuit et observable qu'avant, mais cette fois en maintenant le niveau d'optimisation fixé à 2 et en ajustant la _résilience_ ou les techniques d'atténuation des erreurs utilisées. Ce processus d'atténuation des erreurs se produit plusieurs fois tout au long de la boucle d'optimisation.
...
- Comment exploiter les primitives IBM Quantum pour atténuer et supprimer le bruit
