...
On va charger les packages restants nécessaires à l'exécution de VQE lui-même, comme l'ansatz `efficient_su2` et les minimiseurs SciPy :
...
Les résultats de cette sortie sont discutés ci-dessous dans la section de post-traitement ; pour l'instant, note simplement que la simulation a réussi. Tu es maintenant prêt(e) à exécuter sur du vrai matériel. On va fixer la résilience à `1`, ce qui indique que la mitigation d'erreurs TREX sera utilisée. Maintenant qu'on travaille avec du vrai matériel, on va utiliser Qiskit Runtime et les primitives Runtime. Note que la boucle `for` liée à la géométrie ainsi que les multiples essais variationnels se trouvent à l'intérieur de la session.
