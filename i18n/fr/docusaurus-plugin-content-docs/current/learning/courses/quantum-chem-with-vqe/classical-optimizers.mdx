...
- La fonction de coût (`cost_func`). Elle est liée à l'Hamiltonien, mais inclut également certaines complexités, telles que le calcul de la valeur attendue à l'aide d'Estimator, et dans le cas de calculs d'états excités, peut inclure des conditions d'orthogonalité.
- Un état initial (x0) du système, souvent l'état de Hartree-Fock
...
- Les options de l'optimiseur classique (à ne pas confondre avec les options de Session abordées dans la section suivante)

Quelques exemples de code sont présentés ci-dessous. Nous limitons notre discussion ici aux deux derniers arguments.
