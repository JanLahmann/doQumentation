...
![Un diagramme de circuit avec quatre lignes horizontales et beaucoup de portes.](/learning/images/courses/integrating-quantum-and-high-performance-computing/programming-models/checkin-circuit.avif)
...
Suppose que tu modélises les électrons dans une molécule. Tu veux approximer (a) l'énergie de l'état fondamental de la molécule, et (b) quels états de base computationnels sont les plus dominants dans l'état fondamental de la molécule. Dans chaque cas, utiliserais-tu la primitive Estimator ou Sampler ?
...
* Le flux de contrôle principal du programme est dicté par le rang du processus. C'est le principe SPMD en action : le programme utilise une logique conditionnelle (par exemple, if (rank == 0)) pour s'assurer que seules certaines sections parallélisées du code sont exécutées par les processus worker, tandis qu'un processus maître (souvent le rang 0) gère l'initialisation et l'agrégation finale.
