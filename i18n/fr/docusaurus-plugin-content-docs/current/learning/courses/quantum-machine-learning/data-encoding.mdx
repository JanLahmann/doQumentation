...
en utilisant l'encodage d'amplitude.
<Accordion>
<AccordionItem title="Réponse">
...
$R_{ZZ}(\theta)$ implémente une interaction $ZZ$ et est maximalement intriquant pour $\theta = \textstyle{\frac{1}{2}}\pi$. $R_{ZZ}(\theta)$ peut être décomposé en une série de portes sur deux qubits, comme le montre le code Qiskit suivant utilisant la [porte RZZ](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.RZZGate) et la méthode de classe `QuantumCircuit` ```decompose```. Nous encodons une seule caractéristique du vecteur de données $\vec{x}$ : $\vec{x}_k=\pi.$
...
Les données sont associées à une rotation de phase $P(\theta) = e^{i\theta/2}R_Z(\theta)$ sur le second qubit. La porte $R_{ZZ}(\theta)$ intrique les deux qubits sur lesquels elle opère, avec un degré d'intrication déterminé par la valeur de la feature encodée.
...
Une façon de comprendre l'unitaire ci-dessus est par analogie avec le propagateur dans un système physique. L'unitaire ci-dessus est un opérateur d'évolution unitaire, $\exp(it\mathcal{H})$, pour un hamiltonien, $\mathcal{H}$, similaire au modèle d'Ising, où le paramètre de temps, $t$, est remplacé par des valeurs de données pour piloter l'évolution. Le développement de cet opérateur unitaire donne le circuit PFM. Les connectivités intriquantes dans $S$ peuvent être interprétées comme des couplages d'Ising dans un réseau de spins.
Considérons un exemple d'opérateurs Pauli $Y$ et $XX$ représentant ces interactions de type Ising. Qiskit fournit une classe `pauli_feature_map` pour instancier une PFM avec un choix de portes à un et $n$ qubits, qui dans cet exemple seront passées comme chaînes de Pauli `‘Y’` et `‘XX’`. Typiquement, $n$ vaut 1 ou 2 pour des interactions à un et deux qubits, respectivement. Le schéma d'intrication est « linéaire », ce qui signifie que seuls les qubits voisins directs dans le circuit quantique sont couplés. Notez que cela ne correspond pas aux qubits voisins directs sur l'ordinateur quantique lui-même, car ce circuit quantique est une couche d'abstraction.
...
Le code ci-dessous applique ces quatre feature maps intégrées à l'encodage d'un ensemble de caractéristiques, et trace la profondeur à deux qubits du circuit résultant. Comme les taux d'erreur à deux qubits sont bien plus élevés que les taux d'erreur des portes à un seul qubit, on pourrait raisonnablement s'intéresser surtout à la profondeur des portes à deux qubits. Dans le code ci-dessous, nous obtenons le compte de toutes les portes d'un circuit en décomposant d'abord le circuit puis en utilisant count_ops(), comme montré ci-dessous. Ici, les portes à deux qubits qui nous intéressent sont les portes 'cx' :
