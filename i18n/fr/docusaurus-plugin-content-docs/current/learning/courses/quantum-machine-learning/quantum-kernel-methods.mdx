...
Dans cette leçon, nous allons examiner la profondeur des circuits d'encodage prédéfinis qui utilisent un fort intrication et les comparer à la profondeur de circuits que nous codons nous-mêmes. Il ne s'agit pas ici de privilégier une méthode par rapport à une autre. Tu pourrais trouver que les circuits prédéfinis sont trop profonds, et que l'intrication du circuit personnalisé est insuffisant pour être utile. Encore une fois, ces exemples sont présentés uniquement pour te permettre d'explorer.
...
Utilise la primitive `Sampler` d'IBM Quantum pour reconstruire une distribution de probabilité des états produits par l'échantillonnage du circuit. Note que tu pourrais voir cela qualifié de « distribution de quasi-probabilité », un terme applicable lorsque le bruit est un problème et que des étapes supplémentaires sont introduites, comme dans la mitigation d'erreurs. Dans ces cas, la somme de toutes les probabilités peut ne pas être exactement égale à 1 ; d'où « quasi-probabilité ».
...
Calcule la probabilité de mesurer $|0\rangle^{\otimes N}$ sur le circuit quantique, et remplis la matrice de noyau à la position correspondant aux deux vecteurs de données utilisés. Pour remplir toute la matrice de noyau, nous devons exécuter une expérience quantique pour chaque entrée. Une fois que nous avons une matrice de noyau, nous pouvons l'utiliser dans de nombreux algorithmes d'apprentissage automatique classiques qui acceptent des `pre-calculated kernels`. Par exemple : `qml_svc = SVC(kernel="precomputed")`. Nous pouvons ensuite utiliser des flux de travail classiques pour appliquer notre modèle sur nos données de test, et obtenir un score de précision. En fonction de notre satisfaction quant à notre score de précision, nous devrons peut-être revisiter certains aspects de notre calcul, comme notre feature map.
...
Supposons que quelqu'un de nouveau en informatique quantique essaie d'utiliser un circuit quantique ayant une très grande profondeur à deux qubits, et qu'il n'utilise pas d'atténuation d'erreurs. Supposons en outre que cela entraîne un taux d'erreur de 10 % sur chaque qubit. Si l'élément de matrice de noyau réel (sans erreur) correspondant à ce circuit est très grand, disons 1,0, quelle serait la probabilité de mesurer les 10 qubits dans l'état où chaque qubit est $|0\rangle$ ?
...
* Essaie différents plongements de caractéristiques et schémas d'intrication.
