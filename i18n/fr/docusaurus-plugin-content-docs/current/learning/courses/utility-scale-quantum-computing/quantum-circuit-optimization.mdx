...
`optimization_level` a quatre options, de 0 à 3. Plus le niveau d'optimisation est élevé, plus l'effort de calcul dépensé pour optimiser le circuit est important. Le niveau 0 n'effectue aucune optimisation et fait simplement le minimum de travail pour rendre le circuit exécutable sur le backend sélectionné. Le niveau 3 dépense le plus d'effort (et généralement de temps d'exécution) pour essayer d'optimiser le circuit. Le niveau 1 est le niveau d'optimisation par défaut.
Nous transpilons le circuit sans optimisation (`optimization_level=0`) et avec optimisation (`optimization_level=2`).
Nous voyons une grande différence dans la longueur des circuits transpilés.
...
Nous comparons ensuite les résultats de l'exécution de deux circuits de préparation d'état GHZ à 5 qubits synthétisés différemment ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$).
...
Ce que nous avons fait dans le nouveau circuit peut être vu comme une synthèse de circuit « consciente des contraintes de couplage » manuelle. En d'autres termes : résoudre manuellement la synthèse de circuit et le mappage de circuit en même temps.
