...
# Algorithmes quantiques : Estimation de phase {#quantum-algorithms-phase-estimation}
...
<IBMVideo id="134399598" title="Cette courte vidéo montre comment quatre qubits en précession à des vitesses différentes peuvent être utilisés pour encoder des chiffres."/>
...
Ensuite, nous appliquons des portes Hadamard aux qubits de comptage :
...
Nous appliquons la transformation de Fourier quantique inverse pour convertir l'état du registre de comptage, puis mesurons le registre de comptage :
...
Nous pouvons simuler en utilisant le simulateur Aer :
...
Nous voyons que nous obtenons un résultat (`001`) avec certitude, ce qui se traduit en décimal par : `1`. Nous devons maintenant diviser notre résultat (`1`) par $2^n$ pour obtenir $\theta$ :
...
## 4. Exécution avec la primitive Sampler IBM Quantum {#4-execution-using-the-ibm-quantum-sampler-primitive}
