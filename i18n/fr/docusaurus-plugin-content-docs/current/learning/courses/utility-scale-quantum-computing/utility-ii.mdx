...
#### Définition de l'Hamiltonien {#define-the-hamiltonian-1}
...
Exactement comme dans le cas à 20 qubits, la formule de produit de Lie (premier ordre) est implémentée dans la classe [LieTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter). Encore une fois, la formule du premier ordre consiste en l'approximation énoncée ci-dessus :
...
#### Définition des opérateurs à mesurer {#set-the-operators-to-be-measured-1}
...
#### Exécution de la simulation d'évolution temporelle {#perform-time-evolution-simulation-1}
