...
1.	Qu'est-ce que nos qubits représenteront dans notre modèle ?
...
3.	Enfin, la topologie du problème s'aligne-t-elle avec la topologie du matériel ? Si ce n'est pas le cas, nous pourrions avoir besoin d'implémenter certaines astuces pour que cela fonctionne.
...
Si cela te semble à l'envers (en particulier le signe entre $X$ et $iY$), consulte la note optionnelle ci-dessous sur les conventions concernant l'état $|0\rangle$ ou $|1\rangle$ correspondant à « occupé » et lequel à « inoccupé ».

<Accordion>
<AccordionItem title="Notes optionnelles sur les conventions">

Dans de nombreux contextes en physique, il n'y a pas d'ambiguïté sur l'interprétation des opérateurs $\sigma^\pm$ ; ils augmentent/diminuent la projection en z du moment angulaire de spin d'un système. $\sigma^+$, par exemple, fait passer un état de moment angulaire de spin $|m_s\rangle$ à un état $|m_s+1\rangle$. Cependant, lors de l'application d'un mapper pour exprimer les opérateurs de création et d'annihilation fermioniques à l'aide de ces opérateurs de moment angulaire (combinaisons d'opérateurs de Pauli), l'augmentation/diminution correspond désormais à l'occupation d'une orbitale électronique. « Élévation » et « abaissement » ne présentent aucune ambiguïté si l'action consiste à augmenter ou diminuer la projection du moment angulaire sur un certain axe, et l'étiquette correspondante. Dans l'occupation des orbitales électroniques, on pourrait associer $|0\rangle$ à vide et $|1\rangle$ à occupé. Cela pourrait sembler être la convention évidente, et c'est effectivement celle utilisée dans les addons Qiskit. Mais notez qu'il est possible de choisir $|0\rangle$ comme « occupé » dans le sens d'un électron se relaxant vers l'état fondamental d'un système, et $|1\rangle$ comme « inoccupé ». C'est un choix libre, et une ambiguïté potentielle dans l'étiquetage : quel état correspond à une orbitale occupée ? $|0\rangle$ ou $|1\rangle$ ?

C'est maintenant quelque peu standardisé dans un contexte de calcul quantique, et dans les mappers intégrés comme l'addon Qiskit `FermionicMapper` pour définir
$$
\begin{aligned}
|0\rangle & = \text{ vide}\\
|1\rangle & = \text{ occupé}
...
De manière équivalente, un opérateur qui crée un électron occupant l'orbitale $k$ (représentée par le qubit $q_k$) doit se comporter comme suit :

$$
O_k |q_N,...,q_k=0,...,q_0\rangle = c |q_N,...,q_k=1,...,q_0\rangle
$$

Voyons maintenant les conséquences de ce choix. Notons que dans les bases standards utilisées en calcul quantique,
...
et par un travail similaire, nous pouvons construire l'ensemble complet :
...
À partir de là, il est clair qu'en choisissant la convention apparemment raisonnable selon laquelle $|0\rangle$ correspond à aucun électron, et $|1\rangle$ correspond à un électron, nous nous retrouvons dans la situation confuse où $\sigma^+$ apparaît dans l'opérateur d'annihilation, et $\sigma^-$ dans l'opérateur de création. Ainsi, la convention dans `Qiskit Fermions` (selon laquelle $|0\rangle$ correspond à occupé, et $|1\rangle$ correspond à inoccupé) entraîne que l'opérateur de création fermionique $f_k^\dagger$ contient $\sigma_k^-$, et l'opérateur d'annihilation $f_k$ contient $\sigma_k^+$.

Si tu trouves des sources avec des signes opposés entre les termes $X$ et $iY$ dans les opérateurs de création et d'annihilation, elles utilisent probablement la convention opposée quant à l'état correspondant à une orbitale remplie.
...
1.	Utilise la fonction `from_list` de SparsePauliOp pour créer un opérateur identité correspondant à la taille de l'espace de paramètres à mapper.

2.	En suivant la définition des opérateurs de création et de destruction présentée précédemment, utilise la fonction `from_list` de SparsePauliOp pour définir les opérateurs de Pauli $X$, $Y$, $Z$. Cela mappera les opérateurs de création et d'annihilation fermioniques vers des opérateurs de spin sur les qubits, encodant le nombre d'occupation fermionique dans la base de calcul des qubits.

3.	Génère l'Hamiltonien souhaité dans la base de Pauli en appliquant les opérateurs ci-dessus aux orbitales d'intérêt. Cela correspond généralement à la création d'une matrice identité qui représente les orbitales de cœur (non-interactives), puis à l'application des opérateurs de création et d'annihilation à l'espace actif, avec des poids correspondant aux spécificités du problème considéré.
...
où $\sigma^+$ est l'opérateur de Pauli $X + iY$ et $\sigma^{-}$ est $X - iY$. Une fois que nous avons un Hamiltonien écrit dans ce format, la partie difficile de l'étape de correspondance est terminée, et il peut maintenant être facilement écrit dans un circuit en opérateurs de Pauli.
