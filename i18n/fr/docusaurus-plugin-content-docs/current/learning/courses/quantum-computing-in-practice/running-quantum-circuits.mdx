...
<IBMVideo id="134063416" title="Dans cette vidéo, Olivia Lanes explique les circuits quantiques et les primitives. Elle couvre les primitives IBM Quantum : l'Estimateur et l'Échantillonneur."/>
...
1.	Ce que sont réellement les ordinateurs quantiques IBM&reg;. Tu devras connaître les bases des caractéristiques matérielles pour concevoir de manière optimale tes circuits quantiques afin qu'ils s'exécutent dessus.
...
3.	Le flux de travail typique que nous suivons pour exécuter des expériences à grande échelle. Cela comprend la sélection des meilleures primitives pour ton cas d'usage, le mapping d'un problème vers un circuit quantique, et l'application de techniques de suppression et d'atténuation des erreurs, qui nous permettent de tirer le maximum de puissance possible de ces machines.
...
Chaque processeur peut être caractérisé à l'aide de trois métriques de performance fondamentales qui décrivent son échelle, sa qualité et sa vitesse : les qubits programmables, les opérations sur les qubits, et le nombre maximal de circuits par seconde.

* **Qubits programmables :** ce sont les qubits que les utilisateurs peuvent contrôler directement et intégrer dans des algorithmes quantiques. Cette métrique reflète l'échelle de calcul disponible pour les applications.


* **Opérations sur les qubits (QuOps) :** cette métrique décrit combien des opérations quantiques les plus exigeantes en termes de calcul un processeur peut exécuter de manière fiable avant que les erreurs accumulées ne limitent le calcul. Elle sert de mesure de la qualité et de la capacité de calcul.


* **Nombre maximal de circuits par seconde :** cette métrique mesure le débit de circuits, c'est-à-dire combien de circuits quantiques un système peut exécuter par seconde. Un débit plus élevé permet d'accomplir davantage de travail utile dans un temps donné.


Tu peux également rencontrer des métriques de processeur supplémentaires dans la documentation technique, les études de benchmarking et les rapports de performance :

EPLG (Errors Per Layered Gate). Une mesure de l'erreur moyenne introduite par les opérations de portes d'intrication en couches. Un EPLG plus faible indique généralement une meilleure qualité de porte et une accumulation d'erreurs plus faible.

CLOPS (Circuit Layer Operations Per Second). Un benchmark qui mesure la rapidité avec laquelle un système quantique peut exécuter une charge de travail standardisée qui comprend à la fois des opérations quantiques et le traitement classique associé.

Ces métriques supplémentaires fournissent des informations plus détaillées sur le comportement du matériel, tandis que les qubits programmables, les opérations sur les qubits, et le nombre maximal de circuits par seconde offrent une vue de plus haut niveau de l'échelle, de la qualité et de la vitesse d'un système.
...
Avec la récente sortie de Qiskit v1.0, Qiskit est devenu plus performant et stable que jamais. Donc, si tu commences tout juste, tu es arrivé au bon moment ! Si tu es déjà familier avec Qiskit, tu devras télécharger et réinstaller la version la plus récente. Pour un guide d'installation complet, consulte le guide [Installer Qiskit](/guides/install-qiskit).
...
1.	**Mapper.** Cette étape formalise comment nous prenons un problème général qui nous intéresse et déterminons comment le mapper sur un ordinateur quantique sous la forme d'un circuit quantique.


2.	**Optimiser.** Dans cette étape, nous utilisons le transpileur de Qiskit pour router et disposer le circuit sur un matériel physique de qubits réel. Cela comprend la traduction des portes quantiques individuelles en séquences d'opérations effectuées sur le matériel, ainsi qu'une optimisation de la disposition des portes.


3.	**Exécuter.** Les primitives IBM Quantum fournissent l'interface vers le matériel IBM Quantum qui permet l'exécution des circuits transpilés. Cette étape comprend également l'utilisation de techniques de suppression et d'atténuation des erreurs, qui peuvent être largement masquées à l'utilisateur.


4.	**Post-traiter.** Dans cette étape, les données provenant du processeur quantique lui-même sont traitées, fournissant à l'utilisateur des résultats utiles sur le problème d'origine. Fondamentalement, cela englobe toute analyse supplémentaire des données acquises.
...
Ensuite, nous devrons choisir un processeur quantique qui a suffisamment de qubits de qualité suffisamment élevée pour que nous puissions exécuter notre circuit quantique. Prends ces décisions guidé par les trois métriques principales dont nous avons discuté dans la section matériel : les qubits programmables, les opérations sur les qubits, et le nombre maximal de circuits par seconde. Selon l'application, des métriques supplémentaires telles que l'EPLG et les CLOPS peuvent également fournir des informations utiles sur la qualité et la performance d'exécution des circuits attendues.
...
1.	Tous les qubits ne sont pas connectés. Certains sont très éloignés les uns des autres sur la puce, et nous devons réduire ou éliminer les interactions à longue distance dans la mesure du possible. Tu pourrais appliquer une séquence de portes SWAP entre qubits voisins pour déplacer l'information des qubits, mais les portes SWAP sont coûteuses et sujettes aux erreurs, donc il peut y avoir de meilleures façons de faire cela. Essaie d'éviter trop de portes SWAP coûteuses.


2.	La disposition et le routage sont des processus itératifs. Tu peux le faire à la main, mais il existe aussi un outil Qiskit appelé `mapomatic`, qui peut faire des recommandations pour une disposition physique des qubits basée sur des taux d'erreur approximatifs. Le transpileur (dont nous parlerons bientôt) peut également faire une suggestion éclairée.
...
1.	Initialisation. Cette étape exécute toutes les passes initiales requises avant de commencer à intégrer le circuit sur le backend. Cela implique généralement de dérouler les instructions personnalisées et de convertir le circuit en portes à un et deux qubits uniquement.


2.	Disposition. Cette étape mappe les qubits virtuels du circuit vers les qubits physiques d'un backend. Voir [Layout Stage](https://docs.quantum.ibm.com/api/qiskit/transpiler#layout-stage) pour plus de détails.


3.	Routage. Cette étape s'exécute après l'application d'une disposition et injecte des portes (telles que des portes swap) dans le circuit d'origine pour le rendre compatible avec la connectivité du backend. Voir [Routing Stage](https://docs.quantum.ibm.com/api/qiskit/transpiler#routing-stage) pour plus de détails.


4.	Traduction. Cette étape traduit les portes du circuit vers le jeu de base du backend cible. Voir [Translation Stage](https://docs.quantum.ibm.com/api/qiskit/transpiler#translation-stage) pour plus de détails.


5.	Optimisation. Cette étape exécute la boucle d'optimisation principale de manière répétée jusqu'à ce qu'une condition (comme atteindre une certaine profondeur cible) soit remplie. Nous disposons de quatre niveaux d'optimisation différents parmi lesquels choisir, décrits ci-dessous.


6.	Planification. Cette étape concerne les passes de planification tenant compte du matériel. À un niveau élevé, la planification peut être vue comme l'insertion de délais dans le circuit pour tenir compte du temps d'inactivité des qubits entre l'exécution des instructions.
...
1.	Le découplage dynamique est utilisé pour annuler efficacement une partie du bruit environnemental introduit lorsqu'un qubit reste inactif. En appliquant une série de portes à des moments précis, tu peux faire en sorte que le bruit accumulé dans une partie de la période d'inactivité annule approximativement le bruit dans l'autre partie.

2.	Le Pauli twirling est un moyen d'insérer des portes aléatoires non pas pour annuler le bruit, comme dans le découplage dynamique, mais pour le simplifier. En insérant des portes aléatoires, cela empêche les effets de différentes erreurs de s'accumuler aussi rapidement, et rend le bruit plus facile à caractériser, puisqu'il a désormais une nature stochastique. Cette méthode constitue également la base d'une technique puissante d'atténuation des erreurs, dont nous parlerons ci-dessous.


### Exécution {#execute}

Nous sommes maintenant prêts à exécuter le programme quantique. Les primitives IBM Quantum fournissent une interface avec le matériel IBM Quantum, et elles abstraient également la suppression et l'atténuation d'erreurs de l'utilisateur. Il y a deux primitives parmi lesquelles choisir : l'Échantillonneur et l'Estimateur.

L'[Échantillonneur IBM Quantum](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler) exécute le circuit plusieurs fois sur un appareil quantique, effectuant des mesures à chaque exécution, et reconstruisant la distribution de probabilité à partir des chaînes de bits récupérées. Plus il effectue d'exécutions (ou coups), plus les résultats seront précis, mais cela nécessite plus de temps et de ressources quantiques. En particulier, il calcule la probabilité d'obtenir chaque état de base standard possible en mesurant l'état préparé par le circuit.

L'[Estimateur IBM Quantum](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator) utilise un processus algébrique complexe pour estimer la valeur d'espérance sur un appareil quantique réel en décomposant l'observable en une combinaison d'autres observables avec des bases propres connues.
