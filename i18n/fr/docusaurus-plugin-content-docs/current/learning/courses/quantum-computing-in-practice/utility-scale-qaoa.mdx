...
Afin d'écrire le problème QUBO comme l'Hamiltonien d'Ising, tout ce que nous avons réellement à faire est un simple changement de variables de $x \in \{0, 1\}^n$ vers $z \in \{-1, 1\}^n$ :
...
Tout d'abord, notons que nous pouvons réécrire notre expression QUBO comme la somme de termes matriciels :

$$
x^TQx = \sum_{ij}Q_{ij}x_ix_j
$$

En appliquant le changement de variables on obtient

$$
\sum_{ij}Q_{ij}(\frac{1-z_i}{2})(\frac{1-z_j}{2}) = \sum_{ij}\frac{Q_{ij}}{4}(1 - z_i - z_j + z_iz_j)
$$

qui peut être réarrangé comme suit.

$$
\sum_{ij}\frac{Q_{ij}}{4}z_iz_j - \frac{Q_{ij}}{4}z_i - \frac{Q_{ij}}{4}z_j + \frac{Q_{ij}}{4}
$$

Nous pouvons supprimer le terme constant car il n'affectera pas quel $z$ minimise le problème QUBO. De plus, nous pouvons multiplier cette expression par quatre pour retrouver la valeur d'origine de $Q$, puisque cela n'affecte pas non plus le choix optimal de $z$.

$$
\sum_{ij}Q_{ij}z_iz_j - Q_{ij}z_i - Q_{ij}z_j = z^TQz + (-\sum_{ij} Q_{ij}z_i + Q_{ij}z_j)
$$

En regroupant les termes linéaires semblables, nous pouvons définir notre vecteur de coefficients linéaires $b$ à l'aide de l'équation

$$
b_i = -\sum_{j} Q_{ij} + Q_{ji}
$$

ce qui achève la transformation des variables vers $z \in \{-1, 1\}^n$ après substitution dans l'équation précédente.
...
En fin de compte, la minimisation de l'expression QUBO est la même que la minimisation de cette expression, où $b$ est un coefficient scalaire réel :

$$
\min_{x\in\{0,1\}^n} x^TQx\Longleftrightarrow \min_{z\in\{-1,1\}^n}z^TQz + b^Tz
$$

En réécrivant légèrement à nouveau, nous obtenons notre Hamiltonien de fonction de coût, où le minimum de l'expression représente l'état fondamental et `Z` est l'opérateur de Pauli Z :
