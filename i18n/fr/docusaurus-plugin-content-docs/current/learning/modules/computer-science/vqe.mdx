...
(a) Normalise la fonction d'onde d'essai en déterminant $A$ tel que $$ \int_{-\infty}^{\infty} |\Psi_\text{trial}|^2 dx = 1 $$.
...
(b) Calcule la valeur d'espérance de l'hamiltonien $\hat{H}$ donné par $$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V(x)$$ où $V(x)  = \frac{1}{2}m\omega^2x^2$, ce qui correspond à un potentiel d'oscillateur harmonique simple.

<Accordion>
<AccordionItem title="Answer">

L'hamiltonien pour un oscillateur harmonique est :

$$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + \frac{1}{2} m \omega^2 x^2 $$

**Valeur d'espérance de l'énergie cinétique**
...
**Valeur d'espérance de l'énergie potentielle**
...
**Valeur d'espérance de l'énergie totale**
...
Optimisation de $\alpha$ pour l'énergie minimale
...
### Teste ta compréhension {#check-your-understanding-1}
...
Remplis les blancs avec les termes corrects pour compléter le résumé du processus VQE, puis clique pour vérifier tes réponses.
...
Commençons par charger les paquets nécessaires, y compris les primitives IBM Quantum. Nous sélectionnerons également l'ordinateur quantique le moins occupé disponible.
...
Ensuite, nous avons besoin d'un circuit quantique paramétré, un ansatz, pour préparer une fonction d'onde d'essai $\Psi_\text{trial}$ pour l'état fondamental. L'objectif est de trouver les paramètres $\theta$ qui minimisent la valeur d'espérance de l'énergie $\langle\psi(\theta)|\hat{H}|\psi(\theta)\rangle$. Le choix de l'ansatz est crucial car il détermine l'ensemble des états quantiques possibles que notre circuit peut préparer. Un « bon » ansatz est suffisamment flexible pour représenter un état très proche du vrai état fondamental de l'Hamiltonien étudié, sans pour autant être si complexe qu'il nécessite trop de paramètres ou un circuit trop profond pour les ordinateurs quantiques actuels.
...
Il est maintenant temps d'exécuter notre VQE sur un vrai QPU. Pour cela, nous avons d'abord besoin d'une fonction de coût pour le processus d'optimisation, qui évalue la valeur d'espérance de l'Hamiltonien avec un état quantique généré par l'ansatz. Ne t'inquiète pas ! Tu n'as pas besoin de tout coder toi-même. Nous avons préparé une fonction pour ça, et il te suffit d'exécuter la cellule ci-dessous.
...
Note : Tu peux obtenir un meilleur résultat en définissant un niveau d'atténuation d'erreurs avec [`resilience_level`](/guides/error-mitigation-and-suppression-techniques). La valeur par défaut est 1, et si tu définis une valeur plus élevée, cela utilisera plus de temps QPU mais pourrait retourner un meilleur résultat.
...
### Étape 1 : Mapper le problème sur des circuits quantiques et des opérateurs {#step-1-map-the-problem-to-quantum-circuits-and-operators-1}
...
Contrairement au calcul d'un seul atome d'hydrogène ($H$), afin de calculer l'état fondamental d'une molécule d'hydrogène ($H_2$), nous devons aussi prendre en compte la force répulsive agissant entre les noyaux des deux atomes d'hydrogène, en plus de l'énergie associée aux orbitales électroniques. Dans cette étape, nous donnerons cette valeur comme une constante, et nous calculerons réellement cette valeur dans le problème de vérification.
...
### Étape 2 : Optimiser pour le matériel cible {#step-2-optimize-for-target-hardware-1}

Puisque le nombre de qubits utilisés par le VQE et l'hamiltonien précédents est le même que celui du backend qui sera utilisé pour l'exécution, nous utiliserons l'ansatz existant et sa forme optimisée.

```python
h2_hamiltonian_isa = h2_hamiltonian.apply_layout(layout=ansatz_isa.layout)
```

### Étape 3 : Exécuter sur le matériel cible {#step-3-execute-on-target-hardware-1}

Il est maintenant temps de faire les calculs sur le vrai QPU. Presque tout est identique, mais nous utiliserons le point initial approprié pour ajuster l'hamiltonien. Aussi, dans une partie itérative, certains des paramètres de l'`Estimator`, qui est utilisé pour calculer les valeurs d'attente de l'hamiltonien pour l'ansatz sur le QPU, seront réglés légèrement différemment des calculs précédents. Nous discuterons de ce changement plus en détail dans une question de vérification.
...
### Étape 4 : Post-traitement {#step-4-post-process-1}
...
## Calculer l'énergie de réaction de $H + H = H_2$ {#compute-reaction-energy-of-h-h-h-2}

Utilisons maintenant ce que nous avons obtenu ! Tu as utilisé VQE, un solveur quantique variationnel des valeurs propres, pour calculer l'énergie de l'état fondamental de l'atome $H$ et de la molécule $H_2$. Il reste à utiliser ces valeurs calculées pour obtenir l'énergie de réaction du processus $H+H=H_2$.
