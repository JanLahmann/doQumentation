...
**Porte $X$** : Équivalent à une opération NOT. Porte à un qubit.
...
**Porte $Z$** : Ajoute une « phase » à un état (un préfacteur, qui dans le cas des états propres $|0\rangle$ et $|1\rangle$ de Z vaut respectivement 1 ou -1). Porte à un qubit.
...
Dans Qiskit, créer un circuit avec une porte $Z$ ressemble à ceci :
...
__Alice intrigue les qubits A et Q :__ Alice agit maintenant avec une porte CNOT avec son propre qubit comme contrôle et Q comme cible, puis applique une porte Hadamard à Q. Calculons l'état à trois qubits après cette opération :
