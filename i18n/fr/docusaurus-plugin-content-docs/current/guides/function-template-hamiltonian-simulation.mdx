...
Ce modèle encapsule un flux de travail pour simuler l'évolution temporelle d'un état initial par rapport à un hamiltonien basé sur le spin défini par l'utilisateur, et retourne un ensemble de valeurs d'attente spécifiées en utilisant l'[extension AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/) de Qiskit.

Ce modèle est structuré comme un motif Qiskit avec les étapes suivantes :

#### 1. Collecter les entrées et mapper le problème {#1-collecting-input-and-mapping-the-problem}
...
#### 2. Prépare les circuits générés pour l'exécution {#2-prepare-the-generated-circuits-for-execution}
...
Commence par écrire un modèle de fonction pour la simulation hamiltonienne qui utilise l'[extension AQC-Tensor de Qiskit](https://qiskit.github.io/qiskit-addon-aqc-tensor/) pour mapper la description du problème à un circuit de profondeur réduite qui peut s'exécuter sur du matériel.

Si tu télécharges cette page et la consultes localement dans un éditeur de notebook, tu verras que certaines cellules de code contiennent la [commande magique](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Cette commande magique enregistre le code dans `./source_files/template_hamiltonian_simulation.py`, qui est le modèle de fonction que tu peux téléverser et exécuter à distance avec Qiskit Serverless.
...
<Accordion>
<AccordionItem title="Code source complet du programme">
