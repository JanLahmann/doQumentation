...
3. Si `sampler.options.default_shots` est spécifié, cette valeur est utilisée.
...
2. Si `num_randomizations` et `shots_per_randomization` sont spécifiés comme [options de `twirling`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-twirling-options) (activées par défaut), leur produit est utilisé pour contrôler la quantité de données.
3. Si `estimator.options.default_shots` est spécifié, cette valeur est utilisée pour contrôler la quantité de données.
4. Si `estimator.options.default_precision` est spécifié, cette valeur est utilisée.
