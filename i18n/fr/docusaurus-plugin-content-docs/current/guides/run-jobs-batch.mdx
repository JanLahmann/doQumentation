---
description: 'Comment exécuter des jobs de calcul quantique en mode batch avec IBM Quantum Compute Service.'
...
Avant de démarrer un batch, tu dois [configurer le client IBM Quantum Compute](/guides/install-qiskit) et l'initialiser en tant que service :
...
Pour un aperçu complet de la configuration et du statut d'un batch, y compris son TTL interactif et maximum, utilise la méthode `batch.details() method`.
...
<Admonition type="tip" title="Recommandations">
  ...
  - Comprends les [limites des jobs](/guides/job-limits) lors de l'envoi d'un job à un QPU IBM&reg;.
