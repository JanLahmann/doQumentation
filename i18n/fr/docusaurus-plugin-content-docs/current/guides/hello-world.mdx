...
sidebar_label: 'Exécuter son premier circuit sur du matériel'
title: 'Exécuter son premier circuit sur du matériel'
---
...
1. Si tu n'as pas encore installé Qiskit, consulte les instructions dans le guide [Démarrage rapide](/guides/quick-start).
    ...
    - Installe le client IBM Quantum Compute pour exécuter des jobs sur du matériel quantique :
...
2. Configure ton authentification pour accéder au matériel quantique via le [Plan Ouvert](/guides/plans-overview#open-plan) gratuit.
    ...
    - Rends-toi sur [IBM Quantum Platform](https://quantum.cloud.ibm.com/) pour te connecter ou créer un compte.
         ...
         Si tu te connectes via un serveur proxy, tu dois utiliser `qiskit-ibm-runtime` v0.44.0 ou une version ultérieure.
         </Admonition>
...
3. Tu peux désormais utiliser ce code Python chaque fois que tu veux t'authentifier auprès d'IBM Quantum Compute Service :
...
Soumets le job et active la suppression des erreurs en utilisant une technique appelée [découplage dynamique](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-dynamical-decoupling-options). Le niveau de résilience indique le degré de protection contre les erreurs. Des niveaux plus élevés produisent des résultats plus précis, au prix de temps de traitement plus longs. Pour une explication détaillée des options définies dans le code suivant, consulte [Error mitigation and suppression techniques](/guides/error-mitigation-and-suppression-techniques).
