---
description: "Introduction à l'utilisation de simulateurs pour tester et déboguer des circuits quantiques dans Qiskit."
title: Introduction
---
...
- Pour la simulation exacte de petits circuits quantiques, tu peux utiliser les primitives de référence incluses avec Qiskit. Voir [Simulation exacte avec les primitives Qiskit SDK](/guides/simulate-with-qiskit-sdk-primitives).
...
- Pour créer des modèles de bruit personnalisés, utilise le module [`noise`](https://qiskit.org/ecosystem/aer/apidocs/aer_noise.html) de Qiskit Aer. Voir [Création de modèles de bruit](/guides/build-noise-models).
...
- La seule exigence pour exécuter Qiskit est un environnement Python fonctionnel. Pour plus de détails, consulte la section [Prise en charge des systèmes d'exploitation](/guides/install-qiskit#operating-system-support).
...
- Pour tirer le meilleur parti de ton matériel, utilise la simulation de circuits stabilisateurs (Clifford) locaux autant que possible. Consulte [Simulation Clifford avec le mode de test local de `qiskit-ibm-runtime`](local-testing-mode#clifford-simulation) ou [Simulation efficace de circuits stabilisateurs avec les primitives Qiskit Aer](/guides/simulate-stabilizer-circuits) pour des exemples.
...
- En savoir plus sur la [Simulation exacte et bruitée avec les primitives Qiskit Aer](/guides/simulate-with-qiskit-aer).
- Consulte la documentation sur la [visualisation des circuits](/guides/visualize-circuits).
- Consulte la documentation sur la [visualisation du timing des circuits](/guides/visualize-circuit-timing).
- [Déboguer les tâches avec NEAT](/guides/debug-qiskit-runtime-jobs).
</Admonition>
