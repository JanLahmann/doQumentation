...
La méthode `EstimatorV2.run` accepte deux arguments : une liste de PUB, dont chacun peut spécifier une valeur de précision propre au PUB, et un argument de mot-clé precision. Ces valeurs de précision font partie de l'interface d'exécution de l'Estimator et sont indépendantes des options du Runtime Estimator. Elles ont priorité sur toute valeur spécifiée en tant qu'option afin de respecter l'abstraction Estimator.

Cependant, si `precision` n'est spécifiée par aucun PUB ni dans l'argument de mot-clé run (ou si toutes ces valeurs sont `None`), alors la valeur de precision provenant des options est utilisée, notamment `default_precision`.

<Admonition type="note">
Ces paramètres de précision servent uniquement à spécifier la précision _cible_, et il n'est pas garanti que les résultats atteignent la précision spécifiée.
</Admonition>

Note que les options de l'Estimator contiennent à la fois `default_shots` et `default_precision`. Cependant, comme le gate-twirling est activé par défaut, le produit de `num_randomizations` et `shots_per_randomization` l'emporte sur ces deux options.

Spécifiquement, pour tout PUB d'Estimator :

1. Si le PUB spécifie la précision, utiliser cette valeur.

2. Si l'argument de mot-clé precision est spécifié dans `run`, utilise cette valeur.
...
Par exemple, si la précision est spécifiée aux quatre endroits, celui avec la priorité la plus haute (précision spécifiée dans le PUB) est utilisé.
...
Options de mitigation par annulation probabiliste des erreurs.
...
<Accordion>
  ...
  <AccordionItem title="Gate-folding ZNE">
    Peut ne pas fonctionner avec des portes personnalisées. Incompatible avec :
  - PEA
  - PEC
  </AccordionItem>
