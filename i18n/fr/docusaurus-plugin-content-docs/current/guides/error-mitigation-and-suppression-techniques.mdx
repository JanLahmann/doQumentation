---
description: "Explication des techniques d'atténuation et de suppression des erreurs disponibles via IBM Quantum Compute Service"
...
![Représentation du twirling de Pauli](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)
...
![Représentation du twirling de mesure](/docs/images/guides/error-mitigation-explanation/measurement_twirling.svg)

En présence d'erreurs de lecture, le twirling de mesure a pour effet de diagonaliser la matrice de transfert des erreurs de lecture, ce qui la rend facile à inverser. Un terme de repondération issu du canal de bruit diagonal est appris en effectuant des mesures de référence sur des circuits aléatoires initialisés dans l'état zéro. Cela permet au service de supprimer le biais des valeurs d'espérance résultant du bruit de lecture. L'estimation de la matrice de transfert des erreurs de lecture nécessite l'exécution de circuits d'étalonnage supplémentaires, ce qui introduit un faible surcoût.

### Activer TREX via l'option `measure_mitigation` {#enable-trex-by-using-the-measure-mitigation-option}

TREX peut être activé en définissant `measure_mitigation` sur `True` dans les [options de résilience d'IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) pour l'Estimator. Les options d'apprentissage du bruit de mesure sont décrites [ici](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-measure-noise-learning-options). Comme pour le twirling de portes, tu peux définir le nombre de randomisations de circuits et le nombre de shots par randomisation.

La cellule de code suivante montre comment activer TREX et définir ces options pour l'Estimator. Aucune de ces options ne doit être définie explicitement.
...
### Activer TREX via l'option `resilience_level` {#enable-trex-by-using-the-resilience-level-option}

Tu peux activer TREX en définissant le [niveau de résilience](/guides/estimator-noise-management#resilience) à `1`, comme illustré dans l'exemple suivant.
...
Le surcoût de cette méthode évolue avec le nombre de facteurs de bruit. Les paramètres par défaut échantillonnent la valeur d'espérance à trois facteurs de bruit, entraînant un surcoût d'environ 3×.

### Activer la ZNE via l'option `zne_mitigation` {#enable-zne-by-using-the-zne-mitigation-option}

La ZNE peut être activée en définissant `zne_mitigation` sur `True` dans les [options de résilience d'IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) pour l'Estimator.
Les options pour la ZNE sont décrites [ici](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options). Les options suivantes sont notables :
...
La cellule de code suivante montre comment activer la ZNE et définir ces options pour l'Estimator. Aucune de ces options ne doit être définie explicitement.
...
### Activer la ZNE via l'option `resilience_level` {#enable-zne-by-using-the-resilience-level-option}

Tu peux activer la ZNE (ainsi que TREX et le twirling de portes) en définissant le [niveau de résilience](/guides/estimator-noise-management#resilience) à `2`, comme illustré dans l'exemple suivant.
...
- [Configurer la gestion du bruit avec Estimator.](/guides/estimator-noise-management)

- [Configurer la gestion du bruit avec Sampler.](/guides/sampler-noise-management)

- Explorer d'autres [options](/guides/runtime-options-overview) pour les primitives IBM Quantum.
