---
description: 'Comment initialiser ton compte IBM Quantum Compute Service, instancier ton compte et charger des identifiants enregistrés.'
title: 'Initialiser ton compte IBM Quantum Compute Service'
---

{/* doqumentation-source-hash: ab5dc076 */}

# Initialiser ton compte IBM Quantum Compute Service {#initialize-your-ibm-quantum-compute-service-account}

Avant d'utiliser IBM Quantum&reg; Compute Service, tu dois initialiser (instancier) ton compte en soumettant (chargeant) tes identifiants.  Ces identifiants peuvent être soumis manuellement à chaque fois que tu initialises IBM Quantum Compute Service, ou tu peux les enregistrer pour les réutiliser.
...
2. Crée au moins une instance, ou assure-toi d'avoir accès à une instance existante. Suis ces étapes pour vérifier :
...
1. Assure-toi de travailler dans un environnement Python actif avec le [SDK Qiskit et `qiskit-ibm-runtime` installés](/guides/install-qiskit#local).
...
## Connecter Qiskit à ton instance IBM Quantum Compute Service {#connect-qiskit-with-your-ibm-quantum-compute-service-instance}
...
Voici le code de base pour connecter Qiskit à ton instance Quantum Compute. Il existe cependant plusieurs façons de personnaliser les options de `QiskitRuntimeService` selon tes besoins. Ces options sont décrites dans les sections suivantes.
...
Si tu as nommé un ou plusieurs ensembles d'identifiants — par exemple pour le plan Open et le plan Premium — utilise le code suivant pour utiliser un ensemble d'identifiants nommé.
...
3. Si ton compte dispose de plusieurs instances pouvant accéder au QPU demandé, le système utilise tes préférences de plan spécifiées pour choisir un plan et une instance. Pour `qiskit-ibm-runtime` v0.42 et versions ultérieures, les plans gratuits sont prioritaires par défaut.
...
### Tests locaux avec `qiskit-ibm-runtime` {#local-testing-with-qiskit-ibm-runtime}
...
<Admonition type="tip" title="Recommandations">
   ...
   - Suis les étapes du guide [Exécuter ton premier circuit sur du matériel](/guides/hello-world) pour écrire et exécuter un programme quantique.
