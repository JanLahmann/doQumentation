...
Dans le DAG, il existe trois types de nœuds de graphe : les nœuds d'entrée qubit/clbit (verts), les nœuds d'opération (bleus) et les nœuds de sortie (rouges). Chaque arête indique un flux de données (ou une dépendance) entre deux nœuds. Utilise la fonction qiskit.tools.visualization.dag_drawer() pour visualiser le DAG de ce circuit. (Installe la [bibliothèque Graphviz](https://graphviz.org/download/) pour l'exécuter.)
...
Une opération est ajoutée à la fin du DAGCircuit à l'aide de la méthode `apply_operation_back()`. Cela ajoute la porte spécifiée pour qu'elle agisse sur les qubits donnés après toutes les opérations existantes dans le circuit.
...
Une opération est ajoutée au début du DAGCircuit à l'aide de la méthode `apply_operation_front()`. Cela insère la porte spécifiée avant toutes les opérations existantes dans le circuit, en faisant effectivement la première opération exécutée.
...
- Consulte le guide sur la création d'une [passe de transpileur personnalisée](/guides/custom-transpiler-pass)
- Apprends à [créer des backends personnalisés et à transpiler vers ceux-ci](/guides/custom-backend)
