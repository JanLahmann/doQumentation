...
## Service de transpilation Qiskit {#qiskit-transpiler-service}
Le service de transpilation Qiskit ([nom du package `qiskit-ibm-transpiler`](https://pypi.org/project/qiskit-ibm-transpiler/)) est un nouveau service expérimental qui fournit des capacités de transpilation à distance dans le cloud aux utilisateurs des plans IBM Quantum Premium, Flex et On-Prem (via l'API IBM Quantum Platform). En plus des capacités de transpilation locales du SDK Qiskit, tes tâches de transpilation peuvent bénéficier à la fois des ressources cloud IBM Quantum et des passes de transpilation alimentées par l'IA en utilisant ce service. Pour en savoir plus sur la façon d'intégrer la transpilation basée sur le cloud dans ton flux de travail Qiskit, tu peux [consulter la documentation](/guides/qiskit-transpiler-service).
