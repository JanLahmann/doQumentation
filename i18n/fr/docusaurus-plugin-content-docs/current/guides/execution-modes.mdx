---
description: "Un aperçu des modes d'exécution disponibles dans Quantum Compute, notamment les sessions, les lots et les jobs individuels."
title: "Introduction aux modes d'exécution d'IBM Quantum Compute Service"
---

{/* doqumentation-source-hash: a331013a */}

# Introduction aux modes d'exécution de Quantum Compute {#introduction-to-quantum-compute-execution-modes}

Quand IBM Quantum Compute Service a été introduit, les utilisateurs ne pouvaient exécuter des circuits que sous forme de jobs individuels. Au fur et à mesure que différents types de charges de travail quantiques ont émergé, le besoin de différentes stratégies de planification est devenu évident. Les modes d'exécution déterminent comment tes jobs sont planifiés, et choisir le bon mode d'exécution permet à ta charge de travail de s'exécuter efficacement dans les limites de ton budget. Il existe trois modes d'exécution : job, session et batch.

## Mode job {#job-mode}

Une requête primitive unique effectuée sans gestionnaire de contexte. Les circuits et les entrées sont regroupés en blocs unifiés primitifs (PUBs) et soumis comme une tâche d'exécution sur l'ordinateur quantique. Pour exécuter en mode job, spécifie `mode=backend` lors de l'instanciation d'une primitive. Voir [Exemples d'Estimator](/guides/estimator-examples) et [Exemples de Sampler](/guides/sampler-examples) pour l'utilisation.
