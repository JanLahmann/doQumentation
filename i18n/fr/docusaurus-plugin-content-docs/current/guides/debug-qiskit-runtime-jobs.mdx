---
description: "Utilise la classe Neat du module d'outils de débogage qiskit-ibm-runtime pour déboguer et analyser les jobs."
notebook_path: guides/debug-qiskit-runtime-jobs.ipynb
sidebar_label: 'Déboguer les jobs avec NEAT'
title: 'Déboguer les jobs avec NEAT'
---
...
Tu peux utiliser la classe `Neat` pour analyser l'impact du bruit sur une charge de travail Estimator. Pour la vérification de la syntaxe, utilise le [mode de test local](/guides/local-testing-mode).

## Utilisation de la classe `Neat` {#neat-class-usage}

Avant de soumettre une charge de travail gourmande en ressources pour exécution sur du matériel réel, tu peux utiliser la classe IBM Quantum Compute [NEAT (Noisy Estimator Analyzer Tool)](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/debug-tools-neat#neat) pour vérifier que ta charge de travail Estimator est correctement configurée, qu'elle est susceptible de retourner des résultats précis, qu'elle utilise les options les plus appropriées au problème spécifié, et bien plus encore.
...
Pour commencer, importe les packages nécessaires et [authentifie-toi auprès d'IBM Quantum Compute Service](/guides/cloud-setup).
...
- Lis un aperçu des [outils de débogage Qiskit](/guides/debugging-tools).
- Découvre la [simulation exacte et bruitée avec les primitives Qiskit Aer](/guides/simulate-with-qiskit-aer).
- Découvre les [options IBM Quantum Compute disponibles](/guides/runtime-options-overview).
...
- Apprends [comment transpiler des circuits](/guides/circuit-transpilation-settings#compare-transpiler-settings) dans le cadre des workflows Qiskit patterns.
