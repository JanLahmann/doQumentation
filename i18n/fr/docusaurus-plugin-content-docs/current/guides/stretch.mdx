...
![Image illustrant le circuit suivant](/docs/images/guides/stretch/stretch.svg)
...
:::note
 ...
 Lors de l'utilisation de `stretch` avec IBM Quantum Compute Service, tout reste résultant d'une résolution stretch est ajouté au premier délai qui utilise le stretch.
...
## Afficher les valeurs stretch dans IBM Quantum Compute Service {#view-stretch-values-in-ibm-quantum-compute-service}

La valeur réelle d'une durée stretch est résolue à la compilation, après la planification du circuit. Lors de l'exécution d'un job Sampler dans IBM Quantum Compute Service, tu peux consulter les valeurs stretch résolues dans les métadonnées du résultat du job. La prise en charge de `stretch` dans Quantum Compute est actuellement expérimentale ; tu dois donc d'abord activer une option expérimentale pour en permettre la récupération, puis accéder aux données directement depuis les métadonnées comme suit :
...
Bien que le temps total du circuit soit renvoyé dans les métadonnées « compilation », il ne s'agit PAS du temps utilisé pour la facturation (temps QPU).
</Admonition>
...
Pour tous les détails, consulte la rubrique [Visualiser le timing des circuits](/guides/qiskit-runtime-circuit-timing).

<span id="qiskit-runtime-limitations"></span>

## Limitations de Quantum Compute {#quantum-compute-limitations}

La prise en charge de `stretch` dans Quantum Compute est actuellement expérimentale et présente les contraintes suivantes :
...
- La zone délimitée par un ensemble de barrières est appelée une _région de barrière_. Une variable stretch ne peut pas être utilisée dans plusieurs régions de barrière.
    <Tabs>
        ...
        ![Illustration de la sortie du code précédent](/docs/images/guides/stretch/bad-barrier-region.svg 'Invalid use of stretch in barrier regions')
     ...
     <TabItem value="Valid" label="Valide">
        ...
        ![Illustration de la sortie du code précédent](/docs/images/guides/stretch/good-barrier-region.svg 'Valid use of stretch in a barrier region')
