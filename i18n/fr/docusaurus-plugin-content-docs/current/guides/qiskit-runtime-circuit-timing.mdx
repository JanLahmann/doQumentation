...
Les données de timing renvoyées dans `["compilation"]["scheduler_timing"]["timing"]` sont une liste de chaînes. Chaque chaîne représente une seule instruction sur un canal, et ses champs, séparés par des virgules, correspondent aux types de données suivants :
...
- `T0` - L'instant de début de l'instruction dans l'ordonnancement complet
...
Ensuite, obtiens le timing de l'ordonnancement du circuit :
...
<Admonition type="tip" title="Recommandations">
  - [Anticipation classique (feedforward) et flux de contrôle](/guides/classical-feedforward-and-control-flow) (circuits dynamiques)
