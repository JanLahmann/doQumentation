...
À mesure que les besoins du développement de QPU ont évolué, la liste des fonctionnalités d'OpenQASM s'est étoffée en conséquence ; la dernière version, [OpenQASM 3](https://arxiv.org/abs/2104.14722), intègre des extensions telles que le contrôle de flux classique par anticipation (feed-forward), les modificateurs de portes et les implémentations par impulsions.
...
Le SDK Qiskit permet de convertir du code OpenQASM en objets [`QuantumCircuit`](../api/qiskit/qiskit.circuit.QuantumCircuit), et inversement (voir [OpenQASM 2 et Qiskit](interoperate-qiskit-qasm2) et [OpenQASM 3 et Qiskit](interoperate-qiskit-qasm3) pour les instructions).

Pour plus d'informations, consulte la [spécification évolutive d'OpenQASM](https://openqasm.com/).
...
- Visite la [spécification évolutive (« live ») d'OpenQASM](https://openqasm.com/).
</Admonition>
