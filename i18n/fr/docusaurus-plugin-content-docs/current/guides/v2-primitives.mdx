...
### Mitigation et suppression d'erreurs
...
* Estimator V2 ne supporte pas le niveau de résilience 3. En effet, le niveau 3 dans Estimator V1 utilise la Probabilistic Error Cancellation (PEC), qui a prouvé donnant des résultats sans biais au prix d'un temps de traitement exponentiel. Le niveau 3 a été supprimé pour attirer l'attention sur ce compromis. Tu peux cependant toujours utiliser PEC comme méthode de mitigation d'erreurs en spécifiant l'option `pec_mitigation`.
* Estimator V2 supporte `resilience_level` 0-2, comme décrit dans le tableau suivant. Ces options sont plus avancées que leurs homologues V1. Tu peux également activer/désactiver explicitement les méthodes individuelles de mitigation ou suppression d'erreurs.
