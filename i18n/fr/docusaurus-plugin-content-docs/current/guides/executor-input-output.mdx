...
Comme indiqué précédemment, l'entrée d'une primitive Executor est un [`QuantumProgram`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/quantum-program-quantum-program), qui est un itérable d'objets
[`QuantumProgramItem`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/quantum-program-quantum-program-item). Ces objets peuvent être de deux types :
...
Le second élément contient les résultats de l'exécution de la deuxième tâche (un `SamplexItem`) dans le programme. Cet élément contient plusieurs clés. La clé `meas`, qui est le nom du registre classique du circuit d'entrée, correspond au tableau de résultats de ce registre. Ce tableau a la forme `(randomizations, parameter sets, shots, classical bits)`, soit (28, 10, 1024, 3) dans cet exemple. De plus, la sortie contient une clé `measurement_flips.meas`, qui correspond aux corrections de bit-flip permettant d'annuler le twirling de mesure pour le registre `meas`. La forme de cette sortie sera (28, 10, 1, 3) pour notre exemple, car un seul shot est nécessaire pour effectuer le bit-flip.
