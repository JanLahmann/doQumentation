...
Avant de démarrer une session, tu dois [configurer le client IBM Quantum Compute](/guides/install-qiskit-runtime) et l'initialiser en tant que service :
...
Pour un aperçu complet de la configuration et du statut d'une session, utilise la méthode `session.details() method`.
