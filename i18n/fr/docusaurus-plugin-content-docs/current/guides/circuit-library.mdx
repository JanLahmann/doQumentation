...
{/* doqumentation-source-hash: 587592e7 */}
...
## Portes standard {#standard-gates}
...
Tu peux aussi utiliser ceci pour attribuer ces paramètres à des valeurs réelles à l'aide d'un dictionnaire de la forme `{ Parameter: number }`. Pour démontrer cela, la cellule de code suivante attribue chaque paramètre du circuit à `0`.
...
<Admonition type="tip" title="Recommandations">
  -  Découvre les méthodes avancées de création de circuits dans la rubrique [Construire des circuits](/guides/construct-circuits).
