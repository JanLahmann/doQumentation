...
Dans ce notebook, tu vas parcourir les étapes d'un [patron Qiskit](https://quantum.cloud.ibm.com/docs/guides/intro-to-patterns) en utilisant la **découpe de circuit** pour réduire le nombre de qubits dans un circuit. Tu vas découper des portes pour pouvoir reconstruire la valeur d'espérance d'un circuit à quatre qubits en utilisant uniquement des expériences à deux qubits.
...
### Créer un circuit à découper {#create-a-circuit-to-cut}
...
![Diagramme de circuit quantique](/img/qiskit-addons/cutting/01-gate-cutting-to-reduce-circuit-width/output_1.png)
...
### Séparer le circuit et l'observable selon un partitionnement de qubits spécifié {#separate-the-circuit-and-observable-according-to-a-specified-qubit-partitioning}

Chaque étiquette dans `partition_labels` correspond au qubit du `circuit` au même index. Les qubits partageant une étiquette de partition commune seront regroupés, et les portes non locales couvrant plus d'une partition seront découpées.
...
![Diagramme de circuit quantique](/img/qiskit-addons/cutting/01-gate-cutting-to-reduce-circuit-width/output_2.png)

```python
subcircuits["B"].draw("mpl", scale=0.8)
```

![Diagramme de circuit quantique](/img/qiskit-addons/cutting/01-gate-cutting-to-reduce-circuit-width/output_3.png)

### Calculer le surcoût d'échantillonnage pour les découpes choisies {#calculate-the-sampling-overhead-for-the-chosen-cuts}
...
### Lancer les sous-expériences avec la primitive Sampler de Qiskit Runtime {#run-the-subexperiments-using-the-qiskit-runtime-sampler-primitive}
...
### Comparer la valeur d'espérance reconstruite avec la valeur d'espérance exacte issue du circuit et de l'observable d'origine {#compare-the-reconstructed-expectation-value-with-the-exact-expectation-value-from-the-original-circuit-and-observable}
