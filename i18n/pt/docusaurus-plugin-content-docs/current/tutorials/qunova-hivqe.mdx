---
description: 'Use o Qunova HiVQE para calcular curvas de superfície de energia potencial (PES) de dissociação.'
notebook_path: tutorials/qunova-hivqe.ipynb
sidebar_label: 'Curvas PES de dissociação com Qunova HiVQE'
title: 'Curvas PES de dissociação com Qunova HiVQE'
---
...
As Funções Qiskit são um recurso experimental disponível apenas para usuários dos Planos Premium, Flex e On-Prem (via API da Plataforma IBM Quantum) do IBM Quantum&reg;. Elas estão em status de versão prévia e sujeitas a alterações.
</Admonition>
...
Defina as geometrias de $Li_2S$ em formato de dicionário para diferentes distâncias de ligação Li-S para calcular a curva PES. Estas geometrias são otimizadas usando cálculos B3LYP/631g.
...
### Passos 2 e 3: Otimizar o problema para execução em hardware quântico e executar usando a função HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}
...
O tempo total de execução da QPU usado no trabalho pode ser rastreado fazendo login na [Plataforma IBM Quantum](https://quantum.cloud.ibm.com/) e visualizando os trabalhos submetidos com a tag `qunova-chemistry-hivqe`.

### Passo 4: Pós-processar e comparar com métodos clássicos {#step-4-post-process-and-compare-with-classical-methods}
...
### Plotando a curva de dissociação para Li\_2S {#plotting-the-dissociation-curve-for-li-2s}

Vamos plotar e comparar os resultados do HiVQE com HF e CASCI.
Você pode observar que todos os cálculos HiVQE estão bem alinhados com o resultado de referência clássico (CASCI).
...
Observe que, para executar a função no sistema FeP-NO com as configurações mostradas neste exemplo, você precisa de uma licença que permita usar a função com pelo menos 44 qubits. Envie um e-mail para [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) para consultar sobre como obter uma licença.

### Etapa 1: Mapear entradas clássicas para um problema quântico {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
### Passo 2 e 3: Otimizar o problema para execução em hardware quântico e executar usando a função HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

Com base na configuração do HiVQE e das geometrias, obtenha os resultados sequencialmente.

#### Submeter o cálculo d(Fe-N) = 1,75 $\AA$. {#submit-dfe-n-175-aa-calculation}
...
Acompanhe o job e recupere o resultado para o cálculo d(Fe-N) = 1,75 $\AA$.
...
#### Submeter o cálculo d(Fe-N) = 2,00 $\AA$. {#submit-dfe-n-200-aa-calculation}
...
Acompanhe o job e recupere o resultado para o cálculo d(Fe-N) = 2,00 $\AA$.
...
#### Submeter o cálculo d(Fe-N) = 5,00 $\AA$. {#submit-dfe-n-500-aa-calculation}
...
Acompanhe o job e recupere o resultado para o cálculo d(Fe-N) = 5,00 $\AA$.
...
### Etapa 4: Pós-processar e comparar com métodos clássicos {#step-4-post-process-and-compare-with-classical-methods-1}
...
> **Observação:** Esta pesquisa é da **IBM Quantum** e cobre o **conteúdo do tutorial** (escrito pela IBM). A doQumentation fornece o site, as traduções e a execução de código — para feedback sobre esses aspectos, [abra uma issue no GitHub](https://github.com/JanLahmann/doQumentation/issues).

[Link para a pesquisa](https://your.feedback.ibm.com/jfe/form/SV_9AizRyKFHEGzVs2)
