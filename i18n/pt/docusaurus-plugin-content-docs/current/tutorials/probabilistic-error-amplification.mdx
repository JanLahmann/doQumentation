...
*Estimativa de uso: 14 minutos em um processador Heron r3 (NOTA: Esta é apenas uma estimativa. Seu tempo de execução pode variar.)*

## Resultados de aprendizagem {#learning-outcomes}
Após concluir este tutorial, os usuários deverão entender:
- A teoria por trás da *extrapolação de ruído zero* (ZNE), os diferentes métodos para amplificar o ruído e por que a *amplificação probabilística de erros* (PEA) é preferida para experimentos em escala de utilidade.
- Como implementar ZNE com PEA na prática usando Qiskit.

## Pré-requisitos {#prerequisites}
Sugerimos que os usuários estejam familiarizados com os seguintes tópicos antes de seguir este tutorial:
- A [lição de Mitigação de erros](/learning/courses/utility-scale-quantum-computing/error-mitigation) do curso *Computação quântica em escala de utilidade* para conhecimentos básicos sobre o uso de mitigação de erros no Qiskit.
- A [lição Utility-I](/learning/courses/utility-scale-quantum-computing/utility-i) do curso *Computação quântica em escala de utilidade* para mais contexto sobre o experimento em escala de utilidade usado como exemplo neste tutorial.

## Contexto {#background}

Este tutorial demonstra como executar um experimento de mitigação de erros em escala de utilidade com IBM Quantum Compute Service usando uma versão experimental de *extrapolação de ruído zero* (ZNE) com *amplificação probabilística de erros* (PEA).
...
**Referência**: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, *Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors* [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)
...
## Configuração {#setup}
Na célula abaixo, importamos pacotes relevantes e criamos algumas funções auxiliares para construir os circuitos para a evolução temporal Trotterizada de um modelo de Ising de campo transversal bidimensional que adere à topologia do backend.
...
Ignoraremos esta etapa, pois a mitigação de erros em tempo de execução não é compatível com simuladores.
...
##### Estabelecer um backend {#establish-a-backend}
...
Observe que cada observável é envolvido em uma lista no seguinte bloco de código. Isso é feito para fazer *broadcast* com valores de parâmetro para que cada observável de qubit seja medido para cada valor theta. Encontre as regras de broadcasting para primitivas na [documentação de primitivas](/guides/primitives).
...
* **amplifier**: O método a ser usado ao amplificar ruído para os fatores de ruído pretendidos.
  ...
  modelo de ruído com entrelaçamento de Pauli para camadas de portas de base de dois qubits com entrelaçamento de Pauli. As opções adicionais são `"gate_folding_front"` e `"gate_folding_back"`, que são explicadas na [documentação da API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier).

* **extrapolated\_noise\_factors**: Especifique um ou mais valores de fator de ruído nos quais avaliar os
...
Uma vez que o experimento esteja finalizado, você pode visualizar seus resultados. Você busca os valores esperados brutos e mitigados e os compara com resultados exatos. Em seguida, plota os valores esperados, tanto mitigados (extrapolados) quanto brutos, calculados em média sobre todos os qubits para cada parâmetro. Finalmente, plota valores esperados para sua escolha de qubits individuais.
...
Os valores esperados e erros padrão do resultado PUB (primitive unified bloc) são avaliados como array. Para trabalhos do estimador com ZNE, existem vários campos de dados de valores esperados e erros padrão disponíveis no contêiner `DataBin` do `PubResult`. Discutiremos brevemente os campos de dados para valores esperados aqui (campos de dados semelhantes estão disponíveis para erros padrão (`stds`) também).

1. `pub_result.data.evs`: Valores esperados correspondentes ao ruído zero (baseado na melhor extrapolação heurística).
...
2. `pub_result.data.evs_extrapolated`: Valores esperados para fatores de ruído extrapolados para cada extrapolador. Este array tem dois eixos adicionais.
...
3. `pub_result.data.evs_noise_factors`: Valores esperados brutos para cada fator de ruído.
...
- Um [tutorial](/tutorials/combine-error-mitigation-techniques) focado na combinação de técnicas de mitigação de erros.
- [Documentação](/guides/error-mitigation-and-suppression-techniques) detalhada sobre as técnicas de mitigação de erros disponíveis no Qiskit.
- Lições adicionais sobre experimentos em escala de utilidade: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) e [Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
