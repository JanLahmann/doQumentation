...
## Configuração {#setup}
Primeiro, autentique usando sua [chave de API IBM Quantum](http://quantum.cloud.ibm.com/). Em seguida, selecione a Função Qiskit da seguinte forma. (Este código assume que você já [salvou sua conta](/guides/functions-get-started#install-qiskit-functions-catalog-client) em seu ambiente local.)
