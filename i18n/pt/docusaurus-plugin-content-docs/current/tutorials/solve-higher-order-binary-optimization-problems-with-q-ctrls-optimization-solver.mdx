...
Defina suas credenciais da [Plataforma IBM Quantum](https://quantum.cloud.ibm.com/), que serão usadas ao longo do tutorial para autenticar no IBM Quantum Compute Service e nas Funções Qiskit.
...
O Solver retorna um dicionário com a solução e metadados associados, como a bitstring da solução, número de iterações e mapeamento de variáveis para bitstring. Para uma definição completa das entradas e saídas do Solver, consulte a [documentação](https://docs.quantum.ibm.com/api/functions/q-ctrl-optimization-solver).
...
Como o objetivo deste algoritmo de otimização é encontrar o estado fundamental mínimo do modelo de Ising, valores mais baixos indicam soluções melhores. Portanto, é visualmente evidente que as soluções geradas pelo Fire Opal Optimization Solver superam de longe a seleção aleatória.
