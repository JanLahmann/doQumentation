---
description: 'Combine opções de mitigação de erros para experimentos em escala de utilidade usando QPUs IBM Quantum de 100Q+ e a primitiva Estimator da IBM Quantum.'
...
*Estimativa de uso: 7 minutos em um processador Heron r2 (NOTA: Esta é apenas uma estimativa. Seu tempo de execução pode variar.)*
...
- Os conceitos básicos de desacoplamento dinâmico, mitigação de erros de medição, gate twirling e extrapolação de ruído zero, conforme descrito neste [guia](/guides/error-mitigation-and-suppression-techniques).

## Pré-requisitos {#prerequisites}

Após concluir este tutorial, os usuários devem entender:

- Como as técnicas de mitigação de erros mencionadas são implementadas seletivamente em hardware.

- Como elas se comparam em termos de capacidade de mitigar o ruído do hardware.


## Contexto {#background}

Este tutorial explora as opções de supressão e mitigação de erros disponíveis com a primitiva Estimator da IBM Quantum. Este tutorial mostra como implementar cada um dos seguintes métodos individualmente:
...
Observe que uma alternativa para implementar essas técnicas individualmente é implementá-las usando um [nível de resiliência](/guides/estimator-noise-management), em que `resilience_level` assume os valores 0, 1, 2:

- 0 : Nenhuma mitigação é implementada.

- 1 : A mitigação de erros de medição é implementada.

- 2 : Gate twirling, mitigação de erros de medição e ZNE são implementados.


Neste tutorial, você construirá um circuito e um observável e enviará jobs usando a primitiva Estimator com diferentes combinações de configurações de mitigação de erros. Em seguida, você plotará os resultados para observar os efeitos das várias configurações. A maior parte do tutorial usa um circuito de 10 qubits para facilitar as visualizações e, no final, você escalará o fluxo de trabalho para 50 qubits.
...
Vamos ignorar esta etapa, pois a mitigação de erros em tempo de execução não é suportada em simuladores.
...
Como nosso observável, vamos considerar o operador Pauli $Z$ agindo no último qubit, $Z I \cdots I$.
Observe que o fato de o último qubit corresponder ao primeiro elemento desta string deve-se ao uso de notação little-endian pelo Qiskit.
...
Neste ponto, você poderia prosseguir para executar seu circuito e medir o observável. No entanto, você também quer comparar a saída do dispositivo quântico com a resposta correta &mdash; isto é, o valor teórico do observável, se o circuito tivesse sido executado sem erro. Para circuitos quânticos pequenos você pode calcular este valor simulando o circuito em um computador clássico, mas isso não é possível para circuitos maiores em escala de utilidade. Você pode contornar este problema com a técnica do "circuito espelho" (também conhecida como "compute-uncompute"), que é útil para avaliar o desempenho de dispositivos quânticos.
...
O circuito transpilado agora contém apenas instruções ISA. Todas as portas foram decompostas em termos de portas $\sqrt{X}$ e rotações $R_z$, e [portas CZ](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.CZGate).
...
Aqui você enviará cinco jobs separados, começando sem supressão ou mitigação de erros, e sucessivamente habilitando várias opções de supressão e mitigação de erros disponíveis no IBM Quantum Compute Service. Para informações sobre as opções, consulte as seguintes páginas:
...
Como esses jobs podem ser executados independentemente uns dos outros, você pode usar o [modo batch](/guides/run-jobs-batch) para permitir que o Quantum Compute otimize o tempo de execução deles.
...
## Exemplo em hardware em grande escala {#large-scale-hardware-example}
...
- Todos os experimentos apresentam resultados mais próximos do valor ideal e todas as barras de erro são menores.

- A adição de desacoplamento dinâmico pode ter piorado o desempenho em comparação com o caso sem mitigação. Isso não é surpreendente, porque o circuito é muito denso. O desacoplamento dinâmico é útil principalmente quando há grandes lacunas no circuito, durante as quais os qubits ficam ociosos sem portas sendo aplicadas a eles. Quando essas lacunas não estão presentes, o desacoplamento dinâmico não é eficaz e pode realmente piorar o desempenho devido a erros nos próprios pulsos de desacoplamento dinâmico. O circuito de 10 qubits pode ter sido muito pequeno para observarmos esse efeito.

- Com a extrapolação de ruído zero, o resultado fica muito próximo do valor ideal. Isso demonstra o poder do ZNE.


## Próximos passos {#next-steps}
<Admonition type="tip" title="Recomendações">
Se você achou este trabalho interessante, talvez se interesse pelo seguinte material sobre algumas técnicas adicionais de mitigação e supressão de erros que não foram mencionadas neste tutorial:
...
- [Matrix-free Measurement Mitigation (M3)](https://qiskit.github.io/qiskit-addon-mthree/) (veja também o tutorial [Readout error mitigation for the Sampler primitive using M3](/tutorials/readout-error-mitigation-sampler/))
