...
* Demonstrar o uso de [retropropagação de operadores (OBP)](https://qiskit.github.io/qiskit-addon-obp/) para reduzir a profundidade do circuito. Esta técnica remove operações do final do circuito ao custo de mais medições de operadores.
...
Em seguida, autentique usando sua chave API do painel da [IBM Quantum Platform](http://quantum.cloud.ibm.com/). Depois, selecione a Qiskit Function conforme a seguir. (Observe que, por segurança, é melhor [salvar suas credenciais de conta](/guides/functions-get-started#install-qiskit-functions-catalog-client) em seu ambiente local, se você estiver em uma máquina confiável, para que não precise inserir sua chave API toda vez que autenticar.)
...
Agora usaremos a retropropagação de operadores (OBP). (Consulte a [documentação OBP](https://qiskit.github.io/qiskit-addon-obp/) para mais detalhes sobre o addon Qiskit OBP.) Criaremos uma função que gera as fatias de circuito para retropropagação:
...
Analisamos quatro circuitos com 5, 7 e 9 passos de Trotter com uma precisão de 0.05, comparando seus valores esperados ideais, ruidosos e mitigados de erros:
...
Quando todos os jobs terminarem de executar, podemos comparar seus valores esperados ruidosos e mitigados.
