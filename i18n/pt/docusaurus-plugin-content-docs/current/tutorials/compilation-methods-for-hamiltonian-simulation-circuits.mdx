...
description: "Compare os métodos de compilação SABRE, transpiler com IA e Rustiq em circuitos de simulação Hamiltoniana do Hamlib."
...
*Estimativa de uso: menos de 1 minuto em um processador IBM Heron (NOTA: Esta é apenas uma estimativa. Seu tempo de execução pode variar.)*
...
Após concluir este tutorial, você entenderá:
- Como usar o transpiler do Qiskit com SABRE para otimização de layout e roteamento
- Como aproveitar o transpiler com IA para otimização avançada de circuitos
- Como usar o plugin Rustiq para sintetizar operações `PauliEvolutionGate` em circuitos de simulação Hamiltoniana
- Como comparar métodos de compilação usando profundidade de dois qubits, contagem total de portas e tempo de execução

## Pré-requisitos {#prerequisites}
Sugerimos que você esteja familiarizado com os seguintes tópicos antes de realizar este tutorial:
- [Conceitos de transpilação](/guides/transpile)
- [Estágios do transpiler](/guides/transpiler-stages)
- [Transpilar com gerenciadores de passos](/guides/transpile-with-pass-managers)
## Contexto {#background}

A compilação de circuitos quânticos transforma um algoritmo quântico de alto nível em um circuito físico que respeita as restrições do hardware alvo. Uma compilação eficaz pode reduzir significativamente a profundidade do circuito e a contagem de portas, ambos impactando diretamente a qualidade dos resultados em dispositivos quânticos de curto prazo.

Este tutorial faz o benchmark de três métodos de compilação em circuitos de simulação Hamiltoniana construídos com `PauliEvolutionGate`. Esses circuitos modelam interações em pares entre qubits (como os termos $ZZ$, $XX$ e $YY$) e são comuns em química quântica, física da matéria condensada e ciência dos materiais.

Os circuitos de benchmark vêm da coleção [Hamlib](https://github.com/SRI-International/QC-App-Oriented-Benchmarks/tree/master/qedcbench/hamlib#hamlib-simulation---benchmark-program), acessada por meio do repositório [Benchpress](https://github.com/Qiskit/benchpress). O Hamlib fornece um conjunto padronizado de Hamiltonianos representativos, tornando possível comparar estratégias de compilação em cargas de trabalho de simulação realistas.

### Visão geral dos métodos de compilação {#compilation-methods-overview}

#### Transpiler do Qiskit com SABRE {#qiskit-transpiler-with-sabre}
O transpiler do Qiskit usa o algoritmo SABRE (SWAP-based BidiREctional heuristic search) para otimizar o layout e o roteamento do circuito. O SABRE se concentra em minimizar as portas SWAP e seu impacto na profundidade do circuito, respeitando as restrições de conectividade do hardware. É um método de uso geral que oferece um bom equilíbrio entre desempenho e tempo de compilação. Para mais detalhes, consulte [\[1\]](https://arxiv.org/abs/2409.08368). As vantagens e a exploração de parâmetros do SABRE são abordadas em profundidade em um [tutorial](/tutorials/transpilation-optimizations-with-sabre) separado.

#### Transpiler com IA {#ai-powered-transpiler}
O transpiler com IA usa aprendizado de máquina para prever estratégias ótimas de transpilação, analisando padrões na estrutura do circuito e nas restrições do hardware. Ele também pode aplicar o passe `AIPauliNetworkSynthesis`, que tem como alvo circuitos de rede Pauli usando uma abordagem de síntese baseada em aprendizado por reforço. Para mais informações, consulte [\[2\]](https://arxiv.org/abs/2405.13196) e [\[3\]](https://arxiv.org/abs/2503.14448).

#### Plugin Rustiq {#rustiq-plugin}
O plugin Rustiq fornece técnicas avançadas de síntese especificamente para operações `PauliEvolutionGate`, que representam rotações de Pauli comumente usadas em dinâmicas trotterizadas. Ele é projetado para produzir decomposições de circuito de baixa profundidade para cargas de trabalho de simulação Hamiltoniana. Para mais detalhes, consulte [\[4\]](https://arxiv.org/abs/2404.03280).

### Métricas-chave {#key-metrics}

Comparamos os três métodos nas seguintes métricas:
- **Profundidade de dois qubits**: A profundidade do circuito contando apenas as portas de dois qubits. Este costuma ser o gargalo para fidelidade em hardware real.
- **Tamanho do circuito (contagem total de portas)**: O número total de portas no circuito transpilado.
- **Tempo de execução**: O tempo de relógio para transpilação.
...
### Conectar a um backend {#connect-to-a-backend}

Selecione um backend que será usado tanto nos exemplos de pequena quanto de grande escala. O backend determina o mapa de acoplamento e as portas de base que o transpiler tem como alvo.
...
### Definir gerenciadores de passos {#define-pass-managers}
...
### Definir funções auxiliares {#define-helper-functions}

A função a seguir transpila uma lista de circuitos usando um dado gerenciador de passos e registra as métricas-chave (profundidade de dois qubits, tamanho do circuito e tempo de execução) para cada circuito.
...
### Carregar circuitos Hamiltonianos do Hamlib {#load-hamiltonian-circuits-from-hamlib}

Carregamos um conjunto representativo de Hamiltonianos do repositório Benchpress e construímos circuitos `PauliEvolutionGate`. Circuitos que excedem a contagem de qubits do backend são removidos, assim como circuitos cujo tamanho decomposto excede 1.500 portas (para manter os tempos de transpilação razoáveis).
...
Divida os circuitos em grupos de pequena escala (menos de 20 qubits) e grande escala (20 ou mais qubits).
...
Visualize um dos circuitos Hamiltonianos de pequena escala antes da transpilação.
...
## Exemplo de pequena escala {#small-scale-example}

Nesta seção, fazemos o benchmark dos três métodos de compilação em circuitos Hamiltonianos com menos de 20 qubits. Esses circuitos transpilam rapidamente e fornecem uma visão clara de como cada método lida com circuitos de complexidade moderada.
### Passo 1: Mapear entradas clássicas para um problema quântico {#step-1-map-classical-inputs-to-a-quantum-problem}

Cada Hamiltoniano é codificado como um circuito `PauliEvolutionGate`. Os circuitos já foram construídos na seção de configuração a partir dos dados de benchmark do Hamlib.
### Passo 2: Otimizar o problema para execução em hardware quântico {#step-2-optimize-problem-for-quantum-hardware-execution}

Transpilamos todos os circuitos de pequena escala usando cada um dos três gerenciadores de passos e coletamos as métricas.
...
A tabela abaixo resume a média e o desvio padrão de cada métrica em todos os circuitos de pequena escala, junto com a melhoria percentual em relação ao SABRE. Como os tamanhos dos circuitos variam muito, o desvio padrão fornece contexto importante para interpretar as médias.
...
A tabela por circuito mostra como cada método se compara em circuitos individuais. O melhor valor para cada métrica é marcado com um asterisco. Observe que para os circuitos mais simples, os três métodos frequentemente convergem para o mesmo resultado.
...
Os gráficos abaixo comparam os três métodos em cada métrica por circuito. Os circuitos são ordenados pela contagem de qubits e rotulados por índice no eixo x, uma vez que vários circuitos podem ter o mesmo número de qubits.
...
Nessa escala, os três gerenciadores de passos têm bom desempenho e seus resultados médios ficam próximos uns dos outros. Isso ocorre em grande parte porque os circuitos pequenos deixam pouco espaço para otimização adicional, de modo que os métodos tendem a convergir para soluções semelhantes.

Neste exemplo, o Rustiq produz os resultados mais variáveis, com os maiores valores discrepantes tanto em profundidade de dois qubits quanto em contagem de portas. Embora essa variabilidade signifique que ele às vezes fica atrás, também significa que o Rustiq ocasionalmente encontra soluções melhores do que os outros dois métodos. O transpiler com IA é mais estável em seus resultados em relação ao SABRE e ao Rustiq, acompanhando de perto a maioria dos circuitos sem muitos valores discrepantes.

Quanto ao tempo de execução, SABRE e Rustiq são ambos rápidos, enquanto o transpiler com IA é notavelmente mais lento em determinados circuitos.
#### Melhor método por métrica {#best-performing-method-by-metric}

O gráfico abaixo mostra com que frequência cada método alcançou o melhor valor (mais baixo) para cada métrica. Empates são possíveis: para circuitos mais simples, vários métodos podem atingir a mesma profundidade de dois qubits ou contagem de portas ideal. Quando ocorre um empate, todos os métodos empatados recebem crédito, de modo que as porcentagens de uma determinada métrica podem somar mais de 100%.
...
Neste exemplo, os três métodos têm desempenho muito semelhante nos circuitos de pequena escala. Em profundidade de dois qubits e contagem de portas, a proporção de circuitos em que cada método é o melhor é próxima (aproximadamente 35–55%), e muitos circuitos terminam em empate porque os circuitos mais simples frequentemente têm uma única solução ótima que vários métodos encontram. A diferença mais clara é no tempo de execução: SABRE e Rustiq são cada um o mais rápido em cerca de metade dos circuitos, enquanto o transpiler com IA raramente é o mais ágil. Considerando as três métricas em conjunto, o Rustiq tem uma ligeira vantagem geral &nbsp; ele é o vencedor mais frequente em profundidade de dois qubits e permanece competitivo em contagem de portas e tempo de execução.
### Passo 3: Executar usando primitivas do Qiskit {#step-3-execute-using-qiskit-primitives}

Para avaliar como a qualidade da transpilação afeta a execução sob ruído, usamos uma técnica de **circuito espelho**. Para cada circuito transpilado $U$, adicionamos seu inverso $U^\dagger$ de forma que o circuito combinado $U^\dagger U$ seja teoricamente a identidade. Começando do estado $|0\rangle$, uma execução perfeita (sem ruído) retornaria a string de todos os zeros com probabilidade 1.

Na prática, os erros de porta se acumulam ao longo do circuito, de modo que a probabilidade de recuperar $|0\rangle^{\otimes n}$ cai. Um método de compilação que produz um circuito mais raso com menos portas acumulará menos ruído.

A abordagem do circuito espelho é atraentemente simples e escala para qualquer tamanho de circuito, pois a saída esperada é sempre $|0\rangle^{\otimes n}$ e não é necessária nenhuma simulação clássica do estado ideal. No entanto, observe as seguintes ressalvas: o circuito espelho é uma aproximação do circuito real (não o próprio circuito), ele dobra a contagem de portas (o que exagera o efeito do ruído) e pode subestimar certos erros quando o ruído se cancela simetricamente através da fronteira do espelho.

Escolhemos o circuito de índice 6 do conjunto de pequena escala e executamos os circuitos espelho em um simulador Aer com um modelo de ruído de despolarização simples.
...
Construa os circuitos espelho (adicione $U^\dagger$), remapeie para índices de qubits contíguos para que o simulador lide apenas com os qubits ativos, e execute em um simulador Aer com ruído.
...
O método com a menor profundidade de dois qubits e menos portas alcança a maior fidelidade, consistente com a expectativa de que circuitos mais curtos acumulam menos ruído. Mesmo diferenças modestas em profundidade e contagem de portas se traduzem em diferenças mensuráveis de fidelidade sob o modelo de ruído de despolarização.

Tenha em mente que esses resultados são para um único circuito. A classificação relativa dos métodos pode mudar de circuito para circuito, dependendo da estrutura do Hamiltoniano.
## Exemplo de hardware de grande escala {#large-scale-hardware-example}

Nesta seção, fazemos o benchmark dos mesmos três métodos de compilação em circuitos Hamiltonianos com 20 ou mais qubits. Esses circuitos são mais representativos de cargas de trabalho práticas de simulação Hamiltoniana e testam como cada método escala em termos de qualidade de circuito e tempo de compilação.
### Passos 1-4 combinados {#steps-1-4-combined}

O fluxo de trabalho segue a mesma estrutura do exemplo de pequena escala. Transpilamos todos os circuitos de grande escala com cada método, coletamos métricas e enviamos um circuito espelho para hardware quântico real.
...
## Análise dos resultados de compilação {#analysis-of-compilation-results}

Os benchmarks acima comparam SABRE, o transpiler com IA e o Rustiq em circuitos de simulação Hamiltoniana da coleção Hamlib em pequena e grande escala.

### Profundidade de dois qubits e contagem de portas {#two-qubit-depth-and-gate-count}

Em grande escala, SABRE e o transpiler com IA são os dois melhores desempenhos, e cada um lidera em uma métrica diferente. Como o gráfico de *melhor método por métrica* mostra, o SABRE produz a menor contagem de portas na grande maioria dos circuitos e é o método mais rápido em quase todos eles, consistente com uma heurística projetada para minimizar as portas SWAP inseridas e com as otimizações recentes em seu layout e roteamento. O transpiler com IA produz a menor profundidade de dois qubits na maioria dos circuitos, consistente com a parte de seu objetivo de aprendizado por reforço que tem como alvo a profundidade do circuito. A tabela de resumo reflete a mesma divisão: SABRE tem a menor contagem média de portas, enquanto o transpiler com IA tem a menor profundidade média de dois qubits. Ambos os métodos são consistentes e confiáveis em toda a gama de circuitos.

O Rustiq, que é construído especificamente para síntese de `PauliEvolutionGate`, produz o único melhor resultado em apenas uma pequena fração dos circuitos de grande escala. Suas métricas médias são fortemente distorcidas por alguns poucos valores discrepantes significativos, visíveis como grandes picos no gráfico de comparação de compilação, onde o Rustiq produz profundidade e contagem de portas substancialmente maiores do que os outros métodos. Sem esses valores discrepantes, seu desempenho médio estaria muito mais próximo do SABRE e do transpiler com IA.

A observação-chave é que nenhum método domina em todos os circuitos. Cada método supera os outros em casos específicos, o que torna valioso tentar todas as ferramentas disponíveis e selecionar o melhor resultado para cada circuito.

### Tempo de execução {#runtime}

O SABRE é consistentemente o método mais rápido. O Rustiq geralmente roda em velocidade semelhante, mas pode produzir valores discrepantes onde a compilação demora significativamente mais. Isso é especialmente visível nos resultados de grande escala, onde alguns circuitos fazem o tempo de execução do Rustiq disparar. Esses valores discrepantes impactam fortemente o tempo de execução médio, de modo que a mediana pode ser um resumo mais representativo para o Rustiq. O transpiler com IA é o mais lento dos três, com tempo de execução que cresce notavelmente em circuitos maiores e mais complexos.

### Resultados do circuito espelho {#mirror-circuit-results}

Os experimentos com circuito espelho confirmam a tendência esperada: métodos que produzem menor profundidade de dois qubits e menos portas alcançam maior fidelidade sob ruído. Isso se mantém tanto no simulador com ruído (pequena escala) quanto no hardware real (grande escala).

Tenha em mente que cada gráfico de circuito espelho reflete um único circuito, não o agregado. O exemplo de hardware acima usa um circuito `tfim` de 26 qubits, que acontece ser um caso em que o SABRE produz uma profundidade de dois qubits muito maior do que o transpiler com IA e o Rustiq, então sua fidelidade é correspondentemente muito menor. Isso não é representativo dos resultados mais amplos: em todo o conjunto de circuitos de grande escala, a profundidade de dois qubits do SABRE geralmente está próxima à do transpiler com IA, e os dois métodos cada um lida em métricas diferentes (o transpiler com IA em profundidade de dois qubits, o SABRE em contagem de portas e tempo de execução). Um único resultado de espelho testa uma versão dobrada de um circuito em vez da carga de trabalho completa, portanto não deve ser lido como um veredicto sobre a qualidade geral do método.

### Recomendações {#recommendations}

Não existe uma única estratégia de transpilação melhor para todos os circuitos. A melhor escolha depende da estrutura do circuito, do objetivo de otimização e do orçamento de tempo de compilação disponível:

- **SABRE** é o padrão recomendado. É rápido e confiável, e produz resultados sólidos em uma ampla gama de circuitos. Para ajuste adicional, os usuários podem aumentar os trials de layout e roteamento (consulte o [tutorial de otimização SABRE](/tutorials/transpilation-optimizations-with-sabre)).
- **O transpiler com IA** vale a pena tentar quando o tempo de compilação não é uma restrição, especialmente quando minimizar a profundidade de dois qubits é a prioridade: ele produziu a menor profundidade de dois qubits na maioria dos circuitos de grande escala neste benchmark.
- **O Rustiq** é construído especificamente para circuitos `PauliEvolutionGate` e pode encontrar soluções de muito baixa profundidade e baixa contagem de portas, particularmente em circuitos menores. Em circuitos maiores, ele pode ocasionalmente produzir resultados muito maiores, portanto é melhor usado como um dos vários métodos a tentar, em vez de como padrão.

Na prática, a melhor abordagem é executar todos os métodos disponíveis e escolher o melhor resultado para cada circuito. O custo de compilação de tentar vários métodos é pequeno comparado à melhoria potencial na qualidade de execução no hardware real.
## Próximos passos {#next-steps}

Se você achou este tutorial útil, pode se interessar pelos seguintes recursos:
...
- [Passos de transpiler com IA](/guides/ai-transpiler-passes)
- [Criar um plugin de transpiler](/guides/create-transpiler-plugin)
</Admonition>
