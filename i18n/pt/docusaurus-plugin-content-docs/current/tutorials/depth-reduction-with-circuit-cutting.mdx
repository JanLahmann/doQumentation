...
Este tutorial demonstra como construir um `Qiskit pattern` para cortar portas em um circuito quântico para reduzir a profundidade do circuito. Para uma discussão mais aprofundada sobre corte de circuitos, visite a [documentação do Qiskit addon de corte de circuitos](https://qiskit.github.io/qiskit-addon-cutting/).
...
Escolhemos um layout inicial que requer duas trocas para executar as portas entre os qubits 3 e 0 e mais duas trocas para retornar os qubits às suas posições iniciais. Escolhemos `optimization_level=3`, que é o nível mais alto de otimização disponível com um gerenciador de passes (pass manager) predefinido.
...
> **Nota:** Esta pesquisa é da **IBM Quantum** e cobre o **conteúdo do tutorial** (escrito pela IBM). A doQumentation fornece o site, as traduções e a execução de código — para feedback sobre esses aspectos, [abra uma issue no GitHub](https://github.com/JanLahmann/doQumentation/issues).

[Link para a pesquisa](https://your.feedback.ibm.com/jfe/form/SV_2ftYFf9t72yFNIO)
