...
## Passo 1: Definir propriedades do problema a resolver {#step-1-set-properties-of-the-problem-to-solve}
...
em QPUs IBM&reg; usando QUICK-PDE, dado por:
...
processo de resolução e a solução. Para esta escolha de condição inicial,
a equação admite a seguinte solução analítica:

$$u(t, x) = \frac{ax + b}{at + 1},$$

que se mantém enquanto nenhum choque tiver se formado, ou seja, para $t < -1/a$ quando $a < 0$.
...
## Passo 2 (se necessário): Otimizar problema para execução em hardware quântico {#step-2-if-needed-optimize-problem-for-quantum-hardware-execution}
...
## Etapa 3: Compare o desempenho dos algoritmos {#step-3-compare-the-algorithm-performances}
...
## Etapa 4: Use o resultado {#step-4-use-the-result}
