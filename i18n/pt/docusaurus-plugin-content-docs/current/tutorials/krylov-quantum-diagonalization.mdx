---
description: 'Aprenda a diagonalização quântica de Krylov para Hamiltonianos de rede com o conceito de função Hamiltoniana.'
...
*Estimativa de uso: 70 minutos em um processador Heron ou Nighthawk (NOTA: Esta é apenas uma estimativa. O tempo de execução pode variar.)*

## Resultados de aprendizagem {#learning-outcomes}
Após completar este tutorial, você pode esperar entender as seguintes informações:
- Como interpretar a diagonalização quântica de Krylov (KQD) como o aprendizado de uma função Hamiltoniana finita que atua como um filtro espectral.
- Como construir as matrizes de Hamiltoniano projetado e de sobreposição com medições de teste de troca estendido (extended swap-test).
- Como resolver o problema de autovalor generalizado (GEVP) resultante e recuperar uma estimativa da energia do estado fundamental para um Hamiltoniano de rede.

## Pré-requisitos {#prerequisites}
Recomenda-se que você se familiarize com estes tópicos:
- [Primitivas do Qiskit](/guides/primitives)
- [Estimativa da energia do estado fundamental da cadeia de Heisenberg com VQE](/tutorials/spin-chain-vqe)
- [Curso de algoritmos de diagonalização quântica](/learning/courses/quantum-diagonalization-algorithms/krylov)

## Contexto {#background}

Este tutorial demonstra como implementar a diagonalização quântica de Krylov (KQD) no contexto dos padrões do Qiskit.
Você aprenderá primeiro a teoria por trás do algoritmo e, em seguida, verá uma demonstração de sua execução em uma QPU.

Estimar as propriedades de baixa energia de Hamiltonianos de muitos corpos é uma tarefa central na simulação quântica.
Por exemplo, energias do estado fundamental e excitações de baixa energia estão diretamente relacionadas à estabilidade química, ordenamento magnético, transições de fase quântica e resposta de materiais.
Em um computador clássico, a dimensão do espaço de Hilbert cresce exponencialmente com o número de orbitais ou spins, de modo que a diagonalização direta rapidamente se torna impraticável.

Existem várias abordagens de computação quântica para esse problema.
Métodos variacionais de curto prazo, como o [autosolver variacional quântico (VQE)](/tutorials/spin-chain-vqe), usam circuitos parametrizados relativamente rasos, mas exigem um laço de otimização clássica não linear com muitas avaliações de circuito quântico.
No outro extremo, a [estimativa de fase quântica (QPE)](https://arxiv.org/abs/quant-ph/0604193) fornece uma rota mais direta para a estimativa de autovalores com garantias rigorosas, mas a QPE padrão exige circuitos coerentes longos e é adequada principalmente a computadores quânticos tolerantes a falhas.
A KQD fica entre essas duas abordagens: ela usa evolução Hamiltoniana em tempo real, como nos algoritmos baseados em estimativa de fase, mas substitui a estimativa de fase completa por um problema de autovalor projetado compacto que pode ser resolvido classicamente.

Considere um Hamiltoniano de $n$ qubits $H$ e um estado de referência $\lvert \psi_{0}\rangle$.
O método KQD constrói um subespaço de Krylov a partir de estados evoluídos em tempo real,
...
onde $r$ é a dimensão de Krylov e $\Delta t$ é o passo de tempo.
Qualquer estado no subespaço de Krylov é então representado como uma combinação linear desses estados de base,
...
Com álgebra simples, podemos ver que a energia correspondente é escrita como o quociente de Rayleigh,
...
Aqui, as matrizes $\mathcal{S}$ e $\mathcal{H}$,
...
definem as matrizes projetadas de sobreposição e de Hamiltoniano.
Seus elementos são estimados usando medições de circuito quântico.

Buscamos encontrar o coeficiente $\mathbf{c}$ que fornece o mínimo $E(\mathbf{c})$:
...
Pelo teorema de Rayleigh-Ritz, essa minimização é equivalente a resolver o problema de autovalor generalizado (GEVP),
...
Observe que a dimensão $r$ pode ser pequena o suficiente para que um computador clássico resolva o GEVP.

Este é o mesmo princípio variacional usado na diagonalização de subespaço clássica, mas aqui os estados de base são gerados por evolução temporal quântica.
Comparado ao VQE, a KQD geralmente exige circuitos mais profundos porque depende de evolução em tempo real.
Em troca, a KQD evita a otimização de parâmetros não linear e a execução iterativa em hardware quântico, e melhora sistematicamente à medida que o subespaço projetado é ampliado.
O algoritmo já foi demonstrado em grande escala em hardware quântico existente [\[2\]](#references), e seu desempenho pode ser analisado com garantias comprováveis [\[1\]](#references).

## Requisitos {#requirements}

Antes de iniciar este tutorial, certifique-se de ter o seguinte instalado:

- Qiskit SDK v2.3 ou posterior, com suporte a [visualização](https://docs.quantum.ibm.com/api/qiskit/visualization)
...
A execução em hardware requer o `qiskit-ibm-runtime` e acesso a uma conta IBM Quantum&reg;.

## Configuração {#setup}

A célula de configuração importa os módulos necessários e define funções auxiliares para o fluxo de trabalho:

1. construir o Hamiltoniano de Heisenberg;

2. resolver o GEVP com limiar aplicado;

3. avaliar o filtro de Krylov aprendido;

4. converter valores do filtro em pesos espectrais;

5. plotar as distribuições de energia de referência e filtrada.
...
Na primeira parte deste tutorial, demonstramos o método KQD usando um simulador de vetor de estado local. Depois, usamos um backend quântico real para abordar um problema em escala de utilidade.

Também definimos um backend falso para demonstrar a transpilação específica do backend e inspecionar o circuito resultante.
...
#### Hamiltoniano e estado de referência {#hamiltonian-and-reference-state}
Este exemplo usa uma cadeia de Heisenberg de 12 qubits com fronteira aberta ($n=12$),
...
com um estado produto de excitação única
...
Como o Hamiltoniano de Heisenberg definido acima conserva o número total de excitações, o estado de referência permanece no subespaço de excitação única, cuja dimensão cresce apenas linearmente com o número de qubits. Podemos, portanto, calcular a energia exata do estado fundamental de forma eficiente, diagonalizando o Hamiltoniano restrito a esse subespaço, e usá-la puramente como um benchmark diagnóstico para a estimativa KQD. O próprio fluxo de trabalho KQD estima os elementos de matriz projetados usando primitivas do Qiskit e resolve o problema projetado resultante classicamente.
...
#### Definir os parâmetros do algoritmo {#set-the-parameters-for-the-algorithm}

Com base nos limites superiores da norma do Hamiltoniano, a Ref. [\[1\]](#references) sugere heuristicamente o passo de tempo $\Delta t$ como $\pi/\|H\|$.
Como a norma espectral $\|H\|$ é difícil de calcular, usamos em vez disso o seu limite superior:
...
Definimos a dimensão de Krylov como $r=10$ e o número de passos de Trotter por passo de tempo como $5$: um espaço de Krylov grande o suficiente para resolver o espectro de baixa energia, mantendo o circuito mais profundo ($t_{\max}=(r-1)\Delta t$) viável, e passos de Trotter suficientes para manter o erro de discretização pequeno nesse circuito mais profundo.
...
Aqui, construímos os circuitos para estimar os elementos de matriz $\mathcal{H}_{k\ell}$ e $\mathcal{S}_{k\ell}$.
Como todas as potências de $H$ comutam, temos
$$
\begin{equation*}
\mathcal{H}_{k\ell}
=
...
Aqui, apresentamos o circuito, chamado **teste de troca estendido**, que prepara
$$
\begin{equation*}
|\Phi_{0d}\rangle=
...
\end{equation*}
$$
onde $|\psi_d\rangle=e^{-id\Delta tH}|\psi_{0}\rangle$.

#### Estado de referência {#reference-state}

Preparamos o estado de referência $|\psi_0\rangle$.
...
Realize o operador de evolução temporal gerado pelo Hamiltoniano, aproximado por [Lie-Trotterização](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter) simples.
...
#### Circuito de teste de troca estendido [\[3\]](#references) {#extended-swap-test-circuit-3references}

O circuito primeiro prepara o estado de referência no registro do sistema enquanto o ancilla permanece em $|0\rangle$:
...
Em seguida, aplicar uma porta Hadamard ao ancilla cria uma superposição coerente de dois ramos:
...
Finalmente, a porta de evolução temporal controlada se aplica:
...
apenas quando o ancilla está no ramo $|1\rangle$. Portanto,
...
No próximo bloco de código, implementamos:
$$
\begin{equation*}
|\Psi(t)\rangle = \frac{|0\rangle|\psi_0\rangle + |1\rangle U(t)|\psi_0\rangle}{\sqrt{2}},
\end{equation*}
$$
que será atribuído como $t=d\Delta t$ para $d=1,\cdots r-1$ na etapa de execução.
...
Para qualquer observável hermitiano do sistema $O$, aqui definimos os observáveis a calcular:
...
Isso ocorre porque $O=I$ fornece o elemento de sobreposição $\mathcal{S}_{0d}=\langle\psi_0|\psi_d\rangle$, enquanto $O=H$ fornece o elemento Hamiltoniano $\mathcal{H}_{0d}=\langle\psi_0|H|\psi_d\rangle$.

Usando $X=|0\rangle\langle 1|+|1\rangle\langle 0|$, temos
...
Da mesma forma, usando $Y=-i|0\rangle\langle 1|+i|1\rangle\langle 0|$,
...
Finalmente, para cada estado $|\Psi_{0d}\rangle$, precisamos medir:
$$
\begin{align*}
...
Para a estimativa dos elementos de matriz $\mathcal{H}$, o número de termos de Pauli é muito maior do que para os elementos de matriz $\mathcal{S}$.

Agora podemos reduzir o número de termos Hamiltonianos medidos usando uma técnica de deslocamento [\[4\]](#references).
Dividimos o Hamiltoniano como:
...
onde $T$ é escolhido de forma que o estado de referência seja seu autoestado,
...
é o elemento de matriz Hamiltoniano deslocado.
Portanto, só precisamos medir $X\otimes (H-T)$ e $Y\otimes (H-T)$.
A contribuição de $T$ é reconstruída classicamente usando o elemento de matriz de sobreposição $\mathcal{S}_{0d}$ já medido.

Neste exemplo, uma escolha natural é a parte diagonal do Hamiltoniano de Heisenberg,
...
Como o estado de referência é um estado da base computacional, ele é um autoestado de cada termo $Z_iZ_{i+1}$.

No entanto, uma escolha mais vantajosa é incluir não apenas os termos diagonais $ZZ$, mas também os termos $XX+YY$ que anulam o estado de referência.

Para cada par vizinho, o operador $XX+YY$ satisfaz
...
Portanto, o termo $XX+YY$ contribui apenas quando os dois qubits vizinhos têm ocupações diferentes na cadeia de bits de referência.
Se os dois qubits forem ambos $0$ ou ambos $1$, o termo anula o estado de referência e também pode ser deslocado.

Seja $|\psi_{\rm ref}\rangle = |z_0\cdots z_{n-1}\rangle$, onde $z_i \in \{0,1\}$.
...
porque os termos $ZZ$ atuam diagonalmente em $|\psi_0\rangle$, enquanto os termos deslocados $XX+YY$ dão zero.
O autovalor correspondente é, portanto, determinado apenas pelos termos $ZZ$,
$$
\begin{equation*}
\tau = \sum_{i=0}^{n-2}
(-1)^{z_i}
...
Com essa escolha, o Hamiltoniano deslocado se torna:
$$
\begin{equation*}
H-T = \sum_{z_i\ne z_{i+1}}
\left(
...
Como resultado, apenas as arestas com ocupações diferentes no estado de referência precisam ser medidas.
Todos os termos $ZZ$ e todos os termos inativos $XX+YY$ são reconstruídos por meio da contribuição de sobreposição $\tau \mathcal{S}_{0d}$, ou dão contribuição zero por construção.

Isso fornece um observável menor do que deslocar apenas a parte diagonal.
Em particular, para um estado de referência da base computacional com uma excitação localizada, apenas as arestas adjacentes à excitação permanecem em $H-T$.
Portanto, o número de termos de Pauli em $X\otimes(H-T)$ e $Y\otimes(H-T)$ pode ser substancialmente reduzido, enquanto o elemento de matriz reconstruído
$$
\begin{equation*}
\widetilde{\mathcal{H}}_{0d}
+
...
Agora transformamos o circuito abstrato de teste de troca estendido em um modelo orientado a hardware.
Antes disso, otimizamos ainda mais o circuito primeiro em nível abstrato.

#### Comparar a ordenação de termos Hamiltonianos {#compare-hamiltonian-term-ordering}

Primeiro, comparamos diferentes ordenações dos termos de Pauli no Hamiltoniano de Heisenberg para a simulação Hamiltoniana.
O próprio Hamiltoniano permanece inalterado, mas a ordenação afeta como o circuito de fórmula de produto é gerado e o quanto a estrutura pode ser paralelizada no circuito.
Por exemplo, a ordenação ingênua lista todos os termos $XX$ de vizinhos mais próximos, depois todos os termos $YY$, depois todos os termos $ZZ$.
Isso coloca arestas adjacentes, como $(0,1)$ e $(1,2)$, uma ao lado da outra, de modo que não podem ser executadas em paralelo.
A ordenação par-depois-ímpar visita primeiro arestas pares disjuntas, seguidas pelas arestas ímpares, o que expõe camadas de dois qubits paralelas.
A ordenação agrupada por aresta par-ímpar vai um passo além: para cada aresta, mantém os termos locais $XX$, $YY$ e $ZZ$ juntos, ainda visitando arestas pares antes das ímpares.
Esperamos que as ordenações par-depois-ímpar e agrupada por aresta par-ímpar reduzam a profundidade do circuito ao expor camadas de dois qubits paralelas, e que a ordenação agrupada por aresta reduza adicionalmente o erro de Trotter porque a interação local de dois qubits na mesma aresta é tratada como um bloco compacto.

![Pauli evolutions with different term ordering](/docs/images/tutorials/krylov-quantum-diagonalization/ham_ordering.avif)

A ordenação do Hamiltoniano também afeta o erro de Trotter.
Se colocarmos termos que não comutam um ao lado do outro, as transições de base ocorrem com mais frequência, o que induz mais erro de Trotter.
Ao agrupar termos que exigem a mesma transformação de base de Pauli, mudanças de base redundantes podem ser evitadas.

Aqui, a comparação usa o maior tempo de evolução que aparece nas estimativas de Krylov da primeira linha,
$t_{\max} = (r-1)\Delta t,$
usando a mesma condição de transpilação.

Para medir o erro de Trotter, usamos a **infidelidade do processo** entre o circuito trotterizado e a evolução Hamiltoniana exata,
$$
\begin{equation*}
\text{Infidelity}(U,V)
=
...
Este diagnóstico usa matrizes densas, portanto é adequado para este pequeno exemplo de 12 qubits, mas não se destina a ser uma subrotina escalável.
...
Primeiro mantemos a regra de síntese fixa em um passo de Trotter de primeira ordem e variamos apenas a ordenação dos termos de Pauli.
O objetivo desta comparação é principalmente ver o quanto a profundidade do circuito e o custo de dois qubits podem ser reduzidos ao expor arestas disjuntas de vizinhos mais próximos ao transpilador.
...
Em `reps=1`, observamos que tanto a ordenação par-depois-ímpar quanto a agrupada por aresta par-ímpar reduzem a profundidade de 15 para 6 ao expor camadas de dois qubits paralelas, enquanto a contagem de portas de dois qubits permanece a mesma nas três ordenações.
No entanto, as três ordenações têm infidelidade próxima de 1, então variamos o número de repetições de Trotter para separar as ordenações de forma mais clara.
À medida que o número de repetições aumenta, a infidelidade da ordenação `even-odd edge-grouped` cai mais rápido do que as outras duas, atingindo 0,045 em `reps=8` contra 0,075 para a ordenação ingênua e a par-depois-ímpar.

#### Comparar a síntese de fórmula de produto {#compare-product-formula-synthesis}

Em seguida, exploramos diferentes configurações avançadas de trotterização, com a ordenação Hamiltoniana fixa na ordenação agrupada por aresta par-ímpar.
Consideramos Lie-Trotter de primeira ordem, Suzuki-Trotter de segunda ordem e Suzuki-Trotter de quarta ordem.
...
O Lie-Trotter fornece o circuito mais raso, mas tem o maior erro, enquanto o Suzuki-Trotter de quarta ordem é mais preciso, mas aumenta a profundidade do circuito.
Para o restante do tutorial, escolhemos o Suzuki-Trotter de segunda ordem porque ele fornece um circuito de pequena profundidade enquanto reduz substancialmente o erro de Trotter em relação à fórmula de primeira ordem.

#### Remover a porta de evolução temporal controlada {#remove-the-controlled-time-evolution-gate}

No teste de troca estendido, controlar a evolução temporal com um único qubit ancilla exige que o ancilla controle muitas portas em todo o sistema.
Isso pode introduzir uma sobrecarga substancial de roteamento e, no pior caso, exige efetivamente conectividade de todos para um.
Para evitar isso, é possível uma otimização adicional substituindo a porta de evolução temporal controlada por uma versão livre de controle, explorando a simetria do Hamiltoniano.
Vamos observar o seguinte circuito.

![circuit optimization](/docs/images/tutorials/krylov-quantum-diagonalization/circuit_opt.avif)

Aqui, $B_{\rm ref}$ prepara o estado de referência, $B_{\rm ref}|0^n\rangle = |\psi_{0}\rangle$.

Em vez de primeiro preparar $\frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)|\psi_0\rangle$ e depois aplicar $U(t)$ apenas no ramo $|1\rangle$, o circuito prepara diretamente os dois ramos como
...
O circuito primeiro aplica uma porta Hadamard ao ancilla e prepara o estado de referência apenas no ramo $|1\rangle$:
...
Em seguida, o operador de evolução temporal não controlado é aplicado a ambos os ramos:
...
Como o Hamiltoniano preserva o número de excitações, podemos ver que $|0^n\rangle$ é seu autoestado, e portanto
o operador de evolução acumula apenas uma fase sob o Hamiltoniano:
$$
\begin{equation*}
U(t)|0^n\rangle
=
...
Em seguida, $B_{\rm ref}$ é aplicado apenas no ramo $|0\rangle$.
...
Neste ponto, os dois ramos têm uma fase relativa extra. Para removê-la, aplicamos a porta de fase do ancilla
...
Assim, a menos de uma fase global irrelevante, finalmente preparamos
...
No próximo bloco de código, implementamos este circuito livre de controle.
...
Agora, transpilamos os circuitos controlado e livre de controle para que se tornem executáveis no hardware.
Vamos comparar o resultado dos circuitos transpilados.
...
O próximo passo é submeter o mesmo circuito parametrizado para vários valores de $d$. Para cada $d$, estimamos quatro valores esperados: $X\otimes I$, $Y\otimes I$, $X\otimes (H-T)$, e $Y\otimes (H-T)$. Esses quatro números são então combinados nos elementos complexos da primeira linha $\mathcal{S}_{0d}$ e $\mathcal{H}_{0d}$.

Aqui, $\mathcal{S}_{00}=1$ e $\mathcal{H}_{00}=\langle\psi_{0}|(H-T)|\psi_{0}\rangle=\langle\psi_{0}|H|\psi_{0}\rangle-\tau$ podem ser calculados classicamente, já que $\lvert\psi_0\rangle$ é esparso, então pulamos o caso $d=0$.
...
Depois de estimar as matrizes projetadas, regularizamos e resolvemos o GEVP
$$
\begin{equation*}
\mathcal{H}\mathbf{c} = E\mathcal{S}\mathbf{c}.
\end{equation*}
$$
O menor autovalor generalizado fornece a estimativa KQD da energia do estado fundamental.
...
Agora resolvemos o problema de autovalor generalizado usando as matrizes reconstruídas a partir das estimativas do circuito.
Em um cálculo ideal de vetor de estado com evolução exata em tempo real, isso deve reproduzir o resultado projetado exato.
Na prática, os desvios podem vir da trotterização, do erro de amostragem e da instabilidade numérica da matriz de sobreposição.

Observamos como a energia converge à medida que aumentamos a dimensão do subespaço de Krylov.
...
A seção anterior usou um modelo de 12 qubits para que a simulação de vetor de estado pudesse ser usada como diagnóstico. Agora escalamos o mesmo fluxo de trabalho KQD para uma cadeia de Heisenberg de 30 qubits e preparamos a carga de trabalho para execução em hardware do IBM Quantum.

### Passos 1-4 comprimidos em um único bloco de código {#steps-1-4-compressed-into-a-single-code-block}

Aqui agora juntamos todos esses detalhes em um único fluxo de trabalho em uma escala maior, que é então executado em nosso hardware quântico real. Nesta seção, aplicamos configurações realistas de mitigação de erros para melhorar a confiabilidade dos resultados. Como os elementos de matriz correspondentes a diferentes valores de $d$ podem ser avaliados em paralelo, usamos o modo `Batch` para executá-los de forma eficiente.
...
## Apêndice: A perspectiva da função hamiltoniana (filtro espectral) {#appendix-the-hamiltonian-function-spectral-filter-viewpoint}

O fluxo de trabalho principal apresentou o KQD de forma operacional: construir uma base de Krylov a partir de estados evoluídos em tempo real, estimar as matrizes projetadas $\mathcal{H}$ e $\mathcal{S}$, e resolver o GEVP.
Este apêndice revisita a *mesma* computação a partir de um ângulo complementar que explica **por que** o KQD funciona: a perspectiva da *função hamiltoniana*, ou *filtro espectral* [\[3\]](#references), [\[5\]](#references).
Ele reutiliza o modelo de 12 qubits, o passo de tempo $\Delta t$, e a solução de Krylov já obtida acima; nenhuma nova execução de circuito é necessária.

### O estado de referência como uma distribuição de energia {#the-reference-state-as-an-energy-distribution}

Seja a decomposição espectral do hamiltoniano
...
com autoestados de energia $|E_m\rangle$.
Qualquer estado de referência pode ser expandido nesta autobase,
...
então carrega um **peso espectral** $p_m=|a_m|^2$ em cada energia $E_m$.
A energia de referência é a média desta distribuição, $\langle H\rangle_{0}=\sum_m p_m E_m$.

A decomposição espectral de um hamiltoniano geral de $n$ qubits tem custo exponencial, então esta imagem é apenas um diagnóstico, que *não* faz parte do algoritmo.
Aqui, no entanto, podemos calculá-la de forma barata para o *mesmo* problema com $n=12$: o hamiltoniano de Heisenberg conserva o número total de excitações, e a referência $|000001000000\rangle$ carrega uma única excitação, então todo o seu conteúdo espectral vive no subespaço de excitação única, cuja dimensão cresce apenas linearmente com $n$.
Portanto, reutilizamos o bloco exato de excitação única (já usado acima como referência) e extraímos a distribuição de referência $\{(E_m, p_m)\}$ dentro desse subespaço.
...
### O KQD aprende um filtro que remodela esta distribuição {#kqd-learns-a-filter-that-reshapes-this-distribution}

Uma **função hamiltoniana** $f(H)$ é definida por meio do cálculo espectral,
...
ou, em outras palavras, uma soma ponderada de autoprojetores.
Aplicá-la à referência remodela cada amplitude espectral, $a_m \to a_m f(E_m)$:
...
Se $f$ fosse fortemente concentrada na energia mais baixa ($f(E_0)=1$ e $f(E_m)\approx 0$ caso contrário), então $f(H)$ atuaria como um projetor do estado fundamental, e a saída normalizada seria (próxima) ao estado fundamental.
Portanto, um bom **filtro espectral** passa-baixa em energia é exatamente o que queremos.

O KQD não prescreve $f$ antecipadamente. Em vez disso, expande o filtro na base de evolução em tempo real,
...
uma função *trigonométrica* da energia cujos coeficientes $\{c_\ell\}$ são precisamente o autovetor do GEVP resolvido acima.
Minimizar o quociente de Rayleigh $\mathbf{c}^\dagger\mathcal{H}\mathbf{c}/\mathbf{c}^\dagger\mathcal{S}\mathbf{c}$ é, portanto, o mesmo que *aprender* o filtro que melhor suprime o peso do estado excitado da referência.
Uma dimensão de Krylov $r$ maior dá ao filtro mais graus de liberdade e um pico mais nítido na energia do estado fundamental.

O auxiliar abaixo avalia este filtro aprendido em um eixo de energia; então o aplicamos à distribuição de referência obtida acima.
...
### Visualize o filtro e sua flexibilidade {#visualize-the-filter-and-its-flexibility}

Primeiro mostramos o filtro aprendido na dimensão de Krylov completa usada acima, e então acompanhamos como ele se torna mais nítido à medida que a dimensão $r$ cresce.

As barras mostram os pesos espectrais de referência $p_m$ (antes) e os pesos filtrados $p_m|f_{\rm KQD}(E_m)|^2$ (depois), junto com a intensidade do filtro aprendido $|f_{\rm KQD}(E)|^2$ em um eixo de energia contínuo.
O filtro concentra o peso na energia mais baixa do subespaço de excitação única &mdash; a mesma energia para a qual a estimativa KQD convergiu no exemplo em pequena escala.
Observe que este é o estado fundamental *dentro do setor de excitação única*, que é o alvo relevante para este estado de referência que conserva excitações, não o estado fundamental global.
...
### Aumentando a dimensão de Krylov: flexibilidade da função aprendida {#increasing-the-krylov-dimension-flexibility-of-the-learned-function}

Lembre-se de que o filtro aprendido é um polinômio trigonométrico na energia com $r$ coeficientes,
...
A dimensão de Krylov $r$ é exatamente o número de coeficientes livres, então ela controla a **flexibilidade** da função.
Um $r$ pequeno só pode produzir um filtro amplo e suavemente variável que vaza peso para estados excitados de baixa energia; quando $r$ aumenta, o filtro pode formar um pico mais estreito na energia alvo e suprimir o peso restante do estado excitado de forma mais agressiva.
Este é o análogo, do ponto de vista do filtro espectral, da convergência de energia observada no exemplo em pequena escala: à medida que $r$ cresce, a distribuição filtrada colapsa sobre o estado fundamental do subespaço e a energia estimada diminui em direção a ele.

Reutilizamos as matrizes projetadas já estimadas acima e simplesmente resolvemos o GEVP em cada bloco principal $r\times r$, e então avaliamos e plotamos o filtro correspondente.
