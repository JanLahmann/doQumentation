...
- Como os kernels quânticos projetados (PQK) funcionam e quando oferecem uma possível vantagem quântica.
...
- [Kernels quânticos](/learning/courses/quantum-machine-learning/quantum-kernel-methods) do [curso de aprendizado de máquina quântico](/learning/courses/quantum-machine-learning) no IBM Quantum&reg; Learning
...
Os principais blocos de construção dos PQKs são as matrizes de densidade reduzidas (RDMs, do inglês *reduced density matrices*), obtidas por meio de medições projetivas do mapa de características quântico. Em particular, geralmente se calculam as matrizes de densidade reduzidas de um único qubit (1-RDMs) para cada qubit. Essas quantidades medidas são então usadas como entradas para uma função de kernel clássica, como um kernel exponencial, para construir a matriz de kernel final.

Os PQKs oferecem vantagens potenciais em relação aos [kernels quânticos](/tutorials/quantum-kernel-training) padrão, especialmente para hardware quântico de curto prazo. Os kernels quânticos padrão geralmente dependem da estimativa de sobreposições de estados globais, que se tornam cada vez mais difíceis de medir com precisão à medida que o número de qubits cresce e são altamente sensíveis ao ruído. Em contraste, os PQKs usam observáveis locais, como as matrizes de densidade reduzidas de um único qubit (1-RDMs), levando a uma menor sobrecarga de amostragem, maior robustez ao ruído de hardware e melhor escalabilidade. Ao projetar os estados quânticos em características de medição locais antes de aplicar uma função de kernel clássica, os PQKs podem reter correlações quânticas úteis enquanto permanecem mais práticos para dispositivos de curto prazo.
...
Omitimos o exemplo em simulador de pequena escala neste tutorial, pois nosso objetivo principal é demonstrar como os kernels quânticos projetados podem escalar para sistemas maiores e hardware real.
...
Neste passo, obtemos todas as matrizes de densidade reduzidas de um único qubit (1-RDMs) por meio de medições projetivas do mapa de características quântico, que serão fornecidas posteriormente à função de kernel exponencial clássica.
...
- Curso aprofundado de [aprendizado de máquina quântico](/learning/courses/quantum-machine-learning) do IBM Quantum Learning
- Tutorial de [treinamento de kernel quântico](/tutorials/quantum-kernel-training)
</Admonition>
