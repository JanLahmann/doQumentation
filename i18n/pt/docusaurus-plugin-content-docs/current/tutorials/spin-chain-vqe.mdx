...
- Como executar fluxos de trabalho variacionais em hardware quântico IBM&reg; usando primitivas e sessões do IBM Quantum
...
Minimize a energia estimada do estado fundamental do sistema otimizando os parâmetros do circuito. Use a primitiva `Estimator` do IBM Quantum para avaliar a função de custo durante a otimização.

Como otimizamos o circuito para o backend no Passo 2, podemos evitar a transpilação no servidor Quantum Compute definindo `skip_transpilation=True` e passando o circuito otimizado. Para esta demonstração, executaremos em uma QPU usando primitivas do IBM Quantum. Para executar com primitivas baseadas em statevector do `qiskit`, substitua o bloco de código que usa primitivas do IBM Quantum pelo bloco comentado.
