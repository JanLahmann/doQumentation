---
title: "Fórmulas multi-produto para reduzir o erro de Trotter"
sidebar_label: "Fórmulas multi-produto para reduzir o erro de Trotter"
description: "As fórmulas multi-produto podem ser usadas na estimativa de observáveis para reduzir o erro de Trotter ou para implementar a evolução temporal com erro de Trotter fixo em menor profundidade."
...
*Uso estimado de QPU: Quatro minutos em um processador Heron r2 (NOTA: Esta é apenas uma estimativa. Seu tempo de execução pode variar.)*

## Contexto {#background}
Este tutorial demonstra como usar uma Fórmula Multi-Produto (MPF, do inglês *Multi-Product Formula*) para obter um erro de Trotter menor no nosso observável em comparação ao incorrido pelo circuito de Trotter mais profundo que efetivamente executaremos.
As MPFs reduzem o erro de Trotter da dinâmica hamiltoniana por meio de uma combinação ponderada de várias execuções de circuito. Considere a tarefa de encontrar os valores esperados de observáveis para o estado quântico
$\rho(t)=e^{-i H t} \rho(0) e^{i H t}$ com o Hamiltoniano $H$. É possível usar Fórmulas de Produto (PFs, do inglês *Product Formulas*) para aproximar a evolução temporal $e^{-i H t}$ da seguinte forma:
...
que possui um termo de erro quadrático $S_1(t)=e^{-i H t}+\mathcal{O}\left(t^{2}\right)$. Também é possível usar PFs de ordem superior (fórmulas de Lie-Trotter-Suzuki), que convergem mais rapidamente e são definidas recursivamente como:
...
onde $\chi$ é a ordem da PF simétrica e $s_p = \left( 4 - 4^{1/(2p-1)} \right)^{-1}$. Para evoluções temporais longas, é possível dividir o intervalo de tempo $t$ em $k$ intervalos, chamados passos de Trotter, de duração $t/k$ e aproximar a evolução temporal em cada intervalo com uma fórmula de produto de ordem $\chi$, $S_{\chi}$. Assim, a PF de ordem $\chi$ para o operador de evolução temporal ao longo de $k$ passos de Trotter é:
...
Dado um inteiro $k \geq 1$ e uma fórmula de produto $S_{\chi}(t)$, o estado aproximado evoluído no tempo $\rho_k(t)$ pode ser obtido a partir de $\rho_0$ aplicando $k$ iterações da fórmula de produto $S_{\chi}\left(\frac{t}{k}\right)$.
...
onde $x_j$ são os coeficientes de ponderação, $\rho^{k_j}_j$ é a matriz densidade correspondente ao estado puro obtido pela evolução do estado inicial com a fórmula de produto $S^{k_j}_{\chi}$, envolvendo $k_j$ passos de Trotter, e $j \in {1, ..., l}$ indexa o número de PFs que compõem a MPF. Todos os termos em $\mu(t)$ utilizam a mesma fórmula de produto $S_{\chi}(t)$ como base.
O objetivo é melhorar ||$\rho_k(t)-\rho(t) \|$ encontrando $\mu(t)$ com $\|\mu(t)-\rho(t)\|$ ainda menor.

* $\mu(t)$ não precisa ser um estado físico, pois $x_i$ não precisa ser positivo. O objetivo aqui é minimizar o erro no valor esperado dos observáveis e não encontrar um substituto físico para $\rho(t)$.
* $k_j$ determina tanto a profundidade do circuito quanto o nível de aproximação de Trotter. Valores menores de $k_j$ resultam em circuitos mais curtos, que incorrem em menos erros de circuito, mas serão uma aproximação menos precisa do estado desejado.

O ponto central aqui é que o erro de Trotter residual dado por $\mu(t)$ é menor do que o erro de Trotter que se obteria simplesmente usando o maior valor de $k_j$.

Você pode ver a utilidade disso sob duas perspectivas:

1. Para um orçamento fixo de passos de Trotter que você pode executar, é possível obter resultados com um erro de Trotter menor no total.
2. Dado um número alvo de passos de Trotter que é grande demais para ser executado, você pode usar a MPF para encontrar uma coleção de circuitos de menor profundidade a executar que resulte em um erro de Trotter semelhante.
...
Não há restrição óbvia sobre a escolha do número de passos de Trotter $k_j$ que compõem o estado MPF $\mu(t)$. No entanto, esses valores devem ser escolhidos com cuidado para evitar instabilidades nos valores esperados resultantes calculados a partir de $\mu(t)$. Uma boa regra geral é definir o menor passo de Trotter $k_{\text{min}}$ de forma que $t/k_{\text{min}} \lt 1$. Se desejar aprender mais sobre isso e como escolher os demais valores de $k_j$, consulte o guia [How to choose the Trotter steps for an MPF](https://qiskit.github.io/qiskit-addon-mpf/how_tos/choose_trotter_steps.html).

No exemplo abaixo, exploramos a estabilidade da solução MPF calculando o valor esperado da magnetização para uma faixa de tempos usando diferentes estados evoluídos no tempo. Especificamente, comparamos os valores esperados calculados a partir de cada uma das evoluções temporais aproximadas implementadas com os passos de Trotter correspondentes e os diversos modelos MPF (coeficientes estáticos e dinâmicos) com os valores exatos do observável evoluído no tempo. Primeiro, vamos definir os parâmetros para as fórmulas de Trotter e os tempos de evolução.
...
Para este exemplo, usaremos o estado de Néel como estado inicial $\vert \text{Neel} \rangle = \vert 0101...01 \rangle$ e o modelo de Heisenberg em uma cadeia de 10 sítios para o Hamiltoniano que governa a evolução temporal

$$
\hat{\mathcal{H}}_{Heis} = J \sum_{i=1}^{L-1} \left(X_i X_{(i+1)}+Y_i Y_{(i+1)}+ Z_i Z_{(i+1)} \right) \, ,
$$

onde $J$ é a intensidade de acoplamento para arestas de primeiros vizinhos.
...
Definimos uma passagem de transpilador para coletar as rotações XX e YY no circuito como uma única porta XX+YY. Isso nos permitirá aproveitar as propriedades de conservação de spin do TeNPy durante o cálculo do MPO, acelerando significativamente o cálculo.
...
Em seguida, criamos os circuitos que implementam as evoluções temporais aproximadas de Trotter.
...
Em seguida, calculamos os valores esperados evoluídos no tempo a partir dos circuitos de Trotter.
...
Também calculamos os valores esperados exatos para comparação.
...
As MPFs estáticas são aquelas em que os valores $x_j$ não dependem do tempo de evolução, $t$. Consideremos a PF de ordem $\chi = 1$ com $k_j$ passos de Trotter, que pode ser escrita como:

$$ S_1^{k_j}\left( \frac{t}{k_j} \right)=e^{-i H t}+ \sum_{n=1}^{\infty} A_n \frac{t^{n+1}}{k_j^n}  $$

onde $A_n$ são matrizes que dependem dos comutadores dos termos $F_a$ na decomposição do Hamiltoniano. É importante notar que os próprios $A_n$ são independentes do tempo e do número de passos de Trotter $k_j$. Portanto, é possível cancelar os termos de erro de ordem inferior que contribuem para $\mu(t)$ com uma escolha cuidadosa dos pesos $x_j$ da combinação linear. Para cancelar o erro de Trotter dos primeiros $l-1$ termos (estes darão as maiores contribuições, pois correspondem ao menor número de passos de Trotter) na expressão de $\mu(t)$, os coeficientes $x_j$ devem satisfazer as seguintes equações:

$$ \sum_{j=1}^l x_j = 1 $$
$$ \sum_{j=1}^{l-1} \frac{x_j}{k_j^{n}} = 0 $$

com $n=0, ... l-2$. A primeira equação garante que não há viés no estado construído $\mu(t)$, enquanto a segunda equação assegura o cancelamento dos erros de Trotter. Para PFs de ordem superior, a segunda equação se torna $ \sum_{j=1}^{l-1} \frac{x_j}{k_j^{\eta}} = 0 $ onde $\eta = \chi + 2n$ para PFs simétricas e $\eta = \chi + n$ caso contrário, com $n=0, ..., l-2$. O erro resultante (Refs. [\[1\]](#references),[\[2\]](#references)) é então

$$ \epsilon = \mathcal{O} \left( \frac{t^{l+1}}{k_1^l} \right).$$

Determinar os coeficientes MPF estáticos para um dado conjunto de valores $k_j$ equivale a resolver o sistema linear de equações definido pelas duas equações acima para as variáveis $x_j$: $Ax=b$. Onde $x$ são os coeficientes de interesse, $A$ é uma matriz que depende de $k_j$ e do tipo de PF utilizado ($S$), e $b$ é um vetor de restrições. Especificamente:
...
onde $\chi$ é a ``order``, $s$ é $2$ se ``symmetric`` for ``True`` e $1$ caso contrário, $k_{j}$ são os ``trotter_steps``, e $x$ são as variáveis a resolver. Os índices $i$ e $j$ começam em $0$. Também podemos visualizar isso na forma matricial:
...
Podemos encontrar uma solução para $x$ analiticamente como $x = A^{-1}b$; veja, por exemplo, as Refs. [\[1\]](#references) ou [\[2\]](#references).
No entanto, essa solução exata pode ser "mal-condicionada", resultando em normas L1 muito grandes dos nossos coeficientes $x$, o que pode levar a um desempenho ruim da MPF.
Em vez disso, também é possível obter uma solução aproximada que minimize a norma L1 de $x$ na tentativa de otimizar o comportamento da MPF.
##### Configurar o LSE {#set-up-the-lse}

Agora que escolhemos os valores de $k_j$, devemos primeiro construir o LSE, $Ax=b$, conforme explicado acima.
A matriz $A$ depende não apenas de $k_j$, mas também da nossa escolha de PF, em particular de sua _ordem_.
Adicionalmente, você pode levar em conta se a PF é simétrica ou não (veja [\[1\]](#references)) definindo `symmetric=True/False`.
No entanto, isso não é obrigatório, como demonstrado pela Ref. [\[2\]](#references).
...
Isso pode ser verificado inspecionando o objeto `lse`:
...
O objeto `lse` possui métodos para encontrar os coeficientes estáticos $x_j$ que satisfazem o sistema de equações.
...
##### Otimizar $x$ usando um modelo exato

Como alternativa ao cálculo de $x=A^{-1}b$, você também pode usar [setup_exact_model](https://qiskit.github.io/qiskit-addon-mpf/stubs/qiskit_addon_mpf.static.setup_exact_model.html) para construir uma instância de [cvxpy.Problem](https://www.cvxpy.org/api_reference/cvxpy.problems.html#cvxpy.Problem) que utiliza o LSE como restrições e cuja solução ótima produzirá $x$.
...
Como indicador de se um MPF construído com esses coeficientes produzirá bons resultados, podemos usar a norma L1 (veja também a Ref. [\[1\]](#references)).
...
##### Otimizar $x$ usando um modelo aproximado

Pode acontecer que a norma L1 para o conjunto escolhido de valores $k_j$ seja considerada muito alta.
Se for esse o caso e você não puder escolher um conjunto diferente de valores $k_j$, pode usar uma solução aproximada para o LSE em vez de uma exata.

Para isso, basta usar [setup_approximate_model](https://qiskit.github.io/qiskit-addon-mpf/stubs/qiskit_addon_mpf.static.setup_approximate_model.html) para construir uma instância diferente de [cvxpy.Problem](https://www.cvxpy.org/api_reference/cvxpy.problems.html#cvxpy.Problem), que restringe a norma L1 a um limiar escolhido enquanto minimiza a diferença entre $Ax$ e $b$.
...
Observe que você tem total liberdade sobre como resolver esse problema de otimização, o que significa que pode alterar o solver de otimização, seus limiares de convergência e assim por diante.
Consulte o guia correspondente em [How to use the approximate model.](https://qiskit.github.io/qiskit-addon-mpf/how_tos/using_approximate_model.html)
#### Coeficientes MPF dinâmicos

Na seção anterior, introduzimos um MPF estático que melhora a aproximação de Trotter padrão. No entanto, essa versão estática não minimiza necessariamente o erro de aproximação. Em termos concretos, o MPF estático, denotado $\mu^S(t)$, não é a projeção ótima de $\rho(t)$ no subespaço gerado pelos estados de fórmula produto $\{\rho_{k_i}(t)\}_{i=1}^r$.

Para resolver isso, consideramos um MPF dinâmico (introduzido na Ref. [\[2\]](#references) e demonstrado experimentalmente na Ref. [\[3\]](#references)) que minimiza o erro de aproximação na norma de Frobenius. Formalmente, nos concentramos em minimizar

$$
\|\rho(t) - \mu^D(t)\|_F^2 \;=\; \mathrm{Tr}\bigl[ \left( \rho(t) - \mu^D(t)\right)^2 \bigr],
$$

em relação a alguns coeficientes $x_i(t)$ em cada instante $t$. O projetor *ótimo* na norma de Frobenius é então $\mu^D(t) \;=\; \sum_{i=1}^r x_i(t)\,\rho_{k_i}(t)$, e chamamos $\mu^D(t)$ de MPF *dinâmico*. Substituindo as definições acima:
...
representa a sobreposição entre o estado exato $\rho(t)$ e cada aproximação de fórmula produto $\rho_{k_i}(t)$. Em cenários práticos, essas sobreposições podem ser medidas apenas de forma aproximada devido a ruído ou acesso parcial a $\rho(t)$.

Aqui, $\lvert\psi_{\mathrm{in}}\rangle$ é o estado inicial e $S(\cdot)$ é a operação aplicada na fórmula produto. Ao escolher os coeficientes $x_i(t)$ que minimizam essa expressão (e tratando dados de sobreposição aproximados quando $\rho(t)$ não é completamente conhecido), obtemos a melhor aproximação dinâmica (no sentido da norma de Frobenius) de $\rho(t)$ dentro do subespaço MPF. As quantidades $L_i(t)$ e $M_{i,j}(t)$ podem ser calculadas eficientemente usando métodos de redes tensoriais [\[3\]](#references). O addon MPF do Qiskit fornece vários "backends" para realizar esse cálculo. O exemplo abaixo mostra a forma mais flexível de fazê-lo, e a documentação do [backend baseado em camadas do TeNPy](https://qiskit.github.io/qiskit-addon-mpf/apidocs/qiskit_addon_mpf.backends.tenpy_layers.html#module-qiskit_addon_mpf.backends.tenpy_layers) também explica em detalhes. Para usar esse método, parta do circuito que implementa a evolução temporal desejada e crie modelos que representem essas operações a partir das camadas do circuito correspondente. Por fim, um objeto `Evolver` é criado e pode ser usado para gerar as quantidades com evolução temporal $M_{i,j}(t)$ e $L_i(t)$. Começamos criando o objeto `Evolver` correspondente à evolução temporal aproximada ([`ApproxEvolverFactory`](https://qiskit.github.io/qiskit-addon-mpf/apidocs/qiskit_addon_mpf.dynamic.html#qiskit_addon_mpf.dynamic.ApproxEvolverFactory)) implementada pelos circuitos. Em particular, preste atenção especial à variável `order` para que os valores coincidam. Observe que, ao gerar os circuitos correspondentes à evolução temporal aproximada, usamos valores de espaço reservado para `time = 1.0` e o número de passos de Trotter (`reps=1`). Os circuitos de aproximação corretos são então produzidos pelo solver do problema dinâmico em `setup_dynamic_lse`.
...
As opções do `LayerwiseEvolver` que determinam os detalhes da simulação de redes tensoriais devem ser escolhidas com cuidado para evitar a configuração de um problema de otimização mal definido.
</Admonition>
Em seguida, configuramos o evoluidor exato (por exemplo, [`ExactEvolverFactory`](https://qiskit.github.io/qiskit-addon-mpf/apidocs/qiskit_addon_mpf.dynamic.html#qiskit_addon_mpf.dynamic.ExactEvolverFactory)), que retorna um objeto [`Evolver`](https://qiskit.github.io/qiskit-addon-mpf/apidocs/qiskit_addon_mpf.backends.html#qiskit_addon_mpf.backends.Evolver) que calcula a evolução temporal verdadeira ou de "referência". Na prática, aproximaríamos a evolução exata usando uma fórmula de Suzuki–Trotter de ordem superior ou outro método confiável com um passo de tempo pequeno. Abaixo, aproximamos o estado com evolução temporal exata com uma fórmula de Suzuki-Trotter de quarta ordem usando um passo de tempo pequeno `dt=0.1`, o que significa que o número de passos de Trotter usado no instante $t$ é $k=t/dt$. Também especificamos algumas opções de truncamento específicas do TeNPy para limitar a dimensão máxima de ligação da rede tensorial subjacente, bem como os valores singulares mínimos das ligações de redes tensoriais divididas. Esses parâmetros podem afetar a precisão do valor esperado calculado com os coeficientes MPF dinâmicos, por isso é importante explorar uma faixa de valores para encontrar o equilíbrio ideal entre tempo computacional e precisão. Observe que o cálculo dos coeficientes MPF não depende do valor esperado do PF obtido da execução em hardware e, portanto, pode ser ajustado em pós-processamento.
...
Em seguida, crie o estado inicial do seu sistema em um formato compatível com o TeNPy (por exemplo, um `MPS_neel_state`=$\vert 0101...01 \rangle$). Isso configura a função de onda de muitos corpos que você evoluirá no tempo $\lvert\psi_{\mathrm{in}}\rangle$ como um tensor.
...
Por fim, plote esses valores esperados ao longo do tempo de evolução.
...
Em casos como o exemplo acima, onde o PF de $k=1$ se comporta mal em todos os instantes, a qualidade dos resultados do MPF dinâmico também é fortemente afetada. Nessas situações, é útil investigar a possibilidade de usar PFs individuais com um número maior de passos de Trotter para melhorar a qualidade geral dos resultados. Nessas simulações, observamos a interação entre diferentes tipos de erros: erro proveniente de amostragem finita e erro de Trotter das fórmulas produto. O MPF ajuda a reduzir o erro de Trotter devido às fórmulas produto, mas incorre em um erro de amostragem maior em comparação com as fórmulas produto. Isso pode ser vantajoso, pois as fórmulas produto podem reduzir o erro de amostragem com um aumento da amostragem, mas o erro sistemático devido à aproximação de Trotter permanece inalterado.

Outro comportamento interessante que podemos observar no gráfico é que o valor esperado para o PF com $k=1$ começa a se comportar de forma errática (além de não ser uma boa aproximação para o exato) nos instantes em que $t/k > 1$, conforme explicado no [guia](https://qiskit.github.io/qiskit-addon-mpf/how_tos/choose_trotter_steps.html) sobre como escolher o número de passos de Trotter.
### Etapa 1: Mapear entradas clássicas para um problema quântico
Consideremos agora um único instante $t=1.0$ e calculemos o valor esperado da magnetização com os vários métodos usando uma QPU. A escolha particular de $t$ foi feita de forma a maximizar a diferença entre os vários métodos e observar sua eficácia relativa. Para determinar a janela de tempo na qual o MPF dinâmico garante produzir observáveis com erro menor do que qualquer uma das fórmulas de Trotter individuais dentro do multiproduto, podemos implementar o "teste MPF" — veja a equação (17) e o texto ao redor em [\[3\]](#references).
...
Vamos retornar ao cálculo do valor esperado para um único ponto no tempo. Escolheremos um backend para executar o experimento em hardware.
...
Em seguida, removemos qubits atípicos do mapa de acoplamento para garantir que o estágio de layout do transpilador não os inclua. A seguir, utilizamos as propriedades do backend reportadas e armazenadas no objeto `target`, removendo qubits que apresentem erro de medição ou porta de dois qubits acima de um determinado limiar (`max_meas_err`, `max_twoq_err`), ou tempo $T_2$ (que determina a perda de coerência) abaixo de um determinado limiar (`min_t2`).
...
Queremos definir `max_meas_err`, `min_t2` e `max_twoq_err` de forma que encontremos um subconjunto de qubits suficientemente grande para suportar a execução do circuito. No nosso caso, é suficiente encontrar uma cadeia 1D de 10 qubits.
...
Podemos então mapear o circuito e o observável para os qubits físicos do dispositivo.
...
### Etapa 3: Executar usando primitivas do Qiskit {#step-1-map-classical-inputs-to-a-quantum-problem}
Com a primitiva Estimator, podemos obter a estimativa do valor esperado a partir da QPU. Executamos os circuitos AQC otimizados com técnicas adicionais de mitigação e supressão de erros.
...
A única etapa de pós-processamento consiste em combinar o valor esperado obtido das primitivas do Qiskit Runtime em diferentes passos de Trotter utilizando os respectivos coeficientes MPF. Para um observável $A$, temos:
...
Em seguida, simplesmente os recombinamos com nossos coeficientes MPF para obter os valores esperados totais do MPF. A seguir, fazemos isso para cada uma das diferentes formas pelas quais calculamos $x$.
...
Por fim, para este problema de pequena escala, podemos calcular o valor de referência exato usando [scipy.linalg.expm](https://docs.scipy.org/doc/scipy/reference/generated/scipy.linalg.expm.html) da seguinte forma:
...
No exemplo acima, o método MPF dinâmico apresenta o melhor desempenho em termos de valor esperado, melhorando o resultado que teríamos obtido utilizando apenas o maior número de passos de Trotter. Embora as diversas técnicas MPF nem sempre alcancem um valor esperado melhorado em comparação ao maior número de passos de Trotter (como os modelos exato e aproximado no gráfico acima), o desvio padrão desses valores captura bem a maior variância incorrida ao utilizar a técnica MPF. Isso evidencia a incerteza em torno do valor esperado obtido, que sempre inclui o valor esperado que esperaríamos de uma evolução temporal exata do sistema. Por outro lado, os valores esperados calculados com o menor número de passos de Trotter não conseguem capturar o valor esperado exato dentro de sua incerteza, retornando com confiança o resultado errado.
...
## Parte II: ampliando a escala

Vamos escalar o problema além do que é possível simular de forma exata. Nesta seção, vamos nos concentrar em reproduzir alguns dos resultados apresentados na Ref. [\[3\]](#references).
### Etapa 1: Mapear entradas clássicas para um problema quântico
#### Hamiltoniano {#dynamic-coefficients}

Para o exemplo de grande escala, utilizamos o modelo XXZ em uma cadeia de 50 sítios:
...
Como observável, escolhemos $Z_{24}Z_{25}$, conforme mostrado no painel inferior da Fig. 5 da Ref. [\[3\]](#references).
...
#### Escolher os passos de Trotter {#construct-each-of-the-trotter-circuits-in-our-mpf-decomposition}
O experimento apresentado na Fig. 4 da Ref. [\[3\]](#references) utiliza $k_j = [2, 3, 4]$ passos de Trotter simétricos de ordem $2$. Focamos nos resultados para o tempo $t=3$, onde o MPF e um PF com um número maior de passos de Trotter (6, neste caso) apresentam o mesmo erro de Trotter. No entanto, o valor esperado do MPF é calculado a partir de circuitos correspondentes ao número menor de passos de Trotter e, portanto, mais rasos. Na prática, mesmo que o MPF e o circuito com passos de Trotter mais profundos tenham o mesmo erro de Trotter, esperamos que o valor esperado experimental calculado a partir dos circuitos do MPF seja mais próximo do valor teórico, pois envolve a execução de circuitos mais rasos e menos expostos ao ruído do hardware, em comparação com o circuito correspondente ao PF com maior número de passos de Trotter.
...
#### Configurar o LSE {#construct-trotter-circuit-with-comparable-trotter-error-to-mpf}
Aqui analisamos os coeficientes MPF estáticos para este problema.
...
#### Construir cada um dos circuitos de Trotter na decomposição MPF
...
#### Construir o circuito de Trotter com erro de Trotter comparável ao MPF
...
Ao executar circuitos em hardware, podemos nos deparar com desafios adicionais para obter valores esperados precisos devido à presença de ruído de hardware. Esse ruído não é levado em conta no formalismo MPF e pode trabalhar contra a solução MPF. Por exemplo, essa pode ser a razão pelo qual os coeficientes dinâmicos falham em fornecer uma estimativa melhor do valor esperado em comparação com o coeficiente estático aproximado no gráfico. Ou seja, o evoluidor aproximado, que simula o circuito aproximado, não reflete com precisão os resultados obtidos ao executar os circuitos aproximados na presença de ruído de hardware. Por essas razões, é recomendável combinar diferentes técnicas de mitigação de erros a fim de obter resultados o mais próximos possível dos valores ideais para cada uma das fórmulas de produto. Isso demonstrará benefícios consistentes da abordagem MPF.

No geral, os coeficientes estáticos aproximados ainda fornecem uma solução mais precisa do que a fórmula de produto com um número maior de passos de Trotter com a mesma quantidade de erro de Trotter no cenário sem ruído.

É também importante observar que, no exemplo que reproduz o experimento da Ref. [\[3\]](#references), o ponto de tempo $t=3$ está além do limite em que se espera que a PF com $k=2$ se comporte bem, que é $t/k>1$, conforme discutido neste [guia.](https://qiskit.github.io/qiskit-addon-mpf/how_tos/choose_trotter_steps.html)
## Referências
