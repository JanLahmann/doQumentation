...
Autentique-se usando sua [chave de API](https://quantum.cloud.ibm.com/) e selecione a Função Qiskit conforme descrito a seguir. (Este trecho assume que você já [salvou sua conta](/guides/functions-get-started#install-qiskit-functions-catalog-client) no seu ambiente local.)
...
No caso do Real Amplitudes Ansatz, você pode seguir uma relação linear entre o ``population_size`` e o número de qubits no circuito. Como regra geral aproximada, recomenda-se usar um **mínimo** de ``population_size ~ 0.8 * n_qubits`` para o ansatz `real_amplitudes`.
...
![Visualização da solução da otimização](/docs/images/guides/global-data-quantum-optimizer/cost_distribution.avif)
