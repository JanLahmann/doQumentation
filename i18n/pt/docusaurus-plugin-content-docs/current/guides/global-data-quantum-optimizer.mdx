...
notebook_path: "docs/guides/global-data-quantum-optimizer.ipynb"
---
...
<OpenInLabBanner notebookPath="docs/guides/global-data-quantum-optimizer.ipynb" />
...
** If ``multiple_passmanager`` is set to ``False``, the function uses the default Qiskit pass manager with `optimization_level=3`. If set to ``True``, the ``multiple_passmanager`` subroutine compares three pass managers: the previous default Qiskit pass manager, a pass manager mapping qubits over the QPU first neighbors chain, and the [AI transpiler](/guides/ai-transpiler-passes) services. Then, the pass manager with the estimated lower cumulative error is selected.
## `optimizer_settings`
...
See the [Job limits](/guides/job-limits) guide for more information.
</Admonition>
# Output
