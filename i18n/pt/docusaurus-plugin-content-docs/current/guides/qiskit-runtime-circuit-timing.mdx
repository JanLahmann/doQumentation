...
- Embora o tempo total do Circuit seja retornado nos metadados de "compilação", este NÃO é o tempo usado para cobrança (tempo de QPU).
</Admonition>
...
### Acessar os dados de timing do Circuit {#access-the-circuit-timing-data}

Quando solicitados, os dados de timing do Circuit para cada PUB são retornados nos metadados do resultado do job, em `["compilation"]["scheduler_timing"]["timing"]`. Este campo contém as informações brutas de timing. Para exibir as informações de timing, use a ferramenta de visualização integrada, conforme descrito na seção [Visualizar os timings](#visualize-timings).

Use o código a seguir para acessar os dados de timing do Circuit para o primeiro PUB:
...
Embora visualizar os dados de timing do Circuit usando o método `draw_circuit_schedule_timing` seja o caso de uso mais comum, pode ser útil entender a estrutura dos dados brutos de timing retornados. Isso pode ajudá-lo, por exemplo, a extrair informações programaticamente.
...
A imagem dos dados de timing do Circuit gerada por `draw_circuit_schedule_timing` transmite as seguintes informações:
...
Este exemplo mostra como habilitar a opção, obter as informações de timing do Circuit dos metadados e exibi-las como uma imagem.
...
Em seguida, obtenha o timing do agendamento do Circuit:
