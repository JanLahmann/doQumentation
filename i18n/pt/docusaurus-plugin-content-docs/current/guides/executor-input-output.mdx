...
O primeiro item contém os resultados da execução da primeira tarefa (um `CircuitItem`) no programa. Ele contém uma única chave, `meas`, que é o nome do registrador clássico no circuito de entrada. O valor dessa chave mapeia para um `np.ndarray` de forma `(parameter sets, shots, register bits)`, que é (10, 1024, 3) para o exemplo acima.
...
O segundo item contém os resultados da execução da segunda tarefa (um `SamplexItem`) no programa. Este item contém múltiplas chaves. A chave `meas`, que é o nome do registrador clássico do circuito de entrada, mapeia para a matriz de resultados desse registrador. Esta matriz tem a forma `(randomizations, parameter sets, shots, classical bits)`, ou (28, 10, 1024, 3) neste exemplo. Além disso, a saída contém uma chave `measurement_flips.meas`, que são as correções de bit-flip para desfazer o twirling de medição para o registrador `meas`.  Esta forma de saída será (28, 10, 1, 3) para o nosso exemplo, pois apenas um shot é necessário para realizar o bit-flip.
