...
Os itens de Circuit são executados sem qualquer tipo de randomização. Ao contrário, os itens samplex permitem que você especifique como randomizar seu conteúdo. A próxima célula usa a função `generate_boxing_pass_manager()` para agrupar os gates e medições do Circuit em caixas e adicionar uma anotação de twirling a cada caixa. Em seguida, gera um Circuit modelo e um par samplex usando a função `build()`.
