...
Por padrão, o arquivo `CMakeLists.txt` espera que o arquivo de cabeçalho do Slurm (`slurm.h`) esteja localizado em `/usr/include/slurm`, mas isso pode ser personalizado da seguinte forma.
...
```
Por exemplo, ao interagir com recursos quânticos por meio de um proxy HTTP, as variáveis de ambiente `http_proxy`, `https_proxy` e `no_proxy` são necessárias. Elas podem ser adicionadas da seguinte forma.
```bash
optional /usr/lib64/slurm/spank_qrmi.so /etc/slurm/qrmi_config.json \
