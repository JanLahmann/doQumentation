...
## Desenhar um circuito quântico {#draw-a-quantum-circuit}

A classe `QuantumCircuit` suporta o desenho de circuits pelo método `draw()`, ou imprimindo o objeto do circuito. Por padrão, ambos renderizam uma versão em arte ASCII do diagrama do circuito.
...
Desenhar um circuito de grande escala inline em um notebook Jupyter pode ser lento ou ilegível.
...
### Controlar os desenhos do circuito {#control-circuit-drawings}
...
Dependendo da saída, há também opções para personalizar o diagrama do circuito.
...
O argumento `fold` define uma largura máxima para a saída. No renderizador `text`, isso define o comprimento das linhas do diagrama antes de ele ser quebrado para a próxima linha. Ao usar o renderizador 'mpl', esse é o número de camadas (visuais) antes de quebrar para a próxima linha.
...
Se você tiver uma aplicação na qual prefere desenhar um circuito com uma função autônoma em vez de como método de um objeto Circuit, pode usar diretamente a função `circuit_drawer()`, que faz parte da interface pública estável de `qiskit.visualization`. A função se comporta de forma idêntica ao método `circuit.draw()`, exceto que recebe um objeto Circuit como argumento obrigatório.
...
<Admonition type="tip" title="Recomendações">
  -  Veja um exemplo de visualização de circuito no tutorial [Algoritmo de Grover](/tutorials/grovers-algorithm).
  -  Visualize circuits simples usando o [IBM Quantum Composer.](/guides/composer)
  -  [Visualize o timing do circuito.](/guides/visualize-circuit-timing)
