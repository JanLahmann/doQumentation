...
O método `EstimatorV2.run` aceita dois argumentos: uma lista de PUBs, cada um dos quais pode especificar um valor de precision específico para o PUB, e um argumento nomeado precision. Esses valores de precision fazem parte da interface de execução do Estimator e são independentes das opções do Runtime Estimator. Eles têm precedência sobre qualquer valor especificado como opção, em conformidade com a abstração do Estimator.

No entanto, se `precision` não for especificado por nenhum PUB nem no argumento nomeado run (ou se todos forem `None`), o valor de precision das opções é usado, especialmente `default_precision`.
...
2. Se o argumento nomeado precision for especificado em `run`, use esse valor.

3. Se `twirling` estiver habilitado (True por padrão), então o produto de `num_randomizations` e `shots_per_randomization`, conforme especificado nas [opções `twirling`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-twirling-options), é usado.
...
**Valores**: Valor no intervalo fornecido por `backend.rep_delay_range`
...
**Valores**: Número inteiro de segundos no intervalo [1, 10800]
...
**Valores**: list[int] de 2-10 valores no intervalo [0, 200]
...
**Valores**: `NoiseLearnerResult`, `Sequence[LayerError]`
...
**Valores**: `auto`, float no intervalo [0, 1]
...
**Valores**: `gate_folding`, `gate_folding_front`, `gate_folding_back`, `pea`
...
**Valores**: Um ou mais de: `exponential`, `linear`, `double_exponential`, `polynomial_degree_(1 <= k <= 7)`, `fallback`
...
**Valores**: Lista de floats; cada float >= 1
...
**Valores**: Lista de nomes de gates base para decompor
...
**Valores**: Lista de interações direcionadas de dois qubits
...
**Valores**: [Qiskit Aer NoiseModel](/guides/build-noise-models), ou sua representação
...
**Valores**: `active`, `active-circuit`, `active-accum`, `all`
...
<Accordion>
  ...
  - Não funciona com portas de emaranhamento não-Clifford.
  </AccordionItem>
