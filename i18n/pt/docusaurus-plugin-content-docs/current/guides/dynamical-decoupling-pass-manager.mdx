...
Obtenha as informações de `target` do `backend` e salve os nomes das operações como `basis_gates`, pois o `target` precisará ser modificado para adicionar informações de temporização para os Gates usados no desacoplamento dinâmico.
