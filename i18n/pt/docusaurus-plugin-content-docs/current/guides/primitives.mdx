...
Existem dois tipos de primitivas Qiskit: as classes base e suas implementações. As primitivas Estimator e Sampler são definidas por classes base de primitivas de código aberto que residem no SDK do Qiskit (no módulo [`qiskit.primitives`](https://docs.quantum.ibm.com/api/qiskit/primitives)). Provedores (como o IBM Quantum Compute Service) podem usar essas classes base para derivar suas próprias implementações de Sampler e Estimator. A maioria dos usuários interagirá com as implementações dos provedores, não com as primitivas base.
...
Por exemplo, consulte os tópicos [Opções do Estimator](/guides/estimator-options) e [Opções do Sampler](/guides/sampler-options) para saber sobre as opções das primitivas do IBM Quantum, ou veja as [referências de API do Qiskit Aer](https://qiskit.github.io/qiskit-aer/apidocs/aer_primitives.html) para as opções das primitivas do Qiskit Aer.
...
as capacidades avançadas de hardware de um determinado provedor. Por exemplo, com as primitivas do IBM Quantum,
...
- Saiba mais sobre as [primitivas do IBM Quantum](/guides/qiskit-runtime-primitives), que são usadas para executar Circuits em QPUs da IBM.
</Admonition>
