...
O código a seguir demonstra um exemplo de conjunto de entradas vetorizadas para a primitiva `Sampler` e as executa em um backend IBM&reg; como um único objeto `RuntimeJobV2 `.
...
Os resultados de jobs [`SamplerV2`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2) executados no IBM Quantum Compute Service contêm informações de temporização de execução em seus metadados.
