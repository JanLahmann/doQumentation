---
description: 'Como executar jobs de computação quântica no modo batch usando o IBM Quantum Compute Service.'
...
Antes de iniciar um batch, você deve [configurar o cliente do IBM Quantum Compute](/guides/install-qiskit) e inicializá-lo como um serviço:
...
Para uma visão geral abrangente da configuração e do status de um batch, incluindo seu TTL interativo e máximo, use o método `batch.details() method`.
...
<Admonition type="tip" title="Recomendações">
  ...
  - Entenda os [Limites de jobs](/guides/job-limits) ao enviar um job para um QPU IBM&reg;.
