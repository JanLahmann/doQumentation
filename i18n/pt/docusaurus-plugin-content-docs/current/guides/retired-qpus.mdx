---
title: "QPUs na nuvem aposentadas"
sidebar_label: "QPUs na nuvem aposentadas"
description: "Uma lista de unidades de processamento quântico (QPUs) da IBM que foram aposentadas"
...
As seguintes unidades de processamento quântico (QPUs) da IBM&reg; foram aposentadas. Para a lista completa de QPUs disponíveis, consulte a página de [Recursos de computação](https://quantum.cloud.ibm.com/computers).

<Admonition type="note">
Para recuperar jobs de uma QPU aposentada, veja [estas instruções](#retrieve).
</Admonition>

## Tabela de QPUs na nuvem aposentadas {#table-of-retired-cloud-qpus}
...
## Famílias de processadores aposentadas {#retired-processor-families}
...
## Recuperar um job de uma QPU aposentada {#retrieve-a-job-from-a-retired-qpu}
...
<Admonition type="note" title="Deprecated provider packages">
  `service.jobs()` retorna trabalhos executados tanto pelo `qiskit-ibm-runtime` quanto pelo pacote obsoleto `qiskit-ibm-provider`. Trabalhos enviados pelo pacote mais antigo (também obsoleto) `qiskit-ibmq-provider` não estão mais disponíveis.
</Admonition>
