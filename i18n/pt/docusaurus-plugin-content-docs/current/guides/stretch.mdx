...
{/* doqumentation-source-hash: 7154ffc1 */}
...
A especificação da linguagem OpenQASM 3 contém um tipo `stretch` com o qual você pode especificar a temporização relativa de operações em vez de uma temporização absoluta. O suporte a `stretch` como durações para instruções [`Delay`](https://docs.quantum.ibm.com/api/qiskit/circuit#qiskit.circuit.Delay) foi adicionado no [Qiskit v2.0.0](https://docs.quantum.ibm.com/api/qiskit/release-notes/2.0#200). O valor concreto de uma duração stretch é resolvido em tempo de compilação, após as durações exatas dos portas calibrados serem conhecidas. O compilador tenta minimizar a duração do stretch, sujeita a restrições de temporização em um ou mais qubits. Você pode então expressar designs de porta como espaçamento uniforme de portas (por exemplo, para implementar uma sequência de desacoplamento de eco de ordem superior), alinhamento à esquerda de uma sequência de portas, ou aplicação de um porta pela duração de algum sub-Circuit, sem precisar conhecer a temporização exata.
...
![Imagem ilustrando o circuito a seguir](/docs/images/guides/stretch/stretch.svg)

O circuito correspondente seria como o seguinte. Observe que um par de barreiras é necessário para definir os limites desta temporização relativa.
...
Este exemplo usa `stretch` para garantir que uma sequência de portas entre duas barreiras seja alinhada à esquerda, quaisquer que sejam suas durações reais:
...
:::note
 ...
 Ao usar `stretch` com o IBM Quantum Compute Service, qualquer resto resultante de uma resolução de stretch é adicionado ao primeiro delay que usa o stretch.
...
## Ver valores de stretch no IBM Quantum Compute Service {#view-stretch-values-in-ibm-quantum-compute-service}

O valor real de uma duração stretch é resolvido em tempo de compilação, após o Circuit ser escalonado. Ao executar um job do Sampler no IBM Quantum Compute Service, você pode visualizar os valores de stretch resolvidos nos metadados do resultado do job. O suporte a `stretch` no Quantum Compute é atualmente experimental, portanto você precisa primeiro definir uma opção experimental para habilitar sua recuperação e, em seguida, acessar os dados diretamente dos metadados da seguinte forma:
...
Embora o tempo total do Circuit seja retornado nos metadados de "compilation", este NÃO é o tempo usado para faturamento (tempo de QPU).
</Admonition>
...
Para detalhes completos, consulte o tópico [Visualizar temporização do Circuit](/guides/qiskit-runtime-circuit-timing).

<span id="qiskit-runtime-limitations"></span>

## Limitações do Quantum Compute {#quantum-compute-limitations}

O suporte a `stretch` no Quantum Compute é atualmente experimental e possui as seguintes restrições:
...
- A área delimitada por um conjunto de barreiras é chamada de _região de barreira_. Uma variável stretch não pode ser usada em múltiplas regiões de barreira.
    <Tabs>
        ...
        ![Ilustração da saída do código anterior](/docs/images/guides/stretch/bad-barrier-region.svg 'Invalid use of stretch in barrier regions')
     ...
     <TabItem value="Valid" label="Válido">
        ...
        ![Ilustração da saída do código anterior](/docs/images/guides/stretch/good-barrier-region.svg 'Valid use of stretch in a barrier region')
