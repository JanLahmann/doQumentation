---
description: 'Use o modo de teste local para simular hardware quântico com o IBM Quantum Compute'
notebook_path: guides/local-testing-mode.ipynb
sidebar_label: 'Modo de teste local'
title: 'Modo de teste local'
---
...
O primitivo Executor não suporta o modo de teste local.
</Admonition>

Para usar o modo de teste local, especifique um dos Backends falsos de ``qiskit_ibm_runtime.fake_provider`` ou especifique um Backend do Qiskit Aer ao instanciar uma primitiva do IBM Quantum ou uma Session.
...
- Você pode especificar todas as opções do IBM Quantum Compute no modo de teste local. No entanto, todas as opções, exceto shots, são ignoradas quando executadas em um simulador local.
...
- Use a [classe `Neat`](/guides/debug-qiskit-runtime-jobs) para analisar o impacto do ruído em uma carga de trabalho do Estimator.
- Revise exemplos detalhados do [Sampler](/guides/sampler-examples).
- Revise exemplos detalhados do [Estimator](/guides/estimator-examples).
