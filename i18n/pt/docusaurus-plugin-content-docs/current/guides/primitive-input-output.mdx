...
Ao invocar o método `run()` de um primitivo, o principal argumento necessário é uma `list` de uma ou mais tuplas — uma para cada Circuit sendo executado pelo primitivo. Cada uma dessas tuplas é considerada um PUB, e os elementos obrigatórios de cada tupla na lista dependem do primitivo utilizado. Os dados fornecidos a essas tuplas também podem ser organizados em diversas formas para oferecer flexibilidade em uma carga de trabalho por meio de broadcasting — cujas regras são descritas em uma [seção seguinte](#broadcasting-rules).
...
![Esta imagem ilustra várias representações visuais de broadcasting de arrays.](/docs/images/guides/primitive-input-output/broadcasting.avif "Representação visual de broadcasting")
