...
O backend simulado `FakeSherbrooke` do `qiskit_ibm_runtime` é usado nesses exemplos, mas você pode experimentar com qualquer backend real ou simulado compatível com Qiskit. Seus resultados podem ser diferentes.
</Admonition>

Primeiro, vamos transpilar usando o nível de otimização 0.
...
Ajustar o nível de otimização pode alterar outros aspectos do circuito também, não apenas o número de portas ECR. Para exemplos de como a definição do nível de otimização altera o layout, veja [Representando computadores quânticos](/guides/represent-quantum-computers).
