...
Em seguida, autentique-se usando sua [chave de API do IBM Quantum&reg;](http://quantum.cloud.ibm.com/) e, assumindo que você já [salvou sua conta](/guides/functions-get-started#install-qiskit-functions-catalog-client) no seu ambiente local, selecione a Qiskit Function da seguinte forma:
...
Verifique o [status](/guides/functions-get-started#check-job-status) do workload da sua Qiskit Function ou obtenha os [resultados](/guides/functions-get-started#retrieve-results) da seguinte forma:
...
Envie um e-mail para [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) para consultas sobre licenças.

### Exemplo de uso de função licenciada {#example-of-licensed-function-use}

Usuários licenciados recebem um token de licença e, em seguida, devem usar uma biblioteca wrapper para enviar seu token de licença para a função.
A biblioteca wrapper pode ser instalada [a partir do PyPI](https://pypi.org/project/hivqe-qiskit-function-utils/) com `pip install hivqe-qiskit-function-utils`.
O exemplo abaixo mostra como essa biblioteca deve ser usada para enviar seu token de licença ao chamar a função.
