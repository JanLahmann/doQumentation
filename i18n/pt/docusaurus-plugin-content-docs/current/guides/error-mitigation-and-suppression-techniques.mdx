---
description: 'Explicação das técnicas de mitigação e supressão de erros disponíveis no IBM Quantum Compute Service'
...
A célula de código a seguir mostra como habilitar o desacoplamento dinâmico para Estimator e escolher uma sequência de desacoplamento dinâmico.
...
![Representação do twirling de Pauli](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)
...
A célula de código a seguir mostra como habilitar o twirling de Pauli e definir essas opções para Estimator. Nenhuma dessas opções precisa ser definida explicitamente.
...
![Representação do twirling de medição](/docs/images/guides/error-mitigation-explanation/measurement_twirling.svg)

Na presença de erros de leitura, o twirling de medição tem o efeito de diagonalizar a matriz de transferência de erros de leitura, tornando-a fácil de inverter. Um termo de reescalonamento do canal de ruído diagonal é aprendido por
benchmarking de circuitos aleatórios inicializados no estado zero. Isso permite
que o serviço remova o viés dos valores esperados resultantes do
ruído de leitura. Estimar a matriz de transferência de erros de leitura requer a execução de circuitos de calibração adicionais, o que introduz uma pequena sobrecarga.

### Habilite o TREX usando a opção `measure_mitigation` {#enable-trex-by-using-the-measure-mitigation-option}

O TREX pode ser habilitado definindo `measure_mitigation` como `True` nas [opções de resiliência do IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) para o Estimator. As opções para aprendizado de ruído de medição são descritas [aqui](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-measure-noise-learning-options). Assim como no twirling de portas, você pode definir o número de randomizações de circuito e o número de shots por randomização.

A célula de código a seguir mostra como habilitar o TREX e definir essas opções para Estimator. Nenhuma dessas opções precisa ser definida explicitamente.
...
### Habilite o TREX usando a opção `resilience_level` {#enable-trex-by-using-the-resilience-level-option}

Você pode habilitar o TREX definindo o [nível de resiliência](/guides/estimator-noise-management#resilience) como `1`, conforme mostrado no exemplo a seguir.
...
A sobrecarga deste método escala com o número de fatores de ruído. As
configurações padrão amostram o valor esperado em três fatores de ruído,
resultando em uma sobrecarga de aproximadamente 3x.

### Habilite a ZNE usando a opção `zne_mitigation` {#enable-zne-by-using-the-zne-mitigation-option}

A ZNE pode ser habilitada definindo `zne_mitigation` como `True` nas [opções de resiliência do IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) para o Estimator.
As opções para ZNE são descritas [aqui](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options). As seguintes opções são notáveis:
...
A célula de código a seguir mostra como habilitar a ZNE e definir essas opções para Estimator. Nenhuma dessas opções precisa ser definida explicitamente.
...
### Habilite a ZNE usando a opção `resilience_level` {#enable-zne-by-using-the-resilience-level-option}

Você pode habilitar a ZNE (junto com TREX e twirling de portas) definindo o [nível de resiliência](/guides/estimator-noise-management#resilience) como `2`, conforme mostrado no exemplo a seguir.
...
O PEC pode ser habilitado definindo `pec_mitigation` como `True` nas [opções de resiliência do IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) para o Estimator.
As opções para PEC são descritas [aqui](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-pec-options). Um limite para a sobrecarga de amostragem pode ser definido usando a opção `max_overhead`. Observe que limitar a sobrecarga de amostragem pode fazer com que a precisão do resultado exceda a precisão solicitada. O valor padrão de `max_overhead` é 100.

A célula de código a seguir mostra como habilitar o PEC e definir a opção `max_overhead` para Estimator.
...
- [Configure o gerenciamento de ruído com Estimator](/guides/estimator-noise-management).

- [Configure o gerenciamento de ruído com Sampler](/guides/sampler-noise-management).

- Explore outras [opções](/guides/runtime-options-overview) para as primitivas do IBM Quantum.
