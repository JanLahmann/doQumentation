...
Esta página mostra como usar a classe [`Operator`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Operator). Para uma visão geral de alto nível das representações de operadores no Qiskit, incluindo a classe `Operator` e outras, consulte [Visão geral das classes de operadores](/guides/operators-overview).
...
Note que o último ponto significa que você pode usar a classe `Operator` como um simulador unitário para calcular a matriz unitária final de um circuito quântico, sem precisar chamar um Backend simulador. Se o circuito contiver operações não suportadas, uma exceção será levantada. As operações não suportadas são: measure, reset, operações condicionais, ou um porta que não possui uma definição de matriz ou decomposição em termos de gates com definições de matriz.
...
Por exemplo, para compor um porta de dois qubits com um operador de três qubits:
