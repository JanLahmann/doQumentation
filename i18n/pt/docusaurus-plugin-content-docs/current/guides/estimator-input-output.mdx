...
Os `parameter values` opcionais podem ser uma lista ou um único parâmetro. Os elementos de observables e parameter values são combinados seguindo as regras de broadcasting do NumPy, conforme descrito no tópico [Primitive inputs and outputs](primitive-input-output#broadcasting-rules), e uma estimativa de valor esperado é retornada para cada elemento da forma resultante do broadcasting.
...
O código a seguir demonstra um exemplo de conjunto de entradas vetorizadas para a primitiva `Estimator` e as executa em um backend IBM&reg; como um único objeto `RuntimeJobV2 `.
