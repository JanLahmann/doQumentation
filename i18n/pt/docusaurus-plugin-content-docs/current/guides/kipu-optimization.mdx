...
*Veja a [referência da API](https://docs.quantum.ibm.com/api/functions/kipu-optimization)*
...
## Exemplo de configuração personalizada {#custom-configuration-example}
...
**Otimização de seed**: Note que `seed_transpiler` está definido como `None` por padrão. Isso habilita o processo de otimização automática do transpilador. Quando `None`, o sistema iniciará uma tentativa com múltiplos seeds e selecionará aquele que produz a melhor profundidade de circuito, aproveitando ao máximo o parâmetro `max_trials` para cada nível de transpilação.
...
O exemplo a seguir executa a instância Maxcut com 150 nós.
...
- Visite a [referência da API](https://docs.quantum.ibm.com/api/functions/kipu-optimization) para esta Qiskit Function.
