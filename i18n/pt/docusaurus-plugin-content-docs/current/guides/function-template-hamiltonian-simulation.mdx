---
title: "Criar um template de Qiskit Function para simulação hamiltoniana"
sidebar_label: "Criar um template de Qiskit Function para simulação hamiltoniana"
description: "Como criar um programa de transpilação paralela e implantá-lo na IBM Quantum Platform para usar como serviço remoto reutilizável."
...
Este template encapsula um fluxo de trabalho para simular a evolução temporal de um estado inicial em relação a um Hamiltoniano baseado em spin definido pelo usuário e retorna um conjunto de valores esperados especificados usando o [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/) Qiskit addon.

Este template é estruturado como um padrão Qiskit com as seguintes etapas:

#### 1. Coleta de entradas e mapeamento do problema {#1-collecting-input-and-mapping-the-problem}
...
#### 2. Preparar os circuitos gerados para execução {#2-prepare-the-generated-circuits-for-execution}
...
## Escrever o template de função {#write-the-function-template}

Primeiro, escreva um template de função para simulação hamiltoniana que usa o [addon AQC-Tensor do Qiskit](https://qiskit.github.io/qiskit-addon-aqc-tensor/) para mapear a descrição do problema a um circuito de profundidade reduzida para execução em hardware.

Se você baixar esta página e visualizá-la localmente em um editor de notebooks, verá que algumas células de código contêm o [comando mágico](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Esse comando mágico salva o código em `./source_files/template_hamiltonian_simulation.py`, que é o template de função que você pode enviar e executar remotamente com o Qiskit Serverless.
...
### Coletar e validar as entradas {#collect-and-validate-the-inputs}
...
#### Validar as entradas {#validate-the-inputs}
...
#### Preparar as saídas da função {#prepare-the-function-outputs}
...
### Mapear o problema e pré-processar o circuito com AQC {#map-the-problem-and-pre-process-the-circuit-with-aqc}
...
### Otimizar o circuito final para execução {#optimize-the-final-circuit-for-execution}

Após a parte AQC do fluxo de trabalho, o `final_circuit` é [transpilado para o hardware](/guides/transpile#instruction-set-architecture) normalmente.
...
#### Sair antecipadamente se estiver usando o modo de execução a seco {#exit-early-if-using-dry-run-mode}
...
#### Executar o circuito no hardware {#execute-the-circuit-on-hardware}
...
#### Salvar a saída {#save-the-output}
...
## Implantar a função na IBM Quantum Platform {#deploy-the-function-to-ibm-quantum-platform}
...
## Executar o template de função remotamente {#run-the-function-template-remotely}
...
Para um mergulho mais profundo no addon AQC-Tensor do Qiskit, confira o tutorial [Evolução Temporal Trotterizada Melhorada com Compilação Quântica Aproximada](/tutorials/approximate-quantum-compilation-for-time-evolution) ou o [repositório qiskit-addon-aqc-tensor](https://github.com/Qiskit/qiskit-addon-aqc-tensor).
