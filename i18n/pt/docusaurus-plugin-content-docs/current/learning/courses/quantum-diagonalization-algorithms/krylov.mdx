...
Um __método de subespaço de Krylov__ pode se referir a qualquer um dos vários métodos construídos em torno do que é chamado de __subespaço de Krylov__. Uma revisão completa deles está além do escopo desta lição, mas as Refs [\[2-4\]](#references) podem oferecer muito mais contexto. Aqui, vamos nos concentrar no que é um subespaço de Krylov, como e por que ele é útil na resolução de problemas de autovalores e, finalmente, como pode ser implementado em um computador quântico.
__Definição:__ Dada uma matriz simétrica e semidefinida positiva $N\times N$ $A$, o espaço de Krylov $\mathcal{K}^r$ de ordem $r$ é o espaço gerado por vetores obtidos multiplicando potências mais altas de uma matriz $A$, até $r-1\leq N$, com um vetor de referência $\vert v \rangle$.
...
(a) Como estamos ortonormalizando os vetores à medida que os produzimos, um conjunto de $N$ desses vetores formará uma base completa, o que significa que uma combinação linear deles pode ser usada para criar qualquer vetor no espaço.

(b) O processo de ortogonalização consiste em subtrair a projeção de um novo vetor em todos os vetores anteriores. Se todos os vetores anteriores abrangem todo o espaço vetorial, então subtrair as projeções sobre o subespaço completo sempre nos deixará com um vetor zero.
...
Seu colega escolheu acidentalmente um autovetor para seu vetor inicial. Atuar com a matriz sobre o vetor inicial simplesmente retornará o mesmo vetor, escalado pelo autovalor. Isso não gerará um subespaço de dimensão crescente. Aconselhe seu colega a selecionar um vetor inicial diferente, garantindo que não seja um autovetor.
...
### Teste seu entendimento {#check-your-understanding-1}
...
Consulte o gráfico suavizado dos tempos de diagonalização versus a dimensão da matriz acima.

(a) Em aproximadamente qual dimensão de matriz o método de Krylov se tornou mais rápido, de acordo com esse gráfico?
...
Aqui, potências de $U$ se tornam diferentes passos de tempo (a $k^\text{th}$ potência de $U$ avança um tempo $k \times dt$). Podemos rotular o elemento do subespaço que é evoluído no tempo para um tempo total $k dt$ como $|\psi_k\rangle$.
$$
\begin{aligned}
U&=e^{-iHdt}\\
U^k&=e^{-iH(kdt)}\\
...
$$
Aqui $P_\alpha$ é uma string de Pauli da forma $P_\alpha \sim IZIXII...YZXIX$ ou um conjunto de tais strings de Pauli que comutam entre si. Dado que podemos escrever $H$ como tal soma de operadores mensuráveis, as seguintes expressões para os elementos de matriz de $\tilde{H}$ podem ser realizadas usando o primitivo Estimator do IBM Quantum.
...
### 4.2 Passo 2. Otimizar circuitos e operadores para o hardware alvo {#4-2-step-2-optimize-circuits-and-operators-for-target-hardware}
...
### 4.3 Etapa 3. Executar usando uma primitiva do IBM Quantum {#43-step-3-execute-using-an-ibm-quantum-primitive}
