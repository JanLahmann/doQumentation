...
### 1.1 O operador (Hamiltoniano) {#11-the-operator-hamiltonian}
...
### 1.5 O princípio variacional {#15-the-variational-principle}
...
## 2. Comparação com o fluxo de trabalho clássico {#2-comparison-with-classical-workflow}
...
### 2.1 Fluxo de trabalho clássico {#21-classical-workflow}
...
### 2.2 Fluxo de trabalho quântico e grupos de Paulis comutantes {#22-quantum-workflow-and-commuting-pauli-groups}
...
### 2.3 Fatores que determinam o custo computacional {#23-factors-that-determine-computational-cost}
...
### Teste seus conhecimentos {#check-your-understanding-1}
...
__Quantum Selected Configuration Interaction (QSCI)__[\[13\]](#references) é um algoritmo que produz um estado fundamental aproximado de um hamiltoniano ao amostrar uma função de onda tentativa para identificar os estados de base computacional significativos e gerar um subespaço para uma diagonalização clássica.
Tanto o SQD quanto o QSCI usam um computador quântico para construir um subespaço reduzido. O ponto forte adicional do QSCI está em sua preparação de estado, especialmente no contexto de problemas de química. Ele utiliza várias estratégias, como o uso de estados evoluídos no tempo [\[14\]](#references) e um conjunto de ansätze inspirados na química. Ao focar em uma preparação de estado eficiente, o QSCI reduz os custos computacionais quânticos para hamiltonianos químicos, mantendo alta fidelidade e aproveitando a robustez ao ruído das técnicas de amostragem de estado quântico [\[15\]](#references). O QSCI também oferece uma técnica de construção adaptativa que fornece mais ansätze para um resultado melhor.
