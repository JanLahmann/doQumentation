...
Os limites do que é possível na computação quântica estão mudando a cada dia. Mas é importante ter em mente as restrições atuais ao delinear seu projeto. Para informações detalhadas sobre cada computador quântico, e até mesmo sobre cada qubit, confira a [página de recursos de computação](https://quantum.cloud.ibm.com/computers) na [IBM Quantum Platform](https://quantum.cloud.ibm.com/). 
...
**Contagem de qubits** - Os processadores IBM Nighthawk têm 120 qubits. Alguns sistemas têm um pouco mais. Esses sistemas oferecem pesquisa em escala utilitária para novas descobertas que não são classicamente acessíveis.  
**Profundidade do circuito** - A profundidade máxima do circuito depende de muitos fatores. Certifique-se de considerar a profundidade transpilada de portas de dois qubits como a medida principal de profundidade. Profundidades transpiladas de dois qubits em torno de 30 costumam ser administráveis com técnicas modernas de supressão e mitigação de erros. Algumas aplicações de nicho podem encontrar dificuldades em profundidades menores, e alguns circuitos certamente podem ir além disso. Essa é uma boa profundidade para explorar.  
**Tempo de QPU** - Isso depende inteiramente da sua aplicação. Estimamos que um mínimo de 400 minutos é necessário para pesquisas inovadoras em computação quântica. Você também pode verificar o tempo de QPU necessário para execuções individuais dos projetos listados no advantage tracker. A maioria fica entre 30 e 120 minutos. Quando permitimos experimentação, benchmarking do seu problema e múltiplas tentativas, esse intervalo de tempo é consistente com o mínimo mencionado acima.  
