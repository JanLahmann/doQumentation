...
## 3. Otimização Quântica com padrões do Qiskit {#3-quantum-optimization-with-qiskit-patterns}
...
### 3.1 Padrão Qiskit (pequena escala) para otimização {#31-small-scale-qiskit-pattern-for-optimization}
...
### 3.2 Passo 1. Mapear entradas clássicas para um problema quântico {#32-step-1-map-classical-inputs-to-a-quantum-problem}
...
### 3.3 Passo 2. Otimizar circuitos para execução em hardware quântico {#33-step-2-optimize-circuits-for-quantum-hardware-execution}
...
### 3.4 Passo 3. Executar usando primitivas do IBM Quantum {#34-step-3-execute-using-ibm-quantum-primitives}
...
Use as **primitivas do IBM Quantum** para interagir com backends da IBM&reg;. As duas primitivas são Sampler e Estimator, e a escolha da primitiva depende do tipo de medição que você deseja executar no computador quântico. Para a minimização de $H_C$, use o Estimator, pois a medição da função de custo é simplesmente o valor esperado de $\langle H_C \rangle$.
...
### 3.5 Passo 4. Pós-processar e retornar o resultado em formato clássico {#35-step-4-post-process-return-result-in-classical-format}
