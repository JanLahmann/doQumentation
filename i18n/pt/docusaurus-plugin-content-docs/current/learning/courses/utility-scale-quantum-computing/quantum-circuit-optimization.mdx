...
<OpenInLabBanner notebookPath="learning/courses/utility-scale-quantum-computing/quantum-circuit-optimization.ipynb" />
 ...
 # Otimização de circuito quântico
...
`optimization_level` tem quatro opções, de 0 a 3. Quanto maior o nível de otimização, mais esforço computacional é gasto para otimizar o circuito. O nível 0 não realiza nenhuma otimização e apenas faz a quantidade mínima de trabalho para tornar o circuito executável no backend selecionado. O nível 3 gasta a maior quantidade de esforço (e normalmente de tempo de execução) para tentar otimizar o circuito. O nível 1 é o nível de otimização padrão.
Transpilamos o circuito sem otimização (`optimization_level=0`) e com otimização (`optimization_level=2`).
Vemos uma grande diferença no comprimento dos circuitos transpilados.
...
A seguir, comparamos os resultados da execução de dois circuitos de preparação de estado GHZ de 5 qubits sintetizados de formas diferentes ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$).
...
O que fizemos no novo circuito pode ser visto como uma síntese de circuito manual "com reconhecimento de restrição de acoplamento". Em outras palavras: resolver manualmente a síntese de circuito e o mapeamento de circuito ao mesmo tempo.
...
Em geral, a síntese de circuito depende da aplicação e é difícil demais para um software cobrir todas as aplicações possíveis. Acontece que o transpiler do Qiskit não tem funções para sintetizar o circuito de preparação de estado GHZ. Nesse caso, vale a pena considerar a síntese manual de circuito, como mostrado acima.
Nesta seção, examinamos os detalhes de como o transpiler do Qiskit funciona usando o seguinte circuito de exemplo simples.
