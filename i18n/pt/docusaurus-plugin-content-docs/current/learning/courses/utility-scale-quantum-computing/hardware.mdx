---
description: 'Esta aula explora o hardware de computação quântica moderno. É baseada num curso presencial ministrado na Universidade de Tóquio.'
...
O PDF desta aula foi dividido em duas partes. [Baixe a parte 1](https://ibm.ent.box.com/public/static/ruz8wf353hncenmaywjlfjilflaumnzt.zip) e [baixe a parte 2](https://ibm.ent.box.com/public/static/tg8vv00ern2bmxmm033xt9oe0fcvwamc.zip). Observe que alguns trechos de código podem ficar desatualizados, pois são imagens estáticas.
...
O Qiskit fornece uma API para obter informações, tanto estáticas quanto dinâmicas, sobre um dispositivo quântico. Usamos uma instância de Backend para fazer interface com um dispositivo, que inclui uma instância de Target, um modelo de máquina abstrato que resume os recursos pertinentes, como sua arquitetura de conjunto de instruções (ISA) e quaisquer propriedades ou restrições associadas a ela.
Vamos usar essas instâncias de backend para obter algumas das informações que você vê na página [Recursos de computação](https://quantum.cloud.ibm.com/computers)
na Plataforma IBM Quantum&reg;. Primeiro, criamos uma instância de backend para um dispositivo de interesse. A seguir, escolhemos "ibm_kyoto", "ibm_kawasaki" ou a máquina Eagle menos ocupada. Seu acesso a QPUs pode ser diferente; atualize o nome do backend adequadamente.
...
Tente obter as informações básicas sobre um dispositivo Heron, "ibm_strasbourg". Tente isso por conta própria, mas o código foi adicionado abaixo para você se verificar.
...
### 2.2 Mapa de acoplamento {#22-coupling-map}
...
![Saída da célula de código anterior](/learning/images/courses/utility-scale-quantum-computing/hardware/extracted-outputs/51b87458-a5e8-4e77-a34d-39fe425a5f01-0.avif)

## 3. Propriedades dos qubits {#3-qubit-properties}
...
Vamos calcular a mediana dos tempos T1 dos qubits. Compare o resultado com o exibido para o dispositivo na [IBM Quantum Platform.](https://quantum.cloud.ibm.com/)
...
###  3.2 Erros de porta e de leitura {#32-gate-and-readout-errors}
...
Seus valores também são dicionários. Vamos observar alguns dos itens do valor (dicionário) para a operação 'sx'.
...
Tendo entendido as estruturas de dados, estamos prontos para calcular os erros medianos das portas 'sx' e 'ecr'. Novamente, compare os resultados com os mostrados para o dispositivo na [Plataforma IBM Quantum](https://quantum.cloud.ibm.com/).
...
![Saída da célula de código anterior](/learning/images/courses/utility-scale-quantum-computing/hardware/extracted-outputs/97fead46-0.avif)
...
![Saída da célula de código anterior](/learning/images/courses/utility-scale-quantum-computing/hardware/extracted-outputs/c9e05530-0.avif)
