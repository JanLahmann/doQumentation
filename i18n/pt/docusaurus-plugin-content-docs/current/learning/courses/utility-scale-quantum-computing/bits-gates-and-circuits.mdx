...
$|0\rangle$ e $|1\rangle$ são vetores no espaço vetorial complexo bidimensional:
...
Portanto, um estado quântico arbitrário também é representado como
...
A partir disso, podemos ver que o estado de um bit quântico é um vetor unitário em um espaço de produto interno complexo bidimensional com uma base ortonormal de $|0\rangle$ e $|1\rangle$. Ele é normalizado para 1.
...
Um estado quântico de um único qubit também é representado como
...
onde $\theta$ e $\varphi$ são os ângulos da esfera de Bloch na figura a seguir.
...
Vamos primeiro simular a ação dessa porta de dois qubits quando q0 e q1 são ambos $|0\rangle$, e obter o vetor de estado de saída. A sintaxe do Qiskit usada é ```qc.cx(control qubit, target qubit)```.
...
Em um programa quântico, os circuitos quânticos são o formato nativo para representar instruções quânticas. Ao criar um circuito, você normalmente criará um novo objeto QuantumCircuit, depois adicionará instruções a ele em sequência.
...
Se você não tem o `qiskit-ibm-runtime`, precisa instalá-lo primeiro. Para mais informações, [consulte a referência da API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-service).
...
A execução no dispositivo real pode exigir espera em uma fila, já que computadores quânticos são recursos valiosos e muito procurados. O job_id é usado para verificar posteriormente o status de execução e os resultados do job.
