...
Você pode definir as opções de supressão de erros no Sampler V2. Consulte o guia [Gerenciamento de ruído do Sampler](/guides/sampler-noise-management) e a referência da API [`ExecutionOptionsV2`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-execution-options-v2) para mais informações.
...
Vimos três ideias até agora. Você pode combiná-las e expandi-las, ou criar suas próprias ideias para construir um circuito GHZ melhor. Agora vamos revisar o objetivo novamente.

## 5. Seu objetivo (recapitulação) {#5-your-goal-recap}

Construa um circuito GHZ com 20 qubits ou mais de forma que o resultado da medição atenda aos critérios: a fidelidade do seu estado GHZ deve ser maior que 0,5.

- Você precisa usar um dispositivo Eagle (como `ibm_brisbane`) e definir o número de shots como 40.000.

- Você deve executar o circuito GHZ usando a função `execute_ghz_fidelity` e calcular a fidelidade usando a função `check_ghz_fidelity_from_jobs`.
Você precisa encontrar o maior número de qubits - circuito GHZ que atenda aos critérios. Escreva seu código abaixo, mostre o resultado com a função `check_ghz_fidelity_from_jobs`.
