...
A IBM introduziu uma nova arquitetura de referência para supercomputação quântico-cêntrica (QCSC) que descreve como os processadores quânticos (QPUs) podem se integrar diretamente com CPUs e GPUs em ambientes modernos de HPC. A abordagem é impulsionada por fluxos de trabalho híbridos quântico-clássicos bem-sucedidos&mdash;como o uso do método de diagonalização quântica baseada em amostras (SQD) para simulações moleculares&mdash;que já demonstram precisão comparável a técnicas clássicas avançadas.

A arquitetura é projetada para ser aberta, modular e combinável, de modo que os recursos quânticos possam se integrar aos fluxos de trabalho de HPC existentes sem exigir uma pilha completamente nova. As primeiras integrações com sistemas como RIKEN e Fugaku demonstram que fluxos de trabalho híbridos em nível de produção já são viáveis. Ela fornece uma estrutura de longo prazo para a evolução do co-design quântico-clássico ao longo da próxima década.

![\"Imagem mostrando a arquitetura completa de quântico mais HPC.\"](/learning/images/courses/integrating-quantum-and-high-performance-computing/introduction/qcsc-arch.avif)

A pilha inclui uma camada de aplicação que decompõe problemas entre componentes clássicos e quânticos, um middleware que conecta modelos de programação clássica com a geração de circuitos quânticos (por exemplo, por meio das interfaces atualizadas do Qiskit), e uma camada de orquestração de sistemas que gerencia o agendamento de recursos híbridos usando ferramentas como a Interface de Gerenciamento de Recursos Quânticos (QRMI).

No nível de hardware, a arquitetura define três camadas:
* Sistema quântico com QPUs e runtimes clássicos de baixa latência (FPGAs/ASICs).
* Sistemas de scale-up com CPU/GPU co-localizados conectados via interconexões de tempo próximo para tarefas como mitigação de erros.
* Sistemas de scale-out que fornecem computação clássica flexível para pré-processamento, pós-processamento e cargas de trabalho híbridas.

No geral, a IBM posiciona a arquitetura QCSC como um roteiro prático para que os centros de HPC comecem a incorporar hardware quântico real, possibilitando algoritmos híbridos mais poderosos e preparando a infraestrutura para futuros sistemas quânticos tolerantes a falhas. Para saber mais sobre essa arquitetura, consulte [este artigo](https://research.ibm.com/blog/quantum-centric-supercomputing-system-reference-architecture).
...
* [Execute seu primeiro circuito no hardware](/guides/hello-world): Um breve tutorial sobre Qiskit no qual você configura e executa seu primeiro programa quântico.
