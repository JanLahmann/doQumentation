...
Quantos qubits há no circuito abaixo? ![Um diagrama de circuito com quatro linhas horizontais e muitas portas.](/learning/images/courses/integrating-quantum-and-high-performance-computing/programming-models/checkin-circuit.avif)
...
Suponha que você está modelando os elétrons em uma molécula. Você quer aproximar (a) a energia do estado fundamental da molécula, e (b) quais estados de base computacional são mais dominantes no estado fundamental da molécula. Em cada caso, você usaria o primitivo Estimator ou Sampler?
...
* O fluxo de controle principal do programa é ditado pelo rank do processo. Este é o princípio SPMD em ação: o programa usa lógica condicional (por exemplo, if (rank == 0)) para garantir que apenas certas seções paralelizadas do código sejam executadas pelos processos de trabalho, enquanto um processo mestre (geralmente o Rank 0) cuida da inicialização e da agregação final.
