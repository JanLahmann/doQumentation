...
Nossa suposição é que $\mathsf{Y}$ é *clássico* — ou seja, independentemente do estado com que começamos para $\mathsf{X},$ o estado de $\mathsf{Y}$ que obtemos é representado por uma matriz de densidade diagonal.
...
E agora, uma forma simples de expressar que uma dada matriz de densidade $\sigma$ é diagonal é pela equação
$\sigma = \Delta_m(\sigma).$
Em palavras, zerar todas as entradas fora da diagonal de uma matriz de densidade não tem efeito se e somente se as entradas fora da diagonal já eram todas zero.
...
para toda matriz de densidade $\rho$ representando um estado de $\mathsf{X}.$
...
para toda matriz de densidade $\rho.$
...
Suponha que o estado de $(\mathsf{X},\mathsf{Z})$ antes da medição é descrito por uma matriz de densidade $\rho,$ que podemos escrever da seguinte forma.
...
Note que isso é uma matriz de densidade em virtude do fato de que $\Phi$ é um canal, de modo que cada matriz
...
Ou seja, essa é a matriz de densidade que obtemos normalizando
...
2. Condicionado ao resultado $a,$ o estado de $\mathsf{Z}$ é então representado pela matriz de densidade $\sigma_a$ mostrada na equação $(2),$ que é obtida normalizando $\operatorname{Tr}_{\mathsf{X}}\bigl((P_a \otimes \mathbb{I}_{\mathsf{Z}}) \rho).$
...
2. Condicionado ao resultado $a,$ o estado de $(\mathsf{X}_1,\ldots,\mathsf{X}_{k-1},\mathsf{X}_{k+1},\ldots,\mathsf{X}_r)$ é então representado pela seguinte matriz de densidade.
   $$
