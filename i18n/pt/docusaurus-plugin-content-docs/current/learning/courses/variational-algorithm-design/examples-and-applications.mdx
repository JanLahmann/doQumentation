...
Usaremos a seguinte função para calcular a penalidade de sobreposição. Observe que isso ainda faz parte do mapeamento do problema para circuitos quânticos. No entanto, como discutido na lição anterior, essa função calcula a sobreposição entre um circuito variacional atual e o circuito otimizado a partir de um estado anterior, de menor energia/custo, obtido. O novo circuito sendo gerado também precisa ser transpilado para execução em hardware real. Já vimos essa função antes, usada em um simulador. Aqui, já devemos considerar a transpilação e a otimização relacionada para quando usamos um backend real, daí as linhas em torno de `if realbackend == 1`. Isso mistura um pouco da etapa 2, mas chamaremos a etapa 2 explicitamente mais adiante.
...
Agora adicionamos a função de custo do VQD. Observe que, em comparação com a lição anterior, agora temos dois argumentos adicionais (`realbackend` e `backend`) para nos ajudar com a transpilação quando usamos backends reais.
...
Mais uma vez, usaremos simuladores para depuração e depois passaremos para o hardware real.
...
Agora testaremos o algoritmo usando simuladores:
...
Esses resultados são bastante próximos dos esperados, exceto pelo erro de aproximação e pela fase global. Poderíamos ajustar a tolerância no otimizador clássico e/ou as penalidades para a sobreposição do vetor de estado para obter valores mais precisos.
...
Qualquer otimizador clássico pode ser usado para minimizar a função de custo. Em um sistema quântico real, um otimizador projetado para paisagens de função de custo não suaves geralmente tem um desempenho melhor. Aqui usamos a [rotina COBYLA](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-cobyla.html#optimize-minimize-cobyla) do SciPy via a função minimize.

Como estamos executando iterativamente muitas chamadas ao Runtime, utilizamos uma sessão para executar todas as chamadas dentro de um único bloco. Além disso, para o QAOA, a solução é codificada na distribuição de saída do circuito ansatz vinculado aos parâmetros ótimos da minimização. Portanto, precisaremos de uma primitiva Sampler e a instanciaremos com a mesma sessão
E executamos nossa rotina de minimização:
...
O vetor solução de ângulos de parâmetros (`x`), quando inserido no circuito ansatz, produz o particionamento de grafo que estávamos procurando.
...
Prossiga para a última lição para fazer sua avaliação e ganhar seu selo!
