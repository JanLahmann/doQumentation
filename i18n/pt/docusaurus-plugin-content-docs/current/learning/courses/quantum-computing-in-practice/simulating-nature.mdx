...
Ao simular partículas indistinguíveis, como férmions ou bósons, é necessário aplicar uma transformação de codificação. Essas partículas são descritas em um arcabouço matemático especial chamado `second quantization` (segunda quantização), que rastreia o número de ocupação de cada estado quântico introduzindo operadores de `creation` (criação) e `annihilation` (aniquilação), em que o operador de criação $\hat{a}_i^\dagger$ adiciona uma partícula ao estado $i$ enquanto o operador de aniquilação $\hat{a}_i$ remove uma partícula do estado $i$. Com base nesse arcabouço de segunda quantização, o férmion pode ser transformado por Bravyi-Kitaev e Jordan-Wigner. A transformação de Jordan-Wigner define o operador fermiônico de criação
$\hat{f}_j^\dagger = \Bigl( \prod\limits_{k \lt j}{(-Z_k)} \Bigr)\Bigl( \frac{X_j + i Y_j}{2} \Bigr)$ que preenche o $j$-ésimo estado quântico com um férmion, e o operador fermiônico de aniquilação $\hat{f}_j= \Bigl( \prod\limits_{k \lt j}{(-Z_k)} \Bigr)\Bigl( \frac{X_j - i Y_j}{2} \Bigr)$ que esvazia um férmion do $j$-ésimo estado. Você pode encontrar mais detalhes dessa transformação de Jordan-Wigner em nosso [Quantum Computing in Practice, episódio 5 - Mapping](/learning/courses/quantum-computing-in-practice/mapping). De forma semelhante, os bósons também exigem seus próprios métodos de codificação, como a transformação de Holstein-Primakoff, para serem representados por qubits.
