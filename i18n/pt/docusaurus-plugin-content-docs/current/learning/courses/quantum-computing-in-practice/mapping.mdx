...
1.	O que nossos qubits vão representar em nosso modelo?
...
3.	Por fim, a topologia do problema se alinha com a topologia do hardware? Se não, talvez precisemos implementar alguns truques para fazer isso funcionar.
...
Se isso parece invertido para você (particularmente o sinal entre $X$ e $iY$), veja a nota opcional abaixo sobre convenções quanto a qual estado $|0\rangle$ ou $|1\rangle$ corresponde a "ocupado" e qual a "desocupado".

<Accordion>
<AccordionItem title="Notas opcionais sobre convenções">

Em muitos contextos da física não há ambiguidade sobre a interpretação dos operadores $\sigma^\pm$; eles aumentam/diminuem a projeção z do momento angular de spin de um sistema. $\sigma^+$, por exemplo, leva um estado de momento angular de spin $|m_s\rangle$ a um com $|m_s+1\rangle$. No entanto, ao aplicar um mapper para expressar operadores fermiônicos de criação e aniquilação usando esses operadores de momento angular (combinações de operadores de Pauli), o aumento/diminuição agora corresponde à ocupação de um orbital eletrônico. "Elevar" e "abaixar" não têm ambiguidade se a ação é aumentar ou diminuir a projeção do momento angular em um determinado eixo, e o rótulo correspondente. Na ocupação de orbitais eletrônicos, pode-se associar $|0\rangle$ a vazio e $|1\rangle$ a ocupado. Isso pode parecer a convenção óbvia, e de fato é a usada nos addons do Qiskit. Mas observe que é possível escolher $|0\rangle$ como "ocupado" no sentido de um elétron relaxando para o estado fundamental de um sistema, e $|1\rangle$ como "desocupado". É uma escolha livre, e uma potencial ambiguidade na rotulagem: qual estado corresponde a um orbital estar ocupado? $|0\rangle$ ou $|1\rangle$?

Agora é de certa forma padronizado em um contexto de computação quântica, e em mappers integrados como o addon do Qiskit `FermionicMapper`, definir
$$
\begin{aligned}
|0\rangle & = \text{ empty}\\
...
De forma equivalente, um operador que cria um elétron ocupando o orbital $k$ (representado pelo qubit $q_k$) deve se comportar como:

$$
O_k |q_N,...,q_k=0,...,q_0\rangle = c |q_N,...,q_k=1,...,q_0\rangle
$$

Agora vamos ver as consequências dessa escolha. Observe que nas bases padrão usadas em computação quântica,
...
e por trabalho semelhante, podemos construir o conjunto completo:
...
A partir disso, fica claro que, ao escolher a convenção aparentemente razoável de que $|0\rangle$ corresponde a nenhum elétron, e $|1\rangle$ corresponde a um elétron, acabamos com a situação confusa de que $\sigma^+$ aparece no operador de aniquilação, e $\sigma^-$ no operador de criação. Assim, a convenção em `Qiskit Fermions` (de que $|0\rangle$ corresponde a ocupado, e $|1\rangle$ corresponde a desocupado) resulta no operador de criação fermiônico $f_k^\dagger$ contendo $\sigma_k^-$, e o operador de aniquilação $f_k$ contendo $\sigma_k^+$.

Se você encontrar fontes com os sinais opostos entre os termos $X$ e $iY$ nos operadores de criação e aniquilação, elas provavelmente estão usando a convenção oposta sobre qual estado corresponde a um orbital estar preenchido.
...
2.	Seguindo a definição dos operadores de criação e destruição mostrada anteriormente, use a função `from_list` do SparsePauliOp para definir os operadores de Pauli $X$, $Y$, $Z$. Isso mapeará os operadores fermiônicos de criação e aniquilação para operadores de spin de qubit, codificando o número de ocupação fermiônico na base computacional dos qubits.

3.	Gere o Hamiltoniano desejado na base de Pauli aplicando os operadores acima aos orbitais de interesse. Isso geralmente corresponde à criação de uma matriz identidade que representa os orbitais centrais (não interagentes) e, em seguida, aplicando os operadores de criação e aniquilação ao espaço ativo, com pesos que correspondem às especificidades do problema em questão.
...
onde $\sigma^+$ é o operador de Pauli $X + iY$ e $\sigma^{-}$ é $X - iY$. Uma vez que temos um Hamiltoniano escrito nesse formato, a parte difícil da etapa de mapeamento acabou, e agora ele pode ser facilmente escrito em um circuito em operadores de Pauli.
