...
Para escrever o problema QUBO como o Hamiltoniano de Ising, tudo que precisamos fazer é uma simples mudança de variáveis de $x \in \{0, 1\}^n$ para $z \in \{-1, 1\}^n$:
...
Primeiro, observe que podemos reescrever nossa expressão QUBO como a soma de termos matriciais:

$$
x^TQx = \sum_{ij}Q_{ij}x_ix_j
$$

Aplicando a mudança de variáveis, obtemos
...
Podemos descartar o termo constante porque isso não afetará qual $z$ minimiza o problema QUBO. Além disso, podemos multiplicar essa expressão por quatro para recuperar o valor original de $Q$, já que isso também não afeta a escolha ótima de $z$.

$$
\sum_{ij}Q_{ij}z_iz_j - Q_{ij}z_i - Q_{ij}z_j = z^TQz + (-\sum_{ij} Q_{ij}z_i + Q_{ij}z_j)
$$

Agrupando termos lineares semelhantes, podemos definir nosso vetor de coeficiente linear $b$ usando a equação

$$
b_i = -\sum_{j} Q_{ij} + Q_{ji}
$$

que completa a transformação de variáveis para $z \in \{-1, 1\}^n$ após a substituição na equação anterior.
...
No final, a minimização da expressão QUBO é a mesma que a minimização desta expressão, onde $b$ é um coeficiente escalar real:

$$
\min_{x\in\{0,1\}^n} x^TQx\Longleftrightarrow \min_{z\in\{-1,1\}^n}z^TQz + b^Tz
$$

Reescrevendo novamente um pouco e temos nosso Hamiltoniano de função de custo, onde o mínimo da expressão representa o estado fundamental e `Z` é o operador Z de Pauli:

$$
H_C=\sum_{ij}Q_{ij}Z_iZ_j + \sum_i b_i Z_i
$$

Agora que temos nosso Hamiltoniano, precisamos reescrevê-lo em termos de operadores ZZ de Pauli dois-local (de dois corpos), que podemos facilmente converter em portas de dois qubits no nosso circuito quântico. Acabaremos com seis objetos — ou strings de Pauli — onde cada um corresponde a cada uma das seis arestas no grafo. Cada um dos cinco elementos em uma string representa uma operação em um nó — a identidade se o nó não estiver conectado a essa aresta em particular, e o operador Z de Pauli se estiver. No Qiskit, as bitstrings que representam qubits são indexadas ao contrário. Por exemplo, uma aresta entre os nós 0 e 1 é codificada como `IIIZZ`, e uma aresta entre 2 e 4 é codificada como `ZIZII`.
...
1.	o estado de teste inicial, em cinza, que é o estado fundamental do misturador, criado ao aplicar uma porta Hadamard a cada qubit

2.	a evolução da função de custo, que discutimos anteriormente, em roxo escuro

3.	a evolução sob o Hamiltoniano do misturador, que ainda não cobrimos, em roxo claro.
...
Cada iteração desses operadores é chamada de camada. Essas camadas podem ser vistas como discretização da evolução temporal do sistema, como descrito anteriormente. O padrão alternado vem da decomposição de Trotter e aproxima as funções exponenciais de matrizes que não comutam. Em geral, quanto mais camadas ou passos incluirmos, mais próximos estaremos da evolução contínua do tempo, como no QAA — portanto, em teoria, mais preciso será o resultado. Mas para este exemplo, começaremos apenas amostrando com uma camada. Lembre-se, tanto o Hamiltoniano de função de custo quanto o misturador são parametrizados; ainda precisamos encontrar valores ótimos para $\gamma$ e $\beta$.
...
A biblioteca Qiskit oferece uma série de passes de transpilação que atendem a uma ampla gama de transformações de circuito. Queremos ter certeza de que o circuito está otimizado para nosso propósito.
...
Até agora, transpilamos o circuito deixando os parâmetros gamma e beta intactos — mas na verdade não podemos executar o circuito sem especificar esses parâmetros. No fluxo de trabalho do QAOA, os parâmetros ótimos do QAOA são encontrados em um loop de otimização iterativo, no qual executamos uma série de avaliações de circuito e depois usamos um otimizador clássico para encontrar os parâmetros ótimos 𝛽 e 𝛾. No entanto, precisamos começar de algum lugar, então fazemos um palpite inicial de $\gamma=\pi/2$ e $\beta=\pi$.
