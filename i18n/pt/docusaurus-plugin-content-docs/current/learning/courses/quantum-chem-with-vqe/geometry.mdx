...
# Determinando a geometria molecular {#determining-a-molecular-geometry}
...
Carregaremos os pacotes restantes para executar o VQE em si, como o ansatz efficient_su2, e os minimizadores do SciPy:
...
## Passo 3: Executar usando primitivos do IBM Quantum {#step-3-execute-using-ibm-quantum-primitives}
...
Os resultados dessa saída são discutidos abaixo na seção de pós-processamento; por enquanto, apenas observe que a simulação foi bem-sucedida. Agora você está pronto para executar em hardware real. Vamos definir a resiliência como `1`, indicando que a mitigação de erros TREX será usada. Agora que estamos trabalhando com hardware real, vamos usar o IBM Quantum Compute Service e as primitivas do IBM Quantum. Observe que tanto o loop for relacionado à geometria quanto os múltiplos ensaios variacionais estão dentro da sessão.
