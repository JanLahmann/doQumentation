...
Como ilustração final de como o algoritmo de Grover pode ser útil no contexto de uma subrotina, imaginemos que precisamos de estados quânticos com um determinado número de caracteres `1` na representação em bitstring. Isso é comum em situações que envolvem leis de conservação. Por exemplo, no contexto da química quântica, frequentemente se associa o estado `1` de um qubit à ocupação de um orbital eletrônico. Para que a carga seja conservada, o número total de caracteres `1` no bitstring também deve permanecer constante. Tais restrições são tão comuns que possuem um nome especial: **restrições de peso de Hamming**, e o número de caracteres `1` na representação em bitstring do estado é chamado de **peso de Hamming**.
