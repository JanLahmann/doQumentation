...
Aqui demonstramos como construir oráculos de Grover e usar o `GroverOperator` da biblioteca de circuitos do Qiskit para configurar facilmente uma instância de busca de Grover. O primitivo `Sampler` da IBM Quantum permite a execução contínua de circuitos de Grover.
...
$$
É aqui que a famosa aceleração de $\sqrt{N}$ vem: precisamos apenas de $O(\sqrt{N})$ iterações para atingir o alvo, em vez das $O(N)$ verificações que uma busca clássica exigiria.
...
- Etapa 3: Executar com primitivas do IBM Quantum
...
### Etapa 3: Executar usando primitivas do IBM Quantum {#step-3-execute-using-ibm-quantum-primitives}

Queremos realizar muitas medições e ver qual estado é mais provável. Tal amplificação de amplitude é um problema de amostragem adequado para execução com o primitivo `Sampler` da IBM Quantum.

Note que o método `run()` do SamplerV2 da IBM Quantum aceita um iterável de Primitive Unified Blocks (PUBs). Para o Sampler, cada PUB é um iterável no formato (circuit, parameter_values). No mínimo, porém, requer uma lista de circuitos quânticos.
...
Não. Suponha que repetíssemos a primeira atividade com 20 qubits, e executássemos o circuito quântico um número de vezes `num_shots = 10,000`. Uma distribuição de probabilidade uniforme significaria que cada estado tem uma probabilidade de $10,000/2^{20}=0.00954$ de ser medido mesmo uma única vez. Se a probabilidade de medir o estado alvo fosse 10 vezes a das não-soluções (e a probabilidade de cada não-solução fosse correspondentemente um pouco reduzida), haveria apenas cerca de 10% de chance de medir o estado alvo mesmo uma vez. Seria altamente improvável medir o estado alvo múltiplas vezes, o que o tornaria indistinguível dos muitos estados de não-solução obtidos aleatoriamente. A boa notícia é que podemos obter resultados de fidelidade ainda maior usando supressão e mitigação de erros.
...
### Etapa 3: Executar usando primitivas do IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-1}
...
### Passo 4: Pós-processamento e retorno do resultado no formato clássico desejado {#step-4-post-process-and-return-result-in-desired-classical-format-1}
...
### Etapa 1: Mapear entradas clássicas para um problema quântico {#step-1-map-classical-inputs-to-a-quantum-problem-2}
...
Após aplicar a inversão de fase, precisamos **descomputar** &mdash; desfazer todos os passos de avaliação de cláusulas em ordem inversa &mdash; para redefinir os qubits de espaço de trabalho de volta a $|0\rangle$. Isso é essencial para que os qubits de espaço de trabalho estejam limpos para as iterações subsequentes do operador de Grover.
...
### Etapa 2: Otimizar o problema para execução em hardware quântico {#step-2-optimize-problem-for-quantum-hardware-execution-2}
...
### Etapa 3: Executar usando primitivas do IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-2}
...
### Etapa 4: Pós-processamento e retorno do resultado no formato clássico desejado {#step-4-post-process-and-return-result-in-desired-classical-format-2}
