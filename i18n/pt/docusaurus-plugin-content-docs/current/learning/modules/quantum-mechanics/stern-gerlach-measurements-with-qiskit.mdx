...
- Etapa 3: Executar usando primitivas do IBM Quantum
...
Para executar nosso experimento em um computador quântico real, precisamos carregar o IBM Quantum Compute Service e selecionar um computador quântico (ou um "backend"). Abaixo, simplesmente selecionamos o computador quântico menos ocupado disponível para nós.
...
### Etapa 1: Mapear as entradas clássicas para um problema quântico {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
É isso. Não há resultado experimental. Você simplesmente aplica a matriz ao vetor e obtém um vetor ligeiramente diferente que agora tem um novo sinal "-", e não tem mais magnitude 1, já que tem um prefator $\hbar/2$.

(b) Você obterá $|\uparrow\rangle$ ou $|\downarrow\rangle$. Isso corresponde a observar uma projeção de spin no eixo $z$ de $\hbar/2$ ou $-\hbar/2$, respectivamente. Também podemos determinar a probabilidade de cada resultado, já que
$$
...
$$
Então, embora o estado spin-up seja 20% mais provável, qualquer resultado é possível, e obteremos apenas um desses dois estados.
...
Codifique circuitos quânticos para criar o estado inicial desejado, e então use hardware real ou um simulador para verificar os resultados de uma única medição, e de um conjunto de centenas ou até milhares de medições.
