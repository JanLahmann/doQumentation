...
3. **Тривалий час декогеренції** (час когерентності qubit значно більший за час операції Gate).
...
Тепер використай шумовий симулятор на основі `FakeVigo` (який має часи T1 ~50–100 мкс), щоб запустити ці схема-и. Симулятор автоматично застосує помилки T1/T2 під час інструкцій `delay`. Виконай транспіляцію Circuit-ів для цього Backend-у та запусти їх.
...
**Універсальність на практиці:** Наведена вище вправа показує, що складний трикубітний Gate був побудований із простіших. Загалом **будь-який** багатокубітний унітарний оператор може бути складений із 1- та 2-кубітних Gate-ів. Transpiler є ключовим компонентом будь-якого стека квантового програмного забезпечення, оскільки він усуває розрив між абстрактними алгоритмами, які ми хочемо запустити, та фізичними операціями, які конкретний квантовий пристрій насправді може виконувати.
