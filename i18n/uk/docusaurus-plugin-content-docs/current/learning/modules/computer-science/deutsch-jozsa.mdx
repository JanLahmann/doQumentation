...
Почнемо з завантаження необхідних пакетів, включно з примітивами IBM Quantum. Ми також оберемо найменш завантажений доступний квантовий комп'ютер.
...
У наведеній вище схемі вентиль Адамара «H» переводить кубіт 0, який спочатку перебуває у стані $|0\rangle$, до стану суперпозиції $\frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)$. Потім $U_f$ обчислює функцію $f(x)$ і застосовує її до кубіта 1.
...
Застосування вентиля Адамара переводить стан $|0\rangle$ до $\frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)$, а стан $|1\rangle$ — до $\frac{1}{\sqrt{2}}(|0\rangle-|1\rangle)$. Отже, повний стан стає: $|\pi_1\rangle = [\frac{|0\rangle-|1\rangle}{\sqrt{2}}][\frac{|0\rangle+|1\rangle}{\sqrt{2}}]$
...
Тепер стан кубіта 0 є або $\frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)$, або $\frac{1}{\sqrt{2}}(|0\rangle-|1\rangle)$, залежно від функції. Застосування вентиля Адамара дасть або $|0\rangle$, або $|1\rangle$ відповідно.
...
## Алгоритм Дойча-Йожи {#the-deutsch-jozsa-algorithm-1}
...
Цей алгоритм працює так само, як і алгоритм Дойча: фазовий відкат (phase-kickback) дозволяє зчитати стан кубіта 0, щоб визначити, чи функція є константною чи збалансованою. Це дещо складніше зрозуміти, ніж для двокубітного випадку алгоритму Дойча, оскільки стани включатимуть суми по $n$ кубітах, тому обчислення цих станів залишено як необов'язкову вправу для тебе наприкінці модуля. Алгоритм поверне рядок з усіх 0, якщо функція константна, і рядок із принаймні одним 1, якщо функція збалансована.
...
1. Спочатку до $n$ вхідних кубітів застосовується вентиль Адамара, а до вихідного кубіта — вентиль NOT і вентиль Адамара, що переводить стан у:
...
2. Далі до кубітів застосовується вентиль $U_f$. Цей вентиль приймає перші n кубітів як вхід (які тепер перебувають у рівній суперпозиції всіх можливих n-бітових рядків) і застосовує функцію $f(x)=s \cdot x$ до вихідного кубіта, так що цей кубіт тепер перебуває у стані: $ |- \oplus f(x)\rangle$. Завдяки фазовому відкату (phase-kickback) стан цього кубіта залишається незмінним, але деякі доданки у стані вхідного кубіта отримують знак мінус:
...
3. Тепер до кубітів від 0 до $n-1$ застосовується наступний набір вентилів Адамара. Відстежувати знаки мінус у цьому випадку може бути непросто. Корисно знати, що застосування шару вентилів Адамара до $n$ кубітів у стандартному базисному стані $|x\rangle$ можна записати як:
...
- Фазовий відкат (phase-kickback) — це неочікуване квантове явище, яке переносить операції над одним кубітом на фазу іншого. Алгоритми Дойча та Дойча–Йожи використовують цей механізм.
...
1. П/Х Алгоритм Дойча є окремим випадком алгоритму Дойча–Йожи, де вхід складається з одного кубіта.
...
3. Опиши фазовий відкат (phase-kickback) і те, як він працює при розв'язанні задач Дойча–Йожи та Бернштейна–Вазірані.


### Задача підвищеної складності {#challenge-problem}
1. Алгоритм Дойча–Йожи: Пригадай, що вище було запитання, де тебе просили обчислити проміжні стани кубітів $\pi_1$ та $\pi_2$ алгоритму Дойча. Зроби те саме для проміжних $(n+1)$-кубітних станів $\pi_1$ та $\pi_2$ алгоритму Дойча–Йожи для конкретного випадку $n=2$. Потім перевір, що $\pi_3 = |-\rangle \otimes \sum\limits_{x_0...x_n}(-1)^{f(x_0...x_n)}|x_0 ... x_n\rangle$, знову ж таки для конкретного випадку $n=2$.
