...
Переглянь відеоогляд модуля від доктора Кеті МакКормік нижче або натисни [тут](https://www.youtube.com/watch?v=TZ-sUHK8vVQ&list=PLOFEBzvs-Vvrs2fuvsuT039ariYPsua3d&index=6), щоб переглянути на YouTube.
...
Ми можемо створити квантову суперпозицію так само, як і минулого разу, за допомогою гейта Адамара. Щоб виміряти "орел ліворуч чи праворуч" на нашій квантовій монеті, ми можемо зробити те, що робили з класичною монетою: виміряти вздовж іншої осі. Наші стандартні вимірювання на квантовому комп'ютері виконуються вздовж вертикальної осі, так само як і звичайне вимірювання "орел вгору чи вниз" класичної монети. Але ми також можемо запитати нашу квантову монету, чи вона "орел ліворуч чи праворуч", або еквівалентно, чи вона перебуває у станах $|+\rangle$ або $|-\rangle$, які вказують вздовж осі $x$. `Sampler` виконує вибірку лише в базисі вимірювання Z, але ми можемо використати `Estimator`, щоб отримати очікуване значення X. Значення X дорівнюють +1 і -1 для станів $|+\rangle$ та $|-\rangle$ відповідно.

### Перевір своє розуміння {#check-your-understanding-1}

Якби квантова монета поводилася як класична монета в цьому випадку, ми б мали ймовірність 50-50 виміряти стан як $|+\rangle$ і $|-\rangle$. Яке очікуване значення X ми очікували б, що поверне `Estimator`, якби це було так?

<Accordion>
<AccordionItem title="Відповідь">

Коли ми застосовуємо X до стану $|+\rangle$, ми отримуємо значення +1, а до стану $|-\rangle$ ми отримуємо -1, тож якби ми мали розподіл 50-50, ми отримали б очікуване значення 0.
...
### Перевір свої знання {#check-your-understanding-3}
...
## Ще одна (краща) аналогія з монетою з використанням Gate $\sqrt{\text{NOT}}$ {#another-better-coin-analogy-using-the-sqrttextnot-gate}
...
### Перевір свої знання {#check-your-understanding-4}
...
Тепер *кожен* вентиль, який ми бачили у цьому ноутбуці, можна уявити як обертання (або серію обертань) монети/вектора.
...
Можливо, після цього модуля у тебе складеться враження, що ми щойно розвіяли всю таємничість квантової механіки. Зрештою, один із нібито «дивних» аспектів — стан суперпозиції qubit — насправді такий же простий, як тривимірний вектор. Але май на увазі, що монета — це лише аналогія, а вектор Блоха — лише інструмент візуалізації для обчислення ймовірностей результатів вимірювань. Ми не можемо стверджувати, що квантовий стан *насправді* робить до того, як його виміряють. Адже перевірити це потребує вимірювання!
