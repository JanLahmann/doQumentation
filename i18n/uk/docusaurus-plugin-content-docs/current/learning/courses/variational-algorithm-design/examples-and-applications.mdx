...
$$
Оператор є лінійною комбінацією членів з операторами Z на вузлах, з'єднаних ребром (пригадай, що 0-й кубіт розташований найправіше): $IIZZ + IZIZ + IZZI + ZIIZ + ZZII$. Після побудови оператора анзац для алгоритму QAOA можна легко побудувати, використовуючи схему `QAOAAnsatz` з бібліотеки схем Qiskit.
...
Тепер ми задаємо початковий набір випадкових параметрів:
...
Для мінімізації функції вартості можна використовувати будь-який класичний оптимізатор. На реальній квантовій системі оптимізатор, розроблений для негладких ландшафтів функції вартості, зазвичай працює краще. Тут ми використовуємо [підпрограму COBYLA](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-cobyla.html#optimize-minimize-cobyla) з SciPy через функцію minimize.

Оскільки ми ітеративно виконуємо багато викликів до Runtime, ми використовуємо сесію для виконання всіх викликів в одному блоці. Крім того, для QAOA рішення закодоване в розподілі виходу схеми анзацу, зв'язаної з оптимальними параметрами з мінімізації. Тому нам знадобиться примітив Sampler, і ми створимо його екземпляр з тією ж сесією
І запустимо нашу процедуру мінімізації:
...
Вектор рішення кутових параметрів (`x`), підставлений у схему анзацу, дає розбиття графа, яке ми шукали.
...
Перейди до останнього уроку, щоб пройти оцінювання та отримати свій значок!
