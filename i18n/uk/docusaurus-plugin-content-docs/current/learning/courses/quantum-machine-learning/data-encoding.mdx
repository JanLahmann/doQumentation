...
використовуючи амплітудне кодування.
<Accordion>
<AccordionItem title="Відповідь">
...
Ми можемо візуалізувати обертання в $\phi$ за допомогою визначеної нами функції plot_Nstates.
...
Один зі способів зрозуміти наведену вище унітарну матрицю — це через аналогію з пропагатором у фізичній системі. Наведена вище унітарна матриця — це унітарний оператор еволюції, $\exp(it\mathcal{H})$, для Гамільтоніана, $\mathcal{H}$, схожого на модель Ізінга, де параметр часу, $t$, замінюється значеннями даних для керування еволюцією. Розкладання цього унітарного оператора дає схему PFM. Взаємодії заплутування в $S$ можна інтерпретувати як зв'язки Ізінга в спіновій ґратці.
Розгляньмо приклад операторів Паулі $Y$ та $XX$, що представляють ці взаємодії типу Ізінга. Qiskit надає клас `pauli_feature_map` для створення екземпляра PFM з вибором одно- та $n$-кубітних гейтів, які в цьому прикладі будуть передані як рядки Паулі `‘Y’` та `‘XX’`. Зазвичай $n$ дорівнює 1 або 2 для одно- та двокубітних взаємодій відповідно. Схема заплутування — «лінійна», тобто пов'язані лише сусідні кубіти в квантовій схемі. Зауваж, що це не відповідає сусіднім кубітам на самому квантовому комп'ютері, оскільки ця квантова схема є рівнем абстракції.
...
Код нижче застосовує ці чотири вбудовані карти ознак до кодування набору ознак і будує графік двокубітної глибини отриманої схеми. Оскільки частота помилок двокубітних гейтів набагато вища за частоту помилок однокубітних гейтів, можна цілком розумно найбільше цікавитися глибиною двокубітних гейтів. У коді нижче ми отримуємо кількість усіх гейтів у схемі, спершу розкладаючи схему, а потім використовуючи count_ops(), як показано нижче. Тут двокубітні гейти, які нас цікавлять, — це гейти 'cx':
...
Транспіляція — це процес перетворення квантової схеми з абстракції високого рівня на таку, що готова до виконання на реальному квантовому комп'ютері, з урахуванням обмежень апаратного забезпечення. Квантовий комп'ютер має власний (нативний) набір одно- та двокубітних гейтів. Це означає, що всі гейти в коді Qiskit мають бути транспільовані в набір нативних апаратних гейтів. Наприклад, у ibm_torino, QPU з процесором Heron r1, завершеним у 2023 році, нативними, або базисними, гейтами є `{CZ, ID, RZ, SX, X}`. Це двокубітний керований-Z гейт та однокубітні гейти, які називаються тотожним, $Z$-поворотом, коренем квадратним з NOT, та NOT відповідно, що забезпечують універсальний набір. При реалізації багатокубітних гейтів як еквівалентної підсхеми потрібні фізичні двокубітні $CZ$ гейти разом з іншими однокубітними гейтами, доступними в апаратному забезпеченні. Крім того, для виконання двокубітного гейта на парі кубітів, які фізично не з'єднані, додаються SWAP-гейти для переміщення станів кубітів між кубітами, щоб уможливити зв'язність, що призводить до неминучого подовження схеми. Використовуючи аргумент ```optimization```, який можна встановити від 0 до найвищого рівня 3. Для більшого контролю та можливості налаштування конвеєр транспілятора можна керувати за допомогою [Qiskit Pass Manager](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.PassManager). Дивись [документацію Qiskit Transpiler](https://docs.quantum.ibm.com/api/qiskit/transpiler) для отримання додаткової інформації про транспіляцію.
