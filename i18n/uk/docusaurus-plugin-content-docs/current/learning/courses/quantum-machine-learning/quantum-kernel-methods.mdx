...
Використовуй примітив IBM Quantum `Sampler` для відновлення розподілу імовірностей станів, отриманих при семплюванні схеми. Зверни увагу: це може називатися «розподілом квазіймовірностей» — термін, що застосовується там, де шум є проблемою і де введено додаткові кроки, наприклад пом'якшення помилок. У таких випадках сума всіх імовірностей може бути не рівно 1; звідси «квазіймовірність».
...
### Крок 3: Виконання за допомогою примітивів IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-1}
...
Припустимо, що хтось, новий у квантових обчисленнях, намагається використати квантову схему з дуже великою двокубітною глибиною і не використовує пом'якшення помилок. Далі припустимо, що це призводить до частоти помилок 10% на кожному кубіті. Якщо істинний (безпомилковий) елемент матриці ядра, що відповідає цій схемі, дуже великий, скажімо 1.0, якою буде ймовірність виміряти всі 10 кубітів у стані, де кожен кубіт $|0\rangle$?
...
Тепер, коли у нас є матриця ядра та подібно відформатована test_matrix з квантових методів ядра, ми можемо застосувати класичні алгоритми машинного навчання, щоб зробити прогнози щодо наших тестових даних і перевірити їхню точність. Почнемо з імпорту `sklearn.svc` бібліотеки Scikit-Learn, класифікатора опорних векторів (SVC). Ми маємо вказати, що хочемо, щоб SVC використовував наше попередньо обчислене ядро за допомогою `kernel = precomputed`.
...
Використовуючи `SVC.fit`, ми тепер можемо подати матрицю ядра та навчальні мітки, щоб отримати підгонку. Потім `SVC.score` оцінить наші тестові дані відносно цієї підгонки, використовуючи наш test_matrix, і поверне нашу точність.
...
### Крок 1: Відображення класичних вхідних даних у квантову задачу {#step-1-map-classical-inputs-to-a-quantum-problem-3}
...
### Крок 3: Виконання за допомогою примітивів IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-2}
...
### Крок 4: Постобробка, повернення результату в класичному форматі {#step-4-post-process-return-result-in-classical-format-3}
