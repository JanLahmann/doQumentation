...
Ми почнемо з чотирикубітної прикладної схеми `efficient_su2` з двома типами SU(2)-вентилів, скажімо `rx` та `y`. Ми також вкажемо схему заплутування та кількість повторень. Якщо ти просто виконаєш `.draw()` для схем, ти отримаєш досить абстрактне представлення. Більш зрозуміла діаграма схеми отримується за допомогою `.decompose().draw()`, і тут ми будемо використовувати `output = "mpl"'.
...
SU(2)-вентилі з'являються на початку і в кінці з порядком та елементами, як зазначено у su2_gates = [...]. Схема заплутування `linear` означає, що CX-вентилі проходять через пронумеровані кубіти, заплутуючи 0 та 1, потім 1 та 2, і так далі, вздовж діагональної лінії у схемі. Як і можна очікувати, встановлення `reps = 2` просто додає шар заплутування та завершальний шар SU(2). Налаштування `reps = n` відповідає `n` шарам заплутування, з шарами SU(2) між ними та на кожному кінці.
...
Щоб побудувати власний ансатц, просто конструюєш квантову схему, де деякі гейти є функціями елементів вектора параметрів («theta» у трикубітному прикладі нижче).
