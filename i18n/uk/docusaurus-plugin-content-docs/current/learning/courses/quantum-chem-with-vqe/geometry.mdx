...
# Визначення молекулярної геометрії {#determining-a-molecular-geometry}

У попередньому розділі ми реалізували VQE для визначення енергії основного стану молекули. Це цілком правомірне застосування квантових обчислень, але ще кориснішим було б визначення структури молекули.

## Крок 1: Відображення класичних вхідних даних на квантову задачу {#step-1-map-classical-inputs-to-a-quantum-problem}

Залишаючись у рамках базового прикладу з двоатомним воднем, єдиним геометричним параметром для варіювання є довжина зв'язку. Для цього ми діємо, як і раніше, але використовуємо змінну при початковому конструюванні молекули (довжина зв'язку, *x*, як аргумент). Це доволі проста зміна, але вона вимагає, щоб змінна була включена до функцій упродовж усього процесу, оскільки вона з'являється ще при побудові ферміонного гамільтоніана й поширюється через відображення аж до цільової функції.

Спочатку завантажимо деякі пакети, якими користувались раніше, та визначимо функцію Холецького.
...
Тепер, щоб визначити наш гамільтоніан, ми використаємо PySCF точно так само, як у попередньому прикладі, але тепер включимо змінну `x`, яка відіграватиме роль міжатомної відстані. Це повертатиме, як і раніше, енергію ядра, одноелектронну енергію та двоелектронні енергії.
...
Пригадай, що наведена вище конструкція формує ферміонний гамільтоніан на основі атомних видів, геометрії та електронних орбіталей. Нижче ми відображаємо цей ферміонний гамільтоніан на оператори Паулі. Функція `build_hamiltonian` також міститиме геометричну змінну як аргумент.
...
Ми завантажимо решту пакетів, необхідних для власне запуску VQE, таких як анзац efficient_su2 та мінімізатори SciPy:
...
Знову визначимо цільову функцію. Оскільки вона завжди приймала повністю побудований і відображений гамільтоніан як аргумент, ця функція залишається незмінною.
...
Оскільки гамільтоніан змінюватиметься з кожною новою геометрією, транспіляція оператора теж змінюватиметься на кожному кроці. Проте ми можемо визначити загальний менеджер проходів, який застосовуватиметься на кожному кроці — специфічний для заліза, яке ми хочемо використовувати.

Тут ми використаємо найменш завантажений доступний бекенд. Ми використаємо цей бекенд як модель для нашого AerSimulator, дозволяючи симулятору імітувати, наприклад, поведінку шумів реального бекенду. Ці моделі шумів не ідеальні, але вони можуть допомогти тобі зрозуміти, чого очікувати від реального заліза.
...
Ми імпортуємо менеджер проходів і пов'язані пакети, щоб оптимізувати наш схемний ланцюжок. Цей крок, як і попередній, не залежать від гамільтоніана, тому вони незмінні порівняно з попереднім уроком.
...
У блоці коду нижче ми налаштовуємо масив для зберігання результатів на кожному кроці по міжатомній відстані $x$. Ми обрали діапазон $x$ на основі відомого нам експериментального значення рівноважної довжини зв'язку: 0,74 ангстрема. Спочатку ми запустимо це на симуляторі й тому імпортуємо наш estimator (BackendEstimator) з `qiskit.primitives`. На кожному геометричному кроці ми будуємо гамільтоніан і допускаємо певну кількість кроків оптимізації (тут 500) за допомогою оптимізатора "cobyla". На кожному геометричному кроці ми зберігаємо як повну енергію, так і електронну енергію. Через велику кількість кроків оптимізатора це може зайняти годину й більше. Можливо, ти захочеш змінити вхідні параметри нижче, щоб скоротити необхідний час.
...
Результати цього виведення обговорюються нижче в розділі постобробки; наразі просто зауваж, що симуляція була успішною. Тепер ти готовий запустити на реальному апаратному забезпеченні. Ми встановимо стійкість до `1`, що вказує на використання пом'якшення помилок TREX. Тепер, коли ми працюємо з реальним апаратним забезпеченням, ми будемо використовувати IBM Quantum Compute Service та примітиви IBM Quantum. Зауваж, що як цикл for, пов'язаний з геометрією, так і множинні варіаційні спроби знаходяться всередині сесії.

Оскільки з використанням реального заліза пов'язані витрати й обмеження за часом, ми скоротили кількість геометричних кроків і кроків оптимізатора нижче. Налаштуй ці кроки відповідно до своїх цілей точності й часових обмежень.
...
Як для симулятора, так і для реального заліза, ми можемо побудувати графік енергій основного стану, розрахованих для кожної міжатомної відстані, і побачити, де досягається найнижча енергія. Це має бути міжатомна відстань, яка зустрічається в природі, — і справді, результат близький до неї. Плавнішу криву можна отримати, спробувавши інші анзаци, оптимізатори, а також запускаючи розрахунок кілька разів на кожному кроці геометрії та усереднюючи результати за кількома випадковими початковими умовами.
...
Зауваж, що просте збільшення кількості кроків оптимізації навряд чи покращить результати симулятора, оскільки всі оптимізації фактично сходилися до потрібного допуску менш ніж за максимальну кількість ітерацій.

Результати з реального заліза є порівнянними, якщо не брати до уваги дещо інший діапазон відібраних значень.
...
Окрім очікуваної довжини зв'язку H2 у 0,74 ангстрема, повна енергія має становити -1,17 хартрі. Ми бачимо, що результати реального заліза виявилися ближчими до цих значень, ніж результати симулятора. Імовірно, це пов'язано з тим, що шум був присутній (або змодельований) в обох випадках, однак мітигація помилок застосовувалася лише у випадку реального заліза.

### Підсумок {#closing}

На цьому наш курс із VQE для квантової хімії завершується. Якщо тебе цікавить теорія інформації, що лежить в основі квантових обчислень, — переглянь курс Джона Ватруса [«Основи квантової інформації»](/learning/courses/basics-of-quantum-information). Додатковий короткий приклад робочого процесу VQE можна знайти у нашому [туторіалі з оцінювання енергії основного стану ланцюжка Гейзенберга за допомогою VQE](/tutorials/spin-chain-vqe). Або переглянь [туторіали](https://docs.quantum.ibm.com/tutorials) та [курси](/learning), щоб знайти більше навчальних матеріалів про найновіші технології квантових обчислень.

Не забудь скласти іспит цього курсу. Результат 80% або вище принесе тобі бейдж Credly, який автоматично надійде на твою електронну пошту. Дякуємо, що ти є частиною мережі IBM Quantum&reg;!
