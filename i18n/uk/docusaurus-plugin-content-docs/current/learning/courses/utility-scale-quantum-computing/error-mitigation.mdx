...
### 3.3 Виконання за допомогою Estimator (з різними рівнями стійкості) {#33-execute-using-estimator-with-different-resilience-levels}

Налаштування рівня стійкості (`estimator.options.resilience_level`) — це найпростіший спосіб застосувати пом'якшення помилок під час використання Qiskit Estimator. Estimator підтримує такі рівні стійкості (станом на 28.06.2024). Детальніше дивись у посібнику [Налаштування пом'якшення помилок](/guides/error-mitigation-and-suppression-techniques).
...
### 3.4 Побудова графіку результатів {#34-plot-results}
...
## 4. (Необов'язково) Налаштування параметрів пом'якшення помилок {#4-optional-customize-error-mitigation-options}
