...
## 2. Обчислення у вигляді діаграми {#2-computation-as-a-diagram}
...
## 3. Однокубітовий квантовий вентиль {#3-single-qubit-quantum-gate}

### 3.1 Квантовий стан і сфера Блоха {#31-quantum-state-and-bloch-sphere}

Стан кубіта представлений як суперпозиція $|0\rangle$ та $|1\rangle$. Довільний квантовий стан представлений як
...
$|0\rangle$ та $|1\rangle$ — це вектори у двовимірному комплексному векторному просторі:
...
Отже, довільний квантовий стан також представлений як
...
З цього ми бачимо, що стан кубіта — це одиничний вектор у двовимірному комплексному просторі зі скалярним добутком з ортонормованим базисом $|0\rangle$ та $|1\rangle$. Він нормований до 1.
...
Стан одного кубіта також представлений як
...
де $\theta$ та $\varphi$ — це кути сфери Блоха на наступному рисунку.
...
Цей стан суперпозиції настільки поширений і важливий, що йому надано власне позначення:

$$
|+\rangle \equiv \frac{1}{\sqrt{2}}(|0\rangle+|1\rangle).
$$

Застосувавши гейт $H$ до $|0\rangle$, ми створили суперпозицію $|0\rangle$ та $|1\rangle$, де вимірювання в обчислювальному базисі (вздовж z на сфері Блоха) дало б кожен стан з однаковою ймовірністю.

#### Стан $|-\rangle$ {#rangle-state}

Ти, можливо, здогадався, що існує відповідний стан $|-\rangle$:
$$
|-\rangle \equiv \frac{|0\rangle -|1\rangle}{\sqrt{2}}.
$$
Щоб створити цей стан, спочатку застосуй гейт X, щоб отримати $|1\rangle$, а потім застосуй гейт H.
...
Застосування гейта $H$ до $|1\rangle$ дає рівну суперпозицію $|0\rangle$ та $|1\rangle$, але знак $|1\rangle$ від'ємний.

### 3.2 Однокубітовий квантовий стан та унітарна еволюція {#32-single-qubit-quantum-state-and-unitary-evolution}

Дії всіх гейтів, які ми бачили дотепер, були *унітарними*, що означає, що вони можуть бути представлені унітарним оператором. Іншими словами, вихідний стан можна отримати, діючи на початковий стан унітарною матрицею:
...
Що стосується роботи квантового комп'ютера, ми могли б сказати, що застосування квантового гейта до кубіта еволюціонує квантовий стан. До поширених однокубітних гейтів належать наступні.
...
де зовнішній добуток обчислювався наступним чином:
$$
|0\rangle \langle 0|=
\begin{bmatrix}
1 \\
...
У квантовій програмі квантові схеми є рідним форматом для представлення квантових інструкцій. Створюючи схему, ти зазвичай створюєш новий об'єкт QuantumCircuit, а потім послідовно додаєш до нього інструкції.
...
Якщо у тебе не встановлено `qiskit-ibm-runtime`, спочатку потрібно його встановити. Докладніше можна дізнатися в [довіднику API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-service).
...
Ти можеш побачити, що під час транспіляції схема була переписана з використанням нових вентилів. Для отримання додаткової інформації звернись до документації [ECRGate](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ECRGate#ecrgate).
...
Виконання на реальному пристрої може вимагати очікування в черзі, оскільки квантові комп'ютери є цінним ресурсом, дуже затребуваним. job_id використовується для перевірки статусу виконання та результатів завдання пізніше.
