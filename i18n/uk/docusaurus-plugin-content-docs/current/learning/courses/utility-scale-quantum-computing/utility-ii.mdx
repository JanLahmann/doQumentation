...
# Квантовий експеримент рівня корисності II {#utility-scale-experiment-ii}
...
де $H$ — незалежний від часу гамільтоніан, що описує систему. Розглянемо гамільтоніан, який можна записати як зважену суму членів Паулі $H=\sum_j a_j P_j$, де $P_j$ — тензорний добуток операторів Паулі, що діють на $n$ кубітів. Зокрема, ці члени можуть комутувати між собою або ні. Задавши стан при $t=0$, як отримати стан системи $|\psi(t)\rangle$ у пізніший момент часу за допомогою квантового комп'ютера? Показникову функцію оператора найпростіше зрозуміти через її ряд Тейлора:

$$
e^{-i H t} = 1-iHt-\frac{1}{2}H^2t^2+...
$$

Деякі дуже прості показникові функції, як-от $e^{iZ}$, можна легко реалізувати на квантових комп'ютерах за допомогою компактного набору квантових вентилів. Більшість гамільтоніанів, що становлять інтерес, матимуть не один, а багато членів. Зверни увагу на те, що відбувається, якщо $H = H_1+H_2$:
...
#### Визначення гамільтоніана {#define-the-hamiltonian-1}
...
### 2.4 Завдання 4 {#24-activity-4}

#### Підготовка квантової схеми 2 (одна схема для часової еволюції) для задачі на 70 кубітів {#prepare-the-quantum-circuit-2-single-circuit-for-time-evolution-for-the-70-qubit-problem}
Тут ми будуємо схему для одного часового кроку, використовуючи метод Лі–Троттера.
Точно так само, як і у випадку з 20 кубітами, формула добутку Лі (першого порядку) реалізована у класі [LieTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter). Знову ж таки, формула першого порядку складається з наближення, зазначеного вище:

$$
e^{H_1+H_2} \approx e^{H_1} e^{H_2}
$$

Спробуй зробити це самостійно, відштовхуючись від прикладу з 20-кубітним випадком. Як і раніше, підрахуй операції для цієї схеми.
...
### 2.5 Завдання 5 {#25-activity-5}

#### Визначення операторів для вимірювання {#set-the-operators-to-be-measured-1}
Ми визначаємо *оператор намагніченості* точно за аналогією з випадком 20 кубітів: $\sum_i Z_i  / N$. Спробуй це самостійно, змінивши розв'язок для 20 кубітів.
...
### 2.6 Завдання 6 {#26-activity-6}

#### Виконати симуляцію еволюції в часі {#perform-time-evolution-simulation-1}
Ми будемо стежити за намагніченістю (очікуваним значенням оператора намагніченості). Для отримання еталонного значення, з яким порівнюватимемо результати апаратного обчислення, скористаємося симулятором MPS. Ти вже використовував(-ла) симулятор MPS раніше в цьому посібнику. Зміни той приклад, де потрібно, щоб він підходив для цього нового обчислення.
...
Як і у всіх попередніх уроках, ми будемо використовувати фреймворк Qiskit patterns. До цього місця урок був зосереджений на створенні правильних квантових схем для опису нашої задачі. По суті, це і є Крок 1.

#### Крок 2: Оптимізація для цільового апаратного забезпечення {#step-2-optimize-for-target-hardware}

Починаємо з визначення цільового бекенда.
...
Транспілюємо схеми та збираємо їх у список. Це може зайняти кілька хвилин.
...
Тепер потрібно витягти очікувані значення з цих результатів.
...
Використаємо це для порівняння нижче. Але спочатку подивимось, чи можна оптимізувати наші схеми ще більше.

## 3. Розв'язок з використанням реального квантового комп'ютера II {#3-solution-using-a-real-quantum-computer-ii}

Повернімося до кроку 1 Qiskit patterns і подивимося, чи можна зменшити глибину нашої схеми.
...
#### Завдання 7 {#activity-7}

Побудуй схему для часової еволюції. Використай знання з попередніх уроків, щоб спробувати зменшити глибину схеми.
...
###  3.2 Крок 2. Оптимізація для цільового апаратного забезпечення {#32-step-2-optimize-for-target-hardware}

Транспілюй схеми та збери їх у список. Знову ж таки, це може зайняти кілька хвилин.
...
### 3.3 Крок 3. Виконання на цільовому апаратному забезпеченні {#33-step-3-execute-on-target-hardware}
Запусти задачу.
...
### 3.4 Крок 4. Постобробка {#34-step-4-post-processing}
Витягни очікувані значення з результатів.
...
Тепер побудуємо графіки та порівняємо результати апаратного обчислення (стандартна схема і неглибока схема) з симулятором MPS. Як помилки реального апаратного забезпечення впливають на результати?
...
Вітаємо! Ти просунувся ще на крок у своїй подорожі квантовими обчисленнями рівня корисності. Залишився лише один урок!
