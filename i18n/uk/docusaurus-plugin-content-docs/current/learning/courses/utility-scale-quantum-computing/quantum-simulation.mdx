---
description: 'Цей курс присвячено квантовому моделюванню, зокрема тротеризації для гамільтоніана Ізінга з поперечним полем'
notebook_path: learning/courses/utility-scale-quantum-computing/quantum-simulation.ipynb
sidebar_label: 'Квантове моделювання'
title: 'Квантове моделювання'
---
...
[Завантажити PDF](https://ibm.ent.box.com/s/kzzsmxhw38vph1ioohczaet53euwi310) оригінальної лекції. Зверни увагу, що деякі фрагменти коду могуть бути застарілими, оскільки це статичні зображення.

*Орієнтовний час QPU для цього експерименту — 7 секунд.*

(Цей ноутбук переважно базується на нині застарілому [навчальному ноутбуці](https://github.com/qiskit-community/qiskit-algorithms/blob/main/docs/tutorials/13_trotterQRTE.ipynb) для Qiskit Algorithms.)
...
Сьогодні ми досліджуватимемо часову еволюцію [моделі Ізінга](https://en.wikipedia.org/wiki/Ising_model) на лінійних гратках із $N=2$ та $N=6$ вузлами. Ці гратки складаються з масиву спінів $\sigma_i$, що взаємодіють лише з найближчими сусідами. Ці спіни можуть мати два орієнтації: $\uparrow$ та $\downarrow$, що відповідають намагніченості $+1$ та $-1$ відповідно.
...
### 1.2 Імпорт бібліотек {#12-import-the-libraries}
...
## 2. Формалізація задачі {#2-mapping-your-problem}

### 2.1 Визначення гамільтоніана Ізінга з поперечним полем {#21-defining-the-transverse-field-ising-hamiltonian}

Тут ми розглядаємо одновимірну модель Ізінга з поперечним полем.

Спочатку створимо функцію, яка приймає параметри системи $N$, $J$, $h$ та $\alpha$ і повертає наш гамільтоніан у вигляді `SparsePauliOp`. [SparsePauliOp](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.SparsePauliOp) — це розріджене представлення оператора у вигляді зважених термів [Pauli](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Pauli).
...
### 2.2 Встановлення параметрів симуляції часової еволюції {#22-set-the-parameters-of-the-time-evolution-simulation}
...
### 2.3 Підготовка квантової схеми 1 (початковий стан) {#23-prepare-the-quantum-circuit-1-initial-state}
...
### 2.4 Підготовка квантової схеми 2 (окрема схема для часової еволюції) {#24-prepare-the-quantum-circuit-2-single-circuit-for-time-evolution}
Тут ми будуємо схему для одного часового кроку з використанням Ліє–Троттера.

Формула добутку Ліє (перший порядок) реалізована у класі [LieTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter). Формула першого порядку являє собою наближення, описане у вступі, де матрична експонента суми апроксимується добутком матричних експонент:
...
### 2.5 Визначення операторів для вимірювання {#25-set-the-operators-to-measure}
...
### 2.6 Виконання симуляції часової еволюції {#26-perform-time-evolution-simulation}
...
### 2.7 Побудова графіків часової еволюції спостережуваних {#27-plot-the-time-evolution-of-the-observables}
...
## 3. Вправа 1. Виконання симуляції з використанням Судзукі–Троттера другого порядку {#3-exercise-1-perform-simulation-using-second-order-suzukitrotter}
Тепер спробуймо виконати симуляцію з Судзукі–Троттером другого порядку, наслідуючи приклад Ліє–Троттера, показаний вище.

Судзукі–Троттер другого порядку можна використовувати в Qiskit за допомогою [класу SuzukiTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.SuzukiTrotter). За цією формулою розкладення другого порядку має вигляд:

$$
e^{H_1+H_2} \approx e^{H_1/2}e^{H_2}e^{H_1/2}
$$

### 3.1 Побудова схеми для одного часового кроку {#31-construct-a-circuit-for-a-single-time-step}
Використовуй product_formula_st2 (SuzukiTrotter(order=2)) і побудуй схему для одного часового кроку, використовуючи метод Судзукі-Троттера другого порядку. Також підрахуй кількість гейтів і глибину схеми та порівняй із Лі-Троттером.
...
### 3.2 Виконай симуляцію часової еволюції {#32-perform-time-evolution-simulation}
...
### 3.3 Побудуй графіки результатів формули Сузукі–Троттера другого порядку {#33-plot-the-second-order-suzukitrotter-results}
...
### 3.4 Порівняй з точними результатами {#34-compare-with-exact-results}
...
## 4. Виконання на квантовому залізі {#4-executing-on-the-quantum-hardware}
...
### 4.1 Крок 1. Відображення класичних вхідних даних на квантову задачу {#41-step-1-map-classical-inputs-to-a-quantum-problem}
...
#### Побудова Circuit для експерименту на залізі {#construct-circuit-for-hw-experiment}
...
### 4.2 Крок 2. Оптимізація для цільового заліза {#42-step-2-optimize-for-target-hardware}
Вкажемо Backend.
...
### 4.3 Крок 3. Виконання за допомогою примітивів IBM Quantum {#43-step-3-execute-with-ibm-quantum-primitives}
...
### 4.4 Крок 4. Постобробка результатів {#44-step-4-post-process-results}
...
Тут ми показуємо приклад побудови Circuit з використанням Suzuki–Trotter вищого порядку (четвертого).
Спробуємо побудувати Circuit симуляції з четвертим порядком Suzuki–Trotter, слідуючи прикладам вище.

Четвертий порядок Suzuki–Trotter доступний у Qiskit через [клас SuzukiTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.SuzukiTrotter). Четвертий порядок обчислюється за такою рекуррентною формулою. Зверни увагу, що порядок Suzuki–Trotter позначається як «2k» у наступних рівняннях.
...
#### Побудова Circuit для одного кроку за часом {#construct-a-circuit-for-a-single-time-step}
Використовуй product_formula_st4 (SuzukiTrotter(order=4)) і побудуй схему для одного часового кроку, використовуючи метод Судзукі-Троттера четвертого порядку. Також підрахуй кількість гейтів і глибину схеми та порівняй із Лі-Троттером і методом Судзукі-Троттера другого порядку.
