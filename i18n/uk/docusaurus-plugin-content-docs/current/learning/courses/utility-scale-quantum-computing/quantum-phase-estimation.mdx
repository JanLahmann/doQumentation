...
## 2. Основи Квантового перетворення Фур'є (QFT) {#2-basic-of-quantum-fourier-transformation-qft}
...
### 2.1. Інтуїція <a id="intuition"></a> {#21-intuition}
...
### 2.2. Приклад: QFT для 1 кубіта {#22-example-1-qubit-qft}
...
### 2.3. Добутне представлення QFT {#23-product-representation-of-qft}
...
### 2.4. Приклад: Побудова Circuit для QFT з 3 кубітів {#24-example-circuit-construction-of-3-qubits-qft}
...
### 2.5. Вправа: QFT {#25-exercise-qft}
...
#### Розв'язок вправи: QFT {#solution-of-the-exercise-qft}
(1)
...
## 3. Основи Квантової оцінки фази (QPE) {#3-basics-of-quantum-phase-estimation-qpe}
...
### 3.1. Процедура {#31-procedure}

#### 1. Налаштування {#1-setup}
...
#### 2. Суперпозиція {#2-superposition}
...
#### 3. Операції керованого унітарного оператора {#3-controlled-unitary-operations}
...
### 3.2. Приклад: QPE для T-gate {#32-example-t-gate-qpe}
...
$$
Ми ініціалізуємо $\vert\psi\rangle = \vert1\rangle$ власного вектора гейта $T$, застосовуючи гейт $X$:
...
Далі ми застосовуємо гейти Адамара до кубітів підрахунку:
...
Ми виконуємо контрольовані унітарні операції:
...
Ми застосовуємо обернене квантове перетворення Фур'є, щоб перетворити стан регістру підрахунку, а потім вимірюємо регістр підрахунку:
...
Ми можемо провести симуляцію за допомогою симулятора Aer:
...
Ми бачимо, що отримуємо один результат (`001`) з певністю, що перекладається в десятковий: `1`. Тепер нам потрібно поділити наш результат (`1`) на $2^n$, щоб отримати $\theta$:

$$
\theta = \frac{1}{2^3} = \frac{1}{8}
$$
 Алгоритм Шора дозволяє нам факторизувати число, побудувавши схему з невідомим $\theta$ та отримавши $\theta$.
...
## 4. Виконання за допомогою примітиву Sampler з IBM Quantum {#4-execution-using-the-ibm-quantum-sampler-primitive}
