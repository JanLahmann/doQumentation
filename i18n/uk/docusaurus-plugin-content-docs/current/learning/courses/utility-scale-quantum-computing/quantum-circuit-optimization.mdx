...
`optimization_level` має чотири варіанти, від 0 до 3. Чим вищий рівень оптимізації, тим більше обчислювальних зусиль витрачається на оптимізацію схеми. Рівень 0 не виконує жодної оптимізації і робить лише мінімум роботи, щоб схема могла виконуватися на обраному бекенді. Рівень 3 витрачає найбільше зусиль (і зазвичай часу виконання), щоб спробувати оптимізувати схему. Рівень 1 є типовим рівнем оптимізації.
Ми транспілюємо схему без оптимізації (`optimization_level=0`) та з оптимізацією (`optimization_level=2`).
Ми бачимо велику різницю в довжині транспільованих схем.
...
Далі ми порівняємо результати виконання двох по-різному синтезованих схем підготовки 5-кубітного стану GHZ ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$).
...
Те, що ми зробили в новій схемі, можна розглядати як ручний синтез схеми з урахуванням обмежень зв'язності. Іншими словами: ручне вирішення синтезу схеми та відображення схеми одночасно.
...
Загалом синтез схеми залежить від застосування, і програмному забезпеченню занадто складно охопити всі можливі застосування. Транспілятор Qiskit не має функцій синтезу схеми підготовки стану GHZ. У такому випадку варто розглянути ручний синтез схеми, як показано вище.
У цьому розділі ми детально розглянемо, як працює транспілятор Qiskit, на прикладі наступної навчальної схеми.
...
### 3.4 Стадія оптимізації {#34-optimization-stage}
...
## 4. Поглиблені приклади {#4-in-depth-examples}

### 4.1 Оптимізація двокубітних блоків за допомогою синтезу двокубітних унітарних матриць {#41-two-qubit-block-optimization-using-two-qubit-unitary-synthesis}
...
### 4.2 Чому оптимізація Circuit має значення {#42-circuit-optimization-matters}
...
Ми транспілюємо схему без оптимізації (`optimization_level=0`) та з оптимізацією (`optimization_level=2`).
Як бачиш, є велика різниця в довжині транспільованих схем.
...
### 4.3 Чому синтез Circuit має значення {#43-circuit-synthesis-matters}
Далі ми порівняємо результати виконання двох по-різному синтезованих схем підготовки 5-кубітного стану GHZ ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$).
...
### 4.4 Загальне розкладання однокубітного Gate {#44-general-1-qubit-gate-decomposition}
...
### 4.5 Оптимізація однокубітних блоків {#45-one-qubit-block-optimization}
