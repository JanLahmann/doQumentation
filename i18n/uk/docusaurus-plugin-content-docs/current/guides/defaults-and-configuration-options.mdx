...
<Accordion>
<AccordionItem title="**Параметри QPU (`Backend`)**">
...
<AccordionItem title="**Параметри розміщення та топології**">
...
- `inst_map` (List[InstructionScheduleMap] | None) - Відображення операцій схеми на розклади імпульсів. Якщо `None`, використовується `instruction_schedule_map` QPU.
</AccordionItem>
</Accordion>
...
<Accordion>
<AccordionItem title="**Етап ініціалізації**">
...
- `unitary_synthesis_plugin_config` (dict) - Необов'язковий словник конфігурації, який передається безпосередньо до плагіна унітарного синтезу. За замовчуванням цей параметр не має ефекту, оскільки типовий метод унітарного синтезу не приймає власної конфігурації. Застосування власної конфігурації має бути потрібним лише тоді, коли плагін унітарного синтезу вказано за допомогою аргументу `unitary_synthesis`. Оскільки це унікально для кожного плагіна унітарного синтезу, зверніться до документації плагіна щодо використання цього параметра.
</AccordionItem>
<AccordionItem title="**Етап розташування**">
...
- `layout_method` (str) - Назва проходу вибору розташування (`default`, `dense`, `sabre` та `trivial`). Це також може бути назвою зовнішнього плагіна для етапу розташування. Список встановлених плагінів можна переглянути, запустивши `list_stage_plugins()` з `layout` для аргументу `stage_name`. Типове значення — `sabre`.
</AccordionItem>
<AccordionItem title="**Етап маршрутизації**">

- `routing_method` (str) - Назва проходу маршрутизації (`basic`, `lookahead`, `default`, `sabre` або `none`). Це також може бути назвою зовнішнього плагіна для етапу маршрутизації. Список встановлених плагінів можна переглянути, запустивши `list_stage_plugins()` з `routing` для аргументу `stage_name`. Типове значення — `sabre`.
</AccordionItem>
<AccordionItem title="**Етап трансляції**">

- `translation_method` (str) - Назва проходу трансляції (`default`, `synthesis`, `translator`, `ibm_backend`, `ibm_dynamic_circuits`, `ibm_fractional`) Це також може бути назвою зовнішнього плагіна для етапу трансляції. Список встановлених плагінів можна переглянути, запустивши `list_stage_plugins()` з `translation` для аргументу `stage_name`. Типове значення — `translator`.
</AccordionItem>
<AccordionItem title="**Етап оптимізації**">

- `approximation_degree` (float, у діапазоні 0-1 | None) - Евристичний параметр, що використовується для апроксимації схеми (1.0 = без апроксимації, 0.0 = максимальна апроксимація). Типове значення — 1.0. Вказання `None` встановлює ступінь апроксимації рівним заявленому рівню помилок. Дивись [розділ про ступінь апроксимації](common-parameters#approx-degree) для деталей.

- `optimization_method` (str) - Назва плагіна для етапу оптимізації. За замовчуванням зовнішній плагін не використовується. Список встановлених плагінів можна переглянути, запустивши `list_stage_plugins()` з `optimization` для аргументу `stage_name`.
</AccordionItem>
<AccordionItem title="**Етап планування**">

- `scheduling_method` (str) - Назва проходу планування. Це також може бути назвою зовнішнього плагіна для етапу планування. Список встановлених плагінів можна переглянути, запустивши `list_stage_plugins()` з `scheduling` для аргументу `stage_name`.

  * 'as_soon_as_possible': Планує інструкції жадібно, якомога раніше на ресурсі кубіта (псевдонім: `asap`).

  * 'as_late_as_possible': Планує інструкції пізно, тобто утримуючи кубіти в основному стані, коли це можливо (псевдонім: `alap`). Це типове значення.
</AccordionItem>
<AccordionItem title="**Виконання транспілятора**">

- `seed_transpiler` (int) - Встановлює випадкові зерна для стохастичних частин транспілятора.
</AccordionItem>
</Accordion>
...
- Дізнайся, як [встановити рівень оптимізації](/guides/runtime-options-overview).
- Відвідай розділ [Транспіляція з менеджерами проходів](/guides/transpile-with-pass-managers).
- Для прикладів дивись [Представлення квантових комп'ютерів](/guides/represent-quantum-computers).
- Дізнайся, [як транспілювати схеми](/guides/circuit-transpilation-settings) у рамках робочого процесу Qiskit patterns.
