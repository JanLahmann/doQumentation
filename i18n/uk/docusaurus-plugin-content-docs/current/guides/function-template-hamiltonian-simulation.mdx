---
title: "Побудуй шаблон функції Qiskit для симуляції гамільтоніана"
sidebar_label: "Побудуй шаблон функції Qiskit для симуляції гамільтоніана"
...
Цей шаблон інкапсулює робочий процес симуляції часової еволюції початкового стану відносно визначеного користувачем спін-гамільтоніана та повертає набір вказаних значень математичного сподівання за допомогою надбудови Qiskit [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/).

Цей шаблон структурований як Qiskit-патерн із такими кроками:

#### 1. Збір вхідних даних і відображення задачі {#1-collecting-input-and-mapping-the-problem}
...
#### 2. Підготовка згенерованих схем до виконання {#2-prepare-the-generated-circuits-for-execution}

Згенеровані схеми AQC addon потім транспілюються для виконання на обраному бекенді. Створюється екземпляр [`EstimatorV2`](../api/qiskit-ibm-runtime/estimator-v2) із набором параметрів пом'якшення помилок за замовчуванням для керування виконанням схеми.

#### 3. Виконання {#3-execution}
...
## Написати шаблон функції {#write-the-function-template}

Спочатку напиши шаблон функції для симуляції гамільтоніана, що використовує [AQC-Tensor Qiskit addon](https://qiskit.github.io/qiskit-addon-aqc-tensor/) для відображення опису задачі на схему зменшеної глибини для виконання на апаратному забезпеченні.

Якщо ти завантажиш цю сторінку і переглянеш її локально в редакторі ноутбуків, то побачиш, що деякі клітинки коду містять [магічну команду](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Ця магічна команда зберігає код у `./source_files/template_hamiltonian_simulation.py` — це шаблон функції, який можна завантажити та запустити віддалено за допомогою Qiskit Serverless.
...
### Збір і валідація вхідних даних {#collect-and-validate-the-inputs}
...
#### Валідація вхідних даних {#validate-the-inputs}
...
#### Підготовка вихідних даних функції {#prepare-the-function-outputs}
...
### Відображення задачі та попередня обробка схеми за допомогою AQC {#map-the-problem-and-pre-process-the-circuit-with-aqc}
...
### Оптимізація фінальної схеми для виконання {#optimize-the-final-circuit-for-execution}

Після частини AQC робочого процесу `final_circuit` [транспілюється для апаратного забезпечення](/guides/transpile#instruction-set-architecture) як зазвичай.
...
#### Ранній вихід у режимі пробного запуску {#exit-early-if-using-dry-run-mode}
...
#### Виконання схеми на апаратному забезпеченні {#execute-the-circuit-on-hardware}
...
#### Збереження вихідних даних {#save-the-output}
...
## Розгортання функції на IBM Quantum Platform {#deploy-the-function-to-ibm-quantum-platform}
...
Використай `qiskit-ibm-catalog`, щоб автентифікуватися в `QiskitServerless` за допомогою свого API-ключа, який можна знайти на панелі [IBM Quantum Platform](https://quantum.cloud.ibm.com), і завантажити програму.

Ти можеш необов'язково використовувати `save_account()`, щоб зберегти свої облікові дані (див. посібник [Налаштування акаунта IBM Cloud](/guides/cloud-setup#cloud-save)). Зверни увагу, що це записує твої облікові дані до того самого файлу, що і [`QiskitRuntimeService.save_account()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/qiskit-runtime-service#save_account).
...
## Віддалений запуск шаблону функції {#run-the-function-template-remotely}
...
Щоб детальніше ознайомитися з доповненням Qiskit AQC-Tensor, переглянь навчальний посібник [Покращена троттеризована часова еволюція з апроксимованою квантовою компіляцією](/tutorials/approximate-quantum-compilation-for-time-evolution) або [репозиторій qiskit-addon-aqc-tensor](https://github.com/Qiskit/qiskit-addon-aqc-tensor).
...
<Accordion>
<AccordionItem title="**Повний вихідний код програми**">
