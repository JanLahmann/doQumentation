...
Проходи транспілятора на основі ШІ — це проходи, які слугують повноцінною заміною «традиційних» проходів Qiskit для деяких задач транспіляції. Вони нерідко дають кращі результати, ніж існуючі евристичні алгоритми (зокрема меншу глибину та кількість CNOT-вентилів), і водночас є значно швидшими за оптимізаційні алгоритми на кшталт розв'язувачів булевої виконуваності. Проходи транспілятора на основі ШІ запускаються у локальному середовищі.
...
Щоб використовувати проходи транспілятора на основі ШІ, спочатку встанови пакет `qiskit-ibm-transpiler`. Більше інформації про доступні параметри можна знайти в [документації API qiskit-ibm-transpiler](https://docs.quantum.ibm.com/api/qiskit-ibm-transpiler).

## Запуск проходів транспілятора на основі ШІ локально або в хмарі {#run-the-ai-transpiler-passes-locally-or-on-the-cloud}

Спочатку встанови `qiskit-ibm-transpiler` з додатковими залежностями:

```sh
pip install qiskit-ibm-transpiler[ai-local-mode]
```

Після встановлення додаткових залежностей типовим режимом запуску проходів є локальна машина.
...
Цитування
Якщо ти використовуєш будь-яку функцію на основі ШІ з Qiskit Transpiler Service у своїх дослідженнях, використовуй таке рекомендоване цитування:
