...
Наступний етап пов'язаний з розміткою або зв'язністю бекенду, на який буде надіслано схему. Загалом квантові схеми — це абстрактні сутності, кубіти яких є "віртуальними" або "логічними" представленнями реальних кубітів, що використовуються в обчисленнях. Для виконання послідовності вентилів необхідне взаємно однозначне відображення "віртуальних" кубітів на "фізичні" кубіти реального квантового пристрою. Це відображення зберігається у вигляді об'єкта `Layout` і є частиною обмежень, визначених в [архітектурі набору інструкцій (ISA)](/guides/transpile#instruction-set-architecture) бекенду.

![Це зображення ілюструє відображення кубітів від дротової схеми до діаграми, що показує, як кубіти підключені на QPU.](/docs/images/guides/transpiler-stages/layout-mapping.svg "Відображення кубітів")
...
- `TrivialLayout`: наївно відображає кожен віртуальний кубіт на фізичний кубіт пристрою з таким самим номером (тобто [`0`,`1`,`1`,`3`] -> [`0`,`1`,`1`,`3`]). Це історична поведінка, що використовується лише при `optimzation_level=1` для спроби знайти ідеальне розташування. Якщо це не вдається, наступним пробується `VF2Layout`.
...
- `DenseLayout`: знаходить підграф пристрою з найбільшою зв'язністю, що має таку саму кількість кубітів, що й схема (використовується для рівня оптимізації 1, якщо у схемі присутні операції керування потоком (такі як IfElseOp)).
...
![Вигляд тієї ж схеми через timeline.draw()](/docs/images/guides/transpiler-stages/transpiler-17.svg)
...
<Admonition type="tip" title="Рекомендації">
    ...
    - Переглянь [документацію API транспіляції](https://docs.quantum-computing.ibm.com/api/qiskit/transpiler).
</Admonition>
