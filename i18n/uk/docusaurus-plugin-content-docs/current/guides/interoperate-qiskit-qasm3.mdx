---
description: 'Як конвертувати код між OpenQASM 3 та Qiskit SDK.'
notebook_path: guides/interoperate-qiskit-qasm3.ipynb
sidebar_label: 'OpenQASM 3 та Qiskit SDK'
title: 'OpenQASM 3 та Qiskit SDK'
---

{/* doqumentation-source-hash: 64ed7441 */}


<OpenInLabBanner notebookPath="guides/interoperate-qiskit-qasm3.ipynb" />



{/*
  DO NOT EDIT THIS CELL!!!
  This cell's content is generated automatically by a script. Anything you add
  here will be removed next time the notebook is run. To add new content, create
  a new cell before or after this one.
*/}

<Accordion>
<AccordionItem title="Версії пакетів">

Код на цій сторінці розроблено з використанням наведених нижче вимог.
Рекомендуємо використовувати ці версії або новіші.

```
qiskit[all]~=2.5.2
```
</AccordionItem>
</Accordion>

```python
# Added by doQumentation — required packages for this notebook
!pip install -q qiskit
```

Qiskit SDK надає кілька інструментів для конвертації між представленнями квантових програм у форматі OpenQASM та класом QuantumCircuit. Зверни увагу, що ці інструменти ще перебувають на дослідницькому етапі розробки і продовжуватимуть розвиватися разом із підтримкою можливостей динамічних схем, які виражає OpenQASM 3 у Qiskit.

<Admonition type="note">
Ця функція досі перебуває на дослідницькому етапі. Тому синтаксис і можливості, ймовірно, змінюватимуться.
</Admonition>

## Імпорт програми OpenQASM 3 у Qiskit {#import-an-openqasm-3-program-into-qiskit}

Для використання цієї функції потрібно встановити пакет `qiskit_qasm3_import `. Встанови його за допомогою такої команди.

```bash
pip install qiskit-qasm3-import
```

Наразі для імпорту з OpenQASM 3 у Qiskit доступні дві високорівневі функції. Це `load()`, яка приймає ім'я файлу, та `loads()`, яка приймає саму програму у вигляді рядка:

```python
import qiskit.qasm3
qiskit.qasm3.load(file_name)
qiskit.qasm3.loads(program_string)
```
У цьому прикладі ми визначаємо квантову програму за допомогою OpenQASM 3 і використовуємо `loads()` для безпосереднього перетворення її на QuantumCircuit:

```python
import qiskit.qasm3

program = """
    OPENQASM 3.0;
    include "stdgates.inc";

    input float[64] a;
    qubit[3] q;
    bit[2] mid;
    bit[3] out;

    let aliased = q[0:1];

    gate my_gate(a) c, t {
      gphase(a / 2);
      ry(a) c;
      cx c, t;
    }
    gate my_phase(a) c {
      ctrl @ inv @ gphase(a) c;
    }

    my_gate(a * 2) aliased[0], q[{1, 2}][0];
    measure q[0] -> mid[0];
    measure q[1] -> mid[1];

    while (mid == "00") {
      reset q[0];
      reset q[1];
      my_gate(a) q[0], q[1];
      my_phase(a - pi/2) q[1];
      mid[0] = measure q[0];
      mid[1] = measure q[1];
    }

    if (mid[0]) {
      let inner_alias = q[{0, 1}];
      reset inner_alias;
    }

    out = measure q;
"""
circuit = qiskit.qasm3.loads(program)
circuit.draw("mpl")
```

![Output of the previous code cell](/docs/images/guides/interoperate-qiskit-qasm3/extracted-outputs/e805197c-51fb-4216-8d24-ae26633d29bc-0.svg)

## Експорт у OpenQASM 3 {#export-to-openqasm-3}

Ти можеш експортувати код Qiskit у OpenQASM 3 за допомогою `dumps()`, яка експортує в рядок, або `dump()`, яка експортує у файл.

### Приклад із `dumps()` {#example-with-dumps}

```python
from qiskit import QuantumCircuit
from qiskit.qasm3 import dumps

qc = QuantumCircuit(2)
qc.h(0)
qc.cx(0, 1)
qc.measure_all()

dumps(qc)
```

```text
'OPENQASM 3.0;\ninclude "stdgates.inc";\nbit[2] meas;\nqubit[2] q;\nh q[0];\ncx q[0], q[1];\nbarrier q[0], q[1];\nmeas[0] = measure q[0];\nmeas[1] = measure q[1];\n'
```

### Приклад із `dump()` {#example-with-dump}

```python
from qiskit import QuantumCircuit
from qiskit.qasm3 import dump

qc = QuantumCircuit(2)
qc.h(0)
qc.cx(0, 1)
qc.measure_all()

f = open("my_file.txt", "w")
dump(qc, f)
f.close()
```

Докладнішу інформацію дивись у розділі [Exporting to OpenQASM 3](https://docs.quantum.ibm.com/api/qiskit/qasm3#exporting-to-openqasm-3) довідника API.

## Наступні кроки {#next-steps}

<Admonition type="tip" title="Рекомендації">
- Дізнайся, як генерувати код OpenQASM у посібнику [IBM Quantum Composer](/guides/composer).
- Переглянь довідник [OpenQASM 3 Qiskit API](https://docs.quantum.ibm.com/api/qiskit/qasm3).
- Відвідай тему [Verify your program](/guides/debugging-tools).
- Переглянь [OpenQASM Live Specification](https://openqasm.com/).
</Admonition>
