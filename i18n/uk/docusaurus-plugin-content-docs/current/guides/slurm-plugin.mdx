...
За замовчуванням файл `CMakeLists.txt` очікує, що заголовний файл Slurm (`slurm.h`) знаходиться в `/usr/include/slurm`, але це можна налаштувати таким чином.
...
```
Наприклад, під час взаємодії з квантовими ресурсами через HTTP-проксі потрібні змінні середовища `http_proxy`, `https_proxy` та `no_proxy`. Їх можна додати таким чином.
```bash
optional /usr/lib64/slurm/spank_qrmi.so /etc/slurm/qrmi_config.json \
