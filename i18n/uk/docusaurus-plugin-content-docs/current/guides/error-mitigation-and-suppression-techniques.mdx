---
description: "Пояснення технік пом'якшення та придушення помилок, доступних через IBM Quantum Compute Service"
...
![Зображення скручування Паулі](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)
...
![Зображення скручування вимірювань](/docs/images/guides/error-mitigation-explanation/measurement_twirling.svg)

За наявності помилки зчитування скручування вимірювань має ефект діагоналізації матриці переносу помилок зчитування, що робить її легкою для інвертування. Коефіцієнт масштабування з діагонального каналу шуму вивчається шляхом тестування випадкових схем, ініціалізованих у нульовому стані. Це дозволяє сервісу усувати зміщення в значеннях очікування, спричинене шумом зчитування. Оцінка матриці переносу помилок зчитування потребує виконання додаткових калібрувальних схем, що вносить невеликі накладні витрати.

### Увімкнення TREX за допомогою параметра `measure_mitigation` {#enable-trex-by-using-the-measure-mitigation-option}

TREX можна ввімкнути, встановивши `measure_mitigation` на `True` у [параметрах відмовостійкості IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) для Estimator. Параметри навчання шуму вимірювань описані [тут](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-measure-noise-learning-options). Як і при скручуванні гейтів, можна встановити кількість рандомізацій схем та кількість вимірювань на рандомізацію.
...
### Увімкнення TREX за допомогою параметра `resilience_level` {#enable-trex-by-using-the-resilience-level-option}

Можна ввімкнути TREX, встановивши [рівень відмовостійкості](/guides/estimator-noise-management#resilience) на `1`, як показано в наступному прикладі.
...
Обидва етапи — підсилення шуму та екстраполяція — можуть бути реалізовані різними способами. IBM Quantum Compute реалізує підсилення шуму за допомогою «цифрового складання гейтів», що означає заміну двокубітних гейтів еквівалентними послідовностями гейта та його оберненого. Наприклад, заміна унітарного $U$ на $U U^\dagger U$ дасть коефіцієнт підсилення шуму 3. Для екстраполяції можна вибрати одну з кількох функціональних форм, включаючи лінійну або експоненційну апроксимацію.
...
Накладні витрати цього методу масштабуються з кількістю коефіцієнтів шуму. Налаштування за замовчуванням беруть зразок очікуваного значення при трьох коефіцієнтах шуму, що призводить до приблизно 3-кратних накладних витрат.

### Увімкнення ZNE за допомогою параметра `zne_mitigation` {#enable-zne-by-using-the-zne-mitigation-option}

ZNE можна ввімкнути, встановивши `zne_mitigation` на `True` у [параметрах відмовостійкості IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) для Estimator.
Параметри для ZNE описані [тут](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options). Наступні параметри є важливими:
...
### Увімкнення ZNE за допомогою параметра `resilience_level` {#enable-zne-by-using-the-resilience-level-option}

Можна ввімкнути ZNE (разом із TREX та скручуванням гейтів), встановивши [рівень відмовостійкості](/guides/estimator-noise-management#resilience) на `2`, як показано в наступному прикладі.
...
- [Налаштуй управління шумом за допомогою Estimator](/guides/estimator-noise-management).

- [Налаштуй управління шумом за допомогою Sampler](/guides/sampler-noise-management).

- Досліди інші [параметри](/guides/runtime-options-overview) для примітивів IBM Quantum.

- Вибери, в якому [режимі виконання](/guides/execution-modes) запустити своє завдання.
