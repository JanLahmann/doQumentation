---
description: 'Використовуй клас Neat з модуля інструментів налагодження qiskit-ibm-runtime для налагодження та аналізу завдань.'
notebook_path: guides/debug-qiskit-runtime-jobs.ipynb
sidebar_label: 'Налагодження завдань за допомогою NEAT'
title: 'Налагодження завдань за допомогою NEAT'
---
...
Ти можеш скористатися класом `Neat` для аналізу впливу шуму на навантаження Estimator. Для перевірки синтаксису використовуй [Локальний режим тестування](/guides/local-testing-mode).

## Використання класу `Neat` {#neat-class-usage}

Перед відправкою ресурсомісткого завдання на виконання на реальному залізі, ти можеш скористатися класом [NEAT (Noisy Estimator Analyzer Tool)](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/debug-tools-neat#neat), щоб перевірити, чи правильно налаштоване твоє завдання Estimator, чи воно, ймовірно, поверне точні результати, чи використовує найбільш відповідні опції для вказаної задачі тощо.
...
Для початку імпортуй відповідні пакети та [автентифікуйся в IBM Quantum Compute Service](/guides/cloud-setup).
...
- Прочитай огляд [інструментів налагодження Qiskit](/guides/debugging-tools).
- Дізнайся про [Точну та шумну симуляцію з примітивами Qiskit Aer](/guides/simulate-with-qiskit-aer).
- Дізнайся про [доступні параметри IBM Quantum Compute](/guides/runtime-options-overview).
...
- Дізнайся, [як транспілювати схеми](/guides/circuit-transpilation-settings#compare-transpiler-settings) у рамках робочих процесів Qiskit patterns.
