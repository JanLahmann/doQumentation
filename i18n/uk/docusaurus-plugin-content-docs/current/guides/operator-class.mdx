...
Ця сторінка показує, як використовувати клас [`Operator`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Operator). Загальний огляд представлень операторів у Qiskit, включно з класом `Operator` та іншими, дивись у розділі [Огляд класів операторів](/guides/operators-overview).
...
Унітарні `Operators` можна безпосередньо вставляти в `QuantumCircuit` за допомогою методу `QuantumCircuit.append`. Це перетворює `Operator` на об'єкт `UnitaryGate`, який додається до circuit.
...
Наприклад, щоб скомпонувати двокубітний гейт із трикубітним оператором:
