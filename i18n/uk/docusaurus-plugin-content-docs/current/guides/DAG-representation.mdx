...
У DAG є три види вузлів графа: вузли входу кубіта/класичного біта (зелені), вузли операцій (сині) та вихідні вузли (червоні). Кожне ребро вказує на потік даних (або залежність) між двома вузлами. Використовуй функцію qiskit.tools.visualization.dag_drawer(), щоб переглянути DAG цієї схеми. (Встанови [бібліотеку Graphviz](https://graphviz.org/download/), щоб це запустити.)
...
Операція додається в кінець DAGCircuit за допомогою методу `apply_operation_back()`. Це додає вказаний гейт для дії на задані кубіти після всіх існуючих операцій у схемі.
...
Операція додається на початок DAGCircuit за допомогою методу `apply_operation_front()`. Це вставляє вказаний гейт перед усіма існуючими операціями у схемі, фактично роблячи його першою виконуваною операцією.
...
Вузол, що представляє конкретну операцію в DAGCircuit, замінюється підсхемою. Спочатку будується новий піддаг з бажаною послідовністю гейтів, а потім цільовий вузол замінюється цим піддагом за допомогою `substitute_node_with_dag()`, зберігаючи з'єднання з рештою схеми.
...
- Ознайомся з посібником зі створення [власного проходу транспілятора](/guides/custom-transpiler-pass)
- Дізнайся, як [створювати та транспілювати схеми для власних бекендів](/guides/custom-backend)
