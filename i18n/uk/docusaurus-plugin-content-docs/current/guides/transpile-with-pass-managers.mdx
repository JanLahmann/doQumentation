...
У цих прикладах використовується імітаційний бекенд `FakeSherbrooke` з `qiskit_ibm_runtime`, але ти можеш спробувати його на будь-якому реальному або фейковому бекенді, сумісному з Qiskit. Твої результати можуть відрізнятися.
</Admonition>
...
- Щоб дізнатися більше про функцію `generate_preset_passmanager`, переглянь розділ [Стандартні налаштування транспіляції та параметри конфігурації](defaults-and-configuration-options).
- Спробуй посібник [Порівняння налаштувань транспілятора](/guides/circuit-transpilation-settings).
- Переглянь [документацію API транспілятора](https://docs.quantum.ibm.com/api/qiskit/transpiler).
</Admonition>
