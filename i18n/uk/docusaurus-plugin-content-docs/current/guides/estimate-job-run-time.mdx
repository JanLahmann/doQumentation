...
## Використання для провалених та скасованих завдань {#usage-for-failed-and-canceled-jobs}
...
## Запит фактичного використання робочого навантаження {#query-a-workloads-actual-usage}

Після завершення робочого навантаження є кілька способів переглянути його фактичне використання:

- У `qiskit-ibm-runtime` версії 0.30 або новішої запусти [`job.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-job-v2#usage) для одного завдання, або [`batch.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/batch#usage) чи [`session.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/session#usage) при використанні пакетного режиму або режиму сесії. Якщо використовується старіша версія `qiskit-ibm-runtime` (>= 0.23 та < 0.30), використання можна знайти в `session.details()["usage_time"]` та `batch.details()["usage_time"]`.

- Використай [`GET /sessions/{id}`](https://docs.quantum.ibm.com/api/qiskit-runtime-rest/tags/sessions#tags__sessions__operations__get_session), щоб переглянути використання для конкретного пакету або сесії (кінцева точка однакова для обох режимів виконання).

- Використай [`GET /jobs/{id}`](https://docs.quantum.ibm.com/api/qiskit-runtime-rest/tags/jobs#tags__jobs__operations__get_job_metrics_jid), щоб переглянути використання для одного завдання.


<span id="instance-usage"></span>

## Перегляд використання екземпляра {#view-instance-usage}
