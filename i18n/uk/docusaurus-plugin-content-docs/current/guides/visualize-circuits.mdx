...
Часто корисно бачити схеми, які Вистворюєте. Використовуйте наступні опції для відображення схем Qiskit.
...
Метод `draw()` має необов'язкові аргументи для відображення або збереження виводу. Коли вказано, kwarg `filename` приймає шлях, за яким він зберігає відтворений вивід. Альтернативно, якщо Вивикористовуєте виводи `mpl` або `latex`, Виможете використовувати kwarg `interactive`, щоб відкрити зображення в новому вікні (це не завжди працюватиме з notebook).
...
Перші дві опції є спільними для всіх трьох backend. Вони дозволяють налаштувати як порядок бітів, так і те, чи малюєте Вибар'єри. Вони можуть бути встановлені за допомогою kwarg `reverse_bits` та kwarg `plot_barriers` відповідно. Наступні приклади працюють з будь-яким рендерером виводу; `mpl` використовується тут для стислості.
