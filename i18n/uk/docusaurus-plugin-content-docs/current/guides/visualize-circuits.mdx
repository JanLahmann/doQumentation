...
Часто корисно бачити схеми, які ви створюєте. Використовуйте наступні опції для відображення схем Qiskit.
...
Ви можете зберегти діаграму безпосередньо у файл, а потім відкрити її в програмі перегляду зображень та збільшити за потреби.
...
Метод `draw()` має необов'язкові аргументи для відображення або збереження виводу. Коли вказано, kwarg `filename` приймає шлях, за яким він зберігає відтворений вивід. Альтернативно, якщо ви використовуєте виводи `mpl` або `latex`, ви можете використовувати kwarg `interactive`, щоб відкрити зображення в новому вікні (це не завжди працюватиме з notebook).
...
Перші дві опції є спільними для всіх трьох backend. Вони дозволяють налаштувати як порядок бітів, так і те, чи малюєте ви бар'єри. Вони можуть бути встановлені за допомогою kwarg `reverse_bits` та kwarg `plot_barriers` відповідно. Наступні приклади працюють з будь-яким рендерером виводу; `mpl` використовується тут для стислості.
