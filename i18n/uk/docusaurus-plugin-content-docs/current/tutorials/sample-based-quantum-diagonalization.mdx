...
- Як використовувати [SQD Qiskit addon](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd) для апроксимації енергії основного стану молекулярної системи за допомогою бітових рядків, відібраних з квантового процесора (QPU).
...
У цьому посібнику ми показуємо, як виконати постобробку шумних квантових вибірок для апроксимації основного стану молекули азоту $\text{N}_2$ при рівноважній довжині зв'язку, використовуючи [SQD Qiskit addon](https://github.com/Qiskit/qiskit-addon-sqd) для реалізації [алгоритму квантової діагоналізації на основі вибірок (SQD)](https://arxiv.org/abs/2405.05068). Докладніше про програмне забезпечення можна дізнатися у відповідній [документації](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd), включно з [простим прикладом](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd/guides/quickstart) для початку роботи.
...
Після оптимізації схеми для виконання на обладнанні ми готові запустити її на цільовому обладнанні та зібрати вибірки для оцінки енергії основного стану. Оскільки у нас лише одна схема, ми будемо використовувати [режим виконання Job](/guides/execution-modes) IBM Quantum Compute Service і виконаємо нашу схему.
...
Перший графік показує, що в цій симуляції ми вже знаходимося в межах `1 mH` від точної відповіді після першої ітерації (хімічна точність зазвичай вважається рівною `1 kcal/mol` $\approx$ `1.6 mH`). Однак це невелика система, і оскільки вибірки без шуму, відновлення конфігурації не потрібне. На більшій системі, виконаній на зашумленому QPU, може знадобитися кілька ітерацій відновлення конфігурації, а остаточна точність може бути гіршою. Загалом, енергію можна покращити, дозволивши більше ітерацій відновлення конфігурації або збільшивши кількість вибірок на пакет.
...
- [Scale SQD chemistry workflows with Dice solver](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd/guides/integrate-dice-solver) — сторінка, що показує, як використовувати ефективніше програмне забезпечення Dice для діагоналізації
- [SQD addon API documentation](https://docs.quantum.ibm.com/api/qiskit-addon-sqd/fermion#diagonalize_fermionic_hamiltonian) — довідник функції `diagonalize_fermionic_hamiltonian`
