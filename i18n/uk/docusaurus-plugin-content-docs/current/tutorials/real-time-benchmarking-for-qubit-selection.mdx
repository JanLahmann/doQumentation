---
description: 'Запускай експерименти з характеризації у реальному часі та оновлюй властивості бекенду для покращення вибору кубітів під час відображення схеми на фізичні кубіти на КОП.'
...
*Оцінка використання: 4 хвилини на процесорі Eagle r2 (ПРИМІТКА: Це лише оцінка. Твій час виконання може відрізнятися.)*
...
Цей посібник показує, як запускати експерименти з характеризації у реальному часі та оновлювати властивості бекенду для покращення вибору кубітів під час відображення схеми на фізичні кубіти на КОП. Ти дізнаєшся про базові експерименти з характеризації, які використовуються для визначення властивостей КОП, як виконувати їх у Qiskit, і як оновлювати властивості, збережені в об'єкті бекенду, що представляє КОП, на основі цих експериментів.
...
### Налаштувати бекенд та карту з'єднань {#set-up-backend-and-coupling-map}
Спочатку вибери бекенд
...
Потім отримай його карту з'єднань
...
### Експерименти з характеризації {#characterization-experiments}
...
#### Характеризація помилки підготовки стану та вимірювання (SPAM) {#state-preparation-and-measurement-spam-error-characterization}
...
#### Рандомізований бенчмаркінг однокубітних і двокубітних вентилів {#single-qubit-and-two-qubit-randomized-benchmarking}
...
### Властивості КОП з часом {#qpu-properties-over-time}
...
Ти можеш побачити, що протягом кількох днів деякі властивості кубітів можуть значно змінюватися. Це підкреслює важливість наявності свіжої інформації про стан КОП, щоб мати можливість вибрати кубіти з найкращою продуктивністю для експерименту.
...
### Виконання квантової схеми з вибором кубітів за замовчуванням {#execute-a-quantum-circuit-with-default-qubit-selection}
...
### Виконання квантової схеми з вибором кубітів у реальному часі {#execute-a-quantum-circuit-with-real-time-qubit-selection}
...
Зауваж, що деякі експерименти з характеризації можуть завершитися невдачею, коли підпрограма підгонки не може підігнати криву до виміряних даних. Якщо ти бачиш попередження від цих експериментів, перевір їх, щоб зрозуміти, яка характеризація не вдалася на яких кубітах, і спробуй налаштувати параметри експерименту (наприклад, часи для $T_1$, $T_2$ або кількість довжин експериментів RB).
</Admonition>
...
Нарешті, давай порівняємо точність стану Белла, отриману в двох різних налаштуваннях:
...
Спробуй застосувати цей метод до твоїх виконань і визнач, наскільки великою є отримана вигода! Ти також можеш спробувати і побачити, скільки покращень ти отримуєш від різних бекендів.
</Admonition>

## Опитування щодо підручника {#tutorial-survey}

Будь ласка, пройди це коротке опитування, щоб надати відгук про цей підручник. Твої думки допоможуть нам покращити наші пропозиції контенту та користувацький досвід.

> **Примітка:** Це опитування проводить **IBM Quantum**, і воно стосується **вмісту туторіалу** (написаного IBM). doQumentation надає вебсайт, переклади та виконання коду — для відгуків щодо цього, будь ласка, [відкрий питання на GitHub](https://github.com/JanLahmann/doQumentation/issues).

[Посилання на опитування](https://your.feedback.ibm.com/jfe/form/SV_0w6FZ9QrWkKfTQq)
