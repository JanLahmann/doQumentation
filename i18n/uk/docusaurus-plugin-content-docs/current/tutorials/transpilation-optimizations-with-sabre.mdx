...
*Приблизна оцінка часу виконання: менше однієї хвилини на процесорі Heron r2 (ПРИМІТКА: Це лише оцінка. Твій час виконання може відрізнятися.)*
...
### Що ти дізнаєшся {#what-youll-learn}
...
SABRE може використовуватися в Qiskit для оптимізації квантових схем шляхом обробки як етапів розташування кубітів, так і маршрутизації вентилів. У цьому розділі ми проведемо тебе через **мінімальний приклад** використання SABRE з шаблонами Qiskit, з основним акцентом на кроці 2 оптимізації.

Для запуску SABRE тобі потрібно:
- **DAG** (направлений ациклічний граф) представлення твоєї квантової схеми.
...
Збільшуй `layout_trials` і `swap_trials` для виконання більш ретельної оптимізації за рахунок збільшення часу транспіляції.
...
Згенеруй 20 різних seed для представлення 20 різних спроб компонування.
...
Запусти завантажену програму та передай вхідні дані для евристики lookahead.
...
Отримай логи та результати від serverless runtime.
...
Коли програма має статус `DONE`, ти можеш використовувати `job.results()` для отримання результату, збереженого в `save_result()`.
...
Тепер виконай те ж саме для евристики decay.
...
Отримай результати від serverless runtime та порівняй результати евристик lookahead і decay. Ми порівняємо розміри та глибини.
