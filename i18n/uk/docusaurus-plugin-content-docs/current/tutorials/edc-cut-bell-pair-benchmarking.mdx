...
*Приблизна оцінка використання: 22 секунди на процесорі Heron r2 (ПРИМІТКА: Це лише оцінка. Твій час виконання може відрізнятися.)*
...
Ми рекомендуємо, щоб користувачи були знайомі з такими темами перед проходженням цього посібника:
* [Базові концепції квантових обчислень](/learning/courses/basics-of-quantum-information), включаючи стани Белла, заплутаність та квантові вентилі;
* Знайомство з [динамічними схемами](/guides/classical-feedforward-and-control-flow) ([вимірюваннями в середині схеми](/guides/execute-dynamic-circuits#mid-circuit-measurements) та класичним зворотним зв'язком);
...
Перед запуском на реальному QPU ми перевіряємо, що схема створює пару Белла, тестуючи її на безшумному симуляторі з чотирикубітним ланцюгом `[0, 1, 2, 3]`. Ми використовуємо IBM Quantum `Sampler` з `AerSimulator` як режим бекенду для виконання схем.
...
Для кожного чотирикубітного початкового розміщення наведена вище функція повертає дві схеми (одну для вимірювання стабілізатора $ZZ$, іншу для $XX$). Ці схеми включають вимірювання в середині схеми та умовні (if/else) операції, які є ключовими інструкціями динамічної схеми.
...
![Результат виконання попередньої комірки коду](/docs/images/tutorials/edc-cut-bell-pair-benchmarking/extracted-outputs/160e25c5-0.avif)

### Крок 2: Оптимізація задачі для виконання на квантовому обладнанні {#step-2-optimize-the-problem-for-quantum-hardware-execution}
...
### Крок 4: Постобробка та повернення результату в бажаному класичному форматі {#step-4-post-process-and-return-result-in-the-desired-classical-format}
...
## Приклад на великому реальному обладнанні {#large-scale-hardware-example}
...
Якщо ця робота тебе зацікавила, можливо, тебе зацікавлять наступні матеріали:
- Дізнайся, як реалізувати [далекодіюче заплутування з динамічними схемами](/tutorials/long-range-entanglement)
- Дізнайся, як симулювати [гамільтоніан kicked Ising з динамічними схемами](/tutorials/dc-hex-ising)
</Admonition>
