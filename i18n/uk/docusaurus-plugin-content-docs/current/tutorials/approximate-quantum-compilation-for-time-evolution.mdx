...
*Оцінка використання: П'ять хвилин на процесорі Eagle (ПРИМІТКА: Це лише оцінка. Твій час виконання може відрізнятися.)*
## Передумови {#background}

Цей підручник демонструє, як реалізувати **Приблизну квантову компіляцію** з використанням тензорних мереж (AQC-Tensor) з Qiskit для покращення продуктивності квантових схем. Ми застосовуємо AQC-Tensor у контексті троттерізованої еволюції в часі для зменшення глибини схеми при збереженні точності моделювання, дотримуючись фреймворку Qiskit для підготовки стану та оптимізації. Ти навчишся створювати ансац-схему малої глибини з початкової схеми Троттера, оптимізувати її за допомогою тензорних мереж та підготувати для виконання на квантовому обладнанні.

Основна мета полягає в моделюванні еволюції в часі для модельного гамільтоніана зі зменшеною глибиною схеми. Це досягається за допомогою доповнення Qiskit **AQC-Tensor**, [qiskit-addon-aqc-tensor](https://github.com/Qiskit/qiskit-addon-aqc-tensor), яке використовує тензорні мережі, зокрема матричні добуткові стани (MPS), для стиснення та оптимізації початкової схеми. Шляхом ітеративних налаштувань стиснута ансац-схема зберігає точність відносно оригінальної схеми, залишаючись виконуваною на найближчому квантовому обладнанні. Для отримання додаткової інформації дивись [документацію](https://qiskit.github.io/qiskit-addon-aqc-tensor/).

Приблизна квантова компіляція особливо вигідна для квантових симуляцій, що перевищують час когерентності обладнання, оскільки вона дозволяє виконувати складні симуляції більш ефективно. Цей підручник проведе тебе через налаштування робочого процесу AQC-Tensor у Qiskit, охоплюючи ініціалізацію гамільтоніана, генерацію схем Троттера та транспіляцію фінальної оптимізованої схеми для цільового пристрою.
## Вимоги {#requirements}
...
### Модельний гамільтоніан та спостережувана величина {#model-hamiltonian-and-observable}
...
#### Налаштування гамільтоніана та спостережуваної величини {#set-up-the-hamiltonian-and-observable}
...
#### Визначення частини еволюції в часі для класичного моделювання {#determine-the-portion-of-time-evolution-to-simulate-classically}
...
#### Вибір часу розділення {#choose-the-split-time}
...
#### Генерація ансаца та початкових параметрів зі схеми Троттера з меншою кількістю кроків {#generate-an-ansatz-and-initial-parameters-from-a-trotter-circuit-with-fewer-steps}

Тепер, коли ми сконструювали наші чотири схеми, давай перейдемо до робочого процесу AQC-Tensor. Спочатку ми конструюємо "хорошу" схему, яка має той самий час еволюції, що й цільова схема, але з меншою кількістю кроків Троттера (і, отже, меншою кількістю шарів).
...
#### Вибір налаштувань для моделювання тензорної мережі {#choose-settings-for-tensor-network-simulation}
...
#### Оптимізація параметрів ансаца за допомогою обчислень MPS {#optimize-the-parameters-of-the-ansatz-using-mps-calculations}
...
Вибери обладнання. Тут ми використаємо будь-який із доступних пристроїв IBM Quantum&reg;, що має принаймні 127 кубітів.
...
Виконай транспіляцію для схеми порівняння.
...
Визнач карту з'єднань та ребра для гамільтоніана.
...
1. **Генерація оптимізованого ланцюга AQC**: Використовуй троттеризацію для апроксимації початкової еволюції, а потім стисни цей сегмент для зменшення глибини ланцюга.
2. **Створення ланцюга еволюції для решти часу**: Захопи залишкову еволюцію в часі за межами початкового сегмента.
3. **Об'єднання ланцюгів**: Об'єднай оптимізований ланцюг AQC з ланцюгом залишкової еволюції, щоб створити повний ланцюг еволюції в часі, готовий до виконання.
...
Згенеруй цільовий ланцюг AQC (початковий сегмент).
...
Згенеруй наступний ланцюг (залишковий сегмент).
...
Згенеруй порівняльний ланцюг AQC (початковий сегмент, але з тією ж кількістю кроків Троттера, що й наступний ланцюг).
...
Згенеруй ансатц і початкові параметри з ланцюга Троттера з меншою кількістю кроків.
...
Встанови налаштування для моделювання тензорної мережі, а потім побудуй представлення матричного продукту стану цільового стану для оптимізації. Потім оціни точність між початковим ланцюгом і цільовим станом, щоб кількісно визначити різницю в помилці Троттера.
...
Побудуй фінальний ланцюг для транспіляції, зібравши оптимізований ансатц з ланцюгом залишкової еволюції в часі.
...
Транспілюй завершений ланцюг на цільове обладнання, підготувавши його до виконання. Отриманий ланцюг ISA можна потім відправити для виконання на бекенді.
...
Для повного списку доступних опцій виконання звернись до [документації qiskit-ibm-runtime](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-estimator-options).
...
Побудуй графік результатів для AQC, порівняння та точних ланцюгів для 50-вузлової моделі XXZ.
...
Будь ласка, візьми участь у цьому короткому опитуванні, щоб надати відгук про цей посібник. Твої думки допоможуть нам покращити наш контент і досвід користувачів.
