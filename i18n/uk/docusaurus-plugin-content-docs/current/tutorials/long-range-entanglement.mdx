...
* Базові знання [Qiskit SDK](https://docs.quantum.ibm.com/guides) та [IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime), а також доступ до [облікового запису IBM Quantum](/guides/cloud-setup).
...
Перш ніж запускати на реальному QPU, ми перевіряємо, що як динамічна, так і унітарна схеми виробляють ідеальний стан Белла на симуляторі без шуму. Ми використовуємо IBM Quantum `Sampler` з `AerSimulator` як режим бекенду, з невеликою відстанню 6.
...
#### Ініціалізація схеми {#initialize-circuit}
...
#### Підготовка пар Белла {#prepare-bell-pairs}
...
#### Вимірювання сусідніх пар кубітів у базисі Белла {#measure-neighboring-qubit-pairs-in-the-bell-basis}
...
#### Застосування корекцій прямого зв'язку для виправлення побічних операторів Паулі {#apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
#### Вимірювання керуючого та цільового кубітів {#measure-control-and-target-qubits}
...
Для порівняння ми спочатку розглянемо випадок, коли дальній гейт CNOT реалізується з використанням з'єднань найближчих сусідів та унітарних гейтів. На наступному малюнку зліва показана схема для дальнього гейта CNOT, що охоплює 1D ланцюг з n-кубітів, обмежений лише з'єднаннями найближчих сусідів. В середині показана еквівалентна унітарна декомпозиція, яку можна реалізувати за допомогою локальних гейтів CNOT, з глибиною схеми $O(n)$.
...
- Визначаємо метрики якості для оцінювання продуктивності дальнього CX.
...
## Приклад на реальному обладнанні великого масштабу {#large-scale-hardware-example}
...
Тепер ми генеруємо схеми дальніх CX для діапазону відстаней до 60 кубітів. Для кожної відстані ми будуємо схеми, які вимірюють у базисах $XX$, $YY$ та $ZZ$, які пізніше будуть використані для обчислення точності.

Список відстаней включає як короткі, так і дальні відстані, де `distance = 0` відповідає найближчому сусідньому CX. Ці ж самі відстані також будуть використані для генерації відповідних унітарних схем пізніше для порівняння.
...
Наступний крок гарантує, що бекенд підтримує інструкцію `if_else`, яка потрібна для новішої версії динамічних схем. Оскільки ця функція все ще перебуває на етапі раннього доступу, ми явно додаємо `IfElseOp` до цілі бекенду, якщо вона ще не доступна.
...
#### Використання рядка Layer Fidelity для вибору 1D-ланцюжка {#use-layer-fidelity-string-for-selecting-1d-chain}
Наступний крок гарантує, що бекенд підтримує інструкцію if_else, необхідну для новішої версії динамічних схем. Оскільки ця можливість ще на етапі раннього доступу, ми явно додаємо IfElseOp до цілі бекенду, якщо її ще немає.
...
Тепер ми обчислюємо точність для унітарних схем дальнього CX, і ми робимо це так само, як ми робили для динамічних схем вище.
...
Щоб краще оцінити результати візуально, комірка нижче будує графік оцінених точностей вентилів, виміряних на різних відстанях між заплутаними кубітами для кожного методу.
