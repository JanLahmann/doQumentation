...
Спочатку виберіть бекенд для запуску. Ця демонстрація виконується на 127-кубітному бекенді, але ти можеш змінити це на будь-який доступний тобі бекенд.
...
Подивіться на карту зв'язків для бекенду і перевірте, чи якісь кубіти підключені до зв'язків з високою помилкою. Видаліть ці "погані" кубіти з твого експерименту.
...
* Перекладіть вентилі у твоїй схемі на інструкції [Архітектури набору інструкцій (ISA)](/guides/transpile#instruction-set-architecture), які можуть безпосередньо виконуватися на обладнанні.
...
Зверніть увагу, що кожна спостережувана обгорнута в список у наступному блоці коду. Це зроблено для *трансляції* із значеннями параметрів, щоб кожна спостережувана кубіта вимірювалася для кожного значення theta. Правила трансляції для примітивів можна знайти в [документації примітивів](/guides/primitives).
...
* **extrapolated\_noise\_factors**: Вкажіть одне або більше значень фактора шуму, за якими потрібно оцінити
