---
description: "Запусти експеримент з пом'якшення помилок промислового масштабу з екстраполяцією нульового шуму та ймовірнісним підсиленням помилок."
...
*Оцінка використання: 14 хвилин на процесорі Heron r3 (ПРИМІТКА: Це лише оцінка. Час виконання може відрізнятися.)*

## Результати навчання {#learning-outcomes}
Після проходження цього посібника користувачі повинні розуміти:
- Теорію *екстраполяції нульового шуму* (ZNE), різні методи підсилення шуму та чому *ймовірнісне підсилення помилок* (PEA) є кращим для експериментів промислового масштабу.
- Як реалізувати ZNE з PEA на практиці за допомогою Qiskit.

## Передумови {#prerequisites}
Ми рекомендуємо, щоб користувачі були знайомі з такими темами перед проходженням цього посібника:
- [Урок з пом'якшення помилок](/learning/courses/utility-scale-quantum-computing/error-mitigation) курсу *Квантові обчислення промислового масштабу* для базових знань про використання пом'якшення помилок у Qiskit.
- [Урок Utility-I](/learning/courses/utility-scale-quantum-computing/utility-i) курсу *Квантові обчислення промислового масштабу* для більш детального ознайомлення з експериментом промислового масштабу, що використовується як приклад у цьому посібнику.

## Передісторія {#background}

Цей посібник демонструє, як запустити експеримент з пом'якшення помилок промислового масштабу з IBM Quantum Compute Service, використовуючи експериментальну версію *екстраполяції нульового шуму* (ZNE) з *ймовірнісним підсиленням помилок* (PEA).
...
**Посилання**: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, *Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors* [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)

## Вимоги {#requirements}
Перед початком цього посібника переконайся, що встановлено наступне:
...
## Налаштування {#setup}
У комірці нижче ми імпортуємо відповідні пакети та створюємо деякі допоміжні функції для побудови схем для троттерізованої часової еволюції двовимірної поперечно-польової моделі Ізінга, яка відповідає топології бекенду.
...
Цей крок пропустимо, оскільки пом'якшення помилок в режимі виконання не підтримується на симуляторах.

## Приклад на великому апаратному забезпеченні {#large-scale-hardware-example}
...
##### Вибір бекенду {#establish-a-backend}
Спочатку вибери бекенд для запуску. Ця демонстрація виконується на 127-кубітному бекенді, але ти можеш змінити це на будь-який доступний тобі бекенд.
...
Для реалізації троттерізованої симуляції Ізінга визначте три шари зв'язків двокубітних вентилів для пристрою, які будуть повторюватися на кожному з кроків Троттера. Вони визначають три обернені шари, для яких тобі потрібно вивчити шум для реалізації пом'якшення.
...
Подивись на карту зв'язків для бекенду і перевір, чи якісь кубіти підключені до зв'язків з високою помилкою. Видали ці "погані" кубіти з твого експерименту.
...
Перед запуском схеми на обладнанні оптимізуй її для виконання на апаратному забезпеченні. Цей процес включає кілька кроків:

* Вибери розміщення кубітів, яке відображає віртуальні кубіти твоєї схеми на фізичні кубіти обладнання.

* Вставте вентилі обміну (swap gates) за потреби для маршрутизації взаємодій між кубітами, які не з'єднані.

* Переклади вентилі у твоїй схемі на інструкції [Архітектури набору інструкцій (ISA)](/guides/transpile#instruction-set-architecture), які можуть безпосередньо виконуватися на обладнанні.

* Виконай оптимізації схеми для мінімізації глибини схеми та кількості вентилів.


Хоча транспілятор, вбудований у Qiskit, може виконувати всі ці кроки, цей посібник демонструє побудову схеми Троттера корисного масштабу з нуля. Вибери хороші фізичні кубіти та визнач шари заплутування на з'єднаних парах кубітів з цих обраних кубітів. Тим не менш, тобі все ще потрібно перекласти не-ISA вентилі в схемі та скористатися будь-якою оптимізацією схеми, яку пропонує транспілятор.

Транспілюй свою схему для обраного бекенду, створивши менеджер проходів, а потім запустивши менеджер проходів на схемі. Також зафіксуй початкове розміщення схеми на вже обраних `good_qubits`. Простий спосіб створити менеджер проходів — використати функцію [`generate_preset_pass_manager`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.generate_preset_pass_manager). Дивись [Транспіляція з менеджерами проходів](/guides/transpile-with-pass-managers) для більш детального пояснення транспіляції з менеджерами проходів.
...
Далі створи всі спостережувані $\langle Z \rangle$ ваги-1 для кожного віртуального кубіта, доповнивши необхідну кількість членів $\langle I \rangle$.
...
Процес транспіляції відобразив віртуальні кубіти твоєї схеми на фізичні кубіти обладнання. Інформація про розміщення кубітів зберігається в атрибуті `layout` транспільованої схеми. Твоя спостережувана також визначена у термінах віртуальних кубітів, тому тобі потрібно застосувати це розміщення до спостережуваної. Це робиться за допомогою методу `apply_layout` класу `SparsePauliOp`.

Зверни увагу, що кожна спостережувана обгорнута в список у наступному блоці коду. Це зроблено для *трансляції* із значеннями параметрів, щоб кожна спостережувана кубіта вимірювалася для кожного значення theta. Правила трансляції для примітивів можна знайти в [документації примітивів](/guides/primitives).
...
Далі налаштуй опції `Estimator`, необхідні для запуску експерименту з пом'якшення. Це включає опції для навчання шуму шарів заплутування та для екстраполяції ZNE.
...
Наступне надає деталі щодо додаткових опцій в експериментальній гілці. Зверни увагу, що ці опції та назви не є остаточними, і все тут може змінитися до офіційного випуску.

* **amplifier**: Метод для використання при підсиленні шуму до бажаних факторів шуму.
  ...
  для шарів обертаних двокубітних базових вентилів. Додаткові опції: `"gate_folding_front"` та `"gate_folding_back"`, які пояснюються в [документації API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier).

* **extrapolated\_noise\_factors**: Вкажи одне або більше значень фактора шуму, за якими потрібно оцінити
...
Після завершення експерименту ти можеш переглянути свої результати. Ти отримуєш необроблені та пом'якшені очікувані значення та порівнюєш їх з точними результатами. Потім побудуй графіки очікуваних значень, як пом'якшених (екстрапольованих), так і необроблених, усереднених по всіх кубітах для кожного параметра. Нарешті, побудуй графіки очікуваних значень для обраних тобою окремих кубітів.
...
Якщо ця робота вас зацікавила, можливо, вас зацікавлять такі матеріали:
- [Посібник](/tutorials/combine-error-mitigation-techniques) зосереджений на поєднанні методів пом'якшення помилок.
- Детальна [документація](/guides/error-mitigation-and-suppression-techniques) щодо методів пом'якшення помилок, доступних у Qiskit.
- Додаткові уроки, що охоплюють експерименти промислового масштабу: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) та [Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
