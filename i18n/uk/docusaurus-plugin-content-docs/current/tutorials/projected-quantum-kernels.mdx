---
title: "Покращення класифікації ознак за допомогою проєкційних квантових ядер"
sidebar_label: "Покращення класифікації ознак за допомогою проєкційних квантових ядер"
description: "Навчальний посібник з покращення класифікації ознак за допомогою проєкційних квантових ядер"
...
*Оцінка використання: 80 хвилин на процесорі Heron r3 (ПРИМІТКА: Це лише оцінка. Твій час виконання може відрізнятися.)*

## Результати навчання {#learning-outcomes}

Після проходження цього навчального посібника користувачі повинні розуміти:
- Як працюють проєкційні квантові ядра (PQK) і коли вони пропонують потенційну квантову перевагу.
- Як запустити PQK на реальному обладнанні з використанням реального набору даних.

## Передумови {#prerequisites}

Ми рекомендуємо, щоб користувачі були ознайомлені з такими темами перед проходженням цього навчального посібника:
- [Квантові ядра](/learning/courses/quantum-machine-learning/quantum-kernel-methods) з [курсу квантового машинного навчання](/learning/courses/quantum-machine-learning) в IBM Quantum&reg; Learning

## Загальні відомості {#background}

У цьому навчальному посібнику ми демонструємо, як запустити [проєкційне квантове ядро](https://www.nature.com/articles/s41467-021-22539-9) (PQK) з Qiskit на реальному біологічному наборі даних, на основі статті [Enhanced Prediction of CAR T-Cell Cytotoxicity with Quantum-Kernel Methods](https://arxiv.org/abs/2507.22710) [[1]](#references).
...
Основними будівельними блоками PQK є зведені матриці густини (RDM), отримані через проєктивні вимірювання квантової карти ознак. Зокрема, зазвичай обчислюються однокубітні зведені матриці густини (1-RDM) для кожного кубіту. Ці виміряні величини потім використовуються як вхідні дані для класичної функції ядра, такої як експоненціальне ядро, для побудови кінцевої матриці ядра.

PQK пропонують потенційні переваги перед стандартними [квантовими ядрами](/tutorials/quantum-kernel-training), особливо для близькочасового квантового обладнання. Стандартні квантові ядра зазвичай покладаються на оцінку глобальних перекриттів станів, що стає дедалі складнішим для точного вимірювання зі збільшенням кількості кубітів і є дуже чутливим до шуму. На відміну від цього, PQK використовують локальні спостережувані величини, такі як однокубітні зведені матриці густини (1-RDM), що призводить до меншої потреби у вибірках, покращеної стійкості до апаратного шуму та кращої масштабованості. Проєктуючи квантові стани на локальні вимірювальні ознаки перед застосуванням класичної функції ядра, PQK можуть зберегти корисні квантові кореляції, залишаючись при цьому більш практичними для близькочасових пристроїв.
## Вимоги {#requirements}
Перед початком цього навчального посібника переконайся, що у тебе встановлено наступне:

- Qiskit SDK v2.0 або пізніше, з підтримкою [visualization](https://docs.quantum.ibm.com/api/qiskit/visualization)
...
## Приклад на малих масштабах з симулятором {#small-scale-simulator-example}
Ми опускаємо приклад на малих масштабах з симулятором у цьому навчальному посібнику, оскільки нашою основною метою є демонстрація того, як проєкційні квантові ядра можуть масштабуватися до більших систем і реального обладнання.
## Приклад на великих масштабах на реальному обладнанні {#large-scale-hardware-example}
### Крок 1: Відображення класичних входів на квантову задачу {#step-1-map-classical-inputs-to-a-quantum-problem}
### Підготовка набору даних {#dataset-preparation}

У цьому навчальному посібнику ми використовуємо реальний біологічний набір даних для завдання бінарної класифікації, який згенеровано Daniels et al. (2022) і можна завантажити з [supplementary material](https://www.science.org/doi/full/10.1126/science.abq0225#supplementary-materials), включеного до статті. Дані складаються з CAR T-клітин, які є генетично модифікованими T-клітинами, що використовуються в імунотерапії для лікування певних видів раку. T-клітини, тип імунних клітин, модифікуються в лабораторії для експресії химерних антигенних рецепторів (CAR), які націлені на специфічні білки на ракових клітинах. Ці модифіковані T-клітини можуть розпізнавати і знищувати ракові клітини більш ефективно. Ознаками даних є мотиви CAR T-клітин, які відносяться до специфічного структурного або функціонального компонента CAR, вбудованого в T-клітини. На основі цих мотивів наше завдання полягає в тому, щоб передбачити цитотоксичність даної CAR T-клітини, позначаючи її як токсичну або нетоксичну.
Нижче наведено допоміжні функції для попередньої обробки цього набору даних.
...
Ти можеш запустити цей навчальний посібник, виконавши наступну комірку, яка автоматично створює необхідну структуру папок і завантажує як навчальні, так і тестові файли безпосередньо у твоє середовище. Якщо у тебе вже є ці файли локально, цей крок безпечно перезапише їх, щоб забезпечити узгодженість версій.
...
Ми також перетворюємо набір даних таким чином, що $1$ представлено як $\pi/2$ для цілей масштабування.
...
Ми перевіряємо розміри та форми навчальних і тестових наборів даних.
...
Тепер ми конструюємо карту ознак, яка вбудовує наш класичний набір даних у простір ознак вищої розмірності. Для цього вбудовування ми використовуємо [``ZZFeatureMap``](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ZZFeatureMap) з Qiskit.
...
Іншим варіантом квантового вбудовування є ансатц еволюції гамільтоніану 1D-Heisenberg. Ти можеш пропустити виконання цього розділу, якщо хочеш продовжити з `ZZFeatureMap`.
...
### Вимірювання 1-RDM {#measure-1-rdms}
На цьому кроці ми отримуємо всі однокубітні зведені матриці густини (1-RDM) через проєктивні вимірювання квантової карти ознак, які пізніше будуть передані в класичну експоненціальну функцію ядра.
Давай подивимося, як обчислити 1-RDM для однієї точки даних з набору даних, перш ніж ми запустимо обробку всіх даних. 1-RDM є набором однокубітних вимірювань операторів Паулі ``X``, ``Y`` і ``Z`` на всіх кубітах. Це тому, що однокубітна RDM може бути повністю виражена як: $$\rho = \frac{1}{2} \big( I + \braket \sigma_x \sigma_x  + \braket \sigma_y \sigma_y + \braket \sigma_z \sigma_z  \big)$$
Спочатку ми обираємо бекенд для використання.
...
Потім ми запускаємо квантову схему і вимірюємо проєкції. Зверни увагу, що ми вмикаємо пом'якшення помилок, включаючи екстраполяцію з нульовим шумом (ZNE).
...
Ми виводимо розмір схеми та глибину двокубітних воріт.
...
Тепер ми можемо пройтися циклом по всьому навчальному набору даних, щоб отримати всі 1-RDM.
Ми також надаємо результати з експерименту, який ми запустили на квантовому обладнанні. Ти можеш або запустити навчання самостійно, встановивши прапорець нижче на `True`, або використати результати проєкцій, які ми надаємо.
...
Після завершення робіт ми можемо отримати результати.
...
Ми повторюємо це для тестового набору.
...
Ми можемо отримати результати як і раніше.
...
### Визначення проєктованого квантового ядра {#define-the-projected-quantum-kernel}
Проєктоване квантове ядро визначається наступною функцією ядра: $$k^{\textrm{PQ}}(x_i, x_j) = \textrm{exp} \Big(-\gamma \sum_k \sum_{P \in \{ X,Y,Z \}} (\textrm{Tr}[P \rho_k(x_i)] - \textrm{Tr}[P \rho_k(x_j)])^2 \Big) $$
У наведеному вище рівнянні $\gamma>0$ є налаштовуваним гіперпараметром. $K^{\textrm{PQ}}_{ij} = k^{\textrm{PQ}}(x_i, x_j)$ є елементами матриці ядра $K^{\textrm{PQ}}$.
Використовуючи визначення 1-RDM, ми можемо побачити, що окремі члени в функції ядра можуть бути обчислені як $\textrm{Tr}[P \rho_k (x_i)] = \braket P$, де $P \in \{ X,Y,Z \}$. Ці очікувані значення є саме тим, що ми виміряли вище.
Використовуючи ``scikit-learn``, ми можемо фактично обчислити ядро ще простіше. Це можливо завдяки легкодоступній функції радіальних базисних функцій (``'rbf'``) ядра: $ \textrm{exp} (-\gamma \lVert x - x' \rVert^2)$. Спочатку нам просто потрібно змінити форму нових проєктованих навчальних і тестових наборів даних у двовимірні масиви.
Зверни увагу, що обробка всього набору даних може зайняти близько 80 хвилин на QPU. Щоб переконатися, що решта посібника легко виконується, ми додатково надаємо проєкції з раніше виконаного експерименту (які включені у файли, що ти завантажив в блоці коду `Download dataset`). Якщо ти виконував навчання самостійно, ти можеш продовжити посібник зі своїми власними результатами.
...
### Метод опорних векторів (SVM) {#support-vector-machine-svm}
Тепер ми можемо запустити класичний SVM на цьому попередньо обчисленому ядрі та використати ядро між тестовими і навчальними наборами для прогнозування.
...
### Класичне порівняльне тестування {#classical-benchmarking}
Ми можемо запустити класичний SVM з функцією радіальних базисних функцій як ядром без виконання квантової проєкції. Цей результат є нашим класичним еталоном.
...
## Додаток: Перевірка потенційної квантової переваги набору даних у задачах навчання {#appendix-verify-the-datasets-potential-quantum-advantage-in-learning-tasks}

Не всі набори даних пропонують потенційну перевагу від використання PQK. Існують деякі теоретичні межі, які можна використовувати як попередній тест, щоб перевірити, чи може певний набір даних отримати вигоду від PQK. Щоб кількісно визначити це, автори статті [Power of data in quantum machine learning](https://www.nature.com/articles/s41467-021-22539-9) [2] визначають величини, які називаються класичною та квантовою складністю моделі, а також геометричним розділенням класичних і квантових моделей. Щоб очікувати потенційної квантової переваги від PQK, геометричне розділення між класичними та квантово-проєктованими ядрами повинно бути приблизно на порядку $\sqrt{N}$, де $N$ — це кількість навчальних зразків. Якщо ця умова виконується, ми переходимо до перевірки складності моделей. Якщо класична складність моделі має порядок $N$, тоді як квантово-проєктована складність моделі істотно менша за $N$, ми можемо очікувати потенційної переваги від PQK.
Геометричне розділення визначається наступним чином (F19 у [[2]](#references)):
...
Складність моделі визначається наступним чином (M1 у [[2]](#references)):
...
Якщо ця робота видалася тобі цікавою, тебе може зацікавити такий матеріал:
- Поглиблений [курс з квантового машинного навчання](/learning/courses/quantum-machine-learning) від IBM Quantum Learning
- Навчальний посібник [Quantum kernel training](/tutorials/quantum-kernel-training)
</Admonition>
