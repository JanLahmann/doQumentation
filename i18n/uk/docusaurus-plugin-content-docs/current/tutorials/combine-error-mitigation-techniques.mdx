---
description: "Поєднуй методи пом'якшення помилок для експериментів утилітарного масштабу з використанням квантових процесорів IBM Quantum на 100+ кубітів та примітива IBM Quantum Estimator."
...
*Оцінка використання: 7 хвилин на процесорі Heron r2 (ПРИМІТКА: це лише оцінка. Твій час виконання може відрізнятися.)*
...
- Основи динамічного розв'язання, пом'якшення помилок вимірювання, гейт-твірлінгу та екстраполяції нульового шуму, описані в цьому [посібнику](/guides/error-mitigation-and-suppression-techniques).
...
Цей посібник досліджує параметри придушення та пом'якшення помилок, доступні з примітивом IBM Quantum Estimator. Він показує, як реалізувати кожен із наступних методів окремо:
...
Зверни увагу, що альтернативою окремій реалізації цих технік є їх реалізація за допомогою [рівня стійкості](/guides/estimator-noise-management), де `resilience_level` приймає значення 0, 1, 2:
...
Зверни увагу, що останній кубіт відповідає першому елементу цього рядка через використання Qiskit нотації з молодшим байтом спочатку.
...
- Трансляція гейтів у твоїй схемі до інструкцій [архітектури набору інструкцій (ISA)](/guides/transpile#instruction-set-architecture), які можуть бути безпосередньо виконані на обладнанні.
...
Транспільована схема тепер містить лише інструкції ISA. Всі гейти були розкладені у термінах гейтів $\sqrt{X}$ та обертань $R_z$, а також [гейтів CZ](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.CZGate).
...
Тут ти надішлеш п'ять окремих завдань, починаючи без придушення або пом'якшення помилок, і послідовно вмикаючи різні параметри придушення та пом'якшення помилок, доступні в IBM Quantum Compute Service. Для отримання інформації про параметри звернись до наступних сторінок:
...
Оскільки ці завдання можуть виконуватися незалежно одне від одного, ти можеш використовувати [пакетний режим](/guides/run-jobs-batch), щоб дозволити Quantum Compute оптимізувати час їх виконання.
...
- [Імовірнісне скасування помилок (PEC)](/guides/error-mitigation-and-suppression-techniques#probabilistic-error-cancellation-pec)
- [Безматричне пом'якшення помилок вимірювання (M3)](https://qiskit.github.io/qiskit-addon-mthree/) (див. також посібник [Пом'якшення помилок зчитування для примітива Sampler з використанням M3](/tutorials/readout-error-mitigation-sampler/))
- [Затінені світлові конуси](https://qiskit.github.io/qiskit-addon-slc/)
- [Поглинання розповсюдженого шуму (PNA)](https://qiskit.github.io/qiskit-addon-pna/)
</Admonition>
