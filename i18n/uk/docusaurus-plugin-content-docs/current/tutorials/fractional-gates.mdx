...
```
Щоб уникнути цієї помилки, тобі слід розглянути другий підхід, описаний нижче.
...
Це вимагає від тебе призначити значення параметрів квантовим схемам перед транспіляцією.
...
Зверни увагу, що цей робочий процес вимагає більших обчислювальних витрат, ніж перший підхід, оскільки він включає призначення значень параметрів квантовим схемам та зберігання схем із прив'язаними параметрами локально.
Крім того, існує відома проблема в Qiskit, де трансформація воріт RZZ може не вдатися в певних сценаріях. Для обхідного шляху, будь ласка, звернись до розділу [Усунення несправностей](#troubleshooting).
...
Ти також можеш ознайомитися з підручником [Навчання квантових ядер](/tutorials/quantum-kernel-training) та уроком [Квантові ядра](/learning/courses/quantum-machine-learning/quantum-kernel-methods) у курсі Квантового машинного навчання на IBM Quantum Learning.
### Вимоги {#requirements}
...
У цьому розділі ми демонструємо, як оптимізувати U ворота, використовуючи дробові RX ворота, базуючись на тій самій схемі, представленій у попередньому розділі.

Тобі потрібно буде встановити [пакет](https://github.com/Qiskit/qiskit-basis-constructor) `qiskit-basis-constructor` для цього розділу.
...
Станом на Qiskit v2.0.3 існують відомі проблеми, коли RZZ ворота з недійсними кутами можуть залишатися в схемах навіть після транспіляції.
...
Коли опція `target` використовується з `generate_preset_pass_manager` або `transpiler`, спеціалізований прохід транспілятора `FoldRzzAngle` не викликається.
Для забезпечення правильної обробки кутів RZZ для дробових воріт, ми рекомендуємо завжди використовувати опцію `backend` замість цього.
Дивись [цю issue](https://github.com/Qiskit/qiskit/issues/14318) для більш детальної інформації.
#### Помилка, коли схеми містять певні ворота {#failure-when-circuits-contain-certain-gates}

Якщо твоя схема включає певні ворота, такі як `XXPlusYYGate`, транспілятор Qiskit може генерувати RZZ ворота з недійсними кутами.
Якщо ти зіткнувся з цією проблемою, дивись цю [GitHub issue](https://github.com/Qiskit/qiskit-ibm-runtime/issues/2256#issuecomment-2889487152) для обхідного рішення.
## Опитування щодо навчального посібника {#tutorial-survey}

Будь ласка, пройди це коротке опитування, щоб надати відгук про цей навчальний посібник. Твої думки допоможуть нам покращити наші пропозиції контенту та користувацький досвід.
