...
Коли опція `target` використовується з `generate_preset_pass_manager` або `transpiler`, спеціалізований прохід транспілера `FoldRzzAngle` не викликається.
...
Якщо твоя схема включає певні ворота, такі як `XXPlusYYGate`, транспілер Qiskit може генерувати RZZ ворота з недійсними кутами.
