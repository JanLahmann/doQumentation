---
title: "Динамічна оптимізація портфеля за допомогою Portfolio Optimizer від Global Data Quantum"
sidebar_label: "Динамічна оптимізація портфеля за допомогою Portfolio Optimizer від Global Data Quantum"
description: "Розв'яжи задачу динамічної оптимізації утилітарного масштабу за допомогою Квантового оптимізатора портфеля від Global Data Quantum."
...
Квантове виконання здійснюється на бекенді `ibm_torino`. Для детального пояснення формулювання задачі, методології та оцінки продуктивності звернись до опублікованого рукопису [\[1\]](#references).
## Вимоги {#requirements}
...
## Налаштування {#setup}
Щоб використовувати Квантовий оптимізатор портфеля, вибери функцію через каталог функцій Qiskit. Тобі потрібен обліковий запис IBM Quantum Premium Plan або Flex Plan з ліцензією від Global Data Quantum для запуску цієї функції.

Спочатку автентифікуйся за допомогою твого [ключа API.](https://quantum.cloud.ibm.com) Потім завантаж потрібну функцію з каталогу функцій Qiskit. Тут ти отримуєш доступ до функції `quantum_portfolio_optimizer` з каталогу за допомогою класу `QiskitFunctionsCatalog`. Ця функція дозволяє нам використовувати попередньо визначений розв'язувач квантової оптимізації портфеля.
...
Рекомендуємо запустити наступну комірку, щоб переконатися, що дані були правильно завантажені з API Yahoo Finance. Якщо завантаження не вдасться, з'явиться сповіщення про те, що дані буде отримано з альтернативної онлайн-бази даних.
</Admonition>
...
Словник `ansatz_settings` налаштовує ансац квантової схеми. Параметр `ansatz` вказує на використання підходу `"optimized_real_amplitudes"`, який є апаратно-ефективним ансацем, розробленим для задач фінансової оптимізації. Параметр `multiple_passmanager`, який дозволяє використовувати кілька менеджерів проходів (включаючи стандартний локальний менеджер проходів Qiskit та сервіс транспіляції Qiskit на основі ШІ) для покращення продуктивності та ефективності оптимізації схем, наразі встановлено на false, оскільки цю функцію тимчасово вимкнено.
...
Будь ласка, приділи хвилину, щоб надати відгук про цей навчальний посібник. Твої думки допоможуть нам покращити наші матеріали та досвід користувачів.
