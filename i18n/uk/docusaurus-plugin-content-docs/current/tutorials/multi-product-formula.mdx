...
*Очікуване використання QPU: чотири хвилини на процесорі Heron r2 (ПРИМІТКА: Це лише оцінка. Твій час виконання може відрізнятися.)*
...
Ти можеш розглядати корисність цього з двох точок зору:

1. Для фіксованого бюджету кроків Троттера, які ти можеш виконати, ти можеш отримати результати з похибкою Троттера, яка є меншою в загальному.
2. Маючи деяку цільову кількість кроків Троттера, яка є занадто великою для виконання, ти можеш використовувати MPF для знаходження набору кіл меншої глибини для виконання, що призведе до подібної похибки Троттера.
## Вимоги {#requirements}
...
Для більш детальної інформації звернись до документації лінійної системи рівнянь ([LSE](https://qiskit.github.io/qiskit-addon-mpf/stubs/qiskit_addon_mpf.static.LSE.html)).

Ми можемо знайти рішення для $x$ аналітично як $x = A^{-1}b$; дивись, наприклад, Посилання [\[1\]](#references) або [\[2\]](#references).
...
##### Налаштування LSE {#set-up-the-lse}
...
Додатково, ти можеш врахувати, чи є PF симетричною чи ні (дивись [\[1\]](#references)), встановивши `symmetric=True/False`.
...
Давай пропрацюємо значення, вибрані вище, щоб побудувати матрицю $A$ і вектор $b$. З $j=0,1, 2$ кроками Троттера $k_j = [1, 2, 4]$, порядком $\chi = 2$ і вибором несиметричних кроків Троттера ($s=1$), ми маємо, що матричні елементи $A$ нижче першого рядка визначаються виразом $A_{i>0,j} = k_{j}^{-(2 + 1(i-1))}$, конкретно:
...
Як альтернативу обчисленню $x=A^{-1}b$, ти також можеш використовувати [setup_exact_model](https://qiskit.github.io/qiskit-addon-mpf/stubs/qiskit_addon_mpf.static.setup_exact_model.html) для побудови екземпляра [cvxpy.Problem](https://www.cvxpy.org/api_reference/cvxpy.problems.html#cvxpy.Problem), який використовує LSE як обмеження і чиє оптимальне рішення дасть $x$.
...
Якщо це так, і ти не можеш вибрати інший набір значень $k_j$, ти можеш використовувати наближений розв'язок LSE замість точного.

Для цього просто використовуй [setup_approximate_model](https://qiskit.github.io/qiskit-addon-mpf/stubs/qiskit_addon_mpf.static.setup_approximate_model.html) для побудови іншого екземпляра [cvxpy.Problem](https://www.cvxpy.org/api_reference/cvxpy.problems.html#cvxpy.Problem), який обмежує L1-норму до обраного порогу, мінімізуючи різницю між $Ax$ і $b$.
...
Зверни увагу, що ти маєш повну свободу щодо розв'язання цієї оптимізаційної задачі, що означає, що ти можеш змінювати оптимізаційний розв'язувач, його пороги збіжності тощо.
Ознайомся з відповідним посібником [Як використовувати наближену модель](https://qiskit.github.io/qiskit-addon-mpf/how_tos/using_approximate_model.html).
...
Далі створи початковий стан своєї системи у форматі, сумісному з TeNPy (наприклад, `MPS_neel_state`=$\vert 0101...01 \rangle$). Це налаштовує багаточастинкову хвильову функцію, яку ти будеш еволюціонувати в часі $\lvert\psi_{\mathrm{in}}\rangle$ як тензор.
...
Нарешті, побудуй графік цих очікуваних значень протягом часу еволюції.
...
Іншою цікавою поведінкою, яку можна спостерігати на графіку, є те, що очікуване значення для PF при $k=1$ починає поводитися нестабільно (на додаток до того, що воно не є хорошою апроксимацією точного) у моменти часу, для яких $t/k > 1$, як пояснюється в [посібнику](https://qiskit.github.io/qiskit-addon-mpf/how_tos/choose_trotter_steps.html) щодо вибору кількості кроків Троттера.
...
#### Налаштування схем Троттера {#set-up-the-trotter-circuits}
...
Давай масштабуємо задачу за межі того, що можливо точно змодулювати. У цьому розділі ми зосередимося на відтворенні деяких результатів, показаних у посиланні [\[3\]](#references).
### Крок 1: Відображення класичних вхідних даних на квантову задачу {#step-1-map-classical-inputs-to-a-quantum-problem}
...
#### Вибір кроків Троттера {#choose-trotter-steps}
...
#### Динамічні коефіцієнти {#dynamic-coefficients}
...
#### Побудова кожного з ланцюгів Троттера в нашому розкладі MPF {#construct-each-of-the-trotter-circuits-in-our-mpf-decomposition}
...
#### Побудова ланцюга Троттера з порівнянною похибкою Троттера до MPF {#construct-trotter-circuit-with-comparable-trotter-error-to-mpf}
