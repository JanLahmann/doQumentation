...
*Оцінка використання: 7,5 хвилин на процесорі Heron r3. (ПРИМІТКА: Це лише оцінка. Твій час виконання може відрізнятися.)*
...
Якщо ти отримуєш помилки про те, що `neato` не знайдено з `plot_circuit_layout()`, переконайся, що у тебе встановлено пакет `graphviz` і він доступний у твоєму PATH. Якщо він встановлюється в нестандартне місце (наприклад, при використанні `homebrew` на MacOS), може знадобитися оновити змінну середовища `PATH`. Це можна зробити всередині цього ноутбука, використовуючи таке:
...
### Порівняння глибини двокубітних вентилів унітарних і динамічних схем {#compare-two-qubit-gate-depth-of-unitary-and-dynamic-circuits}
...
Основна перевага схеми на основі вимірювань полягає в тому, що при реалізації кількох взаємодій ZZ шари CX можуть бути паралелізовані, і вимірювання можуть відбуватися одночасно. Це тому, що всі взаємодії ZZ комутують, тому обчислення можна виконати з глибиною вимірювання 1. Після транспіляції схем ми спостерігаємо, що підхід із динамічною схемою дає значно меншу глибину двокубітних вентилів, ніж стандартний унітарний підхід, з тією застережкою, що додаткове вимірювання в середині схеми та класичне випереджувальне керування самі по собі займають час і вносять власні джерела помилок.
...
Перед відправкою завдань на обладнання ми можемо виконати невелику тестову симуляцію динамічної схеми, використовуючи [режим локального тестування](/guides/local-testing-mode).
...
Для великих схем ми можемо використовувати симулятор `matrix_product_state` (MPS), який надає приблизний результат для очікуваного значення відповідно до обраної розмірності зв'язку. Пізніше ми використовуємо результати симуляції MPS як базову лінію для порівняння результатів з обладнання.
...
Маючи підготовлені схеми та спостережувані величини, тепер ми виконуємо їх на обладнанні, використовуючи примітив Sampler.

Тут ми відправляємо три завдання для `unitary_pubs`, `dynamic_pubs` та `dynamic_pubs_dd`. Кожне з них є списком параметризованих схем, що відповідають дев'яти різним крокам Троттера з трьома різними параметрами $\theta$.
...
Після завершення завдань ми можемо отримати тривалість схеми з метаданих результатів завдання та візуалізувати інформацію про розклад схеми. Щоб дізнатися більше про візуалізацію інформації про планування схеми, звернись до [цієї сторінки](/guides/qiskit-runtime-circuit-timing).
...
Ми будуємо графік тривалостей схем для унітарних схем та динамічних схем. З графіка нижче ми бачимо, що, незважаючи на час, необхідний для вимірювань всередині схеми та класичних операцій, реалізація динамічної схеми з `measure_2` призводить до порівнянних тривалостей схеми, як і унітарна реалізація.
...
1. виконання унітарних схем з DD
3. виконання динамічних схем з DD та `MidCircuitMeasure`
...
Коли ми порівнюємо експериментальні результати з симуляцією, ми бачимо, що реалізація динамічної схеми (пунктирна лінія зі зірками) загалом має кращу продуктивність, ніж стандартна унітарна реалізація (пунктирна лінія з колами). Підсумовуючи, ми представляємо динамічні схеми як рішення для симуляції спінових моделей Ізінга на гратці з сотовою структурою, топології, яка не є рідною для обладнання. Рішення з динамічними схемами дозволяє взаємодії ZZ між кубітами, які не є найближчими сусідами, з меншою глибиною двокубітних вентилів, ніж при використанні вентилів SWAP, за рахунок введення додаткових допоміжних кубітів та операцій класичної прямої передачі.
