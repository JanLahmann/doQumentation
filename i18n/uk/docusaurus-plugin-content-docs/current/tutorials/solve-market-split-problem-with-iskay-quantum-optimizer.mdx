---
description: "Дізнайся, як розв'язати проблему розподілу ринку, використовуючи квантовий оптимізатор Iskay з алгоритмом bf-DCQO на квантовому обладнанні IBM"
...
*Оцінка використання: 20 секунд на процесорі Heron r2. (ПРИМІТКА: Це лише оцінка. Твій час виконання може відрізнятися.)*
...
### Виклик розподілу ринку {#the-market-split-challenge}
...
### Що робить підхід Iskay унікальним? {#what-makes-iskays-approach-unique}
...
Тобі також потрібно отримати доступ до [функції квантового оптимізатора Iskay](https://quantum.cloud.ibm.com/functions?id=kipu-quantum-iskay-quantum-optimizer) з каталогу функцій Qiskit.

## Налаштування {#setup}

Спочатку імпортуй всі необхідні пакети для цього посібника.
...
### Налаштування облікових даних IBM Quantum {#configure-ibm-quantum-credentials}

Визнач свої облікові дані [IBM Quantum&reg; Platform](https://quantum.cloud.ibm.com/). Тобі знадобиться:
- **API Token**: Твій 44-символьний API ключ з IBM Quantum Platform
- **Instance CRN**: Твій ідентифікатор екземпляра IBM Cloud&reg;
...
### Підключення до квантового оптимізатора Iskay {#connect-to-iskay-quantum-optimizer}
...
### Завантаження та формулювання задачі {#load-and-formulate-the-problem}

#### Розуміння формату даних задачі {#understand-the-problem-data-format}

Екземпляри задач з QOBLIB (Quantum Optimization Benchmarking Library — бібліотека тестування квантової оптимізації) [\[2\]](#references) зберігаються в простому текстовому форматі. Давай розглянемо фактичний вміст нашого цільового екземпляра `ms_03_200_177.dat`:
...
#### Трансформація QUBO {#qubo-transformation}

## Від обмежень до QUBO: математичне перетворення {#from-constraints-to-qubo-the-mathematical-transformation}
...
### Реалізація функцій завантаження даних та конверсії у QUBO {#implement-data-loading-and-qubo-conversion-functions}
...
### Завантаження екземпляра задачі {#load-the-problem-instance}
...
### Конверсія у формат QUBO {#convert-to-qubo-format}
...
### Конверсія QUBO у формат Iskay {#convert-qubo-to-iskay-format}
...
- Вибираючи `problem_type = "binary"`, ти вказуєш, що функція вартості є у форматі `binary`, що означає, що $D = \{0,  1\}^{n}$, тобто функція вартості записана у формулюванні QUBO/HUBO.
...
Зверни увагу, що ключі словника повинні бути рядками, що містять коректний кортеж неповторюваних цілих чисел. Для бінарних задач ми знаємо, що:

$$
x_i^2 = x_i
$$

для $i=j$ (оскільки $x_i \in \{0,1\}$ означає $x_i \cdot x_i = x_i$). Отже, у твоєму формулюванні QUBO, якщо у тебе є як лінійні доданки $b_i x_i$, так і діагональні квадратичні доданки $c_{i,i} x_i^2$, ці доданки повинні бути об'єднані в один лінійний коефіцієнт:
...
**Приклад:** Якщо твій QUBO має $3x_1 + 2x_1^2 + 4x_1 x_2$, словник Iskay повинен містити:
...
### Розуміння алгоритму bf-DCQO {#understand-the-bf-dcqo-algorithm}

Перед тим як запустити оптимізацію, давай зрозуміємо складний квантовий алгоритм, який працює в Iskay: **bf-DCQO (bias-field digitized counterdiabatic quantum optimization)** [\[1\]](#references).

#### Що таке bf-DCQO? {#what-is-bf-dcqo}
...
#### Математична структура {#mathematical-framework}
...
#### Роль контрадіабатичних доданків {#the-role-of-counterdiabatic-terms}
...
#### Ітераційна оптимізація з полем зміщення {#bias-field-iterative-optimization}
...
#### Інтегрована класична постобробка {#integrated-classical-post-processing}
...
#### Чому bf-DCQO перевершує інших на сучасному обладнанні {#why-bf-dcqo-excels-on-current-hardware}
...
А тепер подивимось на цей потужний алгоритм у дії на нашій задачі Market Split!
...
### Налаштування оптимізації {#configure-the-optimization}

Оптимізатор Iskay вимагає кілька ключових параметрів для ефективного розв'язання твоєї оптимізаційної задачі. Розглянемо кожен параметр та його роль у процесі квантової оптимізації:
...
#### Основні концепції {#essential-concepts}

- **Формат задачі**: Ми використовуємо `"binary"`, оскільки наші змінні є бінарними (0/1), що представляють призначення ринків.

- **Вибір бекенду**: Вибирай між доступними QPU (наприклад, `"ibm_fez"`) на основі твоїх потреб та екземпляра обчислювального ресурсу.

- **Структура QUBO**: Наш словник задачі містить точні коефіцієнти з математичного перетворення.


#### Розширені опції (необов'язкові) {#advanced-options-optional}

Iskay надає можливості тонкого налаштування через необов'язкові параметри. Хоча значення за замовчуванням добре працюють для більшості задач, ти можеш налаштувати поведінку для конкретних вимог:
...
#### Приклад власної конфігурації {#custom-configuration-example}

Ось як ти можеш налаштувати Iskay з різними параметрами:
...
### Моніторинг статусу завдання {#monitor-job-status}

Ти можеш перевірити поточний статус твого оптимізаційного завдання. Можливі статуси:
...
### Очікування завершення {#wait-for-completion}
...
#### Розуміння структури результату {#understand-the-result-structure}
...
#### Перевірка розв'язку {#solution-validation}
...
#### Інтерпретація результатів перевірки {#interpret-the-validation-results}
...
**Аналіз продажів:**
- Порівняй цільові та фактичні продажі для кожного продукту
...
#### Оцінка якості розв'язку {#solution-quality-assessment}
...
- Розглянь можливість налаштування параметрів оптимізатора:
  - Збільш `num_iterations` для більшої кількості проходів оптимізації
  - Збільш `postprocessing_level` для більшого класичного уточнення
  - Збільш `shots` для кращої статистики вимірювань
...
### Що ми досягли {#what-we-accomplished}
...
Щоб поглибити твоє розуміння та продовжити дослідження:

1. **Спробуй різні екземпляри**: Експериментуй з іншими екземплярами QOBLIB різних розмірів

2. **Налаштуй параметри**: Відрегулюй `num_iterations`, `preprocessing_level`, `postprocessing_level`

3. **Порівняй з класичними**: Проведи бенчмаркінг порівняно з класичними оптимізаційними розв'язувачами

4. **Спробуй різні стратегії**: Спробуй знайти краще кодування для задачі або сформулюй її як HUBO (якщо можливо)

5. **Застосуй до своєї галузі**: Адаптуй техніки формулювання QUBO/HUBO до своїх власних оптимізаційних задач
