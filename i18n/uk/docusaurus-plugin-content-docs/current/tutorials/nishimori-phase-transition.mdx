...
## Вимоги
Перш ніж починати цей посібник, переконайтеся, що у Вас встановлено наступне:
...
Клас `GemExperiment` прозоро транслює схеми під час виконання експерименту. Етапи розташування та маршрутизації за замовчуванням перевизначені для бездіяльності, а оптимізація схеми виконується на рівні, що оптимізує лише однокубітні вентилі. Однак Ви можете перевизначити або передати додаткові параметри за допомогою методу `set_transpile_options`. Для цілей візуалізації наступна комірка коду вручну транслює раніше відображену схему та малює трансльовану схему.
