...
Тепер ми використаємо зворотне поширення операторів (OBP). (Переглянь посібник [OBP](/guides/qiskit-addons-obp) для отримання більш детальної інформації про додаток Qiskit OBP.) Ми створимо функцію, яка генерує зрізи схеми для зворотного поширення:
