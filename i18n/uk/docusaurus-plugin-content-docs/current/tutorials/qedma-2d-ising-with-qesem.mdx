...
*Оцінка використання: 20 хвилин на процесорі Heron r2. (ПРИМІТКА: Це лише оцінка. Твій час виконання може відрізнятися.)*
...
* Візуалізуємо зв'язність кубітів пристрою та вибрані підграфи для твого експерименту.
* Продемонструємо використання [зворотного поширення операторів (OBP)](https://qiskit.github.io/qiskit-addon-obp/) для зменшення глибини схеми. Ця техніка обрізає операції з кінця схеми за рахунок додаткових вимірювань операторів.
...
Встанови наступні пакети Python перед запуском notebook:
...
## Налаштування {#setup}
Спочатку імпортуй відповідні бібліотеки:
...
Далі пройди автентифікацію, використовуючи твій API-ключ з панелі [IBM Quantum Platform](http://quantum.cloud.ibm.com/). Потім вибери функцію Qiskit наступним чином. (Зверни увагу, що з міркувань безпеки краще [зберегти облікові дані облікового запису](/guides/functions-get-started#install-qiskit-functions-catalog-client) у твоєму локальному середовищі, якщо ти знаходишся на надійній машині, щоб тобі не потрібно було вводити API-ключ кожного разу при автентифікації.)
...
Ми використовуємо апаратне відображення шарів $R_{ZZ}$ взяте з пристрою Heron, з якого ми вирізаємо шари відповідно до кількості кубітів, які хочемо симулювати. Ми визначаємо підграфи для 10, 21, 28 та 35 кубітів, які зберігають 2D структуру (не соромся змінити на твій улюблений підграф):
...
Зверни увагу, що зв'язність вибраного макету кубітів не обов'язково є лінійною і може покривати великі області пристрою Heron залежно від вибраної кількості кубітів.
...
Тепер ми використаємо зворотне поширення операторів (OBP). (Переглянь [документацію OBP](https://qiskit.github.io/qiskit-addon-obp/) для отримання більш детальної інформації про додаток Qiskit OBP.) Ми створимо функцію, яка генерує зрізи схеми для зворотного поширення:
...
Тепер, коли ми маємо нашу зменшену схему та розширені спостережувані, давай проведемо оцінку часу для схеми після зворотного поширення:
...
Дев'ятий крок має велику статистичну похибку, оскільки ми обмежили час QPU до 5 хвилин. Якщо ти запустиш цей крок на 15 хвилин (як рекомендує емпірична оцінка часу), ти отримаєш меншу похибку. Відповідно, скориговане значення стане ближчим до ідеального.
</Admonition>
