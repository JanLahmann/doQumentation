...
Цей посібник демонструє, як реалізувати алгоритм крилівської квантової діагоналізації (KQD) в контексті патернів Qiskit. Ти спочатку дізнаєшся про теорію, що лежить в основі алгоритму, а потім побачиш демонстрацію його виконання на QPU.
...
Дивись Додаток для детального виведення того, як унітарний простір Крилова дозволяє точно представляти власні стани з низькою енергією.
...
І встанови інші параметри алгоритму. Для цілей цього посібника ми обмежимося використанням простору Крилова лише з п'ятьма вимірами, що є досить обмежуючим.
...
Вибери опорний стан $\vert \psi \rangle$, який має деяке перекриття з основним станом. Для цього гамільтоніана ми використовуємо стан із збудженням у середньому кубіті $\vert 00..010...00 \rangle$ як наш опорний стан.
...
Спочатку обчисли фазу, накопичену станом $\vert 0 \rangle$ під час неконтрольованої часової еволюції
...
для деякого часового кроку $dt$, який ми визначимо пізніше. Тимчасово припустимо, що $r$ є парним: тоді визначимо $d=r/2$. Зверни увагу, що коли ми проєктуємо гамільтоніан у простір Крилова вище, він є нерозрізненним від простору Крилова
...
Ми наводимо доведення нижче, але його можна сміливо пропустити, якщо ти не хочеш зрозуміти повний, строгий аргумент. Зараз ми зосередимося на наслідках наведеного вище твердження. З властивості 3 вище ми можемо побачити, що зміщений простір Крилова вище містить стан $f(H)|\psi\rangle$. Це наш стан з низькою енергією. Щоб зрозуміти чому, запишемо $|\psi\rangle$ в базисі власних енергій:
...
Щоб спростити те, що залишилося, зверни увагу, що для всіх цих $E_k$, за визначенням $f$, ми знаємо, що $f(E_k)^2 \le 4\left(1 + \delta\right)^{-2d}$. Додатково обмежуючи зверху $E_k-E_0<2\|H\|$ і обмежуючи зверху $\sum_{E_k>E_0+\delta}|\gamma_k|^2<1$, отримуємо

$$
\text{energy error} \le \delta + \frac{8}{|\gamma_0|^2}\|H\|\left(1 + \delta\right)^{-2d}.
$$

Це справедливо для будь-якого $\delta>0$, тому якщо ми встановимо $\delta$ рівним нашій цільовій похибці, то верхня межа похибки вище збігається до неї експоненційно з розмірністю Крилова $2d=r$. Також зверни увагу, що якщо $\delta<E_1-E_0$, то доданок $\delta$ фактично повністю зникає у наведеній вище межі.
...
Зверни увагу, що $g(E_0)=0$ і $g(E)$ зростає, коли $E$ віддаляється від $E_0$.
...
Будь ласка, пройди це коротке опитування, щоб надати відгук про цей підручник. Твої ідеї допоможуть нам покращити наш контент і користувацький досвід.
