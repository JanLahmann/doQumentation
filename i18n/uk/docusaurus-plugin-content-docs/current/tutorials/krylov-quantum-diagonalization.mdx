---
description: 'Вивчи крилівську квантову діагоналізацію для решіткових гамільтоніанів з використанням концепції функції гамільтоніана.'
...
*Оцінка використання: 70 хвилин на процесорі Heron або Nighthawk (ПРИМІТКА: Це лише оцінка. Фактичний час виконання може відрізнятися.)*

## Результати навчання {#learning-outcomes}
Завершивши цей підручник, ти зможеш зрозуміти наступну інформацію:
- Як інтерпретувати крилівську квантову діагоналізацію (KQD) як навчання скінченної функції гамільтоніана, яка діє як спектральний фільтр.
- Як побудувати проєктований гамільтоніан і матриці перекриття за допомогою розширених вимірювань swap-тесту.
- Як розв'язати отриману узагальнену задачу власних значень (GEVP) і відновити оцінку енергії основного стану для решіткового гамільтоніана.

## Передумови {#prerequisites}
Рекомендується ознайомитися з такими темами:
- [Примітиви Qiskit](/guides/primitives)
- [Оцінка енергії основного стану ланцюга Гейзенберга за допомогою VQE](/tutorials/spin-chain-vqe)
- [Курс алгоритмів квантової діагоналізації](/learning/courses/quantum-diagonalization-algorithms/krylov)

## Передумови {#background}

Цей посібник демонструє, як реалізувати алгоритм крилівської квантової діагоналізації (KQD) в контексті патернів Qiskit.
Ти спочатку вивчиш теорію, що лежить в основі алгоритму, а потім побачиш демонстрацію його виконання на QPU.

Оцінка низькоенергетичних властивостей багаточастинкових гамільтоніанів є центральним завданням у квантовій симуляції.
Наприклад, енергії основного стану та низьколежачі збудження безпосередньо пов'язані з хімічною стабільністю, магнітним впорядкуванням, квантовими фазовими переходами та відгуком матеріалу.
На класичному комп'ютері розмірність гільбертового простору зростає експоненційно з кількістю орбіталей чи спінів, тож пряма діагоналізація швидко стає непрактичною.

Існує кілька квантово-обчислювальних підходів до цієї проблеми.
Близькотермінові варіаційні методи, такі як [варіаційний квантовий розв'язувач власних значень (VQE)](/tutorials/spin-chain-vqe), використовують відносно неглибокі параметризовані схеми, але вони вимагають нелінійного циклу класичної оптимізації з багатьма обчисленнями квантової схеми.
На іншому кінці, [квантова оцінка фази (QPE)](https://arxiv.org/abs/quant-ph/0604193) забезпечує більш прямий шлях до оцінки власних значень з суворими гарантіями, але стандартна QPE вимагає довгих когерентних схем і в основному підходить для відмовостійких квантових комп'ютерів.
KQD перебуває між цими двома підходами: вона використовує еволюцію гамільтоніана в реальному часі, як в алгоритмах на основі оцінки фази, але замінює повну оцінку фази компактною проєктованою задачею власних значень, яку можна розв'язати класично.

Розглянь $n$-кубітний гамільтоніан $H$ та референсний стан $\lvert \psi_{0}\rangle$.
Метод KQD будує крилівський підпростір з еволюціонованих у реальному часі станів,
...
де $r$ — крилівська розмірність, а $\Delta t$ — крок часу.
Будь-який стан у крилівському підпросторі тоді представляється як лінійна комбінація цих базисних станів,
...
За допомогою простої алгебри ми можемо побачити, що відповідна енергія записується як відношення Релея,
...
Тут матриці $\mathcal{S}$ та $\mathcal{H}$,
...
визначають проєктовані матриці перекриття та гамільтоніана.
Їхні елементи оцінюються за допомогою вимірювань квантової схеми.

Ми прагнемо знайти коефіцієнт $\mathbf{c}$, який дає мінімум $E(\mathbf{c})$:
...
За теоремою Релея-Рітца, ця мінімізація еквівалентна розв'язанню узагальненої задачі власних значень (GEVP),
...
Зауваж, що розмірність $r$ може бути достатньо малою, щоб класичний комп'ютер міг розв'язати GEVP.

Це той самий варіаційний принцип, що використовується в класичній діагоналізації підпростору, але тут базисні стани генеруються квантовою еволюцією в часі.
Порівняно з VQE, KQD зазвичай вимагає глибших схем, оскільки вона покладається на еволюцію в реальному часі.
Взамін, KQD уникає нелінійної оптимізації параметрів та ітеративного виконання на квантовому апаратному забезпеченні, і вона систематично покращується зі збільшенням проєктованого підпростору.
Алгоритм був продемонстрований у великому масштабі на існуючому квантовому апаратному забезпеченні [\[2\]](#references), і його продуктивність можна аналізувати з доказовими гарантіями [\[1\]](#references).

## Вимоги {#requirements}

Перед початком цього посібника переконайся, що у тебе встановлено наступне:

- Qiskit SDK v2.3 або новіше, з підтримкою [візуалізації](https://docs.quantum.ibm.com/api/qiskit/visualization)
...
Виконання на апаратному забезпеченні вимагає `qiskit-ibm-runtime` та доступу до облікового запису IBM Quantum&reg;.

## Налаштування {#setup}

Комірка налаштування імпортує необхідні модулі та визначає допоміжні функції для робочого процесу:

1. побудувати гамільтоніан Гайзенберга;

2. розв'язати порогову GEVP;

3. обчислити навчений крилівський фільтр;

4. перетворити значення фільтра на спектральні ваги;

5. побудувати графіки еталонного та відфільтрованого розподілів енергії.
...
У першій частині цього туторіалу ми демонструємо метод KQD, використовуючи локальний симулятор вектора стану. Далі ми використовуємо реальний квантовий бекенд для розв'язання задачі корисного масштабу.

Ми також визначаємо фейковий бекенд, щоб продемонструвати транспіляцію, специфічну для бекенду, та перевірити отриману схему.
...
## Приклад малого масштабу з симулятором {#small-scale-simulator-example}

### Крок 1: Відображення класичних вхідних даних на квантову задачу {#step-1-map-classical-inputs-to-a-quantum-problem}

#### Гамільтоніан і еталонний стан {#hamiltonian-and-reference-state}
У цьому прикладі використовується ланцюжок Гайзенберга з відкритими межами на 12 кубітів ($n=12$),
...
з добутковим станом з одним збудженням
...
як еталонний стан.
Оскільки визначений вище гамільтоніан Гайзенберга зберігає загальну кількість збуджень, еталонний стан залишається у підпросторі з одним збудженням, розмірність якого зростає лише лінійно з кількістю кубітів. Тому ми можемо ефективно обчислити точну енергію основного стану, діагоналізувавши гамільтоніан, обмежений цим підпростором, і використати це виключно як діагностичний орієнтир для оцінки KQD. Сам робочий процес KQD оцінює проєктовані матричні елементи за допомогою примітивів Qiskit і розв'язує отриману проєктовану задачу класично.
...
#### Встановлення параметрів алгоритму {#set-the-parameters-for-the-algorithm}

На основі верхніх меж норми гамільтоніана, посилання [\[1\]](#references) евристично пропонує часовий крок $\Delta t$ як $\pi/\|H\|$.
Оскільки спектральну норму $\|H\|$ важко обчислити, ми натомість використовуємо її верхню межу:
...
Ми встановлюємо крилівську розмірність $r=10$ і кількість кроків Троттера на часовий крок $5$: достатньо велике крилівське підпростір, щоб розв'язати нижню частину спектра, зберігаючи при цьому доступність найглибшої схеми ($t_{\max}=(r-1)\Delta t$), а також достатню кількість кроків Троттера, щоб зберегти малою похибку дискретизації на цій найглибшій схемі.
...
#### Побудова схеми {#build-circuit}

Тут ми будуємо схеми для оцінки матричних елементів $\mathcal{H}_{k\ell}$ і $\mathcal{S}_{k\ell}$.
Оскільки всі степені $H$ комутують, маємо
$$
\begin{equation*}
\mathcal{H}_{k\ell}
=
...
Тут ми представляємо схему, яка називається **extended-swap-test**, яка готує
$$
\begin{equation*}
|\Phi_{0d}\rangle=
...
\end{equation*}
$$
де $|\psi_d\rangle=e^{-id\Delta tH}|\psi_{0}\rangle$.

#### Еталонний стан {#reference-state}

Ми готуємо еталонний стан $|\psi_0\rangle$.
...
Реалізуйте оператор часової еволюції, згенерований гамільтоніаном, наближений простою [лі-троттеризацією](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter).
...
#### Розширений тест на обмін [\[3\]](#references) {#extended-swap-test-circuit-3references}

Схема спочатку готує еталонний стан на системному регістрі, тоді як анцила залишається у стані $|0\rangle$:
...
Потім застосування вентиля Адамара до анцили створює когерентну суперпозицію двох гілок:
...
Нарешті, застосовується керований вентиль часової еволюції:
...
лише коли анцила перебуває у гілці $|1\rangle$. Отже,
...
У наступному блоці коду ми реалізуємо:
$$
\begin{equation*}
|\Psi(t)\rangle = \frac{|0\rangle|\psi_0\rangle + |1\rangle U(t)|\psi_0\rangle}{\sqrt{2}},
\end{equation*}
$$
яке буде присвоєне як $t=d\Delta t$ для $d=1,\cdots r-1$ на етапі виконання.
...
Для будь-якої ермітової системної спостережуваної величини $O$, тут ми встановлюємо спостережувані величини для обчислення:
...
Це тому, що $O=I$ дає елемент перекриття $\mathcal{S}_{0d}=\langle\psi_0|\psi_d\rangle$, тоді як $O=H$ дає елемент гамільтоніана $\mathcal{H}_{0d}=\langle\psi_0|H|\psi_d\rangle$.

Використовуючи $X=|0\rangle\langle 1|+|1\rangle\langle 0|$, маємо
...
Аналогічно, використовуючи $Y=-i|0\rangle\langle 1|+i|1\rangle\langle 0|$,
...
Нарешті, для кожного стану $|\Psi_{0d}\rangle$ нам потрібно виміряти:
$$
\begin{align*}
...
Для оцінки матричних елементів $\mathcal{H}$ кількість членів Паулі значно більша, ніж для матричних елементів $\mathcal{S}$.

Тепер ми можемо зменшити кількість вимірюваних членів гамільтоніана, використовуючи техніку зсуву [\[4\]](#references).
Ми розбиваємо гамільтоніан як:
...
де $T$ вибрано так, щоб еталонний стан був його власним станом,
...
— це елемент зсунутої матриці гамільтоніана.
Отже, нам потрібно виміряти лише $X\otimes (H-T)$ та $Y\otimes (H-T)$.
Внесок від $T$ відновлюється класично, використовуючи вже виміряний елемент матриці перекриття $\mathcal{S}_{0d}$.

У цьому прикладі природним вибором є діагональна частина гамільтоніана Гайзенберга,
...
Оскільки еталонний стан є станом обчислювального базису, він є власним станом кожного члена $Z_iZ_{i+1}$.

Однак вигідніший вибір — включити не лише діагональні члени $ZZ$, а й члени $XX+YY$, які анулюють еталонний стан.

Для кожної сусідньої пари оператор $XX+YY$ задовольняє
...
Отже, член $XX+YY$ дає внесок лише тоді, коли два сусідні кубіти мають різні заповнення в еталонному бітовому рядку.
Якщо обидва кубіти дорівнюють $0$ або обидва $1$, член анулює еталонний стан і також може бути винесений за дужки.

Нехай $|\psi_{\rm ref}\rangle = |z_0\cdots z_{n-1}\rangle$, де $z_i \in \{0,1\}$.
Таким чином, ми можемо вибрати
$$
\begin{equation*}
...
Цей оператор усе ще задовольняє
...
оскільки члени $ZZ$ діють діагонально на $|\psi_0\rangle$, тоді як зсунуті члени $XX+YY$ дають нуль.
Відповідне власне значення визначається, отже, лише членами $ZZ$,
$$
\begin{equation*}
\tau = \sum_{i=0}^{n-2}
(-1)^{z_i}
...
З цим вибором зсунутий гамільтоніан стає:
$$
\begin{equation*}
H-T = \sum_{z_i\ne z_{i+1}}
\left(
...
Внаслідок цього потрібно вимірювати лише ребра з різними заповненнями в еталонному стані.
Усі члени $ZZ$ і всі неактивні члени $XX+YY$ відновлюються через внесок перекриття $\tau \mathcal{S}_{0d}$, або дають нульовий внесок за побудовою.

Це дає меншу спостережувану величину, ніж зсув лише діагональної частини.
Зокрема, для еталонного стану обчислювального базису з локалізованим збудженням у $H-T$ залишаються лише ребра, суміжні зі збудженням.
Отже, кількість членів Паулі в $X\otimes(H-T)$ та $Y\otimes(H-T)$ можна суттєво зменшити, тоді як відновлений матричний елемент
$$
\begin{equation*}
\widetilde{\mathcal{H}}_{0d}
+
...
Тепер ми перетворюємо абстрактну схему extended-swap-test на шаблон, орієнтований на апаратне забезпечення.
Перед цим ми спочатку додатково оптимізуємо схему на абстрактному рівні.

#### Порівняння порядку членів гамільтоніана {#compare-hamiltonian-term-ordering}

Спочатку ми порівнюємо різні впорядкування членів Паулі в гамільтоніані Гайзенберга для симуляції гамільтоніана.
Сам гамільтоніан не змінюється, але порядок впливає на те, як генерується схема добуткової формули і наскільки структуру можна розпаралелити у схемі.
Наприклад, наївне впорядкування перераховує всі найближчі сусідні члени $XX$, потім усі члени $YY$, потім усі члени $ZZ$.
Це розміщує суміжні ребра, такі як $(0,1)$ і $(1,2)$, поруч одне з одним, тому їх не можна виконати паралельно.
Впорядкування "спочатку парні, потім непарні" відвідує спочатку непересічні парні ребра, а потім непарні, що відкриває паралельні двокубітні шари.
Впорядкування з групуванням парних-непарних ребер йде на крок далі: для кожного ребра воно тримає разом локальні члени $XX$, $YY$ і $ZZ$, все ще відвідуючи парні ребра перед непарними.
Ми очікуємо, що впорядкування "спочатку парні, потім непарні" та впорядкування з групуванням парних-непарних ребер зменшать глибину схеми, відкриваючи паралельні двокубітні шари, а впорядкування з групуванням ребер додатково зменшить похибку Троттера, оскільки локальна двокубітна взаємодія на тому самому ребрі обробляється як компактний блок.

![Pauli evolutions with different term ordering](/docs/images/tutorials/krylov-quantum-diagonalization/ham_ordering.avif)

Порядок гамільтоніана також впливає на похибку Троттера.
Якщо ми розміщуємо некомутуючі члени поруч один з одним, зміни базису відбуваються частіше, що спричиняє більшу похибку Троттера.
Групуючи члени, що вимагають однакового перетворення базису Паулі, можна уникнути надлишкових змін базису.

Тут порівняння використовує найбільший час еволюції, який з'являється в оцінках крилівського простору першого рядка,
$t_{\max} = (r-1)\Delta t,$
використовуючи ту саму умову транспіляції.

Щоб виміряти похибку Троттера, ми використовуємо **невірність процесу** між троттеризованою схемою та точною еволюцією гамільтоніана,
$$
\begin{equation*}
\text{Infidelity}(U,V)
=
...
Ця діагностика використовує щільні матриці, тому вона підходить для цього малого прикладу на 12 кубітів, але не призначена як масштабована підпрограма.
...
Спочатку ми зберігаємо правило синтезу фіксованим на одному кроці Троттера першого порядку і змінюємо лише порядок членів Паулі.
Мета цього порівняння полягає головним чином у тому, щоб побачити, наскільки можна зменшити глибину схеми та вартість двокубітних операцій, відкриваючи транспілятору непересічні найближчі сусідні ребра.
...
При `reps=1` ми спостерігаємо, що обидва впорядкування — "спочатку парні, потім непарні" та з групуванням парних-непарних ребер — зменшують глибину з 15 до 6, відкриваючи паралельні двокубітні шари, тоді як кількість двокубітних вентилів залишається однаковою для всіх трьох впорядкувань.
Однак усі три впорядкування мають невірність, близьку до 1, тому ми проходимо по кількості повторень Троттера, щоб чіткіше розрізнити впорядкування.
Із зростанням кількості повторень невірність впорядкування `even-odd edge-grouped` спадає швидше за інші два, досягаючи 0.045 при `reps=8` проти 0.075 для наївного та впорядкування "спочатку парні, потім непарні".

#### Порівняння синтезу добуткової формули {#compare-product-formula-synthesis}

Далі ми досліджуємо різні розширені налаштування троттеризації, зафіксувавши порядок гамільтоніана на впорядкуванні з групуванням парних-непарних ребер.
Ми розглядаємо Лі-Троттер першого порядку, Судзукі-Троттер другого порядку та Судзукі-Троттер четвертого порядку.
...
Лі-Троттер дає найменш глибоку схему, але має найбільшу похибку, тоді як Судзукі-Троттер четвертого порядку точніший, але збільшує глибину схеми.
У решті туторіалу ми обираємо Судзукі-Троттер другого порядку, оскільки він дає схему малої глибини, суттєво зменшуючи похибку Троттера порівняно з формулою першого порядку.

#### Видалення керованого вентиля часової еволюції {#remove-the-controlled-time-evolution-gate}

У розширеному тесті на обмін керування часовою еволюцією за допомогою одного кубіта-анцили вимагає, щоб анцила керувала багатьма вентилями по всій системі.
Це може призвести до значних накладних витрат на маршрутизацію і в найгіршому випадку фактично вимагати зв'язності "всі до одного".
Щоб уникнути цього, можлива подальша оптимізація шляхом заміни керованого вентиля часової еволюції на версію без керування, використовуючи симетрію гамільтоніана.
Розглянемо наступну схему.

![circuit optimization](/docs/images/tutorials/krylov-quantum-diagonalization/circuit_opt.avif)

Тут $B_{\rm ref}$ готує еталонний стан, $B_{\rm ref}|0^n\rangle = |\psi_{0}\rangle$.

Замість того, щоб спочатку готувати $\frac{1}{\sqrt{2}}(|0\rangle+|1\rangle)|\psi_0\rangle$, а потім застосовувати $U(t)$ лише на гілці $|1\rangle$, схема безпосередньо готує дві гілки як
...
Схема спочатку застосовує вентиль Адамара до анцили та готує еталонний стан лише на гілці $|1\rangle$:
...
Потім некерований оператор часової еволюції застосовується до обох гілок:
...
Оскільки гамільтоніан зберігає кількість збуджень, ми бачимо, що $|0^n\rangle$ є його власним станом, і, отже,
оператор еволюції накопичує лише фазу під дією гамільтоніана:
$$
\begin{equation*}
U(t)|0^n\rangle
=
...
Далі $B_{\rm ref}$ застосовується лише на гілці $|0\rangle$.
...
На цьому етапі дві гілки мають додаткову відносну фазу. Щоб видалити її, ми застосовуємо вентиль фази анцили
...
Отже, з точністю до неважливої глобальної фази, ми нарешті підготували
...
У наступному блоці коду ми реалізуємо цю схему без керування.
...
Тепер ми транспілюємо керовану та безкеровану схеми, щоб вони стали виконуваними на апаратному забезпеченні.
Порівняймо результат транспільованих схем.
...
Наступний крок — надіслати ту саму параметризовану схему для кількох значень $d$. Для кожного $d$ ми оцінюємо чотири очікувані значення: $X\otimes I$, $Y\otimes I$, $X\otimes (H-T)$ та $Y\otimes (H-T)$. Ці чотири числа потім комбінуються в комплексні елементи першого рядка $\mathcal{S}_{0d}$ та $\mathcal{H}_{0d}$.

Тут $\mathcal{S}_{00}=1$ та $\mathcal{H}_{00}=\langle\psi_{0}|(H-T)|\psi_{0}\rangle=\langle\psi_{0}|H|\psi_{0}\rangle-\tau$ можна обчислити класично, оскільки $\lvert\psi_0\rangle$ розріджений, тому ми пропускаємо випадок $d=0$.
...
Після оцінки проєктованих матриць ми регуляризуємо і розв'язуємо GEVP
$$
\begin{equation*}
\mathcal{H}\mathbf{c} = E\mathcal{S}\mathbf{c}.
\end{equation*}
$$
Найменше узагальнене власне значення дає оцінку KQD енергії основного стану.
...
Тепер ми розв'язуємо задачу узагальнених власних значень, використовуючи матриці, реконструйовані з оцінок схеми.
В ідеальному розрахунку вектора стану з точною еволюцією в реальному часі це має відтворити точний проєктований результат.
На практиці відхилення можуть виникати через троттеризацію, похибку вибірки та числову нестабільність матриці перекриття.

Ми спостерігаємо, як енергія збігається, коли ми збільшуємо розмірність крилівського підпростору.
...
## Приклад великого масштабу з апаратним забезпеченням {#large-scale-hardware-example}

У попередньому розділі використовувалася модель на 12 кубітів, щоб можна було використати симуляцію вектора стану як діагностику. Тепер ми масштабуємо той самий робочий процес KQD до ланцюжка Гайзенберга на 30 кубітів і готуємо навантаження для виконання на апаратному забезпеченні IBM Quantum.

### Кроки 1-4, стиснуті в один блок коду {#steps-1-4-compressed-into-a-single-code-block}

Тепер ми об'єднуємо всі ці деталі в єдиний робочий процес більшого масштабу, який потім запускається на нашому реальному квантовому апаратному забезпеченні. У цьому розділі ми застосовуємо реалістичні налаштування пом'якшення похибок для покращення надійності результатів. Оскільки матричні елементи, що відповідають різним значенням $d$, можна обчислювати паралельно, ми використовуємо режим `Batch` для їх ефективного виконання.
...
## Додаток: погляд з точки зору функції гамільтоніана (спектрального фільтра) {#appendix-the-hamiltonian-function-spectral-filter-viewpoint}

Основний робочий процес представив KQD операційно: побудувати крилівський базис зі станів, еволюціонованих у реальному часі, оцінити проєктовані матриці $\mathcal{H}$ та $\mathcal{S}$ і розв'язати GEVP.
Цей додаток переглядає *ту саму* обчислення з додаткового кута, який пояснює, **чому** KQD працює: погляд з точки зору *функції гамільтоніана*, або *спектрального фільтра* [\[3\]](#references), [\[5\]](#references).
Він повторно використовує модель на 12 кубітів, часовий крок $\Delta t$ і крилівський розв'язок, уже отриманий вище; нове виконання схеми не потрібне.

### Еталонний стан як розподіл енергії {#the-reference-state-as-an-energy-distribution}

Нехай гамільтоніан має власний розклад
...
з власними станами енергії $|E_m\rangle$.
Будь-який еталонний стан можна розкласти в цьому власному базисі,
...
тому він несе **спектральну вагу** $p_m=|a_m|^2$ на кожній енергії $E_m$.
Еталонна енергія — це середнє значення цього розподілу, $\langle H\rangle_{0}=\sum_m p_m E_m$.

Власний розклад загального гамільтоніана з $n$ кубітів експоненційно дорогий, тому ця картина є лише діагностичною і *не* є частиною алгоритму.
Тут, однак, ми можемо обчислити її дешево для *тієї самої* задачі $n=12$: гамільтоніан Гайзенберга зберігає загальну кількість збуджень, а еталон $|000001000000\rangle$ несе одне збудження, тому весь його спектральний вміст міститься в підпросторі з одним збудженням, розмірність якого зростає лише лінійно з $n$.
Тому ми повторно використовуємо точний блок з одним збудженням (уже використаний вище як орієнтир) і зчитуємо еталонний розподіл $\{(E_m, p_m)\}$ у цьому підпросторі.
...
### KQD навчає фільтр, який перетворює цей розподіл {#kqd-learns-a-filter-that-reshapes-this-distribution}

**Функція гамільтоніана** $f(H)$ визначається через спектральне числення,
...
або, іншими словами, зважена сума власних проєкторів.
Застосування її до еталона перетворює кожну спектральну амплітуду, $a_m \to a_m f(E_m)$:
...
Якби $f$ була різко піковою на найнижчій енергії ($f(E_0)=1$ і $f(E_m)\approx 0$ в інших випадках), то $f(H)$ діяла б як проєктор основного стану, а нормований результат був би (близьким до) основного стану.
Отже, хороший низькочастотний **спектральний фільтр** за енергією — це саме те, що нам потрібно.

KQD не задає $f$ наперед. Натомість він розкладає фільтр у базисі еволюції в реальному часі,
...
*тригонометрична* функція енергії, коефіцієнти якої $\{c_\ell\}$ є саме тим власним вектором GEVP, який ми розв'язали вище.
Мінімізація коефіцієнта Релея $\mathbf{c}^\dagger\mathcal{H}\mathbf{c}/\mathbf{c}^\dagger\mathcal{S}\mathbf{c}$, отже, є тим самим, що й *навчання* фільтра, який найкраще пригнічує вагу збудженого стану еталона.
Більша крилівська розмірність $r$ дає фільтру більше ступенів свободи та гостріший пік на енергії основного стану.

Допоміжна функція нижче обчислює цей навчений фільтр на осі енергії; потім ми застосовуємо його до еталонного розподілу, отриманого вище.
...
### Візуалізація фільтра та його гнучкості {#visualize-the-filter-and-its-flexibility}

Спочатку ми показуємо навчений фільтр при повній крилівській розмірності, використаній вище, а потім простежуємо, як він загострюється зі зростанням розмірності $r$.

Стовпчики показують еталонні спектральні ваги $p_m$ (до) та відфільтровані ваги $p_m|f_{\rm KQD}(E_m)|^2$ (після), разом з інтенсивністю навченого фільтра $|f_{\rm KQD}(E)|^2$ на неперервній осі енергії.
Фільтр концентрує вагу на найнижчій енергії підпростору з одним збудженням &mdash; тій самій енергії, до якої збіглася оцінка KQD у прикладі малого масштабу.
Зауважте, що це основний стан *в межах сектора з одним збудженням*, який є відповідною ціллю для цього еталонного стану, що зберігає збудження, а не глобальний основний стан.
...
### Збільшення крилівської розмірності: гнучкість навченої функції {#increasing-the-krylov-dimension-flexibility-of-the-learned-function}

Пригадаймо, що навчений фільтр — це тригонометричний многочлен від енергії з $r$ коефіцієнтами,
...
Крилівська розмірність $r$ — це точно кількість вільних коефіцієнтів, тому вона контролює **гнучкість** функції.
Малий $r$ може виробити лише широкий, плавно змінюваний фільтр, який пропускає вагу в низьколежачі збуджені стани; коли $r$ зростає, фільтр може сформувати вужчий пік на цільовій енергії та агресивніше пригнічувати решту ваги збудженого стану.
Це спектрально-фільтровий аналог збіжності енергії, що спостерігається в прикладі малого масштабу: зі зростанням $r$ відфільтрований розподіл стягується до основного стану підпростору, а оцінена енергія зменшується до нього.

Ми повторно використовуємо проєктовані матриці, вже оцінені вище, і просто розв'язуємо GEVP на кожному провідному блоці $r\times r$, а потім обчислюємо та будуємо графік відповідного фільтра.
...
Якщо ви вважаєте цю роботу цікавою, вас можуть зацікавити такі матеріали:
<Admonition type="tip" title="Рекомендації">
- [Крилівська квантова діагоналізація фермінної ґраткової моделі на основі вибірки](/tutorials/sample-based-krylov-quantum-diagonalization)
- [Оцінка енергії основного стану ланцюжка Гайзенберга з VQE](/tutorials/spin-chain-vqe)
- [Курс з алгоритмів квантової діагоналізації](/learning/courses/quantum-diagonalization-algorithms/krylov)
</Admonition>

## Посилання {#references}

[1] E. N. Epperly, L. Lin та Y. Nakatsukasa, *A theory of quantum subspace diagonalization*, *SIAM Journal on Matrix Analysis and Applications* **43**, 1263-1290 (2022).

[2] N. Yoshioka, M. Amico, W. Kirby та ін., *Diagonalization of large many-body Hamiltonians on a quantum processor*, [arXiv:2407.14431](https://arxiv.org/abs/2407.14431) (2024).
