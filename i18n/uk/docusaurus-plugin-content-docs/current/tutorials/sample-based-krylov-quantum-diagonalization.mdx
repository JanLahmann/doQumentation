...
*Оцінка використання: дев'ять секунд на процесорі Heron r2 (ПРИМІТКА: Це лише оцінка. Твій час виконання може відрізнятися.)*
## Результати навчання {#learning-outcomes}
Після проходження цього підручника користувачі повинні розуміти:
- Як використовувати [SQD Qiskit addon](https://github.com/Qiskit/qiskit-addon-sqd) для апроксимації енергії основного стану ґраткової моделі за допомогою бітових рядків, відібраних із квантового процесора (QPU).
- Як використовувати [ffsim](https://github.com/qiskit-community/ffsim) для побудови схем часової еволюції для ферміонного моделювання.
- Як об'єднувати вибірки з кількох схем для постобробки за допомогою алгоритму квантової діагоналізації Крилова на основі вибірок (SKQD).

## Передумови {#prerequisites}
Рекомендуємо ознайомитися з такими темами перед проходженням цього підручника:
- [Квантова діагоналізація на основі вибірок для хімічного гамільтоніана](/tutorials/sample-based-quantum-diagonalization)
- [Квантова діагоналізація Крилова для ґраткових гамільтоніанів](/tutorials/krylov-quantum-diagonalization)
...
Зверни увагу, що гамільтоніан є конкретним випадком загального гамільтоніана взаємодіючих електронів,
...
- ffsim v0.0.72 або новіший (`pip install ffsim`)
...
Для більш детального опису цієї моделі та способу проєктування схем звернись до ["Quantum-Centric Algorithm for Sample-Based Krylov Diagonalization"](https://arxiv.org/abs/2501.09702).
...
Далі ми оптимізуємо схему для цільового апаратного забезпечення. Наразі ми створимо загальний бекенд із заданою кількістю кубітів та набором вентилів, до яких схеми часової еволюції природно розкладаються.
...
Тепер ми запускаємо алгоритм SQD за допомогою функції `diagonalize_fermionic_hamiltonian`. Ознайомся з [документацією API](https://qiskit.github.io/qiskit-addon-sqd/apidocs/qiskit_addon_sqd.fermion.html#qiskit_addon_sqd.fermion.diagonalize_fermionic_hamiltonian) для пояснення аргументів цієї функції.
...
### Перевір енергію {#verify-the-energy}
...
Тепер ми запускаємо більш масштабний приклад на реальному QPU.
Для еталонної енергії ми використовуємо результати обчислення [DMRG](https://en.wikipedia.org/wiki/Density_matrix_renormalization_group), яке було виконано окремо.
...
![Вивід попередньої комірки коду](/docs/images/tutorials/sample-based-krylov-quantum-diagonalization/extracted-outputs/933037d8-847e-4986-80da-5ac8d677b2ff-1.avif)

## Наступні кроки {#next-steps}
<Admonition type="tip" title="Recommendations">
Якщо ця робота тебе зацікавила, тебе можуть зацікавити наступні матеріали:
- [Квантова діагоналізація на основі вибірок для хімічного гамільтоніана](/tutorials/sample-based-quantum-diagonalization) — пов'язаний підручник із використанням евристичного варіаційного анзацу замість схем Троттера
- [Квантова діагоналізація Крилова для ґраткових гамільтоніанів](/tutorials/krylov-quantum-diagonalization) — підручник з методу KQD
- [Документація SQD addon API](https://qiskit.github.io/qiskit-addon-sqd/apidocs/qiskit_addon_sqd.fermion.html#qiskit_addon_sqd.fermion.diagonalize_fermionic_hamiltonian) — довідник для функції `diagonalize_fermionic_hamiltonian`
- [*Quantum-Centric Algorithm for Sample-Based Krylov Diagonalization*](https://arxiv.org/abs/2501.09702) — стаття, на якій базується цей підручник
</Admonition>
