...
Станом на **липень 2025 року**, **Сервіс Транспілятора** був перенесений на нову платформу IBM Quantum&reg; і більше не доступний. Щоб отримати останні оновлення про статус Сервісу Транспілятора, звернись до [документації сервісу транспілятора](/guides/qiskit-transpiler-service). Ти все ще можеш використовувати AI транспілятор локально, подібно до стандартної транспіляції Qiskit. Просто заміни `generate_preset_pass_manager()` на `generate_ai_pass_manager()`. Ця функція створює менеджер проходів, який інтегрує AI-керовані проходи маршрутизації та синтезу безпосередньо у твій локальний робочий процес транспіляції.
...
Для більш детальної інформації про доступні AI проходи, дивись [документацію AI проходів](/guides/ai-transpiler-passes).
...
Тепер давай подивимося, як використовувати сервіс AI транспілятора з простою квантовою схемою, використовуючи патерни Qiskit. Ключовим моментом є створення `PassManager` за допомогою `generate_ai_pass_manager()` замість стандартного `generate_preset_pass_manager()`.
...
Транспілюй схеми та зареєструй час.
...
Важливо відзначити, що ці результати базуються лише на одній схемі. Щоб отримати всебічне розуміння того, як AI транспілятор порівнюється з традиційними методами, необхідно протестувати різноманітні схеми. Продуктивність QTS може значно варіюватися залежно від типу оптимізованої схеми. Для ширшого порівняння звернись до бенчмарків вище або відвідай [блог.](https://www.ibm.com/quantum/blog/qiskit-performance)
...
Для отримання додаткової інформації про методи синтезу в Qiskit звернись до [документації API Qiskit](https://docs.quantum.ibm.com/api/qiskit/synthesis).
Визнач карту зв'язування, що представляє 27-кубітний QPU.
...
Транспілюй кожну зі схем перестановок з використанням проходів синтезу ШІ та загальних методів синтезу.
...
Запиши метрики (глибину, кількість вентилів, час) для кожної схеми після транспіляції.
...
Побудуй графіки результатів для порівняння продуктивності проходів синтезу ШІ з загальними методами синтезу.
...
Будь ласка, пройди це коротке опитування, щоб надати відгук про цей посібник. Твої думки допоможуть нам покращити наші пропозиції контенту та досвід користувачів.
