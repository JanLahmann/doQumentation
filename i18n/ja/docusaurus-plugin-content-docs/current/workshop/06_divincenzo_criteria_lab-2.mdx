...
まず、典型的なIBMバックエンドの基底ゲートが何であるかを見てみましょう。デバイス(またはそのフェイク版)の設定を照会します。例えば、ibmq_brisbaneの基底ゲートは次のとおりです。
遅延時間が長くなるにつれて、確率`P(qubit=1)`がT1緩和特有の指数関数的減衰曲線に従って減少することが観察されるはずです。これは、回路の実行時間が長すぎると、有限のコヒーレンス時間が計算エラーにつながることを直接示しています。

**アルゴリズムへの影響:** より長いアルゴリズム(多くの連続したゲートを持つもの)を試すと、合計実行時間がT2に近づくか、それを超えてしまい、終了前に状態がコヒーレンスを失う可能性があります。これが、コヒーレンス時間を改善し、ゲートを高速化することが、量子ハードウェア研究において最も重要な目標の2つである理由です。
...
これは`['id', 'rz', 'sx', 'x', 'ecr']`のような出力になるかもしれません。これらは、ハードウェアがネイティブにサポートする基本操作です(恒等/無操作、RZ回転、sqrt(X)ゲート、Xゲート、制御Xゲート)。それ以外のゲートは、これらから構成する必要があります。このセットは、量子コンピューティングにとって普遍的であることが知られています(基本的に、単一量子ビット回転ともつれを生成する2量子ビット・ゲートが普遍的なセットを形成します)。

では、**トフォリ(CCX)ゲート**をテストケースとして取り上げます。CCXは、2つの制御量子ビットが両方とも1である場合にのみ、ターゲット量子ビットを反転させます。これはIBMハードウェア上のネイティブ・ゲートではありません。Qiskitは`ccx`命令を提供していますが、内部ではこれを分解します。
...
これらはQiskitの標準的な使用範囲を超えており、Qiskitは主にチップ上の静止Qubitを扱います。ただし、**量子テレポーテーション**という簡単な例でこれらの基準の*概念*を説明できます。テレポーテーションは、静止Qubitの状態を、もつれたペア（「フライング」部分）が運ぶ情報と古典通信に変換し、それを使って別の場所の別の静止Qubitで状態を再構築することを示しています。
