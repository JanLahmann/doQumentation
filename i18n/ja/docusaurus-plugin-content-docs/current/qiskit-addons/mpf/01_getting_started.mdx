...
### 1b: マルチプロダクト公式 {#1b-multi-product-formulas}
...
ここで $x_j$ は重み係数、$\rho^{k_j}_j$ は $k_j$ 個のトロッターステップを含むプロダクト公式 $S^{k_j}$ で初期状態を発展させることにより得られる純粋状態に対応する密度行列、$j$ は MPF を構成するプロダクト公式の数のインデックスです。
...
行列 $A$ は $k_j$ だけでなく、プロダクト公式（PF）の選択、特にその _次数_ にも依存します。
