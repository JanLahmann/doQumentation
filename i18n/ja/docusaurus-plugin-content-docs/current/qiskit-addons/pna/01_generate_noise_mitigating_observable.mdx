...
![ノイズのある実験](/img/qiskit-addons/images/noisy_expt.png)
ノイズモデルを学習すれば、その逆を適用してノイズを除去できます。
![ノイズ軽減実験](/img/qiskit-addons/images/noise_mitigated_expt.png)
...
![PNA概要](/img/qiskit-addons/images/pna_overview.png)
...
![量子Circuitダイアグラム](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_1.png)
...
![量子Circuitダイアグラム](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_2.png)

### 2Qubit Gate層とMeasurementのTwirlおよびユニーク層の発見 {#twirl-the-2-qubit-gate-layers-and-measurements-and-find-unique-layers}

ここでは、pass manager がボックスに ``Twirl`` と ``InjectNoise`` のアノテーションを付けるようにします。これにより、circuit に影響を与えるノイズを学習し、そのノイズを対応する circuit レイヤーに関連付けることができます。

- ``enable_gates/enable_measure: True``: すべての2Q Gate層と終端Measurementをボックス化します。単一Qubit GateはBoxの中で左側にドレッシングされます。

- ``measure_annotations: all`` 測定ボックスに `Twirl` と `ChangeBasis` のアノテーションを含めます

- ``twirling_strategy: active``: もつれGateを含む各BoxのアクティブなすべてのQubitをTwirlします。

- ``inject_noise_targets: gates``：エンタングリング gate を含むすべての ``Twirl`` アノテーション付きボックスに ``InjectNoise`` アノテーションを追加する必要があります
...
![量子Circuitダイアグラム](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_3.png)
...
![量子Circuitダイアグラム](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_4.png)
...
ノイズを学習する前に、circuit 内のユニークな2-qubitレイヤーを見つける必要があります。これにより、circuit 全体のノイズを学習するために必要なショット数を最小限に抑えることができます。ボックス化された circuit から測定レイヤーを含むユニークなレイヤーを取得するために、``samplomatic`` の ``find_unique_box_instructions`` を使用します。これらは、ノイズ学習器に渡すレイヤーです。
...
![プロット出力](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_5.png)

#### Circuit のボックスを学習済みノイズに関連付ける {#associate-circuit-boxes-with-learned-noise}

ここでは、各ボックスの ``InjectNoise`` 参照IDと、そのボックス内のエンタングリング gate に影響する学習済みノイズモデル（`PauliLindbladMap`）との間のマッピングを作成します。
...
PNAを実行する際には、常にノイズのある circuit と observable を提供する必要があります。ノイズのある circuit が `InjectNoise` アノテーションを持つボックス化された circuit である場合、上記のステップで作成したマッピングを提供する必要があります。``qiskit-aer`` からの ``PauliLindbladError`` 命令を含む、ボックス化されていない circuit を渡すこともできます。その場合、``refs_to_noise_models`` を提供する必要はありません。主要な入力に加えて、ユーザーは以下を検討する必要があります。
...
![プロット出力](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_6.png)
...
- ``samplex_arguments``：samplexを完全に定義するために必要なバインディング
    ...
    - ``pauli_lindblad_maps``: ``noise_scales``が渡される場合は必須です。これは単に、ノイズレイヤーを関連するノイズモデルにマッピングするものです。

- ``shape``: ``samplex_arguments``によって定義される暗黙の形状を拡張するための形状タプルです。この拡張によって導入される非自明な軸は、ランダム化を列挙します。
