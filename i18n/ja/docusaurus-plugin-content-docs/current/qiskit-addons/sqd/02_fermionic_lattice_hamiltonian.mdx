...
まず、``7`` 個のバスサイト（``8`` 個の軌道に ``8`` 個の電子）を持つ1次元単一不純物Andersonモデル（SIAM）を記述する1体および2体のHamiltonianを作成します。このモデルは、金属に埋め込まれた磁性不純物を記述するために使用されます。
...
次に、トロッタライズされた量子Circuitのセットを使用して量子クリロフ部分空間を生成します。ここでは、初期（参照）状態を生成するためのヘルパーと、ハミルトニアンの1体および2体部分の時間発展のためのヘルパーを作成します。このモデルの詳細な説明とCircuitの設計方法については、[論文](https://arxiv.org/abs/2501.09702)を参照してください。
...
量子Krylov部分空間を指定する ``d`` 個の時間発展状態を生成します。ここでは、``d=8`` を選択しました。Krylov基底状態のサンプリングによる誤差は、``d`` を増加させることで収束します。この問題インスタンスの特殊性により、`OrbitalRotationJW` を使用した1体発展の効率的なコンパイルが可能ですが、一般的には、完全なHamiltonianのトロッター化された時間発展を実装するために、Qiskitの [PauliEvolutionGate](https://quantum.cloud.ibm.com/docs/api/qiskit/qiskit.circuit.library.PauliEvolutionGate) を使用できることに注意してください。
...
circuit をハードウェア実行のために最適化した後、ターゲットハードウェア上で実行し、基底状態エネルギー推定のためのサンプルを収集する準備が整いました。ここでは、``qiskit-ibm-runtime`` の ``SamplerV2`` を使用して、``ibm_sherbrooke`` backend から取得したノイズのあるサンプルをシミュレーションします。次に、各Krylov基底状態からのカウントを単一のカウント辞書に結合し、最も一般的にサンプリングされた上位20個のビット列をプロットします。
...
次に、`diagonalize_fermionic_hamiltonian`関数を使用してSQDアルゴリズムを実行します。この関数の引数の説明については、[APIドキュメント](../apidocs/qiskit_addon_sqd.fermion.rst#qiskit_addon_sqd.fermion.diagonalize_fermionic_hamiltonian)を参照してください。
...
この例は十分に小さいため、上記の出力文で示されているように、完全なHilbert空間を探索することができます。完全なHilbert空間の次元は ``(num_orbitals choose nelec_a) * (num_orbitals choose nelec_b)`` であることを思い出してください。したがって、この問題の場合：`(8 choose 4)**2 = 4900` です。部分空間の次元は、強化された配置回復により増加し、またSQDアルゴリズムが重要な配置を反復ごとに引き継ぐという事実にも起因します。最後の反復までに、私たちは完全なHilbert空間で対角化を行っています。
