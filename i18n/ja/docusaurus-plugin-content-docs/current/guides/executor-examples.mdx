---
description: 'qiskit-ibm-runtime での Executor プリミティブの実践的な使用例。'
...
Executorは `QuantumProgram` オブジェクトを実行します。各 `QuantumProgram` は複数の項目を含むことができます。この例では、実行用の回路項目とsamplex項目を追加します。詳細については、[Executorの入力と出力](/guides/executor-input-output)を参照してください。
...
### circuitの設定 {#set-up-the-circuit-1}
...
### circuitの実行 {#execute-the-circuits-1}

`Executor`は`QuantumProgram`オブジェクトを実行します。各`QuantumProgram`には、プログラムに追加される複数の*項目*を含めることができます。各項目は、プログラムが実行するタスクです。
