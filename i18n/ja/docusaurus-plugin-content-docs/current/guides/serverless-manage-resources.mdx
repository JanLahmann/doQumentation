...
<Admonition type="tip">
**Qiskit Serverlessはアップグレード中であり、その機能は急速に変化しています。** この開発フェーズでは、リリースノートと最新のドキュメントを[Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html)ページで確認してください。
</Admonition>
...
ノートブックでコードセルをローカルで読んでいる場合、`%%writefile` [マジックコマンド](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile)が表示されます。このマジックコマンドを含むセルを実行すると、実行ではなくディスクに保存されます。
</Admonition>
...
並列化できるクラシカルタスクには、`@distribute_task`デコレーターを使用してタスクの実行に必要なコンピュート要件を定義します。まず、[Write your first Qiskit Serverless program](/guides/serverless-first-program)のトピックにある`transpile_remote.py`の例を以下のコードで参照してください。
...
以下では、ファイルのアップロード方法を示します。IBM Quantumアカウントを使用してQiskit Serverlessに認証されていることを確認してください（手順については[Upload to Qiskit Serverless](/guides/serverless-first-program#upload-to-qiskit-serverless)を参照してください）。
...
- [既存のコードをQiskit Serverlessに移植する](/guides/serverless-port-code)完全な例を参照してください。
- 研究者がQiskit Serverlessと量子中心スーパーコンピューティングを使用して[量子化学を探求した](https://arxiv.org/abs/2405.05068v1)論文を読んでください。
