...
DAG には、qubit/clbit の入力ノード（緑）、演算ノード（青）、出力ノード（赤）の3種類のグラフノードがあります。各エッジは、2つのノード間のデータフロー（または依存関係）を示します。この Circuit の DAG を表示するには、qiskit.tools.visualization.dag_drawer() 関数を使用します。（実行するには [Graphviz ライブラリー](https://graphviz.org/download/)をインストールしてください。）
...
- [カスタムトランスパイラーパス](/guides/custom-transpiler-pass)の作成に関するガイドを確認する
- [カスタムバックエンドの作成とトランスパイル](/guides/custom-backend)の方法を学ぶ
