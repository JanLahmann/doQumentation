...
以下のコードは、`Estimator` プリミティブに対するベクトル化された入力の例のセットを示しており、これらを単一の `RuntimeJobV2 ` オブジェクトとしてIBM&reg; backendで実行します。
...
実行結果に加えて、`PrimitiveResult`オブジェクトと`PubResult`オブジェクトの両方に、送信されたジョブに関するメタデータ属性が含まれています。送信されたすべてのPUBに関する情報（さまざまな[ランタイムオプション](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options)など）を含むメタデータは`PrimitiveResult.metatada`にあり、各PUBに固有のメタデータは`PubResult.metadata`にあります。
