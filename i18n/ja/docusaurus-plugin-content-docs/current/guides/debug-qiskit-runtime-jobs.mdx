---
description: qiskit-ibm-runtimeデバッグツールモジュールのNeatクラスを使用して、ジョブをデバッグおよび分析します。
notebook_path: guides/debug-qiskit-runtime-jobs.ipynb
sidebar_label: NEATでジョブをデバッグする
title: NEATでジョブをデバッグする
---
...
リソース集約型のワークロードをハードウェアで実行する前に、IBM Quantum Compute の[NEAT（Noisy Estimator Analyzer Tool）](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/debug-tools-neat#neat)クラスを使用して、Estimatorワークロードのセットアップが正しいか、正確な結果を返す可能性があるか、指定された問題に最適なオプションを使用しているかなどを確認できます。
...
まず、関連するパッケージをインポートし、[IBM Quantum Compute Serviceへの認証](/guides/cloud-setup)を行います。
...
* ランダムな `RZ` 回転と `CNOT` ゲートのレイヤーを交互に繰り返します。
...
- [利用可能なIBM Quantum Computeオプション](/guides/runtime-options-overview)について学びます。
...
- Qiskitパターンワークフローの一部として[Circuitをトランスパイルする方法](/guides/circuit-transpilation-settings#compare-transpiler-settings)を学びます。
