---
description: 'Qiskit SDK プリミティブの入出力フォーマット（Primitive Unified Bloc（PUB）を含む）を理解する'
...
Estimator プリミティブの場合、PUB の形式には最大4つの値を含めることができます。
- 1つ以上の [`Parameter`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.Parameter) オブジェクトを含む可能性がある単一の `QuantumCircuit`
- 推定する期待値を指定する1つ以上のオブザーバブルのリスト。これらは配列に配置されます（たとえば、単一のオブザーバブルは0次元配列として、オブザーバブルのリストは1次元配列として表されます）。データは、`Pauli`、`SparsePauliOp`、`PauliList`、`str` などの `ObservablesArrayLike` 形式のいずれかにできます。
   <Admonition type="note">
   異なる PUB 内に同じ Circuit を持つ2つの可換なオブザーバブルがある場合、それらは同じ測定を使用して推定されません。各 PUB は測定のための異なる基底を表すため、PUB ごとに別々の測定が必要になります。可換なオブザーバブルが同じ測定を使用して推定されるようにするには、それらを同じ PUB 内にグループ化する必要があります。
   </Admonition>
- Circuit をバインドするためのパラメーター値のコレクション。これは、最後のインデックスが Circuit の `Parameter` オブジェクトに対応する単一の配列様オブジェクトとして指定できます。また、Circuit に `Parameter` オブジェクトがない場合は省略する（あるいは同等に `None` に設定する）ことができます。
- （オプションで）推定する期待値の目標精度
...
以下の演算子のリストは、含まれる情報の観点では等価ですが、形状が異なります。
...
1つ以上の PUB が実行のために QPU に送信され、ジョブが正常に完了すると、データは [`PrimitiveResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PrimitiveResult) コンテナオブジェクトとして返されます。`PrimitiveResult` には、各 PUB の実行結果を含む [`PubResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PubResult) オブジェクトの反復可能なリストが含まれます。たとえば、20個の PUB で送信されたジョブは、20個の `PubResults` のリストを含む `PrimitiveResult` オブジェクトを返し、それぞれが各 PUB に対応します。
...
<Admonition type="tip" title="おすすめ">
  ...
  -  [Qiskit SDK プリミティブ](https://docs.quantum.ibm.com/api/qiskit/primitives) APIを確認する。

  -  [Qiskit Aer primitives](https://qiskit.github.io/qiskit-aer/apidocs/aer_primitives.html) APIを確認する。

  -  [IBM Quantum プリミティブ](/guides/qiskit-runtime-primitives)について詳しく学ぶ。

  -  [`qiskit-ibm-runtime` Estimator](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2) APIを確認する。

  -  [`qiskit-ibm-runtime` Sampler](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2) APIを確認する。
</Admonition>
