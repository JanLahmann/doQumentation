---
description: 'IBM Quantum Compute Serviceで利用可能なエラー軽減・抑制技術の解説'
...
以下のコードセルは、Estimatorのダイナミカル・デカップリングを有効にし、ダイナミカル・デカップリングシーケンスを選択する方法を示しています。
...
![パウリ・トワーリングの描写](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)
...
以下のコードセルは、Estimatorのパウリ・トワーリングを有効にし、これらのオプションを設定する方法を示しています。これらのオプションを明示的に設定する必要はありません。
...
![測定トワーリングの描写](/docs/images/guides/error-mitigation-explanation/measurement_twirling.svg)

読み出しエラーが存在する場合、測定トワーリングは読み出しエラー転送行列を対角化する効果があり、容易に逆行列を計算できます。対角ノイズチャネルからのスケーリング項は、ゼロ状態に初期化されたランダムCircuitのベンチマークによって学習されます。これにより、読み出しノイズに起因する期待値のバイアスをサービスが除去できます。読み出しエラー転送行列の推定には追加のキャリブレーションCircuitの実行が必要であり、わずかなオーバーヘッドが生じます。

### `measure_mitigation`オプションによるTREXの有効化 {#enable-trex-by-using-the-measure-mitigation-option}

TREXは、Estimatorの[IBM Quantum Compute レジリエンスオプション](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2)で`measure_mitigation`を`True`に設定することで有効にできます。測定ノイズ学習のオプションは[こちら](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-measure-noise-learning-options)に説明されています。ゲートトワーリングと同様に、Circuitのランダム化数とランダム化ごとのショット数を設定できます。

以下のコードセルは、TREXを有効にし、Estimatorのこれらのオプションを設定する方法を示しています。これらのオプションを明示的に設定する必要はありません。
...
### `resilience_level`オプションによるTREXの有効化 {#enable-trex-by-using-the-resilience-level-option}

以下の例に示すように、[レジリエンスレベル](/guides/estimator-noise-management#resilience)を`1`に設定することでTREXを有効にできます。
...
ノイズ増幅と外挿の両段階は、多くの異なる方法で実装できます。IBM Quantum Computeは「デジタルゲートフォールディング」によってノイズ増幅を実装します。これは、2量子ビットGateをゲートとその逆行列の等価なシーケンスで置き換えることを意味します。例えば、ユニタリ $U$ を $U U^\dagger U$ に置き換えると、ノイズ増幅係数3が得られます。外挿には、線形フィットや指数フィットなど、いくつかの関数形式から選択できます。
下図は、左側にデジタルゲートフォールディングを、右側に外挿手順を示しています。

![ZNEの描写](/docs/images/guides/error-mitigation-explanation/zne.avif)

この手法のオーバーヘッドはノイズ係数の数に応じてスケールします。
デフォルト設定では3つのノイズ係数で期待値をサンプリングするため、
約3倍のオーバーヘッドが生じます。

### `zne_mitigation`オプションによるZNEの有効化 {#enable-zne-by-using-the-zne-mitigation-option}

ZNEは、Estimatorの[IBM Quantum Compute レジリエンスオプション](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2)で`zne_mitigation`を`True`に設定することで有効にできます。
ZNEのオプションは[こちら](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options)に説明されています。以下のオプションが注目されます：
...
以下のコードセルは、ZNEを有効にし、Estimatorのこれらのオプションを設定する方法を示しています。これらのオプションを明示的に設定する必要はありません。
...
### `resilience_level`オプションによるZNEの有効化 {#enable-zne-by-using-the-resilience-level-option}

以下の例に示すように、[レジリエンスレベル](/guides/estimator-noise-management#resilience)を`2`に設定することでZNE（TREXおよびゲートトワーリングとともに）を有効にできます。
...
PECは、Estimatorの[IBM Quantum Compute レジリエンスオプション](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2)で`pec_mitigation`を`True`に設定することで有効にできます。
PECのオプションは[こちら](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-pec-options)に説明されています。サンプリングオーバーヘッドの上限は`max_overhead`オプションを使用して設定できます。サンプリングオーバーヘッドを制限すると、結果の精度が要求された精度を超える場合があります。`max_overhead`のデフォルト値は100です。
...
- [EstimatorによるノイズManagement](/guides/estimator-noise-management)。

- [SamplerによるノイズManagement](/guides/sampler-noise-management)。

- IBM Quantum プリミティブの他の[オプション](/guides/runtime-options-overview)を探索してください。

- ジョブを実行する[実行モード](/guides/execution-modes)を決定してください。
