---
description: 'qiskit-ibm-runtime で Executor プリミティブを使用する方法。'
...
IBM Quantum Compute Service はマネージド・サービスであるため、まずアカウントを初期化する必要があります。その後、期待値の計算に使用する QPU を選択できます。
...
結果は [`QuantumProgramResult`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/results-quantum-program-result) 型です。結果オブジェクトについては、[Executor の入力と出力](/guides/executor-input-output)を参照してください。
