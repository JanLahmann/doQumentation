...
* ジョブまたはバッチモード：システムエラーにより失敗またはキャンセルされた場合、報告される使用量はゼロです。ユーザーエラーにより失敗したジョブやユーザーがキャンセルしたジョブの場合、報告される使用量はその時点までに発生した消費量（ジョブを実行するために QPU を準備する際に生じたオーバーヘッドを含む）となります。
...
- `qiskit-ibm-runtime` v0.30以降では、単一ジョブの場合は[`job.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-job-v2#usage)を実行し、バッチまたはセッションモードを使用する場合は[`batch.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/batch#usage)または[`session.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/session#usage)を実行します。古いバージョンの`qiskit-ibm-runtime`（>= 0.23 かつ < 0.30）を使用している場合も、`session.details()["usage_time"]` および `batch.details()["usage_time"]` で使用量を確認できます。

- [`GET /sessions/{id}`](https://docs.quantum.ibm.com/api/qiskit-runtime-rest/tags/sessions#tags__sessions__operations__get_session) を使用して、特定のバッチまたはセッションの使用量を確認します（このエンドポイントは両方の実行モードで同じです）。

- [`GET /jobs/{id}`](https://docs.quantum.ibm.com/api/qiskit-runtime-rest/tags/jobs#tags__jobs__operations__get_job_metrics_jid) を使用して、単一ジョブの使用量を確認します。
