---
title: Qiskit Runtime V2プリミティブへの移行
...
```
  ...
  <TabItem value="SamplerOut" label="Convert V2 output to V1 format">
    V1 の出力フォーマットは、キーにビット列（整数）、値に各 Circuit の準確率を持つ辞書でした。V2 フォーマットでは同じキー（ただし文字列として）とカウント数が値として使用されます。V2 フォーマットを V1 に変換するには、カウント数をショット数で割ります。ショット数の選択方法については、[Samplerのオプション](/guides/sampler-options#shots)ガイドを参照してください。
...
5. Estimator の `run()` メソッドを更新して、PUB のリストを渡すようにします。たとえば `run([(circuit1, observable1, parameter_set1)])` のようにします。
...
5. Sampler の `run()` メソッドを更新して、PUB のリストを渡すようにします。たとえば `run([(circuit1, parameter_set1)])` のようにします。
...
- [Samplerのオプション](/guides/sampler-options)、[Estimatorのオプション](/guides/estimator-options)、および[Executorのオプション](/guides/executor-options)ガイドで、オプションの設定についてさらに詳しく学びましょう。
- [プリミティブの入出力](/guides/primitive-input-output#pubs)の詳細を確認しましょう。
