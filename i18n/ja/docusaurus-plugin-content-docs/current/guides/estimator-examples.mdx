---
title: "Estimator の使用例"
sidebar_label: "Estimator の使用例"
description: "Qiskit Runtime で Estimator プリミティブを使用する実践的な例。"
...
このセクションの例は、Estimator の一般的な使用方法を示しています。これらの例を実行する前に、[Qiskit のインストール](install-qiskit)の手順に従ってください。

<Admonition type="note">
これらの例はすべて Qiskit Runtime のプリミティブを使用していますが、代わりに基底プリミティブを使用することもできます。
</Admonition>

Estimator を使用して、多くのアルゴリズムで必要とされる量子演算子の期待値を効率的に計算して解釈します。分子モデリング、機械学習、複雑な最適化問題での活用を探求します。

## 単一実験の実行 {#run-a-single-experiment}

Estimator を使用して、単一の回路-オブザーバブル・ペアの期待値を決定します。
...
Estimator を使用して、複数の回路-オブザーバブル・ペアの期待値を決定します。
...
## パラメーター化された回路の実行 {#run-parameterized-circuits}

Estimator を使用して、パラメーター値を活用して回路の再利用性を高め、1 つのジョブで 3 つの実験を実行します。
...
## バッチと高度なオプションの使用 {#use-batches-and-advanced-options}

バッチ[実行モード](/guides/execution-modes)と高度なオプションを活用して、QPU での回路パフォーマンスを最適化します。
...
- [高度なランタイム・オプションを指定する](runtime-options-overview)。
- IBM Quantum&reg; Learning の[コスト関数レッスン](/learning/courses/variational-algorithm-design/cost-functions)でプリミティブを使った練習をします。
...
- IBM&reg; QPU にジョブを送信する際の[ジョブの制限](/guides/job-limits)を理解します。
</Admonition>
