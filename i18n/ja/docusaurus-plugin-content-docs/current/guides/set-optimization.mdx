...
`qiskit_ibm_runtime` の `FakeSherbrooke` モックバックエンドをこれらの例で使用していますが、Qiskit 互換の実際のバックエンドまたはフェイクバックエンドで試すこともできます。結果は異なる場合があります。
</Admonition>

まず、最適化レベル 0 でトランスパイルしてみましょう。
...
最適化レベルの調整は、ECR ゲートの数だけでなく、回路の他の側面にも変化をもたらす可能性があります。最適化レベルの設定がレイアウトにどのような変化を与えるかの例については、[量子コンピューターの表現](/guides/represent-quantum-computers)を参照してください。
