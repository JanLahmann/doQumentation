...
以下のIBM&reg; 量子処理ユニット（QPU）は廃止されました。利用可能なQPUの完全なリストについては、[コンピュートリソース](https://quantum.cloud.ibm.com/computers)ページを参照してください。

<Admonition type="note">
    廃止されたQPUからジョブを取得するには、[こちらの手順](#retrieve)を参照してください。
</Admonition>
...
廃止されたQPUからジョブを取得するには、以下のコードを使用してください。
...
<Admonition type="note" title="Deprecated provider packages">
  `service.jobs()` は、`qiskit-ibm-runtime` と非推奨の `qiskit-ibm-provider` パッケージの両方から実行されたジョブを返します。古い（同様に非推奨の）`qiskit-ibmq-provider` パッケージから送信されたジョブはもう利用できません。
</Admonition>
