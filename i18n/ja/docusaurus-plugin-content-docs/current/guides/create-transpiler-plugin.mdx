...
この例では、`layout` ステージ用のトランスパイラー・ステージ・プラグインを作成します（Qiskitの組み込みトランスパイル・パイプラインの6つのステージの説明については、[トランスパイラー・ステージ](transpiler-stages)を参照してください）。私たちのプラグインは、要求された最適化レベルに応じた試行回数で単に[VF2Layout](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.passes.VF2Layout)を実行します。

まず、[PassManagerStagePlugin](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.preset_passmanagers.plugin.PassManagerStagePlugin)のサブクラスを作成します。実装する必要があるメソッドは1つで、[`pass_manager`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.preset_passmanagers.plugin.PassManagerStagePlugin#pass_manager)と呼ばれます。このメソッドは、入力として[PassManagerConfig](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.PassManagerConfig)を受け取り、定義しているパスマネージャーを返します。PassManagerConfigオブジェクトは、カップリングマップや基底ゲートなど、ターゲットBackendに関する情報を保存します。
...
次に、Pythonパッケージのメタデータにエントリーポイントを追加して、プラグインを公開します。ここでは、定義したクラスが `my_qiskit_plugin` というモジュールで公開されていると仮定します。例えば、`my_qiskit_plugin` モジュールの `__init__.py` ファイルでインポートすることによってです。パッケージの `pyproject.toml`、`setup.cfg`、または `setup.py` ファイルを編集します（Pythonプロジェクトのメタデータを保存するために選択したファイルの種類に応じて）。
...
各トランスパイラー・ステージのエントリーポイントと期待事項については、[トランスパイラー・プラグイン・ステージの表](https://docs.quantum.ibm.com/api/qiskit/transpiler_plugins#stage-table)を参照してください。
...
サンプルプラグインがインストールされていれば、`my_clifford_synthesis`という名前がこのリストに表示されます。
<Admonition type="tip" title="推奨事項">
- [プラグインをQiskit Ecosystemに提出しましょう!](https://github.com/Qiskit/ecosystem?tab=readme-ov-file#how-to-join)。
- 量子回路をトランスパイルして実行する例については、[チュートリアル](https://docs.quantum.ibm.com/tutorials)をご覧ください。
</Admonition>
