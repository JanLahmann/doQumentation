---
title: "トランスパイルのデフォルト設定と構成オプション"
sidebar_label: "トランスパイルのデフォルト設定と構成オプション"
description: "Qiskit SDK における量子回路トランスパイルのデフォルト設定と構成オプション。"
...
## パラメーターを指定しない基本的な使い方 {#basic-usage-without-parameters}
...
## 利用可能なすべてのパラメーター {#all-available-parameters}
...
### コンパイルターゲットを説明するパラメーター： {#parameters-used-to-describe-the-compilation-target}
...
<Accordion>
<AccordionItem title="**QPU (`Backend`) パラメーター**">
...
<AccordionItem title="**レイアウトとトポロジーのパラメーター**">
...
### Transpiler の動作に影響するパラメーター {#parameters-used-to-influence-how-the-transpiler-works}
...
<AccordionItem title="**ルーティングステージ**">
...
<AccordionItem title="**スケジューリングステージ**">
...
<AccordionItem title="**Transpiler の実行**">
...
- [最適化レベルの設定](set-optimization) の方法を学びましょう。
- [よく使用されるパラメーター](common-parameters) をさらに確認しましょう。
- [Qiskit Runtime を使用する際の最適化レベルの設定方法](/guides/runtime-options-overview) を学びましょう。
- [パスマネージャーを使ったトランスパイル](/guides/transpile-with-pass-managers) のトピックを参照してください。
- 例については、[量子コンピューターの表現](/guides/represent-quantum-computers) を参照してください。
- Qiskit Runtime を使った Qiskit パターンワークフローの一部として [回路のトランスパイル方法](/guides/circuit-transpilation-settings) を学びましょう。
- [Transpile API ドキュメント](https://docs.quantum.ibm.com/api/qiskit/transpiler) を確認してください。
</Admonition>
