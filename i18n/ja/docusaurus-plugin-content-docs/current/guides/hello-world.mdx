...
量子回路を作成する際には、実行後に返されるデータの種類も考慮する必要があります。Qiskitは2つの方法でデータを返します: 測定する量子ビットのセットのサンプリング出力を取得するか、観測量の期待値を取得できます。Qiskitプリミティブ(詳細は[ステップ3](#step-3-execute-using-the-quantum-primitives)で説明)を使用して、これら2つの方法のいずれかで回路を測定するようにワークロードを準備します。
