...
ただし、リセット処理は完全ではないため、状態準備エラーが発生します。このエラーを軽減するために、QPU は回路間に繰り返し遅延時間（`rep_delay`）も挿入します。各バックエンドにはデフォルトの `rep_delay` がありますが、通常はリセット・フィデリティと合計実行時間のバランスを取るよう設定されています。特定の QPU のデフォルト `rep_delay` を確認するには、`backend.default_rep_delay` を実行してください。
...
<Admonition type="note" title="QPU の rep_delay 値を確認する">

使用している特定の QPU でサポートされている `rep_delay` の範囲を必ず確認してください。この値は QPU によって異なり、時間の経過とともに変更される場合があります。

`rep_delay` を増加させると、実行時間と使用容量に直接影響することに注意してください。
</Admonition>
...
<Admonition type="tip" title="Recommendations">
  ...
  - [Estimator の使い方](/guides/get-started-with-estimator)を確認してください。
  - [Sampler の使い方](/guides/get-started-with-sampler)を確認してください。
</Admonition>
