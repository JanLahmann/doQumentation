...
## 乱数ジェネレーターのシード {#random-number-generator-seed}
...
`qiskit_ibm_runtime` の `FakeSherbrooke` モック Backend はこれらの例で使用されていますが、Qiskit 互換の実際のまたは偽の Backend であれば何でも試すことができます。結果は異なる場合があります。
</Admonition>
...
## Transpilerのステージとプラグインオプション {#transpiler-stage-and-plugin-options}
