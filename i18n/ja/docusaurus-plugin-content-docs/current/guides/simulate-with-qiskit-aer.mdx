...
次に、すべてのCXゲートに2%の脱分極エラー（depolarizing error）を含むノイズモデルを初期化します。実際には、ここでのCXゲートのような2量子ビットゲートから生じるエラーが、回路を実行する際の主要なエラーの原因です。Qiskit Aerでのノイズモデルの構築概要については、[ノイズモデルの構築](/guides/build-noise-models)を参照してください。
