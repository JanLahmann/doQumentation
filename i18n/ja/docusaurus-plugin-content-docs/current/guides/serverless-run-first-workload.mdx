...
<Admonition type="tip">
**Qiskit Serverlessはアップグレード中で、機能が急速に変化しています。** この開発フェーズでは、リリースノートと最新ドキュメントを [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html) ページでご確認ください。
</Admonition>
...
始める前に、[最初のQiskit Serverlessプログラムを作成する](/guides/serverless-first-program) のワークフローを完了していることを確認してください。

## 利用可能なプログラムを一覧表示する {#list-programs-available}

`QiskitServerless.list()` を使用して、Qiskit Serverlessで実行可能なプログラムの一覧を取得できます。これには、以前にアップロードした [`transpile_remote_serverless`](/guides/serverless-first-program) も含まれます。
...
- 並列化を含む、プログラムで利用可能な[コンピューティングおよびデータ管理ツール](/guides/serverless-manage-resources)を確認してください。
