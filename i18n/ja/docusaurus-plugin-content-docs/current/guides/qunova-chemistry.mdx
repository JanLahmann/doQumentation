...
次に、[IBM Quantum&reg; APIキー](http://quantum.cloud.ibm.com/)を使用して認証し、すでにローカル環境に[アカウントを保存](/guides/functions-get-started#install-qiskit-functions-catalog-client)していると仮定して、Qiskit Functionを次のように選択してください：
...
Qiskit Functionワークロードの[ステータス](/guides/functions-get-started#check-job-status)を確認したり、次のように[結果](/guides/functions-get-started#retrieve-results)を返したりすることができます：
...
ライセンスに関するお問い合わせは、[qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com)までメールでご連絡ください。

### ライセンスされたFunctionの使用例 {#example-of-licensed-function-use}

ライセンスされたユーザーにはライセンストークンが発行され、それをFunctionに送信するためにラッパーライブラリを使用する必要があります。
このラッパーライブラリは、`pip install hivqe-qiskit-function-utils`を使用して[PyPIから](https://pypi.org/project/hivqe-qiskit-function-utils/)インストールできます。
以下の例は、Functionを呼び出す際にこのライブラリを使用してライセンストークンを送信する方法を示しています。
...
- このQiskit Functionの[APIリファレンス](https://docs.quantum.ibm.com/api/functions/qunova-chemistry)を参照してください。
- [HI-VQEを使用したFeP-NOの解離PES曲線の計算](/tutorials/qunova-hivqe)チュートリアルを試してください。
- [Pellow-Jarman, A., et al. (2025).  HIVQE: Handover Iterative Variational Quantum Eigensolver for Efficient Quantum Chemistry Calculations. arXiv preprint arXiv:2503.06292.](https://arxiv.org/abs/2503.06292)を読んでください。
