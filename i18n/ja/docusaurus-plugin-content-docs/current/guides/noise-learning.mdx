...
結果として得られる `NoiseLearnerResult.data` は、対象回路に属する個々のエンタングリング層ごとの[ノイズモデル](https://arxiv.org/abs/2201.09866)を含む [`LayerError`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/results-layer-error) オブジェクトのリストです。各 `LayerError` は、回路と量子ビットラベルのセットという形で層の情報を格納するとともに、該当する層に対して学習されたノイズモデルの [`PauliLindbladError`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/results-pauli-lindblad-error) も格納します。
...
回路で学習したノイズモデルは、IBM Quantum の `EstimatorV2` プリミティブへの入力としても使用できます。このノイズモデルは、いくつかの異なる方法でプリミティブに渡すことができます。次の3つの例では、ノイズモデルを `estimator.options` 属性に直接渡す方法、Estimator プリミティブをインスタンス化する前に `ResilienceOptionsV2` オブジェクトを使用して渡す方法、および適切にフォーマットされた辞書を渡す方法を示します。
...
### ノイズ学習オプション {#noise-learning-options-1}
...
前の例の Circuit は、命令を注釈付きボックスにグループ化するボクシングパスマネージャーを通じて実行されます。理解しやすいように、関連するコードをここに追加します。
- `inject_noise_targets=”gates”` は、エンタングラーを含むボックスに `InjectNoise` 注釈を追加することを指定します。
- `inject_noise_strategy="uniform_modification"` は、`InjectNoise` 注釈を持つすべての同等のボックスに同じ `ref` と `modifier_ref` を割り当てることを指定します。
      - `InjectNoise.ref` は、そのボックスにノイズモデルを割り当てるために使用される一意の識別子です。
      - `InjectNoise.modifier_ref` を使用すると、ボックスに割り当てられたノイズモデルを乗数係数でスケーリングできます。
...
- 利用可能な[エラー緩和と抑制の技術](error-mitigation-and-suppression-techniques)について詳しく学ぶ。
