...
Qiskit SDK のリファレンスプリミティブは、ローカルの状態ベクトルシミュレーションを実行します。これらのシミュレーションはデバイスノイズのモデリングをサポートしていませんが、より高度なシミュレーション技術（[Qiskit Aer の使用](/guides/simulate-stabilizer-circuits)）や実機での実行（[Qiskit Runtime プリミティブ](primitives)）を検討する前に、アルゴリズムを素早くプロトタイピングするのに役立ちます。
