---
title: "ハミルトニアンシミュレーション向けQiskit Functionテンプレートの構築"
sidebar_label: "ハミルトニアンシミュレーション向けQiskit Functionテンプレートの構築"
description: "並列トランスパイルプログラムを作成し、再利用可能なリモートサービスとしてIBM Quantum Platformにデプロイする方法。"
...
このテンプレートは、ユーザー定義のスピンベースハミルトニアンに対する初期状態の時間発展をシミュレートし、[AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/) Qiskitアドオンを使用して指定された期待値のセットを返すワークフローをカプセル化します。

このテンプレートはQiskitパターンとして構成されており、以下のステップで構成されています：

#### 1. 入力の収集と問題のマッピング {#1-collecting-input-and-mapping-the-problem}

このセクションでは、シミュレートするハミルトニアン、`QuantumCircuit`の形式の初期状態、期待値を推定するオブザーバブルのセット、およびAQCアドオンのオプション仕様を入力として受け取ります。このステップでは、必要なすべての入力データが存在し、正しい形式であることを検証します。

入力引数は、ワークフローに関連する量子回路と演算子を構築するために使用されます。ターゲット回路が作成され、AQCアドオンを使用してこの回路の行列積状態表現が求められます。その後、アンザッツ回路が生成され、テンソルネットワーク手法を使用して最適化され、残りの時間発展を実行する最終回路が生成されます。

#### 2. 生成された回路を実行用に準備する {#2-prepare-the-generated-circuits-for-execution}

AQCアドオンから生成された回路は、選択したバックエンドで実行するために[トランスパイル](/guides/transpile#instruction-set-architecture)されます。デフォルトのエラー軽減オプションセットを持つ[`EstimatorV2`](../api/qiskit-ibm-runtime/estimator-v2)インスタンスが作成され、回路の実行を管理します。

#### 3. 実行 {#3-execution}

最後に、アンザッツ回路がトランスパイルされてQPU上で実行され、指定されたすべての期待値の推定値が収集され、ユーザーがアクセスできるシリアライズ可能な形式で返されます。
## ファンクションテンプレートの作成 {#write-the-function-template}

まず、[AQC-Tensor Qiskitアドオン](https://qiskit.github.io/qiskit-addon-aqc-tensor/)を使用して問題の記述をハードウェアで実行するための深さ削減回路にマッピングする、ハミルトニアンシミュレーション向けファンクションテンプレートを作成します。

このページをダウンロードしてノートブックエディターでローカルに表示すると、一部のコードセルに[マジックコマンド](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`が含まれていることがわかります。このマジックコマンドはコードを`./source_files/template_hamiltonian_simulation.py`に保存します。このファイルがQiskit Serverlessにアップロードしてリモートで実行できるファンクションテンプレートです。
...
### 入力の収集と検証 {#collect-and-validate-the-inputs}

テンプレートの入力を取得することから始めます。この例には、ハミルトニアンシミュレーションに関連するドメイン固有の入力（ハミルトニアンやオブザーバブルなど）と、機能固有のオプション（AQC-Tensorを使用してトロッター回路の初期レイヤーをどの程度圧縮するか、またはこの例に含まれるデフォルトを超えたエラー抑制および軽減の微調整のための高度なオプションなど）があります。
...
ファンクションテンプレートの実行中は、print文を使用してログに情報を返すことが有用で、ワークロードの進捗をより適切に評価できます。以下は、実際に使用されたEstimatorオプションの記録が残るように`estimator_options`を出力する簡単な例です。プログラム全体を通じて、AQC-Tensorの反復コンポーネント中の目的関数の値や、ハードウェアで実行するための最終命令セットアーキテクチャ（ISA）回路の2量子ビット深さなど、実行中の進捗を報告する同様の例が多数あります。
...
#### 入力の検証 {#validate-the-inputs}

テンプレートがさまざまな入力にわたって再利用できることを確認する重要な側面は、入力の検証です。以下のコードは、AQC-Tensor中の停止忠実度が適切に指定されているかを検証し、指定されていない場合はエラーの修正方法に関する有益なエラーメッセージを返す例です。
...
#### ファンクション出力の準備 {#prepare-the-function-outputs}

まず、ファンクションテンプレートのすべての出力を保持する辞書を準備します。ワークフロー全体を通じてこの辞書にキーが追加され、プログラムの最後に返されます。
...
### 問題のマッピングとAQCによる回路の前処理 {#map-the-problem-and-pre-process-the-circuit-with-aqc}

AQC-Tensor最適化はQiskitパターンのステップ1で実行されます。まず、ターゲット状態が構築されます。この例では、AQC部分と同じ時間期間、同じハミルトニアンを発展させるターゲット回路から構築されます。次に、より少ないトロッターステップを持つ同等の回路からアンザッツが生成されます。AQCアルゴリズムのメイン部分では、そのアンザッツが反復的にターゲット状態に近づけられます。最後に、結果は所望の発展時間に到達するために必要な残りのトロッターステップと組み合わせられます。

以下のコードに組み込まれたログの追加例に注意してください。
...
### 最終回路の実行用最適化 {#optimize-the-final-circuit-for-execution}

AQCワークフローの後、`final_circuit`は通常通り[ハードウェア用にトランスパイル](/guides/transpile#instruction-set-architecture)されます。
...
#### ドライランモードの場合は早期終了 {#exit-early-if-using-dry-run-mode}

ドライランモードが選択されている場合、ハードウェアでの実行前にプログラムが停止します。これは、例えばハードウェアでの実行を決定する前にISA回路の2量子ビット深さを確認したい場合に便利です。
...
#### ハードウェア上での回路の実行 {#execute-the-circuit-on-hardware}
...
#### 出力の保存 {#save-the-output}

このファンクションテンプレートは、ハミルトニアンシミュレーションワークフローの関連するドメインレベルの出力（期待値）に加え、途中で生成された重要なメタデータを返します。
...
## ファンクションのIBM Quantum Platformへのデプロイ {#deploy-the-function-to-ibm-quantum-platform}
前のセクションではリモートで実行するプログラムを作成しました。このセクションのコードは、そのプログラムをQiskit Serverlessにアップロードします。

`qiskit-ibm-catalog`を使用して、[IBM Quantum Platform](https://quantum.cloud.ibm.com)ダッシュボードで確認できるAPIキーで`QiskitServerless`に認証し、プログラムをアップロードします。

オプションで`save_account()`を使用して認証情報を保存できます（[IBM Cloudアカウントの設定](/guides/cloud-setup#cloud-save)ガイドを参照）。これにより、[`QiskitRuntimeService.save_account()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/qiskit-runtime-service#save_account)と同じファイルに認証情報が書き込まれることに注意してください。
...
このプログラムにはカスタム`pip`依存関係があります。`QiskitFunction`インスタンスを構築する際に`dependencies`配列に追加します：
...
最後に、プログラムが正常にアップロードされたか確認するには、`serverless.list()`を使用します：
...
## ファンクションテンプレートをリモートで実行する {#run-the-function-template-remotely}

ファンクションテンプレートがアップロードされたので、Qiskit Serverlessを使用してリモートで実行できます。まず、名前でテンプレートを読み込みます：

```python
template = serverless.load("template_hamiltonian_simulation")
```

次に、ハミルトニアンシミュレーションのドメインレベルの入力でテンプレートを実行します。この例では、ランダム結合を持つ50量子ビットXXZモデル、初期状態、およびオブザーバブルを指定します。
...
ジョブの実行中は、`print()`出力から作成されたログを取得できます。これらは、ハミルトニアンシミュレーションワークフローの進捗に関する有用な情報を提供できます。例えば、AQCの反復コンポーネント中の目的関数の値や、ハードウェアで実行するための最終ISA回路の2量子ビット深さなどです。
...
結果が利用可能になるまでプログラムの残りの部分をブロックします。ジョブが完了したら、結果を取得できます。これには、ハミルトニアンシミュレーションのドメインレベルの出力（期待値）と有用なメタデータが含まれます。
...
AQC-Tensor Qiskitアドオンの詳細については、[近似量子コンパイルによる改善されたトロッター化時間発展](/tutorials/approximate-quantum-compilation-for-time-evolution)チュートリアルまたは[qiskit-addon-aqc-tensorリポジトリ](https://github.com/Qiskit/qiskit-addon-aqc-tensor)を参照してください。
...
<Accordion>
<AccordionItem title="**完全なプログラムソースコード**">

以下は`./source_files/template_hamiltonian_simulation.py`全体を1つのコードブロックとしてまとめたものです。
