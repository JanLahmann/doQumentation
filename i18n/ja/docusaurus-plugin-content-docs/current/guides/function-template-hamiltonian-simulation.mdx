...
# ハミルトニアンシミュレーション向けファンクションテンプレートの構築 {#build-a-function-template-for-hamiltonian-simulation}

このテンプレートは、ユーザー定義のスピンベースハミルトニアンに対する初期状態の時間発展をシミュレートし、[AQCアドオン](./qiskit-addons-aqc)を使用して指定された期待値のセットを返すワークフローをカプセル化します。
...
まず、[AQC-Tensor Qiskitアドオン](/guides/qiskit-addons-aqc)を使用して問題の記述をハードウェアで実行するための深さ削減回路にマッピングする、ハミルトニアンシミュレーション向けファンクションテンプレートを作成します。

コード全体を通じて、`./source_files/template_hamiltonian_simulation.py`に保存されます。このファイルがQiskit Serverlessにアップロードしてリモートで実行できるファンクションテンプレートです。
