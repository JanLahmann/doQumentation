...
AQC アドオンから生成された回路は、選択した Backend で実行できるようにトランスパイルされます。回路の実行を管理するために、デフォルトのエラー緩和オプション・セットを使用して [`EstimatorV2`](../api/qiskit-ibm-runtime/estimator-v2) インスタンスが作成されます。
