...
`backend` から `target` 情報を取得し、操作名を `basis_gates` として保存してください。ダイナミカルデカップリングで使用するGateにタイミング情報を追加するために `target` を変更する必要があるためです。

<Admonition type="note">
これらの例では `qiskit_ibm_runtime` の `FakeSherbrooke` モックBackendを使用していますが、Qiskit互換の実際のBackendまたはフェイクBackendで試すこともできます。結果は異なる場合があります。
</Admonition>
...
ダイナミカルデカップリングシーケンスの定期的なタイミングのため、`YGate`に関する情報は`target`に追加する必要があります。なぜなら、これは基底ゲートでは*ない*のに対し、`XGate`はそうであるからです。しかし、`YGate`は`XGate`と同じ持続時間とエラーを持つことが*事前に*わかっているため、これらのプロパティを`target`から取得し、`YGate`オブジェクトのために追加し直すことができます。これは、`basis_gates`が別途保存されていた理由でもあります。なぜなら、`YGate`命令は`ibm_fez`の実際の基底ゲートではないにもかかわらず、`target`に追加しているからです。
