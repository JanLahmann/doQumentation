...
次のものをインストールするには、[インストール](/guides/install-c-api)のトピックの手順を完了してください。

- お使いのプラットフォーム用の標準的な C コンパイラーツールチェーン

- C API ヘッダーを含むバージョンの Python。


また、[Qiskit C API](https://docs.quantum.ibm.com/api/qiskit-c)で利用可能な関数やオブジェクトについて把握しているか、調べる準備ができている必要があります。C言語の基本的な知識も必要です。
...
ビルド時に使用したマイナーバージョンに合わせて、`project.dependencies` で Qiskit のランタイムバージョンを設定します。
