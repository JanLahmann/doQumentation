...
ただし、`shots`がどのPUBでもrunキーワード引数でも指定されていない場合（または全て`None`の場合）、オプションのshots値が使用されます。特に`default_shots`が使用されます。
...
4. `twirling`が有効な場合、[`twirling`オプション](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-twirling-options)で指定された`num_randomizations`と`shots_per_randomization`の積が使用されます。
...
PUBと`run`で指定されたshotsは優先度が高いですが、`twirling`が有効で`num_randomizations`と`shots_per_randomization`の積が`shots`値よりも小さい場合、ジョブは失敗します。このシナリオでは、`SamplerV2`は指定された`num_randomizations`にshotsを割り当てることができません。
</Admonition>
