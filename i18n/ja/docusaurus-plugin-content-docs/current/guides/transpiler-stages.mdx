...
回路をトランスパイルする推奨方法は、プリセットのステージ付きパスマネージャーを作成し、[パスマネージャーによるトランスパイル](transpile-with-pass-managers)で説明されているように、そのパスマネージャーを回路に対して実行することです。ただし、よりシンプルでカスタマイズ性の低い代替方法として、[`transpile`](https://docs.quantum.ibm.com/api/qiskit/compiler#qiskit.compiler.transpile)関数を使用する方法があります。この関数は回路を直接引数として受け取ります。`generate_preset_pass_manager`と同様に、使用される具体的なトランスパイラーパスは、`transpile`に渡される`optimization_level`などの引数に依存します。実際、内部では`transpile`関数が`generate_preset_pass_manager`を呼び出してプリセットのステージ付きパスマネージャーを作成し、それを回路に対して実行します。
...
次のステージは、回路を送信するバックエンドのレイアウト（接続構造）に関するものです。一般に、量子回路は抽象的な存在であり、その量子ビットは実際の計算に使用される物理的な量子ビットの「仮想」または「論理的」な表現です。ゲートのシーケンスを実行するには、「仮想」量子ビットと実際の量子デバイスの「物理」量子ビットとの一対一の対応付けが必要です。このマッピングは `Layout` オブジェクトとして保持され、バックエンドの[命令セットアーキテクチャ（ISA）](/guides/transpile#instruction-set-architecture)で定義される制約の一部です。

![この画像は、ワイヤー表現の量子ビットが QPU 上の接続を表す図にマッピングされる様子を示しています。](/docs/images/guides/transpiler-stages/layout-mapping.svg "Qubit mapping")

マッピングの選択は、入力回路をデバイスのトポロジーにマッピングするために必要なSWAP演算の数を最小限に抑え、最も適切にキャリブレーションされた量子ビットが使用されるようにするために極めて重要です。この段階の重要性から、プリセットパスマネージャーは最適なレイアウトを見つけるためにいくつかの異なる方法を試みます。通常、これには2つのステップが含まれます。まず、「完全な」レイアウト（SWAP演算を全く必要としないレイアウト）を見つけようとし、次に、完全なレイアウトが見つからない場合に使用する最適なレイアウトを見つけようとするヒューリスティック・パスを実行します。この最初のステップには、通常2つの`Passes`が使用されます。

- `TrivialLayout`：各仮想量子ビットを、デバイス上の同じ番号の物理量子ビットに素朴にマッピングします（すなわち、[`0`,`1`,`1`,`3`] -> [`0`,`1`,`1`,`3`]）。これは、完全なレイアウトを見つけるために`optimzation_level=1`でのみ使用される歴史的な動作です。失敗した場合、次に`VF2Layout`が試されます。
...
- `DenseLayout`：回路と同じ数の量子ビットを持ち、最も接続性の高いデバイスのサブグラフを見つけます（回路内に（IfElseOpなどの）制御フロー演算が存在する場合、optimization level 1で使用されます）。
...
量子回路を記述する際は、任意の量子ゲート（ユニタリ操作）を自由に使用できます。また、量子ビットの測定やリセット命令などのゲート以外の操作も利用できます。ただし、ほとんどの量子デバイスがネイティブにサポートするのは、ごく少数の量子ゲートおよびゲート以外の操作に限られています。これらのネイティブゲートはターゲットの [ISA](/guides/transpile#instruction-set-architecture) の定義の一部であり、このプリセット `PassManagers` のステージは、回路に指定されたゲートを指定されたバックエンドのネイティブ基底ゲートに変換（*アンロール*）します。これは回路をバックエンドで実行するための重要なステップですが、通常は深さとゲート数の増加を招きます。
...
以下のコードは、15量子ビットのGHZ状態を構築し、結果として生じる回路の深さ、ゲート数、複数量子ビットゲート数の観点から、トランスパイルの`optimization_levels`を比較します。
...
この最後のステージは、明示的に呼び出された場合にのみ実行され（Initステージと同様）、デフォルトでは実行されません（ただし、`generate_preset_pass_manager`を呼び出す際に`scheduling_method`引数を設定することで、方式を指定できます）。スケジューリング・ステージは、通常、回路がターゲット基底に変換され、デバイスにマッピングされ、最適化された後に使用されます。これらのパスは、回路内のすべてのアイドル時間を考慮することに焦点を当てています。高いレベルで見ると、スケジューリング・パスは、ゲート実行間のアイドル時間を考慮するために遅延命令を明示的に挿入し、回路がバックエンド上でどれくらいの時間実行されるかを調べるものと考えることができます。
