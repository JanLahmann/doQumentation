...
GEM Suiteソフトウェアパッケージでは、GEMプロトコルを実装するための基本的なクラスは`PlaquetteLattice`であり、これはプラケットの格子（ヘビーヘックス格子とは異なるもの）を表します。`PlaquetteLattice`は、量子ビット結合マップから初期化できます。現在、ヘビーヘックス結合マップのみがサポートされています。
