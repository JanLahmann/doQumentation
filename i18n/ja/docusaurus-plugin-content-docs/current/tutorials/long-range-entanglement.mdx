...
### (ii) ベルペアの準備 {#ii-prepare-bell-pairs}
...
### (iii) 隣接量子ビットペアをベル基底で測定する {#iii-measure-neighboring-qubit-pairs-in-the-bell-basis}
...
### (iv) 次に、パウリ副産物演算子を補正するためのフィードフォワード補正を適用する {#iv-next-apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
### (v) 最後に、制御量子ビットとターゲット量子ビットを測定する {#v-finally-measure-control-and-target-qubits}
...
#### 1Dチェーン選択のためのレイヤーフィデリティ文字列の使用 {#use-layer-fidelity-string-for-selecting-1d-chain}
