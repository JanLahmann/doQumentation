...
* ベル状態、エンタングルメント、量子ゲートを含む[基本的な量子計算の概念](/learning/courses/basics-of-quantum-information)；
* [動的回路](/guides/classical-feedforward-and-control-flow)（回路中間測定と古典フィードフォワード）への慣れ；
* [Qiskit SDK](https://docs.quantum.ibm.com/guides) および [IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime) の基本的な知識と [IBM Quantum アカウント](/guides/cloud-setup) へのアクセス。
...
- Qiskit Runtime v0.37以降（`pip install qiskit-ibm-runtime`）
...
実際のQPUで実行する前に、動的回路とユニタリ回路の両方がノイズのないシミュレータ上で理想的なベル状態を生成することを確認します。IBM Quantumの `Sampler` を `AerSimulator` をバックエンドモードとして使用し、距離6の小規模な例で実行します。
...
#### 隣接量子ビットペアをベル基底で測定する {#measure-neighboring-qubit-pairs-in-the-bell-basis}
...
#### パウリ副産物演算子を補正するためのフィードフォワード補正を適用する {#apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
#### 制御量子ビットとターゲット量子ビットを測定する {#measure-control-and-target-qubits}
...
#### ユニタリベースの実装：量子ビットを中央にスワップする方法 {#unitary-based-implementation-swapping-the-qubits-to-the-middle}
...
次に、動的回路で行ったのと同様に、$XX$、$YY$、$ZZ$ 基底で測定する回路を構築します。
...
ノイズのないシミュレーションでは、構築した回路のフィデリティメトリクスが $1$ であることを検証します。実際のQPU上の実験では、この解析により動的回路がユニタリ方式のベースライン実装と比較してどの程度良好に動作するかを明確に把握できます。
...
ノイズのないシミュレーションでは予想通り、動的回路とユニタリ回路の両方のフィデリティは $1$ です。

## 大規模ハードウェアの例 {#large-scale-hardware-example}

ここでは、これらすべての詳細を1つのワークフローにまとめ、より大規模なスケールで実際の量子ハードウェア上で実行します。

### さまざまな距離の回路を生成する {#generate-circuits-for-different-distances}

次に、最大60量子ビット離れたさまざまな量子ビット間距離に対する長距離CX回路を生成します。各距離に対して、$XX$、$YY$、$ZZ$ 基底で測定する回路を構築し、後にフィデリティの計算に使用します。
...
#### 1Dチェーン選択のためのレイヤー忠実度文字列の使用 {#use-layer-fidelity-string-for-selecting-1d-chain}
...
### LRCX回路で使用される量子ビットの可視化 {#visualize-qubits-used-for-the-lrcx-circuit}
...
![前のコードセルの出力](/docs/images/tutorials/long-range-entanglement/extracted-outputs/2d090f8a-0.avif)
...
- [カットベルペアのベンチマークによる動的回路の評価](/tutorials/edc-cut-bell-pair-benchmarking)
- [動的回路によるキックドイジングハミルトニアンのシミュレーション](/tutorials/dc-hex-ising)
</Admonition>
