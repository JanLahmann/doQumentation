...
次に、[IBM Quantum Platform](http://quantum.cloud.ibm.com/) ダッシュボードの API キーを使用して認証します。その後、Qiskit Function を次のように選択します。（セキュリティのため、信頼できるマシンをご使用の場合は、認証のたびに API キーを入力しなくて済むように、[アカウント認証情報をローカル環境に保存する](/guides/functions#install-qiskit-functions-catalog-client)ことをお勧めします。）
