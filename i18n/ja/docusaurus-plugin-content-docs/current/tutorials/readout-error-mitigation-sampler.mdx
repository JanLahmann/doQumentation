...
## ステップ 3: Qiskit プリミティブを使用した回路の実行
...
さらに複雑な点として、（トランスパイル前の）より単純な回路を実現するために、必要な CZ ゲートを隣接する量子ビットのペア間に挿入しました。これはビット列 $a$ と $b$ を $a1 b1 a2 b2 \ldots$ のようにインターリーブすることに相当します。
...
M3 による読み出しエラー緩和を適用する前の、最も確率の高いビット列の確率を記録しましょう。
...
### M3 に必要な CPU 時間がショット数に対してどのようにスケールするかのプロット
...
上記のプロットは、M3 補正の適用に必要な時間がショット数に対して線形にスケールすることを示しています。## スケールアップ
