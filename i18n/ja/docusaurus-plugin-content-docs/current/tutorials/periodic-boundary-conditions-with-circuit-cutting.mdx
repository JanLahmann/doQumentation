...
これらの回路内のすべてのゲートが基底ゲート集合に属することを確実にするため、```BasisTranslator``` と続いて ```Optimize1qGatesDecomposition``` パスを適用します。ルーティングや初期レイアウト選択などの他のステップが再度実行されないため、これら2つのパスを使用する方が、トランスパイル・プロセス全体よりも高速です。
...
> **注記:** このアンケートは **IBM Quantum** によるものであり、(IBMによって書かれた)**チュートリアルの内容**を対象としています。doQumentationはウェブサイト、翻訳、およびコード実行を提供しています。それらに関するフィードバックは、[GitHubのissueを開いて](https://github.com/JanLahmann/doQumentation/issues)ください。
