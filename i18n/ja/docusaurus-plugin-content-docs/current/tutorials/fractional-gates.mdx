...
- 分数ゲートの使用は、[動的回路](/guides/classical-feedforward-and-control-flow)、[Pauliツイリング](/guides/error-mitigation-and-suppression-techniques#pauli-twirling)、[確率的エラーキャンセレーション](/guides/error-mitigation-and-suppression-techniques#probabilistic-error-cancellation-pec)（PEC）、および[ゼロノイズ外挿法](/guides/error-mitigation-and-suppression-techniques#zero-noise-extrapolation-zne)（ZNE）（[確率的エラー増幅法](/guides/error-mitigation-and-suppression-techniques#probabilistic-error-amplification-pea)（PEA）使用時）ではサポートされていません。
