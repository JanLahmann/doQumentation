...
#### オブザーバブルの構築 {#construct-the-observable}
...
## ステップ2と3: 量子ハードウェア実行のための問題の最適化とIBM Circuit関数での実行 {#steps-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-with-the-ibm-circuit-function}
...
## ステップ4: 後処理と希望する古典形式での結果の返却 {#step-4-post-process-and-return-result-in-desired-classical-format}

<ul>
    <li>入力: IBM Circuit関数からの結果</li>
    <li>出力: グローバル磁化</li>
</ul>
#### グローバル磁化の計算 {#compute-the-global-magnetization}
関数を実行した結果は、[Estimator](/guides/estimator-input-output)と同じ形式です。
