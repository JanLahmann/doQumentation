...
* [Qiskit SDK](https://docs.quantum.ibm.com/guides)と[Qiskit Runtime](/guides/compute-services#qiskit-runtime)の基礎知識、および[IBM Quantum&reg; アカウント](/guides/cloud-setup)へのアクセス。
## 背景 {#background}
...
実際のQPU上で実行する前に、4量子ビットチェーン `[0, 1, 2, 3]` を使用してノイズなしシミュレーター上でテストし、回路がベルペアを生成することを確認します。バックエンドモードとして `AerSimulator` を指定したQiskit Runtime `Sampler` を使用して回路を実行します。
