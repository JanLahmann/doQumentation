...
このチュートリアルでは、量子回路内のゲートをカットして回路の深さを削減するための`Qiskit pattern`を構築する方法を説明します。回路カットについてより詳しい議論は、[回路カットQiskitアドオンのドキュメント](https://qiskit.github.io/qiskit-addon-cutting/)をご覧ください。
...
## ステップ4: 後処理を行い、所望の古典形式で結果を返す {#step-4-post-process-and-return-result-in-desired-classical-format}
...
> **注:** このアンケートは**IBM Quantum**によるもので、（IBMが執筆した）**チュートリアルの内容**を対象としています。doQumentationはウェブサイト、翻訳、コード実行を提供しています。これらに関するフィードバックは[GitHub issueを開いて](https://github.com/JanLahmann/doQumentation/issues)ください。
