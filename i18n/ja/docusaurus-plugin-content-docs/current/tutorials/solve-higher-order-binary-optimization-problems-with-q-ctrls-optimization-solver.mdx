...
また、Optimization Solver関数へのアクセス権を取得する必要があります。アクセスをリクエストするには、[こちらのフォーム](https://quantum.cloud.ibm.com/functions?id=q-ctrl-optimization-solver)に記入してください。
...
[IBM Quantum Platform](https://quantum.cloud.ibm.com/)の認証情報を定義します。これらは、チュートリアル全体を通してIBM Quantum Compute ServiceおよびQiskit Functionsへの認証に使用されます。
...
## ステップ1: 問題を目的関数として定義する {#step-1-define-the-problem-as-an-objective-function}
...
## ステップ 2: Fire Opal 最適化ソルバーを使用してハイブリッドアルゴリズムを実行する {#step-2-run-the-hybrid-algorithm-using-the-fire-opal-optimization-solver}
...
ソルバーは、解のビット列、反復回数、変数からビット列へのマッピングなど、解と関連するメタデータを含む辞書を返します。ソルバーの入出力の完全な定義については、[ドキュメント](https://docs.quantum.ibm.com/api/functions/q-ctrl-optimization-solver)をご確認ください。
...
## ステップ 3: 結果を評価する {#step-3-evaluate-results}
...
この最適化アルゴリズムの目標はイジングモデルの最小基底状態を見つけることであるため、値が小さいほど良い解を示します。そのため、Fire Opal Optimization Solverが生成した解がランダム選択を大幅に上回ることが視覚的に明らかです。
