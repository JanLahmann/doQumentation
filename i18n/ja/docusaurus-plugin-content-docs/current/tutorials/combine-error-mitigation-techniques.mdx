---
description: '100 量子ビット以上の IBM Quantum QPU と IBM Quantum Estimator プリミティブを使用した、実用規模の実験におけるエラー軽減オプションの組み合わせ。'
...
- この[ガイド](/guides/error-mitigation-and-suppression-techniques)で説明されている動的デカップリング、測定エラー軽減、ゲートトワーリング、ゼロノイズ外挿の基礎。

## 前提条件 {#prerequisites}

このチュートリアルを終えると、以下について理解できるようになります。

- 上記のエラー軽減手法がハードウェア上でどのように選択的に実装されるか。

- ハードウェアノイズを軽減する能力の観点から、それらがどのように比較されるか。


## 背景 {#background}

このチュートリアルでは、IBM Quantum Estimator プリミティブで利用可能なエラー抑制およびエラー軽減オプションについて探ります。このチュートリアルでは、以下の各手法を個別に実装する方法を示します：
...
これらの手法を個別に実装する代替手段として、[レジリエンスレベル](/guides/estimator-noise-management)を使用して実装する方法があります。`resilience_level`は0、1、2の値を取ります。

- 0: 軽減は実装されません。

- 1: 測定エラー軽減が実装されます。

- 2: ゲートトワーリング、測定エラー軽減、ZNEが実装されます。


このチュートリアルでは、回路とオブザーバブルを構築し、異なるエラー軽減設定の組み合わせを使用してEstimatorプリミティブでジョブを送信します。その後、結果をプロットして、さまざまな設定の効果を観察します。チュートリアルの大部分では、視覚化を容易にするために10量子ビットの回路を使用し、最後に50量子ビットにスケールアップします。
...
シミュレーターではランタイムエラー軽減がサポートされていないため、このステップは省略します。
...
最後の量子ビットがこの文字列の最初の要素に対応するのは、QiskitがリトルエンディアンNotationを使用しているためです。
...
この時点で、回路を実行してオブザーバブルを測定することもできます。しかし、量子デバイスの出力を正しい答え&mdash;つまり回路がエラーなしで実行された場合のオブザーバブルの理論値と比較することも必要です。小規模な量子回路では、古典コンピュータで回路をシミュレーションしてこの値を計算できますが、より大規模な実用規模の回路ではこれは不可能です。この問題は「ミラー回路」手法（「計算-逆計算」とも呼ばれます）を使用して回避できます。この手法は量子デバイスの性能をベンチマークするのに役立ちます。

#### ミラー回路 {#mirror-circuit}
...
次のコードセルでは、回路にランダムなパラメータを割り当て、[`unitary_overlap`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.unitary_overlap)クラスを使用してミラー回路を構築します。回路をミラーリングする前に、[バリア](https://docs.quantum.ibm.com/api/qiskit/circuit#qiskit.circuit.Barrier)命令を追加して、トランスパイラがバリアの両側の回路の2つの部分を統合してゲートのないトランスパイル済み回路になるのを防ぎます。
...
### ステップ2: 量子ハードウェアでの実行に向けて問題を最適化する {#step-2-optimize-problem-for-quantum-hardware-execution}
...
トランスパイルされた回路には、ISA命令のみが含まれるようになりました。すべてのゲートは$\sqrt{X}$ゲートと$R_z$回転、および[CZゲート](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.CZGate)に分解されています。
...
ここでは、エラー抑制や軽減なしの状態から始めて、IBM Quantum Compute Service で利用可能なさまざまなエラー抑制・軽減オプションを順次有効にしながら、5 つの個別のジョブを送信します。各オプションの詳細については、以下のページを参照してください。
...
これらのジョブは互いに独立して実行できるため、[バッチモード](/guides/run-jobs-batch)を使用して、Quantum Compute に実行タイミングを最適化させることができます。
...
### ステップ4: 後処理を行い、結果を目的の古典的な形式で返す {#step-4-post-process-and-return-result-in-desired-classical-format}
...
- すべての実験で理想値に近い結果が得られ、誤差バーもすべて小さくなっています。

- 動的デカップリングの追加によって、軽減なしの場合と比べて性能が悪化した可能性があります。回路が非常に密であるため、これは驚くべきことではありません。動的デカップリングは主に、量子ビットにゲートが適用されずにアイドル状態のまま待機する大きなギャップが回路内にある場合に有用です。これらのギャップが存在しない場合、動的デカップリングは効果がなく、動的デカップリングパルス自体のエラーにより性能が悪化する可能性さえあります。10量子ビットの回路では、この効果を観察するには規模が小さすぎた可能性があります。

- ゼロノイズ外挿を使用すると、結果は理想値に非常に近くなります。これはZNEの威力を示しています。


## 次のステップ {#next-steps}
<Admonition type="tip" title="推奨事項">
このチュートリアルに興味を持った方は、本チュートリアルで触れなかった追加のエラー軽減・エラー抑制手法に関する以下の資料も参照してください。
- [確率的エラーキャンセル（PEC）](/guides/error-mitigation-and-suppression-techniques#probabilistic-error-cancellation-pec)
- [行列フリー測定軽減（M3）](https://qiskit.github.io/qiskit-addon-mthree/)（[M3を使用したSamplerプリミティブの読み出しエラー軽減](/tutorials/readout-error-mitigation-sampler/)チュートリアルも参照）
- [シェーデッド・ライトコーン](https://qiskit.github.io/qiskit-addon-slc/)
- [伝播ノイズ吸収（PNA）](https://qiskit.github.io/qiskit-addon-pna/)
</Admonition>
