...
このセクションでは、ラベリング・コセット・ウィズ・エラー問題の7量子ビットのインスタンスに対して、Qiskitパターンの4つのステップを説明し、Qiskitの`StatevectorSampler`プリミティブを使用して単一のカーネル行列要素を評価します。ステートベクトル・シミュレーターは(ショット・ノイズを除いて)正確であり、QPU時間を消費することなく、この手法をエンドツーエンドで示してくれます。その後、ハードウェア例のセクションで実際のハードウェア上で同じインスタンスを繰り返します。
