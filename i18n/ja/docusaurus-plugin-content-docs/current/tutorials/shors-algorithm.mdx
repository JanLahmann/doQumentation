...
このチュートリアルを修了すると、以下を理解できるようになります。
- 整数素因数分解のためのショアのアルゴリズムの数学的背景
- ハードウェア上でこのアルゴリズムの実行例を動かす方法

## 前提条件 {#prerequisites}
このチュートリアルを始める前に、以下のトピックに慣れておくことを推奨します。
- [量子アルゴリズムの基礎](/learning/courses/fundamentals-of-quantum-algorithms)
- [位相推定と因数分解](/learning/courses/fundamentals-of-quantum-algorithms/phase-estimation-and-factoring/introduction)。このチュートリアルでも一部の内容を扱います。

## 背景 {#background}

1994年にPeter Shorによって開発された[Shorのアルゴリズム](https://epubs.siam.org/doi/abs/10.1137/S0036144598347011)は、整数を多項式時間で因数分解する画期的な量子アルゴリズムです。その重要性は、大きな整数の因数分解の困難さに依存するRSAなど、広く使用されている暗号システムの安全性を脅かすほど、既知のどの古典アルゴリズムよりも指数関数的に速く大きな整数を因数分解できる能力にあります。十分に強力な量子コンピュータでこの問題を効率的に解くことにより、Shorのアルゴリズムは暗号技術、サイバーセキュリティ、計算数学などの分野に革命をもたらす可能性があり、量子計算の変革的な力を裏付けています。

このチュートリアルでは、量子コンピュータ上で15を因数分解することでShorのアルゴリズムを実演することに焦点を当てます。

このチュートリアルでは、量子コンピュータ上で15を素因数分解することにより、ショアのアルゴリズムを実演することに焦点を当てます。

このチュートリアルは、実際のハードウェア上でのShorのアルゴリズムの他の実演についての議論で締めくくります。汎用的な実装と、15や21などの特定の整数の因数分解に特化した実装の両方に焦点を当てます。
注: このチュートリアルは、Shorのアルゴリズムに関する回路の実装と実演により重点を置いています。この内容に関する詳細な教育リソースについては、John Watrous博士による[量子アルゴリズムの基礎](/learning/courses/fundamentals-of-quantum-algorithms/phase-estimation-and-factoring/introduction)コース、および[参考文献](#references)セクションの論文を参照してください。
...
ショアのアルゴリズムでQFT回路が使用される理由のより深い理解については、[量子アルゴリズムの基礎](/learning/courses/fundamentals-of-quantum-algorithms/phase-estimation-and-factoring/introduction)コースを参照してください。
...
位数発見問題を位相推定に結びつけるために、古典状態が$\mathbb{Z}_N$に対応するシステム上で定義された演算を考えます。ここでは、固定された要素$a \in \mathbb{Z}^*_N$を掛け合わせます。特に、この乗算演算子$M_a$を、各$x \in \mathbb{Z}_N$に対して$$M_a \ket{x} = \ket{ax \; (\mathrm{mod} \; N)}$$となるように定義します。方程式の右辺のケットの中では、$N$を法とする積を取っていることが暗黙的であることに注意してください。数学的な解析により、$M_a$がユニタリ演算子であることがわかります。さらに、$M_a$は、$a$の位数$r$を位相推定問題に結びつけることを可能にする固有ベクトルと固有値のペアを持つことがわかります。具体的には、$j \in \{0, \dots, r-1\}$の任意の選択に対して、$$\ket{\psi_j} = \frac{1}{\sqrt{r}} \sum^{r-1}_{k=0} \omega^{-jk}_{r} \ket{a^k}$$は$M_a$の固有ベクトルであり、対応する固有値は$\omega^{j}_{r}$です。ここで、$$\omega^{j}_{r} = e^{2 \pi i \frac{j}{r}}.$$
観察により、便利な固有ベクトル/固有値のペアは、$\omega^{1}_{r} = e^{2 \pi i \frac{1}{r}}$を持つ状態$\ket{\psi_1}$であることがわかります。したがって、固有ベクトル$\ket{\psi_1}$を見つけることができれば、量子回路で位相$\theta=1/r$を推定でき、したがって位数$r$の推定値を得ることができます。しかし、これを行うのは容易ではなく、別の方法を検討する必要があります。
...
任意の $b \in \mathbb{Z}^*_N$ に対する $M_b$ 演算子が置換操作であることを見ました。ここで扱っている置換問題は比較的小さいため（$N=15$ には4個のqubitしか必要としません）、これらの操作を検査によって ``SWAP`` gate で直接合成することができました。一般的には、これはスケーラブルなアプローチではないかもしれません。代わりに、置換行列を明示的に構築し、Qiskitの ``UnitaryGate`` クラスとトランスパイル方法を使用してこの置換行列を合成する必要があるかもしれません。ただし、これにより、著しく深い circuit が生じる可能性があります。以下に例を示します。
...
これまで、位相推定回路を用いて位数発見問題をどのように実装できるかを議論しました。次に、位数発見問題を整数の因数分解に結びつけ、Shorのアルゴリズムを完成させます。アルゴリズムのこの部分は古典的であることに注意してください。
ここで、$N = 15$、$a = 2$の例を使用してこれを実演します。測定した位相は$k / r$であることを思い出してください。ここで、$$a^r \; (\textrm{mod} \; N) = 1$$であり、$k$は$0$から$r - 1$までのランダムな整数です。この式から、$$(a^r - 1) \; (\textrm{mod} \; N) = 0,$$が得られ、これは$N$が$a^r-1$を割り切らなければならないことを意味します。$r$も偶数であれば、$$a^r -1 = (a^{r/2}-1)(a^{r/2}+1).$$と書くことができます。$r$が偶数でない場合、これ以上進めることができないため、$a$の別の値で再試行する必要があります。そうでなければ、$N$と$a^{r/2}-1$または$a^{r/2}+1$のいずれかとの最大公約数が$N$の真の因数である確率が高くなります。
