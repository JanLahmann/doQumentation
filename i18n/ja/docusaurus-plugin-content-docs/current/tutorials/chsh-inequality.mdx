...
V2プリミティブは、ターゲットシステムがサポートする命令および接続性（命令セットアーキテクチャ、すなわちISA CircuitおよびISA observable）に準拠するCircuitとobservableのみを受け付けます。Backendから`AerSimulator`を構築し、シミュレーターのターゲットに対してトランスパイルすることで、同じパスマネージャーがエンドツーエンドで実行されるようにします。
