...
## セットアップ {#setup}
まず、[IBM Quantum APIキー](http://quantum.cloud.ibm.com/)を使用して認証します。次に、以下のようにQiskit Functionを選択します。（このコードは、ローカル環境に[アカウントが保存済み](/guides/functions-get-started#install-qiskit-functions-catalog-client)であることを前提としています。）
