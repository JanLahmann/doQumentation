...
#### ステップ3：射影と古典的対角化 {#step-3-project-and-diagonalize-classically-1}
...
### マッピング（`mapping.py` と `mapping.sh`） {#mapping-mappingpy-and-mappingsh}
...
### 最適化（`optimization.py` と `optimization.sh`） {#optimization-optimizationpy-and-optimizationsh}
...
### 実行（`execution.py` と `execution.sh`） {#execution-executionpy-and-executionsh}
...
### 後処理（`postprocessing.py` と `postprocessing.sh`） {#post-processing-postprocessingpy-and-postprocessingsh}
