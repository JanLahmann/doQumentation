...
![4本の水平線と多くのゲートを持つ回路図。](/learning/images/courses/integrating-quantum-and-high-performance-computing/programming-models/checkin-circuit.avif)
...
分子内の電子をモデル化していると仮定します。(a) 分子の基底状態のエネルギー、および (b) 分子の基底状態において、どの計算基底状態が最も支配的であるかを近似したいとします。それぞれの場合において、EstimatorプリミティブとSamplerプリミティブのどちらを使用しますか？
...
* プログラムのメインの制御フローは、プロセスのランクによって決定されます。これはSPMDの原則の実践例です。プログラムは条件ロジック（例えば if (rank == 0)）を使用して、コードの特定の並列化されたセクションのみがワーカープロセスによって実行されるようにする一方で、マスタープロセス（多くの場合Rank 0）が初期化と最終集約を処理します。
