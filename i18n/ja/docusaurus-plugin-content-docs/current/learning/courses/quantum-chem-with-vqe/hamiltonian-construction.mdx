...
__basis__は、分子軌道をモデル化するために使用される関数を指します。ここで'sto-6g'は一般的な最小基底であり、6個の原始ガウス軌道を使ってスレーター型軌道を近似することにちなんで名付けられています。

__spin__：不対電子数を示す整数値（$2S$ に等しい）。一部のソフトウェアでは多重度（$2S+1$）を使用する点に注意してください。

__charge__：分子の電荷。

__symmetry__ - 分子の点対称群であり、文字列で指定するか、"symmetry = True"を設定することで自動検出されます。ここで"Dooh"は、同じ原子種2つからなる二原子分子に適した対称群です。
...
__rhf__ は、mf内の__mf__ = scf.RHF(mol)のようなもので、制限付きハートリー・フォック計算を使用するソルバーです。この（下記の）Eというカーネルは、原子核間反発と分子軌道を含む全エネルギーです。
