...
VQE自体を実行するための残りのパッケージ、例えばefficient_su2アンザッツやSciPyの最小化ツールなどをロードします。
...
この出力結果については、後処理セクションで詳しく説明します。ここでは単に、シミュレーションが成功したことを確認してください。これで実機で実行する準備が整いました。エラー緩和としてTREX（Twirled Readout Error eXtinction）を使用することを示すために、resilience を `1` に設定します。実機で作業する際は、IBM Quantum Compute Service と IBM Quantum プリミティブを使用します。幾何構造に関するforループと複数の変分試行の両方が、セッションの中に含まれていることに注意してください。
...
これで量子化学のためのVQEコースは終了です。量子コンピューティングで使われる情報理論の基礎をさらに理解したい方は、John Watrousによる[量子情報の基礎](/learning/courses/basics-of-quantum-information)コースを参照してください。VQEワークフローの短いサンプルとして、[VQEによるハイゼンベルク鎖の基底状態エネルギー推定チュートリアル](/tutorials/spin-chain-vqe)も参考にしてください。また、[チュートリアル](https://docs.quantum.ibm.com/tutorials)や[コース](/learning)を閲覧して、量子コンピューティングの最新技術に関する教材をさらに探してみてください。
