...
* Estimatorの組み込みレジリエンス：最適化と同様に、Estimatorにはエラーに対するレジリエンスの組み込み設定があり、異なるエラー軽減手法に対応しています。レジリエンスレベルの設定については、[ドキュメント](/guides/configure-error-mitigation)を参照してください。
