...
これらの状態こそが、データを高次元にマッピングしたものであり、求めるカーネルのエントリは内積です
$$
\langle\psi(\vec{x}_j)|\psi(\vec{x}_i)\rangle = \langle 0 |^{\otimes N}\Phi^\dagger(\vec{x}_j)\Phi(\vec{x}_i)|0\rangle^{\otimes N}.
$$
初期状態 $|0\rangle^{\otimes N}$ に対して回路 $\Phi^\dagger(\vec{x}_j)$ と $\Phi(\vec{x}_i)$ の両方を作用させた場合、状態 $|0\rangle^{\otimes N}$ を測定する確率は
$$
P_0 = |\langle0|^{\otimes N}\Phi^\dagger(\vec{x}_j)\Phi(\vec{x}_i)|0\rangle^{\otimes N}|^2.
$$
これはまさに求めていた値です（$||^2$ を除いて）。回路の測定層は測定確率（あるいは、特定のエラー緩和手法が使用された場合は、いわゆる「準確率」）を返します。関心のある確率は、ゼロ状態 $|0\rangle^{\otimes N}$ のものです。
...
### ステップ3：IBM Quantum プリミティブを使用して実行する {#step-3-execute-using-ibm-quantum-primitives}
...
IBM Quantum `Sampler` プリミティブを使用して、回路をサンプリングして得られる状態の確率分布を再構成します。この用語は、ノイズが問題となり、エラー軽減などの追加ステップが導入された場合に適用される「準確率分布」と呼ばれることがあります。このような場合、すべての確率の合計が正確に 1 にならない場合があります。そのため「準確率」と呼ばれます。
...
### ステップ3：IBM Quantum プリミティブを使用して実行する {#step-3-execute-using-ibm-quantum-primitives-1}
...
量子コンピューティングに不慣れな人が、2量子ビット深度が非常に高い量子回路を使用し、エラー緩和を使用しなかったと仮定しましょう。さらに、これにより各量子ビットに10%のエラー率が生じたと仮定します。この回路に対応する真の（エラーのない）カーネル行列要素が1.0のように非常に大きい場合、10個の量子ビットすべてがそれぞれ $|0\rangle$ の状態にある確率はいくつになるでしょうか？
...
### ステップ4：後処理を行い、古典形式で結果を返す {#step-4-post-process-return-result-in-classical-format-2}
...
### ステップ1：古典的な入力を量子問題にマッピングする {#step-1-map-classical-inputs-to-a-quantum-problem-3}
...
### ステップ2：量子実行のための問題の最適化 {#step-2-optimize-problem-for-quantum-execution-2}
...
### ステップ3：IBM Quantum プリミティブを使用して実行する {#step-3-execute-using-ibm-quantum-primitives-2}
...
### ステップ4：後処理、結果をクラシカル形式で返す {#step-4-post-process-return-result-in-classical-format-3}
