...
上記で導入した記法では、データセット $\text{X}_{\text{ex}}$ における $4$ 番目のデータベクトルの $1$ 番目の特徴量が $\vec{x}^{(4)}_1 = 5$ であると言えます。

### 基底エンコーディング {#basis-encoding}

基底エンコーディングは、古典的な $P$ ビット文字列を $P$ 量子ビット系の計算基底状態にエンコードします。例えば $\vec{x}^{(1)}_3 = 5 = 0(2^3)+1(2^2)+0(2^1)+1(2^0)$ を考えてみましょう。これは $4$ ビット文字列として $(0101)$ と表せ、$4$ 量子ビット系では量子状態 $|0101\rangle$ として表せます。より一般的には、$P$ ビット文字列 $\vec{x}^{(j)}_k = (b_1, b_2, ... , b_P)$ に対応する $P$ 量子ビット状態は $|x^{(j)}_k\rangle = | b_1, b_2, ... , b_P \rangle$ であり、$n = 1 , \dots , P$ に対して $b_n \in \{0,1\}$ です。これは単一の特徴量に対するものであることに注意してください。
...
定義した plot_Nstates 関数を使って、$\phi$ における回転を可視化できます。
...
上記のユニタリを理解する1つの方法は、物理系における伝播子との類推です。上記のユニタリは、Ising モデルに似たハミルトニアン $\mathcal{H}$ に対するユニタリ発展演算子 $\exp(it\mathcal{H})$ であり、時間パラメータ $t$ がデータ値で置き換えられて発展を駆動します。このユニタリ演算子を展開すると PFM 回路が得られます。$S$ における量子もつれの接続性は、スピン格子における Ising 結合として解釈できます。
それらの Ising 型相互作用を表す Pauli の $Y$ と $XX$ 演算子の例を考えてみましょう。Qiskit は、単一量子ビットおよび $n$ 量子ビットゲートの選択によって PFM をインスタンス化するための `pauli_feature_map` クラスを提供しており、この例では Pauli 文字列の `‘Y’` と `‘XX’` が渡されます。通常、$n$ は単一量子ビットおよび2量子ビットの相互作用に対してそれぞれ1または2です。量子もつれのスキームは「linear」であり、量子回路内の最近接量子ビットのみが結合されることを意味します。この量子回路は抽象化層であるため、これは量子コンピューター自体における最近接量子ビットに対応するわけではないことに注意してください。
...
以下のコードは、これら4つの組み込み特徴マップを特徴セットのエンコーディングに適用し、得られた回路の2量子ビット深度をプロットします。2量子ビットのエラー率は単一量子ビットゲートのエラー率よりもはるかに高いため、2量子ビットゲートの深度に最も関心を持つのが妥当でしょう。以下のコードでは、回路をまず分解してから count_ops() を使用することで、回路内のすべてのゲートのカウントを取得します。ここで関心のある2量子ビットゲートは 'cx' ゲートです：
