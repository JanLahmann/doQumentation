...
ここでは、optimization_level を指定し、動的デカップリングを追加することで、実際の Backend で実行するために回路を最適化します。以下のコードは、qiskit.transpiler のプリセット・パスマネージャーを使用してパスマネージャーを生成します。
...
## Qiskit Patterns ステップ3：IBM Quantum プリミティブを使用して実行する {#qiskit-patterns-step-3-execute-using-ibm-quantum-primitives}
...
## Qiskit Patternsステップ2：量子実行のために問題を最適化する {#qiskit-patterns-step-2-optimize-problem-for-quantum-execution-1}
まず実行するバックエンドを選択します。ここでは最も空いているバックエンドを使用します。
...
## Qiskit Patterns ステップ3：IBM Quantum プリミティブを使用して実行する {#qiskit-patterns-step-3-execute-using-ibm-quantum-primitives-1}
