...
アルゴリズムが脱量子化できないとしても、それがすべての古典アルゴリズムより効率的またはスケーラブルであることを意味するわけではありません。極端で作為的な例として、サイズ k の集合から最大の j 個の要素を選び出すアルゴリズムを想像してください。k 個の各要素を Shor のアルゴリズムで素因数分解し、その素因数を使って最大の要素を決定する量子アルゴリズムを書くこともできるでしょう。このようなアルゴリズムはおそらく脱量子化できませんが、同じ最大要素の選択を行う古典アルゴリズム（不要な素因数分解の部分を除く）よりも劇的に非効率です。
...
誤り。ほとんどの機械学習問題は古典アルゴリズムによって非常に効率的に解決されており、量子アルゴリズムが大幅な高速化をもたらす可能性は低いです。QML における目標は、量子状態でうまく記述できる特徴を持つデータセットを見つけることや、モデルの精度を最適化するデータ特徴のマッピングを見つけることです。
...
5. IBM Quantum プリミティブを使用し、Qiskit パターンに従って、量子カーネル法と変分量子分類器を実装します。
...
コースの終わりには20問のクイズがあります。Credly を通じて量子機械学習バッジを取得するには、このクイズで70%以上のスコアを獲得する必要があります。70%以上のスコアを獲得すると、バッジは自動的に間もなくメールで送られてきます。クイズは2回まで提出できます。最初の提出後、間違えた問題に再挑戦する機会があります。2回目の提出後、スコアは確定します。詳細はクイズを参照してください。
...
* ステップ3：IBM Quantum プリミティブを使用して実行する
...
上記のプロセスで、14番目と19番目のデータ点に対するカーネル行列のエントリを計算しました。同じデータ点を2回（例えば14番目と14番目）使用した場合、どのような値が得られるべきでしょうか？言い換えると、カーネル行列の対角成分はどうあるべきでしょうか？ノイズがない場合について答えてください。ただし、ノイズが存在する場合、答えからの逸脱が生じる可能性があることに注意してください。

<Accordion>
<AccordionItem title="Answer">

対角成分は 1.0 であるべきです。このプロセスは、あるベクトルとそれ自身との正規化された内積を計算しているはずであり、それは常に1でなければなりません。
