...
重複ペナルティを計算するために、以下の関数を使用します。これはまだ問題を量子回路にマッピングする作業の一部であることに注意してください。しかし、前のレッスンで説明したように、この関数は現在の変分回路と、以前に得られたより低いエネルギー・コストの状態からの最適化された回路との間の重複を計算します。生成される新しい回路も、実際のハードウェアで実行するためにトランスパイルする必要があります。この関数はシミュレーター上で使用したことがすでにあります。ここでは、実際のバックエンドを使用する際のトランスパイルとそれに関連する最適化をすでに考慮する必要があるため、`if realbackend == 1`の周りの行があります。これは少しステップ2と混ざっていますが、後ほどステップ2を明示的に取り上げます。
...
次に、VQDのコスト関数を追加します。前のレッスンと比較すると、実際のバックエンドを使用する際のトランスパイルを助けるために、2つの追加の引数（`realbackend`と`backend`）があることに注意してください。
...
ここでも、まずデバッグにシミュレーターを使用し、その後実際のハードウェアに移行します。
...
ここでは、計算したい状態の数、ペナルティ、および初期パラメータのセットx0を導入します。
...
コスト関数を最小化するには、任意の古典的な最適化器を使用できます。実際の量子システムでは、滑らかでないコスト関数の風景に対して設計された最適化器の方が通常はうまくいきます。ここでは、SciPyのminimize関数を介して[COBYLAルーチン](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-cobyla.html#optimize-minimize-cobyla)を使用します。

Runtimeへの多くの呼び出しを反復的に実行するため、単一のブロック内ですべての呼び出しを実行するためにセッションを利用します。さらに、QAOAでは、解は最小化から得られる最適なパラメータに束縛されたアンザッツ回路の出力分布に符号化されます。したがって、Samplerプリミティブが必要になるため、同じセッションでこれをインスタンス化します。
そして、最小化ルーチンを実行します。
...
パラメータ角度の解ベクトル（`x`）は、アンザッツ回路に代入すると、求めていたグラフ分割を生成します。
