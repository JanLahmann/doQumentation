...
- まず、[IBM Quantumプリミティブ](/guides/primitives)について学びます。

- *コスト関数* $C(\vec\theta)$ を定義します。これはオプティマイザーが最小化（または最大化）すべき問題の目標を定義する、問題固有の関数です。

- IBM Quantum プリミティブを使った測定戦略を定義し、速度と精度のトレードオフを最適化します。
...
IBM Quantum [`Sampler`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2) は、量子デバイス上で Circuit を複数回実行し、各実行で測定を行い、得られたビット列から確率分布を復元します。実行回数（*ショット数*）が多いほど結果は正確になりますが、より多くの時間と量子リソースが必要です。
...
`Estimator`プリミティブは、量子状態$|\psi\rangle$に対するオブザーバブル$\hat{H}$の期待値を計算します。ここで、オブザーバブルの確率は$p_\lambda = |\langle\lambda|\psi\rangle|^2$と表すことができ、$|\lambda\rangle$はオブザーバブル$\hat{H}$の固有状態です。期待値は、対応する確率で重み付けされた、状態$|\psi\rangle$の測定によって得られるすべての可能な結果$\lambda$（すなわち、オブザーバブルの固有値）の平均として定義されます。

$$
\langle\hat{H}\rangle_\psi := \sum_\lambda p_\lambda \lambda = \langle \psi | \hat{H} | \psi \rangle
$$

しかし、オブザーバブルの固有基底がわからないことが多いため、その期待値を計算することは常に可能とは限りません。IBM Quantumの[`Estimator`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2)は、複雑な代数的プロセスを使用して、オブザーバブルを固有基底のわかっている他のオブザーバブルの組み合わせに分解することで、実際の量子デバイス上での期待値を推定します。

より単純に言うと、`Estimator`は、測定方法がわからない任意のオブザーバブルを、<DefinitionTooltip definition="Set of matrices commonly used in quantum computing to represent and manipulate quantum states, consisting of the identity matrix and the three Pauli matrices (X, Y, and Z).">パウリ演算子</DefinitionTooltip>と呼ばれる、より単純で測定可能なオブザーバブルに分解します。
任意の演算子は、$4^n$個のパウリ演算子の組み合わせとして表すことができます。
...
ここで、$n$はキュビットの数、$k \equiv k_{n-1} \cdots k_0$（$k_l \in \mathbb{Z}_4 \equiv \{0, 1, 2, 3\}$、すなわち4を基数とする整数）、および$(\sigma_0, \sigma_1, \sigma_2, \sigma_3) := (I, X, Y, Z)$です。

この分解を実行した後、`Estimator`は（元の回路から）各オブザーバブル$\hat{P}_k$に対して新しい回路$V_k|\psi\rangle$を導出し、計算基底でパウリ・オブザーバブルを効果的に*対角化*して測定します。$V_k$を事前に知っているため、パウリ・オブザーバブルは容易に測定できます。これは他のオブザーバブルには一般的には当てはまりません。

各$\hat{P}_{k}$について、`Estimator`は対応する回路を量子デバイス上で複数回実行し、計算基底で出力状態を測定して、各可能な出力$j$を得る確率$p_{kj}$を計算します。次に、各出力$j$に対応する$P_k$の固有値$\lambda_{kj}$を求め、$w_k$を掛けて、すべての結果を足し合わせ、与えられた状態$|\psi\rangle$に対するオブザーバブル$\hat{H}$の期待値を得ます。

$$
\langle\hat{H}\rangle_\psi =
\sum_{k=0}^{4^n-1} w_k \sum_{j=0}^{2^n-1}p_{kj} \lambda_{kj},
$$
$4^n$個のパウリの期待値を計算することは（指数関数的に増大するため）実用的ではないため、`Estimator`は大量の$w_k$がゼロである場合（すなわち*密*ではなく*疎*なパウリ分解の場合）にのみ効率的になり得ます。形式的には、この計算が*効率的に解ける*ためには、非ゼロ項の数がキュビット数$n$に対して高々多項式的に増加する必要があると言えます：$\hat{H} = \sum^{\text{Poly}(n)}_k w_k \hat{P}_k$。
...
\end{array}
$$
それでは、$\hat{H}$をパウリと恒等式に関して書き直してみましょう。
...
ここで、$k = \sum_{l=0}^{n-1} 4^l k_l \equiv k_{n-1}...k_0$（$k_{n-1},...,k_0\in \{0,1,2,3\}$、すなわち4を基数とする）、および$\hat{P}_{k} := \sigma_{k_{n-1}}\otimes ... \otimes \sigma_{k_0}$です。
...
ここで、$V_k := V_{k_{n-1}}\otimes ... \otimes V_{k_0}$および$\Lambda_k := \Lambda_{k_{n-1}}\otimes ... \otimes \Lambda_{k_0}$であり、$\hat{P_k}=V_k^\dagger \Lambda_k V_k$を満たします。
...
これから、実際の量子コンピューター上での実行に進みます。構文の変化に注意してください。pass_managerに関する手順は、次の例でさらに詳しく説明します。変分アルゴリズムにおいて特に重要な手順の1つは、IBM Quantum Compute Serviceセッションの使用です。セッションを開始することで、パラメータが更新されるたびに新しいキューで待機することなく、変分アルゴリズムの複数回の反復を実行できます。これは、キューの待ち時間が長い場合や、多くの反復が必要な場合に重要です。Open Planのユーザーは Quantum Compute セッションを使用できないことに注意してください。セッションを利用できない場合は、一度に送信する反復回数を減らし、最新のパラメータを将来の実行のために保存することができます。反復を多く送信しすぎたり、キューの待ち時間が長すぎたりすると、ジョブ送信間の長い遅延を示すエラーコード1217が発生する可能性があります。
...
最大カット（マックスカット）問題は、グラフの頂点を2つの互いに素な集合に分割し、2つの集合間のエッジ数を最大化する組合せ最適化問題です。より正式には、無向グラフ$G=(V,E)$（$V$は頂点の集合、$E$はエッジの集合）が与えられたとき、マックスカット問題は、頂点を2つの互いに素な部分集合$S$と$T$に分割し、一方の端点が$S$、もう一方の端点が$T$にあるエッジの数を最大化することを求めます。

マックスカットは、クラスタリング、ネットワーク設計、相転移など様々な問題を解くために適用できます。まず、問題のグラフを作成することから始めましょう。
...
この問題は二値最適化問題として表現できます。グラフの各ノード$0 \leq i < n$（$n$はグラフのノード数であり、この場合$n=4$です）に対して、二値変数$x_i$を考えます。この変数は、ノード$i$がグループ$1$と呼ぶ方に属する場合は値$1$を、グループ$0$と呼ぶもう一方に属する場合は値$0$をとります。また、隣接行列$w$の要素$(i,j)$である$w_{ij}$を、ノード$i$からノード$j$へ向かうエッジの重みとして表記します。グラフは無向であるため、$w_{ij}=w_{ji}$です。すると、この問題は以下のコスト関数を最大化するものとして定式化できます。
...
この問題を量子コンピューターで解くために、コスト関数をオブザーバブルの期待値として表現します。しかし、Qiskitがネイティブにサポートするオブザーバブルは、$0$と$1$ではなく固有値$1$と$-1$を持つパウリ演算子で構成されています。そのため、以下のように変数を変換します。

ここで、$\vec{x}=(x_0,x_1,\cdots ,x_{n-1})$です。隣接行列$w$を使用することで、すべてのエッジの重みに簡単にアクセスできます。これを使用してコスト関数を求めます。
...
さらに、量子コンピューターの自然な傾向は、最大値ではなく最小値（通常は最も低いエネルギー）を見つけることであるため、$C(\vec{z})$を最大化する代わりに、以下を最小化します。

$$
-C(\vec{z}) =  \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2} z_iz_j -  \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2}
$$

これで、変数が$-1$と$1$の値を取り得る、最小化すべきコスト関数ができたので、パウリ$Z$との次の類似性を利用できます。

$$
z_i \equiv Z_i = \overbrace{I}^{n-1}\otimes ... \otimes \overbrace{Z}^{i} \otimes ... \otimes \overbrace{I}^{0}
$$

言い換えれば、変数$z_i$はキュビット$i$に作用する$Z$ゲートと等価になります。さらに、
...
すると、考えるべきオブザーバブルは次のとおりです。

$$
\hat{H} = \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2} Z_iZ_j
$$

後で、以下の独立項を加える必要があります。

$$
\texttt{offset} = - \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2}
$$
この演算子は、エッジで接続されたノード上のZ演算子を持つ項の線形結合です（0番目のキュビットが一番右にあることを思い出してください）：$IIZZ + IZIZ + IZZI + ZIIZ + ZZII$。演算子が構築されたら、Qiskit回路ライブラリの`QAOAAnsatz`回路を使用することで、QAOAアルゴリズムのアンザッツを簡単に構築できます。
...
IBM Quantum Estimator はハミルトニアンとパラメーター化されたアンザッツを直接受け取り、必要なエネルギーを返します。そのため、QAOA インスタンスのコスト関数は非常にシンプルです：
...
IBM Quantum プリミティブのエラー抑制およびエラー軽減オプションを使用することで、ノイズに対処し、現在の量子コンピューターの有用性を最大化できます。

### エラー抑制 {#error-suppression}

[エラー抑制](/guides/error-mitigation-and-suppression-techniques)とは、エラーを最小化するためにコンパイル時に Circuit を最適化・変換する技術を指します。これは基本的なエラー処理技術であり、通常は全体の実行時間に対して古典的な前処理の<DefinitionTooltip definition="Extra costs introduced by new techniques, relative to a base implementation.">オーバーヘッド</DefinitionTooltip>が生じます。このオーバーヘッドには、量子ハードウェア上で実行するための Circuit の Transpile が含まれます：
...
シミュレーターを使用して、最適化されたトランスパイルの有用性を示すことができます。以下では、実際のハードウェアを使用して誤り緩和の有用性を示すために戻ります。QiskitRuntimeServiceを使用して実際のバックエンド（この場合はibm_brisbane）を取得し、AerSimulatorを使用してそのノイズ挙動を含めてそのバックエンドをシミュレートします。
...
パスマネージャーを使って Circuit を Backend の「命令セットアーキテクチャ（ISA）」にトランスパイルします。これは IBM Quantum Compute Service の新しい要件です。Backend に送信されるすべての Circuit は、Backend のターゲットの制約に準拠していなければなりません。つまり、Backend の ISA（デバイスが理解・実行できる命令セット）に基づいて記述される必要があります。これらのターゲット制約は、デバイスのネイティブな基底ゲート、Qubit の接続性、そして必要に応じてパルスやその他の命令タイミング仕様といった要素によって決まります。

この場合、これを2回行うことに注意してください。1回はoptimization_level = 0で、もう1回はそれを3に設定して行います。それぞれの場合において、Estimatorプリミティブを使用して、位相の異なる値でオブザーバブルの期待値を推定します。
...
[エラー緩和](/guides/error-mitigation-and-suppression-techniques)とは、実行時にデバイスのノイズをモデル化することで、Circuit のエラーを低減できる技術を指します。一般的に、モデルの学習に関連する量子的な前処理オーバーヘッドと、生成したモデルを用いて生の結果のエラーを緩和するための古典的な後処理オーバーヘッドが発生します。

IBM Quantum プリミティブの `resilience_level` オプションは、エラーに対してどの程度の耐性を持たせるかを指定します。レベルが高いほど、量子サンプリングのオーバーヘッドによる処理時間の増加と引き換えに、より正確な結果が得られます。レジリエンスレベルを使用することで、プリミティブクエリにエラー緩和を適用する際のコストと精度のトレードオフを調整できます。

誤り緩和技術を実装する際、結果における<DefinitionTooltip definition="A systematic drift in the measured quantities, usually caused by errors.">バイアス</DefinitionTooltip>が、以前の緩和されていないバイアスに対して減少することを期待します。場合によっては、バイアスが消滅することさえあります。しかし、これには代償が伴います。推定量のバイアスを減らすにつれて、統計的なばらつき（すなわち分散）が増加するため、サンプリング・プロセスにおける回路あたりのショット数をさらに増やすことでこれに対処できます。これにより、バイアスを減らすために必要な分を超えるオーバーヘッドが生じるため、デフォルトでは実行されません。以下の例に示すように、options.executions.shotsで回路あたりのショット数を調整することで、この動作を簡単に選択できます。
...
このコースでは、IBM Quantum プリミティブが実行できるエラー緩和について、完全な実装の詳細を必要とせずに概要レベルで説明するために、これらのエラー緩和モデルを取り上げます。
...
これは `options.resilience_level = 2` で設定できます。さらに、`noise_factors`、`noise_amplifiers`、`extrapolators` などさまざまなオプションを探索することで最適化できますが、それはこのコースの範囲外です。[こちらに記載されているオプション](/guides/error-mitigation-and-suppression-techniques)を試してみることをお勧めします。
...
### 緩和・抑制のためのQuantum Computeのオプションの使い方 {#using-the-options-in-quantum-compute-for-mitigation-and-suppression}

以下に、IBM Quantum Compute Service でエラー緩和と抑制を使用しながら期待値を計算する方法を示します。先ほどとまったく同じ Circuit とオブザーバブルを利用できますが、今回は最適化レベルをレベル 2 に固定したまま、使用する_レジリエンス_（エラー緩和技術）を調整します。このエラー緩和プロセスは、最適化ループ全体を通じて複数回実行されます。
...
- IBM Quantumプリミティブを活用してノイズを緩和・抑制する方法
