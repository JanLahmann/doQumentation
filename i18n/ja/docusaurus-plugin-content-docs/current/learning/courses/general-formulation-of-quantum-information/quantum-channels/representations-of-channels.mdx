...
### 完全デフェーズチャネル {#completely-dephasing-channel-1}
...
### Qubitリセットチャネル {#qubit-reset-channel-1}
...
### 完全デポラライゼーションチャネル {#completely-depolarizing-channel}
...
右辺の $\mathsf{X}$ のコピーに $\Phi$ を適用すると、$n$ で割ったチョイ行列が得られます。
...
言い換えると、正規化係数 $1/n$ を除いて、$\Phi$ のチョイ行列は、次の図が示すように、*最大にもつれた*入力システムのペアの半分に $\Phi$ を評価することで得られる密度行列です。
...
また、チャネル $\Phi$ は右/上のシステムのみに適用されるため、左/下のシステムの縮約状態に影響を与えることはできないことも分かります。
この場合、その状態は完全混合状態 $\mathbb{I}_{\mathsf{X}}/n$ であり、したがって
...
また、チャネルは常にトレースを保存しなければならないという事実を使って、同じ結論を導くこともできます。したがって、
...
まとめると、任意のチャネル $\Phi$ に対するチョイ表現 $J(\Phi)$ は半正定値でなければならず、次を満たさなければなりません。
...
### 完全脱位相化チャネル {#completely-dephasing-channel-2}
...
### 完全デポラライジングチャネル {#completely-depolarizing-channel-1}
...
### Qubitリセットチャネル {#qubit-reset-channel-2}
