...
## 2. 回路の最適化が重要な理由 {#2-circuit-optimization-matters}
...
### 2.1 最適化レベル {#21-optimization-level}
`optimization_level` には0から3までの4つのオプションがあります。最適化レベルが高いほど、回路の最適化により多くの計算努力が費やされます。レベル0は最適化を行わず、選択した Backend で回路を実行可能にするための最低限の作業のみを行います。レベル3は、回路を最適化するために最も多くの努力（そして通常は実行時間）を費やします。レベル1がデフォルトの最適化レベルです。
最適化なし（`optimization_level=0`）と最適化あり（`optimization_level=2`）で回路をトランスパイルします。
トランスパイルされた回路の長さに大きな違いが見られます。
...
## 3. 回路の合成が重要な理由 {#3-circuit-synthesis-matters}
次に、異なる方法で合成された2つの5量子ビット GHZ 状態（$\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$）準備回路を実行した結果を比較します。
...
新しい回路で行ったことは、手動による「結合制約を考慮した」回路合成と見なすことができます。言い換えると、回路合成と回路マッピングを同時に手動で解いたということです。
...
一般に、回路合成はアプリケーションに依存し、ソフトウェアがすべての可能なアプリケーションをカバーするのは困難です。Qiskit の Transpiler は、たまたま GHZ 状態準備回路を合成する機能を持っていません。このような場合、上記で示したような手動での回路合成を検討する価値があります。
このセクションでは、次のおもちゃの例の回路を使って、Qiskit の Transpiler がどのように動作するかの詳細を見ていきます。
...
### 3.1 Qiskitトランスパイルフロー全体を描画する {#31-draw-the-entire-qiskit-transpilation-flow}
...
### 3.2 個別ステージを描画する {#32-draw-an-individual-stage}
...
`translation` ステージについても同じことを行います。
...
注：個々のステージは、常に独立して実行できるとは限りません（一部のステージは、前のステージから情報を引き継ぐ必要があるためです）。

### 3.4 最適化ステージ {#34-optimization-stage}
...
## 4. 詳細な例 {#4-in-depth-examples}

### 4.1 2量子ビットユニタリー合成による2量子ビットブロック最適化 {#41-two-qubit-block-optimization-using-two-qubit-unitary-synthesis}
...
### 4.2 Circuit 最適化の重要性 {#42-circuit-optimization-matters}
...
最適化なし（`optimization_level=0`）と最適化あり（`optimization_level=2`）で回路をトランスパイルします。
ご覧のとおり、トランスパイルされた回路の長さに大きな違いがあります。
...
### 4.3 回路合成の重要性 {#43-circuit-synthesis-matters}
次に、異なる方法で合成された2つの5量子ビット GHZ 状態（$\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$）準備回路を実行した結果を比較します。
...
### 4.4 一般的な1量子ビットゲートの分解 {#44-general-1-qubit-gate-decomposition}
...
### 4.5 1量子ビットブロックの最適化 {#45-one-qubit-block-optimization}
...
### 4.6 トフォリ分解 {#46-toffoli-decomposition}
...
### 4.7 CUゲートの分解 {#47-cu-gate-decomposition}
...
### 4.8 CX、ECR、CZはローカルクリフォードまで等価 {#48-cx-ecr-cz-equal-up-to-local-cliffords}
