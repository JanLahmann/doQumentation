...
任意のユーティリティスケールの量子問題を解くには、量子コンピューター上であるキュビットから別のキュビットへ情報を移動させる必要があります。これを行うためのよく知られたプロトコルがありますが、その中でも最も基本的なものの一部は、遠く離れた当事者間で情報を送信するという文脈で考案されました。このレッスンを通して、「遠く離れた友人に情報を送る」というような、この文脈に沿った表現を使用することがあります。しかし、これらのプロトコルには量子コンピューティングにおけるより広範な意義があることを念頭に置いてください。このレッスンでは、以下の量子通信プロトコルを検討します。
- **量子テレポーテーション**
    共有されたエンタングル状態（e-bitと呼ばれることもあります）を使用して、未知の量子状態を遠く離れた友人に送信する方法で、追加の古典通信が必要です。
- **量子超高密度符号化**
    単一のキュビットを遠く離れた友人に送信することで、2ビットの情報を送信する方法（これも事前に共有されたエンタングル・キュビットを使用します）。

これらのトピックに関するより詳しい背景については、Basics of Quantum Informationのレッスン4にある[Entanglement in action](/learning/courses/basics-of-quantum-information/entanglement-in-action/introduction)をお勧めします。

上記の説明において、「未知の量子状態」とは、単に前のレッスンで説明した以下の形式の状態を指します。
$$
...
\right)
$$
ランダムな任意の量子状態の情報を転送できるようにしたいので、そのような状態を生成することがこのレッスンの出発点になります。

## 2. 密度行列 {#2-density-matrices}
...
## 3. 量子状態トモグラフィー {#3-quantum-state-tomography}
...
### 演習1: $r_x$ 値の推定 {#exercise-1-estimate-r-x-value}
...
### 演習2：$r_y$の値を推定する {#exercise-2-estimate-r-y-value}
...
これで$\vec{r}$のすべての成分を推定できたので、完全なベクトルを書き出すことができます。
...
### 4.1 量子テレポーテーションのプロトコル {#41-the-protocol-of-quantum-teleportation}
**前提**：アリスはボブに送りたい未知の量子状態 $|\psi \rangle$ を持っています。アリスとボブは2 Qubitのエンタングル状態（eビット）を共有しており、それぞれが物理的に自分の場所でそのうちの1 Qubitを持っています。

ここでは、説明なしに手順の概要を示します。これらは以下で詳細に実装されます。
1. アリスはCNOTゲートを使用して、$|\psi \rangle$を彼女のe-bitの部分とエンタングルさせます。
2. アリスは$|\psi \rangle$にアダマールゲートを適用し、彼女の両方のキュビットを計算基底で測定します。
3. アリスはボブに測定結果（「00」、「01」、「10」、または「11」のいずれか）を送信します。
4. ボブはアリスの2ビットの情報に基づいて、彼のe-bitペアの部分に*補正*演算子を実行します。
    - 「00」の場合、ボブは何もしません
    - 「01」の場合、ボブはXゲートを適用します
    - 「10」の場合、ボブはZゲートを適用します
    - 「11」の場合、ボブはiY = ZXゲートを適用します
5. ボブのe-bitの部分は$|\psi \rangle$になります。
...
### 4.2 量子テレポーテーションをシミュレートする量子Circuit {#42-quantum-circuit-simulating-the-quantum-teleportation}
...
### 4.3 実行し、Uの逆行列の適用で結果を確認する {#43-execute-it-and-confirm-the-result-by-applying-u-inverse}
...
### 4.4 実際の量子コンピューターでのテレポーテーション {#44-teleportation-on-a-real-quantum-computer}
