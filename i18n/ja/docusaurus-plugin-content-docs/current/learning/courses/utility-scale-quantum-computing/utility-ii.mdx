...
# ユーティリティスケール実験 II {#utility-scale-experiment-ii}
...
### 2.2 アクティビティ 2 {#22-activity-2}

#### ハミルトニアンを定義する {#define-the-hamiltonian-1}
...
### 2.4 アクティビティ 4 {#24-activity-4}

#### 70 qubit 問題向けの量子 Circuit 2（時間発展のための単一 Circuit）を準備する {#prepare-the-quantum-circuit-2-single-circuit-for-time-evolution-for-the-70-qubit-problem}
ここでは、Lie–Trotterを使用して単一の時間ステップのための回路を構築します。20キュビットの場合とまったく同様に、Lie積公式（1次）は[LieTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter)クラスに実装されています。ここでも、1次公式は上記で述べた近似から構成されます。
...
### 2.5 アクティビティ 5 {#25-activity-5}

#### 測定する演算子を設定する {#set-the-operators-to-be-measured-1}
...
### 2.6 アクティビティ 6 {#26-activity-6}

#### 時間発展シミュレーションを実行する {#perform-time-evolution-simulation-1}
...
## 3. 実際の量子コンピューターを使った解法 II {#3-solution-using-a-real-quantum-computer-ii}

Qiskit パターンのステップ 1 に戻り、Circuit の深さを削減できるか検討します。

###  3.1 ステップ 1. 問題を量子 Circuit と演算子にマッピングする {#31-step-1-map-the-problem-to-quantum-circuits-and-operators}
