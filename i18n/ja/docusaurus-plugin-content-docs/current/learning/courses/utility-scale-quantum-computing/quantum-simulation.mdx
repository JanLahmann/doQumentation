...
### 2.7 観測量の時間発展をプロットする {#27-plot-the-time-evolution-of-the-observables}
...
## 3. 演習1：2次鈴木–トロッター法によるシミュレーションの実行 {#3-exercise-1-perform-simulation-using-second-order-suzukitrotter}
...
### 3.1 1タイムステップ分の回路を構成する {#31-construct-a-circuit-for-a-single-time-step}
product_formula_st2（SuzukiTrotter(order=2)）を使用し、2次の Suzuki–Trotter を使って単一の時間ステップの回路を構築してください。また、ゲート数と回路の深度を数え、Lie–Trotter と比較してください。
...
### 3.2 時間発展シミュレーションを実行する {#32-perform-time-evolution-simulation}
...
### 3.3 2次鈴木–トロッター法の結果をプロットする {#33-plot-the-second-order-suzukitrotter-results}
...
### 3.4 厳密解と比較する {#34-compare-with-exact-results}
...
## 4. 量子ハードウェア上での実行 {#4-executing-on-the-quantum-hardware}
...
### 4.3 ステップ3：IBM Quantum プリミティブで実行する {#43-step-3-execute-with-ibm-quantum-primitives}
...
product_formula_st4（SuzukiTrotter(order=4)）を使用し、4次の Suzuki–Trotter を使って単一の時間ステップの回路を構築してください。また、ゲート数と回路の深度を数え、Lie–Trotter および2次の Suzuki–Trotter と比較してください。
