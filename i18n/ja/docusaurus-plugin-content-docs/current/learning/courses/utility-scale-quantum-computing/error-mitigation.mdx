...
## 2. エラー緩和なしのノイジーシミュレーション {#2-noisy-simulation-without-error-mitigation}
...
### 2.1 テスト回路の構築 {#21-build-a-test-circuit}
...
### 2.2 ノイズモデルの構築 {#22-build-a-noise-model}
...
### 2.3 ノイズモデルを使ったノイジーサンプラーの構築 {#23-build-a-noisy-sampler-with-the-noise-model}
...
### 2.4 ノイジーサンプラー上で量子回路を実行する {#24-run-quantum-circuits-on-the-noisy-sampler}
...
### 2.6 理想シミュレーション {#26-ideal-simulation}
...
### 2.8 より現実的なノイジーシミュレーション {#28-more-realistic-noisy-simulation}
...
## 3. エラー緩和を用いた実際の量子計算 {#3-real-quantum-computation-with-error-mitigation}
...
### 3.2 回路のトランスパイル {#32-transpile-the-circuits}
...
### 3.3 Estimator を使った実行（異なるレジリエンスレベル） {#33-execute-using-estimator-with-different-resilience-levels}

レジリエンスレベル（`estimator.options.resilience_level`）の設定は、Qiskit Estimator を使用する際にエラー緩和を適用する最も簡単な方法です。Estimator は以下のレジリエンスレベルをサポートしています（2024年6月28日時点）。詳細については、[エラー緩和の設定](/guides/error-mitigation-and-suppression-techniques)ガイドを参照してください。
...
## 4. （オプション）エラー緩和オプションのカスタマイズ {#4-optional-customize-error-mitigation-options}
...
- [エラー緩和の設定](/guides/error-mitigation-and-suppression-techniques)
