...
Qiskit は、量子デバイスに関する静的および動的な情報を取得するための API を提供します。デバイスとのインターフェースには Backend インスタンスを使用し、これには Target インスタンス（命令セット・アーキテクチャー（ISA）やそれに関連するプロパティや制約などの重要な特徴をまとめた抽象的なマシンモデル）が含まれます。
これらの Backend インスタンスを使って、IBM Quantum&reg; Platform の[Compute resources](https://quantum.cloud.ibm.com/computers) ページに表示される情報の一部を取得してみましょう。まず、対象のデバイスに対する Backend インスタンスを作成します。以下では、「ibm_kyoto」、「ibm_kawasaki」、または最も空いている Eagle マシンを選択します。QPU へのアクセスは異なる場合がありますので、それに応じて Backend 名を更新してください。
...
次に、ゲート・エラーについて見ていきます。まず、Target インスタンスのデータ構造を調べます。これは、キーが操作名であるディクショナリです。
