...
### 2.1. 直感 <a id="intuition"></a> {#21-intuition}

量子フーリエ変換（QFT）は、計算基底（Z 基底）とフーリエ基底という 2 つの基底間の変換を行います。ではここでのフーリエ基底とはどのような意味でしょうか？フーリエ変換 $F(\omega)$ は関数 $f(x)$ を周波数 $\omega$ の正弦波関数に畳み込んだものを表すことを思い出してください。平たく言えば、フーリエ変換は $f(x)$ を正弦波（または余弦波）で構成するために各周波数 $\omega$ がどれだけ必要かを示す関数です。QFT がこの文脈でどのような意味を持つかをより直感的に理解するため、4 つの qubit を使って整数を 2 進数で符号化した以下のステップ画像を参照してください：
...
（フーリエ基底における状態を表すのに、チルダ（~）を使うことがよくあります）。

（フーリエ基底の状態はチルダ（~）で表すことが多いです。）

フーリエ基底では、Z 軸周りの異なる回転を使って数を格納します：

保存したい数値によって、各量子ビットが Z 軸周りにどの角度で回転するかが決まります。状態 $|\widetilde{0}\rangle$ では、すべての量子ビットが状態 $|{+}\rangle$ にあります。上記の例で見たように、状態 $|\widetilde{5}\rangle$ を4量子ビットにエンコードするために、最も左の量子ビットを $\tfrac{5}{2^n} = \tfrac{5}{16}$ 回転（$\tfrac{5}{16}\times 2\pi$ ラジアン）させました。次の量子ビットはこれの2倍（$\tfrac{10}{16}\times 2\pi$ ラジアン、つまり $10/16$ 回転）回転させ、その次の量子ビットではさらにこの角度が2倍になり、以降も同様に続きます。

改めて、各量子ビットが変化する頻度に注目してください。この場合、最も左の量子ビット（`qubit 0`）が最も低い周波数を持ち、最も右の量子ビットが最も高い周波数を持ちます。
...
$N = 2^n$ に対する変換 $QFT_{N}$ を、状態 $\vert x \rangle = \vert x_1\ldots x_n \rangle$ に作用させる形で一般化してみましょう。
$$
\begin{aligned}
...
(2) $|14\rangle$ に QFT を適用し、Bloch 球を使って状態ベクトルをシミュレートしてプロットします。
...
### 3.2 例：T ゲートの QPE {#32-example-t-gate-qpe}
...
## 4. IBM Quantum の Sampler プリミティブを使用した実行 {#4-execution-using-the-ibm-quantum-sampler-primitive}
...
### 4.1 ステップ 1：問題を量子回路と演算子にマッピングする {#41-step-1-map-problem-to-quantum-circuits-and-operators}
...
### 4.2 ステップ 2：ターゲットハードウェア向けに最適化する {#42-step-2-optimize-for-target-hardware}
...
### 4.3 ステップ 3：ターゲットハードウェアで実行する {#43-step-3-execute-on-target-hardware}
...
### 4.4 ステップ 4：結果を後処理する {#44-step-4-post-process-the-results}
