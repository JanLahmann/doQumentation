...
# ユーティリティスケール実験 I {#utility-scale-experiment-i}
...
このレッスンでは、2023年6月15日発行のNature Vol 618に掲載された、私たちが非公式に[「ユーティリティ論文」](https://www.nature.com/articles/s41586-023-06096-3)と呼んでいる論文に登場するユーティリティスケールの回路を実行します。この論文は2次元横磁場イジングモデルの時間発展を扱っています。特に、彼らはハミルトニアン
$$
H = H_{ZZ} + H_X = - J \sum_{(i,j)} Z_i Z_j + h \sum_{i} X_i
$$
の時間発展を考察しています。ここで、$J > 0$は$i < j$を満たす最近接スピン間の結合であり、$h$はグローバルな横磁場です。
彼らは時間発展演算子の1次のトロッター分解を用いて、初期状態からのスピン動力学をシミュレートしています。
...
\end{aligned}
$$
ここで、発展時間$T$は$T / \delta t$個のトロッター・ステップに離散化されており、$\mathrm{R}_{Z_i Z_j}(\theta_J)$と$\mathrm{R}_{X_i}(\theta_h)$はそれぞれ$ZZ$回転ゲートと$X$回転ゲートです。

ここで $J > 0$ は $i < j$ を満たす最近接スピン間の結合定数、$h$ はグローバル横磁場です。初期状態からのスピンダイナミクスを、時間発展演算子の一次トロッター分解を用いてシミュレートします：

さらに、実験上の単純化のため、彼らは$\theta_J=-\pi /2$の場合に焦点を当てています。

この論文の新規性は、統計ベクトル・シミュレーションの限界を超える規模の量子回路を構築し、ノイズのある量子コンピューターでそれを実行して、信頼性のある結果の抽出に成功したことです。すなわち、彼らはノイズのある量子コンピューターの有用性（ユーティリティ）を実証したのです。その際、ノイズのあるデバイスから生じる誤差を軽減するために、確率的誤差増幅（PEA）を伴うゼロノイズ外挿（ZNE）を適用しました。

それ以来、私たちはこのような実験や回路を「ユーティリティスケール」と呼んでいます。

### 1.1 目標 {#11-your-goal}

このレッスンの目標は、ユーティリティスケールの回路を構築し、Eagleプロセッサー上で実行することです。信頼性のある結果を抽出することは、このノートブックの範囲を超えています。これは、執筆時点でPEAがQiskitの実験的機能であることと、PEAを用いたZNEの適用にかなりの時間がかかることが理由の一部です。

具体的には、論文の図4bに対応する回路を構築して実行し、あなた自身の「未緩和」のポイントをプロットすることが求められます。ご覧のとおり、これは$\langle Z_{62} \rangle$をオブザーバブルとする127キュビット$\times$60層（20トロッター・ステップ）の回路です。
![image.png](/learning/images/courses/utility-scale-quantum-computing/utility-i/utility1.avif)
大変そうに聞こえますか？　&nbsp; 心配いりません。このコースの最後の3つのレッスンが足がかりを提供します。まず、より小規模な実験として、フェイクデバイス上で$\langle Z_{13} \rangle$をオブザーバブルとする27キュビット$\times$6層（2トロッター・ステップ）の回路を構築して実行する方法を示します。
...
これらのルーチンを実際に試してみましょう。27キュビット$\times$15層（5トロッター・ステップ）の回路が表示されるはずです。フェイクデバイスには28本のエッジがあるため、28*5個のエンタングリング・ゲートがあるはずです。
...
同じユーザーが上記と同じ環境で次のセルを実行したところ、Wall Timeで2分19秒かかりました。フェイクデバイス上で回路を実行すると、ノイズのあるシミュレーションが呼び出され、正確なシミュレーションよりもはるかに長い時間がかかります。フェイクデバイス上でより大きな回路（3トロッター・ステップの27キュビット$\times$9層など）を実行することはお勧めしません。
