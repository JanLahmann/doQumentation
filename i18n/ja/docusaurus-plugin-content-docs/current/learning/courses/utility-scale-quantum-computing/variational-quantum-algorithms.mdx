...
（このノートブックは、Open Planの制限時間内に評価が完了しない場合があります。量子コンピューティングのリソースを大切に使ってください。）
...
様々な誤り緩和・抑制技術の適用により、QAOAはユーティリティスケールで対象問題に対する最適解を提供できる可能性を秘めています。VQEには（量子化学など）多くの応用がありますが、それらの拡張性は劣ります。しかし、VQEを補完・拡張するために、クリロフ部分空間対角化やサンプリングベースの量子対角化（SQD）など、固有値に関連する様々なアプローチが登場しています。VQEを理解することは、これまで登場してきた幅広い古典・量子ハイブリッド・アルゴリズムを理解するための重要な第一歩です。
...
### 3.1 （小規模）最適化問題のQiskitパターン {#31-small-scale-qiskit-pattern-for-optimization}
このパートでは、小規模なマックスカット問題を用いて、量子コンピューターを使って最適化問題を解くために必要な手順を説明します。
マックスカット問題は解くのが難しい（より具体的にはNP困難問題である）最適化問題であり、クラスタリング、ネットワーク科学、統計物理学など様々な応用があります。このチュートリアルでは、エッジで接続されたノードから成るグラフを考え、カットされるエッジの数が最大になるように、ノードを2つの集合に「カット」することで分割することを目指します。
!["Maxcut"](/learning/images/courses/utility-scale-quantum-computing/variational-quantum-algorithms/maxcut.avif)
この問題を量子アルゴリズムにマッピングする前に少し背景を説明すると、まず関数$f(x)$の最小化を考えることで、マックスカット問題がどのように古典的な組合せ最適化問題になるかをよりよく理解できます。

$$
\min_{x\in \{0, 1\}^n}f(x),
$$

ここで、入力$x$はグラフの各ノードに対応する成分を持つベクトルです。そして、これらの各成分を$0$または$1$（それぞれカットに含まれるか含まれないかを表す）に制約します。この小規模な例では、$n=5$個のノードを持つグラフを使用します。

ノードのペア$i,j$の関数を書いて、対応するエッジ$(i,j)$がカットに含まれるかどうかを示すことができます。例えば、関数$x_i + x_j - 2 x_i x_j$は、$x_i$または$x_j$のどちらか一方だけが1の場合（つまりエッジがカットに含まれる場合）にのみ1となり、それ以外の場合は0になります。カットに含まれるエッジを最大化する問題は、次のように定式化できます。

$$
\max_{x\in \{0, 1\}^n} \sum_{(i,j)} x_i + x_j - 2 x_i x_j,
$$

これは次のような最小化の形に書き換えることができます。

$$
\min_{x\in \{0, 1\}^n} \sum_{(i,j)}  2 x_i x_j - x_i - x_j.
$$

この場合、$f(x)$の最小値は、カットによって横断されるエッジの数が最大となるときです。ご覧のとおり、ここまでは量子コンピューティングに関連するものは何もありません。この問題を、量子コンピューターが理解できる形に再定式化する必要があります。
$n=5$個のノードを持つグラフを作成して、問題を初期化しましょう。
...
### 3.2 ステップ1：古典的入力を量子問題にマッピングする {#32-step-1-map-classical-inputs-to-a-quantum-problem}
...
生成する量子回路は$\gamma_i$と$\beta_i$によって**パラメータ化**されているため、$\gamma_i$と$\beta_i$の異なる値を試し、得られた状態からサンプリングすることができます。
...
この場合、2つのパラメータ$\gamma_1$と$\beta_1$を含む1つのQAOA層を使用する例を試してみます。
...
### 3.3 ステップ2：量子ハードウェア実行に向けてCircuitを最適化する {#33-step-2-optimize-circuits-for-quantum-hardware-execution}
...
トランスパイルの詳細については[ドキュメント](/guides/transpile)を参照してください。
...
ローカルテストモードの詳細については[ドキュメント](/guides/local-testing-mode)を参照してください。
...
### 3.4 ステップ3：IBM Quantumプリミティブを使って実行する {#34-step-3-execute-using-ibm-quantum-primitives}
...
**IBM Quantum プリミティブ**を使って IBM&reg; Backend と通信します。2つのプリミティブは Sampler と Estimator であり、量子コンピューター上で実行したい測定の種類によって選択します。$H_C$ の最小化には Estimator を使用します。コスト関数の測定は単に $\langle H_C \rangle$ の期待値を取ることだからです。

#### 実行 {#run}

プリミティブは、量子デバイス上でワークロードをスケジュールするための様々な[実行モード](/guides/execution-modes)を提供しており、QAOAのワークフローはセッション内で反復的に実行されます。
...
Samplerベースのコスト関数をSciPyの最小化ルーチンに組み込んで、最適なパラメータを見つけることができます。
...
最適化器は、コストを削減し、回路のより良いパラメータを見つけることができました。
...
回路の最適なパラメータが見つかったら、これらのパラメータを割り当てて、最適化されたパラメータで得られた最終的な分布をサンプリングできます。ここで*Sampler*プリミティブを使用する必要があります。なぜなら、それはグラフの最適なカットに対応するビット列測定の確率分布だからです。

**注：** これは、コンピューター内で量子状態$\psi$を準備し、それを測定することを意味します。測定によって状態は単一の計算基底状態に収縮します。例えば、`010101110000...`は、初期の最適化問題（タスクに応じて$\max f(x)$または$\min f(x)$）に対する候補解$x$に対応します。
...
### 3.5 ステップ4：後処理と古典形式での結果の返却 {#35-step-4-post-process-return-result-in-classical-format}
