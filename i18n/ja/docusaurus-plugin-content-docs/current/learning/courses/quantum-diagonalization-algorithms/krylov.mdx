---
title: "クリロフ量子対角化"
sidebar_label: "クリロフ量子対角化"
description: "クリロフ量子対角化（KQD）について、古典的なクリロフ法から始めて解説します。収束性とリソースの集中度についても議論します。"
...
同僚の研究者がクリロフ法を小さなトイ行列に適用して示そうとしています。行列 $A$ と初期ベクトル $|\psi\rangle$ の選択に問題はありますか？
...
![真の固有ベクトル x、推定値 x0、およびその2つの間の誤差のグラフ表現](/learning/images/courses/quantum-diagonalization-algorithms/krylov/kqd-fig2.svg)
...
![真の固有ベクトルと初期推定値の更新。更新後の推定値は真の固有ベクトルにより近くなっています。](/learning/images/courses/quantum-diagonalization-algorithms/krylov/kqd-fig3.svg)
...
![前のコードセルの出力](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/0912e12b-63bd-4026-ac8e-56bc5db8736e-0.avif)
...
__注意点:__ 後で詳しく説明するように、クリロフ量子対角化（KQD）は、行列表現が疎であるか、少数の可換なパウリ演算子のグループで書ける演算子に最も有効に適用されます。ここで使用するランダム行列はその条件を満たしていません。これらはあくまで、古典的なクリロフ法が有用になりうる規模を調べるためのものです。
...
![前のコードセルの出力](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/d9cb6e36-9ad3-403f-823f-0072e9cf53fe-0.avif)

上の図において、次元が約125付近で異常に高い時間が見られますが、これは行列のランダム性や、使用した古典プロセッサでの実装によるものであり、再現性はありません。コードを再実行すると、異なるプロファイルと異なる異常なピークが現れます。
...
ゲート $\text{Prep} \; \psi_0$ が所望の参照状態 $\ket{\psi_0} = \text{Prep} \; \psi_0 \ket{0} = e^{-i H 0 dt} U_{\psi_0} \ket{0}$ を準備するとすれば、例えば化学の HF 状態を準備するために $\text{Prep} \; \psi_0$ は単一量子ビット NOT の積となり、制御付き $\text{Prep} \; \psi_0$ は単純に CNOT の積になります。
...
![前のコードセルの出力](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/8d35e6cd-c0f6-4871-83da-86472d66be2c-0.avif)
...
![前のコードセルの出力](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/d1d0b9de-65d4-4a46-975d-6cfaaac05f9a-0.avif)
...
![前のコードセルの出力](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/37382668-1999-4475-b50f-2887449d5c93-0.avif)
...
![前のコードセルの出力](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/054bcfba-fde3-4213-bbd7-d7a6dd6c03c8-1.avif)
...
![前のコードセルの出力](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/a5d4a983-1a30-4cea-b695-3e6a67338633-0.avif)
...
量子クリロフ対角化は、ハミルトニアン中の可換パウリグループの数が比較的少ない場合に有用となりうることがわかります。これらのスケーリング依存性は、クリロフ法が有効なアプリケーションとそうでないアプリケーションを示唆しています。
