...
## 1. クリロフ法の概要 {#1-introduction-to-krylov-methods}

__クリロフ部分空間法__ とは、__クリロフ部分空間__ と呼ばれるものを中心に構築されたいくつかの手法のいずれかを指すことができます。これらの完全なレビューはこのレッスンの範囲を超えていますが、参考文献 [\[2-4\]](#references) はいずれもさらに多くの背景情報を提供してくれます。ここでは、クリロフ部分空間とは何か、それが固有値問題を解く際にどのように役立つのか、そして最終的に量子コンピューター上でどのように実装できるかに焦点を当てます。
__定義：__ 対称かつ半正定値の $N\times N$ 行列 $A$ が与えられたとき、次数 $r$ のクリロフ空間 $\mathcal{K}^r$ とは、参照ベクトル $\vert v \rangle$ に行列 $A$ のより高い冪（$r-1\leq N$ まで）を掛けて得られるベクトルによって張られる空間です。
...
### 1.1 簡単な例：手計算によるクリロフ法 {#11-a-simple-example-krylov-by-hand}
...
よくある誤解は、ある問題に対してクリロフ部分空間はただ1つしか存在しないというものです。しかし当然ながら、行列を適用できる初期ベクトルは多数存在するため、可能なクリロフ部分空間も多数存在します。「__the__ クリロフ部分空間」という表現は、特定の例に対してすでに定義された特定のクリロフ部分空間を指す場合にのみ使用します。一般的な問題解決アプローチについては「__a__ クリロフ部分空間」と表現します。
最後に補足すると、「クリロフ __空間__」と呼ぶのも正しいです。「クリロフ __部分空間__」と呼ばれることが多いのは、初期空間から部分空間へ行列を射影する文脈で使われることが多いためです。この文脈に合わせて、ここでは主に部分空間と呼ぶことにします。
...
### 理解度の確認 {#check-your-understanding-1}
...
古典的な文脈でHの冪を直接使って生成した次数rのクリロフ部分空間を「パワー・クリロフ部分空間」と呼ぶことにしましょう。
...
ここで、$U$ の冪は異なる時間ステップになります（$U$ の $k$ 乗は時間 $k \times dt$ だけ前進することに相当します）。合計時間 $k dt$ だけ時間発展させた部分空間の要素を $|\psi_k\rangle$ とラベル付けできます。
$$
\begin{aligned}
U&=e^{-iHdt}\\
U^k&=e^{-iH(kdt)}\\
...
\end{aligned}
$$
このユニタリー・クリロフ部分空間 $\mathcal{K}_U^r$ にハミルトニアンHを射影することができます。言い換えれば、$\mathcal{K}_U^r$ 基底における $H$ の各行列要素を計算します。この射影された行列を $\tilde{H}$ と呼ぶことにします。
...
$$
ここで $P_\alpha$ は $P_\alpha \sim IZIXII...YZXIX$ の形のパウリ文字列、または互いに交換可能なそのようなパウリ文字列の集合です。$H$ をこのような測定可能な演算子の和として書けることを考えると、以下に示す $\tilde{H}$ の行列要素の式は、IBM Quantum プリミティブ Estimator を使用して実現できます。
...
ここで、$\vert \psi_n \rangle = e^{-i H t_n} \vert \psi \rangle$ はユニタリー・クリロフ空間のベクトルであり、$t_n = n dt$ は選択された時間ステップ $dt$ の倍数です。量子コンピューター上では、各行列要素の計算は、量子状態間のオーバーラップを取得できる任意のアルゴリズムを使って行うことができます。このレッスンではHadamardテストに焦点を当てます。$\mathcal{K}_U$ の次元が $r$ であることを考えると、部分空間に射影されたハミルトニアンは $r \times r$ の次元を持ちます。$r$ が十分小さければ（一般的に固有値の推定値の収束を得るには $r<<100$ で十分です）、射影されたハミルトニアン $\tilde{H}$ を古典的に容易に対角化できます。しかし、クリロフ空間ベクトルの非直交性のため、$\tilde{H}$ を直接対角化することはできません。それらのオーバーラップを測定し、行列 $\tilde{S}$ を構築する必要があります
...
この一般化固有値問題の解を調べることで、$H$ の固有値と固有状態の推定値を得ることができます。例えば、基底状態エネルギーの推定値は、最小の固有値 $E$ を取ることで得られ、基底状態は対応する固有ベクトル $\vec{c}$ から得られます。$\vec{c}$ の係数は、$\mathcal{K}_U$ を張る様々なベクトルの寄与度を決定します。

#### 一般化固有値問題 {#generalized-eigenvalue-problem}

なぜ単純に $\tilde{H}$ を対角化できないのでしょうか？$\tilde{S}$ はクリロフ基底の幾何学に関する情報（非常に特殊な場合を除き非直交である）を含んでいるため、$\tilde{H}$ 単独では完全なハミルトニアンの射影を記述しておらず、その固有値は完全なハミルトニアンの固有値と特定の関係を持ちません――それらは任意のランダムな値になり得ます。完全なハミルトニアンをクリロフ空間に射影したものに対応する近似固有値と固有ベクトルを得るには、一般化固有値問題を解く必要があります。
![異なる状態で修正されたHadamardテストを実行するために回路を何度も使用する必要があることを示す、多数のレイヤーを持つ回路図。](/learning/images/courses/quantum-diagonalization-algorithms/krylov/kqd-fig4.avif)
...
$\tilde{H}$ とGram行列 $\tilde{S}$ の両方の行列要素をHadamardテストを使って計算したいことを思い出してください。この文脈でどのように機能するかを、まず $\tilde{H}$ の構築に焦点を当てて確認しましょう。全体のプロセスは以下に図示されています。色分けされた状態準備ブロック $\text{Prep}|\psi_i\rangle$ のレイヤーは、このプロセスが部分空間内の $|\psi_i\rangle$ と $|\psi_j\rangle$ のすべての組み合わせに対して実行されることを思い出させてくれます。
...
### 4.2 ステップ2：ターゲットハードウェア向けに回路と演算子を最適化する {#4-2-step-2-optimize-circuits-and-operators-for-target-hardware}
...
#### アダマールテストによる $\tilde{S}$ と $\tilde{H}$ の行列要素計算のためのテンプレート回路 {#template-circuits-for-calculating-matrix-elements-of-tildes-and-tildeh-via-hadamard-test}
...
### 4.3 ステップ3：IBM Quantum プリミティブを使用して実行する {#43-step-3-execute-using-an-ibm-quantum-primitive}
