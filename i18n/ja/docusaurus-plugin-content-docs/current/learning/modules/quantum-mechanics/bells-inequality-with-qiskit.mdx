...
上記パッケージのセットアップとインストール方法については、[Qiskitのインストール](/guides/install-qiskit)ガイドを参照してください。
...
アインシュタインとボルンの選択肢を実験的に区別できると良いでしょう。どちらの選択肢が正しくても同じ結果が得られる実験にはどのようなものがあるでしょうか。2つの選択肢で異なる結果が得られる実験を考えられますか?
...
### 理解の確認 {#check-your-understanding-1}
...
この結果の数学的な検証を行うにはどうすればよいでしょうか。念のため言っておくと、まだこれが自然界と一致するかどうかを検証してほしいというわけではなく、単にすべての数学的計算に誤りがないことを確認してほしいということです。
...
次に、StatevectorSampler と呼ばれる Qiskit リファレンスプリミティブを使用します。サンプラーとは、システムのすべての可能な状態をサンプリングして、各状態を得る確率（場合によっては準確率）を返すように設計されたプリミティブです。「ショット数」を指定して、各状態の「カウント」を確認することができます。
...
### 理解度チェック {#check-your-understanding-6}
