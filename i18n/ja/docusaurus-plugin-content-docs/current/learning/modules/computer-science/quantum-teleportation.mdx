...
Qiskitでは、$X$ ゲートを含む回路を作成すると次のようになります。
...
**$Z$ ゲート**: 状態に「位相」を加えます（前置係数で、Zの固有状態 $|0\rangle$ と $|1\rangle$ の場合はそれぞれ1または-1になります）。単一キュビット・ゲートです。
...
Qiskitでは、$Z$ ゲートを含む回路を作成すると次のようになります。
...
すべての3つのキュビットの状態に対する上記の式を考えると、キュビットAとQの測定が $|0\rangle_A|0\rangle_Q?$ となる確率はどれくらいですか。

<Accordion>
<AccordionItem title="解答">

25%です。これを確認するには、ボブの状態が正規化されていなければならないことを思い出してください。すると
...
IBM Quantum は、「Qiskit パターン」と呼ばれるフレームワークを使用して量子コンピューティングの問題に取り組むことを推奨しています。このフレームワークは以下のステップで構成されています。
- ステップ 1：問題を量子 Circuit にマッピングする
- ステップ 2：実際の量子ハードウェアで実行できるように Circuit を最適化する
- ステップ 3：IBM Quantum プリミティブを使用して IBM 量子コンピューターでジョブを実行する
- ステップ 4：結果を後処理する
...
「Sampler」は、量子 Circuit から生じる可能性のある状態をサンプリングし、どの状態がどの確率で測定されるかの統計を収集するように設計されたプリミティブです。ここで IBM Quantum Sampler をインポートします：
...
上記の状態のうち、テレポーテーションが成功したことを示すものはどれで、それをどう見分けられますか?

<Accordion>
<AccordionItem title="解答">

状態 $|000\rangle$、$|001\rangle$、$|010\rangle$、$|011\rangle$ はすべてテレポーテーションの成功と一致します。これは、秘密の状態の初期準備を元に戻すためのゲートを追加したためです。秘密の状態がボブのキュビットに正常にテレポートされた場合、その追加のゲートによってボブのキュビットは $|0\rangle$ 状態に戻るはずです。したがって、ボブのキュビット（キュビット0、これは古典レジスターの0番目の成分に測定されるため、最上位/最も右側になります）が $|0\rangle$ 状態にある上記のいずれの状態も、成功を示しています。
...
### ステップ1：問題を量子回路にマッピングする {#step-1-map-your-problem-to-a-quantum-circuit-1}
