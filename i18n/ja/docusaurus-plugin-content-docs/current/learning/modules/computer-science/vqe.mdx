...
これらのパッケージのセットアップとインストール方法については、[Qiskitのインストール](/guides/install-qiskit)ガイドを参照してください。実際の量子コンピューターでジョブを実行するには、[IBMクラウドアカウントの設定](/guides/cloud-setup)ガイドの手順に従ってIBM Cloudアカウントを作成する必要があります。
...
(a) 次の条件を満たす$A$を求めることで、試行波動関数を規格化してください：$$ \int_{-\infty}^{\infty} |\Psi_\text{trial}|^2 dx = 1 $$.
...
(b) 次のハミルトニアン$\hat{H}$の期待値を計算してください：$$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V(x)$$ただし$V(x)  = \frac{1}{2}m\omega^2x^2$は単純調和振動子ポテンシャルに対応します。
...
化学問題に量子コンピューターが必要な理由と、量子・古典コンピューティングリソースを両方使用する理由については、以下の記事を参照してください：
...
空欄に正しい用語を入れて、VQEプロセスの概要を完成させてください。答えを確認するにはクリックしてください。
...
以下に初回使用時に認証情報を保存するためのコードがあります。保存後はノートブックからこの情報を必ず削除してください。ノートブックを共有する際に認証情報が誤って共有されないようにするためです。詳細については、[IBMクラウドアカウントの設定](/guides/initialize-account)と[信頼できない環境でのサービスの初期化](/guides/cloud-setup-untrusted)を参照してください。
...
まず、特定の結合距離における水素分子（$H_2$）のハミルトニアンを定義することでVQEの計算を開始します。このハミルトニアンは、標準的な手順に従って分子系から生成・マッピングされた結果として、量子ビット演算子の形でシステムの全エネルギーを表します。具体的には、1) STO-6G基底関数系（電子軌道を近似するための数学的関数の集合）を用い、2) ジョルダン–ウィグナー変換（電子を記述するフェルミオン演算子を量子ビット演算子に変換する手法）を適用し、3) ハミルトニアンのパリティを利用した量子ビット削減によって問題を簡略化する、という手順を踏んでいます。
...
注：[`resilience_level`](/guides/error-mitigation-and-suppression-techniques) を使用してエラー緩和のレベルを設定することで、より良い結果を得ることができます。デフォルト値は1であり、より高い値を設定すると QPU の使用時間は増加しますが、より良い結果が返ってくる可能性があります。
...
### 理解度確認 {#check-your-understanding-3}
...
### ステップ1：問題を量子Circuit・演算子にマッピングする {#step-1-map-the-problem-to-quantum-circuits-and-operators-1}

ここでも、STO-6G基底とJordan-Wigner変換を使用し、ハミルトニアンのパリティを利用したQubit削減を適用した1-QubitのハミルトニアンをおいてAnを提供します。2つの水素原子間の原子間距離として`0.735` $\mathring A$を使用しています。

単一の水素原子（$H$）の計算とは異なり、水素分子（$H_2$）の基底状態を計算するには、電子軌道に関連するエネルギーに加えて、2つの水素原子の原子核間に働く斥力も考慮する必要があります。このステップでは、この値を定数として与え、実際の値はチェックイン問題で計算します。
...
### ステップ2：ターゲットハードウェア向けに最適化する {#step-2-optimize-for-target-hardware-1}
...
### ステップ3：ターゲットハードウェアで実行する {#step-3-execute-on-target-hardware-1}
...
### ステップ4：後処理 {#step-4-post-process-1}
...
### 理解度確認 {#check-your-understanding-4}
定数値（0.71997 Hartree）として含めた$H_2$分子の核間反発エネルギーを計算してみましょう。

![H2分子](/learning/images/modules/computer-science/vqe/h2.avif)

[クーロンの法則](https://en.wikipedia.org/wiki/Coulomb%27s_law)と[原子単位系](https://en.wikipedia.org/wiki/Atomic_units)を用いて値を`Hartree`単位で求めてください。
...
## $H + H = H_2$の反応エネルギーを計算する {#compute-reaction-energy-of-h-h-h-2}
