...
##  Pozadí {#background}
Tento tutoriál ukazuje, jak zmírnit chyby pomocí doplňku Shaded lightcone (SLC). Tento doplněk je rozvojem [techniky pravděpodobnostního rušení chyb (PEC)](https://quantum.cloud.ibm.com/docs/guides/error-mitigation-and-suppression-techniques#probabilistic-error-cancellation-pec), při níž uživatel naučí šum jedinečných vrstev v obvodu a poté šum ruší pomocí jednoqubitových hradel a technik post-processingu. Ve srovnání s jinými metodami nabízí PEC robustnější hranice zkreslení zmírněného výsledku, ale obvykle trpí vyšší režijní zátěží z hlediska času QPU. Během PEC se pro kompenzaci útlumu střední hodnoty způsobeného šumem průměrný výsledek přeškáluje faktorem $\gamma = \exp(\sum_{l,\sigma} 2\lambda_{l,\sigma})$, kde $\lambda_{l,\sigma}$ je naučená míra šumu chybového Pauliho $\sigma$ ve vrstvě $l$ v obvodu. Toto přeškálování zvyšuje rozptyl faktorem $\gamma^2$, a tedy také násobí počet spuštění obvodů potřebných na QPU faktorem $\gamma^2$, což označujeme jako náklady na vzorkování nebo overhead vzorkování. Protože $\gamma$ roste exponenciálně, je PEC často omezeno na mělké nebo máloqibitové obvody. Více o PEC se dozvíš v [Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors.](https://arxiv.org/abs/2201.09866)

Pokud dokážeme identifikovat chyby, které není třeba zmírňovat, můžeme tyto náklady na vzorkování exponenciálně snížit. Prvním krokem tímto směrem je implementace lokálně uvědomělého zmírňování chyb, které využívá rychle vypočitatelný konvenční „světelný kužel" ke snížení overheadu PEC tím, že ohraničuje citlivost pozorovatelné veličiny na chyby v celém obvodu, čímž rozšiřuje proveditelnost PEC na větší měřítka pro některé problémy. Chyby mimo tento světelný kužel nemohou ovlivnit měřený výsledek, a proto mohou být vyloučeny z procedury rušení chyb. Toto vyloučení snižuje overhead vzorkování — v některých případech výrazně — bez zavedení dalšího zkreslení. Konkrétně při měření lokální pozorovatelné $O$ v Circuit s pevnou hloubkou se požadovaný overhead vzorkování nakonec ustálí při škálování počtu qubitů v obvodu (viz Obr. 2b v [Locality and Error Mitigation of Quantum Circuits.](https://arxiv.org/abs/2303.06496))

Stínované světelné kužely (SLC) jdou dále a využívají klasické simulace k těsnějšímu ohraničení citlivosti na chyby v celém obvodu. To vyměňuje část času QPU za čas CPU a snižuje overhead vzorkování potřebný k renormalizaci zkreslení. Místo tvrdého mezního bodu je každé potenciální chybě v obvodu přiřazen odstupňovaný „stín", který shora ohraničuje náchylnost pozorovatelné veličiny na danou chybu. Tato upřesněná charakterizace umožňuje efektivnější a cílenější aplikace PEC se sníženým rozptylem, přičemž dává uživateli možnost řízeně ladit zkreslení v odhadování pozorovatelné. Více podrobností najdeš v [Lightcone shading for classically accelerated quantum error mitigation](https://arxiv.org/abs/2409.04401).
...
a. CPU — Ohraničení dopadu každé chyby pro 1- a 2-Qubitové Pauliho chyby
  ...
  1. Dopředná propagace (ohraničení dopadu na pozorovatelnou). Propaguj každou chybu na konec obvodu a vypočítej její komutátor s pozorovatelnou.  
  ...
  2. Zpětná propagace (ohraničení dopadu na počáteční stav). Propaguj každou chybu na začátek obvodu a vypočítej její komutátor s počátečním stavem.
...
d. QPU — Vlož antišum a spusť. Proveď obvod zájmu s antišumem (inverzním šumem) specifikovaným pomocí anotací `Box`.
...
|-----------|-----------|-----------|
| Pauliho twirl vrstev dvouqubitových hradel | Opakování identických párů vrstev a učení šumu | Odvození věrnosti (chyba pro každý kanál šumu) |
...
Po spuštění na kvantovém hardwaru pomocí rámce Samplomatic a Executor převedeme naše měření bitových řetězců na požadovanou hodnotu pozorovatelné. V případě našeho zrcadleného Isingova obvodu ideálně dostaneme měřenou pozorovatelnou hodnotu 1, protože všechny qubity by se ideálně měly vrátit do svého počátečního bodu $\ket{0}$. Při výpočtu hodnoty pozorovatelné s naší funkcí `expectation_values` použijeme několik post-processingových technik, které snižují dopad šumu. Patří sem odstraňování měření ovlivněných nemarkovským šumem, zmírňování chyb čtení, jakož i zohledňování podrobností naší implementace PEC. Podrobnosti jsou diskutovány v Kroku 4 níže.
...
Pro jednoduchost ukázky vybereme 1D zrcadlový Isingův řetězec. 1D Isingův řetězec poskytuje příjemně hustou strukturu obvodu, která je vhodná pro demonstraci implementací PEC. Zrcadlový obvod zjednodušuje znalost očekávaného výsledku (měli bychom naměřit pozorovatelnou hodnotu 1).

Dále chceme spustit zrcadlený obvod, takže pro každé hradlo ve druhé polovině obvodu musí existovat inverzní hradlo v první polovině. Protože měřená pozorovatelná **$<X_6 Z_{13}>$** má měření v ne-Z-bázi a exekutor zohledňuje požadovanou bázi na konci Circuit, poskytujeme funkci `prepare_basis`, která vkládá příslušná hradla na začátek zrcadleného obvodu. Tento detail je specifický pro naši demonstraci zrcadleného obvodu. Funkce `get_measurement_bases` nám umožňuje snadno identifikovat, které Gate jsou potřeba a kam je přidat, a také sledovat jemnosti indexování qubitů vyplývající z konvencí v anotaci `box`, jak je popsáno v části „Příprava kanonických bázových měření".
...
![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_1.png)

## Krok 2. Optimalizace {#step-2-optimize}
Optimalizujeme podrobnosti spojené s obvodem, který chceme spustit, s observable, jež chceme měřit, a s parametry učení šumu. Jako výchozí bod zajistíme, aby byl backend instanciován s povolenou volbou frakčních Gate. Tato frakční hradla umožní větší citlivost při některých filtrováních po-selekci.
...
Nejprve transpilujeme náš obvod do ISA instrukcí, [jak je vyžadováno pro spuštění na našich QPU](https://www.ibm.com/quantum/blog/isa-circuits). Pro data shromážděná v tomto experimentu ručně vybereme qubity na základě vyhodnocení řetězce nejvyšší kvality.
...
![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_2.png)

### Zabalení Circuit do boxů {#box-the-circuit}
Pro snadnou implementaci využijeme transpilační pass `generate_boxing_pass_manager`, který umístí instrukce obvodu do anotovaných boxů. Tyto boxy jasně označují místa, kde má být v případě PEC do obvodu vložen antišum. Podrobnosti o nastavení najdeš v [dokumentaci Samplomatic.](https://qiskit.github.io/samplomatic/)

Poznámka: SLC workflow vyžaduje použití `inject_noise_strategy="individual_modification"` v dalším kroku procesu, protože to umožňuje jednoznačně identifikovat každý `BoxOp` v obvodu.

Funkce `find_unique_box_instructions` prochází poskytnutý boxovaný obvod a identifikuje ty, které mají jedinečné 2Q vrstvy nebo měření, za účelem učení šumu a injekce šumu.
...
Kvůli způsobu, jakým jsou qubit označovány při identifikaci jedinečných 2Q vrstev, je třeba věnovat zvláštní pozornost sledování pořadí qubitů. Níže zavádíme pojem `canonical_qubits` jako prostředek pro správnou aktualizaci pořadí qubit při předávání executoru, a to v důsledku toho, jak je pořadí qubitů zachyceno při boxování obvodu a hledání jedinečných instrukcí. Podrobnosti najdeš v dokumentaci [qubit ordering convention](https://qiskit.github.io/samplomatic/guides/samplex_io.html#qubit-ordering-convention).
...
### Pracovní postup pro stínování světelného kužele, učení šumu a injekci anti-šumu {#workflow-for-lightcone-shading-noise-learning-and-anti-noise-injection-1}

> **Poznámka**: Pro implementaci SLC-PEC v tomto tutoriálu spouštíme výpočty SLC hranic **před** dokončením učení šumu, aby byl obsluhovaný obvod spuštěn co nejblíže v čase naučenému modelu šumu. V principu lze tento pracovní postup dále vylepšit tak, aby se prováděl souběžně. Konkrétně lze spustit úlohu učení šumu paralelně s odhadem hranic šumu. Pro libovolný kvantový Circuit může výpočet hranic šumu škálovat se slabě exponenciální závislostí. Proto může být vhodné využít paralelizované spouštění při snaze maximalizovat efektivitu pracovního postupu. Za tímto účelem to krátce demonstrujeme zahrnutím clusterových prostředků (128 vláken) a ukazujeme, jak při stejném časovém limitu výpočtu dosáhnout přesnější sady hranic pro zadaný obvod ve srovnání s laptopem (8 vláken). Dále, ačkoli to v tomto pracovním postupu není implementováno, můžeš paralelizovat spouštění QPU pro učení šumu a výpočty hranic šumu, abys dosáhl/a nejefektivnějšího pracovního postupu.

#### Předpověď Pauliho termů modelu šumu k naučení {#predict-to-be-learned-noise-model-paulis}

Funkce `generate_noise_model_paulis` prochází každou ohraničenou vrstvou zadaného obvodu a generuje všechny relevantní Pauliho termy váhy jedna a váhy dva, přičemž zohledňuje konektivitu qubitů v obvoduu a vybírá termy relevantní pro aktivní uzly a hrany. Tyto termy se pak používají k výpočtu dopředných a zpětných hranic šumu.
...
#### Vizualizace SLC pro ruční kontrolu {#visualize-the-slc-for-manual-inspection}
...
![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_4.png)

![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_5.png)

![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_6.png)
...
![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_7.png)

![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_8.png)

![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_9.png)

#### c. Výpočet zpětných mezí {#c-compute-backward-bounds}

Tato část predikce šumu vyhodnocuje, jak může chyba v určité vrstvě ovlivnit vstupní stav $\rho$. Funkce `compute_backward_bounds` nejprve invertuje obvod, odstraní měřicí hradla a poté provede podobnou analýzu, jaká byla provedena při výpočtech dopředných mezí.
...
![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_10.png)

![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_11.png)

![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_12.png)

#### Náhled sloučených mezí bez naučených rychlostí šumu {#preview-merged-bounds-without-learned-noise-rates}

Funkce `merged_bounds` určuje bod v obvodu, kde přepnutí ze zpětných mezí na dopředné meze minimalizuje celkovou odhadovanou odchylku na požadované observabili. Tato odchylka se počítá jako součet příspěvků zpětných mezí pro všechna šumová místa před tímto bodem a příspěvků dopředných mezí pro všechna šumová místa za ním. V současné době se to provádí jednotně pro všechny qubity.
...
### Vizualizace SLC pro ruční inspekci {#visualize-the-slc-for-manual-inspection-3}
...
![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_13.png)

![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_14.png)

![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_15.png)
...
![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_16.png)

![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_17.png)

![Diagram kvantového obvodu](/img/qiskit-addons/slc/01-getting-started/output_18.png)
...
### b.i. Aktualizace sloučených hranic se skutečně naučenými mírami šumu {#bi-update-merged-bounds-with-actual-learned-noise-rates}
...
#### c.i. Příprava šablonového Circuit pomocí `samplex` {#ci-prepare-template-circuit-by-using-samplex}
...
#### c.ii. Nastavení `QuantumProgram` {#cii-set-up-the-quantumprogram}
...
#### c.iii. Spuštění programu pomocí primitivu `Executor` {#ciii-execute-program-with-the-executor-primitive}
