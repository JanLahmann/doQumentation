---
description: 'Používej lokální testovací režim k simulaci kvantového hardwaru s IBM Quantum Compute'
notebook_path: guides/local-testing-mode.ipynb
sidebar_label: 'Lokální testovací režim'
title: 'Lokální testovací režim'
---
...
Primitiv Executor nepodporuje lokální testovací režim.
</Admonition>

Chceš-li použít lokální testovací režim, zadej jeden z falešných backendů z ``qiskit_ibm_runtime.fake_provider`` nebo zadej Backend Qiskit Aer při vytváření instance primitivu IBM Quantum nebo Session.
...
- **Simulátor Aer**: Simulátory z [Qiskit Aer](/guides/simulate-with-qiskit-aer) poskytují výkonnější simulaci, která zvládne větší Circuit a [vlastní modely šumu](/guides/build-noise-models). Při použití `AerSimulator` v lokálním testovacím režimu jsou k dispozici různé možnosti metod simulace. Viz [příklad simulačního režimu Clifford](#clifford-sim), který ukazuje, jak efektivně simulovat Cliffordovy Circuit s velkým počtem Qubitů.
    ...
    <Accordion>
    <AccordionItem title="**Seznam simulačních metod dostupných v Qiskit Aer**">
...
- V lokálním testovacím režimu můžeš zadat všechny možnosti IBM Quantum Compute. Všechny možnosti kromě shots jsou však při spuštění na lokálním simulátoru ignorovány.
...
- Použij třídu [`Neat`](/guides/debug-qiskit-runtime-jobs) k analýze dopadu šumu na pracovní zátěž Estimatoru.
- Prostuduj si podrobné [příklady Sampleru](/guides/sampler-examples).
- Prostuduj si podrobné [příklady Estimatoru](/guides/estimator-examples).
