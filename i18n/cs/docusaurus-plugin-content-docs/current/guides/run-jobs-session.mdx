...
Před spuštěním Session musíš [nastavit klienta IBM Quantum Compute](/guides/install-qiskit-runtime) a inicializovat ho jako službu:
...
Pro komplexní přehled o konfiguraci a stavu Session použij  `session.details() method`.
...
Session jsou zvláště užitečné pro algoritmy, které vyžadují častou komunikaci mezi klasickými a kvantovými prostředky.
