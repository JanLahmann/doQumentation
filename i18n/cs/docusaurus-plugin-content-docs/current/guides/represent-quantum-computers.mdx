...
sidebar_label: 'Reprezentace kvantových počítačů pro Transpiler'
title: 'Reprezentace kvantových počítačů pro Transpiler'
---
...
- [`Target`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.Target) obsahuje všechna relevantní omezení zařízení, jako jsou jeho podporovaný instrukční set, konektivita Qubitů a informace o pulzech nebo časování.

- [`Backend`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2) má `Target` jako výchozí součást, obsahuje další informace -- například [`InstructionScheduleMap`](https://docs.quantum.ibm.com/api/qiskit/1.4/qiskit.pulse.InstructionScheduleMap), a poskytuje rozhraní pro odesílání úloh kvantových Circuit.
...
Tato stránka ukazuje několik příkladů předávání informací o QPU do Transpileru.
...
Toto téma ukazuje několik příkladů předávání informací Transpileru.

<Admonition type="note">
Tyto příklady používají target z mockového backendu `qiskit_ibm_runtime` [`FakeSherbrooke`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/fake-provider-fake-sherbrooke#fakesherbrooke). Můžeš je však vyzkoušet na libovolném reálném nebo falešném backendu kompatibilním s Qiskit. Výsledky se mohou lišit.
</Admonition>
...
Ukázkový Circuit používá instanci [`efficient_su2`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.efficient_su2) z knihovny Circuit Qiskit.
...
Tento příklad používá výchozí nastavení k transpilaci na `target` backendu `backend`, který poskytuje všechny informace potřebné k převodu obvodu na obvod, který poběží na backendu.
...
Tento příklad je využíván v dalších částech tohoto tématu k ilustraci toho, že coupling map a podporovaný instrukční set jsou zásadní informace, které je třeba předat Transpileru pro optimální sestavení Circuit. QPU obvykle dokáže pro ostatní nepředané informace, jako je časování a plánování, zvolit výchozí nastavení.
...
Každý kvantový počítač podporuje omezený instrukční set. Každá hradlo v Circuit musí být přeložena na prvky tohoto setu. Tento set by měl obsahovat jednoqubitové a dvouqubitové Gate, které tvoří univerzální set Gate -- to znamená, že jakákoli kvantová operace může být rozložena do těchto hradlo. Toto zajišťuje [BasisTranslator](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.passes.BasisTranslator) a `basis_gates` lze jako argument Transpileru zadat explicitně.
...
Výchozí jednoqubitové Gate na `FakeSherbrooke` jsou `rz`, `x` a `sx`, výchozí dvouqubitová Gate je `ecr` (echoed cross-resonance). CX Gate jsou sestaveny z `ecr` Gate, takže na některých QPU je `ecr` zadána jako dvouqubitová basis gate, zatímco na jiných je výchozí `cx`. Gate `ecr` je _entanglující_ částí Gate `cx`. Kromě řídících Gate existují také instrukce `delay` a `measurement`.

<Admonition type="note">
QPU mají výchozí basis gates, ale můžeš si zvolit libovolné Gate -- za předpokladu, že poskytneš příslušnou instrukci nebo přidáš pulzní Gate (viz [Tvorba vlastních průchodů Transpileru](/guides/custom-transpiler-pass).) Výchozí basis gates jsou ty, pro které byly na QPU provedeny kalibrace, takže není třeba poskytovat žádné další instrukce ani pulzní Gate. Například na některých QPU je výchozí dvouqubitová Gate `cx`, na jiných `ecr`. Viz seznam možných [nativních Gate a operací](/guides/qpu-information#native-gates) pro více podrobností.
</Admonition>
...
Všimni si, že objekty `CXGate` byly rozloženy na hradlo `ecr` a jednoqubitové basis gates.

## Chybovosti zařízení {#device-error-rates}
