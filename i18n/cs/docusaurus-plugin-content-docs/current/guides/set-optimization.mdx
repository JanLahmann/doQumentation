...
Mock Backend `FakeSherbrooke` z `qiskit_ibm_runtime` je použit v těchto příkladech, ale můžeš to vyzkoušet na libovolném skutečném nebo fiktivním Backendu kompatibilním s Qiskitem. Tvé výsledky se mohou lišit.
</Admonition>

Nejprve transpilujme s úrovní optimalizace 0.
...
Úprava úrovně optimalizace může změnit i jiné aspekty Circuit, nejen počet Gate ECR. Příklady toho, jak nastavení úrovně optimalizace mění layout, najdeš v části [Reprezentace kvantových počítačů](/guides/represent-quantum-computers).
