...
<span id="qr-support"></span>
## Podpora Qiskit Runtime {#qiskit-runtime-support}

Přestože je časová osa (timeline drawer) zabudovaná v Qiskitu užitečná pro statické Circuit, nemusí přesně odrážet časování [dynamických Circuit](/guides/classical-feedforward-and-control-flow), a to kvůli implicitním operacím, jako je broadcasting a určování větví. Jako součást podpory dynamických Circuit vrací Qiskit Runtime přesné informace o časování Circuit uvnitř výsledků úlohy – pokud jsou vyžádány.

<Admonition type="note" title="Poznámky">
- Jedná se o experimentální funkci. Je ve stavu preview release, a proto se může změnit.
- Tato funkce se vztahuje pouze na úlohy Qiskit Runtime Sampler.
- Ačkoli je celkový čas Circuit vrácen v metadatech „compilation", NENÍ to čas používaný pro fakturaci (kvantový čas).
</Admonition>

### Povolení načítání dat o časování {#enable-timing-data-retrieval}

Chceš-li povolit načítání dat o časování, nastav experimentální příznak `scheduler_timing` na `True` při spuštění primitivní úlohy.
...
### Přístup k datům o časování Circuit {#access-the-circuit-timing-data}

Pokud jsou data o časování Circuit vyžádána, jsou vrácena v metadatech výsledku úlohy pro každý PUB pod klíčem `["compilation"]["scheduler_timing"]["timing"]`. Toto pole obsahuje nezpracované informace o časování. Chceš-li informace o časování zobrazit, použij vestavěný vizualizační nástroj popsaný v části [Vizualizace časování](#visualize-timings).

Následující kód zpřístupní data o časování Circuit pro první PUB:
...
#### Pochopení nezpracovaných dat o časování {#understand-the-raw-timing-data}

Vizualizace dat o časování Circuit pomocí metody `draw_circuit_schedule_timing` je sice nejčastějším případem použití, ale může být užitečné porozumět struktuře vrácených nezpracovaných dat. To ti může pomoct například programově extrahovat informace.

Data o časování vrácená v `["compilation"]["scheduler_timing"]["timing"]` jsou seznam řetězců. Každý řetězec představuje jednu instrukci na určitém kanálu a je oddělen čárkami do následujících datových typů:

- `Branch` – Určuje, zda je instrukce v řízení toku (then / else), nebo v hlavní větvi.
- `Instruction` – Gate a Qubit, na který se operace provádí.
- `Channel` – Kanál, který je instrukci přiřazen. Může být jeden z následujících:
      - `Qubit x` – Řídicí kanál pro Qubit _x_.
      - `AWGRx_y` (arbitrary waveform generator readout) – Používán čtecími kanály ke komunikaci při měření Qubitů. Argumenty _x_ a _y_ odpovídají ID čtecího přístroje a číslu Qubitu.
- `T0` – Čas zahájení instrukce v rámci celého plánu.
- `Duration` – Délka trvání instrukce v jednotkách _dt_ sekund, kde 1 dt = 1 plánovací cyklus. Hodnotu `dt` daného Backend zjistíš pomocí [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt).
- `Pulse` – Typ používané pulsní operace.
...
S `qiskit-ibm-runtime` verze v0.43.0 nebo novější můžeš vizualizovat časování Circuit. Nejprve musíš převést metadata výsledku na `fig` pomocí [metody `draw_circuit_schedule_timing`.](https://github.com/Qiskit/qiskit-ibm-runtime/blob/3d1bf1e1d49e5123841639fce259859c90ce9314/qiskit_ibm_runtime/visualization/draw_circuit_schedule_timings.py#L26) Tato metoda vrací obrázek `plotly`, který můžeš zobrazit přímo, uložit do souboru, nebo obojí. Více informací o příkazech `plotly` najdeš v dokumentaci [`fig.show()`](https://plotly.com/python-api-reference/generated/plotly.io.show.html) a [`fig.write_image("<path.format>")`.](https://plotly.com/python-api-reference/generated/plotly.io.write_image.html)
...
![Po najetí myší na výstup se zobrazí informace, jako je začátek, konec a délka trvání.](/docs/images/guides/visualize-circuit-timing/image_1.svg 'Příklad vygenerovaného obrázku')

#### Pochopení vygenerovaného obrázku {#understand-the-generated-figure}

Obrázek s daty o časování Circuit výstupem `draw_circuit_schedule_timing` předává tyto informace:

- Osa X představuje čas v jednotkách _dt_ sekund, kde 1 dt = 1 plánovací cyklus. Hodnotu `dt` daného Backend zjistíš pomocí [`backend.dt`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2#dt).
- Osa Y představuje kanál (kanály si představ jako přístroje vydávající pulsy).
    - `Receive channel` – Jediný kanál, který sám o sobě není přístrojem. Jde o instrukci přehrávanou na všech kanálech, které jsou v daném okamžiku součástí komunikačního procesu s hubem.
    - `Qubit x` – Řídicí kanál pro Qubit x.
    - `AWGRx_y` (arbitrary waveform generator readout) – Používán čtecími kanály ke komunikaci při měření Qubitů. Argumenty _x_ a _y_ odpovídají ID čtecího přístroje a číslu Qubitu.
    - `Hub` – Řídí broadcasting.

Každá instrukce má navíc formát *X_Y*, kde *X* je název instrukce a *Y* je typ pulsu. Typ `play` aplikuje řídicí pulsy a typ `capture` zaznamenává stav Qubitu. Podrobnosti zobrazíš také najetím myší na každou instrukci. Například předchozí obrázek ukazuje řídicí puls pro Gate X aplikovaný na Qubit 10 v čase 1161 dt.

### Komplexní příklad {#end-to-end-example}

Tento příklad ti ukáže, jak možnost povolit, jak získat informace o časování Circuit z metadat a jak je zobrazit jako obrázek.

Nejprve nastav prostředí, definuj Circuit, převeď je na ISA Circuit a definuj a spusť úlohy.
...
Dále získej časování plánu Circuit:
...
<Admonition type="tip" title="Doporučení">
  - [Klasická zpětná vazba a řízení toku](/guides/classical-feedforward-and-control-flow) (dynamické Circuit)
