---
title: "Vytvoř šablonu funkce Qiskit pro simulaci hamiltoniánu"
sidebar_label: "Vytvoř šablonu funkce Qiskit pro simulaci hamiltoniánu"
...
Tato šablona zapouzdřuje pracovní postup pro simulaci časového vývoje počátečního stavu vůči uživatelsky definovanému spinovému hamiltoniánu a vrací sadu zadaných střední hodnot pomocí doplňku Qiskit [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/).

Šablona je strukturována jako vzor Qiskit s následujícími kroky:

#### 1. Sběr vstupů a mapování problému {#1-collecting-input-and-mapping-the-problem}

Tato sekce přijímá jako vstup hamiltoniána k simulaci, počáteční stav ve formě `QuantumCircuit`, sadu observabilů pro odhad středních hodnot a specifikaci možností doplňku AQC. Tento krok ověřuje, že jsou přítomna všechna požadovaná vstupní data a že jsou ve správném formátu.

Vstupní argumenty jsou pak použity k sestavení relevantních kvantových obvodů a operátorů pro pracovní postup. Vytvoří se cílový obvod a pomocí doplňku AQC se nalezne reprezentace tohoto obvodu pomocí tensor network v podobě matrix product state. Poté se vygeneruje ansatz obvod a optimalizuje se metodami tensor network, čímž vznikne finální obvod, který provede zbývající část časového vývoje.

#### 2. Příprava vygenerovaných obvodů ke spuštění {#2-prepare-the-generated-circuits-for-execution}

Vygenerované obvody z doplňku AQC jsou pak transpilovány pro spuštění na zvoleném backendu. Vytvoří se instance [`EstimatorV2`](../api/qiskit-ibm-runtime/estimator-v2) s výchozí sadou možností pro zmírnění chyb, která spravuje provádění obvodů.

#### 3. Spuštění {#3-execution}

Nakonec je ansatz obvod transpilován a spuštěn na QPU a shromáždí odhady pro všechny zadané střední hodnoty, které jsou vráceny v serializovatelném formátu přístupném uživateli.

## Napiš šablonu funkce {#write-the-function-template}

Nejprve napiš šablonu funkce pro simulaci hamiltoniánu, která využívá [doplněk AQC-Tensor pro Qiskit](https://qiskit.github.io/qiskit-addon-aqc-tensor/) k mapování popisu problému na obvod se sníženou hloubkou pro spuštění na hardwaru.

Pokud si tuto stránku stáhneš a zobrazíš ji lokálně v editoru notebooku, uvidíš, že některé buňky s kódem obsahují [magický příkaz](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Tento magický příkaz ukládá kód do `./source_files/template_hamiltonian_simulation.py`, což je šablona funkce, kterou můžeš nahrát a vzdáleně spouštět pomocí Qiskit Serverless.
...
```
### Sbírání a validace vstupů {#collect-and-validate-the-inputs}

Začni tím, že získáš vstupy pro šablonu. Tento příklad obsahuje vstupy specifické pro danou doménu, relevantní pro simulaci hamiltoniánu (jako je Hamiltonián a pozorovatelná veličina), a možnosti specifické pro danou funkčnost (například to, jak moc chceš komprimovat počáteční vrstvy Trotterova obvodu pomocí AQC-Tensor, nebo pokročilé možnosti pro doladění potlačení a zmírnění chyb nad rámec výchozích nastavení, která jsou součástí tohoto příkladu).
...
Když šablona funkce běží, je užitečné vracet informace do logů pomocí příkazů print, abys mohl/a lépe sledovat průběh úlohy. Níže je jednoduchý příklad výpisu `estimator_options`, díky němuž zůstane záznam o skutečně použitých možnostech Estimatoru. V celém programu najdeš mnoho dalších podobných příkladů hlášení průběhu spuštění, včetně hodnoty účelové funkce během iterativní části AQC-Tensor a hloubky dvouqubitových hradel v závěrečném obvodu pro instrukční sadu architektury (ISA) určeném ke spuštění na hardwaru.
...
#### Validace vstupů {#validate-the-inputs}
...
#### Příprava výstupů funkce {#prepare-the-function-outputs}
...
Appending to ./source_files/template_hamiltonian_simulation.py
```
### Namapuj problém a předzpracuj obvod pomocí AQC {#map-the-problem-and-pre-process-the-circuit-with-aqc}

Optimalizace AQC-Tensor probíhá v kroku 1 Qiskit vzoru. Nejprve se sestaví cílový stav. V tomto příkladu se sestaví z cílového obvodu, který vyvíjí stejný Hamiltonián po stejnou dobu jako část AQC. Poté se vygeneruje ansatz z ekvivalentního obvodu, ale s menším počtem Trotterových kroků. V hlavní části algoritmu AQC je tento ansatz iterativně přibližován k cílovému stavu. Nakonec se výsledek zkombinuje se zbývajícími Trotterovými kroky potřebnými k dosažení požadované doby vývoje.
...
Appending to ./source_files/template_hamiltonian_simulation.py
```
### Optimalizace finálního obvodu ke spuštění {#optimize-the-final-circuit-for-execution}
...
#### Předčasné ukončení při použití režimu dry run {#exit-early-if-using-dry-run-mode}

Pokud byl zvolen režim dry run, program se zastaví před spuštěním na hardware. To může být užitečné například tehdy, když chceš nejprve zkontrolovat hloubku dvouqubitových hradel v ISA obvodu, a teprve pak se rozhodnout, zda spustit výpočet na hardware.
...
#### Spuštění obvodu na hardwaru {#execute-the-circuit-on-hardware}
...
#### Uložení výstupu {#save-the-output}
...
<Accordion>
<AccordionItem title="**Úplný zdrojový kód programu**">
