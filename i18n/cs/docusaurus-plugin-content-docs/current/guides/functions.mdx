...
3. <span id="functions-untrusted"></span>**Vyhni se spouštění kódu na nedůvěryhodném počítači nebo v externím cloudovém prostředí Python, aby se minimalizovala bezpečnostní rizika.** Pokud musíš použít nedůvěryhodné prostředí (například na veřejném počítači), po každém použití změň svůj API klíč jeho smazáním na stránce [IBM Cloud API keys](https://cloud.ibm.com/iam/apikeys), aby se snížilo riziko. Více informací najdeš v tématu [Správa uživatelských API klíčů](https://cloud.ibm.com/docs/account?topic=account-userapikey&interface=ui). Pro inicializaci služby v této situaci rozbal následující část a zobraz kód, který můžeš použít:
    ...
    <Accordion>
        <AccordionItem title="Inicializuj službu v nedůvěryhodném prostředí">
...
- [Prozkoumej Circuit functions](/guides/ibm-circuit-function) pro tvorbu nových algoritmů a aplikací, aniž bys musel/a řešit transpilaci nebo ošetřování chyb.
- [Prozkoumej application functions](/guides/q-ctrl-optimization-solver) pro řešení úloh specifických pro danou doménu s klasickými vstupy a výstupy.
- Podívej se na [referenční dokumentaci API](https://docs.quantum.ibm.com/api/functions/index) pro Qiskit Functions.
- Pro praktické zkušenosti vyzkoušej některé [tutoriály](/tutorials/index#leverage-qiskit-capabilities), které demonstrují Qiskit Functions.
</Admonition>
