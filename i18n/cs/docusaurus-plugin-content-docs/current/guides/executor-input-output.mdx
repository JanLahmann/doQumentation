...
Jak bylo uvedeno dříve, vstupem do primitivu Executor je [`QuantumProgram`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/quantum-program-quantum-program), což je iterovatelná kolekce objektů
[`QuantumProgramItem`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/quantum-program-quantum-program-item). Tyto objekty mohou být dvou typů:
...
Druhá položka obsahuje výsledky spuštění druhé úlohy (`SamplexItem`) v programu. Tato položka obsahuje více klíčů. Klíč `meas`, což je název klasického registru vstupního obvodu, je mapován na pole výsledků tohoto registru. Toto pole má tvar `(randomizations, parameter sets, shots, classical bits)`, tedy (28, 10, 1024, 3) v tomto příkladu. Kromě toho výstup obsahuje klíč `measurement_flips.meas`, což jsou korekce bitových převrácení, které vrací zpět twirling měření pro registr `meas`. Tento výstupní tvar bude pro náš příklad (28, 10, 1, 3), protože k provedení bitového převrácení je zapotřebí pouze jeden shot.
