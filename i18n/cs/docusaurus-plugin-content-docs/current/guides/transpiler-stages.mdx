...
- `DenseLayout`: Najde podgraf zařízení s největší konektivitou, který má stejný počet Qubit jako Circuit (používá se pro úroveň optimalizace 1, pokud jsou v Circuit přítomny operace řízení toku (jako IfElseOp)).
...
Aby bylo možné provést dvouQubitovou Gate mezi Qubity, které nejsou přímo propojeny na kvantovém zařízení, je nutné do obvod vložit jednu nebo více Gate SWAP, které přesunou stavy Qubitů tak, aby byly sousední na mapě Gate zařízení. Každá hradlo SWAP představuje nákladnou a hlučnou operaci k provedení. Nalezení minimálního počtu Gate SWAP potřebných k mapování Circuit na dané zařízení je tedy důležitým krokem v procesu transpilace. Z důvodu efektivity se tato fáze ve výchozím nastavení typicky počítá společně s fází Layout, ale jsou od sebe logicky odděleny. Fáze *Layout* vybírá fyzické Qubity, které se mají použít, zatímco fáze *Routing* vkládá odpovídající počet Gate SWAP, aby bylo možné Circuit provést s použitím vybraného layoutu.
...
![Výstup předchozí buňky kódu](/docs/images/guides/transpiler-stages/extracted-outputs/62479697-cef1-4a89-ac2a-20051dd294f4-1.svg)

Toto široké rozdělení ukazuje, jak obtížné je pro mapper SWAP vypočítat nejlepší mapování. Abychom získali určitý náhled, podívejme se jak na obvod, který se provádí, tak na Qubity vybrané na hardwaru.

```python
ghz.draw("mpl", idle_wires=False)
```

![Výstup předchozí buňky kódu](/docs/images/guides/transpiler-stages/extracted-outputs/ab89c4ea-06c4-4320-b493-feb691b3570d-0.svg)
...
![Výstup předchozí buňky kódu](/docs/images/guides/transpiler-stages/extracted-outputs/f6a3a92a-8656-4518-ba2c-c3b0b038f507-0.svg)

Jak vidíš, tento Circuit musí provést dvouQubitovou Gate mezi Qubity 0 a 14, které jsou na grafu konektivity velmi daleko od sebe. Spuštění tohoto obvod tedy vyžaduje vložení Gate SWAP k provedení všech dvouQubitových Gate pomocí průchodu `SabreSwap`.
...
Při psaní kvantového Circuit můžeš volně používat jakýkoli kvantový Gate (unitární operaci), který se ti líbí, spolu se sadou operací, které nejsou Gate, jako jsou instrukce měření nebo resetu Qubit. Většina kvantových zařízení však nativně podporuje pouze hrstku kvantových Gate a operací, které nejsou Gate. Tyto nativní Gate jsou součástí definice [ISA](/guides/transpile#instruction-set-architecture) cíle a tato fáze předdefinovaných `PassManagers` překládá (nebo *rozbaluje*) Gate specifikované v Circuit do nativních základních Gate zadaného Backend. Toto je důležitý krok, protože umožňuje, aby Circuit mohl být proveden Backend, ale obvykle vede ke zvýšení hloubky a počtu Gate.
