...
Chceš-li používat transpilerové průchody s podporou AI, nejprve nainstaluj balíček `qiskit-ibm-transpiler`. Více informací o dostupných možnostech najdeš v [dokumentaci API qiskit-ibm-transpiler](https://docs.quantum.ibm.com/api/qiskit-ibm-transpiler).
...
Nejprve nainstaluj `qiskit-ibm-transpiler` s některými dalšími závislostmi takto:

```sh
pip install qiskit-ibm-transpiler[ai-local-mode]
```

Po instalaci dalších závislostí je výchozím režimem spuštění transpilerových průchodů s podporou AI tvůj lokální počítač.
...
Citace
Pokud ve svém výzkumu používáš jakoukoli funkci s podporou AI ze Qiskit Transpiler Service, použij následující doporučenou citaci:
