...
Získej informace `target` z `backend` a ulož názvy operací jako `basis_gates`, protože `target` bude potřeba upravit tak, aby obsahoval informace o načasování Gate používaných při dynamickém oddělení.

<Admonition type="note">
V těchto příkladech je použit mock backend `FakeSherbrooke` z `qiskit_ibm_runtime`, ale můžeš to vyzkoušet na libovolném skutečném nebo falešném backendu kompatibilním s Qiskit. Výsledky se mohou lišit.
</Admonition>
