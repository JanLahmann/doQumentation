---
description: 'Používej Qiskit MCP servery k integraci kvantových výpočetních schopností do AI asistentů a agentů.'
title: 'Qiskit MCP servery'
---

{/* doqumentation-source-hash: 5de47ae0 */}
{/* cspell:ignore modelcontextprotocol  */}

# Qiskit MCP servery {#qiskit-mcp-servers}

Qiskit MCP servery jsou kolekce serverů [Model Context Protocol (MCP)](https://modelcontextprotocol.io/), které integrují kvantové výpočetní schopnosti do AI systémů. Tyto servery umožňují AI asistentům, velkým jazykovým modelům (LLM) a agentům přistupovat ke službám IBM Quantum&reg; a knihovnám Qiskit.

<Admonition type="note">
Jedná se o open-source projekt. Nejaktuálnější informace, podrobné pokrytí API a pokročilé konfigurace najdeš v [repozitáři Qiskit MCP serverů na GitHubu](https://github.com/Qiskit/mcp-servers).
</Admonition>

## Kdy mám použít Qiskit MCP servery? {#when-should-i-use-qiskit-mcp-servers}

Qiskit MCP servery umožňují AI asistentům pomáhat ti s kvantovými výpočetními úkoly, například:

- Vytváření, manipulace a transpilace kvantových Circuit

- Vyhledávání a prohledávání dokumentace Qiskit, průvodců a referencí API

- Odpovídání na otázky o kvantových výpočtech přímo ze spolehlivých zdrojů

- Generování kvantového kódu s kontextovým povědomím

- Optimalizace Circuit pomocí AI-powered transpilace

- Automatické připojení k reálnému kvantovému hardwaru

- Analýza a výběr optimálních kvantových Backend

- Spouštění kvantových Circuit a sledování stavu úloh


## Co je součástí? {#what-is-included}

K dispozici jsou následující MCP servery:

### Qiskit MCP Server {#qiskit-mcp-server}

Poskytuje základní kvantové výpočetní schopnosti pro lokální vývoj. Tento server umožňuje AI asistentům vytvářet, manipulovat a transpilovat kvantové Circuit a také je serializovat do formátů jako OpenQASM 3 a QPY.

### Qiskit Runtime MCP Server {#qiskit-runtime-mcp-server}

Poskytuje přístup ke cloudovým službám IBM Quantum prostřednictvím IBM Quantum Compute Service. Díky tomu mohou AI asistenti pracovat s kvantovými Backend, odesílat úlohy a načítat výsledky.

### Qiskit Docs MCP Server {#qiskit-docs-mcp-server}

Poskytuje AI asistentům přístup k celému ekosystému dokumentace Qiskit, včetně dokumentace modulů SDK, implementačních průvodců a osvědčených postupů. Tento server umožňuje inteligentní vyhledávání v dokumentaci Qiskit bez nutnosti autentizace.

### Qiskit IBM Transpiler MCP Server {#qiskit-ibm-transpiler-mcp-server}

Poskytuje AI-powered optimalizaci Circuit prostřednictvím [AI-powered transpiler passes](/guides/ai-transpiler-passes). Tento server umožňuje AI asistentům optimalizovat kvantové Circuit pomocí pokročilých algoritmů routování a optimalizace.

### Komunitní servery {#community-servers}

K dispozici jsou také následující MCP servery přispěné komunitou:

#### Qiskit Gym MCP Server {#qiskit-gym-mcp-server}

Poskytuje schopnosti reinforcement learningu pro syntézu kvantových Circuit pomocí knihovny [qiskit-gym](https://github.com/AI4quantum/qiskit-gym). Tento server umožňuje AI asistentům používat techniky optimalizace založené na RL pro transpilaci a syntézu kvantových Circuit.

## Instalace Qiskit MCP serverů {#install-qiskit-mcp-servers}

Aby ses mohl/a používat Qiskit MCP servery, potřebuješ:

- Python 3.10 nebo novější (doporučujeme 3.11+)

- Účet IBM Quantum a API token (viz [Nastavení účtu IBM Cloud&reg;](/guides/cloud-setup) pro instrukce)


Nainstaluj všechny dostupné Qiskit MCP servery spuštěním následujícího příkazu v terminálu:

```bash
pip install qiskit-mcp-servers[all]
```

Můžeš také nainstalovat jednotlivé MCP servery:

```bash
pip install qiskit-mcp-server
pip install qiskit-code-assistant-mcp-server
pip install qiskit-ibm-runtime-mcp-server
pip install qiskit-docs-mcp-server
pip install qiskit-ibm-transpiler-mcp-server
pip install qiskit-gym-mcp-server
```

## Konfigurace Qiskit MCP serverů {#configure-qiskit-mcp-servers}

### Nastavení proměnných prostředí {#set-environment-variables}

Nastav požadované proměnné prostředí pro autentizaci. Qiskit MCP Server a Qiskit Docs MCP Server nevyžadují autentizaci.

Pro IBM Quantum Compute Service nebo Qiskit IBM Transpiler MCP servery:

```bash
export QISKIT_IBM_TOKEN="<your_token>"
```

Svůj API token najdeš v [nastavení účtu IBM Quantum](https://quantum.cloud.ibm.com/).

### Konfigurace MCP klienta {#configure-the-mcp-client}

MCP-kompatibilního klienta (například Claude Desktop, Cursor nebo jiné AI nástroje) můžeš nakonfigurovat tak, aby používal Qiskit MCP servery. Přesná konfigurace závisí na tvém klientovi. Například konfigurační soubor MCP klienta může vypadat takto:

```json
{
  "mcpServers": {
    "qiskit": {
      "command": "qiskit-mcp-server"
    },
    "qiskit-ibm-runtime": {
      "command": "qiskit-ibm-runtime-mcp-server"
    },
    "qiskit-code-assistant": {
      "command": "qiskit-code-assistant-mcp-server"
    },
    "qiskit-docs": {
      "command": "qiskit-docs-mcp-server"
    },
    "qiskit-ibm-transpiler": {
      "command": "qiskit-ibm-transpiler-mcp-server"
    },
    "qiskit-gym": {
      "command": "qiskit-gym-mcp-server"
    }
  }
}
```

Konkrétní instrukce ke konfiguraci najdeš v dokumentaci svého MCP klienta.

## Testování MCP serverů {#test-the-mcp-servers}

MCP servery můžeš interaktivně testovat pomocí MCP Inspectoru:

```bash
# Test the Qiskit MCP Server
npx @modelcontextprotocol/inspector qiskit-mcp-server

# Test the Qiskit Runtime MCP Server
npx @modelcontextprotocol/inspector qiskit-ibm-runtime-mcp-server

# Test the Qiskit Docs MCP Server
npx @modelcontextprotocol/inspector qiskit-docs-mcp-server

# Test the Qiskit IBM Transpiler MCP Server
npx @modelcontextprotocol/inspector qiskit-ibm-transpiler-mcp-server

# Test the Qiskit Gym MCP Server
npx @modelcontextprotocol/inspector qiskit-gym-mcp-server
```

## Další kroky {#next-steps}

<Admonition type="info" title="Doporučení">

- Navštiv [repozitář Qiskit MCP serverů na GitHubu](https://github.com/Qiskit/mcp-servers) pro podrobnou dokumentaci, pokrytí API a příklady.

- Přečti si o [AI-powered transpiler passes](/guides/ai-transpiler-passes) a zjisti více o schopnostech AI-powered optimalizace Circuit.

- Nastav si [účet IBM Quantum](/guides/cloud-setup), pokud jsi to ještě neudělal/a.

- Prozkoumej [dokumentaci Model Context Protocol](https://modelcontextprotocol.io/) a zjisti více o MCP.


</Admonition>
