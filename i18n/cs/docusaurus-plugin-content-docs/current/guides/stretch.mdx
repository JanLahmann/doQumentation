...
Běžným případem použití `stretch` je aplikování dynamického decouplingu na nečinný qubit, zatímco jiný qubit prochází podmíněnými operacemi.
...
![Obrázek ilustrující následující Circuit](/docs/images/guides/stretch/stretch.svg)
...
:::note
 ...
 Při použití `stretch` s IBM Quantum Compute Service se jakýkoli zbytek vzniklý rozlišením stretch přičte k prvnímu delay, které stretch používá.
...
## Zobrazení hodnot stretch v IBM Quantum Compute Service {#view-stretch-values-in-ibm-quantum-compute-service}

Skutečná hodnota doby trvání stretch se rozlišuje v době kompilace, poté co je Circuit naplánován. Při spuštění úlohy Sampler v IBM Quantum Compute Service můžeš zobrazit rozlišené hodnoty stretch v metadatech výsledku úlohy. Podpora `stretch` v Quantum Compute je momentálně experimentální, takže nejprve musíš nastavit experimentální možnost pro povolení jejího načítání, pak přistupuj k datům přímo z metadat takto:
...
Ačkoli je celková doba Circuit vrácena v metadatech „compilation", NENÍ to čas používaný pro fakturaci (čas QPU).
</Admonition>
...
Úplné podrobnosti najdeš v tématu [Vizualizace časování Circuit](/guides/qiskit-runtime-circuit-timing).

<span id="qiskit-runtime-limitations"></span>

## Omezení Quantum Compute {#quantum-compute-limitations}

Podpora `stretch` v Quantum Compute je momentálně experimentální a má tato omezení:
...
- Oblast ohraničená sadou bariér se nazývá _oblast bariéry_. Proměnnou stretch nelze použít ve více oblastech bariér.
    <Tabs>
        ...
        ![Ilustrace výstupu předchozího kódu](/docs/images/guides/stretch/bad-barrier-region.svg 'Invalid use of stretch in barrier regions')
     ...
     <TabItem value="Valid" label="Platné">
        ...
        ![Ilustrace výstupu předchozího kódu](/docs/images/guides/stretch/good-barrier-region.svg 'Valid use of stretch in a barrier region')
