...
Tato stránka ukazuje, jak používat třídu [`Operator`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Operator). Pro přehled reprezentací operátorů v Qiskitu na vysoké úrovni, včetně třídy `Operator` a dalších, viz [Přehled tříd operátorů](/guides/operators-overview).
...
Unitární `Operators` lze přímo vkládat do `QuantumCircuit` pomocí metody `QuantumCircuit.append`. Tím se `Operator` převede na objekt `UnitaryGate`, který je přidán do obvod.
...
Všimni si, že v předchozím příkladu je operátor inicializován z objektu `Pauli`. Nicméně objekt `Pauli` lze také přímo vložit do obvod samotného a bude převeden na posloupnost jednoqubitových Pauli gates:
