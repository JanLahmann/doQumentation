...
Tato stránka poskytuje přehled vstupů a výstupů primitivu Sampler Qiskit Runtime, který spouští úlohy na výpočetních zdrojích IBM Quantum&reg;. Sampler ti umožňuje efektivně definovat vektorizované úlohy pomocí datové struktury zvané [**Primitive Unified Bloc (PUB)**](/guides/primitive-input-output#pubs). Jsou používány jako vstupy do metody [`run()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2#run) primitivu Sampler, který definovanou úlohu spustí jako úlohu. Po dokončení úlohy jsou výsledky vráceny ve formátu, který závisí jak na použitých PUBech, tak na možnostech runtime specifikovaných primitivem.
## Vstupy {#inputs}
...
Následující kód demonstruje příkladovou sadu vektorizovaných vstupů do primitivu `Sampler` a spustí je na backendu IBM&reg; jako jediný objekt `RuntimeJobV2`.
...
Výsledky úloh [`SamplerV2`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2) spouštěných v Qiskit Runtime obsahují informace o časování spuštění ve svých metadatech.
