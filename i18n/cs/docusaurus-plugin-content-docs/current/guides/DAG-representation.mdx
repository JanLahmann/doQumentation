---
description: 'Jak používat orientované acyklické grafy (DAG) k analýze a transformaci kvantových obvodů v průchodech Transpileru Qiskit'
...
V DAG existují tři druhy uzlů grafu: vstupní uzly qubitů/clbitů (zelené), operační uzly (modré) a výstupní uzly (červené). Každá hrana označuje tok dat (nebo závislost) mezi dvěma uzly. Použij funkci qiskit.tools.visualization.dag_drawer() k zobrazení DAG tohoto obvodu. (Pro spuštění nainstaluj [knihovnu Graphviz](https://graphviz.org/download/).)
...
<Admonition type="tip" title="Doporučení">
    - Prostuduj si průvodce vytvářením [vlastního průchodu Transpileru](/guides/custom-transpiler-pass)
    - Nauč se, jak [vytvářet a transpilovat vlastní Backend](/guides/custom-backend)
