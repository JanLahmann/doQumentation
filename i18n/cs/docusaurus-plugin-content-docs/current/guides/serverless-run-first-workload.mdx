...
Ujisti se, že ses před začátkem řídil/a postupem v průvodci [Napiš svůj první program Qiskit Serverless](/guides/serverless-first-program).

## Seznam dostupných programů {#list-programs-available}

Pomocí `QiskitServerless.list()` můžeš načíst seznam dostupných programů ke spuštění v Qiskit Serverless. Zahrnuje to dříve nahraný [`transpile_remote_serverless`](/guides/serverless-first-program).
...
- Prozkoumej [nástroje pro správu výpočetních prostředků a dat](/guides/serverless-manage-resources) dostupné pro tvůj program, včetně paralelizace.
