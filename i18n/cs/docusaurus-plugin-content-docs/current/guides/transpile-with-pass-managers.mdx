...
V těchto příkladech se používá falešný backend `FakeSherbrooke` z `qiskit_ibm_runtime`, ale můžeš to vyzkoušet na jakémkoli skutečném nebo falešném backendu kompatibilním s Qiskitem. Tvé výsledky se mohou lišit.
</Admonition>
...
Pokud přednastavené pass managery nesplňují tvé potřeby, můžeš transpilaci přizpůsobit vytvořením vlastních (staged) pass managerů nebo dokonce vlastních transpilačních passů. Zbývající část této stránky popisuje, jak vytvářet pass managery. Pokyny k vytváření transpilačních passů najdeš v části [Napiš si vlastní transpiler pass](custom-transpiler-pass).
...
- [Napiš si vlastní transpiler pass](custom-transpiler-pass).
