---
description: 'Vysvětlení technik zmírnění a potlačení chyb dostupných prostřednictvím IBM Quantum Compute Service'
...
Následující buňka kódu ukazuje, jak zapnout dynamické oddělení pro Estimator a vybrat sekvenci dynamického oddělení.
...
Pauliho twirling se implementuje tak, že se zvolená sada Gate obklopí náhodně vybranými jednoqubitovými Pauliho Gate takovým způsobem, aby ideální efekt Gate zůstal stejný. Výsledkem je, že jeden obvod je nahrazen náhodným souborem Circuit, všechny se stejným ideálním efektem. Při vzorkování Circuit jsou vzorky čerpány z více náhodných instancí, nikoli jen z jedné.

![Zobrazení Pauliho twirling](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)

Protože většina chyb v současném kvantovém hardwaru pochází z dvouqubitových Gate, tato technika se často aplikuje výhradně na (nativní) dvouqubitové Gate. Následující diagram zobrazuje některé Pauliho twirly pro Gate CNOT a ECR. Každý obvod v řadě má stejný ideální efekt.
...
![Zobrazení measurement twirling](/docs/images/guides/error-mitigation-explanation/measurement_twirling.svg)

V přítomnosti chyby čtení má measurement twirling efekt diagonalizace matice přenosu chyby čtení, což usnadňuje její inverzi. Korekční faktor škálování z diagonálního šumového kanálu je zjišťován pomocí srovnávání náhodných Circuit inicializovaných v nulovém stavu. To umožňuje službě odstranit zaujatost ze středních hodnot způsobenou šumem čtení. Odhad matice přenosu chyby čtení vyžaduje provádění dodatečných kalibračních Circuit, což přináší malou režii.

### Zapnutí TREX pomocí možnosti `measure_mitigation` {#enable-trex-by-using-the-measure-mitigation-option}

TREX lze zapnout nastavením `measure_mitigation` na `True` v [možnostech odolnosti IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) pro Estimator. Možnosti pro učení šumu měření jsou popsány [zde](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-measure-noise-learning-options). Stejně jako u gate twirling můžeš nastavit počet randomizací Circuit a počet vzorků na randomizaci.
...
### Zapnutí TREX pomocí možnosti `resilience_level` {#enable-trex-by-using-the-resilience-level-option}

TREX lze zapnout nastavením [úrovně odolnosti](/guides/estimator-noise-management#resilience) na `1`, jak je ukázáno v následujícím příkladu.
...
Obě fáze — zesilování šumu i extrapolace — lze implementovat mnoha různými způsoby. IBM Quantum Compute implementuje zesilování šumu pomocí „digitálního skládání Gate", což znamená, že dvouqubitové Gate jsou nahrazeny ekvivalentními sekvencemi Gate a jejich inverzí. Například nahrazení unitárního operátoru $U$ výrazem $U U^\dagger U$ by přineslo faktor zesílení šumu 3. Pro extrapolaci si můžeš vybrat z několika funkčních forem, včetně lineárního nebo exponenciálního fitu.
...
Režie této metody se škáluje s počtem faktorů šumu. Výchozí nastavení vzorkuje střední hodnotu při třech faktorech šumu, což vede k přibližně 3násobné režii.

### Zapnutí ZNE pomocí možnosti `zne_mitigation` {#enable-zne-by-using-the-zne-mitigation-option}

ZNE lze zapnout nastavením `zne_mitigation` na `True` v [možnostech odolnosti IBM Quantum Compute](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) pro Estimator.
Možnosti pro ZNE jsou popsány [zde](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options). Následující možnosti jsou pozoruhodné:
...
### Zapnutí ZNE pomocí možnosti `resilience_level` {#enable-zne-by-using-the-resilience-level-option}

ZNE (spolu s TREX a gate twirlingem) lze zapnout nastavením [úrovně odolnosti](/guides/estimator-noise-management#resilience) na `2`, jak je ukázáno v následujícím příkladu.
...
1. _Učení_: Je naučen twirled model šumu každé vrstvy provazujících Gate v Circuit.
...
- [Nakonfiguruj správu šumu s Estimatorem](/guides/estimator-noise-management).

- [Nakonfiguruj správu šumu se Samplerem](/guides/sampler-noise-management).

- Prozkoumej další [možnosti](/guides/runtime-options-overview) pro primitiva IBM Quantum.

- Rozhodni se, v jakém [režimu spouštění](/guides/execution-modes) spustíš svou úlohu.
