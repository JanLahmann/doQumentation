---
description: 'Pochop formát vstupů a výstupů (včetně Primitive Unified Blocs neboli PUBs) primitiv Qiskit SDK'
...
Příklady párů polí, která lze broadcastovat:
...
Příklady párů polí, která nelze broadcastovat:
...
Následující seznamy operátorů, ačkoli jsou z hlediska obsahu ekvivalentní, mají různé tvary:
...
Jakmile je jeden nebo více PUB odeslán na QPU k provedení a úloha úspěšně dokončí, data se vrátí jako kontejnerový objekt [`PrimitiveResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PrimitiveResult). `PrimitiveResult` obsahuje iterovatelný seznam objektů [`PubResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PubResult), které obsahují výsledky provedení pro každý PUB. Například úloha odeslaná s 20 PUB vrátí objekt `PrimitiveResult`, který obsahuje seznam 20 `PubResults`, jeden odpovídající každému PUB.
...
<Admonition type="tip" title="Doporučení">
  ...
  -  Prostuduj si API [primitiv Qiskit SDK](https://docs.quantum.ibm.com/api/qiskit/primitives).

  -  Prostuduj si API [primitiv Qiskit Aer](https://qiskit.github.io/qiskit-aer/apidocs/aer_primitives.html).

  -  Zjisti více o [primitivech IBM Quantum](/guides/qiskit-runtime-primitives).

  -  Prostuduj si API [`qiskit-ibm-runtime` Estimator](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2).

  -  Prostuduj si API [`qiskit-ibm-runtime` Sampler](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2).
</Admonition>
