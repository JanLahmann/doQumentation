...
Poté se autentizuj pomocí svého [IBM Quantum&reg; API klíče](http://quantum.cloud.ibm.com/) a za předpokladu, že jsi již [uložil/a svůj účet](/guides/functions-get-started#install-qiskit-functions-catalog-client) do lokálního prostředí, vyber Qiskit Function takto:
...
Zkontroluj [stav](/guides/functions-get-started#check-job-status) svého pracovního zatížení Qiskit Function nebo načti [výsledky](/guides/functions-get-started#retrieve-results) takto:
...
Pro přístup k energii základního stavu použij klíč „energy“. Klíč „eigenvector“ poskytuje CI koeficienty s odpovídající notací bitového řetězce konfigurace elektronů uloženou pod klíčem „states“ ve výsledcích.
...
Vezmi na vědomí, že použití této Qiskit Function je omezeno na problémy vyžadující nejvýše 20 qubitů, pokud není získána licence umožňující vyšší limit.

Napiš e-mail na [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) s licenčními dotazy.

### Příklad použití licencované funkce {#example-of-licensed-function-use}

Licencovaným uživatelům je vydán licenční token, který pak musí odeslat funkci pomocí wrapper knihovny.
Wrapper knihovnu lze nainstalovat [z PyPI](https://pypi.org/project/hivqe-qiskit-function-utils/) pomocí `pip install hivqe-qiskit-function-utils`.
Níže uvedený příklad ukazuje, jak by se tato knihovna měla použít k odeslání tvého licenčního tokenu při volání funkce.
