...
Možnosti můžeš nastavit při inicializaci Estimátoru, po inicializaci Estimátoru nebo je aktualizovat po inicializaci. Pokyny k použití těchto technik najdeš v tématu [Úvod do možností](/guides/runtime-options-overview#options-precedence).

Navíc můžeš nastavit hodnotu `precision` v metodě `run()`, jak je popsáno v následující části.
...
3. Pokud je povoleno `twirling` (ve výchozím nastavení True), použije se součin `num_randomizations` a `shots_per_randomization`, zadaný v [možnostech `twirling`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-twirling-options).
...
<Tabs>
  ...
  <TabItem value="Fractional" label="Frakční hradla">
  Nekompatibilní s:
  - Twirlingem hradel
  - PEA
  - PEC
  ...
  Může nefungovat při použití vlastních hradel.
  </TabItem>
  <TabItem value="Twirling" label="Twirling hradel">
  Nekompatibilní s frakčními hradly nebo se stretches.
