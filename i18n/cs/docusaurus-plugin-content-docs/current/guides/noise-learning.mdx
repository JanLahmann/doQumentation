...
Executor se řídí záměry návrhu specifikovanými v anotacích obvodů (ve formě simplexu) a možnostech. `InjectNoise` je anotace pro specifikaci místa injektování šumu a argument simplexu `pauli_lindblad_maps` specifikuje, který model šumu použít.

Obvod z předchozího příkladu prochází boxing pass managerem, který skupuje instrukce do anotovaných boxů. Příslušný kód je zde přidán pro lepší pochopení.
