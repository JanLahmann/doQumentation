...
Název „Qiskit" je obecný termín označující kolekci softwaru pro spouštění programů na kvantových počítačích. Nejvýznamnějším z těchto softwarových nástrojů je open-source Qiskit SDK a běhové prostředí, které se používá prostřednictvím IBM Quantum Compute Service (dříve známé jako Qiskit Runtime Service), prostřednictvím kterého můžeš spouštět úlohy na kvantových procesorových jednotkách (QPU) od IBM&reg;. S tím, jak se kvantová technologie vyvíjí, vyvíjí se i Qiskit – každý rok jsou vydávány nové funkce, které rozšiřují tuto základní sbírku kvantového softwaru.
...
- **Knihovna Circuit** ([`qiskit.circuit.library`](https://docs.quantum.ibm.com/api/qiskit/circuit_library)) – Rozsáhlá škála Circuit, instrukcí a Gate – klíčové stavební bloky pro kvantové výpočty založené na obvod.
...
- **Primitiva** ([`qiskit.primitives`](https://docs.quantum.ibm.com/api/qiskit/primitives)) – Modul obsahující základní definice a referenční implementace primitiv Sampler a Estimator, ze kterých mohou různí poskytovatelé kvantového hardwaru odvodit vlastní implementace. Více informací o primitivech IBM Quantum najdeš [v dokumentaci](/guides/primitives).
...
<Admonition type="tip" title="Doporučení">
  ...
  - Zjisti, jak používat [rozhraní primitiv](/guides/primitives) pro zjednodušení kvantového programování.
</Admonition>
