...
## Nastavení {#setup}
Nejprve se ověř pomocí svého [IBM Quantum API klíče](http://quantum.cloud.ibm.com/). Pak vyber Qiskit Function takto. (Tento kód předpokládá, že jsi již [uložil/a svůj účet](/guides/functions-get-started#install-qiskit-functions-catalog-client) do místního prostředí.)
...
### Odhadni magnetizace jednotlivých Qubitů $\langle Z_i \rangle$ {#estimate-single-qubit-magnetizations-langle-z-i-rangle}
...
Vyhodnotíme dynamiku magnetizace na skutečném kvantovém hardwaru. Fire Opal poskytuje Qiskit Function, která rozšiřuje standardní primitivum IBM Quantum Estimator o automatické potlačení chyb a správu výkonu. Trotterizované Circuit odesíláme přímo do Backend IBM®, zatímco Fire Opal se stará o spouštění s ohledem na šum.
