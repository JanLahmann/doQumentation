...
PQK je metoda používaná v kvantovém strojovém učení (QML) ke kódování klasických dat do kvantového prostoru příznaků a jejich zpětnému promítnutí do klasické domény, přičemž využívá kvantové počítače ke zlepšení výběru příznaků. Zahrnuje kódování klasických dat do kvantových stavů pomocí kvantového obvodu, obvykle procesem zvaným mapování příznaků, kde jsou data transformována do vysokodimenzionálního Hilbertova prostoru. „Projektovaný" aspekt se týká extrakce klasických informací z kvantových stavů měřením specifických observabilních veličin, aby se sestavila matice jádra použitelná v klasických algoritmech založených na jádrech, jako jsou podpůrné vektorové stroje. Tento přístup využívá výpočetní výhody kvantových systémů k potenciálně lepšímu výkonu u určitých úloh ve srovnání s klasickými metodami.

Hlavními stavebními kameny PQK jsou matice redukované hustoty (RDM), získané pomocí projektivních měření kvantové příznakové mapy. Konkrétně se typicky počítají jednokubitové matice redukované hustoty (1-RDM) pro každý kubit. Tyto naměřené hodnoty jsou poté použity jako vstupy do klasické funkce jádra, jako je exponenciální jádro, pro sestavení výsledné matice jádra.

PQK nabízejí potenciální výhody oproti standardním [kvantovým jádrům](/tutorials/quantum-kernel-training), zejména pro kvantový hardware blízké budoucnosti. Standardní kvantová jádra se typicky spoléhají na odhadování globálních překryvů stavů, jejichž přesné měření se s rostoucím počtem kubitů stává stále obtížnějším a jsou vysoce citlivá na šum. Naproti tomu PQK využívají lokální observabilní veličiny, jako jsou jednokubitové matice redukované hustoty (1-RDM), což vede k nižší vzorkovací zátěži, lepší odolnosti vůči hardwarovému šumu a lepší škálovatelnosti. Promítnutím kvantových stavů na lokální měřicí příznaky před aplikací klasické funkce jádra mohou PQK zachovat užitečné kvantové korelace a zároveň zůstat praktičtější pro současná (NISQ) zařízení.
...
V tomto kroku získáme všechny jednokubitové matice redukované hustoty (1-RDM) pomocí projektivních měření kvantové příznakové mapy, které budou později předány do klasické exponenciální funkce jádra.
Podívejme se, jak vypočítat 1-RDM pro jeden datový bod z datové sady, než projdeme všechna data. 1-RDM jsou kolekcí jednokubitových měření operátorů Pauli ``X``, ``Y`` a ``Z`` na všech kubitech. Je to proto, že jednokubitová RDM může být plně vyjádřena jako: $$\rho = \frac{1}{2} \big( I + \braket \sigma_x \sigma_x  + \braket \sigma_y \sigma_y + \braket \sigma_z \sigma_z  \big)$$
...
Poté spustíme kvantový obvod a změříme projekce. Všimni si, že zapínáme zmírnění chyb (error mitigation), včetně extrapolace k nulovému šumu (ZNE).
...
Vypíšeme velikost obvodu a hloubku dvoukubitových hradel.
...
Pomocí definice 1-RDM vidíme, že jednotlivé členy uvnitř funkce jádra lze vyhodnotit jako $\textrm{Tr}[P \rho_k (x_i)] = \braket P$, kde $P \in \{ X,Y,Z \}$. Tyto střední hodnoty jsou přesně to, co jsme měřili výše.
