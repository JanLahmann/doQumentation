---
title: "Vylepšení klasifikace příznaků pomocí projektovaných kvantových jader"
sidebar_label: "Vylepšení klasifikace příznaků pomocí projektovaných kvantových jader"
description: "Tutoriál o vylepšení klasifikace příznaků pomocí projektovaných kvantových jader"
...
*Odhadovaná doba: 80 minut na procesoru Heron r3 (POZNÁMKA: Jedná se pouze o odhad. Skutečná doba běhu se může lišit.)*

## Výstupy výuky {#learning-outcomes}

Po absolvování tohoto tutoriálu by uživatelé měli rozumět:
- Jak fungují projektovaná kvantová jádra (PQK) a kdy nabízejí potenciální kvantovou výhodu.
- Jak spustit PQK na hardware s využitím reálného datasetu.

## Předpoklady {#prerequisites}

Doporučujeme, aby uživatelé před absolvováním tohoto tutoriálu byli obeznámeni s následujícími tématy:
- [Kvantová jádra](/learning/courses/quantum-machine-learning/quantum-kernel-methods) z kurzu [kvantového strojového učení](/learning/courses/quantum-machine-learning) v IBM Quantum&reg; Learning

## Úvod {#background}

V tomto tutoriálu ukazujeme, jak spustit [projektované kvantové jádro](https://www.nature.com/articles/s41467-021-22539-9) (PQK) s Qiskitem na reálném biologickém datasetu, na základě článku [Enhanced Prediction of CAR T-Cell Cytotoxicity with Quantum-Kernel Methods](https://arxiv.org/abs/2507.22710) [[1]](#references).

PQK je metoda používaná v kvantovém strojovém učení (QML) ke kódování klasických dat do kvantového prostoru příznaků a jejich zpětnému promítnutí do klasické domény, přičemž využívá kvantové počítače ke zlepšení výběru příznaků. Zahrnuje kódování klasických dat do kvantových stavů pomocí kvantového Circuit, obvykle procesem zvaným mapování příznaků, kde jsou data transformována do vysokodimenzionálního Hilbertova prostoru. „Projektovaný" aspekt se týká extrakce klasických informací z kvantových stavů měřením specifických observabilních veličin, aby se sestavila maticová jádra použitelná v klasických algoritmech založených na jádrech, jako jsou podpůrné vektorové stroje. Tento přístup využívá výpočetní výhody kvantových systémů k potenciálně lepšímu výkonu u určitých úloh ve srovnání s klasickými metodami.

Hlavními stavebními kameny PQK jsou matice redukované hustoty (RDM), získané pomocí projektivních měření kvantové příznakové mapy. Konkrétně se typicky počítají jednoQubitové matice redukované hustoty (1-RDM) pro každý Qubit. Tyto naměřené hodnoty jsou poté použity jako vstupy do klasické kernelové funkce, jako je exponenciální kernel, pro sestavení výsledné matice jádra.

PQK nabízejí potenciální výhody oproti standardním [kvantovým jádrům](/tutorials/quantum-kernel-training), zejména pro krátkodobý kvantový hardware. Standardní kvantová jádra se typicky spoléhají na odhadování globálních překryvů stavů, jejichž přesné měření se s rostoucím počtem Qubitů stává stále obtížnějším a jsou vysoce citlivá na šum. Naproti tomu PQK využívají lokální observabilní veličiny, jako jsou jednoQubitové matice redukované hustoty (1-RDM), což vede k nižší vzorkovací zátěži, lepší odolnosti vůči hardwarovému šumu a lepší škálovatelnosti. Promítnutím kvantových stavů na lokální měřicí příznaky před aplikací klasické kernelové funkce mohou PQK zachovat užitečné kvantové korelace a zároveň zůstat praktičtější pro krátkodobá zařízení.
...
## Příklad v malém měřítku na simulátoru {#small-scale-simulator-example}
V tomto tutoriálu příklad v malém měřítku na simulátoru vynecháváme, protože naším primárním cílem je ukázat, jak mohou projektovaná kvantová jádra škálovat na větší systémy a skutečný hardware.
## Příklad ve velkém měřítku na hardware {#large-scale-hardware-example}
### Krok 1: Mapování klasických vstupů na kvantový problém {#step-1-map-classical-inputs-to-a-quantum-problem}
### Příprava datasetu {#dataset-preparation}

V tomto tutoriálu používáme reálný biologický dataset pro úlohu binární klasifikace, který vygeneroval Daniels et al. (2022) a lze ho stáhnout z [doplňkových materiálů](https://www.science.org/doi/full/10.1126/science.abq0225#supplementary-materials) přiložených k článku. Data obsahují CAR T-buňky, což jsou geneticky upravené T-buňky používané v imunoterapii k léčbě určitých druhů rakoviny. T-buňky, typ imunitních buněk, jsou v laboratoři modifikovány tak, aby exprimovaly chimérické antigenní receptory (CAR), které cílí na specifické proteiny na rakovinných buňkách. Tyto modifikované T-buňky dokáží rakovinné buňky rozpoznávat a ničit efektivněji. Příznaky dat jsou motivy CAR T-buněk, které označují specifické strukturální nebo funkční složky CAR zabudované do T-buněk. Na základě těchto motivů je naším úkolem předpovědět cytotoxicitu dané CAR T-buňky a označit ji jako toxickou nebo netoxickou.
Níže jsou ukázány pomocné funkce pro předzpracování tohoto datasetu.
...
Tento tutoriál můžeš spustit spuštěním následující buňky, která automaticky vytvoří požadovanou adresářovou strukturu a stáhne trénovací i testovací soubory přímo do tvého prostředí. Pokud tyto soubory již máš lokálně, tento krok je bezpečně přepíše, aby zajistil konzistenci verzí.
...
Dataset také transformujeme tak, aby $1$ bylo reprezentováno jako $\pi/2$ pro účely škálování.
...
Ověříme velikosti a tvary trénovacích a testovacích datasetů.
...
### Krok 2: Optimalizace problému pro provádění na kvantovém hardware {#step-2-optimize-problem-for-quantum-hardware-execution}
### Kvantový Circuit {#quantum-circuit}

Nyní sestavíme mapu příznaků, která vloží náš klasický dataset do vícerozměrného prostoru příznaků. Pro toto vložení používáme [`ZZFeatureMap`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ZZFeatureMap) z Qiskitu.
...
Další možností kvantového vložení je ansatz evoluce 1D-Heisenbergova Hamiltoniánu. Tuto část můžeš přeskočit, pokud chceš pokračovat s `ZZFeatureMap`.
...
### Měření 1-RDM {#measure-1-rdms}
V tomto kroku získáme všechny jednoQubitové matice redukované hustoty (1-RDM) pomocí projektivních měření kvantové příznakové mapy, které budou později předány do klasické exponenciální kernelové funkce.
Podívejme se, jak vypočítat 1-RDM pro jeden datový bod z datové sady, než projdeme všechna data. 1-RDM jsou kolekcí jednoQubitových měření operátorů Pauli ``X``, ``Y`` a ``Z`` na všech Qubitech. Je to proto, že jednoQubitová RDM může být plně vyjádřena jako: $$\rho = \frac{1}{2} \big( I + \braket \sigma_x \sigma_x  + \braket \sigma_y \sigma_y + \braket \sigma_z \sigma_z  \big)$$
Nejprve vybereme Backend, který použijeme.
...
Poté spustíme kvantový Circuit a změříme projekce. Všimni si, že zapínáme korekci chyb, včetně extrapolace nulového šumu (ZNE).
...
Vypíšeme velikost Circuitu a hloubku dvoQubitových Gate operací.
...
![Výstup předchozí buňky kódu](/docs/images/tutorials/projected-quantum-kernels/extracted-outputs/4f573436-ec5c-451b-976c-ad718b3c201d-1.avif)

Nyní můžeme projít celou trénovací datovou sadu a získat všechny 1-RDM.
Poskytujeme také výsledky z experimentu, který jsme spustili na kvantovém hardwaru. Trénování si můžeš spustit sám nastavením příznaku níže na `True`, nebo použít výsledky projekcí, které poskytujeme.
...
Jakmile jsou joby dokončeny, můžeme získat výsledky.
...
To samé zopakujeme pro testovací sadu.
...
Výsledky můžeme získat stejně jako dříve.
...
### Definice projektovaného kvantového jádra {#define-the-projected-quantum-kernel}
Projektované kvantové jádro je definováno následující funkcí jádra: $$k^{\textrm{PQ}}(x_i, x_j) = \textrm{exp} \Big(-\gamma \sum_k \sum_{P \in \{ X,Y,Z \}} (\textrm{Tr}[P \rho_k(x_i)] - \textrm{Tr}[P \rho_k(x_j)])^2 \Big) $$
Ve výše uvedené rovnici je $\gamma>0$ laditelný hyperparametr. $K^{\textrm{PQ}}_{ij} = k^{\textrm{PQ}}(x_i, x_j)$ jsou prvky matice jádra $K^{\textrm{PQ}}$.
Pomocí definice 1-RDM vidíme, že jednotlivé členy uvnitř funkce jádra lze vyhodnotit jako $\textrm{Tr}[P \rho_k (x_i)] = \braket P$, kde $P \in \{ X,Y,Z \}$. Tato střední hodnota jsou přesně to, co jsme výše měřili.
Pomocí ``scikit-learn`` lze jádro vypočítat ještě snáze. Je to díky snadno dostupnému jádru radiální bázové funkce (``'rbf'``): $ \textrm{exp} (-\gamma \lVert x - x' \rVert^2)$. Nejprve musíme nové projektované trénovací a testovací datové sady přetvarovat do dvourozměrných polí.
Upozorňujeme, že zpracování celé datové sady může na QPU trvat přibližně 80 minut. Aby bylo možné zbytek tutoriálu snadno spustit, nabízíme navíc projekce z dříve provedeného experimentu (ty jsou obsaženy v souborech stažených v bloku kódu `Download dataset`). Pokud jsi trénování provedl/a sám/sama, můžeš pokračovat s vlastními výsledky.
...
Nyní můžeme spustit klasické SVM na tomto předem vypočítaném jádře a pro predikci použít jádro mezi testovacími a trénovacími sadami.
...
### Klasická referenční hodnota {#classical-benchmarking}
Můžeme spustit klasické SVM s radiální bázovou funkcí jako jádrem bez kvantové projekce. Tento výsledek je naší klasickou referenční hodnotou.
...
## Dodatek: Ověření potenciální kvantové výhody datové sady v učebních úlohách {#appendix-verify-the-datasets-potential-quantum-advantage-in-learning-tasks}

Ne všechny datové sady nabízejí potenciální výhodu z použití PQK. Existují teoretické meze, které lze použít jako předběžný test k zjištění, zda může konkrétní datová sada těžit z PQK. Autoři článku [Power of data in quantum machine learning](https://www.nature.com/articles/s41467-021-22539-9) [2] definují k měření tohoto jevu veličiny označované jako klasická a kvantová složitost modelu a geometrická separace klasického a kvantového modelu. Aby bylo možné očekávat potenciální kvantovou výhodu z PQK, geometrická separace mezi klasickými a kvantově projektovanými jádry by měla být přibližně řádu $\sqrt{N}$, kde $N$ je počet trénovacích vzorků. Pokud je tato podmínka splněna, přejdeme ke kontrole složitostí modelů. Pokud je klasická složitost modelu řádu $N$ a složitost kvantově projektovaného modelu je podstatně menší než $N$, lze očekávat potenciální výhodu z PQK.
Geometrická separace je definována takto (F19 v [[2]](#references)):
...
Složitost modelu je definována takto (M1 v [[2]](#references)):
...
- Podrobný kurz [kvantového strojového učení](/learning/courses/quantum-machine-learning) od IBM Quantum Learning
