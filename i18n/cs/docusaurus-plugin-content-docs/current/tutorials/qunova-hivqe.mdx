---
description: 'Použij Qunova HiVQE k výpočtu disociačních křivek potencionální energetické plochy (PES).'
notebook_path: tutorials/qunova-hivqe.ipynb
sidebar_label: 'Disociační PES křivky s Qunova HiVQE'
title: 'Disociační PES křivky s Qunova HiVQE'
---
...
Qiskit Functions jsou experimentální funkce dostupné pouze uživatelům IBM Quantum&reg; Premium Plan, Flex Plan a On-Prem (přes IBM Quantum Platform API) Plan. Jsou ve stavu preview vydání a mohou se změnit.
</Admonition>

*Odhadovaná spotřeba (POZNÁMKA: Toto je pouze odhad. Skutečná doba běhu se může lišit.)*

* Li2S: Pět minut QPU času na procesoru Heron r2

* FeP-NO: Pět minut QPU času na procesoru Heron r2


## Pozadí {#background}

Přesný výpočet energií chemických reakcí je klíčový pro vědecký pokrok v materiálové vědě, chemickém inženýrství, vývoji léků a dalších oborech. Mezi různými chemickými systémy si systém Li-S získal značnou pozornost pro pochopení a vývoj nových složení baterií. Tento návod poskytuje praktické zkušenosti s výpočtem křivky potenciální energetické plochy (PES) disociace vazby Li-S v systému $Li_2S$ odstraněním jednoho atomu lithia pomocí výpočtů HiVQE. Výsledky lze porovnat s referenčními výpočty (CASCI) i klasickými metodami jako Hartree-Fock (HF) pro problém s 20 qubity.

## Požadavky {#requirements}

Nainstaluj následující závislosti pro spuštění kódu v tomto návodu.
...
Aby ses mohl řídit tímto návodem, importuj funkci `qunova/hivqe-chemistry` přes `QiskitFunctionCatalog`. K spuštění této funkce potřebuješ účet IBM Quantum Premium Plan, Flex Plan nebo On-Prem (IBM Quantum Platform API) Plan s licencí od Qunova.
...
## Část 1: Li2S (20Q) {#part-1-li2s-20q}

### Krok 1: Mapování klasických vstupů na kvantový problém {#step-1-map-classical-inputs-to-a-quantum-problem}

Definuj geometrie $Li_2S$ ve formátu slovníku pro různé vzdálenosti vazby Li-S pro výpočet PES křivky. Tyto geometrie jsou optimalizovány pomocí výpočtů B3LYP/631g.
...
Výpočty HiVQE budou provedeny s níže definovanými možnostmi. Při použití báze sto3g pro $Li_2S$ existuje 19 prostorových orbitalů s 22 elektrony. Pro spuštění případu (10o,10e) s výpočtem HiVQE můžeš definovat 10 aktivních orbitalů a šest zmrazených orbitalů. V každé iteraci bude použito 100 výstřelů (shots) pro vzorkování elektronových konfigurací generovaných kvantovým obvodem ExcitationPreserving (`epa`) s `circular` provázáním a dvěma opakováními (`reps`). Maximální počet iterací je nastaven na 30, aby bylo zajištěno ukončení iterace při konvergenci energie.
...
### Kroky 2 a 3: Optimalizace problému pro spuštění na kvantovém hardware a spuštění pomocí funkce HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}

Nastav smyčku `for` pro spuštění výpočtů HiVQE s geometriemi a níže definovanými možnostmi. Úlohy jsou odesílány v rámci smyčky `for`. V tomto návodu odešleš šest geometrií a načteš výsledky po dokončení všech. V hlavním spuštění funkce je třeba definovat `max_states` a `max_expansion_states` pro kontrolu maximální velikosti matice podprostoru a kontrolu toho, kolik stavů lze generovat pomocí klasických metod CI expanze v každé iteraci. ID úloh funkce budou uložena ve slovníku s každým popiskem geometrie pro další sledování a zpracování výstupu.
...
Zkontrolujme, zda jsou všechny úlohy stále spuštěné nebo dokončené.
...
Jakmile jsou všechny úlohy dokončeny, načteme všechny výsledky výpočtů.
...
Celková doba běhu QPU použitá v úloze může být sledována přihlášením na [IBM Quantum Platform](https://quantum.cloud.ibm.com/) a zobrazením odeslaných úloh se štítkem `qunova-chemistry-hivqe`.

### Krok 4: Následné zpracování a porovnání s klasickými metodami {#step-4-post-process-and-compare-with-classical-methods}

Klasický referenční výpočet (CASCI) lze provést pro (10o,10e) pro ověření výsledků HiVQE.
...
### Vykreslení disociační křivky pro Li\_2S {#plotting-the-dissociation-curve-for-li-2s}

Vykresli a porovnej výsledky HiVQE s HF a CASCI.
Můžeš pozorovat, že všechny výpočty HiVQE jsou dobře v souladu s klasickým referenčním výsledkem (CASCI).
...
## Část 2: FeP-NO (44Q) {#part-2-fep-no-44q}

Vezmi na vědomí, že pro spuštění funkce na systému FeP-NO s nastaveními uvedenými v tomto příkladu potřebuješ licenci, která umožňuje používat funkci s alespoň 44 qubity. Pro informace o získání licence kontaktuj [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com).

### Krok 1: Mapování klasických vstupů na kvantový problém {#step-1-map-classical-inputs-to-a-quantum-problem-1}

Definuj možnosti pro výpočty HiVQE
...
Definuj geometrie FeP-NO ve formátu slovníku pro různé vzdálenosti vazby Fe-N pro výpočet křivky PES.
...
### Kroky 2 a 3: Optimalizace problému pro spuštění na kvantovém hardwaru a spuštění pomocí funkce HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

Na základě nastavení HiVQE a geometrií získej výsledky postupně.

#### Odeslání výpočtu d(Fe-N) = 1,75 $\AA$. {#submit-dfe-n-175-aa-calculation}
...
Sleduj úlohu a získej výsledek pro výpočet d(Fe-N) = 1,75 $\AA$.
...
#### Odeslání výpočtu d(Fe-N) = 2,00 $\AA$. {#submit-dfe-n-200-aa-calculation}
...
Sleduj úlohu a získej výsledek pro výpočet d(Fe-N) = 2,00 $\AA$.
...
#### Odeslání výpočtu d(Fe-N) = 5,00 $\AA$. {#submit-dfe-n-500-aa-calculation}
...
Sleduj úlohu a získej výsledek pro výpočet d(Fe-N) = 5,00 $\AA$.
...
### Krok 4: Následné zpracování a porovnání s klasickými metodami {#step-4-post-process-and-compare-with-classical-methods-1}

Pro ověření výsledků HiVQE jsou k dispozici výsledky klasického referenčního výpočtu (CASCI-DMRG, maxM=800) pro (22o,22e).
...
![Výstup předchozí buňky s kódem](/docs/images/tutorials/qunova-hivqe/extracted-outputs/49a9bf81-1.avif)

## Průzkum k tutoriálu {#tutorial-survey}

Vyplň prosím tento krátký průzkum a poskytni zpětnou vazbu k tomuto tutoriálu. Tvoje podněty nám pomohou zlepšit obsah a uživatelský zážitek.

> **Poznámka:** Tento průzkum pochází od **IBM Quantum** a zaměřuje se na **obsah tutoriálu** (napsaný IBM). doQumentation poskytuje web, překlady a spouštění kódu — pro zpětnou vazbu k nim prosím [otevři issue na GitHubu](https://github.com/JanLahmann/doQumentation/issues).

[Odkaz na průzkum](https://your.feedback.ibm.com/jfe/form/SV_9AizRyKFHEGzVs2)
