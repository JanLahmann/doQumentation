---
description: 'Spusť experiment chybové mitigace v utility-scale měřítku s extrapolací nulového šumu a probabilistickým zesílením chyb.'
notebook_path: tutorials/probabilistic-error-amplification.ipynb
sidebar_label: 'Chybová mitigace v utility-scale měřítku pomocí probabilistického zesílení chyb'
title: 'Chybová mitigace v utility-scale měřítku pomocí probabilistického zesílení chyb'
---
...
*Odhadovaná spotřeba: 14 minut na procesoru Heron r3 (POZNÁMKA: Jedná se pouze o odhad. Skutečná doba běhu se může lišit.)*

## Výsledky učení {#learning-outcomes}
Po absolvování tohoto tutoriálu by uživatelé měli rozumět:
- Teorii za *extrapolací nulového šumu* (ZNE), různým metodám zesílení šumu a tomu, proč je *probabilistické zesílení chyb* (PEA) preferováno pro experimenty v utility-scale měřítku.
- Jak implementovat ZNE s PEA v praxi pomocí Qiskitu.

## Předpoklady {#prerequisites}
Doporučujeme, aby uživatelé byli před absolvováním tohoto tutoriálu obeznámeni s následujícími tématy:
- [Lekce chybové mitigace](/learning/courses/utility-scale-quantum-computing/error-mitigation) kurzu *Utility-scale quantum computing* pro základní znalosti používání chybové mitigace v Qiskitu.
- [Lekce Utility-I](/learning/courses/utility-scale-quantum-computing/utility-i) kurzu *Utility-scale quantum computing* pro více pozadí k utility-scale experimentu použitému jako příklad v tomto tutoriálu.

## Pozadí {#background}

Tento tutoriál ukazuje, jak spustit experiment chybové mitigace v utility-scale měřítku pomocí IBM Quantum Compute Service s experimentální verzí *extrapolace nulového šumu* (ZNE) s *probabilistickým zesílením chyb* (PEA).
...
### Extrapolace nulového šumu (ZNE) {#zero-noise-extrapolation-zne}
...
kde $\lambda$ parametrizuje sílu šumu a lze ji zesílit.

ZNE lze implementovat pomocí následujících kroků:

1. Zesil šum Circuit pro několik faktorů šumu $\lambda_1, \lambda_2, ... $

2. Spusť každý obvod se zesíleným šumem a změř $\langle A(\lambda_1)\rangle, ...$

3. Extrapoluj zpět k nulové hranici šumu $\langle A(0)\rangle$
...
#### Zesílení šumu pro ZNE {#amplify-noise-for-zne}
...
### Naučení modelu šumu pro PEA {#learn-the-noise-model-for-pea}
...
**Zdroj**: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, *Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors* [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)
...
## Nastavení {#setup}
V buňce níže importujeme relevantní balíčky a vytvoříme pomocné funkce pro konstrukci Circuit pro Trotterizovanou časovou evoluci dvourozměrného transverzálního Isingova modelu v poli, který respektuje topologii Backend.
...
Tento krok přeskočíme, protože chybová mitigace za běhu není na simulátorech podporována.
...
#### Vytvoření parametrizovaného Circuit Isingova modelu {#create-a-parameterized-ising-model-circuit}

##### Nastavení Backend {#establish-a-backend}
...
##### Definování vazeb vrstev pro zapletení {#define-entangling-layer-couplings}
...
##### Odebrání špatných Qubitů {#remove-bad-qubits}
...
##### Generování hlavního Trotterova Circuit {#main-trotter-circuit-generation}
...
#### Vytvoření seznamu hodnot parametrů pro pozdější přiřazení {#create-a-list-of-parameter-values-to-be-assigned-later}
...
* Přelož Gate v našem Circuit na instrukce [Instruction Set Architecture (ISA)](/guides/transpile#instruction-set-architecture), které lze přímo spustit na hardwaru.
...
Transpiluj svůj Circuit pro zvolený Backend tak, že vytvoříš pass manager a poté ho spustíš na obvod. Zároveň zafixuj počáteční rozložení Circuit na již vybrané `good_qubits`. Snadným způsobem, jak vytvořit pass manager, je použít funkci [`generate_preset_pass_manager`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.generate_preset_pass_manager). Podrobnější vysvětlení transpilace pomocí pass managerů najdeš v části [Transpile with pass managers](/guides/transpile-with-pass-managers).
...
#### ISA pozorovatelné veličiny {#isa-observables}
...
Všimni si, že každá pozorovatelná veličina je zabalena do seznamu v následujícím bloku kódu. Je to proto, aby se *vysílala* spolu s hodnotami parametrů tak, aby každá pozorovatelná veličina Qubitu byla měřena pro každou hodnotu theta. Pravidla pro vysílání pro primitiva najdeš v [dokumentaci primitivů](/guides/primitives).
...
#### Konfigurace možností Estimatoru {#configure-estimator-options}
Dále nakonfiguruj možnosti `Estimator`u potřebné ke spuštění experimentu zmírnění. To zahrnuje možnosti pro učení šumu vrstev zapletení a pro ZNE extrapolaci.
...
##### Vysvětlení možností ZNE {#explanation-of-zne-options}
...
* **amplifier**: Metoda, která se použije při zesilování šumu na zamýšlené faktory šumu.
  Povolené hodnoty jsou `"gate_folding"`, která zesiluje opakováním dvoukubitových základních Gate,
  a `"pea"`, která zesiluje pravděpodobnostním vzorkováním po naučení Pauliho-točeného
  šumového modelu pro vrstvy točených dvoukubitových základních Gate. K dispozici jsou také možnosti `"gate_folding_front"` a `"gate_folding_back"`, které jsou vysvětleny v [dokumentaci API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier).

* **extrapolated\_noise\_factors**: Zadej jednu nebo více hodnot faktoru šumu, při kterých se mají vyhodnocovat
  extrapolované modely. Pokud se jedná o posloupnost hodnot, vrácené výsledky budou mít hodnoty pole se zadaným faktorem šumu vyhodnoceným pro extrapolační model. Hodnota
  0 odpovídá extrapolaci s nulovým šumem.


#### Spuštění experimentu {#run-the-experiment}
...
### Krok 4: Zpracování výsledků a jejich vrácení v požadovaném klasickém formátu {#step-4-post-process-and-return-result-in-desired-classical-format}
...
#### Obecné tvary výsledků a metadata {#general-result-shapes-and-metadata}
...
1. `pub_result.data.evs`: Hodnoty střední hodnoty odpovídající nulovému šumu (na základě heuristicky nejlepší extrapolace).
...
2. `pub_result.data.evs_extrapolated`: Hodnoty střední hodnoty pro extrapolované faktory šumu pro každý extrapolátor. Toto pole má dvě další osy.
...
3. `pub_result.data.evs_noise_factors`: Surové hodnoty střední hodnoty pro každý faktor šumu.
...
### Vykreslení výsledků Trotterovy simulace {#plot-trotter-simulation-results}
...
![Výstup předchozí buňky kódu](/docs/images/tutorials/probabilistic-error-amplification/extracted-outputs/e466736a-0.avif)
...
### Vykreslení výsledků extrapolace pro jednotlivé qubity {#plot-extrapolation-results-for-individual-qubits}
...
![Výstup předchozí buňky kódu](/docs/images/tutorials/probabilistic-error-amplification/extracted-outputs/bea9695a-0.avif)

## Další kroky {#next-steps}
<Admonition type="tip" title="Doporučení">
Pokud tě tato práce zaujala, mohlo by tě zajímat následující materiál:
- [Tutoriál](/tutorials/combine-error-mitigation-techniques) zaměřený na kombinování technik chybové mitigace.
- Podrobná [dokumentace](/guides/error-mitigation-and-suppression-techniques) o technikách chybové mitigace dostupných v Qiskitu.
- Další lekce věnované experimentům v utility-scale měřítku: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) a [Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
