---
description: 'Řeš optimalizační problém kvantové utility pomocí Optimization Solveru — Qiskit funkce od Q-CTRL Fire Opal'
notebook_path: tutorials/solve-higher-order-binary-optimization-problems-with-q-ctrls-optimization-solver.ipynb
sidebar_label: 'Binární optimalizace vyššího řádu s Optimization Solverem od Q-CTRL'
title: 'Binární optimalizace vyššího řádu s Optimization Solverem od Q-CTRL'
---
...
Qiskit Functions jsou experimentální funkce dostupné pouze uživatelům plánů IBM Quantum&reg; Premium Plan, Flex Plan a On-Prem (přes IBM Quantum Platform API). Jsou ve stavu preview a mohou se změnit.
</Admonition>
...
Tento tutoriál ukazuje, jak vyřešit problém binární optimalizace vyššího řádu (HOBO) pomocí [Optimization Solveru — Qiskit funkce od Q-CTRL Fire Opal](/guides/q-ctrl-optimization-solver). Ukázkový příklad v tomto tutoriálu je optimalizační problém navržený k nalezení energie základního stavu náhodně vázaného Isingova modelu o 156 qubitech s kubickými členy. Optimization Solver lze použít pro obecné optimalizační problémy definovatelné jako účelová funkce.

Optimization Solver plně automatizuje kroky implementace optimalizačních problémů na kvantovém hardwaru s ohledem na jeho vlastnosti a díky využití [Performance Managementu](/guides/q-ctrl-performance-management) pro kvantové provádění dosahuje přesných řešení v měřítku utility. Podrobný přehled celého pracovního postupu Optimization Solveru a výsledků benchmarkování najdeš v [publikovaném rukopisu](https://arxiv.org/abs/2406.01743).
...
Také budeš potřebovat přístup k funkci Optimization Solver. [Vyplň formulář](https://quantum.cloud.ibm.com/functions?id=q-ctrl-optimization-solver) a požádej o přístup.
...
Definuj svoje přihlašovací údaje pro [IBM Quantum Platform](https://quantum.cloud.ibm.com/), které budou použity v celém tutoriálu k ověření identity v IBM Quantum Compute Service a Qiskit Functions.
...
## Krok 1: Definuj problém jako účelovou funkci {#step-1-define-the-problem-as-an-objective-function}
Optimization Solver přijímá jako vstup buď účelovou funkci, nebo graf. V tomto tutoriálu je problém minimalizace Isingova spinového skla definován jako účelová funkce a byl přizpůsoben pro topologii heavy-hex zařízení IBM&reg;.
...
Podrobnou diskusi o konstrukci definice problému a předchozích výsledcích získaných pomocí Optimization Solveru najdeš v [tomto technickém rukopisu](https://arxiv.org/abs/2406.01743). Problém byl původně definován a vyhodnocen jako součást [článku publikovaného Los Alamos National Laboratory](https://arxiv.org/abs/2312.00997) a byl upraven tak, aby plně využíval šířku 156-qubitových procesorů IBM Quantum Heron.
...
## Krok 2: Spuštění hybridního algoritmu pomocí Fire Opal Optimization Solver {#step-2-run-the-hybrid-algorithm-using-the-fire-opal-optimization-solver}
...
Ke kontrole stavu své úlohy Qiskit Function můžeš použít známá [Qiskit Serverless API](/guides/serverless):
...
Solver vrátí slovník s řešením a přidruženými metadaty, jako je bitový řetězec řešení, počet iterací a mapování proměnných na bitový řetězec. Úplnou definici vstupů a výstupů Solveru najdeš v [dokumentaci](https://docs.quantum.ibm.com/api/functions/q-ctrl-optimization-solver).
...
## Krok 3: Vyhodnocení výsledků {#step-3-evaluate-results}
...
Solver nalezl správné řešení, které bylo ověřeno pomocí klasického optimalizačního softwaru. Složitost tohoto problému v měřítku utility vyžaduje pro klasické řešení pokročilý optimalizační software, jako je [IBM ILOG CPLEX Optimization Studio (CPLEX)](https://www.ibm.com/products/ilog-cplex-optimization-studio) nebo [Gurobi Optimization](https://www.gurobi.com/).
Pro vizuální analýzu kvality výsledků můžeš vykreslit výsledky výpočtem hodnot nákladů z bitových řetězců a jejich pravděpodobností. Pro srovnání vykresli výsledky vedle distribuce náhodně vzorkovaných bitových řetězců, což odpovídá „hrubé síle" klasického řešení. Pokud algoritmus konzistentně nachází nižší náklady, naznačuje to, že kvantový algoritmus efektivně řeší optimalizační problém.
