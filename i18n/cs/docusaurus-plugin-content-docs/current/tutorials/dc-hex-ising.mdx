...
Sekvence DD je přidána funkcí `add_stretch_dd()`, která využívá trvání [`stretch`](/guides/stretch) k určení časových intervalů mezi DD gates. Trvání `stretch` je způsob, jak specifikovat roztažitelnou časovou délku pro operaci `delay`, takže délka zpoždění může narůst, aby zaplnila dobu nečinnosti qubitu. Proměnné trvání specifikované pomocí `stretch` jsou při kompilaci přeloženy na požadované délky splňující určité omezení. To je velmi užitečné, když je načasování DD sekvencí zásadní pro dobrou výkonnost potlačení chyb. Více podrobností o typu `stretch` najdeš v dokumentaci [OpenQASM](https://openqasm.com/language/delays.html#duration-and-stretch-types). Podpora typu `stretch` je v současné době experimentální. Podrobnosti o omezeních jejího použití najdeš v [sekci omezení](/guides/stretch#qiskit-runtime-limitations) dokumentace `stretch`.
