...
- Jak použít [SQD Qiskit addon](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd) k aproximaci energie základního stavu molekulárního systému pomocí bitových řetězců vzorkovaných z kvantové procesorové jednotky (QPU).
...
## Pozadí {#background}
V tomto tutoriálu ukážeme, jak post-procesovat hlučné kvantové vzorky k aproximaci základního stavu molekuly dusíku $\text{N}_2$ při rovnovážné délce vazby pomocí [SQD Qiskit addonů](https://github.com/Qiskit/qiskit-addon-sqd) k implementaci [algoritmu kvantové diagonalizace na základě vzorků (SQD)](https://arxiv.org/abs/2405.05068). Více podrobností o softwaru najdeš v příslušné [dokumentaci](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd), včetně [jednoduchého příkladu](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd/guides/quickstart) pro začátek.
...
Po optimalizaci Circuit pro spuštění na hardwaru jsme připraveni spustit ho na cílovém hardwaru a shromáždit vzorky pro odhad energie základního stavu. Protože máme pouze jeden obvod, použijeme IBM Quantum Compute Service [režim spuštění úlohy](/guides/execution-modes) a spustíme náš obvod.
...
První graf ukazuje, že v této simulaci jsme po první iteraci již v rozmezí `1 mH` od přesné odpovědi (chemická přesnost se obvykle považuje za `1 kcal/mol` $\approx$ `1.6 mH`). Jde ale o malý systém a protože jsou vzorky bezšumové, konfigurační obnova (configuration recovery) není potřeba. U většího systému spuštěného na zašuměném QPU by mohlo být potřeba více iterací konfigurační obnovy a výsledná přesnost by mohla být horší. Obecně lze energii vylepšit povolením více iterací konfigurační obnovy nebo zvýšením počtu vzorků na batch.
...
- [Škálování pracovních postupů SQD pro chemii pomocí solveru Dice](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd/guides/integrate-dice-solver) – stránka ukazující, jak používat efektivnější software Dice pro diagonalizaci
- [Dokumentace API SQD addonů](https://docs.quantum.ibm.com/api/qiskit-addon-sqd/fermion#diagonalize_fermionic_hamiltonian) – reference pro funkci `diagonalize_fermionic_hamiltonian`
