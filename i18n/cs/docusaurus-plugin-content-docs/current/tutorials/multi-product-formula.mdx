...
Dalším zajímavým chováním, které lze z grafu pozorovat, je to, že střední hodnota PF pro $k=1$ začíná vykazovat erratické chování (navíc k tomu, že neposkytuje dobrou aproximaci přesné hodnoty) pro časy, kdy $t/k > 1$, jak je vysvětleno v [průvodci](https://qiskit.github.io/qiskit-addon-mpf/how_tos/choose_trotter_steps.html) k volbě počtu Trotterových kroků.
