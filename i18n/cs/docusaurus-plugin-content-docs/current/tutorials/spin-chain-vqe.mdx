...
- Jak spouštět variační pracovní postupy na kvantovém hardwaru IBM® pomocí primitiv IBM Quantum a sessions
...
Minimalizujeme odhadovanou energii základního stavu systému optimalizací parametrů Circuitu. Použijeme primitivum IBM Quantum `Estimator` k vyhodnocení účelové funkce během optimalizace.

Protože jsme v kroku 2 optimalizovali Circuit pro backend, můžeme se vyhnout transpilaci na serveru Quantum Compute nastavením `skip_transpilation=True` a předáním optimalizovaného Circuitu. Pro tuto ukázku spustíme výpočet na QPU pomocí primitiv IBM Quantum. Pro spuštění s primitivy `qiskit` založenými na stavovém vektoru nahraď blok kódu používající primitiva IBM Quantum komentovaným blokem.
