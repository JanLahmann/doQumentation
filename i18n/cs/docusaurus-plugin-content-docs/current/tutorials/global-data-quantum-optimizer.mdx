---
title: "Dynamická optimalizace portfolia s Portfolio Optimizerem od Global Data Quantum"
sidebar_label: "Dynamická optimalizace portfolia s Portfolio Optimizerem od Global Data Quantum"
...
Doporučujeme spustit následující buňku a ověřit, že data byla správně stažena z Yahoo Finance API. Pokud se stažení nezdaří, zobrazí se upozornění, že data budou načtena z alternativní online databáze.
</Admonition>
...
Slovník `ansatz_settings` konfiguruje ansatz kvantového Circuit. Parametr `ansatz` specifikuje použití přístupu `"optimized_real_amplitudes"`, což je hardwarově efektivní ansatz navržený pro finanční optimalizační problémy. Možnost `multiple_passmanager`, která umožňuje použití více pass managerů (včetně výchozího lokálního pass manageru Qiskit a transpilační služby Qiskit s podporou AI) pro zlepšení výkonu a efektivity optimalizace Circuit, je aktuálně nastavena na false, protože tato funkce je dočasně nedostupná.
