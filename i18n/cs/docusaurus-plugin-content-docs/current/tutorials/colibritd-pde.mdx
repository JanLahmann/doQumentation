...
## Krok 1: Nastavení vlastností problému k řešení {#step-1-set-properties-of-the-problem-to-solve}
...
na IBM&reg; QPU pomocí QUICK-PDE, danou:
...
proces řešení a výsledné řešení. Pro tuto volbu počáteční podmínky
připouští rovnice následující analytické řešení:

$$u(t, x) = \frac{ax + b}{at + 1},$$

které platí, dokud nevznikl šok, tedy pro $t < -1/a$, když $a < 0$.
...
## Krok 2 (v případě potřeby): Optimalizace problému pro spuštění na kvantovém hardwaru {#step-2-if-needed-optimize-problem-for-quantum-hardware-execution}
