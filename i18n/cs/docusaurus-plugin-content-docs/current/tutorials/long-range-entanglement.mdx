...
* Znalost [dynamických obvodů](/guides/classical-feedforward-and-control-flow) (mid-circuit měření a klasická zpětná vazba);
...
Než spustíme na reálném QPU, ověříme, že dynamický i unitární obvod vytvoří ideální Bellův stav na bezhlučném simulátoru. Použijeme IBM Quantum `Sampler` s `AerSimulator` jako backendový režim, při malé vzdálenosti 6.
...
#### Inicializace Circuit {#initialize-circuit}
...
#### Příprava Bellových párů {#prepare-bell-pairs}
...
#### Měření sousedních párů Qubitů v Bellově bázi {#measure-neighboring-qubit-pairs-in-the-bell-basis}
...
#### Aplikace korekcí zpětnou vazbou pro opravu Pauliho vedlejších operátorů {#apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
#### Změř řídicí a cílový qubit {#measure-control-and-target-qubits}
...
Nyní sestav obvody, které měří v bázích $XX$, $YY$ a $ZZ$, stejně jako jsme to udělali pro dynamické obvody výše.
...
V bezhlučné simulaci ověříme, že metrika věrnosti je pro sestavené obvody $1$. V experimentech na reálných QPU tato analýza poskytne jasný přehled o tom, jak dobře dynamické Circuit fungují ve srovnání s unitární základní implementací.
...
Jak se očekávalo v bezhlučné simulaci, věrnosti u dynamických i unitárních obvodů jsou $1$.

## Příklad s reálným hardware ve velkém měřítku {#large-scale-hardware-example}
...
Nyní generujeme dlouhosáhlé CX obvody pro různé vzdálenosti oddělení qubitů až do 60 qubitů. Pro každou vzdálenost sestavíme obvody, které měří v bázích $XX$, $YY$ a $ZZ$, které budou později použity k výpočtu fidelity.
...
Nyní, když máme dynamické i unitární obvody pro řadu vzdáleností, jsme připraveni na transpilaci. Nejprve musíme vybrat backendové zařízení.
...
Následující krok zajišťuje, že backend podporuje instrukci `if_else`, která je vyžadována pro novější verzi dynamických obvodů. Protože je tato funkce stále v raném přístupu, explicitně přidáme `IfElseOp` do cíle backendu, pokud ještě není k dispozici.
...
- [Benchmarking dynamických obvodů pomocí rozdělených Bellových párů](/tutorials/edc-cut-bell-pair-benchmarking)
- [Simulace kicked Ising Hamiltoniánu pomocí dynamických obvodů](/tutorials/dc-hex-ising)
</Admonition>
