...
Bylo by skvělé experimentálně rozlišit mezi Einsteinovou a Bornovou možností. Jaké experimenty by přinesly stejné výsledky bez ohledu na to, která možnost je pravdivá? Napadá tě experiment, který by u obou možností přinesl odlišné výsledky?
...
### Otestuj své porozumění {#check-your-understanding-1}
...
Ve výše uvedeném postupu jsme uvažovali měření podél konkrétních os. Samozřejmě lze provádět měření podél libovolné osy. Označme dva spinové vektory dvou částic $\vec{a}$ a $\vec{b}$. Ať $\lambda$ je nějaká skrytá proměnná taková, že stav dvoučásticového systému odpovídá dobře definované hodnotě $lambda$. Ať $\rho(\lambda)$ je hustota pravděpodobnosti v $\lambda$. Nakonec zvolíme symboly $A(\vec{a},\lambda)$ a $B(\vec{b},\lambda)$ jako předem daný výsledek měření provedeného na kterékoli částici (A nebo B), dáno spinovými vektory a skrytou proměnnou. Kriticky si všimni, že $A$ je nezávislé na $\vec{b}$ a $B$ je nezávislé na $\vec{a}$. Nyní lze klást libovolný počet otázek týkajících se korelací mezi měřeními na A a B. Konkrétně lze klást otázku ohledně střední hodnoty dané výrazem
...
$$
Rihanna změří spin své částice podél jiného směru v úhlu $\theta$ relativně k Lucasovu. Spinový operátor podél libovolného směru $\hat{n}$ je dán vztahem
$$
...
(2) Zkontroluj známý případ. Dosaď $\theta = 0$. Pak $P_{\text{same}}$ odpovídá situaci, kdy oba pozorovatelé měří svůj spin podél stejné osy, což by porušilo zachování momentu hybnosti. Tedy by ses mohl očekávat, že tato pravděpodobnost je nulová, a skutečně dosazení $\theta = 0$ přinese $\sin^2(0/2) = 0$.
...
V souladu s naším výše uvedeným přístupem definujme směr Lucasova měření jako $+z$. To bylo výhodné i v algebraickém přístupu, ale je to obzvlášť výhodné pro kvantové výpočty, protože to, co se typicky měří, je projekce qubitu podél $z$.
...
To nám dává explicitní instrukce, jak sestrojit náš kvantový obvod. Na qubity reprezentující kvantové stavy částic měřených Lucasem a Rihannou aplikujeme hradla X, H, CNOT a $R_y$ a provedeme měření k získání pravděpodobnosti.
...
Pamatuj, že 1/3 času bude Rihannina zvolená osa $2\pi/3$ radiánů od Lucasovy, 1/3 času bude $4 \pi/3$ radiánů od Lucasovy a 1/3 času zvolí stejnou osu. Potřebujeme tedy sestavit 3 kvantové obvody pro tyto 3 případy a výsledky sečíst. První podrobně vysvětlíme a u posledních dvou je pouze uvedeme.
...
Nyní použijeme Qiskit referenční primitivum zvané StatevectorSampler. Sampler je primitivum navržené k vzorkování všech možných stavů systému a vrácení pravděpodobností (nebo v některých případech kvazipravděpodobností) získání každého stavu. Můžeme zadat počet „shots" a podívat se na „counts" pro každý stav.
...
### Ověř si porozumění {#check-your-understanding-6}
...
Navrhli jsme naše kvantové obvody tak, aby existovaly čtyři možné výsledky odpovídající tomu, že Lucas a Rihanna měří jedno nebo druhé znaménko projekce spinu: 00, 01, 10 a 11. V případech 00 a 11 měří Lucas a Rihanna stejné znaménko a v případech 01 a 10 měří opačná znaménka. Vidíme, že s velmi dobrou aproximací je šance, že Lucas a Rihanna dostanou stejné znaménko, přibližně 50 %, rozhodně větší než $4/9$. To znamená, že neexistuje žádná sada skrytých proměnných, která by mohla vysvětlit toto rozdělení pravděpodobností, a přístup skrytých proměnných není slučitelný s experimentem.
...
6. Které z následujících je dvouqubitové hradlo?
