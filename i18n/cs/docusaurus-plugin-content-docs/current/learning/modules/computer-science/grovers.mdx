...
Zde ukážeme, jak sestavit Groverova orákula a použít `GroverOperator` z knihovny Qiskit circuit library k snadnému nastavení instance Groverova vyhledávání. Primitiv IBM Quantum `Sampler` umožňuje bezproblémové spouštění Groverových obvodů.
...
### Příklad {#example}
Abychom lépe pochopili, jak Groverův algoritmus funguje, projděme si malý dvouqubitový příklad. Pro ty, kdo se nezaměřují na kvantovou mechaniku a Diracovu notaci, může být tento příklad volitelný. Ale pro ty, kdo doufají, že budou s kvantovými počítači pracovat důkladněji, je vysoce doporučený.

Zde je diagram obvodu s vyznačenými kvantovými stavy na různých místech v jeho průběhu. Všimni si, že u pouhých dvou qubitů existují jen čtyři možné stavy, které lze za jakýchkoli okolností naměřit: $|00\rangle$, $|01\rangle$, $|10\rangle$ a $|11\rangle$.
...
Předpokládejme, že orákulum ($Z_f$, nám neznámé) označuje stav $|01\rangle$. Projdeme si postupně účinky jednotlivých sad kvantových gates, včetně orákula, a podíváme se, jaké rozdělení možných stavů vznikne v okamžiku měření.
Na úplném začátku máme
$$
|\psi_0\rangle = |00\rangle
$$
Pomocí definice Hadamardových gates dostáváme
$$
...
$$
Všimni si, že v tomto stavu mají všechny čtyři možné výsledky stejnou pravděpodobnost naměření. Všechny mají váhu s velikostí $1/2$, což znamená, že každý má šanci $|1/2|^2=1/4$ na naměření. Takže i když je stav $|01\rangle$ označen prostřednictvím fáze „-“, zatím to nevedlo k žádnému zvýšení pravděpodobnosti naměření tohoto stavu. Pokračujeme aplikací další vrstvy Hadamardových gates.
$$
\begin{aligned}
...
\end{aligned}
$$
Sloučením podobných členů zjistíme
$$
|\psi_3\rangle = \frac{1}{2}\left(|00\rangle+|01\rangle-|10\rangle+|11\rangle\right)
$$
Nyní $Z_{\text{OR}}$ obrátí znaménko u všech stavů kromě $|00\rangle$:
$$
|\psi_4\rangle = \frac{1}{2}\left(|00\rangle-|01\rangle+|10\rangle-|11\rangle\right)
$$
A nakonec aplikujeme poslední vrstvu Hadamardových gates:
$$
\begin{aligned}
...
\end{aligned}
$$
Stojí za to projít si sloučení těchto členů a přesvědčit se, že výsledek je skutečně:
$$
|\psi_5\rangle =|01\rangle
$$
To znamená, že pravděpodobnost naměření $|01\rangle$ je 100 % (bez šumu a chyb) a pravděpodobnost naměření jakéhokoli jiného stavu je nulová.
...
- Krok 3: Spusť pomocí primitiv IBM Quantum
...
### Krok 3: Spuštění pomocí primitiv IBM Quantum {#step-3-execute-using-ibm-quantum-primitives}

Chceme provést mnoho měření a zjistit, který stav je nejpravděpodobnější. Taková amplitudová amplifikace je vzorkovací problém vhodný pro spuštění s primitivem IBM Quantum `Sampler`.

Všimni si, že metoda `run()` IBM Quantum SamplerV2 přijímá iterovatelnou sadu primitivních unifikovaných bloků (PUBs). Pro Sampler je každý PUB iterovatelný objekt ve formátu (circuit, parameter_values). Minimálně však přijímá seznam kvantových obvodů.
...
Ne. Předpokládejme, že bychom zopakovali první aktivitu s 20 qubity a spustili kvantový obvod počet opakování `num_shots = 10,000`. Rovnoměrné rozdělení pravděpodobnosti by znamenalo, že každý stav má pravděpodobnost $10,000/2^{20}=0.00954$, že bude naměřen alespoň jednou. Pokud by pravděpodobnost naměření cílového stavu byla 10krát vyšší než u neřešení (a pravděpodobnost každého neřešení by byla odpovídajícím způsobem mírně snížena), byla by jen asi 10% šance na naměření cílového stavu alespoň jednou. Bylo by velmi nepravděpodobné naměřit cílový stav vícekrát, což by ho učinilo nerozeznatelným od mnoha náhodně získaných stavů, které nejsou řešením. Dobrou zprávou je, že díky potlačení a zmírnění chyb můžeme dosáhnout výsledků s ještě vyšší věrností.
...
### Krok 1: Mapování klasických vstupů na kvantový problém {#step-1-map-classical-inputs-to-a-quantum-problem-1}
...
### Krok 2: Optimalizace problému pro spuštění na kvantovém hardwaru {#step-2-optimize-problem-for-quantum-hardware-execution-1}

Toto probíhá přesně stejně jako předtím.
...
### Krok 3: Spuštění pomocí primitiv IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-1}

Toto je také totožné s procesem z první aktivity.
...
### Krok 4: Následné zpracování a vrácení výsledku v požadovaném klasickém formátu {#step-4-post-process-and-return-result-in-desired-classical-format-1}

Nyní zobraz histogram svých výsledků měření. Jeden nebo více stavů by mělo mít mnohem vyšší pravděpodobnost naměření než ostatní. Nahlas je svému partnerovi/partnerce a zkontrolujte, zda jste správně určili cílové stavy. Ve výchozím nastavení je zobrazený histogram ze stejného obvodu jako v první aktivitě. Měl/a bys dostat jiné výsledky z obvodu svého partnera/partnerky.
...
### Krok 3: Spuštění pomocí primitiv IBM Quantum {#step-3-execute-using-ibm-quantum-primitives-2}
