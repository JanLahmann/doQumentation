...
(a) Normalizuj zkušební vlnovou funkci tak, že určíš $A$ splňující podmínku $$ \int_{-\infty}^{\infty} |\Psi_\text{trial}|^2 dx = 1 $$.

<Accordion>
<AccordionItem title="Answer">

Normalizace dané zkušební vlnové funkce:
...
(b) Vypočítej střední hodnotu hamiltoniánu $\hat{H}$ daného předpisem $$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V(x)$$ kde $V(x)  = \frac{1}{2}m\omega^2x^2$, což odpovídá potenciálu jednoduchého harmonického oscilátoru.

<Accordion>
<AccordionItem title="Answer">

Hamiltonián harmonického oscilátoru je:
...
Optimalizace $ \alpha $ pro minimální energii
...
#### (Kvantový) Ansatz: zkušební vlnová funkce (jak sestavit jednoduché kvantové stavy pomocí kvantového obvodu) {#quantum-ansatz-the-trial-wave-function-how-to-build-a-trivial-quantum-state-with-a-quantum-circuit}

Ve VQE se ansatz (množné číslo: ansätze) skládá ze dvou klíčových složek. První je příprava počátečního stavu, která nastaví stav qubitu aplikací kvantových hradel bez variačního parametru. Druhou složkou je parametrizovaný kvantový obvod — speciální kvantový obvod s nastavitelnými parametry, podobnými knoflíkům na rádiu. Tyto parametry budou použity v poslední části — klasickém optimalizátoru — aby nám pomohly dosáhnout nejlepšího možného základního stavu.

V části o variačním principu jsme se dozvěděli, že kvalita zkušebního stavu ovlivňuje kvalitu výsledků variačního algoritmu. To znamená, že volba dobrého ansatzu je ve VQE důležitá. Opět se jedná o bohaté a složité téma. Různé typy ansatzu ani jejich původ zde nebudeme rozebírat. Pokud tě zajímá více o parametrizovaných kvantových obvodech a ansatzu, můžeš prozkoumat lekci [Ansatz and variational form](/learning/courses/variational-algorithm-design/ansaetze-and-variational-forms) z kurzu [Variational algorithm design](/learning/courses/variational-algorithm-design/index), která poskytuje podrobná vysvětlení a příklady ansätze.

Protože v tomto modulu budeme používat jednoqubitový hamiltonián, potřebujeme jako ansatz jednoqubitový parametrizovaný kvantový obvod. V následující části uvidíme tři typy jednoqubitových ansätze. Porovnáme je a probereme klíčová hlediska při výběru ansatzu.

#### (Klasický) Optimalizátor: doladění kvantového obvodu {#classical-optimizer-fine-tuning-the-quantum-circuit}
...
Doplň prázdná místa správnými pojmy tak, aby byl souhrn procesu VQE úplný, a klikni pro zobrazení správných odpovědí.
...
- Krok 3: Provedení pomocí primitiv IBM Quantum
...
Začněme načtením některých nezbytných balíčků, včetně primitiv IBM Quantum. Také vybereme nejméně vytížený dostupný kvantový počítač.
...
Dále potřebujeme parametrizovaný kvantový obvod – ansatz – pro přípravu zkušební vlnové funkce $\Psi_\text{trial}$ pro základní stav. Cílem je najít parametry $\theta$, které minimalizují střední hodnotu energie $\langle\psi(\theta)|\hat{H}|\psi(\theta)\rangle$. Výběr ansatzu je zásadní, protože určuje množinu kvantových stavů, které náš obvod dokáže připravit. „Dobrý" ansatz je dostatečně flexibilní, aby mohl reprezentovat stav velmi blízký skutečnému základnímu stavu zkoumaného Hamiltoniánu, ale zároveň není tak složitý, že by vyžadoval příliš mnoho parametrů nebo příliš hluboký obvod pro současné kvantové počítače.

Zde vyzkoušíme tři různé jednoqubitové ansätze, abychom zjistili, který z nich poskytuje lepší „pokrytí" možných kvantových stavů, v nichž se může jeden qubit nacházet. „Pokrytím" se rozumí rozsah kvantových stavů, které obvod ansatzu dokáže vytvořit změnou svých parametrů.

Použijeme tři ansätze založené na různých kombinacích jednoqubitových rotačních hradel:
...
Teď, když jsme vybrali náš ansatz, pojďme vykreslit obvod.
...
Když provádíme výpočet na skutečném kvantovém počítači, nezajímá nás jen logika kvantového obvodu. Zajímá nás také, jaké operace daný konkrétní kvantový počítač umí provádět a kde na kvantovém počítači se nacházejí qubity, které používáme. Jsou hned vedle sebe? Jsou daleko od sebe? Dalším krokem je proto přepsat náš obvod pomocí gates, které jsou pro použitý kvantový počítač přirozené, a zohlednit přitom rozložení qubitů. To lze provést pomocí `transpilation` – po tomto procesu uvidíš, že náš jednoduchý ansatz byl převeden na jinou sadu gates a naše abstraktní qubity budou namapovány na fyzické qubity skutečného kvantového počítače.
...
Vidíš, že hradla `rx, rz` z našeho ansatzu byla převedena na sérii hradel `rz, sx`, což jsou nativní hradla našeho backendu. Také vidíš, že náš `q0` je nyní namapován na pátý fyzický qubit. Musíme také namapovat náš Hamiltonian podle těchto změn, jak ukazuje následující kód:
...
Poznámka: Lepšího výsledku lze dosáhnout nastavením úrovně potlačení chyb pomocí [`resilience_level`](/guides/error-mitigation-and-suppression-techniques). Výchozí hodnota je 1 a pokud nastavíš vyšší hodnotu, spotřebuje to více času QPU, ale může to vrátit lepší výsledek.
...
### Otestuj si své porozumění {#check-your-understanding-3}
...
### Krok 1: Mapování problému na kvantové obvody a operátory {#step-1-map-the-problem-to-quantum-circuits-and-operators-1}
...
### Krok 2: Optimalizace pro cílový hardware {#step-2-optimize-for-target-hardware-1}
...
### Krok 3: Spuštění na cílovém hardwaru {#step-3-execute-on-target-hardware-1}

Nyní je čas provést výpočty na skutečném QPU. Téměř všechno je stejné, ale použijeme vhodný počáteční bod pro fitování hamiltoniánu. Také v iterativní části budou některá nastavení `Estimator`, který se používá k výpočtu očekávaných hodnot hamiltoniánu pro ansatz na QPU, nastavena mírně odlišně od předchozích výpočtů. Touto změnou se budeme dále zabývat v kontrolní otázce.
...
### Krok 4: Post-processing {#step-4-post-process-1}
...
### Ověř si znalosti {#check-your-understanding-4}
...
Použij [Coulombův zákon](https://en.wikipedia.org/wiki/Coulomb%27s_law) a [atomové jednotky](https://en.wikipedia.org/wiki/Atomic_units), abys získal hodnotu v `Hartree`.
...
## Výpočet reakční energie $H + H = H_2$ {#compute-reaction-energy-of-h-h-h-2}
...
Dále jsme prozkoumali VQE, hojně využívaný algoritmus pro výpočet energie základního stavu kvantového systému. Spustili jsme kód pro výpočet energií základního stavu atomárního vodíku ($H$) a molekuly vodíku ($H_2$). Zejména jsme se naučili, že je nutné získat příslušný molekulární Hamiltonián pro daný systém a transformovat ho do podoby spustitelné na kvantovém počítači. Také jsme viděli, že ansatz – parametrizovaný kvantový obvod – je v rámci VQE potřeba k přípravě zkušebních kvantových stavů, a diskutovali jsme o důležitosti výběru vhodné struktury ansatzového obvodu. Dozvěděli jsme se také, že VQE spoléhá na iterativní optimalizační proces využívající klasický počítač, který řídí kvantový obvod k nalezení stavu s nejnižší energií, a viděli jsme, jak tento proces konverguje.
...
VQE je výkonný kvantový algoritmus pro blízkou budoucnost, je však důležité si uvědomovat jeho omezení. Výkon VQE silně závisí na volbě ansatzu – nalezení efektivně připravitelného ansatzu, který dokáže přesně reprezentovat skutečný základní stav, je pro větší a složitější molekuly stále náročnější. Současný kvantový hardware je navíc náchylný k šumu, což může ovlivnit přesnost výsledků VQE, zejména pro hlubší obvody nebo větší počty qubitů. Navzdory těmto výzvám VQE slouží jako základní algoritmus a probíhající výzkum zkoumá sofistikovanější variační metody a techniky zmírňování chyb s cílem posunout hranice toho, co je možné v kvantové chemii na kvantových počítačích blízké budoucnosti. Například se vyvíjejí algoritmy jako Sample-based Quantum Diagonalization (SQD), které využívají vzorky získané z kvantových obvodů v kombinaci s klasickou diagonalizací v podprostoru ke zlepšení odhadu energie a řešení některých omezení VQE, zejména pokud jde o efektivitu měření a robustnost vůči šumu.
...
- V VQE začínáme Hamiltonánem našeho systému a mapujeme ho na qubity pro spuštění na kvantovém počítači. Vybereme parametrizovaný kvantový obvod – ansatz – a provádíme opakovaná měření, přičemž měníme parametry ansatzu, dokud nedosáhneme nejnižší hodnoty energie. Prohledávání prostoru parametrů probíhá pomocí klasického optimalizátoru. K dosažení dobrých výsledků je nutné zvolit správný ansatz a vhodný optimalizátor.
...
4. Ve VQE se k přípravě zkušebních vlnových funkcí používá parametrizovaný kvantový obvod (ansatz).
...
- C) Optimalizovat kvantové obvody
