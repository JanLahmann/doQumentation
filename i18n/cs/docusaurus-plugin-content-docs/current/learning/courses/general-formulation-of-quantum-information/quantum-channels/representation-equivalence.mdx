...
sidebar_label: 'Ekvivalence reprezentací'
title: 'Ekvivalence reprezentací'
---
...
Abychom byli přesní, ignorujeme ostatní tvrzení — a zejména nemůžeme předpokládat, že $\Phi$ je kanál.
Vše, s čím musíme pracovat, je, že $\Phi$ je lineární zobrazení, jehož Choiho reprezentace splňuje $J(\Phi) \geq 0$ a
$\operatorname{Tr}_{\mathsf{Y}} (J(\Phi)) = \mathbb{I}_{\mathsf{X}}$.

Toto je ale vše, co potřebujeme, abychom usoudili, že $\Phi$ má Krausovu reprezentaci
...
Začneme kriticky důležitým předpokladem, že $J(\Phi)$ je pozitivně semidefinitní, což znamená, že je možné jej vyjádřit ve tvaru
...
pro nějaký způsob volby vektorů $\vert\psi_0\rangle,\ldots,\vert\psi_{N-1}\rangle$.
Obecně existuje vícero způsobů, jak to udělat — a to přímo odráží svobodu, kterou máme při volbě Krausovy reprezentace pro $\Phi$.

Jeden způsob, jak takový výraz získat, je nejprve použít spektrální větu k zapsání

$$
J(\Phi) = \sum_{k = 0}^{N-1} \lambda_k \vert \gamma_k \rangle \langle \gamma_k \vert,
$$

ve kterém $\lambda_0,\ldots,\lambda_{N-1}$ jsou vlastní hodnoty $J(\Phi)$ (které jsou nutně nezáporná reálná čísla, protože $J(\Phi)$ je pozitivně semidefinitní) a $\vert\gamma_0\rangle,\ldots,\vert\gamma_{N-1}\rangle$ jsou jednotkové vlastní vektory odpovídající vlastním hodnotám $\lambda_0,\ldots,\lambda_{N-1}$.

Všimni si, že zatímco při volbě vlastních hodnot není žádná svoboda (kromě toho, jak jsou uspořádány), existuje svoboda ve volbě vlastních vektorů, zejména když existují vlastní hodnoty s násobností větší než jedna.
Toto tedy není jedinečné vyjádření $J(\Phi)$ — jen předpokládáme, že máme jedno takové vyjádření.
Bez ohledu na to, protože vlastní hodnoty jsou nezáporná reálná čísla, mají nezáporné druhé odmocniny, a tak můžeme zvolit
...
Například pokud jsou $\mathsf{X}$ i $\mathsf{Y}$ oba qubity, a pro nějakou volbu $k$ máme
...
Tím je dokončen důkaz poslední implikace, a tedy jsme prokázali ekvivalenci čtyř tvrzení uvedených na začátku oddílu.
