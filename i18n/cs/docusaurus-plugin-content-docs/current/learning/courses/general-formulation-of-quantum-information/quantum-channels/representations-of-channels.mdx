...
S ohledem na konvenci uspořádání Qiskitu je maticová reprezentace tohoto hradlo následující.
...
Někdy je vhodné pojmenovat matice $A_0,\ldots,A_{N-1}$ jiným způsobem.
Například je můžeme číslovat počínaje $1$, nebo místo čísel použít jako dolní indexy stavy z nějaké libovolné klasické stavové množiny $\Gamma$:
...
Tyto různé způsoby pojmenování těchto matic, které se nazývají *Krausovy matice*, jsou všechny běžné a mohou být v různých situacích vhodné — ale my se v této lekci pro jednoduchost budeme držet názvů $A_0,\ldots,A_{N-1}$.

Číslo $N$ může být libovolné kladné celé číslo, ale nikdy nemusí být příliš velké:
pokud vstupní systém $\mathsf{X}$ má $n$ klasických stavů a výstupní systém $\mathsf{Y}$ má $m$ klasických stavů, pak jakýkoliv daný kanál z $\mathsf{X}$ do $\mathsf{Y}$ bude mít vždy Krausovu reprezentaci, pro kterou je $N$ nejvýše rovno součinu $nm$.

### Úplný defázovací kanál {#completely-dephasing-channel-1}
...
### Kanál pro reset qubitu {#qubit-reset-channel-1}
...
Pokud máme unitární matici $U$ reprezentující operaci na systému $\mathsf{X}$, můžeme vyjádřit působení této unitární operace jako kanál:

$$
\Phi(\rho) = U \rho U^{\dagger}.
$$

Tento výraz je již platnou Krausovou reprezentací kanálu $\Phi$, kde náhodou máme jen jednu Krausovu matici $A_0 = U$. V tomto případě je požadovaná podmínka
...
### Úplně defázující kanál {#completely-dephasing-channel-2}
...
### Úplně depolarizující kanál {#completely-depolarizing-channel-1}
...
### Kanál resetu qubit {#qubit-reset-channel-2}
...
Všimni si zejména, že $J(\operatorname{Id})$ není jednotková matice.
Choiho reprezentace přímo nepopisuje působení kanálu obvyklým způsobem, jakým matice reprezentuje lineární zobrazení.
