...
Dalším častým omylem je představa, že čím více qubitů má kvantový počítač, tím musí být výkonnější. Ačkoli je výpočetní rozsah důležitý, samotný počet qubitů neposkytuje úplný obraz výkonu kvantového počítání. Moderní kvantový hardware se obvykle hodnotí podle tří dimenzí: rozsahu, kvality a rychlosti.

Rozsah se měří počtem programovatelných qubitů dostupných uživatelům. Jsou to qubity, které lze přímo řídit, manipulovat s nimi, měřit je a zahrnout do kvantových algoritmů. Zvýšení počtu programovatelných qubitů rozšiřuje velikost problémů, které lze na kvantovém hardwaru zkoumat.

Kvalita se měří pomocí qubitových operací (QuOps), které popisují, kolik nejnáročnějších kvantových operací dokáže systém spolehlivě provést, než nahromaděné chyby výpočet znehodnotí. Zlepšení koherenčních dob, věrností hradel, kalibračních postupů a architektury přispívají k vyšší výpočetní kvalitě. Koherenční a defázovací doby (běžně označované jako $T_1$ a $T_2$) zůstávají důležitými ukazateli výkonu qubitů.

Rychlost se měří maximálním počtem obvodů za sekundu, také nazývaným propustnost obvodů. Tato metrika zachycuje, kolik užitečného výpočtu dokáže kvantový systém provést za čas, a odráží jak kvantové, tak podpůrné klasické zpracování.

Můžeš se také setkat s dalšími benchmarkovými metrikami, jako jsou CLOPS (Circuit Layer Operations Per Second) a EPLG (Errors Per Layered Gate). CLOPS měří rychlost provádění standardizované benchmarkové zátěže, zatímco EPLG poskytuje přehled o chybovosti vrstvy hradel a věrnosti procesoru. Tyto metriky zůstávají užitečné pro pochopení konkrétních vlastností hardwaru a pracovních zátěží, ale programovatelné qubity, qubitové operace a maximální počet obvodů za sekundu poskytují širší rámec pro porovnávání systémů.
...
Pokračující pokrok ve všech třech dimenzích výkonu, rozsahu, kvalitě a rychlosti, je nezbytný pro vybudování odolného, univerzálního kvantového počítače vůči chybám. Proto si při pohledu na plán IBM Quantum&reg; všimneš, že některé skoky mezi procesory nemají obrovský nárůst počtu programovatelných qubitů. Například si všimni skromného nárůstu počtu programovatelných qubitů mezi Heronem a Nighthawkem, protože to není skutečné zaměření tohoto zlepšení. Místo toho Nighthawk implementuje novou topologii propojení, která umožní různé kódy pro opravu chyb.
