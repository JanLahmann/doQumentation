...
To znamená, že QPU zahrnuje jeden nebo více kvantových čipů (např. Heron), více dalších komponent v odpařovací chladicí jednotce, jako jsou kvantové zesilovače, řídicí elektronika a klasický výpočetní výkon potřebný pro úlohy, jako je uchovávání instrukcí a vlnových tvarů v paměti, akumulace výsledků a budoucí dekódování korekce chyb. Zatímco odpařovací chladicí jednotka je nutná k provádění těchto úloh, vylučujeme odpařovací chladicí jednotku z této definice, abychom umožnili případ více QPU ve stejné chladničce.
...
V kvantovém počítání se schopnosti kvantového systému často popisují pomocí tří základních dimenzí: rozsahu, kvality a rychlosti. IBM Quantum charakterizuje tyto dimenze pomocí tří hardwarových metrik, které lze použít napříč modalitami kvantového počítání: programovatelné qubity (rozsah), operace s qubity (kvalita) a maximální počet obvodů za sekundu (rychlost). Tyto metriky poskytují souhrnný pohled na výpočetní schopnosti a efektivitu systému napříč různými hardwarovými implementacemi.

* **Rozsah** se měří počtem programovatelných qubitů. Programovatelné qubity jsou qubity, které uživatelé mohou přímo ovládat, manipulovat s nimi, měřit je a začlenit do kvantových algoritmů.


* **Kvalita** se měří pomocí operací s qubity (QuOps), metriky, která popisuje, kolik nejvýpočetně náročnějších operací dokáže systém spolehlivě provést, než nahromaděné chyby přemohou výpočet. Vyšší schopnost QuOps obecně umožňuje hlubší obvody a sofistikovanější kvantové algoritmy.


* **Rychlost** se měří maximálním počtem obvodů za sekundu, také známým jako propustnost obvodů. Tato metrika vyjadřuje, kolik užitečných kvantových obvodů dokáže systém provést za dobu a poskytuje indikaci celkové výpočetní efektivity.


Ke charakterizaci specifických aspektů chování procesoru se často používají další hardwarové metriky. Například EPLG (errors per layered gate) a měření vrstvové věrnosti poskytují náhled na kvalitu hradel a akumulaci chyb, zatímco CLOPS (Circuit Layer Operations Per Second) měří rychlost provádění standardizované úlohy kvantového benchmarkingu. Tyto metriky zůstávají užitečné pro vyhodnocování konkrétních úloh a implementačních detailů, ale programovatelné qubity, operace s qubity a maximální počet obvodů za sekundu poskytují širší rámec pro porovnávání kvantových systémů.

### Rozdíly mezi kvantovými a klasickými metrikami {#differences-between-quantum-and-classical-metrics}

Klasické počítačové systémy jsou často charakterizovány pomocí metrik, jako jsou FLOPS (operace s plovoucí desetinnou čárkou za sekundu), kapacita paměti, šířka pásma paměti a taktovací frekvence procesoru. Mezi těmito a metrikami pro kvantové počítače zavedenými výše lze vytvořit několik volných analogií, ale rozdíly v metrikách jsou důležité.

Maximální počet obvodů za sekundu lze považovat za volnou analogii k metrikám výkonu orientovaným na propustnost v klasickém počítání, protože měří, kolik užitečné práce dokáže kvantový systém provést za dobu. Na rozdíl od FLOPS však měří provádění kvantových obvodů, nikoli aritmetických operací.

CLOPS je někdy porovnáván s klasickými benchmarky propustnosti, jako je FLOPS, ale nejlépe se chápe jako benchmark specifický pro danou úlohu, nikoli jako univerzální míra výkonu kvantového počítání. EPLG a související měření věrnosti poskytují náhled na to, jak přesně procesor provádí vrstvy kvantových hradel. Tyto metriky jsou užitečné pro pochopení konkrétních vlastností hardwaru, zatímco primární rámec rozsah-kvalita-rychlost poskytuje obecnější základ pro porovnávání kvantových systémů. Mnoho popisů výkonu kvantového počítače lze najít na stránce [Compute resources](https://quantum.cloud.ibm.com/computers) na IBM Quantum Platform. Všimni si, že zatímco počet programovatelných qubitů je pro daný QPU pevné číslo, metriky rychlosti a kvality se mohou v čase mírně lišit kvůli aktualizacím kalibrace, údržbě, provozním podmínkám a vylepšením okolní hardwarové a softwarové infrastruktury.

Společně tyto doplňkové metriky pomáhají řídit alokaci zdrojů, plánování a návrh aplikací na systémech kvantového počítání.

V mnoha případech je celý kvantový systém považován za jedinou jednotku. Když však úloha přesáhne kapacitu jedné jednotky — ať už co do počtu programovatelných qubitů, hloubky obvodu, nebo propustnosti obvodu — lze použít techniky jako stříhání/spojování obvodů (circuit cutting/knitting). Stříhání obvodů je proces, kdy se velké kvantové úlohy rozkládají na menší, zvládnutelné dílčí úlohy, které lze rozdělit mezi více kvantových čipů, což umožňuje škálovatelné kvantové výpočty navzdory hardwarovým omezením. Spojování obvodů (circuit knitting) označuje proces, který následuje po stříhání obvodů — krok klasického následného zpracování, který „splétá" nebo kombinuje výsledky z menších dílčích obvodů zpět dohromady.
...
* Úlohy jsou odesílány uživatelem s využitím IBM Quantum Compute Service, typicky pomocí primitiva IBM Quantum, jako je `Sampler`, `Estimator` nebo jiné.
...
* Po odeslání úlohy prostřednictvím Quantum Compute úloha vstupuje do fronty pro vybranou QPU. Uživatel nemá kontrolu nad časem čekání ve frontě, ačkoli to může být u vyhrazených systémů odlišné.
...
(b) Zajištění toho, že nalezení cíle na jednom GPU zastaví proces na ostatních GPU, aby se uvolnily pro další problém

(c) Rozdělení prohledávaného prostoru na oblasti pro každé z tvých (řekněme 10) GPU k prohledávání
...
Vezmi na vědomí, že momentálně jsou všechny kvantové i klasické zdroje alokovány po celou dobu experimentu. V současné době neexistuje prokládaná alokace smíšených zdrojů. Poslední výhrada je, že i po spuštění úlohy nebude kvantový systém přímo řízen tak, jak by mohl očekávat zkušený uživatel HPC. Úloha se spustí na libovolném x86 uzlu, který vykonává IBM Quantum Compute Service, a tato služba se pak připojuje k dalšímu plánovači, nad nímž uživatel nemá přímou kontrolu. Tento pracovní postup a s ním spojené problémy mohou být známy uživatelům HPC, kteří měli ranou zkušenost se získáváním výhradního přístupu k GPU uzlům (původní použití `gres`).
...
(d) Ano, mohlo by. Používá kvantovou partition.
