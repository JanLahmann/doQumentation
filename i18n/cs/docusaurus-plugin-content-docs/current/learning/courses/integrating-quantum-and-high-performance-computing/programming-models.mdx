...
Začneme programovacím modelem pro kvantové počítače. Základním programovacím modelem, který je povědomý téměř všem kvantovým vývojářům, je kvantový obvod. Nebudeme zde zacházet do podrobností modelu kvantového obvodu, protože již máme skvělou [přednášku Johna Watrouse](/learning/courses/basics-of-quantum-information/quantum-circuits/introduction), která to vysvětluje podrobně. Zmíníme pouze, že obvod se skládá ze sady čar (nazývaných dráty), které představují qubity, hradla, která představují operace na kvantových stavech, a sady měření.
...
Poslední věcí, o které budeme v této části mluvit, je transpilace. Transpilace je proces přepisu daného vstupního obvodu tak, aby odpovídal fyzickým omezením a architektuře instrukční sady (ISA) konkrétního kvantového zařízení. Podobně jako klasické kompilátory to znamená převod abstraktních unitárních operací na nativní sadu hradel, kterou může cílové zařízení spustit. Také optimalizuje instrukce obvodu pro efektivní spuštění na zašuměných kvantových počítačích, přičemž rutina postupně mění strukturu obvodu použitím několika optimalizačních fází.
...
Kolik qubitů je v níže uvedeném obvodu?
![A circuit diagram with four horizontal lines and many gates.](/learning/images/courses/integrating-quantum-and-high-performance-computing/programming-models/checkin-circuit.avif)
...
Předpokládejme, že modeluješ elektrony v molekule. Chceš aproximovat (a) energii základního stavu molekuly a (b) které bázové výpočetní stavy jsou nejdominantnější v základním stavu molekuly. V každém případě, použil bys primitivum Estimator, nebo Sampler?
...
* Hlavní řídicí tok programu je určen pořadím (rankem) procesu. Toto je princip SPMD v akci: program používá podmíněnou logiku (například if (rank == 0)) k zajištění toho, že pouze určité, paralelizované části kódu jsou prováděny pracovními procesy, zatímco hlavní proces (často Rank 0) se stará o inicializaci a finální agregaci.
...
Poté provede lehkou optimalizaci nastavením `optimization_level=1` pro transpilaci kvantového obvodu a aplikaci rozvržení obvodu na pozorovatelnou veličinu, které pak uloží do datové složky.
...
```
Zde `--ntasks=4` vyžaduje od Slurmu čtyři klasické úlohy pro paralelní zpracování.

### Spuštění {#execution}

Toto je hlavní kvantová fáze, kde je optimalizovaný kvantový obvod z předchozího kroku spuštěn na QPU pomocí Estimatoru. K tomu nejprve přineseme tři soubory - transpilovaný kvantový obvod, pozorovatelnou veličinu a počáteční parametry - a poté je předáme `Estimator`. Ten vrátí odhadovanou hodnotu pozorovatelné veličiny a vypíše ji.

Skript `execution.sh` využívá plugin Slurmu pro přístup ke kvantovému prostředku.
...
A můžeme zkontrolovat frontu spouštění Slurmu.
...
V této lekci jsme si ukázali, jak kombinovat více klasických a kvantových programovacích modelů k sestavení, správě a spuštění kompletního čtyřfázového workflow. Začali jsme základními koncepty kvantových obvodů a primitiv, poté jsme prozkoumali klasické modely jako paralelní programování a task workflow. Kombinací všech konceptů jsme sestavili Qiskit pattern – mapování, optimalizace, spuštění a post-processing – orchestrovaný správcem zátěže Slurm s jednoduchým kvantovým obvodem a pozorovatelnou veličinou.
