...
Výsledky tohoto výstupu jsou rozebrány níže v sekci o post-processingu; prozatím si jen poznamenej, že simulace proběhla úspěšně. Nyní jsi připraven spustit výpočet na reálném hardwaru. Nastavíme resilienci na `1`, což znamená, že bude použita mitigace chyb TREX. Protože teď pracujeme s reálným hardwarem, použijeme IBM Quantum Compute Service a IBM Quantum primitiva. Všimni si, že jak smyčka for související s geometrií, tak i vícenásobné variační pokusy jsou uvnitř session.
