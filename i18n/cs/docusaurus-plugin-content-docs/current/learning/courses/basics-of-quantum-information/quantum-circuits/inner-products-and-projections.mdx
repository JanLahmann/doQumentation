...
Například měření ve standardní bázi jsou ekvivalentní projektivním měřením, kde $\Sigma$ je množina klasických stavů libovolného systému $\mathsf{X}$, o kterém mluvíme, a naše množina projekčních matic je
$\{\vert a\rangle\langle a\vert:a\in\Sigma\}$.

Dalším příkladem projektivního měření, tentokrát na dvou Qubit $(\mathsf{X},\mathsf{Y})$, je množina
$\{\Pi_0,\Pi_1\}$, kde
...
Pokud máme více systémů, které jsou společně v nějakém kvantovém stavu, a na jednom z nich je provedeno projektivní měření, akce je podobná tomu, co jsme měli u měření ve standardní bázi — a ve skutečnosti tuto akci nyní můžeme popsat mnohem jednodušeji, než jsme mohli dříve.

Přesněji řečeno, předpokládejme, že máme dva systémy $(\mathsf{X},\mathsf{Y})$ v kvantovém stavu $\vert\psi\rangle$ a na systému $\mathsf{X}$ je provedeno projektivní měření popsané kolekcí $\{\Pi_a : a\in\Sigma\}$, zatímco se se systémem $\mathsf{Y}$ nic neděje.
To je pak ekvivalentní provedení projektivního měření popsaného kolekcí
...
a s podmínkou, že se objeví výsledek $a$, se stav společného systému $(\mathsf{X},\mathsf{Y})$ stane
