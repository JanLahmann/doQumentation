...
Pro úlohu generování matice jádra nás zajímá zejména pravděpodobnost naměření stavu $|0\rangle^{\otimes N}$, ve kterém jsou všechny $N$ qubity ve stavu $|0\rangle$. Abychom to pochopili, uvažme, že obvod zodpovědný za zakódování a mapování jednoho datového vektoru $\vec{x}_i$ lze zapsat jako $\Phi(\vec{x}_i)$ a ten zodpovědný za zakódování a mapování $\vec{x}_j$ jako $\Phi(\vec{x}_j)$, a označme namapované stavy
$$
...
$$
Pokud na výchozí počáteční stav $|0\rangle^{\otimes N}$ aplikujeme oba obvody $\Phi^\dagger(\vec{x}_j)$ a $\Phi(\vec{x}_i)$, pravděpodobnost následného naměření stavu $|0\rangle^{\otimes N}$ je
$$
P_0 = |\langle0|^{\otimes N}\Phi^\dagger(\vec{x}_j)\Phi(\vec{x}_i)|0\rangle^{\otimes N}|^2.
$$
To je přesně hodnota, kterou hledáme (s výhradou $||^2$). Měřicí vrstva našeho obvodu vrátí pravděpodobnosti měření (nebo tzv. „kvazi-pravděpodobnosti", pokud jsou použity určité metody zmírnění chyb). Pravděpodobnost, o kterou nám jde, je pravděpodobnost nulového stavu $|0\rangle^{\otimes N}$.
