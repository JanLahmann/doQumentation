...
### Krok 3: Spusť pomocí primitiv IBM Quantum {#step-3-execute-using-ibm-quantum-primitives}
...
Výše uvedené dva unitáry přesně odpovídají $U_1$ a $U_2$ popsaným v úvodu. Můžeme je zkombinovat pomocí `unitary_overlap`. Jako vždy chceme mít přehled o hloubce našeho obvodu.
...
### Krok 2: Optimalizace problému pro kvantové spuštění {#step-2-optimize-problem-for-quantum-execution-1}
...
### Krok 4: Post-processing, vrácení výsledku v klasickém formátu {#step-4-post-process-return-result-in-classical-format-1}
...
Předpokládejme, že se někdo nový v kvantovém počítání pokusí použít kvantový obvod s velmi vysokou dvouqubitovou hloubkou a nepoužije zmírnění chyb. Dále předpokládejme, že to vede k chybovosti 10 % na každém qubitu. Pokud je skutečný (bezchybný) prvek jádrové matice odpovídající tomuto obvodu velmi velký, řekněme 1,0, jaká by byla pravděpodobnost naměření všech 10 qubitů ve stavu, kdy je každý qubit $|0\rangle$?
...
V tomto pracovním postupu QKE jsou naše odhady založeny na měřeních nulového stavu, tedy stavu, ve kterém je každý qubit nalezen ve stavu $|0\rangle$. Velmi hluboké obvody zavedou vysoké chybovosti. Když se tato chybovost složí přes mnoho qubitů, sníží to pravděpodobnost měření nulového stavu výrazně.
...
### Krok 1: Namapuj klasické vstupy na kvantový problém {#step-1-map-classical-inputs-to-a-quantum-problem-2}
...
### Krok 4: Post-processing, vrať výsledek v klasickém formátu {#step-4-post-process-return-result-in-classical-format-2}
...
### Krok 1: Přeměň klasické vstupy na kvantový problém {#step-1-map-classical-inputs-to-a-quantum-problem-3}
...
### Krok 2: Optimalizace problému pro kvantové spuštění {#step-2-optimize-problem-for-quantum-execution-2}
...
### Krok 4: Post-processing, vrácení výsledku v klasickém formátu {#step-4-post-process-return-result-in-classical-format-3}
