...
Nepravda. Většinu problémů strojového učení řeší velmi efektivně klasické algoritmy a kvantové algoritmy pravděpodobně nenabídnou žádné podstatné zrychlení. Cílem QML je najít datové sady s příznaky, které jsou dobře popsatelné kvantovými stavy, a/nebo najít mapování datových příznaků, které optimalizuje přesnost modelů.
...
5. Implementovat kvantové kernelové metody a variační kvantové klasifikátory pomocí primitiv IBM Quantum a podle Qiskit patterns.
...
* Krok 3: Vykonat pomocí primitiv IBM Quantum
...
V procesu výše jsme vypočítali prvek jádrové matice pro 14. a 19. datový bod. Jakou hodnotu bychom měli získat, pokud použijeme stejný datový bod dvakrát (například 14. a znovu 14.)? Jinými slovy, jaké by měly být diagonální prvky jádrové matice? Odpověz na tuto otázku bez přítomnosti šumu, avšak měj na paměti, že v přítomnosti šumu jsou odchylky od tvé odpovědi možné.

<Accordion>
<AccordionItem title="Odpověď">

Diagonální prvky by měly být 1,0. Tento proces by měl počítat normalizovaný skalární součin vektoru se sebou samým, který musí být vždy roven jedné.
