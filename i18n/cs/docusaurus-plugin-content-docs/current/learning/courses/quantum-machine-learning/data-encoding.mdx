...
s využitím amplitudového kódování.
<Accordion>
<AccordionItem title="Odpověď">
...
Pro stejný datový vektor $\vec{x}=(9,8,6,2,9,2),$ napiš kód pro vytvoření obvod, který načte tyto datové příznaky pomocí amplitudového kódování.
...
Alternativně lze úhlové kódování provést pomocí hradlo $R_X(\theta)$, zakódovaný stav by však měl komplexní relativní fázi oproti $R_Y(\theta)$.
...
Následující kód v Qiskit nejprve připraví počáteční stav jednoho qubitu jeho otočením Hadamardovým hradlo, a poté ho znovu otočí pomocí phase gate, aby zakódoval datový příznak $\vec{x}^{(j)}_k=\frac{1}{2}\pi$.
...
### Zkontroluj si porozumění {#check-your-understanding-3}
...
### Ověř si své znalosti {#check-your-understanding-4}
...
V této části použijeme a vizualizujeme několik vestavěných kódovacích schémat. Všechny metody v této části kódují $N$ příznaků jako rotace na $N$ parametrizovaných hradlo na $n$ qubitech, kde $n \leq N$. Všimni si, že maximalizace načítání dat pro daný počet qubitů není jediným kritériem. V mnoha případech může být hloubka obvod ještě důležitějším faktorem než počet qubitů.

### Efficient SU2 {#efficient-su2}

Běžným a užitečným příkladem kódování s provázáním je Qiskitův obvod [`efficient_su2`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.EfficientSU2). Pozoruhodné je, že tento obvod dokáže například zakódovat 8 příznaků na pouhé 2 qubity. Podívejme se na to a pak se pokusme pochopit, jak je to možné.
...
To je jen husté kódování, které jsme viděli dříve. Nyní po hradlo CNOT, na druhé bariéře ($b2$), je náš stav
$$
\begin{aligned}
...
Zde zakódujeme datový vektor $\vec{x}$ s 12 příznaky na 3-qubitový obvod `efficient_su2`, přičemž každý parametrizovaný hradlo použijeme k zakódování jiného příznaku.
...
Jak bylo uvedeno dříve, existuje kompromis: obvod s více qubity nebo s více opakováními vrstev provázání a rotací mohou ukládat více parametrů, ale dělají to při větší hloubce obvodu. K hloubkám některých vestavěných feature map se vrátíme níže.
...
Mapa příznaků $Z$ (ZFM) může být interpretována jako přirozené rozšíření fázového kódování. ZFM se skládá ze střídajících se vrstev jednoqubitových hradlo: vrstev Hadamardových hradlo a vrstev fázových hradlo. Nechť datový vektor $\vec{x}$ má $N$ příznaků. Kvantový obvod, který provádí mapování příznaků, je reprezentován jako unitární operátor působící na počáteční stav:

$$
\mathscr{U}_{\text{ZFM}}(\vec{x})|0\rangle^{\otimes N}=|\phi(\vec{x})\rangle
$$
kde $|0\rangle^{\otimes N}$ je $N$-qubitový základní stav. Tato notace se používá pro konzistenci s referencí [\[4\]](#references) Havlicek et al. Příznaky dat $x_i$ jsou mapovány v poměru 1:1 na odpovídající qubity. Například pokud máš v datovém vektoru 8 příznaků, použiješ 8 qubitů. ZFM obvod se skládá z $r$ opakování podobvodu tvořeného vrstvami Hadamardových hradlo a vrstvami fázových hradlo. Hadamardova vrstva se skládá z Hadamardova hradlo působícího na každý qubit v $n$-qubitovém registru, $H \otimes H \otimes \dots \otimes H = H^{\otimes n}$, ve stejné fázi algoritmu. Stejný popis platí i pro vrstvu fázových hradlo, ve které na $i^\text{th}$ qubit působí $P(\vec{x}_i)$. Každý hradlo $P$ má jako argument jeden příznak, ale vrstva fázových hradlo ($P(\vec{x}_1)\otimes\ldots P(\vec{x}_k)\otimes\ldots P(\vec{x}_N)$) je funkcí datového vektoru. Unitární operátor celého ZFM obvodu s jedním opakováním je:
$$
...
Mapa příznaků $ZZ$ (ZZFM) rozšiřuje ZFM o dvoukubitové entanglující hradlo, konkrétně o $ZZ$-rotační hradlo $R_{ZZ}(\theta)$. Předpokládá se, že ZZFM je obecně náročná na výpočet na klasickém počítači, na rozdíl od ZFM.

$R_{ZZ}(\theta)$ implementuje $ZZ$-interakci a je maximálně entanglující pro $\theta = \textstyle{\frac{1}{2}}\pi$. $R_{ZZ}(\theta)$ lze rozložit na sérii hradel na dvou qubitech, jak ukazuje následující kód v Qiskitu, který využívá [hradlo RZZ](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.RZZGate) a metodu třídy `QuantumCircuit` ```decompose```. Zakódujeme jeden příznak datového vektoru $\vec{x}$: $\vec{x}_k=\pi.$
...
Data jsou mapována pomocí fázové rotace $P(\theta) = e^{i\theta/2}R_Z(\theta)$ na druhém qubitu. hradlo $R_{ZZ}(\theta)$ entangluje dva qubity, na které působí, mírou entanglementu určenou hodnotou zakódovaného příznaku.
...
kde $U_{ZZ}(\vec{x})$ obsahuje vrstvu ZZ-hradlo strukturovanou schématem entanglementu. Několik schémat entanglementu je ukázáno v blocích kódu níže. Struktura $U_{ZZ}(\vec{x})$ také zahrnuje funkci, která kombinuje datové příznaky z qubitů entanglovaných následujícím způsobem. Řekněme, že hradlo $R_{ZZ}$ má být aplikována na qubity $p$ a $q$. Ve fázové vrstvě mají tyto qubity fázové hradlo, které na ně zakódují $\vec{x}_p$ a $\vec{x}_q$. Argument $\theta_{q,p}$ hradlo $R_{ZZ,q,p}(\theta_{q,p})$ nebude jednoduše jedním z těchto příznaků nebo druhým, ale funkcí, která se často označuje $\phi$ (nezaměňovat s azimutálním úhlem):
$$
...
Mapa příznaků Pauli (PFM) je zobecněním ZFM a ZZFM pro použití libovolných Pauli hradlo. Mapa příznaků Pauli má velmi podobnou formu jako předchozí dvě feature map. Pro $r$ opakování kódování $N$ příznaků vektoru $\vec{x},$
...
kde $\sigma_i$ je Pauliho operátor, $\sigma_i \in {I,X,Y,Z}$. Zde $\mathcal{I}$ je množina všech konektivit qubit, jak je určeno feature map, včetně množiny qubit, na které působí jednoqubitové hradlo. To znamená, že pro feature map, v níž na qubit 0 působil fázový hradlo a na qubit 2 a 3 působil hradlo $R_{ZZ}$, by množina $\mathcal{I}$ obsahovala $\{\{0\},\{2,3\}\}$. $S$ prochází všemi prvky této množiny. V předchozích feature map byla funkce $\phi_S(\vec{x})$ spojena buď výhradně s jednoqubitovými hradlo, nebo výhradně s dvouqubitovými hradlo. Zde ji definujeme obecně:
$$
\phi_S(\vec{x})=
    \begin{cases}
...
Jeden způsob, jak porozumět výše uvedené unitární operaci, je analogie s propagátorem ve fyzikálním systému. Výše uvedená unitární operace je operátorem unitárního vývoje, $\exp(it\mathcal{H})$, pro Hamiltonián $\mathcal{H}$ podobný Isingovu modelu, kde je časový parametr $t$ nahrazen datovými hodnotami, které řídí vývoj. Rozvoj tohoto unitárního operátoru dává obvod PFM. Provazující propojení v $S$ lze interpretovat jako Isingovy vazby ve spinové mřížce.
Uvažujme příklad operátorů Pauli $Y$ a $XX$ reprezentujících tyto interakce isingovského typu. Qiskit poskytuje třídu `pauli_feature_map` pro vytvoření instance PFM s výběrem jedno- a $n$-qubitových hradel, které v tomto příkladu budou předány jako Pauliho řetězce `‘Y’` a `‘XX’`. Typicky je $n$ rovno 1 nebo 2 pro jedno- a dvouqubitové interakce. Schéma provázání je „lineární“, což znamená, že jsou propojeny pouze nejbližší sousední qubity v kvantovém obvodu. Všimni si, že to neodpovídá nejbližším sousedním qubitům na samotném kvantovém počítači, jelikož tento kvantový obvod je abstrakční vrstvou.
...
Kód níže aplikuje tyto čtyři vestavěné feature mapy na kódování sady příznaků a vykresluje dvouqubitovou hloubku výsledného obvodu. Jelikož chybovost dvouqubitových hradel je mnohem vyšší než chybovost jednoqubitových hradel, může být rozumné zajímat se především o hloubku dvouqubitových hradel. V kódu níže získáme počty všech hradel v obvodu tak, že nejprve obvod dekomponujeme a poté použijeme count_ops(), jak je uvedeno níže. Zde jsou dvouqubitová hradla, o která máme zájem, hradla 'cx':
...
1. Moderní hardware: přítomnost šumu a vysoká režie kódů pro opravu chyb znamenají, že současné aplikace budou muset brát v úvahu faktory jako efektivita hardware a minimalizace hloubky dvou-qubitových hradlo.


2. Mapování vhodná pro daný problém: Jednou věcí je říci, že `zz_feature_map` je například obtížně klasicky simulovatelná, a tedy zajímavá. Úplně jiná věc je, aby `zz_feature_map` byla ideálně vhodná pro __tvůj__ úkol strojového učení nebo datovou sadu. Výkonnost různých parametrizovaných kvantových obvod na různých typech dat je aktivní oblastí výzkumu.
...
Hardwarově efektivní feature map je taková, která zohledňuje omezení skutečných kvantových počítačů s cílem snížit šum a chyby ve výpočtu. Při spouštění kvantových obvod na kvantových počítačích blízké budoucnosti existuje mnoho strategií, jak zmírnit šum inherentní hardwaru. Jednou z hlavních strategií pro hardwarovou efektivitu je minimalizace hloubky kvantového obvod tak, aby šum a dekoherence měly méně času na narušení výpočtu. Hloubka kvantového obvod je počet časově zarovnaných kroků hradlo potřebných k dokončení celého výpočtu (po optimalizaci obvod)[\[5\]](#references). Připomeň si, že hloubka abstraktního, logického obvod může být mnohem nižší než hloubka po transpilaci pro skutečný kvantový počítač.

Transpilace je proces převodu kvantového obvod z vysokoúrovňové abstrakce na podobu připravenou ke spuštění na skutečném kvantovém počítači, přičemž zohledňuje omezení hardwaru. Kvantový počítač má nativní sadu jednoqubitových a dvouqubitových hradlo. To znamená, že všechny hradlo v kódu Qiskit musí být transpilovány do sady nativních hardwarových hradlo. Například v ibm_torino, QPU s procesorem Heron r1 dokončeném v roce 2023, jsou nativní nebo základní hradlo `{CZ, ID, RZ, SX, X}`. Jedná se o dvouqubitový hradlo řízený-Z a jednoqubitové hradlo nazývané identita, $Z$-rotace, druhá odmocnina NOT a NOT, které dohromady tvoří univerzální sadu. Při implementaci vícequbitových hradlo jako ekvivalentního podobvodu jsou vyžadovány fyzické dvouqubitové hradlo $CZ$ spolu s dalšími jednoqubitovými hradlo dostupnými v hardwaru. Navíc, k provedení dvouqubitového hradlo na páru qubit, které nejsou fyzicky propojeny, se přidávají hradlo SWAP pro přesunutí stavů qubit mezi qubity za účelem umožnění propojení, což vede k nevyhnutelnému prodloužení obvod. Pomocí argumentu ```optimization```, který lze nastavit od 0 až po nejvyšší úroveň 3. Pro větší kontrolu a přizpůsobitelnost lze transpilační pipeline spravovat pomocí [Qiskit Pass Manager](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.PassManager). Další informace o transpilaci najdeš v [dokumentaci Qiskit Transpiler](https://docs.quantum.ibm.com/api/qiskit/transpiler).

V Havlicek et al. 2019 [\[2\]](#references) autoři dosahují hardwarové efektivity mimo jiné použitím feature map $ZZ$, protože jde o expanzi druhého řádu (viz výše oddíl „feature map $ZZ$"). Expanze $N$-tého řádu obsahuje $N$-qubitové hradlo. Kvantové počítače IBM&reg; nemají nativní $N$-qubitové hradlo, kde $N>2$, takže jejich implementace by vyžadovala rozklad na dvouqubitové hradlo CNOT dostupné v hardwaru. Druhým způsobem, jak autoři minimalizují hloubku, je volba topologie vazby $ZZ$, která se přímo mapuje na architekturu propojení. Dalším provedením optimalizace, které podnikají, je cílení na výkonnější, vhodně propojenou podstrukturu hardwaru. Dalšími věcmi, které je třeba zvážit, jsou minimalizace počtu opakování feature map a volba přizpůsobeného nízkoúrovňového nebo „lineárního" schématu propletení namísto „úplného" schématu, které proplétá všechny qubity.
