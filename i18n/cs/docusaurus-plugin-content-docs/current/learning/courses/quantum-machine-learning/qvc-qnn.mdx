...
V této lekci implementujeme několik variačních kvantových obvodů pro úlohu klasifikace dat – takzvaných variačních kvantových klasifikátorů (VQC). V určitém období bylo běžné označovat podmnožinu VQC jako kvantové neuronové sítě (QNN) podle analogie s klasickými neuronovými sítěmi. Existují skutečně případy, kdy struktury převzaté z klasických neuronových sítí, jako jsou konvoluční vrstvy, hrají ve VQC důležitou roli. V takových případech, kde je analogie silná, může být označení QNN užitečné. Parametrizované kvantové obvody však nemusí sledovat obecnou strukturu neuronové sítě – například ne všechna data musí být načtena v první (vstupní) vrstvě; můžeme načíst část dat v první vrstvě, aplikovat některá hradla a poté načíst další data (proces nazývaný „reuploading" dat). QNN bychom proto měli považovat za podmnožinu parametrizovaných kvantových obvodů a při hledání užitečných kvantových obvodů bychom se neměli omezovat analogií s klasickými neuronovými sítěmi.
...
* Načíst data z obrazu do kvantového obvodu
...
### Qiskit patterns krok 1: Mapování problému na kvantový obvod {#qiskit-patterns-step-1-map-the-problem-to-a-quantum-circuit}
...
$$
Zde $U(\vec{x})$ je kódovací obvod, pro který máme mnoho možností, jak jsme viděli v předchozích lekcích. $W(\theta)$ je variační, neboli trénovatelný blok obvodu, a $\theta$ je sada parametrů, které se mají trénovat. Tyto parametry budou měněny klasickými optimalizačními algoritmy, aby se našla sada parametrů, která přinese nejlepší klasifikaci obrázků kvantovým obvodem. Tento variační obvod se někdy nazývá „Ansatz". Nakonec $O$ je nějaká pozorovatelná veličina, která bude odhadována pomocí primitivu Estimator. Neexistuje žádné omezení, které by nutilo vrstvy být v tomto pořadí, ani aby byly zcela oddělené. Lze mít více variačních nebo kódovacích vrstev v libovolném pořadí, které je technicky motivováno.
...
Nyní, když máme připraveno kódování dat a variační obvod, můžeme je zkombinovat a vytvořit náš úplný Ansatz. V tomto případě jsou komponenty našeho kvantového obvodu dosti analogické těm v neuronových sítích: $U(\vec{x})$ je nejpodobnější vrstvě, která načítá vstupní hodnoty z obrázku, a $W(\theta)$ je jako vrstva proměnných „vah". Protože tato analogie v tomto případě platí, používáme „qnn" v některých našich jmenovacích konvencích; tato analogie by však neměla omezovat tvé zkoumání VQC.
...
* Měření v Pauli Z bázi je přímočaré bez dalšího režijního výkonu hradel.
...
Máme náš kvantový obvod a pozorovatelnou veličinu, kterou chceme odhadnout. Nyní potřebujeme několik věcí, abychom mohli tento obvod spustit a optimalizovat. Nejprve potřebujeme funkci pro provedení dopředného průchodu. Všimni si, že funkce níže přijímá ```input_params``` a ```weight_params``` odděleně. První je sada statických parametrů popisujících data v obrázku a druhá je sada proměnných parametrů, které mají být optimalizovány.
...
Začneme výběrem backendu pro spuštění. V tomto případě použijeme nejméně vytížený backend.
...
Zde optimalizujeme obvod pro spuštění na reálném backendu zadáním optimization_level a přidáním dynamického oddělování. Níže uvedený kód generuje správce průchodů (pass manager) pomocí přednastavených správců průchodů z qiskit.transpiler.
...
Nyní použijeme správce průchodů na obvod. Výsledné změny rozložení je nutné aplikovat také na observable. U velmi velkých obvodů nemusí heuristiky použité při optimalizaci obvodu vždy přinést nejlepší a nejplošší obvod. V takových případech má smysl spustit tyto správce průchodů několikrát a použít nejlepší obvod. Uvidíme to později, až budeme škálovat naše výpočty.
...
## Krok 3 Qiskit Patterns: Spuštění pomocí primitiv IBM Quantum {#qiskit-patterns-step-3-execute-using-ibm-quantum-primitives}
...
* Sestavili jsme špatný Ansatz? To může znamenat mnoho věcí. Při práci na skutečných kvantových počítačích bude hloubka obvodu zásadním faktorem. Důležitý může být také počet parametrů, stejně jako provázání (entanglement) mezi qubity.
...
Zde vidíme, že drtivá většina chybně klasifikovaných obrázků zobrazuje svislou čáru. Něco v našem modelu neumí zachytit informace o svislých čárách. Možná jsi to tušil/a už na základě prvního variačního obvodu. Pojďme se na něj podívat blíže.

## Vylepšení modelu {#improving-the-model}

### Krok 1 znovu {#step-1-revisited}

Při mapování našeho problému na kvantový obvod jsme měli výslovně přemýšlet o tom, jak informace v sousedních pixelech určuje třídu. Abychom identifikovali vodorovné čáry, chceme vědět „pokud je pixel $i$ žlutý, je pixel $i+1$ žlutý" pro všechny pixely napříč každým řádkem. Chceme také vědět o svislých čárách. Protože je ale klasifikace binární, lze si jednoduše představit, že pokud taková vodorovná čára *není* detekována, jde o čáru svislou. Náš předchozí variační obvod obsahoval hradla CNOT mezi qubity (a tedy pixely) 0 a 1, 1 a 2 a 2 a 3. To pokrývá vodorovné čáry v horní části obrázku, ale přímo nedetekuje svislé čáry a ani úplně nedetekuje vodorovné čáry, protože ignoruje spodní řádek. Pro úplnou detekci všech vodorovných čar bychom chtěli mít podobnou sadu hradel CNOT mezi qubity (pixely) 4 a 5, 5 a 6 a 6 a 7. Mohli bychom mít na paměti, že přidání hradel CNOT mezi qubity odpovídajícími svislým čarám (například 0 a 4 nebo 2 a 6) může být také užitečné. Nejprve ale zkontrolujeme, zda postačí detekovat, zda vodorovná čára *je*, nebo *není* přítomna.
...
Hloubku obvodu jsme nezvýšili. Podívejme se, zda jsme zvýšili jeho schopnost modelovat naše obrázky.

### Krok 2 znovu {#step-2-revisited}

Tento nový obvod budeme muset transpilovat pro spuštění na reálném kvantovém backendu. Tento krok prozatím přeskočíme, abychom zjistili, zda naše revize variačního obvodu měla požadovaný efekt na simulátorech. Transpilaci se podrobněji věnujeme v další podsekci.
...
$100\%$ přesnost na obou sadách! Naše domněnka, že pro správnou detekci stačí přesně rozpoznávat vodorovné čáry, se potvrdila! Navíc naše mapování potřebných informací o pixelech na hradla CNOT v kvantovém obvodu bylo efektivní. Podíváme se teď na to, jak tento proces škáluje při spuštění na skutečných kvantových počítačích.

## Škálování a spuštění na skutečných kvantových počítačích {#scaling-and-running-on-real-quantum-computers}

### Data {#data}

Začněme zvětšením velikosti našich obrázků. Na volbě mřížky 6×6 není nic zvláštního, jen přesahuje počet qubitů (32), které lze simulovat pro obvody s non-Cliffordovými hradly.
...
### Krok 1: Mapování problému na kvantový obvod {#step-1-map-the-problem-to-a-quantum-circuit}
...
Protože používáme ```z_feature_map```, který neobsahuje žádná CNOT hradla, přidání kódovací vrstvy nezvyšuje naši hloubku pro dvouqubitová hradla. Celý obvod si zde můžeme vizualizovat.
...
## Qiskit Patterns krok 2: Optimalizace problému pro kvantové spuštění {#qiskit-patterns-step-2-optimize-problem-for-quantum-execution-1}
Začneme výběrem backendu pro spuštění. V tomto případě použijeme nejméně vytížený backend.
...
Nyní použijeme pass manager několikrát. U velmi širokých nebo velmi hlubokých obvodů může docházet k velké variabilitě v transpilované hloubce dvou-qubitových operací. U takových obvodů je důležité vyzkoušet pass manager mnohokrát a použít nejlepší (nejplošší) výsledek.
...
Vidíme, že v tomto případě byla transpilovaná hloubka dvou-qubitových operací vždy 10. V hloubce jednoho qubitu došlo k menší variaci a použijeme nejplošší výsledek. Na tomto 36-qubitovém obvodu to však není zásadní zlepšení. Tento transpilovaný obvod si můžeme vizualizovat, ačkoli v tomto měřítku je stále obtížnější ho vizuálně analyzovat.
...
## Qiskit Patterns krok 3: Spuštění pomocí primitiv IBM Quantum {#qiskit-patterns-step-3-execute-using-ibm-quantum-primitives-1}
