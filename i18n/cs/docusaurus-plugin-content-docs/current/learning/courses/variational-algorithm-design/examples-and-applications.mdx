...
Problém maximálního řezu (max-cut) je kombinatorický optimalizační problém, který spočívá v rozdělení vrcholů grafu do dvou disjunktních množin tak, aby byl maximalizován počet hran mezi těmito dvěma množinami. Formálněji, mějme neorientovaný graf $G=(V,E)$, kde $V$ je množina vrcholů a $E$ je množina hran, problém max-cut žádá rozdělení vrcholů do dvou disjunktních podmnožin $S$ a $T$ tak, aby byl maximalizován počet hran, které mají jeden koncový bod v $S$ a druhý v $T$.
...
Tento problém lze vyjádřit jako binární optimalizační problém. Pro každý uzel $0 \leq i < n$, kde $n$ je počet uzlů grafu (v tomto případě $n=4$), uvažujeme binární proměnnou $x_i$. Tato proměnná bude mít hodnotu $1$, pokud je uzel $i$ v jedné ze skupin, kterou označíme jako $1$, a $0$, pokud je ve druhé skupině, kterou označíme jako $0$. Váhu hrany, která vede z uzlu $i$ do uzlu $j$, budeme také označovat jako $w_{ij}$ (prvek $(i,j)$ matice sousednosti $w$). Jelikož je graf neorientovaný, $w_{ij}=w_{ji}$. Náš problém pak můžeme formulovat jako maximalizaci následující nákladové funkce:
...
Abychom tento problém vyřešili pomocí kvantového počítače, vyjádříme nákladovou funkci jako očekávanou hodnotu pozorovatelné veličiny. Pozorovatelné veličiny, které Qiskit nativně podporuje, se však skládají z Pauliho operátorů, které mají vlastní čísla $1$ a $-1$ místo $0$ a $1$. Proto provedeme následující změnu proměnné:

Kde $\vec{x}=(x_0,x_1,\cdots ,x_{n-1})$. Matici sousednosti $w$ můžeme pohodlně použít pro přístup k vahám všech hran. To použijeme k získání naší nákladové funkce:
...
Nová nákladová funkce, kterou chceme maximalizovat, je tedy:
...
Přirozenou tendencí kvantového počítače je navíc hledat minima (obvykle nejnižší energii) místo maxim, takže namísto maximalizace $C(\vec{z})$ budeme minimalizovat:

$$
-C(\vec{z}) =  \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2} z_iz_j -  \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2}
$$

Nyní, když máme nákladovou funkci k minimalizaci, jejíž proměnné mohou mít hodnoty $-1$ a $1$, můžeme udělat následující analogii s Pauliho $Z$:
...
ke které následně budeme muset přidat nezávislý člen:

$$
\texttt{offset} = - \sum_{i=0}^n \sum_{j=0}^i \frac{w_{ij}}{2}
$$
Operátor je lineární kombinací členů s operátory Z na uzlech spojených hranou (připomeňme, že 0. qubit je nejvíce vpravo): $IIZZ + IZIZ + IZZI + ZIIZ + ZZII$. Jakmile je operátor sestrojen, lze snadno vytvořit ansatz pro algoritmus QAOA použitím obvodu `QAOAAnsatz` z knihovny obvodů Qiskit.
...
Pokračuj na poslední lekci, absolvuj hodnocení a získej svůj odznak!
