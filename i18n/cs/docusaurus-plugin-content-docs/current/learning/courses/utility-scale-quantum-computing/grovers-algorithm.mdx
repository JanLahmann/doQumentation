...
## 1. Úvod do Groverova algoritmu {#1-introduction-to-grovers-algorithm}
...
### Základní struktura Groverova algoritmu {#the-basic-structure-of-grovers-algorithm}
...
## 2. Implementace Groverova algoritmu {#2-implementing-grovers-algorithm}

### 2.1 Příprava {#21-preparation}
Importuj potřebné knihovny a nastav prostředí pro spuštění kvantového obvodu.
...
### Krok 1: Mapování problému na kvantové obvody a operátory {#step-1-map-problem-to-quantum-circuits-and-operators}
...
V kroku inicializace vytvoříme superpozici všech možných stavů. Toho docílíme aplikací Hadamardova hradla na každý qubit v n-qubitovém registru, což povede k rovnoměrné superpozici $2^n$ stavů. Matematicky to lze vyjádřit takto:
...
### 2.2 Příklad Groverova hledání se 2 qubity. {#22-a-2-qubit-grover-search-example}
...
### 2.3 Experiment se simulátory {#23-experiment-with-simulators}
...
Dostali jsme správnou odpověď $|01\rangle$. Všimni si, že je třeba dávat pozor na pořadí qubitů.
...
Transpilací byl obvod převeden na obvod využívající nativní bázové hradla daného zařízení.

### Krok 3: Spuštění obvodu. {#step-3-executing-the-circuit-1}
...
### Krok 4: Zpracování výsledků. {#step-4-post-processing-the-results-1}
