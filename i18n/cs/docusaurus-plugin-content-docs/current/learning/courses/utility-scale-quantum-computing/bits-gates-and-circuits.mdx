...
### 3.1 Quantum state and Bloch sphere {#31-quantum-state-and-bloch-sphere}
...
### 3.2 Single-qubit quantum state and unitary evolution {#32-single-qubit-quantum-state-and-unitary-evolution}
...
Význam a použití těchto hradlo jsou podrobněji popsány v kurzu [Základy kvantových informací](/learning/courses/basics-of-quantum-information).

### Exercise 1 {#exercise-1}

Použij Qiskit k vytvoření kvantových Circuit, které připraví níže popsané stavy. Poté spusť každý obvod pomocí simulátoru stavového vektoru a zobraz výsledný stav na Bloch sféře. Jako bonus zkus odhadnout, jaký by měl být konečný stav, na základě intuice o hradlo a rotacích v Bloch sféře.
...
Jsme nyní připraveni spustit náš obvod na Aer simulatoru. V tomto příkladu použijeme výchozí shots=1024, což znamená, že budeme měřit 1024krát. Poté zobrazíme tyto počty v histogramu.
...
### 4.3 Quantum entanglement and execution on a real quantum device {#43-quantum-entanglement-and-execution-on-a-real-quantum-device}
...
#### Krok 1. Namapování problému na obvod a operátory {#step-1-map-problem-to-quantum-circuits-and-operators}
...
Qiskit převádí abstraktní Circuit na QISA (Quantum Instruction Set Architecture) Circuit, které respektují omezení cílového hardwaru a optimalizuje výkon Circuit. Před optimalizací tedy určíme cílový hardware.
Pokud nemáš nainstalováno `qiskit-ibm-runtime`, budeš ho nejprve muset nainstalovat. Více informací najdeš v [referenci API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-service).
...
Provedení na skutečném zařízení může vyžadovat čekání ve frontě, jelikož kvantové počítače jsou cenné zdroje, po kterých je velká poptávka. job_id se používá k pozdější kontrole stavu provedení a výsledků úlohy.
