...
[Stáhnout PDF](https://ibm.ent.box.com/public/static/ws8x00xu0pksjzixyyspxzrilvt36iff.zip) originální přednášky. Vezmi prosím na vědomí, že některé úryvky kódu mohou být zastaralé, protože se jedná o statické obrázky.
...
Pro vytvoření náhodného kvantového stavu použijeme obecné unitární hradlo $U$ s parametry $\theta, \varphi$. (Další informace najdeš v dokumentaci [U-gate](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.UGate).)
...
1. Alice provázáže $|\psi \rangle$ se svou částí e-bitu pomocí hradla CNOT.
2. Alice aplikuje Hadamardovo hradlo na $|\psi \rangle$ a měří oba své qubity ve výpočetní bázi.
...
4. Bob provede *korekční* operátor na základě dvoubitové informace od Alice na své části e-bit páru.
    ...
    - Pokud „01", Bob aplikuje hradlo X.
    - Pokud „10", Bob aplikuje hradlo Z.
...
### 4.2 Kvantový obvod simulující kvantovou teleportaci {#4-2-quantum-circuit-simulating-the-quantum-teleportation}

Jako vždy použijeme framework Qiskit patterns. Tato podsekce se zaměří pouze na mapování.

### Krok 1: Namapuj problém na kvantové obvody a operátory {#step-1-map-problem-to-quantum-circuits-and-operators}

Abychom popsali výše uvedený scénář, potřebujeme obvod se třemi qubity: dva pro provázaný pár sdílený Alicí a Bobem a jeden pro neznámý kvantový stav $|\psi\rangle$.
...
Na začátku má Alice neznámý kvantový stav $|\psi \rangle.$ Vytvoříme ho pomocí hradla $U$.
...
Stav, který jsme vytvořili, si můžeme vizualizovat – ale jen proto, že víme, jaké parametry byly použity v hradle $U$. Kdyby tento stav vznikl z komplikovaného kvantového procesu, nebylo by možné stav zjistit bez toho, abychom proces spustili mnohokrát a sbírali statistiky jako při tomografii.
...
Dále Alice provázáže $|\psi \rangle$ se svou částí sdíleného e-bitu pomocí hradla $CX$ a hradla $H$ a měří je ve výpočetní bázi.
...
Dokončil/a jsi obvod kvantové teleportace! Podívejme se na výstupní stav tohoto obvodu pomocí simulátoru statevector.
...
Vidíš, že kvantový stav vytvořený hradlem $U$ qubitu 0 (qubit původně držící tajný stav) byl přenesen na qubit 2 (Bobův qubit).
...
Výše jsme vizuálně ověřili, že teleportovaný stav vypadá správně. Dalším způsobem, jak zkontrolovat, zda byl kvantový stav teleportován správně, je aplikovat inverzní hradlo $U$ na Bobův qubit a změřit '0'. Protože $U^{-1}U$ je identita, pokud je Bobův qubit ve stavu vytvořeném z $U|0\rangle,$ pak by aplikace inverze měla dát $U^{-1}U|0\rangle=|0\rangle.$
...
Nejprve spustíme obvod pomocí AerSimulator, než přejdeme na skutečný kvantový počítač.
...
Nyní provedeme teleportaci na skutečném kvantovém počítači. Pomocí funkce dynamického obvodu můžeme uprostřed obvodu pracovat s výsledky měření a v reálném čase implementovat podmíněné operace v teleportačním obvodu. Při řešení problémů se skutečnými kvantovými počítači budeme postupovat podle čtyř kroků vzorů Qiskit.

    1. Namapuj problém na kvantové obvody a operátory
    2. Optimalizuj pro cílový hardware
    3. Spusť na cílovém hardwaru
    4. Zpracuj výsledky
### Cvičení 3: Sestav teleportační obvod {#exercise-3-build-the-teleportation-circuit}

Zkus sestavit celý teleportační obvod od začátku a otestuj své porozumění. Pokud potřebuješ připomenout postup, posuň se zpět nahoru.
...
Připomínáme, že aplikace inverzního hradla $U$ slouží pouze k ověření očekávaného chování. Není součástí přenosu stavu k Bobovi a nepoužili bychom toto inverzní hradlo $U$, pokud by jediným cílem byl přenos kvantové informace.
...
Různá zařízení mohou mít různé mapy propojení a každé zařízení má qubity a spoje, které jsou výkonnější než ostatní. A konečně, různé kvantové počítače mohou mít různá *nativní hradla* (hradla, která hardware dokáže přímo spustit). Transpilace obvodu přepíše abstraktní kvantový obvod pomocí hradel, která cílový kvantový počítač dokáže spustit, a zvolí optimální mapování na fyzické qubity (mimo jiné). Transpilace je obsáhlé a složité téma. Více informací o transpilaci najdeš v [referenci API.](https://docs.quantum.ibm.com/api/qiskit/transpiler#overview)
...
### Krok 3: Spuštění obvodu. {#step-3-execute-the-circuit}

Pomocí primitiva Runtime `Sampler` spustíme cílový obvod.
...
Jak vidíme, v několika výsledcích jsme naměřili $|1 \rangle$. Tyto odchylky jsou způsobeny šumem a chybami. Dynamické obvody mají zejména vyšší míru chyb kvůli časově náročnému měření uprostřed obvodu.
### 4.5 Klíčové poznatky o kvantové teleportaci {#45-key-takeaways-on-quantum-teleportation}

Kvantový stav můžeme přenést vzdálenému příteli sdílením páru provázaných qubitů (e-bit).
...
1. Alice provede jednu z následujících operací na své části e-bitu.
    ...
    - Pokud $a_1a_2 = 01$, aplikuje hradlo Z
    - Pokud $a_1a_2 = 10$, aplikuje hradlo X
    - Pokud $a_1a_2 = 11$, aplikuje hradlo Z a hradlo X.
2. Alice pošle svou část e-bitu na Bobovo místo.
3. Bob aplikuje hradlo CNOT s qubitem od Alice jako řídicím a svým qubitem jako cílovým, poté aplikuje hradlo H na qubit od Alice a změří oba qubity. Možné počáteční stavy a výsledky Bobových operací jsou:
$$
...
### 5.2 Kvantový obvod simulující superdense kódování {#52-quantum-circuit-simulating-the-superdense-coding}

Na základě protokolu superdense kódování můžeš sestavit obvod superdense kódování níže. Zkus změnit zprávu, `msg`, kterou chce Alice přenést Bobovi.
...
Výsledek je takový, jaký jsme očekávali. Všimni si, že superdense kódování na skutečném kvantovém počítači vykazovalo méně chyb než kvantová teleportace na skutečném kvantovém počítači. Jedním z důvodů může být, že kvantová teleportace využívá dynamické obvody, zatímco superdense kódování ne. O chybách v kvantových obvodech se dozvíme více v dalších lekcích.
## 6. Shrnutí {#6-summary}
