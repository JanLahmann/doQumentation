...
### 2.1 Definování transverzálního Ising Hamiltoniánu {#21-defining-the-transverse-field-ising-hamiltonian}
...
### 2.2 Nastavení parametrů simulace časového vývoje {#22-set-the-parameters-of-the-time-evolution-simulation}
...
### 2.3 Příprava kvantového obvodu 1 (Počáteční stav) {#23-prepare-the-quantum-circuit-1-initial-state}
...
### 2.4 Příprava kvantového obvodu 2 (Jeden obvod pro časový vývoj) {#24-prepare-the-quantum-circuit-2-single-circuit-for-time-evolution}
...
### 2.5 Nastavení operátorů k měření {#25-set-the-operators-to-measure}
...
### 2.6 Provedení simulace časového vývoje {#26-perform-time-evolution-simulation}
...
### 2.7 Vykreslení časového vývoje pozorovatelných veličin {#27-plot-the-time-evolution-of-the-observables}
...
## 3. Cvičení 1. Proveď simulaci pomocí Suzuki–Trotterovy metody druhého řádu {#3-exercise-1-perform-simulation-using-second-order-suzukitrotter}
...
### 3.1 Sestav obvod pro jeden časový krok {#31-construct-a-circuit-for-a-single-time-step}
...
### 3.2 Proveď simulaci časového vývoje {#32-perform-time-evolution-simulation}
...
### 3.3 Vykresli výsledky Suzuki–Trotterovy metody druhého řádu {#33-plot-the-second-order-suzukitrotter-results}
...
### 3.4 Porovnání s přesnými výsledky {#34-compare-with-exact-results}
...
### 4.3 Krok 3. Spuštění pomocí primitiv IBM Quantum {#43-step-3-execute-with-ibm-quantum-primitives}
...
Použij product_formula_st4 (SuzukiTrotter(order=4)) a sestav obvod pro jeden časový krok pomocí Suzukiho-Trotterova rozkladu čtvrtého řádu. Také spočítej počet hradel a hloubku obvodu a porovnej s Lieho-Trotterovým a Suzukiho-Trotterovým rozkladem druhého řádu.
