...
description: "Tento kurz se zabývá kvantovou simulací, včetně trotterizace pro transverzální Isingův Hamiltonián"
...
Trotterizace je technika evoluce v reálném čase, která spočívá v postupné aplikaci kvantového hradla nebo hradel, zvolených tak, aby aproximovaly časovou evoluci systému pro daný časový úsek. Z rovnice Schrödingera vyplývá, že časová evoluce systému, který se na začátku nachází ve stavu $\vert\psi(0)\rangle$, má tvar:
...
Některé velmi jednoduché exponenciály, jako $e^{iZ}$, lze na kvantových počítačích snadno implementovat pomocí kompaktní sady kvantových hradel. Většina zajímavých Hamiltonianů nebude mít jen jeden člen, ale bude jich mít mnoho. Všimni si, co se stane, když $H = H_1+H_2$:
...
Pokud však operátory nekomutují, nelze členy v Taylorově rozvoji přeuspořádat tak, aby se zjednodušily. Vyjádření složitých Hamiltonianů pomocí kvantových hradel je tedy výzva.
...
Zde $t/r$ je časový úsek (evoluční krok), který volíme. Výsledkem je hradlo, které se aplikuje $r$-krát. Menší časový krok vede k přesnější aproximaci. To však zároveň vede k hlubším obvodům, které v praxi způsobují větší hromadění chyb (nezanedbatelný problém u kvantových zařízení blízké budoucnosti).
...
Tento Hamiltonian je užitečný tím, že nám umožňuje snadno studovat účinky vnějšího pole. V výpočetní bázi bude systém zakódován takto:
...
### 2.3 Příprava kvantového obvodu 1 (Počáteční stav) {#2-3-prepare-the-quantum-circuit-1-initial-state}
...
### 2.4 Příprava kvantového obvodu 2 (Jeden obvod pro časový vývoj) {#2-4-prepare-the-quantum-circuit-2-single-circuit-for-time-evolution}
Zde konstruujeme obvod pro jeden časový krok pomocí Lie–Trotter.
...
Jak bylo zmíněno dříve, velmi hluboké obvody vedou k hromadění chyb a způsobují problémy moderním kvantovým počítačům. Protože dvouqubitová hradla mají vyšší míru chyb než jednoqubitová hradla, veličinou zvláštního zájmu je hloubka dvouqubitového obvodu. Na čem skutečně záleží, je hloubka dvouqubitového obvodu po transpilaci (protože to je obvod, který kvantový počítač skutečně vykonává). Ale pojďme si zvyknout počítat operace pro tento obvod, i teď při používání simulátoru.
...
$$
### 3.1 Sestav obvod pro jeden časový krok {#3-1-construct-a-circuit-for-a-single-time-step}
Použij product_formula_st2 (SuzukiTrotter(order=2)) a sestav obvod pro jeden časový krok pomocí Suzuki–Trotterovy metody druhého řádu. Také spočítej počet hradel a hloubku obvodu a porovnej s Lie–Trotterovou metodou.
...
#### Konstrukce obvodu pro experiment na hardwaru {#construct-circuit-for-hw-experiment}
...
Pak transpiluj obvod pro vybraný backend.
...
Primitiva `Sampler` (V2) v Qisku vrací počty naměřených bitových řetězců.
...
Zde ukazujeme příklad konstrukce obvodu s použitím Suzukiho–Trotterovy metody vyššího řádu (čtvrtého řádu).
Zkus nyní sestavit simulaci obvodu s Suzukiho–Trotterovou metodou čtvrtého řádu podle výše uvedených příkladů.
...
$$
#### Konstrukce obvodu pro jeden časový krok {#construct-a-circuit-for-a-single-time-step}
Použij `product_formula_st4` (`SuzukiTrotter(order=4)`) a sestav obvod pro jeden časový krok metodou Suzukiho–Trotterovy metody čtvrtého řádu. Spočítej také počet hradel a hloubku obvodu a porovnej je s metodami Lie–Trotter a Suzuki–Trotter druhého řádu.
