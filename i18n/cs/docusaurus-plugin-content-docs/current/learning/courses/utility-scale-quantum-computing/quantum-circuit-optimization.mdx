---
title: "Optimalizace kvantových obvodů"
sidebar_label: "Optimalizace kvantových obvodů"
description: "Tato lekce se věnuje několika aspektům optimalizace obvodů v kvantových výpočtech."
...
## 2. Na optimalizaci obvodů záleží {#circuit-optimization-matters}

Nejprve porovnáme výsledky spuštění přípravných obvodů pro 5-qubitový stav GHZ ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$) s optimalizací a bez ní.
...
Obvod transpilujeme bez optimalizace (`optimization_level=0`) a s optimalizací (`optimization_level=2`).
Uvidíme velký rozdíl v délce transpilovaných obvodů.
...
## 3. Na syntéze obvodů záleží {#circuit-synthesis-matters}
Dále porovnáme výsledky spuštění dvou různě syntetizovaných obvodů pro přípravu 5-qubitového GHZ stavu ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$).
...
Obecně platí, že syntéza obvodů závisí na konkrétní aplikaci a je příliš obtížné, aby software pokryl všechny možné aplikace. Qiskit transpiler nemá funkce pro syntézu obvodů pro přípravu GHZ stavu. V takovém případě stojí za zvážení ruční syntéza obvodu, jak je ukázáno výše.
V této části se podrobněji podíváme na to, jak Qiskit transpiler funguje, na následujícím jednoduchém příkladovém obvodu.
...
### 4.2 Na optimalizaci obvodů záleží {#circuit-optimization-matters-4-2}
...
Obvod transpilujeme bez optimalizace (`optimization_level=0`) a s optimalizací (`optimization_level=2`).
Jak vidíš, délka transpilovaných obvodů se výrazně liší.
...
### 4.3 Syntéza obvodů má vliv {#circuit-synthesis-matters-4-3}
Nyní porovnáme výsledky spuštění dvou různě syntetizovaných obvodů pro přípravu 5-qubitového stavu GHZ ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$).
...
### 4.4 Obecná dekompozice 1-qubitového hradla {#general-1-qubit-gate-decomposition}
...
### 4.7 Dekompozice hradla CU {#cu-gate-decomposition}
...
Všimni si, že $H$(Hadamard), $S$($\pi/2$ Z-rotace), $S^\dagger$($-\pi/2$ Z-rotace) a $X$(Pauliho X) jsou všechna Cliffordova hradla.
