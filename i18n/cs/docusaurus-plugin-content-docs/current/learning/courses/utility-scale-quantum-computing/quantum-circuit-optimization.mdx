...
## 2. Na optimalizaci obvodů záleží {#2-circuit-optimization-matters}
...
### 2.1 Úroveň optimalizace {#21-optimization-level}
...
## 3. Na syntéze obvodů záleží {#3-circuit-synthesis-matters}
...
### 3.1 Zobrazení celého procesu transpilace Qiskit {#31-draw-the-entire-qiskit-transpilation-flow}
...
### 3.2 Zobrazení jednotlivé fáze {#32-draw-an-individual-stage}
...
### 3.4 Fáze optimalizace {#34-optimization-stage}
...
## 4. Podrobné příklady {#4-in-depth-examples}

### 4.1 Optimalizace dvouqubitových bloků pomocí syntézy dvouqubitových unitárních matic {#41-two-qubit-block-optimization-using-two-qubit-unitary-synthesis}
...
### 4.2 Na optimalizaci obvodů záleží {#42-circuit-optimization-matters}
...
### 4.3 Syntéza obvodů má vliv {#43-circuit-synthesis-matters}
Dále porovnáme výsledky spuštění dvou různě syntetizovaných obvodů pro přípravu 5-qubitového GHZ stavu ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$).
...
### 4.4 Obecná dekompozice 1-qubitového hradla {#44-general-1-qubit-gate-decomposition}
...
### 4.5 Optimalizace 1-qubitového bloku {#45-one-qubit-block-optimization}
...
### 4.6 Toffoli dekompozice {#46-toffoli-decomposition}
...
### 4.7 Dekompozice hradla CU {#47-cu-gate-decomposition}
...
### 4.8 CX, ECR, CZ jsou si rovny až na lokální Cliffordovy operace {#48-cx-ecr-cz-equal-up-to-local-cliffords}
