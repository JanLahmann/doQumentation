...
description: "V této lekci spustíme obvod v měřítku užitečnosti, který se objevuje v publikaci, již neformálně nazýváme článek o užitečnosti, zveřejněné v Nature Vol 618, 15. června 2023."
...
V této lekci spustíme obvod v měřítku užitečnosti, který se objevuje v publikaci, již neformálně nazýváme [„článek o užitečnosti“](https://www.nature.com/articles/s41586-023-06096-3), zveřejněné v Nature Vol 618, 15. června 2023. Článek se zabývá časovým vývojem 2D Isingova modelu s příčným polem. Konkrétně se zaměřuje na časovou dynamiku Hamiltoniánu,
$$
...
jsou rotační hradla $ZZ$ a $X$.

Experimenty prováděli na procesoru IBM Quantum&reg; Eagle, což je 127qubitové zařízení s heavy-hex konektivitou, přičemž aplikovali interakce $X$ na všechny qubity a interakce $ZZ$ na všechny hrany mapy vazeb. Všimni si, že všechny interakce $ZZ$ nelze aplikovat současně kvůli „datové závislosti“. Proto **obarví mapu vazeb**, aby je seskupili do vrstev. Ty ve stejné vrstvě dostanou stejnou barvu a lze je aplikovat paralelně.
...
### 1.1 Tvůj cíl {#1-1-your-goal}

Tvým cílem v této lekci je sestavit obvod v měřítku užitečnosti a spustit jej na procesoru Eagle. Získávání spolehlivých výsledků přesahuje rámec tohoto notebooku, částečně proto, že PEA je v současnosti experimentální funkcí Qiskitu, a částečně proto, že aplikace ZNE s PEA zabere poměrně dost času.

Konkrétně máš za úkol sestavit a spustit obvod odpovídající obrázku 4b v článku a vykreslit své vlastní „nemitigované“ body. Jak vidíš, jedná se o obvod s 127 qubity $\times$ 60 vrstev (20 Trotterových kroků) s pozorovatelnou veličinou $\langle Z_{62} \rangle$.
![image.png](/learning/images/courses/utility-scale-quantum-computing/utility-i/utility1.avif)
Zní to jako velká věc? &nbsp; Žádný strach. Poslední tři lekce tohoto kurzu ti poskytnou odrazové můstky. Pro začátek si předvedeme experiment menšího rozsahu, ve kterém sestavíme a spustíme na falešném zařízení obvod s 27 qubity $\times$ 6 vrstev (2 Trotterovy kroky) s pozorovatelnou veličinou $\langle Z_{13} \rangle$.
...
Nejprve si všimni, že RZZ hradlo obecně vyžaduje dvě hradla $CX$.
...
Jak je uvedeno výše, v tomto experimentu se zaměřujeme na RZZ hradlo s konkrétním úhlem -$\pi$ / 2. Jak ukazuje článek, lze jej realizovat pouze s jedním hradlem $CX$.
...
Pro další použití definujeme hradlo na základě tohoto obvodu.
...
![Output of the previous code cell](/learning/images/courses/utility-scale-quantum-computing/utility-i/extracted-outputs/6753b9a4-90b1-4981-82b2-8177ac258af2-0.avif)
 ...
 Než budeme pokračovat, ověřme logickou ekvivalenci mezi `qc1` (RZZ hradlo) pro -pi/2 a naším nově definovaným `rzz`, tedy `qc2` hradlem:
...
Pojďme definovat rutinu pro sestavení obvodu z článku o užitečnosti pro časovou evoluci 2D Isingova modelu. Rutina přijímá tři parametry: backend, celé číslo udávající počet Trotterových kroků a booleovskou hodnotu řídící vkládání bariér.
...
Všimni si, že jsme pro sestavený obvod již ručně provedli mapování a routing qubitů. Proto, když obvod později transpilujeme, nepožadujeme (__neměli bychom požadovat__) po transpileru, aby prováděl mapování a routing qubitů. Jak za chvíli uvidíš, voláme ho s úrovní optimalizace 1 a metodou layoutu „trivial“.
...
Vyzkoušejme tyto rutiny. Měl bys vidět obvod o 27 qubitech $\times$ 15 vrstev (5 Trotterových kroků). Protože falešné zařízení má 28 hran, mělo by být 28*5 provazujících hradel.
...
Nyní předvedeme menší verzi experimentu v měřítku užitečnosti. Sestavíme obvod o 27 qubitech $\times$ 6 vrstev (2 Trotterovy kroky) s $\langle Z_{13} \rangle$ jako pozorovatelná veličina a spustíme jej jak na AerSimulatoru, tak na falešném zařízení.
...
Když stejný uživatel spustil následující buňku ve stejném prostředí jako výše, trvalo to 2 min 19 s reálného času. Spuštění obvodu na falešném zařízení vyvolá zašuměnou simulaci, která trvá mnohem déle než přesná simulace. Doporučujeme nespouštět na falešném zařízení větší obvod (například 27 qubitů $\times$ 9 vrstev se 3 Trotterovými kroky).
...
Tvým cílem je spustit experiment v měřítku užitečnosti, jak bylo zmíněno na začátku. Vytvoříš a provedeš obvod se 127 qubity a 60 vrstvami (20 Trotterových kroků) s pozorovatelnou veličinou  $\langle Z_{62} \rangle$. Doporučujeme ti zkusit to udělat sám s využitím kódu pro 27qubitovou verzi, kde to dává smysl. Řešení je však uvedeno zde.
...
Uvádíme hodnoty pro „mitigované“ body na obrázku 4b v článku o užitečnosti. Vynes je společně se svými výsledky.
...
Jsou tvé výsledky podobné „nemitigovaným“ hodnotám na obrázku 4b? &nbsp; Mohou se velmi lišit v závislosti na zařízení a jeho stavu v době experimentu. Neznepokojuj se samotnými výsledky. Co budeme kontrolovat, je, zda jsi kódování provedl správně. Pokud ano, gratulujeme, dosáhl jsi startovní čáry éry užitečnosti.

Stejně jako v článku o užitečnosti vědci z celého světa vynaložili ohromnou vynalézavost k získání smysluplných výsledků i v přítomnosti šumu. Konečným cílem tohoto společného úsilí je kvantová výhoda: stav, v němž kvantové počítače dokážou řešit některé problémy využitelné v průmyslu rychleji, s vyšší věrností nebo levněji než klasické počítače. Nepůjde pravděpodobně o jednu událost, ale spíše o éru, v níž klasická reprodukce kvantových výsledků bude trvat stále déle, až se v určitém okamžiku tento kvantový náskok stane kriticky důležitým. Jedno je o kvantové výhodě jasné: dostaneme se k ní pouze prostřednictvím experimentů v měřítku užitečnosti. Pokud tento kurz povede k tvému zapojení do této výpravy, která je plná výzev a zábavy, budeme více než rádi.
## Reference {#reference}
