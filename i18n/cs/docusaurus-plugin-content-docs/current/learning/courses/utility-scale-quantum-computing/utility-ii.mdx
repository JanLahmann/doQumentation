...
### 2.2 Aktivita 2 {#22-activity-2}

#### Definuj Hamiltonian {#define-the-hamiltonian-1}
...
### 2.4 Aktivita 4 {#24-activity-4}

#### Připrav kvantový obvod 2 (jednoduchý obvod pro časový vývoj) pro problém se 70 qubity {#prepare-the-quantum-circuit-2-single-circuit-for-time-evolution-for-the-70-qubit-problem}
Zde konstruujeme obvod pro jediný časový krok pomocí Lie–Trotterovy metody.
...
Zkus to sám/sama a vycházej z příkladu pro 20 qubitů. Stejně jako předtím spočítej operace pro tento obvod.
...
### 2.5 Aktivita 5 {#25-activity-5}

#### Nastavení operátorů k měření {#set-the-operators-to-be-measured-1}
...
### 2.6 Aktivita 6 {#26-activity-6}

#### Proveď simulaci časového vývoje {#perform-time-evolution-simulation-1}
...
Stejně jako ve všech předchozích lekcích budeme implementovat framework Qiskit patterns. Lekce až do tohoto bodu se soustředila na vytváření správných kvantových obvodů pro popis našeho problému. To efektivně odpovídá Kroku 1.

#### Krok 2: Optimalizace pro cílový hardware {#step-2-optimize-for-target-hardware}

Začneme definováním cílového backendu.
...
Transpilujeme obvody a shromáždíme je do seznamu. Může to trvat několik minut.
...
Toto použijeme pro srovnání níže. Nejprve se podíváme, zda můžeme naše obvody ještě dále optimalizovat.

## 3. Řešení s použitím skutečného kvantového počítače II {#3-solution-using-a-real-quantum-computer-ii}

Vraťme se ke kroku 1 Qiskit patterns a podívejme se, zda můžeme snížit hloubku našeho obvodu.

###  3.1 Krok 1. Mapování problému na kvantové obvody a operátory {#31-step-1-map-the-problem-to-quantum-circuits-and-operators}

#### Aktivita 7 {#activity-7}

Sestav obvod pro časový vývoj. Využij znalosti z předchozích lekcí a pokus se snížit hloubku obvodu.
...
Transpiluj obvody a shromáždi je do seznamu. I tentokrát to může trvat několik minut.
...
### 3.3 Krok 3. Spuštění na cílovém hardware {#33-step-3-execute-on-target-hardware}
Spusť úlohu.
...
### 3.4 Krok 4. Post-processing {#34-step-4-post-processing}
...
Nyní si vykreslíme výsledky a porovnáme výsledky z hardware (výchozí okruh a mělký obvod) s MPS simulátorem. Jak ovlivňuje chyba reálného hardware výsledky?
