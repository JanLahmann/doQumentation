---
title: "Zmírnění chyb"
sidebar_label: "Zmírnění chyb"
description: "V této lekci prozkoumáme šum a způsoby, jak jej lze na kvantových počítačích zmírnit."
...
# Kvantový šum a zmírnění chyb {#quantum-noise-and-error-mitigation}

<Admonition type="note">

Toshinari Itoko (28. června 2024)

[Stáhni si pdf](https://ibm.ent.box.com/public/static/a0zgies7bh91hm2lwev9o0bfeybxc6n6.zip) původní přednášky. Pozor, některé úryvky kódu mohou být zastaralé, protože se jedná o statické obrázky.
...
## 2. Zašuměná simulace bez zmírnění chyb {#2-noisy-simulation-without-error-mitigation}
...
```
### 2.1 Sestavení testovacího obvodu {#2-1-build-a-test-circuit}
...
![Výstup předchozí buňky s kódem](/learning/images/courses/utility-scale-quantum-computing/error-mitigation/extracted-outputs/b4863c66-0.avif)
...
### 2.2 Sestavení modelu šumu {#2-2-build-a-noise-model}

Abychom mohli provádět zašuměnou simulaci, potřebujeme zadat `NoiseModel`. V této části ukážeme, jak `NoiseModel` sestavit.
Nejprve musíme definovat kvantové (nebo měřicí) chyby, které do modelu šumu přidáme.
...
### 2.3 Sestavení zašuměného sampleru s modelem šumu {#2-3-build-a-noisy-sampler-with-the-noise-model}
...
### 2.4 Spuštění kvantových obvodů na zašuměném sampleru {#2-4-run-quantum-circuits-on-the-noisy-sampler}
...
### 2.5 Vykreslení výsledků {#2-5-plot-results}
...
plt.show()
```
### 2.6 Ideální simulace {#2-6-ideal-simulation}
...
### 2.8 Realističtější zašuměná simulace {#2-8-more-realistic-noisy-simulation}
...
## 3. Reálný kvantový výpočet se zmírněním chyb {#3-real-quantum-computation-with-error-mitigation}

V této části si ukážeme, jak pomocí Qiskit Estimatoru získat výsledky (střední hodnoty) se zmírněním chyb.
Uvažujeme 6qubitové Trotterizované obvody pro simulaci časového vývoje jednorozměrného Isingova modelu a podíváme se, jak chyba škáluje v závislosti na počtu časových kroků.
...
### 3.1 Sestavení obvodů {#3-1-build-circuits}
...
Abychom znali ideální výstup předem, používáme obvody typu compute-uncompute, které se skládají z první fáze, kde je aplikován původní obvod $U$, a druhé fáze, kde je obrácen $U^\dagger$.
Všimni si, že ideální výsledek takových obvodů bude triviálně vstupní stav $|000000\rangle$, který má triviální střední hodnoty pro libovolné Pauliho pozorovatelné, například $\langle IIIIIZ \rangle = 1$.
...
![Výstup předchozí buňky kódu](/learning/images/courses/utility-scale-quantum-computing/error-mitigation/extracted-outputs/20296b5a-0.avif)

Poznámka: Jak je ukázáno výše, obvod s $k$ časovými kroky bude mít $4k$ vrstev dvouqubitových hradel.
...
### 3.2 Transpilace obvodů {#3-2-transpile-the-circuits}
...
![Výstup předchozí buňky kódu](/learning/images/courses/utility-scale-quantum-computing/error-mitigation/extracted-outputs/87b861e2-0.avif)
### 3.3 Spuštění pomocí Estimatoru (s různými úrovněmi odolnosti) {#3-3-execute-using-estimator-with-different-resilience-levels}

Nastavení úrovně odolnosti (`estimator.options.resilience_level`) je nejjednodušší způsob, jak při použití Qiskit Estimatoru aplikovat zmírnění chyb. Estimator podporuje následující úrovně odolnosti (ke dni 28. 6. 2024). Další podrobnosti najdeš v příručce [Configure error mitigation](/guides/error-mitigation-and-suppression-techniques).
...
### 3.4 Vykreslení výsledků {#3-4-plot-results}
...
## 4. (Volitelné) Přizpůsobení možností zmírnění chyb {#4-optional-customize-error-mitigation-options}
...
- [Konfigurace zmírnění chyb](/guides/error-mitigation-and-suppression-techniques)
- [Úvod do možností](/guides/runtime-options-overview)
