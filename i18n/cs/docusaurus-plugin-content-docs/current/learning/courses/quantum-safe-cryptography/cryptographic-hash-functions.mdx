...
Skutečnost, že velikost vstupní domény $Σ^*$ funkce $H$ je neomezená, zatímco ko-doména $\{0,1\}^n$ je omezená, znamená, že $H$ je nutně zobrazením mnoho-na-jeden, mapujícím nekonečně mnoho vstupů na libovolný n-bitový řetězec.

Vlastnosti uniformity a determinismu jsou pěkně zachyceny v <DefinitionTooltip definition="Idealizovaný kryptografický model často používaný v teoretických diskuzích a analýzách kryptografických protokolů a schémat; umožňuje jasnou a rigorózní analýzu bezpečnostních vlastností.">*modelu náhodného orákula*</DefinitionTooltip> kryptografického hašování.
...
Pro danou CHF $H$ a digest $g$ by mělo být výpočetně neproveditelné nalézt jakýkoli vstup $m$ z pre-image hodnoty $g$ takový, že $H(m) = g$.
...
Odolnost vůči kolizím je klíčová pro aplikace jako digitální podpisy a certifikáty, kde by bylo katastrofální, kdyby zlomyslná strana dokázala vytvořit padělek, který se hašuje na stejnou hodnotu.
