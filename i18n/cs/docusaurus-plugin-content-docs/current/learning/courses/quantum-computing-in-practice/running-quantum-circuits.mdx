...
3.	**Spuštění (Execute).** Primitiva Qiskit Runtime poskytují rozhraní k hardwaru IBM Quantum, které umožňuje spouštět transpilované obvody. Tento krok zahrnuje také použití technik pro potlačení a zmírnění chyb, které lze do velké míry abstrahovat od uživatele.
...
Dále budeme muset zvolit kvantový procesor s dostatečným počtem qubitů dostatečně vysoké kvality, abychom mohli spustit náš kvantový obvod. Při těchto rozhodnutích se řiď třemi metrikami, které jsme probrali v části o hardwaru: počtem qubitů, EPLG a CLOPS.
...
[Sampler Qiskit Runtime](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler) spouští obvod opakovaně na kvantovém zařízení, provádí měření při každém běhu a rekonstruuje rozdělení pravděpodobnosti z načtených bitových řetězců. Čím více běhů (shots) provede, tím přesnější budou výsledky — to ale vyžaduje více času a kvantových prostředků. Konkrétně počítá pravděpodobnost získání každého možného stavu standardní báze měřením stavu připraveného obvodem.
