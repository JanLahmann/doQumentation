...
Teď jsme připraveni přistoupit k samotnému mapování. Přechod od grafu, který jsme právě nakreslili, ke kvantovému obvodu může působit skličujícím dojmem – provedeme ho ale krok za krokem.

Pamatuj, že budeme řešit Max-Cut pomocí QAOA. V metodologii QAOA potřebujeme nakonec mít operátor (jinak řečeno Hamiltonián), který bude reprezentovat nákladovou funkci našeho hybridního algoritmu, a dále parametrizovaný obvod (ansatz), jímž vyjadřujeme možná řešení problému.
...
Abychom mohli QUBO problém zapsat jako Isingův Hamiltonián, stačí provést jednoduchou změnu proměnných z $x \in \{0, 1\}^n$ na $z \in \{-1, 1\}^n$:
...
<Accordion>
<AccordionItem title="Podrobnosti">

Nejprve si všimni, že náš QUBO výraz můžeme přepsat jako součet maticových členů:

$$
x^TQx = \sum_{ij}Q_{ij}x_ix_j
$$

Aplikací změny proměnných dostaneme

$$
\sum_{ij}Q_{ij}(\frac{1-z_i}{2})(\frac{1-z_j}{2}) = \sum_{ij}\frac{Q_{ij}}{4}(1 - z_i - z_j + z_iz_j)
$$

což lze přeuspořádat následovně.

$$
\sum_{ij}\frac{Q_{ij}}{4}z_iz_j - \frac{Q_{ij}}{4}z_i - \frac{Q_{ij}}{4}z_j + \frac{Q_{ij}}{4}
$$

Konstantní člen můžeme vynechat, protože neovlivní, které $z$ minimalizuje QUBO problém. Kromě toho můžeme tento výraz vynásobit čtyřmi, abychom obnovili původní hodnotu $Q$, jelikož to také neovlivňuje optimální volbu $z$.

$$
\sum_{ij}Q_{ij}z_iz_j - Q_{ij}z_i - Q_{ij}z_j = z^TQz + (-\sum_{ij} Q_{ij}z_i + Q_{ij}z_j)
$$

Seskupením podobných lineárních členů můžeme definovat náš vektor lineárních koeficientů $b$ pomocí rovnice

$$
b_i = -\sum_{j} Q_{ij} + Q_{ji}
$$

což po dosazení do předchozí rovnice dokončuje transformaci proměnných na $z \in \{-1, 1\}^n$.
...
Výsledkem je, že minimalizace QUBO výrazu je totéž co minimalizace tohoto výrazu, kde $b$ je reálný skalární koeficient:

$$
\min_{x\in\{0,1\}^n} x^TQx\Longleftrightarrow \min_{z\in\{-1,1\}^n}z^TQz + b^Tz
$$

Po dalším mírném přepsání dostaneme Hamiltonián nákladové funkce, kde minimum výrazu reprezentuje základní stav a `Z` je Pauliho operátor Z:

$$
H_C=\sum_{ij}Q_{ij}Z_iZ_j + \sum_i b_i Z_i
$$

Teď, když máme svůj Hamiltonián, ho potřebujeme přepsat pomocí dvoulokálních Pauliho operátorů ZZ, které lze snadno převést na dvouqubitová hradla v našem kvantovém obvodu. Výsledkem bude šest objektů – tzv. Pauliho řetězců (Pauli strings) –, kde každý odpovídá jedné z šesti hran grafu. Každý z pěti prvků řetězce reprezentuje operaci na uzlu: identita, pokud uzel k dané hraně nepatří, a Pauliho operátor Z, pokud patří. V Qiskitu jsou bitové řetězce reprezentující qubity indexovány pozpátku. Například hrana mezi uzly 0 a 1 je zakódována jako `IIIZZ` a hrana mezi uzly 2 a 4 jako `ZIZII`.

#### Sestavení kvantového obvodu {#construct-the-quantum-circuit}

Když máme Hamiltonián zapsaný pomocí Pauliho operátorů, jsme připraveni sestavit kvantový obvod, který nám umožní vzorkovat dobrá řešení na kvantovém počítači:

![Schéma obvodu s vrstvami QAOA](/learning/images/courses/quantum-computing-in-practice/utility-scale-qaoa/circuit-diagram.svg)
...
Pro sestavení kvantového obvodu použijeme střídající se operátory parametrizované hodnotami $\gamma$ a $\beta$, které reprezentují diskretizaci časového vývoje.

Tři hlavní části obvodu QAOA jsou tedy:

1.	počáteční zkušební stav, zobrazený šedě, což je základní stav mixeru, vytvořený aplikací Hadamardova hradla na každý qubit
...
Ačkoliv obvod, který jsme právě vytvořili, vypadá dost jednoduše a hodí se k budování intuitivního porozumění, pamatuj, že kvantový čip neví, co je to hradlo QAOA. Musíme ho převést na sérii jednoqubitových a dvouqubitových „nativních" hradel, která lze přímo provádět na hardwaru. Nativní hradla jsou ta, jež lze přímo aplikovat na qubity. O takovýchto obvodech se říká, že jsou zapsány v instrukční sadě architektury (ISA) backendu.
