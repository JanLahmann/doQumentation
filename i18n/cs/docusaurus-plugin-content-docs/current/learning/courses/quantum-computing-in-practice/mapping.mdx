...
Pokud ti to připadá obrácené (zejména znaménko mezi $X$ a $iY$), podívej se na volitelnou poznámku níže o konvencích ohledně toho, který stav $|0\rangle$ nebo $|1\rangle$ odpovídá „obsazenému“ a který „neobsazenému“.

<Accordion>
<AccordionItem title="Volitelné poznámky ke konvencím">

V mnoha fyzikálních kontextech není interpretace operátorů $\sigma^\pm$ nejednoznačná; zvyšují/snižují z-projekci spinového momentu hybnosti systému. $\sigma^+$ například převádí stav spinového momentu hybnosti $|m_s\rangle$ na stav s $|m_s+1\rangle$. Když ale při použití mapperu vyjadřujeme fermionové kreační a anihilační operátory pomocí těchto operátorů momentu hybnosti (kombinací Pauliho operátorů), zvýšení/snížení nyní odpovídá obsazení elektronového orbitalu. „Zvyšování“ a „snižování“ není nejednoznačné, pokud je akcí zvýšení nebo snížení projekce momentu hybnosti na určité ose a odpovídající označení. U obsazování elektronových orbitalů by někdo mohl spojit $|0\rangle$ s prázdným a $|1\rangle$ s obsazeným. To by se mohlo zdát jako zjevná konvence, a skutečně je to ta, která se používá v doplňcích Qiskit. Je ale možné zvolit $|0\rangle$ jako „obsazený“ ve smyslu elektronu relaxujícího do základního stavu systému a $|1\rangle$ jako „neobsazený“. Je to svobodná volba a potenciální nejednoznačnost v označování: který stav odpovídá obsazenému orbitalu? $|0\rangle$ nebo $|1\rangle$?

V kontextu kvantových výpočtů je to nyní do jisté míry standardizováno, a to i ve vestavěných mapperech, jako je doplněk Qiskit `FermionicMapper`, který nastavuje
$$
\begin{aligned}
|0\rangle & = \text{ prázdný}\\
|1\rangle & = \text{ obsazený}
\end{aligned}
$$

Ekvivalentně, operátor, který vytváří elektron obsazující orbital $k$ (reprezentovaný qubitem $q_k$), se musí chovat takto:

$$
O_k |q_N,...,q_k=0,...,q_0\rangle = c |q_N,...,q_k=1,...,q_0\rangle
$$

Nyní se podívejme na důsledky této volby. Všimni si, že ve standardních bázích používaných v kvantových výpočtech,
...
a podobným postupem můžeme sestavit celou množinu:
...
Z toho je zřejmé, že zvolením zdánlivě rozumné konvence, že $|0\rangle$ odpovídá žádnému elektronu a $|1\rangle$ odpovídá jednomu elektronu, skončíme v matoucí situaci, kdy se $\sigma^+$ objevuje v anihilačním operátoru a $\sigma^-$ v kreačním operátoru. Konvence v `Qiskit Fermions` (kde $|0\rangle$ odpovídá obsazenému a $|1\rangle$ odpovídá neobsazenému) tak vede k tomu, že fermionový kreační operátor $f_k^\dagger$ obsahuje $\sigma_k^-$ a anihilační operátor $f_k$ obsahuje $\sigma_k^+$.

Pokud narazíš na zdroje s opačnými znaménky mezi členy $X$ a $iY$ v kreačních a anihilačních operátorech, pravděpodobně používají opačnou konvenci ohledně toho, který stav odpovídá obsazenému orbitalu.
...
kde $\sigma^+$ je Pauliho operátor $X + iY$ a $\sigma^{-}$ je $X - iY$. Jakmile máme Hamiltonián zapsaný v tomto formátu, nejtěžší část fáze zobrazení je za námi a Hamiltonián lze nyní snadno zapsat do Circuit pomocí Pauliho operátorů.
