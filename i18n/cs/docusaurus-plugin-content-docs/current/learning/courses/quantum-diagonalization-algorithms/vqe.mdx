...
### 1.1 Operátor (Hamiltonián) {#11-the-operator-hamiltonian}
...
Obecně však není možné měřit $H$ najednou. $H$ může obsahovat více nekomutujících Pauliho operátorů X, Y a Z. Hamiltonián se proto musí rozdělit do skupin operátorů, které lze měřit současně, a každá taková skupina se musí odhadovat zvlášť, přičemž výsledky se kombinují za účelem získání střední hodnoty. Tomu se budeme podrobněji věnovat v další lekci, kde budeme diskutovat škálování klasických a kvantových přístupů. Tato složitost měření je jedním z důvodů, proč potřebujeme vysoce efektivní kód pro provádění takového odhadování. V této lekci i dále budeme k tomuto účelu používat primitiv Estimator IBM Quantum.

### 1.4 Klasické optimalizátory {#14-classical-optimizers}
...
### 1.5 Variační princip {#15-the-variational-principle}
...
### 3.3 Krok 3: Spuštění na cílovém hardwaru {#33-step-3-execute-on-target-hardware}

#### 3.3.1 Hlášení výstupních hodnot {#331-reporting-out-values}
...
### 3.4 Krok 4: Zpracování výsledků {#34-step-4-post-process-results}
...
### Ověř si znalosti {#check-your-understanding-2}
...
## 4. Vztah VQE k jiným metodám {#4-vqes-relationship-to-other-methods}

V této části si projdeme výhody a nevýhody původního přístupu VQE a poukážeme na jeho vztahy k jiným, novějším algoritmům.

### 4.1 Silné a slabé stránky VQE {#41-the-strengths-and-weaknesses-of-vqe}
...
### 4.2 Vztah k jiným přístupům {#42-relationship-to-other-approaches}
...
### Zkontroluj své porozumění {#check-your-understanding-3}
