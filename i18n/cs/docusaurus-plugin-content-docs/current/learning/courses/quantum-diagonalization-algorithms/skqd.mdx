...
## 1. Namapování problému na kvantové obvody a operátory {#1-map-problem-to-quantum-circuits-and-operators}
...
Pro zvolenou hodnotu $r$ (a $dt$) vytvoříme $r$ samostatných kvantových obvodů a budeme z nich vzorkovat. Každý kvantový obvod se vytvoří spojením kvantového obvodu reprezentujícího referenční stav a operátoru časového vývoje pro danou hodnotu $k$.
...
Kromě Krylovovy dimenze a časového kroku musíme nastavit počet Trotterových kroků pro časový vývoj. Příliš málo kroků vede k větším chybám trotterizace, zatímco příliš mnoho kroků vede k hlubším obvodům. V této lekci nastavíme počet Trotterových kroků na $6$.
...
Nakonec musíme namapovat operátor časového vývoje na kvantový obvod. To bylo provedeno v lekci 2, ale zde využijeme metody v Qiskitu, konkrétně metodu nazvanou _synthesis_. Existují různé metody syntézy matematických operátorů na kvantové obvody s kvantovými hradly. Mnoho takových technik je dostupných v [modulu syntézy Qiskit](https://docs.quantum.ibm.com/api/qiskit/synthesis). Použijeme přístup [`LieTrotter`](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter) pro syntézu [\[3\]](#references) [\[4\]](#references).
...
<Accordion>
<AccordionItem title="Co by bylo možné udělat pro zlepšení konvergence na grafu výše?">
...
<AccordionItem title="Jaké jsou hlavní výhody SKQD oproti (a) SQD a (b) KQD?">

__Answer:__

Jaké jsou hlavní výhody SKQD oproti (a) SQD a (b) KQD?

   (a) SKQD přichází se zárukami konvergence, které SQD nemá. U SQD buď musíš udělat velmi dobrý odhad ansatzu, který má vynikající překryv s podporou základního stavu ve výpočetní bázi, nebo musíš do výpočtu zavést variační komponentu, aby bylo možné vzorkovat rodinu ansatzů.
