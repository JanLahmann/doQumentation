...
(b) Proces ortogonalizace spočívá v odečtení projekce nového vektoru na všechny předchozí vektory. Pokud všechny předchozí vektory pokrývají celý vektorový prostor, pak odečtením projekcí na celý podprostor vždy získáme nulový vektor.

</AccordionItem>
</Accordion>

Předpokládej, že kolega výzkumník demonstruje Krylovovu metodu aplikovanou na malou ukázkovou matici. Je něco špatně s jeho volbou matice $A$ a počátečního vektoru $|\psi\rangle$?
...
Tvůj kolega si nešťastně zvolil za počáteční vektor vlastní vektor. Působení matice na počáteční vektor jednoduše vrátí tentýž vektor škálovaný vlastním číslem. Tím nevznikne podprostor rostoucí dimenze. Poraď kolegovi, aby zvolil jiný počáteční vektor a ujistil se, že to není vlastní vektor.

</AccordionItem>
</Accordion>

Aplikuj Krylovovu metodu na danou matici a zvol vhodný nový počáteční vektor. Zapiš odhady minimálního vlastního čísla v 0. a 1. řádu tvého Krylovova podprostoru.
...
$$
Abychom získali $|v_1\rangle$, aplikujeme jednou $A$ na $|v_0\rangle$ a poté učiníme $|v_1\rangle$ ortogonální k $|v_0\rangle.$
$$
...
$$
V 0. řádu je projekce na náš Krylovův podprostor
...
$$
V 1. řádu je projekce na tento Krylovův podprostor
...
$$
Toto lze provést ručně, ale nejsnadnější je použít numpy:
...
```
Odhad minimálního vlastního čísla je -0,414.
...
### Ověř své porozumění {#check-your-understanding-1}
...
Podívej se na vyhlazený graf časů diagonalizace v závislosti na dimenzi matice výše.

(a) Při jaké přibližné dimenzi matice se podle tohoto grafu stala Krylovova metoda rychlejší?

(b) Jaké aspekty výpočtu by mohly změnit dimenzi, při které se Krylovova metoda stává rychlejší?
...
$$
Zde $P_\alpha$ je Pauli řetězec tvaru $P_\alpha \sim IZIXII...YZXIX$ nebo sada takových Pauli řetězců, které spolu navzájem komutují. Vzhledem k tomu, že $H$ lze zapsat jako takovýto součet měřitelných operátorů, lze následující výrazy pro maticové prvky $\tilde{H}$ realizovat pomocí primitiva Estimator IBM Quantum.
...
Obrázek ukazuje obvodovou reprezentaci modifikovaného Hadamardova testu, metody používané k výpočtu překryvu mezi různými kvantovými stavy. Pro každý maticový prvek $\tilde{H}_{i,j}$ se provede Hadamardův test mezi stavy $\vert \psi_i \rangle$ a $\vert \psi_j \rangle$. To je v obrázku zvýrazněno barevným schématem pro maticové prvky a odpovídající operace $\text{Prep} \; \psi_i$, $\text{Prep} \; \psi_j$. K výpočtu všech maticových prvků promítnutého Hamiltoniánu $\tilde{H}$ je tedy potřeba sada Hadamardových testů pro všechny možné kombinace vektorů Krylovova prostoru. Horní vodič v obvodu Hadamardova testu je pomocný qubit (ancilla), který se měří buď v bázi X nebo Y; jeho střední hodnota určuje hodnotu překryvu mezi stavy. Dolní vodič představuje všechny qubity systémového Hamiltoniánu. Operace $\text{Prep} \; \psi_i$ připraví systémový qubit ve stavu $\vert \psi_i \rangle$ řízeném stavem pomocného qubitu (stejně tak pro $\text{Prep} \; \psi_j$) a operace $P$ představuje Pauli rozklad systémového Hamiltoniánu $H = \sum_i P_i$. Implementace tohoto postupu na kvantovém počítači je podrobněji ukázána níže.
...
Zde $P$ je Pauliho člen v rozkladu Hamiltoniánu (všimni si, že nemůže jít o lineární kombinaci více komutujících Pauliho členů, protože ta by nebyla unitární -- seskupování je možné pomocí jiné konstrukce, kterou ukážeme později). $\text{Prep} \; \psi_i$, $\text{Prep} \; \psi_j$ jsou řízené operace, které připravují vektory $|\psi_i\rangle$, $|\psi_j\rangle$ unitárního Krylovova prostoru, přičemž $|\psi_k\rangle = e^{-i H k dt } \vert \psi \rangle = e^{-i H k dt } U_{\psi} \vert 0 \rangle^N$. Měření $X$ a $Y$ aplikovaná na tento obvod vypočítají reálnou, respektive imaginární část požadovaných maticových prvků.

Počínaje krokem 4 výše, aplikuj Hadamardovo hradlo $H$ na nultý qubit.
...
Obvod takové hloubky nemůže na moderních kvantových počítačích vrátit použitelné výsledky. Pokud chceme sestavit $\tilde{H}$ a $\tilde{S},$ potřebujeme lepší přístup. To je důvod pro zavedení efektivního Hadamardova testu, který je popsán níže.

### 4. 2 Krok 2. Optimalizace obvodů a operátorů pro cílový hardware {#4-2-step-2-optimize-circuits-and-operators-for-target-hardware}
...
Vzhledem k tomu, že hradlo $\text{Prep} \; \psi_0$ připravuje požadovaný referenční stav $\ket{\psi_0} = \text{Prep} \; \psi_0 \ket{0} = e^{-i H 0 dt} U_{\psi_0} \ket{0}$, například pro přípravu HF stavu pro chemii je $\text{Prep} \; \psi_0$ součinem jednoqubitových hradel NOT, takže řízená-$\text{Prep} \; \psi_0$ je jen součinem hradel CNOT.
Výše uvedený obvod pak před měřením implementuje následující stav:
...
Díky těmto předpokladům jsme dokázali vyjádřit střední hodnoty požadovaných operátorů s menším počtem řízených operací. Ve skutečnosti potřebujeme implementovat pouze řízenou přípravu stavu $\text{Prep} \; \psi_0$, nikoli řízené časové evoluce. Přeformulování výpočtu výše nám umožní výrazně snížit hloubku výsledných obvodů.

Poznamenejme, že jako bonus -- protože Pauliho operátor se nyní objevuje jako měření na konci obvodu namísto jako řízené hradlo uprostřed -- lze jej měřit společně s dalšími komutujícími Pauliho operátory, jako v rozkladu $$H=\sum_{\alpha = 1}^{N_\text{GCP}}c_\alpha P_\alpha $$ uvedeném výše.
...
Místo přesné implementace operátoru časového vývoje můžeme použít Trotter dekompozici a implementovat jeho aproximaci. Opakováním Trotter dekompozice určitého řádu vícekrát dosáhneme dalšího snížení chyby vzniklé z aproximace. V následujícím textu přímo sestavíme Trotter implementaci nejefektivnějším způsobem pro interakční graf Hamiltoniánu, který uvažujeme (pouze interakce nejbližších sousedů). V praxi vkládáme Pauli rotace $R_{xx}$, $R_{yy}$, $R_{zz}$ s vazebními konstantami $J_x$, $J_y$, $J_z$ a parametrizovaným úhlem $t$, které odpovídají přibližné implementaci $e^{-i (J_x XX + J_y YY + J_z ZZ) t}$. Vzhledem k rozdílu v definici Pauli rotací a časového vývoje, který se snažíme implementovat, budeme muset použít parametr $2*dt$ pro dosažení časového vývoje $dt$. Navíc obrátíme pořadí operací pro lichý počet opakování Trotter kroků, což je funkčně ekvivalentní, ale umožňuje syntézu sousedních operací do jediného $SU(2)$ unitárního operátoru. To dává mnohem mělčí obvod než ten, který se získá pomocí generické funkcionality `PauliEvolutionGate()`.
...
![Výstup předchozí buňky kódu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/8d35e6cd-c0f6-4871-83da-86472d66be2c-0.avif)
...
![Výstup předchozí buňky kódu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/d1d0b9de-65d4-4a46-975d-6cfaaac05f9a-0.avif)

#### Šablonové obvody pro výpočet maticových prvků $\tilde{S}$ a $\tilde{H}$ pomocí Hadamardova testu {#template-circuits-for-calculating-matrix-elements-of-tildes-and-tildeh-via-hadamard-test}
...
![Výstup předchozí buňky kódu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/37382668-1999-4475-b50f-2887449d5c93-0.avif)
...
![Výstup předchozí buňky kódu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/054bcfba-fde3-4213-bbd7-d7a6dd6c03c8-1.avif)

Po optimalizaci je hloubka dvouqubitových hradel našeho transpilovaného obvodu dále snížena.

### 4.3 Krok 3. Spuštění pomocí primitivu IBM Quantum {#43-step-3-execute-using-an-ibm-quantum-primitive}
...
![Výstup předchozí buňky kódu](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/a5d4a983-1a30-4cea-b695-3e6a67338633-0.avif)
...
### Appendix II: proof of Claim 1 {#appendix-ii-proof-of-claim-1}
