...
description: "Verwend d'QUICK-PDE-Funktion zur Modellierig vun ere fließende nit-viskose Flüssigkeit."
...
Beachte, dass d'Ausführungszeit vun dere Funktion im Allgemeine meh als 20 Minute beträgt,
drum ka mr des Tutorial vielleicht in zwoi Abschnitt ufteile: d'erschte, wo
mr's durchlest un d'Jobs startet, un d'zwote e paar Stunde später
(um d'Jobs gnueg Zeit zur Fertigstellung z'gebe), um mit d'Ergebniss vun d'Jobs z'arbeite.

## Hintergrund {#hintergrund}

Des Tutorial isch dazu do, uf einführendem Niveau z'zeige, wie d'QUICK-PDE-Funktion
verwendet wird, um komplexe Multi-Physik-Probleme uf 156Q Heron R2 QPUs under Verwendung
...
Beachte, dass derre Solver au nit-lineare Gleichunge löse kann.
...
* Zugang zur QUICK-PDE-Funktion. Füll des [Formular us, um Zugang anzufordere.](https://forms.cloud.microsoft/e/3Wi9cbjQPK)

## Setup {#setup}

Authentifizier dich mit deim [API-Schlüssel](http://quantum.cloud.ibm.com/) un wähl d'Funktion so us:
...
Mr kenne $a$ un $b$ anpasse un luege, wie se d'Lösungsprozess un d'Lösung beeinflussen.
...
Je noch d'Konfiguration, wo z'löse versucht wird, müsse d'Parameter vum Algorithmus
...
in d'falsche Richtung lenke; in dem Fall kenne mr des deaktiviere, indem mr d'
`initialization`-Strategie uf `"RANDOM"` setzt.
...
Mr kenne d'Konvergenzprozess vun unserer Lösung (HDES) vun job_2 mit d'Leistung vun emm Physics-Informed Neural Networks (PINN)-Algorithmus un -Solver vergleiche (lueg des [Paper](https://arxiv.org/abs/1711.10561) un des zugehörige [GitHub-Repository](https://github.com/314arhaam/burger-pinn)).
...
Beachte d'Unterschied vun d'Anfangsbedingung für d'zweite Durchlauf un sini Auswirkung uf
des Ergebnis:
