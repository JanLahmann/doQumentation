...
description: "Des Tutorial zeigt, wie mir grundlegende Wiederholungscodes mit IBM Dynamic Circuits erstelle, e Beispiel für grundlegende Quantenfehlerkorrektur (QEC)."
...
*Nutzungsschätzung: weniger als 1 Minute uf eim Heron-Prozessor (HINWEIS: Des isch bloß e Schätzung. Ihre Laufzeit ka variiere.)*
...
Mir könne zusätzliche Hilfs-Qubits un Verschränkung nutze, um *Stabilisatoren* z'messe, die kodierte Quanteninformation nit transformiere, während se uns dennoch über etliche Fehlerklasse informiere, die vielleicht aufgträtte sin. E Quanten-Stabilisator-Code kodiert $k$ logische Qubits in $n$ physische Qubits. Stabilisator-Codes konzentriere sich vor allem uf d'Korrektur von eim diskreten Fehlersatz mit Unterstützung us dr Pauli-Gruppe $\Pi^n$.

Für weitere Informatione über QEC luege Se bei [Quantum Error Correction for Beginners](https://arxiv.org/abs/0905.2794) nei.

## Anforderungen {#anforderungen}

Stelle Se sicher, bevor Se mit dem Tutorial afange, dass Se Folgendes installiert hän:
...
## Schritt 1. Klassische Eingabe uf e Quanteproblem abbilden {#schritt-1-klassische-eingaben-auf-ein-quantenproblem-abbilden}

### En Bit-Flip-Stabilisator-Schaltkreis erstelle {#einen-bit-flip-stabilisator-schaltkreis-erstellen}

D'r Bit-Flip-Code isch ois von de eifachste Beispiele von eim Stabilisator-Code. Er schützt den Zustand vor eim einzelne Bit-Flip (X)-Fehler uf eim von de Kodierungs-Qubits. Betrachte mir d'Wirkung vom Bit-Flip-Fehler $X$, der $|0\rangle \rightarrow |1\rangle$ un $|1\rangle \rightarrow |0\rangle$ uf eim von unsere Qubits abbildet, dann hän mir $\epsilon = \{E_0, E_1, E_2 \} = \{IIX, IXI, XII\}$. D'r Code bruucht fünf Qubits: Drei werde verwendet, um den geschützten Zustand z'kodiere, un d'verbleibende zwei werde als Stabilisatormessungs-Ancillas verwendet.
...
## Schritt 2. Problem für d'Quanteausführung optimiere {#schritt-2-problem-fuer-die-quantenausfuehrung-optimieren}

Um d'Gesamtausführungszeit vom Job z'reduziere, akzeptiere Qiskit-Primitive bloß Schaltkreise un Observablen, die de vom Zielsystem unterstützte Anweisunge un d'Konnektivität entspreche (bezeichnet als Instruction Set Architecture (ISA)-Schaltkreise un -Observablen). [Erfahre Se mehr über Transpilation.](/guides/transpile)
...
## Schritt 3. Usführe mit Qiskit-Primitive {#schritt-3-ausfuehren-mit-qiskit-primitiven}

Führe Se d'Version mit angewendeter Korrektur un eine ohne Korrektur us.
...
Bitte mache Se bei dere kurze Umfrage mit, um Feedback z'dem Tutorial z'gehe. Ihre Erkenntnisse helfe uns, unseri Inhaltsangebote un d'Benutzererfahrung z'verbessere.
