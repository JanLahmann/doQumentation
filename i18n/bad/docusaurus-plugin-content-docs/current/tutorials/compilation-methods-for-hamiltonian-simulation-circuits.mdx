---
title: "Kompilierungsmethode für Hamilton-Simulationsschaltunge"
sidebar_label: "Kompilierungsmethode für Hamilton-Simulationsschaltunge"
description: "Des Tutorial git en vergleichende Überblick über drei Kompilierungsmethode in Qiskit für Hamilton-Simulations-Workloads."
...
### Was mir lerne werdet {#was-mir-lerne-werdet}
...
- Networkx-Graphebibliothek (`pip install networkx`)
...
### Schritt 1: Klassische Eingabe uf es Quanteproblem abbildet {#schritt-1-klassische-eingaben-auf-ein-quantenproblem-abbilden}
...
#### Transpiliere und Metrike erfasse {#transpilieren-und-metriken-erfassen}
...
#### Analyse vun de SU2-Schaltungskompilierungsergebniss {#analyse-der-su2-schaltungskompilierungsergebnisse}
...
### Schritt 1: Untersuchung vun Schaltunge mit `PauliEvolutionGate` {#schritt-1-untersuchung-von-schaltungen-mit-paulievolutiongate}
...
D' in dem Benchmark verwendete Hamiltonians beschreibet paarweise Interaktione zwische Qubits, ischließlich Terme wie $ZZ$, $XX$ und $YY$. Dene Hamiltonians werdet häufig in Quantenchemie, Festkörperphysik und Materialwissenschaft verwändet, wo se Systeme vun interagierenden Teilche modelliert.

Als Referenz kenne Benutzer en breiterere Satz vun Hamiltonians in dem Paper erkunde: [Efficient Hamiltonian Simulation on Noisy Quantum Devices](https://arxiv.org/pdf/2306.13126).

#### Benchmark-Quelle: Hamlib und Benchpress {#benchmark-quelle-hamlib-und-benchpress}

D' in dem Benchmark verwendete Schaltunge stammet us em [Hamlib-Benchmark-Repository](https://github.com/SRI-International/QC-App-Oriented-Benchmarks/tree/master/hamlib), wo realistische Hamilton-Simulations-Workloads enthält.

Dene gleiche Schaltunge sind zuvor mit [Benchpress](https://github.com/Qiskit/benchpress) benchmarked wore, emme Open-Source-Framework zur Bewertig vun dr Quanten-Transpilationsleischtig. Durch d' Verwendung vun dem standardisierten Satz vun Schaltunge kenne mir d' Effektivität verschiedener Kompilierungsstrategie bi repräsentativen Simulationsproblem direkt vergleicha.
...
### Schritt 2: Problem für d' Ausführung uf Quantehardware optimiere {#schritt-2-problem-fuer-die-ausfuehrung-auf-quantenhardware-optimieren-2}
...
#### Analyse vun de Hamilton-Schaltungskompilierungsergebniss {#analyse-der-hamilton-schaltungskompilierungsergebnisse}
...
### Zusammefassung {#zusammenfassung}
...
- **AI-Transpiler** isch typischerweise zuverlässig und liefert tiefenoptimierte Schaltunge, bringt aber Kompromisse bi dr Laufzeit mit sich und hett au andere Einschränkunge, ischließlich unterstützter Coupling-Maps und Synthese-Fähigkeite. Dene sind in dr [Qiskit Transpiler Service-Dokumentation](/guides/qiskit-transpiler-service) detailliert beschriebe.

- In manchen Fälle, insbesondere bi sehr große oder hardwarespezifische Schaltunge, isch dr AI-Transpiler möglicherweise nit so effektiv. In dene Fäll bleibt dr Standard-SABRE-Transpiler äußerst zuverlässig und ka durch Agpassung sinere Parameter weiter optimiert werre (gseet s [SABRE-Optimierungs-Tutorial](/tutorials/transpilation-optimizations-with-sabre)).

- Es isch au wichtig, d' Schaltungsstruktur bi dr Wahl vun ere Methode z berücksichtige. Zum Beispiel isch `rustiq` speziell für Schaltunge mit `PauliEvolutionGate` konzipiert und liefert oft d' beste Leischtig für Hamilton-Simulationsproblem.
...
### Schritt 4: Nachbearbeitung und Rückgab vun em Ergebnis im gwünschte klassische Format {#schritt-4-nachbearbeitung-und-rueckgabe-des-ergebnisses-im-gewuenschten-klassischen-format}
