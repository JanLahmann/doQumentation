...
(a) Normalkan fungsi gelombang percubaan dengan menentukan $A$ supaya $$ \int_{-\infty}^{\infty} |\Psi_\text{trial}|^2 dx = 1 $$.

<Accordion>
<AccordionItem title="Jawapan">

Untuk menormalkan fungsi gelombang percubaan yang diberikan:
...
(b) Kira nilai jangkaan Hamiltonian $\hat{H}$ yang diberikan oleh $$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V(x)$$ di mana $V(x)  = \frac{1}{2}m\omega^2x^2$, yang sepadan dengan potensi pengayun harmonik mudah.

<Accordion>
<AccordionItem title="Jawapan">

Hamiltonian untuk pengayun harmonik ialah:
...
Optimumkan $ \alpha $ untuk tenaga minimum
...
#### (Kuantum) Ansatz: Fungsi gelombang percubaan (Cara membina keadaan kuantum percubaan dengan litar kuantum) {#quantum-ansatz-the-trial-wave-function-how-to-build-a-trivial-quantum-state-with-a-quantum-circuit}

Untuk VQE, ansatz (jamak: ansätze) terdiri daripada dua komponen utama. Yang pertama ialah penyediaan keadaan awal, yang menetapkan keadaan qubit dengan menggunakan get kuantum tanpa parameter variatif. Komponen kedua ialah litar kuantum berparameter, sebuah litar kuantum khas dengan parameter boleh laras, sama seperti tombol pada radio. Parameter-parameter ini akan digunakan untuk bahagian terakhir — pengoptimum klasik — untuk membantu kita mencapai keadaan asas terbaik yang mungkin.

Dalam bahagian prinsip variatif, kita telah belajar bahawa kualiti keadaan percubaan mempengaruhi kualiti keputusan algoritma variatif. Ini bermakna memilih ansatz yang baik adalah penting dalam VQE. Sekali lagi, ini adalah topik yang kaya dan kompleks. Kita tidak akan membincangkan jenis-jenis ansatz yang berbeza atau asal-usulnya di sini. Jika anda berminat untuk mengetahui lebih lanjut tentang litar kuantum berparameter dan ansatz, anda boleh meneroka pelajaran [Ansatz dan bentuk variatif](/learning/courses/variational-algorithm-design/ansaetze-and-variational-forms) daripada [kursus Reka bentuk algoritma variatif](/learning/courses/variational-algorithm-design/index), yang memberikan penjelasan dan contoh terperinci tentang ansätze.

Oleh kerana kita akan menggunakan Hamiltonian satu-qubit dalam modul ini, kita perlukan litar kuantum berparameter satu-qubit sebagai ansatz. Kita akan melihat tiga jenis ansätze satu-qubit dalam bahagian berikut. Kita akan membandingkannya dan membincangkan pertimbangan utama dalam memilih ansatz.

#### (Klasik) Pengoptimum: menala halus litar kuantum {#classical-optimizer-fine-tuning-the-quantum-circuit}
...
- Langkah 3: Jalankan menggunakan primitif IBM Quantum
...
Mari mulakan dengan memuatkan beberapa pakej yang diperlukan, termasuk primitif IBM Quantum. Kita juga akan memilih komputer kuantum yang paling tidak sibuk yang tersedia untuk kita.
...
Seterusnya, kita perlukan litar kuantum berparameter, sebuah ansatz, untuk menyediakan fungsi gelombang percubaan $\Psi_\text{trial}$ bagi keadaan asas. Matlamatnya adalah untuk mencari parameter $\theta$ yang meminimumkan nilai jangkaan tenaga $\langle\psi(\theta)|\hat{H}|\psi(\theta)\rangle$. Pilihan ansatz adalah sangat penting kerana ia menentukan set keadaan kuantum yang mungkin boleh disediakan oleh litar kita. Ansatz yang "baik" ialah yang cukup fleksibel untuk mewakili keadaan yang sangat hampir dengan keadaan asas sebenar Hamiltonian yang kita kaji, tetapi tidak terlalu kompleks sehingga memerlukan terlalu banyak parameter atau litar yang terlalu dalam untuk komputer kuantum semasa.

Di sini, kita akan mencuba tiga ansätze satu-qubit yang berbeza untuk melihat mana yang memberikan "liputan" yang lebih baik terhadap keadaan kuantum yang mungkin boleh dimiliki oleh satu qubit. "Liputan" merujuk kepada julat keadaan kuantum yang boleh dihasilkan oleh litar ansatz dengan mengubah parameternya.

Kita akan menggunakan tiga ansätze berdasarkan kombinasi get putaran satu-qubit yang berbeza:

- Ansatz get putaran 1-paksi: Ansatz ini menggunakan putaran hanya sekitar satu paksi tunggal ($R_x(\theta)$). Pada sfera Bloch, ini sepadan dengan bergerak hanya di sepanjang bulatan tertentu. Ini adalah yang paling kurang fleksibel dan merangkumi set keadaan yang terhad.

- Dua ansätze get putaran 2-paksi: Ansätze ini menggabungkan putaran sekitar dua paksi berbeza ($R_x(\theta_1) R_z(\theta_2)$ dan $R_x(\theta_1) R_z(\theta_2) R_x(\theta_3)$). Ini membolehkan kita mencapai bahagian sfera Bloch yang lebih besar, berbanding dengan putaran satu paksi.
...
Sekarang kita telah memilih ansatz kita, mari lukis litar.
...
Apabila menjalankan pengiraan pada komputer quantum sebenar, kita tidak hanya mengambil berat tentang logik litar quantum. Kita juga mengambil berat tentang perkara seperti operasi apa yang boleh dilakukan oleh komputer quantum tertentu itu, dan di mana pada komputer quantum itu qubit yang kita gunakan berada. Adakah mereka bersebelahan antara satu sama lain? Adakah mereka jauh terpisah? Oleh itu, langkah seterusnya adalah untuk menulis semula litar kita menggunakan gate yang semula jadi untuk komputer quantum yang akan kita gunakan, dan mengambil kira susun atur qubit. Ini boleh dilakukan melalui `transpilation` - selepas proses ini, anda boleh melihat ansatz mudah kita ditukar kepada set gate yang berbeza, dan qubit abstrak kita akan dipetakan kepada qubit fizikal pada komputer quantum sebenar.
...
Anda dapat melihat get `rx, rz` ansatz kita telah ditukar kepada siri get `rz, sx`, yang merupakan get asli Backend kita. Juga, anda dapat melihat `q0` kita kini dipetakan ke dalam qubit fizikal kelima. Kita juga perlu memetakan Hamiltonian kita mengikut perubahan-perubahan ini, seperti dalam kod berikut:
...
Nota: Anda boleh mendapatkan keputusan yang lebih baik dengan menetapkan tahap pengurangan ralat menggunakan [`resilience_level`](/guides/error-mitigation-and-suppression-techniques). Nilai lalai ialah 1, dan jika anda menetapkan nilai yang lebih tinggi, ia akan menggunakan lebih banyak masa QPU tetapi mungkin mengembalikan keputusan yang lebih baik.
...
### Langkah 1: Petakan masalah kepada litar kuantum dan operator {#step-1-map-the-problem-to-quantum-circuits-and-operators-1}

Di sini kami juga menyediakan Hamiltonian satu-qubit yang menggunakan asas STO-6G dan transformasi Jordan-Wigner, dengan pengurangan qubit menggunakan simetri Hamiltonian. Perhatikan bahawa kami menggunakan jarak atom antara dua atom hidrogen sebesar `0.735` $\mathring A$.
...
### Langkah 2: Optimumkan untuk perkakasan sasaran {#step-2-optimize-for-target-hardware-1}
...
### Langkah 3: Jalankan pada perkakasan sasaran {#step-3-execute-on-target-hardware-1}
...
### Langkah 4: Pasca-proses {#step-4-post-process-1}
...
Gunakan [hukum Coulomb](https://en.wikipedia.org/wiki/Coulomb%27s_law) dan [unit atomik](https://en.wikipedia.org/wiki/Atomic_units) untuk memastikan Anda mendapat nilai dalam `Hartree`.
...
## Kira tenaga tindak balas $H + H = H_2$ {#compute-reaction-energy-of-h-h-h-2}

Sekarang mari gunakan apa yang kita perolehi! Anda telah menggunakan VQE, penyelesai eigen kuantum variasi, untuk mengira tenaga keadaan asas atom $H$ dan molekul $H_2$. Yang tinggal ialah menggunakan nilai yang dikira untuk mendapatkan tenaga tindak balas proses $H+H=H_2$.

Tenaga tindak balas ialah perubahan tenaga yang berlaku apabila bahan-bahan bertindak balas untuk membentuk bahan baru. Bayangkan Anda sedang membina sesuatu: kadang-kadang Anda perlu memasukkan tenaga ke dalamnya (seperti menindih blok), dan kadang-kadang tenaga dilepaskan (seperti bola menggelinding ke bawah bukit). Dalam kimia, tindak balas sama ada menyerap tenaga (endotermik) atau melepaskan tenaga (eksotermik).
...
VQE adalah algoritma kuantum jangka pendek yang berkuasa, tetapi penting untuk menyedari hadnya. Prestasi VQE sangat bergantung kepada pilihan ansatz – mencari ansatz yang boleh disediakan dengan cekap yang boleh mewakili keadaan asas sebenar dengan tepat menjadi mencabar untuk molekul yang lebih besar dan lebih kompleks. Tambahan pula, perkakasan kuantum semasa mudah terdedah kepada hingar, yang boleh mempengaruhi ketepatan hasil VQE, terutamanya untuk litar yang lebih dalam atau bilangan qubit yang lebih besar. Walaupun menghadapi cabaran ini, VQE berfungsi sebagai algoritma asas, dan penyelidikan berterusan sedang meneroka kaedah variasi yang lebih canggih dan teknik mitigasi ralat untuk menolak sempadan apa yang mungkin dalam kimia kuantum pada komputer kuantum jangka pendek. Sebagai contoh, algoritma seperti Sample-based Quantum Diagonalization (SQD) sedang dibangunkan, yang memanfaatkan sampel yang diperoleh daripada litar kuantum digabungkan dengan pendiagonalan klasik dalam subruang untuk meningkatkan anggaran tenaga dan mengatasi beberapa batasan yang dihadapi oleh VQE, terutamanya berkaitan kecekapan pengukuran dan keteguhan terhadap hingar.
...
1. Prinsip variatif menyatakan bahawa nilai jangkaan tenaga bagi sebarang fungsi gelombang percubaan sentiasa lebih besar daripada atau sama dengan tenaga keadaan asas sebenar.
...
4. Dalam VQE, litar kuantum berparameter (ansatz) digunakan untuk menyediakan fungsi gelombang percubaan.
...
- C) Untuk mengoptimumkan litar kuantum
...
- A) Untuk mencari tenaga keadaan asas suatu Hamiltonian
...
- C) Untuk mengurangkan kerumitan litar
...
- B) Ansatz perlu disesuaikan dengan sistem anda untuk memastikan ia boleh menjana keadaan yang hampir dengan keadaan asas.
...
Terdapat cabaran intrinsik dalam memilih ansatz variatif yang tepat. Ansatz berinspirasi kimia (seperti UCCSD) memberikan ketepatan fizikal tetapi memerlukan litar yang dalam, manakala ansatz cekap perkakasan mempunyai litar yang lebih cetek tetapi mungkin kurang interpretasi fizikal. Selain itu, banyak ansatz memperkenalkan parameter variatif berlebihan yang menyumbang sedikit kepada peningkatan ketepatan tetapi meningkatkan kesukaran pengoptimuman dengan ketara.
...
Berbanding dengan bergantung pada litar qubit tunggal yang mudah dan ansatz yang ringkas, kerumitan pengoptimuman meningkat apabila litar kuantum yang lebih besar dan ansatz berstruktur lebih kompleks diperlukan. Ini menyerlahkan cabaran yang sudah diketahui dalam VQE: overhed pengoptimum.
