...
Berikut adalah contoh untuk observable berikut:
...
Kita akan men-transpil Circuit menggunakan preset pass manager dengan tahap pengoptimuman 3, dan kita akan menggunakan susun atur yang sepadan kepada observable.
...
[SSVQE](https://arxiv.org/abs/1810.09434) adalah varian VQE yang membolehkan mendapatkan $k$ nilai eigen pertama bagi observable $\hat{H}$ dengan nilai eigen $\{\lambda_0, \lambda_1,...,\lambda_{N-1}\}$, di mana $N\geq k$. Tanpa kehilangan keumuman, kita andaikan bahawa $\lambda_0<\lambda_1<...<\lambda_{N-1}$. SSVQE memperkenalkan idea baharu dengan menambah pemberat untuk membantu mengutamakan pengoptimuman bagi sebutan dengan pemberat terbesar.
...
- Nilaikan fungsi kos $C(\vec\theta_{i,j}) := \sum_{j=0}^{k-1} w_j \langle \psi_{j}(\vec{\theta}) | \hat{H} | \psi_{j}(\vec{\theta}) \rangle$ untuk semua keadaan yang disediakan pada komputer kuantum.
  - Ini boleh dipisahkan kepada pengiraan nilai jangkaan bagi observable $\langle \psi_{j}(\vec{\theta}) | \hat{H} | \psi_{j}(\vec{\theta}) \rangle$ dan darab keputusan itu dengan $w_j$.
...
Anda akan membina semula fungsi kos SSVQE dalam penilaian, tetapi kami mempunyai petikan berikut untuk memotivasikan penyelesaian anda:
...
[VQD](https://arxiv.org/abs/1805.08138) adalah kaedah berulang yang melanjutkan VQE untuk mendapatkan $k$ nilai eigen pertama bagi observable $\hat{H}$ dengan nilai eigen $\{\lambda_0, \lambda_1,...,\lambda_{N-1}\}$, di mana $N\geq k$, bukan hanya yang pertama sahaja. Untuk selebihnya bahagian ini, kita akan andaikan, tanpa kehilangan keumuman, bahawa $\lambda_0\leq\lambda_1\leq...\leq\lambda_{N-1}$. VQD memperkenalkan konsep kos penalti untuk membimbing proses pengoptimuman.
...
Sebagai alternatif, masalah baharu ini boleh ditafsirkan sebagai menjalankan VQE pada observable baharu:
...
Seperti kes sebelumnya, masalah baharu ini juga boleh ditafsirkan sebagai VQE dengan observable:
...
Secara analog, untuk mendapatkan nilai eigen ke-$k$ iaitu $\lambda_{k-1}$, anda akan meminimumkan fungsi kos:
...
Masalah ini bersamaan dengan VQE dengan observable:
...
Seperti yang anda dapat lihat dari proses ini, untuk mendapatkan nilai eigen ke-$k$, anda memerlukan keadaan eigen (anggaran) bagi $k-1$ nilai eigen sebelumnya, jadi anda perlu menjalankan VQE sebanyak $k$ kali. Oleh itu, fungsi kos VQD adalah seperti berikut:
...
- Nilaikan fungsi kos $C_k(\vec{\theta})$, yang melibatkan pengiraan $k$ keadaan teruja dan tatasusunan $\beta$ yang mentakrifkan penalti pertindanan untuk setiap sebutan pertindanan.
  - Kira nilai jangkaan bagi observable $\langle \psi_{j}(\vec{\theta}) | \hat{H} | \psi_{j}(\vec{\theta}) \rangle$ untuk setiap $k$
...
Kita akan memperkenalkan observable yang perlu dianggarkan. Dalam pelajaran seterusnya kita akan menambah konteks fizikal kepada ini, seperti keadaan teruja molekul. Mungkin berguna untuk memikirkan observable ini sebagai Hamiltonian bagi suatu sistem yang boleh mempunyai keadaan teruja, walaupun observable ini tidak dipilih untuk sepadan dengan mana-mana molekul atau atom tertentu.
