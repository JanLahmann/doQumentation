...
- Pertama, kita akan belajar tentang [primitif IBM Quantum](/guides/primitives)
...
- Menentukan strategi pengukuran dengan primitif IBM Quantum untuk mengoptimumkan kelajuan berbanding ketepatan
...
Jika anda tertanya-tanya cara mengukur sistem kuantum dan apa yang boleh diukur, Qiskit menawarkan dua <DefinitionTooltip definition="Operasi asas dan fundamental atau jenis data. Qiskit mempunyai primitif Sampler dan Estimator yang berfungsi sebagai blok binaan untuk membina beban kerja algoritma yang kompleks dengan mudah.">primitif</DefinitionTooltip> yang boleh membantu:
...
Walau bagaimanapun, mengira nilai jangkaan sesuatu boleh cerapan tidak selalunya boleh dilakukan, kerana kita sering tidak tahu asas eigennya. IBM Quantum [`Estimator`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2) menggunakan proses algebra yang kompleks untuk menganggar nilai jangkaan pada peranti kuantum sebenar dengan menguraikan boleh cerapan tersebut kepada gabungan boleh cerapan lain yang asas eigennya kita sudah ketahui.
...
Mungkin anda tertanya-tanya cara mendapatkan matriks $V$ dan nilai eigen $\Lambda$. Jika anda sudah mempunyai nilai eigen, maka tidak perlu menggunakan komputer kuantum kerana matlamat algoritma variatif adalah untuk mencari nilai eigen $\hat{H}$ tersebut.

Nasib baik, ada jalan keluarnya: mana-mana matriks $2^n \times 2^n$ boleh ditulis sebagai gabungan linear bagi $4^n$ hasil darab tensor bagi $n$ matriks Pauli dan identiti, yang semuanya adalah Hermitian dan unitari dengan $V$ dan $\Lambda$ yang diketahui. Inilah yang dilakukan oleh primitif IBM Quantum `Estimator` secara dalaman dengan menguraikan mana-mana objek [`Operator`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Operator) kepada [`SparsePauliOp`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.SparsePauliOp).
...
Mari kita pertimbangkan contoh mudah untuk mencari keadaan asas sesuatu sistem. Objektif kita adalah untuk meminimumkan nilai jangkaan bagi boleh cerapan yang mewakili tenaga (Hamiltonian $\hat{\mathcal{H}}$):
...
Kita akan meneruskan dengan menjalankan pada komputer kuantum sebenar. Perhatikan perubahan sintaks. Langkah-langkah yang melibatkan pass_manager akan dibincangkan lebih lanjut dalam contoh seterusnya. Satu langkah yang amat penting dalam algoritma variasi ialah penggunaan sesi Perkhidmatan Pengkomputeran Kuantum IBM Quantum. Memulakan sesi membolehkan anda menjalankan berbilang iterasi algoritma variasi tanpa menunggu dalam giliran baru setiap kali parameter dikemas kini. Ini penting jika masa giliran panjang dan/atau banyak iterasi diperlukan. Perhatian bahawa pengguna Open Plan tidak dapat menggunakan sesi Quantum Compute. Jika anda tidak mempunyai akses kepada sesi, anda boleh mengurangkan bilangan iterasi yang anda hantar pada satu masa, dan simpan parameter terkini untuk digunakan dalam larian masa depan. Jika anda menghantar terlalu banyak iterasi atau menghadapi masa giliran yang terlalu panjang, anda mungkin akan menemui kod ralat 1217, yang merujuk kepada kelewatan panjang antara penghantaran kerja.
...
Untuk menyelesaikan masalah ini dengan komputer kuantum, kita akan menyatakan fungsi kos sebagai nilai jangkaan boleh cerapan. Walau bagaimanapun, boleh cerapan yang Qiskit terima secara asli terdiri daripada operator Pauli, yang mempunyai nilai eigen $1$ dan $-1$ bukannya $0$ dan $1$. Itulah sebabnya kita akan membuat penggantian pemboleh ubah berikut:
...
Dengan kata lain, pemboleh ubah $z_i$ akan setara dengan get $Z$ yang bertindak pada qubit $i$. Tambahan pula:
...
$$
Operator ini ialah gabungan linear bagi sebutan dengan operator Z pada nod yang dihubungkan oleh tepi (ingat bahawa qubit ke-0 berada paling kanan): $IIZZ + IZIZ + IZZI + ZIIZ + ZZII$. Setelah operator dibina, ansatz untuk algoritma QAOA boleh dibina dengan mudah menggunakan litar `QAOAAnsatz` daripada pustaka litar Qiskit.
...
Dengan Estimator IBM Quantum yang secara langsung menerima Hamiltonian dan ansatz berparameter, serta mengembalikan tenaga yang diperlukan, fungsi kos untuk instance QAOA adalah agak mudah:
...
Kita boleh menggunakan pilihan penindasan ralat dan mitigasi ralat dalam primitif IBM Quantum untuk menangani bunyi bising dan memaksimumkan utiliti komputer kuantum hari ini.

### Penindasan Ralat {#error-suppression}

[Penindasan ralat](/guides/error-mitigation-and-suppression-techniques) merujuk kepada teknik yang digunakan untuk mengoptimumkan dan mengubah litar semasa proses kompilasi bagi meminimumkan ralat. Ini adalah teknik pengendalian ralat asas yang biasanya mengakibatkan sedikit <DefinitionTooltip definition="Kos tambahan yang diperkenalkan oleh teknik baharu, berbanding pelaksanaan asas.">overhed</DefinitionTooltip> pra-pemprosesan klasik kepada masa jalan keseluruhan. Overhed tersebut termasuk transpilasi litar untuk dijalankan pada perkakasan kuantum dengan cara:
...
Litar di atas boleh menghasilkan nilai jangkaan sinusoidal bagi boleh cerapan yang diberikan, dengan syarat kita memasukkan fasa yang merentangi selang yang sesuai, seperti $[0,2\pi]$.
...
Kita kini boleh menggunakan pass manager untuk mentranspilasi litar ke dalam "seni bina set arahan" atau ISA bagi backend. Ini adalah keperluan baharu dalam Perkhidmatan Pengkomputeran Kuantum IBM Quantum: semua litar yang diserahkan kepada backend mesti mematuhi kekangan sasaran backend, bermakna ia mesti ditulis dalam sebutan ISA backend — iaitu, set arahan yang boleh difahami dan dilaksanakan oleh peranti. Kekangan sasaran ini ditentukan oleh faktor seperti gate asas natif peranti, ketersambungan qubit-nya, dan — apabila relevan — spesifikasi masa arahan denyut dan lain-lain.

Perhatikan bahawa dalam kes ini, kita akan melakukannya dua kali: sekali dengan optimization_level = 0, dan sekali lagi dengan ia ditetapkan kepada 3. Setiap kali kita akan menggunakan Estimator primitive untuk menganggar nilai jangkaan boleh cerapan pada nilai fasa yang berbeza.
...
[Mitigasi ralat](/guides/error-mitigation-and-suppression-techniques) merujuk kepada teknik yang membolehkan pengguna mengurangkan ralat litar dengan memodelkan bunyi bising peranti pada masa pelaksanaan. Biasanya, ini mengakibatkan overhed pra-pemprosesan kuantum berkaitan latihan model dan overhed pasca-pemprosesan klasik untuk memitigasi ralat dalam keputusan mentah menggunakan model yang dijana.

Pilihan `resilience_level` dalam primitif IBM Quantum menentukan tahap daya tahan terhadap ralat yang hendak dibina. Tahap yang lebih tinggi menghasilkan keputusan yang lebih tepat dengan mengorbankan masa pemprosesan yang lebih lama akibat overhed pensampelan kuantum. Tahap daya tahan boleh digunakan untuk mengkonfigurasi pertukaran antara kos dan ketepatan apabila menggunakan mitigasi ralat pada pertanyaan primitive anda.
...
Untuk kursus ini, kita akan meneroka model mitigasi ralat ini pada tahap tinggi untuk menggambarkan mitigasi ralat yang boleh dilakukan oleh primitif IBM Quantum tanpa memerlukan butiran pelaksanaan penuh.
...
1. Perkuatkan bunyi bising litar untuk beberapa faktor bunyi bising

2. Jalankan setiap litar yang telah diperkuatkan bunyi bisingnya
...
Kita boleh menetapkan ini dengan `options.resilience_level = 2`. Kita boleh mengoptimumkan ini lebih lanjut dengan meneroka pelbagai `noise_factors`, `noise_amplifiers`, dan `extrapolators`, tetapi ini di luar skop kursus ini. Kami menggalakkan anda untuk bereksperimen dengan [pilihan-pilihan ini seperti yang dihuraikan di sini](/guides/error-mitigation-and-suppression-techniques).
...
### Menggunakan pilihan dalam Quantum Compute untuk mitigasi dan penindasan {#using-the-options-in-quantum-compute-for-mitigation-and-suppression}

Berikut adalah cara untuk mengira nilai jangkaan sambil menggunakan mitigasi ralat dan penindasan dalam Perkhidmatan Pengkomputeran Kuantum IBM Quantum. Kita boleh menggunakan litar dan boleh cerapan yang sama seperti sebelumnya, tetapi kali ini memastikan tahap pengoptimuman tetap pada tahap 2, dan kini menala _daya tahan_ atau teknik mitigasi ralat yang digunakan. Proses mitigasi ralat ini berlaku beberapa kali sepanjang gelung pengoptimuman.
...
- Cara memanfaatkan primitif IBM Quantum untuk memitigasi dan menindas bunyi bising
