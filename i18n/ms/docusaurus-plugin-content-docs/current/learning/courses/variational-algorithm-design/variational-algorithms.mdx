...
2. **Sediakan ansatz**: Untuk mula mengoptimumkan secara berulang dari keadaan lalai $|0\rangle$ ke keadaan sasaran $|\psi(\vec\theta)\rangle$, kita mesti mentakrifkan *bentuk variasional* $U_V(\vec\theta)$ untuk mewakili koleksi keadaan berparameter bagi algoritma variasional kita untuk diterokai.
   ...
   Kita merujuk mana-mana kombinasi tertentu keadaan rujukan dan bentuk variasional sebagai ansatz, supaya: $U_A(\vec\theta) := U_V(\vec\theta) U_R$. Ansatze akhirnya akan mengambil bentuk litar kuantum berparameter yang mampu membawa keadaan lalai $|0\rangle$ ke keadaan sasaran $|\psi(\vec\theta)\rangle$.
...
Dengan pelajaran ini, anda telah mempelajari pandangan peringkat tinggi tentang algoritma variasional. Sepanjang pelajaran-pelajaran berikut, kita akan meneroka setiap langkah dengan lebih terperinci, dan pertukaran yang berkaitan dengannya.
