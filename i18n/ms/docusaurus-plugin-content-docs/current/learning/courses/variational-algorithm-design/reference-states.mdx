...
Untuk mendapatkan keadaan ini daripada $|000\rangle$, salah satu pendekatan ialah menggunakan [get Hadamard](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.HGate) pada qubit $0$ ($H_0$), get [CNOT (CX)](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.CXGate) dengan qubit $0$ sebagai qubit kawalan dan qubit $1$ sebagai qubit sasaran ($CNOT_{01}$), dan akhirnya get $X$ yang diterapkan pada qubit $2$ ($X_2$).
...
![Rajah litar operator unitary yang menyediakan keadaan rujukan.](/learning/images/courses/variational-algorithm-design/reference-states/reference-circuit.svg)
