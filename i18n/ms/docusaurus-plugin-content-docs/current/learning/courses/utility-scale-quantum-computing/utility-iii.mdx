---
description: 'Pelajaran ini adalah tentang membina litar GHZ untuk 20 qubit atau lebih supaya, semasa pengukuran, kesetiaan keadaan GHZ melebihi 0.5.'
...
# Eksperimen berskala utiliti III {#utility-scale-experiment-iii}

<Admonition type="note">

Toshinari Itoko, Tamiya Onodera, Kifumi Numata (19 Julai 2024)

[Muat turun pdf](https://ibm.ent.box.com/public/static/fw1538dogvyv0qbfqg8tan1k2fs27mfv.zip) kuliah asal. Perhatikan bahawa beberapa coretan kod mungkin telah lapuk kerana ini adalah imej statik.

*Anggaran masa QPU untuk menjalankan eksperimen pertama ini ialah 12 m 30 s. Terdapat eksperimen tambahan di bawah yang memerlukan kira-kira 4 m.*

(Nota: buku nota ini mungkin tidak dapat dinilai dalam masa yang dibenarkan pada Pelan Terbuka. Pastikan anda menggunakan sumber pengkomputeran kuantum dengan bijak.)
...
Mari kita ulas secara ringkas keadaan GHZ, dan jenis taburan yang mungkin anda jangkakan daripada ```Sampler``` yang digunakan pada satu keadaan tersebut. Kemudian kita akan nyatakan matlamat pelajaran ini dengan jelas.

###  1.1 Keadaan GHZ {#11-ghz-state}

Keadaan GHZ (Greenberger-Horne-Zeilinger state) untuk $n$ qubit ditakrifkan sebagai

$$
\frac{1}{\sqrt 2}(|0\rangle ^ {\otimes n}+ |1\rangle^ {\otimes n})
$$

Secara semula jadi, ia boleh dicipta untuk 6 qubit dengan Circuit kuantum berikut.
...
Kedalamannya tidak terlalu besar, walaupun anda tahu daripada pelajaran sebelumnya bahawa anda boleh melakukan lebih baik. Mari pilih Backend dan transpil Circuit ini.
...
Sekali lagi kedalaman dua qubit selepas transpilasi tidak terlalu besar. Tetapi untuk bekerja dengan keadaan GHZ pada lebih banyak qubit, anda jelas perlu memikirkan tentang mengoptimumkan Circuit. Mari jalankan ini menggunakan ```Sampler``` dan lihat apa yang dikembalikan oleh komputer kuantum sebenar.
...
Ini adalah hasil Circuit GHZ 6 qubit. Seperti yang anda lihat, keadaan semua $|0\rangle$ dan semua $|1\rangle$ memang mendominasi, tetapi ralat adalah ketara. Mari cuba lihat seberapa besar Circuit GHZ yang boleh anda buat dengan peranti Eagle, sambil masih mendapat keputusan di mana keadaan yang betul sekurang-kurangnya lebih daripada 50% berkemungkinan.

### 1.2 Matlamat anda {#12-your-goal}

Bina Circuit GHZ untuk 20 qubit atau lebih supaya, semasa pengukuran, **kesetiaan keadaan GHZ anda > 0.5.**
Nota:
- Anda perlu menggunakan peranti Eagle (`min_num_qubits=127`) dan tetapkan bilangan shots sebagai 40,000.
- Anda perlu melaksanakan Circuit GHZ menggunakan fungsi `execute_ghz_fidelity`, dan mengira kesetiaan menggunakan fungsi `check_ghz_fidelity_from_jobs`.

Ini dimaksudkan sebagai latihan bebas, di mana anda memanfaatkan apa yang telah anda pelajari setakat ini dalam kursus ini.
...
Dalam buku nota ini, kita akan menerapkan tiga strategi untuk mencipta keadaan GHZ yang baik menggunakan 16 qubit dan 30 qubit. Pendekatan ini dibina berdasarkan strategi yang telah anda ketahui daripada pelajaran sebelumnya.

## 2. Strategi 1. Pemilihan qubit Sedar Hingar {#2-strategy-1-noise-aware-qubit-selection}

Kita mula dengan menentukan satu backend. Oleh kerana kita akan bekerja secara intensif dengan sifat-sifat sebuah backend tertentu, adalah lebih baik untuk menetapkan satu backend sahaja berbanding menggunakan pilihan ```least_busy```.
...
Kita akan membina Circuit yang melibatkan banyak gate dua-Qubit. Adalah masuk akal untuk menggunakan qubit yang mempunyai ralat paling rendah semasa melaksanakan gate dua-Qubit tersebut. Mencari "rantai qubit" terbaik berdasarkan ralat gate 2q yang dilaporkan merupakan masalah yang tidak mudah. Namun, kita boleh menakrifkan beberapa fungsi untuk membantu kita menentukan qubit terbaik yang perlu digunakan.
...
Kita akan menggunakan fungsi-fungsi di atas untuk mencari semua laluan mudah N qubit antara semua pasang nod dalam graf (Rujukan: [all_pairs_all_simple_paths](https://www.rustworkx.org/apiref/rustworkx.all_pairs_all_simple_paths.html#rustworkx-all-pairs-all-simple-paths)).

Kemudian, menggunakan fungsi `path_fidelity` yang dibuat di atas, kita akan mencari rantai qubit terbaik yang mempunyai kesetiaan laluan paling tinggi.
...
Mari kita plotkan rantai qubit terbaik, yang ditunjukkan dalam warna merah jambu, dalam rajah peta gandingan.
...
### 2.1 Bina Circuit GHZ pada rantai qubit terbaik {#21-build-a-ghz-circuit-on-the-best-qubit-chain}
Kita pilih satu qubit di tengah-tengah rantai untuk mengaplikasikan gate H terlebih dahulu. Ini sepatutnya mengurangkan kedalaman Circuit hingga kira-kira separuh.
...
Berhati-hati untuk melaksanakan sel seterusnya selepas status kerja di atas menjadi 'DONE', bagi memaparkan keputusan menggunakan fungsi `check_ghz_fidelity_from_jobs`.
...
Keputusan ini tidak memenuhi kriteria. Mari kita beralih ke idea seterusnya.

## 3. Strategi 2. Pokok qubit yang seimbang {#3-strategy-2-balanced-tree-of-qubits}
Idea seterusnya ialah mencari pokok qubit yang seimbang. Dengan menggunakan pokok berbanding rantaian, kedalaman Circuit sepatutnya menjadi lebih rendah. Sebelum itu, kita buang nod dengan ralat bacaan yang "teruk" dan tepi dengan ralat gate yang "teruk" daripada graf penyambungan.
...
Mari kita lukis graf peta penyambungan tanpa tepi yang teruk dan qubit yang teruk.
...
Kita cuba mencipta keadaan GHZ 16-Qubit seperti sebelumnya.

```python
N = 16
```

Kita panggil fungsi `betweenness_centrality` untuk mencari qubit bagi nod akar. Nod dengan nilai kepusatan kebersamaan (betweenness centrality) tertinggi berada di pusat graf. Rujukan: https://www.rustworkx.org/tutorial/betweenness_centrality.html

Atau anda boleh memilihnya secara manual.
...
Bermula dari nod akar, jana pokok menggunakan carian mengikut lebar (BFS). Rujukan: https://qiskit.org/ecosystem/rustworkx/apiref/rustworkx.bfs_search.html#rustworkx-bfs-search
...
Mari kita plotkan qubit yang dipilih, ditunjukkan dalam warna merah jambu, dalam gambar rajah peta penyambungan.
...
Mari kita tunjukkan struktur pokok bagi qubit.
...
Kedalaman Circuit kini telah menjadi jauh lebih rendah berbanding dengan struktur rantaian.
...
Kita berjaya lulus kriteria dengan struktur pokok yang seimbang!
...
Sekarang, mari kita cuba mencipta keadaan GHZ yang lebih besar: keadaan GHZ 30-Qubit.

### 3.1 N = 30 {#31-n-30}

Kita akan mengikuti kerangka [corak Qiskit](/guides/intro-to-patterns).
...
- Langkah 4: Proses keputusan pasca-pelaksanaan


#### Langkah 1: Petakan masalah kepada Circuit dan pengendali kuantum serta Langkah 2: Optimumkan untuk perkakasan sasaran {#step-1-map-problem-to-quantum-circuits-and-operators-and-step-2-optimize-for-target-hardware}

Di sini kita pilih nod akar secara manual.
...
### 3.2 Pilih nod akar yang berbeza secara manual {#32-select-a-different-root-node-manually}
...
Sungguh mengejutkan, walaupun kedalaman pokok meningkat dari 5 ke 6, kedalaman dua-Qubit berkurang dari 9 ke 8! Jadi mari kita gunakan Circuit yang terakhir ini.

#### Langkah 3: Laksanakan pada perkakasan sasaran {#step-3-execute-on-target-hardware}
...
#### Langkah 4: Proses keputusan pasca-pelaksanaan {#step-4-post-process-results}
...
Seperti yang anda lihat, keputusan ini tidak memenuhi kriteria.
...
## 4. Strategi 3. Jalankan dengan pilihan penindasan ralat {#4-strategy-3-run-with-the-error-suppression-options}
Anda boleh tetapkan pilihan penindasan ralat dalam Sampler V2. Rujuk panduan [Pengurusan hingar Sampler](/guides/sampler-noise-management) dan rujukan API [`ExecutionOptionsV2`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-execution-options-v2) untuk maklumat lanjut.
...
Keputusan telah bertambah baik tetapi masih belum memenuhi kriteria.

Kita telah melihat tiga idea setakat ini. Anda boleh menggabungkan dan mengembangkan idea-idea ini atau anda boleh mencetuskan idea sendiri untuk mencipta Circuit GHZ yang lebih baik. Sekarang mari kita semak semula matlamat tersebut.

## 5. Matlamat anda (ringkasan semula) {#5-your-goal-recap}

Bina Circuit GHZ untuk 20 qubit atau lebih supaya keputusan pengukuran memenuhi kriteria: Kesetiaan keadaan GHZ anda > 0.5.

- Anda perlu menggunakan peranti Eagle (seperti `ibm_brisbane`) dan tetapkan bilangan tembakan sebagai 40,000.

- Anda perlu melaksanakan litar GHZ menggunakan fungsi `execute_ghz_fidelity`, dan mengira fidelity menggunakan fungsi `check_ghz_fidelity_from_jobs`.
Anda perlu mencari litar GHZ - qubit terbesar yang memenuhi kriteria. Tulis kod anda di bawah, tunjukkan hasilnya dengan fungsi `check_ghz_fidelity_from_jobs` .

Sekarang kita laksanakan aliran kerja GHZ yang sama seperti dalam bahan sebelumnya, tetapi pada peranti Heron. Ini memberi anda pengalaman dengan susun atur dan ciri-ciri pemproses Heron. Tiada strategi baru diperkenalkan.

*Anggaran masa QPU untuk menjalankan eksperimen seterusnya ini ialah 4 minit 40 saat.*
...
Tahniah! Anda telah menyelesaikan pengenalan kepada pengkomputeran kuantum berskala utiliti! Anda kini bersedia untuk memberi sumbangan yang bermakna dalam pencarian kelebihan kuantum! Terima kasih kerana menjadikan IBM Quantum&reg; sebahagian daripada perjalanan kuantum peribadi anda.

## Kaji selidik pasca-kursus {#post-course-survey}

Tahniah kerana berjaya menamatkan kursus ini! Luangkan masa untuk membantu kami memperbaiki kursus kami dengan mengisi [kaji selidik ringkas](https://your.feedback.ibm.com/jfe/form/SV_5vvmdT5yFFetkgK) berikut. Maklum balas anda akan digunakan untuk meningkatkan kandungan dan pengalaman pengguna kami. Terima kasih!
