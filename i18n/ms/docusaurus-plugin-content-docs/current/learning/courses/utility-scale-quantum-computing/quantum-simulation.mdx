...
description: "Kursus ini mengenai simulasi kuantum, termasuk Trotterisasi untuk Hamiltonian medan Ising transversal"
...
[Muat turun pdf](https://ibm.ent.box.com/s/kzzsmxhw38vph1ioohczaet53euwi310) kuliah asal. Perhatikan bahawa beberapa coretan kod mungkin sudah lapuk kerana ianya imej statik.

*Anggaran masa QPU untuk menjalankan eksperimen ini ialah 7 saat.*

(Buku nota ini sebahagian besarnya diambil daripada [buku nota tutorial](https://github.com/qiskit-community/qiskit-algorithms/blob/main/docs/tutorials/13_trotterQRTE.ipynb) untuk Qiskit Algorithms yang kini sudah tidak aktif.)
...
### 1.2 Import pustaka {#12-import-the-libraries}
...
## 2. Pemetaan masalah anda {#2-mapping-your-problem}

### 2.1 Mendefinisikan Hamiltonian Ising medan transversal {#21-defining-the-transverse-field-ising-hamiltonian}

Di sini kita mempertimbangkan model Ising medan transversal 1-D.

Pertama, kita akan membuat fungsi yang menerima parameter sistem $N$, $J$, $h$ dan $\alpha$, dan mengembalikan Hamiltonian kita sebagai `SparsePauliOp`. Sebuah [SparsePauliOp](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.SparsePauliOp) ialah perwakilan jarang bagi pengendali dalam sebutan [Pauli](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Pauli) berwajaran.
...
### 2.2 Tetapkan parameter simulasi evolusi masa {#22-set-the-parameters-of-the-time-evolution-simulation}
...
### 2.3 Sediakan litar kuantum 1 (Keadaan awal) {#23-prepare-the-quantum-circuit-1-initial-state}
...
### 2.4 Sediakan litar kuantum 2 (litar tunggal untuk evolusi masa) {#24-prepare-the-quantum-circuit-2-single-circuit-for-time-evolution}

Di sini kita membina satu litar untuk satu langkah masa menggunakan Lie–Trotter.

Formula hasil Lie (peringkat pertama) dilaksanakan dalam kelas [LieTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter). Formula peringkat pertama terdiri daripada penghampiran yang dinyatakan dalam pengenalan, di mana eksponen matriks bagi satu jumlah dihampiri dengan hasil darab eksponen matriks:
...
### 2.5 Tetapkan pengendali untuk diukur {#25-set-the-operators-to-measure}
...
### 2.6 Jalankan simulasi evolusi masa {#26-perform-time-evolution-simulation}

Kita akan memantau tenaga (nilai jangkaan Hamiltonian), magnetisasi (nilai jangkaan pengendali magnetisasi), dan korelasi spin purata (nilai jangkaan pengendali korelasi spin purata). `StatevectorEstimator` (EstimatorV2) Qiskit menganggar nilai jangkaan bagi kuantiti boleh cerap (observable), $\langle\psi\vert\hat{O}\vert\psi\rangle$.
...
### 2.7 Plot evolusi masa bagi kuantiti boleh cerap (observable) {#27-plot-the-time-evolution-of-the-observables}
...
## 3. Latihan 1. Lakukan simulasi menggunakan Suzuki–Trotter peringkat kedua {#3-exercise-1-perform-simulation-using-second-order-suzuki-trotter}
...
Suzuki-Trotter peringkat kedua boleh digunakan dalam Qiskit melalui [kelas SuzukiTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.SuzukiTrotter). Menggunakan formula ini, penguraian peringkat kedua ialah:

$$
e^{H_1+H_2} \approx e^{H_1/2}e^{H_2}e^{H_1/2}
$$
### 3.1 Bina litar untuk satu langkah masa {#31-construct-a-circuit-for-a-single-time-step}
Gunakan product_formula_st2 (SuzukiTrotter(order=2)) dan bina litar untuk satu langkah masa menggunakan Suzuki–Trotter peringkat kedua. Selain itu, kira bilangan gate dan kedalaman litar serta bandingkan dengan Lie–Trotter.
...
### 3.2 Lakukan simulasi evolusi masa {#32-perform-time-evolution-simulation}
...
### 3.3 Plot keputusan Suzuki–Trotter peringkat kedua {#33-plot-the-second-order-suzuki-trotter-results}
...
### 3.4 Bandingkan dengan keputusan tepat {#34-compare-with-exact-results}
...
## 4. Melaksanakan pada perkakasan kuantum {#4-executing-on-the-quantum-hardware}
...
### 4.1  Langkah 1. Petakan input klasik kepada masalah kuantum {#41-step-1-map-classical-inputs-to-a-quantum-problem}
...
#### Bina litar untuk eksperimen perkakasan {#construct-circuit-for-hw-experiment}
...
### 4.3 Langkah 3. Laksanakan dengan primitif Qiskit Runtime {#43-step-3-execute-with-qiskit-runtime-primitives}
...
### 4.4 Langkah 4. Proses keputusan selepas pelaksanaan {#44-step-4-post-process-results}
...
Di sini kita menunjukkan contoh pembinaan litar menggunakan Suzuki–Trotter peringkat lebih tinggi (peringkat keempat).
Sekarang mari kita cuba membina simulasi litar dengan Suzuki–Trotter peringkat keempat mengikuti contoh yang ditunjukkan di atas.

Suzuki–Trotter peringkat keempat boleh digunakan dalam Qiskit melalui [kelas SuzukiTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.SuzukiTrotter). Peringkat keempat boleh dinilai menggunakan hubungan rekursi berikut. Perhatikan bahawa peringkat Suzuki–Trotter dilambangkan sebagai "2k" dalam persamaan berikut.
...
$$
#### Bina litar untuk satu langkah masa {#construct-a-circuit-for-a-single-time-step}
Gunakan product_formula_st4 (SuzukiTrotter(order=4)) dan bina litar untuk satu langkah masa menggunakan Suzuki–Trotter peringkat keempat. Juga, kira bilangan gate dan kedalaman litar serta bandingkan dengan Lie–Trotter dan Suzuki–Trotter peringkat kedua.
