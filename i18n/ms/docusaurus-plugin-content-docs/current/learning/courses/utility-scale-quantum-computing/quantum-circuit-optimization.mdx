...
### 2.1 Tahap pengoptimuman {#21-optimization-level}
`optimization_level` mempunyai empat pilihan, dari 0 hingga 3. Semakin tinggi tahap pengoptimuman, semakin banyak usaha pengiraan yang dibelanjakan untuk mengoptimumkan litar. Tahap 0 tidak melakukan sebarang pengoptimuman dan hanya melakukan jumlah kerja yang minimum untuk menjadikan litar boleh dijalankan pada backend yang dipilih. Tahap 3 membelanjakan jumlah usaha yang paling banyak (dan biasanya masa jalan) untuk cuba mengoptimumkan litar. Tahap 1 adalah tahap pengoptimuman lalai.
Kita mentranspilasi litar tanpa pengoptimuman (`optimization_level=0`) dan dengan pengoptimuman (`optimization_level=2`).
Kita melihat perbezaan besar dalam panjang litar bagi litar yang telah ditranspilasi.
...
Seterusnya kita membandingkan keputusan menjalankan dua litar penyediaan keadaan GHZ 5-qubit ($\frac{1}{\sqrt{2}} \left( |00000\rangle + |11111\rangle \right)$) yang disintesis secara berbeza.
...
Apa yang telah kita lakukan dalam litar baharu boleh dilihat sebagai sintesis litar "sedar kekangan gandingan" secara manual. Dengan kata lain: menyelesaikan sintesis litar dan pemetaan litar secara manual pada masa yang sama.
...
Secara amnya, sintesis litar bergantung pada aplikasi dan ia terlalu sukar bagi sesuatu perisian untuk merangkumi semua aplikasi yang mungkin. Transpiler Qiskit kebetulan tidak mempunyai fungsi untuk mensintesis litar penyediaan keadaan GHZ. Dalam kes sedemikian, sintesis litar secara manual seperti yang ditunjukkan di atas patut dipertimbangkan.
Dalam bahagian ini, kita meneliti butiran cara transpiler Qiskit berfungsi menggunakan litar contoh mainan berikut.
...
### 3.1 Lukis keseluruhan aliran transpilasi Qiskit {#31-draw-the-entire-qiskit-transpilation-flow}
...
### 3.2 Lukis peringkat individu {#32-draw-an-individual-stage}
...
### 3.4 Peringkat Pengoptimuman {#34-optimization-stage}
...
Kita mentranspilasi litar tanpa pengoptimuman (`optimization_level=0`) dan dengan pengoptimuman (`optimization_level=2`).
Seperti yang anda lihat, terdapat perbezaan besar dalam panjang litar bagi litar yang telah ditranspilasi.
...
### 4.4 Penguraian gate 1-Qubit Umum {#44-general-1-qubit-gate-decomposition}
...
### 4.5 Pengoptimuman Blok 1-Qubit {#45-one-qubit-block-optimization}
...
### 4.6 Penguraian Toffoli {#46-toffoli-decomposition}
...
### 4.7 Penguraian gate CU {#47-cu-gate-decomposition}
...
### 4.8 CX, ECR, CZ Setara Hingga Clifford Setempat {#48-cx-ecr-cz-equal-up-to-local-cliffords}
