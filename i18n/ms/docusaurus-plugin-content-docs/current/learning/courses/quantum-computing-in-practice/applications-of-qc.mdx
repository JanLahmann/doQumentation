...
{/* doqumentation-source-hash: 1b17b8ec */}
...
## Kesukaran klasik vs. kuantum {#classical-vs-quantum-difficulty}
...
### Kelas kerumitan {#complexity-classes}
...
### Komputer kuantum hari ini dan hadnya {#todays-quantum-computers-and-their-limits}
...
## Kawasan Aplikasi dan Kes Penggunaan {#application-areas-and-use-cases}
...
1.	Simulasi alam semula jadi. Kaedah klasik semasa bagi simulasi atom dan molekul terhad disebabkan oleh penerangan matematik struktur atom yang tidak cekap. Menyimpan dan memanipulasi keadaan kuantum memerlukan sumber yang bertambah secara eksponen pada komputer klasik tetapi boleh dilakukan dengan cekap pada komputer kuantum. Ini boleh membawa kepada pembangunan dalam penangkapan karbon dioksida, bateri alternatif, atau penciptaan ubat baharu. Beberapa algoritma yang amat relevan dalam bidang ini ialah: Variational Quantum Eigensolver (VQE), yang digunakan untuk menganggarkan sifat tertentu suatu bahan, seperti keadaan keseimbangan atau tenaga minimum; algoritma Time Dynamics Simulation (TDS), yang digunakan untuk menganggarkan fungsi respons atau sifat spektrum bahan; dan pendatang baharu, [Sample-based Quantum Diagonalization (SQD)](/learning/courses/quantum-diagonalization-algorithms/sqd-overview), yang kami rasa akan kita dengar lebih banyak lagi dalam masa terdekat.


2.	Pengoptimuman. Bidang ini terdapat di mana-mana dalam pengkomputeran, jadi kes penggunaannya banyak dan pelbagai. Beberapa contoh yang sering kita dengar ialah pengoptimuman portfolio dalam kewangan, reka bentuk industri, dan pengedaran serta rantaian bekalan. Algoritma paling biasa yang mungkin anda akan dengar berkaitan kewangan ialah yang telah kita bincangkan dengan agak mendalam: quantum approximate optimization algorithm, atau QAOA.


3.	Pembelajaran mesin kuantum. Bidang ini telah menjana banyak kegemparan dalam beberapa tahun kebelakangan ini, tetapi berkemungkinan QML tidak akan berguna secepat simulasi. Namun begitu, terdapat beberapa algoritma yang mengagumkan sedang dibangunkan untuk menangani beberapa kes penggunaan yang sangat penting. Sesetengah kes penggunaan yang mungkin ini ialah pemprosesan bahasa semula jadi, analisis trafik rangkaian, dan juga pengesanan penipuan dalam transaksi kewangan. Algoritma yang relevan dalam bidang ini ialah quantum support vector machine (QSVM), quantum neural networks (QNN), dan quantum generative adversarial networks.
...
### Kes Penggunaan 1: Mensimulasi dinamik hadron {#use-case-1-simulating-hadron-dynamics}
...
Jika langkah-langkah itu terdengar agak biasa, tolak bahagian paket gelombang hadron, itu kerana langkah-langkah ini sangat mirip dengan apa yang kita liputi dalam contoh QAOA dalam pelajaran sebelumnya. Kita mulai dalam keadaan yang biasa (di sini keadaan vakum), dan kemudian kita biarkan ia berevolusi dalam masa dengan satu siri Hamiltonian yang dieksponenkan. Banyak algoritma variasi mengikuti pendekatan umum ini. Perbezaan besar di sini, bagaimanapun, adalah bahawa kita mencipta paket gelombang hadron yang berpusat dalam litar kita, sebelum kita mula membiarkannya berevolusi.

Jadi, bagaimana kita mencipta paket gelombang? Pada vakum, hadron boleh dirangsang dengan mencipta pasangan fermion-antifermion pada tapak bersebelahan. Dengan menyediakan superposisi hadron seperti itu di lokasi yang berbeza, paket gelombang sewenang-wenang boleh disediakan. Para penulis memusatkan paket gelombang mereka di tengah-tengah litar untuk memerhati evolusi tanpa mencapai sempadan.

Tetapi ingat: nama permainan ketika bekerja dengan QPU yang bising adalah memastikan kedalaman litar boleh diurus. Untuk melakukan ini, protokol SC-ADAPT-VQE menggunakan simetri dan hierarki dalam skala panjang untuk menentukan litar kuantum kedalaman rendah untuk penyediaan keadaan. Ini akan mencipta ansatz dengan bilangan parameter yang lebih kecil, dan oleh itu, kedalaman yang lebih cetek.
...
### Kes Penggunaan 3: Ramalan struktur sekunder mRNA {#use-case-3-mrna-secondary-structure-prediction}
