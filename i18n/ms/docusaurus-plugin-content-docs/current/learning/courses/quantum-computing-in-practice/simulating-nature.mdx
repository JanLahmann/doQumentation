...
sidebar_label: 'Mensimulasi alam semula jadi'
title: 'Mensimulasi alam semula jadi'
---
...
Tonton video ini daripada Olivia Lanes tentang mensimulasi alam semula jadi dengan komputer kuantum, atau buka video dalam tetingkap berasingan di [YouTube.](https://www.youtube.com/watch?v=7cbNnxDHqeQ&list=PLOFEBzvs-VvoZxe2ClFy27yOt6VzsTEJK&index=7)
...
Kepentingan ceramah itu berkembang pada tahun 1980-an apabila teknologi maju. Semasa tempoh ini, Feynman kembali kepada idea-idea ini dalam ceramah penting lain di Caltech, membentangkan makalah yang bertajuk "Simulating Nature with Computers." Di sana, beliau mengemukakan soalan yang berani: bolehkah komputer digunakan untuk melakukan simulasi tepat yang meniru tingkah laku alam semula jadi pada peringkat kuantum? Feynman mencadangkan bahawa, daripada bergantung pada penghampiran kasar untuk memodelkan proses atom, kita boleh menggunakan komputer yang memanfaatkan undang-undang mekanik kuantum itu sendiri—bukan sekadar untuk memodelkan alam semula jadi, tetapi untuk menirukannya.
...
Di hujung yang bertentangan terdapat masalah yang sangat mencabar yang menuntut mesin kuantum tahan ralat sepenuhnya — teknologi yang masih belum tersedia. Nasib baik, banyak masalah simulasi dipercayai terletak di suatu tempat di tengah-tengah garis masa ini, dalam lingkungan di mana komputer kuantum hari ini sudah boleh digunakan dengan berkesan. Terdapat banyak sebab untuk teruja dan tertarik dengan prospek ini, kerana mensimulasi alam semula jadi membentuk asas untuk pelbagai aplikasi yang menjanjikan.

Maklumat berikut merangkumi aliran kerja umum dalam simulasi alam semula jadi dan kemudian contoh khusus aliran kerja untuk mereplikasi keputusan dari kajian yang terkenal.

## Aliran kerja umum {#general-workflow}
...
Apabila mensimulasikan zarah-zarah tak boleh dibezakan bagi fermion atau boson, adalah perlu untuk menggunakan transformasi pengekodan. Zarah-zarah ini diterangkan dalam kerangka matematik khas yang dipanggil `second quantization`, yang menjejaki nombor pekerjaan setiap keadaan kuantum dengan memperkenalkan operator `creation` dan `annihilation`, di mana operator penciptaan $\hat{a}_i^\dagger$ menambah satu zarah kepada keadaan $i$ manakala operator pemusnahan $\hat{a}_i$ mengeluarkan satu zarah daripada keadaan $i$. Berdasarkan kerangka pengkuantuman kedua ini, fermion boleh ditransformasikan oleh Bravyi-Kitaev dan Jordan-Wigner. Transformasi Jordan-Wigner mentakrifkan operator penciptaan fermionik
$\hat{f}_j^\dagger = \Bigl( \prod\limits_{k \lt j}{(-Z_k)} \Bigr)\Bigl( \frac{X_j + i Y_j}{2} \Bigr)$ yang mengisi keadaan kuantum ke-$j$ dengan fermion dan operator pemusnahan fermionik $\hat{f}_j= \Bigl( \prod\limits_{k \lt j}{(-Z_k)} \Bigr)\Bigl( \frac{X_j - i Y_j}{2} \Bigr)$ yang mengosongkan fermion daripada keadaan ke-$j$. Anda boleh mendapatkan maklumat lanjut mengenai transformasi Jordan-Wigner ini di [Quantum Computing in Practice, episode 5 - Mapping](/learning/courses/quantum-computing-in-practice/mapping) kami. Begitu juga, boson juga memerlukan kaedah pengekodan tersendiri, seperti transformasi Holstein-Primakoff, untuk diwakili oleh qubit.
...
## Mereplikasi makalah "Utiliti" {#replicating-the-utility-paper}
...
### Langkah corak Qiskit 1: Peta {#qiskit-patterns-step-1-map}
...
### Langkah corak Qiskit 2: Optimumkan {#qiskit-patterns-step-2-optimize}
...
### Langkah corak Qiskit 3: Laksanakan menggunakan primitif {#qiskit-patterns-step-3-execute-using-primitives}
...
### Langkah corak Qiskit 4: Pasca-proses {#qiskit-patterns-step-4-post-process}
...
Tahniah atas melengkapkan kursus ini! Luangkan masa untuk membantu kami menambah baik kursus kami dengan mengisi [tinjauan ringkas](https://your.feedback.ibm.com/jfe/form/SV_cLU5FElCNVJOhfw) berikut. Maklum balas anda akan digunakan untuk meningkatkan tawaran kandungan dan pengalaman pengguna kami. Terima kasih!
