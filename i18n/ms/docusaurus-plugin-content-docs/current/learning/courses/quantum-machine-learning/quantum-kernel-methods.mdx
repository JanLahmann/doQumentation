...
Kira kebarangkalian mengukur $|0\rangle^{\otimes N}$ pada litar kuantum, dan isikan matriks kernel pada kedudukan yang sepadan dengan dua vektor data yang digunakan. Untuk mengisi keseluruhan matriks kernel, kita perlu menjalankan eksperimen kuantum untuk setiap entri. Setelah kita mempunyai matriks kernel, kita boleh menggunakannya dalam banyak algoritma pembelajaran mesin klasik yang menerima `pre-calculated kernels`. Sebagai contoh: `qml_svc = SVC(kernel="precomputed")`. Kita kemudian boleh menggunakan aliran kerja klasik untuk menerapkan model kita pada data pengujian kita, dan mendapatkan skor ketepatan. Bergantung pada kepuasan kita terhadap skor ketepatan kita, kita mungkin perlu menyemak semula aspek pengiraan kita, seperti peta ciri kita.
...
### Langkah 2: Optimumkan masalah untuk pelaksanaan kuantum {#step-2-optimize-problem-for-quantum-execution-1}
...
### Langkah 4: Pasca-proses, kembalikan keputusan dalam format klasik {#step-4-post-process-return-result-in-classical-format-1}
...
Mari kita andaikan bahawa seseorang yang baru dalam pengkomputeran kuantum cuba menggunakan litar kuantum yang mempunyai kedalaman dua-qubit yang sangat tinggi, dan mereka tidak menggunakan mitigasi ralat. Mari kita andaikan selanjutnya bahawa ini menghasilkan kadar ralat sebanyak 10% pada setiap qubit. Jika elemen matriks kernel yang benar (tanpa ralat) yang sepadan dengan litar ini sangat besar, katakan 1.0, apakah kebarangkalian untuk mengukur semua 10 qubit berada dalam keadaan dengan setiap qubit $|0\rangle$?
