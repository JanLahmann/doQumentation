...
Di sini, empat qubit pertama mewakili ciri pertama, empat qubit berikutnya mewakili ciri kedua, dan empat qubit terakhir mewakili ciri ketiga. Kod di bawah menukar vektor data (5,7,0) kepada keadaan kuantum, dan digeneralisasikan untuk berbuat demikian bagi ciri digit tunggal yang lain.
...
DAE boleh digeneralisasikan kepada fungsi sewenang-wenang bagi dua ciri tersebut daripada fungsi sinusoid yang digunakan di sini. Ini dipanggil pengekodan qubit umum[\[7\]](#references).
...
Peta ciri Pauli (PFM) ialah generalisasi (pengitlakan) ZFM dan ZZFM untuk menggunakan get Pauli sewenang-wenang. Peta ciri Pauli mengambil bentuk yang sangat serupa dengan dua peta ciri sebelumnya. Untuk $r$ pengulangan pengekodan $N$ ciri vektor $\vec{x},$

$$
\mathscr{U}_{\text{PFM}}(\vec{x}) = \prod_{s=1}^{r} U(\vec{x}) H^{\otimes n}.
$$

Untuk PFM, $U(\vec{x})$ digeneralisasikan (diperluaskan) kepada operator unitari pengembangan Pauli. Di sini kami membentangkan bentuk peta ciri yang lebih umum yang dipertimbangkan setakat ini:
