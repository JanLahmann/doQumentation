...
Satu cara untuk memahami operator uniter di atas adalah melalui analogi dengan propagator dalam sistem fizikal. Uniter di atas adalah operator evolusi uniter, $\exp(it\mathcal{H})$, bagi Hamiltonian, $\mathcal{H}$, yang serupa dengan model Ising, di mana parameter masa, $t$, digantikan dengan nilai data untuk memacu evolusi. Pengembangan operator uniter ini menghasilkan litar PFM. Ketersambungan keterikatan dalam $S$ boleh ditafsirkan sebagai gandingan Ising dalam kekisi spin.
Mari kita pertimbangkan contoh operator Pauli $Y$ dan $XX$ yang mewakili interaksi jenis Ising tersebut. Qiskit menyediakan kelas `pauli_feature_map` untuk membuat instans PFM dengan pilihan get satu- dan $n$-qubit, yang dalam contoh ini akan dihantar sebagai rentetan Pauli `‘Y’` dan `‘XX’`. Biasanya, $n$ adalah 1 atau 2 untuk interaksi satu- dan dua-qubit, masing-masing. Skema keterikatan adalah "linear," bermakna hanya qubit jiran terdekat dalam litar kuantum yang digandingkan. Ambil perhatian bahawa ini tidak sepadan dengan qubit jiran terdekat pada komputer kuantum itu sendiri, kerana litar kuantum ini adalah lapisan abstraksi.
