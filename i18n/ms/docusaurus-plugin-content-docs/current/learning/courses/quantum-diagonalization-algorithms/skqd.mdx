...
{/* doqumentation-source-hash: b248f1bb */}
...
Satu langkah penting dalam kaedah pepenjuruan kuantum berasaskan sampel ialah menghasilkan vektor berkualiti untuk subruang. Dalam pelajaran sebelumnya, kita menggunakan ansatz LUCJ untuk menghasilkan vektor subruang bagi Hamiltonian kimia. Dalam pelajaran ini, kita akan menggunakan keadaan Krylov kuantum[\[1\]](#references) seperti yang dibincangkan dalam pelajaran 2. Pertama, kita akan meninjau semula cara mencipta ruang Krylov pada komputer kuantum menggunakan operasi evolusi masa. Kita kemudian akan mengambil sampel daripadanya. Kita akan mengunjurkan Hamiltonian sistem ke atas subruang yang disampel dan mempenjurukannya untuk menganggarkan tenaga keadaan dasar. Algoritma ini terbukti dan secara cekap menumpu kepada keadaan dasar, di bawah andaian yang diterangkan dalam pelajaran 2.
...
## 1. Petakan masalah kepada Circuit kuantum dan operator {#1-map-problem-to-quantum-circuits-and-operators}
...
Ruj [\[2\]](#references) menunjukkan bahawa langkah masa yang cukup kecil untuk KQD ialah $\pi / \vert \vert H \vert \vert$, dan bahawa adalah lebih baik untuk menganggar rendah nilai ini daripada menganggar tinggi. Sebaliknya, memilih $dt$ yang terlalu kecil membawa kepada pengkondisian yang lebih buruk bagi subruang Krylov, kerana vektor asas Krylov berbeza kurang dari langkah masa ke langkah masa. Selain itu, walaupun pilihan $dt$ ini terbukti mencukupi untuk penumpuan SKQD, dalam konteks berasaskan sampel ini pilihan optimum $dt$ dalam amalan adalah topik kajian yang berterusan. Dalam pelajaran ini, kita menetapkan $dt = 0.15$.
...
Akhirnya, kita perlu memetakan operator evolusi masa kepada litar kuantum. Ini telah dilakukan dalam pelajaran 2, tetapi di sini kita akan memanfaatkan kaedah dalam Qiskit, khususnya kaedah bernama _synthesis_. Terdapat kaedah yang berbeza untuk mensintesis operator matematik kepada litar kuantum dengan get kuantum. Banyak teknik sedemikian terdapat dalam [Qiskit synthesis module](https://docs.quantum.ibm.com/api/qiskit/synthesis). Kita akan menggunakan pendekatan [`LieTrotter`](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter) untuk sintesis [\[3\]](#references) [\[4\]](#references).
...
Seterusnya, kita memplot tenaga yang dikira sebagai fungsi dimensi Krylov dan membandingkannya dengan tenaga tepat. Tenaga tepat dikira secara berasingan menggunakan kaedah Brute force klasik. Kita dapat melihat bahawa anggaran tenaga keadaan dasar menumpu dengan peningkatan dimensi ruang Krylov. Walaupun dimensi Krylov sebanyak $5$ adalah terhad, keputusan masih menunjukkan penumpuan yang mengagumkan, yang dijangka bertambah baik dengan dimensi Krylov yang lebih besar [\[1\]](#references).
...
<Accordion>
<AccordionItem title="Apakah yang boleh dilakukan untuk meningkatkan penumpuan dalam plot di atas?">
...
<AccordionItem title="Apakah kelebihan utama SKQD berbanding (a) SQD, dan (b) KQD?">

__Answer:__

Apakah kelebihan utama SKQD berbanding (a) SQD, dan (b) KQD?

   (a) SKQD disertakan dengan jaminan penumpuan yang tiada pada SQD. Dalam SQD anda sama ada perlu membuat tekaan yang sangat baik untuk ansatz anda yang mempunyai pertindihan yang cemerlang dengan sokongan keadaan dasar dalam asas pengiraan, atau anda perlu memperkenalkan komponen variasi kepada pengiraan untuk mensampel satu keluarga ansaetze.

   (a) SKQD dilengkapi dengan jaminan penumpuan yang tidak dimiliki SQD. Dalam SQD anda sama ada perlu membuat tekaan yang sangat baik untuk ansatz anda yang mempunyai pertindihan yang sangat baik dengan sokongan keadaan asas dalam asas pengkomputeran, atau anda perlu memperkenalkan komponen variasional ke dalam pengiraan untuk membuat sampel keluarga ansaetze.
