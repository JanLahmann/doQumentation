...
Terangkan mengapa ia tidak (a) berguna, dan (b) mungkin untuk melanjutkan dimensi subruang Krylov $r$ melebihi dimensi $N$ matriks yang diminati.

<Accordion>
<AccordionItem title="Jawapan">

(a) Oleh kerana kita mengortonormalkan vektor semasa kita menghasilkannya, satu set $N$ vektor sedemikian akan membentuk asas lengkap, bermakna gabungan linear daripadanya boleh digunakan untuk mencipta sebarang vektor dalam ruang tersebut.
...
Andaikan seorang penyelidik rakan sedang menunjukkan kaedah Krylov yang digunakan pada matriks kecil percubaan. Adakah terdapat sesuatu yang salah dengan pilihan matriks $A$ dan vektor awal $|\psi\rangle$ mereka?
...
Rakan sekerja anda telah secara tidak sengaja memilih vektor eigen untuk vektor awalnya. Bertindak dengan matriks pada vektor awal hanya akan mengembalikan vektor yang sama, diskalakan oleh nilai eigen. Ini tidak akan menghasilkan subruang dengan dimensi yang semakin meningkat. Nasihatkan rakan sekerja anda untuk memilih vektor awal yang berbeza, dengan memastikan ia bukan vektor eigen.

</AccordionItem>
</Accordion>

Gunakan kaedah Krylov pada matriks yang diberikan, dengan memilih vektor awal baharu yang sesuai. Tuliskan anggaran nilai eigen minimum pada peringkat ke-0 dan ke-1 subruang Krylov anda.
...
$$
Untuk mendapatkan $|v_1\rangle$ kita menggunakan $A$ sekali pada $|v_0\rangle$, dan kemudian menjadikan $|v_1\rangle$ ortogon kepada $|v_0\rangle.$
$$
...
$$
Pada peringkat ke-0, unjuran ke atas subruang Krylov kita ialah
...
$$
Pada peringkat ke-1, unjuran ke atas subruang Krylov ini ialah
...
$$
Ini boleh dilakukan secara manual, tetapi paling mudah dilakukan menggunakan numpy:
...
Rujuk plot yang dilicinkan bagi masa pepenjuruan berbanding dimensi matriks di atas.

(a) Pada kira-kira dimensi matriks manakah kaedah Krylov menjadi lebih pantas, menurut plot ini?

(b) Aspek pengiraan yang manakah boleh mengubah dimensi di mana kaedah Krylov menjadi lebih pantas?
...
$$
Di sini $P_\alpha$ ialah rentetan Pauli berbentuk $P_\alpha \sim IZIXII...YZXIX$ atau satu set rentetan Pauli sedemikian yang berkomut antara satu sama lain. Memandangkan kita boleh menulis $H$ sebagai jumlah operator yang boleh diukur sedemikian, ungkapan berikut untuk elemen matriks $\tilde{H}$ boleh direalisasikan menggunakan primitif Estimator IBM Quantum.
...
Bermula dari Langkah 4 di atas, kenakan get Hadamard $H$ pada qubit sifar.
...
Satu-satunya perbezaan antara litar yang digunakan dalam ujian Hadamard ialah fasa dalam operator evolusi masa dan observable yang diukur. Oleh itu kita boleh menyediakan litar templat yang mewakili litar generik untuk ujian Hadamard, dengan ruang tempat untuk gate yang bergantung pada operator evolusi masa.
...
### 4.3 Langkah 3. Laksanakan menggunakan primitif IBM Quantum {#43-step-3-execute-using-an-ibm-quantum-primitive}
