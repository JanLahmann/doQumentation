...
![Imej orbital atom dan imej rangkaian banyak nod dan sambungan antara mereka.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig1.svg)
...
![Imej litar kuantum dengan parameter variasi berlabel "theta".](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig2.svg)
...
Sebelum kita beralih ke proses variasi itu, perhatikan bahawa sering berguna untuk memulakan keadaan anda dari keadaan "tekaan yang baik". Anda mungkin tahu cukup tentang sistem anda untuk membuat tekaan awal yang lebih baik daripada $|0\rangle^{\otimes N}$. Sebagai contoh, adalah biasa untuk memulakan qubit kepada keadaan Hartree-Fock dalam aplikasi kimia. Tekaan permulaan yang tidak mengandungi sebarang parameter variasi ini dipanggil __keadaan rujukan__. Mari kita panggil litar kuantum yang digunakan untuk mencipta keadaan rujukan $U_{ref}$. Setiap kali penting untuk membezakan keadaan rujukan daripada ansatz yang selebihnya, gunakan: $U_{\text{ansatz}}(\vec{\theta}) =U_{\text{var}}(\vec{\theta})U_{\text{ref}}.$ Secara setara
...
![Imej yang menunjukkan garis melengkung yang mewakili tenaga dengan beberapa titik di mana nilai sedang diuji untuk mencari minimum.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig3.svg)
...
Tetapi kita tidak boleh mengukur semua operator Pauli tersebut serentak. Operator Pauli (kecuali I) tidak berkomut antara satu sama lain jika mereka dikaitkan dengan qubit yang sama. Sebagai contoh, kita boleh mengukur `IZIZ` dan `ZZXZ` serentak, kerana kita boleh mengukur I dan Z serentak untuk qubit ketiga, dan kita boleh mengetahui I dan X serentak untuk qubit pertama. Tetapi kita tidak boleh mengukur `ZZZZ` dan `ZZZX` serentak, kerana Z dan X tidak berkomut, dan keduanya bertindak pada qubit ke-0. Pembaca yang berpengalaman mungkin ingat bahawa dua kumpulan operator Pauli mungkin berkomut sebagai set walaupun pengukuran qubit individu tidak berkomut. Estimator mengandaikan pengukuran Pauli hasil tensor (melalui putaran asas), yang sepadan dengan pengelompokan operator yang berkomut mengikut qubit. Jadi untuk menganggar dua rentetan (A dan B) operator Pauli secara serentak menggunakan Estimator, operator Pauli bagi setiap qubit dalam A dan B mestilah berkomut. Ini bermakna kita juga tidak boleh mengukur `ZZZZ` dan `ZZXX` serentak.

![Jadual rentetan Pauli yang berbeza, ada yang berkomut dan ada yang tidak.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig4.svg)
...
![Dua imej garis melengkung dengan nilai minimum. Dalam satu, titik diperiksa secara rawak dalam pencarian minimum, dalam yang lain kecerunan dianggar dengan melukis garis antara dua titik bersebelahan.](/learning/images/courses/quantum-diagonalization-algorithms/vqe/vqe-fig5.svg)
...
Ia boleh dilakukan dalam 4 kumpulan. Perhatikan bahawa penyelesaian sedemikian biasanya tidak unik.
...
Yang manakah anda jangka biasanya menyukarkan kimia kuantum dengan VQE: bilangan sebutan dalam Hamiltonian, atau mencari ansatz yang baik?
...
Ansatz ditulis menggunakan gate yang biasa, tetapi litar kita mesti ditranspilasi untuk menggunakan gate asas yang boleh dilaksanakan pada setiap komputer kuantum. Kita pilih Backend yang paling tidak sibuk.
...
Kita kini boleh mentranspilasi litar kita untuk perkakasan ini dan memvisualisasikan ansatz yang telah ditranspilasi.
...
Perhatikan bahawa gate yang digunakan telah berubah, dan qubit dalam litar abstrak kita telah dipetakan kepada qubit bernombor berbeza pada komputer kuantum. Kita mesti memetakan Hamiltonian kita secara identik agar keputusan kita bermakna.
...
* Litar cetek: VQE sering menggunakan litar kuantum yang agak cetek. Ini menjadikan VQE kurang terdedah kepada ralat gate yang terkumpul dan menjadikannya sesuai untuk banyak teknik pengurangan ralat. Sudah tentu, litar tidak selalunya cetek; ini bergantung pada ansatz yang digunakan.
...
* Sifat heuristik: VQE tidak menjamin penumpuan ke tenaga keadaan tanah yang betul, kerana prestasinya bergantung pada pilihan ansatz dan kaedah pengoptimuman[\[1-2\]](#references). Jika ansatz yang lemah dipilih yang tidak mempunyai belitan yang diperlukan untuk keadaan tanah yang diinginkan, tiada pengoptimum klasik yang boleh mencapai keadaan tanah itu.
...
* Kesan hingar: Apabila pengoptimum klasik sedang mencari minimum, pengiraan yang berhingar boleh mengelirukan dan menjauhkannya dari minimum sebenar atau melambatkan penumpuannya. Satu penyelesaian yang mungkin adalah dengan memanfaatkan teknik pengurangan ralat dan penekanan ralat[\[2-3\]](#references) terkini dari IBM.
* Dataran tandus (Barren plateaus): Kawasan-kawasan dengan kecerunan yang menghilang[\[2-3\]](#references) ini wujud walaupun tanpa hingar, tetapi hingar menjadikannya lebih menyusahkan kerana perubahan dalam nilai jangkaan akibat hingar boleh lebih besar daripada perubahan akibat mengemas kini parameter dalam dataran tandus ini.
...
ADAPT-VQE menangani had-had VQE melalui pembinaan ansatz yang adaptif. Berbanding memulakan dengan ansatz tetap, ADAPT-VQE membina ansatz secara dinamik secara berulang. Pada setiap langkah, ia memilih operator dari kumpulan yang telah ditakrifkan terlebih dahulu (seperti operator pengujaan fermionik) yang mempunyai kecerunan terbesar berkenaan tenaga. Ini memastikan hanya operator yang paling berkesan ditambah, menghasilkan ansatz yang padat dan cekap[\[4-6\]](#references). Pendekatan ini boleh memberi beberapa kesan yang bermanfaat:

1. **Kedalaman Litar yang Berkurang**: Dengan mengembangkan ansatz secara berperingkat dan hanya memberi tumpuan kepada operator yang diperlukan, ADAPT-VQE meminimumkan operasi gate berbanding pendekatan VQE tradisional[\[5,7\]](#references).
...
3. **Kebolehskalaan dan Ketahanan Hingar**: Ansatz yang padat mengurangkan pengumpulan ralat gate, mengurangkan overhead pengkomputeran, dan mengehadkan bilangan parameter variasi yang mesti diminimumkan.
...
QPE serupa dalam tujuan dengan VQE, tetapi sangat berbeza dalam pelaksanaan. QPE memerlukan komputer kuantum toleran kesalahan kerana litar kuantumnya yang umumnya dalam dan tahap koherensi tinggi yang diperlukan. Setelah QPE dapat dilaksanakan, ia akan lebih tepat daripada VQE. Satu cara untuk menerangkan perbezaannya adalah melalui ketepatan sebagai fungsi kedalaman litar. QPE mencapai ketepatan $\epsilon$ dengan kedalaman litar yang berskala sebagai $O(1/\epsilon)$ [\[10\]](#references). VQE memerlukan $O(1/\epsilon^2)$ sampel untuk mencapai ketepatan yang sama[\[10,11\]](#references).
...
Jawapan lain yang betul adalah bahawa semuanya paling mudah dilaksanakan apabila Hamiltonian mudah diukur (boleh disusun ke dalam kumpulan operator Pauli berkomut yang agak sedikit).
