...
- [Litar dinamik](/guides/classical-feedforward-and-control-flow)

- [Asas pembetulan ralat kuantum](https://arxiv.org/abs/0905.2794)
...
Dalam tutorial ini, kami menunjukkan kod bit-flip menggunakan eksperimen memori kuantum yang mudah. Kami akan menyediakan qubit yang dikodkan dalam keadaan logik $|\bar{1}\rangle \equiv |111\rangle$ dan kemudian melaksanakan berbilang kitaran masa terbiar (untuk mengumpul ralat) diikuti oleh pengesanan dan pembetulan ralat. Kami kemudian mengukur kebarangkalian ralat logik sebagai fungsi bilangan kitaran (masa), di mana kebarangkalian ralat logik ialah kebarangkalian mendapati qubit dalam keadaan yang tidak memulihkan $|\bar{1}\rangle$ selepas pengukuran akhir (secara konkrit, keadaan yang sepadan dengan ralat ialah $|000\rangle$, $|001\rangle$, $|010\rangle$, dan $|100\rangle$).
...
Nota: kod pengulangan hanya membenarkan pembetulan ralat bit-flip dan oleh itu bukanlah kod pembetulan ralat yang lengkap. Walau bagaimanapun, kerana kesederhanaannya, ia adalah titik permulaan yang baik untuk menggambarkan cara melaksanakan pembetulan ralat pada komputer kuantum. Eksperimen memori di bawah hanya menguji satu jenis ralat ($|\bar{1}\rangle$ yang mereput kepada $|\bar{0}\rangle$) dan secara teknikal hanya menunjukkan perlindungan maklumat klasik.
...
Untuk mengesan ralat semasa litar, kita perlu memilih backend yang mempunyai akses kepada kaedah `MidCircuitMeasure` (lihat [dokumentasi](/guides/execute-dynamic-circuits#mid-circuit-measurements)).
...
#### Bina satu siri litar stabiliser bit-flip yang melaksanakan berbilang pusingan pengesanan dan pembetulan ralat {#build-a-sequence-of-bit-flip-stabilizer-circuits-implementing-multiple-rounds-of-error-detection-and-correction}

Kod bit-flip adalah antara contoh kod penstabil yang paling mudah. Ia melindungi keadaan daripada ralat bit-flip (X) tunggal pada mana-mana qubit pengekodan. Pertimbangkan tindakan ralat bit-flip $X$, yang memetakan $|0\rangle \rightarrow |1\rangle$ dan $|1\rangle \rightarrow |0\rangle$ pada mana-mana qubit kita, maka kita ada $\epsilon = \{E_0, E_1, E_2 \} = \{IIX, IXI, XII\}$. Kod ini memerlukan lima qubit: tiga digunakan untuk mengekodkan keadaan yang dilindungi ("qubit data"), dan dua yang lain digunakan sebagai ancilla pengukuran penstabil.

Di bawah anda akan membina litar yang (1) menyediakan qubit data dalam keadaan logik $|\bar{1} \rangle$, kemudian (2) menjalankan berbilang kitaran kelewatan $5 \mu s$ diikuti oleh pembetulan ralat (termasuk set semula qubit sindrom), dan (3) membaca keadaan qubit data.

Kami juga akan menguji jangka hayat keadaan $|1\rangle$ tanpa menggunakan pembetulan ralat dengan menyertakan tiga qubit rujukan yang kami sediakan dalam keadaan $|1\rangle$, biarkan terbiar, dan kemudian dibaca.
...
### Langkah 2: Optimumkan masalah untuk pelaksanaan hardware kuantum {#step-2-optimize-the-problem-for-quantum-hardware-execution}
...
Bagi qubit rujukan yang akan kami bandingkan dengan memori kuantum logik kami, kami akan memilih qubit yang terbaik yang ada dari segi masa keselarasan redaman amplitud ($T_1$).
...
### Langkah 4: Memproses pasca dan mengembalikan keputusan dalam format klasik yang dikehendaki {#step-4-post-process-and-return-result-in-desired-classical-format}
...
Kami dengan jelas melihat jangka hayat yang lebih baik bagi keadaan $|\bar{1}\rangle$, walaupun berbanding dengan qubit fizikal (tidak dikodkan) yang terbaik. Walau bagaimanapun, ingat bahawa eksperimen ini hanya membetulkan satu jenis ralat, iaitu bit-flip. Bolehkah anda meningkatkan jangka hayat qubit logik? Anda mungkin ingin meneroka pengoptimuman masa kelewatan, meningkatkan skala saiz kod pengulangan melebihi $n=3$, dan sebagainya.
...
- [Kursus asas pembetulan ralat kuantum](/learning/courses/foundations-of-quantum-error-correction) - ketahui lebih lanjut tentang pembetulan ralat kuantum

- [Tutorial pengesanan ralat overhed rendah dengan kod ruang-masa](/tutorials/spacetime-codes) - ketahui cara menggunakan semakan Pauli untuk mengesan ralat dan sampel pasca-pilih
