...
description: "Bandingkan kaedah kompilasi SABRE, transpiler berkuasa AI, dan Rustiq pada litar simulasi Hamiltonian daripada Hamlib."
...
*Anggaran penggunaan: kurang daripada 1 minit pada pemproses IBM Heron (NOTA: Ini adalah anggaran sahaja. Masa jalan sebenar anda mungkin berbeza.)*
...
Selepas melalui tutorial ini, anda akan memahami:
- Cara menggunakan Qiskit transpiler dengan SABRE untuk pengoptimuman susunan dan penghalaan
- Cara memanfaatkan transpiler berkuasa AI untuk pengoptimuman litar yang lebih canggih
- Cara menggunakan plugin Rustiq untuk mensintesis operasi `PauliEvolutionGate` dalam litar simulasi Hamiltonian
- Cara menanda aras dan membandingkan kaedah kompilasi menggunakan kedalaman dua-Qubit, jumlah bilangan gate, dan masa jalan

## Prasyarat {#prerequisites}
Kami cadangkan anda biasa dengan topik berikut sebelum melalui tutorial ini:
- [Konsep transpilasi](/guides/transpile)
- [Peringkat transpiler](/guides/transpiler-stages)
- [Transpil dengan pengurus laluan](/guides/transpile-with-pass-managers)
## Latar belakang {#background}

Kompilasi litar kuantum mengubah algoritma kuantum peringkat tinggi kepada litar fizikal yang mematuhi kekangan perkakasan sasaran. Kompilasi yang berkesan boleh mengurangkan kedalaman litar dan bilangan gate dengan ketara, yang secara langsung mempengaruhi kualiti keputusan pada peranti kuantum jangka pendek.

Tutorial ini menanda aras tiga kaedah kompilasi pada litar simulasi Hamiltonian yang dibina dengan `PauliEvolutionGate`. Litar ini memodelkan interaksi berpasangan qubit (seperti sebutan $ZZ$, $XX$, dan $YY$) dan lazim digunakan dalam kimia kuantum, fizik jirim pekat, dan sains bahan.

Litar penanda aras diambil daripada koleksi [Hamlib](https://github.com/SRI-International/QC-App-Oriented-Benchmarks/tree/master/qedcbench/hamlib#hamlib-simulation---benchmark-program), yang diakses melalui repositori [Benchpress](https://github.com/Qiskit/benchpress). Hamlib menyediakan set Hamiltonian perwakilan yang distandardkan, memungkinkan perbandingan strategi kompilasi pada beban kerja simulasi yang realistik.

### Gambaran keseluruhan kaedah kompilasi {#compilation-methods-overview}

#### Qiskit transpiler dengan SABRE {#qiskit-transpiler-with-sabre}
Qiskit transpiler menggunakan algoritma SABRE (SWAP-based BidiREctional heuristic search) untuk mengoptimumkan susunan dan penghalaan litar. SABRE menumpukan pada meminimumkan gate SWAP dan kesannya terhadap kedalaman litar sambil mematuhi kekangan kesambungan perkakasan. Ini adalah kaedah tujuan umum yang memberikan keseimbangan yang baik antara prestasi dan masa kompilasi. Untuk maklumat lanjut, lihat [\[1\]](https://arxiv.org/abs/2409.08368). Kelebihan dan penerokaan parameter SABRE dibincangkan secara mendalam dalam [tutorial berasingan](/tutorials/transpilation-optimizations-with-sabre).

#### Transpiler berkuasa AI {#ai-powered-transpiler}
Transpiler berkuasa AI menggunakan pembelajaran mesin untuk meramalkan strategi transpilasi optimum dengan menganalisis corak dalam struktur litar dan kekangan perkakasan. Ia juga boleh menggunakan laluan `AIPauliNetworkSynthesis`, yang mensasar litar rangkaian Pauli menggunakan pendekatan sintesis berasaskan pembelajaran pengukuhan. Untuk maklumat lanjut, lihat [\[2\]](https://arxiv.org/abs/2405.13196) dan [\[3\]](https://arxiv.org/abs/2503.14448).

#### Plugin Rustiq {#rustiq-plugin}
Plugin Rustiq menyediakan teknik sintesis lanjutan khusus untuk operasi `PauliEvolutionGate`, yang mewakili putaran Pauli yang biasa digunakan dalam dinamik Trotterisasi. Ia direka untuk menghasilkan penguraian litar berkedalaman rendah bagi beban kerja simulasi Hamiltonian. Untuk maklumat lanjut, lihat [\[4\]](https://arxiv.org/abs/2404.03280).

### Metrik utama {#key-metrics}

Kami membandingkan ketiga-tiga kaedah berdasarkan metrik berikut:
- **Kedalaman dua-Qubit**: Kedalaman litar yang mengira hanya gate dua-Qubit. Ini sering menjadi kesesakan bagi ketepatan pada perkakasan sebenar.
- **Saiz litar (jumlah bilangan gate)**: Jumlah bilangan gate dalam litar yang ditranspilasi.
- **Masa jalan**: Masa jam dinding untuk transpilasi.
...
### Sambung ke backend {#connect-to-a-backend}

Pilih backend yang akan digunakan untuk kedua-dua contoh kecil dan besar. Backend menentukan peta gandingan dan gate asas yang disasarkan oleh transpiler.
...
### Tentukan pengurus laluan {#define-pass-managers}

Sediakan ketiga-tiga kaedah kompilasi.
...
### Tentukan fungsi pembantu {#define-helper-functions}

Fungsi berikut mentranspilasi senarai litar menggunakan pengurus laluan yang diberikan, dan merekod metrik utama (kedalaman dua-Qubit, saiz litar, dan masa jalan) untuk setiap litar.
...
### Muatkan litar Hamiltonian daripada Hamlib {#load-hamiltonian-circuits-from-hamlib}

Kami memuatkan set Hamiltonian perwakilan daripada repositori Benchpress dan membina litar `PauliEvolutionGate`. Litar yang melebihi bilangan Qubit backend dibuang, bersama litar yang saiz terurai melebihi 1,500 gate (untuk memastikan masa transpilasi yang munasabah).
...
Pisahkan litar kepada kumpulan berskala kecil (kurang daripada 20 Qubit) dan berskala besar (20 atau lebih Qubit).
...
Pratonton salah satu litar Hamiltonian berskala kecil sebelum transpilasi.
...
## Contoh berskala kecil {#small-scale-example}

Dalam bahagian ini, kami menanda aras ketiga-tiga kaedah kompilasi pada litar Hamiltonian dengan kurang daripada 20 Qubit. Litar ini ditranspilasi dengan cepat dan memberikan gambaran jelas tentang bagaimana setiap kaedah mengendalikan litar dengan kerumitan sederhana.
### Langkah 1: Petakan input klasikal kepada masalah kuantum {#step-1-map-classical-inputs-to-a-quantum-problem}

Setiap Hamiltonian dikodkan sebagai litar `PauliEvolutionGate`. Litar-litar ini telah dibina dalam bahagian persediaan daripada data penanda aras Hamlib.
### Langkah 2: Optimumkan masalah untuk pelaksanaan perkakasan kuantum {#step-2-optimize-problem-for-quantum-hardware-execution}

Kami mentranspilasi semua litar berskala kecil menggunakan setiap daripada tiga pengurus laluan, kemudian mengumpul metrik.
...
Jadual di bawah merumuskan purata dan sisihan piawai setiap metrik merentasi semua litar berskala kecil, bersama peratusan penambahbaikan berbanding SABRE. Oleh kerana saiz litar berbeza-beza dengan ketara, sisihan piawai memberikan konteks penting untuk mentafsir purata.
...
Jadual per-litar menunjukkan bagaimana setiap kaedah dibandingkan pada litar individu. Nilai terbaik untuk setiap metrik ditandai dengan asterisk. Perhatikan bahawa untuk litar yang paling mudah, ketiga-tiga kaedah sering menumpu kepada hasil yang sama.
...
#### Visualisasi keputusan {#visualize-results}

Plot di bawah membandingkan ketiga-tiga kaedah merentasi setiap metrik berdasarkan per-litar. Litar disusun mengikut bilangan Qubit dan dilabelkan mengikut indeks pada paksi-x, kerana beberapa litar boleh mempunyai bilangan Qubit yang sama.
...
Pada skala ini, ketiga-tiga pengurus laluan berprestasi baik, dan purata keputusan mereka hampir sama. Ini sebahagian besarnya kerana litar kecil meninggalkan ruang terhad untuk pengoptimuman lanjut, jadi kaedah-kaedah cenderung menumpu kepada penyelesaian yang serupa.

Dalam contoh ini, Rustiq menghasilkan keputusan yang paling pelbagai, dengan pencilan terbesar dalam kedalaman dua-Qubit dan bilangan gate. Walaupun kepelbagaian ini bermakna ia kadangkala ketinggalan, ia juga bermakna Rustiq kadangkala menemui penyelesaian yang lebih baik daripada dua kaedah lain. Transpiler AI lebih stabil dalam keputusannya berbanding SABRE dan Rustiq, mengikuti rapat pada kebanyakan litar tanpa banyak pencilan.

Untuk masa jalan, SABRE dan Rustiq kedua-duanya pantas, manakala transpiler berkuasa AI jauh lebih perlahan pada litar tertentu.
#### Kaedah berprestasi terbaik mengikut metrik {#best-performing-method-by-metric}

Carta di bawah menunjukkan seberapa kerap setiap kaedah mencapai nilai terbaik (terendah) untuk setiap metrik. Seri adalah mungkin: untuk litar yang lebih mudah, beberapa kaedah boleh mencapai kedalaman dua-Qubit atau bilangan gate optimum yang sama. Apabila seri berlaku, semua kaedah yang seri menerima kredit, jadi peratusan untuk metrik tertentu mungkin berjumlah lebih daripada 100%.
...
Dalam contoh ini, ketiga-tiga kaedah berprestasi sangat serupa pada litar berskala kecil. Pada kedalaman dua-Qubit dan bilangan gate, bahagian litar di mana setiap kaedah adalah terbaik hampir sama (lebih kurang 35–55%), dan banyak litar berakhir dengan seri kerana litar yang paling mudah sering mempunyai satu penyelesaian optimum yang ditemui oleh beberapa kaedah. Perbezaan yang paling jelas adalah masa jalan: SABRE dan Rustiq masing-masing adalah yang terpantas pada lebih kurang separuh litar, manakala transpiler berkuasa AI jarang menjadi yang tercepat. Mempertimbangkan ketiga-tiga metrik bersama, Rustiq mempunyai kelebihan keseluruhan yang sedikit &nbsp; ia adalah pemenang paling kerap pada kedalaman dua-Qubit dan kekal berdaya saing pada bilangan gate dan masa jalan.
### Langkah 3: Laksanakan menggunakan primitif Qiskit {#step-3-execute-using-qiskit-primitives}

Untuk menilai bagaimana kualiti transpilasi mempengaruhi pelaksanaan di bawah hingar, kami menggunakan teknik **litar cermin**. Untuk setiap litar yang ditranspilasi $U$, kami menambahkan inversnya $U^\dagger$ supaya litar gabungan $U^\dagger U$ adalah identiti secara teori. Bermula dari keadaan $|0\rangle$, pelaksanaan sempurna (tanpa hingar) akan mengembalikan bitstring semua-sifar dengan kebarangkalian 1.

Dalam praktik, ralat gate terkumpul sepanjang litar, jadi kebarangkalian memulihkan $|0\rangle^{\otimes n}$ menurun. Kaedah kompilasi yang menghasilkan litar yang lebih cetek dengan gate yang lebih sedikit akan mengumpul lebih sedikit hingar.

Pendekatan litar cermin ini mudah dan berskala kepada mana-mana saiz litar, kerana output yang dijangka sentiasa $|0\rangle^{\otimes n}$ dan tiada simulasi klasikal keadaan ideal diperlukan. Walau bagaimanapun, perhatikan kaveat berikut: litar cermin adalah proksi untuk litar sebenar (bukan litar itu sendiri), ia menggandakan bilangan gate (yang membesar-besarkan kesan hingar), dan ia boleh meremehkan ralat tertentu apabila hingar membatal secara simetri merentasi sempadan cermin.

Kami memilih litar indeks 6 daripada set berskala kecil dan menjalankan litar cermin pada simulator Aer dengan model hingar depolarisasi mudah.
...
Bina litar cermin (tambahkan $U^\dagger$), petakan semula kepada indeks Qubit berterusan supaya simulator hanya mengendalikan Qubit aktif, dan jalankan pada simulator Aer yang berising.
...
Kaedah dengan kedalaman dua-Qubit paling rendah dan gate paling sedikit mencapai ketepatan paling tinggi, selaras dengan jangkaan bahawa litar yang lebih pendek mengumpul lebih sedikit hingar. Walaupun perbezaan kecil dalam kedalaman dan bilangan gate menghasilkan perbezaan ketepatan yang boleh diukur di bawah model hingar depolarisasi.

Ingat bahawa keputusan ini adalah untuk satu litar. Kedudukan relatif kaedah boleh berubah dari litar ke litar bergantung pada struktur Hamiltonian.
## Contoh perkakasan berskala besar {#large-scale-hardware-example}

Dalam bahagian ini, kami menanda aras tiga kaedah kompilasi yang sama pada litar Hamiltonian dengan 20 atau lebih Qubit. Litar ini lebih mewakili beban kerja simulasi Hamiltonian praktikal dan menguji bagaimana setiap kaedah berskala dari segi kualiti litar dan masa kompilasi.
### Langkah 1-4 digabungkan {#steps-1-4-combined}

Aliran kerja mengikut struktur yang sama seperti contoh berskala kecil. Kami mentranspilasi semua litar berskala besar dengan setiap kaedah, mengumpul metrik, dan menghantar litar cermin ke perkakasan kuantum sebenar.
...
## Analisis keputusan kompilasi {#analysis-of-compilation-results}

Penanda aras di atas membandingkan SABRE, transpiler berkuasa AI, dan Rustiq pada litar simulasi Hamiltonian daripada koleksi Hamlib pada skala kecil dan besar.

### Kedalaman dua-Qubit dan bilangan gate {#two-qubit-depth-and-gate-count}

Pada skala besar, SABRE dan transpiler berkuasa AI adalah dua prestasi terkuat, dan masing-masing memimpin pada metrik yang berbeza. Seperti yang ditunjukkan oleh carta *kaedah berprestasi terbaik mengikut metrik*, SABRE menghasilkan bilangan gate terendah pada majoriti besar litar dan merupakan kaedah yang paling pantas pada hampir semua litar, selaras dengan heuristik yang direka untuk meminimumkan gate SWAP yang disisipkan, dan dengan pengoptimuman terkini pada susunan dan penghalannya. Transpiler berkuasa AI menghasilkan kedalaman dua-Qubit terendah pada kebanyakan litar, selaras dengan bahagian objektif pembelajaran pengukuhannya yang mensasar kedalaman litar. Jadual ringkasan mencerminkan pembahagian yang sama: SABRE mempunyai purata bilangan gate yang lebih rendah, manakala transpiler AI mempunyai purata kedalaman dua-Qubit yang lebih rendah. Kedua-dua kaedah adalah konsisten dan boleh dipercayai merentasi keseluruhan julat litar.

Rustiq, yang dibina khas untuk sintesis `PauliEvolutionGate`, menghasilkan hasil tunggal terbaik hanya pada sebahagian kecil litar berskala besar. Purata metriknya sangat dipengaruhi oleh beberapa pencilan yang ketara, kelihatan sebagai lonjakan besar dalam plot perbandingan kompilasi, di mana Rustiq menghasilkan kedalaman dan bilangan gate yang jauh lebih tinggi daripada kaedah lain. Tanpa pencilan ini, prestasi puratanya akan jauh lebih hampir dengan SABRE dan transpiler berkuasa AI.

Pemerhatian utama ialah tiada satu kaedah yang mendominasi setiap litar. Setiap kaedah mengatasi yang lain dalam kes tertentu, yang menjadikannya berbaloi untuk mencuba semua alat yang tersedia dan memilih hasil terbaik untuk setiap litar.

### Masa jalan {#runtime}

SABRE secara konsisten adalah kaedah yang paling pantas. Rustiq umumnya berjalan pada kelajuan yang serupa, tetapi boleh menghasilkan pencilan di mana kompilasi mengambil masa yang jauh lebih lama. Ini amat ketara dalam keputusan berskala besar, di mana beberapa litar menyebabkan masa jalan Rustiq melonjak. Pencilan ini sangat mempengaruhi purata masa jalan, jadi median mungkin merupakan ringkasan yang lebih mewakili untuk Rustiq. Transpiler berkuasa AI adalah yang paling perlahan daripada tiga kaedah, dengan masa jalan yang berkembang dengan ketara pada litar yang lebih besar dan lebih kompleks.

### Keputusan litar cermin {#mirror-circuit-results}

Eksperimen litar cermin mengesahkan trend yang dijangka: kaedah yang menghasilkan kedalaman dua-Qubit yang lebih rendah dan gate yang lebih sedikit mencapai ketepatan yang lebih tinggi di bawah hingar. Ini berlaku pada kedua-dua simulator berising (berskala kecil) dan perkakasan sebenar (berskala besar).

Ingat bahawa setiap plot litar cermin mencerminkan satu litar, bukan agregat. Contoh perkakasan di atas menggunakan satu litar `tfim` 26-Qubit, yang kebetulan merupakan kes di mana SABRE menghasilkan kedalaman dua-Qubit yang jauh lebih tinggi daripada transpiler berkuasa AI dan Rustiq, jadi ketepatannya jauh lebih rendah. Ini tidak mewakili keputusan keseluruhan: merentasi set penuh litar berskala besar, kedalaman dua-Qubit SABRE biasanya hampir dengan transpiler berkuasa AI, dan kedua-dua kaedah masing-masing memimpin pada metrik yang berbeza (transpiler berkuasa AI pada kedalaman dua-Qubit, SABRE pada bilangan gate dan masa jalan). Satu keputusan cermin menguji versi berganda satu litar dan bukannya beban kerja penuh, jadi ia tidak boleh dibaca sebagai keputusan tentang kualiti keseluruhan kaedah.

### Cadangan {#recommendations}

Tiada satu strategi transpilasi terbaik untuk semua litar. Pilihan terbaik bergantung pada struktur litar, matlamat pengoptimuman, dan belanjawan masa kompilasi yang tersedia:

- **SABRE** adalah lalai yang disyorkan. Ia pantas dan boleh dipercayai, dan menghasilkan keputusan yang kukuh merentasi pelbagai jenis litar. Untuk penalaan lanjut, pengguna boleh meningkatkan percubaan susunan dan penghalaan (lihat [tutorial pengoptimuman SABRE](/tutorials/transpilation-optimizations-with-sabre)).
- **Transpiler berkuasa AI** patut dicuba apabila masa kompilasi bukan kekangan, terutamanya apabila meminimumkan kedalaman dua-Qubit adalah keutamaan: ia menghasilkan kedalaman dua-Qubit terendah pada kebanyakan litar berskala besar dalam penanda aras ini.
- **Rustiq** dibina khas untuk litar `PauliEvolutionGate` dan boleh menemui penyelesaian berkedalaman rendah dan bilangan gate rendah, terutamanya pada litar yang lebih kecil. Pada litar yang lebih besar ia kadangkala boleh menghasilkan keputusan yang jauh lebih besar, jadi penggunaannya adalah terbaik sebagai salah satu daripada beberapa kaedah untuk dicuba dan bukannya sebagai lalai.

Dalam praktik, pendekatan terbaik adalah menjalankan semua kaedah yang tersedia dan memilih hasil terbaik untuk setiap litar. Overhed kompilasi daripada mencuba beberapa kaedah adalah kecil berbanding potensi peningkatan dalam kualiti pelaksanaan pada perkakasan sebenar.
## Langkah seterusnya {#next-steps}

Jika anda mendapati tutorial ini berguna, anda mungkin berminat dengan yang berikut:
...
- [Cipta plugin transpiler](/guides/create-transpiler-plugin)
</Admonition>
## Rujukan {#references}
