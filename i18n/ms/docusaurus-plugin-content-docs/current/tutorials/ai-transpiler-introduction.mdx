...
Mulai **Julai 2025**, **Perkhidmatan Transpiler** telah dipindahkan ke Platform IBM Quantum&reg; baharu dan tidak lagi tersedia. Untuk kemas kini terkini tentang status Perkhidmatan Transpiler, rujuk [dokumentasi perkhidmatan transpiler](/guides/qiskit-transpiler-service). Kamu masih boleh menggunakan transpiler AI secara tempatan, serupa dengan transpilasi Qiskit standard. Cuma gantikan `generate_preset_pass_manager()` dengan `generate_ai_pass_manager()`. Fungsi ini membina pengurus laluan yang mengintegrasikan laluan routing dan sintesis berkuasa AI terus ke dalam aliran kerja transpilasi tempatan kamu.
