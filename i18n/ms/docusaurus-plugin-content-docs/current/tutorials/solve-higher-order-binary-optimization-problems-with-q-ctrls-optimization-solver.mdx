---
description: 'Selesaikan masalah pengoptimuman kuantum skala utiliti menggunakan Optimization Solver, Fungsi Qiskit oleh Q-CTRL Fire Opal'
notebook_path: tutorials/solve-higher-order-binary-optimization-problems-with-q-ctrls-optimization-solver.ipynb
sidebar_label: 'Pengoptimuman binari peringkat tinggi dengan Optimization Solver Q-CTRL'
title: 'Pengoptimuman binari peringkat tinggi dengan Optimization Solver Q-CTRL'
---
...
Qiskit Functions ialah ciri eksperimental yang hanya tersedia untuk pengguna Pelan Premium IBM Quantum&reg;, Pelan Flex, dan Pelan On-Prem (melalui IBM Quantum Platform API). Ia berada dalam status pratonton dan tertakluk kepada perubahan.
</Admonition>
...
Anda juga perlu mendapat akses kepada fungsi Optimization Solver. [Isi borang](https://quantum.cloud.ibm.com/functions?id=q-ctrl-optimization-solver) untuk memohon akses.
...
Takrifkan kelayakan [IBM Quantum Platform](https://quantum.cloud.ibm.com/) anda, yang akan digunakan sepanjang tutorial ini untuk mengesahkan identiti kepada IBM Quantum Compute Service dan Qiskit Functions.
...
## Langkah 1: Takrifkan masalah sebagai fungsi objektif {#step-1-define-the-problem-as-an-objective-function}
Optimization Solver menerima sama ada fungsi objektif atau graf sebagai input. Dalam tutorial ini, masalah minimisasi kaca spin Ising ditakrifkan sebagai fungsi objektif, dan ia telah disesuaikan untuk topologi hex-berat peranti IBM&reg;.
...
Untuk perbincangan terperinci tentang pembinaan definisi masalah dan keputusan terdahulu yang diperoleh daripada Optimization Solver, rujuk [manuskrip teknikal ini](https://arxiv.org/abs/2406.01743). Masalah ini pada asalnya ditakrifkan dan dinilai sebagai sebahagian daripada [kertas yang diterbitkan oleh Makmal Kebangsaan Los Alamos](https://arxiv.org/abs/2312.00997), dan ia telah disesuaikan untuk memanfaatkan lebar peranti penuh pemproses IBM Quantum Heron 156-Qubit.
...
## Langkah 2: Jalankan algoritma hibrid menggunakan Fire Opal Optimization Solver {#step-2-run-the-hybrid-algorithm-using-the-fire-opal-optimization-solver}
...
Solver mengembalikan kamus dengan penyelesaian dan metadata yang berkaitan, seperti rentetan bit penyelesaian, bilangan lelaran, dan pemetaan pemboleh ubah kepada rentetan bit. Untuk definisi penuh input dan output Solver, lihat [dokumentasi](https://docs.quantum.ibm.com/api/functions/q-ctrl-optimization-solver).
...
## Langkah 3: Nilai keputusan {#step-3-evaluate-results}
...
Solver menemui penyelesaian yang betul, yang telah disahkan menggunakan perisian pengoptimuman klasik. Kerumitan masalah skala utiliti ini memerlukan perisian pengoptimuman lanjutan untuk diselesaikan secara klasik, seperti [IBM ILOG CPLEX Optimization Studio (CPLEX)](https://www.ibm.com/products/ilog-cplex-optimization-studio) atau [Gurobi Optimization](https://www.gurobi.com/).
Sebagai analisis visual kualiti keputusan, anda boleh memplot keputusan dengan mengira nilai kos daripada rentetan bit dan kebarangkalian mereka. Untuk perbandingan, plotkan keputusan bersama taburan rentetan bit yang disampel secara rawak, yang bersamaan dengan penyelesaian klasik "kasar". Jika algoritma secara konsisten menemui kos yang lebih rendah, ini menunjukkan algoritma kuantum sedang menyelesaikan masalah pengoptimuman dengan berkesan.
