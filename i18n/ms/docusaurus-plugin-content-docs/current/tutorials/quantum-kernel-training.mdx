...
Untuk melakukan ini, pindahkan kod sumber di atas ke satu fail, `./source/generate_kernel_entry.py`, bungkus kod dalam skrip yang mengambil input dan mengembalikan penyelesaian akhir, dan akhirnya muat naik ke kluster jauh menggunakan kelas `QiskitFunction` dari `Qiskit Serverless`. Untuk panduan tentang menentukan kebergantungan luaran, menghantar argumen input, dan lain-lain, semak [panduan Qiskit Serverless](https://qiskit.github.io/qiskit-serverless/function_features/index.html#function-features).
