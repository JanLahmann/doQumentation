---
description: 'Gunakan Qunova HiVQE untuk mengira lengkung permukaan tenaga potensi (PES) penceraian.'
notebook_path: tutorials/qunova-hivqe.ipynb
sidebar_label: 'Lengkung PES penceraian dengan Qunova HiVQE'
title: 'Lengkung PES penceraian dengan Qunova HiVQE'
---
...
Qiskit Functions ialah ciri eksperimental yang hanya tersedia untuk pengguna Pelan Premium IBM Quantum&reg;, Pelan Flex, dan Pelan On-Prem (melalui API Platform IBM Quantum). Ciri ini berada dalam status pratonton dan tertakluk kepada perubahan.
</Admonition>

*Anggaran penggunaan (NOTA: Ini adalah anggaran sahaja. Masa jalan anda mungkin berbeza.)*

* Li2S: Lima minit masa QPU pada pemproses Heron r2

* FeP-NO: Lima minit masa QPU pada pemproses Heron r2


## Latar Belakang {#background}

Mengira tenaga tindak balas kimia dengan tepat adalah penting untuk kemajuan saintifik dalam sains bahan, kejuruteraan kimia, penemuan ubat, dan bidang-bidang lain. Dalam pelbagai sistem kimia, sistem Li-S telah mendapat perhatian besar untuk memahami dan membangunkan komposisi bateri baharu. Tutorial ini memberikan pengalaman praktikal dalam mengira permukaan tenaga potensi (PES) penceraian ikatan Li-S bagi sistem $Li_2S$ dengan mengeluarkan satu atom litium menggunakan pengiraan HiVQE. Keputusan boleh dibandingkan dengan pengiraan rujukan (CASCI) serta kaedah klasik seperti Hartree-Fock (HF) untuk masalah 20-qubit.

## Keperluan {#requirements}

Pasang kebergantungan berikut untuk menjalankan kod dalam tutorial ini.
...
Untuk menjalankan tutorial ini, import fungsi `qunova/hivqe-chemistry` melalui `QiskitFunctionCatalog`. Anda memerlukan akaun Pelan Premium IBM Quantum, Pelan Flex, atau Pelan On-Prem (API Platform IBM Quantum) dengan lesen daripada Qunova untuk menjalankan fungsi ini.
...
## Bahagian 1: Li2S (20Q) {#part-1-li2s-20q}

### Langkah 1: Petakan input klasik kepada masalah kuantum {#step-1-map-classical-inputs-to-a-quantum-problem}

Tentukan geometri $Li_2S$ dalam format kamus untuk jarak ikatan Li-S yang berbeza bagi mengira lengkung PES. Geometri-geometri ini dioptimumkan menggunakan pengiraan B3LYP/631g.
...
Pengiraan HiVQE akan dilakukan dengan pilihan yang ditentukan di bawah. Menggunakan asas sto3g untuk $Li_2S$, terdapat 19 orbital spatial dengan 22 elektron. Untuk menjalankan kes (10o,10e) dengan pengiraan HiVQE, anda boleh menentukan 10 orbital aktif dan enam orbital beku. Pada setiap iterasi, 100 tembakan akan digunakan untuk mengambil sampel konfigurasi elektron yang dijana oleh litar kuantum ExcitationPreserving (`epa`) dengan jalinan `circular` dan dua ulangan (`reps`). Bilangan iterasi maksimum ditetapkan kepada 30 untuk memastikan penamatan iterasi dengan penumpuan tenaga.
...
### Langkah 2 dan 3: Optimumkan masalah untuk pelaksanaan perkakasan kuantum dan laksanakan menggunakan fungsi HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}

Sediakan gelung `for` untuk menjalankan pengiraan HiVQE dengan geometri menggunakan pilihan yang ditentukan di bawah. Job dihantar dalam gelung `for`. Dalam tutorial ini, anda akan menghantar enam geometri dan mendapatkan semula keputusan apabila semuanya selesai. Dalam jalankan fungsi utama, anda perlu menentukan `max_states` dan `max_expansion_states` untuk mengawal saiz maksimum matriks subruang dan mengawal berapa banyak keadaan yang boleh dijana menggunakan kaedah CI pengembangan klasik setiap iterasi. ID job fungsi akan disimpan dalam kamus dengan setiap label geometri untuk menjejak dan memproses output selanjutnya.
...
Mari semak sama ada semua job masih berjalan atau sudah selesai.
...
Setelah semua job selesai, mari kita dapatkan semula semua keputusan pengiraan.
...
Jumlah masa jalan QPU yang digunakan dalam job boleh dijejaki dengan log masuk ke [IBM Quantum Platform](https://quantum.cloud.ibm.com/) dan melihat job yang dihantar dengan tag `qunova-chemistry-hivqe`.

### Langkah 4: Pasca-proses dan bandingkan dengan kaedah klasik {#step-4-post-process-and-compare-with-classical-methods}

Pengiraan rujukan klasik (CASCI) boleh dijalankan untuk (10o,10e) bagi mengesahkan keputusan HiVQE.
...
### Memplot lengkung penceraian untuk Li\_2S {#plotting-the-dissociation-curve-for-li-2s}

Mari plot dan bandingkan keputusan HiVQE dengan HF dan CASCI.
Anda boleh perhatikan bahawa semua pengiraan HiVQE sepadan dengan baik dengan keputusan rujukan klasik (CASCI).
...
## Bahagian 2: FeP-NO (44Q) {#part-2-fep-no-44q}

Harap maklum bahawa untuk menjalankan fungsi pada sistem FeP-NO dengan tetapan yang ditunjukkan dalam contoh ini, anda memerlukan lesen yang membenarkan penggunaan fungsi tersebut dengan sekurang-kurangnya 44 qubit. Emel [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) untuk bertanya tentang cara mendapatkan lesen.

### Langkah 1: Petakan input klasik kepada masalah kuantum {#step-1-map-classical-inputs-to-a-quantum-problem-1}

Tentukan pilihan untuk pengiraan HiVQE
...
Tentukan geometri FeP-NO dalam format kamus untuk jarak ikatan Fe-N yang berbeza bagi mengira lengkung PES.
...
### Langkah 2 dan 3: Optimumkan masalah untuk pelaksanaan perkakasan kuantum dan laksanakan menggunakan fungsi HiVQE Chemistry {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

Berdasarkan persediaan HiVQE dan geometri, dapatkan keputusan secara berurutan.

#### Hantar pengiraan d(Fe-N) = 1.75 $\AA$. {#submit-dfe-n-175-aa-calculation}
...
Jejak job dan dapatkan semula keputusan untuk pengiraan d(Fe-N) = 1.75 $\AA$.
...
#### Hantar pengiraan d(Fe-N) = 2.00 $\AA$. {#submit-dfe-n-200-aa-calculation}
...
Jejak job dan dapatkan semula keputusan untuk pengiraan d(Fe-N) = 2.00 $\AA$.
...
#### Hantar pengiraan d(Fe-N) = 5.00 $\AA$. {#submit-dfe-n-500-aa-calculation}
...
Jejak job dan dapatkan semula keputusan untuk pengiraan d(Fe-N) = 5.00 $\AA$.
...
### Langkah 4: Pasca-proses dan bandingkan dengan kaedah klasik {#step-4-post-process-and-compare-with-classical-methods-1}

Keputusan pengiraan rujukan klasik (CASCI-DMRG, maxM=800) disediakan untuk (22o,22e) bagi mengesahkan keputusan HiVQE.
...
> **Nota:** Tinjauan ini adalah oleh **IBM Quantum** dan merangkumi **kandungan tutorial** (ditulis oleh IBM). doQumentation menyediakan laman web, terjemahan, dan pelaksanaan kod — untuk maklum balas mengenai perkara tersebut, sila [buka isu GitHub](https://github.com/JanLahmann/doQumentation/issues).
