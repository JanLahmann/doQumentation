...
* Pengetahuan asas tentang [Qiskit SDK](https://docs.quantum.ibm.com/guides) dan [IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime), serta akses kepada [akaun IBM Quantum](/guides/cloud-setup).
...
Tutorial ini menunjukkan eksperimen pada peranti IBM Quantum&reg; untuk menggambarkan cara litar dinamik boleh digunakan untuk menjana dan menilai pembelitan antara qubit yang jauh. Kita akan memetakan semua rantai empat-Qubit linear pada peranti, menjalankan litar teleportasi pada setiap satunya, kemudian menggambarkan taburan nilai MSE. Prosedur hujung-ke-hujung ini menunjukkan cara memanfaatkan IBM Quantum Compute Service dan ciri litar dinamik untuk memaklumkan pilihan yang sedar perkakasan bagi memotong litar atau mengedarkan algoritma kuantum merentasi sistem modular.
...
Sebelum menjalankan pada QPU sebenar, kita mengesahkan bahawa Circuit menghasilkan pasangan Bell dengan mengujinya pada simulator tanpa hingar menggunakan rantai empat-Qubit `[0, 1, 2, 3]`. Kita menggunakan IBM Quantum `Sampler` dengan `AerSimulator` sebagai mod Backend untuk melaksanakan Circuit.
...
Hasilnya mendedahkan pelbagai kualiti keterikatan yang luas merentas peranti. Ini mengesahkan penemuan kertas kerja bahawa boleh terdapat lebih daripada satu order magnitud variasi dalam fidelity keadaan Bell bergantung kepada qubit fizikal mana yang digunakan. Dalam erti kata praktikal, ini bermakna bahawa kawasan atau pautan tertentu dalam cip adalah jauh lebih baik dalam melakukan pengukuran pertengahan-litar dan operasi feedforward berbanding yang lain. Faktor seperti ralat pembacaan qubit, jangka hayat qubit, dan crosstalk mungkin menyumbang kepada perbezaan ini. Sebagai contoh, jika satu rantai mengandungi qubit pembacaan yang sangat bising, pengukuran pertengahan-litar boleh menjadi tidak boleh dipercayai, membawa kepada fidelity yang lemah untuk pasangan terikat itu (MSE tinggi).
Akhir sekali, kita menggambarkan prestasi keseluruhan dengan memplot fungsi taburan kumulatif (CDF) bagi nilai MSE untuk semua rantai. Plot CDF menunjukkan ambang MSE pada paksi-x, dan pecahan pasangan qubit yang mempunyai MSE paling banyak sebanyak itu pada paksi-y. Lengkung ini bermula pada sifar dan menghampiri satu apabila ambang berkembang untuk merangkumi semua titik data. Kenaikan tajam berhampiran MSE rendah akan menunjukkan bahawa banyak pasangan mempunyai fidelity tinggi; kenaikan perlahan bermakna banyak pasangan mempunyai ralat yang lebih besar. Kita anotasi CDF dengan identiti pasangan terbaik. Dalam plot itu, setiap titik dalam CDF sepadan dengan MSE satu rantai empat-qubit, dan kita label titik itu dengan pasangan indeks qubit $[q0, q3]$ yang terikat dalam eksperimen tersebut. Ini memudahkan untuk mengenal pasti pasangan qubit fizikal mana yang berprestasi terbaik (titik paling kiri pada CDF).
