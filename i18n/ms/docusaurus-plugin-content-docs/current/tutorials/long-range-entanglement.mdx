...
* Pengetahuan asas [Qiskit SDK](https://docs.quantum.ibm.com/guides) dan [IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime), serta akses kepada [akaun IBM Quantum](/guides/cloud-setup).
...
Sebelum dijalankan pada QPU sebenar, kita sahkan bahawa kedua-dua litar dinamik dan uniter menghasilkan keadaan Bell yang ideal pada simulator tanpa hingar. Kita gunakan IBM Quantum `Sampler` dengan `AerSimulator` sebagai mod backend, pada jarak kecil iaitu 6.
...
Circuit di tengah boleh diimplementasikan seperti berikut:
...
Sekarang bina Circuit yang mengukur dalam asas $XX$, $YY$, dan $ZZ$, sama seperti yang kita lakukan untuk Circuit dinamik di atas.
...
Setelah kita membina kedua-dua Circuit dinamik dan Circuit unitari untuk contoh skala kecil dengan `distance=6`, kita kemudian mentranspile mereka untuk dijalankan dahulu pada simulator tanpa hingar.
...
Langkah berikut memastikan backend menyokong arahan `if_else`, yang diperlukan untuk versi terbaru Circuit dinamik. Oleh kerana ciri ini masih dalam akses awal, kita tambahkan `IfElseOp` secara eksplisit ke sasaran backend jika ia belum tersedia.
