---
description: 'Jalankan eksperimen pengurangan ralat berskala utiliti dengan ekstrapolasi sifar hingar dan penguatan ralat probabilistik.'
notebook_path: tutorials/probabilistic-error-amplification.ipynb
sidebar_label: 'Pengurangan ralat berskala utiliti dengan penguatan ralat probabilistik'
title: 'Pengurangan ralat berskala utiliti dengan penguatan ralat probabilistik'
---
...
*Anggaran penggunaan: 14 minit pada pemproses Heron r3 (NOTA: Ini adalah anggaran sahaja. Masa larian anda mungkin berbeza.)*

## Hasil pembelajaran {#learning-outcomes}
Setelah melalui tutorial ini, pengguna harus memahami:
- Teori di sebalik *ekstrapolasi sifar hingar* (ZNE), pelbagai kaedah untuk memperkuatkan hingar, dan mengapa *penguatan ralat probabilistik* (PEA) lebih diutamakan untuk eksperimen berskala utiliti.
- Cara melaksanakan ZNE dengan PEA dalam amalan menggunakan Qiskit.

## Prasyarat {#prerequisites}
Kami cadangkan pengguna biasa dengan topik-topik berikut sebelum melalui tutorial ini:
- [Pelajaran Pengurangan Ralat](/learning/courses/utility-scale-quantum-computing/error-mitigation) dalam kursus *Pengkomputeran kuantum berskala utiliti* untuk pengetahuan asas tentang penggunaan pengurangan ralat dalam Qiskit.
- [Pelajaran Utility-I](/learning/courses/utility-scale-quantum-computing/utility-i) dalam kursus *Pengkomputeran kuantum berskala utiliti* untuk latar belakang lanjut tentang eksperimen berskala utiliti yang digunakan sebagai contoh dalam tutorial ini.

## Latar Belakang {#background}

Tutorial ini menunjukkan cara menjalankan eksperimen pengurangan ralat berskala utiliti dengan IBM Quantum Compute Service menggunakan versi eksperimental *ekstrapolasi sifar hingar* (ZNE) dengan *penguatan ralat probabilistik* (PEA).
...
di mana $\lambda$ menjadi parameter kekuatan hingar dan boleh diperkuatkan.

Kita boleh melaksanakan ZNE dengan langkah-langkah berikut:

1. Perkuatkan hingar Circuit untuk beberapa faktor hingar $\lambda_1, \lambda_2, ... $

2. Jalankan setiap Circuit yang diperkuatkan hingarnya untuk mengukur $\langle A(\lambda_1)\rangle, ...$

3. Ekstrapolasi balik ke had sifar hingar $\langle A(0)\rangle$
...
#### Perkuatkan hingar untuk ZNE {#amplify-noise-for-zne}
...
| **Peregangan denyutan**                                                                                                                                                                               | **Pelipatan gate**                                                                                                                                                                               | **Penguatan ralat probabilistik**                                                                                                                                                |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Skala tempoh denyutan melalui penentukuran                                                                                                                                                               | Ulangi gate dalam kitaran identiti $U\mapsto U(U^{-1}U)^{\lambda-1}/2$                                                                                                                            | Tambah hingar melalui pensampelan saluran Pauli                                                                                                                                                |
...
* PEA boleh digunakan pada mana-mana Circuit yang boleh dijalankan dengan faktor hingar natif ($\lambda=1$) tetapi memerlukan pembelajaran model hingar.


### Pelajari model hingar untuk PEA {#learn-the-noise-model-for-pea}
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Twirl Pauli lapisan gate dua-Qubit                                                                                                                                                            | Ulangi pasangan lapisan identiti dan pelajari hingarnya                                                                                                                                           | Terbitkan ketepatan (ralat bagi setiap saluran hingar)                                                                                                                                                |
...
**Rujukan**: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, *Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors* [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)
...
## Persediaan {#setup}
Dalam sel di bawah, kami mengimport pakej yang berkaitan dan mencipta beberapa fungsi pembantu untuk membina Circuit bagi evolusi masa Trotterisasi model Ising medan melintang dua dimensi yang mematuhi topologi Backend.
...
Kami akan melangkau langkah ini kerana pengurangan ralat runtime tidak disokong pada simulator.
...
#### Buat Circuit model Ising berparameter {#create-a-parameterized-ising-model-circuit}

##### Tetapkan backend {#establish-a-backend}
...
##### Tentukan gandingan lapisan penyepaduan {#define-entangling-layer-couplings}
...
##### Buang qubit yang buruk {#remove-bad-qubits}
...
##### Jana Circuit Trotter utama {#main-trotter-circuit-generation}
...
#### Buat senarai nilai parameter untuk ditetapkan kemudian {#create-a-list-of-parameter-values-to-be-assigned-later}
...
* Terjemahkan gate dalam Circuit kita kepada arahan [Instruction Set Architecture (ISA)](/guides/transpile#instruction-set-architecture) yang boleh dilaksanakan terus pada perkakasan.
...
Transpilkan Circuit anda untuk Backend yang dipilih dengan mencipta pengurus pas kemudian jalankan pengurus pas pada Circuit tersebut. Juga, tetapkan susun atur awal Circuit kepada `good_qubits` yang telah dipilih. Cara mudah untuk mencipta pengurus pas adalah dengan menggunakan fungsi [`generate_preset_pass_manager`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.generate_preset_pass_manager). Lihat [Transpilkan dengan pengurus pas](/guides/transpile-with-pass-managers) untuk penjelasan lebih terperinci tentang transpilasi dengan pengurus pas.
...
#### Pemerhatian ISA {#isa-observables}
Seterusnya, buat semua pemerhatian $\langle Z \rangle$ pemberat-1 untuk setiap qubit maya dengan menambah bilangan sebutan $\langle I \rangle$ yang diperlukan.
...
Perhatikan bahawa setiap pemerhatian dibungkus dalam senarai dalam blok kod berikut. Ini dilakukan untuk *menyiarkan* dengan nilai parameter supaya setiap pemerhatian qubit diukur untuk setiap nilai theta. Cari peraturan penyiaran untuk primitif dalam [dokumentasi primitif](/guides/primitives).
...
#### Konfigurasikan pilihan Estimator {#configure-estimator-options}
...
##### Penjelasan pilihan ZNE {#explanation-of-zne-options}
...
* **amplifier**: Kaedah yang digunakan semasa memperkuatkan hingar kepada faktor hingar yang dikehendaki.
  Nilai yang dibenarkan adalah `"gate_folding"`, yang memperkuatkan dengan mengulangi gate asas dua-Qubit,
  dan `"pea"`, yang memperkuatkan dengan pensampelan probabilistik selepas mempelajari model hingar Pauli-twirled
  untuk lapisan gate asas dua-Qubit yang di-twirl. Pilihan tambahan adalah `"gate_folding_front"` dan `"gate_folding_back"`, yang dijelaskan dalam [dokumentasi API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier).

* **extrapolated\_noise\_factors**: Tentukan satu atau lebih nilai faktor hingar di mana model yang diekstrapolasi akan dinilai. Jika merupakan urutan nilai, keputusan yang dikembalikan akan berbentuk array dengan faktor hingar yang ditentukan dinilai untuk model ekstrapolasi. Nilai
  0 sepadan dengan ekstrapolasi sifar hingar.
...
### Langkah 4: Proses semula dan kembalikan keputusan dalam format klasik yang dikehendaki {#step-4-post-process-and-return-result-in-desired-classical-format}

Setelah eksperimen selesai, anda boleh melihat keputusannya. Anda mengambil nilai jangkaan mentah dan yang telah dikurangkan ralatnya, lalu bandingkan dengan keputusan tepat. Kemudian, plot nilai jangkaan tersebut — sama ada yang dimitigasi (extrapolated) mahupun yang mentah — diambil purata ke atas semua qubit untuk setiap parameter. Akhir sekali, plot nilai jangkaan untuk qubit individu pilihan anda.

```python
primitive_result = job.result()
```

#### Bentuk keputusan umum dan metadata {#general-result-shapes-and-metadata}
...
2. `pub_result.data.evs_extrapolated`: Nilai jangkaan untuk faktor hingar yang di-extrapolasi bagi setiap extrapolator. Array ini ada dua paksi tambahan.
   ...
   * Paksi ketiga mengindeks kaedah extrapolasi ($2$ extrapolator, `exponential` dan `linear`)

   * Paksi terakhir mengindeks `extrapolated_noise_factors` ($20$ titik extrapolasi yang dinyatakan dalam pilihan)

3. `pub_result.data.evs_noise_factors`: Nilai jangkaan mentah bagi setiap faktor hingar.

   * Paksi ketiga mengindeks `noise_factors` mentah ($3$ faktor)
...
### Plot keputusan simulasi Trotter {#plot-trotter-simulation-results}
...
### Plot keputusan extrapolasi untuk qubit individu {#plot-extrapolation-results-for-individual-qubits}
...
- [Tutorial](/tutorials/combine-error-mitigation-techniques) yang menumpukan pada penggabungan teknik pengurangan ralat.
- [Dokumentasi](/guides/error-mitigation-and-suppression-techniques) terperinci tentang teknik pengurangan ralat yang tersedia dalam Qiskit.
- Pelajaran tambahan yang merangkumi eksperimen berskala utiliti: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) dan [Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
