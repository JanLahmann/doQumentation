---
title: "Tingkatkan pengelasan ciri menggunakan projected quantum kernels"
sidebar_label: "Tingkatkan pengelasan ciri menggunakan projected quantum kernels"
description: "Tutorial tentang cara meningkatkan pengelasan ciri menggunakan projected quantum kernels"
...
- Bagaimana projected quantum kernels (PQK) berfungsi dan bila ia menawarkan potensi kelebihan kuantum.
- Cara menjalankan PQK pada perkakasan menggunakan dataset dunia sebenar.

## Prasyarat {#prasyarat}

Kami mencadangkan pengguna biasa dengan topik berikut sebelum melalui tutorial ini:
- [Quantum kernels](/learning/courses/quantum-machine-learning/quantum-kernel-methods) daripada [kursus pembelajaran mesin kuantum](/learning/courses/quantum-machine-learning) dalam IBM Quantum&reg; Learning

## Latar belakang {#latar-belakang}

Dalam tutorial ini, kami menunjukkan cara menjalankan [projected quantum kernel](https://www.nature.com/articles/s41467-021-22539-9) (PQK) dengan Qiskit pada dataset biologi dunia sebenar, berdasarkan kertas kerja [Enhanced Prediction of CAR T-Cell Cytotoxicity with Quantum-Kernel Methods](https://arxiv.org/abs/2507.22710) [[1]](#references).
...
Blok pembina utama PQK ialah matriks ketumpatan tereduksi (RDM), yang diperoleh melalui pengukuran projektif peta ciri kuantum. Khususnya, seseorang biasanya mengira matriks ketumpatan tereduksi satu-Qubit (1-RDM) untuk setiap qubit. Kuantiti yang diukur ini kemudiannya digunakan sebagai input kepada fungsi kernel klasikal, seperti kernel eksponen, untuk membina matriks kernel akhir.

PQK menawarkan potensi kelebihan berbanding [quantum kernels](/tutorials/quantum-kernel-training) standard, khususnya untuk perkakasan kuantum jangka hampir. Quantum kernels standard biasanya bergantung pada menganggar pertindihan keadaan global, yang semakin sukar diukur dengan tepat apabila bilangan qubit bertambah dan sangat sensitif kepada hingar. Sebaliknya, PQK menggunakan observable tempatan seperti matriks ketumpatan tereduksi satu-Qubit (1-RDM), yang membawa kepada overhed pensampelan yang lebih rendah, ketahanan yang lebih baik terhadap hingar perkakasan, dan skalabiliti yang lebih baik. Dengan mengunjurkan keadaan kuantum ke ciri pengukuran tempatan sebelum menggunakan fungsi kernel klasikal, PQK boleh mengekalkan korelasi kuantum yang berguna sambil kekal lebih praktikal untuk peranti jangka hampir.
## Keperluan {#requirements}
Sebelum memulakan tutorial ini, pastikan anda telah memasang perkara berikut:

- Qiskit SDK v2.0 atau lebih baru, dengan sokongan [visualization](https://docs.quantum.ibm.com/api/qiskit/visualization)
...
Kami mengetepikan contoh simulator skala kecil dalam tutorial ini, kerana matlamat utama kami adalah untuk menunjukkan bagaimana projected quantum kernels boleh diskala kepada sistem yang lebih besar dan perkakasan sebenar.
## Contoh perkakasan skala besar {#contoh-perkakasan-skala-besar}
### Langkah 1: Petakan input klasikal kepada masalah kuantum {#langkah-1-petakan-input-klasikal-kepada-masalah-kuantum}
### Penyediaan dataset {#penyediaan-dataset}

Dalam tutorial ini kami menggunakan dataset biologi dunia sebenar untuk tugasan pengelasan binari, yang dijana oleh Daniels et al. (2022) dan boleh dimuat turun dari [bahan tambahan](https://www.science.org/doi/full/10.1126/science.abq0225#supplementary-materials) yang disertakan bersama kertas kerja tersebut. Data terdiri daripada sel T CAR, iaitu sel T yang direkayasa secara genetik dan digunakan dalam imunoterapi untuk merawat kanser tertentu. Sel T, sejenis sel imun, diubah suai di makmal untuk mengekspresikan reseptor antigen chimeric (CAR) yang menyasarkan protein tertentu pada sel kanser. Sel T yang diubah suai ini boleh mengenali dan memusnahkan sel kanser dengan lebih berkesan. Ciri-ciri data ialah motif sel T CAR, yang merujuk kepada komponen struktur atau fungsi khusus CAR yang direkayasa ke dalam sel T. Berdasarkan motif-motif ini, tugas kita adalah untuk meramal sitotoksisiti sel T CAR tertentu, melabelkannya sebagai toksik atau tidak toksik.
Berikut menunjukkan fungsi pembantu untuk pra-proses dataset ini.
...
Kami juga mengubah dataset supaya $1$ diwakili sebagai $\pi/2$ untuk tujuan penskalaan.
...
Kini kami membina peta ciri yang membenamkan dataset klasikal kami ke dalam ruang ciri berdimensi lebih tinggi. Untuk pembenaman ini, kami menggunakan [``ZZFeatureMap``](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ZZFeatureMap) daripada Qiskit.
...
### Ukur 1-RDM {#ukur-1-rdm}
Dalam langkah ini, kami memperoleh semua matriks ketumpatan tereduksi satu-Qubit (1-RDM) melalui pengukuran projektif peta ciri kuantum, yang kemudiannya akan dimasukkan ke dalam fungsi kernel eksponen klasikal.
Mari kita lihat cara mengira 1-RDM diberi satu titik data dari dataset sebelum kami menjalani semua data. 1-RDM ialah koleksi pengukuran satu-Qubit operator Pauli ``X``, ``Y`` dan ``Z`` pada semua qubit. Ini kerana satu-Qubit RDM boleh dinyatakan sepenuhnya sebagai: $$\rho = \frac{1}{2} \big( I + \braket \sigma_x \sigma_x  + \braket \sigma_y \sigma_y + \braket \sigma_z \sigma_z  \big)$$
...
```
### Langkah 4: Pasca-proses dan kembalikan keputusan dalam format klasikal yang dikehendaki {#langkah-4-pasca-proses-dan-kembalikan-keputusan-dalam-format-klasikal-yang-dikehendaki}
### Takrifkan projected quantum kernel {#takrifkan-projected-quantum-kernel}
Projected quantum kernel ditakrifkan dengan fungsi kernel berikut: $$k^{\textrm{PQ}}(x_i, x_j) = \textrm{exp} \Big(-\gamma \sum_k \sum_{P \in \{ X,Y,Z \}} (\textrm{Tr}[P \rho_k(x_i)] - \textrm{Tr}[P \rho_k(x_j)])^2 \Big) $$
Dalam persamaan di atas, $\gamma>0$ ialah hiperparameter yang boleh dilaras. $K^{\textrm{PQ}}_{ij} = k^{\textrm{PQ}}(x_i, x_j)$ ialah entri matriks kernel $K^{\textrm{PQ}}$.
Menggunakan takrif 1-RDM, kita dapat melihat bahawa sebutan individu dalam fungsi kernel boleh dinilai sebagai $\textrm{Tr}[P \rho_k (x_i)] = \braket P$, di mana $P \in \{ X,Y,Z \}$. Nilai jangkaan ini adalah tepat seperti yang kami ukur di atas.
Dengan menggunakan ``scikit-learn``, kita sebenarnya boleh mengira kernel dengan lebih mudah. Ini disebabkan oleh fungsi kernel radial basis (``'rbf'``) yang tersedia: $ \textrm{exp} (-\gamma \lVert x - x' \rVert^2)$. Pertama, kita hanya perlu membentuk semula dataset latihan dan ujian yang diunjurkan baharu ke dalam tatasusunan dua dimensi.
...
### Penanda aras klasikal {#penanda-aras-klasikal}
...
## Lampiran: Sahkan potensi kelebihan kuantum dataset dalam tugasan pembelajaran {#lampiran-sahkan-potensi-kelebihan-kuantum-dataset-dalam-tugasan-pembelajaran}

Tidak semua dataset menawarkan potensi kelebihan daripada penggunaan PQK. Terdapat beberapa had teori yang boleh digunakan sebagai ujian awal untuk melihat sama ada dataset tertentu boleh mendapat manfaat daripada PQK. Untuk mengukur ini, pengarang [Power of data in quantum machine learning](https://www.nature.com/articles/s41467-021-22539-9) [2] mentakrifkan kuantiti yang dirujuk sebagai kerumitan model klasikal dan kuantum serta pemisahan geometri model klasikal dan kuantum. Untuk mengharapkan potensi kelebihan kuantum daripada PQK, pemisahan geometri antara kernel klasikal dan kernel yang diunjurkan secara kuantum hendaklah lebih kurang pada orde $\sqrt{N}$, di mana $N$ ialah bilangan sampel latihan. Jika syarat ini dipenuhi, kita teruskan dengan memeriksa kerumitan model. Jika kerumitan model klasikal adalah pada orde $N$ manakala kerumitan model yang diunjurkan secara kuantum jauh lebih kecil daripada $N$, kita boleh mengharapkan potensi kelebihan daripada PQK.
Pemisahan geometri ditakrifkan seperti berikut (F19 dalam [[2]](#references)):
...
Kerumitan model ditakrifkan seperti berikut (M1 dalam [[2]](#references)):
...
- [Kursus pembelajaran mesin kuantum](/learning/courses/quantum-machine-learning) yang mendalam dari IBM Quantum Learning
