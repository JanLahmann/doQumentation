...
<Admonition type="tip">
**Qiskit Serverless sedang ditingkatkan, dan ciri-cirinya berubah dengan pantas.** Semasa fasa pembangunan ini, dapatkan nota keluaran dan dokumentasi terkini di halaman [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html).
</Admonition>
...
Pastikan anda telah mengikuti aliran kerja dalam [Tulis program Qiskit Serverless pertama anda](/guides/serverless-first-program) sebelum anda bermula.

## Senaraikan program yang tersedia {#list-programs-available}

Anda boleh menggunakan `QiskitServerless.list()` untuk mendapatkan senarai program yang tersedia untuk dijalankan dengan Qiskit Serverless. Ini termasuk [`transpile_remote_serverless`](/guides/serverless-first-program) yang dimuat naik sebelumnya.
...
## Jalankan program yang dimuat naik dan hantar input {#run-an-uploaded-program-and-pass-inputs}
...
## Dapatkan semula log dan keputusan {#retrieve-logs-and-results}
...
## Senaraikan kerja yang pernah dijalankan dengan Qiskit Serverless {#list-jobs-previously-run-with-qiskit-serverless}
...
- Terokai [alat pengurusan pengkomputeran dan data](/guides/serverless-manage-resources) yang tersedia untuk program anda, termasuk selarian.
