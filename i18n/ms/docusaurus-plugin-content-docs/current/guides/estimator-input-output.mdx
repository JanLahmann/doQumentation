...
Pertimbangkan boleh pantau tunggal $\mathcal{O}$. Tanpa [ZNE](/guides/error-mitigation-and-suppression-techniques#zero-noise-extrapolation-zne), kamu boleh menganggap setiap tembakan pelaksanaan Estimator sebagai menyediakan anggaran titik bagi nilai jangkaan $\langle \mathcal{O} \rangle$. Jika anggaran titik berada dalam vektor `Os`, maka nilai yang dikembalikan dalam `ensemble_standard_error` adalah bersamaan dengan berikut (di mana $\sigma_{\mathcal{O}}$ adalah [sisihan piawai anggaran nilai jangkaan](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.BackendEstimatorV2) dan $N_{shots}$ adalah bilangan tembakan):

$$\frac{ \sigma_{\mathcal{O}} }{ \sqrt{N_{shots}} },$$

yang menganggap semua tembakan sebagai sebahagian daripada satu ensemble. Jika kamu meminta [twirling](/guides/error-mitigation-and-suppression-techniques#pauli-twirling) Gate (`twirling.enable_gates = True`), kamu boleh menyisih anggaran titik $\langle \mathcal{O} \rangle$ ke dalam set yang berkongsi twirl yang sama. Panggil set anggaran ini `O_twirls`, dan terdapat `num_randomizations` (bilangan twirl) daripadanya. Kemudian `stds` adalah ralat piawai min `O_twirls`, seperti dalam

$$\frac{ \sigma_{\mathcal{O}} }{ \sqrt{N_{twirls}} },$$

di mana $\sigma_{\mathcal{O}}$ adalah sisihan piawai `O_twirls` dan $N_{twirls}$ adalah bilangan twirl. Apabila kamu tidak mengaktifkan twirling, `stds` dan `ensemble_standard_error` adalah sama.

Jika kamu mengaktifkan ZNE, maka `stds` yang diterangkan di atas menjadi pemberat dalam regresi bukan-linear kepada model ekstrapolator. Apa yang akhirnya dikembalikan dalam `stds` dalam kes ini adalah ketidakpastian model padanan yang dinilai pada faktor hingar sifar. Apabila terdapat padanan yang lemah, atau ketidakpastian yang besar dalam padanan, `stds` yang dilaporkan boleh menjadi sangat besar. Apabila ZNE diaktifkan, `pub_result.data.evs_noise_factors` dan `pub_result.data.stds_noise_factors` juga diisi, supaya kamu boleh melakukan ekstrapolasi sendiri.
