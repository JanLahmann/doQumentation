...
`parameter values` yang pilihan boleh menjadi senarai atau parameter tunggal. Elemen daripada observable dan nilai parameter digabungkan dengan mengikut peraturan penyiaran NumPy seperti yang diterangkan dalam topik [Input dan output primitif](primitive-input-output#broadcasting-rules), dan satu anggaran nilai jangkaan dikembalikan untuk setiap elemen bentuk yang disiarkan.
...
Kod berikut menunjukkan contoh set input vektor kepada primitif `Estimator` dan melaksanakannya pada Backend IBM&reg; sebagai satu objek `RuntimeJobV2 ` tunggal.
