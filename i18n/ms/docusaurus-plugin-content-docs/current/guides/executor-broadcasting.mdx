...
- **Paksi intrinsik** (paling kanan): Ditentukan oleh jenis data. Sebagai contoh, jika Circuit anda mempunyai tiga parameter, maka nilai parameter memerlukan tiga nombor, memberikan bentuk intrinsik `(3,)`.
...
Jika anda memberikan input dengan bentuk ekstrinsik `(4, 1)` dan `(3,)`, bentuk ekstrinsik yang disiarkan adalah `(4, 3)`. Kod berikut menggunakan Circuit dengan 1024 tembakan dan daftar klasikal 4-bit (seperti yang ditakrifkan dalam contoh [Permulaan pantas](#quickstart)):
