...
Jika anda membaca sel kod ini secara tempatan dalam notebook, anda akan melihat [arahan magik](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Melaksanakan sel dengan arahan magik ini akan menyimpannya ke cakera dan bukannya melaksanakannya.
</Admonition>
...
Untuk tugas klasik yang boleh diselariakn, gunakan penghias `@distribute_task` untuk menentukan keperluan pengkomputeran yang diperlukan bagi melaksanakan tugas. Mulakan dengan mengingat semula contoh `transpile_remote.py` daripada topik [Tulis program Qiskit Serverless pertama anda](/guides/serverless-first-program) dengan kod berikut.
...
Berikut menunjukkan cara memuat naik fail. Pastikan anda telah mengesahkan ke Qiskit Serverless dengan akaun IBM Quantum anda (lihat [Muat naik ke Qiskit Serverless](/guides/serverless-first-program#upload-to-qiskit-serverless) untuk arahan).
...
- Lihat contoh penuh yang [memindahkan kod sedia ada ke Qiskit Serverless](/guides/serverless-port-code).
