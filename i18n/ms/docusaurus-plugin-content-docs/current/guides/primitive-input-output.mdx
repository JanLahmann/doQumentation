...
- Satu `QuantumCircuit` tunggal, yang mungkin mengandungi satu atau lebih objek [`Parameter`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.Parameter)
   *Nota: Circuit ini juga mesti mengandungi arahan pengukuran untuk setiap Qubit yang hendak dipersamplkan.*
...
Setiap objek `PubResult` ini mempunyai atribut `data` dan atribut `metadata` yang opsyenal. Atribut `data` ialah [`DataBin`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.DataBin) yang disesuaikan dan mengandungi anggaran nilai jangkaan dalam kes Estimator, atau sampel output Circuit dalam kes Sampler.
