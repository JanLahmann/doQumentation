---
title: "Bina templat Qiskit Function untuk simulasi Hamiltonian"
sidebar_label: "Bina templat Qiskit Function untuk simulasi Hamiltonian"
description: "Cara membuat program transpilasi selari dan mengerahkannya ke IBM Quantum Platform untuk digunakan sebagai perkhidmatan jauh yang boleh digunakan semula."
...
Templat ini merangkumi aliran kerja untuk mensimulasikan evolusi masa sesuatu keadaan awal terhadap Hamiltonian berasaskan spin yang ditentukan pengguna, dan mengembalikan set nilai jangkaan yang ditetapkan menggunakan addon Qiskit [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/).

Templat ini disusun sebagai corak Qiskit dengan langkah-langkah berikut:

#### 1. Mengumpul input dan memetakan masalah {#1-collecting-input-and-mapping-the-problem}
...
#### 2. Menyediakan litar yang dijana untuk pelaksanaan {#2-prepare-the-generated-circuits-for-execution}

Litar yang dijana daripada addon AQC kemudiannya ditranspilkan untuk dilaksanakan pada Backend yang dipilih. Sebuah instans [`EstimatorV2`](../api/qiskit-ibm-runtime/estimator-v2) dicipta dengan set pilihan pengurangan ralat lalai untuk menguruskan pelaksanaan litar.

#### 3. Pelaksanaan {#3-execution}
...
## Tulis templat fungsi {#write-the-function-template}

Pertama, tulis templat fungsi untuk simulasi Hamiltonian yang menggunakan [addon Qiskit AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/) untuk memetakan huraian masalah kepada litar kedalaman berkurangan untuk dilaksanakan pada perkakasan.

Jika anda memuat turun halaman ini dan melihatnya secara tempatan dalam editor buku nota, anda akan melihat beberapa sel kod mengandungi [arahan magic](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Arahan magic ini menyimpan kod ke `./source_files/template_hamiltonian_simulation.py`, iaitu templat fungsi yang boleh anda muat naik dan jalankan dari jauh dengan Qiskit Serverless.
...
### Kumpul dan sahkan input {#collect-and-validate-the-inputs}
...
#### Sahkan input {#validate-the-inputs}
...
#### Sediakan output fungsi {#prepare-the-function-outputs}
...
### Petakan masalah dan pra-proses litar dengan AQC {#map-the-problem-and-pre-process-the-circuit-with-aqc}
...
### Optimumkan litar akhir untuk pelaksanaan {#optimize-the-final-circuit-for-execution}

Selepas bahagian AQC aliran kerja, `final_circuit` [ditranspilkan untuk perkakasan](/guides/transpile#instruction-set-architecture) seperti biasa.
...
#### Keluar awal jika menggunakan mod dry run {#exit-early-if-using-dry-run-mode}
...
#### Laksanakan litar pada perkakasan {#execute-the-circuit-on-hardware}
...
Appending to ./source_files/template_hamiltonian_simulation.py
```
## Kerahkan fungsi ke IBM Quantum Platform {#deploy-the-function-to-ibm-quantum-platform}
...
Gunakan `qiskit-ibm-catalog` untuk mengesahkan diri ke `QiskitServerless` dengan kunci API anda, yang boleh anda temui di papan pemuka [IBM Quantum Platform](https://quantum.cloud.ibm.com), dan muat naik program tersebut.

Anda boleh menggunakan `save_account()` secara pilihan untuk menyimpan kelayakan anda (lihat panduan [Sediakan akaun IBM Cloud anda](/guides/cloud-setup#cloud-save)). Perhatikan bahawa ini menulis kelayakan anda ke fail yang sama seperti [`QiskitRuntimeService.save_account()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/qiskit-runtime-service#save_account).
...
## Jalankan templat fungsi dari jauh {#run-the-function-template-remotely}
...
Untuk menyelami lebih dalam addon Qiskit AQC-Tensor, lihat tutorial [Improved Trotterized Time Evolution with Approximate Quantum Compilation](/tutorials/approximate-quantum-compilation-for-time-evolution) atau [repositori qiskit-addon-aqc-tensor](https://github.com/Qiskit/qiskit-addon-aqc-tensor).
...
<Accordion>
<AccordionItem title="**Kod sumber program penuh**">
