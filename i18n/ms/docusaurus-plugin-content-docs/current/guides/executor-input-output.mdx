...
Item pertama mengandungi hasil daripada menjalankan tugas pertama (`CircuitItem`) dalam program. Ia mengandungi satu kunci tunggal, `meas`, iaitu nama daftar klasik dalam Circuit input. Nilai kunci ini dipetakan kepada `np.ndarray` berbentuk `(parameter sets, shots, register bits)`, iaitu (10, 1024, 3) untuk contoh di atas.
...
Item kedua mengandungi hasil daripada menjalankan tugas kedua (`SamplexItem`) dalam program. Item ini mengandungi beberapa kunci. Kunci `meas`, iaitu nama daftar klasik Circuit input, dipetakan kepada tatasusunan hasil bagi daftar tersebut. Tatasusunan ini mempunyai bentuk `(randomizations, parameter sets, shots, classical bits)`, atau (28, 10, 1024, 3) dalam contoh ini. Selain itu, output mengandungi kunci `measurement_flips.meas`, iaitu pembetulan bit-flip untuk membatalkan twirling pengukuran bagi daftar `meas`. Bentuk output ini akan menjadi (28, 10, 1, 3) untuk contoh kita kerana hanya satu shot diperlukan untuk melakukan bit-flip.
