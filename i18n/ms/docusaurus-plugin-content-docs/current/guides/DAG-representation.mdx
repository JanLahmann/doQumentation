...
## Laksanakan pas BasicMapper {#implement-a-basicmapper-pass}
Struktur DAG boleh dimanfaatkan untuk menulis transpiler passes. Dalam contoh di bawah, pas `BasicMapper` dilaksanakan untuk memetakan litar sembarangan ke peranti dengan sambungan qubit yang terhad. Untuk panduan tambahan, rujuk panduan tentang [menulis transpiler passes khusus](/guides/custom-transpiler-pass).

Pas ini ditakrifkan sebagai `TransformationPass`, bermakna ia mengubah litar. Ia berbuat demikian dengan melintasi DAG lapisan demi lapisan, memeriksa sama ada setiap arahan memenuhi kekangan yang dikenakan oleh peta gandingan peranti. Jika pelanggaran dikesan, laluan swap ditentukan dan get SWAP yang diperlukan dimasukkan sewajarnya.

Apabila mencipta transpiler pass, keputusan pertama melibatkan pemilihan sama ada pas harus mewarisi daripada `TransformationPass` atau `AnalysisPass`. Pas transformasi direka untuk mengubah litar, manakala pas analisis hanya bertujuan untuk mengekstrak maklumat untuk digunakan oleh pas seterusnya. Fungsi utama kemudian dilaksanakan dalam kaedah `run(dag)`. Akhirnya, pas perlu didaftarkan dalam modul `qiskit.transpiler.passes`.

Dalam pas khusus ini, DAG dilintasi lapisan demi lapisan (di mana setiap lapisan mengandungi operasi yang bertindak pada set qubit yang tidak bertindan dan oleh itu boleh dilaksanakan secara bebas). Untuk setiap operasi, jika kekangan peta gandingan tidak dipenuhi, laluan swap yang sesuai dikenal pasti, dan swap yang diperlukan dimasukkan untuk membawa qubit yang terlibat ke kedudukan bersebelahan.
...
Pas kini boleh diuji pada litar contoh kecil. Pengurus pas dibina dengan pas yang baru ditakrifkan disertakan. Litar contoh kemudian diberikan kepada pengurus pas ini, dan litar baru yang telah ditransformasi diperoleh sebagai output.
...
- Semak panduan tentang mencipta [transpiler pass khusus](/guides/custom-transpiler-pass)
- Pelajari cara [Cipta dan transpil terhadap Backend khusus](/guides/custom-backend)
