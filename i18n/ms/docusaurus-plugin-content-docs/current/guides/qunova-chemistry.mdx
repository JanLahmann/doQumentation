...
Gelung pengoptimuman memfokuskan pada melaraskan parameter Circuit untuk menghasilkan subspace berkualiti tinggi. HI-VQE menawarkan tiga pilihan Circuit: [`excitation_preserving`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.excitation_preserving), [efficient_su2](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.efficient_su2), dan [LUCJ](https://qiskit-community.github.io/ffsim/explanations/lucj.html). Pengoptimuman dimulakan dekat dengan keadaan rujukan Hartree-Fock kerana kesesuaiannya yang umum. Circuit kemudian dilaksanakan pada peranti kuantum dan konfigurasi disampling dari keadaan kuantum yang terhasil sebelum dikembalikan sebagai rentetan binari. Disebabkan hingar peranti kuantum, beberapa konfigurasi yang disampling mungkin tidak sah secara fizikal, gagal memulihara bilangan elektron atau spin. HI-VQE menangani ini menggunakan proses pemulihan konfigurasi dari pakej [qiskit-addon-sqd](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd), supaya pengguna boleh sama ada membetulkan konfigurasi tidak sah atau membuangnya.
...
Kemudian, sahkan menggunakan [kunci API IBM Quantum&reg;](http://quantum.cloud.ibm.com/) anda dan, dengan mengandaikan anda telah [menyimpan akaun anda](/guides/functions-get-started#install-qiskit-functions-catalog-client) ke persekitaran tempatan anda, pilih Fungsi Qiskit seperti berikut:
...
Semak [status](/guides/functions-get-started#check-job-status) atau kembalikan [keputusan](/guides/functions-get-started#retrieve-results) beban kerja Fungsi Qiskit anda seperti berikut:
...
Ambil perhatian bahawa penggunaan Fungsi Qiskit ini terhad kepada masalah yang memerlukan paling banyak 20 qubit, melainkan lesen diperoleh yang memberikan had yang lebih tinggi.

E-mel [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com) untuk pertanyaan lesen.

### Contoh penggunaan fungsi berlesen {#example-of-licensed-function-use}

Pengguna berlesen diberikan token lesen, dan kemudian mesti menggunakan pustaka wrapper untuk menyerahkan token lesen mereka kepada fungsi tersebut.
Pustaka wrapper boleh dipasang [daripada PyPI](https://pypi.org/project/hivqe-qiskit-function-utils/) dengan `pip install hivqe-qiskit-function-utils`.
Contoh di bawah menunjukkan cara pustaka ini sepatutnya digunakan untuk menyerahkan token lesen anda apabila memanggil fungsi tersebut.
...
#### Lengkung PES dissosiasi untuk iron(II)-porfirin berlima koordinat dengan sistem NO dengan 44 qubit {#dissociation-pes-curve-for-five-coordinated-ironii-porphyrin-with-an-no-system-with-44-qubits}
