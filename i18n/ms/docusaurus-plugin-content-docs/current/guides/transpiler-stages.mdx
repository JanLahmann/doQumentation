...
Pilihan pemetaan sangat penting untuk meminimumkan bilangan operasi SWAP yang diperlukan untuk memetakan litar input ke topologi peranti dan memastikan Qubit yang paling ditentukur digunakan. Disebabkan kepentingan peringkat ini, pengurus pas praset mencuba beberapa kaedah berbeza untuk mencari susun atur terbaik. Biasanya ini melibatkan dua langkah: pertama, cuba cari susun atur "sempurna" (susun atur yang tidak memerlukan sebarang operasi SWAP), dan kemudian, pas heuristik yang cuba mencari susun atur terbaik untuk digunakan jika susun atur sempurna tidak dapat ditemui. Terdapat dua `Passes` yang biasanya digunakan untuk langkah pertama ini:
...
Apabila menulis litar kuantum, anda bebas menggunakan sebarang Gate kuantum (operasi unitari) yang anda suka, bersama-sama dengan koleksi operasi bukan-Gate seperti arahan pengukuran atau tetapan semula Qubit. Walau bagaimanapun, kebanyakan peranti kuantum hanya menyokong secara asli segelintir operasi Gate kuantum dan bukan-Gate. Gate asli ini adalah sebahagian daripada takrifan [ISA](/guides/transpile#instruction-set-architecture) sasaran dan peringkat `PassManagers` praset ini menterjemah (atau *unrolls*) Gate yang dinyatakan dalam litar kepada Gate asas asli Backend yang dinyatakan. Ini adalah langkah penting, kerana ia membolehkan litar dilaksanakan oleh Backend, tetapi biasanya membawa kepada peningkatan kedalaman dan bilangan Gate.
