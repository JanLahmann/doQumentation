...
<Admonition type="tip">
**Qiskit Serverless sedang menerima naik taraf, dan ciri-cirinya berubah dengan pantas.** Sepanjang fasa pembangunan ini, dapatkan nota keluaran dan dokumentasi terkini di halaman [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html).
</Admonition>
...
## Gambaran keseluruhan aliran kerja {#workflow-overview}

1. Cipta direktori tempatan dan fail program kosong (`./source_files/transpile_remote.py`)
1. Tambah kod ke program anda yang, apabila dimuat naik ke Qiskit Serverless, akan mentranspil litar
1. Gunakan `qiskit-ibm-catalog` untuk mengesahkan ke Qiskit Serverless
1. Muat naik program ke Qiskit Serverless

Selepas memuat naik program anda, anda boleh menjalankannya untuk mentranspil litar dengan mengikuti panduan [Jalankan beban kerja Qiskit Serverless pertama anda dari jauh](/guides/serverless-run-first-workload).
...
## Contoh: Transpilasi jauh dengan Qiskit Serverless {#example-remote-transpilation-with-qiskit-serverless}
...
### Cipta direktori dan fail program kosong {#create-the-directory-and-an-empty-program-file}
...
### Tambah kod ke fail program anda {#add-code-to-your-program-file}

Isikan fail program anda dengan kod berikut, kemudian simpannya.

<Admonition type="caution">
Jika anda membaca sel kod secara tempatan dalam notebook, anda akan melihat [magic command](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. Melaksanakan sel dengan magic command ini menyimpannya ke cakera dan bukannya melaksanakannya.
</Admonition>
...
### Tambah kod untuk mendapatkan argumen program {#add-code-to-get-program-arguments}
...
### Tambah kod yang memanggil Backend {#add-code-that-calls-the-backend}

Tambah kod berikut ke fail program anda, yang memanggil Backend anda dengan `QiskitRuntimeService`.

Kod berikut mengandaikan bahawa anda telah mengikuti proses untuk menyimpan kelayakan anda dengan menggunakan `QiskitRuntimeService.save_account`, dan akan memuatkan akaun lalai yang anda simpan melainkan anda menyatakan sebaliknya. Lihat [Simpan kelayakan log masuk anda](/guides/save-credentials) dan [Mulakan akaun perkhidmatan Qiskit Runtime anda](/guides/initialize-account) untuk maklumat lanjut.
...
### Tambah kod untuk mentranspil {#add-code-to-transpile}
...
### Mengesahkan ke Qiskit Serverless {#authenticate-to-qiskit-serverless}

Gunakan `qiskit-ibm-catalog` untuk mengesahkan ke `QiskitServerless` dengan kunci API anda (anda boleh menggunakan kunci API `QiskitRuntimeService` anda, atau mencipta kunci API baharu pada [papan pemuka IBM Quantum Platform](https://quantum.cloud.ibm.com)).
...
### Jalankan kod untuk memuat naik {#run-code-to-upload}
...
### Sahkan muat naik {#verify-upload}
...
- Ketahui cara menghantar input dan menjalankan program anda dari jauh dalam topik [Jalankan beban kerja Qiskit Serverless pertama anda dari jauh](/guides/serverless-run-first-workload).
