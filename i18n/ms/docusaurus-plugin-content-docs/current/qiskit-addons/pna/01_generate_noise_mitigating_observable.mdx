...
Dalam tutorial ini, kita akan belajar cara memanfaatkan alat terkini dalam ekosistem Qiskit untuk melaksanakan aliran kerja yang boleh disesuaikan sepenuhnya dengan pengurangan ralat. Kita akan memperkenalkan teknik PNA dan menggunakannya untuk mengurangkan ralat get. Kita juga akan menggunakan TREX untuk mengurangkan ralat pembacaan dan pemilihan pasca untuk mengurangkan ralat yang tidak ditangkap dalam model hingar yang dipelajari.

**Garis Besar**
- Berikan gambaran ringkas tentang ``PNA``
- Cipta litar Trotter kuantum dan boleh cerap (observable). Transpail ke Backend dan masukkan ukuran pemilihan pasca.
- Guna ``samplomatic`` untuk memutar lapisan get 2Q dan ukuran. Cari lapisan 2Q unik untuk mengurangkan kos pembelajaran hingar.
- Guna ``NoiseLearnerV3`` untuk mempelajari model ralat yang mempengaruhi get 2Q dan ukuran.
- Guna ``qiskit-addon-pna`` untuk menjana boleh cerap yang mengurangkan hingar
...
***Teknik untuk mengurangkan ralat get dengan mempropagat boleh cerap melalui saluran hingar songsang yang mempengaruhi get 2-qubit, menghasilkan boleh cerap yang mengurangkan hingar.***
get 2Q dalam eksperimen yang ingin kita jalankan akan dipengaruhi oleh hingar yang ketara.
![Eksperimen beringar](/img/qiskit-addons/images/noisy_expt.png)
Jika kita mempelajari model hingar, kita boleh menerapkan songsangannya dan membatalkan hingar tersebut.
![Eksperimen dengan hingar dikurangkan](/img/qiskit-addons/images/noise_mitigated_expt.png)
Daripada melaksanakan saluran hingar songsang dengan mengambil sampelnya di QPU seperti dalam PEC, kita boleh melaksanakannya secara klasik dalam boleh cerap yang diukur menggunakan perambatan Pauli. Ini menghasilkan boleh cerap yang lebih kompleks yang, apabila diukur, memberi kesan mengurangkan hingar get yang dipelajari.
![Gambaran keseluruhan PNA](/img/qiskit-addons/images/pna_overview.png)
### Jana litar Trotter yang dicerminkan dan boleh cerap {#generate-the-mirrored-trotter-circuit-and-observable}
...
di mana $J>0$ menerangkan gandingan spin jiran terdekat, $i<j$, dan medan melintang global, $h$, ditetapkan kepada $\frac{\pi}{8}$. Semakin jauh $h$ dari sudut Clifford (iaitu $\theta=n\frac{\pi}{2}, n \in \mathbb{Z}$), semakin sukar untuk mempropagat penjana anti-hingar melalui litar.

Untuk pilihan boleh cerap, kita akan mempertimbangkan purata magnet tapak tunggal, $\frac{1}{N} \sum_{i=1}^{N} \langle z_i \rangle$, di mana $N$ ialah bilangan tapak.
...
![Rajah litar kuantum](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_1.png)

Seterusnya, kita akan memilih rantai qubit pada ``ibm_kingston`` yang melaporkan kadar ralat rendah dan mentranspail litar ke Backend.
...
![Rajah litar kuantum](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_2.png)

### Putar lapisan get 2-qubit dan ukuran serta cari lapisan unik {#twirl-the-2-qubit-gate-layers-and-measurements-and-find-unique-layers}

Di sini kita memastikan pengurus laluan (pass manager) menganotasi kotak dengan anotasi ``Twirl`` dan ``InjectNoise``, yang membolehkan kita mempelajari hingar yang akan mempengaruhi litar kita dan mengaitkan hingar tersebut dengan lapisan litar yang sepadan.

- ``enable_gates/enable_measure: True``: Kotakkan semua lapisan get 2q dan ukuran terminal. get qubit tunggal akan dipakaikan di dalam kotak.
- ``measure_annotations: all`` Masukkan anotasi `Twirl` dan `ChangeBasis` pada kotak ukuran
- ``twirling_strategy: active``: Pusing semua qubit aktif dalam setiap kotak yang mengandungi get penjalin
- ``inject_noise_targets: gates``: Anotasi ``InjectNoise`` perlu ditambah pada semua kotak beranotasi ``Twirl`` yang mengandungi get penjalin
...
![Rajah litar kuantum](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_3.png)

### Jana litar templat dan samplex, tentukan cara litar akan disampling {#generate-the-template-circuit-and-samplex-define-how-the-circuit-will-be-sampled}
...
![Rajah litar kuantum](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_4.png)

#### Pelajari hingar {#learn-the-noise}

Sebelum kita menjalankan eksperimen, kita mempelajari model hingar yang mempengaruhi get penjalin dan ukuran dalam litar. Mempunyai model hingar yang tepat adalah perlu untuk mengurangkan ralat dengan berkesan. Mempelajari hingar sejurus sebelum melaksanakan eksperimen memberi peluang terbaik agar model hingar menggambarkan hingar sebenar yang mempengaruhi get semasa pelaksanaan.

Sebelum kita mempelajari hingar, kita perlu mencari lapisan 2-qubit unik dalam litar kita, supaya kita boleh meminimumkan bilangan shot yang diperlukan untuk mempelajari hingar bagi keseluruhan litar. Kita menggunakan ``find_unique_box_instructions`` dari ``samplomatic`` untuk mendapatkan lapisan unik dari litar berkotak, termasuk lapisan ukuran. Lapisan-lapisan inilah yang kita hantar ke pemelajar hingar.
...
- `num_randomizations`: Bilangan litar rawak yang digunakan per konfigurasi litar pembelajaran
- `shots_per_randomization`: Jumlah shot yang digunakan per litar pembelajaran rawak
- `layer_pair_depths`: Kedalaman litar (diukur dalam bilangan pasangan) yang digunakan dalam eksperimen pembelajaran.
- `post_selection`: Kita akan menggunakan pemilihan pasca berasaskan tepi semasa pembelajaran menggunakan get `rx` untuk melaksanakan denyutan pasca-ukuran
...
Apabila menjalankan PNA, Anda perlu sentiasa menyediakan litar berderau dan observable. Jika litar berderau Anda ialah litar berkotak dengan anotasi `InjectNoise`, Anda perlu menyediakan pemetaan yang kita buat dalam langkah di atas. Seseorang juga boleh menghantar litar tanpa kotak yang mengandungi arahan ``PauliLindbladError`` daripada ``qiskit-aer``. Dalam kes itu, ``refs_to_noise_models`` tidak perlu disediakan. Selain input utama, pengguna perlu mempertimbangkan:
