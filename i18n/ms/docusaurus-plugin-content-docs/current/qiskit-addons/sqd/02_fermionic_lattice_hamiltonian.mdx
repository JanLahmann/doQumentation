...
Dalam tutorial ini kita melaksanakan sebuah [corak Qiskit](https://quantum.cloud.ibm.com/docs/guides/intro-to-patterns) yang menunjukkan cara memproses sampel kuantum yang bising selepas pengukuran untuk mencari penghampiran kepada keadaan dasar Hamiltonian kekisi Fermionic yang dikenali sebagai model Anderson single-impurity. Kita akan mengikuti pendekatan pemepenjuruan (pendiagonalan) kuantum berasaskan sampel untuk memproses sampel yang diambil daripada satu set keadaan basis Krylov ``16``-qubit ke atas selang masa yang semakin meningkat. Keadaan-keadaan ini direalisasikan pada peranti kuantum menggunakan Trotterization evolusi masa. Bagi mengambil kira kesan hingar kuantum, teknik pemulihan konfigurasi digunakan. Dengan mengandaikan keadaan awal yang baik dan kejarangan keadaan dasar, [pendekatan ini terbukti menumpu dengan cekap](https://arxiv.org/abs/2501.09702).

Corak ini boleh diterangkan dalam empat langkah:

1. **Langkah 1: Petakan kepada masalah kuantum**

    - Jana satu set keadaan basis Krylov (iaitu litar evolusi masa Trotterized) ke atas selang masa yang semakin meningkat untuk menganggar keadaan dasar
...
4. **Langkah 4: Proses keputusan**
   ...
   - Gelung pemulihan konfigurasi kendiri-konsisten
       ...
       - Unjur dan pepenjurukan Hamiltonian kekisi Fermionic ke atas setiap subruang yang disampel

       - Simpan tenaga keadaan dasar minimum yang ditemui merentasi semua kelompok dan kemas kini purata penghunian orbital


### Langkah 1: Petakan masalah kepada litar kuantum {#step-1-map-problem-to-a-quantum-circuit}
Pertama, kita akan mencipta Hamiltonian satu-jasad dan dua-jasad yang menerangkan model Anderson single-impurity (SIAM) satu dimensi dengan ``7`` tapak mandian (bath) (``8`` elektron dalam ``8`` orbital). Model ini digunakan untuk menerangkan bendasing magnet yang terbenam dalam logam.
...
Seterusnya, kita akan menjana subruang Krylov kuantum dengan satu set litar kuantum Trotterized. Di sini kita mencipta pembantu untuk menjana keadaan awal (rujukan) serta evolusi masa untuk bahagian satu-jasad dan dua-jasad Hamiltonian. Untuk penerangan yang lebih terperinci mengenai model ini dan cara litar direka bentuk, rujuk [kertas kerja](https://arxiv.org/abs/2501.09702).
...
![Rajah litar kuantum](/img/qiskit-addons/sqd/02-fermionic-lattice-hamiltonian/output_1.png)

```python
circuits[-1].draw("mpl", scale=0.4, fold=-1)
```

![Rajah litar kuantum](/img/qiskit-addons/sqd/02-fermionic-lattice-hamiltonian/output_2.png)
...
Setelah mengoptimumkan litar untuk pelaksanaan perkakasan, kita bersedia untuk menjalankannya pada perkakasan sasaran dan mengumpul sampel bagi anggaran tenaga keadaan dasar. Di sini kita menggunakan ``SamplerV2`` daripada ``qiskit-ibm-runtime`` untuk mensimulasikan sampel bising yang diambil daripada backend ``ibm_sherbrooke``. Kemudian kita menggabungkan kiraan daripada setiap keadaan basis Krylov ke dalam satu kamus kiraan tunggal dan memplot 20 bitstring yang paling kerap disampel.
...
![Rajah litar kuantum](/img/qiskit-addons/sqd/02-fermionic-lattice-hamiltonian/output_3.png)
...
Sekarang, kita plot keputusan. Plot pertama menunjukkan bahawa selepas beberapa lelaran, kita mendapat tenaga keadaan dasar yang tepat.

Contoh ini cukup kecil sehingga kita dapat meneroka ruang Hilbert penuh, seperti yang dilihat dalam kenyataan cetak di atas. Ingat, ruang Hilbert penuh adalah dimensi ``(num_orbitals choose nelec_a) * (num_orbitals choose nelec_b)``. Jadi untuk masalah ini: `(8 choose 4)**2 = 4900`. Dimensi subruang bertambah kerana pemulihan konfigurasi yang dipertingkatkan, dan juga hakikat bahawa algoritma SQD membawa konfigurasi penting dari satu lelaran ke lelaran seterusnya. Menjelang lelaran terakhir, kita mendiagonalkan dalam keseluruhan ruang Hilbert.
...
![Output plot](/img/qiskit-addons/sqd/02-fermionic-lattice-hamiltonian/output_4.png)
