...
Tutorial ini menunjukkan cara mengurangkan ralat menggunakan addon Shaded lightcone (SLC). Addon ini merupakan evolusi daripada [teknik pembatalan ralat kebarangkalian (PEC)](https://quantum.cloud.ibm.com/docs/guides/error-mitigation-and-suppression-techniques#probabilistic-error-cancellation-pec), di mana pengguna mempelajari hingar lapisan unik dalam litar dan kemudian membatalkan hingar tersebut dengan menggunakan gate satu-qubit dan teknik pemprosesan pasca. Berbanding kaedah lain, PEC menawarkan sempadan yang lebih teguh terhadap pincang hasil yang dikurangkan, tetapi cenderung mengalami overhed yang lebih tinggi dari segi masa QPU. Semasa PEC, untuk mengimbangi pelemahan nilai jangkaan oleh hingar, hasil purata diskala semula dengan faktor $\gamma = \exp(\sum_{l,\sigma} 2\lambda_{l,\sigma})$, di mana $\lambda_{l,\sigma}$ ialah kadar hingar yang dipelajari bagi ralat Pauli $\sigma$ pada lapisan $l$ dalam litar. Penskalan semula ini meningkatkan varians sebanyak faktor $\gamma^2$, dan dengan itu turut mendarabkan bilangan pelaksanaan litar yang diperlukan pada QPU dengan $\gamma^2$, yang kita panggil kos pensampelan atau overhed pensampelan. Memandangkan $\gamma$ berkembang secara eksponen, PEC sering terhad kepada litar yang cetek atau sedikit qubit. Ketahui lebih lanjut tentang PEC dalam [Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors.](https://arxiv.org/abs/2201.09866)

Jika kita boleh mengenal pasti ralat yang tidak perlu dikurangkan, kita boleh mengurangkan kos pensampelan ini secara eksponen. Langkah pertama ke arah ini ialah melaksanakan pengurangan ralat sedar-tempatan, yang menggunakan "lightcone" konvensional yang boleh dikira dengan cepat untuk mengurangkan overhed PEC dengan membatasi kepekaan observable terhadap ralat di seluruh litar, memperluaskan kebolehlaksanaan PEC ke skala yang lebih besar untuk sesetengah masalah. Ralat di luar lightcone ini tidak dapat mempengaruhi hasil yang diukur dan oleh itu boleh dikecualikan daripada prosedur pembatalan ralat. Pengecualian ini mengurangkan overhed pensampelan, dalam sesetengah kes secara ketara, tanpa memperkenalkan pincang tambahan. Khususnya, untuk mengukur observable tempatan $O$ bagi litar kedalaman tetap, overhed pensampelan yang diperlukan akhirnya mendatar apabila menskalakan bilangan qubit dalam litar (lihat Rajah 2b dalam [Locality and Error Mitigation of Quantum Circuits.](https://arxiv.org/abs/2303.06496))

Lightcone berlorek (SLC) melangkah lebih jauh, menggunakan simulasi klasikal untuk membatasi kepekaan terhadap ralat di seluruh litar dengan lebih ketat. Ini menukar sebahagian masa QPU dengan masa CPU dan mengurangkan overhed pensampelan yang diperlukan untuk menormalkan semula pincang. Bukannya had potongan keras, setiap ralat yang berpotensi dalam litar diberi "lorek" bergred yang membatasi atas kerentanan observable terhadap ralat tersebut. Pencirian yang lebih halus ini membenarkan aplikasi PEC yang lebih cekap dan tersasar dengan varians yang dikurangkan, sambil memberi pengguna keupayaan untuk menala pincang dalam anggaran observable secara terkawal. Lihat [Lightcone shading for classically accelerated quantum error mitigation](https://arxiv.org/abs/2409.04401) untuk maklumat lanjut.
...
a. CPU — Batasi impak setiap ralat bagi ralat Pauli 1- dan 2-qubit
  ...
  2. Perambatan ke belakang (batasi kesan pada keadaan awal). Rambatkan setiap ralat ke permulaan litar dan kira komutatornya dengan keadaan awal.
...
c. CPU — Utamakan pengurangan
  ...
  1. Kemas kini sempadan gabungan dengan kadar hingar yang dipelajari. Gabungkan sempadan ke hadapan dan ke belakang yang telah dikira sebelumnya dan kemas kini dengan kadar hingar yang dipelajari.  

  2. Kedudukan komponen hingar yang perlu dikurangkan menggunakan sempadan dan kadar yang dikira. Utamakan setiap kemungkinan ralat hingar berdasarkan anggaran kesannya terhadap pincang dan kos berkaitan untuk membetulkan. 


d. QPU — Masukkan anti-hingar dan jalankan. Laksanakan litar yang diminati dengan anti-hingar (hingar songsang) yang ditentukan menggunakan anotasi `Box`.

e. CPU — Anggarkan observable. Kira nilai jangkaan, menggunakan pemilihan pasca berasaskan pengukuran untuk mengurangkan impak hingar bukan-Markov.

### Gambaran keseluruhan pembelajaran hingar {#noise-learning-overview}
Pembelajaran hingar ialah langkah umum dalam beberapa kaedah pengurangan ralat, yang dijalankan oleh [NoiseLearner](https://quantum.cloud.ibm.com/docs/en/guides/noise-learning), dan boleh dilihat dalam tutorial [pengurangan ralat PEA](https://quantum.cloud.ibm.com/docs/tutorials/probabilistic-error-amplification) kami, serta [tutorial Penyerapan hingar terambat (PNA)](https://github.com/qiskit-community/qdc-challenges-2025/blob/main/day3_tutorials/Track_A/pna/propagated_noise_absorption.ipynb) kami. Dalam `NoiseLearnerV3`, pengguna boleh mengenal pasti secara khusus lapisan hingar yang akan dipelajari sebagai objek [`CircuitInstruction`](https://quantum.cloud.ibm.com/docs/api/qiskit/qiskit.circuit.CircuitInstruction), yang membolehkan pengguna mengira sempadan hingar SLC yang dikehendaki untuk setiap lapisan dengan cara yang diterangkan di atas. Model Pauli-Lindblad yang dipelajari menyediakan pekali yang akan digunakan dalam keutamaan PEC-SLC. Cara gate dikumpulkan ke dalam lapisan boleh ditentukan menggunakan fungsi kemudahan `generate_boxing_pass_manager` dan `unique_2q_instructions`, kemudian dimasukkan ke dalam fungsi utiliti SLC `generate_noise_model_paulis`, seperti yang diterangkan dalam Langkah 2 di bawah.

| **Bahagian 1** | **Bahagian 2** | **Bahagian 3** |
|-----------|-----------|-----------|
| Pauli-twirl lapisan gate dua-qubit | Ulang pasangan identiti lapisan dan pelajari hingar | Terbitkan kesetiaan (ralat untuk setiap saluran hingar) |
...
Selepas melaksanakan pada perkakasan kuantum menggunakan rangka kerja Samplomatic dan Executor, kami menukar pengukuran rentetan bit kami kepada nilai observable yang dikehendaki. Dalam kes litar Ising cermin kami, kami idealnya akan mendapat observable yang diukur sebesar 1, kerana semua qubit sepatutnya kembali ke titik permulaan mereka iaitu $\ket{0}$. Apabila mengira nilai observable dengan fungsi `expectation_values` kami, kami akan menggunakan beberapa teknik pemprosesan pasca yang mengurangkan impak hingar. Ini termasuk mengalih keluar shot yang terjejas oleh hingar bukan-Markov, pengurangan ralat pembacaan, serta mengambil kira butiran pelaksanaan PEC kami. Butiran dibincangkan dalam Langkah 4 di bawah.
...
Untuk kemudahan demonstrasi, kami memilih rantai Ising cermin 1D. Rantai Ising 1D memberikan struktur litar yang padat dan bagus, yang memudahkan untuk mempamerkan pelaksanaan PEC. litar cermin memudahkan kita mengetahui hasil yang dijangkakan (iaitu, kita sepatutnya mengukur observable sebesar 1).

Selain itu, kami ingin menjalankan litar cermin, jadi untuk setiap gate dalam separuh kedua litar, perlu ada gate songsang dalam separuh pertama. Memandangkan observable yang diukur **$<X_6 Z_{13}>$** mempunyai pengukuran bukan asas-Z, dan Executor mengambil kira asas yang dikehendaki pada penghujung litar, kami menyediakan fungsi `prepare_basis` yang memasukkan gate yang sesuai pada permulaan litar cermin. Butiran ini khusus untuk demonstrasi litar cermin kami. Fungsi `get_measurement_bases` membolehkan kami mengenal pasti dengan mudah gate yang diperlukan dan di mana perlu menambahkannya, serta menjejaki kehalusan indeks qubit yang timbul daripada konvensyen dalam anotasi `box` seperti yang dibincangkan dalam bahagian "Sediakan pengukuran asas kanonikal".
...
Kita akan optimumkan butiran berkaitan dengan litar yang hendak dijalankan, observable yang hendak diukur, dan parameter pembelajaran hingar. Sebagai titik permulaan, kita pastikan backend dinyatakan dengan fractional gates dihidupkan sebagai pilihan. Fractional gates ini membolehkan kepekaan yang lebih tinggi dalam beberapa penapisan pasca-pemilihan kita.
...
Pertama, kita akan transpil litar kita kepada arahan ISA, [seperti yang diperlukan untuk pelaksanaan pada QPU kita](https://www.ibm.com/quantum/blog/isa-circuits). Untuk data yang dikumpul dalam eksperimen ini, kita pilih sendiri qubit kita berdasarkan penilaian rantai berkualiti tertinggi.
...
![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_2.png)

### Kotak litar tersebut {#box-the-circuit}
Untuk kemudahan pelaksanaan, kita akan gunakan laluan transpilasi `generate_boxing_pass_manager`, yang meletakkan arahan litar ke dalam kotak beranotasi. Kotak-kotak ini menunjukkan dengan jelas di mana, dalam kes PEC, antinoise perlu disuntik ke dalam litar. Untuk butiran tetapan, rujuk [dokumentasi Samplomatic.](https://qiskit.github.io/samplomatic/)

Perlu diambil perhatian bahawa aliran kerja SLC menggunakan `inject_noise_strategy="individual_modification"` kemudian dalam proses kerana ini membolehkan kita mengenal pasti setiap `BoxOp` dalam litar secara unik.

Fungsi `find_unique_box_instructions` mengulangi litar berkotak yang diberikan dan mengenal pasti yang mempunyai lapisan 2Q unik atau pengukuran, bagi tujuan pembelajaran hingar dan suntikan hingar.
...
### Alur kerja untuk pembayang kon cahaya, pembelajaran hingar, dan suntikan anti-hingar {#workflow-for-lightcone-shading-noise-learning-and-anti-noise-injection-1}
...
Fungsi `compute_forward_bounds` menilai hubungan komutasi antara get dalam setiap lapisan dan sebutan Pauli yang dihasilkan di atas dari segi bagaimana ralat perambatan ke hadapan mempengaruhi observable yang dikehendaki $A$. Untuk get yang berkomut dengan sebutan Pauli, tiada apa yang dilakukan. Untuk get Clifford, ia ditolak ke arah permulaan litar. Untuk get bukan Clifford, kita menganggarkan pengaruhnya terhadap observable sasaran untuk kemudiannya diprioritikan bagi pembatalan hingar (selepas semua sempadan digabungkan). Sempadan ini dicapai dengan terlebih dahulu menggunakan norma L2 (iaitu, punca kuasa dua bagi jumlah kuasa dua pekali sebutan Pauli yang relevan). Apabila terlalu banyak sebutan qubit terlibat, kita bertukar kepada sempadan yang lebih longgar menggunakan ketaksamaan segitiga.
...
- Kita dapat membezakan dengan jelas dua kon yang timbul daripada dua Pauli bukan identiti dalam observable.
...
#### Visualkan SLC untuk pemeriksaan manual {#visualize-the-slc-for-manual-inspection-1}
...
![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_7.png)

![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_8.png)

![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_9.png)

#### c. Kira sempadan ke belakang {#c-compute-backward-bounds}

Bahagian ramalan hingar ini menilai bagaimana ralat pada lapisan tertentu boleh mempengaruhi keadaan input $\rho$. Fungsi `compute_backward_bounds` terlebih dahulu menyongsangkan litar, membuang get pengukuran, kemudian meneruskan dengan analisis yang serupa seperti yang dilakukan untuk pengiraan sempadan ke hadapan.
...
![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_10.png)

![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_11.png)

![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_12.png)

#### Pratonton sempadan yang digabungkan tanpa kadar hingar yang dipelajari {#preview-merged-bounds-without-learned-noise-rates}

Fungsi `merged_bounds` menentukan titik dalam litar di mana peralihan daripada sempadan ke belakang kepada sempadan ke hadapan meminimumkan jumlah anggaran pincang pada observable yang dikehendaki. Pincang ini dikira sebagai jumlah sumbangan sempadan ke belakang untuk semua lokasi hingar sebelum titik tersebut, ditambah dengan sumbangan sempadan ke hadapan untuk semua lokasi hingar selepasnya. Pada masa ini, ini dilakukan secara seragam untuk semua qubit.
...
![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_13.png)

![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_14.png)

![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_15.png)
...
![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_16.png)

![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_17.png)

![Rajah litar kuantum](/img/qiskit-addons/slc/01-getting-started/output_18.png)
...
2. Laksana litar pengurangan ralat dengan rangka kerja Samplomatic dan Estimator baharu.
...
Pemelajar hingar membolehkan pencirian proses hingar yang mempengaruhi gate dalam satu atau lebih litar yang diminati, berdasarkan model hingar Pauli-Lindblad yang dihuraikan dalam kertas kerja [Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors](https://arxiv.org/abs/2201.09866). Kaedah `run()` melancarkan kerja pembelajaran hingar untuk lapisan 2-qubit unik yang diberikan, berdasarkan pilihan yang ditentukan dalam konfigurasi pemelajar hingar. Dalam pilihan ini, Anda boleh laraskan strategi Pauli-twirling, yang membantu memastikan perkakasan dihuraikan dengan baik oleh model hingar Pauli-Lindblad.

Butiran model hingar Anda berisiko hanyut dari masa ke masa. Oleh itu, kita tetapkan parameter untuk memastikan model hingar yang dipelajari dikira semula bagi eksperimen yang lebih lama daripada empat jam. Ini adalah peraturan kasar dan perlu dipertimbangkan dengan teliti semasa mengaplikasikannya pada kerja Anda sendiri.
...
#### b.ii. Kira `local_scales` untuk pelaksanaan perkakasan {#bii-compute-the-local-scales-for-the-hardware-execution}
...
### c. Laksanakan litar yang diminati dengan antinoise {#c-execute-the-circuit-of-interest-with-antinoise}

#### c.i. Sediakan template litar menggunakan `samplex` {#ci-prepare-template-circuit-by-using-samplex}
