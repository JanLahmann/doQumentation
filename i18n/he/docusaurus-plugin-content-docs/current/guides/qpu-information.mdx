...
<span id="prob-meas0-prep1"></span>
  ...
  פרמטר זה מציין את ההסתברות למדידת Qubit במצב $|0\rangle$ כאשר התכוון להכין אותו במצב $|1\rangle$, המסומן כ-$P(0 | 1)$. הוא משקף שגיאות בהכנת מצב ומדידה (SPAM), בפרט שגיאות מדידה ב-Qubit על-מוליך.
    ...
    <table>
      <tbody>
        <tr>
          <th scope="row">צפייה עם Qiskit</th>
          <td>`backend.properties().qubit_property(<qubit>, 'prob_meas0_prep1')`</td>
        </tr>
        <tr>
          <th scope="row">צפייה ב-IBM Quantum Platform</th>
          <td>קטע נתוני כיול: תפריט נפתח של קיוביט</td>
        </tr>
      </tbody>
    </table>
  </AccordionItem>
...
שגיאה ממוצעת לשער בשכבות (EPLG) בשרשרת של 100 qubit. EPLG ממוצע מודד את שגיאת השער הממוצעת בשרשרת שכבות של $N$ qubit ($N$=100 כאן). הוא נגזר מכמות דומה המכונה נאמנות שכבה (LF) כאשר EPLG$_{100}$ = 4/5(1-LF$^{\frac{1}{99}}$) ונאמנות השכבה היא נאמנות התהליך של שרשרת השכבות של $N$ qubit. לפרטים, ראה את המאמר [Benchmarking quantum processor performance at scale](https://arxiv.org/abs/2311.05933). שים לב שבמאמר EPLG מוגדר עבור שגיאת תהליך, אך לעקביות עם שגיאות השער המדווחות בנפרד כאן הוא מצוטט עבור שגיאת שער ממוצעת, ומכאן הגורם 4/5.
  ...
  ב-IBM Quantum Platform, כרטיס המידע המפורט של כל QPU כולל מקטע בשם Two-qubit gate error (layered), שמציג את התצוגה המורחבת של ה-`two-qubit gate error (layered)` הנמוך ביותר שנמדד כפונקציה של מספר הקיוביטים בשרשרת. הערך הסופי, באורך שרשרת 100, הוא הערך שמוצג במקטע Details. בפועל נמדדות שש שרשראות של 100 קיוביטים (שנבחרו מראש על סמך הביצועים האופטימליים הצפויים), והערך המדווח עבור מספר קיוביטים `N` הוא השגיאה הנמוכה ביותר שנמצאה בתת-שרשרת באורך `N` בחיפוש על פני שש השרשראות של 100 קיוביטים.
...
שגיאת הקריאה מכמתת את ההסתברות הממוצעת למדידה שגויה של מצב qubit. היא מחושבת בדרך כלל כממוצע של `prob_meas0_prep1` ו-`prob_meas1_prep0`, ומספקת מדד יחיד לנאמנות המדידה.
  ...
  נאמנות פעולת הקריאה. שגיאת הקריאה נמדדת על ידי הכנת ה-Qubit במצב $|0\rangle$ ($|1\rangle$) ומדידת ההסתברות לפלט במצב $|1\rangle$ ($|0\rangle$). הערך המדווח הוא הממוצע של שתי שגיאות אלה. החציון לוקח בחשבון את כל ה-Qubit.
  ...
  משך הפעולה של שער דו-קיוביטי. שים לב שהערכים המוצגים ב-IBM Quantum Platform הם בננו-שניות. הערכים המוחזרים ב-Qiskit הם בשניות.
    ...
    <table>
      <tbody>
        ...
        <tr>
          <th scope="row">צפייה ב-IBM Quantum Platform</th>
          <td>קטע כיול: תפריט נפתח של קיוביט</td>
        </tr>
      </tbody>
    </table>
  </AccordionItem>
  <span id="readout-length"></span>
  <AccordionItem title="אורך קריאה">
...
אורך הקריאה מציין את משך פעולת הקריאה עבור qubit. הוא מודד את הזמן מתחילת פולס המדידה ועד להשלמת דיגיטיזציה של האות, לאחר מכן המערכת מוכנה לפעולה הבאה. הבנת פרמטר זה חיונית לאופטימיזציית ביצוע המעגל, במיוחד בעת שילוב מדידות אמצע-מעגל.
  ...
  שגיאת האתחול מכמתת את אחוז האוכלוסייה במצב $|1\rangle$ לאחר השהיית החזרה המחדלית ו[הליך אתחול הקיוביט](/guides/repetition-rate-execution) כאשר הניסוי הקודם הכין את הקיוביט במצב $|1\rangle$. שים לב שזה שונה במעט מהכמות [`prob_meas1_prep0`](#prob-meas1-prep0), שמודדת את אחוז המדידות שהוקצו למצב $|1\rangle$. הכמות האחרונה היא מדידה גם של שגיאת האתחול וגם של הליך ההבחנה בין המצבים $|0\rangle$ ו-$|1\rangle$. בגבול של מדידה מושלמת, שתי הכמויות זהות.
...
<span id="mcps"></span>
<Accordion>
<AccordionItem title="מספר מעגלים מקסימלי לשנייה (MCPS)">

מספר מעגלים מקסימלי לשנייה (MCPS) כולל מדידה, איפוס ואתחול מחדש של מעגל עם שער הדמר ומדידה בודדת.
...
<span id="clops"></span>
<AccordionItem title="CLOPS (או CLOPS_h)">
