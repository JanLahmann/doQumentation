---
title: "דוגמאות לפרימיטיבים"
sidebar_label: "דוגמאות לפרימיטיבים"
description: "דוגמאות מעשיות לשימוש בפרימיטיבים ב-Qiskit Runtime."
...
גרסת הבטא של מודל הרצה חדש זמינה עכשיו. מודל ההרצה המכוון מספק גמישות רבה יותר בהתאמה אישית של תהליך העבודה שלך להפחתת שגיאות. ראה את המדריך [מודל הרצה מכוון](/guides/directed-execution-model) למידע נוסף.
</Admonition>
{/*
...
הדוגמאות בחלק זה ממחישות כמה דרכים נפוצות להשתמש בפרימיטיבים. לפני הרצת הדוגמאות הללו, עקוב אחר ההוראות ב[התקנה והגדרה.](install-qiskit)

<Admonition type="note">
    כל הדוגמאות הללו משתמשות בפרימיטיבים מ-Qiskit Runtime, אך תוכל להשתמש במקום זאת בפרימיטיבים הבסיסיים.
</Admonition>

## דוגמאות ל-Estimator {#estimator-examples}
...
השתמש ב-Estimator כדי לקבוע את ערך הציפייה של זוג Circuit-אובזרבל יחיד.
...
### הרצת ניסויים מרובים בעבודה בודדת {#run-multiple-experiments-in-a-single-job}

השתמש ב-Estimator כדי לקבוע את ערכי הציפייה של זוגות מרובים של Circuit-אובזרבל.
...
בלוק הקוד הבא יחזיר שגיאה עבור משתמשי התוכנית הפתוחה (Open Plan) מכיוון שהוא משתמש ב-sessions. עומסי עבודה בתוכנית הפתוחה יכולים לפעול רק ב[מצב עבודה (job mode)](/guides/execution-modes#job-mode) או ב[מצב אצווה (batch mode)](/guides/execution-modes#batch-mode).
</Admonition>
...
## דוגמאות ל-Sampler {#sampler-examples}
...
### הרצת ניסוי בודד {#run-a-single-experiment-1}
...
### הרצת ניסויים מרובים בעבודה בודדת {#run-multiple-experiments-in-a-single-job-1}
...
### הרצת Circuits עם פרמטרים {#run-parameterized-circuits-1}
...
### שימוש ב-Sessions ואפשרויות מתקדמות {#use-sessions-and-advanced-options-1}
...
<Admonition type="tip" title="המלצות">
    - [ציין אפשרויות runtime מתקדמות.](runtime-options-overview)
    - תרגל עם פרימיטיבים על ידי עבודה על [שיעור פונקציית העלות](/learning/courses/variational-algorithm-design/cost-functions) בלמידת IBM Quantum.
    ...
    - קרא [מעבר לפרימיטיבים V2](/guides/v2-primitives).
    - הבן את [מגבלות העבודה (Job limits)](/guides/job-limits) בעת שליחת עבודה ל-QPU של IBM&reg;.
</Admonition>
