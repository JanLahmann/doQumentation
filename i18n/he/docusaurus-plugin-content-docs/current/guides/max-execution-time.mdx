...
כדי להבטיח הוגנות, קיים [זמן מקסימלי המחושב על ידי השירות](#max-QPU) שכל משימת Qiskit Runtime יכולה לרוץ. אם משימה חורגת ממגבלת זמן זו, היא מבוטלת בכפייה ונזרקת חריגה מסוג `RuntimeJobMaxTimeoutError`. באפשרותך גם [להגדיר זמן ביצוע מקסימלי](#max-job) על המשימות שלך כדי לסייע בשליטה על עלויות.
