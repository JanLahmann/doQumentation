...
כברירת מחדל, קובץ `CMakeLists.txt` מצפה שקובץ הכותרת של Slurm (`slurm.h`) יימצא ב-`/usr/include/slurm`, אך ניתן להתאים זאת כדלקמן.
...
```
לדוגמה, כאשר מתקשרים עם משאבים קוונטיים דרך פרוקסי HTTP, נדרשים משתני הסביבה `http_proxy`, `https_proxy` ו-`no_proxy`. ניתן להוסיף אותם כדלקמן.
```bash
optional /usr/lib64/slurm/spank_qrmi.so /etc/slurm/qrmi_config.json \
...
הטבלה הבאה מציגה לאילו צמתי הקשר של Slurm יש להעתיק את הקבצים הללו. לפרטים נוספים על כל הקשר, ראה ב[תיעוד](https://slurm.schedmd.com/spank.html#SECTION_SPANK-PLUGINS) של תוסף SPANK.
| קבצים | הקשרי Slurm |
| ------- | ------- |
