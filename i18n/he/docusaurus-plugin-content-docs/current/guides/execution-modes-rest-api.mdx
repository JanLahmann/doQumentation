...
תיעוד זה משתמש במודול Python‏ `requests` כדי להדגים את Qiskit Runtime REST API. עם זאת, ניתן לבצע תהליך עבודה זה בכל שפה או מסגרת התומכת בעבודה עם REST APIs. ראה ב[תיעוד ה-API Reference](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/tags/jobs) לפרטים נוספים.
</Admonition>

## מצב Job עם REST API {#job-mode-with-rest-api}

במצב job, מתבצעת בקשה בודדת של Estimator או Sampler ללא מנהל הקשר. ראה כיצד להריץ Circuit קוונטי באמצעות [Estimator](/guides/estimator-rest-api) ו-[Sampler](/guides/sampler-rest-api) לדוגמאות.

## מצב Session עם REST API {#session-mode-with-rest-api}

session הוא תכונה של Qiskit Runtime המאפשרת להריץ בצורה יעילה עומסי עבודה איטרטיביים רב-משימתיים במחשבים קוונטיים. שימוש ב-sessions מסייע למנוע עיכובים הנגרמים מהמתנה בתור לכל משימה בנפרד, דבר שיכול להיות שימושי במיוחד למשימות איטרטיביות הדורשות תקשורת תכופה בין משאבים קלאסיים לקוונטיים. פרטים נוספים על Sessions ניתן למצוא ב[תיעוד](/guides/execution-modes).
...
לחלופין, ניתן לשלוח משימת batch על ידי ציון ה-`mode` ב-payload של הבקשה. מצב batch יכול לסייע בקיצור זמן העיבוד אם ניתן לספק את כל המשימות מראש. למד על מצב batch במדריך [מבוא למצבי הרצה](/guides/execution-modes#batch-mode).
...
- ראה בדוגמאות מפורטות של [Sampler](/guides/sampler-rest-api) primitives באמצעות REST API.
- ראה בדוגמאות מפורטות של [Estimator](/guides/estimator-rest-api) primitives באמצעות REST API.
...
- למד כיצד לבצע Transpile מקומי בחלק [Transpile](/guides/transpile).
</Admonition>
