...
שלב ראשון זה עושה מעט מאוד כברירת מחדל והוא שימושי בעיקר אם רוצה לכלול אופטימיזציות ראשוניות משלו. מכיוון שרוב אלגוריתמי הפריסה והניתוב מתוכננים לעבוד רק עם שערים של qubit יחיד ושני qubits, שלב זה משמש גם לתרגום שערים שפועלים על יותר משני qubits לשערים שפועלים רק על אחד או שניים.
...
השלב הבא עוסק בפריסה או בקישוריות של ה-Backend שאליו ישלח מעגל. באופן כללי, מעגלים קוונטיים הם ישויות מופשטות שה-Qubits שלהן הם ייצוגים "וירטואליים" או "לוגיים" של qubits פיזיים בפועל המשמשים בחישובים. כדי לבצע רצף של שערים, יש צורך במיפוי אחד-לאחד מ-Qubits "וירטואליים" ל-Qubits "פיזיים" במכשיר קוונטי ממשי. מיפוי זה מאוחסן כאובייקט `Layout` והוא חלק מהמגבלות המוגדרות בתוך [ארכיטקטורת מערכת ההוראות (ISA)](/guides/transpile#instruction-set-architecture) של Backend.

![תמונה זו ממחישה qubits הממופים מייצוג החוט לדיאגרמה המייצגת כיצד ה-Qubits מחוברים ב-QPU.](/docs/images/guides/transpiler-stages/layout-mapping.svg "מיפוי qubit")
...
- `SabreLayout`: pass זה בוחר פריסה על ידי התחלה מפריסה אקראית ראשונית והפעלה חוזרת של אלגוריתם `SabreSwap`. pass זה משמש רק ברמות אופטימיזציה 1, 2 ו-3 אם לא נמצאת פריסה מושלמת דרך ה-`VF2Layout` pass. לפרטים נוספים על אלגוריתם זה, ראה בעיתון [arXiv:1809.02573](https://arxiv.org/abs/1809.02573).
...
התפלגות רחבה זו מדגימה עד כמה קשה לממפה ה-SWAP לחשב את המיפוי הטוב ביותר. כדי לקבל תובנה, נסתכל גם על המעגל המבוצע וגם על ה-Qubits שנבחרו בחומרה.
...
![תצוגת timeline.draw() של אותו מעגל](/docs/images/guides/transpiler-stages/transpiler-17.svg)
