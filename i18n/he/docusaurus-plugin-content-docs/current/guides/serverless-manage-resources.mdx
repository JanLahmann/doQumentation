...
<Admonition type="tip">
**Qiskit Serverless עובר שדרוג, והתכונות שלו משתנות במהירות.** בשלב פיתוח זה, מצא הערות גרסה ואת התיעוד העדכני ביותר בדף [Qiskit Serverless GitHub](https://qiskit.github.io/qiskit-serverless/index.html).
</Admonition>
...
אם אתה קורא את תאי הקוד מקומית במחברת, תראה את [פקודת ה-magic](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile`. הרצת תאים עם פקודת magic זו שומרת אותם לדיסק במקום להריץ אותם.
</Admonition>
...
למשימות קלאסיות שניתן לבצע במקביל, השתמש בדקורטור `@distribute_task` להגדרת דרישות המחשוב הנדרשות לביצוע משימה. התחל בהיזכרות בדוגמת `transpile_remote.py` מנושא [כתיבת תוכנית Qiskit Serverless הראשונה שלך](/guides/serverless-first-program) עם הקוד הבא.
...
הקוד הבא מציג כיצד להעלות קבצים. ודא שאמת את עצמך ל-Qiskit Serverless עם חשבון IBM Quantum שלך (ראה [העלאה ל-Qiskit Serverless](/guides/serverless-first-program#upload-to-qiskit-serverless) להוראות).
...
- ראה דוגמה מלאה שעושה [port לקוד קיים ל-Qiskit Serverless](/guides/serverless-port-code).
