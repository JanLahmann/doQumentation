...
sidebar_label: 'הרץ את ה-Circuit הראשון שלך על חומרה'
title: 'הרץ את ה-Circuit הראשון שלך על חומרה'
---
...
דוגמה זו מכילה שני חלקים. תחילה תיצור תוכנית קוונטית "Hello world" פשוטה ותריץ אותה על יחידת עיבוד קוונטי (QPU). מכיוון שמחקר קוונטי אמיתי דורש תוכניות מאוד מקיפות, בחלק השני ([הרחבה למספרים גדולים של קיוביטים](#scale-to-large-numbers-of-qubits)), תרחיב את התוכנית הפשוטה לרמת שימושיות.
...
1. אם עדיין לא התקנת את Qiskit, מצא הוראות במדריך [Quickstart](/guides/quick-start).
    ...
    - התקן את לקוח IBM Quantum Compute כדי להריץ משימות על חומרה קוונטית:
...
2. הגדר את האימות שלך לגישה לחומרה קוונטית דרך ה-[Open Plan](/guides/plans-overview#open-plan) החינמי.
    ...
    - היכנס ל-[IBM Quantum Platform](https://quantum.cloud.ibm.com/) כדי להתחבר או ליצור חשבון.
         ...
         אם אתה מתחבר דרך שרת פרוקסי, עליך להשתמש ב-`qiskit-ibm-runtime` גרסה 0.44.0 ומעלה.
         </Admonition>
...
3. עכשיו תוכל להשתמש בקוד Python הזה בכל פעם שתרצה לאמת מול IBM Quantum Compute Service:
...
מחשבים קוונטיים יכולים לייצר תוצאות אקראיות, לכן בדרך כלל אוספים דגימה של הפלטים על ידי הרצת ה-Circuit פעמים רבות. ניתן להעריך את ערך ה-observable באמצעות מחלקת `Estimator`. `Estimator` הוא אחד משני [primitives](/guides/get-started-with-estimator); השני הוא `Sampler`, שניתן להשתמש בו לקבלת נתונים ממחשב קוונטי. לאובייקטים אלה יש שיטת `run()` שמבצעת את בחירת ה-Circuit, האופרטורים והפרמטרים (אם רלוונטי), באמצעות [primitive unified bloc (PUB)](/guides/get-started-with-sampler).
...
:::note[חלופה: הרצת הדוגמה באמצעות סימולטור]


  כאשר אתה מריץ את התוכנית הקוונטית שלך על מכשיר אמיתי, העומס שלך חייב להמתין בתור לפני שירוץ. כדי לחסוך זמן, תוכל להשתמש בקוד הבא כדי להריץ עומס קטן זה על [`fake_provider`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/fake-provider) עם מצב הבדיקה המקומי של `qiskit-ibm-runtime`. שים לב שזה אפשרי רק עבור Circuit קטן. כאשר תרחיב בסעיף הבא, יהיה עליך להשתמש במכשיר אמיתי.
...
הגש את המשימה והפעל דיכוי שגיאות באמצעות טכניקה להפחתת שגיאות הנקראת [dynamical decoupling](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-dynamical-decoupling-options). רמת החוסן מציינת כמה חוסן לבנות מפני שגיאות. רמות גבוהות יותר מייצרות תוצאות מדויקות יותר, על חשבון זמני עיבוד ארוכים יותר. להסבר נוסף על האפשרויות שנקבעו בקוד הבא, ראה [הפחתת שגיאות ודיכוי שגיאות](/guides/error-mitigation-and-suppression-techniques).
