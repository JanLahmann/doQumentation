...
השערים החד-קיוביטיים המחדליים ב-`FakeSherbrooke` הם `rz`, `x` ו-`sx`, והשער הדו-קיוביטי המחדלי הוא `ecr` (echoed cross-resonance). שערי CX נבנים משערי `ecr`, כך שבחלק מה-QPU-ים `ecr` מוגדר כשער הבסיס הדו-קיוביטי, בעוד שבאחרים `cx` הוא ברירת המחדל. השער `ecr` הוא החלק ה_משזר_ של השער `cx`. בנוסף לשערי הבקרה, קיימות גם הוראות `delay` ו-`measurement`.
