---
title: "ייצוג מחשבים קוונטיים עבור ה-Transpiler"
sidebar_label: "ייצוג מחשבים קוונטיים עבור ה-Transpiler"
...
- ה-[`Target`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.Target) מכיל את כל האילוצים הרלוונטיים של מכשיר, כגון סט ההוראות הנתמך שלו, קישוריות Qubit, ומידע פולס או תזמון.
- ה-[`Backend`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2) מחזיק `Target` כברירת מחדל, מכיל מידע נוסף -- כגון [`InstructionScheduleMap`](https://docs.quantum.ibm.com/api/qiskit/1.4/qiskit.pulse.InstructionScheduleMap), ומספק את הממשק להגשת עבודות Circuit קוונטי.
...
דף זה מציג מספר דוגמאות של העברת מידע QPU ל-Transpiler.
...
נושא זה מציג מספר דוגמאות של העברת מידע ל-Transpiler.

<Admonition type="note">
דוגמאות אלו משתמשות ב-target מה-`qiskit_ibm_runtime` [`FakeSherbrooke`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/fake-provider-fake-sherbrooke#fakesherbrooke) Backend המדומה. עם זאת, תוכל לנסות זאת על כל Backend אמיתי או מדומה התואם ל-Qiskit. התוצאות שלך עשויות להיות שונות.
</Admonition>
...
דוגמה זו משמשת בסעיפים מאוחרים יותר של נושא זה להמחשה שמפת הקישוריות וסט ההוראות הנתמך הם מקטעי המידע החיוניים להעביר ל-Transpiler לבנייה אופטימלית של Circuit. ה-QPU יכול בדרך כלל לבחור הגדרות ברירת מחדל למידע אחר שאינו מועבר, כגון תזמון ולוח זמנים.
...
כל מחשב קוונטי תומך בסט הוראות מוגבל. כל Gate ב-Circuit חייב להיות מתורגם לאלמנטים של סט זה. סט זה צריך להיות מורכב מ-Gate חד- ודו-Qubit המספקים סט Gate אוניברסלי, כלומר כל פעולה קוונטית ניתנת לפירוק לאותם Gate. זה נעשה על ידי [BasisTranslator](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.passes.BasisTranslator), וניתן לציין את `basis_gates` כארגומנט מילת מפתח ל-Transpiler לספק מידע זה.
...
ה-Gate חד-Qubit ברירת המחדל על `FakeSherbrooke` הם `rz`, `x`, ו-`sx`, וה-Gate דו-Qubit ברירת המחדל הוא `ecr` (echoed cross-resonance). Gate CX בנויים מ-Gate `ecr`, לכן על חלק מה-QPU `ecr` מצוין כ-Gate הבסיס דו-Qubit, בעוד על אחרים `cx` הוא ברירת המחדל. Gate ה-`ecr` הוא החלק _המשזר_ של Gate ה-`cx`. בנוסף ל-Gate הבקרה, ישנן גם הוראות `delay` ומדידה.

<Admonition type="note">
ל-QPU יש Gate בסיס ברירת מחדל, אך תוכל לבחור כל Gate שתרצה, כל עוד אתה מספק את ההוראה או מוסיף Gate פולס (ראה [יצירת מעברי Transpiler](/guides/custom-transpiler-pass).) Gate הבסיס ברירת המחדל הם אלה שכיול בוצע עליהם ב-QPU, לכן אין צורך לספק הוראות/Gate פולס נוספים. לדוגמה, על חלק מה-QPU `cx` הוא ה-Gate דו-Qubit ברירת המחדל ו-`ecr` על אחרים. ראה את רשימת ה-[Gate ופעולות ילידיות אפשריות](/guides/qpu-information#native-gates) לפרטים נוספים.
</Admonition>
...
- ראה בנושא [פרמטרים נפוצים לשימוש ב-Transpilation](common-parameters).
