...
ב-DAG יש שלושה סוגי צמתים בגרף: צמתי קלט של qubit/clbit (ירוק), צמתי פעולה (כחול), וצמתי פלט (אדום). כל קשת מציינת זרימת נתונים (או תלות) בין שני צמתים. יש להשתמש בפונקציה qiskit.tools.visualization.dag_drawer() כדי לראות את ה-DAG של המעגל הזה. (יש להתקין את [ספריית Graphviz](https://graphviz.org/download/) כדי להריץ זאת.)
...
לאחר השלמת כל הטרנספורמציות, ניתן להמיר את ה-DAG בחזרה לאובייקט `QuantumCircuit` רגיל. כך פועל צינור הטרנספילציה. מעגל נלקח, מעובד בצורת DAG, ומעגל מותמר מיוצר כפלט.
...
ניתן לנצל את מבנה ה-DAG לכתיבת מעברי טרנספילציה. בדוגמה הבאה, מממשים מעבר `BasicMapper` שממפה מעגל שרירותי למכשיר עם קישוריות qubit מוגבלת. להנחיות נוספות, ניתן לפנות למדריך על [כתיבת מעברי טרנספילציה מותאמים אישית](/guides/custom-transpiler-pass).
...
כעת ניתן לבדוק את המעבר על מעגל דוגמה קטן. בונים מנהל מעברים עם המעבר החדש שהוגדר. מעגל הדוגמה מסופק למנהל מעברים זה, ומעגל חדש ומותמר מתקבל כפלט.
...
- ראה במדריך על יצירת [מעבר טרנספילציה מותאם אישית](/guides/custom-transpiler-pass)
- למד כיצד [ליצור ולטרנספל מול backends מותאמים אישית](/guides/custom-backend)
- נסה את מדריך [השוואת הגדרות טרנספילציה](/guides/circuit-transpilation-settings).
- ראה ב[תיעוד API של DAG Circuit](https://docs.quantum.ibm.com/api/qiskit/dagcircuit).
</Admonition>
