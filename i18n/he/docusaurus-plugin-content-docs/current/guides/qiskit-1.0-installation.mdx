---
title: התקנת חבילת Qiskit v1.0 החדשה
description: התאמה לשינויים בהתקנה ותלות של Qiskit v1.0
---
...
# התקנת חבילת Qiskit v1.0 החדשה {#install-the-new-qiskit-v1-0-package}
...
* אם נתקלת בבעיות בהתקנה או ייבוא של Qiskit v1.0, ראה בסעיף [פתרון בעיות](#troubleshooting).

אם אתה מעוניין להבין את מבנה החבילה הישן ומדוע הוא השתנה, ראה ב[סקירה של שינויי האריזה השוברים](/guides/metapackage-migration).

<span id="for-users"></span>
## למשתמשים {#for-users}

עליך להתחיל סביבה וירטואלית חדשה כדי להתקין את Qiskit v1.0.
שדרוג התקנה קיימת במקום ל-Qiskit v1.0 הוא מסובך מאוד ועלול לגרום לשגיאות.
...
3. התקן חבילות לפי הצורך.
   ...
   <Admonition type="caution">
   אל תתקין חבילות שאינן תואמות ל-Qiskit v1.0 בסביבה וירטואלית זו. אם עליך להשתמש בחבילות כאלה, התקן אותן בסביבה וירטואלית נפרדת עם Qiskit 0.45 או 0.46.
   </Admonition>

   אם יש לך סביבה קיימת, תוכל להשתמש ב-[`pipdeptree`](https://github.com/tox-dev/pipdeptree/blob/main/README.md#pipdeptree) כדי לשאול את דרישות החבילות המותקנות שלך ולראות אם הן דורשות `qiskit<1`. לכל חבילה שדורשת `qiskit<1`, בדוק אם קיימים עדכונים שהופכים אותה תואמת ל-Qiskit v1.0.
...
מועמד לשחרור של Qiskit v1.0, גרסה 1.0.0rc1, יושחרר ב-1 בפברואר 2024.
עליך לבדוק את החבילה שלך מולו, ובהקדם האפשרי, לשחרר גרסה חדשה (תואמת) של החבילה שלך עם דרישת הגג שלה ללא הגבלה.

### המלצות לבדיקה מול Qiskit v1.0 {#recommendations-for-testing-against-qiskit-v1-0}
...
אם לפחות אחת מהתלויות הטרנזיטיביות שלך עדיין אין לה גרסת שחרור שמאפשרת תמיכה ב-Qiskit v1.0, עליך לבצע שינויים ידניים.
...
* החרג את השימוש בתלות זו מסביבת הבדיקה, אם אפשרי.
...
3. התקן את גרסת היעד של Qiskit v1.0 עם אחת מהפקודות הבאות:
...
תוכל (בנימוס) לשאול את המפתחים הללו מתי ישחררו גרסה חדשה של החבילה שלהם התואמת ל-Qiskit v1.0, אבל תחילה בדוק שאין להם סוגיה פתוחה או בקשת משיכה המבקשת זאת בכל מקום שהם מקבלים תגובות.
שים לב שזה לוקח זמן; תן למפתחים כחודש לקראת גרסאות חדשות של ההפצות שלהם!
עד אז, לא ניתן להתקין את ההפצה הזו לצד Qiskit v1.0.
...
#### יצירת סביבה עובדת ל-Qiskit v1.0 {#create-a-working-environment-for-qiskit-v1-0}
...
ראיית תלות ב-`qiskit-terra` יכולה להעיד על אחד מכמה דברים:

* התלות היא חבילה ישנה, ולא תתעדכן לתמיכה ב-Qiskit v1.0.

  במקרה זה, אין סיכוי להשתמש בחבילה עם Qiskit v1.0, ותצטרך להמשיך להשתמש בגרסה קודמת של Qiskit.
...
אם לפחות הפצה אחת דורשת Qiskit v1.0 או גדול יותר, `pip` אמור לתת לך הודעת שגיאה המסבירה זאת, שנראית כמו זו ב[סעיף על כשלי פתרון](#pip-resolution-impossible).

תוכל גם להשתמש בפקודות `pipdeptree` המפורטות ב[יצירת סביבה עובדת ל-Qiskit v1.0](#debug-venv-for-1.0) מתוך הסביבה השבורה כדי לקבוע אילו הפצות יש להן דרישה מפורשת על `qiskit>=1`.
