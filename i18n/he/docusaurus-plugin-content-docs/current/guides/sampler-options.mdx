...
אפשר להגדיר אפשרויות בעת אתחול Sampler, אחרי אתחול Sampler, או שניתן לעדכן את האפשרויות לאחר ש-Sampler אותחל. להנחיות לשימוש בטכניקות אלו, ראה את הנושא [מבוא לאפשרויות](/guides/runtime-options-overview#options-precedence).

בנוסף, אפשר להגדיר את הערך `shots` בשיטה `run()`, כפי שמתואר בקטע הבא.
...
4. אם `twirling` מופעל, אזי משמש מכפלת `num_randomizations` ו-`shots_per_randomization`, כפי שצוינו כ-[אפשרויות `twirling`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-twirling-options).
