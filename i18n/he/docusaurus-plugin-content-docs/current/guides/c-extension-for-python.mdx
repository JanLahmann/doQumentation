...
יש להשלים את ההוראות בנושא [התקנה](/guides/install-c-api) כדי להתקין את הדברים הבאים:

- שרשרת הכלים הסטנדרטית של מהדר C עבור הפלטפורמה שלך

- גרסה של Python הכוללת את headers ה-C API שלה. 


כדאי שתהיה מוכר גם, או מוכן לחפש, את הפונקציות והאובייקטים הזמינים
ב-[Qiskit C API](https://docs.quantum.ibm.com/api/qiskit-c), וכדאי שתהיה לך היכרות מסוימת עם תכנות ב-C.
...
יש להגדיר את גרסת ה-runtime של Qiskit ב-
`project.dependencies` כך שתתאים לגרסת המשנה (minor version) שבה נעשה שימוש בזמן ה-build.
