---
title: מעבר ל-Qiskit Runtime V2 primitives
...
```
  ...
  <TabItem value="SamplerOut" label="המרת פלט V2 לפורמט V1">
    פורמט הפלט של V1 היה מילון של bitstrings (כמספר שלם) כמפתח ו-quasi-probabilities כערך עבור כל Circuit. פורמט V2 כולל את אותו מפתח (אבל כמחרוזת) ו-counts כערך. כדי להמיר את פורמט V2 ל-V1, מחלקים את ה-counts במספר ה-shots, כאשר מספר ה-shots שנבחר מתואר במדריך [אפשרויות Sampler](/guides/sampler-options#shots).
...
3. ראה בכל [האפשרויות הנתמכות](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options) ועדכן בהתאם.
...
5. עדכן את מתודת `run()` של Estimator כדי להעביר את רשימת ה-PUBs. לדוגמה, `run([(circuit1, observable1, parameter_set1)])`.
...
5. עדכן את מתודת `run()` של Sampler כך שתעביר את רשימת ה-PUBs. לדוגמה, `run([(circuit1, parameter_set1)])`.
...
### הרצת ניסויים מרובים בעבודה אחת {#run-multiple-experiments-in-a-single-job}

השתמש ב-Sampler כדי לקבוע את הספירות או התפלגויות הקוואזי-הסתברות של מספר Circuits בעבודה אחת.
...
הרץ מספר ניסויים בעבודה אחת, תוך ניצול ערכי פרמטרים להגברת שימוש חוזר ב-Circuit.
...
<Admonition type="caution">
בלוק הקוד הבא יחזיר שגיאה למשתמשים בתוכנית הפתוחה (Open Plan) כי הוא משתמש ב-Sessions. עומסי עבודה בתוכנית הפתוחה יכולים לרוץ רק ב[מצב עבודה](/guides/execution-modes#job-mode) או [מצב אצווה](/guides/execution-modes#batch-mode).
</Admonition>
...
- למד עוד על הגדרת אפשרויות במדריכי [אפשרויות Sampler](/guides/sampler-options), [אפשרויות Estimator](/guides/estimator-options), ו-[אפשרויות Executor](/guides/executor-options).
- למד פרטים נוספים על [קלטים ופלטים של Primitive](/guides/primitive-input-output#pubs).
