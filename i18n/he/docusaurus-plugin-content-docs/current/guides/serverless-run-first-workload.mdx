...
ודא שעקבת אחרי זרימת העבודה ב[כתוב את תוכנית Qiskit Serverless הראשונה שלך](/guides/serverless-first-program) לפני שתתחיל.

## רשימת תוכניות זמינות {#list-programs-available}

תוכל להשתמש ב-`QiskitServerless.list()` כדי לאחזר רשימה של התוכניות הזמינות להרצה עם Qiskit Serverless. זה כולל את [`transpile_remote_serverless`](/guides/serverless-first-program) שהועלה בעבר.
...
תוכל גם להגדיר סטטוסי עבודה מפורטים יותר ב[ניהול משאבי מחשוב ונתונים ב-Qiskit Serverless](/guides/serverless-manage-resources).
...
## רשימת עבודות שהורצו בעבר עם Qiskit Serverless {#list-jobs-previously-run-with-qiskit-serverless}
...
- חקור [כלי ניהול חישוב ונתונים](/guides/serverless-manage-resources) הזמינים לתוכנית שלך, כולל הרצה מקבילית.
