...
ארגומנטי הקלט משמשים לאחר מכן לבניית מעגלים קוונטיים ואופרטורים רלוונטיים לזרימת העבודה. נוצר מעגל מטרה, ומוצג ייצוג מצב מכפלת מטריצות (MPS) של המעגל הזה בעזרת תוסף AQC. בהמשך, נוצר מעגל ansatz ומבוצעת אופטימיזציה שלו בשיטות רשת טנסורים, ומייצרת מעגל סופי שמבצע את שארית התפתחות הזמן.
...
היבט חשוב להבטחת שהתבנית ניתנת לשימוש חוזר על פני טווח קלטים הוא אימות קלט. הקוד הבא הוא דוגמה לאימות שנאמנות העצירה במהלך AQC-Tensor צוינה כראוי, ואם לא, החזרת הודעת שגיאה אינפורמטיבית כיצד לתקן את השגיאה.
...
לאחר שהמשימה פועלת, ניתן לאחזר לוגים שנוצרו מפלטי `print()`. אלה יכולים לספק מידע מעשי לגבי התקדמות זרימת העבודה של סימולציית המילטוניאן. לדוגמה, ערך פונקציית המטרה במהלך הרכיב האיטרטיבי של AQC, או עומק שני-הקיוביטים של מעגל ה-ISA הסופי המיועד לביצוע על חומרה.
