...
גרסת הבטא של מודל הרצה חדש זמינה כעת. מודל ה-Directed execution מספק גמישות רבה יותר בהתאמה אישית של זרימת העבודה להפחתת שגיאות. ראה במדריך [Directed execution model](/guides/directed-execution-model) לקבלת מידע נוסף.
</Admonition>
...
פרימיטיב Estimator מחשב ערכי ציפייה עבור אובזרבל אחד או יותר ביחס למצבים המוכנים על ידי Circuits קוונטיות. ה-Circuits יכולות להיות בעלות פרמטרים, כל עוד ערכי הפרמטרים ניתנים גם הם כקלט לפרימיטיב.
...
ה-Estimator משלב אלמנטים מאובזרבלים וערכי פרמטרים בעקבות כללי שידור NumPy כמתואר בנושא [קלט ופלט של פרימיטיבים](primitive-input-output#broadcasting-rules).
...
הפלט הוא ספירות או מדידות לכל shot, כאובייקטים של [`PubResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PubResult#pubresult), ללא משקלים. עם זאת, למחלקת התוצאות יש מתודות להחזרת דגימות משוקללות, כגון ספירות. ראה ב[קלט ופלט של פרימיטיבים](primitive-input-output#broadcasting-rules) לפרטים מלאים.
...
<Admonition type="tip" title="המלצות">
    ...
    - ראה ב[דוגמאות מפורטות של פרימיטיבים.](primitives-examples)
    ...
    - ראה ב[עזר API של EstimatorV2](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/estimator-v2) וב[עזר API של SamplerV2](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2).
