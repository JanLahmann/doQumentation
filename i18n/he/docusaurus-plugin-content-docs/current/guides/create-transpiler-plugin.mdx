...
בחלקים הבאים נציג דוגמאות לשלבים אלו עבור סוגי התוספים השונים. בדוגמאות אלו אנחנו מניחים שאנחנו יוצרים חבילת Python בשם `my_qiskit_plugin`. למידע על יצירת חבילות Python, תוכל לעיין ב-[מדריך זה](https://packaging.python.org/en/latest/tutorials/packaging-projects/) באתר Python.

## דוגמה: יצירת תוסף שלב Transpiler {#example-create-a-transpiler-stage-plugin}

בדוגמה זו, אנחנו יוצרים תוסף שלב טרנספיילר עבור שלב ה-`layout` (ראה [שלבי הטרנספיילר](transpiler-stages) לתיאור של 6 השלבים של צינור הטרנספילציה המובנה של Qiskit).
התוסף שלנו פשוט מריץ את [VF2Layout](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.passes.VF2Layout) למספר ניסיונות שתלוי ברמת האופטימיזציה המבוקשת.

ראשית, אנחנו יוצרים תת-מחלקה של [PassManagerStagePlugin](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.preset_passmanagers.plugin.PassManagerStagePlugin). יש שיטה אחת שאנחנו צריכים ליישם, שנקראת [`pass_manager`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.preset_passmanagers.plugin.PassManagerStagePlugin#pass_manager). שיטה זו מקבלת כקלט [PassManagerConfig](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.PassManagerConfig) ומחזירה את מנהל המעברים שאנחנו מגדירים. אובייקט ה-PassManagerConfig שומר מידע על ה-backend היעד, כמו מפת הצימוד ושערי הבסיס שלו.
...
עכשיו, אנחנו חושפים את התוסף על ידי הוספת נקודת כניסה במטא-דאטה של חבילת ה-Python שלנו.
כאן, אנחנו מניחים שהמחלקה שהגדרנו חשופה במודול שנקרא `my_qiskit_plugin`, לדוגמה על ידי כך שהיא מיובאת בקובץ `__init__.py` של המודול `my_qiskit_plugin`.
אנחנו עורכים את הקובץ `pyproject.toml`, `setup.cfg`, או `setup.py` של החבילה שלנו (בהתאם לסוג הקובץ שבחרת לאחסן בו את המטא-דאטה של פרויקט ה-Python שלך):
...
ראה את [טבלת שלבי תוסף הטרנספיילר](https://docs.quantum.ibm.com/api/qiskit/transpiler_plugins#stage-table) לנקודות הכניסה והציפיות עבור כל שלב טרנספיילר.
...
אינם מספיקים למטרותיך, [פתח issue](https://github.com/Qiskit/qiskit/issues/new/choose) שמסביר את הדרישות שלך. שינויים בממשק התוסף, כגון הוספת קלטים אופציונליים נוספים, יתבצעו בצורה תואמת לאחור כדי שלא יידרשו שינויים מתוספים קיימים.
...
כאן אנחנו מניחים שהמחלקה שהגדרנו חשופה במודול בשם `my_qiskit_plugin`, לדוגמה על ידי ייבואה בקובץ `__init__.py` של מודול `my_qiskit_plugin`.
...
אם התוסף לדוגמה שלנו היה מותקן, אז השם `my_clifford_synthesis` היה מופיע ברשימה זו.
...
- בדוק את [המדריכים](https://docs.quantum.ibm.com/tutorials) לדוגמאות של טרנספילציה והרצה של מעגלים קוונטיים.
</Admonition>
