---
description: 'הבנת פורמט הקלט והפלט של פרימיטיבי Sampler'
notebook_path: guides/sampler-input-output.ipynb
sidebar_label: 'קלטים ופלטים של Sampler'
title: 'קלטים ופלטים של Sampler'
---
...
- `QuantumCircuit` בודד, שעשוי להכיל אובייקט [`Parameter`](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.Parameter) אחד או יותר
   *הערה: מעגלים אלה צריכים לכלול גם הוראות מדידה לכל אחד מהקיוביטים שיש לדגום.*
- אוסף ערכי פרמטרים לקשירת המעגל לפיהם $\theta_k$ (נדרש רק אם נעשה שימוש באובייקטי `Parameter` כלשהם שיש לקשור אותם בזמן ריצה)
- (אופציונלי) מספר shots למדידת המעגל
---

הקוד הבא מדגים דוגמה של קלטים ממוּדרגים לפרימיטיב `Sampler` ומריץ אותם על Backend של IBM&reg; כאובייקט `RuntimeJobV2 ` בודד.
...
לאחר שאחד או יותר PUBs נשלחים ל-QPU לביצוע ומשימה מסתיימת בהצלחה, הנתונים מוחזרים כאובייקט מיכל [`PrimitiveResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PrimitiveResult) שניגש אליו על ידי קריאה לשיטת `RuntimeJobV2.result()`. ה-`PrimitiveResult` מכיל רשימה ניתנת לחזרה של אובייקטי [`SamplerPubResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.SamplerPubResult) המכילים את תוצאות הביצוע לכל PUB. נתונים אלה הם דגימות של פלט המעגל.
...
לכל אובייקט `SamplerPubResult` יש גם מאפיין `data` וגם מאפיין `metadata`.
    - מאפיין `data` הוא [`DataBin`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.DataBin) מותאם אישית המכיל את ערכי המדידה בפועל, סטיות תקן וכן הלאה. מיכלי הנתונים הם אובייקטים דמויי-מילון המכילים `BitArray` אחד לכל `ClassicalRegister` במעגל.
    ...
    - מאפיין `metadata` מכיל מידע על אפשרויות ה-Runtime שנעשה בהן שימוש (מוסבר מאוחר יותר בסעיף [מטא-דאטה של תוצאות](#result-metadata) בדף זה).
...
במילים פשוטות, משימה בודדת מחזירה אובייקט [`PrimitiveResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.PrimitiveResult) ומכילה רשימה של אחד או יותר אובייקטי [`SamplerPubResult`](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.SamplerPubResult). אובייקטי `SamplerPubResult` אלה מאחסנים את נתוני המדידה לכל PUB שהוגש למשימה.
...
### שימוש באובייקטי `BitArray` לעיבוד לאחר יעיל {#use-bitarray-objects-for-performant-post-processing}
...
## מטא-דאטה של תוצאות {#result-metadata}

בנוסף לתוצאות הביצוע, גם אובייקט `PrimitiveResult` וגם אובייקט `SamplerPubResult` מכילים מאפיין מטא-דאטה על המשימה שהוגשה. המטא-דאטה המכילה מידע לכל ה-PUBs שהוגשו (כגון [אפשרויות ה-Runtime](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options) השונות הזמינות) ניתן למצוא ב-`PrimitiveResult.metatada`, בעוד שהמטא-דאטה הספציפית לכל PUB נמצאת ב-`SamplerPubResult.metadata`.

מטא-דאטה של תוצאות Sampler כוללת גם מידע עיתוי ביצוע הנקרא [_טווחי ביצוע_](#execution-spans).
...
### הצגת טווחי ביצוע {#view-execution-spans}

תוצאות משימות [`SamplerV2`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2) המורצות ב-IBM Quantum Compute Service מכילות מידע עיתוי ביצוע במטא-דאטה שלהן. ניתן להשתמש במידע עיתוי זה להגדרת גבולות זמן עליונים ותחתונים על זמן ביצוע ה-shots הספציפיים על QPU. ה-shots מקובצים לאובייקטי [`ExecutionSpan`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/execution-span-execution-span), שכל אחד מהם מצביע על זמן התחלה, זמן עצירה ואילו shots נאספו בטווח זה.

טווח ביצוע מזהה את הנתונים שהורצו במהלך חלון הזמן שלו על ידי ספקת שיטת [`ExecutionSpan.mask`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/execution-span-execution-span#mask). שיטה זו, כאשר ניתן לה אינדקס [Primitive Unified Block (PUB)](/guides/primitive-input-output#pubs) כלשהו, מחזירה מסיכה בוליאנית שהיא `True` לכל ה-shots שבוצעו במהלך חלון הזמן שלה. ה-PUBs מאונדקסים לפי הסדר שבו הם נמסרו לקריאת ריצת Sampler. אם ל-PUB יש צורה `(2, 3)` והוא הורץ עם ארבעה shots, הצורה של המסיכה היא `(2, 3, 4)`. ראה את דף ה-API של [execution_span](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/execution-span) לפרטים מלאים.
