...
התהליך מורכב מארבעה שלבים עיקריים. ראשית, נתוני הקלט ממופים לבעיה תואמת-קוונטית, תוך בניית ה-QUBO של בעיית אופטימיזציה דינמית של תיק ההשקעות והמרתה לאופרטור קוונטי (Ising Hamiltonian). לאחר מכן, בעיית הקלט ואלגוריתם ה-VQE מותאמים להרצה על חומרה קוונטית. לאחר מכן מופעל אלגוריתם VQE על החומרה הקוונטית, ולבסוף, התוצאות עוברות עיבוד לאחור (post-processing) על מנת לספק את מסלולי ההשקעה האופטימליים. המערכת כוללת גם עיבוד לאחור מודע-רעש (מבוסס [SQD](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd)) לשיפור איכות הפלט.
...
# התחלה {#get-started}
בצע אימות באמצעות [מפתח ה-API](https://quantum.cloud.ibm.com/) שלך ובחר את פונקציית Qiskit כדלקמן. (קטע קוד זה מניח שכבר [שמרת את חשבונך](/guides/functions-get-started#install-qiskit-functions-catalog-client) בסביבה המקומית שלך.)
...
![הדמיה של פתרון האופטימיזציה](/docs/images/guides/global-data-quantum-optimizer/cost_distribution.avif)
...
מספר הדורות הוגדר ל-20, מאחר שנמצא שערך זה מספיק להגעה להתכנסות. בנוסף, ערכי ברירת המחדל עבור הפרמטרים הפנימיים של האופטימייזר נותרו ללא שינוי, מאחר שהם מספקים עקביות ביצועים טובים ומומלצים בדרך כלל על ידי הספרות והנחיות המימוש.

# קבלת תמיכה {#get-support}
אם אתה זקוק לעזרה, תוכל לשלוח דוא"ל לכתובת qpo.support@globaldataquantum.com. בהודעתך, ספק את מזהה המשימה (job ID) של הפונקציה.
