...
הקוד הבא מניח שכבר עקבת אחר התהליך לשמירת האישורים שלך באמצעות `QiskitRuntimeService.save_account`, וייטען את החשבון השמור כברירת מחדל שלך אלא אם תציין אחרת. ראה [שמור את האישורים שלך](/guides/save-credentials) ו[אתחול חשבון שירות IBM Quantum Compute Service שלך](/guides/initialize-account) למידע נוסף.
