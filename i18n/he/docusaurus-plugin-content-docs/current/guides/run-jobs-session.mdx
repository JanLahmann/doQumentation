---
description: 'כיצד להריץ עבודת מחשוב קוונטי ב-session של IBM Quantum Compute Service.'
...
משתמשי Open Plan אינם יכולים להגיש עבודות session. עומסי עבודה חייבים לרוץ ב[מצב עבודה](/guides/execution-modes#job-mode) או [מצב אצווה](/guides/execution-modes#batch-mode).
</Admonition>
...
לפני תחילת session, עליך [להגדיר את לקוח IBM Quantum Compute](/guides/install-qiskit-runtime) ולאתחל אותו כשירות:
...
<Admonition type="tip">
אם אינך משתמש ב-context manager, סגור את ה-session ידנית כדי למנוע עלויות בלתי רצויות. ניתן לסגור session ברגע שסיימת להגיש אליו עבודות. כאשר session נסגר עם `session.close()`, הוא אינו מקבל עוד עבודות חדשות, אך העבודות שכבר הוגשו ימשיכו לרוץ עד להשלמתן וניתן יהיה לאחזר את תוצאותיהן.
</Admonition>
...
לסקירה מקיפה של התצורה והסטטוס של session, השתמש בשיטה `session.details() method`.
...
<Admonition type="tip" title="Recommendations">
  - נסה דוגמה במדריך [Quantum approximate optimization algorithm (QAOA)](/tutorials/quantum-approximate-optimization-algorithm).
