...
- ב-`qiskit-ibm-runtime` גרסה 0.30 ואילך, הרץ [`job.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-job-v2#usage) עבור עבודה בודדת, או [`batch.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/batch#usage) או [`session.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/session#usage) כשמשתמשים במצב אצווה או session. אם אתה משתמש בגרסה ישנה יותר של `qiskit-ibm-runtime` (>= 0.23 ו-< 0.30), ניתן עדיין למצוא את השימוש ב-`session.details()["usage_time"]` וב-`batch.details()["usage_time"]`.

- השתמש ב-[`GET /sessions/{id}`](https://docs.quantum.ibm.com/api/qiskit-runtime-rest/tags/sessions#tags__sessions__operations__get_session) כדי לראות את השימוש עבור אצווה או session ספציפיים (נקודת הקצה זהה עבור שני מצבי ההרצה).

- השתמש ב-[`GET /jobs/{id}`](https://docs.quantum.ibm.com/api/qiskit-runtime-rest/tags/jobs#tags__jobs__operations__get_job_metrics_jid) כדי לראות את השימוש עבור עבודה בודדת.
