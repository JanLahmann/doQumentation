...
תהליך העבודה שלנו עבור תוסף SLC מנצל את מסגרת Samplomatic ו-Executor החדשה, המאפשרת למשתמשים שליטה מודולרית יותר על הגדרות הביצוע לצמצום ומניעת שגיאות, תוך שמירה על קלות שימוש למשתמשים מתקדמים. להבנה מעמיקה יותר של היתרונות של מסגרת זו ותכונותיה הכלליות, ראה במדריך [Hello samplomatic](https://github.com/qiskit-community/qdc-challenges-2025/blob/main/day3_tutorials/Track_A/hello_samplomatic/Samplomatic%20-%20Hello%20World.ipynb).
...
### תהליך העבודה לצלילת lightcone, לימוד רעש והזרקת anti-noise {#workflow-for-lightcone-shading-noise-learning-and-anti-noise-injection-1}
...
- ניתן להבחין בבירור בשני הקונוסים הנובעים משני פאולי שאינם זהות (non-identity) באובזרבל.
...
לאחר מכן אנחנו מהדקים את הגבולות על-ידי שימוש בפונקציה `tighten_with_speed_limit`, שעוקבת אחר האופן שבו האובזרבל מתפשט לאחור דרך ה-Circuit ומשתמשת בהתפשטות זו כדי להציב גבולות עליונים על ההשפעה של כל אופרטור רעש, תוך בחירת הקטן מבין הגבול הקדמי שחושב זה עתה לבין גבול ההפצה לאחור.
...
#### ויזואליזציה של ה-SLC לבדיקה ידנית {#visualize-the-slc-for-manual-inspection-1}
...
#### ויזואליזציה של ה-SLC לבדיקה ידנית {#visualize-the-slc-for-manual-inspection-2}
...
### ויזואליזציה של ה-SLC לבדיקה ידנית {#visualize-the-slc-for-manual-inspection-3}
...
### b.i. עדכון גבולות ממוזגים עם קצבי רעש נלמדים בפועל {#bi-update-merged-bounds-with-actual-learned-noise-rates}
...
#### b.ii. חישוב ה-`local_scales` לביצוע בחומרה {#bii-compute-the-local-scales-for-the-hardware-execution}
...
ה-`samplex` הוא פלט של השיטה `build` של Samplomatic, המקודד את כל המידע הדרוש כדי ליצור פרמטרים אקראיים עבור `template_circuit`. אלה משמשים לאחר מכן להגדרת אובייקטי `QuantumProgram`, שבתורם רצים על QPU עם ה-`Executor` primitive. כל `QuantumProgram` יכול להכיל מספר פריטים, שאפשר לחשוב עליהם כזוג של `template` ו-`samplex`.
...
#### c.iii. הרצת התוכנית באמצעות ה-`Executor` primitive {#ciii-execute-program-with-the-executor-primitive}
