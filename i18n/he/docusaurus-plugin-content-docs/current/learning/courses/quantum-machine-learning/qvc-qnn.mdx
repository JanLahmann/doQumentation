...
sidebar_label: 'מעגלים וריאציוניים קוונטיים ורשתות עצביות קוונטיות'
...
שים/י לב שהקוד שלמעלה גם יצר תוויות המציינות האם התמונות מכילות קו אנכי (+1) או אופקי (-1). כעת נשתמש ב-sklearn לפיצול מערך נתונים של 100 תמונות לסט אימון וסט בדיקה (יחד עם התוויות המתאימות שלהם). כאן אנחנו משתמשים ב-$70%$ ממערך הנתונים לאימון, כאשר ה-$30%$ הנותרים נשמרים לבדיקה.
...
כעת עלינו להחליט על ansatz שיאומן. ישנם שיקולים רבים בבחירת ansatz. תיאור מלא חורג מהיקף מבוא זה; כאן אנחנו רק מצביעים על כמה קטגוריות של שיקולים.
...
לאחר שהכנו את קידוד הנתונים ואת ה-Circuit הוריאציוני, נוכל לשלב אותם ליצירת ה-ansatz המלא שלנו. במקרה זה, הרכיבים של ה-Circuit הקוונטי שלנו דומים למדי לאלה שברשתות עצביות, כאשר $U(\vec{x})$ דומה ביותר לשכבה שטוענת ערכי קלט מהתמונה, ו-$W(\theta)$ דומה לשכבה של "משקולות" משתנות. מאחר שהקבלה זו נכונה במקרה זה, אנחנו מאמצים "qnn" בחלק ממוסכמות השמות שלנו; אבל הקבלה זו לא צריכה להגביל את חקירתך של VQCs.
...
יש לנו את ה-Circuit הקוונטי שלנו ואת האובייקטיב שאנו רוצים להעריך. כעת אנחנו זקוקים לכמה דברים כדי להריץ ולאופטמז את ה-Circuit הזה. ראשית, אנחנו זקוקים לפונקציה להרצת forward pass. שים/י לב שהפונקציה שלהלן מקבלת את ```input_params``` ואת ```weight_params``` בנפרד. הראשון הוא קבוצת הפרמטרים הסטטיים המתארים את הנתונים בתמונה, והשני הוא קבוצת הפרמטרים המשתנים שיש לאופטמז.
...
לאחר מכן, אנחנו זקוקים לפונקציית אובדן לחישוב ההפרש בין הערכים החזויים לערכי התוויות המחושבים. הפונקציה תקבל את התוויות שהאלגוריתם חזה ואת התוויות הנכונות ותחזיר את ממוצע ההפרש הריבועי. ישנן פונקציות אובדן רבות. כאן, MSE הוא דוגמה שבחרנו.
...
## תבניות Qiskit שלב 3: הרצה באמצעות IBM Quantum primitives {#qiskit-patterns-step-3-execute-using-ibm-quantum-primitives}
...
## שלב 2 בתבניות Qiskit: אופטימיזציה של הבעיה להרצה קוונטית {#qiskit-patterns-step-2-optimize-problem-for-quantum-execution-1}
אנחנו מתחילים בבחירת Backend להרצה. במקרה הזה נשתמש ב-Backend הפחות עמוס.
...
עכשיו נחיל את ה-pass manager מספר פעמים. עבור circuits רחבים או עמוקים מאוד, יכולה להיות שונות גדולה בעומקי ה-two-qubit לאחר transpile. עבור circuits כאלה חשוב לנסות את ה-pass manager פעמים רבות ולהשתמש בתוצאה הטובה ביותר (הרדודה ביותר).
...
## תבניות Qiskit שלב 3: הרצה באמצעות IBM Quantum primitives {#qiskit-patterns-step-3-execute-using-ibm-quantum-primitives-1}
...
* שימוש ב-pass manager לטרנספילציה של המעגל והאובזרבל לארכיטקטורת ה-Backend שנבחר.
