...
לרקע נוסף הרלוונטי לנושאים אלה, אנחנו ממליצים על שיעור 4 ב"יסודות מידע קוונטי" בנושא [שזירה בפעולה](/learning/courses/basics-of-quantum-information/entanglement-in-action/introduction).
...
### הערכת ערך $r_z$ {#estimate-r-z-value}
...
באמצעות שיטת טומוגרפיית המצב הקוונטי, הערכנו את ערך $r_z$. במקרה זה, מכיוון שבחרנו פרמטר למצב ה"אקראי", אנחנו יודעים את ערך $r_z$ ויכולים לבדוק את עבודתנו. אך מטבעה, עבודה בסדר גודל שימושי אינה תמיד כה פשוטה לבדיקה. נדון יותר על בדיקת תוצאות קוונטיות בהמשך הקורס. לעת עתה, פשוט שימו לב שההערכה שלנו הייתה סבירה למדי.

### תרגיל 1: הערכת ערך $r_x$ {#exercise-1-estimate-r-x-value}
...
### תרגיל 2: הערכת ערך $r_y$ {#exercise-2-estimate-r-y-value}
...
הנה אנחנו מתארים את ההליך ללא הסבר. פרטים אלה ייושמו להלן.
...
כדי לתאר את התרחיש לעיל, אנחנו זקוקים ל-Circuit עם שלושה qubit: שניים לזוג השזור שבין אליס לבוב, ואחד למצב הקוונטי הלא ידוע $|\psi\rangle$.
...
לפני שפרוטוקול זה בכלל מתחיל, אנחנו מניחים שלאליס ולבוב יש זוג שזור משותף. אם אליס ובוב נמצאים באמת במקומות שונים, ייתכן שהם הגדירו את המצב המשותף *לפני* שמצב $|\psi\rangle$ הלא ידוע נוצר. מכיוון שהדברים האלה קורים על qubit שונים, סדרם כאן לא משנה, וסדר זה נוח לויזואליזציה.
...
בשלב הבא, נבצע טלפורטציה על מחשב קוונטי אמיתי. באמצעות פונקציית ה-Circuit הדינמי, אנחנו יכולים לפעול אמצע-Circuit באמצעות תוצאות מדידה, ולממש בזמן אמת את הפעולות המותנות ב-Circuit הטלפורטציה. לפתרון בעיות עם מחשבים קוונטיים אמיתיים, נעקוב אחר ארבעת השלבים של דפוסי Qiskit.
...
באמצעות ה-`Sampler` Runtime primitive, נריץ את המעגל היעד.
...
## 5. קידוד צפוף קוונטי {#5-superdense-coding}
...
### 5.1 הפרוטוקול של קידוד צפוף קוונטי {#51-the-protocol-of-superdense-coding}
...
### 5.2 Circuit קוונטי המדמה קידוד צפוף קוונטי {#52-quantum-circuit-simulating-the-superdense-coding}
