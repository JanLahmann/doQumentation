---
description: 'קורס זה עוסק בסימולציה קוונטית הכוללת טרוטריזציה עבור ההמילטוניאן של מודל איזינג בשדה רוחבי'
notebook_path: learning/courses/utility-scale-quantum-computing/quantum-simulation.ipynb
sidebar_label: 'סימולציה קוונטית'
title: 'סימולציה קוונטית'
---
...
[הורד את ה-PDF](https://ibm.ent.box.com/s/kzzsmxhw38vph1ioohczaet53euwi310) של ההרצאה המקורית. שים לב שחלק מקטעי הקוד עשויים להיות מיושנים מכיוון שאלו תמונות סטטיות.

*זמן QPU משוער להרצת הניסוי הוא 7 שניות.*

(מחברת זו לקוחה ברובה ממחברת [הדרכה שהוצאה משימוש](https://github.com/qiskit-community/qiskit-algorithms/blob/main/docs/tutorials/13_trotterQRTE.ipynb) עבור Qiskit Algorithms.)

</Admonition>

## 1. מבוא {#1-introduction}

כטכניקת אבולוציה בזמן אמת, טרוטריזציה מורכבת מהפעלה עוקבת של Gate קוונטי אחד או יותר, שנבחרו לקירוב האבולוציה בזמן של מערכת עבור פרוסת זמן. בעקבות משוואת שרדינגר, האבולוציה בזמן של מערכת שנמצאת בתחילה במצב $\vert\psi(0)\rangle$ לובשת את הצורה:

$$
\vert \psi(t) \rangle = e^{-i H t} \vert \psi(0) \rangle \text{,}
$$

כאשר $H$ הוא ההמילטוניאן הבלתי תלוי בזמן המנהל את המערכת. אנחנו מתייחסים להמילטוניאן שניתן לכתוב כסכום משוקלל של איברי פאולי $H=\sum_j a_j P_j$, כאשר $P_j$ מייצג מכפלה טנזורית של איברי פאולי הפועלים על $n$ qubits. בפרט, ייתכן שאיברי פאולי אלו מתחלפים זה עם זה, וייתכן שלא. בהינתן מצב בזמן $t=0$, כיצד מקבלים את מצב המערכת בזמן מאוחר יותר $|\psi(t)\rangle$ באמצעות מחשב קוונטי? את האקספוננט של אופרטור ניתן להבין בצורה הקלה ביותר דרך טור טיילור שלו:

$$
e^{-i H t} = 1-iHt-\frac{1}{2}H^2t^2+...
$$

אקספוננטים בסיסיים מאוד, כמו $e^{iZ}$, ניתנים לממוש בקלות במחשבים קוונטיים באמצעות קבוצה קומפקטית של Gates קוונטיים. לרוב ההמילטוניאנים המעניינים לא יהיה רק איבר בודד, אלא הרבה איברים. שימו לב למה שקורה כאשר $H = H_1+H_2$:

$$
e^{-i H t} = 1-i(H_1+H_2)t-\frac{1}{2}(H_1+H_2)^2t^2+...
$$

כאשר $H_1$ ו-$H_2$ מתחלפים, יש לנו את המקרה המוכר (שתקף גם למספרים, ולמשתנים $a$ ו-$b$ להלן):
...
פתרון אחד הוא לשקול זמן קצר מאוד $t$, כך שהאיבר מסדר ראשון בפיתוח טיילור שולט. תחת הנחה זו:
...
כאן $t/r$ הוא פרוסת הזמן (צעד האבולוציה) שאנו בוחרים. כתוצאה מכך, נוצר Gate שמופעל $r$ פעמים. צעד זמן קטן יותר מוביל לקירוב מדויק יותר. עם זאת, הדבר גם מוביל ל-Circuits עמוקים יותר, אשר בפועל גורמים לצבירת שגיאות גדולה יותר (דאגה לא מבוטלת על מכשירים קוונטיים מהדור הנוכחי).

היום נלמד את האבולוציה בזמן של [מודל איזינג](https://en.wikipedia.org/wiki/Ising_model) על סריגים לינאריים עם $N=2$ ו-$N=6$ אתרים. סריגים אלו מורכבים ממערך של ספינים $\sigma_i$ המקיימים אינטראקציה רק עם שכניהם הקרובים ביותר. לספינים אלו יכולים להיות שתי אוריינטציות: $\uparrow$ ו-$\downarrow$, המתאימות למגנטיזציה של $+1$ ו-$-1$ בהתאמה.

$$
H = - J \sum_{i=0}^{N-2} Z_i Z_{i+1} - h \sum_{i=0}^{N-1} X_i  \text{,}
$$

כאשר $J$ מתאר את אנרגיית האינטראקציה, ו-$h$ את עצמת השדה החיצוני (בכיוון ה-x לעיל, אך נשנה זאת). בואו נכתוב ביטוי זה באמצעות מטריצות פאולי, תוך התחשבות בכך שלשדה החיצוני יש זווית $\alpha$ ביחס לכיוון הרוחבי,

$$
H = -J \sum_{i=0}^{N-2} Z_i Z_{i+1} -h \sum_{i=0}^{N-1} (\sin\alpha Z_i + \cos\alpha X_i) \text{.}
$$

המילטוניאן הזה שימושי בכך שהוא מאפשר לנו לחקור בקלות את השפעות השדה החיצוני. בבסיס החישובי, המערכת תקודד באופן הבא:

| מצב קוונטי                | ייצוג ספינים                              |
|:------------------------:|:------------------------------------------:|
...
### 1.1 דרישות {#11-requirements}

לפני תחילת הדרכה זו, וודאו שהכלים הבאים מותקנים:

- Qiskit SDK v1.2 ומעלה ( `pip install qiskit` )

- Qiskit Runtime v0.30 ומעלה ( `pip install qiskit-ibm-runtime` )

- Numpy v1.24.1 ומעלה < 2 ( `pip install numpy` )


### 1.2 ייבוא הספריות {#12-import-the-libraries}
...
## 2. מיפוי הבעיה {#2-mapping-your-problem}

### 2.1 הגדרת ההמילטוניאן של מודל איזינג בשדה רוחבי {#21-defining-the-transverse-field-ising-hamiltonian}

אנחנו מתייחסים כאן למודל איזינג חד-ממדי בשדה רוחבי.

ראשית, ניצור פונקציה שמקבלת את פרמטרי המערכת $N$, $J$, $h$ ו-$\alpha$, ומחזירה את ההמילטוניאן שלנו כ-`SparsePauliOp`. [SparsePauliOp](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.SparsePauliOp) הוא ייצוג דליל של אופרטור במונחים של איברי [Pauli](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Pauli) משוקללים.
...
המערכת שאנו מתייחסים אליה כעת היא בגודל $N=6$, $J=0.2$, $h=1.2$ ו-$\alpha=\frac{\pi}{8.0}$ כדוגמה.
...
### 2.2 הגדרת פרמטרי סימולציית האבולוציה בזמן {#22-set-the-parameters-of-the-time-evolution-simulation}
...
### 2.3 הכנת ה-Circuit הקוונטי 1 (מצב התחלתי) {#23-prepare-the-quantum-circuit-1-initial-state}
יצירת מצב התחלתי. כאן נתחיל עם קונפיגורציית הספינים $\uparrow\uparrow\downarrow\downarrow\uparrow\uparrow$
.
...
### 2.4 הכנת ה-Circuit הקוונטי 2 (Circuit יחיד לאבולוציה בזמן) {#24-prepare-the-quantum-circuit-2-single-circuit-for-time-evolution}
אנחנו בונים כאן Circuit עבור צעד זמן בודד תוך שימוש ב-Lie–Trotter.

נוסחת המכפלה של Lie (סדר ראשון) מממושת במחלקה [LieTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter). נוסחה מסדר ראשון מורכבת מהקירוב שצוין במבוא, שבו האקספוננט המטריצי של סכום מקורב על ידי מכפלה של אקספוננטים מטריציים:
...
### 2.5 הגדרת האופרטורים למדידה {#25-set-the-operators-to-measure}
בואו נגדיר *אופרטור מגנטיזציה* $\sum_i \langle Z_i \rangle / N$, ו-*אופרטור מתאם ספינים ממוצע* $\sum_i \langle Z_i Z_{i+1} \rangle/ (N - 1)$.
...
### 2.6 ביצוע סימולציית האבולוציה בזמן {#26-perform-time-evolution-simulation}
נעקוב אחר האנרגיה (ערך הציפייה של ההמילטוניאן), המגנטיזציה (ערך הציפייה של אופרטור המגנטיזציה), ומתאם הספינים הממוצע (ערך הציפייה של אופרטור מתאם הספינים הממוצע). ה-`StatevectorEstimator` (EstimatorV2) של Qiskit מעריך ערכי ציפייה של אובייקטים, $\langle\psi\vert\hat{O}\vert\psi\rangle$.
...
### 2.7 שרטוט האבולוציה בזמן של האובייקטים {#27-plot-the-time-evolution-of-the-observables}
...
## 3. תרגיל 1. ביצוע סימולציה באמצעות Suzuki–Trotter מסדר שני {#3-exercise-1-perform-simulation-using-second-order-suzukitrotter}
עכשיו בואו ננסה לבצע סימולציה עם Suzuki–Trotter מסדר שני בהמשך לדוגמה של Lie–Trotter שראינו למעלה.

ניתן להשתמש ב-Suzuki-Trotter מסדר שני ב-Qiskit באמצעות [מחלקת SuzukiTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.SuzukiTrotter). בשימוש בנוסחה זו, פירוק מסדר שני הוא:

$$
e^{H_1+H_2} \approx e^{H_1/2}e^{H_2}e^{H_1/2}
$$

### 3.1 בניית Circuit לצעד זמן יחיד {#31-construct-a-circuit-for-a-single-time-step}
...
### 3.2 ביצוע סימולציית התפתחות בזמן {#32-perform-time-evolution-simulation}
...
### 3.3 הצגת תוצאות Suzuki–Trotter מסדר שני {#33-plot-the-second-order-suzukitrotter-results}
...
### 3.4 השוואה עם תוצאות מדויקות {#34-compare-with-exact-results}
הנתונים שלהלן הם התוצאות המדויקות שחושבו מראש על ידי המחשב הקלאסי.
...
## 4. הרצה על חומרת הקוואנטום {#4-executing-on-the-quantum-hardware}
בשלב הבא נריץ את סימולציית האבולוציה בזמן על חומרת הקוואנטום. נעבוד על בעיה קטנה יותר, גודל סריג N=2. נשנה את הפרמטר $\alpha$ ונראה את ההבדל בדינמיקה של פונקציית הגל.

### 4.1  שלב 1. מיפוי קלטים קלאסיים לבעיה קוואנטית {#41-step-1-map-classical-inputs-to-a-quantum-problem}

בחר את ההגדרה הראשונית של הסימולציה:
...
לאחר מכן הגדר את ה-Circuit ההתחלתי:

תצורת הספין ההתחלתית תהיה "למטה-למעלה"
...
כעת חשב את ערך הייחוס באמצעות סימולטור statevector אידיאלי.
...
הכנו מערכת עם רצף ספינים ראשוני $\downarrow\uparrow$, המתאים ל-$\vert\psi(0)\rangle = \vert10\rangle$. לאחר שאיפשרנו לה להתפתח למשך $t=1.6$ תחת שדה רוחבי ($\alpha=0^\circ$), כמעט בוודאות נמדוד $\uparrow\downarrow$, כלומר, החלפת ספינים. (שים לב שהתוויות מפורשות מימין לשמאל). אם השדה הוא אורכי ($\alpha=\pm90^\circ$), לא תהיה אבולוציה, ולכן נמדוד את המערכת כפי שהוכנה בהתחלה, $\downarrow\uparrow$. בזוויות ביניים, ב-$\alpha=\pm45^\circ$, נוכל למדוד את כל הצירופים עם הסתברויות שונות, כשהחלפת הספינים היא הסבירה ביותר עם הסתברות של 67%.

#### בניית Circuit לניסוי חומרה {#construct-circuit-for-hw-experiment}
...
לאחר מכן נבצע Transpile ל-Circuit עבור ה-Backend שנבחר.
...
### 4.3 שלב 3. הרצה עם פרימיטיביות IBM Quantum {#43-step-3-execute-with-ibm-quantum-primitives}

הפרימיטיב `Sampler` (V2) של Qiskit מספק את ספירת המחרוזות הבינאריות שנמדדו.
...
### 4.4 שלב 4. עיבוד תוצאות לאחר מדידה {#44-step-4-post-process-results}
בנה את ההיסטוגרמה של המחרוזות הבינאריות, המתאימה לניתוח פונקציית הגל, והשווה אותה לערכים האידיאליים שהוצגו למעלה.
...
כאן אנחנו מציגים דוגמה לבניית Circuit באמצעות Suzuki–Trotter מסדר גבוה יותר (סדר רביעי).
כעת ננסה לבנות סימולציית Circuit עם Suzuki–Trotter מסדר רביעי בעקבות הדוגמאות שהוצגו למעלה.

ניתן להשתמש ב-Suzuki–Trotter מסדר רביעי ב-Qiskit באמצעות [המחלקה SuzukiTrotter](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.SuzukiTrotter). את הסדר הרביעי ניתן לחשב באמצעות יחס הרקורסיה הבא. שים לב שסדר ה-Suzuki–Trotter מסומן כ-"2k" במשוואות הבאות.
...
#### בניית Circuit לצעד זמן יחיד {#construct-a-circuit-for-a-single-time-step}
השתמש ב-product_formula_st4 (SuzukiTrotter(order=4)) ובנה Circuit לצעד זמן יחיד באמצעות Suzuki–Trotter מסדר רביעי. כמו כן, ספור את מספר ה-Gate-ים ועומק ה-Circuit והשווה עם Lie–Trotter ו-Suzuki–Trotter מסדר שני.
