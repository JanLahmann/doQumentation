...
Qiskit מספק API לקבלת מידע, סטטי ודינמי כאחד, על מכשיר קוונטי. אנחנו משתמשים במופע Backend לממשק עם מכשיר, הכולל מופע Target — מודל מכונה מופשט המסכם את המאפיינים הרלוונטיים כמו ארכיטקטורת מערכת ההוראות (ISA) וכל מאפיין או אילוץ המשויך לה.
בואו נשתמש במופעי Backend אלה כדי לקבל חלק מהמידע שאתם רואים בדף [Compute resources](https://quantum.cloud.ibm.com/computers)
ב-IBM Quantum&reg; Platform. תחילה, נצור מופע Backend למכשיר הנדרש. בהמשך, אנחנו בוחרים "ibm_kyoto"‏, "ibm_kawasaki" או מכונת Eagle הפחות עמוסה. הגישה שלכם ל-QPUs עשויה להיות שונה; עדכנו את שם ה-Backend בהתאם.
...
## 3. מאפייני qubit {#3-qubit-properties}
...
###  3.2 שגיאות Gate וקריאה {#32-gate-and-readout-errors}
...
לאחר שהבנו את מבני הנתונים, אנחנו מוכנים לחשב את שגיאות החציון עבור Gate ה-'sx' וה-'ecr'. שוב, השוו את התוצאות לאלה המוצגות עבור המכשיר ב-[IBM Quantum Platform](https://quantum.cloud.ibm.com/).
