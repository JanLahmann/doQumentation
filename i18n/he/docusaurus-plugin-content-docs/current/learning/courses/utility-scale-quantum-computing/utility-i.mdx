---
description: "בשיעור זה, אנחנו מריצים מעגל בסדר גודל של utility, המופיע במה שאנו מכנים בצורה לא רשמית 'מאמר ה-utility', שפורסם ב-Nature כרך 618, 15 ביוני 2023."
...
בשיעור זה, אנחנו מריצים מעגל בסדר גודל של utility, המופיע במה שאנו מכנים בצורה לא רשמית ["מאמר ה-utility"](https://www.nature.com/articles/s41586-023-06096-3), שפורסם ב-Nature כרך 618, 15 ביוני 2023. המאמר עוסק באבולוציית הזמן של מודל איזינג דו-ממדי בשדה רוחבי. בפרט, הם בוחנים את דינמיקת הזמן של ההמילטוניאן,
$$
...
מאז, אנחנו מכנים ניסויים ומעגלים כאלה "בסדר גודל utility".
...
באופן קונקרטי, אתה מתבקש לבנות ולהריץ את המעגל המתאים לאיור 4b של המאמר, ולשרטט את הנקודות "הלא-מוקזזות" בעצמך. כפי שאתה רואה, מדובר במעגל של 127 qubit $\times$ 60 שכבות (20 צעדי טרוטר) עם $\langle Z_{62} \rangle$ כ-Observable.
![image.png](/learning/images/courses/utility-scale-quantum-computing/utility-i/utility1.avif)
נשמע מאתגר? &nbsp; אל דאגה. שלושת השיעורים האחרונים של קורס זה מספקים אבני דרך. להתחלה, נדגים ניסוי בסדר גודל קטן יותר — בניה והרצה על מכשיר מדומה של מעגל 27 qubit $\times$ 6 שכבות (2 צעדי טרוטר) עם $\langle Z_{13} \rangle$ כ-Observable.
...
### 2.1 בניית RZZ(-$\pi$ / 2) {#21-construct-rzz-pi-2}
...
כאמור לעיל, אנחנו מתמקדים ב-Gate RZZ עם זווית ספציפית, -$\pi$ / 2, עבור ניסוי זה. כפי שמוצג במאמר, ניתן לממש זאת עם Gate $CX$ אחד בלבד.
...
אנחנו מגדירים Gate במונחים של Circuit זה לעיון עתידי.
...
בואו נבחן כיצד אנחנו צובעים את מפת הצימוד של Backend. זה נדרש כדי לקבץ אינטראקציות $ZZ$ לשכבות.
...
לצביעת מפת הצימוד, אנחנו משתמשים ב-[rustworkx](https://www.rustworkx.org/), שהוא חבילת Python לעבודה עם גרפים ורשתות מורכבות. היא מספקת מספר אלגוריתמי צביעה, שכולם היוריסטיים ולכן אינם מובטחים למצוא צביעה מינימלית.

עם זאת, מאחר שגרפי heavy-hex הם דו-צדדיים, אנחנו בוחרים ב-[graph_bipartite_edge_color](https://www.rustworkx.org/apiref/rustworkx.graph_bipartite_edge_color.html), שאמור למצוא צביעה מינימלית עבור גרפים אלה.
...
שים לב שכבר ביצענו ידנית את מיפוי ה-Qubit והניתוב עבור ה-Circuit שנבנה. לכן, כשנבצע Transpile של ה-Circuit מאוחר יותר, אנחנו לא נבקש (ו__לא כדאי__ לבקש) מה-Transpiler לבצע מיפוי ניתוב של qubit. כפי שתראה בקרוב, נפעיל אותו עם רמת אופטימיזציה 1 ושיטת layout "trivial".

לאחר מכן, אנחנו מגדירים שגרה פשוטה לקבלת מידע על ה-Circuit שנבנה לבדיקה מהירה.
...
### 4.2 שלבים 2 ו-3: Optimize ו-Execute (סימולטור) {#42-steps-2-and-3-optimize-and-execute-simulator}
...
### 4.3 שלבים 2 ו-3: Optimize ו-Execute (מכשיר מדומה) {#43-steps-2-and-3-optimize-and-execute-fake-device}
...
### 4.4 שלב 4: Post-process {#44-step-4-post-process}
...
## 5. פתרו את גרסת ה-127 qubit של הבעיה {#5-solve-the-127-qubit-version-of-the-problem}
...
### 5.2 שלבים 2 ו-3: Optimize ו-Execute {#52-steps-2-and-3-optimize-and-execute}
