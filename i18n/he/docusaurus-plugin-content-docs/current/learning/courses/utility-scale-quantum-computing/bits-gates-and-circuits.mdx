...
## 2. חישוב כדיאגרמה {#2-computation-as-a-diagram}
...
## 3. שער קוונטי של qubit בודד {#3-single-qubit-quantum-gate}

### 3.1 מצב קוונטי וספירת Bloch {#31-quantum-state-and-bloch-sphere}
...
### 3.2 מצב קוונטי של qubit בודד ואבולוציה יוניטרית {#32-single-qubit-quantum-state-and-unitary-evolution}
...
### 3.3 מדידה {#33-measurement}

מדידה היא נושא תיאורטי מורכב מאוד. אבל במונחים מעשיים, ביצוע מדידה לאורך $z$ (כפי שכל מחשבי הקוונטום של IBM&reg; עושים) פשוט כופה על מצב ה-Qubit $\alpha|0\rangle+\beta|1\rangle \quad (s.t.|\alpha|^2+|\beta|^2=1)$ לקפוץ למצב $|0\rangle$ או $|1\rangle$, ואנחנו צופים בתוצאה.
...
אנחנו רואים ש-0 ו-1 נמדדו עם הסתברות של כמעט 50% כל אחד. למרות שרעש לא הדמינו כאן, המצבים עדיין הסתברותיים. לכן, בעוד שאנחנו מצפים לפיזור של 50-50 בערך, נדיר שנמצא בדיוק כזה. בדיוק כמו שהטלת מטבע 100 פעמים נדיר שתניב בדיוק 50 מופעים של כל צד.
...
אם אין לך את `qiskit-ibm-runtime`, תצטרך להתקין אותה תחילה. למידע נוסף, [ראה את חומר העזר של ה-API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-service).
...
ניתן לראות שבמהלך הטרנספילציה המעגל נכתב מחדש עם Gateים חדשים. למידע נוסף, ראה בתיעוד של [ECRGate](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ECRGate#ecrgate).
