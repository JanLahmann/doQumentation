...
כאן אנחנו שוקלים את מודל איזינג עם שדה רוחבי חד-ממדי.
...
כאן אנחנו משווים את התוצאות של הסימולטורים Statevector ו-Matrix Product State.
...
צור מצב התחלתי. נתחיל ממצב היסוד, שהוא מצב פרומגנטי (כולם למעלה או כולם למטה). כאן, אנחנו משתמשים בדוגמה של כולם למעלה (שהוא כולם '0').
...
כאן אנחנו בונים Circuit לצעד זמן יחיד באמצעות Lie–Trotter.
...
נשרטט את ערכי הציפייה שמדדנו מול הזמן. ודא שהתוצאות מסימולטורי statevector ו-matrix product state מסכימות.
...
כאן אנחנו משווים את התוצאות של סימולטור Matrix Product State והמכשיר הקוונטי האמיתי.

### 2.2 פעילות 2 {#22-activity-2}

#### הגדרת ה-Hamiltonian {#define-the-hamiltonian-1}
...
#### הכנת Circuit קוונטי 2 (Circuit יחיד לאבולוציה בזמן) עבור בעיית 70 ה-Qubit {#prepare-the-quantum-circuit-2-single-circuit-for-time-evolution-for-the-70-qubit-problem}
כאן אנחנו בונים Circuit לצעד זמן יחיד תוך שימוש ב-Lie–Trotter.
...
#### הגדרת האופרטורים למדידה {#set-the-operators-to-be-measured-1}
אנחנו מגדירים *אופרטור מגנוטיזציה* בדיוק באותו האופן שבו עשינו זאת במקרה של 20 qubit: $\sum_i Z_i  / N$. נסה זאת בעצמך על ידי שינוי הפתרון של 20 qubit.
...
#### ביצוע סימולציה של אבולוציה בזמן {#perform-time-evolution-simulation-1}
נעקוב אחר המגנוטיזציה (ערך הציפייה של אופרטור המגנוטיזציה). נשתמש בסימולטור MPS כדי לקבל את ערך הייחוס להשוואה עם התוצאות שנחושבו על חומרה. כבר השתמשת בסימולטור MPS בשיעור זה בעבר. שנה את הדוגמה הזו לפי הצורך כדי שתתאים לחישוב החדש.
...
כל הכבוד! התקדמת עוד צעד אחד במסע הקוונטי שלך בקנה-מידה של שימושיות. נותר רק שיעור אחד!
