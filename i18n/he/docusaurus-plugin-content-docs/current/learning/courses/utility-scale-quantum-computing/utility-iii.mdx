...
description: "שיעור זה עוסק בבניית Circuit GHZ עבור 20 Qubit ומעלה, כך שבמדידה, נאמנות מצב ה-GHZ תעלה על 0.5."
...
# ניסוי בקנה-מידה שימושי III {#utility-scale-experiment-iii}

<Admonition type="note">

Toshinari Itoko, Tamiya Onodera, Kifumi Numata (19 ביולי 2024)

[הורד את קובץ ה-PDF](https://ibm.ent.box.com/public/static/fw1538dogvyv0qbfqg8tan1k2fs27mfv.zip) של ההרצאה המקורית. שים לב שחלק מקטעי הקוד עשויים להיות מיושנים, מאחר שאלה תמונות סטטיות.

*זמן QPU המשוער להרצת הניסוי הראשון הוא 12 דקות ו-30 שניות. ישנו ניסוי נוסף בהמשך שדורש כ-4 דקות.*

(הערה: ייתכן שמחברת זו לא תסיים להתבצע בתוך הזמן המוקצב בתוכנית Open Plan. יש להשתמש במשאבי המחשוב הקוונטי בחוכמה.)
...
בואו נסקור בקצרה את מצבי GHZ, ואיזה פיזור היינו מצפים לקבל מה-`Sampler` כשמחילים אותו על אחד מהם. לאחר מכן נציג בבירור את המטרה של שיעור זה.

###  1.1 מצב GHZ {#11-ghz-state}

מצב GHZ (Greenberger-Horne-Zeilinger) עבור $n$ Qubit מוגדר כך:

$$
\frac{1}{\sqrt 2}(|0\rangle ^ {\otimes n}+ |1\rangle^ {\otimes n})
$$

ניתן ליצור אותו באופן טבעי עבור 6 Qubit בעזרת Circuit קוונטי הבא.
...
העומק אינו גדול מדי, אם כי מהשיעורים הקודמים ידוע לנו שניתן לשפר זאת. בואו נבחר Backend ונעביר את ה-Circuit דרך ה-Transpiler.
...
גם כאן עומק שתי-ה-Qubit לאחר ה-Transpiler אינו גדול מדי. אבל כדי לעבוד עם מצב GHZ על יותר Qubit, ברור שנצטרך לחשוב על אופטימיזציה של ה-Circuit. בואו נריץ זאת באמצעות `Sampler` ונראה מה מחזיר מחשב קוונטי אמיתי.
...
זהו התוצאה של Circuit ה-GHZ בן 6 ה-Qubit. כפי שניתן לראות, המצבים של כל $|0\rangle$ וכל $|1\rangle$ אכן שולטים, אך השגיאות משמעותיות. בואו ננסה לראות עד כמה גדול Circuit GHZ ניתן לבנות עם מכשיר Eagle, תוך שמירה על תוצאות שבהן המצבים הנכונים הם לפחות יותר מ-50% סבירים.

### 1.2 המטרה שלך {#12-your-goal}

בנה Circuit GHZ עבור 20 Qubit ומעלה כך שבמדידה, **נאמנות מצב ה-GHZ שלך תהיה > 0.5.**
הערות:
- עליך להשתמש במכשיר Eagle (`min_num_qubits=127`) ולהגדיר את מספר ה-shots ל-40,000.
- עליך להריץ את Circuit ה-GHZ באמצעות הפונקציה `execute_ghz_fidelity`, ולחשב את הנאמנות באמצעות הפונקציה `check_ghz_fidelity_from_jobs`.

זוהי תרגיל עצמאי שבו תנצל את מה שלמדת עד כה בקורס זה.
...
במחברת זו נחיל שלוש אסטרטגיות ליצירת מצבי GHZ טובים תוך שימוש ב-16 Qubit ו-30 Qubit. גישות אלה מבוססות על אסטרטגיות שכבר ידועות לך מהשיעורים הקודמים.

## 2. אסטרטגיה 1: בחירת Qubit מודעת-רעש {#2-strategy-1-noise-aware-qubit-selection}

ראשית נציין Backend. מאחר שנעבוד רבות עם המאפיינים של Backend ספציפי, כדאי לציין אחד ספציפי במקום להשתמש באפשרות `least_busy`.
...
אנחנו הולכים לבנות Circuit הכולל שערי שני-Qubit רבים. הגיוני להשתמש ב-Qubit שיש להם שגיאות הנמוכות ביותר בעת יישום אותם שערים. מציאת "שרשרת ה-Qubit" הטובה ביותר על סמך שגיאות שער ה-2q המדווחות היא בעיה לא טריוויאלית. אבל ניתן להגדיר כמה פונקציות שיעזרו לנו לקבוע אילו Qubit כדאי להשתמש.
...
נשתמש בפונקציות שלמעלה כדי למצוא את כל הנתיבים הפשוטים של N Qubit בין כל זוגות הצמתים בגרף (הפניה: [all_pairs_all_simple_paths](https://www.rustworkx.org/apiref/rustworkx.all_pairs_all_simple_paths.html#rustworkx-all-pairs-all-simple-paths)).

לאחר מכן, באמצעות הפונקציה `path_fidelity` שיצרנו לעיל, נמצא את שרשרת ה-Qubit הטובה ביותר בעלת הנאמנות הגבוהה ביותר.
...
בואו נצייר את שרשרת ה-Qubit הטובה ביותר, המוצגת בצבע ורוד, בתרשים מפת ה-coupling.
...
### 2.1 בנה Circuit GHZ על שרשרת ה-Qubit הטובה ביותר {#21-build-a-ghz-circuit-on-the-best-qubit-chain}
בוחרים Qubit באמצע השרשרת כדי להחיל עליו קודם את שער ה-H. פעולה זו אמורה להפחית את עומק ה-Circuit בכמחצית.
...
שימו לב להריץ את התא הבא רק לאחר שסטטוס המשימות שלמעלה הפך ל-'DONE', כדי להציג את התוצאה באמצעות הפונקציה `check_ghz_fidelity_from_jobs`.
...
תוצאה זו אינה עומדת בקריטריונים. נעבור לרעיון הבא.
## 3. אסטרטגיה 2. עץ מאוזן של Qubits {#strategy-2-balanced-tree-of-qubits}
הרעיון הבא הוא למצוא עץ מאוזן של Qubits. שימוש בעץ במקום בשרשרת אמור להוריד את עומק ה-Circuit. לפני כן, נסיר צמתים עם שגיאות קריאה "גרועות" וקשתות עם שגיאות Gate "גרועות" מגרף הזיווג.
...
נצייר את גרף מפת הזיווג ללא הקשתות והצמתים הגרועים.
...
ננסה ליצור מצב GHZ בן 16 Qubits כמו קודם.

```python
N = 16
```

נפעיל את הפונקציה `betweenness_centrality` כדי למצוא Qubit עבור צומת השורש. הצומת עם הערך הגבוה ביותר של מרכזיות-הביניים נמצא במרכז הגרף. ראו: https://www.rustworkx.org/tutorial/betweenness_centrality.html

לחלופין, אפשר לבחור אותו ידנית.
...
החל מצומת השורש, נייצר עץ באמצעות חיפוש רוחב-ראשון (BFS). ראו: https://qiskit.org/ecosystem/rustworkx/apiref/rustworkx.bfs_search.html#rustworkx-bfs-search
...
נציג את ה-Qubits הנבחרים, המסומנים בוורוד, בדיאגרמת מפת הזיווג.
...
עומק ה-Circuit ירד עכשיו בצורה משמעותית בהשוואה למבנה השרשרת.
...
עברנו בהצלחה את הקריטריונים עם מבנה העץ המאוזן!
...
עכשיו, ננסה ליצור מצב GHZ גדול יותר: מצב GHZ בן 30 Qubits.
עכשיו, בוא ננסה ליצור מצב GHZ גדול יותר: מצב GHZ של 30 Qubit.
### 3.1 N = 30

נעקוב אחרי מסגרת [תבניות Qiskit](/guides/intro-to-patterns).
...
#### שלב 1: מיפוי הבעיה ל-Circuit קוונטי ואופרטורים ושלב 2: אופטימיזציה לחומרת היעד {#step-1-map-problem-to-quantum-circuits-and-operators-and-step-2-optimize-for-target-hardware}

כאן אנחנו בוחרים את צומת השורש ידנית.
...
![פלט של תא הקוד הקודם](/learning/images/courses/utility-scale-quantum-computing/utility-iii/extracted-outputs/12d3ac00-7dfc-4734-9881-4e6385f2d880-0.avif)
...
![פלט של תא הקוד הקודם](/learning/images/courses/utility-scale-quantum-computing/utility-iii/extracted-outputs/6afd6c54-b427-4f34-8c4c-285c0122aebd-0.avif)
...
![פלט של תא הקוד הקודם](/learning/images/courses/utility-scale-quantum-computing/utility-iii/extracted-outputs/d36cdb58-59fe-474e-9a31-a2d9e806ab59-0.avif)
...
![פלט של תא הקוד הקודם](/learning/images/courses/utility-scale-quantum-computing/utility-iii/extracted-outputs/46df385d-dd28-4fab-8344-6ef2fd53906d-0.avif)
...
### 3.2 בחירת צומת שורש שונה ידנית {#select-a-different-root-node-manually}
...
![פלט של תא הקוד הקודם](/learning/images/courses/utility-scale-quantum-computing/utility-iii/extracted-outputs/5e2e2af5-34b5-4bb5-b393-18e87d56db08-0.avif)
...
![פלט של תא הקוד הקודם](/learning/images/courses/utility-scale-quantum-computing/utility-iii/extracted-outputs/5d81d890-a939-455f-81a6-c7e671671ba5-0.avif)
...
![פלט של תא הקוד הקודם](/learning/images/courses/utility-scale-quantum-computing/utility-iii/extracted-outputs/b9823b6d-a658-40ef-a509-f03e33534af6-0.avif)
...
באופן מפתיע, למרות שעומק העץ גדל מ-5 ל-6, עומק שני ה-Qubit ירד מ-9 ל-8! לכן נשתמש ב-Circuit האחרון.
#### שלב 3: הרצה על חומרת היעד {#step-3-execute-on-target-hardware}
...
כפי שאפשר לראות, תוצאה זו לא עמדה בקריטריונים.
...
![פלט של תא הקוד הקודם](/learning/images/courses/utility-scale-quantum-computing/utility-iii/extracted-outputs/66c87870-0.avif)
## 4. אסטרטגיה 3. הרצה עם אפשרויות דיכוי שגיאות {#4-strategy-3-run-with-the-error-suppression-options}
ניתן להגדיר את אפשרויות דיכוי השגיאות ב-Sampler V2. עיין במדריך [הגדרת מיתון שגיאות](/guides/configure-error-mitigation#advanced-error) ובתיעוד ה-API של [`ExecutionOptionsV2`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-execution-options-v2) למידע נוסף.
...
התוצאה השתפרה אך עדיין לא עמדה בקריטריונים.

ראינו שלוש רעיונות עד כה. אפשר לשלב ולהרחיב רעיונות אלה, או לחשוב על רעיונות משלך כדי ליצור Circuit GHZ טוב יותר. עכשיו בוא נחזור על המטרה שוב.
## 5. המטרה שלך (סיכום) {#5-your-goal-recap}

בנה Circuit GHZ עבור 20 Qubits או יותר כך שתוצאת המדידה תעמוד בקריטריונים: הנאמינות של מצב ה-GHZ שלך > 0.5.

- עליך להשתמש במכשיר Eagle (כגון `ibm_brisbane`) ולהגדיר את מספר ה-shots ל-40,000.
- עליך להריץ את Circuit ה-GHZ באמצעות הפונקציה `execute_ghz_fidelity`, ולחשב את הנאמינות באמצעות הפונקציה `check_ghz_fidelity_from_jobs`.
עליך למצוא את Circuit ה-GHZ הגדול ביותר ב-Qubits שעומד בקריטריונים. כתוב את הקוד שלך למטה, והצג את התוצאה עם הפונקציה `check_ghz_fidelity_from_jobs`.

עכשיו נממש את אותו תהליך עבודה GHZ כמו בחומר הקודם, אך על מכשיר Heron. זה נותן לך ניסיון עם הפריסה והתכונות של מעבדי Heron. לא מוצגות אסטרטגיות חדשות.

*זמן QPU משוער להרצת הניסוי הבא הוא 4 דק' 40 שנ'.*
...
כל הכבוד! סיימת את ההקדמה שלך לחישוב קוונטי בסדר גודל שימושי! עכשיו אתה ערוך לתרום תרומות משמעותיות במרוץ לעליונות קוונטית! תודה שהפכת את IBM Quantum&reg; לחלק מהמסע הקוונטי האישי שלך.
## סקר לאחר הקורס {#post-course-survey}

כל הכבוד על סיום הקורס! אנא הקדש רגע לעזור לנו לשפר את הקורס שלנו על ידי מילוי [הסקר הקצר](https://your.feedback.ibm.com/jfe/form/SV_5vvmdT5yFFetkgK) הבא. המשוב שלך ישמש לשיפור תוכן הקורסים שלנו וחוויית המשתמש. תודה!
