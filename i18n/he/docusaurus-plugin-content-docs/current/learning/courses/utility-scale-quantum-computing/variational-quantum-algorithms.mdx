...
(מחברת זו עשויה שלא להסתיים בזמן המוקצב בתוכנית Open Plan. השתמש במשאבי המחשוב הקוונטי בתבונה.)
...
קל לקבל את הערכים העצמיים באופן קלאסי, כך שנוכל לבדוק את עבודתנו. זה עשוי להיות קשה ככל שנתרחב לכיוון שימושיות. כאן אנחנו משתמשים ב-numpy.
...
כדי לקבל ערכים עצמיים באמצעות אלגוריתם קוונטי וריאציוני, אנחנו בונים Circuit עם Gate שמקבלים פרמטרים וריאציוניים:
...
אנחנו יודעים שניתן לחשב את ערך הציפייה של Z על ידי $\langle Z \rangle = p_0 - p_1$ עם ההסתברויות $\{0: p_0, 1: p_1\}$.
...
### 3.4 שלב 3. ביצוע באמצעות פרימיטיבים של IBM Quantum {#34-step-3-execute-using-ibm-quantum-primitives}
...
2. יצירת מופע חדש של `Session` המכיל את לולאת האופטימיזציה ואת ה-primitive המשמש לדגימת המעגל
...
השתמשו ב-**פרימיטיבים של IBM Quantum** לאינטראקציה עם Backend-ים של IBM&reg;. שני הפרימיטיבים הם Sampler ו-Estimator, והבחירה בפרימיטיב תלויה בסוג המדידה שרוצים להריץ על המחשב הקוונטי. למינימיזציה של $H_C$, השתמשו ב-Estimator מאחר שמדידת פונקציית העלות היא פשוט ערך הציפייה של $\langle H_C \rangle$.
