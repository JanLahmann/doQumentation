...
ל-`optimization_level` יש ארבע אפשרויות, מ-0 עד 3. ככל שרמת האופטימיזציה גבוהה יותר, כך מושקע יותר מאמץ חישובי באופטימיזציה של ה-circuit. רמה 0 לא מבצעת אופטימיזציה ופשוט עושה את כמות העבודה המינימלית כדי להפוך את ה-circuit לניתן להרצה על ה-backend הנבחר. רמה 3 מוציאה הכי הרבה מאמץ (ובדרך כלל runtime) בניסיון לבצע אופטימיזציה של ה-circuit. רמה 1 היא רמת האופטימיזציה כברירת מחדל.
אנחנו מבצעים transpile ל-circuit ללא אופטימיזציה (`optimization_level=0`) ועם אופטימיזציה (`optimization_level=2`).
אנחנו רואים הבדל גדול באורך ה-circuit של ה-circuits לאחר transpile.
...
### 4.1 אופטימיזציית בלוקים של שני קיוביטים באמצעות סינתזת אוניטרי {#41-two-qubit-block-optimization-using-two-qubit-unitary-synthesis}
...
### 4.2 אופטימיזציית Circuit חשובה {#42-circuit-optimization-matters}
...
אנחנו מבצעים טרנספילציה של המעגל ללא אופטימיזציה (`optimization_level=0`) ועם אופטימיזציה (`optimization_level=2`).
כפי שאתם יכולים לראות, יש הבדל גדול באורך המעגל בין המעגלים המטרנספלים.
...
### 4.4 פירוק שערים כלליים של קיוביט אחד {#44-general-1-qubit-gate-decomposition}
...
### 4.5 אופטימיזציית בלוק של קיוביט אחד {#45-one-qubit-block-optimization}
...
### 4.6 פירוק Toffoli {#46-toffoli-decomposition}
...
### 4.7 פירוק שער CU {#47-cu-gate-decomposition}
...
### 4.8 CX, ECR, CZ שקולים עד כדי Cliffordים מקומיים {#48-cx-ecr-cz-equal-up-to-local-cliffords}
