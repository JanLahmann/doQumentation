...
נניח רשימה של 4 אלמנטים, כאשר המטרה שלנו היא לזהות את האינדקס של אלמנט העומד בתנאי מסוים. לדוגמה, אנחנו רוצים למצוא את האינדקס של האלמנט השווה ל-2. בדוגמה זו, המצב הקוונטי $|01\rangle$ מייצג את האינדקס של האלמנט המקיים תנאי זה, מכיוון שהוא מצביע על המיקום שבו נמצא הערך 2.
### שלב 2: אופטימיזציה לחומרת המטרה {#step-2-optimize-for-target-hardware}
### 1: אתחול {#1-initialization}

בשלב האתחול, אנחנו יוצרים סופרפוזיציה של כל המצבים האפשריים. הדבר מושג על ידי הפעלת Gate של האדמר על כל qubit ברגיסטר של n-Qubit, מה שיוצר סופרפוזיציה שווה של $2^n$ מצבים. מתמטית, ניתן לייצג זאת כ:

$$
\frac{1}{\sqrt{N}} \sum_{x=0}^{N-1} |x\rangle
$$

כאשר $N = 2^n$ הוא המספר הכולל של המצבים האפשריים. כמו כן, אנחנו משנים את מצב ביט העזר ל-$|-\rangle$.
