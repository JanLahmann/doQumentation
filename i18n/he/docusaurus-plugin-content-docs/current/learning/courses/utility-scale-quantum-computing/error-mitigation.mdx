...
## 2. סימולציה רועשת ללא הפחתת שגיאות {#2-noisy-simulation-without-error-mitigation}
...
### 2.1 בנה Circuit בדיקה {#21-build-a-test-circuit}
...
### 2.2 בנה מודל רעש {#22-build-a-noise-model}
...
### 2.3 בנה Sampler רועש עם מודל הרעש {#23-build-a-noisy-sampler-with-the-noise-model}
...
### 2.4 הרץ Circuit קוונטי על ה-Sampler הרועש {#24-run-quantum-circuits-on-the-noisy-sampler}
...
### 2.5 הצג תוצאות {#25-plot-results}
...
### 2.6 סימולציה אידיאלית {#26-ideal-simulation}
...
### 2.8 סימולציה רועשת ריאליסטית יותר {#28-more-realistic-noisy-simulation}
...
הגדרת רמת ה-resilience (`estimator.options.resilience_level`) היא הדרך הקלה ביותר להחיל הפחתת שגיאות כשמשתמשים ב-Qiskit Estimator. ה-Estimator תומך ברמות ה-resilience הבאות (נכון ל-28/06/2024). ראו פרטים נוספים במדריך [הגדרת הפחתת שגיאות](/guides/error-mitigation-and-suppression-techniques).
...
- [הגדרת הפחתת שגיאות](/guides/error-mitigation-and-suppression-techniques)
