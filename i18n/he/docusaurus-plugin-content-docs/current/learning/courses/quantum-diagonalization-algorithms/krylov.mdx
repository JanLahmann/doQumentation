---
description: 'מתוארת דיאגונליזציה קוונטית של קרילוב (KQD), תוך התחלה בשיטות קרילוב הקלאסיות. נדונים ההתכנסות ועוצמת הדרישות החישוביות.'
...
__שיטת מרחב קרילוב__ יכולה להתייחס לכל אחת מכמה שיטות שנבנות סביב מה שנקרא __מרחב קרילוב__. סקירה מלאה של אלה חורגת מהיקף השיעור הזה, אבל מקור [\[2-4\]](#references) יכול לספק רקע רב יותר משמעותית. כאן, נתמקד במה זה מרחב קרילוב, איך ולמה הוא שימושי בפתרון בעיות ערכים עצמיים, ולבסוף איך אפשר ליישם אותו על מחשב קוונטי.
__הגדרה:__ בהינתן מטריצה סימטרית, חצי-חיובית-מוגדרת $N\times N$ בשם $A$, מרחב קרילוב $\mathcal{K}^r$ מסדר $r$ הוא המרחב הנפרש על ידי וקטורים המתקבלים מהכפלת חזקות גבוהות יותר של מטריצה $A$, עד $r-1\leq N$, בוקטור ייחוס $\vert v \rangle$.
...
\end{aligned}
$$
אנחנו רוצים לחקור כיצד תהליך זה עובד (או נכשל) כאשר אנחנו מגדילים את ממד מרחב קרילוב שלנו, $r$. לצורך זה, נישם את התהליך הבא:
...
כעת אנחנו מטילים את המטריצה שלנו $A$ על תת-המרחב של וקטור זה בלבד:
$$
...
$$
זוהי ההטלה של המטריצה על מרחב קרילוב שלנו כאשר הוא מכיל וקטור בודד, $|v_0\rangle$. ערך העצמי של מטריצה זו הוא 4 באופן טריוויאלי. אנחנו יכולים לחשוב על זה כאומדן מסדר אפס של ערכי העצמי (במקרה זה רק אחד) של $A$. למרות שזהו אומדן גרוע, הוא בסדר גודל הנכון.
...
$$
כעת אנחנו מטילים את המטריצה שלנו $A$ על תת-המרחב של הוקטורים האלה:
...
נניח שחוקר עמית מדגים את שיטת קרילוב המיושמת על מטריצת צעצוע קטנה. האם יש משהו שגוי בבחירת המטריצה $A$ והוקטור הראשוני $|\psi\rangle$?
...
$$
כדי לקבל את $|v_1\rangle$ אנחנו מיישמים את $A$ פעם אחת על $|v_0\rangle$, ולאחר מכן עושים את $|v_1\rangle$ ניצב ל-$|v_0\rangle.$
$$
...
נניח שהמטריצה המעניינת שלנו $A$ היא סימטרית וחיובית מוגדרת. זה הופך את הטיעון שלנו לרלוונטי ביותר לשיטת CG לעיל. אנחנו לא מניחים כלום לגבי דלילות כאן; ולא אנחנו טוענים ש-$A$ חייבת להיות הרמיטית (מה שהיא חייבת להיות אם היא המילטוניאן).
...
אנחנו מתחילים עם וקטור $|x_0\rangle$ שהוא פתרון משוער. למרות שיש קווי דמיון בין ניחוש זה $|x_0\rangle$ ל-$|v_0\rangle$ בסעיף 1.1, אנחנו לא מנצלים אותם כאן. לניחוש שלנו $|x_0\rangle$ יש שגיאה, שאנו קוראים לה $|e_0\rangle:$
$$
|e_0\rangle:=|x\rangle−|x_0\rangle.
$$
אנחנו גם מגדירים את השארית $R_0:$
$$
|R_0\rangle=|b\rangle−A|x_0\rangle.
$$
כאן אנחנו משתמשים ב-$R$ גדולה כדי להבחין בין השארית לממד מרחב קרילוב שלנו $r$.
...
אנחנו מעוניינים כיצד מתנהגת השגיאה שלנו כאשר היא עוברת טרנספורמציה על ידי המטריצה שלנו. אז בואו נחשב את ה-$A$-נורמה של השגיאה. כלומר
$$
\begin{aligned}
∥|e_0\rangle−|p_0\rangle∥_A^2&=\left(\langle e_0|A−\langle p_0|A\right)\left(|e_0\rangle−|p_0\rangle\right)\\
 ...
 \end{aligned}
$$
כאשר השתמשנו בסימטריה של $A$ וגם ב-$A |e_0\rangle = |R_0\rangle.$ כאן $d$ הוא קבוע עצמאי מ-$|p_0\rangle$. כאמור בסעיף 1.2, ה-$A$-נורמה של השגיאה אינה הכמות היחידה שנוכל לבחור למזער, אבל היא טובה. אנחנו רוצים לראות כיצד כמות זו משתנה עם בחירת וקטורי התיקון $|p_0\rangle.$ אז אנחנו מגדירים את הפונקציה $f$ על ידי הגדרה
$$
f(|p_0\rangle)=\langle p_0|A|p_0\rangle−2\langle R_0|p_0\rangle+d.
$$
$f$ היא פשוט השגיאה $|e_1\rangle$ כפונקציה של התיקון $|p_0\rangle$ הנמדדת ב-$A$-נורמה. לכן, אנחנו רוצים לבחור $|p_0\rangle$ כך ש-$f(|p_0\rangle)$ יהיה קטן ככל האפשר. לצורך זה, אנחנו מחשבים את הגרדיאנט של $f$. תוך שימוש בסימטריה של $A$ יש לנו
$$
...
בשלב הבא, אנחנו בוחרים שוב וקטור $|p_1\rangle$ ומוסיפים את ערכו לקירוב הנוכחי. תוך שימוש באותו טיעון כקודם אנחנו בוחרים $|p_1\rangle = \alpha_1 |R_1\rangle$ עבור סקלר $\alpha_1$ כלשהו. אנחנו ממשיכים בדרך זו, כך שהאיטרציה ה-$k$ של הוקטור שלנו היא
$$
|x_{k+1}\rangle=|x_0\rangle+\alpha_0 |R_0\rangle+\alpha_1 |R_1\rangle+⋯+\alpha_k |R_k\rangle.
$$
באופן שקול, אנחנו רוצים לבנות את המרחב ממנו אנחנו בוחרים את האומדנים המשופרים שלנו על ידי הוספת $|R_0\rangle$, $|R_1\rangle$, וכן הלאה, בסדר. הוקטור המוערך ה-$k$ נמצא ב-
$$
...
אנחנו רואים שהערך העצמי המינימלי מגיע לדיוק סביר ברגע שתת-מרחב Krylov גדל ל-$\mathcal{K}^2,$ ומדויק לחלוטין ב-$\mathcal{K}^3.$

### 2.2 קנה המידה של הזמן עם ממד המטריצה {#22-time-scaling-with-matrix-dimension}

בואו נשכנע את עצמנו שהשיטה של קרילוב יכולה להיות עדיפה על פני פותרי ערכים עצמיים מספריים מדויקים באופן הבא:
* בנה מטריצות אקראיות (לא דלילות, לא היישום האידיאלי עבור KQD)
* קבע ערכים עצמיים באמצעות שתי שיטות: ישירות באמצעות NumPy ובאמצעות מרחב קרילוב.
* אנחנו בוחרים סף לרמת הדיוק הנדרשת מהערכים העצמיים שלנו, לפני שאנחנו מקבלים את האומדנים של קרילוב.
* השווה את זמן ה-wall הנדרש לפתרון בשתי הדרכים האלה.
...
המורכבות הזמנית של אלכסון מספרי היא $O(n^3)$ (עם וריאציה מסוימת בין אלגוריתמים). המורכבות הזמנית של יצירת בסיס אורתונורמלי של $n$ וקטורים היא גם $O(n^3)$. אז היתרון של שיטת קרילוב __אינו__ קשור לשימוש ב-$\it{some}$ בסיס אורתונורמלי כלשהו, אלא לשימוש בבסיס אורתונורמלי מסוים שבורר ביעילות את הערכים העצמיים המעניינים. כבר ראינו זאת מתוך שרטוט ההוכחה בסעיף הראשון של השיעור הזה, וזה קריטי לערבויות ההתכנסות בשיטות קרילוב.
בואו נסקור את ההתקדמות שלנו עד כה:
* עבור מטריצות גדולות מאוד, שיטת מרחב קרילוב עשויה להניב ערכים עצמיים מקורבים בתוך הסבילויות הנדרשות מהר יותר מאלגוריתמי אלכסון מסורתיים.
* עבור מטריצות גדולות מאוד כאלה, יצירת מרחב קרילוב היא החלק הכי גוזל זמן בשיטת מרחב קרילוב.
* לכן, דרך יעילה ליצירת מרחב קרילוב תהיה בעלת ערך רב.
זה סוף סוף המקום שבו מחשב קוונטי נכנס לתמונה.
...
כך שזה לא טריוויאלי, אבל זהו תהליך שנחקר היטב במחשוב קוונטי. אנחנו מבצעים אבולוציית זמן על מחשבים קוונטיים באמצעות תהליך הנקרא טרוטריזציה, שבפני עצמו הוא נושא עשיר[\[10\]](#references). אבל ברמה גבוהה מאוד, על ידי פיצול אבולוציית הזמן לצעדים קטנים מאוד, נגיד $m$ צעדים בגודל $dt$, אנחנו מגבילים את השפעות אי-ההתחלפות של האיברים.
...
\end{aligned}
$$
אנחנו יכולים להטיל את ההמילטוניאן H על תת-מרחב קריילוב היוניטרי, $\mathcal{K}_U^r$. במילים אחרות, אנחנו מחשבים כל איבר מטריצה של $H$ בבסיס $\mathcal{K}_U^r$. נתייחס למטריצה המוטלת הזו כ-$\tilde{H}$.
...
$$
כאן $P_\alpha$ הוא מחרוזת פאולי מהצורה $P_\alpha \sim IZIXII...YZXIX$ או קבוצה של מחרוזות פאולי כאלה שמתחלפות אחת עם השנייה. בהינתן שאנו יכולים לכתוב את $H$ כסכום כזה של אופרטורים הניתנים למדידה, הביטויים הבאים לאיברי המטריצה של $\tilde{H}$ ניתנים למימוש באמצעות ה-Estimator הפרימיטיבי של IBM Quantum.
...
למה אנחנו לא יכולים פשוט לאלכסן את $\tilde{H}$? מכיוון ש-$\tilde{S}$ מכיל את המידע על הגאומטריה של בסיס קרילוב (שאינו אורתוגונלי בכל המקרים חוץ ממקרים מיוחדים מאוד), $\tilde{H}$ בפני עצמו לא מתאר הטלה של ההמילטוניאן המלא, כך שהערכים העצמיים שלו אינם קשורים במיוחד לאלה של ההמילטוניאן המלא -- הם יכולים להיות כל ערכים אקראיים. פתרון בעיית הערכים העצמיים המוכללת נדרש כדי לקבל את הערכים העצמיים והוקטורים העצמיים המקורבים המתאימים להטלה של ההמילטוניאן המלא למרחב קרילוב..
![תרשים circuit עם שכבות רבות המציין שיש להשתמש ב-circuit פעמים רבות עם מצבים שונים כדי לבצע את מבחן ה-Hadamard המשונה.](/learning/images/courses/quantum-diagonalization-algorithms/krylov/kqd-fig4.avif)
...
אנחנו בוחרים המילטוניאן המתאים לשרשרת ספינים שיכולים לקבל כל כיוון במרחב, הנקראת "שרשרת היזנברג". אנחנו מניחים שהספין ה-$i^\text{th}$ מושפע רק משכניו הקרובים (ספינים $(i-1)^\text{th}$ ו-$(i+1)^\text{th}$) ולא משכנים רחוקים יותר. אנחנו גם מאפשרים את האפשרות שהאינטראקציה בין הספינים שונה כאשר הם מצביעים לאורך צירים שונים. אסימטריה זו מתעוררת לעיתים, למשל, בשל מבנה סריג הגביש שבו הספינים משובצים.
...
בחרו מצב ייחוס $\vert \psi \rangle$ שיש לו חפיפה כלשהי עם מצב היסוד. עבור המילטוניאן זה, אנחנו משתמשים במצב עם עירור ב-Qubit האמצעי $\vert 00..010...00 \rangle$ כמצב הייחוס שלנו.
...
ניתן לממש את אופרטור התפתחות הזמן הנוצר על ידי המילטוניאן נתון: $U=e^{-iHt}$ באמצעות [קירוב ליי-טרוטר](https://docs.quantum.ibm.com/api/qiskit/qiskit.synthesis.LieTrotter). לשם הפשטות, אנחנו משתמשים ב-```PauliEvolutionGate``` המובנה במעגל התפתחות הזמן. התחביר הכללי לכך הוא כדלקמן.
...
מעגל בעומק כזה אינו יכול להחזיר תוצאות שמישות במחשבים קוונטיים מודרניים. אם אנחנו רוצים לבנות את $\tilde{H}$ ו-$\tilde{S},$ אנחנו זקוקים לדרך טובה יותר. זו הסיבה למבחן הדמארד היעיל שיוצג להלן.

### 4. 2 שלב 2. אופטימיזציה של מעגלים ואופרטורים לחומרת היעד {#4-2-step-2-optimize-circuits-and-operators-for-target-hardware}
...
במקום לממש את אופרטור האבולוציה בזמן במדויק, אנחנו יכולים להשתמש בפירוק Trotter כדי לממש קירוב שלו. חזרה מספר פעמים על פירוק Trotter מסדר מסוים מאפשרת לנו להפחית עוד יותר את השגיאה הנובעת מהקירוב. בהמשך, אנחנו בונים ישירות את המימוש של Trotter בדרך הכי יעילה לגרף האינטראקציה של ההמילטוניאן שאנחנו בוחנים (אינטראקציות של שכנים קרובים בלבד). בפועל אנחנו מכניסים סיבובי פאולי $R_{xx}$, $R_{yy}$, $R_{zz}$ עם עוצמות צימוד $J_x,$ $J_y,$ ו-$J_z$ וזווית פרמטרית $t$, המתאימים למימוש המקורב של $e^{-i (J_x XX + J_y YY + J_z ZZ) t}$. בהתחשב בהבדל בהגדרה של סיבובי הפאולי ואבולוציית הזמן שאנחנו מנסים לממש, נצטרך להשתמש בפרמטר $2*dt$ כדי להשיג אבולוציית זמן של $dt$. בנוסף, אנחנו הופכים את סדר הפעולות עבור מספר אי-זוגי של חזרות של צעדי Trotter, שזה שקול פונקציונלית אך מאפשר לסנתז פעולות סמוכות ביחידה $SU(2)$ אחת. זה נותן מעגל הרבה יותר רדוד מזה שמתקבל עם הפונקציונליות הגנרית `PauliEvolutionGate()`.
...
#### מעגלי תבנית לחישוב איברי המטריצה של $\tilde{S}$ ו-$\tilde{H}$ דרך מבחן הדאמר {#template-circuits-for-calculating-matrix-elements-of-tildes-and-tildeh-via-hadamard-test}
...
### 4.3 שלב 3. הרצה עם פרימיטיב IBM Quantum {#43-step-3-execute-using-an-ibm-quantum-primitive}
...
![פלט תא הקוד הקודם](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/a5d4a983-1a30-4cea-b695-3e6a67338633-0.avif)
