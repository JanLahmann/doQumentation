...
כדי להריץ על מחשב קוונטי אמיתי של IBM, תצטרך גם (בחינם) [**חשבון IBM Quantum® Platform**](https://quantum.cloud.ibm.com/signin?redirectTo=%2F). אפשר לפתוח מופע דרך ה-Open Plan ולקבל 10 דקות של זמן יחידת עיבוד קוונטי (QPU) לכל חלון גלגול של 28 יום, וזה בעצם הרבה! *אם יש לך בעיות עם החשבון שלך, ראה את [דף התמיכה](/guides/support).*
...
פתח את Composer [כאן](https://quantum.cloud.ibm.com/composer).
...
האם אתה יכול לראות סופרפוזיציה ישירות ממדידה בודדת?
<Accordion>
<AccordionItem title="תשובה">
לא. מדידה בודדת תמיד מחזירה ערך קלאסי: 0 או 1. רעיון ה"תערובת" מופיע רק בסטטיסטיקות שרואים אחרי הרצות רבות, המכונות גם shots.
</AccordionItem>
</Accordion>
...
אם qubit נמצא בסופרפוזיציה, מה רואים במדידה בודדת, ולמה צריך shots רבים?
<Accordion>
<AccordionItem title="תשובה">
במדידה בודדת רואים רק תוצאה קלאסית אחת: 0 או 1. צריך shots רבים כי ה"סופרפוזיציה" מופיעה כהתפלגות הסתברות, שאותה אפשר להעריך רק על ידי איסוף סטטיסטיקות על פני הרצות חוזרות.
</AccordionItem>
</Accordion>

הסתכל על היסטוגרמת המדידה של ה-circuit "שלום עולם" שלך ב-Composer. מה אתה רואה? למה זו חתימה של הסתבכות?
<Accordion>
<AccordionItem title="תשובה">
היא מראה לך סיכוי של 50% להיות במצב $|00\rangle$ וסיכוי של 50% להיות במצב $|11\rangle$. זה אומר שאם תמדוד אחד להיות 0, השני יהיה 0, ואם תמדוד אחד להיות 1, גם השני יהיה 1. זו קורלציה בין שני ה-qubit שאפשר להסביר באמצעות הסתבכות.
</AccordionItem>
</Accordion>
...
עכשיו בואו נגדיר פונקציית עזר שמטפלת בתהליך של הרצת circuit קוונטי ואחזור תוצאות מדידה. הפונקציה הזו דואגת ל-transpile של ה-circuit כדי להתאים לסט ההוראות של ה-backend, הרצתו דרך פרימיטיב `Sampler`, וחילוץ הספירות מהתוצאות.
...
בסימולטור אידיאלי, אילו שתי מחרוזות ביטים אמורות לשלוט בהיסטוגרמה?
<Accordion>
<AccordionItem title="תשובה">
...
למה סימולטור מושלם לא יראה תמיד בדיוק את אותו מספר ספירות של 00 כמו של 11?
<Accordion>
<AccordionItem title="תשובה">
גם אם סימולטור "מושלם" ומוביל למצב Bell מושלם, הוא עדיין מדמה תהליך אקראי מטבעו, כך שתנודות סטטיסטיות עדיין יתרחשו. זה כמו להטיל מטבע 1000 פעמים: גם אם יש בדיוק סיכוי 50-50 שהמטבע יינחת על עץ או פלי, זה לא אומר שתמיד תקבל בדיוק 500 מקרים של עץ ו-500 של פלי.
</AccordionItem>
</Accordion>

למה מחשב קוונטי אמיתי עשוי להראות כמה תוצאות של 01 או 10 למרות שהסימולטור לא הראה?
<Accordion>
<AccordionItem title="תשובה">
כי במכשירים אמיתיים יש רעש. שערים ומדידות אינם מושלמים, וזה יכול להכניס שגיאות מדי פעם.
</AccordionItem>
</Accordion>
