...
כמו כן, במימוש של Qiskit ל-VQD, במקום להתחשב באובזרבבלים האפקטיביים שתוארו במחברת הקודמת, הנאמנויות $|\langle \psi(\vec{\theta})| \psi(\vec{\theta^j})\rangle  |^2$ מחושבות ישירות באמצעות האלגוריתם `ComputeUncompute`, שממנף primitive `Sampler` כדי לדגום את ההסתברות לקבל $|0\rangle$ עבור המעגל
$U_A^\dagger(\vec{\theta})U_A(\vec{\theta^j})$. זה עובד בדיוק בגלל שההסתברות הזו היא
...
נשתמש בפונקציה הבאה כדי לחשב את עונש החפיפה. שימו לב שזה עדיין חלק ממיפוי הבעיה למעגלים קוונטיים. עם זאת, כפי שנדון בשיעור הקודם, פונקציה זו מחשבת את החפיפה בין מעגל וריאציוני נוכחי לבין המעגל המותאם ממצב אנרגיה/עלות נמוכה יותר קודם שהתקבל. המעגל החדש שנוצר צריך גם הוא לעבור טרנספילציה כדי לרוץ על חומרה אמיתית. ראינו את הפונקציה הזו בעבר, בשימוש על סימולטור. כאן, עלינו כבר להתחשב בטרנספילציה ובאופטימיזציה הקשורה אליה עבור המקרה שבו אנו משתמשים ב-Backend אמיתי, ומכאן השורות סביב `if realbackend == 1`. זה מערבב מעט את שלב 2, אך נתייחס לשלב 2 במפורש בהמשך.
...
עכשיו נוסיף את פונקציית העלות של VQD. שימו לב שבהשוואה לשיעור הקודם, יש לנו כעת שני ארגומנטים נוספים (`realbackend` ו-`backend`) שיעזרו לנו עם הטרנספילציה כאשר אנחנו משתמשים ב-Backend אמיתיים.
...
שוב, נשתמש בסימולטורים לצורך ניפוי באגים, ולאחר מכן נעבור לחומרה אמיתית.
...
התוצאות האלה קרובות למדי לתוצאות הצפויות, למעט שגיאת קירוב ופאזה גלובלית. נוכל לכוונן את הסיבולת של האופטימיזטור הקלאסי ו/או את העונשים על חפיפת וקטור המצב כדי להשיג ערכים מדויקים יותר.
...
כדי להריץ את זה על חומרה אמיתית, עלינו לבצע טרנספילציה של המעגלים הקוונטיים למחשב הקוונטי שבחרנו. לצורכינו כאן, פשוט נשתמש ב-Backend הפחות עמוס.
...
המטרה שלנו היא למזער את ערך הציפייה של האובזרבל המייצג אנרגיה (ההמילטוניאן $\hat{\mathcal{H}}$):
...
נוכל למזער את פונקציית העלות הזו כדי לחשב פרמטרים אופטימליים, ונוכל לבדוק את הקוד שלנו קודם באמצעות סימולטור מקומי.
...
אפשר גם להשתמש ב-VQD כדי לפתור עבור $k=2$ מצבים כולל (מצב היסוד והמצב המעורר הראשון).
...
ניתן להשתמש בכל אופטימייזר קלאסי כדי למזם את פונקציית העלות. במערכת קוונטית אמיתית, אופטימייזר שנועד לנופי פונקציות עלות לא חלקים בדרך כלל עובד טוב יותר. כאן אנחנו משתמשים ב[שגרת COBYLA](https://docs.scipy.org/doc/scipy/reference/optimize.minimize-cobyla.html#optimize-minimize-cobyla) מ-SciPy דרך פונקציית minimize.

מכיוון שאנו מבצעים בצורה איטרטיבית קריאות רבות ל-Runtime, אנחנו משתמשים ב-Session כדי לבצע את כל הקריאות בתוך בלוק אחד. יתרה מזאת, עבור QAOA, הפתרון מקודד בהתפלגות הפלט של Circuit ה-ansatz הקשור עם הפרמטרים האופטימליים מהמינוי. לכן נזדקק ל-Primitive של Sampler, ונאתחל אותו עם אותה Session.
...
המשיכו לשיעור האחרון כדי לבצע את המבחן ולזכות בתג שלכם!
