...
אלגוריתם SKQD (דיאגונליזציה קוונטית של קריילוב מבוססת-דגימה) הוא אלגוריתם קוונטי חזק נוסף מבוסס-דגימה, הבנוי על עקרונות SQD. בעוד מטרתו זהה — למצוא תת-מרחב טוב לדיאגונליזציה — SKQD משתמש בשיטה מובנית יותר ליצירת מחרוזות-הסיביות, במיוחד עבור בעיות כמו האמילטוניאנים של סריג.
...
בשלב הבא, אוספים דגימות של מחרוזות-סיביות מכל אחד מ-$d$ המצבים השונים ($|\phi_0\rangle, |\phi_1\rangle, \dots, |\phi_{d-1}\rangle$) שהוכנו בשלב הקודם. כל מחרוזות-הסיביות הללו מאוחדות יחד ליצירת הבסיס של תת-המרחב.

#### שלב 3: השלכה ודיאגונליזציה קלאסית {#step-3-project-and-diagonalize-classically}

שלב זה זהה לזה שב-SQD. מחרוזות-הסיביות שנאספו משמשות להשלכת ההאמילטוניאן המלא לתת-המרחב שהן פורסות. המטריצה הקטנה שמתקבלת, $\tilde{H}$, עוברת דיאגונליזציה על מחשב קלאסי למציאת אנרגיית מצב היסוד.

### יתרונות מרכזיים וערבויות של SKQD {#key-advantages-and-guarantees-of-skqd}

הגישה המובנית של SKQD מספקת יתרונות ייחודיים:

* התכנסות מוכחת: היתרון המרכזי של SKQD הוא הערבות התיאורטית שלו להתכנסות בתנאים ספציפיים ומוגדרים היטב. אם מצב היסוד האמיתי הוא דליל (ניתן לקרב אותו היטב במספר פולינומי של מחרוזות-סיביות) ופרק האנרגיה למצב העירור הראשון אינו קטן מדי, הוכח שהשיטה עובדת ביעילות. בתנאים אלה, SKQD מבטיח שהוא ימצא את מחרוזות-הסיביות הקריטיות המרכיבות את מצב היסוד ויוכל לקרב את אנרגיית מצב היסוד בדיוק גבוה. הדבר דורש רק מספר פולינומי של ניסויים קוונטיים ו-shots. ערבות זו מעמידה את הגישה מבוססת-הדגימה על בסיס תיאורטי קפדני, בדומה לשיטות מבוססות כמו אמידת פאזה קוונטית.

* יתרונות משותפים עם SQD: בדומה ל-SQD, ל-SKQD יש גם תכונת עמידות לרעש. כלומר, כל עוד קיימות כל מחרוזות-הסיביות הטובות בתוך קבוצת מחרוזות-הסיביות שנדגמו, הדיאגונליזציה מקצה משקל כמעט אפסי למחרוזות-סיביות שגויות, מה שהופך את התהליך לעמיד לרעש. יתר על כן, מאחר שהפתרון נוצר ממחשב HPC קלאסי, ניתן לאמת את אנרגיית הפתרון בצורה קלאסית.
...
בואו נתרגל את אלגוריתם SKQD, תוך הדגמת זרימת העבודה ההטרוגנית שתוארה קודם לכן. התהליך מחולק לארבעה שלבים נפרדים, כל אחד עם סקריפט Python משלו וסקריפט shell מתאים להגשת עבודות.
...
```
בסיום הביצוע, התוצאות שנמדדו (מחרוזות-הסיביות) מכל ה-Circuits נאספות ומשולבות, וספירותיהן נשמרות לקובץ `counts.json`.
...
תוכנית `postprocessing.py` קוראת תחילה את קובץ `counts.json` המכיל את תוצאות המדידה. לאחר מכן היא בונה מחדש את ההאמילטוניאן של מודל אנדרסון (תוך שימוש באותם פרמטרים כמו ב-`mapping.py`). לאחר מכן היא מעבירה את מחרוזות-הסיביות שנמדדו ואת הגדרת ההאמילטוניאן לפונקציה `diagonalize_fermionic_hamiltonian`. פונקציה זו מבצעת את לוגיית ה-SKQD המרכזית: היא משתמשת במחרוזות-הסיביות לבניית ההאמילטוניאן המושלך $\tilde{H}$ ומבצעת עליו דיאגונליזציה למציאת אנרגיית מצב היסוד.
...
סקריפט העבודה האחרון רץ על מחיצה קלאסית (`--partition=normal`), מאחר שכל הניתוח הוא קלאסי. עבור תת-מרחבים גדולים, שלב זה עשוי לדרוש משאבי HPC קלאסיים רבים יותר.
...
כדי לחקור עוד מקרי שימוש ואלגוריתמים, ראה ב[תיעוד](https://docs.quantum.ibm.com/guides) וב[מדריכים](https://docs.quantum.ibm.com/tutorials) שלנו ב-IBM Quantum Platform, ודאג לבקר במשאבים המשותפים בשיעור הבא לקבלת מידע נוסף על אלגוריתמים ותוכנות עבור מדעני מחשוב ומנהלי מרכזי נתונים כאחד.
