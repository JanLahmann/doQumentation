...
כאן אנחנו משתמשים בתכונה המחזורית של העקבה לשוויון השני, ולשוויון השלישי אנחנו משתמשים בעובדה שכל $\Pi_a$ היא מטריצת הטלה, ולכן מקיימת $\Pi_a^2 = \Pi_a.$
...
1. כל ערך $\operatorname{Tr}(P_a \rho)$ חייב להיות אי-שלילי, בשל העובדה שהעקבה של מכפלת שתי מטריצות חיוביות למחצה היא תמיד אי-שלילית:
...
2. התנאי $P_0 + \cdots + P_{m-1} = \mathbb{I}_{\mathsf{X}}$ יחד עם הלינאריות של העקבה מבטיחים שסכום ההסתברויות הוא $1.$
...
התכונה השנייה היא שאם נסכום את המטריצות הללו נקבל את מטריצת הזהות.
...
עכשיו בוא נאמת שעבור כל אוסף $\{P_0,\ldots,P_{m-1}\}$ של מטריצות חיוביות למחצה המקיימות $P_0 + \cdots + P_{m-1} = \mathbb{I}_{\mathsf{X}},$ המיפוי המוגדר על ידי
...
זה מאפשר להופיע לביטויים $\vert b \rangle \langle b \vert$ ו-$\vert c \rangle \langle c \vert$, המצטמצמים למטריצת הזהות עם סיכום על $b$ ו-$c$ בהתאמה.

מהנחה ש-$P_0,\ldots,P_{m-1}$ הן חיוביות למחצה, כך גם $P_0^{T},\ldots,P_{m-1}^{T}.$
בפרט, טרנספוז של מטריצה הרמיטית מניב מטריצה הרמיטית נוספת, וערכי העצמי של כל מטריצה ריבועית והטרנספוז שלה תמיד זהים.
מכאן נובע ש-$J(\Phi)$ היא חיובית למחצה.
...
הנה רמז: השקילות בין הביטוי השני לשלישי אינה תלויה ב-$\rho$ כמטריצת צפיפות או בכל $P_a$ כחיובית למחצה. נסו להוכיח זאת תחילה עבור מכפלות טנסוריות מהצורה $\rho = M\otimes N$ ואז הסיקו שזה חייב להיות נכון בכלליות על ידי לינאריות.
...
$\operatorname{Tr}_{\mathsf{X}}\bigl((P_a \otimes \mathbb{I}_{\mathsf{Z}}) \rho)$ היא בהכרח חיובית למחצה.
