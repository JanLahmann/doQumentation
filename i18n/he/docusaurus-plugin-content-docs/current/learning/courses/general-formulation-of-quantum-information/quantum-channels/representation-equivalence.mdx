...
כדי להבהיר, אנחנו מתעלמים מהטענות האחרות — ובפרט אין לנו את ההנחה ש-$\Phi$ הוא ערוץ.
...
לפיכך, זו אינה ביטוי יחיד של $J(\Phi)$ — אנחנו רק מניחים שיש לנו ביטוי כזה.
...
ועכשיו אנחנו מגיעים לתחבולה שגורמת לחלק זה של ההוכחה לעבוד.
אנחנו מגדירים את מטריצות Kraus שלנו $A_0,\ldots,A_{N-1}$ לפי המשוואה הבאה.
...
פירוש וויקטור של $A_k$ הוא שאנו ערמים את העמודות זו על זו (כאשר העמודה השמאלית ביותר נמצאת בראש ועד לעמודה הימנית ביותר בתחתית), ליצירת וקטור.
...
(שבו אנחנו מתייחסים ל*טרנספוז של המטריצה* בצד שמאל).
...
לכן, אם נניח בשתיקה ש-$mN/n$ הוא מספר שלם — מה ששקול לכך ש-$N$ הוא כפולה של $n/\operatorname{gcd}(n,m)$ — אנחנו חופשיים לקחת את $\mathsf{G}$ כך שקבוצת המצבים הקלאסיים שלה תהיה $\{0,\ldots,N-1\}$.
...
זה משלים את ההוכחה של המסקנה האחרונה, ולכן הראינו את שקילות ארבע ההגדרות המפורטות בתחילת הסעיף.
