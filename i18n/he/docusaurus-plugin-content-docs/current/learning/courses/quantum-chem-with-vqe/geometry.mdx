...
התוצאות של פלט זה נדונות להלן בסעיף העיבוד לאחר; לעת עתה, שים לב פשוט שהסימולציה הצליחה. עכשיו אתה מוכן להריץ על חומרה אמיתית. נגדיר את ה-resilience ל-`1`, מה שמציין שישמש מיטוב שגיאות TREX. כעת שאנחנו עובדים עם חומרה אמיתית, נשתמש ב-Qiskit Runtime ו-Runtime primitives. שים לב שגם לולאת ה-for הקשורה לגיאומטריה וגם ניסיונות הווריאציה המרובים נמצאים בתוך ה-Session.
...
בנוסף לציפייה לאורך קשר H2 של 0.74 אנגסטרום, האנרגיה הכוללת אמורה להיות 1.17- Hartrees. אנחנו רואים שתוצאות החומרה האמיתית היו קרובות יותר לערכים אלה מאשר הסימולטור. זה כנראה משום שרעש היה נוכח (או מדומה) בשני המקרים, אך רק במקרה של חומרה אמיתית הופעל מיטוג שגיאות.

### סיכום {#closing}

זה מסיים את הקורס שלנו על VQE לכימיה קוונטית. אם אתה מעוניין להבין חלק מתורת המידע הבסיסית המשמשת בחישוב קוונטי, עיין בקורס של John Watrous על [יסודות מידע קוונטי](/learning/courses/basics-of-quantum-information). לדוגמה קצרה נוספת של תהליך עבודה של VQE, ראה את [מדריך אומדן אנרגיית מצב היסוד של שרשרת Heisenberg עם VQE](/tutorials/spin-chain-vqe). או עיין ב[מדריכים](https://docs.quantum.ibm.com/tutorials) ו[קורסים](/learning) כדי למצוא חומרי לימוד נוספים על הטכנולוגיה העדכנית ביותר בחישוב קוונטי.
