...
# קביעת גיאומטריה מולקולרית {#determining-a-molecular-geometry}

בחלק הקודם, מימשנו VQE כדי לקבוע את אנרגיית המצב היסודי של מולקולה. זה שימוש לגיטימי למחשוב קוונטי, אך שימושי עוד יותר יהיה לקבוע את המבנה של מולקולה.

## שלב 1: מיפוי קלטים קלאסיים לבעיה קוונטית {#step-1-map-classical-inputs-to-a-quantum-problem}

נישאר עם הדוגמה הבסיסית שלנו של מימן דו-אטומי — הפרמטר הגיאומטרי היחיד לשינוי הוא אורך הקשר. לשם כך, נפעל כבעבר, אך נשתמש במשתנה בבנייה הראשונית של המולקולה (אורך קשר, *x*, כארגומנט). זה שינוי פשוט יחסית, אך הוא מחייב שהמשתנה ייכלל בפונקציות לאורך כל התהליך, מכיוון שהוא מתחיל בבניית ה-Hamiltonian הפרמיוני ומתפשט דרך המיפוי ולבסוף אל פונקציית העלות.

ראשית, נטען כמה מהחבילות שהשתמשנו בהן קודם ונגדיר את פונקציית Cholesky.
...
כעת כדי להגדיר את ה-Hamiltonian שלנו, נשתמש ב-PySCF בדיוק כמו בדוגמה הקודמת, אבל הפעם נכלול משתנה, `x`, שישמש כמרחק הבין-אטומי שלנו. זה יחזיר את אנרגיית הליבה, אנרגיית האלקטרון הבודד, ואנרגיות שני-האלקטרונים כבעבר.
...
זכור שהבנייה לעיל יוצרת Hamiltonian פרמיוני בהתבסס על מין האטום, הגיאומטריה, והאורביטלים האלקטרוניים. להלן, אנחנו ממפים את ה-Hamiltonian הפרמיוני הזה לאופרטורי Pauli. פונקציית `build_hamiltonian` זו תכלול גם היא משתנה גיאומטרי כארגומנט.
...
נטען את החבילות הנותרות להרצת VQE עצמו, כגון ה-ansatz של efficient_su2 וממזערי SciPy:
...
נגדיר שוב את פונקציית העלות, אבל מאחר שזו תמיד קיבלה Hamiltonian בנוי ומומפה כארגומנט, אין שינוי בפונקציה זו.
...
מכיוון שה-Hamiltonian ישתנה עם כל גיאומטריה חדשה, גם הטרנספיילציה של האופרטור תשתנה בכל שלב. עם זאת, אנחנו יכולים להגדיר pass manager כללי שיוחל בכל שלב, ספציפי לחומרה שברצוננו להשתמש בה.

כאן נשתמש ב-Backend הפחות עמוס הזמין. נשתמש ב-Backend הזה כמודל ל-AerSimulator שלנו, כך שהסימולטור יחקה, למשל, את התנהגות הרעש של ה-Backend האמיתי. מודלי הרעש הללו אינם מושלמים, אך הם עשויים לעזור לך לדעת מה לצפות מחומרה אמיתית.
...
אנחנו מייבאים את ה-pass manager והחבילות הקשורות כדי לעזור לנו לאפטמז את ה-Circuit שלנו. שלב זה, והשלב שמעליו, הם בלתי תלויים ב-Hamiltonian, ולכן אינם משתנים מהשיעור הקודם.
...
## שלב 3: הרצה באמצעות primitives של IBM Quantum. {#step-3-execute-using-ibm-quantum-primitives}

בבלוק הקוד שלמטה אנחנו מגדירים מערך לאחסון התוצאות מכל שלב של המרחק הבין-אטומי $x$. טווח הערכים של $x$ נבחר על בסיס הידע שלנו על אורך הקשר שיווי-המשקל הניסיוני: 0.74 אנגסטרום. נריץ זאת קודם על סימולטור, ולכן נייבא את ה-Estimator שלנו (BackendEstimator) מ-`qiskit.primitives`. בכל שלב גיאומטרי, אנחנו בונים את ה-Hamiltonian ומאפשרים מספר מסוים של צעדי אופטימיזציה (כאן 500) באמצעות האופטימיזר "cobyla". בכל שלב גיאומטרי, אנחנו שומרים גם את האנרגיה הכוללת וגם את האנרגיה האלקטרונית. בגלל מספר צעדי האופטימיזציה הגבוה, זה עלול לקחת שעה או יותר. ייתכן שתרצה לשנות את הפרמטרים שלמטה כדי לקצר את הזמן הדרוש.
...
התוצאות של פלט זה נדונות להלן בסעיף העיבוד לאחר; לעת עתה, שים לב פשוט שהסימולציה הצליחה. עכשיו אתה מוכן להריץ על חומרה אמיתית. נגדיר את ה-resilience ל-`1`, מה שמציין שישמש מיתון שגיאות TREX. כעת שאנחנו עובדים עם חומרה אמיתית, נשתמש ב-IBM Quantum Compute Service ו-IBM Quantum primitives. שים לב שגם לולאת ה-for הקשורה לגיאומטריה וגם ניסיונות הווריאציה המרובים נמצאים בתוך ה-Session.

מכיוון שיש עלויות ומגבלות זמן הקשורות להרצות על חומרה אמיתית, הפחתנו את מספר שלבי הגיאומטריה ושלבי האופטימיזציה שלמטה. הקפד להתאים שלבים אלה בהתאם למטרות הדיוק שלך ולמגבלות הזמן.
...
## שלב 4: עיבוד לאחר {#step-4-post-processing}

עבור גם הסימולטור וגם החומרה האמיתית, אנחנו יכולים לשרטט את אנרגיות מצב היסוד שחושבו עבור כל מרחק בין-אטומי ולראות היכן מושגת האנרגיה הנמוכה ביותר. זה אמור להיות המרחק הבין-אטומי הנמצא בטבע, ואכן הוא קרוב. עקומה חלקה יותר ניתן להשיג על ידי ניסוי ansaetze ואופטימיזרים אחרים, והרצת החישוב מספר פעמים בכל שלב גיאומטרי ומיצוע על פני מספר תנאים ראשוניים אקראיים.
...
שים לב שפשוט הגדלת מספר צעדי האופטימיזציה לא צפויה לשפר את התוצאות מהסימולטור, שכן כל האופטימיזציות התכנסו בפועל לסבילות הנדרשת בפחות ממספר האיטרציות המרבי.

התוצאות מהחומרה האמיתית דומות, פרט לטווח ערכים שנדגם שהוא מעט שונה.
...
בנוסף לציפייה לאורך קשר H2 של 0.74 אנגסטרום, האנרגיה הכוללת אמורה להיות 1.17- Hartrees. אנחנו רואים שתוצאות החומרה האמיתית היו קרובות יותר לערכים אלה מאשר הסימולטור. זה כנראה משום שרעש היה נוכח (או מדומה) בשני המקרים, אך רק במקרה של חומרה אמיתית הופעל מיתון שגיאות.

### סיכום {#closing}

זה מסיים את הקורס שלנו על VQE לכימיה קוונטית. אם אתה מעוניין להבין חלק מתורת המידע הבסיסית המשמשת בחישוב קוונטי, ראה בקורס של John Watrous על [יסודות מידע קוונטי](/learning/courses/basics-of-quantum-information). לדוגמה קצרה נוספת של תהליך עבודה של VQE, ראה את [מדריך אומדן אנרגיית מצב היסוד של שרשרת Heisenberg עם VQE](/tutorials/spin-chain-vqe). או ראה ב[מדריכים](https://docs.quantum.ibm.com/tutorials) ו[קורסים](/learning) כדי למצוא חומרי לימוד נוספים על הטכנולוגיה העדכנית ביותר בחישוב קוונטי.

אל תשכח לגשת לבחינה של הקורס הזה. ציון של 80% ומעלה יזכה אותך בתג Credly, שישלח אליך אוטומטית במייל. תודה שאתה חלק מרשת IBM Quantum&reg;!
