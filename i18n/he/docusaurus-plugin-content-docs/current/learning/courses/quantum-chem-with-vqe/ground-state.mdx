...
# מאחדים את הכול עם IBM Quantum Compute Service {#bringing-it-all-together-with-ibm-quantum-compute-service}
...
אנחנו בוחרים Circuit מסוג efficient_su2 ואת האופטימייזר COBYLA כנקודת התחלה.
...
עכשיו אנחנו בונים את פונקציית העלות שלנו. היא כמובן קשורה ל-Hamiltonian, אך שונה ממנו בכך שה-Hamiltonian הוא אופרטור, ואנחנו רוצים פונקציה שמחזירה את ערך הציפייה של אותו אופרטור, תוך שימוש ב-Estimator. כמובן, היא משיגה זאת באמצעות ה-ansatz והפרמטרים הווריאציוניים, כך שכל אלה מופיעים כארגומנטים. להלן, אנחנו מגדירים גרסאות מעט שונות לשימוש על חומרה אמיתית או סימולטורים.
...
אנחנו רוצים שהקוד שלנו ירוץ ביעילות האפשרית על החומרה בה אנחנו משתמשים. לכן עלינו לבחור Backend כדי להתחיל את שלב האופטימיזציה. הקוד שלהלן בוחר את ה-Backend הפחות עמוס הזמין עבורך.
...
* **[Dynamical Decoupling](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.passes.PadDynamicalDecoupling)**: אנחנו יכולים להחיל רצף של Gates על qubit בלתי פעילים. זה מבטל חלק מהאינטראקציות הלא רצויות עם הסביבה.
  ראה בתיעוד המקושר לקבלת מידע נוסף על אופטימיזציה של Circuits. הקוד שלהלן מייצר מנהל pass manager תוך שימוש ב-preset pass managers מ-`qiskit.transpiler`.
...
זכור שעבור כל בחירת פרמטרים ב-Circuit הווריאציוני, יש לחשב ערך ציפייה (שכן זה הערך שיש למזער). כפי שאולי כבר ניחשת, הדרך היעילה ביותר לעשות זאת היא שימוש ב-Primitive של Qiskit SDK, Estimator. נתחיל עם סימולטור מקומי, אשר ידרוש מאיתנו להשתמש בגרסה המקומית של Estimator הנקראת BackendEstimator.
...
הקוד הורץ כהלכה, אם כי לא התכנס — כפי שציפינו. נמשיך להריץ את החישוב על חומרה אמיתית, ואז נדון בפלטים. עבור Backends אמיתיים, נשתמש ב-IBM Quantum Estimator. נרצה להריץ זאת בתוך Session ובדרך כלל נרצה לציין אפשרויות עבור אותה Session.
...
* Sessions: VQE הוא מטבעו איטרטיבי, כאשר האופטימייזר הקלאסי בוחר פרמטרים ווריאציוניים חדשים, ובכך Gates חדשים נמצאים בשימוש, בכל ניסיון עוקב. ללא שימוש ב-sessions, הדבר עלול לגרום לזמן תור נוסף בין כל Circuit של ניסיון. עטיפת חישוב ה-VQE בתוך Session מביאה לתור ראשוני אחד בלבד לפני תחילת העבודה, ללא זמן תור נוסף בין שלבים ווריאציוניים. אסטרטגיה זו כבר שימשה בדוגמת השיעור הקודם, אך עשויה למלא תפקיד חשוב אף יותר בעת שינוי גיאומטריה. למידע נוסף על sessions, ראה ב[תיעוד מצבי הביצוע](/guides/execution-modes).

* האופטימיזציה המובנית של Estimator: ב-Estimator ישנן אפשרויות מובנות לאופטימיזציה של חישוב. בהקשרים רבים (כולל Estimator), ההגדרות מוגבלות ל-0 ו-1, כאשר 0 מציין אין אופטימיזציה, ו-1 (ברירת המחדל) מציין אופטימיזציה מסוימת של המעגל שלכם לחומרה שנבחרה. הקשרים אחרים מאפשרים הגדרות של 0, 1, 2, או 3. למידע נוסף על השיטות הספציפיות בהן נעשה שימוש בהגדרות שונות, ראו את [התיעוד](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-estimator-options). כאן, למעשה נגדיר את האופטימיזציה ל-0, ונשתמש ב-'skip\_transpilation = true', מכיוון שכבר ביצענו transpile למעגל שלנו באמצעות ה-pass manager שלמעלה, בחלק האופטימיזציה.
...
עבור הרצת החומרה הראשונה הזו, הגדרנו חוסן ל-0 כדי לאפשר הרצה מהירה יחסית. לכל יישום רציני, תרצה להשתמש במיתון שגיאות. שים לב שבתא שלהלן יש שני ערכות אפשרויות: (1) אפשרויות ל-Session, שכינינו "session\_options", ו-(2) אפשרויות לאופטימייזר הקלאסי, שנקראות כאן פשוט "options".
