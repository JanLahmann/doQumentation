...
## VQE - אלגוריתם קוונטי וריאציוני לבעיות ערכים עצמיים {#vqe---variational-quantum-algorithm-for-eigenvalue-problems}

### טכניקות קירוב בכימיה - עיקרון הוריאציה וסט הבסיס {#approximation-techniques-for-chemistry---variational-principle-and-the-basis-set}
...
1. עיקרון הוריאציה
    ...
    - אם אנחנו מנחשים פונקציית גל $\Psi_\text{trial}$ ("פונקציית ניסיון"), האנרגיה המחושבת ממנה תהיה תמיד שווה או גבוהה מאנרגיית מצב היסוד ($E_0$) של המערכת.
...
לסיכום, כדי להפוך חישובים לאפשריים ולהפחית את העלות החישובית, אנחנו משתמשים בעיקרון הוריאציה על ידי קירוב פונקציית הגל, מה שמפחית את המורכבות החישובית ומאפשר אופטימיזציה איטרטיבית למזעור אנרגיה. בינתיים, גישת סט הבסיס מפשטת חישובים על ידי ייצוג אורביטלים אטומיים כשילוב של פונקציות מוגדרות מראש, במקום לפתור פונקציית גל רציפה ישירות.
...
(a) נרמל את פונקציית הגל לניסיון על ידי קביעת $A$ כך ש- $$ \int_{-\infty}^{\infty} |\Psi_\text{trial}|^2 dx = 1 $$.
...
(b) חשב את ערך הציפייה של ההמילטוניאן $\hat{H}$ הנתון על ידי $$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V(x)$$ כאשר $V(x)  = \frac{1}{2}m\omega^2x^2$, המתאים לפוטנציאל אוסילטור הרמוני פשוט.
...
אופטימיזציה של $ \alpha $ לאנרגיה מינימלית
...
ב-VQE, ההמילטוניאן המולקולרי/אטומי הוא אובייקט הניתן למדידה, כלומר ניתן למדוד את ערכו דרך ניסוי. המטרה שלנו היא למצוא את האנרגיה הנמוכה ביותר האפשרית (אנרגיית מצב היסוד) של המולקולה. לשם כך, אנחנו משתמשים במצב קוונטי לניסיון, שנוצר על ידי Circuit קוונטי מפרמטרים (ansatz). אנחנו מודדים את האובייקט הניתן למדידה ומבצעים אופטימיזציה על המצב הקוונטי עד שנגיע לאנרגיה הנמוכה ביותר האפשרית.
...
## חישוב אנרגיית מצב היסוד של אטום מימן עם VQE {#compute-the-ground-state-energy-of-a-hydrogen-atom-with-vqe}
...
לאחר מכן, אנחנו צריכים Circuit קוונטי עם פרמטרים, ansatz, להכנת פונקציית גל ניסיונית $\Psi_\text{trial}$ עבור מצב היסוד. המטרה היא למצוא את הפרמטרים $\theta$ שממזערים את ערך הציפיה של האנרגיה $\langle\psi(\theta)|\hat{H}|\psi(\theta)\rangle$. בחירת ה-ansatz היא קריטית מכיוון שהיא קובעת את קבוצת המצבים הקוונטיים האפשריים ש-Circuit שלנו יכול להכין. "ansatz טוב" הוא כזה שגמיש מספיק לייצוג מצב קרוב מאוד למצב היסוד האמיתי של ההמילטוניאן שאנחנו חוקרים, אך לא כל כך מורכב שהוא דורש יותר מדי פרמטרים או Circuit עמוק מדי עבור מחשבים קוונטיים נוכחיים.
...
הערה: אפשר לקבל תוצאה טובה יותר על ידי הגדרת רמת מיתון שגיאות באמצעות [`resilience_level`](/guides/error-mitigation-and-suppression-techniques). ערך ברירת המחדל הוא 1, ואם מגדירים ערך גבוה יותר, ייעשה שימוש בזמן QPU רב יותר אך ייתכן שיוחזרו תוצאות טובות יותר.
...
## חישוב אנרגיית מצב היסוד של מולקולת המימן עם VQE {#compute-the-ground-state-energy-of-the-hydrogen-molecule-with-vqe}

עכשיו שהסתכלנו על התהליך הכולל של VQE באמצעות אטומי $H$, נחשב את אנרגיית מצב היסוד של מולקולת $H_2$ בצורה מהירה יותר.
### שלב 1: מיפוי הבעיה ל-Circuits ואופרטורים {#step-1-map-the-problem-to-quantum-circuits-and-operators}

כאן אנחנו גם מספקים לכם המילטוניאן חד-Qubit שמשתמש בבסיס STO-6G ובהמרת Jordan-Wigner, עם הפחתת qubit באמצעות סימטריה של ההמילטוניאן. שימו לב שהשתמשנו במרחק אטומי בין שני אטומי מימן של `0.735` $\mathring A$.

שלא כמו בחישוב של אטום מימן יחיד ($H$), כדי לחשב את מצב היסוד של מולקולת מימן ($H_2$), אנחנו חייבים לשקול גם את הכוח הדוחה הפועל בין הגרעינים של שני אטומי המימן, בנוסף לאנרגיה הקשורה לאורביטלים האלקטרוניים. בשלב זה, נתן ערך זה כקבוע, ובפועל נחשב ערך זה בשאלת הביניים.
...
השתמש ב[חוק קולון](https://en.wikipedia.org/wiki/Coulomb%27s_law) ובמערכת [יחידות אטומיות](https://en.wikipedia.org/wiki/Atomic_units) כדי לוודא שאתה מקבל את הערך ב-`Hartree`.
...
## חישוב אנרגיית התגובה של $H + H = H_2$ {#compute-reaction-energy-of-h--h--h_2}

עכשיו בואו נשתמש במה שהשגנו! השתמשת ב-VQE, פותר ערכים עצמיים קוונטי ורציאציוני, כדי לחשב את אנרגיית מצב היסוד של אטום $H$ ושל מולקולת $H_2$. מה שנותר הוא להשתמש בערכים שחישבנו כדי לקבל את אנרגיית התגובה של התהליך $H+H=H_2$.
...
ניתן לחשב את אנרגיית התגובה של התהליך $H+H = H_2$ לפי הנוסחה הבאה:
...
ראוי לציין כאן שאנרגיית התגובה של תהליך זה היא שלילית, מה שאומר שאנרגיה משתחררת במהלך התהליך, והמולקולה שנוצרת בעלת אנרגיה נמוכה יותר משני אטומים בודדים.
6. סיכום
...
1. עקרון הווריאציה קובע שערך הציפייה של האנרגיה עבור כל פונקציית גל ניסיונית הוא תמיד גדול מאנרגיית מצב היסוד האמיתי או שווה לה.
...
6. ה-`Estimator` של Qiskit משמש לחישוב ישיר של ערכי ציפייה של המילטוניאנים ב-VQE.
