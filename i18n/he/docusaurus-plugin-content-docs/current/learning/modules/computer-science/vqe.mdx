...
## VQE - אלגוריתם קוונטי וריאציוני לבעיות ערכים עצמיים {#vqe-a-variational-quantum-algorithm-for-eigenvalue-problems}

### טכניקות קירוב בכימיה - עקרון הווריאציה וסט הבסיס {#approximation-techniques-for-chemistry-variational-principle-and-the-basis-set}
...
(c) השתמש בעקרון הווריאציה כדי למצוא את $\alpha$ האופטימלי על ידי מזעור $E_\text{approx}(\alpha)$.
...
#### (קוונטי) גודל ניתן למדידה: ההמילטוניאן המולקולרי (אנרגיית מולקולה) {#quantum-observable-the-molecular-hamiltonian-energy-of-a-molecule}
...
בוא נתחיל בטעינת כמה חבילות נחוצות, כולל פרימיטיבים של IBM Quantum. כמו כן נבחר את המחשב הקוונטי הפחות עמוס שזמין לנו.
...
### שלב 1: מיפוי הבעיה ל-Circuits ואופרטורים {#step-1-map-the-problem-to-quantum-circuits-and-operators-1}
...
### שלב 2: אופטימיזציה עבור חומרת היעד {#step-2-optimize-for-target-hardware-1}
...
### שלב 3: הרצה על חומרה אמיתית {#step-3-execute-on-target-hardware-1}

עכשיו הגיע הזמן לבצע את החישובים על ה-QPU בפועל. כמעט הכל זהה, אבל נשתמש בנקודת ההתחלה המתאימה כדי להתאים את ההמילטוניאן. כמו כן, בחלק איטרטיבי, חלק מההגדרות של ה-`Estimator`, המשמש לחישוב הציפיות של ההמילטוניאן עבור ה-ansatz ב-QPU, ייקבעו מעט אחרת מהחישובים הקודמים. נדון בשינוי הזה בהמשך בשאלת בדיקה.
...
### שלב 4: עיבוד לאחר מעשה {#step-4-post-process-1}
...
## חישוב אנרגיית התגובה של $H + H = H_2$ {#compute-reaction-energy-of-h-h-h-2}

עכשיו בואו נשתמש במה שהשגנו! השתמשת ב-VQE, פותר ערכים עצמיים קוונטי וריאציוני, כדי לחשב את אנרגיית מצב היסוד של אטום $H$ ושל מולקולת $H_2$. מה שנותר הוא להשתמש בערכים שחישבנו כדי לקבל את אנרגיית התגובה של התהליך $H+H=H_2$.
...
3. VQE הוא אלגוריתם קוונטי המשמש לפתרון מדויק של משוואת שרדינגר עבור המילטוניאן נתון.
...
1. מה המטרה של המילטוניאן ב-VQE?
