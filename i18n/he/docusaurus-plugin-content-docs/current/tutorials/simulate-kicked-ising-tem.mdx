...
שיטת Tensor-network Error Mitigation (TEM) של Algorithmiq היא אלגוריתם היברידי קוונטי-קלאסי שנועד לביצוע הפחתת רעש לחלוטין בשלב העיבוד הקלאסי שלאחר המדידה. עם TEM, המשתמש יכול לחשב ערכי ציפייה של אובייקטים, תוך הפחתת שגיאות הרעש הבלתי נמנעות המתרחשות בחומרה קוונטית בדיוק ויעילות עלות גבוהים יותר, מה שהופך אותו לאפשרות אטרקטיבית ביותר עבור חוקרי קוונטים ואנשי תעשייה כאחד.
...
לאחר מכן, בנה את האובייקט, $\hat{X}_{n=t}$. הוא בנוי כמחרוזת Pauli פשוטה עם הסדר התואם את זה שנעשה בו שימוש על ידי Qiskit:
...
בדוגמת 12 הקיוביטים הקטנה שלנו, האובייקט נראה כך:
...
פונקציות Qiskit משתמשות ב-PUBs כדרך לאסוף את הקלטים. במקרה שלנו, בואו נשקול מעגל ואובייקט בודדים כ-PUB שלנו:
...
כאשר הסטטוס הוא `DONE`, ניתן לבדוק את התוצאות הגולמיות והמוקלות. ה-`tem_evs` המוגדרים להלן הם ערכי הציפייה של האובייקטים המבוקשים, במקרה זה רק אובייקט אחד, $\langle \hat X_{n=t}\rangle$, ו-`tem_std` הם סטיות התקן המתאימות.
