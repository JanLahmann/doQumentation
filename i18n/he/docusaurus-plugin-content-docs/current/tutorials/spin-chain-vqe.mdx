...
מדריך זה מראה כיצד לבנות, לפרוס ולהריץ `תבנית Qiskit` לסימולציה של שרשרת היזנברג ואומדן אנרגיית מצב היסוד שלה. למידע נוסף על `תבניות Qiskit` וכיצד ניתן להשתמש ב-`Qiskit Serverless` כדי לפרוס אותן לענן להרצה מנוהלת, כנסו ל[דף התיעוד שלנו על IBM Quantum&reg; Platform](/guides/serverless).
## דרישות {#requirements}
...
השלימו סקר קצר זה כדי לספק משוב על מדריך זה. התובנות שלכם יעזרו לנו לשפר את הצעות התוכן ואת חוויית המשתמש שלנו.
