...
- כיצד להריץ תהליכי עבודה וריאציוניים על חומרת קוונטום של IBM&reg; באמצעות IBM Quantum primitives וסשנים
...
מזעור אנרגיית מצב-היסוד המוערכת של המערכת על ידי אופטימיזציה של פרמטרי המעגל. השתמשו ב-IBM Quantum primitive `Estimator` כדי להעריך את פונקציית העלות במהלך האופטימיזציה.

מאחר שאופטמנו את המעגל עבור ה-backend בשלב 2, ניתן להימנע מטרנספילציה בשרת Quantum Compute על ידי הגדרת `skip_transpilation=True` והעברת המעגל המאופטם. להדגמה זו, נריץ על QPU באמצעות IBM Quantum primitives. כדי להריץ עם primitives מבוססי statevector של `qiskit`, החליפו את בלוק הקוד המשתמש ב-IBM Quantum primitives בבלוק המוערת.
