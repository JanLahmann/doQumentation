---
title: "אופטימיזציה דינמית של תיק השקעות עם Portfolio Optimizer של Global Data Quantum"
sidebar_label: "אופטימיזציה דינמית של תיק השקעות עם Portfolio Optimizer של Global Data Quantum"
...
מדריך זה מדגים כיצד לבצע אופטימיזציה דינמית של תיק השקעות באמצעות פונקציית Qiskit Quantum Portfolio Optimizer. במיוחד, אנחנו ממחישים כיצד להשתמש בפונקציית יישום זו כדי לפתור בעיית הקצאת השקעות על פני צעדי זמן מרובים.

הגישה כוללת ניסוח אופטימיזציית התיק כבעיית Quadratic Unconstrained Binary Optimization (QUBO) רב-יעדית. במיוחד, אנחנו מנסחים את פונקציית QUBO $O$ לאופטימיזציה סימולטנית של אַרבעה יעדים שונים:
...
לסיכום, כדי להתמודד עם יעדים אלה אנחנו מנסחים את פונקציית QUBO כ
...
אנחנו פותרים באמצעות שיטה קוונטית-קלאסית היברידית המבוססת על Variational Quantum Eigensolver (VQE). במערך זה, המעגל הקוונטי מעריך את פונקציית העלות, בעוד שהאופטימיזציה הקלאסית מבוצעת באמצעות אלגוריתם Differential Evolution, המאפשר ניווט יעיל בנוף הפתרון. מספר הקיוביטים הנדרש תלוי בשלושה גורמים עיקריים: מספר הנכסים ``na``, מספר תקופות הזמן ``nt`` ורזולוציית הביטים המשמשת לייצוג ההשקעה ``nq``. במיוחד, מספר הקיוביטים המינימלי בבעיה שלנו הוא `na*nt*nq`.

למדריך זה, אנחנו מתמקדים באופטימיזציה של תיק אזורי המבוסס על מדד IBEX 35 הספרדי. במיוחד, אנחנו משתמשים בתיק של שבעה נכסים כפי שמצוין בטבלה להלן:

| **תיק IBEX 35** | ACS.MC | ITX.MC | FER.MC | ELE.MC | SCYR.MC | AENA.MC | AMS.MC |
|-----------------------|--------|--------|--------|--------|---------|---------|--------|

אנחנו מאזנים מחדש את התיק שלנו בארבעה צעדי זמן, כל אחד מופרד במרווח של 30 יום החל מ-1 בנובמבר 2022. כל משתנה השקעה מקודד באמצעות שני ביטים. זה מביא לבעיה הדורשת 56 קיוביטים לפתרון.

אנחנו משתמשים ב-Optimized Real Amplitudes ansatz, התאמה מותאמת אישית ויעילה מבחינת חומרה של ה-Real Amplitudes ansatz הסטנדרטי, המותאמת במיוחד לשיפור הביצועים עבור סוג זה של בעיית אופטימיזציה פיננסית.

הביצוע הקוונטי מבוצע על ה-backend `ibm_torino`. להסבר מפורט של ניסוח הבעיה, המתודולוגיה והערכת הביצועים, ראה בכתב היד שפורסם [\[1\]](#references).
## דרישות {#requirements}
...
בשלב זה, אנחנו טוענים נתונים היסטוריים עבור שבעת הנכסים הנבחרים ממדד IBEX 35, במיוחד מ-**1 בנובמבר 2022** עד **1 באפריל 2023**.

אנחנו מביאים את הנתונים באמצעות Yahoo Finance API, תוך התמקדות במחירי סגירה. הנתונים עוברים עיבוד כדי להבטיח שלכל הנכסים יש את אותו מספר ימים עם נתונים. כל נתון חסר (ימים שאינם ימי מסחר) מטופל בהתאם, תוך הבטחה שכל הנכסים מיושרים על אותם תאריכים.
...
אנחנו ממליצים להריץ את התא הבא כדי לוודא שהנתונים הורדו מ-Yahoo Finance API בצורה תקינה. אם ההורדה נכשלת, תישלח הודעה שמציינת שהנתונים יאוחזרו במקום זאת ממסד נתונים מקוון חלופי.
</Admonition>
...
הפרמטרים הדרושים להגדרת בעיית QUBO מוגדרים במילון `qubo_settings`. אנחנו מגדירים את מספר צעדי הזמן (`nt`), את מספר הביטים למפרט ההשקעה (`nq`) ואת חלון הזמן לכל צעד זמן (`dt`). בנוסף, אנחנו מגדירים את ההשקעה המקסימלית לכל נכס, את מקדם סלידת הסיכון, את עמלת העסקה ואת מקדם המגבלות (ראה [המאמר שלנו](https://arxiv.org/pdf/2412.19150) לפרטים על ניסוח הבעיה). הגדרות אלה מאפשרות לנו להתאים את בעיית QUBO לתרחיש ההשקעה הספציפי.
...
המילון `ansatz_settings` מגדיר את ה-ansatz של המעגל הקוונטי. הפרמטר `ansatz` מציין את השימוש בגישת `"optimized_real_amplitudes"`, שהוא ansatz יעיל מבחינת חומרה המיועד לבעיות אופטימיזציה פיננסיות. האפשרות `multiple_passmanager`, המאפשרת שימוש במספר מנהלי מעבר (כולל מנהל המעבר המקומי הסטנדרטי של Qiskit ושירות ה-transpiler המונע בינה מלאכותית של Qiskit) לשיפור ביצועי ויעילות אופטימיזציית המעגל, מוגדרת כרגע ל-false מכיוון שתכונה זו מושבתת זמנית.
...
לבסוף, אנחנו מבצעים את האופטימיזציה על ידי הרצת הפונקציה `dpo_solver.run()`, תוך העברת הקלטים המוכנים. אלה כוללים את מילון נתוני הנכסים (`assets`), את תצורת QUBO (`qubo_settings`), פרמטרי אופטימיזציה (`optimizer_settings`) ואת הגדרות ה-ansatz של המעגל הקוונטי (`ansatz_settings`). בנוסף, אנחנו מציינים את פרטי הביצוע כגון ה-backend, והאם להחיל עיבוד שלאחר על התוצאות. זה מתחיל את תהליך אופטימיזציית תיק ההשקעות הדינמית על ה-backend הקוונטי הנבחר.
...
בקטע זה, אנחנו מחלצים ומציגים את הפתרון עם עלות היעד הנמוכה ביותר מתוצאות האופטימיזציה. יחד עם עלות היעד המינימלית, אנחנו מציגים גם מדדים מרכזיים הקשורים לפתרון הקשור, כולל סטיית המגבלות, יחס שארפ ותשואת ההשקעה.
...
הקדש דקה למתן משוב על מדריך זה. התובנות שלך יעזרו לנו לשפר את הצעות התוכן ואת חוויית המשתמש שלנו.
