...
בסימולציות, אנחנו יכולים לחשב זאת ישירות מתוצאות המדידה. הפונקציה `z_expectation` מעבדת את ספירת מחרוזות הביטים ומחזירה את הערך של $\langle Z_i \rangle$ עבור אינדקס קיוביט נבחר. על חומרה אמיתית, אנחנו מעריכים את אותו כמות על ידי ציון אופרטור Pauli באמצעות הפונקציה `generate_z_observables`, ולאחר מכן ה-backend מחשב את ערך התוחלת.
