...
גישה נפוצה לסימולציה של מודל זה היא שימוש בפירוק טרוטר כדי לקרב את אופרטור האבולוציה בזמן, תוך בניית מעגלים המחליפים בין סיבובים של קיוביט בודד ואינטראקציות שזירה של שני קיוביטים. עם זאת, סימולציה זו על חומרה אמיתית היא מאתגרת בגלל רעש ודה-קוהרנטיות, מה שמוביל לסטיות מהדינמיקה האמיתית. כדי להתגבר על זה, אנחנו משתמשים בכלי דיכוי שגיאות וניהול ביצועים Fire Opal של Q-CTRL, המוצעים כפונקציית Qiskit (ראה [תיעוד Fire Opal](/guides/q-ctrl-performance-management)). Fire Opal מבצע אופטימיזציה אוטומטית של ביצוע המעגל על ידי יישום ניתוק דינמי, פריסה מתקדמת, ניתוב וטכניקות דיכוי שגיאות נוספות, כולן מכוונות להפחתת רעש. עם שיפורים אלה, תוצאות החומרה מתיישרות בצורה הדוקה יותר עם סימולציות ללא רעש, ובכך אנחנו יכולים לחקור את דינמיקת המגנטיזציה של TFIM בנאמנות גבוהה יותר.
...
## הגדרה {#setup}
ראשית, בצע אימות באמצעות [מפתח API של IBM Quantum](http://quantum.cloud.ibm.com/). לאחר מכן, בחר את פונקציית Qiskit כדלקמן. (קוד זה מניח שכבר [שמרת את החשבון שלך](/guides/functions-get-started#install-qiskit-functions-catalog-client) לסביבה המקומית שלך.)
...
אנחנו מתחילים בהגדרת סריג הספינים והצימודים ביניהם. במדריך זה, הסריג בנוי ממשולשים מחוברים המסודרים בשרשרת ליניארית. כל משולש מורכב משלושה צמתים מחוברים בלולאה סגורה, והשרשרת נוצרת על ידי קישור צומת אחד של כל משולש למשולש הקודם.
...
כדי להציג ויזואלית את הסריג שהגדרנו זה עתה, אנחנו יכולים לשרטט את שרשרת המשולשים המחוברים ולתייג כל צומת. הפונקציה שלהלן בונה את הגרף עבור מספר נבחר של משולשים ומציגה אותו.
...
כדי ליישם את צימוד הספין-ספין, שימושי לקבץ קצוות שאינם חופפים. זה מאפשר לנו ליישם שערים של שני קיוביטים במקביל. אנחנו יכולים לעשות זאת באמצעות נוהל פשוט של צביעת קצוות [\[1\]](#references), המקצה צבע לכל קצה כך שקצוות הנפגשים באותו צומת ממוקמים בקבוצות שונות.
...
כדי לדמות את הדינמיקה של ה-TFIM, אנחנו בונים מעגלים המקרבים את אופרטור האבולוציה בזמן.

$$
U(t) = e^{-i H t}, \quad \text{where} \quad H = -J \sum_{\langle i,j \rangle} Z_i Z_j - h \sum_i X_i .
$$

אנחנו משתמשים בפירוק טרוטר מסדר שני:
...
הזוויות של שערים אלה נקבעות על ידי השדה הרוחבי $h$, קבוע הצימוד $J$, וצעד הזמן $\Delta t$. על ידי ערימת מספר צעדי טרוטר, אנחנו מייצרים מעגלים של עומק הולך וגדל המקרבים את הדינמיקה של המערכת. הפונקציות `generate_tfim_circ_custom_graph` ו-`trotter_circuits` בונות מעגל קוונטי מטרוטר מגרף אינטראקציית ספין שרירותי.
...
### הערכת מגנטיזציות קיוביט בודד $\langle Z_i \rangle$ {#estimate-single-qubit-magnetizations-langle-z-i-rangle}
כדי לחקור את הדינמיקה של המודל, אנחנו רוצים למדוד את המגנטיזציה של כל קיוביט, המוגדרת על ידי ערך התוחלת $\langle Z_i \rangle = \langle \psi | Z_i | \psi \rangle$.

בסימולציות, אנחנו יכולים לחשב זאת ישירות מתוצאות המדידה. הפונקציה `z_expectation` מעבדת את ספירת מחרוזות הביטים ומחזירה את הערך של $\langle Z_i \rangle$ עבור אינדקס קיוביט נבחר. על חומרה אמיתית, אנחנו מעריכים את אותה כמות על ידי ציון אופרטור Pauli באמצעות הפונקציה `generate_z_observables`, ולאחר מכן ה-backend מחשב את ערך התוחלת.
...
כעת אנחנו מגדירים את הפרמטרים ליצירת המעגלים המטרוטרים. במדריך זה, הסריג הוא שרשרת של 20 משולשים מחוברים, המתאימה למערכת של 41 קיוביטים.
...
רשימת המעגלים המטרוטרים מבוצעת באמצעות סימולטור `matrix_product_state` עם בחירה שרירותית של $4096$ shots. שיטת MPS מספקת קירוב יעיל של דינמיקת המעגל, כאשר הדיוק נקבע על ידי ממד הקשר הנבחר. עבור גדלי המערכת הנבחנים כאן, ממד הקשר ברירת המחדל מספיק כדי לתפוס את דינמיקת המגנטיזציה בנאמנות גבוהה. הספירות הגולמיות מנורמלות, ומהן אנחנו מחשבים את ערכי התוחלת של קיוביט בודד $\langle Z_i \rangle$ בכל צעד טרוטר. לבסוף, אנחנו מחשבים את הממוצע על פני כל הקיוביטים כדי לקבל עקומה אחת המראה כיצד המגנטיזציה משתנה לאורך זמן.
...
אנחנו מעריכים את דינמיקת המגנטיזציה על חומרה קוונטית אמיתית. Fire Opal מספק פונקציית Qiskit המרחיבה את ה-primitive של IBM Quantum Estimator הסטנדרטי עם דיכוי שגיאות אוטומטי וניהול ביצועים. אנחנו שולחים את המעגלים המטרוטרים ישירות ל-backend של IBM&reg; בעוד Fire Opal מטפל בביצוע המודע לרעש.

אנחנו מכינים רשימה של `pubs`, כאשר כל פריט מכיל מעגל ואת משתני Pauli-Z המתאימים. אלה מועברים לפונקציית ה-estimator של Fire Opal, המחזירה את ערכי התוחלת $\langle Z_i \rangle$ עבור כל קיוביט בכל צעד טרוטר. לאחר מכן ניתן לחשב את הממוצע של התוצאות על פני הקיוביטים כדי לקבל את עקומת המגנטיזציה מהחומרה.
...
לבסוף, אנחנו משווים את עקומת המגנטיזציה מהסימולטור עם התוצאות שהתקבלו על חומרה אמיתית. שרטוט שני העקומות זו לצד זו מראה כמה קרוב ביצוע החומרה עם Fire Opal מתאים לבסיס ללא רעש על פני צעדי טרוטר.
...
הקדש דקה למתן משוב על מדריך זה. התובנות שלך יעזרו לנו לשפר את הצעות התוכן שלנו וחווית המשתמש.
