...
מדריך זה מראה כיצד לבנות `תבנית Qiskit` להערכת ערכים במטריצת ליבת קוונטים המשמשת לסיווג בינארי. למידע נוסף על `תבניות Qiskit` וכיצד ניתן להשתמש ב-`Qiskit Serverless` לפרוס אותן לענן לביצוע מנוהל, כנסו ל[דף התיעוד שלנו על IBM Quantum&reg; Platform](/guides/serverless).
## דרישות {#requirements}
...
צרו את המעגל הקוונטי המשמש להערכת ערך אחד במטריצת הליבה. אנחנו משתמשים בנתוני הקלט כדי לקבוע את זוויות הסיבוב של השערים הפרמטריים של המעגל. נשתמש בדגימות הנתונים `x1=14` ו-`x2=19`.
...
השתמשו בפונקציה `generate_preset_pass_manager` מ-Qiskit כדי לציין שגרת אופטימיזציה עבור המעגל שלנו ביחס ל-QPU שעליו אנחנו מתכננים להריץ את הניסוי. אנחנו מגדירים `optimization_level=3`, מה שאומר שנשתמש במנהל המעברים המוגדר מראש המספק את רמת האופטימיזציה הגבוהה ביותר.
...
השתמשו בפרימיטיב `Sampler` מ-Qiskit Runtime כדי לשחזר התפלגות קוואזי-הסתברות של מצבים שהתקבלו מדגימת המעגל. למשימה של יצירת מטריצת ליבה, אנחנו מעוניינים במיוחד בהסתברות למדידת המצב |0>.

עבור הדגמה זו, נריץ על QPU עם פרימיטיבים של `qiskit-ibm-runtime`. להרצה על פרימיטיבים מבוססי-statevector של `qiskit`, החליפו את הבלוק של קוד המשתמש בפרימיטיבים של Qiskit IBM&reg; Runtime עם הבלוק המוסתר בהערות.
...
מלאו סקר קצר זה כדי לספק משוב על מדריך זה. התובנות שלכם יעזרו לנו לשפר את הצעות התוכן והחוויה המשתמש שלנו.
