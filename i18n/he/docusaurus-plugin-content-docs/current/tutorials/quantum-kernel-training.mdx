...
בחלק זה, נעבור על ארבעת השלבים של תבנית Qiskit על מופע בן שבעה קיוביטים של בעיית "labeling-cosets-with-error" ונעריך ערך יחידי של מטריצת הליבה באמצעות הפרימיטיב `StatevectorSampler` מ-Qiskit. סימולטור statevector הוא מדויק (עד לרעש ירייה) ומאפשר לנו לראות את השיטה מקצה לקצה מבלי לצרוך זמן QPU. לאחר מכן נחזור על אותו המופע על חומרה אמיתית בחלק דוגמת החומרה.
...
למטריצת ליבה קוונטית יש $\mathcal{O}(N^2)$ ערכים עבור $N$ דגימות אימון, וכל ערך דורש הרצת מעגל חפיפה שעומק שעריו הדו-קיוביטיים גדל עם גודל מפת המאפיינים. כתוצאה מכך, הרחבת מדריך זה לבעיה גדולה יותר כרוכה בשני עלויות מצטברות: זמן ה-QPU לכל מטריצת ליבה גדל ריבועית עם $N$, ועומקו של `unitary_overlap` (המרכיב את מפת המאפיינים עם הצמוד שלה) פוגם בנאמנות בגודל המערכת ובקישוריות החומרה הנוכחית. כדי לשמור על ההדגמה קצרה ולאפשר השוואה נקייה, אנחנו מריצים את אותו מופע בן שבעה קיוביטים מהדוגמה בקנה מידה קטן על QPU אמיתי ומשווים את הנאמנות של ערך יחיד במטריצת הליבה לערך הסימולטור שחושב לעיל.
