...
בחר מצב ייחוס $\vert \psi \rangle$ שיש לו חפיפה מסוימת עם מצב היסוד. עבור המילטוניאן זה, אנחנו משתמשים במצב עם עירור ב-qubit האמצעי $\vert 00..010...00 \rangle$ כמצב הייחוס שלנו.
...
במקום ליישם את אופרטור אבולוציית הזמן בדיוק אנחנו יכולים להשתמש בפירוק טרוטר כדי ליישם קירוב שלו. חזרה מספר פעמים על פירוק טרוטר של סדר מסוים נותנת לנו הפחתה נוספת של השגיאה שהוכנסה מהקירוב. בהמשך, אנחנו בונים ישירות את יישום טרוטר בצורה היעילה ביותר עבור גרף האינטראקציה של ההמילטוניאן שאנו שוקלים (אינטראקציות שכנים קרובים בלבד). בפועל אנחנו מכניסים סיבובי פאולי $R_{xx}$, $R_{yy}$, $R_{zz}$ עם זווית מפרמטרית $t$ התואמים ליישום המקורב של $e^{-i (XX + YY + ZZ) t}$. בהינתן ההבדל בהגדרה של סיבובי פאולי ואבולוציית הזמן שאנו מנסים ליישם, נצטרך להשתמש בפרמטר $2*dt$ כדי להשיג אבולוציית זמן של $dt$. יתר על כן, אנחנו הופכים את סדר הפעולות עבור מספר אי-זוגי של חזרות של שלבי טרוטר, שהוא שווה ערך פונקציונלי אך מאפשר סינתזה של פעולות סמוכות ביוניטרי $SU(2)$ יחיד. זה נותן מעגל הרבה יותר רדוד ממה שמתקבל באמצעות הפונקציונליות הגנרית `PauliEvolutionGate()`.
...
ההבדל היחיד בין המעגלים המשמשים במבחן הדאמאר יהיה הפאזה באופרטור אבולוציית הזמן והתצפיות הנמדדות. לכן אנחנו יכולים להכין תבנית מעגל המייצגת את המעגל הגנרי עבור מבחן הדאמאר, עם מצייני מיקום עבור השערים שתלויים באופרטור אבולוציית הזמן.
...
ברגע שיש לנו את התוצאות של ביצועי המעגל אנחנו יכולים לבצע עיבוד מאוחר של הנתונים כדי לחשב את האלמנטים המטריציאליים של $S$
...
לבסוף, אנחנו יכולים לפתור את בעיית הערך העצמי המוכללת עבור $\tilde{H}$:
...
עבור סקטור חלקיק יחיד, אנחנו יכולים לחשב ביעילות את מצב היסוד של סקטור זה של ההמילטוניאן באופן קלאסי
...
עבור צעד זמן $dt$ שנקבע מאוחר יותר. נניח זמנית ש-$r$ זוגי: אז נגדיר $d=r/2$. שים לב שכאשר אנחנו מקרינים את ההמילטוניאן למרחב קרילוב לעיל, הוא בלתי ניתן להבחנה ממרחב קרילוב
...
אינם משתנים תחת הסטות כלליות של זמן האבולוציה, מכיוון שאבולוציות הזמן מתחלפות עם ההמילטוניאן. עבור $r$ אי-זוגי, אנחנו יכולים להשתמש בניתוח עבור $r-1$.

אנחנו רוצים להראות שאיפשהו במרחב קרילוב הזה, יש ערובה שיהיה מצב אנרגיה נמוכה. אנחנו עושים זאת באמצעות התוצאה הבאה, שנגזרת ממשפט 3.1 ב-[\[3\]](#references):
...
אנחנו נותנים הוכחה להלן, אך ניתן לדלג עליה בבטחה אלא אם רוצים להבין את הטיעון המלא והקפדני. לעת עתה אנחנו מתמקדים בהשלכות של הטענה לעיל. לפי תכונה 3 לעיל, אנחנו יכולים לראות שמרחב קרילוב המוסט לעיל מכיל את המצב $f(H)|\psi\rangle$. זהו מצב האנרגיה הנמוכה שלנו. כדי לראות למה, כתוב את $|\psi\rangle$ בבסיס האנרגיה העצמית:
...
כדי להפוך את זה לגבול עליון שקל יותר להבין, אנחנו קודם כל מפרידים את הסכום במונה לאיברים עם $E_k-E_0\le\delta$ ואיברים עם $E_k-E_0>\delta$:
...
כאשר השלב הראשון נובע מכך ש-$E_k-E_0\le\delta$ עבור כל $E_k$ בסכום, והשלב השני נובע מכך שהסכום במונה הוא תת-קבוצה של הסכום במכנה. עבור האיבר השני, ראשית אנחנו מגבילים את המכנה מלמטה על ידי $|\gamma_0|^2$, מכיוון ש-$f(E_0)^2=1$: חיבור הכל בחזרה ביחד, זה נותן

$$
\text{energy error} \le \delta + \frac{1}{|\gamma_0|^2}\sum_{E_k>E_0+\delta}|\gamma_k|^2f(E_k)^2(E_k-E_0).
$$

כדי לפשט את מה שנשאר, שים לב שעבור כל ה-$E_k$ האלה, לפי ההגדרה של $f$ אנחנו יודעים ש-$f(E_k)^2 \le 4\left(1 + \delta\right)^{-2d}$. בנוסף הגבלה מלמעלה של $E_k-E_0<2\|H\|$ והגבלה מלמעלה של $\sum_{E_k>E_0+\delta}|\gamma_k|^2<1$ נותנת
...
כעת באמצעות הפולינום $p^*(x)$ עם הפרמטרים a, b, d מוגדרים ל-$a = g(E_0 + \delta)$, $b = 1$, ו-d = int(r/2), אנחנו מגדירים את הפונקציה:
...
כאשר $E_0$ היא אנרגיית מצב היסוד. אנחנו יכולים לראות על ידי הכנסת $\cos(x)=\frac{e^{ix}+e^{-ix}}{2}$ ש-$f(E)$ הוא פולינום טריגונומטרי מדרגה $d$, כלומר, צירוף לינארי של $e^{ijE\,dt}$ עבור $j=-d,-d+1,...,d-1,d$. יתר על כן, מההגדרה של $p^*(x)$ לעיל יש לנו ש-$f(E_0)=p(0)=1$ ועבור כל $E$ בטווח הספקטרלי כך ש-$\vert E-E_0 \vert > \delta$ יש לנו
...
מלא את הסקר הקצר הזה כדי לספק משוב על המדריך הזה. התובנות שלך יעזרו לנו לשפר את הצעות התוכן שלנו וחוויית המשתמש.
