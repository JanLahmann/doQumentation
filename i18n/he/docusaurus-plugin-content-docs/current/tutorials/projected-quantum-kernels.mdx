...
מדריך זה מניח גם היכרות כללית עם שיטות QML. לחקירה נוספת של QML, עיינו בקורס [Quantum machine learning](/learning/courses/quantum-machine-learning) ב-IBM Quantum Learning.
### דרישות {#requirements}
...
אנו גם ממירים את מערך הנתונים כך ש-$1$ ייוצג כ-$\pi/2$ לצורכי קנה-מידה.
...
אנו מאמתים את הגדלים והצורות של מערכי הנתונים לאימון ולבדיקה.
...
כעת נבנה את מפת המאפיינים שמטמיעה את מערך הנתונים הקלאסי שלנו במרחב מאפיינים רב-ממדי. לצורך הטמעה זו, אנו משתמשים ב-[``ZZFeatureMap``](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ZZFeatureMap) מ-Qiskit.
...
אנו מספקים גם את תוצאות הניסוי שהרצנו על חומרה קוונטית. ניתן להריץ את האימון בעצמכם על ידי הגדרת הדגל שלהלן ל-`True`, או להשתמש בתוצאות ההקרנה שאנו מספקים.
...
שימו לב שמעבר על כלל מערך הנתונים עשוי לארוך כ-80 דקות על ה-QPU. כדי להבטיח שיתר המדריך ניתן לביצוע בנוחות, אנו מספקים גם היטלים מניסוי שבוצע קודם לכן (הכלולים בקבצים שהורדתם בבלוק הקוד `Download dataset`). אם ביצעתם את האימון בעצמכם, תוכלו להמשיך את המדריך עם התוצאות שלכם.
