---
title: "שיפור סיווג מאפיינים באמצעות גרעינים קוונטיים מוקרנים"
sidebar_label: "שיפור סיווג מאפיינים באמצעות גרעינים קוונטיים מוקרנים"
description: "מדריך לשיפור סיווג מאפיינים באמצעות גרעינים קוונטיים מוקרנים"
...
לאחר סיום מדריך זה, המשתמשים יבינו:
- כיצד גרעינים קוונטיים מוקרנים (PQK) פועלים ומתי הם מציעים יתרון קוונטי פוטנציאלי.
- כיצד להריץ PQK על חומרה תוך שימוש במערך נתונים מהעולם האמיתי.

## דרישות מוקדמות {#prerequisites}

אנו ממליצים שהמשתמשים יהיו מכירים את הנושאים הבאים לפני ביצוע מדריך זה:
- [גרעינים קוונטיים](/learning/courses/quantum-machine-learning/quantum-kernel-methods) מתוך [קורס למידת מכונה קוונטית](/learning/courses/quantum-machine-learning) ב-IBM Quantum&reg; Learning

## רקע {#background}

במדריך זה נדגים כיצד להריץ [גרעין קוונטי מוקרן](https://www.nature.com/articles/s41467-021-22539-9) (PQK) עם Qiskit על מערך נתונים ביולוגי מהעולם האמיתי, בהתבסס על המאמר [Enhanced Prediction of CAR T-Cell Cytotoxicity with Quantum-Kernel Methods](https://arxiv.org/abs/2507.22710) [[1]](#references).

PQK היא שיטה בלמידת מכונה קוונטית (QML) לקידוד נתונים קלאסיים למרחב מאפיינים קוונטי והקרנתם בחזרה לתחום הקלאסי, תוך שימוש במחשבים קוונטיים לשיפור בחירת המאפיינים. השיטה כוללת קידוד נתונים קלאסיים למצבים קוונטיים באמצעות מעגל קוונטי, בדרך כלל דרך תהליך הנקרא מיפוי מאפיינים, שבו הנתונים מומרים למרחב הילברט רב-ממדי. ה"הקרנה" מתייחסת לחילוץ מידע קלאסי מהמצבים הקוונטיים — על ידי מדידת אובסרבבלים ספציפיים — לשם בניית מטריצת גרעין שניתן להשתמש בה באלגוריתמים קלאסיים מבוססי גרעין כמו מכונות וקטור תמיכה. גישה זו מנצלת את היתרונות החישוביים של מערכות קוונטיות כדי להשיג ביצועים טובים יותר — בפוטנציאל — על משימות מסוימות בהשוואה לשיטות קלאסיות.

אבני הבניין המרכזיות של PQKs הן מטריצות הצפיפות המצומצמות (RDMs), המתקבלות דרך מדידות הקרנה של מפת המאפיינים הקוונטית. בפרט, בדרך כלל מחשבים את מטריצות הצפיפות המצומצמות לחד-קיוביט (1 RDMs) לכל קיוביט. כמויות מדודות אלו משמשות לאחר מכן כקלטים לפונקציית גרעין קלאסית, כגון גרעין אקספוננציאלי, לבניית מטריצת הגרעין הסופית.

ל-PQKs יש יתרונות פוטנציאליים על פני [גרעינים קוונטיים](/tutorials/quantum-kernel-training) סטנדרטיים, בעיקר עבור חומרה קוונטית קרובת-טווח. גרעינים קוונטיים סטנדרטיים מסתמכים בדרך כלל על הערכת חפיפות מצב גלובליות, שהולכות וקשות יותר למדידה מדויקת ככל שמספר הקיוביטים גדל ורגישות מאוד לרעש. לעומת זאת, PQKs משתמשים באובסרבבלים מקומיים כמו מטריצות צפיפות מצומצמות לחד-קיוביט (1 RDMs), מה שמוביל לתקורת דגימה נמוכה יותר, שיפור בחוסן לרעש חומרה ומדרגיות טובה יותר. על ידי הקרנת מצבים קוונטיים על מאפייני מדידה מקומיים לפני החלת פונקציית גרעין קלאסית, PQKs יכולים לשמר מתאמים קוונטיים שימושיים תוך כדי הישארות מעשיים יותר למכשירים קרובי-טווח.
## דרישות {#requirements}
...
- Qiskit Runtime גרסה 0.40 ואילך (`pip install qiskit-ibm-runtime`)
...
אנו משמיטים את דוגמת הסימולטור בקנה מידה קטן במדריך זה, מכיוון שמטרתנו העיקרית היא להדגים כיצד גרעינים קוונטיים מוקרנים יכולים להתרחב למערכות גדולות יותר ולחומרה אמיתית.
...
אנחנו גם ממירים את מערך הנתונים כך ש-$1$ ייוצג כ-$\pi/2$ לצורכי קנה-מידה.
...
כעת נבנה את מפת המאפיינים שמטמיעה את מערך הנתונים הקלאסי שלנו במרחב מאפיינים רב-ממדי. לצורך הטמעה זו, אנחנו משתמשים ב-[``ZZFeatureMap``](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ZZFeatureMap) מ-Qiskit.
...
### מדידת 1-RDMs {#measure-1-rdms}
בשלב זה, נחשב את כל מטריצות הצפיפות המצומצמות לחד-קיוביט (1-RDMs) דרך מדידות הקרנה של מפת המאפיינים הקוונטית, אשר יוזנו לאחר מכן אל פונקציית הגרעין האקספוננציאלי הקלאסית.
נבחן כיצד לחשב את ה-1-RDM עבור נקודת נתונים בודדת מהמסד לפני שנריץ על כלל הנתונים. ה-1-RDMs הן אוסף של מדידות חד-קיוביט של אופרטורי פאולי ``X``, ``Y`` ו-``Z`` על כל הקיוביטים. הסיבה לכך היא שניתן לבטא RDM חד-קיוביט באופן מלא כך: $$\rho = \frac{1}{2} \big( I + \braket \sigma_x \sigma_x  + \braket \sigma_y \sigma_y + \braket \sigma_z \sigma_z  \big)$$
...
![פלט תא הקוד הקודם](/docs/images/tutorials/projected-quantum-kernels/extracted-outputs/4f573436-ec5c-451b-976c-ad718b3c201d-1.avif)
...
```
### שלב 4: עיבוד לאחר-מדידה והחזרת התוצאה בפורמט קלאסי רצוי {#step-4-post-process-and-return-result-in-desired-classical-format}
### הגדרת הגרעין הקוונטי המוקרן {#define-the-projected-quantum-kernel}
הגרעין הקוונטי המוקרן מוגדר באמצעות פונקציית הגרעין הבאה: $$k^{\textrm{PQ}}(x_i, x_j) = \textrm{exp} \Big(-\gamma \sum_k \sum_{P \in \{ X,Y,Z \}} (\textrm{Tr}[P \rho_k(x_i)] - \textrm{Tr}[P \rho_k(x_j)])^2 \Big) $$
במשוואה לעיל, $\gamma>0$ הוא היפר-פרמטר ניתן לכיוון. הכניסות $K^{\textrm{PQ}}_{ij} = k^{\textrm{PQ}}(x_i, x_j)$ הן איברי מטריצת הגרעין $K^{\textrm{PQ}}$.
בעזרת הגדרת ה-1-RDMs ניתן לראות שהאיברים הבודדים בתוך פונקציית הגרעין ניתנים לחישוב כ-$\textrm{Tr}[P \rho_k (x_i)] = \braket P$, כאשר $P \in \{ X,Y,Z \}$. ערכי התוחלת הללו הם בדיוק מה שמדדנו לעיל.
בשימוש ב-``scikit-learn``, ניתן לחשב את הגרעין בקלות רבה אף יותר. הדבר נובע מזמינותה של פונקציית הגרעין הרדיאלית (``'rbf'``) המוכנה לשימוש: $ \textrm{exp} (-\gamma \lVert x - x' \rVert^2)$. ראשית, עלינו לעצב מחדש את מערכי ההקרנה של נתוני האימון והבדיקה לפי מערכים דו-ממדיים.
שימו לב שמעבר על כלל מערך הנתונים עשוי לארוך כ-80 דקות על ה-QPU. כדי להבטיח שיתר המדריך ניתן לביצוע בנוחות, אנחנו מספקים גם הקרנות מניסוי שבוצע קודם לכן (הכלולים בקבצים שהורדתם בבלוק הקוד `Download dataset`). אם ביצעתם את האימון בעצמכם, תוכלו להמשיך את המדריך עם התוצאות שלכם.
...
### מכונת וקטורי תמיכה (SVM) {#support-vector-machine-svm}
...
### השוואה לרף קלאסי {#classical-benchmarking}
ניתן להריץ SVM קלאסי עם פונקציית הגרעין הרדיאלית, ללא ביצוע הקרנה קוונטית. תוצאה זו משמשת כרף הביצועים הקלאסי שלנו.
...
## נספח: אימות יתרון קוונטי פוטנציאלי של מערך הנתונים במשימות למידה {#appendix-verify-the-datasets-potential-quantum-advantage-in-learning-tasks}

לא כל מערכי הנתונים מציעים יתרון פוטנציאלי מהשימוש ב-PQKs. קיימים חסמים תיאורטיים שניתן להשתמש בהם כבדיקה מקדימה לבחון האם מערך נתונים מסוים עשוי להפיק תועלת מ-PQKs. לכימות זאת, מחברי [Power of data in quantum machine learning](https://www.nature.com/articles/s41467-021-22539-9) [2] מגדירים כמויות המכונות סיבוכיות מודל קלאסית וקוונטית, והפרדה גיאומטרית בין המודלים הקלאסי והקוונטי. כדי לצפות ליתרון קוונטי פוטנציאלי מ-PQKs, ההפרדה הגיאומטרית בין הגרעינים הקלאסי והקוונטי-המוקרן צריכה להיות בסדר גודל של $\sqrt{N}$ בערך, כאשר $N$ הוא מספר דגימות האימון. אם תנאי זה מתקיים, נבדוק את סיבוכיות המודל. אם סיבוכיות המודל הקלאסי היא בסדר גודל של $N$ בעוד שסיבוכיות המודל הקוונטי-המוקרן קטנה באופן משמעותי מ-$N$, ניתן לצפות ליתרון פוטנציאלי מה-PQK.
ההפרדה הגיאומטרית מוגדרת כדלקמן (F19 ב-[[2]](#references)):
...
סיבוכיות המודל מוגדרת כדלקמן (M1 ב-[[2]](#references)):
...
- קורס מעמיק על [למידת מכונה קוונטית](/learning/courses/quantum-machine-learning) מ-IBM Quantum Learning
- מדריך [אימון גרעין קוונטי](/tutorials/quantum-kernel-training)
</Admonition>
## מקורות {#references}
