...
### Circuit קוונטי {#quantum-circuit}
...
בשימוש ב-``scikit-learn``, ניתן לחשב את הגרעין בקלות רבה אף יותר. הדבר נובע מזמינותה של פונקציית הגרעין הרדיאלית (``'rbf'``) המוכנה לשימוש: $ \textrm{exp} (-\gamma \lVert x - x' \rVert^2)$. ראשית, עלינו לעצב מחדש את מערכי ההיטל של נתוני האימון והבדיקה לפי מערכים דו-ממדיים.
