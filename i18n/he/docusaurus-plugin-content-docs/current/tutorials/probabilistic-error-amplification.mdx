---
description: 'הפעלת ניסוי הפחתת שגיאות בקנה מידה של תועלת עם אקסטרפולציה לאפס רעש והגברת שגיאות הסתברותית.'
notebook_path: tutorials/probabilistic-error-amplification.ipynb
sidebar_label: 'הפחתת שגיאות בקנה מידה של תועלת עם הגברת שגיאות הסתברותית'
title: 'הפחתת שגיאות בקנה מידה של תועלת עם הגברת שגיאות הסתברותית'
---
...
*אומדן שימוש: 14 דקות על מעבד Heron r3 (הערה: זהו אומדן בלבד. זמן הריצה בפועל עשוי להשתנות.)*

## תוצאות למידה {#learning-outcomes}
לאחר השלמת מדריך זה, המשתמשים אמורים להבין:
- את התיאוריה מאחורי *אקסטרפולציה לאפס רעש* (ZNE), השיטות השונות להגברת רעש, ומדוע *הגברת שגיאות הסתברותית* (PEA) עדיפה לניסויים בקנה מידה של תועלת.
- כיצד לממש ZNE עם PEA בפועל באמצעות Qiskit.

## דרישות מוקדמות {#prerequisites}
אנו ממליצים למשתמשים להכיר את הנושאים הבאים לפני עבודה עם מדריך זה:
- את [שיעור הפחתת שגיאות](/learning/courses/utility-scale-quantum-computing/error-mitigation) של קורס *Utility-scale quantum computing* לידע בסיסי בשימוש בהפחתת שגיאות ב-Qiskit.
- את [שיעור Utility-I](/learning/courses/utility-scale-quantum-computing/utility-i) של קורס *Utility-scale quantum computing* לרקע נוסף על הניסוי בקנה מידה של תועלת המשמש כדוגמה במדריך זה.

## רקע {#background}

מדריך זה מדגים כיצד להפעיל ניסוי הפחתת שגיאות בקנה מידה של תועלת עם IBM Quantum Compute Service, תוך שימוש בגרסה ניסויית של *אקסטרפולציה לאפס רעש* (ZNE) עם *הגברת שגיאות הסתברותית* (PEA).
...
אקסטרפולציה לאפס רעש (ZNE) היא טכניקת הפחתת שגיאות המסירה את השפעות הרעש ה*בלתי ידוע* במהלך הרצת מעגל, כאשר ניתן לשנות את עוצמת הרעש בדרך *ידועה*.
...
1. הגברת רעש המעגל עבור מספר גורמי רעש $\lambda_1, \lambda_2, ... $

2. הרצת כל מעגל מוגבר-רעש למדידת $\langle A(\lambda_1)\rangle, ...$
...
| **מתיחת פולס**                                                                                                                                                                               | **קיפול שער**                                                                                                                                                                               | **הגברת שגיאות הסתברותית**                                                                                                                                                |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| הרחבת משך הפולס באמצעות כיול                                                                                                                                                                       | חזרה על שערים במחזורי זהות $U\mapsto U(U^{-1}U)^{\lambda-1}/2$                                                                                                                               | הוספת רעש על ידי דגימת ערוצי פאולי                                                                                                                                                  |
...
עבור ניסויים בקנה מידה של תועלת, *הגברת שגיאות הסתברותית* (PEA) היא הגישה האטרקטיבית ביותר.

* מתיחת פולס מניחה שרעש השער פרופורציונלי למשכו, דבר שאינו נכון בדרך כלל. בנוסף, הכיול יקר מבחינת משאבים.

* קיפול שער דורש גורמי מתיחה גדולים המגבילים מאוד את עומק המעגל שניתן להריץ.

* PEA ניתנת ליישום על כל מעגל שניתן להריץ עם גורם הרעש המקורי ($\lambda=1$), אך דורשת לימוד מודל הרעש.


### לימוד מודל הרעש עבור PEA {#learn-the-noise-model-for-pea}
PEA מניחה את אותו מודל רעש מבוסס-שכבות כמו *ביטול שגיאות הסתברותי* (PEC); עם זאת, היא נמנעת מעומס הדגימה המתרחב אקספוננציאלית עם רעש המעגל.

| **שלב 1**                                                                                                                                                                                       | **שלב 2**                                                                                                                                                                                    | **שלב 3**                                                                                                                                                                                      |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| סיבוב פאולי של שכבות שערים דו-קיוביטיות                                                                                                                                                         | חזרה על זוגות זהות של שכבות ולמידת הרעש                                                                                                                                                     | גזירת ערך נאמנות (שגיאה לכל ערוץ רעש)                                                                                                                                                         |
...
**מקור**: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, *Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors* [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)
...
## הגדרה {#setup}
בתא הבא, מייבאים חבילות רלוונטיות ויוצרים מספר פונקציות עזר לבניית המעגלים עבור התפתחות הזמן ב-Trotter של מודל איזינג הרוחבי דו-ממדי, בהתאמה לטופולוגיית ה-Backend.
...
נדלג על שלב זה מכיוון שהפחתת השגיאות של Qiskit Runtime אינה נתמכת בסימולטורים.

## דוגמה בחומרה אמיתית בקנה מידה גדול {#large-scale-hardware-example}

### שלב 1: מיפוי קלטים קלאסיים לבעיה קוונטית {#step-1-map-classical-inputs-to-a-quantum-problem}

#### יצירת מעגל מפורמטר של מודל איזינג {#create-a-parameterized-ising-model-circuit}

##### בחירת Backend {#establish-a-backend}
...
כדי לממש את סימולציית איזינג ב-Trotter, הגדירו שלוש שכבות של קישורי שער דו-קיוביטי עבור המכשיר, אשר יחזרו על עצמן בכל אחד משלבי Trotter. שכבות אלו מגדירות את שלוש השכבות המסובבות שנדרש ללמוד עבורן את הרעש לצורך יישום ההפחתה.
...
##### יצירת מעגל ה-Trotter הראשי {#main-trotter-circuit-generation}
...
שימו לב שכל observable עטוף ברשימה בבלוק הקוד הבא. הדבר נעשה כדי לבצע _broadcast_ עם ערכי הפרמטרים, כך שכל observable של קיוביט יימדד עבור כל ערך theta. כללי ה-broadcasting עבור primitives ניתן למצוא [כאן](/guides/primitives).
...
* **amplifier**: השיטה לשימוש בעת הגברת הרעש לגורמי הרעש המיועדים.
  הערכים המותרים הם `"gate_folding"`, שמגביר על ידי חזרה על שערי בסיס דו-קיוביטיים,
  ו-`"pea"`, שמגביר על ידי דגימה הסתברותית לאחר לימוד מודל הרעש המפוצל-פאולי (Pauli-twirled)
  עבור שכבות של שערי בסיס דו-קיוביטיים מפוצלים. אפשרויות נוספות הן `"gate_folding_front"` ו-`"gate_folding_back"`, שמוסברות ב-[תיעוד ה-API](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier).

* **extrapolated\_noise\_factors**: מציינים ערך אחד או יותר של גורם רעש שבו יש להעריך את
  המודלים האקסטרפולטיביים. אם ניתן רצף ערכים, התוצאות המוחזרות יהיו מסוג מערך עם גורם הרעש המצוין שהוערך עבור מודל האקסטרפולציה. ערך
  של 0 מתאים לאקסטרפולציה לרעש אפס.
...
1. `pub_result.data.evs`: ערכי ציפייה המתאימים לאפס-רעש (מבוססים על האקסטרפולציה הטובה ביותר הוריסטית).
   ...
   * הציר הראשון הוא אינדקס הקיוביט הווירטואלי עבור התצפית (observable) $\langle Z_i\rangle$ ($124$ קיוביטים-וירטואליים/תצפיות)

   * הציר השני מאנדקס את ערך הפרמטר עבור $\theta$ ($12$ ערכי פרמטר)

2. `pub_result.data.evs_extrapolated`: ערכי ציפייה עבור גורמי רעש שעברו אקסטרפולציה, עבור כל אקסטרפולטור. למערך זה שני צירים נוספים.

   * הציר השלישי מאנדקס את שיטות האקסטרפולציה ($2$ אקסטרפולטורים, `exponential` ו-`linear`)
...
* `resilience/zne/extrapolator`: האקסטרפולטורים שנעשה בהם שימוש עבור כל תוצאה
...
- [מדריך](/tutorials/combine-error-mitigation-techniques) המתמקד בשילוב טכניקות הפחתת שגיאות.
- [תיעוד מפורט](/guides/error-mitigation-and-suppression-techniques) על טכניקות הפחתת השגיאות הזמינות ב-Qiskit.
- שיעורים נוספים המכסים ניסויים בקנה מידה של תועלת: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) ו-[Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
