...
לאחר שתעברו על הדרכה זו, תבינו:
* כיצד לבנות מעגלים דינמיים עם מדידות באמצע מעגל והזנה קדימה קלאסית כדי לטלפורט שזירה בין קיוביטים מרוחקים;
* כיצד לחשב ולפרש את מדד השגיאה לכימות נאמנות זוג הבל על פני מכשיר;
* כיצד לזהות אילו זוגות קיוביטים מתאימים ביותר לפעולות מבוססות LOCC באמצעות תוצאות הבדיקה.

## דרישות מוקדמות {#prerequisites}
מומלץ שתכירו את הנושאים הבאים לפני שתתחילו הדרכה זו:
* [מושגי בסיס בחישוב קוונטי](/learning/courses/basics-of-quantum-information), כולל מצבי בל, שזירה, ושערים קוונטיים;
* היכרות עם [מעגלים דינמיים](/guides/classical-feedforward-and-control-flow) ([מדידות באמצע מעגל](/guides/execute-dynamic-circuits#mid-circuit-measurements) והזנה קדימה קלאסית);
* ידע בסיסי ב-[Qiskit SDK](https://docs.quantum.ibm.com/guides) וב-[IBM Quantum&reg; Compute Service](/guides/compute-services#qiskit-runtime), וגישה ל[חשבון IBM Quantum](/guides/cloud-setup).

## רקע {#background}

חומרה קוונטית מוגבלת בדרך כלל לאינטראקציות מקומיות, אך אלגוריתמים רבים דורשים שזירה של קיוביטים מרוחקים או אפילו [קיוביטים במעבדים נפרדים](#references). מעגלים דינמיים - כלומר, מעגלים עם מדידה באמצע המעגל והזנה קדימה - מספקים דרך להתגבר על מגבלות אלה על ידי שימוש בתקשורת קלאסית בזמן אמת כדי ליישם ביעילות פעולות קוונטיות לא-מקומיות. בגישה זו, תוצאות מדידה מחלק אחד של מעגל (או מעבד קוונטי אחד) יכולות להפעיל באופן מותנה שערים על חלק אחר, ולאפשר לנו לטלפורט שזירה למרחקים ארוכים. זה מהווה את הבסיס לסכימות **פעולות מקומיות ותקשורת קלאסית (LOCC)**, שבהן אנחנו צורכים מצבי משאבים משוזרים (זוגות בל) ומעבירים תוצאות מדידה באופן קלאסי כדי לקשר קיוביטים מרוחקים.

שימוש מבטיח אחד ב-LOCC הוא לממש שערי CNOT וירטואליים לטווח ארוך באמצעות טלפורטציה, כפי שמוצג ב[הדרכה על שזירה לטווח ארוך](/tutorials/long-range-entanglement). במקום CNOT ישיר לטווח ארוך (שייתכן שקישוריות החומרה אינה מאפשרת), אנחנו יוצרים זוגות בל ומבצעים מימוש שער מבוסס טלפורטציה. עם זאת, הנאמנות של פעולות כאלה תלויה במאפייני החומרה. דה-קוהרנטיות של קיוביטים במהלך העיכוב ההכרחי (בזמן המתנה לתוצאות מדידה) וחביון תקשורת קלאסי יכולים לפגוע במצב המשוזר. כמו כן, שגיאות במדידות באמצע המעגל קשות יותר לתיקון מאשר שגיאות במדידות סופיות מכיוון שהן מתפשטות לשאר המעגל דרך השערים המותנים.

ב[ניסוי הייחוס](#references), המחברים מציגים מדד ביצועים לנאמנות זוג בל כדי לזהות אילו חלקים במכשיר מתאימים ביותר לשזירה מבוססת LOCC. הרעיון הוא להריץ מעגל דינמי קטן על כל קבוצה של ארבעה קיוביטים מחוברים במעבד. מעגל ארבעת הקיוביטים הזה יוצר תחילה זוג בל על שני קיוביטים אמצעיים, ואז משתמש בהם כמשאב לשזור את שני הקיוביטים הקצויים באמצעות LOCC. באופן קונקרטי, קיוביטים 1 ו-2 מוכנים מקומית בזוג בל _לא-חתוך_ (זוג בל שנוצר ישירות באמצעות Hadamard ו-CNOT, ללא טלפורטציה), ולאחר מכן שגרת טלפורטציה צורכת את זוג הבל הזה כדי לשזור את קיוביטים 0 ו-3. קיוביטים 1 ו-2 נמדדים במהלך ביצוע המעגל, ובהתבסס על תוצאות אלה, תיקוני Pauli (X על קיוביט 3 ו-Z על קיוביט 0) מיושמים. קיוביטים 0 ו-3 נשארים אז במצב בל בסוף המעגל.

כדי לכמת את איכות הזוג המשוזר הסופי, אנחנו מודדים את המייצבים שלו: במיוחד, הזוגיות בבסיס $Z$ ($Z_0Z_3$) ובבסיס $X$ ($X_0X_3$). עבור זוג בל מושלם, שתי התוחלות האלה שוות ל-+1. בפועל, רעש חומרה יפחית ערכים אלה. לכן אנחנו חוזרים על המעגל פעמיים עבור כל זוג קיוביטים: מעגל אחד מודד את קיוביטים 0 ו-3 בבסיס $Z$, ואחר מודד אותם בבסיס $X$. מהתוצאות, אנחנו מקבלים אומדן של $\langle Z_0Z_3\rangle$ ו-$\langle X_0X_3\rangle$ עבור זוג הקיוביטים הזה. אנחנו משתמשים בשגיאה הריבועית הממוצעת (MSE) של מייצבים אלה ביחס לערך האידיאלי (1) כמדד פשוט לנאמנות השזירה. MSE נמוך יותר פירושו ששני הקיוביטים השיגו מצב בל קרוב יותר לאידיאלי (נאמנות גבוהה יותר), בעוד ש-MSE גבוה יותר מצביע על שגיאה רבה יותר. על ידי סריקת ניסוי זה על פני המכשיר, אנחנו יכולים לבדוק את יכולת המדידה וההזנה קדימה של קבוצות קיוביטים שונות ולזהות את זוגות הקיוביטים הטובים ביותר לפעולות LOCC.

הדרכה זו מדגימה את הניסוי על מכשיר IBM Quantum&reg; כדי להמחיש כיצד ניתן להשתמש במעגלים דינמיים כדי ליצור ולהעריך שזירה בין קיוביטים מרוחקים. נמפה את כל השרשראות הליניאריות בנות ארבעה קיוביטים במכשיר, נריץ את מעגל הטלפורטציה על כל אחת, ולאחר מכן נמחיש את התפלגות ערכי ה-MSE. הליך מקצה לקצה זה מראה כיצד למנף את IBM Quantum Compute Service ותכונות מעגל דינמי כדי להנחות בחירות מודעות-חומרה לחיתוך מעגלים או הפצת אלגוריתמים קוונטיים על פני מערכת מודולרית.
...
* Qiskit Aer v0.17 ואילך (`pip install qiskit-aer`)
...
## דוגמה קטנת היקף עם סימולטור {#small-scale-simulator-example}

לפני הרצה על ה-QPU האמתי, אנחנו מאמתים שהמעגל מייצר זוג בל על ידי בדיקתו בסימולטור נטול רעשים עם שרשרת של ארבעה קיוביטים `[0, 1, 2, 3]`. אנחנו משתמשים ב-`Sampler` של IBM Quantum עם `AerSimulator` כמצב בקאנד לביצוע המעגלים.

### שלב 1: מיפוי קלטים קלאסיים לבעיה קוונטית {#step-1-map-classical-inputs-to-a-quantum-problem}

השלב הראשון הוא יצירת סט של מעגלים קוונטיים לבדיקת ביצועים של כל קישורי זוגות הבל המועמדים המותאמים לטופולוגיה של המכשיר. אנחנו מתחילים בבניית מעגלים כאלה עם שרשרת של ארבעה קיוביטים `[0, 1, 2, 3]`.

השגרה `create_bell_stab()` עושה את הדברים הבאים עבור כל שרשרת:
...
* מדידות באמצע מעגל והזנה קדימה: קיוביטים 1 ו-2 (הקיוביטים האמצעיים) נמדדים בבסיס החישובי, ומניבים שני ביטים קלאסיים. בהתבסס על תוצאות מדידה אלה, אנחנו מחילים פעולות מותנות: אם מדידת קיוביט 1 (נקרא לביט הזה $m_{12}$) היא 1, אנחנו מחילים שער $X$ על קיוביט 3; אם מדידת קיוביט 2 ($m_{21}$) היא 1, אנחנו מחילים שער $Z$ על קיוביט 0. שערים מותנים אלה (הממומשים באמצעות המבנה `if_test`/`if_else` של Qiskit) מיישמים את תיקוני הטלפורטציה הסטנדרטיים. הם "מבטלים" את היפוכי Pauli האקראיים המתרחשים עקב הקרנת קיוביטים 1 ו-2, ומבטיחים שקיוביטים 0 ו-3 מסתיימים במצב בל ידוע, ללא קשר לתוצאות המדידה. לאחר שלב זה, קיוביטים 0 ו-3 צריכים באופן אידיאלי להיות משוזרים במצב בל $|\Phi^+\rangle$.

* מדידת מייצבי זוג בל: אנחנו מתפצלים אז לשתי גרסאות של המעגל. בגרסה הראשונה, אנחנו מודדים את מייצב $ZZ$ על קיוביטים 0 ו-3. בגרסה השנייה, אנחנו מודדים את מייצב $XX$ על קיוביטים אלה.


עבור כל פריסה ראשונית של ארבעה קיוביטים, הפונקציה לעיל מחזירה שני מעגלים (אחד עבור $ZZ$, אחד עבור מדידת מייצב $XX$). מעגלים אלה כוללים מדידות באמצע מעגל ופעולות מותנות (if/else), שהן ההוראות המרכזיות של המעגל הדינמי.
...
לפני ביצוע המעגלים שלנו, אנחנו צריכים לטרנספל אותם לפעולות שער הנתמכות בבקאנד הנבחר. טרנספילציה תמפה את המעגל המופשט על הקיוביטים הפיזיים וסט השערים של הבקאנד שנבחר. מכיוון שכבר בחרנו קיוביטים פיזיים ספציפיים עבור כל שרשרת (על ידי מתן `initial_layout` ליוצר המעגל), אנחנו משתמשים ב-transpiler `optimization_level=0` עם הפריסה הקבועה הזאת. זה אומר ל-Qiskit לא להקצות מחדש קיוביטים או לבצע אופטימיזציות כבדות שיכולות לשנות את מבנה המעגל. אנחנו רוצים לשמור על רצף הפעולות (במיוחד השערים המותנים) בדיוק כפי שצוין.
...
כעת אנחנו יכולים להריץ את הניסוי על בקאנד הסימולטור נטול הרעשים.
...
השלב האחרון הוא לחשב את מדד השגיאה הריבועית הממוצעת (MSE) עבור כל קבוצת קיוביטים שנבדקה ולסכם את התוצאות. עבור כל שרשרת, יש לנו כעת את $\langle Z_0Z_3\rangle$ ו-$\langle X_0X_3\rangle$ הנמדדים. אם קיוביטים 0 ו-3 היו משוזרים בצורה מושלמת במצב בל $|\Phi^+\rangle$, היינו מצפים ששניהם יהיו +1. אנחנו מכמתים את הסטייה באמצעות ה-MSE:

$$\text{MSE} = \frac{( \langle Z_0Z_3\rangle - 1)^2 + (\langle X_0X_3\rangle - 1)^2}{2}.$$

ערך זה הוא 0 עבור זוג בל מושלם, ועולה ככל שהמצב המשוזר נעשה רועש יותר (עם תוצאות אקראיות שנותנות תוחלת סביב 0, ה-MSE יתקרב ל-1). הקוד מחשב את ה-MSE הזה עבור כל קבוצת ארבעה קיוביטים. ללא רעש, אנחנו רואים MSE = 0 בדוגמה קטנת ההיקף עם הסימולטור, כצפוי.

## דוגמה גדולת היקף על חומרה {#large-scale-hardware-example}

כאן אנחנו מאחדים את כל הפרטים הללו לזרימת עבודה אחת בקנה מידה גדול יותר, אשר מורצת לאחר מכן על חומרה קוונטית אמתית.
...
לאחר מכן, אנחנו בונים את המעגל עבור כל פס של 16 קיוביטים באמצעות הפונקציה `create_bell_stab()`. בסוף שלב זה, יש לנו רשימה של מעגלים המכסה כל שרשרת ארבעה-קיוביטים במכשיר. לאחר מכן אנחנו מטרנספלים ומריצים את המעגלים על הבקאנד האמתי ומעבדים את התוצאות.
...
התוצאות חושפות טווח רחב של איכות שזירה על פני ההתקן. זה מאשר את ממצא המאמר שיכולה להיות שונות של מעל סדר גודל בנאמנות מצב בל, בהתאם לאילו qubits פיזיים נעשה בהם שימוש. במונחים מעשיים, זה אומר שאזורים או קישורים מסוימים בשבב טובים הרבה יותר בביצוע מדידה ומשוב באמצע-מעגל מאחרים. גורמים כמו שגיאת קריאת qubit, חיי qubit, וקרוסטוק כנראה תורמים להבדלים האלה. לדוגמה, אם שרשרת אחת כוללת qubit קריאה רועש במיוחד, המדידה באמצע-מעגל עלולה להיות לא אמינה, מה שמוביל לנאמנות ירודה עבור זוג שזור זה (MSE גבוה).
לבסוף, אנחנו מדמיינים את הביצועים הכוללים על ידי שרטוט פונקציית ההתפלגות המצטברת (CDF) של ערכי ה-MSE עבור כל השרשראות. שרטוט ה-CDF מציג את סף ה-MSE על ציר ה-x, ואת החלק של זוגות ה-qubit שיש להם לכל היותר MSE זה על ציר ה-y. העקומה הזו מתחילה באפס ומתקרבת לאחד ככל שהסף גדל כדי לכלול את כל נקודות הנתונים. עלייה תלולה ליד MSE נמוך תצביע על כך שזוגות רבים הם נאמנות-גבוהה; עלייה איטית פירושה שלזוגות רבים יש שגיאות גדולות יותר. אנחנו מסמנים את ה-CDF עם הזהויות של הזוגות הטובים ביותר. בשרטוט, כל נקודה ב-CDF מתאימה ל-MSE של שרשרת ארבעה-qubits אחת, ואנחנו מתייגים את הנקודה עם זוג אינדקסי ה-qubit $[q0, q3]$ שהיו שזורים בניסוי הזה. זה מקל על זיהוי אילו זוגות qubit פיזיים הם המבצעים הטובים ביותר (הנקודות בקצה השמאלי בעקומת ה-CDF).
...
אם מצאת עבודה זו מעניינת, יתכן שתתעניין בחומרים הבאים:
- למד כיצד ליישם [שזירה לטווח ארוך עם מעגלים דינמיים](/tutorials/long-range-entanglement)
- למד כיצד לדמות את [האמילטוניאן Kicked Ising עם מעגלים דינמיים](/tutorials/dc-hex-ising)
</Admonition>
