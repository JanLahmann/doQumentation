...
במחברת זו, אנחנו מתאימים את הפרוטוקול לחומרה של IBM Quantum&reg; ומרחיבים אותו להפעלת פעולות LRCX מרובות במקביל, מה שמאפשר לנו לחקור כיצד הביצועים משתנים עם מספר הפעולות המותנות המתבצעות בו-זמנית.
## דרישות {#requirements}
...
השלב הבא מבטיח שה-backend תומך בהוראת `if_else`, הנדרשת לגרסה החדשה יותר של מעגלים דינמיים. מכיוון שתכונה זו עדיין בגישה מוקדמת, אנחנו מוסיפים במפורש את `IfElseOp` ליעד ה-backend אם הוא עדיין לא זמין.
...
מכיוון שאנו רוצים להשוות את הביצועים של מעגלים דינמיים ויוניטריים על שרשרת 1D, אנחנו משתמשים במחרוזת נאמנות שכבה כדי לבחור טופולוגיה ליניארית של השרשרת הטובה ביותר של qubits מההתקן. זה מבטיח ששני סוגי המעגלים מ-transpiled תחת אותם אילוצי קישוריות, מה שמאפשר השוואה הוגנת של הביצועים שלהם.
...
בשלב זה, אנחנו מבצעים את הניסוי על ה-backend שצוין. אנחנו גם עושים שימוש באצווה כדי להפעיל ביעילות את הניסוי על פני ניסיונות מרובים. הרצת ניסיונות חוזרים מאפשרת לנו לחשב ממוצעים להשוואה מדויקת יותר בין השיטות היוניטרית והדינמית, כמו גם לכמת את השונות שלהן על ידי השוואת הסטיות בין הריצות.
