...
לפני הרצה על ה-QPU האמיתי, נוודא ששני המעגלים הדינמיים והיוניטריים מייצרים מצב Bell אידיאלי על סימולטור ללא רעש. נשתמש ב-Qiskit Runtime `Sampler` עם `AerSimulator` כמצב ה-backend, במרחק קטן של 6.
