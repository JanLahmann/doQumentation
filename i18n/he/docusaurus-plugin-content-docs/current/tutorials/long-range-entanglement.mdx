...
לאחר השלמת מדריך זה, תלמד את הדברים הבאים:
* כיצד לממש שער CNOT ארוך טווח באמצעות מעגלים דינמיים עם מדידות בתוך המעגל (MCMs) והזנה קדימה קלאסית;
* כיצד לממש את השער השווה ערך באמצעות גישה יוניטרית מבוססת SWAP;
* כיצד להשוות את שתי הגישות על ידי מדידת נאמנות השער כפונקציה של מרחק ה-qubit.

## דרישות מוקדמות {#prerequisites}

מומלץ שהמשתמשים יהיו מכירים את הנושאים הבאים לפני עבור מדריך זה:
* [מושגי חישוב קוונטי בסיסיים](/learning/courses/basics-of-quantum-information), כולל מצבי Bell, שזירה ושערים קוונטיים;
* היכרות עם [מעגלים דינמיים](/guides/classical-feedforward-and-control-flow) (מדידות בתוך המעגל והזנה קדימה קלאסית);
...
במחברת זו, אנחנו מתאימים את הפרוטוקול לחומרה של IBM Quantum ומבצעים benchmark של ביצועיו כפונקציה של הפרדת הבקרה-מטרה, ומשווים אותו למימוש בסיסי יוניטרי מבוסס SWAP.
...
- Qiskit Runtime v0.37 או מאוחר יותר (`pip install qiskit-ibm-runtime`)

- Qiskit Aer v0.17 או מאוחר יותר (`pip install qiskit-aer`)
...
לפני הרצה על ה-QPU האמיתי, נוודא ששני המעגלים הדינמיים והיוניטריים מייצרים מצב Bell אידיאלי על סימולטור ללא רעש. נשתמש ב-IBM Quantum `Sampler` עם `AerSimulator` כמצב ה-backend, במרחק קטן של 6.
...
#### הכנת זוגות Bell {#prepare-bell-pairs}
...
#### מדידת זוגות qubits שכנים בבסיס Bell {#measure-neighboring-qubit-pairs-in-the-bell-basis}
...
#### החלת תיקוני הזנה קדימה לתיקון אופרטורי תוצר לוואי של Pauli {#apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
#### מדידת qubits הבקרה והמטרה {#measure-control-and-target-qubits}
...
נשלב את השלבים השונים שהוגדרו לעיל כדי ליצור שער CX ארוך טווח על שני קצוות של קו חד-ממדי (1D). השלבים כוללים את הבאים:
- אתחול qubit הבקרה ב-$|+\rangle$
- הכנת זוגות Bell
...
כעת נבנה את המעגלים שמודדים בבסיסים $XX$, $YY$, ו-$ZZ$, בדיוק כפי שעשינו עבור המעגלים הדינמיים לעיל.
...
![פלט של תא הקוד הקודם](/docs/images/tutorials/long-range-entanglement/extracted-outputs/b899e143-0.avif)

כעת שבנינו הן את המעגלים הדינמיים והן את המעגלים היוניטריים לדוגמה בקנה מידה קטן עם `distance=6`, נבצע transpile להם כדי להריץ תחילה על סימולטור ללא רעש.
...
כעת נוכל להריץ את הניסוי על ה-backend של הסימולטור ללא רעש. נשתמש ב-Qiskit Runtime Sampler עם AerSimulator כמצב ה-backend כדי להפעיל את המעגלים.
...
בסימולציה ללא רעש, נוודא שמדד הנאמנות הוא $1$ עבור המעגלים שנבנו. בניסויים על ה-QPUs האמיתיים, ניתוח זה יספק תמונה ברורה של עד כמה המעגלים הדינמיים מתפקדים בהשוואה למימוש היוניטרי הבסיסי.

#### מדדי איכות {#quality-metrics}

כדי להעריך את ההצלחה של פרוטוקול CX ארוך הטווח, אנחנו מודדים עד כמה מצב הפלט קרוב למצב Bell האידיאלי. דרך נוחה לכמת זאת היא על ידי חישוב נאמנות המצב באמצעות ערכי ציפייה של אופרטורי Pauli. נאמנות למצב Bell על מצב הבקרה והמטרה ניתן לחישוב לאחר ידיעת $\braket{XX}$, $\braket{YY}$, ו-$\braket{ZZ}$. באופן ספציפי,
...
כצפוי בסימולציה ללא רעש, הנאמנויות במעגלים הדינמיים וביוניטריים הן $1$.

## דוגמת חומרה בקנה מידה גדול {#large-scale-hardware-example}

כאן נרכיב את כל הפרטים הללו לתהליך עבודה אחד בקנה מידה גדול יותר, שמופעל לאחר מכן על חומרה קוונטית אמיתית.

### יצירת מעגלים למרחקים שונים {#generate-circuits-for-different-distances}

כעת ניצור מעגלי CX ארוכי טווח עבור טווח של הפרדות qubit עד 60 qubits. עבור כל מרחק, נבנה מעגלים שמודדים בבסיסים $XX$, $YY$, ו-$ZZ$, שישמשו מאוחר יותר לחישוב נאמנויות.
...
עכשיו כשיש לנו גם מעגלים דינמיים וגם מעגלים יוניטריים עבור טווח של מרחקים, אנחנו מוכנים ל-transpilation. תחילה אנחנו צריכים לבחור התקן backend.
...
השלב הבא מוודא שה-backend תומך בהוראה `if_else`, שנדרשת עבור הגרסה החדשה יותר של מעגלים דינמיים. מכיוון שתכונה זו עדיין בגישה מוקדמת, אנחנו מוסיפים במפורש את `IfElseOp` ליעד ה-backend אם היא עדיין לא זמינה.
...
#### שימוש במחרוזת Layer Fidelity לבחירת שרשרת חד-ממדית {#use-layer-fidelity-string-for-selecting-1d-chain}
מכיוון שאנחנו רוצים להשוות את הביצועים של מעגלים דינמיים ויוניטריים על שרשרת חד-ממדית, אנחנו משתמשים במחרוזת Layer Fidelity כדי לבחור טופולוגיה לינארית של שרשרת ה-qubits הטובה ביותר מהמכשיר. זה מבטיח ששני סוגי המעגלים עוברים טרנספילציה תחת אותן מגבלות קישוריות, מה שמאפשר השוואה הוגנת של ביצועיהם.
...
לאחר מכן, נבצע את הניסוי על ה-backend האמיתי. אנחנו גם עושים שימוש באצווה כדי להפעיל ביעילות את הניסוי על פני ניסיונות מרובים. הרצת ניסיונות חוזרים מאפשרת לנו לחשב ממוצעים להשוואה מדויקת יותר בין השיטות היוניטרית והדינמית, כמו גם לכמת את השונות שלהן על ידי השוואת הסטיות בין הריצות.
...
כדי להעריך את התוצאות באופן ויזואלי, התא שלהלן מציג את נאמנויות השער המוערכות שנמדדו במרחקים משתנים בין qubits משוזרים עבור כל שיטה.
...
עם זאת, אנחנו צופים בנקודת מעבר שבה הגישה הדינמית עולה על היוניטרית. זוהי תוצאה ישירה של הסקלה השונה: העומק של המעגל היוניטרי גדל באופן לינארי עם המרחק בין qubits, בעוד שעומק המעגל הדינמי נשאר קבוע.
...
- **מבט קדימה:** ככל שהחומרה משתפרת, במיוחד קריאה מהירה יותר, איחור בקרה קלאסי קצר יותר ותקורת מעגל-ביניים מופחתת, אנחנו צריכים לצפות שהפחתות העומק והמשך הללו יתורגמו לשיפורי נאמנות מדידים.
...
ככל שאיחורי המדידה והקלאסיים משתפרים, אנחנו צופים שהסקלה של עומק קבוע ומדידות קבועות של מעגלים דינמיים תניב יתרונות נאמנות וזמן ריצה ברורים במעגלים גדולים יותר.
