...
להלן, אנחנו יוצרים `CouplingMap` שרשרת ליניארית של 10 קיוביטים.
...
מאופרטור הקיוביטים, אנחנו יכולים ליצור מעגל קוונטי המדגמן את התפתחותו בזמן.
...
במהלך הפצה אחורה, מספר האיברים באופרטור יתקרב בדרך כלל ל-$4^N$ במהירות, כאשר $N$ הוא מספר הקיוביטים. כאשר שני איברים באופרטור אינם קומוטטיביים מבחינת קיוביטים, אנחנו זקוקים למעגלים נפרדים כדי לקבל את ערכי התוחלת המתאימים להם. לדוגמה, אם יש לנו אובזרבל של 2 קיוביטים $O = 0.1 XX + 0.3 IZ - 0.5 IX$, אז מכיוון ש-$[XX,IX] = 0$, מדידה בבסיס בודד מספיקה לחישוב ערכי התוחלת עבור שני איברים אלה. עם זאת, $IZ$ אנטי-קומוטטיבי עם שני האיברים האחרים. לכן אנחנו זקוקים למדידת בסיס נפרדת כדי לחשב את ערך התוחלת של $IZ$. במילים אחרות, אנחנו זקוקים לשניים, במקום אחד, מעגל כדי לחשב $\langle O \rangle$. ככל שמספר האיברים באופרטור גדל, יש אפשרות שמספר ביצועי המעגלים הנדרשים גם יגדל.
...
כדי לשלוט בכמות המשאבים הנוספים (זמן) המוקצים, אנחנו מגבילים את המספר המרבי של קבוצות Pauli קומוטטיביות מבחינת קיוביטים שהאובזרבל המופץ אחורה רשאי להחזיק. כאן אנחנו מציינים שההפצה האחורה צריכה להיפסק כאשר מספר קבוצות Pauli הקומוטטיביות מבחינת קיוביטים באופרטור גדל מעבר ל-8.
...
שים לב שעל ידי הקצאת שגיאת `5e-3` לכל פרוסה לקיטום, אנחנו מסוגלים להסיר פרוסה נוספת אחת מהמעגל, תוך הישארות בתקציב המקורי של שמונה קבוצות Pauli קומוטטיביות באובזרבל. כברירת מחדל, `backpropagate` משתמש בנורמת L1 של המקדמים הקטומים כדי להגביל את השגיאה הכוללת שנגרמה מקיטום. לאפשרויות אחרות ראה ב[מדריך הוראות על ציון p_norm](https://qiskit.github.io/qiskit-addon-obp/how_tos/bound_error_using_p_norm.html).
...
אנחנו שמים לב שקיטום מאפשר לנו להפיץ אחורה עוד יותר ללא עלייה במספר הקבוצות הקומוטטיביות באובזרבל.
#### עכשיו שיש לנו את ה-ansatz המופחת והאובזרבלים המורחבים שלנו, אנחנו יכולים להעביר את הניסויים שלנו ל-backend. {#now-that-we-have-our-reduced-ansatz-and-expanded-observables-we-can-transpile-our-experiments-to-the-backend}
...
אנחנו יוצרים את <a href="/guides/primitive-input-output">Primitive Unified Bloc (PUB)</a> עבור כל אחד משלושת המקרים.
...
לבסוף, אנחנו יכולים להריץ את הניסויים המופצים אחורה ולהשוות אותם עם הניסוי המלא באמצעות [StatevectorEstimator](https://docs.quantum.ibm.com/api/qiskit/qiskit.primitives.StatevectorEstimator) ללא רעש.
...
אנחנו שוקלים המילטוניאן של 50 קיוביטים $\hat{\mathcal{H}}_{XYZ}$ עבור הבעיה בקנה מידה מוגדל עם אותם ערכים עבור מקדמי $J$ ו-$h$ כמו בדוגמה בקנה מידה קטן. האובזרבל $M_Z = \frac{1}{N} \sum_{i=1}^N \langle Z_i \rangle$ זהה גם כן כמו קודם. בעיה זו חורגת מסימולציה קלאסית בכוח גס.
...
אנחנו מציינים את `max_error_per_slice` להיות 0.005 כמו קודם. עם זאת, מכיוון שמספר הפרוסות עבור בעיה בקנה מידה גדול זו גבוה בהרבה מבעיה בקנה מידה קטן, אפשור שגיאה של 0.005 לכל פרוסה עשוי בסופו של דבר ליצור שגיאת הפצה אחורה כוללת גדולה. אנחנו יכולים להגביל זאת על ידי ציון `max_error_total` שמגביל את שגיאת ההפצה האחורה הכוללת, ואנו מגדירים את ערכה ל-0.03 (שהוא בערך זהה לדוגמה בקנה מידה קטן).

עבור דוגמה בקנה מידה גדול זו, אנחנו מאפשרים ערך גבוה יותר למספר הקבוצות הקומוטטיביות, ומגדירים אותו ל-15.
...
תחילה נקבל את המעגל המופץ אחורה והאובזרבל ללא כל קיטום.
...
אנחנו שמים לב שאפשור קיטום מוביל להפצה אחורה של שלוש פרוסות נוספות. אנחנו יכולים לאמת את העומק של 2 קיוביטים של המעגל המקורי, המעגל המופץ אחורה, והמעגל המופץ אחורה עם קיטום לאחר transpilation.
...
מלא את הסקר הקצר הזה כדי לספק משוב על מדריך זה. התובנות שלך יעזרו לנו לשפר את הצעות התוכן וחוויית המשתמש שלנו.
