...
לאחר מכן אנחנו בונים את ה-`pub` ומריצים את המעגל הלא חתוך.
...
אנחנו מבחינים שערך התוחלת שהתקבל באמצעות חיתוך חוטים קרוב יותר לערך האידיאלי של $+1$ מאשר זה הלא חתוך. הבה נגדיל כעת את גודל הבעיה.
## חלק II. הגדלת קנה המידה! {#part-ii-scale-it-up}

קודם לכן, הראינו את התוצאות עבור מעגל MBL של 10 קיוביטים. לאחר מכן, אנחנו מראים שהשיפור בערך התוחלת מתקבל גם עבור מעגלים גדולים יותר. כדי להראות זאת, אנחנו חוזרים על התהליך עבור מעגל MBL של 60 קיוביטים.
...
אנחנו יוצרים קבוצה אקראית של ערכים עבור $\vec{\phi}$
...
לאחר מכן אנחנו בונים את המעגל החתוך
...
כפי שהוצג עבור הדוגמה בקנה מידה קטן, אנחנו מחלקים את המעגל והנצפה עבור ניסויי החיתוך.
...
אנחנו גם יוצרים אובייקט `SparsePauliOp` עבור הנצפה עם מקדמים מתאימים.
...
לאחר מכן אנחנו מייצרים את ה-subexperiments ומעבירים כל מעגל ב-subexperiment להתאמה לחומרה.
...
אנחנו משתמשים במצב `Batch` כדי לבצע את כל המעגלים ב-subexperiments.
...
כפי שצוין קודם לכן, מעגל הוא ידידותי לחיתוך כאשר ניתן לחלק את המעגל למעגלי משנה מנותקים קטנים יותר עם מספר קטן של חיתוכים. כל מעגל יעיל לחומרה, כלומר, מעגל שדורש מעט עד ללא שערי SWAP כאשר הוא ממופה למפת הצימוד של החומרה, הוא, באופן כללי, ידידותי לחיתוך. להלן, אנחנו מראים דוגמה של ansatz משמר עירור, המשמש בכימיה קוונטית. שימו לב שניתן לחלק מעגל כזה לשני מעגלי משנה עם חיתוך יחיד ללא תלות במספר הקיוביטים.
...
מעגל הוא לא ידידותי לחיתוך אם, באופן כללי, מספר החיתוכים הנדרשים ליצירת מחיצות מנותקות גדל משמעותית עם העומק של מספר הקיוביטים. נזכיר שעם כל חיתוך נדרש קיוביט נוסף. לכן עם מספר החיתוכים, מספר הקיוביטים האפקטיבי גם גדל. להלן אנחנו מראים דוגמה של מעגל Grover של 3 קיוביטים עם מקרה חיתוך אפשרי.

![wc-7.png](/docs/images/tutorials/wire-cutting-to-improve-performance/b31dc57e-e6d7-49fd-9f46-304a328b3764.avif)

אנחנו מבחינים ששלושה חיתוכים נדרשים, והחיתוך הוא יותר אנכי מאשר אופקי. המשמעות היא שמספר החיתוכים צפוי להתרחב באופן ליניארי עם מספר הקיוביטים, מה שאינו נוח לחיתוך.
## אסמכתאות {#references}
...
השתתפו בסקר הקצר הזה כדי לספק משוב על מדריך זה. התובנות שלכם יעזרו לנו לשפר את הצעות התוכן שלנו ואת חוויית המשתמש.
