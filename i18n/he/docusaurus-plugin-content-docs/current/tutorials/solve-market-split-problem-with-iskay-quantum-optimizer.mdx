...
אנחנו מחפשים וקטור השמה בינארי $x$, שבו:
...
כוחה של האופטימיזציה הקוונטית טמון בהמרת בעיות עם אילוצים לצורות ריבועיות ללא אילוצים [\[4\]](#references). עבור בעיית פיצול השוק, אנחנו ממירים את אילוצי השוויון
...
**שיטת הקנס:**
מכיוון שעלינו ש-$Ax = b$ יתקיים במדויק, אנחנו ממזערים את ההפרה בריבוע:
$$f(x) = ||Ax - b||^2$$
...
- **פורמט הבעיה**: אנחנו משתמשים ב-`"binary"` מאחר שהמשתנים שלנו הם בינאריים (0/1), המייצגים השמות לשווקים.
...
כעת אנחנו שולחים את הבעיה שלנו להרצה על חומרה של IBM Quantum. אלגוריתם bf-DCQO יבצע את הפעולות הבאות:
...
כעת אנחנו מבצעים עיבוד לאחר ביצוע של תוצאות הביצוע הקוונטי. תהליך זה כולל:
- ניתוח מבנה הפתרון
- אימות עמידה באילוצים
...
- **`solution_info`**: מידע מפורט הכולל:
  ...
  - `cost`: ערך פונקציית המטרה (אמור להיות 0 לסיפוק אילוצים מושלם)
...
כעת אנחנו מאמתים האם הפתרון הקוונטי עומד באילוצים של בעיית פיצול השוק. תהליך האימות בודק:

**מהי הפרת אילוץ?**
- עבור כל מוצר $i$, אנחנו מחשבים את המכירות בפועל באזור A: $(Ax)_i$
- אנחנו משווים זאת למכירות היעד $b_i$
- ה**הפרה** היא ההפרש המוחלט: $|(Ax)_i - b_i|$
- **פתרון ישים** הוא כזה שאין בו הפרות עבור כל המוצרים

**מה אנחנו מצפים:**
- **מקרה אידיאלי**: סך ההפרות = 0 (כל האילוצים מסופקים בצורה מושלמת)
...
**בדיקת ישימות:**
- **`is_feasible = True`** פירושו שהפתרון מספק את כל האילוצים בצורה מושלמת (סך הפרות = 0)
- **`is_feasible = False`** פירושו שחלק מהאילוצים הופרו

**ניתוח מכירות:**
- השוואת מכירות יעד מול מכירות בפועל לכל מוצר
- לפתרון מושלם: בפועל = יעד לכל המוצרים בשני האזורים
- ההפרש מעיד עד כמה אנחנו קרובים לחלוקת השוק הרצויה
...
- כל האילוצים העסקיים מסופקים בצורה מושלמת
...
- עלות = 0 מעידה על סיפוק אילוצים מושלם
- ערכי עלות גבוהים יותר מעידים על הפרות אילוצים גדולות יותר
## סיכום {#conclusion}
...
4. **לקבל פתרונות אופטימליים**: מציאת פתרונות ישימים העומדים בכל האילוצים
