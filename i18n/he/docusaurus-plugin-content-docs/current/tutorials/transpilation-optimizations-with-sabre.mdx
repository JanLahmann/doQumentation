...
למטרות מדריך זה, אנחנו מוציאים מחוץ לתהליך הן את HighLevelSynthesis והן את StarPrerouting כדי לבודד ולהדגיש את ההשפעה הישירה של הגדרת SABRE על זמן הריצה ועומק המעגל. על ידי מדידת ערך הציפייה $ \langle Z_0 Z_i \rangle $ לכל זוג Qubit, אנחנו מנתחים:
...
בשלב זה, אנחנו מתמקדים באופטימיזציה של פריסת המעגל לביצוע על מכשיר חומרה קוונטית ספציפי עם 127 Qubits. זהו המוקד העיקרי של המדריך, שבו אנחנו מבצעים **אופטימיזציות SABRE וטרנספילציה** להשגת ביצועי המעגל הטובים ביותר. באמצעות מעבר `SabreLayout`, אנחנו קובעים מיפוי Qubit ראשוני שממזער את הצורך בפעולות SWAP במהלך הניתוב. על ידי העברת `coupling_map` של ה-Backend היעד, `SabreLayout` מתאים את הפריסה למגבלות הקישוריות של המכשיר.
...
על ידי אופטימיזציה של מדדים אלו, אנחנו משפרים את אמינות המעגל ודיוק הביצוע על חומרה קוונטית רועשת.
בחרו את ה-Backend.
...
על ידי השוואת תוצאות ההגדרות הללו, אנחנו שואפים לקבוע איזו מהן משיגה את האיזון הטוב ביותר בין איכות המעגל (לדוגמה, גודל ועומק) לבין עלות חישובית.
...
בשלב זה, אנחנו משתמשים ב-primitive `Estimator` לחישוב ערכי הציפייה $\langle Z_0 Z_i \rangle$ עבור אופרטורי `ZZ`, להערכת השזירה ואיכות ביצוע המעגלים המטורנספלים. בהתאמה לתהליכי עבודה אופייניים של משתמשים, אנחנו מגישים את המשימה לביצוע ומיישמים דיכוי שגיאות באמצעות **פענוח דינמי (dynamical decoupling)**, טכניקה שמפחיתה דה-קוהרנציה על ידי הוספת רצפי Gates לשמירה על מצבי Qubit. בנוסף, אנחנו מציינים רמת חוסן להתמודדות עם רעש, כאשר רמות גבוהות יותר מספקות תוצאות מדויקות יותר במחיר של זמן עיבוד מוגבר. גישה זו מעריכה את ביצועי כל הגדרת מנהל מעברים בתנאי ביצוע מציאותיים.
...
לאחר השלמת המשימה, אנחנו מנתחים את התוצאות על ידי ציור גרף של ערכי הציפייה $\langle Z_0 Z_i \rangle$ עבור כל Qubit. בסימולציה אידיאלית, כל ערכי $\langle Z_0 Z_i \rangle$ אמורים להיות 1, המשקפים שזירה מושלמת על פני ה-Qubits. עם זאת, בשל רעש ומגבלות חומרה, ערכי הציפייה יורדים בדרך כלל ככל ש-`i` גדל, ומגלים כיצד השזירה מתדרדרת עם המרחק.

בשלב זה, אנחנו משווים את התוצאות מכל הגדרת מנהל מעברים לסימולציה האידיאלית. על ידי בחינת הסטיה של $\langle Z_0 Z_i \rangle$ מ-1 לכל הגדרה, אנחנו יכולים לכמת עד כמה כל מנהל מעברים משמר שזירה ומקטין את השפעות הרעש. ניתוח זה מעריך ישירות את ההשפעה של אופטימיזציות SABRE על נאמנות הביצוע ומדגיש איזו הגדרה מאזנת בצורה הטובה ביותר את איכות האופטימיזציה ואת ביצועי הביצוע.
...
כעת בצעו את אותה הפעולה עבור ה-heuristic מסוג decay.
...
מלאו סקר קצר זה כדי לספק משוב על מדריך זה. התובנות שלכם יסייעו לנו לשפר את תוכן ההצעות שלנו ואת חווית המשתמש.
