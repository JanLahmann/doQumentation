...
במחברת זו, אנחנו בוחנים את הסימולציה של שרשרת מחזורית של qubits שבה קיימת פעולת שני qubits בין כל שני qubits סמוכים, כולל הראשון והאחרון. שרשראות מחזוריות נמצאות לעתים קרובות בבעיות פיזיקה וכימיה כגון מודלים של Ising וסימולציה מולקולרית.
...
במחברת זו אנחנו מציגים את השימוש בחיתוך מעגלים להתמודדות עם בעיית שרשרת מחזורית בסדר גודל שימושי כזו שבה ה-Qubits הראשון והאחרון אינם שכנים. חיתוך קישוריות ארוכת טווח זו נמנע משערי SWAP נוספים במחיר של ביצוע מופעים מרובים של המעגל, וקצת עיבוד קלאסי לאחר הביצוע. לסיכום, ניתן לשלב חיתוך כדי לחשב באופן לוגי את פעולות 2-Qubit למרחק ארוך. במילים אחרות, גישה זו מובילה לעלייה אפקטיבית בקישוריות של מפת הצימוד, ובכך מובילה למספר קטן יותר של שערי SWAP.

שימו לב שישנם שני סוגים של חיתוכים - חיתוך חוט של מעגל (הנקרא `wire cutting`), או החלפת שער 2-Qubit במספר פעולות יחיד qubit (הנקראות `gate cutting`). במחברת זו, נתמקד בחיתוך שערים. לפרטים נוספים על חיתוך שערים, ראה ב<a href="https://qiskit.github.io/qiskit-addon-cutting/explanation/index.html">חומרי הסבר</a> ב-`qiskit-addon-cutting`, ובהפניות המתאימות. לפרטים נוספים על חיתוך חוטים, ראה ב[חיתוך חוטים להערכת ערכי ציפייה](/tutorials/wire-cutting), או במדריכים ב-<a href='https://qiskit.github.io/qiskit-addon-cutting/tutorials/index.html'>qiskit-addon-cutting</a>.
...
אנחנו שוקלים `entangler map` יעילה לחומרה עבור מעגל TwoLocal עם קישוריות מחזורית בין ה-Qubits האחרון והראשון של ה-`entangler map`. אינטראקציה ארוכת טווח זו יכולה להוביל לשערי SWAP נוספים במהלך הטרנספילציה, ובכך להגדיל את עומק המעגל.
...
כדי לאמת את איכות התוצאה באמצעות חיתוך מעגלים, אנחנו צריכים לדעת את התוצאה האידיאלית. המעגל הנוכחי שנבחר הוא מעבר לסימולציה קלאסית בכוח גס. לכן, אנחנו קובעים את הפרמטרים למעגל בזהירות כדי להפוך אותו ל-clifford.
...
כדי לכמת את היתרונות של חיתוך שערים אנחנו מודדים את ערכי הציפייה של התצפיות $\frac{1}{n}\sum_{i=1}^n \langle Z_i \rangle$ ו-$\frac{1}{n-1}\sum_{i=1}^{n-1} \langle Z_i Z_{i+1} \rangle$. כפי שנדון קודם, ערכי הציפייה האידיאליים הם $-1$ ו-$+1$ בהתאמה.
...
כעת אנחנו חותכים את השערים במעגל שעבר טרנספילציה. שימו לב ששערי 2-Qubit שצריכים להיות חתוכים הם אלה המחברים בין ה-Qubits האחרון והראשון של ה-Layout.
...
במקום להחיל את תהליך הטרנספילציה המלא על כל אחד מהמעגלים בתתי-הניסויים, אנחנו יכולים להשתמש במעברי טרנספילציה ספציפיים. ראה ב<a href="https://docs.quantum.ibm.com/api/qiskit/transpiler_passes">תיעוד זה</a> לתיאור מפורט של כל מעברי הטרנספילציה הזמינים.
...
אנחנו משתמשים ב-primitive `SamplerV2` לביצוע המעגלים החתוכים. אנחנו משביתים `dynamical decoupling` ו-`twirling` כך שכל שיפור שנקבל בתוצאה יהיה אך ורק בגלל יישום אפקטיבי של חיתוך שערים עבור סוג זה של מעגל.
...
לסיכום, חישבנו את ערכי הציפייה הממוצעים של תצפיות מסוג Z במשקל-1 ומשקל-2 עבור שרשרת 1D מחזורית של 109 Qubits. כדי לעשות זאת, אנחנו
...
אנחנו שמים לב מהתוצאות שהממוצע של תצפיות סוג $\langle Z \rangle$ במשקל-1 ו-$\langle ZZ \rangle$ במשקל-2 משתפר באופן משמעותי על ידי חיתוך השערים המחזוריים. שימו לב שמחקר זה אינו כולל טכניקות דיכוי או הפחתת שגיאות כלשהן. השיפור שנצפה נובע אך ורק מהשימוש הנכון בחיתוך שערים עבור בעיה זו. התוצאות יכלו להשתפר עוד יותר על ידי שימוש בטכניקות ההפחתה והדיכוי.

מחקר זה מציג דוגמה לשימוש אפקטיבי בחיתוך שערים כדי לשפר את ביצועי החישוב.

## סקר המדריך {#tutorial-survey}

השתתף בסקר קצר זה כדי לספק משוב על מדריך זה. התובנות שלך יעזרו לנו לשפר את הצעות התוכן ואת חוויית המשתמש שלנו.

> **הערה:** הסקר הזה הוא של **IBM Quantum** ומכסה את **תוכן המדריך** (שנכתב על ידי IBM). doQumentation מספקת את האתר, התרגומים והרצת הקוד — למשוב על אלה, אנא [פתחו issue ב-GitHub](https://github.com/JanLahmann/doQumentation/issues).
