...
- כיצד להשתמש ב[תוסף SQD ל-Qiskit](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd) כדי לקרב את אנרגיית מצב היסוד של מערכת מולקולרית באמצעות מחרוזות ביטים שנדגמו מיחידת עיבוד קוונטי (QPU).
...
## רקע {#background}
במדריך זה, נראה כיצד לבצע עיבוד-לאחר על דגימות קוונטיות רועשות כדי לקרב את מצב היסוד של מולקולת החנקן $\text{N}_2$ באורך קשר שיווי משקל, על ידי שימוש ב[תוסף SQD ל-Qiskit](https://github.com/Qiskit/qiskit-addon-sqd) ליישום [אלגוריתם דיאגונליזציה קוונטית מבוססת דגימות (SQD)](https://arxiv.org/abs/2405.05068). פרטים נוספים על התוכנה ניתן למצוא ב[תיעוד](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd) המתאים, כולל [דוגמה פשוטה](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd/guides/quickstart) כדי להתחיל.
...
לאחר אופטימיזציה של המעגל לביצוע חומרה, אנחנו מוכנים להריץ אותו על חומרת היעד ולאסוף דגימות לאומדן אנרגיית מצב היסוד. מכיוון שיש לנו רק מעגל אחד, נשתמש ב[מצב ביצוע עבודה](/guides/execution-modes) של IBM Quantum Compute Service ונבצע את המעגל שלנו.
...
- [שינוי קנה מידה של זרימות עבודה של כימיה SQD עם פותר Dice](https://docs.quantum.ibm.com/addons/qiskit-addon-sqd/guides/integrate-dice-solver) - דף המראה כיצד להשתמש בתוכנת Dice היעילה יותר לדיאגונליזציה
- [תיעוד API של תוסף SQD](https://docs.quantum.ibm.com/api/qiskit-addon-sqd/fermion#diagonalize_fermionic_hamiltonian) - עזר לפונקציה `diagonalize_fermionic_hamiltonian`
