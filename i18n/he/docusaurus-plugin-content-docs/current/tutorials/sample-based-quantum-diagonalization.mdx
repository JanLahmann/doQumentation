...
כדוגמה, שקול טופולוגיית קיוביט של סריג מרובע. אנחנו יכולים למקם את האורביטלים $\alpha$ ו-$\beta$ בקווים מקבילים על הסריג, עם חיבורים בין קווים אלה היוצרים "שלבי סולם" של צורת סולם, כזה:
...
החומרה של IBM&reg; יש לה טופולוגיית קיוביט של סריג heavy-hex, ובמקרה זה אנחנו יכולים לאמץ תבנית "זיג-זג", המתוארת למטה. בתבנית זו, אורביטלים עם אותו ספין ממופים לקיוביטים עם טופולוגיית קו (עיגולים אדומים וכחולים), וחיבור בין אורביטלים של ספינים שונים קיים בכל אורביטל מרחבי רביעי, כאשר החיבור מתבצע על ידי קיוביט עזר (עיגולים סגולים). במקרה זה, אילוצי האינדקס הם
...
במדריך זה, נמצא קירוב למצב היסוד של המולקולה בשיווי משקל בקבוצת הבסיס cc-pVDZ. ראשית, אנחנו מציינים את המולקולה ואת תכונותיה.
...
כעת, אנחנו משתמשים ב-[ffsim](https://github.com/qiskit-community/ffsim) כדי ליצור את מעגל האנסץ. מכיוון שלמולקולה שלנו יש מצב Hartree-Fock של קליפה סגורה, אנחנו משתמשים בוריאנט מאוזן-ספין של אנסץ ה-UCJ, [UCJOpSpinBalanced](https://qiskit-community.github.io/ffsim/api/ffsim.html#ffsim.UCJOpSpinBalanced). אנחנו מעבירים זוגות אינטראקציה המתאימים לטופולוגיית קיוביט של סריג heavy-hex (ראה את [סעיף הרקע על אנסץ LUCJ](#local-unitary-cluster-jastrow-lucj-ansatz)). אנחנו מגדירים `optimize=True` בשיטת `from_t_amplitudes` כדי לאפשר את הפקטוריזציה הכפולה ה"דחוסה" של אמפליטודות ה-$t_2$ (ראה [The local unitary cluster Jastrow (LUCJ) ansatz](https://qiskit-community.github.io/ffsim/explanations/lucj.html#Parameter-initialization-from-CCSD) בתיעוד של ffsim לפרטים).
...
לאחר מכן, אנחנו מבצעים אופטימיזציה של המעגל עבור חומרת יעד.
...
אנחנו ממליצים על השלבים הבאים כדי לבצע אופטימיזציה של האנסץ ולהפוך אותו תואם-חומרה.

- בחר קיוביטים פיזיים (`initial_layout`) מחומרת היעד התואמת לתבנית ה"זיג-זג" שתוארה קודם לכן. פריסת קיוביטים בתבנית זו מובילה למעגל תואם-חומרה יעיל עם פחות שערים. כאן, אנחנו כוללים קוד לאוטומציה של בחירת תבנית ה"זיג-זג" המשתמשת באבחון הערכה כדי למזער את השגיאות הקשורות לפריסה הנבחרת.
...
התרשים הראשון מראה שאחרי כמה איטרציות אנחנו מעריכים את אנרגיית מצב היסוד בטווח של ``~41 mH`` (דיוק כימי מקובל בדרך כלל להיות ``1 kcal/mol`` $\approx$ ``1.6 mH``). ניתן לשפר את האנרגיה על ידי אפשור יותר איטרציות של שחזור תצורה או הגדלת מספר הדגימות לכל batch.
...
מלא את הסקר הקצר הזה כדי לספק משוב על המדריך הזה. התובנות שלך יעזרו לנו לשפר את הצעות התוכן וחוויית המשתמש שלנו.
