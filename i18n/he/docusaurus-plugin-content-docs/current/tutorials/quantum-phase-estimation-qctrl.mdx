...
title: 'אומדן פאזה קוונטי עם Q-CTRL'
---
...
## הגדרה {#setup}
ראשית, בצעו אימות באמצעות [מפתח API של IBM Quantum](http://quantum.cloud.ibm.com/) שלכם. לאחר מכן, בחרו את פונקציית Qiskit כדלקמן. (קוד זה מניח שכבר [שמרתם את החשבון שלכם](/guides/functions-get-started#install-qiskit-functions-catalog-client) לסביבה המקומית שלכם.)
...
אנחנו מכינים את המצב העצמי שלו $|\psi\rangle=|1\rangle$. מכיוון ש-$|1\rangle$ הוא וקטור עצמי של $U(\theta)$ עם ערך עצמי $e^{i\theta}$, הפאזה העצמית שיש להעריך היא:

$$
\varphi = \frac{\theta}{2\pi} \pmod{1}
$$

אנחנו מגדירים $\theta=\tfrac{1}{6}\cdot 2\pi$, כך שהפאזה האמיתית היא $\varphi=1/6$. מעגל QPE מיישם את החזקות המבוקרות $U^{2^k}$ על ידי הפעלת סיבובי פאזה מבוקרים עם זוויות $\theta\cdot2^k$, לאחר מכן מיישם את ה-QFT ההפוך על רגיסטר הספירה ומודד אותו. סדרות הביטים המתקבלות מתרכזות סביב הייצוג הבינארי של $1/6$.
...
אנחנו מגדירים את מספר הירי (shots) והקיוביטים לניסוי, ומקודדים את פאזת היעד $\varphi = 1/6$ באמצעות $m$ ספרות בינאריות. עם פרמטרים אלה, אנחנו בונים את מעגל QPE שיבוצע בסימולציה, בחומרה ברירת מחדל ובחומרה משופרת עם Fire Opal.
...
ראשית, אנחנו מייצרים התפלגות ייחוס באמצעות סימולטור `matrix_product_state` וממירים את הספירות להסתברויות מנורמלות לשם השוואה מאוחרת יותר עם תוצאות חומרה.
...
הקדישו דקה כדי לספק משוב על מדריך זה. התובנות שלכם יעזרו לנו לשפר את הצעות התוכן ואת חוויית המשתמש שלנו.
