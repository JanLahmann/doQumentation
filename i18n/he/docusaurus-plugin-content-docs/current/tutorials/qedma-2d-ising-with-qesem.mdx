...
Qiskit Functions הן תכונה ניסיונית הזמינה רק למשתמשי IBM Quantum&reg; Premium Plan, Flex Plan ו-On-Prem (דרך IBM Quantum Platform API) Plan. הן נמצאות בסטטוס מהדורת תצוגה מקדימה וכפופות לשינויים.
</Admonition>
...
במקום לדמות דינמיקה בזמן רציף, אנחנו מאמצים את מודל Ising המופעל הקשור קרוב. הדינמיקה יכולה להתבטא בדיוק כמעגל קוונטי תקופתי, כאשר כל שלב התפתחות מורכב משלוש שכבות של שערי שבר של שני קיוביטים $ R_{ZZ} (\alpha_{ZZ}) $, משולבות עם שכבות של שערים של קיוביט בודד $ R_X (\alpha_X) $ ו-$ R_Z (\alpha_Z) $.
...
* הדגמת השימוש ב-[הפצה לאחור של אופרטורים (OBP)](https://qiskit.github.io/qiskit-addon-obp/) להפחתת עומק המעגל. טכניקה זו קוצצת פעולות מסוף המעגל במחיר של יותר מדידות אופרטור.
...
לאחר מכן, אמת את הזהות באמצעות מפתח ה-API שלך מלוח המחוונים של [IBM Quantum Platform](http://quantum.cloud.ibm.com/). לאחר מכן, בחר את פונקציית Qiskit באופן הבא. (שים לב שלמען האבטחה, עדיף [לשמור את אישורי החשבון שלך](/guides/functions-get-started#install-qiskit-functions-catalog-client) בסביבה המקומית שלך, אם אתה על מכונה מהימנה, כדי שלא תצטרך להזין את מפתח ה-API שלך בכל פעם שאתה מאמת זהות.)
...
אנחנו מתחילים על ידי הגדרת פונקציה שיוצרת את מעגל Trotter:
...
לאחר מכן אנחנו יוצרים פונקציה לחישוב ערכי תוחלת אידיאליים באמצעות `AerSimulator`.

שים לב שעבור מעגלים גדולים (30 קיוביטים או יותר) אנחנו ממליצים להשתמש בערכים מחושבים מראש מסימולציות PEPS של הפצה-אמונה (BP). קוד זה כולל ערכים מחושבים מראש עבור 35 קיוביטים כדוגמה, בהתבסס על גישת BP לאבולוציה של רשת טנזור PEPS המוצגת [במאמר זה](https://www.science.org/doi/10.1126/sciadv.adk4321) (שאליה אנחנו מתייחסים כ-PEPS-BP), תוך שימוש בחבילת Python לרשתות-טנזור [quimb](https://joss.theoj.org/papers/10.21105/joss.00819).
...
אנחנו משתמשים במיפוי שכבות $R_{ZZ}$ מבוסס-חומרה שנלקח מהתקן Heron, שממנו אנחנו חותכים את השכבות בהתאם למספר הקיוביטים שברצוננו לדמות. אנחנו מגדירים תתי-גרפים עבור 10, 21, 28 ו-35 קיוביטים השומרים על מבנה דו-ממדי (אל תהסס לשנות לתת-גרף האהוב עליך):
...
כעת אנחנו מציגים חזותית את פריסת הקיוביטים על התקן Heron עבור תת-הגרף שנבחר:
...
כעת אנחנו יוצרים את מעגל Trotter ואת התצפית ממוצע המגנטיזציה עבור מספר הקיוביטים והפרמטרים שנבחרו:
...
כעת נשתמש בהפצה לאחור של אופרטורים (OBP). (ראה את תיעוד [OBP](https://qiskit.github.io/qiskit-addon-obp/) לפרטים נוספים על תוסף Qiskit של OBP.) נצור פונקציה שמייצרת את פרוסות המעגל להפצה לאחור:
...
אנחנו יכולים לראות שההפצה לאחור הפחיתה שתי שכבות של המעגל.
...
אנחנו רואים ש-OBP מפחיתה את עלות הזמן להפחתה של המעגל.
...
כעת אנחנו מריצים את הניסוי המלא על מספר שלבי Trotter. מספר הקיוביטים, הדיוק הנדרש וזמן QPU מקסימלי יכולים להשתנות בהתאם למשאבי QPU הזמינים. שים לב שהגבלת זמן QPU המקסימלי תשפיע על הדיוק הסופי, כפי שתראה בגרף הסופי למטה.

אנחנו מנתחים ארבעה מעגלים עם 5, 7 ו-9 שלבי Trotter בדיוק של 0.05, תוך השוואה בין ערכי התוחלת האידיאליים, הרועשים והמופחתי-שגיאות שלהם:
...
שוב, אנחנו מבצעים OBP על כל מעגל כדי להפחית את זמן הריצה:
...
כעת אנחנו מריצים אצווה של עבודות QESEM מלאות. אנחנו מגבילים את זמן הריצה המקסימלי של QPU עבור כל אחת מהנקודות לשליטה טובה יותר בתקציב QPU.
...
כאן אנחנו בודקים את הסטטוס של כל עבודה:
...
כאשר כל העבודות מסיימות לרוץ, אנחנו יכולים להשוות בין ערך התוחלת הרועש והמופחת-שגיאות שלהן.
...
לבסוף, אנחנו יכולים לשרטט את המגנטיזציה מול מספר השלבים. זה מסכם את היתרון של שימוש בפונקציית Qiskit של QESEM להפחתת שגיאות ללא הטיה על מכשירים קוונטיים רועשים.
