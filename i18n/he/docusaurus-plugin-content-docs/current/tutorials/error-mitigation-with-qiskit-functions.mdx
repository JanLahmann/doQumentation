...
כנסו לתיעוד למידע על [מבוא לפונקציות Qiskit](/guides/functions) ולמדו איך להתחיל עם [פונקציית IBM Circuit](/guides/ibm-circuit-function).
...
אנחנו רוצים להיות זהירים באופן שבו אנחנו מעצבים את השכבות של המעגל. נעשה זאת על ידי צביעת הקצוות של מפת הצימוד (כלומר, קיבוץ הקצוות הלא-חופפים) ונשתמש בצביעה זו כדי למקם שערים במעגל בצורה יעילה יותר. זה יוביל למעגל רדוד יותר עם שכבות של שערים שניתן להריץ בו-זמנית על החומרה.
...
התוצאה מהרצת הפונקציה היא באותו פורמט כמו [Estimator](/guides/estimator-input-output).

```python
result = job.result()[0]
```

אנחנו משיגים את ערכי התוחלת המופחתים והלא-מופחתים מתוצאה זו. ערכי תוחלת אלו מייצגים את המגנטיות החד-אתרית לאורך כיוון $\hat{z}$. אנחנו ממצעים אותם כדי להגיע למגנטיות הגלובלית ולהשוות לערך האידיאלי של `1.0` עבור מקרה בעיה זה.
...
מלאו את הסקר הקצר הזה כדי לספק משוב על מדריך זה. התובנות שלכם יעזרו לנו לשפר את הצעות התוכן וחוויית המשתמש שלנו.
