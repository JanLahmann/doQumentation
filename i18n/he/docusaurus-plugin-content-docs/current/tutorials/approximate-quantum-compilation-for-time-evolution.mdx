...
description: "מדריך זה מדגים כיצד ליישם AQC-Tensor עם Qiskit כדי לשפר את ביצועי המעגל הקוונטי"
...
מדריך זה מדגים כיצד ליישם **קומפילציה קוונטית קירובית** באמצעות רשתות טנזור (AQC-Tensor) עם Qiskit כדי לשפר את ביצועי המעגל הקוונטי. אנחנו מיישמים את AQC-Tensor בהקשר של אבולוציית זמן Trotterized כדי להפחית את עומק המעגל תוך שמירה על דיוק הסימולציה, תוך ביצוע תכנית העבודה של Qiskit להכנת מצב ואופטימיזציה. תלמדו כיצד ליצור מעגל ansatz בעומק נמוך ממעגל Trotter ראשוני, לבצע אופטימיזציה שלו באמצעות רשתות טנזור ולהכין אותו לביצוע על חומרה קוונטית.

המטרה העיקרית היא לדמות אבולוציית זמן עבור Hamiltonian מודל עם עומק מעגל מופחת. זה מושג באמצעות הרחבה של Qiskit בשם **AQC-Tensor**, [qiskit-addon-aqc-tensor](https://github.com/Qiskit/qiskit-addon-aqc-tensor), המשתמשת ברשתות טנזור, במיוחד במצבי מכפלת מטריצה (MPS), כדי לדחוס ולבצע אופטימיזציה של המעגל הראשוני. באמצעות התאמות איטרטיביות, מעגל ה-ansatz המדוחס שומר על נאמנות למעגל המקורי תוך שמירה על היתכנות עבור חומרה קוונטית של הדור הקרוב. פרטים נוספים ניתן למצוא ב[תיעוד](/guides/qiskit-addons-aqc) המתאים עם [דוגמה פשוטה](/guides/qiskit-addons-aqc-get-started) להתחלה.
...
* Qiskit SDK גרסה 1.0 ואילך, עם תמיכה ב[ויזואליזציה](https://docs.quantum.ibm.com/api/qiskit/visualization)
...
מודל XXZ נחקר באופן נרחב בפיזיקה לבחינת אינטראקציות ספין ותכונות מגנטיות. אנחנו מגדירים את ה-Hamiltonian כך שיהיו לו תנאי גבול פתוחים עם אינטראקציות תלויות-אתר בין אתרים שכנים לאורך השרשרת.

### Hamiltonian מודל ו-observable {#model-hamiltonian-and-observable}
...
כאשר $J_{i,(i+1)}$ הוא מקדם אקראי התואם לקצה $(i, i+1)$, ו-$L=10$ הוא מספר האתרים.
...
#### הגדרת ה-Hamiltonian וה-observable {#set-up-the-hamiltonian-and-observable}
...
1. **יצירת מעגל AQC מותאם**: באמצעות Trotterization, אנחנו מקרבים את האבולוציה הראשונית, אשר לאחר מכן נדחסת כדי להפחית את עומק המעגל.
...
#### קביעת חלק אבולוציית הזמן לסימולציה קלאסית {#determine-the-portion-of-time-evolution-to-simulate-classically}

המטרה שלנו היא לדמות את אבולוציית הזמן של ה-Hamiltonian המודל שהוגדר קודם לכן באמצעות אבולוציית Trotter. כדי להפוך תהליך זה ליעיל עבור חומרה קוונטית, אנחנו מפצלים את האבולוציה לשני מקטעים:

- **מקטע ראשוני**: חלק ראשוני זה של האבולוציה, מ-$ t_i = 0.0 $ עד $ t_f = 0.2 $, ניתן לסימולציה עם MPS וניתן "לקמפל" ביעילות באמצעות AQC-Tensor. באמצעות [הרחבת AQC-Tensor של Qiskit](https://github.com/Qiskit/qiskit-addon-aqc-tensor), אנחנו יוצרים מעגל מדוחס עבור מקטע זה, המכונה `aqc_target_circuit`. מכיוון שמקטע זה יידמה על סימולטור רשת-טנזור, אנחנו יכולים להרשות לעצמנו להשתמש במספר גבוה יותר של שכבות Trotter מבלי להשפיע על משאבי החומרה באופן משמעותי. אנחנו קובעים `aqc_target_num_trotter_steps = 32` עבור מקטע זה.

- **מקטע עוקב**: חלק נותר זה של האבולוציה, מ-$ t = 0.2 $ עד $ t = 0.4 $, יבוצע על חומרה קוונטית, המכונה `subsequent_circuit`. בהתחשב במגבלות החומרה, אנחנו שואפים להשתמש בכמה שפחות שכבות Trotter כדי לשמור על עומק מעגל ניתן לניהול. עבור מקטע זה, אנחנו משתמשים ב-`subsequent_num_trotter_steps = 3`.

#### בחירת זמן הפיצול {#choose-the-split-time}
אנחנו בוחרים ב-$t = 0.2$ כזמן הפיצול כדי לאזן בין יכולת סימולציה קלאסית והיתכנות חומרה. בתחילת האבולוציה, ההסתבכות במודל XXZ נשארת נמוכה מספיק עבור שיטות קלאסיות כמו MPS כדי לקרב במדויק.
...
#### יצירת ansatz ופרמטרים ראשוניים ממעגל Trotter עם פחות צעדים {#generate-an-ansatz-and-initial-parameters-from-a-trotter-circuit-with-fewer-steps}

כעת לאחר שבנינו את ארבעת המעגלים שלנו, נמשיך עם תהליך העבודה של AQC-Tensor. ראשית, אנחנו בונים מעגל "טוב" שיש לו אותו זמן אבולוציה כמו מעגל היעד, אך עם פחות צעדי Trotter (ולכן פחות שכבות).

לאחר מכן אנחנו מעבירים מעגל "טוב" זה לפונקציה `generate_ansatz_from_circuit` של AQC-Tensor. פונקציה זו מנתחת את הקישוריות של שני-qubit של המעגל ומחזירה שני דברים:
...
#### בחירת הגדרות לסימולציית רשת טנזור {#choose-settings-for-tensor-network-simulation}

כאן, אנחנו משתמשים בסימולטור מעגל מצב מכפלת-מטריצה של Quimb, יחד עם jax כדי לספק את הגרדיאנט.
...
לאחר מכן, אנחנו בונים ייצוג MPS של מצב היעד שיתקרב באמצעות AQC-Tensor. ייצוג זה מאפשר טיפול יעיל בהסתבכות, ומספק תיאור קומפקטי של המצב הקוונטי לאופטימיזציה נוספת.
...
שימו לב שבבחירת מספר גדול יותר של צעדי Trotter עבור מצב היעד, הפחתנו למעשה את שגיאת ה-Trotter שלו בהשוואה למעגל הראשוני. אנחנו יכולים להעריך את הנאמנות ($ |\langle \psi_1 | \psi_2 \rangle|^2 $) בין המצב שהוכן על ידי המעגל הראשוני למצב היעד כדי לכמת הבדל זה.
...
#### אופטימיזציה של הפרמטרים של ה-ansatz באמצעות חישובי MPS {#optimize-the-parameters-of-the-ansatz-using-mps-calculations}
בשלב זה, אנחנו מבצעים אופטימיזציה של פרמטרי ה-ansatz על ידי מזעור פונקציית עלות פשוטה, `MaximizeStateFidelity`, באמצעות אופטימייזר L-BFGS מ-SciPy. אנחנו בוחרים קריטריון עצירה לנאמנות המבטיח שהיא עולה על הנאמנות של המעגל הראשוני ללא AQC-Tensor. ברגע שסף זה מושג, המעגל המדוחס יציג גם שגיאת Trotter נמוכה יותר וגם עומק מופחת בהשוואה למעגל המקורי. באמצעות זמן CPU נוסף, אופטימיזציה נוספת יכולה להמשיך להגדיל את הנאמנות.
...
בחר את החומרה. כאן נשתמש באחד מהתקני IBM Quantum&reg; הזמינים שיש להם לפחות 127 qubits.
...
אנחנו מבצעים טרנספילציה של PUBs (מעגל ו-observables) כדי להתאים ל-ISA של הקצה (Instruction Set Architecture). על ידי הגדרת `optimization_level=3`, ה-transpiler מבצע אופטימיזציה של המעגל כדי להתאים לשרשרת חד-ממדית של qubits, ומפחית את הרעש המשפיע על נאמנות המעגל. ברגע שהמעגלים מומרים לפורמט תואם עם הקצה, אנחנו מיישמים טרנספורמציה מתאימה על ה-observables כדי להבטיח שהם מתיישרים עם פריסת ה-qubit המשונה.
...
בשלב זה, אנחנו מבצעים את המעגל המטורנספל על חומרה קוונטית (או קצה מדומה). באמצעות המחלקה `EstimatorV2` מ-`qiskit_ibm_runtime`, אנחנו מגדירים estimator להרצת המעגל ומדידת ה-observable שצוין. תוצאת העבודה מספקת את התוצאה הצפויה עבור ה-observable, ומעניקה לנו תובנות לגבי ביצועי המעגל על חומרת היעד.
...
במקרה זה, שחזור אינו נחוץ. אנחנו יכולים לבחון ישירות את התוצאה על ידי גישה לערך הציפייה מפלט הביצוע.
...
החלק השני של מדריך זה מתבסס על הדוגמה הקודמת על ידי הגדלת קנה המידה למערכת גדולה יותר עם 50 אתרים, הממחישה כיצד למפות בעיות סימולציה קוונטיות מורכבות יותר למעגלים קוונטיים ניתנים לביצוע. כאן, אנחנו חוקרים את הדינמיקה של מודל XXZ בן 50 אתרים, מה שמאפשר לנו לבנות ולבצע אופטימיזציה של מעגל קוונטי משמעותי המשקף גדלי מערכת ריאליסטיים יותר.
...
כאשר $J_{i,(i+1)}$ הוא מקדם אקראי התואם לקצה $(i, i+1)$, ו-$L=50$ הוא מספר האתרים.
הגדרת מפת הצימוד והקצוות עבור ה-Hamiltonian.
...
עבור בעיה גדולה יותר זו, אנחנו מתחילים בבניית ה-Hamiltonian עבור מודל XXZ בן 50 האתרים, המגדיר אינטראקציות ספין-ספין ושדות מגנטיים חיצוניים על פני כל האתרים. לאחר זאת, אנחנו עוקבים אחר שלושה שלבים עיקריים:
...
3. **שילוב המעגלים**: מיזוג מעגל ה-AQC המותאם עם מעגל האבולוציה הנותר כדי ליצור מעגל אבולוציית-זמן שלם המוכן לביצוע.
...
יצירת מעגל יעד ה-AQC (המקטע הראשוני).
...
כדי לבצע אופטימיזציה של פרמטרי ה-ansatz, אנחנו ממזערים את פונקציית העלות `MaximizeStateFidelity` באמצעות אופטימייזר L-BFGS מ-SciPy, עם קריטריון עצירה שנקבע לעבור את הנאמנות של המעגל הראשוני ללא AQC-Tensor. זה מבטיח שלמעגל המדוחס יש גם שגיאת Trotter נמוכה יותר וגם עומק מופחת.
...
### שלב 2: אופטימיזציה של הבעיה לביצוע חומרה קוונטית {#step-2-optimize-problem-for-quantum-hardware-execution-1}
בחירת הקצה.
...
בשלב זה, אנחנו מריצים את המעגל המטורנספל על חומרה קוונטית (או קצה מדומה) באמצעות `EstimatorV2` מ-`qiskit_ibm_runtime` כדי למדוד את ה-observable שצוין. תוצאת העבודה תספק תובנות חשובות לגבי ביצועי המעגל על חומרת היעד.
...
לרשימה מלאה של אפשרויות ביצוע זמינות, ראה ב[תיעוד qiskit-ibm-runtime](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-estimator-options).
...
### שלב 4: עיבוד לאחר ביצוע והחזרת תוצאה בפורמט קלאסי רצוי {#step-4-post-process-and-return-result-in-desired-classical-format-1}
כאן, אין צורך בשחזור, כמו קודם; אנחנו יכולים לגשת ישירות לערך הציפייה מפלט הביצוע כדי לבחון את התוצאה.
...
קח את הסקר הקצר הזה כדי לספק משוב על מדריך זה. התובנות שלך יעזרו לנו לשפר את הצעות התוכן ואת חוויית המשתמש שלנו.

[קישור לסקר](https://your.feedback.ibm.com/jfe/form/SV_eF01c2sfeSt6cqq)
