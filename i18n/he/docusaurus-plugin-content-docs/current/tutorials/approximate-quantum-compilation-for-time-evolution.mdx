...
- **מקטע ראשוני**: חלק ראשוני זה של האבולוציה, מ-$ t_i = 0.0 $ עד $ t_f = 0.2 $, ניתן לסימולציה עם MPS וניתן "לקמפל" ביעילות באמצעות AQC-Tensor. באמצעות [הרחבת AQC-Tensor של Qiskit](https://github.com/Qiskit/qiskit-addon-aqc-tensor), אנו יוצרים מעגל מדוחס עבור מקטע זה, המכונה `aqc_target_circuit`. מכיוון שמקטע זה יידמה על סימולטור רשת-טנזור, אנו יכולים להרשות לעצמנו להשתמש במספר גבוה יותר של שכבות Trotter מבלי להשפיע על משאבי החומרה באופן משמעותי. אנו קובעים `aqc_target_num_trotter_steps = 32` עבור מקטע זה.

- **מקטע עוקב**: חלק נותר זה של האבולוציה, מ-$ t = 0.2 $ עד $ t = 0.4 $, יבוצע על חומרה קוונטית, המכונה `subsequent_circuit`. בהתחשב במגבלות החומרה, אנו שואפים להשתמש בכמה שפחות שכבות Trotter כדי לשמור על עומק מעגל ניתן לניהול. עבור מקטע זה, אנו משתמשים ב-`subsequent_num_trotter_steps = 3`.

#### בחירת זמן הפיצול {#choose-the-split-time}
אנו בוחרים ב-$t = 0.2$ כזמן הפיצול כדי לאזן בין יכולת סימולציה קלאסית והיתכנות חומרה. בתחילת האבולוציה, ההסתבכות במודל XXZ נשארת נמוכה מספיק עבור שיטות קלאסיות כמו MPS כדי לקרב במדויק.
...
שימו לב שבבחירת מספר גדול יותר של צעדי Trotter עבור מצב היעד, הפחתנו למעשה את שגיאת ה-Trotter שלו בהשוואה למעגל הראשוני. אנו יכולים להעריך את הנאמנות ($ |\langle \psi_1 | \psi_2 \rangle|^2 $) בין המצב שהוכן על ידי המעגל הראשוני למצב היעד כדי לכמת הבדל זה.
...
לרשימה מלאה של אפשרויות ביצוע זמינות, עיין ב[תיעוד qiskit-ibm-runtime](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-estimator-options).
