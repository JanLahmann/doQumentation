...
לאחר המעבר על מדריך זה, המשתמשים צריכים להבין:
- כיצד להשתמש ב-[SQD Qiskit addon](https://github.com/Qiskit/qiskit-addon-sqd) כדי לקרב את אנרגיית מצב היסוד של מודל סריג באמצעות מחרוזות ביטים שנדגמו מיחידת עיבוד קוונטית (QPU).
- כיצד להשתמש ב-[ffsim](https://github.com/qiskit-community/ffsim) כדי לבנות מעגלי אבולוציית זמן לסימולציה פרמיונית.
- כיצד לשלב דגימות ממספר מעגלים לעיבוד לאחר מכן עם אלגוריתם האלכסון Krylov מבוסס דגימה (SKQD).

## דרישות מקדימות {#prerequisites}
אנחנו מציעים שהמשתמשים יהיו מכירים את הנושאים הבאים לפני המעבר על מדריך זה:
- [אלכסון קוונטי מבוסס דגימה של המילטוניאן כימיה](/tutorials/sample-based-quantum-diagonalization)
...
ו-$H_1$ מכיל את שאר המונחים בהמילטוניאן. כדי לייצג את ההמילטוניאן באופן תוכניתי, אנחנו מאחסנים את המטריצה $h_{pq}$ ואת הטנזור $h_{pqrs}$.

### בסיסי מיקום ותנע {#position-and-momentum-bases}

בשל הסימטריה הטרנסלציונית המשוערת ב-$H_\textrm{bath}$, אנחנו לא מצפים שמצב היסוד יהיה דליל בבסיס המיקום (בסיס האורביטלים שבו ההמילטוניאן מוגדר למעלה). הביצועים של SQD מובטחים רק אם מצב היסוד דליל, כלומר, יש לו משקל משמעותי רק על מספר קטן של מצבי בסיס חישוביים. כדי לשפר את הדלילות של מצב היסוד, אנחנו מבצעים את הסימולציה בבסיס האורביטלים שבו $H_\textrm{bath}$ הוא אלכסוני. אנחנו קוראים לבסיס זה *בסיס תנע*. מכיוון ש-$H_\textrm{bath}$ הוא המילטוניאן פרמיוני ריבועי, ניתן לאלכסן אותו ביעילות על ידי סיבוב אורביטלי.
...
- ffsim v0.0.72 או מאוחר יותר (`pip install ffsim`)
...
ראשית, אנחנו מייצרים את ההמילטוניאן SIAM בבסיס המיקום. ההמילטוניאן מיוצג על ידי המטריצה $h_{pq}$ והטנזור $h_{pqrs}$. לאחר מכן, אנחנו מסובבים אותו לבסיס התנע. בבסיס המיקום, אנחנו ממקמים את הזיהום באתר הראשון. עם זאת, כאשר אנחנו מסובבים לבסיס התנע, אנחנו מעבירים את הזיהום לאתר מרכזי כדי להקל על אינטראקציות עם אורביטלים אחרים.
...
לאחר מכן, אנחנו מייצרים את המעגלים לייצור מצבי בסיס Krylov.
...
לתיאור מפורט יותר של מודל זה וכיצד המעגלים מתוכננים, ראה ב-["Quantum-Centric Algorithm for Sample-Based Krylov Diagonalization"](https://arxiv.org/abs/2501.09702).
...
כעת, אנחנו מבצעים אופטימיזציה של המעגל עבור חומרת יעד. לעת עתה, נייצר backend גנרי עם מספר קיוביטים מוגדר וסט שערים שמעגלי אבולוציית הזמן מתפרקים אליהם באופן טבעי.
...
כעת, אנחנו משתמשים ב-Qiskit כדי לבצע transpile של המעגלים ל-backend היעד.
...
כעת, אנחנו מריצים את אלגוריתם SQD באמצעות הפונקציה `diagonalize_fermionic_hamiltonian`. ראה את [תיעוד ה-API](https://qiskit.github.io/qiskit-addon-sqd/apidocs/qiskit_addon_sqd.fermion.html#qiskit_addon_sqd.fermion.diagonalize_fermionic_hamiltonian) להסברים על הארגומנטים לפונקציה זו.

תא הקוד הבא משרטט את התוצאות. העלילה הראשונה מציגה את האנרגיה המחושבת כפונקציה של מספר איטרציות שחזור התצורה, והעלילה השנייה מציגה את תפוסת הממוצע של כל אורביטל מרחבי לאחר האיטרציה הסופית. מכיוון שמדובר בבעיה קטנה כל כך, האיטרציה הראשונה כבר מביאה אותנו קרוב מאוד לאנרגיה המדויקת (שים לב לסקלה של ציר ה-y).
...
### אמת את האנרגיה {#verify-the-energy}

האנרגיה המוחזרת על ידי SQD מובטחת להיות חסם עליון לאנרגיית מצב היסוד האמיתית. ניתן לאמת את הערך של האנרגיה מכיוון ש-SQD מחזיר גם את המקדמים של וקטור המצב המקרב את מצב היסוד. ניתן לחשב את האנרגיה מוקטור המצב באמצעות מטריצות הצפיפות המופחתות של חד-חלקיק ודו-חלקיק, כפי שמודגם בתא הקוד הבא.
...
כעת, אנחנו מריצים דוגמה גדולה יותר על QPU אמיתי.
עבור אנרגיית ההתייחסות, אנחנו משתמשים בתוצאות של חישוב [DMRG](https://en.wikipedia.org/wiki/Density_matrix_renormalization_group) שבוצע בנפרד.
...
- [אלכסון קוונטי מבוסס דגימה של המילטוניאן כימיה](/tutorials/sample-based-quantum-diagonalization) - מדריך קשור המשתמש ב-ansatz וריאציוני היוריסטי במקום מעגלי Trotter
- [אלכסון קוונטי Krylov של המילטוניאנים סריג](/tutorials/krylov-quantum-diagonalization) - מדריך על שיטת KQD
- [תיעוד ה-API של SQD addon](https://qiskit.github.io/qiskit-addon-sqd/apidocs/qiskit_addon_sqd.fermion.html#qiskit_addon_sqd.fermion.diagonalize_fermionic_hamiltonian) - עיון בפונקציה `diagonalize_fermionic_hamiltonian`
- [*Quantum-Centric Algorithm for Sample-Based Krylov Diagonalization*](https://arxiv.org/abs/2501.09702) - המאמר עליו מבוסס מדריך זה
</Admonition>
