...
כעת, לאחר שיצרנו את המעגלים, אנחנו יכולים לבצע אופטימיזציה עליהם עבור חומרת יעד. אנחנו בוחרים את ה-QPU הכי פחות עסוק עם לפחות 127 qubit. בדוק את [מסמכי Qiskit IBM&reg; Runtime](/guides/get-started-with-primitives#get-started-with-sampler) למידע נוסף.
...
תא הקוד הבא משרטט את התוצאות. העלילה הראשונה מציגה את האנרגיה המחושבת כפונקציה של מספר איטרציות שחזור התצורה, והעלילה השנייה מציגה את תפוסת הממוצע של כל אורביטל מרחבי לאחר האיטרציה הסופית. עבור אנרגיית ההתייחסות, אנחנו משתמשים בתוצאות של חישוב [DMRG](https://en.wikipedia.org/wiki/Density_matrix_renormalization_group) שבוצע בנפרד.
