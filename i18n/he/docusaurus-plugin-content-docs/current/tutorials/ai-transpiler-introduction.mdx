...
לפרטים נוספים על ה-AI passes הזמינים, ראה ב[תיעוד AI passes](/guides/ai-transpiler-passes).
...
מעגל `efficient_su2` מורכב משכבות מתחלפות של סיבובים של qubit בודד ושערי שזירה כמו CNOT. שכבות אלו מאפשרות חקירה גמישה של מרחב המצב הקוונטי תוך שמירה על עומק השער ניתן לניהול. על ידי אופטימיזציה של מעגל זה, אנחנו שואפים להפחית את ספירת השער, לשפר את הנאמנות ולמזער את הרעש. זה הופך אותו למועמד חזק לבדיקת היעילות של ה-AI transpiler.
...
לדוגמה זו, אנחנו בוחרים את ה-backend התפעולי הפחות עסוק של IBM Quantum שאינו סימולטור ויש לו לפחות 100 qubit:
...
במבחן זה, אנחנו משווים את הביצועים של ה-AI transpiler ושיטת ה-transpilation הסטנדרטית על מעגל efficient_su2. ה-AI transpiler משיג עומק מעגל רדוד באופן בולט תוך שמירה על ספירת שער דומה.
...
חשוב לציין שתוצאות אלו מבוססות על מעגל אחד בלבד. כדי לקבל הבנה מקיפה כיצד ה-AI transpiler משתווה לשיטות מסורתיות, יש צורך לבדוק מגוון מעגלים. הביצועים של QTS יכולים להשתנות מאוד בהתאם לסוג המעגל המיועל. להשוואה רחבה יותר, ראה ב-benchmarks לעיל או כנס ל[בלוג.](https://www.ibm.com/quantum/blog/qiskit-performance)
...
כדי לייצג בעיית תמורה קלאסית על מחשב קוונטי, אנחנו מתחילים בהגדרת המבנה של המעגלים הקוונטיים. לדוגמה זו:

1. אתחול מעגל קוונטי:
   אנחנו מקצים 27 qubit כדי להתאים ל-backend שבו נשתמש, שיש לו 27 qubit.

2. החלת תמורות:
   אנחנו מייצרים עשרה דפוסי תמורה אקראיים (`pattern_1` עד `pattern_10`) תוך שימוש ב-seed קבוע לשחזוריות. כל דפוס תמורה מוחל על מעגל קוונטי נפרד (`qc_1` עד `qc_10`).

3. פירוק מעגל:
   כל פעולת תמורה מפורקת לסטים של שערים מקוריים התואמים לחומרה הקוונטית היעד. אנחנו מנתחים את העומק ואת מספר השערים הדו-qubit (שערים לא מקומיים) עבור כל מעגל מפורק.
...
בשלב זה אנחנו ממשיכים עם אופטימיזציה תוך שימוש ב-AI synthesis passes.
...
לפרטים נוספים על שיטות סינתזה ב-Qiskit, ראה ב[תיעוד Qiskit API](https://docs.quantum.ibm.com/api/qiskit/synthesis).
...
מלא סקר קצר זה כדי לספק משוב על מדריך זה. התובנות שלך יעזרו לנו לשפר את תכני ההצעה וחוויית המשתמש שלנו.
