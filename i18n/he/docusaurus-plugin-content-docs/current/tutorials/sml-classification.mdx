...
- אפשרויות אופטימייזר: סימולטור מול QPU, חזרות, אופטימייזר קלאסי והאפשרויות שלו, עומק טרנספילציה והגדרות sampler/estimator של IBM Quantum
...
בסעיף זה, אנחנו מאתחלים את לקוח Qiskit Serverless וטוענים את פונקציית Singularity Machine Learning – Classification שמסופקת על ידי Multiverse Computing.
...
כדי להריץ מדריך זה, אנחנו משתמשים ב**מערך נתוני סיווג יציבות רשת חשמל** מעובד המכיל קריאות חיישני מערכת חשמל מתויגות.
...
אנחנו גם מייבאים את `QiskitRuntimeService` ו-`QiskitFunctionsCatalog` כדי להתחבר עם שירותי IBM Quantum&reg; ולגשת לפונקציית Singularity Machine Learning.
...
כעת אנחנו מריצים את תהליך העבודה המלא באמצעות פעולת `create_fit_predict` של פונקציית Singularity כדי לאמן, לבצע אופטימיזציה ולהעריך את ה-`QuantumEnhancedEnsembleClassifier` מקצה לקצה על תשתית IBM. הפונקציה בונה את האנסמבל, מיישמת אופטימיזציה קוונטית דרך primitives של Qiskit, ומחזירה גם תחזיות וגם מטא-נתוני עבודה (כולל זמן ריצה ושימוש במשאבים). החלוקה הקלאסית של הנתונים משלב 1 נעשית בה שימוש חוזר לצורך יכולת שחזור, כאשר נתוני ולידציה מועברים דרך `fit_params` כך שהאופטימיזציה יכולה לכוון היפר-פרמטרים באופן פנימי תוך שמירה על מערך המבחן שהושארה בצד ללא נגיעה.

בשלב זה, אנחנו חוקרים מספר תצורות של האנסמבל הקוונטי כדי להבין איך פרמטרי מפתח - במיוחד `num_learners` ו-`regularization` - משפיעים הן על איכות התוצאה והן על שימוש ב-QPU.
...
על ידי שינוי פרמטרים אלה, אנחנו יכולים לראות איך רוחב האנסמבל ו-regularization מתקשרים: הגדלת רוחב בדרך כלל משפרת F1 אך עולה יותר בזמן QPU, בעוד ש-regularization חזקה יותר או אדפטיבית יכולה לשפר הכללה בערך באותו טביעת רגל חומרה. תת-הסעיפים הבאים עוברים על שלוש תצורות מייצגות כדי להמחיש אפקטים אלה.
...
כאן אנחנו מגדילים `num_learners` מ-10 ← 30 תוך שמירה על `regularization = 7`.
...
בתצורה זו, אנחנו מגדילים ל-`num_learners = 60` ומציגים regularization אדפטיבית לניהול דלילות בצורה אינטואיטיבית יותר.
...
כעת אנחנו מעבדים פלטים מההרצות הקלאסיות והקוונטיות כאחד, וממירים אותם לפורמט עקבי להערכה במורד הזרם. שלב זה משווה איכות חיזוי באמצעות מדדים סטנדרטיים - accuracy, precision, recall ו-F1 - ומנתח איך רוחב אנסמבל (`num_learners`) ובקרת דלילות (`regularization`) משפיעים הן על ביצועים והן על התנהגות חישובית.
...
כעת אנחנו מעריכים את כל התצורות - בסיס AdaBoost הקלאסי ושלושת האנסמבלים הקוונטיים - באמצעות עזר `evaluate_predictions` כדי לחשב accuracy, precision, recall ו-F1 על אותו מערך מבחן. השוואה זו מבהירה איך אופטימיזציה קוונטית מתרחבת ביחס לגישה הקלאסית: ברוחבים קטנים, שתיהן מתפקדות באופן דומה; ככל שהאנסמבלים גדלים, השיטה הקוונטית יכולה לחקור מרחבי השערות גדולים יותר בצורה יעילה יותר. הטבלה המתקבלת לוכדת מגמות אלה בצורה עקבית וכמותית.
...
הקדישו דקה למתן משוב על מדריך זה. התובנות שלכם יעזרו לנו לשפר את הצעות התוכן שלנו וחווית המשתמש.
