---
title: "Dissoziationskurve mit Qunova HiVQE"
sidebar_label: "Dissoziationskurve mit Qunova HiVQE"
description: "Bruuch Qunova HiVQE zum Berächne vo Dissoziations-Potentialenergieoberfläche (PES)."
...
Qiskit Functions sind e experimentelli Funktion, wo nume für IBM Quantum&reg; Premium Plan, Flex Plan und On-Prem (über IBM Quantum Platform API) Plan Benutzer verfüegbar isch. Si sind im Vorschau-Status und chönd sich no ändere.
</Admonition>

*Nutzigsschtätzig (HINWYS: Das isch nume e Schtätzig. Dyni Laufzyt chan anders sy.)*

* Li2S: Föif Minute QPU-Zyt uf emene Heron r2 Prozässor
* FeP-NO: Föif Minute QPU-Zyt uf emene Heron r2 Prozässor
## Hingergrund {#background}

S gnau Berächne vo chemische Reaktionsenergiene isch wichtig für wüsseschaftlichi Fortschritt i de Materialwüsseschaft, chemische Technik, Medikamenteentdeckig und andere Fälder. Under verschidene chemische System het s Li-S System viel Ufmerksamkeit übercho für s Verschtah und Entwickle vo neue Batterie-Zämmesetzige. Das Tutorial git praktischi Erfahrig bim Berächne vo de Li-S Bindigsdisoziations-Potentialenergieoberflächi (PES) vo emene $Li_2S$ System dur s Ewägnäh vo eim Lithium-Atom mit HiVQE Berächnige. D Resultat chönd mit Referänzberächnige (CASCI) und klassische Methode wie Hartree-Fock (HF) für es 20-Qubit-Problem vergliche werde.
## Vorussetzige {#requirements}

Installiered di folgende Abhängigkeite zum Usfüehre vom Code i dem Tutorial.
...
Zum das Tutorial laufe z lah, importiered d `qunova/hivqe-chemistry` Funktion über s `QiskitFunctionCatalog`. Dir bruchet es IBM Quantum Premium Plan, Flex Plan, oder On-Prem (IBM Quantum Platform API) Plan Konto mit ere Lizänz vo Qunova zum Usfüehre vo dere Funktion.
...
catalog = QiskitFunctionsCatalog(
    ...
    token="YOUR_API_KEY",  # Bruchet der 44-Zeiche API_KEY, wo dir erstellt und gspeicheret händ vom IBM Quantum Platform Home Dashboard
)

hivqe = catalog.load("qunova/hivqe-chemistry")
```

## Teil 1: Li2S (20Q) {#part-1-li2s-20q}

### Schritt 1: Klassischi Ygabe uf es Quanteproblem abbildə {#step-1-map-classical-inputs-to-a-quantum-problem}

Definiered $Li_2S$ Geometrie im Dictionary-Format für verschideni Bindigsabschtänd vo Li-S zum Berächne vo de PES-Kurve. Die Geometrie sind optimiert mit B3LYP/631g Berächnige.
...
HiVQE Berächnige werdet mit de Optione usgfüehrt, wo unne definiert sind. Mit sto3g Basis für $Li_2S$ gits 19 rüümlichi Orbitale mit 22 Elektrone. Zum Usfüehre vom (10o,10e) Fall mit HiVQE Berächnig chönd dir 10 aktivi Orbitale und sächs ygfrorne Orbitale definiere. Bi jeder Iteration werdet 100 Shots brucht zum Sample vo Elektronekonfiguratione, wo dur de ExcitationPreserving Quanteschaltkreis (`epa`) mit `circular` Verschränkig und zwei Widerholige (`reps`) generiert werdet. D maximali Zahl vo Iteratione isch uf 30 gsetzt zum Sicherschtelle vo Beändigig mit Energiekonvergänz.
...
### Schritt 2 und 3: Optimiered s Problem für Quantehardware-Usfüehrig und füehreds us mit de HiVQE Chemistry Funktion {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}

Richted d `for`-Schleife y zum Laufe vo HiVQE Berächnige mit Geometrie mit de definierte Optione. Jobs werdet i de `for`-Schleife ygreicht. I dem Tutorial werdet dir sächs Geometrie yreiche und d Resultat abruefe, wenn si alli fertig sind. Im Hauptfunktionslauf müend dir d `max_states` und `max_expansion_states` definiere zum Kontroliere vo de maximale Grössi vo de Unterruum-Matrix und zum Feschtlege, wieviel Zueschtänd dur klassischi CI-Erwyterigsmethode pro Iteration generiert werde chönd. D Funktions-Job-IDs werdet im Dictionary mit jedem Geometrielabel gspeicheret zum wytere Verfolge und Verarbeite vom Output.
...
Lömmer luege, öb alli Jobs no am Laufe sind oder fertig sind.
...
Sobald alli Jobs fertig sind, lömmer alli Berächnigsresultat abruefe.
...
D gsamti QPU-Laufzyt, wo im Job brucht worde isch, chan verfolgt werde dur s Ylogge uf [IBM Quantum Platform](https://quantum.cloud.ibm.com/) und s Aaluege vo ygreichte Jobs mit em `qunova-chemistry-hivqe` Tag.
### Schritt 4: Nachbearbeitig und Verglyche mit klassische Methode {#step-4-post-process-and-compare-with-classical-methods}

Klassischi Referänzberächnig (CASCI) chan für (10o,10e) durchgfüehrt werde zum Validiere vo HiVQE Resultat.
...
### S Plotte vo de Dissoziationskurve für Li\_2S {#plotting-the-dissociation-curve-for-li_2s}

Lömmer plotte und HiVQE Resultat mit HF und CASCI verglyche.
Dir chönd gseh, dass alli HiVQE Berächnige guet mit em klassische Referänzresultat (CASCI) überyschtime.
...
## Teil 2: FeP-NO (44Q) {#part-2-fep-no-44q}

### Schritt 1: Klassischi Ygabe uf es Quanteproblem abbildə {#step-1-map-classical-inputs-to-a-quantum-problem-1}

Definiered d Optione für HiVQE Berächnige
...
Definiered FeP-NO Geometrie im Dictionary-Format für verschideni Bindigsabschtänd vo Fe-N zum Berächne vo de PES-Kurve.
...
### Schritt 2 und 3: Optimiered s Problem für Quantehardware-Usfüehrig und füehreds us mit de HiVQE Chemistry Funktion {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

Basierend uf em Setup vo HiVQE und de Geometrie, überchömet d Resultat sequenziell.
#### Yreiche d(Fe-N) = 1.75 $\AA$ Berächnig. {#submit-dfe-n-175-aa-calculation}
...
Verfolged der Job und ruefed s Resultat ab für d(Fe-N) = 1.75 $\AA$ Berächnig.
...
#### Yreiche d(Fe-N) = 2.00 $\AA$ Berächnig. {#submit-dfe-n-200-aa-calculation}
...
Verfolged der Job und ruefed s Resultat ab für d(Fe-N) = 2.00 $\AA$ Berächnig.
...
#### Yreiche d(Fe-N) = 5.00 $\AA$ Berächnig. {#submit-dfe-n-500-aa-calculation}
...
Verfolged der Job und ruefed s Resultat ab für d(Fe-N) = 5.00 $\AA$ Berächnig.
...
### Schritt 4: Nachbearbeitig und Verglyche mit klassische Methode {#step-4-post-process-and-compare-with-classical-methods-1}

Klassischi Referänzberächnigs (CASCI-DMRG, maxM=800) Resultat sind bereitgschtellt für (22o,22e) zum Validiere vo HiVQE Resultat.
...
Nämed öich bitte chli Zyt für die churzi Umfraag zum Feedback über das Tutorial z geh. Öyeri Yblick werdet eus helfe, üsi Inhält und Benutzererfahrig z verbessere.

[Link zur Umfraag](https://your.feedback.ibm.com/jfe/form/SV_9AizRyKFHEGzVs2)
