---
title: "Repetitionscodes"
sidebar_label: "Repetitionscodes"
description: "Das Tutorial zeigt, wie me grundlegendi Repetitionscodes mit IBM Dynamic Circuits baut — es Bispiel vo elementarer Quantefehlerkorrektur (QEC)."
...
## Hintergrundinformation {#background}
...
Du chasch zusätzlichi Ancilla-Qubits und Verschränkig nutze, um *Stabilisatore* z'mässe, wo d'kodierti Quanteinformation nöd transformiered, dir aber trotzdem Uschluus über gewissi Fehlerklasse gänd, wo eventuell passiert si. En Quante-Stabilisatorcode kodiert $k$ logischi Qubits in $n$ physikalischi Qubits. Stabilisatorcodes konzentriered sich vor allem druf, es diskrets Fehlerset mit Understützig vo de Pauli-Gruppe $\Pi^n$ z'korrigiere.

Mer Informazione über QEC findsch in [Quantum Error Correction for Beginners.](https://arxiv.org/abs/0905.2794)

## Vorrussetzige {#requirements}

Bevor du mit däm Tutorial afangsch, sorg derfür, dass du Folgends installiert hesch:

- Qiskit SDK v2.0 oder neuer, mit [Visualisierigs](https://docs.quantum.ibm.com/api/qiskit/visualization)-Understützig
...
## Schritt 1. Klassischi Yyabe uf es Quanteproblem abbilden {#step-1-map-classical-inputs-to-a-quantum-problem}
### En Bit-Flip-Stabilisator-Schaltkreis baue {#build-a-bit-flip-stabilizer-circuit}

De Bit-Flip-Code ghört zu de einfachschte Bischpiel vom Stabilisatorcode. Er schützt de Zuestand vor emene einzelne Bit-Flip-(X-)Fehler uf jedem vo de kodierungsbeteiligte Qubits. Betrachte mir d'Wirkig vom Bit-Flip-Fehler $X$, wo $|0\rangle \rightarrow |1\rangle$ und $|1\rangle \rightarrow |0\rangle$ uf jedem unsere Qubits mappt, denn händ mir $\epsilon = \{E_0, E_1, E_2 \} = \{IIX, IXI, XII\}$. De Code bruucht fünf Qubits: drüü wärded zum Kodiere vom gschützte Zuestand verwendet, und d'übrige zwei dienend als Stabilisatormässigs-Ancillas.
...
## Schritt 2. Das Problem für d'Quanteusfüehrig optimiere {#step-2-optimize-the-problem-for-quantum-execution}

Zum Verringere vo de totale Job-Usfüehrizyt akzeptiert Qiskit-Primitives nur Schaltkreis und Observablen, wo de Instrukzione und de Verbindigkeitsstruktur vom Zielsystem entspräche (als ISA-Schaltkreis und -Observablen bezeichnet). [Mer über Transpilation erfahre.](/guides/transpile)

### ISA-Schaltkreis generiere {#generate-isa-circuits}
...
## Schritt 3. Mit Qiskit-Primitives usfüehre {#step-3-execute-using-qiskit-primitives}
...
## Schritt 4. Nachbearbeitig und Rückgabe vom Resultat im klassische Format {#step-4-post-process-return-result-in-classical-format}
...
[Link zur Umfrag](https://your.feedback.ibm.com/jfe/form/SV_5onAlfA2Y7ac1FA)
