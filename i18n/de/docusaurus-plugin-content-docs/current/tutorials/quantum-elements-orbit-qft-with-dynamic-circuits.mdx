---
description: 'Schätze die Prozessfidelity von QFT gefolgt von Messung (QFT+M) über verschiedene Circuit-Größen hinweg; vergleiche unitäre Roh-, dynamische Roh- und Orbit-verbesserte dynamische Implementierungen'
notebook_path: tutorials/quantum-elements-orbit-qft-with-dynamic-circuits.ipynb
sidebar_label: 'QFT+M-Prozessfidelity mit Orbit benchmarken, einer Qiskit Function von Quantum Elements'
title: 'QFT+M-Prozessfidelity mit Orbit benchmarken, einer Qiskit Function von Quantum Elements'
---
...
*Warnung:* Dynamische Circuits sind derzeit ein experimentelles Feature und unterliegen Einschränkungen in Quantum Compute [[3]](#references), die zu Job-Fehlern führen können. Zum Beispiel zeigt der Fehler 6073 an, dass ein Job das Speicherlimit der klassischen Steuerungshardware überschritten hat [[4]](#references). Dieses Notebook verringert dieses Risiko, indem es die Circuit-Größen auf drei Quantum-Compute-Jobs in einem Batch aufteilt [[5]](#references). Jeder Vergleich mit fester Größe bleibt in einem Job, während große und kleine Größen gepaart werden, um die Workloads der klassischen Steuerung der Jobs auszugleichen.

## Lernziele {#learning-outcomes}

Nach Abschluss dieses Tutorials lernst du, wie man:

- die Produktzustände $\mathrm{QFT}^\dagger|x\rangle$ vorbereitet, die vom gesampelten Prozessfidelity-Schätzer in Abbildung 2a von Ref. [[1]](#references) verwendet werden.

- äquivalente unitäre und dynamische Implementierungen der Quanten-Fourier-Transformation gefolgt von einer Messung (QFT+M) baut.

- physische Qubits für die dynamischen Circuits anhand aktueller Kalibrierungs- und Konnektivitätsdaten auswählt.

- Schätzungen der QFT+M-Prozessfidelity für unitäre Roh-, dynamische Roh- und Orbit-verbesserte dynamische Varianten mit wachsender Circuit-Größe vergleicht.

- die optimierte Transpilations-API von Orbit mit `mode="raw"` und `transpilation_mode="validate"` verwendet.

- mehrere Orbit-Workloads über die Batch-Mode-API einreicht und dabei jeden Vergleich der drei Strategien mit fester Größe in einem Job beibehält.

- Orbit-Metadaten überprüfst, um zu bestätigen, ob Dynamical Decoupling (DD) und Measurement-Error-Mitigation (MEM) angewendet wurden.


## Hintergrund {#background}

Abbildung 2a von Ref. [[1]](#references) vergleicht die Prozessfidelity des idealen QFT+M-Kanals mit verrauschten unitären und dynamischen Implementierungen. Für ein gesampeltes Label $x$ in der Rechenbasis bereitet der Benchmark $\mathrm{QFT}^\dagger|x\rangle$ vor, wendet die verrauschte QFT+M-Implementierung an und schätzt die Wahrscheinlichkeit $p_x$, die entsprechende ideale Ausgabe zu erhalten. Diese Inverse-QFT-Zustände sind separabel und können effizient mit Hadamard-Gates und virtuellen Phasenrotationen vorbereitet werden.

Für $m$ unabhängig gesampelte Labels verwendet das Notebook den unverzerrten Schätzer, der in Ref. [[1]](#references) hergeleitet wird:
...
Die dynamische Konstruktion ersetzt die kontrollierten Phasen-Gates des unitären QFT+M durch Mid-Circuit-Messungen und klassisch bedingte Phasenrotationen [[1]](#references). Durch aufgeschobene Messung (deferred measurement) haben beide Circuits dieselbe ideale Ausgabeverteilung. Die dynamische Form entfernt die Anforderung von All-to-all-Zwei-Qubit-Gates und verwendet stattdessen $O(n)$ Mid-Circuit-Messungen mit Feedforward und ohne Konnektivitätsbeschränkung. Messung und Feedforward hinterlassen zudem lange Leerlaufzeiten bei Qubits, die noch nicht gemessen wurden, wodurch DD besonders relevant wird.
...
![QFT-Prozessfidelity auf „ibm_aachen“](/docs/images/tutorials/quantum-elements-orbit/dynamic_qft_orbit_tutorial_aachen_notebook_3job_average.svg)
...
Authentifiziere dich bei der [IBM Quantum&reg; Platform](https://quantum.cloud.ibm.com), lade `ibm_aachen` und lade Quantum Elements Orbit aus dem [Qiskit Functions Catalog](https://quantum.cloud.ibm.com/functions). Der Standard-Sweep wertet 15 Circuit-Größen, 20 gesampelte Bitstrings pro Größe und drei Strategien aus. `NUM_BATCH_JOBS=3` teilt die Größen auf drei Jobs in einem [Batch](/guides/run-jobs-batch) auf. Verringere `N_VALUES` oder `M`, oder erhöhe `NUM_BATCH_JOBS`, wenn ein einzelner dynamischer-Circuit-Job weiterhin das Speicherlimit der klassischen Steuerung des Backends erreicht. Verringere `SHOTS`, wenn das Ziel darin besteht, die Ausführungsnutzung zu senken, statt die Anzahl oder Komplexität der Circuits.
...
## QFT+M-Circuits erstellen {#build-qftm-circuits}

Für jede gesampelte ganze Zahl $x$ bereitet `bit_inv_qft` den Produktzustand $\mathrm{QFT}^\dagger|x\rangle$ mit Hadamard-Gates gefolgt von Phasenrotationen vor. Das Notebook hängt dann entweder die standardmäßige unitäre QFT oder ihr semiklassisches dynamisches QFT+M-Äquivalent an.

Beide Implementierungen lassen das abschließende Swap-Netzwerk weg. Die Anzeigereihenfolge der klassischen Bits in Qiskit macht daher den erwarteten gemessenen String zur Umkehrung der mit Nullen aufgefüllten Binärdarstellung von $x$, die durch `format(x, f"0{n}b")[::-1]` kodiert wird.
...
## Physische Qubits für den dynamischen Circuit auswählen {#select-dynamic-circuit-physical-qubits}

Die dynamische Implementierung benötigt keine Zwei-Qubit-Gates, daher müssen ihre physischen Qubits keinen zusammenhängenden Teilgraphen bilden. Für jede Circuit-Größe bewertet der Selektor die aktuellen Backend-Qubits anhand eines Scores, der zu 80 % auf einen niedrigeren Readout-Fehler und zu je 10 % auf höhere $T_1$- und $T_2$-Werte gewichtet ist. Er wählt zunächst nach Möglichkeit hoch bewertete Qubits ohne direkte Kopplungen untereinander aus, was die Anfälligkeit für Nearest-Neighbor-Crosstalk verringern kann, und füllt dann verbleibende Positionen nach Score auf.
...
## Die Benchmark-PUBs vorbereiten {#prepare-the-benchmark-pubs}
...
## Orbit-DD-Metadaten überprüfen {#inspect-orbit-dd-metadata}
...
## Prozessfidelity-Kurven plotten {#plot-process-fidelity-curves}
...
![Ausgabe der vorherigen Code-Zelle](/docs/images/tutorials/quantum-elements-orbit/extracted-outputs/plot-code-0.avif)
...
2. [Einführung in Qiskit Functions](/guides/functions)


3. [Einschränkungen von Quantum Compute für Stretch-Variablen](/guides/stretch#qiskit-runtime-limitations)


4. [IBM Quantum-Fehlercodes: 6073](https://ibm.biz/error_codes#6073)


5. [Jobs in einem Batch ausführen](/guides/run-jobs-batch)


## Nächste Schritte {#next-steps}

- Sieh dir den [Orbit-Guide](/guides/quantum-elements-orbit) und die [API-Referenz](https://docs.quantum.ibm.com/api/functions/quantum-elements-orbit)-Dokumentation an.

- Probiere ein anderes Backend, ein alternatives Layout aus oder experimentiere mit einer alternativen Orbit-gestützten Dynamical-Decoupling-Sequenz, indem du die Option `dd_strategy` änderst. Bedenke, dass du aufgrund der experimentellen Natur dynamischer Circuits auf mögliche Job-Fehlermodi achten musst (siehe [[3]](#references) und [[4]](#references)). Wenn du negative Stretch-Werte feststellst [[3]](#references), probiere eine kleinere DD-Sequenz (mit weniger Pulsen) aus. Wenn du [[4]](#references) feststellst, erhöhe `NUM_BATCH_JOBS`, verringere `M` oder reduziere die größten Werte in `N_VALUES`.
