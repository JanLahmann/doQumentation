...
---
{/* doqumentation-source-hash: c315029f */}
{/* source-hash: fb4f4fe9 */}
...
Der projizierte Quantenkern ist mit der folgenden Kernfunktion definiert: $$k^{\textrm{PQ}}(x_i, x_j) = \textrm{exp} \Big(-\gamma \sum_k \sum_{P \in \{ X,Y,Z \}} (\textrm{Tr}[P \rho_k(x_i)] - \textrm{Tr}[P \rho_k(x_j)])^2 \Big) $$
In der obigen Gleichung ist $\gamma>0$ ein einstellbarer Hyperparameter. Die $K^{\textrm{PQ}}_{ij} = k^{\textrm{PQ}}(x_i, x_j)$ sind die Einträge der Kernmatrix $K^{\textrm{PQ}}$.
Mithilfe der Definition der 1-RDMs sehen wir, dass die einzelnen Terme innerhalb der Kernfunktion als $\textrm{Tr}[P \rho_k (x_i)] = \braket P$ ausgewertet werden können, wobei $P \in \{ X,Y,Z \}$. Diese Erwartungswerte sind genau das, was wir oben gemessen haben.
Durch die Verwendung von ``scikit-learn`` können wir den Kern sogar noch einfacher berechnen. Dies liegt am direkt verfügbaren Radial-Basis-Funktions-Kern (``'rbf'``): $ \textrm{exp} (-\gamma \lVert x - x' \rVert^2)$. Wir müssen lediglich die neuen projizierten Trainings- und Testdatensätze in zweidimensionale Arrays umformen.
Beachte, dass das Durchlaufen des gesamten Datensatzes auf dem QPU etwa 80 Minuten dauern kann. Um sicherzustellen, dass der Rest des Tutorials leicht ausführbar ist, stellen wir zusätzlich Projektionen aus einem zuvor durchgeführten Experiment bereit (diese sind in den Dateien enthalten, die du im Code-Block `Download dataset` heruntergeladen hast). Wenn du das Training selbst durchgeführt hast, kannst du das Tutorial mit deinen eigenen Ergebnissen fortsetzen.
