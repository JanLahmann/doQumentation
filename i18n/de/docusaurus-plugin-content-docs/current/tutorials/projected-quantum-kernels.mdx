---
description: 'Tutorial zur Verbesserung der Merkmalsklassifikation mithilfe projizierter Quantenkerne'
notebook_path: tutorials/projected-quantum-kernels.ipynb
sidebar_label: 'Merkmalsklassifikation mit projizierten Quantenkernen verbessern'
title: 'Merkmalsklassifikation mit projizierten Quantenkernen verbessern'
---

{/* doqumentation-source-hash: 50ba7267 */}
...
*Geschätzter Aufwand: 80 Minuten auf einem Heron-r3-Prozessor (HINWEIS: Dies ist nur eine Schätzung. Deine Laufzeit kann abweichen.)*

## Lernziele {#learning-outcomes}

Nach diesem Tutorial sollten die Nutzer Folgendes verstehen:
- Wie projizierte Quantenkerne (PQK) funktionieren und wann sie einen potenziellen Quantenvorteil bieten.
- Wie man einen PQK auf Hardware mit einem realen Datensatz ausführt.

## Voraussetzungen {#prerequisites}

Wir empfehlen, dass Nutzer vor diesem Tutorial mit folgenden Themen vertraut sind:
- [Quantenkerne](/learning/courses/quantum-machine-learning/quantum-kernel-methods) aus dem [Kurs zum Quanten-Maschinenlernens](/learning/courses/quantum-machine-learning) in IBM Quantum&reg; Learning

## Hintergrund {#background}

In diesem Tutorial zeigen wir, wie du einen [projizierten Quantenkern](https://www.nature.com/articles/s41467-021-22539-9) (PQK) mit Qiskit auf einem realen biologischen Datensatz ausführst, basierend auf dem Paper [Enhanced Prediction of CAR T-Cell Cytotoxicity with Quantum-Kernel Methods](https://arxiv.org/abs/2507.22710) [[1]](#references).

PQK ist eine Methode des Quanten-Maschinenlernens (QML), um klassische Daten in einen Quanten-Merkmalsraum zu kodieren und sie durch den Einsatz von Quantencomputern zur verbesserten Merkmalsselektion wieder in die klassische Domäne zu projizieren. Dabei werden klassische Daten mithilfe eines Quantum Circuits in Quantenzustände kodiert – typischerweise durch einen als Feature Mapping bezeichneten Prozess, bei dem die Daten in einen hochdimensionalen Hilbert-Raum transformiert werden. Der „projizierte" Aspekt bezieht sich auf die Extraktion klassischer Information aus den Quantenzuständen durch Messung spezifischer Observablen, um eine Kernmatrix aufzubauen, die in klassischen kernbasierten Algorithmen wie Support-Vektor-Maschinen verwendet werden kann. Dieser Ansatz nutzt die Rechenvorteile von Quantensystemen, um bei bestimmten Aufgaben potenziell bessere Ergebnisse als klassische Methoden zu erzielen.

Die wichtigsten Bausteine von PQKs sind reduzierte Dichtematrizen (RDMs), die durch projektive Messungen der Quanten-Feature-Map gewonnen werden. Insbesondere werden typischerweise die Einzel-Qubit-reduzierten Dichtematrizen (1-RDMs) für jedes Qubit berechnet. Diese gemessenen Größen werden anschließend als Eingabe für eine klassische Kernfunktion, wie einen Exponentialkern, verwendet, um die endgültige Kernmatrix zu erstellen.

PQKs bieten potenzielle Vorteile gegenüber standardmäßigen [Quantenkernen](/tutorials/quantum-kernel-training), insbesondere für kurzfristige Quantenhardware. Standardmäßige Quantenkerne basieren typischerweise auf der Schätzung globaler Zustandsüberlappungen, was mit zunehmender Qubit-Anzahl immer schwieriger zu messen wird und sehr empfindlich gegenüber Rauschen ist. Im Gegensatz dazu verwenden PQKs lokale Observablen wie Einzel-Qubit-reduzierte Dichtematrizen (1-RDMs), was zu geringerem Stichprobenaufwand, verbesserter Robustheit gegenüber Hardware-Rauschen und besserer Skalierbarkeit führt. Indem Quantenzustände auf lokale Messmerkmale projiziert werden, bevor eine klassische Kernfunktion angewendet wird, können PQKs nützliche Quantenkorrelationen erhalten und gleichzeitig für kurzfristige Geräte praktikabler bleiben.
...
Wir verzichten in diesem Tutorial auf das kleine Simulatorbeispiel, da unser Hauptziel darin besteht zu zeigen, wie projizierte Quantenkerne auf größere Systeme und echte Hardware skaliert werden können.
...
### Datensatzvorbereitung {#dataset-preparation}

In diesem Tutorial verwenden wir einen realen biologischen Datensatz für eine binäre Klassifikationsaufgabe, der von Daniels et al. (2022) erstellt wurde und aus dem [Zusatzmaterial](https://www.science.org/doi/full/10.1126/science.abq0225#supplementary-materials) des Papers heruntergeladen werden kann. Der Datensatz besteht aus CAR-T-Zellen – gentechnisch veränderten T-Zellen, die in der Immuntherapie zur Behandlung bestimmter Krebsarten eingesetzt werden. T-Zellen, eine Art von Immunzellen, werden im Labor so modifiziert, dass sie chimäre Antigenrezeptoren (CARs) exprimieren, die auf spezifische Proteine auf Krebszellen abzielen. Diese veränderten T-Zellen können Krebszellen wirksamer erkennen und zerstören. Die Merkmale des Datensatzes sind die CAR-T-Zell-Motive, die die spezifischen strukturellen oder funktionalen Komponenten des in T-Zellen eingebauten CARs beschreiben. Basierend auf diesen Motiven ist unsere Aufgabe, die Zytotoxizität einer gegebenen CAR-T-Zelle vorherzusagen und sie entweder als toxisch oder nicht-toxisch zu kennzeichnen.
Im Folgenden werden die Hilfsfunktionen zur Vorverarbeitung dieses Datensatzes gezeigt.
...
Du kannst dieses Tutorial ausführen, indem du die folgende Zelle ausführst. Diese erstellt automatisch die erforderliche Ordnerstruktur und lädt sowohl die Trainings- als auch die Testdateien direkt in deine Umgebung herunter. Wenn du diese Dateien bereits lokal hast, überschreibt dieser Schritt sie sicher, um Versionskonsistenz zu gewährleisten.
...
Wir transformieren den Datensatz außerdem so, dass $1$ aus Skalierungsgründen als $\pi/2$ dargestellt wird.
...
Wir überprüfen die Größen und Formen der Trainings- und Testdatensätze.
...
Wir konstruieren nun die Feature Map, die unseren klassischen Datensatz in einen höherdimensionalen Merkmalsraum einbettet. Für diese Einbettung verwenden wir die [``ZZFeatureMap``](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.ZZFeatureMap) aus Qiskit.
...
![Ausgabe der vorherigen Code-Zelle](/docs/images/tutorials/projected-quantum-kernels/extracted-outputs/45956df4-5472-4394-a3e1-5514c456791d-0.avif)

Eine weitere Quanten-Einbettungsoption ist der 1D-Heisenberg-Hamiltonoperator-Evolutionsansatz. Du kannst diesen Abschnitt überspringen, wenn du mit der `ZZFeatureMap` fortfahren möchtest.
...
![Ausgabe der vorherigen Code-Zelle](/docs/images/tutorials/projected-quantum-kernels/extracted-outputs/659dbf23-fd3f-4e01-94b4-33e6d672172c-0.avif)

### Schritt 3: Mit Qiskit-Primitiven ausführen {#step-3-execute-using-qiskit-primitives}

### 1-RDMs messen {#measure-1-rdms}
In diesem Schritt ermitteln wir alle Einzel-Qubit-reduzierten Dichtematrizen (1-RDMs) durch projektive Messungen der Quanten-Feature-Map, die anschließend in die klassische Exponentialkernfunktion eingehen.
Schauen wir uns an, wie man die 1-RDM für einen einzelnen Datenpunkt aus dem Datensatz berechnet, bevor wir über alle Daten iterieren. Die 1-RDMs sind eine Sammlung von Einzel-Qubit-Messungen der Pauli-Operatoren ``X``, ``Y`` und ``Z`` auf allen Qubits. Dies liegt daran, dass eine Einzel-Qubit-RDM vollständig ausgedrückt werden kann als: $$\rho = \frac{1}{2} \big( I + \braket \sigma_x \sigma_x  + \braket \sigma_y \sigma_y + \braket \sigma_z \sigma_z  \big)$$
Zunächst wählen wir das zu verwendende Backend aus.
...
Dann führen wir den Quantum Circuit aus und messen die Projektionen. Beachte, dass wir Fehlerminderung aktivieren, einschließlich Zero Noise Extrapolation (ZNE).
...
Als Nächstes rufen wir die Ergebnisse ab.
...
Wir geben die Circuit-Größe und die Zwei-Qubit-Gate-Tiefe aus.
...
![Ausgabe der vorherigen Code-Zelle](/docs/images/tutorials/projected-quantum-kernels/extracted-outputs/4f573436-ec5c-451b-976c-ad718b3c201d-1.avif)

Wir können nun über den gesamten Trainingsdatensatz iterieren, um alle 1-RDMs zu erhalten.
Wir stellen außerdem Ergebnisse aus einem Experiment bereit, das wir auf Quantenhardware durchgeführt haben. Du kannst das Training entweder selbst durchführen, indem du das Flag unten auf `True` setzt, oder die von uns bereitgestellten Projektionsergebnisse verwenden.
...
Sobald die Jobs abgeschlossen sind, können wir die Ergebnisse abrufen.
...
Wir wiederholen dies für das Testset.
...
Wir können die Ergebnisse wie zuvor abrufen.
...
### Den projizierten Quantenkern definieren {#define-the-projected-quantum-kernel}
Der projizierte Quantenkernel wird durch die folgende Kernelfunktion definiert: $$k^{\textrm{PQ}}(x_i, x_j) = \textrm{exp} \Big(-\gamma \sum_k \sum_{P \in \{ X,Y,Z \}} (\textrm{Tr}[P \rho_k(x_i)] - \textrm{Tr}[P \rho_k(x_j)])^2 \Big) $$
In der obigen Gleichung ist $\gamma>0$ ein einstellbarer Hyperparameter. Die $K^{\textrm{PQ}}_{ij} = k^{\textrm{PQ}}(x_i, x_j)$ sind die Einträge der Kernelmatrix $K^{\textrm{PQ}}$.
Mithilfe der Definition von 1-RDMs können wir sehen, dass die einzelnen Terme innerhalb der Kernelfunktion als $\textrm{Tr}[P \rho_k (x_i)] = \braket P$ ausgewertet werden können, wobei $P \in \{ X,Y,Z \}$ gilt. Diese Erwartungswerte sind genau das, was wir oben gemessen haben.
Durch die Verwendung von ``scikit-learn`` können wir den Kernel tatsächlich noch einfacher berechnen. Dies liegt an dem sofort verfügbaren Radialbasisfunktions-Kernel (``'rbf'``): $ \textrm{exp} (-\gamma \lVert x - x' \rVert^2)$. Zunächst müssen wir lediglich die neuen projizierten Trainings- und Testdatensätze in zweidimensionale Arrays umformen.
Beachte, dass das Durchlaufen des gesamten Datensatzes auf der QPU etwa 80 Minuten dauern kann. Um sicherzustellen, dass der Rest des Tutorials leicht ausführbar ist, stellen wir zusätzlich Projektionen aus einem zuvor durchgeführten Experiment bereit (die in den Dateien enthalten sind, die du im Codeblock `Download dataset` heruntergeladen hast). Wenn du das Training selbst durchgeführt hast, kannst du das Tutorial mit deinen eigenen Ergebnissen fortsetzen.
...
Wir können nun eine klassische SVM auf diesem vorberechneten Kern ausführen und den Kern zwischen Test- und Trainingsset für die Vorhersage verwenden.
...
### Klassisches Benchmarking {#classical-benchmarking}
Wir können eine klassische SVM mit der Radial-Basis-Funktion als Kern ausführen, ohne eine Quantenprojektion durchzuführen. Dieses Ergebnis ist unser klassischer Benchmark.
...
## Anhang: Den potenziellen Quantenvorteil des Datensatzes bei Lernaufgaben überprüfen {#appendix-verify-the-datasets-potential-quantum-advantage-in-learning-tasks}

Nicht alle Datensätze bieten einen potenziellen Vorteil durch den Einsatz von PQKs. Es gibt einige theoretische Schranken, die man als Vortest verwenden kann, um zu prüfen, ob ein bestimmter Datensatz von PQKs profitieren kann. Um dies zu quantifizieren, definieren die Autoren von [Power of data in quantum machine learning](https://www.nature.com/articles/s41467-021-22539-9) [2] Größen, die als klassische und Quanten-Modellkomplexitäten sowie als geometrische Trennung der klassischen und Quantenmodelle bezeichnet werden. Um einen potenziellen Quantenvorteil durch PQKs zu erwarten, sollte die geometrische Trennung zwischen den klassischen und Quanten-projizierten Kernen ungefähr in der Größenordnung von $\sqrt{N}$ liegen, wobei $N$ die Anzahl der Trainingsproben ist. Wenn diese Bedingung erfüllt ist, prüfen wir die Modellkomplexitäten. Wenn die klassische Modellkomplexität in der Größenordnung von $N$ liegt, während die Quanten-projizierte Modellkomplexität wesentlich kleiner als $N$ ist, können wir einen potenziellen Vorteil durch den PQK erwarten.
Die geometrische Trennung ist wie folgt definiert (F19 in [[2]](#references)):
...
Die Modellkomplexität ist wie folgt definiert (M1 in [[2]](#references)):
...
- Ausführlicher [Kurs zum Quanten-Maschinenlernens](/learning/courses/quantum-machine-learning) von IBM Quantum Learning
- Tutorial zum [Quantenkern-Training](/tutorials/quantum-kernel-training)
</Admonition>
