...
- Wie man variationelle Workflows auf IBM&reg;-Quantenhardware mit IBM Quantum-Primitiven und Sessions ausführt
...
Minimiere die geschätzte Grundzustandsenergie des Systems, indem du die Circuit-Parameter optimierst. Verwende das IBM Quantum `Estimator`-Primitiv, um die Kostenfunktion während der Optimierung auszuwerten.

Da wir den Circuit für das Backend in Schritt 2 optimiert haben, können wir die Transpilation auf dem Quantum-Compute-Server vermeiden, indem wir `skip_transpilation=True` setzen und den optimierten Circuit übergeben. Für diese Demo führen wir die Berechnungen auf einem QPU mit IBM Quantum-Primitiven aus. Um stattdessen `qiskit`-Zustandsvektor-basierte Primitive zu verwenden, ersetze den Codeblock mit den IBM-Quantum-Primitiven durch den auskommentierten Block.
