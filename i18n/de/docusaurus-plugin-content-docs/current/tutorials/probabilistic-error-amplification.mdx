---
description: 'Führe ein Fehlerminderungsexperiment im Utility-Maßstab mit Zero-Noise-Extrapolation und probabilistischer Fehleramplifikation durch.'
notebook_path: tutorials/probabilistic-error-amplification.ipynb
sidebar_label: 'Fehlerminderung im Utility-Maßstab mit probabilistischer Fehleramplifikation'
title: 'Fehlerminderung im Utility-Maßstab mit probabilistischer Fehleramplifikation'
---
...
*Schätzung der Nutzungsdauer: 14 Minuten auf einem Heron-r3-Prozessor (HINWEIS: Dies ist nur eine Schätzung. Deine tatsächliche Laufzeit kann abweichen.)*

## Lernergebnisse {#learning-outcomes}
Nach diesem Tutorial solltest du Folgendes verstehen:
- Die Theorie hinter der *Zero-Noise-Extrapolation* (ZNE), die verschiedenen Methoden zur Rauschverstärkung und warum die *probabilistische Fehleramplifikation* (PEA) für Experimente im Utility-Maßstab bevorzugt wird.
- Wie man ZNE mit PEA in der Praxis mit Qiskit implementiert.

## Voraussetzungen {#prerequisites}
Wir empfehlen, dass du dich mit den folgenden Themen vertraut machst, bevor du dieses Tutorial durcharbeitest:
- Die [Lektion zur Fehlerminderung](/learning/courses/utility-scale-quantum-computing/error-mitigation) des Kurses *Quantum Computing im Utility-Maßstab* für grundlegendes Wissen zur Nutzung von Fehlerminderung in Qiskit.
- Die [Utility-I-Lektion](/learning/courses/utility-scale-quantum-computing/utility-i) des Kurses *Quantum Computing im Utility-Maßstab* für mehr Hintergrundwissen zu dem in diesem Tutorial verwendeten Experiment im Utility-Maßstab.

## Hintergrund {#background}

Dieses Tutorial zeigt, wie du mit IBM Quantum Compute Service ein Fehlerminderungsexperiment im Utility-Maßstab durchführst – unter Verwendung einer experimentellen Version der *Zero-Noise-Extrapolation* (ZNE) mit *probabilistischer Fehleramplifikation* (PEA).
...
**Referenz**: Y. Kim et al. *Evidence for the utility of quantum computing before fault tolerance.* [Nature 618.7965 (2023)](https://www.nature.com/articles/s41586-023-06096-3)
...
$$
wobei $\lambda$ die Rauschstärke parametrisiert und verstärkt werden kann.
ZNE lässt sich mit den folgenden Schritten umsetzen:

wobei $\lambda$ die Rauschstärke parametrisiert und verstärkt werden kann.

Wir können ZNE mit den folgenden Schritten implementieren:
...
#### Rauschen für ZNE verstärken {#amplify-noise-for-zne}
...
* Pulse Stretching geht davon aus, dass das Gate-Rauschen proportional zur Dauer ist, was typischerweise nicht zutrifft. Die Kalibrierung ist zudem kostspielig.

* Gate Folding erfordert große Stretch-Faktoren, die die Tiefe der ausführbaren Schaltungen stark einschränken.

* PEA kann auf jede Schaltung angewendet werden, die mit nativem Rauschfaktor ($\lambda=1$) ausgeführt werden kann, erfordert jedoch das Lernen des Rauschmodells.


### Das Rauschmodell für PEA erlernen {#learn-the-noise-model-for-pea}
PEA geht vom gleichen schichtbasierten Rauschmodell wie *probabilistic error cancellation* (PEC) aus, vermeidet jedoch den Sampling-Overhead, der exponentiell mit dem Schaltungsrauschen skaliert.
...
__Referenz__: E. van den Berg, Z. Minev, A. Kandala, and K. Temme, _Probabilistic error cancellation with sparse Pauli-Lindblad models on noisy quantum processors_ [arXiv:2201.09866](https://arxiv.org/abs/2201.09866)
...
## Setup {#setup}
In der folgenden Zelle importieren wir relevante Pakete und erstellen einige Hilfsfunktionen, um die Schaltungen für die trotterisierte Zeitentwicklung eines zweidimensionalen Transversalfeld-Ising-Modells zu konstruieren, das der Topologie des Backends entspricht.
...
## Beispiel im kleinen Maßstab mit Simulator {#small-scale-simulator-example}
Diesen Schritt lassen wir aus, da Laufzeit-Fehlerminderung auf Simulatoren nicht unterstützt wird.

## Beispiel im großen Maßstab auf Hardware {#large-scale-hardware-example}

### Schritt 1: Klassische Eingaben auf ein Quantenproblem abbilden {#step-1-map-classical-inputs-to-a-quantum-problem}

#### Einen parametrisierten Ising-Modell-Circuit erstellen {#create-a-parameterized-ising-model-circuit}

##### Backend festlegen {#establish-a-backend}
...
##### Kopplungen der Verschränkungsschichten definieren {#define-entangling-layer-couplings}
...
##### Schlechte Qubits entfernen {#remove-bad-qubits}
...
![Ausgabe der vorherigen Code-Zelle](/docs/images/tutorials/probabilistic-error-amplification/extracted-outputs/fccef708-0.avif)
...
##### Hauptgenerierung des Trotter-Circuits {#main-trotter-circuit-generation}
...
#### Eine Liste von Parameterwerten für die spätere Zuweisung erstellen {#create-a-list-of-parameter-values-to-be-assigned-later}
...
* Wähle ein Qubit-Layout, das die virtuellen Qubits deiner Schaltung auf physische Qubits der Hardware abbildet.

* Füge bei Bedarf Swap-Gates ein, um Interaktionen zwischen nicht verbundenen Qubits zu routen.

* Übersetze die Gates in unserer Schaltung in [Instruction Set Architecture (ISA)](/guides/transpile#instruction-set-architecture)-Instruktionen, die direkt auf der Hardware ausgeführt werden können.

* Führe Schaltungsoptimierungen durch, um die Schaltungstiefe und die Gate-Anzahl zu minimieren.
...
Beachte, dass jede Observable im folgenden Code-Block in einer Liste verpackt ist. Dies dient dazu, mit den Parameterwerten zu _broadcasen_, sodass jede Qubit-Observable für jeden Theta-Wert gemessen wird. Die Broadcasting-Regeln für Primitive findest du [hier](/guides/primitives).
...
#### Estimator-Optionen konfigurieren {#configure-estimator-options}
...
##### Erklärung der ZNE-Optionen {#explanation-of-zne-options}
...
* **amplifier**: Die Methode zur Verstärkung des Rauschens auf die gewünschten Rauschfaktoren.
  Erlaubte Werte sind `"gate_folding"`, das durch Wiederholen von Zwei-Qubit-Basis-Gates verstärkt,
  und `"pea"`, das durch probabilistisches Sampling nach dem Erlernen des Pauli-getwirbelten
  Rauschmodells für Schichten von getwirbelten Zwei-Qubit-Basis-Gates verstärkt. Es gibt auch die Optionen `"gate_folding_front"` und `"gate_folding_back"`, die in der [API-Dokumentation](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options#amplifier) erläutert werden.

* **extrapolated\_noise\_factors**: Gib einen oder mehrere Rauschfaktorwerte an, für die die extrapolierten Modelle ausgewertet werden sollen. Bei einer Folge von Werten sind die zurückgegebenen Ergebnisse array-wertig mit dem angegebenen Rauschfaktor, der für das Extrapolationsmodell ausgewertet wird. Ein Wert von 0 entspricht der Zero-Noise-Extrapolation.


#### Das Experiment ausführen {#run-the-experiment}
...
### Schritt 4: Nachverarbeiten und Ergebnis im gewünschten klassischen Format zurückgeben {#step-4-post-process-and-return-result-in-desired-classical-format}
...
#### Allgemeine Ergebnisformen und Metadaten {#general-result-shapes-and-metadata}
...
### Trotter-Simulationsergebnisse darstellen {#plot-trotter-simulation-results}
...
### Extrapolationsergebnisse für einzelne Qubits darstellen {#plot-extrapolation-results-for-individual-qubits}
...
- Ein [Tutorial](/tutorials/combine-error-mitigation-techniques) mit dem Schwerpunkt auf der Kombination von Fehlerminderungstechniken.
- Ausführliche [Dokumentation](/guides/error-mitigation-and-suppression-techniques) zu den in Qiskit verfügbaren Fehlerminderungstechniken.
- Weitere Lektionen zu Experimenten im Utility-Maßstab: [Utility II](/learning/courses/utility-scale-quantum-computing/utility-ii) und [Utility III](/learning/courses/utility-scale-quantum-computing/utility-iii).
</Admonition>
