---
description: 'Verwende Qunova HiVQE, um Dissoziations-Potenzialenergieflächenkurven (PES) zu berechnen.'
notebook_path: tutorials/qunova-hivqe.ipynb
sidebar_label: 'Dissoziations-PES-Kurven mit Qunova HiVQE'
title: 'Dissoziations-PES-Kurven mit Qunova HiVQE'
---
...
Qiskit Functions sind ein experimentelles Feature, das nur für Nutzer des IBM Quantum&reg; Premium Plan, Flex Plan und On-Prem (über die IBM Quantum Platform API) Plan verfügbar ist. Sie befinden sich im Vorschau-Status und können sich ändern.
</Admonition>
...
Definiere $Li_2S$-Geometrien im Dictionary-Format für verschiedene Bindungsabstände von Li-S, um die PES-Kurve zu berechnen. Diese Geometrien wurden mit B3LYP/631g-Berechnungen optimiert.
...
### Schritte 2 und 3: Problem für die Quantenhardwareausführung optimieren und mit der HiVQE-Chemistry-Funktion ausführen {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function}
...
Die gesamte im Job verwendete QPU-Laufzeit lässt sich verfolgen, indem du dich bei [IBM Quantum Platform](https://quantum.cloud.ibm.com/) anmeldest und eingereichte Jobs mit dem Tag `qunova-chemistry-hivqe` ansiehst.

### Schritt 4: Nachbearbeitung und Vergleich mit klassischen Methoden {#step-4-post-process-and-compare-with-classical-methods}
...
### Darstellung der Dissoziationskurve für Li\_2S {#plotting-the-dissociation-curve-for-li-2s}

Lass uns die HiVQE-Ergebnisse mit HF und CASCI grafisch darstellen und vergleichen.
Du kannst beobachten, dass alle HiVQE-Berechnungen gut mit dem klassischen Referenzergebnis (CASCI) übereinstimmen.
...
![Ausgabe der vorherigen Codezelle](/docs/images/tutorials/qunova-hivqe/extracted-outputs/2db64a7d-0.avif)

## Teil 2: FeP-NO (44Q) {#part-2-fep-no-44q}

Beachte, dass du zur Ausführung der Funktion für das FeP-NO-System mit den in diesem Beispiel gezeigten Einstellungen eine Lizenz benötigst, die die Nutzung der Funktion mit mindestens 44 Qubits erlaubt. Wende dich per E-Mail an [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com), um eine Lizenz zu erhalten.
...
### Schritte 2 und 3: Problem für die Quantenhardwareausführung optimieren und mit der HiVQE-Chemistry-Funktion ausführen {#step-2-and-3-optimize-problem-for-quantum-hardware-execution-and-execute-using-the-hivqe-chemistry-function-1}

Basierend auf der HiVQE-Einrichtung und den Geometrien werden die Ergebnisse sequenziell ermittelt.

#### Einreichen der Berechnung für d(Fe-N) = 1,75 $\AA$. {#submit-dfe-n-175-aa-calculation}
...
Job verfolgen und Ergebnis für d(Fe-N) = 1,75 $\AA$ abrufen.
...
#### Einreichen der Berechnung für d(Fe-N) = 2,00 $\AA$. {#submit-dfe-n-200-aa-calculation}
...
Job verfolgen und Ergebnis für d(Fe-N) = 2,00 $\AA$ abrufen.
...
#### Einreichen der Berechnung für d(Fe-N) = 5,00 $\AA$. {#submit-dfe-n-500-aa-calculation}
...
Job verfolgen und Ergebnis für d(Fe-N) = 5,00 $\AA$ abrufen.
...
### Schritt 4: Nachbearbeitung und Vergleich mit klassischen Methoden {#step-4-post-process-and-compare-with-classical-methods-1}
...
![Ausgabe der vorherigen Codezelle](/docs/images/tutorials/qunova-hivqe/extracted-outputs/49a9bf81-1.avif)

## Tutorial-Umfrage {#tutorial-survey}

Bitte nimm dir einen Moment für diese kurze Umfrage, um Feedback zu diesem Tutorial zu geben. Deine Anmerkungen helfen uns, unsere Inhalte und die Nutzererfahrung zu verbessern.

> **Hinweis:** Diese Umfrage stammt von **IBM Quantum** und bezieht sich auf den **Tutorial-Inhalt** (verfasst von IBM). doQumentation stellt die Website, Übersetzungen und Code-Ausführung bereit – für Feedback dazu bitte [ein GitHub-Issue öffnen](https://github.com/JanLahmann/doQumentation/issues).

[Link zur Umfrage](https://your.feedback.ibm.com/jfe/form/SV_9AizRyKFHEGzVs2)
