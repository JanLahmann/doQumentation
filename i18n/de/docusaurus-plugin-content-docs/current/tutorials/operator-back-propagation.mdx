...
*Geschätzter Verbrauch: 16 Minuten auf einem Eagle-r3-Prozessor (HINWEIS: Dies ist nur eine Schätzung. Ihre tatsächliche Laufzeit kann abweichen.)*
...
Beachten Sie, dass die Funktion ``backpropagate`` jeweils ganze Schaltkreis-Schichten zurückpropagiert. Daher kann die Wahl der Aufteilung einen Einfluss darauf haben, wie gut die Rückpropagation für ein gegebenes Problem funktioniert. Hier gruppieren wir Gatter gleichen Typs in Schichten mithilfe der Funktion [slice_by_gate_types](https://docs.quantum.ibm.com/api/qiskit-addon-utils/slicing#slice_by_gate_types).

Für eine ausführlichere Diskussion zur Schaltkreisaufteilung lesen Sie diese [Anleitung](https://qiskit.github.io/qiskit-addon-utils/how_tos/create_circuit_slices.html) des Pakets [qiskit-addon-utils](https://github.com/Qiskit/qiskit-addon-utils).
...
Im Folgenden werden Sie sehen, dass wir sechs Schichten zurückpropagiert haben und die Terme in sechs und nicht acht Gruppen zusammengefasst wurden. Dies impliziert, dass das Zurückpropagieren einer weiteren Schicht dazu führen würde, dass die Anzahl der Pauli-Gruppen acht überschreitet. Wir können dies überprüfen, indem wir die zurückgegebenen Metadaten untersuchen. Beachten Sie auch, dass in diesem Abschnitt die Schaltkreistransformation exakt ist. Das heißt, es wurden keine Terme der neuen Observable $O'$ abgeschnitten. Der zurückpropagierte Schaltkreis und der zurückpropagierte Operator liefern exakt das gleiche Ergebnis wie der ursprüngliche Schaltkreis und Operator.
...
Als Nächstes definieren wir dasselbe Problem mit denselben Einschränkungen für die Größe der Ausgangs-Observable. Diesmal weisen wir jedoch jeder Schicht ein Fehlerbudget mithilfe der Funktion [setup_budget](https://docs.quantum.ibm.com/api/qiskit-addon-obp/utils-truncating#setup_budget) zu. Pauli-Terme mit kleinen Koeffizienten werden aus jeder Schicht abgeschnitten, bis das Fehlerbudget aufgebraucht ist, und das verbleibende Budget wird dem Budget der nächsten Schicht hinzugefügt. Beachten Sie, dass in diesem Fall die Transformation durch die Rückpropagation approximativ ist, da einige der Terme im Operator abgeschnitten werden.
...
Für eine weiterführende Diskussion zur Verteilung eines Fehlerbudgets über Ihre Schichten lesen Sie [diese Anleitung](https://qiskit.github.io/qiskit-addon-obp/how_tos/truncate_operator_terms.html).
...
Lassen Sie uns zunächst den zurückpropagierten Schaltkreis und die Observable ohne jegliche Trunkierung erhalten.
...
Bitte nehmen Sie an dieser kurzen Umfrage teil, um Feedback zu diesem Tutorial zu geben. Ihre Erkenntnisse helfen uns, unsere Inhalte und das Nutzererlebnis zu verbessern.
