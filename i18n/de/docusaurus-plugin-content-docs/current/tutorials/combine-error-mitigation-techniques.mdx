---
description: 'Kombiniere Fehlerminderungsoptionen für Utility-Scale-Experimente mit 100Q+ IBM Quantum QPUs und dem IBM Quantum Estimator-Primitive.'
...
- Die Grundlagen von Dynamical Decoupling, Messfehlerkompensation, Gate Twirling und Zero-Noise Extrapolation, wie in diesem [Leitfaden](/guides/error-mitigation-and-suppression-techniques) beschrieben.
...
Dieses Tutorial untersucht die Fehlerunterdrückungs- und Fehlerminderungsoptionen, die mit dem IBM Quantum Estimator-Primitive verfügbar sind. Dieses Tutorial zeigt, wie jede der folgenden Methoden einzeln implementiert wird:
...
Beachte, dass eine Alternative zur individuellen Implementierung dieser Techniken deren Implementierung über ein [Resilience-Level](/guides/estimator-noise-management) ist, wobei `resilience_level` die Werte 0, 1, 2 annimmt:
...
Als unsere Observable nehmen wir den Pauli-$Z$-Operator, der auf das letzte Qubit wirkt, $Z I \cdots I$.
Beachte, dass die Tatsache, dass das letzte Qubit dem ersten Element dieser Zeichenkette entspricht, auf Qiskits Verwendung der Little-Endian-Notation zurückzuführen ist.
...
Die transpilierte Schaltung enthält jetzt nur noch ISA-Instruktionen. Alle Gates wurden in Bezug auf $\sqrt{X}$-Gates und $R_z$-Rotationen sowie [CZ-Gates](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.CZGate) zerlegt.
...
Hier reichst du fünf separate Jobs ein, beginnend ohne Fehlerunterdrückung oder -minderung, und aktivierst sukzessive verschiedene Fehlerunterdrückungs- und -minderungsoptionen, die im IBM Quantum Compute Service verfügbar sind. Informationen zu den Optionen findest du auf den folgenden Seiten:
...
Da diese Jobs unabhängig voneinander ausgeführt werden können, kannst du den [Batch-Modus](/guides/run-jobs-batch) verwenden, um Quantum Compute die Optimierung des Timings ihrer Ausführung zu ermöglichen.
...
- [Matrix-free Measurement Mitigation (M3)](https://qiskit.github.io/qiskit-addon-mthree/) (siehe auch das Tutorial [Readout error mitigation for the Sampler primitive using M3](/tutorials/readout-error-mitigation-sampler/))
