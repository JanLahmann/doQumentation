...
* Wie man ein weitreichendes CNOT-Gate mithilfe dynamischer Circuits mit Mid-Circuit-Messungen (MCMs) und klassischem Feedforward implementiert;
* Wie man das äquivalente Gate mit einem unitären SWAP-basierten Ansatz implementiert;
* Wie man beide Ansätze durch Messen der Gate-Fidelity als Funktion des Qubit-Abstands vergleicht.
## Voraussetzungen {#prerequisites}

Wir empfehlen, dass du vor diesem Tutorial mit folgenden Themen vertraut bist:
* [Grundlegende Quantencomputing-Konzepte](/learning/courses/basics-of-quantum-information), einschließlich Bell-Zustände, Verschränkung und Quantengates;
* Vertrautheit mit [dynamischen Circuits](/guides/classical-feedforward-and-control-flow) (Mid-Circuit-Messungen und klassisches Feedforward);
* Grundkenntnisse des [Qiskit SDK](https://docs.quantum.ibm.com/guides) und [Qiskit Runtime](/guides/compute-services#qiskit-runtime) sowie Zugang zu einem [IBM Quantum&reg;-Konto](/guides/cloud-setup).
...
In diesem Notebook passen wir das Protokoll für IBM Quantum-Hardware an und benchmarken seine Performance als Funktion des Kontroll-Ziel-Abstands, wobei wir es mit einer unitären SWAP-basierten Baseline vergleichen.
...
- Qiskit Runtime v0.37 oder neuer (`pip install qiskit-ibm-runtime`)
...
Bevor wir auf dem echten QPU ausführen, überprüfen wir, dass sowohl der dynamische als auch der unitäre Circuit einen idealen Bell-Zustand auf einem rauschfreien Simulator erzeugen. Wir verwenden den Qiskit Runtime `Sampler` mit `AerSimulator` als Backend-Modus, bei einem kleinen Abstand von 6.
...
#### Circuit initialisieren {#initialize-circuit}
...
#### Bell-Paare vorbereiten {#prepare-bell-pairs}
...
#### Benachbarte Qubit-Paare in der Bell-Basis messen {#measure-neighboring-qubit-pairs-in-the-bell-basis}
...
#### Feedforward-Korrekturen anwenden, um Pauli-Nebenprodukte zu korrigieren {#apply-feedforward-corrections-to-correct-pauli-byproduct-operators}
...
#### Kontroll- und Ziel-Qubit messen {#measure-control-and-target-qubits}
...
- Initialisierung des Kontroll-Qubits in $|+\rangle$
...
Nun werden die Circuits erstellt, die in der $XX$-, $YY$- und $ZZ$-Basis messen — genau wie bei den dynamischen Circuits oben.
...
![Ausgabe der vorherigen Code-Zelle](/docs/images/tutorials/long-range-entanglement/extracted-outputs/b899e143-0.avif)

Nachdem wir nun sowohl die dynamischen als auch die unitären Circuits für ein kleinskaliges Beispiel mit `distance=6` erstellt haben, transpilieren wir diese, um sie zunächst auf einem rauschfreien Simulator auszuführen.
...
Wir können das Experiment jetzt auf dem rauschfreien Simulator-Backend ausführen. Wir verwenden den Qiskit Runtime Sampler mit AerSimulator als Backend-Modus, um die Circuits auszuführen.
...
In einer rauschfreien Simulation werden wir überprüfen, dass die Fidelity-Metrik für die konstruierten Circuits $1$ ist. In den Experimenten auf den echten QPUs liefert diese Analyse ein klares Bild davon, wie gut die dynamischen Circuits im Vergleich zur unitären Basisimplementierung abschneiden.
...
Wie in einer rauschfreien Simulation erwartet, beträgt die Fidelity bei dynamischen und unitären Circuits jeweils $1$.
## Großskaliges Hardware-Beispiel {#large-scale-hardware-example}

Hier setzen wir all diese Details in einem einzigen Workflow in größerem Maßstab zusammen und führen ihn auf echter Quantenhardware aus.
### Circuits für verschiedene Abstände generieren {#generate-circuits-for-different-distances}

Wir generieren jetzt weitreichende CX-Circuits für eine Reihe von Qubit-Abständen bis zu 60 Qubits. Für jeden Abstand erstellen wir Circuits, die in der $XX$-, $YY$- und $ZZ$-Basis messen — diese werden später zur Berechnung der Fidelity verwendet.
...
Nachdem wir nun sowohl dynamische als auch unitäre Circuits für eine Reihe von Abständen haben, sind wir für die Transpilierung bereit. Wir müssen zunächst ein Backend-Gerät auswählen.
...
Falls du diese Arbeit interessant findest, könnten dich folgende Materialien interessieren:
