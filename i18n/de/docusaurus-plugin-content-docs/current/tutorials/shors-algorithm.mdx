...
- Den mathematischen Hintergrund von Shors Algorithmus zur ganzzahligen Faktorisierung
- Wie man ein Beispiel dieses Algorithmus auf echter Hardware ausführt

## Voraussetzungen {#prerequisites}
Wir empfehlen, dass Nutzer mit den folgenden Themen vertraut sind, bevor sie dieses Tutorial durcharbeiten:
- [Grundlagen quantenalgorithmen](/learning/courses/fundamentals-of-quantum-algorithms).
- [Phasenschätzung und Faktorisierung](/learning/courses/fundamentals-of-quantum-algorithms/phase-estimation-and-factoring/introduction). Einen Teil dieses Materials behandeln wir in diesem Tutorial.
...
Beim Problem der Phasenschätzung erhalten wir einen Quantenzustand $\ket{\psi}$ von $n$ Qubits sowie eine unitäre Quantenschaltung, die auf $n$ Qubits wirkt. Uns wird zugesichert, dass $\ket{\psi}$ ein Eigenvektor der unitären Matrix $U$ ist, die die Wirkung der Schaltung beschreibt, und unser Ziel ist es, den Eigenwert $\lambda = e^{2 \pi i \theta}$, dem $\ket{\psi}$ entspricht, zu berechnen oder zu approximieren. Mit anderen Worten: Die Schaltung soll eine Näherung an die Zahl $\theta \in [0, 1)$ ausgeben, die $$U \ket{\psi}= e^{2 \pi i \theta} \ket{\psi}.$$ erfüllt.
Das Ziel der Phasenschätzungsschaltung ist es, $\theta$ auf $m$ Bits genau zu approximieren. Mathematisch ausgedrückt möchten wir $y$ so finden, dass $\theta \approx y / 2^m$, wobei $y \in {0, 1, 2, \dots, 2^{m-1}}$. Das folgende Bild zeigt die Quantenschaltung, die $y$ auf $m$ Bits schätzt, indem eine Messung an $m$ Qubits durchgeführt wird.
![Schaltung zur Quantenphasenschätzung](/learning/images/courses/fundamentals-of-quantum-algorithms/phase-estimation-and-factoring/phase-estimation-procedure.svg)
In der obigen Schaltung werden die oberen $m$ Qubits im Zustand $\ket{0^m}$ initialisiert, und die unteren $n$ Qubits werden in $\ket{\psi}$ initialisiert, von dem zugesichert wird, dass es ein Eigenvektor von $U$ ist. Der erste Baustein der Phasenschätzungsschaltung sind die kontrolliert-unitären Operationen, die dafür verantwortlich sind, einen *Phase-Kickback* auf das jeweils zugehörige Kontroll-Qubit auszuführen. Diese kontrollierten Unitären werden entsprechend der Position des Kontroll-Qubits potenziert, vom niedrigstwertigen bis zum höchstwertigen Bit. Da $\ket{\psi}$ ein Eigenvektor von $U$ ist, wird der Zustand der unteren $n$ Qubits durch diese Operation nicht beeinflusst, aber die Phaseninformation des Eigenwerts pflanzt sich zu den oberen $m$ Qubits fort.
Es stellt sich heraus, dass nach der Phase-Kickback-Operation mittels kontrollierter Unitärer alle möglichen Zustände der oberen $m$ Qubits für jeden Eigenvektor $\ket{\psi}$ der Unitären $U$ zueinander orthonormal sind. Diese Zustände sind daher perfekt unterscheidbar, und wir können die von ihnen gebildete Basis zurück in die Rechenbasis rotieren, um eine Messung durchzuführen. Eine mathematische Analyse zeigt, dass diese Rotationsmatrix der inversen Quanten-Fourier-Transformation (QFT) im $2^m$-dimensionalen Hilbertraum entspricht. Die Intuition dahinter ist, dass die periodische Struktur der modularen Exponentiationsoperatoren im Quantenzustand codiert ist und die QFT diese Periodizität in messbare Spitzen im Frequenzbereich umwandelt.
...
Wir haben gesehen, dass $M_b$-Operatoren für ein gegebenes $b \in \mathbb{Z}^*_N$ Permutationsoperationen sind. Aufgrund der relativ geringen Größe des Permutationsproblems, das wir hier haben, da $N=15$ nur vier Qubits erfordert, konnten wir diese Operationen direkt durch Betrachtung mit ``SWAP``-Gates synthetisieren. Im Allgemeinen ist dies möglicherweise kein skalierbarer Ansatz. Stattdessen müssen wir die Permutationsmatrix möglicherweise explizit konstruieren und Qiskits ``UnitaryGate``-Klasse sowie Transpilationsmethoden verwenden, um diese Permutationsmatrix zu synthetisieren. Dies kann jedoch zu deutlich tieferen Circuits führen. Ein Beispiel folgt.
...
Bisher haben wir besprochen, wie sich das Problem der Ordnungsbestimmung mit einer Phasenschätzungsschaltung implementieren lässt. Nun verbinden wir das Problem der Ordnungsbestimmung mit der Faktorisierung ganzer Zahlen, wodurch Shors Algorithmus vervollständigt wird. Beachte, dass dieser Teil des Algorithmus klassisch ist.
Wir demonstrieren dies nun anhand unseres Beispiels mit $N = 15$ und $a = 2$. Erinnere dich daran, dass die gemessene Phase $k / r$ ist, wobei $$a^r \; (\textrm{mod} \; N) = 1$$ gilt und $k$ eine zufällige ganze Zahl zwischen $0$ und $r - 1$ ist. Aus dieser Gleichung folgt $$(a^r - 1) \; (\textrm{mod} \; N) = 0,$$ was bedeutet, dass $N$ $a^r-1$ teilen muss. Wenn $r$ außerdem gerade ist, können wir schreiben $$a^r -1 = (a^{r/2}-1)(a^{r/2}+1).$$ Ist $r$ nicht gerade, kommen wir nicht weiter und müssen es mit einem anderen Wert für $a$ erneut versuchen; andernfalls besteht eine hohe Wahrscheinlichkeit, dass der größte gemeinsame Teiler von $N$ und entweder $a^{r/2}-1$ oder $a^{r/2}+1$ ein echter Teiler von $N$ ist.
