...
Wähle einen Referenzzustand $\vert \psi \rangle$, der eine gewisse Überlappung mit dem Grundzustand aufweist. Für diesen Hamiltonoperator verwenden wir einen Zustand mit einer Anregung im mittleren Qubit $\vert 00..010...00 \rangle$ als unseren Referenzzustand.
...
Dabei ist $P$ einer der Terme in der Zerlegung des Hamiltonoperators $H=\sum P$ und $\text{Prep} \; \psi_i$, $\text{Prep} \; \psi_j$ sind kontrollierte Operationen, die die $|\psi_i\rangle$-, $|\psi_j\rangle$-Vektoren des unitären Krylov-Raums vorbereiten, wobei $|\psi_k\rangle = e^{-i H k dt } \vert \psi \rangle = e^{-i H k dt } U_{\psi} \vert 0 \rangle^N$. Um $X$ zu messen, wende zunächst $H$ an...
...
Nimm an, dass wir $E_0$, den Eigenwert von $|0\rangle^N$ unter dem Hamiltonoperator $H$, klassisch berechnen können.
...
Führe die Schaltkreise für $S$ und $\tilde{H}$ mit Estimator aus.
...
Berechne zunächst die Phase, die der Zustand $\vert 0 \rangle$ während der unkontrollierten Zeitentwicklung akkumuliert
...
für einen Zeitschritt $dt$, den wir später bestimmen werden. Nimm vorübergehend an, dass $r$ gerade ist: dann definiere $d=r/2$. Beachte, dass wenn wir den Hamilton-Operator in den Krylov-Raum oben projizieren, er nicht vom Krylov-Raum
...
Wir geben unten einen Beweis an, der jedoch sicher übersprungen werden kann, es sei denn, man möchte das vollständige, rigorose Argument verstehen. Zunächst konzentrieren wir uns auf die Implikationen der obigen Behauptung. Aus Eigenschaft 3 oben können wir erkennen, dass der verschobene Krylov-Raum oben den Zustand $f(H)|\psi\rangle$ enthält. Dies ist unser niederenergetischer Zustand. Um zu sehen warum, schreibe $|\psi\rangle$ in der Energieeigenbasis:
...
Zur Vereinfachung des Restes beachte, dass für all diese $E_k$ nach der Definition von $f$ gilt: $f(E_k)^2 \le 4\left(1 + \delta\right)^{-2d}$. Zusätzlich ergibt die obere Abschätzung $E_k-E_0<2\|H\|$ und die obere Abschätzung $\sum_{E_k>E_0+\delta}|\gamma_k|^2<1$

$$
\text{energy error} \le \delta + \frac{8}{|\gamma_0|^2}\|H\|\left(1 + \delta\right)^{-2d}.
$$

Dies gilt für jedes $\delta>0$, wenn wir also $\delta$ gleich unserem Zielfehler setzen, konvergiert die obige Fehlerschranke exponentiell mit der Krylov-Dimension $2d=r$ gegen diesen Wert. Beachte auch, dass wenn $\delta<E_1-E_0$, der $\delta$-Term in der obigen Schranke tatsächlich vollständig verschwindet.
...
Dazu ist es zweckmäßig, die folgende Transformation von Energien innerhalb des Spektralbereichs des Hamilton-Operators auf Zahlen im Bereich $[0,1]$ einzuführen: Definiere
...
Beachte, dass $g(E_0)=0$ und $g(E)$ wächst, wenn sich $E$ von $E_0$ entfernt.
...
[Link zur Umfrage](https://your.feedback.ibm.com/jfe/form/SV_82nennpKIjjD8rQ)
