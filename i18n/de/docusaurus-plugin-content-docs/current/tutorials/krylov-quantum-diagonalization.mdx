...
Hier erstellen wir die Schaltungen zur Schätzung der Matrixelemente $\mathcal{H}_{k\ell}$ und $\mathcal{S}_{k\ell}$.
Da alle Potenzen von $H$ kommutieren, haben wir
$$
\begin{equation*}
\mathcal{H}_{k\ell}
=
...
\end{equation*}
$$
Solche Matrizen, bei denen die Elemente auf diese Weise von den Indexdifferenzen abhängen, werden *Toeplitz*-Matrizen genannt und können aus den mit $d=\ell-k$ indizierten Elementen der ersten Zeile rekonstruiert werden.
...
Um den Trotter-Fehler zu messen, verwenden wir die **Prozess-Infidelity** zwischen dem trotterisierten Circuit und der exakten Hamiltonian-Entwicklung,
$$
\begin{equation*}
\text{Infidelity}(U,V)
=
...
Da der Hamiltonian die Anregungszahl erhält, sehen wir, dass $|0^n\rangle$ sein Eigenzustand ist, und somit
akkumuliert der Entwicklungsoperator unter dem Hamiltonian nur eine Phase:
$$
\begin{equation*}
U(t)|0^n\rangle
=
