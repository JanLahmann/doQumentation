...
*Geschätzte Nutzungsdauer: unter einer Minute auf einem Heron-r2-Prozessor (HINWEIS: Dies ist nur eine Schätzung. Ihre tatsächliche Laufzeit kann abweichen.)*
...
1. Lernen Sie, SABRE mit **Qiskit-Patterns** für die fortgeschrittene Optimierung großer Schaltkreise zu verwenden.
2. Nutzen Sie **qiskit_serverless**, um das Potenzial von SABRE für skalierbare und effiziente Transpilation voll auszuschöpfen.
...
SABRE kann in Qiskit verwendet werden, um Quantenschaltkreise zu optimieren, indem es sowohl das Qubit-Layout als auch die Gate-Routing-Phase behandelt. In diesem Abschnitt führen wir Sie durch das **Minimalbeispiel** der Verwendung von SABRE mit Qiskit-Patterns, wobei der Schwerpunkt auf Schritt 2 der Optimierung liegt.

Um SABRE auszuführen, benötigen Sie:
- Eine **DAG**-Darstellung (Directed Acyclic Graph) Ihres Quantenschaltkreises.
...
In diesem Teil konzentrieren wir uns auf den **SabreLayout**-Pass. Er führt sowohl Layout- als auch Routing-Versuche durch und arbeitet daran, das effizienteste anfängliche Layout zu finden und gleichzeitig die Anzahl der benötigten SWAP-Gates zu minimieren. Wichtig ist, dass `SabreLayout` allein intern sowohl das Layout als auch das Routing optimiert, indem es die Lösung speichert, die die geringste Anzahl von SWAP-Gates hinzufügt. Beachten Sie, dass bei alleiniger Verwendung von **SabreLayout** die Heuristik von SABRE nicht geändert werden kann, aber die Anzahl der `layout_trials` angepasst werden kann.
...
Erhöhen Sie `layout_trials` und `swap_trials`, um eine gründlichere Optimierung durchzuführen, allerdings auf Kosten einer erhöhten Transpilationszeit.
...
Neben der Anpassung der Anzahl der Versuche unterstützt SABRE die Anpassung der Routing-Heuristik, die während der Transpilation verwendet wird. Standardmäßig verwendet `SabreLayout` die Decay-Heuristik, die Qubits dynamisch basierend auf ihrer Wahrscheinlichkeit, getauscht zu werden, gewichtet. Um eine andere Heuristik (wie die `lookahead`-Heuristik) zu verwenden, können Sie einen benutzerdefinierten `SabreSwap`-Pass erstellen und ihn mit `SabreLayout` verbinden, indem Sie einen `PassManager` mit `FullAncillaAllocation`, `EnlargeWithAncilla` und `ApplyLayout` ausführen. Wenn `SabreSwap` als Parameter für `SabreLayout` verwendet wird, wird standardmäßig nur ein Layout-Versuch durchgeführt. Um mehrere Layout-Versuche effizient auszuführen, nutzen wir die Serverless-Laufzeitumgebung zur Parallelisierung. Weitere Informationen zu Serverless finden Sie in der [Serverless-Dokumentation](/guides/serverless).

### So ändern Sie die Routing-Heuristik {#how-to-change-the-routing-heuristic}
1. Erstellen Sie einen benutzerdefinierten `SabreSwap`-Pass mit der gewünschten Heuristik.
2. Verwenden Sie diesen benutzerdefinierten `SabreSwap` als Routing-Methode für den `SabreLayout`-Pass.
...
Hier sehen wir, dass die `lookahead`-Heuristik in Bezug auf Schaltkreistiefe, -größe und -zeit besser abschneidet als die `decay`-Heuristik. Diese Verbesserungen zeigen, wie wir SABRE über bloße Versuche und Iterationen hinaus für Ihren spezifischen Schaltkreis und Ihre Hardware-Einschränkungen optimieren können. Beachten Sie, dass diese Ergebnisse auf einem einzelnen Layout-Versuch basieren. Um genauere Ergebnisse zu erzielen, empfehlen wir die Durchführung mehrerer Layout-Versuche, was effizient mithilfe der Serverless-Laufzeitumgebung erfolgen kann.
...
Qiskit Serverless erfordert die Einrichtung der `.py`-Dateien Ihrer Arbeitslast in einem dedizierten Verzeichnis. Die folgende Codezelle ist eine Python-Datei im Verzeichnis `source_files` mit dem Namen `transpile_remote.py`. Diese Datei enthält die Funktion, die den Transpilationsprozess ausführt.
...
Generieren Sie 20 verschiedene Seeds, die 20 verschiedene Layout-Versuche repräsentieren.
...
Führen Sie das hochgeladene Programm aus und übergeben Sie die Eingaben für die Lookahead-Heuristik.
...
Empfangen Sie die Protokolle und Ergebnisse von der Serverless-Laufzeitumgebung.
...
Sobald ein Programm den Status `DONE` hat, können Sie `job.results()` verwenden, um das in `save_result()` gespeicherte Ergebnis abzurufen.
...
Führen Sie nun dasselbe für die Decay-Heuristik durch.
...
Rufen Sie die Ergebnisse von der Serverless-Laufzeitumgebung ab und vergleichen Sie die Ergebnisse der Lookahead- und Decay-Heuristiken. Wir werden die Größen und Tiefen vergleichen.
