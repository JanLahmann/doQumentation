...
![Ausgabe der vorherigen Codezelle](/learning/images/courses/variational-algorithm-design/examples-and-applications/extracted-outputs/e4d59381-43ff-4322-a6f2-df55cef18536-0.avif)

Beginnen wir damit, das folgende Observable zu untersuchen:
...
Wir verwenden die folgende Funktion, um die Overlap-Penalty zu berechnen. Beachte, dass dies immer noch Teil der Abbildung des Problems auf Quantenschaltkreise ist. Wie in der vorherigen Lektion besprochen, berechnet diese Funktion die Überlappung zwischen einem aktuellen variationellen Circuit und dem optimierten Circuit aus einem zuvor erhaltenen Zustand mit niedrigerer Energie/niedrigeren Kosten. Der neu erzeugte Circuit muss ebenfalls transpiliert werden, um auf echter Hardware zu laufen. Wir haben diese Funktion schon zuvor gesehen, dort auf einem Simulator eingesetzt. Hier müssen wir das Transpilieren und die zugehörige Optimierung bereits berücksichtigen, wenn wir ein echtes Backend verwenden, daher die Zeilen um `if realbackend == 1`. Dies vermischt sich etwas mit Schritt 2, aber wir werden Schritt 2 später explizit behandeln.
...
Jetzt fügen wir die Kostenfunktion von VQD hinzu. Beachte, dass wir im Vergleich zur vorherigen Lektion jetzt zwei zusätzliche Argumente (`realbackend` und `backend`) haben, die uns bei der Transpilierung helfen, wenn wir echte Backends verwenden.
...
Auch hier verwenden wir zunächst Simulatoren zum Debuggen und wechseln anschließend zu echter Hardware.
...
Diese Ergebnisse liegen, abgesehen vom Approximationsfehler und der globalen Phase, recht nahe an den erwarteten Werten. Wir könnten die Toleranz des klassischen Optimierers und/oder die Strafterme für die Statevector-Überlappung anpassen, um genauere Werte zu erhalten.
...
### Schritt 3: Mit IBM Quantum Primitives ausführen {#step-3-execute-using-ibm-quantum-primitives}
...
Unser Ziel ist es, den Erwartungswert des Observablen zu minimieren, das die Energie repräsentiert (Hamiltonoperator $\hat{\mathcal{H}}$):
...
![Ausgabe der vorherigen Codezelle](/learning/images/courses/variational-algorithm-design/examples-and-applications/extracted-outputs/de006d72-db04-49d4-806b-89bd1e5b9947-0.avif)
