...
- Zunächst lernst du die [IBM Quantum Primitives](/guides/primitives) kennen
...
- Du legst eine Messstrategie mit den IBM Quantum Primitives fest, um Geschwindigkeit und Genauigkeit gegeneinander abzuwägen
...
Zum Glück gibt es einen Ausweg: Jede $2^n \times 2^n$-Matrix lässt sich als Linearkombination von $4^n$ Tensorprodukten von $n$ Pauli-Matrizen und Identitäten schreiben, die alle sowohl hermitesch als auch unitär sind und für die $V$ und $\Lambda$ bekannt sind. Genau das macht der IBM Quantum `Estimator` intern, indem er jedes [`Operator`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.Operator)-Objekt in ein [`SparsePauliOp`](https://docs.quantum.ibm.com/api/qiskit/qiskit.quantum_info.SparsePauliOp) zerlegt.
...
Jetzt führen wir die Berechnung auf einem echten Quantencomputer durch. Beachte die geänderte Syntax. Die Schritte mit dem pass_manager werden im nächsten Beispiel näher erläutert. Ein besonders wichtiger Schritt bei variationellen Algorithmen ist die Verwendung einer IBM Quantum Compute Service Session. Wenn du eine Session startest, kannst du mehrere Iterationen eines variationellen Algorithmus ausführen, ohne jedes Mal neu in die Warteschlange eingereiht zu werden, wenn Parameter aktualisiert werden. Das ist wichtig, wenn die Wartezeiten lang sind und/oder viele Iterationen benötigt werden. Beachte, dass Open-Plan-Nutzer keine Quantum Compute Sessions verwenden können. Falls du keinen Zugang zu Sessions hast, kannst du die Anzahl der Iterationen reduzieren und die zuletzt verwendeten Parameter für spätere Durchläufe speichern. Bei zu vielen Iterationen oder zu langen Wartezeiten kann Fehlercode 1217 auftreten, der auf zu große Verzögerungen zwischen Job-Einreichungen hinweist.
...
Da der IBM Quantum Estimator direkt einen Hamiltonian und einen parametrisierten Ansatz entgegennimmt und die benötigte Energie zurückgibt, ist die Kostenfunktion für eine QAOA-Instanz recht einfach:
...
Wir können die Fehlerunterdrückungs- und Fehlerminderungsoptionen der IBM Quantum Primitives nutzen, um Rauschen zu begegnen und den Nutzen heutiger Quantencomputer zu maximieren.

### Fehlerunterdrückung {#error-suppression}

[Fehlerunterdrückung](/guides/error-mitigation-and-suppression-techniques) bezeichnet Techniken zur Optimierung und Transformation eines Circuits während der Kompilierung, um Fehler zu minimieren. Das ist eine grundlegende Fehlerbehandlungstechnik, die in der Regel zu einem gewissen klassischen Vorverarbeitungs-<DefinitionTooltip definition="Zusätzliche Kosten, die durch neue Techniken im Vergleich zu einer Basisimplementierung entstehen.">Overhead</DefinitionTooltip> führt. Dieser Overhead umfasst das Transpilieren von Circuits für die Ausführung auf Quantenhardware durch:
...
[Fehlerminderung](/guides/error-mitigation-and-suppression-techniques) bezeichnet Techniken, die es ermöglichen, Circuit-Fehler zu reduzieren, indem das Geräterauschen zum Ausführungszeitpunkt modelliert wird. In der Regel entsteht dabei ein Quantum-Vorverarbeitungs-Overhead für das Modelltraining sowie ein klassischer Nachverarbeitungs-Overhead, um Fehler in den Rohergebnissen mithilfe des erstellten Modells zu korrigieren.

Die Option `resilience_level` der IBM Quantum Primitives gibt an, wie viel Widerstandsfähigkeit gegenüber Fehlern aufgebaut werden soll. Höhere Stufen erzeugen genauere Ergebnisse, erfordern aber aufgrund des Quantum-Sampling-Overheads mehr Verarbeitungszeit. Mit den Resilience-Stufen lässt sich der Kompromiss zwischen Kosten und Genauigkeit beim Einsatz von Fehlerminderung steuern.
...
In diesem Kurs betrachten wir diese Fehlerminderungsmodelle auf einem hohen Abstraktionsniveau, um die Fehlerminderung zu veranschaulichen, die IBM Quantum Primitives durchführen können, ohne alle Implementierungsdetails vorauszusetzen.
...
### Verwendung der Optionen in Quantum Compute für Fehlerminderung und -unterdrückung {#using-the-options-in-quantum-compute-for-mitigation-and-suppression}

So berechnet man einen Erwartungswert unter Verwendung von Fehlerminderung und Fehlerunterdrückung im IBM Quantum Compute Service. Wir können genau denselben Circuit und dieselbe Observable wie zuvor verwenden, diesmal jedoch die Optimierungsstufe auf Stufe 2 festlegen und die _Resilience_-Stufe bzw. die verwendeten Fehlerminderungstechniken anpassen. Dieser Fehlerminderungsprozess findet mehrfach während einer Optimierungsschleife statt.
...
- IBM Quantum Primitives zur Rauschminderung und -unterdrückung einsetzen
