...
\end{aligned}
$$
Da wir die Eigenwerte oder Eigenzustände des Ziel-Observablen $\hat{H}$ nicht kennen, müssen wir zunächst seine Diagonalisierung betrachten. Da $\hat{H}$ <DefinitionTooltip definition="A Hermitian is a square matrix that is equal to its own conjugate transpose, or a linear operator that is self-adjoint.">hermitesch</DefinitionTooltip> ist, existiert eine unitäre Transformation $V$, sodass $\hat{H}=V^\dagger \Lambda V$ gilt, wobei $\Lambda$ die diagonale Eigenwertmatrix ist, sodass $\langle j | \Lambda | k \rangle = 0$ für $j\neq k$ und $\langle j | \Lambda | j \rangle = \lambda_j$.
...
\end{array}
$$
Schreiben wir $\hat{H}$ nun in Bezug auf die Paulis und Identitäten um:
