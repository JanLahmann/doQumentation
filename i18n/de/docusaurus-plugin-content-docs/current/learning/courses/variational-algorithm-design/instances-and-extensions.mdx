...
### Theoretischer Aufbau {#theoretical-layout-1}
...
### Theoretischer Aufbau {#theoretical-layout-2}
...
- Verwende einen klassischen Optimierer, um den nächsten Satz von Parametern $\Theta_{i+1}$ auszuwählen.

- Wiederhole diesen Prozess, bis Konvergenz erreicht ist.


### Implementierung {#implementation-1}
...
In der Praxis ist dieses System im Allgemeinen nicht konsistent, da die Werte der Kostenfunktion $c$ nicht exakt sind. Daher ist es normalerweise sinnvoll, sie zu normalisieren, indem man sie von links mit $F^\dagger$ multipliziert, was Folgendes ergibt:

$$
F^\dagger Fa = F^\dagger c.
$$

Dieses neue System ist stets konsistent, und seine Lösung ist eine Least-Squares-Lösung des ursprünglichen Problems. Wenn wir $k$ Parameter statt nur einem haben und jeder Parameter $\theta^i$ sein eigenes $S_i$ für $i \in {1,...,k}$ besitzt, dann ist die insgesamt benötigte Anzahl an Samples:

$$
T=\prod_{i=1}^k(2S_i+1)\leq \prod_{i=1}^k(2S_{max}+1) = (2S_{max}+1)^n,
$$

wobei $S_{\max} = \max_i(S_i)$. Darüber hinaus eröffnet die Anpassung von $S_{\max}$ als einstellbaren Parameter (anstatt ihn abzuleiten) neue Möglichkeiten, wie zum Beispiel:
...
### Theoretischer Aufbau {#theoretical-layout-3}
...
- Bootstrappe bei $i=0$, wenn ein ähnliches Problem vorliegt (typischerweise durch klassische Simulation oder Sampling gefunden).
