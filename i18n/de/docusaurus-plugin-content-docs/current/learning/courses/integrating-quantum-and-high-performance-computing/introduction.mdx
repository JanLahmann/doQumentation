...
IBM hat eine neue Referenzarchitektur für quantenzentriertes Supercomputing (QCSC) vorgestellt, die beschreibt, wie Quantenprozessoren (QPUs) direkt mit CPUs und GPUs in modernen HPC-Umgebungen integriert werden können. Der Ansatz wird durch erfolgreiche hybride Quanten-Klassik-Workflows vorangetrieben – zum Beispiel durch die Verwendung der Sample-based Quantum Diagonalization (SQD)-Methode für Molekülsimulationen –, die bereits eine mit fortgeschrittenen klassischen Techniken vergleichbare Genauigkeit zeigen.

Die Architektur ist offen, modular und komposierbar konzipiert, sodass Quantenressourcen ohne einen vollständig neuen Stack in bestehende HPC-Workflows integriert werden können. Frühe Integrationen mit Systemen wie RIKEN und Fugaku zeigen, dass hybride Workflows auf Produktionsniveau bereits machbar sind. Sie bietet einen langfristigen Rahmen für die Weiterentwicklung des Quanten-Klassik-Co-Designs über das nächste Jahrzehnt.

![„Bild der vollständigen Quanten- plus HPC-Architektur."](/learning/images/courses/integrating-quantum-and-high-performance-computing/introduction/qcsc-arch.avif)

Der Stack umfasst eine Anwendungsschicht, die Probleme auf klassische und Quantenkomponenten aufteilt, eine Middleware, die klassische Programmiermodelle mit der Quantenschaltkreisgenerierung verknüpft (zum Beispiel über aktualisierte Qiskit-Schnittstellen), und eine Systemorchestrierungsschicht, die hybrides Ressourcen-Scheduling mit Werkzeugen wie dem Quantum Resource Management Interface (QRMI) verwaltet.

Auf Hardware-Ebene definiert die Architektur drei Schichten:
* Quantensystem mit QPUs und klassischen Laufzeiten mit niedriger Latenz (FPGAs/ASICs).
* Co-lokalisierte CPU/GPU-Scale-up-Systeme, die über Near-Time-Verbindungen für Aufgaben wie Fehlerminderung verbunden sind.
* Scale-out-Systeme, die flexible klassische Rechenkapazität für Vorverarbeitung, Nachverarbeitung und hybride Workloads bereitstellen.

Insgesamt positioniert IBM die QCSC-Architektur als praktischen Fahrplan für HPC-Zentren, um echte Quantenhardware zu integrieren, leistungsfähigere hybride Algorithmen zu ermöglichen und die Infrastruktur für zukünftige fehlertolerante Quantensysteme vorzubereiten. Weitere Informationen zu dieser Architektur findest du in [diesem Artikel](https://research.ibm.com/blog/quantum-centric-supercomputing-system-reference-architecture).
...
Zusätzliche Ressourcen werden auch in diesem [GitHub-Repo](https://github.com/christopherporter1/hpc-course-demos) bereitgestellt.
...
* [Run your first circuit on hardware](/guides/hello-world): Ein kurzes Tutorial zu Qiskit, in dem du dein erstes Quantenprogramm einrichtest und ausführst.
...
Lernende mit unterschiedlichstem Hintergrund finden [diesen Leitfaden](/guides/slurm-plugin) möglicherweise nützlich; er behandelt das SPANK-Plugin für das Quanten-Ressourcenmanagement und ein paar Worte zu Slurm.
...
Am Ende dieses Kurses kannst du mehr, als nur Fachbegriffe wiederzugeben – du verstehst, wie man einen modernen Hybrid-Workflow verwaltet, der bestimmte Teilaufgaben einem Quantenprozessor (QPU) zuweist, während CPUs und GPUs den Rest der Arbeit übernehmen. Du lernst, Skripte für Jobs zu schreiben, die nahtlos zwischen klassischen Knoten und QPUs wechseln, die Ergebnisse präzise zu interpretieren und zu erkennen, wo Quantenbeschleunigung Berechnungen wirklich verbessern kann (und wo nicht). Ebenso wichtig ist, dass du übst, eine wachstumsorientierte Denkweise beizubehalten: In einem neuen und sich schnell weiterentwickelnden Feld lernt niemand alles auf einmal, und echter Fortschritt entsteht durch Ausprobieren, Experimentieren und Fragenstellen.
Dieser Kurs ist in 5 Kapitel unterteilt, die folgende Themen behandeln:
