...
### Schritt 3: Ausführung mit IBM Quantum Primitives {#step-3-execute-using-ibm-quantum-primitives}
...
### Schritt 3: Ausführung mit IBM Quantum Primitives {#step-3-execute-using-ibm-quantum-primitives-1}
...
Nehmen wir an, jemand, der neu im Quantencomputing ist, versucht, eine Quantenschaltung mit sehr hoher Zwei-Qubit-Tiefe zu verwenden, ohne Fehlerminderung einzusetzen. Nehmen wir außerdem an, dass dies zu einer Fehlerrate von 10 % pro Qubit führt. Wenn das wahre (fehlerfreie) Element der Kernel-Matrix, das dieser Schaltung entspricht, sehr groß ist, sagen wir 1,0, wie hoch wäre dann die Wahrscheinlichkeit, alle 10 Qubits im Zustand zu messen, in dem sich jedes Qubit in $|0\rangle$ befindet?
...
Nachdem wir nun eine Kernel-Matrix und eine ähnlich formatierte test_matrix aus den Quanten-Kernel-Methoden haben, können wir klassische Machine-Learning-Algorithmen anwenden, um Vorhersagen über unsere Testdaten zu treffen und deren Genauigkeit zu überprüfen. Wir beginnen mit dem Import von Scikit-Learns `sklearn.svc`, einem Support Vector Classifier (SVC). Wir müssen angeben, dass der SVC unseren vorab berechneten Kernel verwenden soll, indem wir `kernel = precomputed` festlegen.
...
Mit `SVC.fit` können wir nun die Kernel-Matrix und die Trainingslabels eingeben, um eine Anpassung zu erhalten. `SVC.score` bewertet dann unsere Testdaten anhand dieser Anpassung mithilfe unserer test_matrix und gibt unsere Genauigkeit zurück.
...
### Schritt 3: Ausführung mit IBM Quantum Primitives {#step-3-execute-using-ibm-quantum-primitives-2}
