...
Selbst wenn ein Algorithmus nicht dequantisiert werden kann, bedeutet das nicht, dass er effizienter oder skalierbarer ist als alle klassischen Algorithmen. Ein extremes, konstruiertes Beispiel: Stell dir einen Algorithmus vor, der die j größten Elemente aus einer Menge der Größe k auswählt. Man könnte einen Quantenalgorithmus schreiben, der Shors Algorithmus verwendet, um jedes der k Elemente in Primfaktoren zu zerlegen, und dann die größten Elemente anhand der Primfaktoren bestimmt. Ein solcher Algorithmus lässt sich wahrscheinlich nicht dequantisieren, ist aber drastisch weniger effizient als klassische Algorithmen, um dieselbe Auswahl der größten Elemente zu erreichen (abgesehen vom unnötigen Faktorisierungsteil).
...
Falsch. Die meisten Probleme des maschinellen Lernens werden von klassischen Algorithmen sehr effizient gelöst, und Quantenalgorithmen bieten wahrscheinlich keinen wesentlichen Geschwindigkeitsvorteil. Das Ziel von QML besteht darin, Datensätze mit Merkmalen zu finden, die sich gut durch Quantenzustände beschreiben lassen, und/oder Abbildungen von Datenmerkmalen zu finden, die die Genauigkeit von Modellen optimieren.
...
5. Quantenkernel-Methoden und variationelle Quantenklassifikatoren mit IBM Quantum Primitives und gemäß Qiskit Patterns zu implementieren.
...
Im obigen Prozess haben wir einen Eintrag der Kernmatrix für den 14. und den 19. Datenpunkt berechnet. Welchen Wert sollten wir erhalten, wenn wir denselben Datenpunkt zweimal verwenden (also zum Beispiel den 14. und erneut den 14.)? Mit anderen Worten: Was sollten die Diagonaleinträge der Kernmatrix sein? Beantworte diese Frage für den rauschfreien Fall, beachte aber, dass in Gegenwart von Rauschen Abweichungen von deiner Antwort möglich sind.
