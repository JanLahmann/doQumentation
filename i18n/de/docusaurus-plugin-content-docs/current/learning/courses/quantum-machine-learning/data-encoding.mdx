...
Wir können die Rotation in $\phi$ mithilfe der von uns definierten Funktion plot_Nstates visualisieren.
...
Das ist einfach nur Dense Encoding, das wir bereits gesehen haben. Jetzt, nach dem CNOT-Gate, an der zweiten Schranke ($b2$), ist unser Zustand
$$
\begin{aligned}
...
\end{aligned}
$$
Wir wenden nun den letzten Satz von Einzel-Qubit-Rotationen an und fassen gleichartige Zustände zusammen, um Folgendes zu erhalten:
$$
\begin{align*}
...
\end{align*}
$$
Das ist vermutlich zu kompliziert, um es zu durchschauen. Tritt stattdessen einen Schritt zurück und überlege, wie viele Parameter wir auf den Zustand geladen haben: acht. Wir haben aber nur vier Rechenbasiszustände. Auf den ersten Blick könnte es so aussehen, als hätten wir mehr Parameter geladen, als sinnvoll ist, da sich der finale Zustand als $\psi_\text{final} = c_0|00\rangle+c_1|01\rangle+c_2|10\rangle+c_3|11\rangle$ schreiben lässt. Beachte jedoch, dass jeder Vorfaktor komplex ist! So geschrieben:
$$
\psi_\text{final} = (a_0+ib_0)|00\rangle+(a_1+ib_1)|01\rangle+(a_2+ib_2)|10\rangle+(a_3+ib_3)|11\rangle
$$
Man sieht, dass wir tatsächlich acht Parameter auf dem Zustand haben, mit denen wir unsere acht Merkmale codieren können.
...
Wie so oft sehen wir dies als eine einzelne, gate-ähnliche Einheit dargestellt, bis wir mit .decompose() alle einzelnen Gates sichtbar machen.
...
Eine Möglichkeit, die obige unitäre Transformation zu verstehen, ist die Analogie zum Propagator in einem physikalischen System. Die obige unitäre Transformation ist ein unitärer Zeitentwicklungsoperator, $\exp(it\mathcal{H})$, für einen Hamiltonoperator $\mathcal{H}$, ähnlich dem Ising-Modell, wobei der Zeitparameter $t$ durch Datenwerte ersetzt wird, um die Entwicklung anzutreiben. Die Entwicklung dieses unitären Operators ergibt die PFM-Schaltung. Die verschränkenden Konnektivitäten in $S$ können als Ising-Kopplungen in einem Spin-Gitter interpretiert werden.
Betrachten wir ein Beispiel mit Pauli-$Y$- und $XX$-Operatoren, die solche Ising-artigen Wechselwirkungen darstellen. Qiskit stellt eine Klasse `pauli_feature_map` zur Instanziierung einer PFM mit einer Auswahl von Ein- und $n$-Qubit-Gates bereit, die in diesem Beispiel als Pauli-Strings `‘Y’` und `‘XX’` übergeben werden. Typischerweise ist $n$ 1 oder 2 für Ein- bzw. Zwei-Qubit-Wechselwirkungen. Das Verschränkungsschema ist „linear“, das heißt, es werden nur benachbarte Qubits in der Quantenschaltung gekoppelt. Beachte, dass dies nicht den benachbarten Qubits auf dem Quantencomputer selbst entspricht, da diese Quantenschaltung eine Abstraktionsebene darstellt.
...
Der folgende Code wendet diese vier integrierten Feature Maps auf die Kodierung eines Feature-Sets an und stellt die Zwei-Qubit-Tiefe der resultierenden Schaltung dar. Da die Fehlerraten von Zwei-Qubit-Gates deutlich höher sind als die von Einzel-Qubit-Gates, dürfte man sich vernünftigerweise am meisten für die Tiefe der Zwei-Qubit-Gates interessieren. Im folgenden Code ermitteln wir die Anzahl aller Gates in einer Schaltung, indem wir die Schaltung zunächst zerlegen und dann count_ops() verwenden, wie unten gezeigt. Die Zwei-Qubit-Gates, die uns hier interessieren, sind 'cx'-Gates:
