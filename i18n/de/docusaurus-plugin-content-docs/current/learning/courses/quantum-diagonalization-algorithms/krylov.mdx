...
Eine __Krylov-Unterraum-Methode__ kann sich auf mehrere Methoden beziehen, die um den sogenannten __Krylov-Unterraum__ herum aufgebaut sind. Eine vollständige Übersicht darüber würde den Rahmen dieser Lektion sprengen, aber die Referenzen [\[2-4\]](#references) liefern deutlich mehr Hintergrundwissen. Hier konzentrieren wir uns darauf, was ein Krylov-Unterraum ist, wie und warum er beim Lösen von Eigenwertproblemen nützlich ist, und schließlich, wie er auf einem Quantencomputer implementiert werden kann.
__Definition:__ Gegeben sei eine symmetrische, positiv semidefinite $N\times N$-Matrix $A$; der Krylov-Raum $\mathcal{K}^r$ der Ordnung $r$ ist der Raum, der von Vektoren aufgespannt wird, die man durch Multiplikation höherer Potenzen einer Matrix $A$, bis zu $r-1\leq N$, mit einem Referenzvektor $\vert v \rangle$ erhält.
...
Angenommen, ein Kollege demonstriert die Krylov-Methode, angewendet auf eine kleine Spielzeugmatrix. Gibt es etwas an der Wahl der Matrix $A$ und des Anfangsvektors $|\psi\rangle$ auszusetzen?
...
In der Regel wollen wir ein Problem der folgenden Form lösen:
$$
A|x\rangle=|b\rangle.
$$
Man könnte sich vorstellen, dass $|b\rangle=c|x\rangle$ gilt, wobei $c$ eine Konstante ist, wie bei einem Eigenwertproblem. Unsere Problemstellung bleibt vorerst jedoch allgemeiner gehalten.
...
$$
durchführen, von dem wir hoffen, dass er unsere Näherung verbessert. Dabei ist $|p_0\rangle$ ein noch zu bestimmender Vektor. Sei $|e_1\rangle$ der Fehler nach der Korrektur. Dann gilt
$$
...
Uns interessiert, wie sich unser Fehler verhält, wenn er durch unsere Matrix transformiert wird. Berechnen wir also die $A$-Norm des Fehlers. Das heißt
$$
\begin{aligned}
∥|e_0\rangle−|p_0\rangle∥_A^2&=\left(\langle e_0|A−\langle p_0|A\right)\left(|e_0\rangle−|p_0\rangle\right)\\
 ...
 \end{aligned}
$$
wobei wir die Symmetrie von $A$ genutzt haben sowie die Tatsache, dass $A |e_0\rangle = |R_0\rangle.$ Hier ist $d$ eine Konstante, die unabhängig von $|p_0\rangle$ ist. Wie in Abschnitt 1.2 erwähnt, ist die $A$-Norm des Fehlers nicht die einzige Größe, die wir minimieren könnten, aber sie ist eine gute Wahl. Wir wollen sehen, wie sich diese Größe je nach der Wahl unserer Korrekturvektoren $|p_0\rangle.$ verändert. Dazu definieren wir die Funktion $f$, indem wir setzen
$$
f(|p_0\rangle)=\langle p_0|A|p_0\rangle−2\langle R_0|p_0\rangle+d.
$$
$f$ ist einfach der Fehler $|e_1\rangle$ als Funktion der Korrektur $|p_0\rangle$, gemessen in der $A$-Norm. Wir wollen also $|p_0\rangle$ so wählen, dass $f(|p_0\rangle)$ so klein wie möglich ist. Dazu berechnen wir den Gradienten von $f$. Unter Verwendung der Symmetrie von $A$ erhalten wir
$$
\nabla f(|p_0\rangle) = 2(A|p_0\rangle−|R_0\rangle).
$$
Der Gradient zeigt in Richtung des steilsten Anstiegs; sein Gegenteil ergibt somit die Richtung, in der die Funktion am stärksten abnimmt: die Richtung des __steilsten Abstiegs__. Bei unserer Anfangsschätzung $|x_0\rangle$, wobei $|p_0\rangle=0$ ist, gilt
$\nabla f(0) = -2|R_0\rangle.$
Daher nimmt die Funktion $f$ am stärksten in Richtung des Residuums $|R_0\rangle.$ ab. Unsere Anfangswahl würde also am meisten von der Addition des Vektors $|p_0\rangle=\alpha_0 |R_0\rangle$ für einen Skalar $\alpha_0$ profitieren.

Im nächsten Schritt wählen wir erneut einen Vektor $|p_1\rangle$ und addieren seinen Wert zur aktuellen Näherung. Mit demselben Argument wie zuvor wählen wir $|p_1\rangle = \alpha_1 |R_1\rangle$ für einen skalaren Wert $\alpha_1$. Wir setzen dies auf diese Weise fort, sodass die $k^\text{th}$-Iteration unseres Vektors lautet
$$
|x_{k+1}\rangle=|x_0\rangle+\alpha_0 |R_0\rangle+\alpha_1 |R_1\rangle+⋯+\alpha_k |R_k\rangle.
$$
Äquivalent dazu wollen wir den Raum aufbauen, aus dem wir unsere verbesserten Schätzungen wählen, indem wir der Reihe nach $|R_0\rangle$, $|R_1\rangle$ und so weiter hinzufügen. Der $k^\text{th}$ geschätzte Vektor liegt in
$$
...
$$
Das heißt, der Raum, den wir aufbauen und der die korrekte Lösung $|x\rangle$ am effizientesten approximiert, ist genau der Raum, der durch sukzessive Anwendung der Matrix $A$ auf $|R_0\rangle$ aufgebaut wird. Ein Krylov-Unterraum _ist_ der Raum, der von den Vektoren der sukzessiven Richtungen des steilsten Abstiegs aufgespannt wird.

Abschließend betonen wir noch einmal, dass wir keine numerischen Aussagen über die Skalierung dieses Ansatzes getroffen und auch den vergleichenden Nutzen für dünnbesetzte Matrizen nicht besprochen haben. Dies soll lediglich die Verwendung von Krylov-Unterraum-Methoden motivieren und ein intuitives Verständnis dafür vermitteln. Wir werden nun das Verhalten dieser Methoden numerisch untersuchen.
...
Man könnte sich vorstellen, statt der $A$-Norm des Fehlers den Residuumsvektor zu verwenden. Es könnte Fälle geben, in denen es sinnvoll ist, den Fehlervektor selbst zu betrachten.
...
Überzeugen wir uns davon, dass die Krylov-Methode gegenüber exakten numerischen Eigenlösern auf folgende Weise vorteilhaft sein kann:
* Wir konstruieren zufällige Matrizen (nicht dünn besetzt, also nicht die ideale Anwendung für KQD)
* Wir bestimmen die Eigenwerte mit zwei Methoden: direkt mit NumPy und mithilfe eines Krylov-Unterraums.
* Wir wählen eine Schwelle dafür, wie präzise unsere Eigenwerte sein müssen, bevor wir die Krylov-Schätzungen akzeptieren.
* Wir vergleichen die für beide Wege benötigte Rechenzeit.

__Einschränkungen:__ Wie wir weiter unten ausführlich besprechen werden, lässt sich die Krylov-Quanten-Diagonalisierung am besten auf Operatoren anwenden, deren Matrixdarstellungen dünn besetzt sind und/oder sich mit einer kleinen Anzahl von Gruppen kommutierender Pauli-Operatoren schreiben lassen. Die hier verwendeten zufälligen Matrizen entsprechen dieser Beschreibung nicht. Sie sind nur nützlich, um die Größenordnung zu untersuchen, ab der klassische Krylov-Methoden hilfreich sein könnten.
Zweitens werden wir bei der Krylov-Methode Eigenwerte mithilfe vieler unterschiedlich großer Krylov-Unterräume berechnen. Wir geben die Zeit an, die für den Krylov-Unterraum mit der minimalen Dimension benötigt wird, der die geforderte Genauigkeit für den Grundzustands-Eigenwert erreicht. Auch das unterscheidet sich etwas vom Lösen eines Problems, das für exakte Eigenlöser unlösbar ist, da wir hier die exakte Lösung verwenden, um die benötigte Dimension zu bestimmen.

Wir beginnen damit, unsere Menge an Zufallsmatrizen zu erzeugen.
...
![Ausgabe der vorherigen Codezelle](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/d9cb6e36-9ad3-403f-823f-0072e9cf53fe-0.avif)

Beachte, dass die ungewöhnlich hohe Zeit im obigen Bild bei einer Dimension von etwa 125 auf die Zufälligkeit der Matrizen oder auf die Implementierung auf dem verwendeten klassischen Prozessor zurückzuführen sein könnte, jedoch nicht reproduzierbar ist. Ein erneutes Ausführen des Codes ergibt ein anderes Profil mit anderen Ausreißern.

Nun bauen wir für jede Matrix schrittweise einen Krylov-Unterraum auf und berechnen Eigenwerte. Bei jedem Schritt prüfen wir, ob der niedrigste Eigenwert innerhalb unseres angegebenen absoluten Fehlers erreicht wurde. Der Unterraum, der uns zuerst Eigenwerte innerhalb unseres angegebenen Fehlers liefert, ist der Unterraum, für den wir die Berechnungszeiten aufzeichnen. Die Ausführung dieser Zelle kann je nach Prozessorgeschwindigkeit mehrere Minuten dauern. Du kannst die Auswertung gerne überspringen oder die maximale Dimension der diagonalisierten Matrizen reduzieren. Ein Blick auf die vorab berechneten Ergebnisse genügt.
...
![Ausgabe der vorherigen Codezelle](/learning/images/courses/quantum-diagonalization-algorithms/qda-2-krylov/extracted-outputs/ed3302d5-a0a8-479b-9731-755fc5a1b684-0.avif)
...
![Ausgabe der vorherigen Codezelle](/learning/images/courses/quantum-diagonalization-algorithms/qda-2-krylov/extracted-outputs/6963ee9b-55a8-42ab-bc4f-d470bb81e543-0.avif)
...
Die Zeitkomplexität der numerischen Diagonalisierung beträgt $O(n^3)$ (mit gewissen Abweichungen zwischen den Algorithmen). Die Zeitkomplexität zur Erzeugung einer orthonormalen Basis aus $n$ Vektoren beträgt ebenfalls $O(n^3)$. Der Vorteil der Krylov-Methode liegt also __nicht__ in der Verwendung $\it{some}$ orthonormalen Basis, sondern in der Verwendung einer bestimmten orthonormalen Basis, die die interessierenden Eigenwerte effektiv herausgreift. Das haben wir bereits anhand der Beweisskizze im ersten Abschnitt dieser Lektion gesehen, und das ist entscheidend für die Konvergenzgarantien bei Krylov-Methoden.
Sehen wir uns unseren bisherigen Fortschritt an:
* Bei sehr großen Matrizen kann die Krylov-Unterraum-Methode approximative Eigenwerte innerhalb der geforderten Toleranzen schneller liefern als traditionelle Diagonalisierungsalgorithmen.
* Bei solch sehr großen Matrizen ist die Erzeugung eines Krylov-Unterraums der zeitaufwendigste Teil der Krylov-Unterraum-Methode.
* Ein effizienter Weg zur Erzeugung eines Krylov-Unterraums wäre daher äußerst wertvoll.
An dieser Stelle kommt schließlich der Quantencomputer ins Spiel.

### Teste dein Verständnis {#check-your-understanding-2}
...
$$
Hier ist $P_\alpha$ ein Pauli-String der Form $P_\alpha \sim IZIXII...YZXIX$ oder eine Menge solcher Pauli-Strings, die miteinander kommutieren. Da wir $H$ als solche Summe messbarer Operatoren schreiben können, lassen sich die folgenden Ausdrücke für Matrixelemente von $\tilde{H}$ mithilfe des IBM Quantum Primitiv Estimator realisieren.
...
Warum können wir nicht einfach $\tilde{H}$ diagonalisieren? Da $\tilde{S}$ die Information über die Geometrie der Krylov-Basis enthält (die außer in sehr speziellen Fällen nicht orthogonal ist), beschreibt $\tilde{H}$ für sich genommen keine Projektion des vollständigen Hamiltonoperators, sodass seine Eigenwerte keinen besonderen Bezug zu denen des vollständigen Hamiltonoperators haben -- sie könnten beliebige Zufallswerte sein. Um die approximativen Eigenwerte und Eigenvektoren zu erhalten, die der Projektion des vollständigen Hamiltonoperators in den Krylov-Raum entsprechen, muss das verallgemeinerte Eigenwertproblem gelöst werden..
![Ein Schaltungsdiagramm mit vielen Schichten, das zeigt, dass die Schaltung mit unterschiedlichen Zuständen viele Male verwendet werden muss, um den modifizierten Hadamard-Test durchzuführen.](/learning/images/courses/quantum-diagonalization-algorithms/krylov/kqd-fig4.avif)
...
![Ausgabe der vorherigen Code-Zelle](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/ac536079-96f2-435b-a15f-72c207fa24d1-1.avif)
...
![Ausgabe der vorherigen Code-Zelle](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/8d35e6cd-c0f6-4871-83da-86472d66be2c-0.avif)
...
![Ausgabe der vorherigen Code-Zelle](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/d1d0b9de-65d4-4a46-975d-6cfaaac05f9a-0.avif)

#### Template-Schaltkreise zur Berechnung der Matrixelemente von $\tilde{S}$ und $\tilde{H}$ via Hadamard-Test {#template-circuits-for-calculating-matrix-elements-of-tildes-and-tildeh-via-hadamard-test}
...
![Ausgabe der vorherigen Code-Zelle](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/37382668-1999-4475-b50f-2887449d5c93-0.avif)
...
![Ausgabe der vorherigen Code-Zelle](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/054bcfba-fde3-4213-bbd7-d7a6dd6c03c8-1.avif)

Nach der Optimierung ist die transpilierte Zwei-Qubit-Tiefe weiter reduziert.

### 4.3 Schritt 3. Mit einem IBM Quantum Primitive ausführen {#43-step-3-execute-using-an-ibm-quantum-primitive}
...
### 4.4 Schritt 4. Nachbearbeitung und Analyse der Ergebnisse {#44-step-4-post-process-and-analyze-results}
...
![Ausgabe der vorherigen Code-Zelle](/learning/images/courses/quantum-diagonalization-algorithms/krylov/extracted-outputs/a5d4a983-1a30-4cea-b695-3e6a67338633-0.avif)

## 5. Diskussion und Erweiterung {#5-discussion-and-extension}
