...
### 1.1 Der Operator (Hamiltonoperator) {#11-the-operator-hamiltonian}
...
Ein gängiger Workflow (siehe Abschnitt 3.3) besteht darin, einen dieser Algorithmen als Methode innerhalb eines Minimierers wie der scipy-Funktion ```minimize``` zu verwenden. Diese nimmt folgende Argumente entgegen:
* Eine zu minimierende Funktion. Häufig handelt es sich dabei um den Energieerwartungswert. Diese werden allgemein als "Kostenfunktionen" bezeichnet.
* Eine Reihe von Parametern, mit denen die Suche beginnt. Oft $x_0$ oder $\theta_0$ genannt.
* Argumente, einschließlich der Argumente der Kostenfunktion. Beim Quantencomputing mit Qiskit umfassen diese Argumente den Ansatz, den Hamiltonoperator und die Estimator-Primitive, die im nächsten Unterabschnitt genauer besprochen wird.
* Eine 'method' der Minimierung. Dies bezieht sich auf den spezifischen Algorithmus, der zur Suche im Parameterraum verwendet wird. Hier würden wir beispielsweise COBYLA oder SLSQP angeben.
* Optionen. Die verfügbaren Optionen können je nach Methode variieren. Ein Beispiel, das praktisch alle Methoden enthalten, ist jedoch die maximale Anzahl an Iterationen des Optimierers, bevor die Suche beendet wird: 'maxiter'.
...
### 1.5 Das Variationsprinzip {#15-the-variational-principle}
...
## 2. Vergleich mit dem klassischen Workflow {#2-comparison-with-classical-workflow}
...
### 2.1 Klassischer Workflow {#21-classical-workflow}
...
### 2.2 Quantenworkflow und kommutierende Pauli-Gruppen {#22-quantum-workflow-and-commuting-pauli-groups}
...
\end{aligned}
$$
Das erfordert keine umfangreiche Multiplikation und keinen Prozess, der zwangsläufig wie $N^2$ skaliert. Stattdessen sind mehrere Messungen auf dem Quantencomputer erforderlich. Wenn du nicht zu viele davon brauchst, könnte dieser Ansatz effizient sein. Und das ist der Quantenanteil von VQE.
...
### 2.3 Faktoren, die die Rechenkosten bestimmen {#23-factors-that-determine-computational-cost}
...
## 3. Beispiel-Hamiltonoperator {#3-example-hamiltonian}
...
### 3.1 Schritt 1: Das Problem auf Quantenschaltkreise und Operatoren abbilden {#31-step-1-map-the-problem-to-quantum-circuits-and-operators}
...
#### 3.3.1 Werte ausgeben {#331-reporting-out-values}
...
### 3.4 Schritt 4: Ergebnisse nachverarbeiten {#34-step-4-post-process-results}
...
## 4. VQEs Beziehung zu anderen Methoden {#4-vqes-relationship-to-other-methods}
...
### 4.1 Die Stärken und Schwächen von VQE {#41-the-strengths-and-weaknesses-of-vqe}
...
### 4.2 Beziehung zu anderen Ansätzen {#42-relationship-to-other-approaches}
...
* Häufig werden Konfigurationsrekonstruktion[\[16\]](#references) und Symmetrie-Postselection[\[15\]](#references) als Nachverarbeitungstechniken eingesetzt.
