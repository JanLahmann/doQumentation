...
Ein weiterer verbreiteter Irrtum ist, dass ein Quantencomputer umso leistungsfähiger sein muss, je mehr Qubits er hat. Der Rechenumfang ist zwar wichtig, aber die Qubit-Anzahl allein liefert kein vollständiges Bild der Quantencomputing-Leistung. Moderne Quantenhardware wird in der Regel anhand von drei Dimensionen bewertet: Umfang, Qualität und Geschwindigkeit.

Der Umfang wird anhand der Anzahl der für Nutzer verfügbaren programmierbaren Qubits gemessen. Das sind die Qubits, die direkt gesteuert, manipuliert, gemessen und in Quantenalgorithmen eingebunden werden können. Eine steigende Anzahl programmierbarer Qubits vergrößert die Größe der Probleme, die auf Quantenhardware untersucht werden können.

Die Qualität wird anhand von Qubit-Operationen (QuOps) gemessen, die angeben, wie viele der rechnerisch anspruchsvollsten Quantenoperationen ein System zuverlässig ausführen kann, bevor sich angesammelte Fehler negativ auf die Berechnung auswirken. Verbesserungen bei Kohärenzzeiten, Gate-Genauigkeiten (Fidelities), Kalibrierungsverfahren und der Architektur tragen alle zu einer höheren rechnerischen Qualität bei. Kohärenz- und Dephasierungszeiten (üblicherweise als $T_1$ und $T_2$ bezeichnet) bleiben wichtige Indikatoren für die Qubit-Leistung.

Die Geschwindigkeit wird anhand der maximalen Circuits pro Sekunde gemessen, auch als Circuit-Durchsatz bezeichnet. Diese Metrik erfasst, wie viel nützliche Berechnung ein Quantensystem im Zeitverlauf durchführen kann, und spiegelt sowohl die Quanten- als auch die unterstützende klassische Verarbeitung wider.

Möglicherweise begegnest du auch weiteren Benchmarking-Metriken wie CLOPS (Circuit Layer Operations Per Second) und EPLG (Errors Per Layered Gate). CLOPS misst die Ausführungsgeschwindigkeit einer standardisierten Benchmark-Arbeitslast, während EPLG Einblick in die Fehlerraten auf Gate-Ebene und die Prozessorgenauigkeit gibt. Diese Metriken sind weiterhin wertvoll, um bestimmte Hardware-Eigenschaften und Arbeitslasten zu verstehen, aber programmierbare Qubits, Qubit-Operationen und maximale Circuits pro Sekunde bieten einen umfassenderen Rahmen für den Vergleich von Systemen.

![Diagramm, das die drei Schlüsselmetriken für den Quantencomputing-Fortschritt zeigt: Umfang, Qualität und Geschwindigkeit](/learning/images/courses/use-a-qc-today/quantum-computing-context/scaling.avif)

Kontinuierlicher Fortschritt in allen drei Leistungsdimensionen – Umfang, Qualität und Geschwindigkeit – ist notwendig, um einen fehlertoleranten, universellen Quantencomputer zu bauen. Deshalb wirst du beim Blick auf die IBM Quantum&reg; Roadmap feststellen, dass einige Sprünge zwischen Prozessoren keine großen Zuwächse bei der Anzahl programmierbarer Qubits aufweisen. Bemerke zum Beispiel den moderaten Zuwachs an programmierbaren Qubits zwischen Heron und Nighthawk, denn das ist nicht der eigentliche Fokus dieser Verbesserung. Stattdessen implementiert Nighthawk eine neue Konnektivitätstopologie, die andere Fehlerkorrekturcodes ermöglichen wird.
