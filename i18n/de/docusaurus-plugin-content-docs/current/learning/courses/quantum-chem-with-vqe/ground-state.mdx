...
# Alles zusammen mit IBM Quantum Compute Service {#bringing-it-all-together-with-ibm-quantum-compute-service}
...
## Schritt 3: Mit IBM-Quantum-Primitiven ausführen. {#step-3-execute-using-ibm-quantum-primitives}
...
Denk daran, dass für jede Parameterwahl im variationellen Circuit ein Erwartungswert berechnet werden muss (da das der zu minimierende Wert ist). Wie du vielleicht schon vermutest, ist der effizienteste Weg dazu die Verwendung des Qiskit-SDK-Primitives Estimator. Wir beginnen mit einem lokalen Simulator, für den wir die lokale Version von Estimator namens BackendEstimator verwenden müssen.
...
Der Code wurde korrekt ausgeführt, hat aber nicht konvergiert – was wir erwartet haben. Wir fahren mit der Ausführung der Berechnung auf echter Hardware fort und besprechen anschließend die Ausgaben. Für echte Backends verwenden wir den IBM Quantum Estimator. Wir möchten dies innerhalb einer Session ausführen und werden in der Regel Optionen für diese Session festlegen wollen.
...
* Sessions: VQE ist von Natur aus iterativ, wobei der klassische Optimierer bei jedem weiteren Versuch neue variationelle Parameter auswählt und damit neue Gates verwendet werden. Ohne Sessions könnte dies zu zusätzlichen Wartezeiten in der Warteschlange zwischen jedem Versuch führen. Wenn die VQE-Berechnung innerhalb einer Session gekapselt wird, gibt es nur eine initiale Warteschlange vor dem Start des Jobs, aber keine zusätzliche Wartezeit zwischen den variationellen Schritten. Diese Strategie wurde bereits im Beispiel der vorherigen Lektion verwendet, kann aber eine noch wichtigere Rolle spielen, wenn die Geometrie variiert wird. Mehr über Sessions erfährst du in der [Dokumentation zu Ausführungsmodi](/guides/execution-modes).

* Integrierte Optimierung des Estimators: Im Estimator gibt es integrierte Optionen zur Optimierung einer Berechnung. In vielen Kontexten (einschließlich Estimator) sind die Einstellungen auf 0 und 1 begrenzt, wobei 0 keine Optimierung bedeutet und 1 (die Standardeinstellung) eine gewisse Optimierung deiner Schaltung auf die ausgewählte Hardware bedeutet. Einige andere Kontexte erlauben Einstellungen von 0, 1, 2 oder 3. Weitere Informationen zu den in den verschiedenen Einstellungen verwendeten Methoden findest du in der [Dokumentation](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-estimator-options). Hier setzen wir die Optimierung tatsächlich auf 0 und verwenden 'skip\_transpilation = true', weil wir unsere Schaltung bereits oben im Abschnitt zur Optimierung mit dem Pass Manager transpiliert haben.

* Estimators integrierte Resilienz: Wie bei der Optimierung hat Estimator integrierte Einstellungen für Resilienz gegenüber Fehlern, die verschiedenen Ansätzen zur Fehlerminderung entsprechen. Informationen zu Resilienzstufeneinstellungen findest du in der [Dokumentation](/guides/error-mitigation-and-suppression-techniques).
