...
1.	Was werden unsere Qubits in unserem Modell repräsentieren?

2.	Ist unser Problem kontinuierlich? Da Quantencomputer digital sind, müssen wir, falls das Problem kontinuierlich ist, einen Weg finden, es zu diskretisieren.

3.	Stimmt schließlich die Topologie des Problems mit der Topologie der Hardware überein? Falls nicht, müssen wir möglicherweise einige Tricks anwenden, damit es funktioniert.
...
Falls dir das rückwärts vorkommt (insbesondere das Vorzeichen zwischen $X$ und $iY$), sieh dir die optionale Anmerkung unten zu den Konventionen an, welcher Zustand $|0\rangle$ oder $|1\rangle$ „besetzt“ und welcher „unbesetzt“ entspricht.

<Accordion>
<AccordionItem title="Optionale Anmerkungen zu Konventionen">

In vielen physikalischen Kontexten gibt es keine Mehrdeutigkeit bei der Interpretation der Operatoren $\sigma^\pm$; sie erhöhen/verringern die z-Projektion des Spin-Drehimpulses eines Systems. $\sigma^+$ überführt beispielsweise einen Zustand mit Spin-Drehimpuls $|m_s\rangle$ in einen Zustand mit $|m_s+1\rangle$. Wenn jedoch ein Mapper angewendet wird, um fermionische Erzeugungs- und Vernichtungsoperatoren mithilfe dieser Drehimpulsoperatoren (Kombinationen von Pauli-Operatoren) auszudrücken, entspricht das Erhöhen/Verringern nun der Besetzung eines elektronischen Orbitals. „Erhöhen“ und „Verringern“ sind eindeutig, wenn die Aktion darin besteht, die Projektion des Drehimpulses auf eine bestimmte Achse und die entsprechende Bezeichnung zu erhöhen oder zu verringern. Bei der Besetzung elektronischer Orbitale könnte man $|0\rangle$ mit leer und $|1\rangle$ mit besetzt assoziieren. Das mag wie die naheliegende Konvention erscheinen, und tatsächlich ist es diejenige, die in Qiskit Addons verwendet wird. Beachte jedoch, dass es möglich ist, $|0\rangle$ im Sinne eines Elektrons, das in den Grundzustand eines Systems relaxiert, als „besetzt“ zu wählen, und $|1\rangle$ als „unbesetzt“. Es ist eine freie Wahl und eine mögliche Mehrdeutigkeit bei der Bezeichnung: Welcher Zustand entspricht einem besetzten Orbital? $|0\rangle$ oder $|1\rangle$?

Es ist inzwischen im Kontext des Quantencomputings sowie in integrierten Mappern wie dem Qiskit-Addon `FermionicMapper` einigermaßen standardisiert, Folgendes festzulegen:
$$
\begin{aligned}
|0\rangle & = \text{ empty}\\
...
Äquivalent dazu muss sich ein Operator, der ein Elektron erzeugt, das das Orbital $k$ besetzt (dargestellt durch das Qubit $q_k$), wie folgt verhalten:

$$
O_k |q_N,...,q_k=0,...,q_0\rangle = c |q_N,...,q_k=1,...,q_0\rangle
$$

Sehen wir uns nun die Konsequenzen dieser Wahl an. Beachte, dass in den in der Quanteninformatik üblichen Standardbasen,
...
und durch ähnliche Rechnungen können wir die vollständige Menge konstruieren:
...
Daraus wird deutlich, dass wir bei der scheinbar vernünftigen Konvention, dass $|0\rangle$ keinem Elektron und $|1\rangle$ einem Elektron entspricht, in die verwirrende Situation geraten, dass $\sigma^+$ im Vernichtungsoperator und $\sigma^-$ im Erzeugungsoperator auftaucht. Die Konvention in `Qiskit Fermions` (dass $|0\rangle$ besetzt und $|1\rangle$ unbesetzt entspricht) führt somit dazu, dass der fermionische Erzeugungsoperator $f_k^\dagger$ den Term $\sigma_k^-$ enthält und der Vernichtungsoperator $f_k$ den Term $\sigma_k^+$ enthält.

Wenn du Quellen findest, die entgegengesetzte Vorzeichen zwischen den Termen $X$ und $iY$ in den Erzeugungs- und Vernichtungsoperatoren verwenden, nutzen sie wahrscheinlich die entgegengesetzte Konvention dafür, welcher Zustand einem besetzten Orbital entspricht.
...
1.	Verwende die Funktion `from_list` von SparsePauliOp, um einen Identitätsoperator zu erzeugen, der der Größe des zu mappenden Parameterraums entspricht.

2.	Verwende entsprechend der zuvor gezeigten Definition der Erzeugungs- und Vernichtungsoperatoren die Funktion `from_list` von SparsePauliOp, um die Pauli-Operatoren $X$, $Y$, $Z$ zu definieren. Dadurch werden die fermionischen Erzeugungs- und Vernichtungsoperatoren auf Qubit-Spin-Operatoren abgebildet, wodurch die fermionische Besetzungszahl in die Rechenbasis der Qubits kodiert wird.

3.	Erzeuge den gewünschten Hamiltonianer in der Pauli-Basis, indem du die oben genannten Operatoren auf die interessierenden Orbitale anwendest. Dies entspricht in der Regel der Erzeugung einer Identitätsmatrix, die die Kern- (nicht wechselwirkenden) Orbitale repräsentiert, und der anschließenden Anwendung der Erzeugungs- und Vernichtungsoperatoren auf den aktiven Raum, mit Gewichtungen, die den Besonderheiten des jeweiligen Problems entsprechen.
...
wobei $\sigma^+$ der Pauli-Operator $X + iY$ und $\sigma^{-}$ $X - iY$ ist. Sobald wir einen Hamiltonianer in diesem Format haben, ist der schwierige Teil der Mapping-Phase vorbei, und er kann nun leicht in einen Circuit in Pauli-Operatoren geschrieben werden.
