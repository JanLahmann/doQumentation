...
1.	Simulation von Naturphänomenen
...
Um das QUBO-Problem als Ising-Hamiltonianer zu schreiben, müssen wir eigentlich nur eine einfache Variablensubstitution von $x \in \{0, 1\}^n$ zu $z \in \{-1, 1\}^n$ durchführen:
...
Beachte zunächst, dass wir unseren QUBO-Ausdruck als Summe von Matrixtermen umschreiben können:

$$
x^TQx = \sum_{ij}Q_{ij}x_ix_j
$$

Durch Anwendung der Variablensubstitution ergibt sich
...
Wir können den konstanten Term weglassen, da er keinen Einfluss darauf hat, welches $z$ das QUBO-Problem minimiert. Zusätzlich können wir diesen Ausdruck mit vier multiplizieren, um den ursprünglichen Wert von $Q$ wiederherzustellen, da dies ebenfalls keinen Einfluss auf die optimale Wahl von $z$ hat.

$$
\sum_{ij}Q_{ij}z_iz_j - Q_{ij}z_i - Q_{ij}z_j = z^TQz + (-\sum_{ij} Q_{ij}z_i + Q_{ij}z_j)
$$

Indem wir gleichartige lineare Terme gruppieren, können wir unseren linearen Koeffizientenvektor $b$ mithilfe der folgenden Gleichung definieren

$$
b_i = -\sum_{j} Q_{ij} + Q_{ji}
$$

was die Transformation der Variablen zu $z \in \{-1, 1\}^n$ nach dem Einsetzen in die vorherige Gleichung abschließt.
...
Am Ende ist die Minimierung des QUBO-Ausdrucks identisch mit der Minimierung dieses Ausdrucks, wobei $b$ ein reeller skalarer Koeffizient ist:

$$
\min_{x\in\{0,1\}^n} x^TQx\Longleftrightarrow \min_{z\in\{-1,1\}^n}z^TQz + b^Tz
$$

Nochmals leicht umgeschrieben haben wir unseren Kosten-Hamiltonianer, wobei das Minimum des Ausdrucks den Grundzustand darstellt und `Z` der Pauli-Z-Operator ist:
...
1.	den initialen Versuchszustand, in Grau, der der Grundzustand des Mixers ist und durch Anwendung eines Hadamard-Gates auf jedes Qubit erzeugt wird

2.	die Kostenfunktionsentwicklung, die wir zuvor besprochen haben, in Dunkellila

3.	die Entwicklung unter dem Mixer-Hamiltonianer, die wir noch nicht behandelt haben, in Hellila.
...
Bisher haben wir den Circuit transpiliert und dabei die Parameter Gamma und Beta unberührt gelassen – aber wir können den Circuit nicht wirklich ausführen, ohne diese Parameter anzugeben. Im QAOA-Workflow werden die optimalen QAOA-Parameter in einer iterativen Optimierungsschleife gefunden, bei der wir eine Reihe von Circuit-Auswertungen durchführen und dann einen klassischen Optimierer verwenden, um die optimalen 𝛽- und 𝛾-Parameter zu finden. Wir müssen jedoch irgendwo beginnen, also machen wir eine anfängliche Schätzung von $\gamma=\pi/2$ und $\beta=\pi$.
