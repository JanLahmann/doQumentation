...
__Option 1__ (Einstein): Die beiden Spins (des Elektrons und des Positrons) sind determiniert, in dem Sinne, dass das Ergebnis jeder Messung entlang jeder Achse von der Natur vorherbestimmt ist, auch wenn wir es nicht kennen. Man kann sich das so vorstellen, dass die Spins eine reale, wohldefinierte Ausrichtung im Raum haben, die uns nicht bekannt ist, aber existiert. Oder man kann sich das als eine Menge von Informationen oder Anweisungen vorstellen, die die Ergebnisse von Messungen entlang $x$, $y$, $z$ oder allem dazwischen festlegen. Das Messen des Spins des Positrons (etwa entlang z) zwingt ihn dazu, sich in Richtung z oder -z auszurichten. Dies hat keinen kausalen Einfluss auf den Elektronenspin, obwohl wir wissen, dass der Elektronenspin anfangs dem Positronenspin entgegengesetzt war – wird also der Positronenspin entlang +z gemessen, wird der Elektronenspin entlang -z gemessen. Abgesehen von der Anfangsbedingung der Anweisungen, die den Drehimpuls erhalten (die Spins sind antiparallel ausgerichtet), gibt es keine Verbindung zwischen den beiden Spins. Diese Option wird manchmal „verborgene Variablen" genannt, im Sinne von: Die Projektionen entlang verschiedener Achsen sind festgelegt, aber vor uns verborgen.

__Option 2__ (Born): Beide Spins sind in ihren Anfangszuständen unbestimmt … nicht nur unbekannt, sondern physikalisch nicht wohldefiniert, ohne feste Ausrichtung oder Vorgaben für Messergebnisse, bis sie gemessen werden. Das Messen des Positronenspins lässt den Raum aller Möglichkeiten auf einen einzigen bestimmten Zustand „kollabieren", entweder entlang der +z- oder der -z-Achse. Diese Messung des Positrons zwingt auch den Elektronenspin dazu, in eine wohldefinierte Projektion entlang z zu kollabieren, die genau entgegengesetzt zu der des Positrons ist. Dieser Effekt erstreckt sich über den gesamten Raum zwischen Positron und Elektron. Dies wurde als „spukhafte Fernwirkung" bezeichnet, man könnte es aber auch weniger dramatisch „nichtlokale Physik" nennen.

### Verständnisfragen {#check-your-understanding}

Es wäre schön, zwischen Einsteins und Borns Option experimentell zu unterscheiden. Welche Experimente würden unabhängig davon, welche Option zutrifft, dieselben Ergebnisse liefern? Fällt dir ein Experiment ein, das für die beiden Optionen unterschiedliche Ergebnisse ergäbe?

**Hinweis:** Es wäre sehr beeindruckend, wenn du ein Experiment findest, das unterschiedliche Ergebnisse für Einsteins und Borns Option liefert; die Menschheit brauchte Jahrzehnte, um eines zu entwickeln.
...
Nun verwenden wir ein Qiskit-Referenz-Primitive namens StatevectorSampler. Ein Sampler ist ein Primitive, das dazu dient, alle möglichen Zustände eines Systems abzutasten und die Wahrscheinlichkeiten (oder in manchen Fällen Quasi-Wahrscheinlichkeiten) für das Auftreten jedes Zustands zurückzugeben. Wir können eine Anzahl von „Shots" angeben und uns die „Counts" für jeden Zustand ansehen.
...
### Überprüfe dein Verständnis {#check-your-understanding-6}
...
Lass uns noch einen Moment über die Bedeutung dieses Ergebnisses nachdenken. Zwei Teilchen entstehen bei einem Zerfallsereignis und bewegen sich in verschiedene Richtungen, möglicherweise über lange Zeit. Ihre Spins befinden sich in keinem wohldefinierten Zustand, und sie tragen keine verborgenen Variablen mit sich, die die Ergebnisse zukünftiger Messungen bestimmen würden. Doch eine Messung des einen Teilchens (etwa entlang $+z$) legt zwingend das Ergebnis eines Experiments am Spin des anderen Teilchens entlang der $z$-Richtung fest (er muss $-z$ sein). Das bedeutet, dass etwas an der Physik des einen Teilchens durch das bestimmt wird, was mit dem anderen Teilchen – möglicherweise weit entfernt – geschieht. Dies ist eine der Situationen, die dazu geführt hat, dass Menschen die Realität als „nicht-lokal" bezeichnen.
...
3. W/F Je zwei korrelierte Systeme sind quantenmechanisch verschränkt.
