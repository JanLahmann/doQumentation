...
(a) Normiere die Testwellenfunktion, indem du $A$ so bestimmst, dass $$ \int_{-\infty}^{\infty} |\Psi_\text{trial}|^2 dx = 1 $$.

<Accordion>
<AccordionItem title="Lösung">
...
(b) Berechne den Erwartungswert des Hamiltonoperators $\hat{H}$ gemäß $$ \hat{H} = -\frac{\hbar^2}{2m} \frac{d^2}{dx^2} + V(x)$$ wobei $V(x)  = \frac{1}{2}m\omega^2x^2$ dem Potential eines harmonischen Oszillators entspricht.

<Accordion>
<AccordionItem title="Lösung">

Der Hamiltonoperator für einen harmonischen Oszillator lautet:
...
Optimierung von $ \alpha $ für minimale Energie
...
Wenn es bereits so viele Strategien für chemische Probleme gibt, warum brauchen wir dann einen Quantencomputer? Und welchen Zweck hat es, Quanten- und klassische Computer gemeinsam einzusetzen?
...
Fülle die Lücken mit den richtigen Begriffen aus, um die Zusammenfassung des VQE-Prozesses zu vervollständigen, und klicke anschließend, um deine Antworten zu überprüfen.
...
Beginnen wir damit, einige notwendige Pakete zu laden, darunter die IBM Quantum-Primitive. Außerdem wählen wir den am wenigsten ausgelasteten verfügbaren Quantencomputer aus.
...
Mit diesem definierten Hamiltonoperator können wir nun VQE verwenden, um dessen Grundzustandsenergie zu berechnen. Es ist sinnvoll, unsere berechnete Grundzustandsenergie mit bekannten Erwartungswerten zu vergleichen. Für ein einzelnes, isoliertes Wasserstoffatom (H) beträgt die Grundzustandsenergie genau -0,5 Hartree (ohne relativistische Effekte). Berechnen wir nun die exakte Grundzustandsenergie *unseres spezifischen Qubit-Hamiltonoperators*, wie oben definiert, und vergleichen wir sie mit relevanten bekannten Werten.
...
Man sieht, dass die `rx, rz`-Gatter unseres Ansatzes in eine Abfolge von `rz, sx`-Gattern umgewandelt wurden, welche die nativen Gatter unseres Backends sind. Außerdem ist unser `q0` nun auf das fünfte physische Qubit abgebildet. Wir müssen auch unseren Hamiltonoperator entsprechend dieser Änderungen anpassen, wie im folgenden Code:
...
Jetzt ist es an der Zeit, unser VQE auf einem echten QPU laufen zu lassen. Dafür brauchen wir zunächst eine Kostenfunktion für den Optimierungsprozess, die den Erwartungswert des Hamiltonoperators für einen vom Ansatz erzeugten Quantenzustand berechnet. Keine Sorge! Du musst nicht alles selbst programmieren. Wir haben eine Funktion dafür vorbereitet — du musst nur die folgende Zelle ausführen.
...
### Verständnistest {#check-your-understanding-3}
...
### Schritt 1: Problem auf Quantenschaltkreise und Operatoren abbilden {#step-1-map-the-problem-to-quantum-circuits-and-operators-1}
...
Anders als bei der Berechnung eines einzelnen Wasserstoffatoms ($H$) müssen wir zur Berechnung des Grundzustands eines Wasserstoffmoleküls ($H_2$) zusätzlich zur Energie der Elektronenorbitale auch die abstoßende Kraft zwischen den Kernen der beiden Wasserstoffatome berücksichtigen. In diesem Schritt geben wir diesen Wert als Konstante an, und wir werden diesen Wert tatsächlich in der Check-in-Aufgabe berechnen.
...
### Schritt 2: Optimierung für die Ziel-Hardware {#step-2-optimize-for-target-hardware-1}

Da die Anzahl der Qubits, die vom vorherigen VQE und Hamiltonian verwendet wurden, mit dem für die Ausführung verwendeten Backend übereinstimmt, verwenden wir den bestehenden Ansatz und dessen optimierte Form.

```python
h2_hamiltonian_isa = h2_hamiltonian.apply_layout(layout=ansatz_isa.layout)
```

### Schritt 3: Ausführung auf der Ziel-Hardware {#step-3-execute-on-target-hardware-1}

Jetzt ist es Zeit, die Berechnungen auf der eigentlichen QPU durchzuführen. Fast alles bleibt gleich, aber wir verwenden den passenden Startpunkt, um den Hamiltonoperator anzupassen. Außerdem werden in einem iterativen Teil einige Einstellungen des `Estimator`, der verwendet wird, um die Erwartungswerte des Hamiltonoperators für den Ansatz auf der QPU zu berechnen, leicht anders eingestellt als bei den vorherigen Berechnungen. Diese Änderung besprechen wir näher in einer Check-in-Frage.
...
### Schritt 4: Nachbearbeitung {#step-4-post-process-1}
...
### Verständnis überprüfen {#check-your-understanding-4}
...
## Reaktionsenergie von $H + H = H_2$ berechnen {#compute-reaction-energy-of-h-h-h-2}
...
## (Optional) Anhang: Optimierer-Overhead nach Ansatz-Komplexität {#optional-appendix-optimizer-overhead-by-ansatz-complexity}
...
Die Wahl des richtigen variationellen Ansatzes ist von Natur aus schwierig. Chemisch inspirierte Ansätze (wie UCCSD) liefern physikalische Genauigkeit, erfordern aber tiefe Schaltkreise, während hardwareeffiziente Ansätze flachere Schaltkreise haben, dafür aber möglicherweise an physikalischer Interpretierbarkeit einbüßen. Zudem führen viele Ansätze übermäßig viele variationelle Parameter ein, die kaum zur Verbesserung der Genauigkeit beitragen, die Optimierung aber erheblich erschweren.

2. Schwierigkeiten bei der Optimierung


Die Optimierungslandschaft von VQE kann Bereiche aufweisen, in denen Gradienten exponentiell verschwinden (sogenannte Barren Plateaus), was es klassischen Optimierern erschwert, die variationellen Parameter effizient zu aktualisieren. Forscher haben daher versucht, verschiedene Optimierer-Typen einzusetzen – gradientenbasierte und gradientenfreie –, aber beide stoßen auf Schwierigkeiten. Gradientenbasierte Optimierer leiden unter Barren Plateaus, während gradientenfreie Methoden eine große Anzahl von Funktionsauswertungen benötigen.

3. Optimierer-Overhead


Eine weitere bekannte Herausforderung ist der Optimierer-Overhead, der mit der Skalierung des Problems zusammenhängt. Die für VQE benötigten Quantenschaltkreise wachsen in Tiefe und Komplexität mit zunehmender Problemgröße; dies erhöht in der Regel auch die Anzahl der zu optimierenden Parameter. Der Optimierungsprozess wird schwer handhabbar, wenn die Anzahl der Parameter zunimmt, was zu langsamer Konvergenz und Schwierigkeiten beim Finden der optimalen Lösung führt.
...
Statt sich auf einfache Ein-Qubit-Schaltkreise und einen unkomplizierten Ansatz zu verlassen, steigt die Komplexität der Optimierung, wenn größere Quantenschaltkreise und komplexer strukturierte Ansätze erforderlich sind. Dies verdeutlicht eine bekannte Herausforderung bei VQEs: den Overhead des Optimierers.
