...
Hier demonstrieren wir, wie man Grover-Orakel konstruiert und den `GroverOperator` aus der Qiskit-Schaltkreisbibliothek verwendet, um eine Grover-Suchinstanz einfach einzurichten. Das IBM Quantum `Sampler`-Primitiv ermöglicht eine nahtlose Ausführung von Grover-Schaltkreisen.
...
- Schritt 3: Ausführung mit IBM Quantum-Primitiven
...
### Schritt 3: Ausführung mit IBM Quantum-Primitiven {#step-3-execute-using-ibm-quantum-primitives}

Wir möchten viele Messungen durchführen und sehen, welcher Zustand am wahrscheinlichsten ist. Eine solche Amplituden-Amplifikation ist ein Sampling-Problem, das für die Ausführung mit dem `Sampler` IBM Quantum-Primitiv geeignet ist.

Beachte, dass die `run()`-Methode von IBM Quantum SamplerV2 ein Iterable von Primitive Unified Blocks (PUBs) annimmt. Für Sampler ist jedes PUB ein Iterable im Format (circuit, parameter_values). Mindestens benötigt es jedoch eine Liste von Quantenschaltkreisen.
...
Erinnere dich daran, dass wir, solange die Anzahl der Lösungen im Vergleich zum gesamten Lösungsraum klein ist, die Sinusfunktion um kleine Winkel entwickeln und Folgendes verwenden können
$$
...
Nein. Nehmen wir an, wir wiederholen die erste Aktivität mit 20 Qubits und führen die Quantenschaltung `num_shots = 10,000` Mal aus. Eine gleichmäßige Wahrscheinlichkeitsverteilung würde bedeuten, dass jeder Zustand eine Wahrscheinlichkeit von $10,000/2^{20}=0.00954$ hat, auch nur ein einziges Mal gemessen zu werden. Wäre die Wahrscheinlichkeit, den Zielzustand zu messen, zehnmal so hoch wie die der Nicht-Lösungen (und die Wahrscheinlichkeit jeder Nicht-Lösung entsprechend leicht verringert), gäbe es nur etwa eine 10-prozentige Chance, den Zielzustand auch nur einmal zu messen. Es wäre höchst unwahrscheinlich, den Zielzustand mehrfach zu messen, was ihn von den vielen zufällig erhaltenen Nicht-Lösungszuständen ununterscheidbar machen würde. Die gute Nachricht ist, dass wir durch den Einsatz von Fehlerunterdrückung und -minderung noch genauere Ergebnisse erzielen können.
...
### Schritt 2: Problem für die Ausführung auf Quanten-Hardware optimieren {#step-2-optimize-problem-for-quantum-hardware-execution-1}
...
### Schritt 3: Ausführung mit IBM Quantum-Primitiven {#step-3-execute-using-ibm-quantum-primitives-1}
...
### Schritt 2: Problem für die Ausführung auf Quanten-Hardware optimieren {#step-2-optimize-problem-for-quantum-hardware-execution-2}
...
### Schritt 3: Ausführung mit IBM Quantum-Primitiven {#step-3-execute-using-ibm-quantum-primitives-2}
...
### Schritt 4: Nachbearbeitung und Rückgabe des Ergebnisses im gewünschten klassischen Format {#step-4-post-process-and-return-result-in-desired-classical-format-2}
