...
Als abschließende Illustration, wie der Grover-Algorithmus im Kontext einer Unterroutine nützlich sein könnte, stellen wir uns vor, dass wir Quantenzustände mit einer bestimmten Anzahl von `1`-Zeichen in der Bitstring-Darstellung benötigen. Dies ist üblich in Situationen, in denen Erhaltungsgesetze beteiligt sind. Zum Beispiel ordnet man im Kontext der Quantenchemie oft einen `1`-Zustand eines Qubits einer Besetzung eines elektronischen Orbitals zu. Damit die Ladung erhalten bleibt, muss die Gesamtzahl der `1`-Zeichen im Bitstring ebenfalls konstant bleiben. Solche Einschränkungen sind so üblich, dass sie einen besonderen Namen haben: **Hamming-Gewichtsbeschränkungen**, und die Anzahl der `1`-Zeichen in der Bitstring-Darstellung des Zustands wird **Hamming-Gewicht** genannt.
