---
title: "Quantencomputer für den Transpiler darstellen"
sidebar_label: "Quantencomputer für den Transpiler darstellen"
...
- Das [`Target`](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.Target) enthält alle relevanten Einschränkungen eines Geräts, wie z. B. seinen unterstützten Befehlssatz, die Qubit-Konnektivität sowie Puls- oder Timing-Informationen.
- Das [`Backend`](https://docs.quantum.ibm.com/api/qiskit/qiskit.providers.BackendV2) besitzt standardmäßig ein `Target`, enthält zusätzliche Informationen — wie die [`InstructionScheduleMap`](https://docs.quantum.ibm.com/api/qiskit/1.4/qiskit.pulse.InstructionScheduleMap) — und stellt die Schnittstelle zur Übermittlung von Quantum-Circuit-Jobs bereit.
...
Diese Seite zeigt mehrere Beispiele für die Übergabe von QPU-Informationen an den Transpiler.
...
Dieses Thema zeigt mehrere Beispiele für die Übergabe von Informationen an den Transpiler.

<Admonition type="note">
Diese Beispiele verwenden das Target des `qiskit_ibm_runtime` Mock-Backends [`FakeSherbrooke`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/fake-provider-fake-sherbrooke#fakesherbrooke). Du kannst es aber mit jedem Qiskit-kompatiblen realen oder Fake-Backend ausprobieren. Deine Ergebnisse können abweichen.
</Admonition>
...
Dieses Beispiel wird in späteren Abschnitten dieses Themas verwendet, um zu verdeutlichen, dass die Coupling Map und der unterstützte Befehlssatz die wesentlichen Informationsstücke sind, die dem Transpiler für eine optimale Circuit-Konstruktion übergeben werden müssen. Der QPU kann in der Regel Standardeinstellungen für andere nicht übergebene Informationen wählen, wie z. B. Timing und Scheduling.
...
## Unterstützte Befehle {#supported-instructions}

Jeder Quantencomputer unterstützt einen begrenzten Befehlssatz. Jedes Gate im Circuit muss in die Elemente dieses Satzes übersetzt werden. Dieser Satz sollte aus Einzel- und Zwei-Qubit-Gates bestehen, die einen universellen Gate-Satz bilden, was bedeutet, dass jede Quantenoperation in diese Gates zerlegt werden kann. Dies geschieht durch den [BasisTranslator](https://docs.quantum.ibm.com/api/qiskit/qiskit.transpiler.passes.BasisTranslator), und `basis_gates` können dem Transpiler als Schlüsselwortargument übergeben werden, um diese Informationen bereitzustellen.
