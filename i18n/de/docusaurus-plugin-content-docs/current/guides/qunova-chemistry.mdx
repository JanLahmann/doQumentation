...
Authentifiziere dich dann mit deinem [IBM Quantum&reg; API-Schlüssel](http://quantum.cloud.ibm.com/) und – vorausgesetzt, du hast dein Konto bereits [in deiner lokalen Umgebung gespeichert](/guides/functions-get-started#install-qiskit-functions-catalog-client) – wähle die Qiskit Function wie folgt aus:
...
Überprüfe den [Status](/guides/functions-get-started#check-job-status) deiner Qiskit Function-Arbeitslast oder rufe die [Ergebnisse](/guides/functions-get-started#retrieve-results) wie folgt ab:
...
Um auf die Grundzustandsenergie zuzugreifen, verwende den Schlüssel "energy". Der Schlüssel "eigenvector" liefert die CI-Koeffizienten mit der entsprechenden Bitstring-Notation der Elektronenkonfiguration, die unter "states" der Ergebnisse gespeichert sind.
...
Beachte, dass die Nutzung dieser Qiskit Function auf Probleme mit höchstens 20 Qubits beschränkt ist, sofern keine Lizenz erworben wird, die ein höheres Limit gewährt.

Wende dich bei Lizenzanfragen per E-Mail an [qiskit.support@qunovacomputing.com](mailto:qiskit.support@qunovacomputing.com).

### Beispiel für die Verwendung einer lizenzierten Function {#example-of-licensed-function-use}

Lizenzierte Nutzerinnen und Nutzer erhalten ein Lizenz-Token und müssen dann eine Wrapper-Bibliothek verwenden, um ihr Lizenz-Token an die Function zu übermitteln.
Die Wrapper-Bibliothek kann [von PyPI](https://pypi.org/project/hivqe-qiskit-function-utils/) mit `pip install hivqe-qiskit-function-utils` installiert werden.
Das folgende Beispiel zeigt, wie diese Bibliothek verwendet werden sollte, um dein Lizenz-Token beim Aufruf der Function zu übermitteln.
