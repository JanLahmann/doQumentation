...
Folge den Anweisungen im Thema [Installation](/guides/install-c-api), um Folgendes zu installieren:

- Die Standard-C-Compiler-Toolchain für deine Plattform

- Eine Python-Version, die ihre C-API-Header enthält. 


Du solltest außerdem mit den verfügbaren Funktionen und Objekten der [Qiskit C API](https://docs.quantum.ibm.com/api/qiskit-c) vertraut sein oder bereit sein, sie nachzuschlagen, und solltest über einige Grundkenntnisse der C-Programmierung verfügen.
...
Lege die Runtime-Version von Qiskit in
`project.dependencies` so fest, dass sie mit der beim Build verwendeten Minor-Version übereinstimmt.
