...
Um deine Qiskit-Python-Programme mit C zu beschleunigen, kannst du die Qiskit-C-Erweiterung für Python verwenden.
...
Stelle zunächst sicher, dass du die [Qiskit C API](/guides/install-c-api) installiert hast. Installiere als Nächstes die Qiskit-Python-Schnittstelle wie folgt:
```bash
pip install -r requirements.txt -c constraints.txt
pip install .
```

## Definition der C-Erweiterung {#define-the-c-extension}
...
## Manuelle C-Erweiterung {#manual-c-extension}

Anstatt `ctypes` zu verwenden, ist es möglich, manuell eine Erweiterung für Python zu erstellen, indem du direkt [Pythons C API](https://docs.python.org/3/c-api/intro.html) verwendest. Dies hat das Potenzial, schneller zu sein als die Verwendung von `ctypes`, erfordert aber mehr Aufwand bei der Implementierung.
...
```
Um eine Shared Library zu kompilieren, verlinke sowohl die Python- als auch die Qiskit-Bibliotheken, wie im Abschnitt [Build](#build) oben beschrieben. Das Python-Skript benötigt dann keine `ctypes`, sondern kann das `cextension`-Modul direkt importieren (stelle sicher, dass es sich in deinem Python-Pfad befindet):
```python
# file: main.py
import qiskit
import cextension
