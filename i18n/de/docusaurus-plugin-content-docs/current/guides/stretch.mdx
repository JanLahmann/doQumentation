...
![Abbildung des folgenden Schaltkreises](/docs/images/guides/stretch/stretch.svg)
...
:::note
 ...
 Wenn du `stretch` mit IBM Quantum Compute Service verwendest, wird jeder Rest, der aus einer stretch-Auflösung entsteht, zur ersten Verzögerung hinzugefügt, die den stretch verwendet.
...
## Stretch-Werte im IBM Quantum Compute Service anzeigen {#view-stretch-values-in-ibm-quantum-compute-service}

Der tatsächliche Wert einer stretch-Dauer wird zur Kompilierzeit aufgelöst, nachdem der Circuit geplant wurde. Wenn du einen Sampler-Job im IBM Quantum Compute Service ausführst, kannst du die aufgelösten stretch-Werte in den Job-Ergebnis-Metadaten einsehen.  Die Unterstützung für `stretch` in Quantum Compute ist derzeit experimentell, daher musst du zunächst eine experimentelle Option aktivieren, um den Abruf zu ermöglichen, und dann wie folgt direkt auf die Daten aus den Metadaten zugreifen:
...
Obwohl die gesamte Schaltkreiszeit in den „compilation"-Metadaten zurückgegeben wird, ist dies NICHT die für die Abrechnung verwendete Zeit (QPU-Zeit).
</Admonition>
...
Weitere Details findest du im Thema [Schaltkreis-Zeitplanung visualisieren](/guides/qiskit-runtime-circuit-timing).

<span id="qiskit-runtime-limitations"></span>

## Einschränkungen von Quantum Compute {#quantum-compute-limitations}

Die Unterstützung für `stretch` in Quantum Compute ist derzeit experimentell und hat folgende Einschränkungen:
...
- Der von einer Menge von Barrieren umgebene Bereich wird als _Barrierenregion_ bezeichnet. Eine stretch-Variable darf nicht in mehreren Barrierenregionen verwendet werden.
    <Tabs>
        ...
        ![Illustration der vorherigen Codeausgabe](/docs/images/guides/stretch/bad-barrier-region.svg 'Invalid use of stretch in barrier regions')
     ...
     <TabItem value="Valid" label="Gültig">
        ...
        ![Illustration der vorherigen Codeausgabe](/docs/images/guides/stretch/good-barrier-region.svg 'Valid use of stretch in a barrier region')
