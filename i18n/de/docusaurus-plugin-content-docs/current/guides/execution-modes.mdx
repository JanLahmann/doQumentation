...
Als Qiskit Runtime eingeführt wurde, konnten Benutzer Schaltkreise nur als einzelne Jobs ausführen. Mit dem Aufkommen verschiedener Arten von Quanten-Workloads wurde der Bedarf an unterschiedlichen Scheduling-Strategien deutlich. Die Ausführungsmodi bestimmen, wie Ihre Jobs geplant werden, und die Wahl des richtigen Ausführungsmodus ermöglicht es Ihrem Workload, effizient innerhalb Ihres Budgets zu laufen. Es gibt drei Ausführungsmodi: Job, Session und Batch.

## Job-Modus {#job-mode}

Eine einzelne Primitive-Anfrage des Estimators oder Samplers, die ohne Context Manager ausgeführt wird. Schaltkreise und Eingaben werden als Primitive Unified Blocs (PUBs) verpackt und als Ausführungsaufgabe auf dem Quantencomputer eingereicht. Um im Job-Modus zu laufen, geben Sie `mode=backend` an, wenn Sie ein Primitive instanziieren. Siehe [Primitives-Beispiele](/guides/primitives-examples) für die Verwendung.
...
Das System parallelisiert oder verarbeitet die Vorverarbeitungsphase (klassisches Computing) jedes Primitive-Jobs gleichzeitig, um die Quantenausführung über Jobs hinweg enger zu packen, und führt dann die Quantenausführung jedes Jobs in schneller Folge aus, um die effizientesten Ergebnisse zu liefern. Weitere Details zum Threading finden Sie auf der [FAQ-Seite zu Ausführungsmodi.](/guides/execution-modes-faq#parallel)

![Eine Reihe von Jobs, die im Batch-Modus ausgeführt werden. Der klassische Berechnungsteil jedes Jobs erfolgt gleichzeitig, dann werden alle Jobs an die QPU gesendet. Die QPU ist von dem Zeitpunkt an für Ihre Nutzung gesperrt, an dem der erste Job die QPU erreicht, bis der letzte Job auf der QPU verarbeitet wurde. Es gibt keine Lücke zwischen den Jobs, in der die QPU inaktiv ist.](/docs/images/guides/execution-modes/batch.svg 'Wie ein Batch funktioniert')

<Admonition type="note" title="Hinweise">
    - Beim Batching ist nicht garantiert, dass Jobs in der Reihenfolge ausgeführt werden, in der sie eingereicht wurden. Obwohl Ihre Batch-Jobs so eng wie möglich zusammen ausgeführt werden, erhalten sie keinen exklusiven Zugriff auf das Backend. Daher können Ihre Batch-Jobs parallel zu den Jobs anderer Benutzer laufen, wenn auf der QPU ausreichend Verarbeitungskapazität vorhanden ist. Zusätzlich können QPU-Kalibrierungsjobs zwischen den Batch-Jobs ausgeführt werden.
...
Um im Batch-Modus zu laufen, geben Sie `mode=batch` an, wenn Sie ein Primitive instanziieren, oder führen Sie den Job in einem Batch-Context-Manager aus. Siehe [Jobs im Batch ausführen](/guides/run-jobs-batch) für Beispiele.
...
![Eine Reihe von Jobs wird im Session-Modus ausgeführt und die andere wird im Batch-Modus ausgeführt. Zwischen jedem Job befindet sich die Interactive TTL (Interactive Time to Live). Das aktive Fenster beginnt, wenn der erste Job startet, und endet, nachdem der letzte Job abgeschlossen ist. Nachdem der letzte Job der ersten Gruppe von Jobs abgeschlossen ist, endet das aktive Fenster und die Session wird pausiert (aber nicht geschlossen). Eine weitere Gruppe von Jobs beginnt dann und Jobs werden auf ähnliche Weise fortgesetzt. Die QPU ist während der gesamten Session für Ihre Nutzung reserviert.](/docs/images/guides/execution-modes/session.svg 'Wie eine Session funktioniert')

Um im Session-Modus zu laufen, geben Sie `mode=session` an, wenn Sie ein Primitive instanziieren, oder führen Sie den Job in einem Session-Context-Manager aus. Siehe [Jobs in einer Session ausführen](/guides/run-jobs-session) für Beispiele.
...
Vollständige Details zu den TTL-Timern finden Sie im Leitfaden [Maximale Ausführungszeit](/guides/max-execution-time).
...
<Admonition type="tip" title="Empfehlungen">
    ...
    - [Wählen Sie den richtigen Ausführungsmodus.](/guides/choose-execution-mode)
    - [Lesen Sie die FAQs zu Ausführungsmodi.](/guides/execution-modes-faq)
</Admonition>
