...
Hole die `target`-Informationen vom `backend` und speichere die Operationsnamen als `basis_gates`, da das `target` modifiziert werden muss, um Timing-Informationen für die im Dynamical Decoupling verwendeten Gates hinzuzufügen.

<Admonition type="note">
Das Mock-Backend `FakeSherbrooke` aus `qiskit_ibm_runtime` wird in diesen Beispielen verwendet, du kannst es aber auf jedem Qiskit-kompatiblen echten oder Fake-Backend ausprobieren. Deine Ergebnisse können abweichen.
</Admonition>
