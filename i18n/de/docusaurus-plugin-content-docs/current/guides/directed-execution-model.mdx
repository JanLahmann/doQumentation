---
description: 'Verwende dieses komponierbare, explizite Ausführungsmodell für deine Utility-Scale-Experimente, um Fehlerminderung und andere Techniken feinabzustimmen.'
...
Alle Komponenten im Directed Execution Model befinden sich derzeit in der Beta-Phase und sind möglicherweise nicht stabil. Du bist eingeladen, sie zu testen und Feedback zu geben, indem du ein Issue im [Samplomatic](https://github.com/Qiskit/samplomatic/issues)- oder [Qiskit Runtime](https://github.com/Qiskit/qiskit-ibm-runtime/issues)-GitHub-Repository öffnest.
</Admonition>
...
Ein Hauptziel des Directed Execution Models besteht darin, eine modulare Möglichkeit zur Anwendung von Fehlerminderungsmethoden bereitzustellen. Du kannst beispielsweise definieren, welche Schichten in der Schaltung mitigiert werden sollen, oder die in die Schaltung injizierten Rauschpegel anpassen.

Um Fehlerminderung auf eine Schaltung im Rahmen dieses Frameworks anzuwenden, umfasst dein Workflow typischerweise die folgenden Schritte (die hier erwähnten Tools werden im [nächsten Abschnitt](#samplomatic-tools) ausführlicher beschrieben):

1. Gruppiere Anweisungen in Boxen und wende Annotationen auf sie an. Die Annotationen erfassen die beabsichtigte Transformation, ohne tatsächlich die Schaltungsvarianten zu generieren.


    *Bei Bedarf*: Lerne die Rauschmodelle der eindeutigen Schichten mit dem neuen [NoiseLearnerV3](#noiselearnerv3).

2. Erstelle die Template-Schaltung und Samplex aus der geboxten Schaltung.


3. Führe die Template-Schaltung und Samplex mit dem [Executor](#executor)-Primitiv aus, das die Schaltungsvarianten wie angewiesen generiert und ausführt.


4. Nachbearbeitung der Ausführungsergebnisse. Du kannst beispielsweise Postselection anwenden oder mitigierte Erwartungswerte aus den Ausführungsergebnissen extrapolieren.
...
## Tools für das Directed Execution Model {#tools-for-the-directed-execution-model}
...
Um mehr über Samplomatic zu erfahren, besuche die [Guides](https://qiskit.github.io/samplomatic/guides/index.html) und die [API-Referenz](https://qiskit.github.io/samplomatic/)-Dokumentation. Sende gerne Feedback und melde Fehler in seinem [GitHub](https://github.com/Qiskit/samplomatic)-Repository.

<span id="executor"></span>

### Executor-Primitiv {#executor-primitive}

Executor ist ein neues IBM Quantum-Primitiv, das das (Template-Schaltung, Samplex)-Paar als Eingabe nimmt, Parameterwerte gemäß dem Samplex generiert und bindet, die gebundenen Schaltungen auf der Hardware ausführt und die Ausführungsergebnisse und Metadaten zurückgibt. Es folgt den Anweisungen des Eingabepaars und trifft keine impliziten Entscheidungen für dich, sodass der Prozess transparent und dennoch leistungsfähig ist. 

### NoiseLearnerV3 {#noiselearnerv3}

Ähnlich wie der aktuelle [`NoiseLearner`](/guides/noise-learning) gibt dieses `qiskit-ibm-runtime`-Hilfsprogramm das sparse [Pauli-Lindblad-Rauschmodell](https://arxiv.org/abs/2201.09866) zurück, das in vielen Fehlerminderungsmethoden verwendet wird, einschließlich [PEC](/guides/error-mitigation-and-suppression-techniques#probabilistic-error-cancellation-pec), [PEA](/guides/error-mitigation-and-suppression-techniques#probabilistic-error-amplification-pea) und [PNA](https://github.com/Qiskit/qiskit-addon-pna/tree/main). Im ursprünglichen `NoiseLearner` übergibst du eine Liste von Schaltungen, und das Programm stratifiziert die Schaltungen in Schichten und gibt das Rauschmodell für jede eindeutige Schicht zurück. `NoiseLearnerV3` hingegen gibt dir Kontrolle darüber, wie du deine Schaltungen stratifizierst, und das Programm nimmt einfach eine Liste von geboxten Schaltungsanweisungen (z. B. eindeutige Schichten) als Eingaben. 
...
`NoiseLearnerV3` wurde mit [`qiskit-ibm-runtime` v0.47.0](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/release-notes#0470-2026-05-12) veröffentlicht. Um mehr zu erfahren, lies den [Noise-Learning-Leitfaden](/guides/noise-learning#noiselearnerv3) und die [API-Referenz](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/noise-learner-v3-noise-learner-v3)-Dokumentation.

## Nächste Schritte {#next-steps}

<Admonition type="tip" title="Empfehlungen">
- Sieh dir zwei Qiskit-Addons an, [Shaded lightcones](https://qiskit.github.io/qiskit-addon-slc/) und [Propagated noise absorption](https://github.com/Qiskit/qiskit-addon-pna), die auf diesem Ausführungsmodell aufbauen.
</Admonition>
