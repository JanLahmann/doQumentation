...
1. Gehe zu [Manage → IAM → Roles](https://cloud.ibm.com/iam/roles) und klicke auf `Create` (nur für Kontoadministratoren sichtbar).

2. Gib einen Namen, eine ID, eine Beschreibung ein und wähle `Quantum Compute` für den Service.
...
1. Gehe zu [Manage → IAM → Roles](https://cloud.ibm.com/iam/roles) in IBM Cloud. Wähle Quantum Compute aus dem Dropdown-Menü, um die Rollen anzuzeigen, die derzeit für diesen Service existieren.
...
6. Wähle im Service-Dropdown-Menü Quantum Compute. Es erscheint eine Tabelle, die alle Aktionen enthält, die du der Rolle zuordnen kannst.
...
10. Wähle unter Service Quantum Compute aus und klicke dann auf Next.
