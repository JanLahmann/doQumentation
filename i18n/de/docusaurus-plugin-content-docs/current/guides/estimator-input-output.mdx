...
Die optionalen `parameter values` können eine Liste oder ein einzelner Parameter sein. Elemente aus observables und parameter values werden gemäß den NumPy-Broadcasting-Regeln kombiniert, wie im Thema [Primitive inputs and outputs](primitive-input-output#broadcasting-rules) beschrieben, und für jedes Element der gebroadcasteten Form wird eine Erwartungswertschätzung zurückgegeben.
...
Der folgende Code zeigt ein Beispielset vektorisierter Eingaben für das `Estimator`-Primitive und führt sie auf einem IBM&reg;-Backend als einzelnes `RuntimeJobV2 `-Objekt aus.
...
#### Wie das Estimator-Primitive den Fehler berechnet {#how-the-estimator-primitive-calculates-error}
