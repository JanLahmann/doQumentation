---
description: 'Erklärung der über den IBM Quantum Compute Service verfügbaren Fehlerminderungs- und Fehlerunterdrückungstechniken'
notebook_path: guides/error-mitigation-and-suppression-techniques.ipynb
sidebar_label: 'Fehlerminderungs- und Fehlerunterdrückungstechniken'
title: 'Fehlerminderungs- und Fehlerunterdrückungstechniken'
---
...
Die Beta-Version eines neuen Ausführungsmodells ist jetzt verfügbar. Das Directed Execution Model bietet mehr Flexibilität bei der Anpassung deines Fehlerminderungs-Workflows. Weitere Informationen findest du im [Directed execution model](/guides/directed-execution-model)-Guide.
</Admonition>
{/*
...
Die folgende Code-Zelle zeigt, wie Dynamical Decoupling für Estimator aktiviert und eine Dynamical Decoupling-Sequenz ausgewählt wird.
...
![Depiction of Pauli twirling](/docs/images/guides/error-mitigation-explanation/pauli_twirling.svg)
...
Die folgende Code-Zelle zeigt, wie Pauli Twirling aktiviert und diese Optionen für Estimator gesetzt werden. Keine dieser Optionen muss explizit gesetzt werden.
...
### TREX mit der Option `resilience_level` aktivieren {#enable-trex-by-using-the-resilience-level-option}

Du kannst TREX aktivieren, indem du das [Resilience-Level](/guides/estimator-noise-management#resilience) auf `1` setzt, wie im folgenden Beispiel gezeigt.
...
Zero-Noise Extrapolation (ZNE) ist eine Technik zur Fehlerminderung bei der Schätzung von Erwartungswerten von Observablen. Obwohl sie oft die Ergebnisse verbessert, ist nicht garantiert, dass sie ein unvoreingenommenes Ergebnis liefert.
...
Der Overhead dieser Methode skaliert mit der Anzahl der Rauschfaktoren. Die Standardeinstellungen nehmen den Erwartungswert bei drei Rauschfaktoren ab, was zu einem ungefähr dreifachen Overhead führt.

### ZNE mit der Option `zne_mitigation` aktivieren {#enable-zne-by-using-the-zne-mitigation-option}

ZNE kann aktiviert werden, indem `zne_mitigation` in den [IBM Quantum Compute resilience options](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) für Estimator auf `True` gesetzt wird.
Die Optionen für ZNE werden [hier](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-zne-options) beschrieben. Folgende Optionen sind bemerkenswert:
...
Die folgende Code-Zelle zeigt, wie ZNE aktiviert und diese Optionen für Estimator gesetzt werden. Keine dieser Optionen muss explizit gesetzt werden.
...
### ZNE mit der Option `resilience_level` aktivieren {#enable-zne-by-using-the-resilience-level-option}

Du kannst ZNE (zusammen mit TREX und Gate Twirling) aktivieren, indem du das [Resilience-Level](/guides/estimator-noise-management#resilience) auf `2` setzt, wie im folgenden Beispiel gezeigt.
...
Probabilistic Error Cancellation (PEC) ist eine Technik zur Fehlerminderung bei der Schätzung von Erwartungswerten von Observablen. Im Gegensatz zu ZNE liefert sie eine unvoreingenommene Schätzung des Erwartungswerts. Sie verursacht jedoch im Allgemeinen einen größeren Overhead.
...
PEC kann aktiviert werden, indem `pec_mitigation` in den [IBM Quantum Compute resilience options](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-resilience-options-v2) für Estimator auf `True` gesetzt wird.
Die Optionen für PEC werden [hier](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options-pec-options) beschrieben. Eine Begrenzung des Sampling-Overheads kann mit der Option `max_overhead` festgelegt werden. Beachte, dass die Begrenzung des Sampling-Overheads dazu führen kann, dass die Präzision des Ergebnisses die angeforderte Präzision überschreitet. Der Standardwert von `max_overhead` ist 100.

Die folgende Code-Zelle zeigt, wie PEC aktiviert und die Option `max_overhead` für Estimator gesetzt wird.
...
- Sieh dir das [Tutorial](/tutorials/combine-error-mitigation-techniques) zur Kombination von Fehlerminderungs-Optionen mit dem Estimator-Primitiv an.

- [Konfiguriere das Noise Management mit Estimator.](/guides/estimator-noise-management)

- [Konfiguriere das Noise Management mit Sampler.](/guides/sampler-noise-management)

- Erkunde andere [Optionen](/guides/runtime-options-overview) für die IBM Quantum Primitives.
