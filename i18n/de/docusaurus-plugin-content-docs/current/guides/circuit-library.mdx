---
description: 'Erfahre mehr über vorgefertigte Schaltkreise, die von der Qiskit-Schaltkreis-Bibliothek bereitgestellt werden, einschließlich N-lokaler, Zeitentwicklungs- und Datenkodierungs-Schaltkreise'
...
Diese Seite listet die verschiedenen Schaltkreis-Kategorien auf, die die Bibliothek bereitstellt. Eine vollständige Liste der Schaltkreise findest du in der [Schaltkreis-Bibliothek API-Dokumentation](https://docs.quantum.ibm.com/api/qiskit/circuit_library).
...
## N-lokale Schaltkreise {#n-local-circuits}
...
Damit kannst du diesen Parametern auch mithilfe eines Dictionarys der Form `{ Parameter: number }` reale Werte zuweisen. Zur Veranschaulichung weist die folgende Codezelle jedem Parameter im Circuit den Wert `0` zu.
...
Für weitere Informationen siehe [N-local gates](https://docs.quantum.ibm.com/api/qiskit/circuit_library#n-local-circuits) in der Schaltkreis-Bibliothek API-Dokumentation oder besuche den [Variational algorithm design course](/learning/courses/variational-algorithm-design) im IBM Quantum Learning.

## Datenkodierungs-Schaltkreise {#data-encoding-circuits}
...
## Zeitentwicklungs-Schaltkreise {#time-evolution-circuits}
...
Lies die [`PauliEvolutionGate` API-Dokumentation](https://docs.quantum.ibm.com/api/qiskit/qiskit.circuit.library.PauliEvolutionGate).

## Benchmarking- und Komplexitätstheorie-Schaltkreise {#benchmarking-and-complexity-theory-circuits}
...
## Arithmetik-Schaltkreise {#arithmetic-circuits}
...
<Admonition type="tip" title="Empfehlungen">
  -  Lerne fortgeschrittene Methoden zum Erstellen von Schaltkreisen im Thema [Schaltkreise konstruieren](/guides/construct-circuits).
  -  Sieh ein Beispiel für verwendete Schaltkreise im Tutorial [Grover's Algorithm](/tutorials/grovers-algorithm).
  -  Lies die [Schaltkreis-Bibliothek API](https://docs.quantum.ibm.com/api/qiskit/circuit_library)-Referenz.
</Admonition>
