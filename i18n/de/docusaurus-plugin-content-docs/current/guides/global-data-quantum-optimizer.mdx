...
Authentifiziere dich mit deinem [API-Schlüssel](https://quantum.cloud.ibm.com/) und wähle die Qiskit Function wie folgt aus. (Dieses Snippet setzt voraus, dass du bereits [dein Konto gespeichert hast](/guides/functions-get-started#install-qiskit-functions-catalog-client) in deiner lokalen Umgebung.)
...
Im Fall des Real Amplitudes Ansatzes kannst du einem linearen Zusammenhang zwischen ``population_size`` und der Anzahl der Qubits im Circuit folgen. Als grobe Faustregel wird empfohlen, für den `real_amplitudes`-Ansatz ein **Minimum** von ``population_size ~ 0.8 * n_qubits`` zu verwenden.

Es wird erwartet, dass die Optimized Real Amplitudes eine bessere Optimierungsleistung als der Real Amplitudes Ansatz erzielen. Allerdings steigt die Anzahl der zu optimierenden Variablen bei diesem Ansatz viel schneller als beim Real Amplitudes Fall (siehe das [Manuskript](https://arxiv.org/pdf/2412.19150)). Daher benötigt Optimized Real Amplitudes bei großen Problemen mehr Circuit-Ausführungen. Optimized Real Amplitudes ist wahrscheinlich für Probleme mit bis zu 100 Qubits nützlich, aber es wird empfohlen, beim Festlegen der ``population_size``-Parameter vorsichtig zu sein. Als Beispiel für diese Skalierung von ``population_size`` zeigt die vorherige Tabelle, dass Optimize Real Amplitudes für ein Problem mit 84 Qubits 120 ``population_size`` benötigt, während für ein Problem mit 56 Qubits eine ``population_size`` von 40 ausreicht.
...
![Visualisierung der Lösung der Optimierung](/docs/images/guides/global-data-quantum-optimizer/cost_distribution.avif)
