...
Obwohl diese Arten von Gates in der [Bibliothek der Standardgates](/guides/circuit-library) enthalten sind, die ein `QuantumCircuit` besitzen kann, können sie nur auf spezifischen IBM Quantum QPUs verwendet werden und müssen mit dem auf `True` gesetzten Flag `use_fractional_gates` geladen werden (siehe unten). Dieses Flag stellt sicher, dass fraktionale Gates im `Target` des Backends für den Transpiler enthalten sind.
...
## Welche Backends unterstützen fraktionale Gates? {#which-backends-support-fractional-gates}

Nicht alle Backends unterstützen fraktionale Gates. Führe `service.backends(use_fractional_gates=True)` aus, um diejenigen zu finden, die es tun. Mit der Veröffentlichung von `qiskit-ibm-runtime` v0.48 kannst du auch `service.least_busy(fractional_gates=True)` ausführen, um das am wenigsten ausgelastete Backend zu finden, das fraktionale Gates unterstützt. Ein Fake-Backend unterstützt fraktionale Gates nur, wenn das zugehörige "echte" Backend sie unterstützt.

Beachte, dass Fake-Backends fraktionale Gates standardmäßig nicht unterstützen, da fraktionale Gates standardmäßig deaktiviert sind. Um fraktionale Gates auf einem Fake-Backend zu aktivieren, das sie unterstützt, aktualisiere dessen Konfiguration.
...
<Admonition type="tip" title="Empfehlungen">
  ...
  -  Verstehe, wie du [Fehlerminderung konfigurierst](/guides/configure-error-mitigation).
</Admonition>
