---
description: 'Wie man einen Quantum-Computing-Job in einer Session des IBM Quantum Compute Service ausführt.'
...
Im Job-Modus wird eine einzelne Primitiv-Anfrage des Estimator oder des Sampler ohne Context Manager gestellt. Beispiele für die Ausführung einer Quantenschaltung mit [Estimator](/guides/estimator-rest-api) und [Sampler](/guides/sampler-rest-api) findest du hier.

## Session-Modus mit REST-API {#session-mode-with-rest-api}

Eine Session ist eine Funktion, mit der du Multi-Job-iterative Workloads effizient auf Quantencomputern ausführen kannst. Die Verwendung von Sessions hilft, Verzögerungen zu vermeiden, die durch das separate Einreihen jedes Jobs verursacht werden, was besonders nützlich für iterative Aufgaben sein kann, die häufige Kommunikation zwischen klassischen und Quantenressourcen erfordern. Weitere Details zu Sessions findest du in der [Dokumentation](/guides/execution-modes).
...
Alternativ kannst du einen Batch-Job einreichen, indem du den `mode` in der Request-Payload angibst. Der Batch-Modus kann helfen, die Verarbeitungszeit zu verkürzen, wenn alle Jobs im Voraus bereitgestellt werden können. Erfahre mehr über den Batch-Modus im [Introduction to execution modes](/guides/execution-modes#batch-mode)-Guide.
...
- Überprüfe detaillierte [Sampler](/guides/sampler-rest-api)-Primitiv-Beispiele mit REST-API.
- Überprüfe detaillierte [Estimator](/guides/estimator-rest-api)-Primitiv-Beispiele mit REST-API.
- Übe mit Primitiven, indem du die [Cost function lesson](/learning/courses/variational-algorithm-design/cost-functions) in IBM Quantum&reg; Learning durcharbeitest.
