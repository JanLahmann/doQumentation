...
Die KI-gestützten Transpiler-Passes sind Passes, die als direkter Ersatz für „traditionelle" Qiskit-Passes bei bestimmten Transpilierungsaufgaben dienen. Sie liefern oft bessere Ergebnisse als bestehende heuristische Algorithmen (z. B. geringere Tiefe und CNOT-Anzahl) und sind zugleich deutlich schneller als Optimierungsalgorithmen wie SAT-Solver. Die KI-Transpiler-Passes laufen in deiner lokalen Umgebung.
...
Um die KI-Transpiler-Passes zu verwenden, installiere zunächst das Paket `qiskit-ibm-transpiler`. In der [API-Dokumentation von qiskit-ibm-transpiler](https://docs.quantum.ibm.com/api/qiskit-ibm-transpiler) findest du weitere Informationen zu den verschiedenen verfügbaren Optionen.
...
Die folgenden Optionen werden in diesem Beispiel verwendet:
* `coupling_map` - Gibt an, welche Coupling Map für die Transpilation verwendet werden soll.
* `ai_optimization_level` - Gibt das Optimierungslevel (1-3) an, das für die KI-Komponenten des PassManager verwendet wird.
* `optimization_level` - Gibt an, wie viel Optimierung am Circuit für die heuristischen Komponenten des PassManager durchgeführt werden soll.
* `ai_layout_mode` - Gibt an, wie der KI-Routing-Teil des PassManager das Layout handhabt. Im Abschnitt [KI-Routing-Pass](#ai-routing-pass) findest du die Konfigurationsoptionen für diesen `ai_layout_mode`-Parameter.

## Zitierung {#citation}

Zitierung
Wenn du KI-gestützte Funktionen aus dem Paket `qiskit-ibm-transpiler` in deiner Forschung verwendest, verwende die folgende empfohlene Zitierung:
