...
Installiere zunächst `qiskit-ibm-transpiler` mit einigen zusätzlichen Abhängigkeiten:

```sh
pip install qiskit-ibm-transpiler[ai-local-mode]
```

Nach der Installation der zusätzlichen Abhängigkeiten ist der Standardmodus für die KI-gestützten Transpiler-Passes der lokale Betrieb auf deinem Rechner.
...
Zitierung
Wenn du KI-gestützte Funktionen des Qiskit Transpiler Service in deiner Forschung verwendest, verwende die folgende empfohlene Zitierung:
