...
{/* doqumentation-source-hash: 53028a1b */}
...
Diese Vorlage kapselt einen Workflow, um die Zeitentwicklung eines Anfangszustands gegenüber einem benutzerdefinierten, spinbasierten Hamiltonian zu simulieren und gibt eine Reihe von angegebenen Erwartungswerten zurück. Dabei kommt das [AQC-Tensor](https://qiskit.github.io/qiskit-addon-aqc-tensor/) Qiskit-Addon zum Einsatz.
...
Schreibe zunächst eine Function-Vorlage für die Hamiltonsche Simulation, die das [AQC-Tensor Qiskit-Addon](https://qiskit.github.io/qiskit-addon-aqc-tensor/) verwendet, um die Problembeschreibung auf einen tiefenreduzierten Schaltkreis für die Ausführung auf Hardware abzubilden.

Wenn du diese Seite herunterlädst und lokal in einem Notebook-Editor öffnest, siehst du, dass einige Code-Zellen den [Magic Command](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) `%%writefile` enthalten. Dieser Magic Command speichert den Code in `./source_files/template_hamiltonian_simulation.py`, welche die Function-Vorlage ist, die du in Qiskit Serverless hochladen und remote ausführen kannst.
...
Während das Function Template ausgeführt wird, ist es hilfreich, mithilfe von print-Anweisungen Informationen in die Logs zu schreiben, damit du den Fortschritt des Workloads besser einschätzen kannst. Im Folgenden findest du ein einfaches Beispiel für die Ausgabe von `estimator_options`, damit ein Protokoll der tatsächlich verwendeten Estimator-Optionen vorliegt. Im gesamten Programm gibt es viele weitere ähnliche Beispiele, um den Fortschritt während der Ausführung zu melden, darunter den Wert der Zielfunktion während der iterativen Komponente von AQC-Tensor sowie die Zwei-Qubit-Tiefe des finalen Instruction Set Architecture (ISA)-Circuits, der für die Ausführung auf Hardware vorgesehen ist.
...
<Accordion>
<AccordionItem title="**Vollständiger Quellcode des Programms**">
