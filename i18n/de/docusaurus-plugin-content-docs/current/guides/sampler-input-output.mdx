...
Der folgende Code zeigt ein Beispielset vektorisierter Eingaben für das `Sampler`-Primitive und führt sie auf einem IBM&reg;-Backend als einzelnes `RuntimeJobV2 `-Objekt aus.
...
Zusätzlich zu den Ausführungsergebnissen enthalten sowohl das `PrimitiveResult`- als auch das `SamplerPubResult`-Objekt ein Metadata-Attribut zum übermittelten Job. Die Metadaten mit Informationen zu allen übermittelten PUBs (wie den verschiedenen verfügbaren [runtime options](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/options)) findest du in `PrimitiveResult.metatada`, während die für jeden PUB spezifischen Metadaten in `SamplerPubResult.metadata` zu finden sind.
...
Die Ergebnisse von [`SamplerV2`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/sampler-v2)-Jobs, die im IBM Quantum Compute Service ausgeführt werden, enthalten in ihren Metadaten Informationen zur Ausführungszeit.
