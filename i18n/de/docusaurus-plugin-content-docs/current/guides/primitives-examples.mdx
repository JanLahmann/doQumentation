---
title: "Primitives-Beispiele"
sidebar_label: "Primitives-Beispiele"
description: "Praktische Beispiele zur Verwendung von Primitives in Qiskit Runtime."
notebook_path: "guides/primitives-examples.ipynb"
---

{/* source-hash: 2647e702 */}
...
Die Beta-Version eines neuen Ausführungsmodells ist jetzt verfügbar. Das gerichtete Ausführungsmodell bietet mehr Flexibilität beim Anpassen deines Fehlerminderungsworkflows. Weitere Informationen findest du im Leitfaden zum [gerichteten Ausführungsmodell](/guides/directed-execution-model).
</Admonition>
{/*
...
Die Beispiele in diesem Abschnitt zeigen einige gängige Verwendungsweisen von Primitives. Bevor du diese Beispiele ausführst, befolge die Anweisungen unter [Installieren und einrichten.](install-qiskit)

<Admonition type="note">
    Diese Beispiele verwenden alle die Primitives aus Qiskit Runtime, du könntest jedoch stattdessen die Basis-Primitives verwenden.
</Admonition>

## Estimator-Beispiele {#estimator-examples}

Berechne und interpretiere Erwartungswerte der Quantenoperatoren, die für viele Algorithmen benötigt werden, effizient mit dem Estimator. Erkunde Anwendungen in der Molekülmodellierung, im maschinellen Lernen und bei komplexen Optimierungsproblemen.
...
Verwende den Estimator, um drei Experimente in einem einzigen Job auszuführen, wobei Parameterwerte genutzt werden, um die Wiederverwendbarkeit von Circuits zu erhöhen.
...
## Sampler-Beispiele {#sampler-examples}

Erzeuge vollständige fehlergeminderte Quasi-Wahrscheinlichkeitsverteilungen, die aus Quantencircuit-Ausgaben gesampelt werden. Nutze die Fähigkeiten des Samplers für Such- und Klassifikationsalgorithmen wie Grover's und QVSM.

### Ein einzelnes Experiment ausführen {#run-a-single-experiment-sampler}

Verwende den Sampler, um das Messergebnis als Bitstrings oder Zählungen eines einzelnen Circuits zurückzugeben.
...
### Mehrere Experimente in einem einzigen Job ausführen {#run-multiple-experiments-in-a-single-job-sampler}

Verwende den Sampler, um das Messergebnis als Bitstrings oder Zählungen mehrerer Circuits in einem Job zurückzugeben.
...
### Parametrisierte Circuits ausführen {#run-parameterized-circuits-sampler}

Führe mehrere Experimente in einem einzigen Job aus und nutze Parameterwerte, um die Wiederverwendbarkeit von Circuits zu erhöhen.
...
### Sessions und erweiterte Optionen verwenden {#use-sessions-and-advanced-options-sampler}
...
<Admonition type="tip" title="Empfehlungen">
    - [Erweiterte Runtime-Optionen festlegen.](runtime-options-overview)
    - Übe mit Primitives, indem du die [Lektion zu Kostenfunktionen](/learning/courses/variational-algorithm-design/cost-functions) im IBM Quantum Learning absolvierst.
    - Lerne, wie du lokal transpilierst, im Abschnitt [Transpile](./transpile/).
    - Probiere den Leitfaden [Transpiler-Einstellungen vergleichen](/guides/circuit-transpilation-settings) aus.
