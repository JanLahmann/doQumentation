...
- Führe in `qiskit-ibm-runtime` v0.30 oder später [`job.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/runtime-job-v2#usage) für einen einzelnen Job aus, oder [`batch.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/batch#usage) bzw. [`session.usage()`](https://docs.quantum.ibm.com/api/qiskit-ibm-runtime/session#usage), wenn du den Batch- oder Session-Modus verwendest. Wenn du eine ältere Version von `qiskit-ibm-runtime` (>= 0.23 und < 0.30) verwendest, kann die Nutzung immer noch in `session.details()["usage_time"]` und `batch.details()["usage_time"]` gefunden werden.

- Verwende [`GET /sessions/{id}`](https://docs.quantum.ibm.com/api/qiskit-runtime-rest/tags/sessions#tags__sessions__operations__get_session), um die Nutzung für einen bestimmten Batch oder eine Session anzuzeigen (der Endpunkt ist für beide Ausführungsmodi gleich).

- Verwende [`GET /jobs/{id}`](https://docs.quantum.ibm.com/api/qiskit-runtime-rest/tags/jobs#tags__jobs__operations__get_job_metrics_jid), um die Nutzung für einen einzelnen Job anzuzeigen.
...
- `<per sub-job overhead>` ist ein Overhead von etwa 2s pro Sub-Job. Dazu gehören Vorgänge wie das Laden der Payload in die Steuerelektronik. Dein Primitive-Job kann in mehrere Sub-Jobs aufgeteilt werden, wenn er zu groß ist, um von der Ausführungs-Engine auf einmal verarbeitet zu werden.
