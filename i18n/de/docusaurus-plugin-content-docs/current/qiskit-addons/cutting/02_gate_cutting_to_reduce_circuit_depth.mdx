...
### Einen Circuit erstellen, der auf dem Backend ausgeführt wird {#create-a-circuit-to-run-on-the-backend}
...
### Ein Backend festlegen {#specify-a-backend}
...
### Den Circuit transpilieren, die Swaps visualisieren und die Tiefe notieren {#transpile-the-circuit-visualize-the-swaps-and-note-the-depth}
...
### Entfernte Gates durch `TwoQubitQPDGate`s ersetzen, indem ihre Indizes angegeben werden {#replace-distant-gates-with-twoqubitqpdgates-by-specifying-their-indices}
...
**Hinweis:** Das ``observables``-Kwarg von `generate_cutting_experiments` ist vom Typ `PauliList`. Koeffizienten und Phasen der Observablen-Terme werden bei der Zerlegung des Problems und der Ausführung der Subexperimente ignoriert. Sie können bei der Rekonstruktion des Erwartungswerts wieder angewendet werden.
...
### Demonstrieren, dass die QPD-Teilexperimente nach dem Schneiden entfernter Gates flacher sein werden {#demonstrate-that-the-qpd-subexperiments-will-be-shallower-after-cutting-distant-gates}
...
### Teilexperimente für das Backend vorbereiten {#prepare-subexperiments-for-the-backend}
