...
{/* doqumentation-source-hash: 3d5a6a04 */}
...
![Verrauschtes Experiment](/img/qiskit-addons/images/noisy_expt.png)
Wenn wir das Rauschmodell erlernen, können wir dessen Inverse anwenden und das Rauschen aufheben.
![Rauschgemindertes Experiment](/img/qiskit-addons/images/noise_mitigated_expt.png)
...
![PNA-Übersicht](/img/qiskit-addons/images/pna_overview.png)
...
![Quantencircuit-Diagramm](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_1.png)
...
![Quantencircuit-Diagramm](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_2.png)
...
Hier stellen wir sicher, dass der Pass-Manager die Boxen mit ``Twirl``- und ``InjectNoise``-Annotationen versieht, mit denen wir das Rauschen erlernen können, das unseren Circuit beeinflusst, und dieses Rauschen der entsprechenden Circuit-Schicht zuordnen können.
...
- ``measure_annotations: all`` Fügt `Twirl`- und `ChangeBasis`-Annotationen zur Messbox hinzu

- ``twirling_strategy: active``: Alle aktiven Qubits in jeder Box, die verschränkende Gates enthält, twirlen.

- ``inject_noise_targets: gates``: ``InjectNoise``-Annotationen sollten zu allen mit ``Twirl`` annotierten Boxen hinzugefügt werden, die verschränkende Gates enthalten
...
![Quantencircuit-Diagramm](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_3.png)
...
![Quantencircuit-Diagramm](/img/qiskit-addons/pna/01-generate-noise-mitigating-observable/output_4.png)

#### Learn the noise {#learn-the-noise}
...
Beim Ausführen von PNA musst du immer einen verrauschten Circuit und eine Observable angeben. Wenn dein verrauschter Circuit ein geboxter Circuit mit `InjectNoise`-Annotationen ist, musst du die im obigen Schritt erstellte Zuordnung angeben. Man kann auch einen nicht geboxten Circuit übergeben, der ``PauliLindbladError``-Instruktionen aus ``qiskit-aer`` enthält. In diesem Fall muss ``refs_to_noise_models`` nicht angegeben werden. Zusätzlich zu den primären Eingaben solltest du Folgendes berücksichtigen:
...
- ``samplex_arguments``: Bindungen, die zur vollständigen Definition des Samplex notwendig sind
    ...
    - ``noise_scales.ref``: Wir setzen die Skala jeder Rauschschicht auf `0.0`, um zu verhindern, dass zusätzliches Rauschen in unsere Samples injiziert wird
...
- ``shape``: Ein Shape-Tupel zur Erweiterung der impliziten Form, die durch ``samplex_arguments`` definiert wird. Nicht-triviale Achsen, die durch diese Erweiterung eingeführt werden, zählen Randomisierungen auf.
...
Nachdem wir nun unser ``QuantumProgram`` definiert haben, ist die Ausführung des Experiments unkompliziert. Wir instanziieren einfach das ``Executor``-Objekt, geben ihm das Backend an und führen das Programm aus.
