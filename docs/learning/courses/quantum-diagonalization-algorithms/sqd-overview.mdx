...
Hs = np.array([[x1.T @ H @ x1, x1.T @ H @ x2], [x2.T @ H @ x1, x2.T @ H @ x2]])
print(Hs)
```
...
Note that in writing the projected Hamiltonian elements as `x1.T @ H @ x2`, we are implicitly adopting the common convention that the state vectors `x1` and `x2` correspond to kets (column vectors), consistent with expressions such as $x_1 = |011\rangle$. However, NumPy's 1D arrays do not intrinsically distinguish between row and column vectors, and for the vectors used here the transpose operation has no numerical effect. Consequently, writing `x1 @ H @ x2.T` would produce the same numerical result, although `x1.T @ H @ x2` more closely matches the usual Dirac-notation correspondence $\langle x_1|H|x_2\rangle$.
...
Hs = np.array(
    [
        [x1.T @ H @ x1, x1.T @ H @ x2, x1.T @ H @ x3],
        [x2.T @ H @ x1, x2.T @ H @ x2, x2.T @ H @ x3],
        [x3.T @ H @ x1, x3.T @ H @ x2, x3.T @ H @ x3],
    ]
)
print(Hs)
```
...
H_new_s = np.array(
    [
        [x1.T @ H_new @ x1, x1.T @ H_new @ x2, x1.T @ H_new @ x3, x1.T @ H_new @ x4],
        [x2.T @ H_new @ x1, x2.T @ H_new @ x2, x2.T @ H_new @ x3, x2.T @ H_new @ x4],
        [x3.T @ H_new @ x1, x3.T @ H_new @ x2, x3.T @ H_new @ x3, x3.T @ H_new @ x4],
        [x4.T @ H_new @ x1, x4.T @ H_new @ x2, x4.T @ H_new @ x3, x4.T @ H_new @ x4],
    ]
)
print(H_new_s)
```
