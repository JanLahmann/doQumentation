...
1. Retrieve your API key from the [IBM Quantum Platform dashboard](https://quantum.cloud.ibm.com/), and activate your Python virtual environment.  See the [installation instructions](/guides/install-qiskit#local) if you do not already have a virtual environment set up.
   ...
   The instance you authenticate with must have Qiskit Functions access enabled. To configure it on an existing instance, see [Configure Qiskit Functions access on an instance](/guides/access-instances-platform-apis#configure-qiskit-functions-access-on-an-instance).
