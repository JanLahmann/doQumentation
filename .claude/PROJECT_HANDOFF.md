...
- **Deployed app** — `ce-doqumentation-01.27boe8ie8nv4.eu-de.codeengine.appdomain.cloud` (eu-de region), project `ce-doqumentation-01`. Min scale 0, max scale 1. Default size varies (use `resize-pod.yml` workflow or IBM Cloud Console to set). Token must be 32+ chars. Sizing validated via stress tests: 8 vCPU/16 GB handles 80 users with retry helpers (see `.claude/STRESS-TEST-FINDINGS.md`).
- **Scale-to-zero fix** — CE/Knative doesn't reliably scale multi-process containers to zero (Istio mesh connections + queue-proxy keepalive make the pod look "active"). Two-part fix: (1) `keepalive_timeout 0` in `nginx-codeengine.conf` eliminates the nginx↔queue-proxy keepalive connection. (2) Idle shutdown watchdog in `sse-build-server.py`: checks every 60s, after `IDLE_SHUTDOWN_MINUTES` (default 15, env var) of zero kernels + zero connections + zero SSE, sends SIGTERM to PID 1 (supervisord) → pod terminates → CE won't restart (minScale=0). Next request triggers standard cold start (~12s). WebSocket timeouts reduced from 86400s (24h) to 7200s (2h). Investigation: `kubectl exec` + `ss -tnp` showed ESTAB connections from Istio sidecar and queue-proxy even with zero external traffic; `target-burst-capacity=0` annotation and app delete+recreate both failed to fix it.
...
- **User flow** — Two paths on Settings page (`/jupyter-settings#code-engine`):
  ...
  - **Join Workshop**: smart URL field accepts single URL, comma-separated URLs, or base64-encoded pool config `{"pool":[...]}` (no token inside — security). Token entered separately. Calls `saveWorkshopPool(urls, token)` which enables workshop pool mode with sticky session assignment per tab. The base64 pool config (without token) is generated by `workshop-start.yml` and posted to the job summary — safe for public repos.
...
- **Workshop mode — single-pod stress testing COMPLETE** (Apr 11-12 2026). Validated 1/4/8/12 vCPU pod sizes. Found and fixed: nginx rate limit (5→100 r/m), kernel cull tuning (600→300s), cgroup-aware `/stats` metrics, SSE shim async refactor (tornado), Jupyter race-condition retries (server-side `/api/status` retry + client-side WS handshake retry). **Final sizing with retries**: 8 vCPU/16 GB handles 80 users with ZERO failures at 5s burst (was 15 failures pre-retry). 12 vCPU/24 GB handles 80-100 users. Capacity scales ~6 sessions/vCPU with diminishing returns above 8 vCPU (SSE shim GIL contention, then Jupyter race conditions). Memory never the constraint for 5-qubit workloads (max 44% across all tests). Full writeup: `.claude/STRESS-TEST-FINDINGS.md`.
  ...
  - **Instructor flow**: run Workshop Start from GH Actions UI → read pod URL from summary, token from IBM Cloud Console or CLI → share URL publicly (slide/QR), token privately (verbal/handout). Students: open `/jupyter-settings#code-engine` → Join Workshop → paste URL(s) or base64 pool config + token separately → Join.
  ...
  - **Multi-pod still untested end-to-end**: `workshop-start.yml` supports `instance_count` > 1 and generates base64 pool config. Settings page "Join Workshop" accepts base64 pool (no token). But the actual multi-pod pool with frontend random-assignment has never been tested with real CE deploys. Required for workshops >80 users.
