...
- **Scale-to-zero fix** — CE/Knative doesn't reliably scale multi-process containers to zero because the queue-proxy maintains an HTTP/1.1 keepalive connection to nginx (port 8080), which the autoscaler counts as active. Fix: `keepalive_timeout 0` in `nginx-codeengine.conf` forces nginx to close connections after each response. WebSocket timeouts reduced from 86400s (24h) to 7200s (2h) — long enough for any workshop session, short enough to not linger. Investigation: `kubectl exec` + `ss -tnp` showed ESTAB connection from queue-proxy→nginx even with zero external traffic; `target-burst-capacity=0` annotation and app delete+recreate both failed to fix it.
