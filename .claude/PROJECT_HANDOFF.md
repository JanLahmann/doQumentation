...
**Content:** 42 Tutorials, 171 Guides, 160 Course pages (14 courses), 14 Modules, 11 Qiskit Addon tutorials (~397 pages total). Qiskit Addons are built and URL-accessible (`/qiskit-addons/...`) but currently hidden from navbar and main sidebar.
...
- **Single canonical location**: notebook-extracted output images (the `output_N.png` files emitted by `extract_cell_outputs` from notebook cells) live ONCE under `static/img/<section>/<stem-slug>/output_N.png`. Served by Docusaurus from absolute URL `/img/<section>/<stem-slug>/output_N.png`.
  - `<section>` = relative path from `docs/` to the MDX parent dir. Examples: `workshop`, `qiskit-addons/cutting`, `qiskit-addons/sqd`.
  - `<stem-slug>` = lowercased notebook stem with non-alphanumeric chars collapsed to `-`. Example stem `04_Hands-on Introduction to Qiskit` → slug `04-hands-on-introduction-to-qiskit`.
- **All MDX (EN + 17 i18n locales) reference the same files** via absolute paths — no per-locale duplication. Adding a new locale does NOT require copying any output PNGs.
- **Why**: prior layout used `docs/<section>/_<stem>_imgs/` plus a duplicate copy under each `i18n/<loc>/.../current/<section>/_<stem>_imgs/`. With 17 main locales × 14 notebook image dirs, this meant ~12 MB of byte-identical PNGs duplicated across the repo and 1,800 relative `./_<stem>_imgs/output_N.png` references in MDX. Migration date: 2026-05-06 (PR #34, branch `feat/dedupe-output-images`). One-shot migration script: `scripts/migrate_output_imgs.py`.
- **`sync-content.py` enforces this for new notebooks**: `convert_notebook()` writes images to `STATIC_DIR / 'img' / section / stem_slug` and emits `![alt](/img/<section>/<stem-slug>/<file>)` (absolute URL) instead of the legacy `./_<stem>_imgs/<file>` relative path. This means future upstream syncs cannot re-introduce the legacy layout.
- **Other image asset trees (untouched)**: `docs/qiskit-addons/images/` (shared diagrams across multiple addons, copied per-locale by `populate-locale`), `docs/qiskit-addons/_static/` (notebook-static assets), `static/img/logo.svg`, `static/img/rasqberry-social-card.png`. These are NOT extracted notebook outputs and follow their existing conventions.
- **CI guard**: `.github/workflows/lint-image-paths.yml` runs on every PR touching `docs/`, `i18n/`, or `sync-content.py`. Fails the build if (a) any MDX has `](./_<stem>_imgs/...)` relative refs, or (b) any `_*_imgs/` dir is committed under `docs/` or `i18n/`. Bypassable only by deleting the workflow — don't.
- **For contributors adding new images by hand** (e.g. a new diagram for a guide): place them under `static/img/<section>/<descriptive-name>/...` and reference with absolute paths. Don't put PNGs alongside MDX files.
...
- **Admin page** (`/admin`): Hidden reference page for admins/workshop hosts. Analytics dashboard link, CI/CD workflow links, translation commands, workshop checklist, satellite repo list. Not in navbar, excluded from search engines (`robots.txt` Disallow). Password-gated via SHA-256 hash of `ADMIN_PASSWORD` env var. Sensitive URLs (e.g. Umami share dashboard) are AES-256-GCM encrypted at build time (`scripts/encrypt-for-admin.mjs`) and decrypted client-side with the admin password — plaintext never appears in source or JS bundle. Secrets: `ADMIN_PASSWORD`, `UMAMI_SHARE_URL` in GitHub Secrets; workflow encrypts at build time via `ADMIN_ENCRYPTED_UMAMI_SHARE_URL`.
...
- **Analytics**: Umami Cloud (cookie-free, GDPR-compliant, no consent banner). Script in `headTags`, centralized `src/config/analytics.ts` module. Custom events: Run Code, Run All, Binder Launch, Colab Open, Notebook Download, Tutorial Feedback, Translation Feedback, Outbound, Outbound IBM. Locale tracked via custom Pageview event with hostname-derived locale. Auto-disabled on localhost/Docker. Dashboard: https://cloud.umami.is
  - **Outbound link tracking** (`src/clientModules/outboundTracker.ts`): Single capture-phase listener (mousedown + auxclick, so middle-click and cmd-click are caught before navigation tears down) walks up from `event.target` to the enclosing `<a>` and fires an `Outbound` event for every external click. Skips same-host and `*.doqumentation.org` (locale subdomains). IBM hosts additionally fire `Outbound IBM` for backward compatibility. Categories (`category` property): `quantum-platform`, `quantum-docs`, `quantum-learning`, `ibm-cloud`, `ibm-marketing`, `ibm-video`, `ibm-other`, `github`, `external-other`. Path-aware for IBM consolidation: `quantum.cloud.ibm.com/docs/*` → `quantum-docs`, `/learning/*` → `quantum-learning`, root → `quantum-platform`. Properties: `category`, `host`, `path`, `url` (origin+pathname, no query/hash to bound cardinality), `from` (source page), `locale`. Captures navbar API Reference, navbar GitHub icon, footer GitHub links, MDX content links — no per-link annotation needed.
...
├── docs/                       # Content (tracked in git — regenerated by sync-content.py, but committed so per-file history is available for the source-date footer and PR review of upstream syncs)
...
├── i18n/                       # Translations: all 17 standard locales at 412/412 (100% — incl. use-a-qc-today course added 2026-05-07). German dialects: ksh (46), nds (43), gsw (42), sax (39), bln (36), aut (34), swg/bad/bar (31 each)
├── local-content/              # In-repo course overlay (e.g. use-a-qc-today) — merged into upstream-docs/learning/courses/ at sync time. Used to add a single course without pulling other upstream changes into the JanLahmann/Qiskit-documentation fork.
...
**Generated but tracked in git:** `docs/tutorials/`, `docs/guides/`, `docs/learning/`, `docs/qiskit-addons/`, `sidebar-*.json` — produced by `sync-content.py` but committed so each upstream sync produces a reviewable diff and `git log` returns real per-file history (used by the source-date footer).

**Generated and gitignored (large/binary):** `notebooks/`, `static/docs/`, `static/learning/images/`
...
|--------|-----|-------|--------|
| DE | [de.doqumentation.org](https://de.doqumentation.org) | **412/412 + UI (100%)** | Live |
| ES | [es.doqumentation.org](https://es.doqumentation.org) | **412/412 + UI (100%)** | Live |
| FR | [fr.doqumentation.org](https://fr.doqumentation.org) | **412/412 + UI (100%)** | Live |
| UK | [uk.doqumentation.org](https://uk.doqumentation.org) | **412/412 + UI (100%)** | Live |
| JA | [ja.doqumentation.org](https://ja.doqumentation.org) | **412/412 + UI (100%)** | Live |
| IT | [it.doqumentation.org](https://it.doqumentation.org) | **412/412 + UI (100%)** | Live |
| PT | [pt.doqumentation.org](https://pt.doqumentation.org) | **412/412 + UI (100%)** | Live |
| TL | [tl.doqumentation.org](https://tl.doqumentation.org) | **412/412 + UI (100%)** | Live |
| AR | [ar.doqumentation.org](https://ar.doqumentation.org) | **412/412 + UI (100%)** | Live (RTL) |
| HE | [he.doqumentation.org](https://he.doqumentation.org) | **391/412 + UI (95%)** | Live (RTL) |
| MS | [ms.doqumentation.org](https://ms.doqumentation.org) | **412/412 + UI (100%)** | Live |
| ID | [id.doqumentation.org](https://id.doqumentation.org) | **412/412 + UI (100%)** | Live |
| TH | [th.doqumentation.org](https://th.doqumentation.org) | **391/412 + UI (95%)** | Live |
| KO | [ko.doqumentation.org](https://ko.doqumentation.org) | **412/412 + UI (100%)** | Live |
| PL | [pl.doqumentation.org](https://pl.doqumentation.org) | **412/412 + UI (100%)** | Live |
| RO | [ro.doqumentation.org](https://ro.doqumentation.org) | **412/412 + UI (100%)** | Live |
| CS | [cs.doqumentation.org](https://cs.doqumentation.org) | **412/412 + UI (100%)** | Live |
...
### TODO
- **Translation expansion** — **All 17 standard locales now at 100% (412/412)** including KO+PL after the May 5 archive-branch merge (DE, ES, FR, IT, UK, JA, AR, PT, TL, HE, MS, ID, TH, CS, RO, KO, PL). HE/TH still have 21 unrelated files (about + qiskit-addons) at fallback (391/412, 95%). The use-a-qc-today course was added 2026-05-07: 6 new files per locale, with 2 (`build-and-run-...`, `quantum-mechanics-basics`) seeded from the now-removed workshop notebooks 01/02 to save translation tokens. Validation pass rates vary: TH/MS/ID/CS still have 13–213 ✗ files needing fixes despite full structural coverage. Run `python translation/scripts/translation-status.py --all` for current counts. German dialects: KSH 46, NDS 43, GSW 42, SAX 39, BLN 36, AUT 34.
- **Full linguistic review** — Sample-based review (6 files/locale) done for CS, PL, KO, RO (Mar 28). Found and fixed: PL meaning error (Pauli twirling→entanglement), PL false cognate (transpose→transpile, 4x), CS fabricated verb, CS/KO/RO register slips, RO terminology drift, typos/diacritics. **AR full review COMPLETE** (Mar 28, Opus): all 387 files reviewed → 252 PASS, 120 MINOR_ISSUES, 14 FAIL. All issues fixed (72 files, 383 edits). Key findings: Egyptian dialect contamination in 9 learning/module files (retranslated to MSA), 2 files with untranslated content (translated), terminology disasters fixed ("bachelors"→"spins", "distillation"→"diagonalization"). Cross-cutting terminology harmonized: observable, quasi-probability, resilience, Pauli twirling. Full issue log: `memory/project_ar_review_issues.md`. Post-fix validation: 386/387 PASS (1 false positive). **DE/ES/FR/IT full reviews COMPLETE** (Mar 28): merged from dedicated review branches. DE ~150 fixes (84 files): Sie→du conversion, untranslated sections, spelling/math errors. ES accent/diacritic fixes + tú/usted consistency (110 files, +1727/-1730). FR gate→porte terminology + capitalization + ~120 fixes (146 files). IT ~1160 fixes across 207 files (most comprehensive). Review reports: `translation/drafts/de/linguistic-review-2026-03-28.md`, `translation/drafts/fr/linguistic-review.md`, `translation/reviews/it-linguistic-review.md`. **Phase 1 Sonnet top-up review COMPLETE** (May 7–8): 9 mostly-done locales (de/es/fr/it/ja/uk/pt/tl/ar) topped up to 100% reviewed via Sonnet 5-file chunks — 222 late-promote files reviewed, plus a 27-file spot-check of prior Haiku PASS verdicts that revealed a 33% miss rate. Lesson: Haiku is unreliable for review (fabricates "word salad" verdicts on coherent text); always use Sonnet, 5 files/chunk. Phase 4 cheap fixes applied (May 8, 619 replacements / 289 files): JA `ご〜ください`/`ご参照ください`/`お読みください`/`ご利用ください` honorifics neutralized site-wide (409/157), UK formal-plural imperatives `зверніться`/`перегляньте`/`беріть` → informal singular (125/83), TL `Ang inyong runtime` boilerplate → `Ang iyong runtime` (57/40), DE `slc/01_getting_started.mdx` PEC terminology + 4 typos, FR `veuillez` slips, PT broken env-var fragment, AR `slc/01` observable canonicalization (`المتغير الملاحظ` → `العنصر القابل للرصد`, 11), TL `panganib na matematika` (danger) → `pinagbabatayan` (underlying). 12 files re-recorded as FIXED in `status.json`. **HE Phase 2 review COMPLETE** (May 8, four passes): 422 of 412 files reviewed (100%+ — overcount due to multi-pass re-reviews) via Sonnet 5-file chunks. All 94 chunks of the bin-packed plan have been exercised. Earlier reports of 235/411 had stale "workshop/01-02" entries that have since been cleaned up. First pass (99 files) revealed dominant formal-register pattern; site-wide find-replace cleared 1448 register/terminology slips: `אנו`→`אנחנו` (1011), `עיין`→`ראה` (219), `אנא`→drop (77), `בקר ב`→`כנס ל` (50), `משרה/משרות`→`עבודה/עבודות` (91, calque "employment" used for compute "job"), `דויטש-יוזה`→`דויטש-ג'וזה` (37 across two passes). Second pass (136 more files) exercised remaining content with the register noise removed. 26 surgical fixes for accuracy/typos surviving find-replace, including critical reversals: `quantum-mechanics-basics.mdx` little-endian description (rightmost was called leftmost), `bits-gates-and-circuits.mdx` "encrypt" used for "observe", `qiskit-code-assistant-openai-api.mdx` negation inversion, `dynamical-decoupling-pass-manager.mdx` Bloch sphere → counting/sefirah, Cyrillic intrusion (Russian "произвольный" in `DAG-representation.mdx`). Third pass (May 8, 25 chunks c070–c094) added 46 more verdicts and surfaced two more systemic terminology cleanups: `מצבות` (tombstones) → `מצבים` (states) — 16 fixes / 4 files; `אובזרבבל*` (doubled-ב transliteration of "observable") → `אובזרבל*` — 107 fixes / 26 files. Plus a content gap in `learning/index.mdx` (the 14th course, use-a-qc-today, was missing from the HE list). HE structural now 412 PASS / 0 FAIL (was 411/1 before fixes). Lesson: HE translator had strong biblical/literary register defaults; one find-replace pass cleared the noise and made real accuracy issues visible. Full triage: `.claude/PHASE_4_LINGUISTIC_TRIAGE_2026-05-07.md`. **Phase 3 progress through May 8**: Register sections for ko/pl/ms/id/ro/cs added to `translation/review-prompt.md` (commit `3450fda21`). **KO Phase 3 COMPLETE — 100% reviewed (414/414)** via 4 Sonnet waves: 383 PASS / 22 MINOR / 5 FAIL (all FAILs are intentional `doqumentation-untranslated-fallback` markers). Site-wide deferential cleanup landed across 26 KO files: `보여드릴게요`/`보여드렸습니다`/`보여드리겠습니다`/`보여드린 것처럼` → 살펴봐요/살펴봤어요/살펴봐요/앞서 본 것처럼; `사용하시길 권장합니다`→`사용하길 권장해요`; `안내해 드립니다`/`도와드립니다`/`키워드립니다`/`보여 드립니다` → 안내해요/도와드릴게요/키워드릴게요/보여드릴게요; `원하시는`/`완료하시는` → plain forms; `되고 싶으신가요?`→`되고 싶나요?`; `## Lesson video` → `## 강의 영상` (4 introduction.mdx files). KO commits: `cbf1b2016` (wave 3), `15adcc8a6` (wave 4). **TH/PL/CS Phase 3 stratified-sample reviews COMPLETE** (May 8): 4-wave parallel Sonnet runs (~45 files each, balanced across guides/tutorials/courses/qiskit-addons/workshop). **TH** (`ce6071604`) — 44 files: 41 PASS + 2 MINOR + 1 FAIL pre-fix; 3 fixes applied: `tutorials/chsh-inequality.mdx` quantum/classical Bell bound labels were swapped (now ±2√2 outer / ±2 inner), `workshop/04` dropped closing teleportation paragraph translated and appended, `cutting/03_wire_cutting_via_move_instruction` editorial annotation about EN typo "(detination)" removed; cross-cutting: zero polite particles (ครับ/ค่ะ) across all 44 files, hybrid technical-term style (Qubit/Circuit kept English) consistent. **PL** (`90a81d95c`) — 45 files: 33 PASS + 11 MINOR + 1 FAIL pre-fix; 6 fixes: `ansaetze-and-variational-forms` dropped closing paragraph translated; `workshop/04` Polish-convention `Bramka X/H/CX` headings; `workshop/05` `Proszę utworzyć konto` → informal imperative; `workshop/06_solution` person-number drift `Zachęcam ... życzymy` → `Zachęcamy ... życzymy`; `index.mdx` untranslated `<strong>No IBM Quantum account?</strong> Enable...` translated; `classical-optimizers.mdx` ~8-line untranslated EN bullet block describing `minimize` arguments translated. Zero Pan/Pani register slips. Capitalization of Circuit/Qubit/Gate as proper nouns is widespread but consistent — cosmetic only. **CS** (in flight on session pause) — 45 files: 30 PASS + 13 MINOR + 2 FAIL pre-fix; 2 fixes for the FAILs: `quantum-business-foundations/quantum-computing-fundamentals.mdx` 6 untranslated EN headings translated (Provázání, Interference, Ověř si porozumění ×2, Kvantové obvody, Programování kvantového obvodu, Klíčové poznatky) with anchor pins `{#check-your-understanding}` `{-2}` `{-3}`; `classical-optimizers.mdx` same minimize-bullet block as PL/CS — translated. Zero Vy/Vám register slips. **TODO remaining**: RO + MS + ID stratified samples (each at ~10/412 reviewed); deeper TH/PL/CS coverage if needed (current samples at 59/55/55 of 412 with high PASS rate). Translator passes for TL `workshop/03*` (~75% English) + AR `quantum-diagonalization-algorithms/` (`تقطير`→`قطرنة` systematic) + KO `tutorials/transpilation-optimizations-with-sabre.mdx` (8 untranslated EN passages in Part II Serverless section, lines 579-823).
...
- **Download notebook option** — ✅ DONE. "Download ↓" button added to `OpenInLabBanner` (between Colab and "What's modified?"). Links to `/notebooks/{path}.ipynb` (static assets from `sync-content.py`). Uses HTML `download` attribute. i18n-wrapped (`openInLab.download`, `openInLab.download.title`). Analytics event `Notebook Download` added to `analytics.ts`.
- **Remove Qamposer** — ✅ DONE (Apr 24). Removed `/qamposer` page, `src/components/QamposerEmbed/` directory, `@qamposer/react` dependency, all i18n `qamposer.*` keys (11 per locale × 18 locales), and `qamp-*.mjs`/`find-chunk*.mjs` debug scripts.
- **Plan-type disclaimer for non-Open Plan users** — On the IBM Quantum Account Settings section, add a visible disclaimer when "Plan type" is set to anything other than `Open` (i.e. `payg` or `premium`): credentials are stored in the browser's localStorage (plain text, readable by extensions and anyone with device access), so this site is **not recommended** for paid-tier accounts. doQumentation's target audience is Open Plan / entry-level Qiskit users; users on paid plans should use the official IBM Quantum platform directly or run notebooks locally with their own auth. Implementation: add a yellow/red `<div className="alert alert--warning">` below the Plan dropdown in `src/pages/jupyter-settings.tsx`, conditional on `executionMode === 'credentials' && ibmPlan !== 'open'`. Translate via existing `<Translate id="settings.ibm.paidPlanWarning">` pattern.
- **Make kernel-side code injections transparent** — `injectKernelSetup()` silently runs Python in the kernel before user cells (save_account guard, simulator monkey-patches `_DQ_MockService`, Open Plan `_DQ_JobModeSession`, warning suppression, save_account placeholder guard). Users running the same notebook elsewhere (local Jupyter, IBM platform) won't have these injections and may be confused when behavior differs. Add an expandable "Show injected setup code" panel — either in `OpenInLabBanner` next to the existing "What's modified?" link, or as a dedicated `/about/code-modifications` enhancement (page already exists). Should display the **exact** Python being injected (not just descriptions) so users can: (1) learn the patterns, (2) reproduce manually elsewhere, (3) audit what runs in their kernel. Sources: `getSaveAccountCode()`, `getSimulatorPatchCode()`, `getOpenPlanPatchCode()` in [src/components/ExecutableCode/index.tsx](src/components/ExecutableCode/index.tsx). Build a `getInjectedSetupCode()` helper that concatenates whatever would actually run for the current execution mode + page (respecting `SIMULATOR_EXEMPT_PAGES`, plan type, etc.) and renders it in a collapsible code block. Translate the surrounding UI; keep the Python code itself untranslated.
...
- **Qiskit Addons Phase 2 expansion** — Phase 1 shipped 11 tutorial notebooks from 7 addons. Each addon repo has substantial additional documentation in `docs/how_tos/` (or `how-tos/`) and `docs/explanation(s)/` that is not yet integrated. Inventoried 2026-04-10:
  ...
  - **Caveats before shipping**: (1) dependency bloat — each addon's pip package may pull additional data-processing libs; how-to notebooks can be heavier than tutorials. (2) Binder image size already ~3.5 GB; Phase 2 may push enough to hurt startup. Mitigation: keep addons as `%pip install` cells only (no pre-install). (3) Translation fallback disk/build cost — 19 × 22 locales = 418 new fallback MDX files. Real translations (when produced) duplicate per-notebook PNG outputs into each locale dir per existing convention — see "i18n PNG deduplication" TODO above. (4) Addons are currently hidden from navbar (`9cb954fe`); decide separately when to unhide. (5) All 10 addon repos are Apache 2.0 per MEMORY.md — no license re-work needed. (6) After May 5 sync: `docs/qiskit-addons/` now uses per-notebook `_<stem>_imgs/` subdirs uniformly — Phase 2 syncs will follow the same convention since `convert_notebook` always uses subdirs (line 462 of `sync-content.py`).
...
- **Translate `index.mdx` new sections** — ✅ DONE (PRs #12, #14). All 26 non-EN locales now have the "When you're ready for more" section with anchor `#when-youre-ready-for-more`. PR #12 (`354dec580`) covered the 17 standard languages (de/es/uk/fr/it/pt/ja/tl/ar/he/ms/id/th/ko/pl/ro/cs). PR #14 (`ea00b7967`) covered the 9 dialects (swg/bad/bar/ksh/nds/gsw/sax/bln/aut). Dialect translations were authored by Claude with no native speaker review — flagged for a review pass at next translation review cycle.
- **i18n PNG deduplication** — Translations under `i18n/<loc>/.../qiskit-addons/` currently duplicate the supporting PNGs into each locale dir (precedent set by main's pre-existing CS/KO/PL/RO). After May 5 merges there are ~660 PNG copies (~3-5 MB total). Git deduplicates blobs internally so .git size unchanged, but working-tree size grows linearly with locale count. If size becomes a concern, a Docusaurus build plugin or static-routes config could resolve `./output_N.png` from `docs/` when missing from the locale — larger refactor affecting every translation that references local images.

### Resolved (May 2026)
- **Add "Use a quantum computer today" course** (May 7) — Cherry-picked the new IBM course (5 lessons + index + 12 images, ~1833 lines) into the repo without modifying the JanLahmann/Qiskit-documentation fork or pulling unrelated upstream changes. **Mechanism:** new `local-content/` overlay folder; patched `sync-content.py` so `process_courses()`, `generate_course_sidebar()`, and `sync_upstream_images()` iterate `upstream-docs/learning/courses/` then merge `local-content/learning/courses/` on top. Sidebar grew from 13 to 14 courses. Removed two now-redundant workshop notebooks (`workshop-notebooks/01_build-and-run-...ipynb`, `02_quantum-mechanics-basics.ipynb`) since the course supersedes them; their EN MDX outputs auto-pruned by `process_workshops()`. Translations: 17 locales × 6 files = **102 files**, with the 2 lessons that overlap the workshop seeded from the existing workshop translations (saved ~5–6× tokens vs. cold). Validation pass rate 100% after fixing: PL/RO/DE YAML quote bugs (curly-open + straight-close mismatch in description fields), IT/TL/HE spurious `<OpenInLabBanner>` injected into code-free files, HE wrong heading levels and relative `/signin` link, and 9 locales' `index.mdx` missing the `{#overview}` anchor.
- **Index "When you're ready for more" translations — all 26 locales** (May 5, PRs #12 + #14) — Translated the new homepage section (introduced by main's `docs/index.mdx` commit `708d5774c`) into all 26 non-EN locales. PR #12 (`354dec580`) covered the 17 standard languages (de/es/uk/fr/it/pt/ja/tl/ar/he/ms/id/th/ko/pl/ro/cs). PR #14 (`ea00b7967`) added the 9 German/Austrian dialect locales (swg/bad/bar/ksh/nds/gsw/sax/bln/aut), each matched to that locale's existing orthography conventions (du/ihr-Anrede, vowel shifts, particle verbs). Each adds 10 lines (heading + lead + 3 bullets + acknowledgement) just before the locale's `## Getting started` heading. Dialect translations need native speaker review.
- **Archive-branch merge — `claude/translation-status-review-nal3k`** (May 5) — 6-step staged merge of the 662-commit archive branch (PRs #5–#10). 699 files total, +83k / −16k lines. Steps: (1) 56 workshop + about/code-modifications mdx across 10 locales (PR #5 `c87099c7a`); (2) 102 KO+PL fallback→real upgrades (PR #6 `6ddf2a855`); (3) 30 workshop 03+04 mdx missed by step 1's filename glob due to spaces (PR #7 `2014b07b9`); (4) 6 qiskit-addons `index.mdx` translations for de/es/fr/it/ja/uk (PR #8 `da1b5588c`); (5) 50 single-notebook qiskit-addons translations + 276 PNGs sed-flattened from per-notebook _imgs/ subdirs to flat layout (PR #9 `107058b95`); (6a) regenerated `docs/qiskit-addons/` via `sync-content.py --skip everything-else` to fix a pre-existing image-clobber bug from initial sync `ee479dc56` where cutting/ and sqd/ used flat `output_N.png` and notebook 1/2/3 outputs were overwritten by notebook 4 / notebook 2's outputs (PR #10 commit `4fc9b5616`); (6b) 28 cutting/01-03 + sqd/01 translations + 81 PNGs (PR #10 commit `fd75f34f8`). Source branch `claude/translation-status-review-nal3k` deleted from origin; archive tag `archive/claude-translation-status-review-nal3k` preserves full 662-commit history. Sibling archive `archive/claude-verify-translation-status-ujt0Y` from a smaller deleted branch also preserved. Both KO and PL now at 100% (408/408) coverage.

### Resolved (April 2026)
- **Branch integration (Apr 9)** — Merged 4 `claude/*` branches into main. **`claude/video-transcript-translations-P5nk1`**: 374 new VTT files completing all 17-locale translations for 22 IBM Video transcripts (55/55 videos now fully translated across all locales). **`claude/plan-ai-features-I1bzx`**: cherry-picked — adds `.claude/AI_FEATURES_BRAINSTORMING.md` (~30 AI feature ideas across search, learning, debugging, content, infra — tiered by cost $0 / $0–5 / $140+; Smart Search, Adaptive Learning Paths, Granite/Code Engine RAG). **`claude/document-todos-ideas-UQz3F`**: cherry-picked 8 of 9 commits — Qiskit Addons Phase 1 (7 submodules, sync-content.py extension, `sidebar-addons.json`, navbar entry later hidden via `Hide Qiskit Addons from navbar and main sidebar`), UX polish commit (`86a97b0f`): breadcrumbs enabled, Code Engine status messages (was still saying "Binder" for cache-miss/slow-startup), `TutorialFeedback` widget (Umami-tracked, appended to all tutorial MDX via sync-content.py), `TranslationFeedback` banner on non-EN pages, "View in English" link at top of locale dropdown, code-injection transparency (setup notice cell in exported notebooks, "What's modified?" link on `OpenInLabBanner`, new `/about/code-modifications` page listing every automatic transform). Skipped one commit (`499645f5` regenerated tutorial MDX — reproducible via sync-content.py) and deferred `7e50ebba` (upstream-docs submodule bump). Conflict resolution: 22 tutorial MDX files had bottom-chunk conflicts from the feedback widget injection — resolved with `--theirs` since content is regenerable. **`claude/add-qamposer-doqumentation-4IIR4`**: merged — unlisted `/qamposer` page (later removed Apr 24). Added `@qamposer/react` npm dependency (later removed).
...
- **UI translations 100%** — 210 new keys translated across all 17 non-dialect locales (DE, ES, FR, IT, UK, JA, PT, TL, AR, HE, TH, MS, ID, KO, PL, RO, CS). Coverage: 308/484 → 518+/518. Covers Settings, ExecutableCode, Features, feedback widgets, "View original" link, workshop banner.

## Additional Changes (April 20, 2026)

- **Diff-based translation update tool** — `translation/scripts/update-translations.py` (838 lines). Three-phase pipeline: DETECT (parse MDX → align blocks by heading anchors → classify severity NOOP/MINOR/MODERATE/MAJOR) → AUTO-FIX (sync code blocks, imports, images, source hash) → WORKFILE (JSON for translation agents). Reuses `check-translation-freshness.py`, `validate-translation.py`, `sync-translations.py`. Needs runtime testing before production use.
- **Workshop notebooks renumbered** — 8 notebooks now `01_` through `06_` prefix, with two new IBM "Use a QC Today" course notebooks at `01_` (Build & Run First) and `02_` (Quantum Mechanics Basics). Existing notebooks shifted +2.
- **Broken image refs stripped from IBM course notebooks** — `process_workshops()` in `sync-content.py` strips `/learning/images/courses/use-a-qc-today/...` references from the two new IBM notebooks (3 image refs in notebook 01). The images are generated by IBM's proprietary build pipeline and don't exist in the open-source repo. Hardcoded list `STRIP_IMAGES_NOTEBOOKS = {'01_build-and-run-your-first-quantum-program', '02_quantum-mechanics-basics'}`.
- **`Accordion` / `AccordionItem` MDX stubs** — Added to `src/theme/MDXComponents.tsx`. New IBM course notebooks use these for collapsible Q&A sections. Render as native `<details>`/`<summary>` for graceful fallback.
- **Homepage: "When you're ready for more" section** — Guides users to IBM's official platform for real hardware, latest content, local Qiskit install. Acknowledges IBM Quantum team for open-source content (CC BY-SA 4.0). Placed after "What this project adds", before "Getting started".
- **Content version on Features page** — `scripts/sync-content.py` now generates `src/config/contentMeta.ts` with `UPSTREAM_COMMIT` + `UPSTREAM_COMMIT_DATE` (git-tracked, regenerated on each sync). The "Auto-Sync from Upstream" feature card shows: "Last upstream content update: YYYY-MM-DD (commit abcdef)". Date reflects upstream submodule HEAD, not when sync ran locally.
- **Vague numbers on Features page** — Replaced concrete page/language counts ("381 Pages", "20 Languages", "42 Tutorials, 171 Guides...") with ballpark phrasing ("Hundreds of Pages", "Multi-Language Support") to avoid stale numbers as content grows. Stable implementation facts kept (Binder cold-start, workshop participant cap, font sizes).
- **Binder build context fix (reverted)** — Briefly added `.dockerignore` to notebooks branch excluding 26 locale dirs (40K+ files) to fix mybinder.org build failures. Reverted same day — the failures were transient mybinder.org issues, not content size. Translated notebooks needed in image for locale-aware JupyterLab.
