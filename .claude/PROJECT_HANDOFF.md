...
- **Info icons on key UI elements** — Reusable `InfoIcon` component (`src/components/InfoIcon/`) with CSS-only tooltips via `data-tooltip` + `::after`. Initial placement: 5 Settings page fields (Simulator Mode, API Token, CRN, CE URL, CE Token) + toolbar mode badge. All strings i18n-wrapped via `translate()`. More locations can be added over time.
