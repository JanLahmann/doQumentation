...
- **Video transcript generation (Whisper)** — Generate English VTT transcripts for all 59 course videos using OpenAI Whisper locally on Mac. POC script existed in `claude/synced-translated-transcripts-yKoir` branch (now deleted): `scripts/generate-transcripts.py` downloaded audio via `yt-dlp` from IBM Video or YouTube, ran Whisper, and saved VTT to `static/transcripts/{ibm_video_id}/en.vtt`. Automate as much as possible — batch all videos, skip existing, configurable model (`tiny` for drafts, `large-v3` for production). One sample VTT was generated for video 134413658 (Quantum Key Distribution).
- **Video transcript translation** — Translate English VTT files to all locales using Claude Code (not API). POC script existed on the same branch: `scripts/translate-transcripts.py` preserved timestamps, kept quantum terminology in English, used locale-appropriate register. Adapt for Claude Code CLI: `Read translation/translation-prompt.md`-style one-liner, translate VTT files in `static/transcripts/`. Future: synced transcript panel below video (click-to-seek).
