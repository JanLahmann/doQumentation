...
- **Sidebar**: Home link at top, then Tutorials/Guides/Courses/Modules categories (autogenerated from `sidebar-*.json`). Pages with executable code cells show `</>` icon (via `customProps.notebook` in sidebar JSON — detection uses `<OpenInLabBanner>` presence, not `notebook_path` frontmatter, to exclude text-only notebook pages). API Reference + Settings in navbar only.
