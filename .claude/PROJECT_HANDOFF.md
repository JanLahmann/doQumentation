...
- **Backend selection UI** — When multiple execution backends are configured (CE + Binder, or CE + custom), let the user choose which one to use for the current session. Currently `detectJupyterConfig()` applies a fixed priority (Custom > CE > Binder) with no override. Add a selector (e.g., dropdown near the execution toolbar or in Settings) so users can switch between configured backends without removing credentials.
