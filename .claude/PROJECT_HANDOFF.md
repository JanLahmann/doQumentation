...
- **Binder-enable this repo — COMPLETE** — Full Binder setup on `notebooks` branch: `deploy.yml` pushes EN notebooks + `binder/Dockerfile` (FROM `quay.io/jupyter/base-notebook:python-3.12` + pip install all 9 packages + cleanup in one RUN layer — bypasses repo2docker conda solver). `deploy-locales.yml` consolidates locale notebooks into subdirs. `binder.yml` warms all 3 federation members (2i2c, BIDS, GESIS) daily + on push. Both "Open in Binder JupyterLab" and thebelab code execution use `doQumentation/notebooks` branch with shared session reuse (`ensureBinderSession()`). Image: ~3–3.5 GB (jupyter base ~800 MB + qiskit pip ~2 GB) vs 7.6 GB conda-based previously.
