...
- **IBM Video transcript sync not working** — postMessage API for progress polling/seek not syncing. Need to debug the message format (`cycapi:getProperty` vs `apiGetProperty`), response event key, `?html5ui` parameter, and target origin. YouTube sync works fine.
...
- **Video subtitles pipeline** — 59 course videos need subtitles. Analysis (Mar 28):
  ...
  - **Phase 1 — Transcription**: `scripts/generate-transcripts.py` runs Whisper locally on Mac (`pip install openai-whisper yt-dlp`). POC done for 3 videos (medium model, good quality). YouTube download works (32 videos). IBM Video download blocked by CDN (503 on HLS fragments) — use Claude Cowork with Chrome plugin for browser-based audio capture of the 27 IBM Video-only files (future task). Run `--model medium` for production quality. **Post-processing needed**: Whisper has recurring name/term errors (e.g. "Kisket" → "Qiskit", "Watchras" → "Watrous"). Add a post-processing step to `generate-transcripts.py` with a corrections dict to fix known errors before saving VTT files.
...
*Last updated: March 28, 2026 (merged DE/ES/FR/IT full linguistic review branches — ~2,580 fixes across 547 files; AR full review also complete)*
