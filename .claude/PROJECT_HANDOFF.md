...
- **Deployed app** — `doqumentation-02.28m8ettpkpts.eu-de.codeengine.appdomain.cloud` (eu-de region), project `doQumentation-02`. Min scale 0, max scale 1. Default size 1 vCPU / 2 GB (use `resize-pod.yml` workflow or IBM Cloud Console to set). Token must be 32+ chars. Sizing validated via stress tests: 8 vCPU/16 GB handles 80 users with retry helpers (see `.claude/STRESS-TEST-FINDINGS.md`). Previous project `ce-doqumentation-01` retired Apr 12 2026 — had stale Knative state that prevented scale-to-zero.
- **Scale-to-zero** — CE/Knative scale-to-zero works reliably in **fresh projects**. The old project `ce-doqumentation-01` (32 days, 47+ revisions) had accumulated stale Knative/Istio state that prevented scale-to-zero despite minScale=0 and zero traffic. Verified by side-by-side test: two fresh projects with old (Mar 25) and new images both scaled to zero within 10 min. Solution: retired old project, created fresh `doQumentation-02`. Additional nginx improvements kept: `keepalive_timeout 0` (closes connections promptly), WebSocket timeouts reduced from 86400s to 7200s. **CE monitor workflow** (`ce-monitor.yml`): every 2h checks for running instances and oversized apps (>1 vCPU); daily 06:00 UTC summary of all projects/apps/config.
