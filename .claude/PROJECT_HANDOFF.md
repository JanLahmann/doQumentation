...
- **Simulator mode** — Monkey-patches `QiskitRuntimeService` with `_DQ_MockService` (AerSimulator or FakeBackend). Fake backend discovery cached, 55-backend fallback list. **Exempt pages**: `SIMULATOR_EXEMPT_PAGES` in `ExecutableCode/index.tsx` — pages like `tutorials/hello-world` that demonstrate real hardware access bypass simulator interception and session annotations. Credential skip-hints still appear on exempt pages when the user has credentials configured via Settings.
...
├── i18n/                       # Translations: de/es/fr/it/uk/ja (387 each — 100%), ar (385), pt (386), tl (383), ro (307+12d), cs (222+178d), pl (218+50d), ko (45+175d), he (47), ksh (46), nds (43), gsw (42), sax (39), bln (36), aut (34), swg/bad/bar (31 each)
...
### TODO
- **Translation expansion** — **13 locales at 100% (387/387)**: DE, ES, FR, IT, UK, JA, AR, PT, TL, HE, MS, ID, TH. **4 new locales expanded** (Apr 12 squash-merge of 4 `claude/*` branches): RO 307/387 genuine + 12 drafts, CS 222/393 genuine + 178 drafts (includes addons), PL 218/387 genuine + 50 drafts, KO 45/387 genuine + 175 drafts. Drafts need validation (`validate-translation.py`) and promotion (`promote-drafts.py`). Run `python translation/scripts/translation-status.py --all` for current counts. German dialects: KSH 46, NDS 43, GSW 42, SAX 39, BLN 36, AUT 34.
