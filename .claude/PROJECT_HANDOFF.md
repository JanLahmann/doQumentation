...
- **Workshop mode testing** — Test the multi-instance Code Engine workshop setup end-to-end: deploy 2-3 instances via `codeengine-image.yml` workflow (set `instance_count`/`cpu`/`memory`), generate workshop URL, verify participant auto-import (`#workshop=BASE64`), confirm sticky session assignment, test failover when an instance is stopped, run `scripts/workshop-stress-test.py` to determine per-instance capacity and validate load distribution across the pool. Verify the monitoring dashboard (auto-refresh, memory/connections/busy columns, peak stats, structured logs in CE console). See `src/pages/workshop-setup.mdx` for the instructor guide (published at `/workshop-setup`). Key files: `src/config/jupyter.ts` (pool logic + `InstanceStats` type), `src/pages/jupyter-settings.tsx` (dashboard UI with auto-refresh), `binder/sse-build-server.py` (`/stats` endpoint with memory, counters, structured logging). Optional: enable IBM Cloud Monitoring (Sysdig) for historical metrics — see workshop-setup.mdx "Monitoring" section. **Single-pod stress testing complete** (Apr 11 2026): validated 1/4/8/12 vCPU pod sizes, found and fixed nginx rate limit + cull tuning + cgroup-aware metrics + harness `--ramp-interval` flag + SSE shim async refactor. Sizing recommendations validated: 1 × 8 vCPU/16 GB handles 50 users, 1 × 12 vCPU/24 GB handles 80 users. Multi-pod still untested. Full writeup: `.claude/STRESS-TEST-FINDINGS.md`. Live admin monitoring dashboard now at `/admin` (live `/stats` polling + sparklines + health interpretation).
- **GH Actions automations using `IBM_CLOUD_API_KEY`** — The repo secret already exists (used by `codeengine-image.yml` deploy step). Four follow-up workflows worth adding to make CE management self-service:
  - **(a) Smarter `Deploy to Code Engine` step** — Today the deploy step always sets `--cpu` and `--memory` from the `workflow_dispatch.inputs` defaults (1 vCPU / 4 GB), which silently resets pod size on every container code push. Burned ~10 min during stress testing across two manual resizes back to 12 vCPU. Fix: read current `--cpu`/`--memory` via `ibmcloud ce app get`, only override if user explicitly chose values in the dispatch UI. ~15 lines, no new secrets. **Highest priority** of the four. Audit trail in `.claude/STRESS-TEST-FINDINGS.md`.
  - **(b) `workflow_dispatch` "Stress Test" job** — Runs `scripts/workshop-stress-test.py` from CI on demand, with inputs: `users`, `ramp_interval`, `cells_per_user`, optionally `cpu`/`memory` (resize pod for the test, restore after). Could also be scheduled (weekly health check). Lets workshop instructors validate pod capacity without local IBM Cloud login. Posts results as a job summary. ~30 lines + the existing harness.
  - **(c) `workflow_dispatch` "Resize Pod" job** — Dropdown for `cpu` (1, 2, 4, 8, 12) and `memory` (auto-snapped to valid CE ratios). One-click pod resize from GitHub UI. Useful for instructors who want to scale up before a workshop without learning the `ibmcloud` CLI. ~20 lines.
  - **(d) Daily "Warm Pod" scheduled job** — Hits the CE app's root URL once every 24h to keep the image cached on its K8s node. Eliminates the cold-start tax (15-150s) for the first user of the day. Equivalent to the existing daily Binder cache warm but for CE. ~10 lines, runs on `schedule: cron`.
  - **Total**: ~75 lines across one workflow file edit + one or two new workflow files. No additional secrets needed. ~1-1.5h work for all four. **Suggested order**: (a) first as a defensive fix, (d) second as a low-risk standalone, then (b) and (c) as a pair if instructor self-service becomes a need.
