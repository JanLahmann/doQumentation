...
- **Sidebar**: Home link at top, then Tutorials/Guides/Courses/Modules categories (autogenerated from `sidebar-*.json`). Pages with executable code cells show `</>` icon (via `customProps.notebook` in sidebar JSON — detection uses `<OpenInLabBanner>` presence, not `notebook_path` frontmatter, to exclude text-only notebook pages). API Reference + Settings in navbar only. **Indicator spacing**: Link items have `padding-inline-end: 3.5rem` and category headers `4.5rem` to prevent text overlapping with absolutely-positioned indicators (`</>`, `✓`/`▶`, `N/M` badges). Long titles truncate with `text-overflow: ellipsis`.
...
- **Sidebar indicator overlap fix (Mar 2026)** — Increased `padding-inline-end` on sidebar links from `2.5rem` → `3.5rem` and category headers from `3.5rem` → `4.5rem`. Adjusted `inset-inline-end` on notebook icon (`1.75rem` → `2rem`) and category badge (`2rem` → `2.25rem`). Prevents text overlapping with absolutely-positioned indicators (`</>`, `✓`/`▶`, `N/M` badges) on longer titles.
...
*Last updated: March 17, 2026 (translation prompt improvements)*
