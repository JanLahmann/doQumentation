...
- **Run All button** — Page-level "Run All" in toolbar. Sequential execution with `waitForKernelIdle()`, progress counter "Stop (3/8)", abort on Stop/Back/Restart/navigation. Cells get green done/red error feedback immediately after each execution (explicit `settleCellFeedback()` call bypasses the 1500ms idle debounce).
