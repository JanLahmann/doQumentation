...
### TODO
- **Translation expansion** — DE at 81/387, ES at 76/387, UK at 56/387, JA at 56/387, FR at 48, IT/PT/TL at 48, HE at 47, AR at 44 (needs re-translation). German dialects: KSH (46), NDS (43), GSW (42), SAX (39), BLN (36), AUT (34). Run `python translation/scripts/translation-status.py` for current counts, or see `translation/STATUS.md`.
...
- **Error submission for contextual help** — When a cell execution fails, offer a simple "Report this error" button that captures the error message, cell source code, notebook path, and execution context (simulator vs real, packages installed). Submit to a lightweight backend (GitHub issue, Google Form, or simple API endpoint). Keep the UX minimal: one click, no form fields, optional email for follow-up.
...
- **Run All button** — Page-level "Run All" in toolbar. Sequential execution with `waitForKernelIdle()`, progress counter "Stop (3/8)", abort on Stop/Back/Restart/navigation.
...
- **Binder layer caching** — Option C deployed: `binder/Dockerfile` with `FROM quay.io/jupyter/base-notebook:python-3.12` + pip install. Bypasses repo2docker conda solver. Fallback plan in `.claude/plans/binder-layer-cache.md`.
...
- **Update features page** — 22→29 cards across 6 sections. Added CE, Colab, Bookmarks, Recent Pages, Display Preferences, Multi-Language section.
