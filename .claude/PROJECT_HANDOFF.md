...
- **Binder-enable this repo (Phase 4 pending)** — CI infra done and verified: `deploy.yml` pushes EN notebooks + Binder config to `notebooks` branch (core packages in `binder/requirements.txt` for Docker caching, 6 lighter packages + cleanup in `binder/postBuild`, `binder/runtime.txt` for Python 3.12). First successful Binder build completed on 2i2c (image layers: 7.6 GB conda base [cached], 2 GB pip install, 506 MB notebooks). `deploy-locales.yml` consolidates locale notebooks into subdirs. `binder.yml` warms 3 federation members (push trigger temporarily disabled for manual verification). **Pending**: verify Binder builds with Python 3.12 (was defaulting to 3.10 due to `runtime.txt` in wrong location — now fixed), then switch URLs in `jupyter.ts` (Phase 4). Plan: `.claude/plans/binder-enable-notebooks.md`.
