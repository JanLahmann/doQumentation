...
- **GH Actions automations using `IBM_CLOUD_API_KEY`** — three of four implemented and verified working (commit `410c2ea5`):
  ...
  - **Result**: workshop instructors can now resize and stress test pods entirely from the GitHub UI without local IBM Cloud CLI. The CI rebuild path no longer needs manual resize follow-ups. Burned ~10 min of manual resizes during the Apr 11 stress test session — fixed.
