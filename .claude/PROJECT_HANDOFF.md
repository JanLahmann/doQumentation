...
- **Binder layer caching** — Every Binder build rebuilds all layers (7.6 GB conda + 2 GB pip) from scratch because mybinder.org build workers don't share a Docker layer cache between machines. Plan in `.claude/plans/binder-layer-cache.md`. Try Option C first (pip-only, no conda → eliminates 7.6 GB base), fall back to Option B (pre-built base image on GHCR) if Qiskit packages require conda. Current mitigation: daily warming + `workflow_run` trigger keeps image alive.
