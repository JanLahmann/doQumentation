...
│   └── scripts/                # validate, lint, fix-anchors, promote, populate, status dashboard
...
- **MDX lint script** — New `lint-translation.py` catches build-breaking MDX syntax errors that `validate-translation.py` misses: duplicate heading anchors, garbled XML namespace tags, heading markers mid-line, invalid anchor characters, unmatched code fences, missing imports. Integrated into review workflow (`gemini-review-instructions.md`, `review-prompt.md`). Found 8 pre-existing unmatched code fences in AR (known broken locale).
