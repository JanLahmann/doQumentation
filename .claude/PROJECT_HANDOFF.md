...
- **OpenInLabBanner buttons**: "Open in: JupyterLab | Colab | Binder" — ordered by recommendation. JupyterLab (filled/primary, session-managed via `ensureBinderSession()`), Colab (outlined, always available), Binder (outlined, GitHub Pages only — raw mybinder.org build page, no session interception). Descriptive tooltips on hover. Uses `getRawBinderUrl()`, `getColabUrl()`, `getBinderLabUrl()` from `jupyter.ts`. **Only shown on pages with executable code cells** — `sync-content.py` skips injection for code-cell-free notebooks; 1,634 i18n pages similarly cleaned up.
- **Binder tab reuse**: "Open in Lab" uses named window target `binder-lab` to reuse the same tab
- **Run All**: "Run All" button (visible when kernel is ready) executes all thebelab cells sequentially in DOM order. Uses `waitForKernelIdle()` helper (subscribes to `kernel.statusChanged` signal) to wait for each cell to complete before starting the next. Shows "Stop (N/M)" progress during execution — click Stop to abort after current cell finishes. Also aborts on Back, Restart, Clear Session, or page navigation.
...
- **Features page**: `/features` — 29 cards across 6 sections (Content Library, Live Code Execution, IBM Quantum Integration, Learning & Progress, Multi-Language, Search/UI/Deployment)
...
- **Binder-enable this repo — COMPLETE** — Full Binder setup on `notebooks` branch: `deploy.yml` pushes EN notebooks + `binder/Dockerfile` + `binder/jupyter-requirements.txt` (FROM `quay.io/jupyter/base-notebook:python-3.12` + `pip install -r jupyter-requirements.txt` — full Qiskit ecosystem, 21 packages, bypasses repo2docker conda solver). `deploy-locales.yml` consolidates locale notebooks into subdirs. `binder.yml` warms all 3 federation members (2i2c, BIDS, GESIS) daily + on push. Both "Open in Binder JupyterLab" and thebelab code execution use `doQumentation/notebooks` branch with shared session reuse (`ensureBinderSession()`). Image: ~3.5–4.5 GB (jupyter base ~800 MB + full qiskit pip ~2.5–3 GB) vs 7.6 GB conda-based previously.
- **Cross-locale Binder session** — Switching languages (e.g., EN → DE) starts a new Binder session because `sessionStorage` is domain-scoped. Deliberate: Binder tokens are security-sensitive and short-lived.
...
- **Translation structural sync script** — When English source changes (code blocks, imports, frontmatter), translated files go stale. Code blocks should be byte-identical between EN and translations, so they can be mechanically synced without re-translating prose. Need a script that takes a translated MDX file + latest EN MDX, replaces code blocks/imports/frontmatter from EN while preserving translated text. Would complement `check-translation-freshness.py` (which detects staleness but doesn't fix it). The `%pip` → `!pip` fix (292 files via sed) was a one-off example of this pattern.
- **Raspberry Pi** — `scripts/setup-pi.sh` written but untested on actual hardware
- **Binder layer caching — Option C deployed** — `binder/Dockerfile` installs from `jupyter-requirements.txt` (full Qiskit ecosystem, 21 packages including `qiskit[all]`, all addons, scipy, pyscf, plotly, pandas, etc.). Uses `FROM quay.io/jupyter/base-notebook:python-3.12` — bypasses repo2docker's conda solver. **Important**: Dockerfile `COPY` paths must be relative to repo root (repo2docker uses repo root as Docker build context with `--file binder/Dockerfile`). Fixed Mar 7: `COPY jupyter-requirements.txt` → `COPY binder/jupyter-requirements.txt`. If cold builds are still slow, upgrade to Option B (pre-built GHCR base image, plan in `.claude/plans/binder-layer-cache.md`).
- **IBM Quantum Ecosystem listing** — Submit doQumentation.org to https://www.ibm.com/quantum/ecosystem to increase visibility.
- **"Run All" button — DONE** — Page-level "Run All" button in toolbar (visible when kernel is ready). Sequentially executes all thebelab cells in DOM order using `waitForKernelIdle()` (subscribes to `kernel.statusChanged` busy→idle signal with 2s fallback). Shows progress counter "Stop (3/8)" and toggles to a Stop button during execution. Abort on: Stop click, Back, Restart Kernel, Clear Session, or page navigation. Errors don't halt execution (mirrors Jupyter behavior).
...
- **Binder startup UX** — DONE (Mar 6): Real-time progress shown in three places: (1) `OpenInLabBanner` JupyterLab button — elapsed timer + phase hint line below banner + cache miss warning. (2) `ExecutableCode` Run button toolbar — same phase labels with durations, elapsed timer, cache miss warning banner. (3) JupyterLab loading tab (`jupyter.ts` `openBinderLab`) — live phase text, elapsed timer, cache miss warning box, failure message with Colab fallback. All three show realistic durations (fetching/pushing 2–5 min, building 5–10 min, launching 2–5 min) and immediately warn on cache miss (`building` phase = cache warming failed, total 10–25 min). Added missing `waiting` phase everywhere. Also fixed `binder.yml` — 2i2c/BIDS now require `Accept: text/event-stream` header (started failing Mar 6).
- **Rethink user intro & settings page — DONE** — Settings page reorganized: essentials at top (Simulator Mode, Display Preferences, Learning Progress), advanced sections (Code Engine, IBM Quantum, Binder Packages, Custom Server, Setup Help) collapsed in `<details>` under "Advanced Settings" heading. Hash deep-links auto-open the enclosing `<details>`. Simulator mode now defaults to ON for new users (stored as explicit `'false'` when disabled). Onboarding tip simplified to "Click Run — no setup needed" (2 visits, down from 3).
- **Info icons on key UI elements** — Add small ⓘ icons next to key UI elements (e.g., simulator mode badge, Binder status, credential fields, execution toolbar) that link to or pop up brief explanations of what the element does and why it matters. Helps new users understand the interface without reading external docs.
- **Error submission for contextual help** — When a cell execution fails, offer a simple "Report this error" button that captures the error message, cell source code, notebook path, and execution context (simulator vs real, packages installed). Submit to a lightweight backend (GitHub issue, Google Form, or simple API endpoint). This builds a dataset of real user errors that enables future context-aware help and recommendations (e.g., "Users running this cell commonly hit X — try Y"). Keep the UX minimal: one click, no form fields, optional email for follow-up.
- **Consolidate Docker images — DONE** — Merged `Dockerfile.jupyter` and `binder/Dockerfile.codeengine` into a single multi-stage `Dockerfile.jupyter` with shared `jupyter-base` stage and two build targets: `jupyter-local` (full stack + site build) and `jupyter-codeengine` (CE kernel + SSE server). Both share the same base image (`quay.io/jupyter/base-notebook:python-3.12`) and pip install layer. `binder/Dockerfile` (mybinder.org) and `Dockerfile.web` (static site) remain separate — different purposes and constraints. Updated CI workflows (`docker.yml`, `codeengine-image.yml`) and `docker-compose.yml` to use `--target`. Local Docker user changed from `jupyter` to `jovyan` (aligns with Binder/CE).
- **Skip redundant pip install cells on Binder/CE** — The prerequisite cells (`!pip install -q qiskit qiskit-ibm-runtime ...`) are injected by `sync-content.py` for Colab (where packages aren't pre-installed), but on Binder and CE all packages are already in `jupyter-requirements.txt`. These cells add ~10-30s of unnecessary `pip check` time. Options: (a) auto-hide/collapse them on Binder/CE environments, (b) detect environment at runtime and skip execution, or (c) add a "Skip — packages already installed" annotation. The cells must remain for Colab and JupyterLab-opened notebooks where the environment is unknown.
- **IBM Cloud spending limits — RESOLVED** — IBM Cloud does not support hard spending limits (per-service or account-wide). Set up spending notification at $5/month threshold via IBM Cloud console (Manage → Billing and usage → Spending notifications). Existing cost controls: CE max-scale=1 (single instance cap), min-scale=0 (scales to zero when idle), nginx rate limiting. Helper script: `scripts/ibmcloud-spending-limit.sh`. Estimated monthly cost: $0–7.
- **Audit "Open in" button visibility** — Understand why some pages show 2 buttons (JupyterLab + Colab) and others show 3 (+ Binder). The logic in `OpenInLabBanner` selects based on `detectJupyterConfig()` environment: CE/custom shows JupyterLab + Colab (no Binder), GitHub Pages shows all 3. But the notebook being served may differ: CE serves from its container (`/lab/tree/`), Binder from the `notebooks` branch, Colab from `static/notebooks/`. Verify all three point to the same notebook content and that the button set is correct for each environment.
- **Update features page — DONE** — Updated `/features` from 22→29 cards across 6 sections. Added: Code Engine card, Colab card, Bookmarks, Recent Pages, Display Preferences, new Multi-Language section (20 Languages + Automatic Fallback). Updated: "Four Execution Backends" (was Three), "Open in JupyterLab" (Binder progress UI + session reuse), intro text.
- **Backend selection UI — DONE** — Settings page shows "Execution Backend" radio buttons when multiple backends are available (e.g., CE + Binder on GitHub Pages). Options: "Auto-detect (recommended)" + one per available backend with detail text. Stored in `doqumentation_backend_override` (localStorage/cookies). `detectJupyterConfig()` refactored: extracted `buildConfigFor()` helper + `isGitHubPagesHostname()`/`isLocalHostname()`, checks override first, auto-clears if backend becomes invalid (e.g., CE expired). `getAvailableBackends()` enumerates all valid backends. No changes needed in ExecutableCode or OpenInLabBanner — they call `detectJupyterConfig()` automatically.
...
### Resolved (Mar 2026)
- **Backend selection UI** — Radio buttons on settings page when multiple backends available. `detectJupyterConfig()` refactored with `buildConfigFor()` helper and override support (`doqumentation_backend_override` key). Auto-clears stale overrides. Credential save/delete handlers refresh the backend list.
- **Rethink user intro & settings page** — Settings page reorganized: essentials at top (Simulator Mode, Display Preferences, Learning Progress), advanced sections (Code Engine, IBM Quantum, Binder Packages, Custom Server, Setup Help) collapsed in `<details>` under "Advanced Settings" heading. Hash deep-links auto-open the enclosing `<details>`. Simulator mode defaults to ON for new users. Onboarding tip simplified ("Click Run — no setup needed", 2 visits).
