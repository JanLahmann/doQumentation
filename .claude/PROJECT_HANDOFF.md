...
- **Weekly stale-translation refresh**: when a sync makes existing translations stale, use [`translation/retranslation-prompt.md`](../translation/retranslation-prompt.md). It pairs with `translation/scripts/update-translations.py --auto-fix --generate-workfile` to emit per-paragraph workfiles so sub-agents touch only changed paragraphs (preserving voice/terminology in unchanged sections). Skip NOOP severity (auto-fix handles it); translate MAJOR severity whole-file via the regular `translation-prompt.md`.
