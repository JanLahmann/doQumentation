...
- **Rethink user intro & settings page** — Simplify for new users. The default experience should "just work" with zero configuration (Binder + simulator mode). Advanced options (IBM credentials, custom Jupyter server, display prefs) should be tucked away behind an "Advanced" toggle or expandable section — visible but not overwhelming. Consider: a minimal first-run banner ("Click Run to execute code — no setup needed") instead of multi-step onboarding, a settings page that shows only essentials by default (simulator on/off, font size) with advanced sections collapsed, and contextual settings that surface where relevant (e.g., simulator toggle near the execution toolbar) rather than requiring a trip to the settings page.
