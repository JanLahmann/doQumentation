...
- **Weekly stale-translation refresh** (validated & in production, 2026-05-17): when a sync makes existing translations stale, use [`translation/retranslation-prompt.md`](../translation/retranslation-prompt.md) + `translation/scripts/update-translations.py`. **Architecture**: recovers the EXACT old EN (via `baseline-hashes.json` commit, or the pre-sync snapshot `9f2948310^` for baselines predating `docs/` tracking — EN was byte-static 2026-03-11→sync) and `git diff`s it vs current EN. Each workfile entry is a unified-diff **hunk**; Sonnet sub-agents mirror that exact change (incl. structural: Admonition→`:::`, heading renames, added `<CardGroup>`). NOOP = structural-only (auto-fixed); MAJOR/new-file → whole-file via `translation-prompt.md`. Earlier passage-hash heuristic produced 7 bug classes (fixed PRs #71–#76); the git-diff rewrite (#76) is the one in use. Workflow: `--auto-fix` → `--generate-workfile` → sub-agents → `--finalize` (pins English-derived anchors, validates, bumps source-hash only on PASS, writes `_finalize_failures.txt`) → commit only passing `.mdx`. Validated across 33 files / 7 locales / all sections; ~60 files shipped (PRs #77–#80). Use `--exclude-open-prs` for parallel batches (unmerged PR files still read STALE → dup work/conflicts). **Pending**: pipeline-hardening #81 + prompt-antipatterns #82 (auto-anchor, validate-gated finalize, n=5 context). Backlog: ~260 de + other locales remain (mechanical scaling). Note: i18n tutorials are broadly post-sync STALE — not a "TutorialFeedback widget" issue (that 2026-05-06 theory is superseded); refresh them via this pipeline like guides.
...
- **Draft → review pipeline** — `translation/drafts/{locale}/{path}` → validate → fix anchors → promote to `i18n/`. Status in `translation/status.json` (status, validation, source hash, dates, failures). Scripts default to `i18n/` without `--dir`.
  ...
  - **Sync & update**: `sync-translations.py` mechanically fixes code-block drift (run after EN changes: `--all-locales` or `--locale XX --dry-run`). `update-translations.py` — git-diff-based: recovers exact old EN → `git diff` vs current → unified-diff **hunk** per workfile entry → Sonnet sub-agents mirror it. Flags: `--auto-fix`, `--generate-workfile --output`, `--finalize` (anchor-pin + validate-gated hash-bump), `--exclude-open-prs`, `--file`, `--section`. **Validated in production** (PRs #76–#80; ~60 files, 7 locales). See the "Weekly stale-translation refresh" bullet above for the full recipe.
