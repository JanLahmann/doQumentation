...
├── translation/                # status.json, STATUS.md, *-prompt.md, v2/ (PO pipeline: extract, update, translate, render, check, bootstrap, mdxcheck), scripts/{validate,lint,review,review-prefilter,sample-deep-review,translation-status,contributing-status,get-register-fails,check-*,fix-glossary-leaks,make-*-run}
...
- **v2 translation pipeline — remaining migration work** (after PRs #487/#488, 2026-09-06; the pipeline itself is documented in [`translation/v2/README.md`](../translation/v2/README.md)). Everything below is a consequence of the same fact: a locale's translation is now its PO tree, the rendered pages are derived, and the v1 scripts that wrote `translation/status.json` are gone.
  - **`status.json` provenance is frozen.** `source_hash`, `validated_against`, `en_base_commit_date`, `en_base_source` and `promoted_at` are read by `translation-status.py`, the review scripts, `sync-content.py --freshness-report` and the `page-dates` plugin, but nothing writes them since v2. Visible effect: the translated-page footer ("This translation based on the English version of *date*") shows the last v1 promotion date (e.g. 2026-03-11 for `de/guides/hello-world`) although every locale was re-rendered against `723fc910`. The rendered pages themselves are correct: each carries the current English hash (`{/* doqumentation-source-hash */}`), so the "translation vs fallback" classification still works. Fix path: have `render.py` (or a small `translation/v2/status.py`) write `source_hash` + `en_base_commit_date` per page from the PO header (`X-Doq-EN-Hash`, `X-Doq-Page`) and `upstreamFileMeta.json`, or teach `page-dates` to read the PO instead; then drop the fields the review pipeline no longer needs.
...
- **Upstream sync strategy follow-ups** — `sync-upstream.yml` has now had **two successful real runs**: `833ab77dc → 47abf7714` (PR #191, 2026-06-03) and `47abf7714 → c6874ac` (PR #287, 2026-06-19, 119 EN files). Both still triggered manually (`workflow_dispatch`); the **bot-PR checks need a close+reopen** to fire CI (the fork-approve API 403s on same-repo `create-pull-request` PRs). Still TODO: **schedule it weekly** (the cron is still commented out — two clean runs now prove the auto-PR path); teach `sync-content.py` to flag conflicts on our locally-customized files (`hello-world.ipynb`, `_toc.json`); document the rollback recipe in CONTRIBUTING. **Post-sync, run the locale refresh** — see the June 2026 Recently-Resolved entry for the full 17-locale recipe + the c6874ac-era hardening (pre-filter + the 5 pipeline fixes) that makes the next one cleaner.
