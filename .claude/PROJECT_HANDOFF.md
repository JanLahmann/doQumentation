...
- **Weekly stale-translation refresh** (validated & in production, hardened 2026-05-17): when a sync makes existing translations stale, use [`translation/retranslation-prompt.md`](../translation/retranslation-prompt.md) + `translation/scripts/update-translations.py`. **Architecture**: recovers the EXACT old EN (via `baseline-hashes.json` commit, or the pre-sync snapshot `9f2948310^` for baselines predating `docs/` tracking — EN was byte-static 2026-03-11→sync) and `git diff`s it (`n=5` context) vs current EN. **Shallow-clone gotcha (web/CI sessions)**: if `9f2948310^` isn't in the clone every file misclassifies as `MAJOR`/`full_retranslation` — fetch it first with `git fetch --depth=2 origin 9f29483105b21a91d0b3abf0ba9cfd735a084b9b` (fetch-by-SHA is allowed; persists in `.git`). Each workfile entry is a unified-diff **hunk**; Sonnet sub-agents mirror that exact change (incl. structural: Admonition→`:::`, heading renames, added `<CardGroup>`). NOOP = structural-only (auto-fixed); MAJOR/new-file → whole-file via `translation-prompt.md` (whole-file chunking there is a *different* mechanism — do not conflate with hunks). Earlier passage-hash heuristic produced 7 bug classes (fixed PRs #71–#76); the git-diff rewrite (#76) is the one in use. **Workflow**: `--auto-fix` → `--generate-workfile` → Sonnet sub-agents (3-wide waves) → `--finalize` → commit only passing `.mdx`. `--finalize` (PR #81) pins English-derived anchors, then **dual-gates**: structural validation AND a content gate that rejects a file if any non-cosmetic removed-EN line is still present (= agent silently skipped a hunk → would re-introduce #73 silent-data-loss). Failures → `_finalize_failures.txt`; per-file status → durable `translation/manifests/<locale>.json` (gitignored). **Scaling**: one branch/PR per ~25–30-file batch, **always `--exclude-open-prs --skip-manifest-done`** (unmerged-PR files still read STALE → dup work + the #80 rebase conflict), don't wait for merge between batches; full recipe in retranslation-prompt.md "Orchestration recipe". Improvements A–G all merged (PRs #81/#82/#84). **Backlog: 0** — all 17 main locales are 0-stale as of the c6874ac sync (2026-06-20). **c6874ac-era hardening (run #287 → PRs #288–305), use these next time:**
  - **Per-sync baseline**: `export DQ_PRE_SYNC_REF="<sync-commit>^"` (e.g. `34e8d3e19^` for c6874ac) — NOT the universal `9f2948310^`. Manifest `old_en_commit` is used per-file when present; the env var is the fallback for un-finalized files. Verify all stale files' baselines resolve to the prior-sync EN before running (else you double-count the previous sync).
  - **Pre-filter (~65% fewer agents)**: only dispatch an agent for a file whose EN diff changes a line OUTSIDE a code fence (checking BOTH `+` and `-` lines) + the "mega" files. Code-only files (f-string/comment reformatting) are handled deterministically by `--auto-fix`/finalize — no agent. Safe because the finalize CONTENT gate re-flags anything wrongly skipped. ~41 agents/locale instead of ~117.
  - **Route to full-retranslation (not splice)**: any file with a single huge `new_en` block (>~8k chars, the "conform to new template" rewrites), PLUS always `index.mdx` (customized homepage — splice mangles its JSX) and `guides/execute-dynamic-circuits.mdx` (the new `## Store` :::note block gets misplaced by splice).
  - **5 pipeline fixes merged in #305**: `fix_duplicate_adjacent_code_block` + `strip_jsx_heading_anchors` in the finalize chain; validator + anchor-fixer EXEMPT headings inside JSX containers (`<Accordion>/<Tabs>/...` — an anchor there is a JS expression that breaks the MDX build); classifier checks removed lines too.
  - **Drop `--exclude-open-prs`** for one-PR-per-locale fan-out — it matches by relative path across locales and wrongly drops a locale's stale `index.mdx` because an open de/es PR also touches `index.mdx`.
  - **BUILD each locale before its PR** — `validate` + `lint` PASS files that the build then rejects (JSX-nested anchors; a dropped `[` in a link; a dangling `<a>`). The build is the only gate that caught these.
  - **Parallel pairs** (disjoint `i18n/<loc>/` dirs) ~halve wall-clock; **session-token limits** are the real cap (hit twice mid-run) — resume cleanly via `Workflow resumeFromRunId` (cached agents return instantly, only failures re-run). Reusable drivers + classification saved in `~/.claude/projects/.../c6874ac-resume/`.
...
- A one-shot backfill (`backfill-en-base-date.py`, deleted 2026-06-04 — in git history) filled 4,964/7,342 `en_base_commit_date` entries with hash-verified dates; 2,170 fell back to promoted date with `en_base_source: "promoted-fallback"`. The field is now auto-populated at promotion time.
...
├── translation/                # status.json, STATUS.md, *-prompt.md, manifests/ (gitignored), scripts/{validate,lint,review,fix-anchors,promote,populate,get-register-fails,translation-status,sync-translations,update-translations}
...
- **Review pipeline** — the current path edits `i18n/{locale}/` directly (git-diff hunk-splice for refreshes; whole-file `Write` for new locales), then validate → lint → review. Status in `translation/status.json` (status, validation, source hash, dates, failures). Scripts default to `i18n/` without `--dir`. *(The legacy `translation/drafts/{locale}/{path}` → promote staging flow is retired — `promote-drafts.py` still exists for from-scratch new-locale work; the empty `drafts/` scaffolding was removed 2026-06-04.)*
...
- **Linguistic review — ~100 files/locale PENDING after the c6874ac refresh.** The c6874ac sync refresh (2026-06-20) re-translated ~117 files/locale across all 17, of which ~100/locale had real prose changes (the rest code-only); `--finalize` auto-invalidated their review verdict to `STALE_REFRESH`. **These files passed structural + content VALIDATION but their newly-spliced/retranslated prose has had no linguistic read** — quality rests on the Sonnet splice + full-retranslate agents until reviewed. (Supersedes the prior "~35 files/locale after #191" note.) Re-queue with `review-translations.py --next-chunk` (Sonnet, 5/chunk; Haiku unreliable). **This is now automatic**: `update-translations.py --finalize` invalidates a file's review whenever it re-stamps it, so every future content refresh self-flags its touched files for re-review. Older translator passes still owed: TL `workshop/03*`, AR `quantum-diagonalization-algorithms/`, KO `tutorials/transpilation-optimizations-with-sabre.mdx`. (Triage history: the May-7 linguistic-review pass — see `PROJECT_HANDOFF_ARCHIVE.md` "Resolved — May 2026" and git history; the dated `.claude/PHASE_4_LINGUISTIC_TRIAGE_2026-05-07.md` was deleted 2026-06-04.)
