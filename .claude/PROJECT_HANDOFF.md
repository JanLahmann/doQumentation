...
- **Translate `index.mdx` new sections** — Main's `docs/index.mdx` now has "Develop locally" + "When you're ready for more" sections (commit `708d5774c`) that are not yet translated to any locale. The May 5 archive-branch merge could not harvest these from the archive because the archive's per-locale `index.mdx` versions had English placeholders for those sections AND would have regressed unrelated linguistic-review fixes (DE Sie→du, IT 1160 fixes, FR 120 fixes, AR 14 FAILs). Cleanest approach: translate the 2 new paragraphs fresh across 19 locales next translation pipeline run. Don't merge from archive. ~1h.
