...
├── upstream-docs/              # Submodule → JanLahmann/Qiskit-documentation (CC BY-SA 4.0)
...
- **Binder cold build broken** (since Apr 17) — mybinder.org Docker 27→28 upgrade ([deploy#3768](https://github.com/jupyterhub/mybinder.org-deploy/pull/3768)) wiped layer caches. Build completes but pod is killed during image push (matches [binderhub#2094](https://github.com/jupyterhub/binderhub/issues/2094)). Root cause: repo2docker clones all 44K files regardless of `.dockerignore` (in-memory tarball, [#3680](https://github.com/jupyterhub/mybinder.org-deploy/issues/3680)) and image export exceeds the build-pod memory limit changed Apr 13 ([#2071](https://github.com/jupyterhub/binderhub/pull/2071), [#2093](https://github.com/jupyterhub/binderhub/pull/2093)). GESIS decommissioned. **Interim fix**: `binderUrl` and `binder.yml` pinned to commit `0fc67252` (last cached, Apr 13). Thebelab unaffected. **Fix paths**: (1) wait for mybinder.org memory fix; (2) split into thebelab-only repo (next item); (3) file mybinder issue. `.dockerignore` does NOT help. CE image unaffected.
- **Split Binder into two instances** — thebelab and "Open in JupyterLab" share one image (`doQumentation/notebooks`) but have different needs. Thebelab only needs Python kernel (Qiskit + deps, ~800 MB pip). Lab needs full repo. Split: lightweight `doQumentation-binder` repo (just `requirements.txt`, ~1 GB, builds <3 min, rarely invalidated) for thebelab; existing `notebooks` branch for Lab. Decouples thebelab from notebook-content churn and solves the cold-build timeout for the common case.
- **Upstream sync strategy** — `sync-content.py` clones from `JanLahmann/Qiskit-documentation` fork; needs automated fork sync, conflict handling for our modified files (`hello-world.ipynb`, `_toc.json`), freshness checks, and rollback strategy.
