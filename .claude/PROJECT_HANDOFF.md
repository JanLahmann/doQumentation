...
- **Rework PR #159 — 275 defective weak-5 files** (DEFERRED to the week of Sat 2026-05-30, awaiting a fresh token window). PR #159 (`landing/dev-weak5`, open, marked DO-NOT-MERGE) holds the ~275 ko/pl/cs/ro/th files that FAIL `validate-translation` on real defects (Link URLs ~162/locale, JSX tags ~116, heading-count ~106, code-blocks ~70) — the unfinished-bulk-pass residue. The 872 clean ones already merged (#161). Rework path now well-supported: the `--finalize` repair-chain (#154) + the false-positive-free `check_jsx_tag_balance` gate (#163) + `--audit` (#160) are all on main. Approach: per-locale `--auto-fix` → `--finalize` (auto-repairs mechanical Link/image/anchor classes) → agent hunk-splice/whole-file for the rest → commit only `--finalize`-passing files → PR. Do NOT merge #159 as-is (would ship build-rendering-broken pages; EN-fallback does NOT rescue them — it only triggers on the explicit `doqumentation-untranslated-fallback` marker).
