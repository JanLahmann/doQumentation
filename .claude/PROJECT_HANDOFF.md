...
- **Binder startup UX** — DONE (Mar 5): `OpenInLabBanner` now shows real-time UX during Binder builds. Elapsed timer in button text (`Building... 1m 23s`). Per-phase hint line below banner with realistic durations: fetching/pushing 2–5 min, building 5–10 min, launching 2–5 min. Cache miss warning shown immediately when `building` phase starts (means daily cache warming failed): "Cache not warmed — total build time 10–25 min. Use the Colab button above, or come back later." Added missing `waiting` phase. Auto-clears on ready/failed.
