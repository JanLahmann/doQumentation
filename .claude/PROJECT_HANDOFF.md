...
### TODO
- **Translation review** — Systematic linguistic review of all 885 existing translations (created before QA process). Automated baseline complete: 456/885 structural PASS, 877/885 lint CLEAN, 440 files ready for linguistic review (AR auto-skipped). Use `review-translations.py --next-chunk` for next batch, `--record-review` to track verdicts, `--progress` for dashboard. Each session reviews ~20 files. Priority: DE > ES > FR > JA > UK > IT > PT > HE > TL > dialects.
...
- **Comprehensive prerequisites cell** — Current notebook pip install injection uses two cells (base + extras), with extras filtered against stale `BINDER_PROVIDED` set. Bug: packages like `qiskit-ibm-catalog`, `pyscf` are filtered out but missing on Colab. Fix: stdlib-only filtering, merge into single comprehensive cell. Plan: `.claude/plans/comprehensive-prereqs-cell.md`.
