...
- **Deployed app** — `ce-doqumentation-01.27boe8ie8nv4.eu-de.codeengine.appdomain.cloud` (eu-de region), project `ce-doqumentation-01`. Min scale 0 (scales to zero when idle), max scale 1. Default size varies (use `resize-pod.yml` workflow or IBM Cloud Console to set). Token must be 32+ chars. Sizing validated via stress tests: 8 vCPU/16 GB handles 80 users with retry helpers (see `.claude/STRESS-TEST-FINDINGS.md`).
