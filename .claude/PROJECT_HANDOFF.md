...
- **Simulator mode** — Monkey-patches `QiskitRuntimeService` with `_DQ_MockService` (AerSimulator or FakeBackend). Fake backend discovery cached, 55-backend fallback list. **Exempt pages**: `SIMULATOR_EXEMPT_PAGES` in `ExecutableCode/index.tsx` — pages like `tutorials/hello-world` that demonstrate real hardware access bypass simulator interception, `save_account()` skip-hints, and session annotations.
...
- **Review orchestration**: `review-translations.py` manages systematic review across sessions. `--auto-check` runs structural validation + lint for all locales in bulk. `--progress` shows per-locale dashboard (struct/lint/review counts). `--next-chunk [--size N]` returns prioritized batch of files needing linguistic review. `--record-review` persists verdicts (PASS/MINOR_ISSUES/FAIL/SKIPPED) to status.json. Baseline: 885 files, 456 structural PASS, 440 ready for review, AR production-ready (Opus re-assessment 8/10, retranslation not needed).
...
### TODO
- **Translation expansion** — **13 locales at 100% (387/387)**: DE, ES, FR, IT, UK, JA, AR, PT, TL, HE, MS, ID, TH. **4 new locales in progress** (tutorials done, guides/courses/modules remaining): CS 45/387, PL 45/387, KO 45/387, RO 45/387 — all validated, promoted, and linguistically reviewed (sample-based, Mar 28). Run `python translation/scripts/translation-status.py --all` for current counts. German dialects: KSH 46, NDS 43, GSW 42, SAX 39, BLN 36, AUT 34.
- **Full linguistic review** — Sample-based review (6 files/locale) done for CS, PL, KO, RO (Mar 28). Found and fixed: PL meaning error (Pauli twirling→entanglement), PL false cognate (transpose→transpile, 4x), CS fabricated verb, CS/KO/RO register slips, RO terminology drift, typos/diacritics. **TODO**: Full review of ALL files for ALL locales (Haiku for all except AR which needs Opus). AR re-assessed with Opus (8/10): production-ready, retranslation NOT needed.
...
- **Video subtitles pipeline** — 59 course videos need subtitles. Analysis (Mar 28):
  - **6 videos** (Basics of QI by John Watrous): YouTube auto-translated captions available — `cc_lang_pref` works (already implemented in IBMVideo.tsx)
  - **26 YouTube videos**: English auto-captions only, YouTube does NOT offer auto-translation
  - **27 IBM Video-only**: No caption support at all
  - **53 videos need our pipeline**: Whisper transcription → Claude Code VTT translation → custom player
  - **Phase 1 — Transcription**: `scripts/generate-transcripts.py` runs Whisper locally on Mac (`pip install openai-whisper yt-dlp`). POC done for video 134413658 (tiny model). Run `--model medium` for production quality, ~5-15h overnight for all 59 videos. Script auto-detects YouTube vs IBM Video source.
  - **Phase 2 — Translation**: Use Claude Code (not API). Create `translation/vtt-translation-prompt.md`. VTT files are small (~100-200 lines), no chunking needed. One agent per file, 3 parallel.
  - **Phase 3 — Custom player**: YouTube `<iframe>` embeds can't load external VTT tracks (sandboxed). Replace IBMVideo.tsx with a custom player (e.g. video.js or native `<video>`) that loads YouTube/IBM Video as source + our VTT `<track>` elements. Enables: locale-aware subtitle selection, synced transcript panel (click-to-seek), consistent UX across all 59 videos.
  - **Scripts**: `scripts/generate-transcripts.py` (Whisper pipeline), `scripts/translate-transcripts.py` (Claude API reference — to be replaced by Claude Code workflow)
...
*Last updated: March 28, 2026 (linguistic review + fixes for CS/PL/KO/RO tutorials; AR re-assessed production-ready; KO + RO promoted to 45/387 each; all 4 new locales at tutorials-complete)*
