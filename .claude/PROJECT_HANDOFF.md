...
- **Binder URLs** — `getBinderLabUrl(config, notebookPath, locale?)` generates locale-aware "Open in Binder JupyterLab" links pointing to the `doQumentation/notebooks` branch (enhanced notebooks with pip install cells). Same path mapping as Colab: strips `docs/` prefix; bare filenames get `tutorials/` prefix; non-EN locales get `{locale}/` prefix. `OpenInLabBanner` passes `currentLocale` from Docusaurus context.
...
- **Binder tab reuse fix** — `noopener`/`noreferrer` on Binder links caused browsers to discard the `"binder-lab"` named window target (per HTML spec, `noopener` creates unnamed browsing contexts). Removed from both `OpenInLabBanner` (`rel` attribute) and `ExecutableCode` (`window.open` features string). mybinder.org is trusted — `noopener` protection not needed.
