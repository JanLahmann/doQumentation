...
### IBM Cloud Code Engine (`binder/Dockerfile.codeengine`)
...
- **Consolidate Docker images** — Currently multiple Dockerfiles/configs serve different purposes: `binder/Dockerfile` (Binder), `Dockerfile.jupyter` (full Docker stack), `Dockerfile.web` (static site), `scripts/setup-pi.sh` (RasQberry). Analyse all variants and minimize to ideally one or two images: a "typical" image (core Qiskit + most-used packages, suitable for Binder/CE/RasQberry) and optionally an "all dependencies" image (adds amd64-only packages like gem-suite, AI transpiler). Goal: single `jupyter-requirements.txt` already serves as the dep source — now unify the Dockerfiles that consume it. Reduces maintenance, CI complexity, and divergence between environments.
