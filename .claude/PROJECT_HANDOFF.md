...
- **Binder cold build broken** (since Apr 17) — mybinder.org upgraded Docker 27→28 on Apr 13 ([jupyterhub/mybinder.org-deploy#3768](https://github.com/jupyterhub/mybinder.org-deploy/pull/3768)), wiping all Docker layer caches. Cold builds now exceed the 20-min timeout (base image pull + pip install alone take ~10 min). **Interim fix**: `binderUrl` and `binder.yml` cache-warming pinned to commit `0fc67252` (last cached image, Apr 13). GESIS removed from cache warming (301 redirect — instance decommissioned). `.dockerignore` added to exclude 20 locales from build context (keeps EN + de/es/fr/ja/ko/ro). **To fix properly**: (1) Wait for mybinder to stabilize Docker 28 layer caching, then revert pin to `notebooks`. (2) Or restructure Dockerfile to use multi-stage build — first stage installs deps (cacheable), second stage copies notebooks (fast). (3) Or switch from DockerBuildPack to `requirements.txt` + `postBuild` (repo2docker handles file copy automatically, no COPY step). (4) Consider opening an issue on [jupyterhub/mybinder.org-deploy](https://github.com/jupyterhub/mybinder.org-deploy/issues) about Docker 28 regression with large repos. Note: CE image (Dockerfile.jupyter) is unaffected — builds from main branch, not notebooks.
