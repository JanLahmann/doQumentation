...
- **Binder session reuse** — `openBinderLab()` in `jupyter.ts` manages Binder sessions. First click: calls Binder build API (`EventSource` on `/build/gh/...`), captures server URL + token on `"ready"` event, stores in `sessionStorage` (`dq-binder-session`), opens JupyterLab in new browser tab. Subsequent clicks (within 8 min): reuses stored session — opens new tab on same Binder server (instant, no rebuild). All tabs share the same container/packages. Session expires after 8 min idle (2 min safety before Binder's 10 min timeout) or when browser closes (sessionStorage). `OpenInLabBanner` shows build phase progress in the button text during first launch. `getBinderLabUrl()` still generates `href` for right-click fallback.
- **Thebelab** — Uses the sister repo `JanLahmann/Qiskit-documentation/main` for its Binder kernel (original upstream notebooks, no translations needed). Configured in `ExecutableCode/index.tsx` `getThebelabOptions()`.
...
- **Binder-enable this repo — COMPLETE** — Full Binder setup on `notebooks` branch: `deploy.yml` pushes EN notebooks + Binder config (core packages in `binder/requirements.txt` for Docker caching, 6 lighter packages + cleanup in `binder/postBuild`, `binder/runtime.txt` for Python 3.12). `deploy-locales.yml` consolidates locale notebooks into subdirs. `binder.yml` warms all 3 federation members (2i2c, BIDS, GESIS) daily + on push. "Open in Binder JupyterLab" links point to `doQumentation/notebooks` branch (locale-aware). Thebelab stays on sister repo `Qiskit-documentation/main` (original upstream notebooks). Image layers: 7.6 GB conda base (cached), 2 GB pip install, ~550 MB notebooks.
...
- **Binder session reuse** — "Open in Binder JupyterLab" now reuses existing Binder sessions. First click calls Binder build API via `EventSource`, captures session URL + token, stores in `sessionStorage`. Subsequent clicks open notebooks instantly as new browser tabs on the same Binder server (no rebuild, no wait). All tabs share the same container. Session auto-expires after 8 min idle. `OpenInLabBanner` shows build progress in button text. Fixed initial tab reuse issue: `noopener`/`noreferrer` was discarding named window targets (per HTML spec).
