...
- **v2 translation pipeline — remaining migration work** (after PRs #487/#488, 2026-09-06; the pipeline itself is documented in [`translation/v2/README.md`](../translation/v2/README.md)). Everything below is a consequence of the same fact: a locale's translation is now its PO tree, the rendered pages are derived, and the v1 scripts that wrote `translation/status.json` are gone.
  ...
  - **Port the in-place fix workflows to PO entries.** `.claude/workflows/curate-and-fix-glossary.js`, `fix-consistency.js`, `fix-misleading-translations.js` and `translation/scripts/fix-glossary-leaks.py` edit rendered pages; such an edit is lost at the next render. Until ported (agent proposes new msgstr values → `translate.py --apply`), record findings, do not fix pages.
    - **Two more instances found 2026-09-08** during the `fr` review round: `check-known-mistranslations.py --fix` and `fix-tutorialfeedback-import.py --apply` also wrote rendered pages. All three Python fixers now **refuse to write and exit 2** with a pointer to the PO path (`_common.refuse_rendered_page_write`); their read-only/`--dry-run` modes are unchanged, so they still report. The `fr` round hit this concretely: `--fix` would have "fixed" 19 hits that the next render discards. The three `.js` workflows still only carry a warning in their SKILL.md.
    - For `fix-tutorialfeedback-import.py` the write was doubly wrong: the misplaced import comes from the English skeleton, so it belongs in `sync-content.py` / `docs/`, not in a locale page.
