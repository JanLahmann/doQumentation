...
- **Stale-translation refresh** — *superseded 2026-09-06 by the v2 PO pipeline*: see [`translation/v2/README.md`](../translation/v2/README.md) (segments in PO files, `msgmerge` after every sync, agent batches gated by `check.py`, rendered pages derived at build time). The git-diff hunk-splice pipeline (`update-translations.py`, `retranslation-prompt.md`, the passage-hash baselines) and its c6874ac-era hardening notes are in git history and in `PROJECT_HANDOFF_ARCHIVE.md`.
...
- Migration: 2026-05-06 (PR #34, branch `feat/dedupe-output-images`). Saved ~12 MB / 1,800 relative refs. (One-shot script `scripts/migrate_output_imgs.py` deleted 2026-06-04 after the migration completed; in git history.)
...
- ~~`en_base_commit_date` per locale × path lives in `translation/status.json`.~~ (retired 2026-09-10; the footer shows the English page date) It was recorded by `promote-drafts.py`, deleted 2026-09-06 with the v1 pipeline; **nothing in v2 writes it yet**, so every value dates from the last v1 promotion even though all 17 locales have since been re-rendered against upstream `723fc910` (see the v2 entry under Active TODO).
- A one-shot backfill (`backfill-en-base-date.py`, deleted 2026-06-04 — in git history) filled 4,964/7,342 `en_base_commit_date` entries with hash-verified dates; 2,170 fell back to promoted date with `en_base_source: "promoted-fallback"`. The field was auto-populated at promotion time until v2 replaced promotion.
...
- `Dockerfile.jupyter` — multi-stage with shared `jupyter-base` + targets `jupyter-local` (~3 GB, also serves the static site via nginx) and `jupyter-codeengine` (~3 GB). (A standalone static-only `Dockerfile.web` was removed 2026-06-04 — nothing built it; `jupyter-local` covers local site preview.)
...
├── scripts/                    # sync-content.py, sync-deps.py, docker-entrypoint.sh, setup-pi.sh, encrypt-for-admin.mjs, workshop-stress-test.py, notebook-sweep/
├── translation/                # STATUS.md (generated), *-prompt.md, v2/ (PO pipeline: extract, update, translate, render, check, bootstrap, mdxcheck), scripts/{validate,lint,review,review-prefilter,sample-deep-review,translation-status,contributing-status,get-register-fails,check-*,fix-glossary-leaks,make-*-run}
├── static/                     # logo.svg (favicon), CNAME, robots.txt, img/<section>/<stem-slug>/output_N.png
├── Dockerfile.jupyter          # Multi-stage: jupyter-local (also serves static site) + jupyter-codeengine
├── docker-compose.yml          # jupyter profile (jupyter-local target)
...
- **Review pipeline** — a translation lives in `i18n/{locale}/po/` and changes only through `translation/v2/translate.py --apply`; the pages under `i18n/{locale}/docusaurus-plugin-content-docs/current/` are derived (render first: `python3 translation/v2/render.py --locale XX`), then validate → lint → review run on the rendered pages. Never fix a rendered page in place — the next render overwrites it. Status in `translation/status.json` (status, validation, source hash, dates, failures); its provenance and review fields are read but no longer written by the pipeline (see the v2 entry under Active TODO). Scripts default to `i18n/` without `--dir`. *(The drafts → promote staging flow, `promote-drafts.py` and the hunk-splice refresh were removed 2026-09-06.)*
  ...
  - **Sync & update**: `translation/v2/` (`extract.py`, `update.py`, `translate.py --sweep/--prepare/--apply`, `render.py`, `check.py`, `mdxcheck.mjs`); the file-splicing `sync-translations.py` / `update-translations.py` were deleted 2026-09-06.
  - **Orchestration** (since 2026-09-10): `sample-deep-review.py` draws the round from the PO files (eligibility = rendered, not a stub, no fuzzy/empty entry, no `X-Doq-Review-Opus` header yet); `review-translations.py --record-opus` writes the verdict into each reviewed page's PO header at PR time; `--progress` reports per locale. The v1 Tier-3 Haiku orchestration (`--auto-check`, `--next-chunk`, `--record-review`, `review-prefilter.py`, `review-build-batches.py`, `get-register-fails.py`) and its docs were deleted the same day; `status.json` is frozen and unread by the review path.
...
- **Heading anchors**: every heading carries `{#english-anchor}` in the PO msgid (v2 pre-rule 1), so the translator keeps it and `check.py` verifies it; cross-page links work in every locale.
...
6. **Translate content**:
   ...
   # v2 (2026-09): bootstrap the empty locale, then run the sync procedure in translation/v2/README.md
   python3 translation/v2/bootstrap.py --locale {XX}
   python3 translation/v2/render.py --locale {XX}
   python3 translation/scripts/translate-content.py populate-locale --locale {XX}
   DQ_LOCALE_URL=https://{XX}.doqumentation.org npx docusaurus build --locale {XX}
   git add i18n/{XX}/po                                            # the PO files; rendered pages are derived
   ```
...
- **v2 translation pipeline — remaining migration work** (after PRs #487/#488, 2026-09-06; the pipeline itself is documented in [`translation/v2/README.md`](../translation/v2/README.md)). Everything below is a consequence of the same fact: a locale's translation is now its PO tree, the rendered pages are derived, and the v1 scripts that wrote `translation/status.json` are gone.
  - **~~`status.json` provenance is frozen~~ — RETIRED 2026-09-10: `translation/status.json` deleted.** Its last readers moved: the page-dates plugin shows the English page's own date (a v2 page is rendered from the current English, so that IS the base), the sync PR's freshness report points at the per-locale `update.py` worklist instead of a per-locale staleness table computed from frozen dates, `STATUS.md` is generated from the PO files (`translation-status.py`, rewritten), and lint/validate lost their `--record` modes. Original note follows.
  - **~~Review verdicts are not invalidated on retranslation~~ — DONE 2026-09-10: `translate.py --apply` withdraws a page's `X-Doq-Review-Opus` header (kept as `-Prior`) when a model retranslated an entry after an English change; carry-overs and review fixes leave it.** Original note: v1's `--finalize` set `review: STALE_REFRESH` on a re-stamped file. In v2 a fuzzy entry retranslated by `translate.py --apply` leaves the page's verdict in `status.json` and the PO header (`X-Doq-Review-Tier3` / `X-Doq-Review-Opus`) untouched. Needed: `translate.py --apply` marks the page stale (status.json + PO header) when at least one prose entry changed, and `review-translations.py --next-chunk` queues it.
  - **~~Port the in-place fix workflows to PO entries~~ — DONE 2026-09-10 by deletion.** `.claude/workflows/curate-and-fix-glossary.js`, `fix-consistency.js`, `fix-misleading-translations.js`, their bakers `make-{consistency,glossary,fix}-run.py` and `translation/scripts/fix-glossary-leaks.py` edited rendered pages, which are derived now; the PO replacements are `fix.py --fixes` (any review finding) and `fix.py --leaks` (the deterministic glossary pass, which now carries the v1 determiner list itself). `check-known-mistranslations.py --fix` and `fix-tutorialfeedback-import.py --apply` still exist but **refuse to write and exit 2** (`_common.refuse_rendered_page_write`); their read-only modes still report. For `fix-tutorialfeedback-import.py` the write was doubly wrong: the misplaced import comes from the English skeleton, so it belongs in `sync-content.py` / `docs/`, not in a locale page.
  - **House style settled 2026-09-08: Qiskit/Qubit/Gate/Circuit/Backend/Transpiler/Session/Sampler/Estimator/PUB/IBM Quantum/QPU stay English in prose.** The list now lives once, in `_common.KEEP_ENGLISH_TERMS`, and is interpolated into `translate.py`'s translator instructions and read by `sample-deep-review.py`, so the two cannot drift again — they had: the instructions said KEEP while the sampler counted every one as a leak and excluded the page, hiding **154 de / 92 he / 70 cs** pages from the deep review. Eligible pools after the fix: **de 35→189, he 38→130, cs 21→91**; es/fr stay at 6 (never leak-limited — their remainder is 55 sub-40-line stubs). A leak is now only what a locale's `translation/glossary/<loc>.json` records, minus the kept terms; every glossary's rules were exactly `gate`/`circuit` translations plus `qubit`/`circuit`/`gate` decapitalisations, so all 15 were cleared and `fix.py --leaks` is currently a deliberate no-op — it waits for a real curated glossary. **`CONTRIBUTING-NOW.md` is stale** until `contributing-status.py --write` runs with every locale rendered.
  - **`fuzzy` is a confirmation flag, and only the translate path may clear it.** `msgmerge` marks an entry fuzzy when the English changed, keeps the old translation and records the old English as `#| msgid`; po4a then renders **English** until the flag clears. Measured 2026-09-08: **644 such entries, 33–40 per locale**, every one with a real English change — so they are NOT clearable by a deterministic sweep. Only **62** pass `translate.mechanical_transfer` (punctuation/emphasis/attribute-only edits); the other **582 need a model**, i.e. a normal `update.py` → `translate.py --prepare` round per locale. `translate.apply` now infers `confirm_fuzzy` from the prefix: the translate path treats an unchanged string as confirmation (the agent was shown the entry), the **fix** path does not (its agents are told to copy unflagged entries back verbatim, so an unchanged string means "not looked at" and clearing the flag would publish a translation of the previous English). Regression-tested in `translation/v2/tests/test_v2.py`.
  - **~~Opus deep-review verdicts into the PO~~ — DONE 2026-09-10 as the page-level header `X-Doq-Review-Opus: <VERDICT> <date>`** (per-entry comments were not needed: eligibility is a page property). Written by `review-translations.py --record-opus` when a round ships; the 112 verdicts recorded in `status.json` after bootstrap and 79 from three never-banked August rounds (tl 59, ko 20) were imported once (`--import-status` + the review files). Only page reads (PASS/MINOR_ISSUES/FAIL) count; a repair round's FIXED records never mark a page reviewed. `status.json` keeps its frozen provenance fields for the page-dates plugin and `STATUS.md` only.
  - **Delta review (2026-09-10).** Every msgstr a model writes carries a dated stamp (`doq: fixed after review D`, `doq: translated after an English change D`); a page whose verdict stands but which has unverified stamped entries enters the review pool in *delta* mode (the sampler row carries `delta_entries`; `opus-deep-review.js` asks the reader to judge only those, with the page as context). `review-translations.py --record-opus` marks entries stamped before the verdict date as `· verified D`; a round's own same-day repairs stay pending for the next round. The 40-line stub cutoff is gone (default `--min-lines 1`). This is how repairs and sync retranslations get an Opus read; backlog at introduction: every entry repaired by the 2026-09-08/10 rounds and syncs.
...
- **Translation expansion** — **since 2026-09-06 (v2, PRs #487/#488):** all 17 main locales are translated per segment against upstream **723fc910** (25,388/25,388 PO entries each, 0 fuzzy, lint 0 errors, `mdxcheck` 0 failed); rendered pages are derived from `i18n/{locale}/po/` at build time and are no longer in git. Live counts: `translation/v2/update.py --locale XX` (entries) and `translation-status.py --all` (pages). The file-level figures that follow are the pre-v2 history: all 17 main locales at **100% (428/428), 0 structural-FAIL, 0 stale** (synced to **c6874ac**, 2026-06-20; the prior 47abf7714 state is superseded). The c6874ac weekly run refreshed ~117 stale files/locale across all 17 (PRs #288–304) and merged. Locale homepages also now carry the Qiskit Ecosystem badge + SEO `<head>`/`description` (PR #306). HE/TH/CS completed (the old "412/412, HE/TH 95%" is done). German dialects remain intentionally partial: KSH 46, NDS 43, GSW 42, SAX 39, BLN 36, AUT 34, SWG/BAD/BAR 31 (16–28 structural FAILs each, NOT being fixed — user decision 2026-05-31). Potential future: Turkish (TR). Run `translation-status.py --all` for live counts.
- **Linguistic review — ~100 files/locale PENDING after the c6874ac refresh.** The c6874ac sync refresh (2026-06-20) re-translated ~117 files/locale across all 17, of which ~100/locale had real prose changes (the rest code-only); `--finalize` auto-invalidated their review verdict to `STALE_REFRESH`. **These files passed structural + content VALIDATION but their newly-spliced/retranslated prose has had no linguistic read** — quality rests on the Sonnet splice + full-retranslate agents until reviewed. (Supersedes the prior "~35 files/locale after #191" note.) Re-queue with `review-translations.py --next-chunk` (Sonnet, 5/chunk; Haiku unreliable). **No longer automatic since v2 (2026-09-06)**: `update-translations.py --finalize`, which set `review: STALE_REFRESH` on every file it re-stamped, is deleted, and `translate.py --apply` does not touch a page's verdict when it retranslates fuzzy entries — see the v2 entry below. Render the locale before `--next-chunk`; the review scripts read the rendered pages. Older translator passes still owed: TL `workshop/03*`, AR `quantum-diagonalization-algorithms/`, KO `tutorials/transpilation-optimizations-with-sabre.mdx`. (Triage history: the May-7 linguistic-review pass — see `PROJECT_HANDOFF_ARCHIVE.md` "Resolved — May 2026" and git history; the dated `.claude/PHASE_4_LINGUISTIC_TRIAGE_2026-05-07.md` was deleted 2026-06-04.)
...
- **~~Upstream sync strategy follow-ups~~ — settled 2026-09-10.** `sync-upstream.yml` runs weekly (Monday 06:00 UTC); its bot PR is a signal, not a merge (no CI on a workflow-token PR, and the English must not merge without the locales). `translation/v2/sync.py prepare --from sync/upstream-content` → fill → `status` → `finish` carries English + POT + all 17 PO trees on one branch with CI; the bot PR is closed as superseded. Two real syncs done this way (#534/#537: 723fc910 → 3f7a44d0). Original note:
