...
- **Download notebook option** — `OpenInLabBanner` currently has "Open in JupyterLab" + "Open in Colab" buttons. Add a third "Download .ipynb" option so users can save the original notebook (with output cells preserved) to their own machine for offline use, sharing, or running in their preferred local Jupyter setup. Source notebooks already exist in `static/notebooks/{path}.ipynb` (mirrored by `sync-content.py`); just need a simple anchor link with `download` attribute and the matching path. Locale-aware: link to translated notebook when `currentLocale !== 'en'`.
