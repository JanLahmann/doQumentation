...
- **Remove Qamposer (open-source composer) entirely** — The `/qamposer` page (drag-and-drop circuit composer ported from QAMP-62/qamposer-backend, embedded as inline Python in [src/components/QamposerEmbed/thebelabAdapter.ts](src/components/QamposerEmbed/thebelabAdapter.ts)) doesn't fit doQumentation's target audience (first-steps Qiskit learners reading notebooks). Remove: the `/qamposer` page route, `src/components/QamposerEmbed/` directory, `src/components/QamposerEmbed/executionMode.ts`, any navbar/sidebar/homepage links, `qamp-*.mjs` debug scripts at repo root, and references in PROJECT_HANDOFF/features page. Also delete the `Qamposer Python backend reuse (Option 2)` TODO above (becomes obsolete).
