...
- **Sidebar**: Home link at top, then Tutorials/Guides/Courses/Modules categories (autogenerated from `sidebar-*.json`). API Reference + Settings in navbar only. **Single unified indicator per item** — 5 states: pure MDX unvisited (nothing), pure MDX visited (`✓` gray), notebook unvisited (`</>` gray, non-clickable), notebook visited (`</>` blue, clickable), notebook executed (`</>` green, clickable). Clicking a clickable indicator clears visited/executed status. Notebook detection via `customProps.notebook` in sidebar JSON (uses `<OpenInLabBanner>` presence, not `notebook_path` frontmatter). **Caret alignment**: padding for badge space is on `.menu__list-item-collapsible` (not `.menu__link`) so both Docusaurus caret mechanisms (`::after` for no-href categories, `.menu__caret` button for href categories) stay at their native aligned position. Badge at `inset-inline-end: 0.25rem` in the collapsible's padding area. Link items have `padding-inline-end: 2.25rem`. Long titles truncate with `text-overflow: ellipsis`.
