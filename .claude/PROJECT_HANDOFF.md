...
- **Weekly stale-translation refresh** (validated & in production, hardened 2026-05-17): when a sync makes existing translations stale, use [`translation/retranslation-prompt.md`](../translation/retranslation-prompt.md) + `translation/scripts/update-translations.py`. **Architecture**: recovers the EXACT old EN (via `baseline-hashes.json` commit, or the pre-sync snapshot `9f2948310^` for baselines predating `docs/` tracking — EN was byte-static 2026-03-11→sync) and `git diff`s it (`n=5` context) vs current EN. **Shallow-clone gotcha (web/CI sessions)**: if `9f2948310^` isn't in the clone every file misclassifies as `MAJOR`/`full_retranslation` — fetch it first with `git fetch --depth=2 origin 9f29483105b21a91d0b3abf0ba9cfd735a084b9b` (fetch-by-SHA is allowed; persists in `.git`). Each workfile entry is a unified-diff **hunk**; Sonnet sub-agents mirror that exact change (incl. structural: Admonition→`:::`, heading renames, added `<CardGroup>`). NOOP = structural-only (auto-fixed); MAJOR/new-file → whole-file via `translation-prompt.md` (whole-file chunking there is a *different* mechanism — do not conflate with hunks). Earlier passage-hash heuristic produced 7 bug classes (fixed PRs #71–#76); the git-diff rewrite (#76) is the one in use. **Workflow**: `--auto-fix` → `--generate-workfile` → Sonnet sub-agents (3-wide waves) → `--finalize` → commit only passing `.mdx`. `--finalize` (PR #81) pins English-derived anchors, then **dual-gates**: structural validation AND a content gate that rejects a file if any non-cosmetic removed-EN line is still present (= agent silently skipped a hunk → would re-introduce #73 silent-data-loss). Failures → `_finalize_failures.txt`; per-file status → durable `translation/manifests/<locale>.json` (gitignored). **Scaling**: one branch/PR per ~25–30-file batch, **always `--exclude-open-prs --skip-manifest-done`** (unmerged-PR files still read STALE → dup work + the #80 rebase conflict), don't wait for merge between batches; full recipe in retranslation-prompt.md "Orchestration recipe". Improvements A–G all merged (PRs #81/#82/#84). Validated across 33 files / 7 locales / all sections; ~60 files shipped (PRs #77–#80). Backlog (post 2026-05-23 auto-fix pass): ~1,470 MINOR/MODERATE + ~26 MAJOR stale files across the 10 main locales remain for agent retranslation (de now ~1; ~1,090 NOOP already refreshed). Mechanical, resumable via manifest. Note: i18n tutorials are broadly post-sync STALE — not a "TutorialFeedback widget" issue (that 2026-05-06 theory is superseded); refresh them via this pipeline like guides.
...
- **Deployed** — `jupyter.28mc794qh1og.eu-de.codeengine.appdomain.cloud` (eu-de), project `doQumentation`, app `jupyter`. Min 0 / max 1. Default 1 vCPU / 2 GB. Token 32+ chars. Sizing: 8 vCPU/16 GB handles 80 users (see `memory/project_workshop_mode.md`).
...
**⚠ Shallow-clone guard (PR #164, do not remove).** `write_page_dates_manifest()` requires FULL upstream history — it hashes each on-disk file and walks `git log -- <file>` to find the authoring commit. A shallow upstream clone (the content sync does `git clone --depth 1`) has only the tip commit, so every file's dates collapse to that tip (a tree-wide dependency/CI commit) → identical, too-new dates for the whole site. So `write_page_dates_manifest()` now SKIPS (preserving the committed manifest) when it detects a shallow clone (`.git/shallow` or ≤1 reachable commit). Only `refresh-page-dates.yml` (checks out `fetch-depth: 0`) regenerates this file; content-sync deploys must not. Memory: `project_page_dates_shallow_clone`.
...
├── .generated/                 # sync-content.py output kept out of repo root: sidebar-*.json (gitignored, #189)
├── docusaurus.config.ts
├── sidebars.ts                 # require()s ./.generated/sidebar-*.json with try/catch fallback
└── README.md
```

**Generated but tracked:** `docs/tutorials/`, `docs/guides/`, `docs/learning/`, `docs/qiskit-addons/` — committed so each upstream sync is a reviewable diff and `git log` returns real per-file history.

**Generated and gitignored:** `notebooks/`, `static/docs/`, `static/learning/images/`, `.generated/sidebar-*.json` (moved out of repo root in #189; `sidebars.ts` falls back to empty/autogenerated if absent so a fresh clone builds before first sync), `translation/workfiles/` (per-locale refresh workfiles; moved off `/tmp` in #211 because the OS reaps it mid-run).
...
- **Draft → review pipeline** — `translation/drafts/{locale}/{path}` → validate → fix anchors → promote to `i18n/`. Status in `translation/status.json` (status, validation, source hash, dates, failures). Scripts default to `i18n/` without `--dir`.
  ...
  - **Sync & update**: `sync-translations.py` mechanically fixes code-block drift (run after EN changes: `--all-locales` or `--locale XX --dry-run`). `update-translations.py` — git-diff-based: recovers exact old EN → `git diff` (`n=5`) vs current → unified-diff **hunk** per workfile entry → Sonnet sub-agents mirror it. Flags: `--auto-fix`, `--generate-workfile --output`, `--finalize` (anchor-pin → structural validate AND content-gate → hash-bump only if both pass; writes `_finalize_failures.txt` + durable `translation/manifests/<locale>.json`), `--exclude-open-prs`, `--skip-manifest-done`, `--audit`, `--audit-cross-locale`, `--file`, `--section`. **Baseline**: diffs against `PRE_SYNC_REF`; **export `DQ_PRE_SYNC_REF=<this-sync-commit>^` for every refresh** (default is the first-ever sync → over-escalates everything to MAJOR; #192). **`--finalize` now also**: (a) runs a RECONCILE guard — re-scans freshness and exits nonzero if any intended file is still stale (catches silently-no-op'd agent runs; #211); (b) invalidates each re-stamped file's linguistic-review verdict → `STALE_REFRESH` so it re-queues for review (#213). **Do NOT pass `--skip-manifest-done` for a sync refresh** — it drops re-staled files (`feedback_refresh_must_cover_full_stale_set`). macOS: if `git` hits the Xcode-license error, `export PATH="/opt/homebrew/bin:$PATH"`. **Validated & hardened in production** (PRs #76–#84, #192, #211, #213). See the "Weekly stale-translation refresh" bullet + `retranslation-prompt.md` for the full recipe.
...
- **Translation expansion** — all 17 main locales at **100% (428/428), 0 structural-FAIL, 0 stale** (synced to 47abf7714, Jun 2026). HE/TH/CS completed (the old "412/412, HE/TH 95%" is done). German dialects remain intentionally partial: KSH 46, NDS 43, GSW 42, SAX 39, BLN 36, AUT 34, SWG/BAD/BAR 31 (16–28 structural FAILs each, NOT being fixed — user decision 2026-05-31). Potential future: Turkish (TR). Run `translation-status.py --all` for live counts.
- **Linguistic review — ~35 files/locale PENDING after the #191 refresh.** All 17 main locales were `428/428 reviewed` (2026-06-02), but the 47abf7714 sync refresh re-translated ~35 files/locale (595 total); their review verdict was invalidated to `STALE_REFRESH` (#213), so the dashboard now reads `393/428 reviewed`. **These files passed structural + content VALIDATION but their newly-spliced prose has had no linguistic read** — quality rests on the Sonnet splice agents until reviewed. Re-queue with `review-translations.py --next-chunk` (Sonnet, 5/chunk; Haiku unreliable). **This is now automatic**: `update-translations.py --finalize` invalidates a file's review whenever it re-stamps it, so every future content refresh self-flags its touched files for re-review. Older translator passes still owed: TL `workshop/03*`, AR `quantum-diagonalization-algorithms/`, KO `tutorials/transpilation-optimizations-with-sabre.mdx`. Triage history: `.claude/PHASE_4_LINGUISTIC_TRIAGE_2026-05-07.md`.
...
- **Evaluate migrating `codeengine-image.yml` to a QuBins base image** — QuBins ([QuBins/qiskit-images](https://github.com/QuBins/qiskit-images), GHCR `ghcr.io/qubins/images:{version}-{small,xl}`) publishes maintained, versioned Qiskit Docker images — not just the mybinder stub branches. The CE workshop image is currently hand-built (base + pip-install the qiskit stack); a QuBins base could shrink build time and drop maintenance, and QuBins is already a validated environment twin (the notebook-sweep runs locally in `ghcr.io/qubins/images:2.3-xl`, confirmed matching `binder/jupyter-requirements.txt` pins). **Decide, don't assume**: need a real size + cold-start comparison (QuBins `:2.3-xl` is ~2.98 GB uncompressed vs our ~905 MB compressed CE image — ties into the [image-size-reduction](#) workstream). **Open gap**: QuBins `:2.3-xl` is **missing `qiskit-ibm-transpiler`** *and* `qiskit-ibm-ai-local-transpiler` (notebook-sweep finding F2 — installing the first alone still fails local mode) → migrating without layering both back regresses ai-transpiler notebooks. The sweep also found graphviz missing from the image (F1, ~14 nb) — if migrating CE off a hand-built image, fold these deps into the QuBins base or the migration regresses ~32 notebooks; see **`notebook-sweep-report.md`** for the full impacted set + action order. (Stale `ghcr.io/qubins/qiskit:...` → `ghcr.io/qubins/images` refs: FIXED 2026-05-17 in `jupyter.ts:237` + this file.) Full context: `memory/project_qubins_image_migration.md`, `memory/project_notebook_sweep.md`.
- **Re-run the full notebook sweep** — the 2026-05-17 sweep (`notebook-sweep-report.md`, harness `scripts/notebook-sweep/`) was deliberately scoped; a *full* follow-up run should close these gaps: (1) **after F1/F2 land in the QuBins image** — re-run to confirm the ~32 graphviz/transpiler notebooks actually recover on the real image (not just the Pass-B deps-patched one) and to surface whatever was masked behind them; (2) **on a bigger box** (≥16 GB, lower parallelism) so the ~9 Aer-OOM/timeout results resolve into real pass/fail instead of VM artifacts; (3) **after any QuBins image bump or upstream content sync** — this is the natural trigger, the sweep is the regression gate; (4) optionally **spot-check a few locale notebooks** to confirm the EN-twin assumption holds. Re-run: `python3 scripts/notebook-sweep/sweep.py`. Consider wiring the import-only subset (~30 s, no shim) into CI as a cheap guard once the systemic findings are cleared. Full context: `memory/project_notebook_sweep.md`.
- **Upstream sync strategy follow-ups** — `sync-upstream.yml` has now had its **first successful real run** (2026-06-03): synced `833ab77dc → 47abf7714` (PR #191), after a dry-run proved the path and #190 fixed a stale `sidebar-*.json` glob in its `add-paths` (would have failed PR creation post-#189). Still TODO: **schedule it weekly** (the cron is commented out — now that the auto-PR path is proven, enabling it is the next step); teach `sync-content.py` to flag conflicts on our locally-customized files (`hello-world.ipynb`, `_toc.json`); document the rollback recipe in CONTRIBUTING. **Post-sync, run the locale refresh** — see the June 2026 Recently-Resolved entry for the full 17-locale recipe + the hardening that makes the next one cleaner.
...
- **~~Submodule pointer drift~~** — RESOLVED: the committed `upstream-docs` pointer is now `47abf7714` (via the #191 sync PR), in lockstep with `docs/` content and `contentMeta.ts`.
...
#### June 2026
- **Full upstream sync `47abf7714` + all-17-locale retranslation + pipeline hardening + repo cleanup** (Jun 2–4, PRs #189–#213, all merged). One continuous session, start to finish:
...
#### May 2026
- **Cleared the entire main-locale structural-FAIL + build-fatal backlog (9 PRs #171–#180) + two validator/tooling upgrades** (May 30–31). Closed out the deferred #159 weak-5 rework *by locale* and swept the rest of the corpus. **Polish PRs #171–#177** took every main locale to **0 structural FAILs** (428 PASS each): #171 fr/ms/ar (16 files, incl. the 5 reverted re-translations + whole-file fr `runtime-options-overview` & ar `computer-science/vqe` — the latter via a 4-chunk splice, the file being too large for a single agent Write to land), #172 ko (8), #173 pl (12), #174 ja (13), #175 pt (14, incl. the same vqe `26→2 $$` chunked re-translation), #176 ro (1), #177 it (40 — 11 whole-file re-translations + 29 mechanical). Method: classify each FAIL as **drift** (whole-file re-translate) vs **mechanical** (surgical: links, anchors, `.avif↔.svg`, heading levels, Admonition counts, stray/missing fences); fan out ≤5 parallel Sonnet agents per wave; **one locale = one PR**. **Lint gate was load-bearing** — validator-PASS alone shipped a build-fatal duplicate `</Accordion>` in pt vqe (caught only by a separate `lint-translation.py` run) and ~17 untranslated-English TabItem bodies in pl `qpu-information`; every changed file was lint-checked for JSX imbalance + verbatim-EN. **Tooling PR #178**: (1) folded the build-fatal **JSX-tag-balance** check INTO `validate-translation.py` (`check_jsx_tag_balance` — `count_jsx_tags` only counted *opening* tags vs EN, so it was blind to an orphan/duplicate *closer*; 0 false positives across clean locales, catches the dup-closer that previously passed); (2) added `update-translations.py --audit-cross-locale` (read-only; validates every locale, groups FAILing EN files by how many locales fail them, shared-drift first) — its first run surfaced that the **9 German dialects** (aut/bad/bar/bln/gsw/ksh/nds/sax/swg) carry 16–28 structural FAILs + ~387 misplaced-import build-fatal files each, almost all the SAME shared-drift files. **Dialects intentionally NOT fixed** (user decision 2026-05-31). **Build-fatal sweep PR #179**: `lint-translation.py` ERROR sweep of all main locales found genuine build-breakers independent of the polish work — misplaced `TutorialFeedback` import *inside a code fence* in ar(1)/id(4)/pl(1) tutorials (relocated via the idempotent `fix-tutorialfeedback-import.py`) + ar `quantum-machine-learning/data-encoding.mdx` had a spurious duplicate `import qiskit/get_version_info()` block leaving the final fence unterminated (removed dup + closed → 26 blocks == EN). ar + id now 0 build-fatal. **Latent-bug PR #180**: pt `workshop/04_Hands-on…:253` had a stray `"/>` after a self-closing `<img/>` → odd double-quote count → build-fatal (pt-only; stripped to match EN); and `fix-heading-anchors.py` math-heading corruption root-caused & fixed — `extract_headings_with_positions()` didn't track `$$` display-math fences (a `#`-LaTeX line inside `$$` was miscounted as a heading), and on any EN/TR heading-count mismatch the function *fell through* to a misaligned `zip` that appended `{#anchor}` onto the wrong line (historically inside `$...$` math); now skips `$$` blocks AND aborts (writes nothing) on count mismatch. **Note on each-locale-builds-independently**: per-locale `deploy-locales.yml` matrix + satellite repos + deploy only with a `DEPLOY_KEY`, so a build-fatal error in locale X breaks only X's deploy, not the main site. Memory: `feedback_polish_backlog_workflow` (canonical workflow + the dialect-backlog note + the lint-gate lesson). **All 9 PRs (#171–#180) open at session end, awaiting merge.**
- **Landed the parallel dev branch + 5 build-fatal-JSX fixes + page-dates shallow-clone fix** (May 26, PRs #158–#164, all merged to main, deploy 27/27 green). The 247-commit parallel branch `claude/retranslation-status-instructions-VaqMP` (the ms/id/th + falsely-fresh work in the entry below) was assessed in an isolated worktree — **0 MDX-fatal across all 17 locales**, strong-12 ~90%+ structural pass, weak-5 (ko/pl/cs/ro/th) ~107 FAIL each — and split: **#158** (12 strong locales, ~3,123 files + the dev's pipeline improvements: ESM-fence-aware import auto-fix + the `--audit` *prompt text*), **#161** (the 872 validation-clean weak-5 files → 5 new locales partially live, 0 broken pages), **#159** held (the 275 genuinely-defective weak-5 files — real Link/JSX/heading defects — marked DO-NOT-MERGE, awaiting rework). **#160** then *implemented* the `--audit` flag the prompt documented but never shipped (falsely-fresh detector: validates files whose embedded hash already == current EN and reports FAILs; `update-translations.py --locale X --audit`). **Build-fatal JSX class found post-deploy** — passes `validate-translation.py` but aborts `docusaurus build`: orphan `</content>` (ar/pt/cs QAOA) + a `</details>` closing an `<Accordion>` left by a details→Accordion migration (pt qiskit-addons-sqd-get-started, id vqe). Fixed in **#162**/**#163** (5 files) + new `check_jsx_tag_balance` lint wired into the `--finalize` MDX-fatal gate — **rewritten to the false-positive-free rule "raw `</tag>` count > raw `<tag>` opener count"** after an earlier fence-strip/EN-delta version wrongly flagged 5 build-GREEN SQD files (their `<details>` wrap orbital dumps full of ``` fences that confused fence tracking). The `--finalize` MDX-fatal gate now covers all three build-fatal-but-validator-invisible classes: import-isolation (#108), acorn-invalid bare import (#151), JSX tag-balance (#163). Memory: `feedback_mdx_build_fatal_classes`. **A GitHub Actions incident** (auth 403/500, ~11:00–13:00 UTC, confirmed on githubstatus) was tangled in — it failed every locale at `actions/checkout`/submodule-clone; separated from the real defects and waited out. **Page-dates footer wrong dates** (user-reported: ansatz showed the sync date on both footer lines instead of its real 2026-02-09): root cause = `deploy.yml`'s full `sync-content.py` re-clones upstream `git clone --depth 1` (shallow), so `write_page_dates_manifest()` collapsed *every* file's `upstream_date`+`en_date` to the tip commit's date and OVERWROTE the correct committed manifest at build time. Fixed in **#164**: `write_page_dates_manifest()` now detects a shallow clone (`.git/shallow` or ≤1 reachable commit) and PRESERVES the committed manifest — the full-history `refresh-page-dates.yml` job remains the sole accurate producer. Re-deployed; live ansatz now correctly shows Feb 9, site-wide. Memory: `project_page_dates_shallow_clone`. **Also this session:** pruned 55 dead merged-PR local branches; reset another dev's uncommitted local uk scratch per request; rewrote `CONTRIBUTING-TRANSLATIONS.md` for the current git-diff pipeline (it was documenting the deprecated drafts/promote flow) + added a maintainer onboarding section; recorded `feedback_staging_loop_nul_delimited` (write batch file-lists NUL-terminated + stage via `xargs -0`; a `while read` loop silently drops the last path). **Still outstanding:** the #159 rework (275 weak-5 files); th's hunk-splice queue (~66 files); and the broader es/fr/it/uk/ja/pt + remaining backlog.
- **Full STALE retranslation of ms/id/th + cleared the falsely-fresh MAJOR backlog across all 13 main locales** (May 26, branch `claude/retranslation-status-instructions-VaqMP`) — continued the git-diff pipeline (auto-fix → workfile → Sonnet sub-agent hunk-splice in waves of ≤10, later throttled to 5 → CLI-PASS hash-bump → commit continuously). Refreshed every hunk-splice stale file for **ms** (~205), **id** (204), and **th** (205). Two big files per locale (`qpu-information` 735 lines, `qiskit-code-assistant` — missing the whole "local mode" section) plus the `multiverse-computing-singularity` / `runtime-options-overview` / `tools-intro` restructures were whole-file/section rewrites done inline. **Key discovery (user-flagged): falsely-fresh MAJOR files.** `check-translation-freshness` is hash-based, so a deferred MAJOR file whose `{/* doqumentation-source-hash */}` had been bumped to the current EN hash *without* a real translation reported FRESH forever while its content was a stale, much-longer old version that FAILS `validate-translation`. A validation matrix (3 MAJOR files × 13 locales) found **`estimator-rest-api.mdx`** broken in de/ja/ar/he/ms/id/th and **`tutorials/quantum-approximate-optimization-algorithm.mdx`** broken in **12** locales (all but de) — both invisible to the stale report. Re-translated whole-file via `translation-prompt.md` (Write tool, preserving the source-hash marker, mirroring EN heading/code/math/JSX/image counts); `observability-runtime-rest.mdx` was genuinely complete everywhere. **Final: all 3 MAJOR files PASS validation in all 13 locales.** **Reusable tooling built this session** (in `/tmp`): `autofix.py` (image-pair swaps + heading-anchor append + conditional stray-H1 removal, validate-gated) and `linksync.py` (positional markdown link-URL sync, guarded to fire only when EN/TR link counts match) — cleared most mechanical failures (link-URL swaps, `.avif→.svg`, duplicate image galleries, date-heading anchors, the `# Instantiate the new Estimator` code-comment case) without spending translation agents. **`retranslation-prompt.md` updated** with three durable rules: (1) NEVER hash-bump a deferred/MAJOR file (bump only after validation passes; track deferred in a manifest); (2) add a `--audit` mode that validates every file whose hash already matches EN and reports FAILs (surfaces falsely-fresh files instead of relying on memory); (3) codify the orchestrator-side mechanical auto-fixes into `--auto-fix`. **Not yet implemented in `update-translations.py`:** the `--audit` flag and the `autofix.py`/`linksync.py` logic are documented but still need to be folded into the script. **Still outstanding:** th's regular hunk-splice queue (~66 content files, paused mid-run) and the ko/pl/ro/cs locales not yet started (their `observability-runtime-rest` is also stale).
- **Retranslation auto-fix pass + two pipeline bugfixes** (May 23, branch `claude/retranslation-status-instructions-VaqMP`) — ran `update-translations.py --auto-fix` for the 10 main locales (`de es it fr uk ja pt he tl ar`) after root-causing why every stale file was misclassified `MAJOR`. **Two issues fixed:** (1) **Shallow-clone baseline gotcha** — the pipeline diffs each file against `PRE_SYNC_REF` (`9f2948310^`); a shallow clone (web/CI sessions) doesn't contain that commit, so `old_en_content()` returned `None` for every file → 100% `MAJOR`/`full_retranslation`. Fix: `git fetch --depth=2 origin 9f29483105b21a91d0b3abf0ba9cfd735a084b9b` (proxy allows fetch-by-SHA); the commit + its parent then persist in `.git`. Classification became realistic (e.g. es 126 NOOP / 42 MINOR / 46 MODERATE / 3 MAJOR). `translation/manifests/` is also absent in a fresh clone, so PRE_SYNC_REF is the only baseline. (2) **Auto-fix import-sync corrupted code fences** (`update-translations.py:794`) — the old `^import\s+.+$` match conflated MDX imports with **Python** imports inside ` ```python ` fences. It inserted the missing `import TutorialFeedback …` after the last python import (i.e. *inside* a code block — 280/342 files in the first run) and set-diff-deleted/added in-fence python imports (`-import numpy as np`, `+import math`). Fixed to be ESM-only (`import X from '...'` / bare `import '...';`) and code-fence-aware, inserting blank-isolated after the frontmatter close. **Result:** 1,221 `.mdx` auto-fixed; ~1,090 NOOP files hash-bumped fresh (es 217→83 stale, ja 336→172). Verified 0 in-fence misplacements, 0 python imports touched, 0 MDX-fatal lint errors across all 1,221. Remaining ~1,470 MINOR/MODERATE + ~26 MAJOR files across the 10 locales await agent retranslation (waves of 3 Sonnet, per `retranslation-prompt.md`). Pre-existing non-stale tutorials still missing the `TutorialFeedback` import are a separate legacy class → `fix-tutorialfeedback-import.py`.
- **Hello World refresh + AccordionItem stub bugfix** (May 21) — `local-content/hello-world.ipynb` rewritten: dropped all stale fork URLs (`janlahmann.github.io/...`, `mybinder.org/v2/gh/JanLahmann/...`), inlined the 5-step IBM Quantum account setup (no q-docs link), modernized `save_account()` (`channel="ibm_quantum_platform"`, `set_as_default=True`, explicit `instance`/CRN), and rewrote "What next?" to point at `doqumentation.org` entry routes used by the navbar (Tutorials, Guides, Courses, Modules). The shared setup-cell template injected by `sync-content.py` was modernized in lockstep (touches every Binder notebook on re-sync). **AccordionItem bug discovered during this work**: our minimal stub in `src/theme/MDXComponents.tsx` was passing the `title` prop straight to `<summary>`, so upstream inline markdown/HTML (`**bold**`, `` `code` ``, `<em>…</em>`) rendered literally — 99 broken accordion summaries across 15 pages (tutorials index, estimator-options & siblings, cryptographic-hash-functions, etc.). Fixed in one component change: `renderAccordionTitle()` parses the three patterns into `<strong>`/`<code>`/`<em>` and normalizes the upstream `<en>` typo to `<em>`. No content edits required; build passes. Memory note `feedback_jsx_stub_prop_parsing` captures the rule for future stubs.
- **Notebook execution sweep** (May 17) — executed all 261 EN notebooks cell-by-cell in the real production image `ghcr.io/qubins/images:2.3-xl` (qiskit 2.3.1 / qiskit-ibm-runtime 0.43.1 — what live users hit), simulator/fake only via a monkeypatch shim (no IBM hardware, no jobs). Triggered by the `dc-hex-ising` import crash on `de.doqumentation.org`. **Result: ~166 clean, ~26 cloud-only-by-design, ~66 broken for real users.** Systemic causes (each one fix clears many): **F1** graphviz binaries missing from the QuBins image (~14 nb — *proven*: same notebooks pass in a graphviz-patched image); **F2** `qiskit-ibm-transpiler` missing (~5 nb) and its local mode also needs `qiskit-ibm-ai-local-transpiler` (deeper finding); **F4** ~13 nb import packages absent from the image AND not in their own pip-install cell; plus ~10 genuine upstream notebook bugs (API drift, NameError, an unedited Windows placeholder path, the dc-hex-ising wrong-module import). Punch list + recommended action order in **`notebook-sweep-report.md`** (repo root). Reusable capture-only harness in **`scripts/notebook-sweep/`** (`sweep.py` host driver + `sim_shim.py` + `Dockerfile.depspatch`); re-run via `python3 scripts/notebook-sweep/sweep.py` (esp. after a QuBins image bump or to verify F1/F2 landed). Methodology/triage in `.claude/NOTEBOOK_SWEEP_PLAN.md`; memory `project_notebook_sweep`. Also fixed here: stale `ghcr.io/qubins/qiskit` → `ghcr.io/qubins/images` in `src/config/jupyter.ts:237` comment (the runtime `binderUrl` was already correct — comment-only).
- **`.claude/` working-doc cleanup + consolidation** (May 17) — pruned 11 dead/redundant docs, consolidated AI ideation and security tracking. Committed on branch `chore/claude-cleanup` (3 commits: `83a1327a6` A–F prune+AI-merge, `88e5684d8` BOB salvages, `f3ec64d0b` PROJECT_REVIEW security migration) — **not yet merged to main**. Deleted (git-recoverable): STRESS-TEST-FINDINGS, TRANSLATION-VALIDATION-IMPROVEMENTS (all 7 items verified shipped), translation-queue-after-sync-62, freshness-report-2026-05-15, notebook-deps-report (auto-regenerated by `sync-content.py --scan-deps`), test-checklist, BINDER-EXECUTION-TEST-PLAN, AI_INTEGRATION_IDEAS (merged into `AI_FEATURES_BRAINSTORMING.md`). Deleted untracked-only (unrecoverable, uniques salvaged first): the 3 redundant BOB docs (10_OUT_OF_10_ROADMAP, FOUNDATION_REVIEW, AI_INNOVATION_RECOMMENDATIONS). **`AI_FEATURES_BRAINSTORMING.md`** is now the single canonical AI-ideation doc (NotebookLM ideas, UX backlog, cost models). **`.claude/SECURITY_REVIEW_2026-05.md`** is the single code-verified security tracker (13 findings S1–S13, **local-only/untracked** per directive — unresolved security findings stay off the remote) with `BOB_SECURITY_REVIEW_DEEP_DIVE.md` + `BOB_SECURITY_INFRASTRUCTURE_2026-05.md` as untracked appendices. PROJECT_REVIEW.md's 4 security items migrated into the tracker (struck there). **Verified discrepancy**: PROJECT_REVIEW marked CI-1/CI-2 FIXED but they are actually still OPEN (0/47 GH-Actions SHA-pinned, `trivy-action@master`) — now tracked as S6. Memory updated (`feedback_deletion_safety`, `project_security_findings_tracker`, `project_qubins_image_migration`).
...
- **QuBins org migration** (May 15) — QuBins moved from `JanLahmann/qubins` to its own org and was renamed: now [QuBins/qiskit-images](https://github.com/QuBins/qiskit-images). GHCR namespace moved too — package is `ghcr.io/qubins/images:{version}-{small,xl}` (NOT `.../qiskit`; build-matrix.yml sets `IMAGE: images` — verified 2026-05-17). Updated `src/config/jupyter.ts` `binderUrl`, `admin.tsx` Binder/federation links, and `.github/workflows/binder-warmup.yml` cron URLs. Old paths 301-redirect on GitHub, but mybinder caches the resolved repo path — run `gh workflow run binder-warmup.yml` once after merge so the federation rebuilds the cache under the new path.
- **Binder cold-build broken + image split** (May 14) — both resolved by switching Binder to QuBins ([QuBins/qiskit-images](https://github.com/QuBins/qiskit-images), at the time `JanLahmann/qubins` — see May 15 entry above), the sibling repo that publishes versioned Qiskit images (`ghcr.io/qubins/images:{version}-{small,xl}` — package is `images`, not `qiskit`) with daily-rebuilt stub branches on mybinder. doQumentation now points `binderUrl` at `mybinder.org/v2/gh/QuBins/qiskit-images/2.3-xl`; thebelab and "Open in JupyterLab" both consume it. **nbgitpuller pulls notebooks** at session-launch time from this repo's `notebooks` branch (EN) or the locale's satellite repo's `notebooks` branch (translated). `deploy-locales.yml` gains a step to maintain those satellite branches. Eliminates doQumentation's image-publishing pipeline entirely — no `Dockerfile.jupyter` `jupyter-binder` target, no `binder-kernel` stub branch, no `binder.yml` cache warmer (QuBins owns its own warm-up). Retires the `0fc67252` Apr-13 pin. **Bump in lockstep when doQumentation's Qiskit pin moves** (see TODO above).
...
> **Update (May 15, 2026):** [QuBins/qiskit-images](https://github.com/QuBins/qiskit-images) now covers the spirit of this workstream — it publishes versioned Qiskit images (`ghcr.io/qubins/images:{1.4..2.4,latest}-{small,xl}` — GHCR package name is `images`, not `qiskit`) with mybinder stub branches, daily-rebuilt and Trivy-gated. doQumentation already consumes `qiskit-images/2.3-xl` (PR #52). Workstream 2 reshapes to: use QuBins tags as `env=` values, plus a small JSON registry mapping friendly names (`qgss-2025`) to QuBins tags, and contribute any QGSS-specific dep deltas (e.g. `matplotlib`) back to QuBins.
