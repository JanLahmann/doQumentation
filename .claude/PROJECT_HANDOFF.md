...
### CI/CD
- `deploy.yml` — Sync → build → GitHub Pages (English only). Also pushes EN notebooks + Binder config to `notebooks` branch (preserves locale subdirs). Binder config: `binder/Dockerfile` (`FROM quay.io/jupyter/base-notebook:python-3.12` + `pip install -r jupyter-requirements.txt` + cleanup — all in one RUN layer). Installs full Qiskit ecosystem (21 packages). Bypasses repo2docker's conda solver (repo2docker uses binder/Dockerfile as-is). No `postBuild` or `runtime.txt` needed.
...
- **Binder layer caching — Option C deployed, pending verification** — `binder/Dockerfile` installs from `jupyter-requirements.txt` (full Qiskit ecosystem, 21 packages including `qiskit[all]`, all addons, scipy, pyscf, plotly, pandas, etc.). Uses `FROM quay.io/jupyter/base-notebook:python-3.12` — bypasses repo2docker's conda solver. Expected: ~3.5–4.5 GB image. If cold builds are still slow, upgrade to Option B (pre-built GHCR base image, plan in `.claude/plans/binder-layer-cache.md`).
...
- **Code Engine: Classroom tab** — Add dedicated "Classroom" tab to the CE settings section so students can paste a teacher-shared URL + token separately from the "Own Account" flow. Teacher sees sharing guidance ("Share this URL and token with your students").
- **Code Engine: Guided setup wizard** — Replace the current collapsible `<details>` setup instructions with a 4-step interactive wizard (IBM Cloud account → Create CE app → Configure token → Connect & test). Include a Quick Deploy link that pre-fills the IBM Cloud console with image, port, and scaling defaults.
- **Code Engine: CORS setup guide** — Add instructions for configuring `CORS_ORIGIN` env var for custom domains (current default is `https://janlahmann.github.io`). Relevant when users deploy to their own doQumentation fork domain.
- **IBM Cloud Code Engine — COMPLETE (Mar 6)** — Full CE integration as Binder alternative. Container (`binder/Dockerfile.codeengine`) with nginx + supervisord + Jupyter + SSE build server on single port 8080. Frontend detects CE via `STORAGE_KEY_CE_URL` in localStorage, uses Binder-compatible SSE `/build/` protocol (3 phases: connecting → launching → ready). Settings page has CE section (Recommended) with URL/token form + test connection + setup instructions. `OpenInLabBanner` and `ExecutableCode` show CE-specific labels/hints. CI workflow (`codeengine-image.yml`) builds + pushes to `quay.io/janlahmann/doqumentation-jupyter`. Concept doc: `docs/concepts/ibm-cloud-serverless-jupyter.md`.
